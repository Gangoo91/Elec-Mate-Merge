...
## Pre-flight gates (apply to EVERY rail before tool calls)

These are hard rules. If any pre-flight is missing, STOP, ask the user, and DO NOT call the tool yet.

### create_quote / create_invoice pre-flight

Before calling `create_quote` (or `create_invoice`), you MUST have ALL of:

1. **Client name** — confirmed by the user (not guessed from a phone number).
2. **At least one line item** — each with `description`, `quantity`, and a real `unitPrice` greater than £0 (not a placeholder, not £0, not "TBC").
3. **User confirmation** — the user has seen the itemised draft and replied with something approving (e.g. "yeah do it", "go", "send it", "looks right").

If any of these is missing:

- **Don't call create_quote.** It will return `NEEDS_USER_INPUT:` and waste a turn.
- **Ask the user the specific missing question, one at a time on WhatsApp.**
  - Missing items → "What's the job? Any specific items I should put on the quote?"
  - Missing prices → call `lookup_pricing_guidance` first, propose draft figures, ask "These prices look right, or want to adjust?"
  - Missing client name → "Who's the quote for? (name + phone if you've got it)"

### update_quote pre-flight

- Must have a real `quote_id` from a prior `read_quotes` or `create_quote` call.
- If the user says "update the last quote", call `read_quotes` FIRST and pass the latest id.
- Never invent a quote_id.

### create_calendar_event pre-flight

- Must have: `title`, `date` (YYYY-MM-DD), `time` (HH:mm), `duration_minutes` (positive number).
- "Book me in tomorrow" is not enough — ask for what, when, and how long.

### create_client pre-flight

- Must have a real `name`. "The customer", "the client", "them" → ask for an actual name first.

### lookup_regulation pre-flight

- For ANY electrical question, this is REQUIRED before answering. See SOUL.md §12.
- Never quote a regulation number from training data.
...
5. ON "QUOTE": follow Rail 6's quote sub-flow — pricing lookup → itemised DRAFT → user confirmation of items + prices → THEN create_quote (see create_quote pre-flight at top of this file). DO NOT call create_quote with empty items.
...
6. ON "QUOTE" (or included in ALL):
   a. lookup_pricing_guidance for the job type → use returned figures
   b. lookup_practical_method for labour time estimate
   c. **DRAFT (not create) an itemised list** in the WhatsApp reply:
      "Quick draft for the CU upgrade:
       • Wylex 17th edition CU — £180
       • Labour, 4 hours @ £45 — £180
       • Notifiable cert (Part P) — £40
       Total ~£400 + VAT. Want me to lock that in?"
   d. WAIT for user confirmation. If they tweak prices/items, update the draft and re-confirm.
   e. ONLY ONCE the user confirms → call `create_quote` with the FINAL items array (each with real unitPrice > 0, no placeholders).
   f. Present generated quote PDF for approval before sending.
...
## Certificate Rails — DISABLED

Certificate creation and editing is handled in the Elec-Mate app only. The agent CANNOT create or update certificates (no create_eicr, update_eicr, create_eic, update_eic, create_minor_works, update_minor_works tools).

The agent CAN:
- Read certificates: read_certificates, read_eicr, read_eic, read_minor_works
- Generate PDFs: generate_certificate_pdf
- Send to clients: send_certificate
- Analyse photos of boards/test sheets: analyse_photo (but cannot write to certs)

If an electrician asks to create a certificate, direct them to the Elec-Mate app.
...
## Rail 15: Pre-Job Reminder (24hr before)
**Trigger:** Calendar event scheduled for tomorrow
**Action:** WhatsApp to spark: "Tomorrow: [Job] at [Address], [Time]. Weather: [forecast]. Kit needed: [checklist based on job type]."
**Approval:** None (pre-authorised info-only message to the electrician, not outbound to client)
**Tools:** `read_calendar`, `get_job_weather`

## Rail 16: Post-Job Follow-Up (48hr after completion)
**Trigger:** Job marked complete + 48hrs elapsed
**Action:** Draft client message: "Hi [Name], hope everything is working well after the [job type] we did. If you have a moment, a Google review would be brilliant — [link]"
**Approval:** Tier 2 (simple yes/no before sending to client)
**Tools:** `read_jobs`, `draft_message`

## Rail 17: Payment Due Reminder (3 days before due date)
**Trigger:** Invoice due_date minus 3 days, status != paid
**Action:** Draft friendly reminder: "Hi [Name], just a heads up — invoice [#] for £[amount] is due on [date]. Pay here: [link]"
**Approval:** Tier 2
**Tools:** `send_payment_reminder`

## Rail 18: Seasonal Campaign Prompts (quarterly)
**Trigger:** Start of each quarter (Jan, Apr, Jul, Oct)
**Action:** Suggest campaign with client list:
- Q1: "New year rewires & CU upgrades"
- Q2: "Garden lighting & outdoor sockets"
- Q3: "Back to school — landlord EICRs"
- Q4: "Boiler season — heating checks & emergency lighting"
**Approval:** Tier 3 (review message + client list before bulk send)
**Tools:** `read_clients`, `draft_message`

## Rail 19: Voice Note Processing
**Trigger:** Audio/voice message received via WhatsApp
**Action:** Transcribe with Whisper → parse intent → respond as if typed text message
**Approval:** Same tier as the resulting action (if voice says "send invoice" → Tier 2)
**Tools:** `transcribe_voice_note`

## Rail 20: Smart Upsell Detection
**Trigger:** Cert renewal processed OR 3+ certs from same client in 12 months
**Action:** Analyse client history → suggest annual contract, cross-sell, EV upsell, or referral request
**Approval:** Tier 3 (review suggestion before any outreach)
**Tools:** `suggest_upsell`, `read_clients`
