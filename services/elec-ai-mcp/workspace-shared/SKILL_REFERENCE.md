...
Create a quote with manual line items — exactly like the app. Each item MUST have a price.

- **Maps to:** Direct Supabase insert into `quotes` table
- **Inputs:** `{ client_data: { name, email?, phone?, address?, postcode? }, job_details?: { title, description?, estimatedDuration? }, items: [{ description: string, category?: "labour"|"materials"|"equipment", quantity: number, unitPrice: number, unit?: string, notes?: string }], vat_registered?: boolean, vat_rate?: number, notes?: string }`
- **Returns:** `{ quote_id, quote_number, total, vat, status, items_count, message }`
- **Approval:** **YES — show full quote breakdown before creating**
- **Example:** `create_quote { client_data: { name: "John Smith", phone: "07700900000" }, items: [{ description: "Consumer unit upgrade", category: "labour", quantity: 8, unitPrice: 50 }, { description: "18th Edition consumer unit", category: "materials", quantity: 1, unitPrice: 185 }] }`

#### `create_quote_pdf`

Generate a branded PDF version of a quote.

- **Maps to:** Edge function `generate-pdf-monkey` → PDFMonkey
- **Inputs:** `{ quote_id: uuid }`
- **Returns:** `{ documentId, downloadUrl, previewUrl, quote_id, message }`
...
- **Approval:** None (read-only)
- **Scoping examples:**
  - Reg lookup: `document_types=["bs7671"]`
  - Inspection / testing procedure: `document_types=["gn3","bs7671"]`
  - "How do I wire X" practical: `document_types=["osg","gn3"]`
...
| `practical_work_intelligence` | Labour timing, installation methods, trade knowledge | `lookup_practical_method`, `create_quote` |
| `pricing_embeddings`          | Material costs, supplier data, regional pricing      | `lookup_pricing_guidance`, `create_quote` |
