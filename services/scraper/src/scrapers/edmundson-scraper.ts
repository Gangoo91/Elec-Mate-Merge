...
/**
 * Edmundson Electrical Scraper
 * Major UK electrical wholesaler - largest in the UK
 * https://www.edmundson-electrical.co.uk
 */

export class EdmundsonScraper extends BaseScraper {
  constructor() {
    super(SUPPLIERS['edmundson']);
  }

  /**
   * Scrape products from Edmundson Electrical
   */
  async scrapeProducts(category?: string): Promise<ScrapedProduct[]> {
    ...
    try {
      ...
      for (const [categoryName, urls] of Object.entries(categoriesToScrape)) {
        for (const urlPath of urls) {
          ...
          console.log(`Scraping Edmundson ${categoryName}: ${fullUrl}`);
    ...
    console.log(`Edmundson: Scraped ${products.length} products total`);
    return products;
  }

  /**
   * Scrape a single product listing page
   * Edmundson uses a modern e-commerce platform
   */
  private async scrapeProductPage(
    ...
    try {
      ...
      console.log(`  Edmundson: Navigation failed for ${url}`);
      return products;
    }
    ...
    try {
      extractedProducts = await page.evaluate(() => {
        ...
        scripts.forEach((script) => {
          try {
            ...
            if (content.includes('"products"') || content.includes('"sku"') || content.includes('"price"')) {
            ...
            cards.forEach((card) => {
              try {
                ...
                const sku = card.getAttribute('data-sku') ||
                           ...
                           (skuFromUrl ? skuFromUrl[1] : `EDM-${Math.random().toString(36).substr(2, 8)}`);
      ...
      console.log(`  Edmundson: Extraction failed for ${url}`);
      return products;
    }
    ...
    for (const item of extractedProducts) {
      ...
      const brands = ['Schneider', 'ABB', 'Hager', 'Chint', 'Eaton', 'Legrand', 'MK', 'Click', 'Honeywell', 'Deta', 'Scolmore'];
    ...
    console.log(`  Edmundson: Scraped ${products.length} products from ${url}`);
    return products;
  }

  /**
   * Scrape deals from Edmundson offers page
   */
  async scrapeDeals(): Promise<ScrapedDeal[]> {
    ...
    try {
      ...
      console.log(`Scraping Edmundson deals: ${dealsUrl}`);
      ...
      console.log(`Edmundson: Found ${deals.length} deals`);
    } finally {
      await page.close();
    }
  ...
  /**
   * Edmundson doesn't have public coupons
   */
