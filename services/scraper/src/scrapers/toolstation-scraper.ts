...
/**
 * Toolstation Scraper
 * Scrapes products, deals from toolstation.com
 */
...
export class ToolstationScraper extends BaseScraper {
  ...
  private async scrapeProductPage(
    ...
    // Toolstation uses infinite scroll, so scroll to load more
    if (this.config.pagination.type === 'scroll') {
      await this.scrollToLoadAll(page);
    }

    // Extract products
    const extractedProducts = await page.evaluate((selectors, baseUrl) => {
      ...
      cards.forEach((card) => {
        try {
          ...
          const brandEl = selectors.brand ? card.querySelector(selectors.brand) : null;

          // Extract SKU
          let sku = skuEl?.getAttribute('data-product-code') || '';
          if (!sku && linkEl?.href) {
            // Try to extract from URL pattern /product/12345
          ...
          if (name) {
            ...
            if (!sku) {
              sku = `TS-${name.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20)}`;
            }
            ...
            items.push({
              ...
              productUrl: linkEl?.href ? (linkEl.href.startsWith('http') ? linkEl.href : `${baseUrl}${linkEl.href}`) : null,
    ...
    for (const item of extractedProducts) {
      ...
      if (!brand) {
        const brands = ['DeWalt', 'Makita', 'Bosch', 'Milwaukee', 'Fluke', 'Stanley', 'Knipex', 'Wera', 'Bahco', 'Irwin', 'Draper', 'Faithfull'];
    ...
    try {
      ...
      const extractedDeals = await page.evaluate((selectors) => {
        ...
        cards.forEach((card) => {
          ...
          const name = nameEl?.textContent?.trim() || '';
          const sku = skuEl?.getAttribute('data-product-code') || null;
