...
/**
 * Rexel/RFR Electrical Scraper
 * Major UK/International electrical distributor
 * https://www.rfrelectrical.co.uk
 */

export class RexelScraper extends BaseScraper {
  constructor() {
    super(SUPPLIERS['rexel']);
  }

  /**
   * Scrape products from Rexel
   */
  async scrapeProducts(category?: string): Promise<ScrapedProduct[]> {
    ...
    try {
      ...
      for (const [categoryName, urls] of Object.entries(categoriesToScrape)) {
        for (const urlPath of urls) {
          ...
          console.log(`Scraping Rexel ${categoryName}: ${fullUrl}`);
    ...
    console.log(`Rexel: Scraped ${products.length} products total`);
    return products;
  }

  /**
   * Scrape a single product listing page
   * Rexel uses a modern e-commerce platform
   */
  private async scrapeProductPage(
    ...
    try {
      ...
      console.log(`  Rexel: Navigation failed for ${url}`);
      return products;
    }
    ...
    try {
      ...
      console.log(`  Rexel: Extraction failed for ${url}`);
      return products;
    }
    ...
    console.log(`  Rexel: Scraped ${products.length} products from ${url}`);
    return products;
  }

  /**
   * Scrape deals from Rexel offers page
   */
  async scrapeDeals(): Promise<ScrapedDeal[]> {
    ...
    try {
      ...
      console.log(`Scraping Rexel deals: ${dealsUrl}`);
      ...
      console.log(`Rexel: Found ${deals.length} deals`);
    } finally {
      await page.close();
    }
  ...
  /**
   * Rexel doesn't have public coupons
   */
