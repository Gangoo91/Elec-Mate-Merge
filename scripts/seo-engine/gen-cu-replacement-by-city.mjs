#!/usr/bin/env node
/**
 * gen-cu-replacement-by-city.mjs — Consumer Unit Replacement Cost
 * programmatic landing page generator.
 *
 * Same pattern as gen-eicr-cost-by-city.mjs but for CU replacement queries.
 * High commercial intent — homeowners search "consumer unit replacement cost
 * [city]" before booking. ~30 pages × £150–600 jobs = direct lead value.
 *
 * Output:
 *   src/pages/seo/generated/cuReplacementCost{City}Config.ts
 *   src/pages/seo/CUReplacementCost{City}Page.tsx
 *
...
// Derived CU replacement pricing — 1.8x the EICR cost band approximates
// typical CU swap cost (consumer unit + RCBOs + labour day rate).
function cuPriceBand(city) {
  const parseBand = (s) => s.split('\u2013').map((n) => parseInt(n, 10));
  const [eicrLow, eicrHigh] = parseBand(city.threeBedTerraceCost);
  const low = Math.round(eicrLow * 1.7);
  const high = Math.round(eicrHigh * 2.0);
  return `${low}\u2013${high}`;
}

function configFor(city) {
  const cuBand = cuPriceBand(city);
  const [cuLow, cuHigh] = cuBand.split('\u2013').map((n) => parseInt(n, 10));
  return `import type { GeneratedGuideConfig } from '@/pages/seo/generated/GeneratedGuidePage';

// Programmatically generated consumer-unit-replacement cost landing page for ${city.name}.
// Market-rate data only — pricing bands derived from ${city.name} labour rates
...
export const cuReplacementCost${pascalCase(city.slug)}Config: GeneratedGuideConfig = {
  pagePath: '/guides/consumer-unit-replacement-cost-${city.slug}',
  title: 'Consumer Unit Replacement Cost ${city.name} 2026 — Full Pricing Guide',
  description: 'Consumer unit (fuse board) replacement cost in ${city.name} — typical 2026 prices £${cuBand} for a domestic swap with RCBOs, BS 7671:2018+A4:2026 compliant. What is included, what is not, and how to get a fair quote.',
  datePublished: published,
  dateModified: modified,
  readingTime: 8,
  badge: 'Cost Guide',
  badgeIcon: 'PoundSterling',
  breadcrumbLabel: 'Consumer Unit Replacement Cost in ${city.name}',
  heroPrefix: 'Consumer Unit Replacement Cost in ${city.name}:',
  heroHighlight: '2026 Full Pricing Guide',
  heroSuffix: '— RCBO Boards & BS 7671:2018+A4:2026',
  heroSubtitle:
    'Whether you are an electrician quoting consumer unit replacements in ${city.name} or a homeowner trying to understand the market rate, this guide gives the fair-margin 2026 prices for a NICEIC or NAPIT scheme-registered electrician. Old wired-fuse boards, plastic enclosures and missing RCD protection are all driving replacements. Labour rates in ${city.name} typically run £${city.hourlyLow}–£${city.hourlyHigh}/hour. Use it as a market-rate reference for your next quote, or as a buyer\\u2019s guide to a fair price.',
  keyTakeaways: [
    'A typical 10–14 way domestic consumer unit replacement in ${city.name} costs £${cuBand} (2026 prices) including the new board, RCBOs, surge protection, labour and the Electrical Installation Certificate.',
    'Larger 18+ way boards, three-phase boards, or boards needing tail upgrades push the price toward £${Math.round(cuHigh * 1.2)} and beyond.',
    ...
    'The DNO for ${city.region} is ${city.dno}. Any work that requires the meter to be pulled (e.g. tails upgrade) must be coordinated with the DNO or a metal-seal-cutting service.',
  ],
  sections: [
    {
      id: 'typical-prices',
      heading: 'Typical Consumer Unit Replacement Prices in ${city.name} (2026)',
      tocLabel: 'Typical prices',
      blocks: [
        {
          type: 'paragraph',
          text: 'These are working prices for ${city.name} based on competent-person scheme members in the area, current copper / steel pricing, and typical labour. Each scenario assumes a same-day swap with no unexpected remedial work.',
        },
        {
          type: 'list',
          ordered: false,
          items: [
            'Small property (1-bed flat or maisonette, 6–8 way board) — £${Math.round(cuLow * 0.85)}–${Math.round(cuLow * 1.05)} including board, RCBOs, labour, EIC',
            'Standard 3-bed house (10–14 way all-RCBO board) — £${cuBand} (typical 4–6 hours on site)',
            'Larger 4-bed house (14–18 way board with SPD) — £${Math.round(cuHigh * 1.0)}–${Math.round(cuHigh * 1.2)}',
            'HMO or rented property (with AFDDs on relevant circuits) — £${Math.round(cuHigh * 1.1)}–${Math.round(cuHigh * 1.4)}',
            'Three-phase domestic / light commercial board — £${Math.round(cuHigh * 1.3)}–${Math.round(cuHigh * 1.8)}+ (priced on site)',
    ...
    },
    {
      ...
      blocks: [
        {
          type: 'paragraph',
          text: 'A compliant consumer unit replacement in ${city.name} should include all of the following — anything missing should be flagged in the quote:',
        },
        {
          type: 'list',
          ordered: false,
    ...
    },
    {
      ...
      blocks: [
        {
          type: 'paragraph',
          text: 'Beyond the board itself, six factors push the final ${city.name} price into the upper band:',
        },
        {
          type: 'list',
          ordered: true,
          items: [
            'Tails upgrade — if existing tails are < 25mm² or in poor condition, they must be replaced and coordinated with the DNO ${city.dno}.',
    ...
    {
      id: 'for-electricians',
      heading: 'For Electricians: Quote CU Replacements in ${city.name} at the Right Price',
      tocLabel: 'For electricians',
      blocks: [
        {
          type: 'paragraph',
          text: 'If you are quoting consumer unit replacements in ${city.name}, the £${cuBand} band above is your fair-margin floor for a 10–14 way RCBO board with SPD, EIC and labour. Quoting below this band means you are absorbing the cost of your scheme fee, professional indemnity, calibrated test kit, and the certificate liability you sign. Use the bands as anchors, then adjust for: tails upgrade, bonding remedials, AFDDs (mandatory in HMOs / certain residential under A4:2026), and any DNO coordination through ${city.dno}.',
        },
        {
          type: 'callout',
          tone: 'info',
          ...
          body:
            'Elec-Mate gives ${city.name} electricians the tools to scan the existing board, generate an itemised quote with RCBOs, SPDs, AFDDs and labour, and issue the EIC on-site. 7-day free trial.',
        },
        {
          type: 'paragraph',
  ...
  faqs: [
    {
      question: 'How much does it cost to replace a consumer unit in ${city.name}?',
      answer: 'A typical 10–14 way domestic consumer unit replacement in ${city.name} costs £${cuBand} in 2026, including the new metal-clad board, all RCBOs, surge protection, labour and the EIC. Larger or HMO-spec boards push toward £${Math.round(cuHigh * 1.2)}+.',
    },
    {
    ...
    },
    {
      ...
      answer: 'Sometimes — a competent person can add an RCD upstream of an existing board as a stop-gap. However if the board is old, plastic, or has missing labelling, the EICR will continue to flag observations. For most ${city.name} properties, a full replacement is the better long-term option.',
    },
  ],
  howToHeading: 'How to Book a Consumer Unit Replacement in ${city.name}',
  howToDescription:
  ...
  relatedPages: [
    ...
    {
      href: '/guides/eicr-cost-${city.slug}',
      title: 'EICR Cost in ${city.name}',
  ...
  ctaHeading: 'For Electricians: Quote CU Swaps in ${city.name}',
  ctaSubheading:
...
import { cuReplacementCost${name}Config } from '@/pages/seo/generated/cuReplacementCost${name}Config';

export default function CUReplacementCost${name}Page() {
  return <GeneratedGuidePage config={cuReplacementCost${name}Config} />;
}
`;
}
...
for (const city of DATA.cities) {
  const name = pascalCase(city.slug);
  const configFile = join(GEN_DIR, `cuReplacementCost${name}Config.ts`);
  const wrapperFile = join(SEO_DIR, `CUReplacementCost${name}Page.tsx`);
...
console.log(`Generated ${generated} CU replacement config + wrapper pairs, skipped ${skipped} existing.`);
