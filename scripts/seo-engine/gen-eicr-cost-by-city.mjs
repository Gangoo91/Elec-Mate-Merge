#!/usr/bin/env node
/**
 * gen-eicr-cost-by-city.mjs — Programmatic landing pages.
 *
 * Generates a GeneratedGuideConfig + wrapper React page per UK city for
 * the high-intent commercial query "EICR cost in [city]". Each page targets
 * the city-specific cost band derived from market data — no BS 7671 content
 * is invented; technical references are kept short and link out to grounded
 * pages (existing EICR guides, Pricing guide, etc).
 *
 * Output:
 *   src/pages/seo/generated/eicrCost{City}Config.ts
 *   src/pages/seo/EICRCost{City}Page.tsx
 *
 * Route registration is NOT done here — produces a route-snippet file
 * (reports/programmatic-routes-eicr-cost.txt) that can be pasted into
 * SEORoutes.tsx in one block.
 *
...
const DATA = JSON.parse(readFileSync(join(__dirname, 'programmatic-cities-data.json'), 'utf-8'));
const SEO_DIR = join(ROOT, 'src/pages/seo');
const GEN_DIR = join(ROOT, 'src/pages/seo/generated');
const ROUTES_OUT = join(ROOT, 'reports/programmatic-routes-eicr-cost.txt');
...
function configFor(city) {
  return `import type { GeneratedGuideConfig } from '@/pages/seo/generated/GeneratedGuidePage';

// Programmatically generated EICR cost landing page for ${city.name}.
...
export const eicrCost${pascalCase(city.slug)}Config: GeneratedGuideConfig = {
  pagePath: '/guides/eicr-cost-${city.slug}',
  title: 'EICR Cost ${city.name} 2026 — Domestic & Commercial Pricing',
  description: 'How much does an EICR cost in ${city.name}? Fair-margin 2026 prices for scheme-registered electricians — £${city.bedroomSemiCost} for a 1-bed, £${city.fourBedHouseCost} for a 4-bed, commercial from £${city.commercialPerSocket} per socket. What is included, and why below-market quotes are usually missing something.',
  datePublished: published,
  dateModified: modified,
  readingTime: 7,
  badge: 'Cost Guide',
  badgeIcon: 'PoundSterling',
  breadcrumbLabel: 'EICR Cost in ${city.name}',
  heroPrefix: 'EICR Cost in ${city.name}:',
  heroHighlight: '2026 Pricing Guide',
  heroSuffix: '— Domestic & Commercial',
  heroSubtitle:
    'Whether you are an electrician quoting EICRs in ${city.name} or a landlord trying to understand the market rate, this guide gives the fair-margin 2026 prices charged by NICEIC and NAPIT scheme-registered electricians, what is included, and why below-market quotes are usually missing something. Hourly rates for inspection work in ${city.name} run £${city.hourlyLow}–£${city.hourlyHigh}, with a typical 3-bed terrace EICR landing at £${city.threeBedTerraceCost}. Use it as a market-rate reference for your next quote, or as a buyer\\u2019s guide to a fair price.',
  keyTakeaways: [
    'A 1-bed flat EICR in ${city.name} typically costs £${city.bedroomSemiCost}, a 3-bed terrace £${city.threeBedTerraceCost}, and a 4-bed house £${city.fourBedHouseCost} (2026 prices for a scheme-registered electrician, excluding remedial work).',
    'Commercial EICRs in ${city.name} are priced per socket / fitting — typically £${city.commercialPerSocket} per item, with minimum visit fees of £${city.hourlyHigh}–£${Math.round(city.hourlyHigh * 1.5)}.',
    ...
    'Hourly rates for ${city.name} sit at £${city.hourlyLow}–£${city.hourlyHigh} because a NICEIC / NAPIT registered electrician carries professional indemnity insurance, scheme fees, calibrated test equipment (typically £1,200+ MFT), and is liable for the certificate they sign.',
    ...
    '${city.region}\\u2019s DNO is ${city.dno}. Any remedial work involving DNO notification (PME, supply upgrades) routes through them.',
  ...
  sections: [
    {
      id: 'typical-prices',
      heading: 'Typical EICR Prices in ${city.name} (2026)',
      tocLabel: 'Typical prices',
      blocks: [
        {
          type: 'paragraph',
          text: 'Below is a working price guide for ${city.name} based on competent-person scheme members operating in the area, hourly rate aggregates, and competitor pricing. Prices exclude any remedial work, which is quoted separately.',
        },
        {
          type: 'list',
          ordered: false,
          items: [
            '1-bedroom flat or maisonette — £${city.bedroomSemiCost} (typically 1.5–2 hours on site)',
            '2-bedroom terrace — £${city.threeBedTerraceCost.split('\u2013')[0]}–${Math.round(parseInt(city.threeBedTerraceCost.split('\u2013')[1], 10) * 0.95)} (typically 2–3 hours)',
            '3-bedroom semi or terrace — £${city.threeBedTerraceCost} (typically 2.5–3.5 hours)',
            '4-bedroom detached — £${city.fourBedHouseCost} (typically 3.5–5 hours)',
            '5+ bedroom / large houses — £${Math.round(parseInt(city.fourBedHouseCost.split('\u2013')[1], 10) * 1.15)}+ (priced on visit)',
            'Commercial property — £${city.commercialPerSocket} per socket / fitting with a minimum visit fee of £${city.hourlyHigh}–£${Math.round(city.hourlyHigh * 1.5)}',
          ],
        },
        {
          type: 'callout',
    ...
    },
    {
      ...
      blocks: [
        {
          type: 'paragraph',
          text: 'Six factors push an EICR price up or down within ${city.name}:',
        },
        {
          type: 'list',
          ordered: true,
    ...
    {
      id: 'when-required',
      heading: 'When You Need an EICR in ${city.name}',
      tocLabel: 'When required',
      blocks: [
        {
          type: 'paragraph',
          text: 'EICRs are mandatory in several scenarios under UK regulation, regardless of ${city.region} location:',
        },
        {
          type: 'list',
          ordered: false,
    ...
    {
      id: 'for-electricians',
      heading: 'For Electricians: Quote EICRs in ${city.name} at the Right Price',
      tocLabel: 'For electricians',
      blocks: [
        {
          type: 'paragraph',
          text: 'If you are a self-employed electrician or small contractor working in ${city.name}, this page is your local market-rate reference. The £${city.threeBedTerraceCost} band for a 3-bed terrace EICR is the fair-margin floor for a scheme-registered electrician — quoting below it means you are undercutting yourself before you start. Use the figures above to anchor your own quoting, with adjustments for property complexity, accessibility, and your scheme + insurance overhead.',
        },
        {
          type: 'callout',
          tone: 'info',
          ...
          body:
            'Elec-Mate gives ${city.name} electricians voice-driven test entry, 16 certificate types, on-site Schedule of Test Results, and a professional PDF quote that the client cannot haggle. 7-day free trial — see how much faster you can quote and certify in your area.',
        },
        {
          type: 'paragraph',
    ...
    {
      id: 'get-a-quote',
      heading: 'For Property Owners: Booking an EICR in ${city.name}',
      tocLabel: 'For property owners',
      blocks: [
        {
          type: 'paragraph',
          text: 'If you are a landlord, owner or managing agent in ${city.name} looking for an EICR, Elec-Mate connects you to vetted competent-person scheme electricians. Send a brief — property type, postcode, last EICR date — and get a price from a local electrician. Most ${city.name} jobs are booked within 5–10 working days, with same-day availability typical for smaller properties or urgent landlord deadlines under the Electrical Safety Standards in the Private Rented Sector Regulations 2020.',
        },
        {
          type: 'paragraph',
  ...
  faqs: [
    {
      question: 'How much does a standard EICR cost in ${city.name}?',
      answer: 'For a 3-bedroom property in ${city.name} the typical 2026 EICR price from a NICEIC or NAPIT scheme-registered electrician is £${city.threeBedTerraceCost}, excluding any remedial work. Prices vary with property size, consumer unit condition, and number of circuits. Quotes substantially below this band usually mean the EIC certificate, RCD test sweep or insurance is missing — verify what is included.',
    },
    {
      question: 'How long is an EICR valid for in ${city.name}?',
    ...
    {
      question: 'Who can carry out an EICR in ${city.name}?',
    ...
    {
      question: 'Can I get a same-day EICR in ${city.name}?',
      answer: 'Same-day or next-day EICRs are possible in ${city.name} for smaller properties (1–2 bed) where an electrician is local and a slot is available. Larger jobs typically require 5–10 working days lead time.',
    },
  ],
  howToHeading: 'How to Book an EICR in ${city.name}',
  howToDescription:
  ...
  howToSteps: [
    ...
    },
    {
      ...
      text: 'Use Elec-Mate to find vetted ${city.name} electricians, or check NICEIC / NAPIT directories directly. Avoid trader-aggregator sites that bid jobs out without checking competence.',
    },
    {
  ...
  relatedPages: [
    {
      href: '/electricians/${city.slug}',
      title: 'Find an Electrician in ${city.name}',
      description: 'Vetted scheme-registered electricians in ${city.name}.',
      icon: 'MapPin',
      category: 'Guide',
    },
    {
  ...
  ctaHeading: 'For Electricians: Quote and Certify EICRs in ${city.name}',
  ctaSubheading:
...
function wrapperFor(city) {
  const name = pascalCase(city.slug);
  return `import GeneratedGuidePage from '@/pages/seo/generated/GeneratedGuidePage';
import { eicrCost${name}Config } from '@/pages/seo/generated/eicrCost${name}Config';

export default function EICRCost${name}Page() {
  return <GeneratedGuidePage config={eicrCost${name}Config} />;
}
`;
}
...
for (const city of DATA.cities) {
  const name = pascalCase(city.slug);
  const configFile = join(GEN_DIR, `eicrCost${name}Config.ts`);
  const wrapperFile = join(SEO_DIR, `EICRCost${name}Page.tsx`);
  ...
  writeFileSync(configFile, configFor(city));
  writeFileSync(wrapperFile, wrapperFor(city));
  generated++;

  routeSnippets.push(
    `      <Route\n        path="/guides/eicr-cost-${city.slug}"\n        element={\n          <LazyRoute>\n            <EICRCost${name}Page />\n          </LazyRoute>\n        }\n      />`,
  );
}

const lazyImports = DATA.cities
  .map((c) => {
    const name = pascalCase(c.slug);
    return `const EICRCost${name}Page = lazy(() => import('@/pages/seo/EICRCost${name}Page'));`;
  })
  .join('\n');

writeFileSync(
  ROUTES_OUT,
  `// Paste lazy imports near the top of SEORoutes.tsx (after the existing lazy imports):\n\n${lazyImports}\n\n// Paste Route elements before the closing </> at the bottom of SEORoutes.tsx:\n\n${routeSnippets.join('\n')}\n`,
);

console.log(`Generated ${generated} config + wrapper pairs, skipped ${skipped} existing.`);
console.log(`Route snippet written to ${ROUTES_OUT}`);
console.log(`Total cities in data: ${DATA.cities.length}`);
