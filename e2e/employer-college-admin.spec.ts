...
test.describe('Employer Hub', () => {
  ...
  test('employer dashboard page loads', async ({ page }) => {
    ...
    const pageLoaded = await Promise.race([
      page
        .waitForSelector('[data-testid="employer-hub"], h1, h2, .card, [class*="employer"]', {
          timeout: 10000,
        })
        .then(() => true),
      page
        .getByText(/employer|company|business|apprentice|staff|hub/i)
        .first()
...
test.describe('College Hub', () => {
  ...
  test('college dashboard page loads', async ({ page }) => {
    ...
    const pageLoaded = await Promise.race([
      page
        .waitForSelector('[data-testid="college-hub"], h1, h2, .card, [class*="college"]', {
          timeout: 10000,
        })
        .then(() => true),
      page
        .getByText(/college|tutor|student|course|class|hub/i)
        .first()
        .waitFor({ state: 'attached', timeout: 10000 })
        .then(() => true),
    ]).catch(() => false);
...
test.describe('PDF Generator', () => {
  test('PDF generator page loads', async ({ page }) => {
    ...
    await expect(page.locator('body')).toBeVisible();
    await expect(page.getByText(/PDF|generate|document|latex/i).first()).toBeVisible({
      timeout: 10000,
    });
  });
});
...
test.describe('Public Token Routes', () => {
  test.describe('Public Quote', () => {
    ...
    test('public quote via alternate route handles invalid token', async ({ page }) => {
