...
test.describe('Admin Dashboard Functionality', () => {
  ...
  test('displays real-time user statistics', async ({ page }) => {
    ...
    const statsCards = page
      .locator('[class*="card"]')
      .filter({ hasText: /users|revenue|signups/i });
...
test.describe('Admin Users Management', () => {
  ...
  test('displays user list with profile information', async ({ page }) => {
    ...
    await page.waitForSelector('[class*="card"], [class*="user"], [data-testid]', {
      timeout: 15000,
    });
  ...
  test('role filter buttons work', async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("All"), button:has-text("Sparks")', {
      timeout: 15000,
    });
  ...
  test('clicking user opens detail sheet', async ({ page }) => {
    ...
    const userRow = page
      ...
      .filter({ hasText: /@.*\.(com|co\.uk)/i })
      .first();
    if (await userRow.isVisible()) {
      await userRow.click();

      // Sheet should open - wait for dialog/sheet
      await page.waitForSelector('[role="dialog"], [class*="sheet"], [class*="Sheet"]', {
        timeout: 5000,
      });
...
test.describe('Admin Elec-ID Verification', () => {
  ...
  test('elec-id detail sheet shows verify/reject buttons for pending profiles', async ({
    page,
  }) => {
    await page.goto('/admin/elec-ids');
    ...
    const profileCard = page
      .locator('[class*="card"]')
      .filter({ hasText: /elec|id|pending/i })
      .first();
...
test.describe('Admin Data Export', () => {
  ...
  test('export format selector exists', async ({ page }) => {
    ...
    const formatSelector = page
      ...
      .filter({ hasText: /csv|json|format/i });
    const hasFormatOption = await formatSelector
      .first()
      .isVisible()
      .catch(() => false);
...
test.describe('Admin Refresh Functionality', () => {
  test('refresh button reloads data on users page', async ({ page }) => {
    ...
    const refreshButton = page
      .locator('button')
      .filter({ has: page.locator('svg[class*="refresh"], [class*="RefreshCw"]') });
  ...
  test('refresh button reloads data on elec-ids page', async ({ page }) => {
    ...
    // Find refresh button
    const refreshButton = page
      .locator('button[class*="icon"], button')
