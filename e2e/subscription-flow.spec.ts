...
test.describe('7-Day Free Trial System', () => {
  ...
  test('trial countdown shows correct time', async ({ page }) => {
    ...
    const hasTimeDisplay = await timeDisplay
      .first()
      .isVisible()
      .catch(() => false);
...
test.describe('Subscription Plans Page', () => {
  ...
  test('shows all plan options', async ({ page }) => {
    ...
    const apprenticeVisible = await apprenticePlan
      .first()
      .isVisible()
      .catch(() => false);
    const electricianVisible = await electricianPlan
      .first()
      .isVisible()
      .catch(() => false);
    const employerVisible = await employerPlan
      .first()
      .isVisible()
      .catch(() => false);

    console.log(
      `Plans visible - Apprentice: ${apprenticeVisible}, Electrician: ${electricianVisible}, Employer: ${employerVisible}`
    );
    ...
    const hasComingSoon = await comingSoonBadge
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test('billing toggle works', async ({ page }) => {
    ...
    if ((await monthlyButton.isVisible()) && (await annualButton.isVisible())) {
      ...
      const hasSavings = await savingsBadge
        .first()
        .isVisible()
        .catch(() => false);
    ...
    if (await getStartedButton.isVisible()) {
      ...
      if (isEnabled) {
        ...
        const navigationPromise = page
          .waitForURL(/stripe|checkout|subscriptions/, { timeout: 10000 })
          .catch(() => null);
...
test.describe('Stripe Checkout Integration', () => {
  ...
  test('checkout creates Stripe session', async ({ page }) => {
    ...
    if ((await subscribeButton.isVisible()) && (await subscribeButton.isEnabled())) {
      ...
      const checkoutRequest = page
        .waitForRequest((request) => request.url().includes('create-checkout'), { timeout: 10000 })
        .catch(() => null);
  ...
  test('promo codes can be applied', async ({ page }) => {
    ...
    const hasPromoOption = await promoText
      .first()
      .isVisible()
      .catch(() => false);
...
test.describe('Feature Access Control (Paywall)', () => {
  ...
  test('premium features show upgrade prompt for free users', async ({ page }) => {
    ...
    const showsUpgrade = await upgradePrompt
      .first()
      .isVisible()
      .catch(() => false);
    const showsFeature = await featureContent
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test('AI tools check subscription status', async ({ page }) => {
    ...
    const hasAI = await aiInterface
      .first()
      .isVisible()
      .catch(() => false);
    const hasUpgrade = await upgradePrompt
      .first()
      .isVisible()
      .catch(() => false);
...
test.describe('Webhook & Payment Verification', () => {
  ...
  test('subscription status updates after payment', async ({ page }) => {
    ...
    const statusBadge = page
      ...
      .filter({ hasText: /active|subscribed|trial|free/i });
