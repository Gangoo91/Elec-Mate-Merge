  ...
  test.describe('EICR Form Client Data Entry', () => {
    test('can fill EICR client details', async ({ page }) => {
      ...
      const emailInput = page
        .locator('input[name="clientEmail"], input[type="email"], input[placeholder*="email" i]')
        .first();
      ...
      const phoneInput = page
        .locator('input[name="clientPhone"], input[type="tel"], input[placeholder*="phone" i]')
        .first();
      ...
      const installAddressInput = page
        .locator(
  ...
  test.describe('Quote Builder Page', () => {
    ...
    test('quote builder has postcode field', async ({ page }) => {
      ...
      const postcodeInput = page
        .locator('input[name="postcode"], input[placeholder*="postcode" i]')
        .first();
...
test.describe('Full End-to-End Certificate to Quote Flow', () => {
  test('complete flow: EICR form -> Quote button -> Quote builder with pre-filled data', async ({
    page,
  }) => {
    await loginViaUI(page);
    ...
    const installAddressInput = page
      .locator('input[name="installationAddress"], textarea[name="installationAddress"]')
      .first();
    ...
    if (await quoteButton.isVisible()) {
      ...
      const quoteClientName = page
        .locator('input[name="clientName"], input[placeholder*="client" i]')
        .first();
  ...
  test('complete flow: EICR form -> Invoice button -> Invoice builder with pre-filled data', async ({
    page,
  }) => {
    await loginViaUI(page);
