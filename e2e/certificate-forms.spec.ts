...
test.describe('EIC Certificate Form', () => {
  ...
  test.describe('Client Details Tab', () => {
    ...
    test('can select installation type', async ({ page }) => {
      ...
      const typeSelector = page
        .locator('select[name*="type" i], [role="combobox"], button[aria-haspopup="listbox"]')
        .first();
  ...
  test.describe('Schedule of Tests Tab', () => {
    ...
    test('can add circuit test results', async ({ page }) => {
      ...
      const testingTab = page
        .locator('button:has-text("Testing"), button:has-text("Schedule")')
        .first();
      ...
      const addCircuitBtn = page
        .locator('button:has-text("Add Circuit"), button:has-text("Add Row"), button:has-text("+")')
        .first();
  ...
  test.describe('Save and PDF Generation', () => {
    test('save draft button is available', async ({ page }) => {
      ...
      const saveButton = page
        .locator('button:has-text("Save"), button:has-text("Draft"), button[aria-label*="save" i]')
        .first();
    ...
    test('generate PDF button exists', async ({ page }) => {
      ...
      const pdfButton = page
        .locator('button:has-text("PDF"), button:has-text("Generate"), button:has-text("Download")')
        .first();
...
test.describe('EICR Certificate Form', () => {
  ...
  test('EICR form has observations section', async ({ page }) => {
    ...
    const observationElements = page.locator('text=/C1|C2|C3|observation|defect/i');
  ...
  test('can add observations with defect codes', async ({ page }) => {
    ...
    if (await addObsBtn.isVisible()) {
      ...
      const obsForm = page.locator('[class*="modal"], [class*="dialog"], [role="dialog"], form');

      if ((await obsForm.count()) > 0) {
        // Look for defect code selector
        const codeSelector = page
          .locator(
...
test.describe('Certificate List Management', () => {
  ...
  test('can view list of certificates', async ({ page }) => {
    ...
    await expect(page.getByText(/certificate|inspection|EICR|EIC|report/i).first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test('can filter certificates by type', async ({ page }) => {
    ...
    const filterSelect = page
      .locator('select[name*="filter" i], select[name*="type" i], [role="combobox"]')
      .first();
