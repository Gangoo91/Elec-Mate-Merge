...
test.describe('AI Agent Interactions', () => {
  ...
  test.describe('Cost Engineer Agent', () => {
    ...
    test('shows processing state when generating (without full execution)', async ({ page }) => {
      await page.goto('/electrician/cost-engineer');
      await page.waitForTimeout(2000);
      ...
      if (await textarea.isVisible()) {
        ...
        const generateBtn = page
          .locator('button:has-text("Generate"), button:has-text("Analyse"), button[type="submit"]')
          .first();
  ...
  test.describe('Circuit Designer Agent', () => {
    test('circuit designer page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/circuit|design|BS7671/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('can select supply type', async ({ page }) => {
      ...
      const supplySelector = page
        .locator('button:has-text("Single"), button:has-text("Three"), select[name*="supply" i]')
        .first();
    ...
    test('can add circuits to design', async ({ page }) => {
      ...
      const addCircuitBtn = page
        .locator('button:has-text("Add Circuit"), button:has-text("Add"), button:has-text("+")')
        .first();
  ...
  test.describe('Health & Safety Agent', () => {
    test('health and safety page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/health|safety|risk|RAMS/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe('Installation Specialist Agent', () => {
    test('installation specialist page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/install|method|guidance/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe('Agent Selector Hub', () => {
    ...
    test('can navigate to specific agent from selector', async ({ page }) => {
      ...
      const costEngineerCard = page
        .locator('button:has-text("Cost"), a:has-text("Cost"), [class*="card"]:has-text("Cost")')
        .first();
    ...
    test('agent cards show descriptions', async ({ page }) => {
      ...
      const descriptions = page.locator('text=/quote|circuit|safety|install|maintenance/i');
...
test.describe('AI Tool Outputs', () => {
  ...
  test.describe('Export Functionality', () => {
    test('export to quote option available in cost engineer', async ({ page }) => {
      await page.goto('/electrician/cost-engineer');
      await page.waitForTimeout(2000);
    ...
    test('copy to clipboard option available', async ({ page }) => {
      ...
      const copyBtn = page.locator('button:has-text("Copy"), button[aria-label*="copy" i]');
...
test.describe('AI Tooling Suite (Secondary Tools)', () => {
  ...
  test('component identification tool available', async ({ page }) => {
    ...
    const componentTool = page
      .locator('button:has-text("Component"), text=/component.*identify/i, a:has-text("Component")')
      .first();
  ...
  test('wiring instructions tool available', async ({ page }) => {
    ...
    const wiringTool = page
      .locator('button:has-text("Wiring"), text=/wiring.*instruction/i')
      .first();
  ...
  test('fault diagnosis tool available', async ({ page }) => {
    ...
    const faultTool = page.locator('button:has-text("Fault"), text=/fault.*diagnos/i').first();
