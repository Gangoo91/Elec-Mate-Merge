  ...
  test.describe('Founder Signup Flow', () => {
    test('signup page shows founder branding', async ({ page }) => {
      ...
      const hasCrown = await page
        .locator('[class*="Crown"], svg')
        .first()
        .isVisible()
        .catch(() => false);
      const hasFounderText = await page
        .getByText(/founder/i)
        .first()
        .isVisible()
        .catch(() => false);
    ...
    test('signup form validation works', async ({ page }) => {
      ...
      const invalidError = await page
        .getByRole('heading', { name: /invalid/i })
        .isVisible()
        .catch(() => false);
    ...
    test('password requirements are shown', async ({ page }) => {
      ...
      if (await passwordInput.isVisible()) {
        ...
        const hasIndicators = await requirementIndicators
          .first()
          .isVisible()
          .catch(() => false);
    ...
    test('signup redirects to Stripe checkout', async ({ page }) => {
      ...
      const subscribeButton = page.getByRole('button', {
        name: /subscribe|checkout|create account/i,
      });
  ...
  test.describe('Founder Success Page', () => {
    ...
    test('success page has call to action', async ({ page }) => {
      ...
      const dashboardButton = page.getByRole('button', {
        name: /dashboard|start|get started|continue/i,
      });
