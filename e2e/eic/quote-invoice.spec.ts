...
test.describe('EIC Certificate - Quote Creation', () => {
  ...
  test('1. Create Quote button is visible', async ({ page }) => {
    ...
    const hasButton = await quoteButton
      .first()
  ...
  test('2. Quote button navigates to quote builder with data', async ({ page }) => {
    ...
    const quoteButton = page
      .locator('button:has-text("Quote"), button:has-text("Create Quote")')
      .first();
...
test.describe('EIC Certificate - Invoice Creation', () => {
  ...
  test('4. Create Invoice button is visible', async ({ page }) => {
    ...
    const invoiceButton = page.locator(
      'button:has-text("Invoice"), button:has-text("Create Invoice")'
    );
    const hasButton = await invoiceButton
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test('5. Invoice button navigates to invoice builder', async ({ page }) => {
    ...
    const invoiceButton = page
      .locator('button:has-text("Invoice"), button:has-text("Create Invoice")')
      .first();
