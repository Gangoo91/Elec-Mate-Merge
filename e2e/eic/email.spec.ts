...
test.describe('EIC Certificate Email - Button Availability', () => {
  ...
  test('1. Email button is visible', async ({ page }) => {
    ...
    const emailButton = page.locator(
      'button:has-text("Email"), button:has(svg.lucide-mail), button:has-text("Send")'
    );
    const hasButton = await emailButton
      .first()
  ...
  test('2. Email button opens dialog/sheet', async ({ page }) => {
    const emailButton = page
      .locator('button:has-text("Email"), button:has(svg.lucide-mail)')
      .first();
  ...
  test('3. Recipient email field pre-fills from client', async ({ page }) => {
    ...
    if (await emailButton.isVisible({ timeout: 3000 })) {
      ...
      const recipientField = page.locator(
        '[role="dialog"] input[type="email"], [class*="sheet"] input[name*="recipient" i]'
      );
  ...
  test('4. Subject line is editable', async ({ page }) => {
    ...
    if (await emailButton.isVisible({ timeout: 3000 })) {
      ...
      const subjectField = page.locator(
        'input[name*="subject" i], input[placeholder*="subject" i]'
      );
  ...
  test('6. Send button is available in dialog', async ({ page }) => {
    ...
    if (await emailButton.isVisible({ timeout: 3000 })) {
      ...
      const sendButton = page.locator(
        '[role="dialog"] button:has-text("Send"), [class*="sheet"] button:has-text("Send")'
      );
      const hasButton = await sendButton
        .first()
  ...
  test('7. Cancel button closes dialog', async ({ page }) => {
    ...
    if (await emailButton.isVisible({ timeout: 3000 })) {
      ...
      const cancelButton = page
        .locator('[role="dialog"] button:has-text("Cancel"), button:has-text("Close")')
        .first();
