...
test.describe('EIC Certificate PDF - Generate Button', () => {
  ...
  test('1. Generate PDF button is visible', async ({ page }) => {
    ...
    const generateButton = page.locator(
      'button:has-text("Generate"), button:has-text("PDF"), button:has-text("Certificate"), button:has-text("Complete")'
    );
  ...
  test('2. Generate button disabled when form incomplete', async ({ page }) => {
    const generateButton = page
      .locator('button:has-text("Generate Certificate"), button:has-text("Generate PDF")')
      .first();
...
test.describe('EIC Certificate PDF - Preview', () => {
  ...
  test('4. Preview button is available', async ({ page }) => {
    ...
    const hasButton = await previewButton
      .first()
  ...
  test('6. Preview shows certificate layout', async ({ page }) => {
    ...
    if (await previewButton.isVisible({ timeout: 3000 })) {
      ...
      const certificateContent = page.locator(
        'text=/EIC|Electrical Installation Certificate|BS 7671/i'
      );
      const hasContent = await certificateContent
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
...
test.describe('EIC Certificate PDF - Download', () => {
  ...
  test('7. Download button is available', async ({ page }) => {
    const downloadButton = page.locator(
      'button:has-text("Download"), button:has(svg.lucide-download)'
    );
  ...
  test('8. Download triggers file save', async ({ page }) => {
    ...
    const downloadButton = page
      .locator('button:has-text("Download PDF"), button:has-text("Download")')
      .first();
  ...
  test('9. PDF filename includes certificate reference', async ({ page }) => {
    ...
    const hasRefField = await refField
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
...
test.describe('EIC Certificate PDF - Options', () => {
  ...
  test('10. Include schedule of tests option exists', async ({ page }) => {
    const includeTests = page.locator(
      'input[type="checkbox"][name*="includeTests" i], [role="checkbox"]:has-text("Schedule")'
    );
    const hasOption = await includeTests
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test('11. Include circuit chart option exists', async ({ page }) => {
    const includeChart = page.locator(
      'input[type="checkbox"][name*="includeChart" i], [role="checkbox"]:has-text("Chart")'
    );
    const hasOption = await includeChart
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test('12. Company logo option exists', async ({ page }) => {
    const logoOption = page.locator(
      'input[name*="logo" i], button:has-text("Logo"), [role="checkbox"]:has-text("Logo")'
    );
    const hasOption = await logoOption
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
