...
async function selectIfVisible(page: Page, selector: string, value: string) {
  ...
  if (await element.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    const option = page
      .locator(`[role="option"]:has-text("${value}"), option:has-text("${value}")`)
      .first();
...
test.describe('EICR Full Flow - Installation Details', () => {
  ...
  test('can set earthing arrangement', async ({ page }) => {
    ...
    const earthingSelect = page
      .locator('[name="earthingArrangement"], select:has-text("TN-C-S"), button:has-text("TN")')
      .first();
    ...
    if (await earthingSelect.isVisible()) {
      ...
      const option = page
        .locator('[role="option"]:has-text("TN-C-S"), option:has-text("TN-C-S")')
        .first();
...
test.describe('EICR Full Flow - Schedule of Tests', () => {
  ...
  test('can access schedule of tests section', async ({ page }) => {
    ...
    const testElements = page.locator('text=/circuit|R1|R2|Zs|insulation|continuity/i');
...
test.describe('EICR Full Flow - Email Sending', () => {
  ...
  test('email dialog can be opened', async ({ page }) => {
    ...
    if (await emailButton.isVisible()) {
      ...
      const emailDialog = page.locator('[role="dialog"], [class*="dialog"], [class*="modal"]');
...
test.describe('EICR Complete End-to-End Flow', () => {
  ...
  test('complete flow: EICR form -> Quote button -> Quote builder with all data', async ({
    page,
  }) => {
    await loginViaUI(page);
...
test.describe('EICR Full Flow - Certificates List', () => {
  ...
  test('can view list of certificates', async ({ page }) => {
    ...
    const listContent = page.locator('text=/certificate|report|EICR|EIC|inspection|new|create/i');
