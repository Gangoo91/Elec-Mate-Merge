...
test.describe('Calculator Accuracy Tests', () => {
  ...
  test.describe('Hourly Rate Calculator', () => {
    ...
    test('calculates hourly rate correctly', async ({ page }) => {
      ...
      const salaryInput = page.locator('input[name*="salary" i], input[id*="salary" i]').first();
      ...
      // Find result
      const result = page.locator('text=/£[0-9]+\.[0-9]{2}/i, [class*="result"], [class*="rate"]');
    ...
    test('VAT calculation is applied correctly', async ({ page }) => {
      ...
      const vatToggle = page
        .locator('input[type="checkbox"][name*="vat" i], button:has-text("VAT"), [role="switch"]')
        .first();
  ...
  test.describe('Job Profitability Calculator', () => {
    ...
    test('calculates profit correctly', async ({ page }) => {
      ...
      const materialInput = page
        .locator('input[name*="material" i], input[id*="material" i]')
        .first();
      ...
      const hoursInput = page.locator('input[name*="hour" i], input[id*="hour" i]').first();
      ...
      const rateInput = page.locator('input[name*="rate" i], input[id*="rate" i]').first();
      ...
      const quoteInput = page.locator('input[name*="quote" i], input[id*="quote" i]').first();
    ...
    test('shows loss warning when quote is too low', async ({ page }) => {
      ...
      const materialInput = page.locator('input[name*="material" i]').first();
    ...
    test('overhead percentage affects profit calculation', async ({ page }) => {
      ...
      const overheadInput = page
        .locator('input[name*="overhead" i], select[name*="overhead" i]')
        .first();
  ...
  test.describe('VAT Scheme Comparison', () => {
    ...
    test('calculates standard vs flat rate VAT', async ({ page }) => {
      ...
      const revenueInput = page
        .locator('input[name*="revenue" i], input[placeholder*="revenue" i]')
        .first();
      ...
      const labourInput = page.locator('input[name*="labour" i], input[name*="labor" i]').first();
  ...
  test.describe('Minimum Charge Calculator', () => {
    ...
    test('calculates first hour charge correctly', async ({ page }) => {
      ...
      const travelInput = page
        .locator('input[name*="travel" i], input[placeholder*="travel" i]')
        .first();
      ...
      const hourlyInput = page.locator('input[name*="hourly" i], input[name*="cost" i]').first();
  ...
  test.describe('Staff Cost Calculator', () => {
    ...
    test('calculates true hourly cost including on-costs', async ({ page }) => {
      ...
      const basePayInput = page.locator('input[name*="pay" i], input[name*="base" i]').first();
  ...
  test.describe('Equipment ROI Calculator', () => {
    ...
    test('calculates NPV and payback period', async ({ page }) => {
      ...
      const equipmentInput = page
        .locator('input[name*="equipment" i], input[name*="cost" i]')
        .first();
      ...
      const savingsInput = page
        .locator('input[name*="saving" i], input[name*="revenue" i]')
        .first();
  ...
  test.describe('Tax & NI Estimator', () => {
    ...
    test('calculates UK tax bands correctly', async ({ page }) => {
      ...
      const incomeInput = page.locator('input[name*="income" i], input[name*="gross" i]').first();
  ...
  test.describe('CIS DRC Helper', () => {
    test('calculator loads correctly', async ({ page }) => {
      ...
      await expect(page.getByText(/CIS|construction|deduction/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Capacity Planning Tool', () => {
    test('tool loads correctly', async ({ page }) => {
      await page.goto('/electrician/business-development/tools/capacity-planning');
      await page.waitForTimeout(2000);
...
test.describe('Calculator Input Validation', () => {
  ...
  test('hourly rate calculator rejects negative values', async ({ page }) => {
    ...
    const salaryInput = page.locator('input[name*="salary" i], input[type="number"]').first();
  ...
  test('percentage fields are bounded 0-100', async ({ page }) => {
    ...
    const overheadInput = page
      .locator('input[name*="overhead" i], input[name*="margin" i]')
      .first();
