...
test.describe('Mental Health Hub', () => {
  ...
  test.describe('Breathing Exercise', () => {
    test('breathing exercise opens and displays correctly', async ({ page }) => {
      ...
      await expect(page.getByText(/breath|inhale|exhale|calm/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Main Sections', () => {
    test('Mental Health Mate section opens', async ({ page }) => {
      ...
      await expect(page.getByText(/chat|support|talk|peer/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('Safety Plan section opens', async ({ page }) => {
      ...
      await expect(page.getByText(/safety|crisis|plan|warning/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('Mood Insights section opens', async ({ page }) => {
      ...
      await expect(page.getByText(/insight|pattern|trend|mood/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('Resources section opens', async ({ page }) => {
      ...
      await expect(page.getByText(/resource|guide|video|help/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('Support Network section opens', async ({ page }) => {
      ...
      await expect(page.getByText(/support|connect|peer|professional/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('Podcasts section opens', async ({ page }) => {
      ...
      await expect(page.getByText(/podcast|listen|episode/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Crisis Resources', () => {
    test('crisis button is visible and clickable', async ({ page }) => {
      ...
      const crisisButton = page.locator('button.fixed:has(svg), button:has-text("Crisis")');
    ...
    test('electrical industry charity contact is displayed', async ({ page }) => {
      await page.goto('/apprentice/mental-health');

      // Check for Electrical Industries Charity info
      await expect(page.getByText(/Electrical|Industries|Charity/i).first()).toBeVisible({
        timeout: 10000,
      });
  ...
  test.describe('Sub-pages', () => {
    ...
    test('crisis resources page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/crisis|emergency|help|support/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('support network page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/support|network|connect/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
