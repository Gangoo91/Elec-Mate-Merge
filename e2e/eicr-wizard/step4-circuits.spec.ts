...
test.describe('EICR Wizard Step 4 - Stats Cards Display', () => {
  ...
  test('2. Add circuit button is visible', async ({ page }) => {
    const addButton = page
      .locator(
        'button:has-text("Add"), button:has-text("New Circuit"), button:has-text("+ Circuit")'
      )
      .first();
    ...
    const hasPlusIcon = await plusIcon
      .first()
  ...
  test('3. Adding circuit auto-generates designation (C1, C2, etc.)', async ({ page }) => {
    const addButton = page
      .locator('button:has-text("Add"), button:has-text("Circuit"), button:has(svg.lucide-plus)')
      .first();
    ...
    if (await addButton.isVisible({ timeout: 3000 })) {
      ...
      const hasDesignation = await circuitDesignation
        .first()
...
test.describe('EICR Wizard Step 4 - Circuit Card Display', () => {
  ...
  test('4. Circuit card shows badge, title, and details', async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 3000 })) {
      ...
      const hasCard = await circuitCard
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test('5. New circuit shows pending status (yellow indicator)', async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 3000 })) {
      ...
      const pendingIndicator = page.locator(
        '[class*="yellow"], [class*="pending"], [class*="warning"], text=/pending/i'
      );
      const hasPending = await pendingIndicator
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test('6. Complete circuit shows green status indicator', async ({ page }) => {
    ...
    const statusIndicators = page.locator(
      '[class*="green"], [class*="complete"], [class*="success"]'
    );
  ...
  test('7. Failed test shows red status indicator', async ({ page }) => {
    ...
    const failedIndicators = page.locator(
      '[class*="red"], [class*="failed"], [class*="error"], [class*="danger"]'
    );
  ...
  test('8. Clicking circuit card opens edit sheet/modal', async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 3000 })) {
      ...
      const circuitCard = page
        .locator('[class*="card"]:has-text("C1"), [class*="circuit"]')
        .first();
      if (await circuitCard.isVisible()) {
        ...
        const editSheet = page.locator(
          '[role="dialog"], [class*="sheet"], [class*="modal"], [class*="drawer"]'
        );
        const hasSheet = await editSheet
          .first()
          .isVisible({ timeout: 3000 })
          .catch(() => false);
  ...
  test('9. Delete circuit button removes circuit and updates count', async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 3000 })) {
      ...
      const deleteButton = page
        .locator(
          'button:has(svg.lucide-trash), button:has-text("Delete"), button[aria-label*="delete" i]'
        )
        .first();
...
test.describe('EICR Wizard Step 4 - Empty State', () => {
  ...
  test("11. Empty state has 'Add First Circuit' button", async ({ page }) => {
    const addFirstButton = page
      .locator('button:has-text("Add"), button:has-text("First"), button:has-text("Get Started")')
      .first();
...
test.describe('EICR Wizard Step 4 - Board Tabs', () => {
  ...
  test('12. Multiple boards show tab bar', async ({ page }) => {
    // Look for board tabs
    const boardTabs = page.locator(
      '[role="tablist"], [class*="tabs"], button:has-text("Main DB"), button:has-text("Board")'
    );
    const hasTabBar = await boardTabs
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test('13. Clicking board tab switches circuit view', async ({ page }) => {
    const boardTab = page
      .locator('[role="tab"], button:has-text("Board"), button:has-text("DB")')
      .first();
