...
test.describe('EICR Form Navigation - Tabs', () => {
  ...
  test('1. Form loads with default tab selected', async ({ page }) => {
    ...
    const selectedTab = page.locator(
      '[role="tab"][data-state="active"], button.active, [aria-selected="true"]'
    );
    const hasSelectedTab = await selectedTab
      .first()
  ...
  test('2. All tabs are clickable', async ({ page }) => {
    const tabs = page.locator(
      'button:has-text("Details"), button:has-text("Inspection"), button:has-text("Testing"), button:has-text("Inspector"), button:has-text("Certificate")'
    );
  ...
  test('3. Tab click changes content', async ({ page }) => {
    ...
    if (await inspectionTab.isVisible({ timeout: 3000 })) {
      ...
      const hasContent = await inspectionContent
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test('4. Back button returns to dashboard', async ({ page }) => {
    ...
    if (await backButton.isVisible({ timeout: 3000 })) {
      ...
      const isDashboard = await dashboardContent
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
...
test.describe('EICR Form Navigation - Save Controls', () => {
  ...
  test('7. Sync status indicator present', async ({ page }) => {
    ...
    const hasIndicator = await syncIndicator
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
...
test.describe('EICR Form Navigation - Progress', () => {
  ...
  test('9. Certificate number is displayed', async ({ page }) => {
    ...
    const hasCertNumber = await certNumber
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
