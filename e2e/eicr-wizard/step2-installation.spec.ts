...
async function navigateToStep2(page: Page) {
  ...
  await fillIfVisible(
    page,
    'input[name="propertyPostcode"], input[name="postcode"]',
    testClient.postcode
  );
...
async function clickOption(page: Page, text: string): Promise<boolean> {
  const option = page
    .locator(
      `button:has-text("${text}"), [role="button"]:has-text("${text}"), div:has-text("${text}") >> button, label:has-text("${text}")`
    )
    .first();
...
test.describe('EICR Wizard Step 2 - Supply Type Selection', () => {
  ...
  test('1. Supply Type - Single Phase (1P) can be selected', async ({ page }) => {
    ...
    const singlePhaseClicked =
      (await clickOption(page, 'Single')) ||
    ...
    if (singlePhaseClicked) {
      ...
      const selectedIndicator = page
        .locator(
          '[data-state="checked"], [aria-checked="true"], .selected, .active, [class*="border-yellow"], [class*="border-elec"]'
        )
        .first();
  ...
  test('2. Supply Type - Three Phase (3P) can be selected', async ({ page }) => {
    const threePhaseClicked =
      (await clickOption(page, 'Three')) ||
...
test.describe('EICR Wizard Step 2 - Voltage Auto-fill', () => {
  ...
  test('7. Selecting 1P auto-fills voltage to 230V', async ({ page }) => {
    ...
    const voltageInput = page
      .locator('input[name*="voltage" i], input[placeholder*="voltage" i]')
      .first();
  ...
  test('9. Frequency field has default value of 50Hz', async ({ page }) => {
    ...
    const frequencyInput = page
      .locator('input[name*="frequency" i], input[placeholder*="frequency" i]')
      .first();
...
test.describe('EICR Wizard Step 2 - Complete Flow', () => {
  test('Complete step 2 with all selections and proceed', async ({ page }) => {
    ...
    if ((await nextButton.isVisible()) && (await nextButton.isEnabled())) {
      ...
      const movedToStep3 = await step3Content
        .first()
