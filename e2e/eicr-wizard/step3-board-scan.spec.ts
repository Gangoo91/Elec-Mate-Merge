...
test.describe('EICR Form - Testing Tab Content', () => {
  ...
  test('1. Testing tab loads correctly', async ({ page }) => {
    ...
    const hasContent = await testingContent
      .first()
  ...
  test('2. Circuit list or table is present', async ({ page }) => {
    ...
    const circuitContent = page.locator(
      'text=/circuit|C1|C2|designation/i, table, [class*="circuit"]'
    );
    const hasCircuits = await circuitContent
      .first()
  ...
  test('3. Add circuit functionality exists', async ({ page }) => {
    const addButton = page.locator(
      'button:has-text("Add"), button:has-text("New Circuit"), button:has-text("+")'
    );
...
test.describe('EICR Form - Board Scan Feature', () => {
  ...
  test('4. Board scan button is available', async ({ page }) => {
    ...
    const scanButton = page.locator(
      'button:has-text("Scan"), button:has-text("Board"), button:has(svg.lucide-camera), button:has(svg.lucide-sparkles)'
    );
  ...
  test('5. Schedule of tests section exists', async ({ page }) => {
    ...
    const hasSchedule = await scheduleContent
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test('6. Test result fields are present', async ({ page }) => {
    ...
    const testFields = page.locator(
      'input[name*="zs" i], input[name*="r1" i], input[name*="insulation" i], input[placeholder*="Ω"]'
    );
...
test.describe('EICR Form - Testing Tab Actions', () => {
  ...
  test('8. Tab can switch to other tabs', async ({ page }) => {
    ...
    if (await detailsTab.isVisible({ timeout: 3000 })) {
      ...
      const hasDetails = await detailsContent
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
