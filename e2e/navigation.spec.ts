...
test.describe('Navigation & Dashboard', () => {
  ...
  test.describe('Dashboard', () => {
    test('dashboard page loads after login', async ({ page }) => {
      ...
      await expect(page.getByText(/dashboard|welcome|home|hub/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe('Profile & Settings', () => {
    test('profile page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/profile|account|details|personal/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('settings page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/setting|preference|notification/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('notifications page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/notification|alert|message/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Subscriptions', () => {
    test('subscriptions page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/subscription|plan|premium|upgrade/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Elec-ID', () => {
    test('elec-id page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/elec.?id|identity|card|verification/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Tools Routes', () => {
    test('regulation search page loads', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/regulation|search|BS7671|wiring/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Sidebar Navigation', () => {
    ...
    test('can navigate to apprentice section from sidebar', async ({ page }) => {
      await page.goto('/dashboard');

      // Try to find and click apprentice link
      const apprenticeLink = page.locator('a[href*="apprentice"], button:has-text("Apprentice")');
    ...
    test('can navigate to electrician section from sidebar', async ({ page }) => {
      await page.goto('/dashboard');
  ...
  test.describe('Mobile Navigation', () => {
    ...
    test('bottom navigation is visible on mobile', async ({ page }) => {
      ...
      const bottomNav = page.locator('nav.fixed, [class*="bottom"], footer nav');
...
test.describe('Public Pages', () => {
  test.describe('Landing Page', () => {
    test('landing page loads for unauthenticated users', async ({ page }) => {
      ...
      await expect(page.locator('body')).toBeVisible();
      await expect(page.getByText(/Elec.?Mate|electrician|apprentice/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
...
test.describe('Authentication Flow', () => {
  ...
  test('forgot password page accessible', async ({ page }) => {
    ...
    await expect(page.locator('body')).toBeVisible();
    await expect(page.getByText(/forgot|password|reset|email/i).first()).toBeVisible({
      timeout: 10000,
    });
  });
...
test.describe('Error Handling', () => {
  ...
  test('404 page displays for unknown routes', async ({ page }) => {
    ...
    await expect(page.getByText(/404|not found|page.*exist/i).first()).toBeVisible({
      timeout: 10000,
    });
  });
});
