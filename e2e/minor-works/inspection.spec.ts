...
async function selectOutcome(page: Page, itemText: string, outcome: string): Promise<boolean> {
  ...
  const itemRow = page
    .locator(
      `[class*="inspection"]:has-text("${itemText}"), tr:has-text("${itemText}"), div:has-text("${itemText}")`
    )
    .first();

  if (await itemRow.isVisible({ timeout: 2000 }).catch(() => false)) {
    // Find and click the outcome button within that row
    const outcomeButton = itemRow
      .locator(`button:has-text("${outcome}"), [value="${outcome}"], [data-value="${outcome}"]`)
      .first();
...
test.describe('Minor Works Inspection - Connections & Terminations', () => {
  ...
  test('1. Connections item 1 can toggle outcomes', async ({ page }) => {
    ...
    const hasInspectionItems = await inspectionContent
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);

    if (hasInspectionItems) {
      const outcomeButtons = page.locator(
        'button:has-text("Sat"), button:has-text("Pass"), button:has-text("C1"), [role="button"]'
      );
...
test.describe('Minor Works Inspection - Installation Methods', () => {
  ...
  test('5. Installation method item 1 can toggle outcomes', async ({ page }) => {
    ...
    const hasContent = await installationContent
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
...
test.describe('Minor Works Inspection - Safety & Protection', () => {
  ...
  test('9. Safety item 1 can toggle outcomes', async ({ page }) => {
    ...
    const hasContent = await safetyContent
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
...
test.describe('Minor Works Inspection - Overall Result', () => {
  ...
  test('17. Overall Result dropdown can be selected (Sat/Unsat/Limited)', async ({ page }) => {
    const resultDropdown = page
      .locator('[name*="result" i], [name*="overall" i], button:has-text("Overall result")')
      .first();
    ...
    if (await resultDropdown.isVisible({ timeout: 3000 })) {
      ...
      const satisfactoryOption = page
        .locator('[role="option"]:has-text("Satisfactory"), option:has-text("Satisfactory")')
        .first();
  ...
  test('18. Limitations textarea accepts input', async ({ page }) => {
    const limitationsTextarea = page
      .locator('textarea[name*="limitation" i], textarea[placeholder*="limitation" i]')
      .first();
