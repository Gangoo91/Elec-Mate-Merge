...
test.describe('Minor Works - Property & Client Details', () => {
  ...
  test('1. Property Address textarea accepts multi-line input', async ({ page }) => {
    ...
    if (filled) {
      const textarea = page
        .locator('textarea[name="propertyAddress"], textarea[name="installationAddress"]')
        .first();
...
test.describe('Minor Works - Work Type Selection', () => {
  ...
  test('10. Subcategory dropdown appears after selecting main type', async ({ page }) => {
    ...
    const subcategoryDropdown = page.locator(
      '[name*="subcategory" i], [name*="subCategory" i], button:has-text("Select subcategory")'
    );
    const hasSubcategory = await subcategoryDropdown
      .first()
...
test.describe('Minor Works - Work Description', () => {
  ...
  test('11. Work Description textarea accepts detailed input', async ({ page }) => {
    const description =
      'Installation of additional 13A socket outlet to kitchen worktop. Cable run from existing ring final circuit in DB1.';
    const filled = await fillIfVisible(
      page,
...
test.describe('Minor Works - BS7671 Compliance', () => {
  ...
  test('14. Risk Assessment checkbox can be toggled', async ({ page }) => {
    const checkbox = page
      .locator('input[type="checkbox"][name*="risk" i], [role="checkbox"][id*="risk" i]')
      .first();
...
test.describe('Minor Works - Part P Notification', () => {
  ...
  test('18. Building Control field shows when Part P = Yes', async ({ page }) => {
    ...
    const buildingControlField = page.locator(
      'input[name*="buildingControl" i], input[name*="building" i]'
    );
    const hasField = await buildingControlField
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
