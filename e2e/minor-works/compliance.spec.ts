...
test.describe('Minor Works Compliance - Score Display', () => {
  ...
  test('1. Compliance score displays percentage', async ({ page }) => {
    const scoreDisplay = page.locator(
      'text=/\\d+%|complete|compliance/i, [class*="score"], [class*="progress"]'
    );
    const hasScore = await scoreDisplay
      .first()
...
test.describe('Minor Works Compliance - Item Details', () => {
  ...
  test('5. Clicking compliance item expands details', async ({ page }) => {
    const complianceItem = page
      .locator(
        '[class*="collapsible"], button[class*="compliance"], [role="button"]:has-text("Required")'
      )
      .first();
  ...
  test('6. Pass count updates when items pass validation', async ({ page }) => {
    ...
    const hasPassCount = await passCount
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test('7. Fail count updates when items fail validation', async ({ page }) => {
    ...
    const hasFailCount = await failCount
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
...
test.describe('Minor Works Compliance - Generation Eligibility', () => {
  ...
  test('8. Certificate generation eligible when 80%+ and no critical failures', async ({
    page,
  }) => {
    ...
    const addressInput = page
      .locator('textarea[name="propertyAddress"], input[name="propertyAddress"]')
      .first();
    ...
    const actionButtons = page.locator(
      'button:has-text("Generate"), button:has-text("Save"), button:has-text("Complete"), button:has-text("PDF")'
    );
