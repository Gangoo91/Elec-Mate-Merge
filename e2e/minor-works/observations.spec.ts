...
test.describe('Minor Works Observations - Adding Observations', () => {
  ...
  test('2. Add observation button creates new card', async ({ page }) => {
    const addButton = page
      .locator(
        'button:has-text("Add Observation"), button:has-text("Add Defect"), button:has-text("New Observation")'
      )
      .first();
    ...
    if (await addButton.isVisible({ timeout: 3000 })) {
      ...
      const observationCard = page.locator(
        '[class*="observation"], [class*="defect"], [class*="card"]:has-text("C")'
      );
      const hasCard = await observationCard
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
...
test.describe('Minor Works Observations - Defect Codes', () => {
  ...
  test('3. Defect code C1 shows red border and urgent text', async ({ page }) => {
    ...
    const addButton = page
      .locator('button:has-text("Add Observation"), button:has-text("Add")')
      .first();
    ...
    const selected = await selectOption(
      page,
      '[name*="code" i], [name*="defect" i], button:has-text("Code")',
      'C1'
    );

    if (selected) {
      const redElement = page.locator('[class*="red"], [class*="danger"], [class*="c1" i]');
      const hasRed = await redElement
        .first()
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test('4. Defect code C2 shows orange border', async ({ page }) => {
    ...
    if (selected) {
      ...
      const hasOrange = await orangeElement
        .first()
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test('5. Defect code C3 shows blue border', async ({ page }) => {
    ...
    if (selected) {
      const blueElement = page.locator(
        '[class*="blue"], [class*="info"], [class*="c3" i], [class*="amber"]'
      );
      const hasBlue = await blueElement
        .first()
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test('6. Defect code FI shows purple border', async ({ page }) => {
    ...
    if (selected) {
      ...
      const hasPurple = await purpleElement
        .first()
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test('7. Defect code N/A shows gray border', async ({ page }) => {
    ...
    if (selected) {
      ...
      const hasGray = await grayElement
        .first()
        .isVisible({ timeout: 2000 })
        .catch(() => false);
...
test.describe('Minor Works Observations - Quick Select & Actions', () => {
  ...
  test('8. Quick select dropdown auto-fills description', async ({ page }) => {
    ...
    const quickSelect = page.locator(
      '[name*="quickSelect" i], [name*="common" i], button:has-text("Quick")'
    );
  ...
  test('9. Delete observation button removes card', async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 2000 })) {
      ...
      const deleteButton = page
        .locator(
          'button:has(svg.lucide-trash), button[aria-label*="delete" i], button:has-text("Delete")'
        )
        .first();
  ...
  test('10. Mark All N/A button sets all observations to N/A', async ({ page }) => {
    const markAllButton = page
      .locator('button:has-text("Mark All N/A"), button:has-text("All N/A")')
      .first();
