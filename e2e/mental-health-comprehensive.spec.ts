...
test.describe('Mental Health Hub - Comprehensive Tests', () => {
  ...
  test.describe('Hub Page Structure', () => {
    test('displays all main UI elements correctly', async ({ page }) => {
      ...
      await expect(
        page.getByText(/Electrical.*Charity|For Electrical Workers/i).first()
      ).toBeVisible();
  ...
  test.describe('Quick Mood Check', () => {
    ...
    test('low mood (1-2) triggers supportive message and redirect', async ({ page }) => {
      ...
      const showsMessage = await page
        .getByText(/I hear you|Let's talk|opening support/i)
      ...
      const navigated = await page
        .getByText(/Mental Health Mates|Find Support|My Chats/i)
        .first()
    ...
    test('Check In button opens detailed mood logging', async ({ page }) => {
      ...
      await expect(page.getByText(/mood|feeling|how are you/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('AI Chat - Mental Health Mate', () => {
    test('Mental Health Mate section opens peer support hub', async ({ page }) => {
      ...
      await expect(
        page.getByText(/Mental Health Mates|Find Support|My Chats/i).first()
    ...
    test('Find Support tab shows available supporters or empty state', async ({ page }) => {
      ...
      await expect(
        page
          .getByText(
            /No supporters|Peer Supporter|Available|Connect|training|Mental Health First Aider/i
          )
          .first()
    ...
    test('displays safety notice about peer support limitations', async ({ page }) => {
      ...
      await expect(
        page.getByText(/peer support|not professional|counsellor|116 123|Samaritans/i).first()
      ).toBeVisible({ timeout: 10000 });
    });
    ...
    test('Become a Mental Health Mate button functionality', async ({ page }) => {
      ...
      if (isVisible) {
        ...
        await expect(page.getByText(/display name|bio|training|topics/i).first()).toBeVisible({
          timeout: 10000,
        });
      }
    });
    ...
    test('supporter profile view shows details', async ({ page }) => {
      ...
      const onCorrectPage = await page
        .getByText(
          /Mental Health Mates|Find Support|No supporters|training|Available|Connect|Browse/i
        )
        .first()
      ...
      const supporterCard = page
        .locator('[class*="supporter"], [class*="card"]')
        .filter({ hasText: /supporter|Peer|Chat/i })
        .first();
  ...
  test.describe('Breathing Exercise', () => {
    test('opens with animated breathing guide', async ({ page }) => {
      ...
      await expect(page.getByText(/breath|inhale|exhale|calm|relax/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays visual animation elements', async ({ page }) => {
      ...
      const animatedElements = page.locator(
        '[class*="animate"], [class*="transition"], [class*="pulse"]'
      );
  ...
  test.describe('Coping Toolkit', () => {
    test('displays quick relief strategies', async ({ page }) => {
      ...
      await expect(page.getByText(/coping|relief|strategy|technique|help/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Wellbeing Journal', () => {
    test('opens with journal interface', async ({ page }) => {
      ...
      await expect(page.getByText(/journal|entry|write|gratitude/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays new entry form or existing entries', async ({ page }) => {
      ...
      if (!found) {
        found = await page
          .locator('[class*="journal"], [data-testid*="journal"]')
          .first()
          .isVisible()
          .catch(() => false);
      }
  ...
  test.describe('Sleep Tracker', () => {
    test('opens with tracking interface', async ({ page }) => {
      ...
      await expect(page.getByText(/sleep|hours|rest|quality|bed|wake/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays time inputs for bed and wake time', async ({ page }) => {
      ...
      await expect(page.getByText(/bed.*time|wake.*time|hours/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe('Safety Plan', () => {
    test('opens with crisis prevention sections', async ({ page }) => {
      ...
      await expect(page.getByText(/safety|crisis|plan|warning|coping/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays warning signs section', async ({ page }) => {
      ...
      await expect(page.getByText(/warning.*sign|trigger/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays coping strategies section', async ({ page }) => {
      ...
      await expect(page.getByText(/coping.*strateg|help.*myself/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays support contacts section', async ({ page }) => {
      ...
      await expect(page.getByText(/support|contact|reach out|people/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays professional support section', async ({ page }) => {
      ...
      await expect(page.getByText(/professional|helpline|GP|therapist/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test('displays reasons for living section', async ({ page }) => {
      ...
      await expect(page.getByText(/reason.*living|value|matter/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe('Mood Insights', () => {
    test('opens with analytics view', async ({ page }) => {
      ...
      await expect(page.getByText(/insight|pattern|trend|mood|average|track/i).first()).toBeVisible(
        { timeout: 10000 }
      );
    });
    ...
    test('displays data or encourages logging', async ({ page }) => {
      ...
      const hasPrompt = await page
        .getByText(/log.*mood|check.*in|start.*track/i)
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test.describe('Interactive Tools', () => {
    test('opens with tools interface', async ({ page }) => {
      ...
      await expect(page.getByText(/tool|track|goal|stress|mood/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe('Resources Library', () => {
    test('opens with self-help content', async ({ page }) => {
      ...
      await expect(
        page.getByText(/resource|guide|video|help|self-help|article/i).first()
      ).toBeVisible({ timeout: 10000 });
    });
  });
  ...
  test.describe('Support Network', () => {
    test('displays professional support options', async ({ page }) => {
      ...
      await expect(
        page.getByText(/support|connect|peer|professional|organisation|charity/i).first()
      ).toBeVisible({ timeout: 10000 });
    });
  });
  ...
  test.describe('Podcasts', () => {
    test('displays podcast content', async ({ page }) => {
      ...
      await expect(
        page.getByText(/podcast|listen|episode|audio|tradesperson|mental health/i).first()
      ).toBeVisible({ timeout: 10000 });
    });
  ...
  test.describe('URL Deep Linking', () => {
    ...
    test('URL updates when navigating between sections', async ({ page }) => {
      ...
      const urlUpdated = await page
        .waitForURL(/section=journal/, { timeout: 5000 })
        .then(() => true)
        .catch(() => false);
      const contentVisible = await page
        .getByText(/Wellbeing Journal|Track your thoughts/i)
        .first()
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test.describe('Electrician Mental Health Hub', () => {
    test('electrician mental health page loads', async ({ page }) => {
      ...
      await expect(page.getByText(/mental|health|wellbeing|support/i).first()).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe('Accessibility', () => {
    ...
    test('headings are present for screen readers', async ({ page }) => {
      ...
      const headings = page.locator(
        "h1, h2, h3, h4, h5, h6, [class*='font-bold'], [class*='text-xl'], [class*='text-2xl']"
      );
...
test.describe('Mental Health Hub - Backend API Tests', () => {
  ...
  test('peer conversations API loads correctly', async ({ page }) => {
    ...
    // Should show conversations or empty state
    await expect(page.getByText(/No conversations|conversation|chat|Start/i).first()).toBeVisible({
      timeout: 15000,
    });
  });
