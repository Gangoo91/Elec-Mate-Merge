...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `📊 Aggregation complete: ${uniqueJobs.length} unique jobs from ${sourceResults.filter((s) => s.success).length} sources`
    );
    ...
    await captureException(error, { functionName: 'live-job-aggregator', requestUrl: req.url, requestMethod: req.method });
