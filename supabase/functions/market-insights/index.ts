  ...
  try {
    ...
    const adzunaPromise = adzunaUrl
      ? fetch(adzunaUrl)
          .then((r) => r.json())
          .then((j) => {
            console.log('Adzuna ok');
            return parseAdzuna(j);
          })
          .catch((e) => {
    ...
    const reedPromise = (async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.error('Reed non-200', res.status);
    ...
    console.log('market-insights: job counts', {
    ...
    if (jobs.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No jobs returned from providers. Check API keys/quotas.' }),
    ...
    await captureException(e, { functionName: 'market-insights', requestUrl: req.url, requestMethod: req.method });
