...
serve(async (req) => {
  ...
  try {
    const {
      ...
      clientDetails,
      companyDetails,
      selectedDocuments = ['design', 'quote', 'rams', 'checklist', 'test', 'eic'],
    } = (await req.json()) as PackageRequest;
    ...
    console.log(
      `📦 Generating Professional Package: ${selectedDocuments.length} of 6 PDFs selected`
    );
    ...
    if (selectedDocuments.includes('design')) {
      ...
      pdfPromises.push({
        key: 'design',
        promise: generateDesignSpecWithPDFMonkey(
          extractedData,
          companyDetails,
          clientDetails,
          userId,
    ...
    if (authHeader) {
      try {
        ...
        if (user) {
          const savedDesign = await supabase
            .from('saved_designs')
            .insert({
              user_id: user.id,
              ...
              rams_data: {
                hazards: extractedData.safetyHazards,
                steps: extractedData.installationSteps,
              },
...
function extractDataFromConversation(messages: any[], designData: any) {
  ...
  return {
    projectName:
      designData.projectName || extractProjectName(conversationText) || 'Electrical Installation',
...
function extractDesignerNotes(messages: any[]): string {
  const designerMessages = messages.filter(
    (m) => m.role === 'assistant' && m.activeAgents?.includes('designer')
  );

  if (designerMessages.length === 0)
    return 'Design completed in accordance with BS 7671:2018+A3:2024.';
...
function extractSafetyData(messages: any[]): any[] {
  const safetyMessages = messages.filter(
    (m) => m.role === 'assistant' && m.activeAgents?.includes('health-safety')
  );
...
function extractInstallationSteps(messages: any[]): string[] {
  const installerMessages = messages.filter(
    (m) => m.role === 'assistant' && m.activeAgents?.includes('installer')
  );
...
async function generateDesignSpec(data: any, companyDetails: any): Promise<Uint8Array> {
  ...
  const notes = doc.splitTextToSize(
    data.designerNotes || 'Design completed to BS 7671:2018+A3:2024',
    170
  );
...
function extractCircuitsForTesting(data: any, messages: any[]): any[] {
  ...
  const designerMessages = messages.filter(
    (m: any) => m.role === 'assistant' && m.content?.includes('mm²')
  );
