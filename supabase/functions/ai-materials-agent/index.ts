...
Return ONLY valid JSON in this format:
{
  ...
  "confidence": 0.95
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a JSON parser for electrical materials. Always return valid JSON only.',
          },
    ...
    console.log(
      '[AI-MATERIALS-AGENT] Starting parallel search for',
    ...
    const searchPromises = parsedList.items.map(async (item) => {
      ...
      console.log(
        `[AI-MATERIALS-AGENT] Searching for: ${searchQuery}`,
      ...
      if (
        !searchResult.error &&
        (!searchResult.data?.materials || searchResult.data.materials.length === 0)
      ) {
      ...
      if (settled.status === 'fulfilled') {
        ...
        if (result.data?.materials && result.data.materials.length > 0) {
          ...
          if (filteredProducts.length === 0) {
            console.log(
              `[AI-MATERIALS-AGENT] ⚠️ Keyword filter removed all results for ${item.product}, using unfiltered`
            );
    ...
    const scoreProduct = (product: any, pref: string, requestedProduct: string): number => {
      ...
      if (pref === 'cheapest') {
        ...
        const brandScore = ['Hager', 'MK', 'Schneider', 'Crabtree', 'BG'].some(
          (b) => product.name.includes(b) || product.supplier.includes(b)
        )
          ? 1.0
          : 0.7;
        ...
        baseScore = priceScore * 0.35 + stockScore * 0.3 + brandScore * 0.35;
      }
        ...
        .map((result) => {
          ...
          return {
            ...
            alternatives: includeAlternatives
              ? scoredProducts.slice(1, 4).map((s: ProductScore) => s.product)
              : [],
          };
        })
