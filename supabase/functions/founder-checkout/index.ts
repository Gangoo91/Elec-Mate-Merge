...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      logger.error('JSON parse failed', { error: msg });
      return new Response(JSON.stringify({ error: 'Invalid JSON', details: msg }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json', 'x-request-id': requestId },
        status: 400,
      });
    }
    ...
    switch (action) {
      case 'validate': {
        ...
        const existingUser = usersData?.users?.find(
          (u: { email?: string }) => u.email === invite.email
        );
        ...
        if (isAlreadyFounder) {
          result = {
            valid: false,
            reason:
              "Great news! You're already subscribed as a Founder. Sign in to access your account.",
      ...
      case 'create_checkout': {
        ...
        const existingUserCheckout = usersListCheckout?.users?.find(
          (u: { email?: string }) => u.email === invite.email
        );
        ...
        if (existingUserCheckout) {
          ...
          if (checkoutProfile?.subscribed && checkoutProfile?.subscription_tier === 'founder') {
            logger.info('User is already a founder subscriber during checkout', {
              userId: existingUserCheckout.id,
            });
            ...
            result = {
              ...
              message:
                "You're already subscribed as a Founder! Please sign in to access your account.",
            };
            break;
          }
        }
      ...
      case 'create_account': {
        ...
        const { data: usersListData } = await supabaseAdmin.auth.admin.listUsers({ perPage: 1000 });
        ...
        if (existingUser) {
          ...
          if (existingProfile?.subscribed && existingProfile?.subscription_tier === 'founder') {
            logger.info('User is already a founder subscriber, marking invite as claimed', {
              userId,
            });
            // Mark invite as claimed
            await supabaseAdmin
          ...
          await supabaseAdmin
            .from('founder_invites')
            .update({ status: 'in_progress', user_id: userId })
            .eq('id', invite.id);
        } else {
        ...
        const origin =
          req.headers.get('origin') || Deno.env.get('SITE_URL') || 'https://elec-mate.com';
        const session = await stripe.checkout.sessions.create({
          customer: customerId,
          line_items: [{ price: FOUNDER_PRICE_ID, quantity: 1 }],
          mode: 'subscription',
      ...
      case 'complete': {
        ...
        if (!userId) {
          ...
          const userByEmail = usersListData?.users?.find(
            (u: { email?: string }) => u.email === invite.email
          );
    ...
    logger.error('Request failed', { error: errMsg });
    await captureException(error, {
      functionName: 'founder-checkout',
      requestUrl: req.url,
