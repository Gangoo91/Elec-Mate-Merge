...
Deno.serve(withSentry('generate-invoice-pdf', async (req) => {
  ...
  try {
    ...
    const { data: company } = await supabase
      ...
      .select(
        'company_name, company_email, company_phone, company_website, company_address, vat_number, utr, logo_url, bank_details, lead_page_slug, lead_page_enabled'
      )
      .eq('user_id', user.id)
      .maybeSingle();
...
<body><div class="page">
<div class="header"><div class="company">${company?.logo_url ? `<img src="${company.logo_url}" style="height:48px;margin-bottom:8px" />` : ''}<h1>${companyName}</h1>${company?.company_address ? `<p style="white-space:pre-line">${company.company_address}</p>` : ''}<p>${[company?.company_phone, company?.company_email].filter(Boolean).join(' · ')}</p>${company?.vat_number ? `<p>VAT Reg No: ${company.vat_number}</p>` : ''}${cisAmount > 0 && company?.utr ? `<p>UTR: ${company.utr}</p>` : ''}</div>
...
${lineItems.length > 0 ? `<table><thead><tr><th>Description</th><th>Qty</th><th>Unit price</th><th>Amount</th></tr></thead><tbody>${lineItems.map((i) => `<tr><td>${i.description}</td><td>${i.quantity}</td><td>${money(i.unitPrice)}</td><td>${money(i.total)}</td></tr>`).join('')}</tbody></table>` : ''}
