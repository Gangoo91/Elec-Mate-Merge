...
serve(async (req) => {
  ...
  try {
    ...
    const statusColour: StatusColour = TYPE_STATUS[obsType] || 'info';
    ...
    // Observation Details
    const capitalised = record.observation_type
      ? record.observation_type.charAt(0).toUpperCase() + record.observation_type.slice(1)
      : 'N/A';
