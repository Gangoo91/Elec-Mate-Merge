...
async function enrichBatch(
  ...
  for (const record of records) {
    try {
      ...
      const needsCableUpdate =
        extractedCables.length > 0 && (!record.cable_sizes || record.cable_sizes.length === 0);
      const needsLoadUpdate =
        extractedLoads.length > 0 && (!record.load_types || record.load_types.length === 0);
      ...
      if (needsCableUpdate || needsLoadUpdate) {
        ...
        if (updateError) {
          ...
          updated++;
          console.log(
            `✓ Updated record ${record.id}: cables=${extractedCables.join(', ')}, loads=${extractedLoads.join(', ')}`
          );
        }
      }
    } catch (err) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    while (currentBatch < maxBatches) {
      ...
      console.log(
        `Batch ${currentBatch + 1} complete: processed=${result.processed}, updated=${result.updated}, errors=${result.errors}`
      );
