  ...
  try {
    ...
    for (let i = 0; i < lines.length; i += chunkSize) {
      ...
      if (content.toLowerCase().includes('lighting') || content.toLowerCase().includes('switch')) {
        ...
        content.toLowerCase().includes('socket') ||
    ...
    await captureException(error, { functionName: 'parse-wiring-diagrams', requestUrl: req.url, requestMethod: req.method });
