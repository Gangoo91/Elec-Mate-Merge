  ...
  try {
    ...
    if (!reedApiKey) {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Reed API key is not configured. Please add the 'REED_API_KEY' to your Supabase secrets.",
        }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const { keywords, location, distance, permanent, temp, fullTime, partTime, page } =
      await req.json();
    ...
    const searchKeywords =
      keywords || 'electrical,electrician,electrical engineer,electrical technician';
