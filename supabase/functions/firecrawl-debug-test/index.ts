  ...
  try {
    ...
    try {
      ...
      results.push({
        ...
        preview: basicResponse.success
          ? basicResponse.data?.markdown?.substring(0, 500) || 'No content'
          : 'No content',
    ...
    try {
      const extractResponse = (await firecrawl.scrapeUrl(urlToTest, {
        ...
        extract: {
          ...
          prompt:
            'Extract all product listings from this page. Look for product names, prices, and links. Products might be in cards, lists, or grid layouts.',
        },
      })) as any;
      ...
      results.push({
        ...
        productsFound: extractResponse.success
          ? extractResponse.data?.extract?.products?.length || 0
          : 0,
    ...
    try {
      ...
      results.push({
        ...
        contentLength: screenshotResponse.success
          ? screenshotResponse.data?.markdown?.length || 0
          : 0,
    ...
    if (successfulMethods === 0) {
      analysis.recommendations.push(
        'All extraction methods failed - check if the website is blocking Firecrawl or if there are network issues'
      );
    }
    ...
    if (basicResult?.success && basicResult.contentLength === 0) {
      analysis.recommendations.push(
        'Website returned empty content - likely anti-bot protection or JavaScript-heavy content'
      );
    }
    ...
    if (extractResult?.success && extractResult.productsFound === 0) {
      analysis.recommendations.push(
        'Structured extraction succeeded but found no products - schema may need updating or different extraction prompt'
      );
    }
