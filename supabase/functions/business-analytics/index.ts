...
serve(async (req) => {
  ...
  try {
    ...
    const certificatesByType = Object.entries(typeLabels)
      .map(([type, label]) => ({
        type,
        label,
    ...
    const monthlyMap = new Map<
      string,
      { certificates: number; quotes: number; invoices: number; total: number }
    >();
    ...
    (allCerts || []).forEach((c) => {
      ...
      if (!monthlyMap.has(key))
        monthlyMap.set(key, { certificates: 0, quotes: 0, invoices: 0, total: 0 });
    ...
    const clientMetrics: any = {
      ...
      avgCertsPerClient:
        totalClients > 0 ? Math.round((totalCertsForAvg / totalClients) * 10) / 10 : 0,
      topClients,
    };
    ...
    await captureException(err, { functionName: 'business-analytics', requestUrl: req.url, requestMethod: req.method });
