...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('[generate-isolation-cert-pdf] Creating PDF document');
    console.log('[generate-isolation-cert-pdf] Reference:', formData.reference_number);
    console.log('[generate-isolation-cert-pdf] Equipment:', formData.equipment_description);
    console.log('[generate-isolation-cert-pdf] Site:', formData.site_name);

    const document = await createPDFMonkeyDocument(formData, templateId);
    console.log('[generate-isolation-cert-pdf] Document created:', document.id);

    const completedDocument = await waitForPDFGeneration(document.id);
    console.log('[generate-isolation-cert-pdf] PDF generated successfully');
