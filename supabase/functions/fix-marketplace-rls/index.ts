...
serve(async (req) => {
  ...
  try {
    ...
    for (const table of tables) {
      ...
      try {
        await sql.unsafe(
          `DROP POLICY IF EXISTS "Public read access for ${table.replace('marketplace_', '')}" ON public.${table}`
        );
        await sql.unsafe(
          `DROP POLICY IF EXISTS "Service role full access ${table.replace('marketplace_', '')}" ON public.${table}`
        );
      } catch (e) {
        // Ignore errors
      }
