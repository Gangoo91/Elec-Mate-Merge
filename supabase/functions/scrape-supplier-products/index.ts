...
const cablesWiringSchema = {
  ...
  items: {
    ...
    properties: {
      ...
      price: {
        type: 'string',
        description: 'The price of the product, including currency and VAT info',
      },
  ...
  items: {
    ...
    properties: {
      ...
      highlights: {
        ...
        items: { type: 'string' },
        description:
          'Product highlights, features, certifications (BS/CE compliance), safety features, key specifications, or protection capabilities',
      },
    },
  },
};
...
function buildSearchUrl(slug: SupplierSlug, query: string) {
  ...
  const isToolsSearch =
    /testers.*hand.*tools.*power.*tools|multimeter|socket tester|cable detector|voltage detector|testing|tester|meter/i.test(
      query
    );
  const isProtectionSearch = /mcb|rcd|rcbo|breaker|protection|switch|isolator|surge|spd/i.test(
    query
  );
...
async function fetchLightingSolutions(): Promise<MaterialItem[]> {
  ...
  const productSchema = {
    ...
    items: {
      ...
      properties: {
        ...
        highlights: {
          type: 'array',
          description: 'The highlight or lighting features of the product',
        },
        price: {
          type: 'string',
          description: 'The price of the product, including currency and VAT info',
  ...
  try {
    console.log(
      `[LIGHTING-SOLUTIONS] Firecrawl v2 scraping lighting solutions with comprehensive search...`
    );
    console.log(
      `[LIGHTING-SOLUTIONS] Search URL: https://www.screwfix.com/search?search=LED%2C+downlights%2C+battens%2C+emergency%2C+controls&page_size=100`
    );
    ...
    if (data.success && data.data && data.data.json) {
      ...
      return products.map((product: any, index: number) => ({
        ...
        supplier: 'Screwfix',
        image:
          product.image && product.image.startsWith('http') ? product.image : '/placeholder.svg',
    ...
    console.error(
      `⚠️ Error fetching Lighting Solutions:`,
...
async function fetchInstallationAccessories(): Promise<MaterialItem[]> {
  ...
  const productSchema = {
    ...
    items: {
      ...
      properties: {
        ...
        highlights: {
          type: 'array',
          description: 'The highlight or cable highlight of the product',
        },
        price: {
          type: 'string',
          description: 'The price of the product, including currency and VAT info',
  ...
  try {
    ...
    if (data.success && data.data && data.data.json) {
      const products = Array.isArray(data.data.json) ? data.data.json : [];
      console.log(
        `[INSTALLATION-ACCESSORIES] Extracted ${products.length} products from Firecrawl v2`
      );
    ...
    console.error(
      `⚠️ Error fetching Installation Accessories:`,
      error instanceof Error ? error.message : 'Unknown error occurred'
    );
    return [];
  }
  ...
  try {
    ...
    if (data.success && data.data && data.data.json) {
      ...
      return products.map((product: any, index: number) => ({
        ...
        highlights:
          product.highlights ||
          generateComponentHighlights(product.name || '', product.description || ''),
      }));
    } else {
    ...
    console.error(
      `⚠️ Error fetching Protection Equipment:`,
      error instanceof Error ? error.message : 'Unknown error occurred'
    );
    return [];
  }
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[SCRAPE-SUPPLIER-PRODUCTS] Request details: category=${category}, supplierSlug=${supplierSlug}, searchTerm=${searchTerm}, forceRefresh=${forceRefresh}`
    );

    if (!supplierSlug || !(supplierSlug in SUPPLIER_NAMES)) {
      return new Response(JSON.stringify({ error: 'Invalid or missing supplierSlug' }), {
        status: 400,
    ...
    const isCablesSearch =
      /cable|twin|earth|wiring|6242y|swa|flex/i.test(searchTerm) || category === 'cables';
    const isProtectionSearch =
      /earthing|surge protection|circuit protection|mcb|rcd|rcbo|breaker|protection equipment|switch|isolator|earth rod/i.test(
        searchTerm
      ) || category === 'protection';
    const isAccessoriesSearch =
      /junction|glands|trunking|fixings|accessories/i.test(searchTerm) ||
      category === 'accessories';
    const isLightingSearch =
      /LED\+downlights|downlight|batten|emergency|lighting|controls/i.test(searchTerm) ||
      category === 'lighting';

    if (isCablesSearch || isAccessoriesSearch || isProtectionSearch || isLightingSearch) {
      const currentCategory = isCablesSearch
        ? 'cables'
        : isProtectionSearch
          ? 'protection'
          : isLightingSearch
            ...
            : 'accessories';
      console.log(
        `[SCRAPE-SUPPLIER-PRODUCTS] CACHE DISABLED - forcing live scraping for ${currentCategory}`
      );
      ...
      console.log(
        `[SCRAPE-SUPPLIER-PRODUCTS] Live scraping ${currentCategory} returned ${freshResults.length} products`
      );
        ...
        JSON.stringify({
          ...
          note:
            freshResults.length === 0
              ? `No ${currentCategory} products found via live scraping`
              : undefined,
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    if (firecrawlKey && !isCablesSearch && !isAccessoriesSearch && !isProtectionSearch) {
      try {
        ...
        if (isProtectionSearch && supplierSlug === 'screwfix') {
          ...
          console.log(
            '[SCRAPE-SUPPLIER-PRODUCTS] Firecrawl v2 status:',
            fcRes.status,
            fcJson?.success
          );
          ...
          if (fcJson.success && fcJson.data?.json) {
            ...
            products = jsonProducts.slice(0, 8).map((item: any, index: number) => ({
              ...
              highlights:
                item.highlights ||
                generateComponentHighlights(item.name || '', item.description || ''),
            }));
          }
        } else {
          ...
          const fcJson: any = await fcRes.json().catch(() => ({}));
          console.log(
            '[SCRAPE-SUPPLIER-PRODUCTS] Firecrawl status:',
            fcRes.status,
            fcJson?.status || fcJson?.success
          );
          ...
          if (html) {
            ...
            while ((match = anchorRegex.exec(html)) !== null && results.length < 48) {
              ...
              if (
                /account|basket|cart|help|delivery|contact|login|register|cookie|privacy|terms/i.test(
                  text
                )
              )
                continue;
              ...
              for (const m of priceMatches) {
                const idx = m.index ?? 0;
                const window = snippet.slice(
                  Math.max(0, idx - 24),
                  Math.min(snippet.length, idx + 24)
                );
              ...
              const isCable =
                /\b(cable|t\s*&\s*e|twin\s*&\s*earth|twin|earth|swa|flex|mm2|mm²|cat\d|6242y|coax|data)\b/i.test(
                  text
                ) || /cable/i.test(searchTerm);
              const isTool =
                /\b(multimeter|tester|detector|meter|test|testing|socket|voltage|continuity|clamp|insulation)\b/i.test(
                  text
                ) || /multimeter|tester|detector/i.test(searchTerm);
    ...
    if (!products || products.length === 0) {
      console.log(
        `[SCRAPE-SUPPLIER-PRODUCTS] No products found, checking for category-specific fallbacks`
      );
      ...
      if (isCablesSearch || isAccessoriesSearch || isProtectionSearch) {
        console.log(
          `[SCRAPE-SUPPLIER-PRODUCTS] Cached category ${category} had no results, returning empty array`
        );
        return new Response(
          JSON.stringify({
            products: [],
      ...
      JSON.stringify({
        ...
        note: firecrawlKey
          ? products.length
            ...
            : 'Fallback products - live scraping returned no results'
          : 'Curated products - Firecrawl key not configured',
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error) {
