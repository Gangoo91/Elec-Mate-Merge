...
const TEMPLATE_ID = 'CD291754-3E95-4444-BED7-AFDC4B2D89FF';
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('[generate-non-compliance-notice-pdf] Ref:', formData.reference_number);
    console.log('[generate-non-compliance-notice-pdf] Items:', formData.items?.length || 0);
    console.log('[generate-non-compliance-notice-pdf] Immediate action:', formData.immediate_action_required);
    ...
    await captureException(error, { functionName: 'generate-non-compliance-notice-pdf', requestUrl: req.url, requestMethod: req.method });
    console.error('[generate-non-compliance-notice-pdf] Error:', error);
