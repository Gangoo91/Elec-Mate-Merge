  ...
  try {
    ...
    if (batchNumber && batchNumber >= 1 && batchNumber <= TRAINING_BATCHES.length) {
      ...
      if (!forceRefresh) {
        ...
        if (cached) {
          console.log(
            `✅ Using cached data for batch ${batchNumber} (${cached.course_count} courses)`
          );
          return new Response(
            JSON.stringify({
              success: true,
      ...
      // Save to cache
      const { error: cacheError } = await supabase.from('training_courses_cache').insert({
...
function inferCategory(title: string, explicitCategory?: string): string {
  ...
  if (
    ...
    titleLower.includes('electric vehicle') ||
    titleLower.includes('2919')
  ) {
...
function inferRegion(city: string): string {
  ...
  if (
    ['brighton', 'reading', 'southampton', 'oxford', 'cambridge'].some((c) => cityLower.includes(c))
  ) {
    return 'Southeast';
  }
  if (
    ['birmingham', 'coventry', 'leicester', 'nottingham', 'derby'].some((c) =>
  ...
  if (
    ['manchester', 'liverpool', 'preston', 'bolton', 'blackpool'].some((c) => cityLower.includes(c))
  ) {
    return 'Northwest';
  }
...
function generateFallbackCourses(batch: (typeof TRAINING_BATCHES)[0]): any[] {
  ...
  const courseTitles = [
    ...
    {
      title: 'EV Charging Installation (C&G 2919)',
      category: 'EV Charging Installation',
      price: '£595',
    },
    {
...
async function mergeAllCourses(supabase: any): Promise<Response> {
  ...
  console.log(
    `📊 Found ${allCourses.length} unique courses from ${cachedBatches?.length || 0} batches`
  );
  ...
  for (let i = 0; i < allCourses.length; i += batchSize) {
    ...
    const { error: insertError } = await supabase.from('training_courses').insert(batch);
