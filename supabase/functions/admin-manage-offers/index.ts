  ...
  try {
    ...
    switch (action) {
      ...
      case 'create': {
        ...
        if (discountAmount > 0) {
          try {
            ...
            if (offer.expires_at) {
              promoCodeOptions.expires_at = Math.floor(new Date(offer.expires_at).getTime() / 1000);
            }
      ...
      case 'update': {
        ...
        if (offer.is_active !== undefined && existingOffer?.stripe_promotion_code_id) {
          try {
      ...
      case 'delete': {
        ...
        const { error } = await supabaseAdmin.from('promo_offers').delete().eq('id', offer.id);
        if (error) throw error;
