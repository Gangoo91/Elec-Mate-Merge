...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const validation = emergencyLightingPayloadSchema.safeParse(formData);
    if (!validation.success) {
      console.error('[generate-emergency-lighting-pdf] Schema validation failed:',
        JSON.stringify(validation.error.issues.slice(0, 10)));
      await captureException(new Error('Emergency Lighting payload schema drift detected'), {
        functionName: 'generate-emergency-lighting-pdf',
    ...
    console.log(
      '[generate-emergency-lighting-pdf] Certificate number:',
      formData.certificate_number || formData.metadata?.certificate_number
    );
    console.log(
      '[generate-emergency-lighting-pdf] Client:',
      JSON.stringify(formData.client || formData.client_details, null, 2)
    );
    console.log(
      '[generate-emergency-lighting-pdf] Premises:',
      JSON.stringify(formData.premises || formData.premises_details, null, 2)
    );
    console.log(
      '[generate-emergency-lighting-pdf] System:',
      JSON.stringify(formData.system || formData.system_details, null, 2)
    );
    console.log(
      '[generate-emergency-lighting-pdf] Equipment:',
      JSON.stringify(formData.equipment || formData.equipment_counts, null, 2)
    );
    console.log(
      '[generate-emergency-lighting-pdf] Luminaire count:',
      formData.luminaires?.length || 0
    );
    console.log(
      '[generate-emergency-lighting-pdf] Luminaire summary:',
      JSON.stringify(formData.luminaire_summary, null, 2)
    );
    console.log(
      '[generate-emergency-lighting-pdf] Monthly test:',
      JSON.stringify(
        formData.monthly_test || formData.test_results?.monthly_functional_test,
        null,
        2
      )
    );
    console.log(
      '[generate-emergency-lighting-pdf] Annual test:',
      JSON.stringify(formData.annual_test || formData.test_results?.annual_duration_test, null, 2)
    );
    console.log(
      '[generate-emergency-lighting-pdf] Tester:',
      JSON.stringify(formData.tester || formData.declarations?.tester, null, 2)
    );
