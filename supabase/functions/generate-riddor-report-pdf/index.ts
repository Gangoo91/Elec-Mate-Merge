...
serve(async (req) => {
  ...
  try {
    ...
    const html = riddorTemplate(record, branding);
    const pdfBytes = await htmlToPdf(html);

    // ── Upload PDF ─────────────────────────────────────────────────────
    const fileName = `riddor-report-${recordId}-${Date.now()}.pdf`;
    ...
    // RIDDOR uses the accident_records table but does not update pdf_url
    // (the accident PDF function handles that separately)

    return new Response(JSON.stringify({ success: true, url: urlData.publicUrl }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    await captureException(error, { functionName: 'generate-riddor-report-pdf', requestUrl: req.url, requestMethod: req.method });
