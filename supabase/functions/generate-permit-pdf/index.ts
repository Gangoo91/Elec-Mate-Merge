...
serve(async (req) => {
  ...
  try {
    ...
    await supabase.from('permits_to_work').update({ pdf_url: urlData.publicUrl }).eq('id', id);

    return new Response(JSON.stringify({ success: true, url: urlData.publicUrl }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    await captureException(error, { functionName: 'generate-permit-pdf', requestUrl: req.url, requestMethod: req.method });
