  ...
  try {
    ...
    for (let i = 0; i < lines.length; i += chunkSize) {
      ...
      if (
        ...
        content.toLowerCase().includes('mounting')
      ) {
        ...
        content.toLowerCase().includes('residential') ||
        content.toLowerCase().includes('domestic')
      ) {
        topic = 'Domestic EV Charging';
        keywords.push('residential', 'domestic', 'home charging');
      } else if (
        content.toLowerCase().includes('commercial') ||
        content.toLowerCase().includes('workplace')
      ) {
    ...
    await captureException(error, { functionName: 'parse-ev-charging', requestUrl: req.url, requestMethod: req.method });
