...
serve(async (req) => {
  ...
  try {
    ...
    await captureException(error, { functionName: 'parse-abb-equipment', requestUrl: req.url, requestMethod: req.method });
...
function detectTopic(content: string, equipmentType: string): string {
  ...
  if (equipmentType === 'transformer') {
    ...
    if (lower.includes('cooling') || lower.includes('temperature'))
      return 'Transformer Cooling Systems';
  ...
  if (equipmentType === 'circuit-breaker' || equipmentType === 'gcb') {
    ...
    if (lower.includes('breaking') || lower.includes('interruption'))
      return 'Breaking Capacity & Arc Quenching';
    ...
    if (lower.includes('generator') || lower.includes('gcb'))
      return 'Generator Circuit Breaker (GCB)';
  ...
  if (equipmentType === 'motor') {
    if (lower.includes('design') || lower.includes('construction'))
      return 'Motor Design & Construction';
    if (lower.includes('insulation') || lower.includes('winding'))
      return 'Motor Insulation Systems';
...
function detectVoltageLevel(content: string): string {
  ...
  if (
    lower.match(/\b[1-9]\d{2,3}\s*kv\b/) ||
  ...
  if (
    lower.match(/\b[1-3]?[0-9]\s*kv\b/) ||
    lower.includes('medium voltage') ||
    lower.includes('mv ')
  ) {
    return 'MV (1-36kV)';
  }
...
function getSourceIdentifier(fileName: string): string {
  ...
  if (fileName.includes('Circuit') || fileName.includes('Erection'))
    return 'abb-circuit-breaker-manual';
