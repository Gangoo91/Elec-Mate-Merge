...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Sending reminder email to: ${email} (${firstName}), days left: ${daysLeft}, role: ${profile.role}`
    );
    ...
    await captureException(error, { functionName: 'send-trial-reminder', requestUrl: req.url, requestMethod: req.method });
