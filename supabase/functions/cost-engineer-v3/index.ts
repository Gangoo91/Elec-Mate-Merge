...
If ANY category missing, estimate it and flag in response.`;
    ...
    if (shouldCalculateProfitability) {
      // Fire-and-forget profitability calculation using modular helper
      EdgeRuntime.waitUntil(
        calculateProfitabilityAnalysis(costResult, businessSettings, LOVABLE_API_KEY, logger)
          .then(result => {
            costResult.profitabilityAnalysis = result;
            logger.info('✅ Profitability analysis completed', { breakEvenTotal: result.breakEven.total });
          })
          .catch(err => {
            logger.error('❌ Profitability analysis failed', { error: err.message });
            costResult.profitabilityAnalysis = createFallbackProfitability(costResult);
          })
      )
