...
async function scrapeSource(source: ScrapingSource) {
  ...
  try {
    ...
    console.log(
      `Successfully processed ${source.name}: ${projectsAdded}/${projectsFound} projects added`
    );
  } catch (error) {
...
async function parseProjectsFromHtml(
  html: string,
...
async function parseContractsFinderProjects(
  html: string,
...
async function parseFindTenderProjects(
  html: string,
  projects: ScrapedProject[],
  sourceName: string
) {
...
function extractProjectFromMatch(
  html: string,
  sourceName: string,
  sourceType: string
): ScrapedProject | null {
  try {
    ...
    if (
      textContent.toLowerCase().includes('awarded') ||
      textContent.toLowerCase().includes('won')
    ) {
      status = 'awarded';
    } else if (
      textContent.toLowerCase().includes('closed') ||
...
async function saveProject(project: ScrapedProject, sourceName: string): Promise<boolean> {
  try {
    ...
    const { error: trackingError } = await supabase.from('scraped_project_tracking').insert({
