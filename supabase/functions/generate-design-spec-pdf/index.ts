...
serve(async (req) => {
  ...
  try {
    const { designData, userId } = await req.json();
    const pdfMonkeyApiKey = Deno.env.get('PDF_MONKEY_API_KEY');

    console.log('Generating design specification PDF');
    ...
    if (!template?.pdf_monkey_template_id || !pdfMonkeyApiKey) {
      console.log('No custom template configured, using fallback jsPDF generation');
    ...
    const mappedData = applyFieldMapping(designData, template.field_mapping || {});
    ...
    const response = await fetch('https://api.pdfmonkey.io/api/v1/documents', {
      ...
      body: JSON.stringify({
        document: {
          ...
          payload: mappedData,
          meta: {
            _filename: `Design_Specification_${Date.now()}.pdf`
          }
        }
      }),
    });
    ...
    console.error('Error in generate-design-spec-pdf function:', error);
    return new Response(JSON.stringify({ 
      success: false,
      useFallback: true,
