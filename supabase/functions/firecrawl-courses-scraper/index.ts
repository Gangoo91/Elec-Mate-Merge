...
serve(async (req) => {
  ...
  try {
    ...
    const courseWebsites = {
      ...
      cityandguilds:
        'https://www.cityandguilds.com/qualifications/construction-and-the-built-environment/electrical-installation',
      niceic: 'https://www.niceic.com/find-an-installer/electrical-training-courses',
      stanmore:
        'https://certificates.stanmoreuk.org/Home/Courses/6026646/Electrical-Career-Courses-%26-Training',
    };

    const targetUrl =
      courseWebsites[source as keyof typeof courseWebsites] || courseWebsites.findcourses;
...
Skip non-electrical courses, general education courses, or courses outside the UK.`,
    ...
    try {
      ...
      if (extractedCourses.length === 0 && firecrawlData.data?.markdown) {
        ...
        if (
          markdown.includes('course') ||
          markdown.includes('training') ||
          markdown.includes('electrical')
        ) {
          console.log(
            `📝 Found content but no structured courses from ${source}, creating fallback course`
          );
          extractedCourses = [
            {
    ...
    const courses = extractedCourses.map((course: any, index: number) => {
      ...
      return {
        ...
        futureProofing:
          parseFutureScope(course.futureScope) ||
          calculateFutureProofing(title, course.description),
        ...
        careerOutcomes:
          course.careerOutcomes && course.careerOutcomes.length > 0
            ...
            : extractCareerOutcomes(course.description),
        accreditation:
