...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `📦 Created ${textChunks.length} smart chunks (avg ${Math.round(fullText.length / textChunks.length)} chars each)`
    );
    ...
    for (let i = 0; i < textChunks.length; i++) {
      ...
      if (estimatedTokens > 2000) {
        console.warn(
          `⚠️ Large chunk at index ${i}: ${content.length} chars (~${estimatedTokens} tokens)`
        );
      }
      ...
      if (
        ...
        content.toLowerCase().includes('overseeing')
      ) {
        topic = 'Planning & Supervision';
        keywords.push('planning', 'supervision', 'work programmes');
      } else if (
        content.toLowerCase().includes('electrical science') ||
        ...
        content.toLowerCase().includes('motor') &&
        ...
        content.toLowerCase().includes('thermal')
      ) {
        ...
        content.toLowerCase().includes('arrangement')
      ) {
        ...
        content.toLowerCase().includes('fault') &&
    ...
    await captureException(error, { functionName: 'parse-city-guilds-level-3', requestUrl: req.url, requestMethod: req.method });
