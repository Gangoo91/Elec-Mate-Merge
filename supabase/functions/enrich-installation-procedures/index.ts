...
serve(withSentry('enrich-installation-procedures', async (req) => {
...
[{
  ...
  "regulations_cited": ["BS 7671 Section X"]
        ...
        const response = await callOpenAIWithRetry(
          openAIKey,
          extractionPrompt,
          'You are an electrical installation expert. Extract structured procedures. Return valid JSON only.'
        );
        ...
        for (const proc of procedures) {
          await supabase.from('installation_procedures').upsert(
            {
              source_id: doc.id,
