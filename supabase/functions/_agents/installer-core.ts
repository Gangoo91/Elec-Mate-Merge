...
async function generateMethodStatementInternal(
  ...
  console.log(
    `✅ RAG complete: ${practicalWork.length} practical docs, ${regulations.length} regulations (${ragDuration}ms)`
  );
...
${regulations.map((r) => `- ${r.regulation_number || r.id}: ${r.content || r.primary_topic}`).join('\n')}
  ...
  console.log(
    `✅ Installer complete: ${result.installationSteps.length} steps, ${result.testingProcedures.length} tests (${aiDuration}ms)`
  );
