  ...
  const designPromise = (async () => {
    ...
    try {
      ...
      if (error) {
        ...
        const { data: v2Data, error: v2Error } = await supabase.functions.invoke(
          'designer-agent-v2',
          {
            body: designRequest,
          }
      ...
      if (!data || !data.success) {
        ...
        throw new Error(`Designer agent returned failure: ${failureReason} [Code: ${errorCode}]`);
      }
      ...
      const duration = Date.now() - startTime;
      console.log(
        `✅ Designer completed ${data.circuits.length} circuit designs in ${(duration / 1000).toFixed(1)}s`
      );
...
export async function designCircuits(
  ...
  if (attemptNumber === 1 && jobInputs.circuits?.length > 0) {
    ...
    if (circuitCount > 8 || complexityScore > 12) {
      console.log(
        `⚡ Pre-splitting due to high complexity (score: ${complexityScore}, circuits: ${circuitCount})`
      );
      ...
      console.log(
        `🔄 Pre-split: Batch 1 (${firstHalf.length} circuits), Batch 2 (${secondHalf.length} circuits)`
      );
      ...
      console.log(
        `✅ Pre-split design completed: ${firstResult.circuits.length + secondResult.circuits.length} total circuits`
      );
  ...
  try {
    ...
    if (attemptNumber === 1 && jobInputs.circuits && jobInputs.circuits.length > 5) {
      console.warn(
        `⚠️ Design failed with ${jobInputs.circuits.length} circuits. Splitting into batches...`
      );
      ...
      console.log(
        `🔄 Splitting: Batch 1 (${firstHalf.length} circuits), Batch 2 (${secondHalf.length} circuits)`
      );
      ...
      console.log(
        `✅ Split design completed: ${firstResult.circuits.length + secondResult.circuits.length} total circuits`
      );
