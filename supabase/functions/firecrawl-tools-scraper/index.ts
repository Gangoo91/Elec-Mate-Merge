  ...
  items: {
    ...
    properties: {
      ...
      brand: {
        type: 'string',
        description:
          'Brand or manufacturer name (e.g., Makita, DeWalt, Bosch, Hilti, Bahco, Wiha, Wera)',
      },
      category: {
        type: 'string',
        description:
          'The product category (e.g., Drills, Screwdrivers, Power Tools, Cables) or type of cable',
      },
      productType: {
        type: 'string',
      ...
      price: {
        type: 'string',
        description:
          'The price of the product, including numeric value, currency, and VAT info if applicable',
      },
      inStock: {
        type: 'boolean',
...
async function fetchElectricalTools(logger: any) {
  ...
  try {
    ...
    const response = await withRetry(
      () => withTimeout(fetch(url, options), Timeouts.LONG, 'Firecrawl tools scrape'),
    ...
    logger.error('Error fetching electrical tools', {
...
serve(async (req) => {
  ...
  try {
    ...
    const tools = await logger.time('Tools scraping', () => fetchElectricalTools(logger));
    ...
    logger.error('Firecrawl tools scraper error', {
      error: error instanceof Error ? error.message : String(error),
    });
