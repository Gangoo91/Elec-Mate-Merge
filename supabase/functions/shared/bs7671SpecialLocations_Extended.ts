...
export const SECTION_702_SWIMMING_POOLS: SpecialLocationRequirement = {
  ...
  zones: [
    {
      ...
      dimensions:
        'Inside the pool basin including recesses in walls/floors, foot cleaning basins, waterjets, waterfalls and space below',
      restrictions: [
    ...
    },
    {
      ...
      dimensions:
        'Vertical plane 2m from pool rim. Height to 2.5m above floor. For diving boards: 1.5m from periphery',
      restrictions: [
  ...
  equipmentRestrictions: [
    {
      zone: 'Zone 0',
      restriction: 'Only underwater pool equipment (pumps, lights) designed for immersion',
    },
    {
      zone: 'Zone 1',
      restriction:
        'Filtration systems, jet stream pumps in insulated enclosure with tool/key access',
    },
    { zone: 'Zone 2', restriction: 'General equipment permitted with RCD protection' },
  ],
  bondingRequirements:
    'Supplementary bonding MANDATORY in all zones (0, 1, 2) connecting all exposed & extraneous parts. NOT permitted to omit (702.415.2)',
  keyRegulations: [
...
export const SECTION_704_CONSTRUCTION_SITES: SpecialLocationRequirement = {
  ...
  equipmentRestrictions: [
    {
      zone: 'All areas',
      restriction: 'Socket-outlets ≤32A MUST be via ACS (Assembly for Construction Sites)',
    },
    {
      zone: 'All areas',
      restriction:
        'Hand-held equipment ≤32A: RCD, Reduced low voltage (110V), electrical separation, or SELV',
    },
    {
      zone: 'All areas',
      restriction:
        'Portable handlamps: Reduced low voltage (110V centre-tapped 55V) STRONGLY PREFERRED',
    },
...
export const SECTION_708_CARAVAN_PARKS: SpecialLocationRequirement = {
  ...
  ipRatings: [
    {
      ...
      description: 'Protected against splashing water (AD4)',
    },
    {
      zone: 'All equipment',
      ipRating: 'IP4X minimum',
      description: 'Protected against small objects >1mm (AE3)',
    },
    {
      zone: 'Impact areas',
      ipRating: 'IK08 minimum',
      description: 'Mechanical impact protection (AG3)',
    },
  ],
  rcdRequirements: [
    {
  ...
  equipmentRestrictions: [
    {
      zone: 'All areas',
      restriction: 'Socket-outlets BS EN 60309-2 with interlocking to prevent live when accessible',
    },
    {
      zone: 'All areas',
      restriction: 'Each socket-outlet individually overcurrent protected (Chapter 43)',
    },
    ...
    {
      zone: 'Pitch equipment',
      restriction: 'Sockets 0.5m to 1.5m height (higher if flood/snow risk with special measures)',
    },
    { zone: 'Pitch equipment', restriction: 'Minimum 16A rating per socket-outlet' },
  ],
  bondingRequirements:
    'Socket-outlet protective conductors SHALL NOT be connected to PME earthing facility (708.553.1.14). TT system required for pitch sockets.',
  keyRegulations: [
...
export const SECTION_717_MOBILE_UNITS: SpecialLocationRequirement = {
  ...
  equipmentRestrictions: [
    {
      zone: 'Supply connection',
      restriction: 'Main switch-disconnector rated ≥supply current capacity',
    },
    {
      zone: 'Supply connection',
      restriction: 'Connection point suitable for anticipated supply system',
    },
  ...
  bondingRequirements:
    'All exposed-conductive-parts of unit bonded to main earthing terminal. Extraneous-conductive-parts bonded if external metalwork.',
  keyRegulations: [
...
export function getSpecialLocationRequirements(
  locationCode: string
): SpecialLocationRequirement | undefined {
...
export function checkPMEAllowedSpecialLocation(locationCode: string): {
  ...
  if (locationCode === '705') {
    return {
      ...
      restrictions:
        'TN-C NOT PERMITTED. PME not recommended for livestock areas. TN-C-S allowed for non-livestock areas.',
    };
  }
...
export function getRCDRequirementsForLocation(locationCode: string): string {
  ...
  return location.rcdRequirements
    .map(
      (r) =>
        `${r.mandatory ? 'MANDATORY' : 'Recommended'}: ${r.rating}mA Type ${r.type} RCD - ${r.reason} (${r.regulation})`
    )
    .join('\n');
}
