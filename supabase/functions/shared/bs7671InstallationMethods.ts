...
export const SAFE_ZONES: SafeZone[] = [
  ...
  },
  {
    ...
    additionalProtection:
      'If less than 50mm vertical distance, provide mechanical protection or RCD',
  },
];
...
export function checkSafeZoneCompliance(
  ...
  const isInSafeZone = distanceFromTop <= 150 || distanceFromCorner <= 150 || hasAccessoryNearby;
...
export const FIRE_RATED_SUPPORT: FireRatedSupportRequirement = {
  ...
  notes:
    'This regulation prevents cables from collapsing and blocking escape routes during fire. Metal supports maintain integrity longer in fire conditions.',
};
...
export const ELECTRICAL_CONNECTION_REQUIREMENTS: ConnectionRequirement = {
  ...
  requirement:
    'Every connection shall provide durable electrical continuity and adequate mechanical strength',
  considerations: [
...
export function getExternalInfluenceProtection(
  code: string
): ExternalInfluenceProtection | undefined {
...
export function validateCableDepth(
  ...
  return {
    ...
    requiresRCD: true,
    reasoning:
      'Cable <50mm and outside safe zone - RCD + mechanical protection required per Reg 522.6.202',
  };
}
