...
export const GS38_CORE_REQUIREMENTS: GS38Requirement[] = [
  {
    ...
    content:
      'GS 38 applies to people using electrical test equipment on low voltage electrical systems and equipment (up to 1000V AC). This includes electricians, contractors, test supervisors, technicians, and appliance repairers.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Electricity at Work Regulations 1989 require systems to be safe and maintained safely. Live working only permitted when: (1) unreasonable to work dead AND (2) reasonable to work live AND (3) suitable precautions taken.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Test probes and leads MUST conform to BS EN 61010-031 or BS EN 61243-3. Must be marked with CAT rating (II, III, or IV) and manufacturer identification.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Measurement categories reflect overvoltage protection levels. Equipment CAT rating MUST match or exceed the installation category being tested.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Test leads must be adequately insulated, colour-coded, flexible, sheathed for mechanical protection, and appropriate length. No exposed conductors except probe tips.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Two approved types: (1) Test lamps with HBC fuse protection (2) 2-pole detectors with built-in current limiting and dual indication systems.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Single-pole/non-contact voltage detectors ("voltage sticks") should ONLY be used for identifying live equipment, NOT for proving dead.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Test equipment used to prove dead may fail to indicate danger. MUST be proved before and after use on a proving unit or known live source.',
    category: 'critical',
    keyPoints: [
  ...
  },
  {
    ...
    content:
      'Most accidents occur from incorrect multimeter settings. Ensure correct function setting, appropriate CAT rating, and secure lead connections.',
    category: 'critical',
    keyPoints: [
 ...
 */
export function getGS38RequirementsByCategory(
  category: 'critical' | 'important' | 'reference'
): GS38Requirement[] {
...
export function getGS38RequirementsForActivity(activity: string): GS38Requirement[] {
  return GS38_CORE_REQUIREMENTS.filter(
    (req) => req.appliesTo?.includes(activity) || req.appliesTo?.includes('all_testing')
  );
}
