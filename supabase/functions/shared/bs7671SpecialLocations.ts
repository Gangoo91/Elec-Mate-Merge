  ...
  zones: [
    {
      ...
      dimensions:
        'Inside bath/shower basin. For showers without basin: 0.10m height, same horizontal extent as Zone 1',
      restrictions: [
    ...
    },
    {
      ...
      dimensions:
        'From floor to 2.25m or highest shower head (whichever higher). Vertical surface around bath/shower basin, or 1.20m from shower outlet for walk-in showers',
      restrictions: [
    ...
    },
    {
      ...
      dimensions:
        'From floor to 2.25m or highest shower head (whichever higher). 0.60m horizontal from Zone 1 boundary',
      restrictions: [
  ...
  rcdRequirements: [
    {
      ...
      reason:
        'ALL circuits serving the location AND circuits passing through zones 1/2 not serving the location',
    },
  ],
  equipmentRestrictions: [
    {
      zone: 'Zone 0',
      restriction:
        'ONLY SELV ≤12V AC / 30V DC ripple-free, IPX7 rated, fixed & permanently connected',
    },
    {
      zone: 'Zone 1',
      restriction:
        'ONLY specific fixed equipment (showers, pumps, SELV/PELV ≤25V AC/60V DC, ventilation, towel rails, water heaters, luminaires). IPX4 minimum.',
    },
    {
      zone: 'Zone 2',
      restriction:
        'NO socket-outlets except SELV (source outside zones) or shaver supply units. IPX4 minimum. Switches only for SELV or shaver units.',
    },
    {
      zone: 'Outside zones',
      restriction:
        'Socket-outlets prohibited within 2.5m horizontally from Zone 1 boundary (except SELV sockets & shaver units)',
    },
  ],
  ...
  rcdRequirements: [
    {
      ...
      reason:
        'DC fault protection required for EV charging (EVs contain DC components). Type B RCD OR Type A/F + RDC-DD device.',
    },
  ],
  equipmentRestrictions: [
    {
      ...
      restriction:
        'Each charging point MUST be individually protected by 30mA RCD (Type A, Type F, or Type B)',
    },
    {
      zone: 'Socket-outlets',
      restriction:
        'One socket-outlet or vehicle connector per EV only. BS 1363-2 marked "EV" OR BS EN 60309-2 (interlocked) OR BS EN 62196-2 Type 1/2/3 for Mode 3 charging',
    },
    {
      zone: 'Outdoor installations',
      restriction:
        'Minimum IP4X (solid bodies) AND IPX4 (water) protection. IK08 impact protection in public areas/car parks (Reg 722.512.2)',
    },
  ],
...
export const SAFE_ZONES_522_6: SafeZoneRequirement = {
  ...
  description:
    'Cables in walls/partitions at depth <50mm MUST be in safe zones OR have additional protection',
  horizontalZones: [
  ...
  depthRequirement:
    'If cable depth <50mm from surface: MUST be in safe zone OR comply with Reg 522.6.204',
  ...
  rcdRequirement:
    '522.6.202: If cable in safe zone but depth <50mm, 30mA RCD protection REQUIRED (unless alternative protection 522.6.204 used)',
};
...
export const METALLIC_WALL_CABLES_522_6_203: SafeZoneRequirement = {
  ...
  description:
    'Cables in walls/partitions with internal metallic parts (NOT just nails/screws) MUST have RCD protection OR alternative protection',
  ...
  depthRequirement:
    'IRRESPECTIVE of buried depth - applies to ANY depth if metallic parts in wall construction',
  alternativeProtection: [
    '30mA RCD protection (Reg 415.1.1) - MANDATORY',
    'OR comply with Reg 522.6.204 alternative protection methods',
  ],
  rcdRequirement:
    '522.6.203(i): MANDATORY 30mA RCD if wall contains metallic parts (stud walls, metal framework, etc.) - UNLESS alternative protection per 522.6.204',
};

/**
 * Get special location requirements by name or section
 */
export function getSpecialLocationRequirements(
  locationQuery: string
): SpecialLocationRequirement | null {
...
export function checkSafeZoneCompliance(params: {
  ...
  if (params.cableDepth < 50) {
    ...
    if (!params.inSafeZone && !params.hasAlternativeProtection) {
      issues.push(
        'Cable depth <50mm and NOT in safe zone - MUST comply with Reg 522.6.204 OR have 30mA RCD'
      );
      rcdRequired = true;
      compliant = false;
    }
    ...
    if (params.inSafeZone && !params.hasAlternativeProtection) {
      ...
      recommendations.push(
        'Cable in safe zone but depth <50mm - 30mA RCD protection required per Reg 522.6.202'
      );
    }
  }
