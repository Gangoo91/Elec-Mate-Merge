...
export const SELECTIVITY_GUIDELINES: SelectivityRequirement[] = [
  {
    ...
    requirement:
      'Time/current curves must NOT overlap - upstream device MUST NOT trip before downstream',
  ...
  },
  {
    ...
    requirement:
      'Upstream RCD must be time-delayed OR have higher IΔn to allow downstream to trip first',
  ...
  },
  {
    ...
    requirement:
      'Fuse let-through energy (I²t) must be less than MCB withstand at all fault current levels',
...
export const LUMINAIRE_CONNECTION_METHODS = [
  ...
  {
    method: 'Luminaire Supporting Coupler (LSC)',
    standard: 'BS 6972 / BS 7001',
    voltagelimit: 'N/A',
    regulation: '559.5.1(ii)',
  },
  {
    method: 'Batten lampholder',
    standard: 'BS EN 60598',
    voltagelimit: 'N/A',
    regulation: '559.5.1(iii)',
  },
  {
    method: 'Luminaire complying',
    standard: 'BS EN 60598',
    voltagelimit: 'N/A',
    regulation: '559.5.1(iv)',
  },
  {
    method: 'Socket-outlet',
    standard: 'BS 1363-2 / BS 546 / BS EN 60309-2',
    voltagelimit: 'N/A',
    regulation: '559.5.1(v)',
  },
  {
    method: 'Device for Connecting Luminaire (DCL)',
    standard: 'BS EN 61995-1',
    voltagelimit: 'N/A',
    regulation: '559.5.1(ix)',
  },
];
...
export const LAMPHOLDER_POLARITY_REQUIREMENT = {
  systems: ['TN', 'TT'],
  requirement:
    'Edison screw/single centre bayonet OUTER CONTACT must be connected to NEUTRAL conductor',
...
export const POLYPHASE_LUMINAIRE_GROUPS = {
  requirement:
    'Groups divided between line conductors with ONE common neutral MUST have device to disconnect ALL lines simultaneously',
...
export function getMCBDisconnectionTime(
  curveType: 'B' | 'C' | 'D',
  faultCurrent: number,
  ratedCurrent: number
): number {
...
export function validateLampholderCircuit(
  lampholderType: string,
  ocpdRating: number
): {
  compliant: boolean;
  issue?: string;
