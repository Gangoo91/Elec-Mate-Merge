...
export const isolationRequirements: IsolationRequirement[] = [
  {
    ...
    examples: [
      ...
      'Isolator with lockable handle',
      'Service head fuse (DNO)',
    ],
  },
  {
...
export const mechanicalMaintenanceSwitching: SwitchingDeviceSpec[] = [
  ...
  },
  {
    ...
    poles: 'All poles',
    operationalRequirements: ['Rated ≤16A', 'Readily accessible', 'Suitable for load'],
...
// Utility Functions
export function getSPDRequirementForInstallation(
  type: string,
  earthingSystemLength?: number
): SPDRequirement | null {
  if (type === 'residential') {
  ...
  if (type === 'commercial' || type === 'industrial') {
    if (earthingSystemLength && earthingSystemLength > 10) {
      return (
        spdRequirements.find((req) => req.installationType.includes('Total earthing system')) ||
        null
      );
    }
  }
  ...
  if (type === 'sensitive-equipment') {
    return (
      spdRequirements.find((req) => req.installationType.includes('Sensitive equipment')) || null
    );
  }
...
export function validateSPDInstallation(params: {
  ...
  if (params.connectingCableLength > 0.5) {
    issues.push(
      `Connecting cable length ${params.connectingCableLength}m exceeds 0.5m maximum (Reg 534.4.6)`
    );
  ...
  if (params.earthingImpedance > 0.1) {
    issues.push(
      `Earthing impedance ${params.earthingImpedance}Ω too high for effective SPD operation`
    );
