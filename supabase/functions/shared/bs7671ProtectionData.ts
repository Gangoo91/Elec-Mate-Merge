 ...
 */
export function checkRCDRequirement(circuitType: string, location?: string): RCDRequirement[] {
  ...
  if (
    circuitType.toLowerCase().includes('concealed') ||
    circuitType.toLowerCase().includes('buried in wall')
  ) {
...
export function getMaxZsForDevice(
  ...
  const match = table.find(
    (z) => z.deviceRating === deviceRating && z.disconnectionTime === disconnectionTime
  );
