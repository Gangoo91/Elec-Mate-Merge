...
If in doubt about whether an article is UK-specific, DO NOT include it. We only want confirmed UK content.`,
      ...
      try {
        ...
        const response = await withRetry(
          () => withTimeout(fetch(url, options), Timeouts.LONG, 'Firecrawl batch job creation'),
          RetryPresets.STANDARD
        );

        if (!response.ok) {
          const errorText = await response.text();
          logger.error('HTTP error creating batch job', {
        ...
        if (status.data && Array.isArray(status.data)) {
          ...
          const validateUKRelevance = (article: any): boolean => {
            ...
            const isUKDomain =
              url.includes('.co.uk') || url.includes('.gov.uk') || url.includes('.org.uk');
          ...
          const getSourceFromUrl = (url: string): { category: string; source: string } => {
            ...
            if (urlLower.includes('electricaltimes.co.uk'))
              return { category: 'Industry', source: 'Electrical Times' };
            if (urlLower.includes('professional-electrician.com'))
              return { category: 'Technical', source: 'Professional Electrician' };
            if (urlLower.includes('electricalcontractingnews.com'))
              return { category: 'Safety', source: 'Electrical Contracting News' };
            if (urlLower.includes('constructionenquirer.com'))
              return { category: 'Projects', source: 'Construction Enquirer' };
            if (urlLower.includes('theconstructionindex.co.uk'))
              return { category: 'Projects', source: 'Construction Index' };
            if (urlLower.includes('building.co.uk'))
              return { category: 'Projects', source: 'Building' };
            if (urlLower.includes('bsria.com')) return { category: 'Technical', source: 'BSRIA' };
            if (urlLower.includes('voltimum.co.uk'))
              return { category: 'Industry', source: 'Voltimum UK' };
            ...
            if (
              urlLower.includes('.co.uk') ||
              urlLower.includes('.org.uk') ||
              urlLower.includes('.gov.uk')
            ) {
              const match = urlLower.match(/https?:\/\/(?:www\.)?([^\/\.]+)/);
              const domainName = match
          ...
          for (const result of status.data) {
            if (result.json && Array.isArray(result.json)) {
                  ...
                  return {
                    ...
                    source_category:
                      sourceInfo.category !== 'General'
                        ? sourceInfo.category
                        : baseSourceInfo.category,
                    source_name:
                      sourceInfo.source !== 'Industry News'
                        ? sourceInfo.source
              ...
              logger.info(`Processed articles from ${baseSourceInfo.source}`, {
    ...
    const transformedArticles = await Promise.all(
      articles.map(async (article) => {
        ...
        return {
          ...
          summary: (article.description || article.excerpt || 'No summary available').substring(
            0,
            500
          ),
          content:
            article.description || article.excerpt || article.title || 'No content available',
          category: article.source_category || 'General',
          source_name: article.source_name || 'Unknown',
          regulatory_body:
            article.source_category === 'BS7671'
              ? 'IET'
              : article.source_category === 'Safety'
                ? 'HSE'
                : 'Industry',
    ...
    const existingHashes = await supabase
      ...
      .in(
        'content_hash',
        transformedArticles.map((a) => a.content_hash)
      );

    const existingHashSet = new Set(existingHashes.data?.map((item) => item.content_hash) || []);
    ...
    logger.error('Firecrawl news scraping error', {
      error: error instanceof Error ? error.message : String(error),
    });
