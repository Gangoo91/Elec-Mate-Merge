...
Include all safety controls, PPE requirements, and emergency procedures.`;
    ...
    // ✅ PHASE 1.5: Intelligent hazard-to-step linking validation
    const installationSteps = previousAgentOutputs?.find((o: any) => o.agent === 'installer')?.response?.data?.steps || [];
    
    if (installationSteps && installationSteps.length > 0) {
      logger.info(`🔗 Validating hazard-to-step linking against ${installationSteps.length} installation steps`);
      
      // Helper: Validate and fix hazard linking
      function validateAndFixHazardLinking(hazards: any[], steps: any[]): any[] {
        return hazards.map(hazard => {
          const hazardText = (hazard.hazard + ' ' + (hazard.controlMeasure || '')).toLowerCase();
          
          // Find matching step by keyword analysis
          let bestMatchStep = 0; // Default to general
          ...
          steps.forEach((step: any, idx: number) => {
            ...
            // Direct activity matches
            if (hazardText.includes('isolation') && stepText.includes('isolation')) score += 10;
            if (hazardText.includes('height') && stepText.includes('height')) score += 10;
            if (hazardText.includes('lifting') && stepText.includes('lift')) score += 8;
            if (hazardText.includes('testing') && stepText.includes('test')) score += 10;
            if (hazardText.includes('drilling') && stepText.includes('drill')) score += 10;
            
            // Equipment matches
            if (hazardText.includes('mewp') && stepText.includes('mewp')) score += 15;
            if (hazardText.includes('ladder') && stepText.includes('ladder')) score += 15;
            if (hazardText.includes('scaffold') && stepText.includes('scaffold')) score += 15;
            
            // Environment matches
            if (hazardText.includes('confined space') && stepText.includes('enclosure')) score += 12;
            if (hazardText.includes('overhead') && stepText.includes('ceiling')) score += 8;
            
            if (score > bestMatchScore) {
              bestMatchScore = score;
              bestMatchStep = idx + 1; // Step numbers are 1-indexed
            }
          });
          
          // If AI linked to wrong step and we found better match, override
          if (bestMatchScore >= 8 && hazard.linkedToStep !== bestMatchStep) {
            logger.info(`🔄 Fixed hazard linking: "${hazard.hazard.substring(0, 40)}..." from step ${hazard.linkedToStep} → ${bestMatchStep}`);
            hazard.linkedToStep = bestMatchStep;
          }
      ...
      // Apply validation
      const beforeLinkingCount = extractedHazards.length;
      const validatedHazards = validateAndFixHazardLinking(extractedHazards, installationSteps);
      
      logger.info(`✅ Validated hazard-to-step linking (${validatedHazards.length} hazards processed)`);
      
      // Replace extractedHazards with validated version
      extractedHazards.length = 0;
      extractedHazards.push(...validatedHazards);
    }
    
    // ✅ PHASE 1.6: Filter irrelevant hazards
    const beforeFilterCount = extractedHazards.length;
    
    // Helper: Filter irrelevant hazards
    function filterIrrelevantHazards(hazards: any[], jobDescription: string, steps: any[]): any[] {
      const jobText = jobDescription.toLowerCase();
      const stepsText = steps.map((s: any) => (s.title || '') + ' ' + (s.description || '')).join(' ').toLowerCase();
      const combinedContext = jobText + ' ' + stepsText;
      
      return hazards.filter(hazard => {
        const hazardText = hazard.hazard.toLowerCase();
        
        // Remove generic "office hazards" for field work
        if (hazardText.includes('office') && !combinedContext.includes('office')) {
          logger.info(`❌ Removed irrelevant hazard: ${hazard.hazard.substring(0, 50)}...`);
          return false;
        }
        
        // Remove height hazards if no height work mentioned
        if (hazardText.match(/height|ladder|scaffold|mewp/) && 
            !combinedContext.match(/height|overhead|ceiling|ladder|scaffold|mewp|elevated/)) {
          logger.info(`❌ Removed irrelevant height hazard: ${hazard.hazard.substring(0, 50)}...`);
          return false;
        }
        
        // Remove confined space hazards if not applicable
        if (hazardText.includes('confined space') && 
            !combinedContext.match(/confined|enclosure|vault|chamber|basement|roof void/)) {
          logger.info(`❌ Removed irrelevant confined space hazard: ${hazard.hazard.substring(0, 50)}...`);
          return false;
        }
        
        // Remove asbestos hazards for new builds
        if (hazardText.includes('asbestos') && 
            combinedContext.match(/new build|new construction|2020|2021|2022|2023|2024/)) {
          logger.info(`❌ Removed asbestos hazard (new build): ${hazard.hazard.substring(0, 50)}...`);
          return false;
        }
        
        // Keep all other hazards
        return true;
      });
    }
    
    const filteredHazards = filterIrrelevantHazards(extractedHazards, effectiveQuery, installationSteps || []);
    logger.info(`✅ Filtered hazards: ${beforeFilterCount} → ${filteredHazards.length} (removed ${beforeFilterCount - filteredHazards.length} irrelevant)`);
    
    // Replace extractedHazards with filtered version
    extractedHazards.length = 0;
    extractedHazards.push(...filteredHazards);
