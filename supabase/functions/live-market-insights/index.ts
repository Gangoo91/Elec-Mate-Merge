...
const getFallbackMarketData = (): MarketInsights => {
  ...
  return {
    ...
    regionalData: {
      london: {
        minSalary: Math.round(38000 * (1 + dailyVariation)),
        maxSalary: Math.round(85000 * (1 + dailyVariation)),
        demandLevel: 'Very High',
      },
      manchester: {
        minSalary: Math.round(32000 * (1 + dailyVariation)),
        maxSalary: Math.round(72000 * (1 + dailyVariation)),
        demandLevel: 'High',
      },
      birmingham: {
        minSalary: Math.round(30000 * (1 + dailyVariation)),
        maxSalary: Math.round(68000 * (1 + dailyVariation)),
        demandLevel: 'High',
      },
      glasgow: {
        minSalary: Math.round(29000 * (1 + dailyVariation)),
        maxSalary: Math.round(65000 * (1 + dailyVariation)),
...
serve(async (req) => {
  ...
  try {
    ...
    if (!forceRefresh) {
      ...
      const { data: cached } = await logger.time('Cache lookup', () =>
        withTimeout(
          supabase
    ...
    const marketData = await logger.time('Market data aggregation', () =>
      withRetry(() => scrapeMarketData(), RetryPresets.STANDARD)
    );
    ...
    await withTimeout(
      supabase.from('market_insights_cache').upsert({
    ...
    logger.error('Market insights error', {
