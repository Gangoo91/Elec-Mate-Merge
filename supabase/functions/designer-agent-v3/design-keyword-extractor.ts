...
export function extractDesignKeywords(
  ...
  Object.values(CALCULATION_KEYWORDS).forEach((group) => group.forEach((kw) => keywords.add(kw)));
  ...
  Object.values(EARTHING_KEYWORDS).forEach((group) => group.forEach((kw) => keywords.add(kw)));

  // Add diversity keywords
  Object.values(DIVERSITY_KEYWORDS).forEach((group) => group.forEach((kw) => keywords.add(kw)));
  ...
  if (installationType === 'domestic') {
    Object.values(DOMESTIC_KEYWORDS).forEach((group) => group.forEach((kw) => keywords.add(kw)));
    ...
    Object.values(COMMERCIAL_KEYWORDS).forEach((group) => group.forEach((kw) => keywords.add(kw)));
    ...
    Object.values(INDUSTRIAL_KEYWORDS).forEach((group) => group.forEach((kw) => keywords.add(kw)));
  ...
  if (supply.phases === 'three' || supply.phases === 3) {
    Object.values(THREE_PHASE_KEYWORDS).forEach((group) => group.forEach((kw) => keywords.add(kw)));
  }
  ...
  [
    ...
    '185mm²',
    '240mm²',
  ].forEach((size) => cableSizes.add(size));
