...
export class FormNormalizer {
  normalize(rawInput: any): NormalizedInputs {
    ...
    return {
      ...
      circuits: rawInput.circuits.map((c: any, idx: number) => this.normalizeCircuit(c, idx)),
    };
  }
  ...
  private normalizeSupply(supply: any, rawInput: any): NormalizedSupply {
    ...
    this.installationType =
      supply.installationType || rawInput.projectInfo?.installationType || 'domestic';
  ...
  private normalizeCircuit(circuit: any, index: number): NormalizedCircuit {
    ...
    const cableLength =
      typeof circuit.cableLength === 'number' && circuit.cableLength > 0 ? circuit.cableLength : 25; // Default 25m for voltage drop estimation
    ...
    if (
      !calculatedIb &&
      (loadTypeLower.includes('motor') ||
        loadTypeLower.includes('pump') ||
        loadTypeLower.includes('compressor'))
    ) {
      ...
      console.log(
        `Motor circuit FLC pre-calculated: ${calculatedIb.toFixed(2)}A for ${circuit.loadPower}W motor`
      );
    }
  ...
  private detectRingFinal(circuit: any): boolean {
    ...
    if (['commercial', 'industrial'].includes(this.installationType.toLowerCase())) {
      ...
      if (circuit.loadPower > 7360) {
        console.log(
          `High-power load detected: ${circuit.name} (${circuit.loadPower}W) - MUST be RADIAL (exceeds 32A ring limit)`
        );
        return false;
      }
      ...
      if (
        dedicatedIndicators.some((ind) => nameLower.includes(ind) || loadTypeLower.includes(ind))
      ) {
      ...
      if (circuit.loadPower < 3680) {
        console.log(
          `Low-load commercial circuit: ${circuit.name} (${circuit.loadPower}W) - using RADIAL`
        );
        return false;
      }
      ...
      if (
        ...
        circuit.loadPower >= 3680 &&
        generalIndicators.some((ind) => nameLower.includes(ind) || loadTypeLower.includes(ind))
      ) {
        console.log(
          `General-purpose commercial sockets: ${circuit.name} (${circuit.loadPower}W) - using RING`
        );
        return true;
      }
