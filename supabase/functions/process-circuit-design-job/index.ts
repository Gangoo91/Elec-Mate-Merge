...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const designTask = supabase.functions
      .invoke('process-circuit-design-parallel', {
        body: {
          jobId: jobId,
        },
      })
    ...
    const watchdogDelay = setTimeout(async () => {
      ...
      const hasProgress =
        (currentJob?.designer_progress || 0) > 5 ||
        (currentJob?.installer_progress || 0) > 5 ||
      ...
      if (currentJob && currentJob.status === 'pending' && !hasProgress) {
        ...
        await supabase
          ...
          .update({
            status: 'failed',
            error_message:
              'Design agents failed to start. This may be a temporary platform issue. Please try generating the design again.',
    ...
    EdgeRuntime.waitUntil(designTask.finally(() => clearTimeout(watchdogDelay)));
    ...
    await captureException(error, {
      functionName: 'process-circuit-design-job',
      requestUrl: req.url,
