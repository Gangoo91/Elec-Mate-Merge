  ...
  try {
    ...
    for (let i = 0; i < lines.length; i += chunkSize) {
      ...
      if (content.toLowerCase().includes('motor') && content.toLowerCase().includes('control')) {
        ...
        content.toLowerCase().includes('substation')
      ) {
        topic = 'Transformers & Substations';
        keywords.push('transformer', 'substation', 'HV');
      } else if (
        content.toLowerCase().includes('power factor') ||
        content.toLowerCase().includes('harmonics')
      ) {
        topic = 'Power Quality';
    ...
    await captureException(error, { functionName: 'parse-city-guilds-book-2', requestUrl: req.url, requestMethod: req.method });
