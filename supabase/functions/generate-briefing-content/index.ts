        ...
        parameters: {
          ...
          properties: {
            ...
            additionalInfo: {
              type: 'array',
              description:
                'MANDATORY: Additional critical information (permits, post-work procedures, disposal requirements, documentation, contact details) as structured paragraphs. MUST include at least 2-4 relevant items.',
              items: {
                type: 'object',
                properties: {
                  paragraph: { type: 'number' },
                  ...
                  type: {
                    ...
                    description:
                      'detail=specific facts/procedures, note=reminders/observations, context=background info, reference=permits/documents/contacts',
                  },
                },
                required: ['paragraph', 'content', 'type'],
              },
        ...
        parameters: {
          ...
          properties: {
            ...
            additionalInfo: {
              type: 'array',
              description:
                'MANDATORY: Additional relevant information (documentation requirements, contacts, references, next steps) as structured paragraphs. MUST include at least 2-4 relevant items.',
              items: {
                type: 'object',
                properties: {
                  paragraph: { type: 'number' },
                  ...
                  type: {
                    ...
                    enum: ['detail', 'note', 'context', 'reference'],
                    description:
                      'detail=factual info, note=observations, context=background, reference=links to docs/contacts',
                  },
                },
                required: ['paragraph', 'content', 'type'],
              },
      ...
      parameters: {
        ...
        properties: {
          ...
          additionalInfo: {
            type: 'array',
            description:
              'MANDATORY: Additional relevant information as structured paragraphs (contacts, references, next steps, documentation). MUST include at least 2-4 relevant items.',
            items: {
              type: 'object',
              properties: {
                paragraph: { type: 'number' },
                ...
                type: {
                  ...
                  enum: ['detail', 'note', 'context', 'reference'],
                  description:
                    'Information category: detail=factual info, note=observations, context=background, reference=links to docs/contacts',
                },
              },
              required: ['paragraph', 'content', 'type'],
            },
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      '[BRIEFING-AI] Generating content for:',
      briefingContext.briefingTitle,
      'Type:',
      briefingType
    );
...
${briefingType !== 'site-work' ? 'This is NOT an electrical safety briefing - adjust content accordingly.' : ''}`;
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to generate briefing content' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!toolCall) {
      console.error(
        '[BRIEFING-AI] No tool call in response. Full response:',
    ...
    if (!briefingContent.additionalInfo || briefingContent.additionalInfo.length === 0) {
      console.warn(
        '[BRIEFING-AI] ⚠️ WARNING: AI did not generate additionalInfo - adding default fallback'
      );
      briefingContent.additionalInfo = [
        {
          paragraph: 1,
          content:
            'Please consult with your supervisor or safety officer for additional site-specific requirements.',
          type: 'note',
        },
        {
          paragraph: 2,
          content:
            'Ensure all relevant documentation and permits are in place before commencing work.',
          type: 'reference',
        },
      ];
    }
