...
async function getFurtherEducationDetails(): Promise<FirecrawlCourse[]> {
  ...
  do {
    ...
    console.log(
      `📈 Status: ${status.status} - Progress: ${status.completed || 0}/${status.total || 0}`
    );
...
function transformFirecrawlToEducationData(
  firecrawlCourses: FirecrawlCourse[]
): LiveEducationData[] {
  return firecrawlCourses.map((course, index) => {
    const rating = course.rating
      ? parseFloat(course.rating.replace(/[^0-9.]/g, '')) || 0
      : Math.random() * 2 + 3;
    ...
    else if (text.includes('renewable') || text.includes('solar') || text.includes('wind'))
      category = 'Renewable Energy';
    else if (text.includes('domestic') || text.includes('residential'))
      category = 'Domestic Installation';
    else if (text.includes('commercial') || text.includes('industrial'))
      category = 'Commercial Installation';
    else if (text.includes('safety') || text.includes('health')) category = 'Health & Safety';
    else if (text.includes('management') || text.includes('supervisor'))
      category = 'Management & Leadership';

    // Determine level
    let level = 'Level 3';
    if (text.includes('level 1') || text.includes('basic') || text.includes('introduction'))
      level = 'Level 1';
    else if (text.includes('level 2') || text.includes('intermediate')) level = 'Level 2';
    else if (text.includes('level 4') || text.includes('advanced') || text.includes('higher'))
      level = 'Level 4';
    ...
    return {
      ...
      keyTopics: course.tags
        ? course.tags.split(',').map((t) => t.trim())
        : ['Electrical Systems', 'Safety Procedures'],
...
function generateAnalytics(educationData: LiveEducationData[]) {
  const categories = educationData.reduce(
    (acc, course) => {
...
serve(async (req) => {
  ...
  try {
    ...
    // Cache the data
    const { error: cacheError } = await supabase.from('live_education_cache').upsert(
      {
        category: 'all',
