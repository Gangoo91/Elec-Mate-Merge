...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[FIND-SCRAP-MERCHANTS] Using API key found in secrets: ${MAPS_API_KEY ? 'Yes (key exists)' : 'No'}`
    );
    ...
    if (!geocodeResponse.ok) {
      console.error(
        `[FIND-SCRAP-MERCHANTS] HTTP error when calling Geocoding API: ${geocodeResponse.status}`
      );
      return new Response(
    ...
    if (geocodeData.error_message) {
      console.error(
        `[FIND-SCRAP-MERCHANTS] Geocode API error message: ${geocodeData.error_message}`
      );
    }
    ...
    if (!placesResponse.ok) {
      console.error(
        `[FIND-SCRAP-MERCHANTS] HTTP error when calling Places API: ${placesResponse.status}`
      );
      return new Response(
        JSON.stringify({
        ...
        return new Response(
          JSON.stringify({
            ...
            details: {
              ...
              message:
                errorDetails ||
                'You may need to enable the Places API in your Google Cloud Console',
            },
          }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    ...
    console.log(
      `[FIND-SCRAP-MERCHANTS] Found ${merchants.length} scrap merchants near ${postcode}`
    );
    ...
    return new Response(
      JSON.stringify({
        error: 'Error searching for scrap merchants',
