...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('[generate-ev-charging-pdf] Client details:', JSON.stringify(formData.client_details, null, 2));
    console.log('[generate-ev-charging-pdf] Charger details:', JSON.stringify(formData.charger_details, null, 2));
    console.log('[generate-ev-charging-pdf] Test results:', JSON.stringify(formData.test_results, null, 2));
    console.log('[generate-ev-charging-pdf] Installer:', JSON.stringify(formData.installer, null, 2));
    ...
    console.error('EV Charging PDF generation error:', error);
    await captureException(error, { functionName: 'generate-ev-charging-pdf', requestUrl: req.url, requestMethod: req.method });
    return new Response(
      JSON.stringify({
        success: false,
