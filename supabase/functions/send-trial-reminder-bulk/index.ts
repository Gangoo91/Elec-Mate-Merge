...
Deno.serve(async (req) => {
  ...
  try {
    ...
    } = (await req.json()) as BulkReminderRequest;
    ...
    console.log(
      `Starting bulk email: ${userIds.length} users, batch size ${batchSize}, delay ${batchDelayMs}ms, skipAlreadySent=${skipAlreadySent}`
    );
    ...
    console.log(
      `Skipping ${alreadySentUserIds.size} users (already sent today), sending to ${usersToEmail.length} users`
    );
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      if (batchIndex < totalBatches - 1) {
        console.log(
          `Batch ${batchIndex + 1} complete. Waiting ${batchDelayMs / 1000}s before next batch...`
        );
    ...
    console.log(
      `Bulk email complete: ${successCount} sent, ${failCount} failed, ${skippedCount} skipped`
    );
    ...
    await captureException(error, { functionName: 'send-trial-reminder-bulk', requestUrl: req.url, requestMethod: req.method });
