...
Return valid JSON only.`;
        ...
        const { error: insertError } = await supabase.from('pricing_intelligence').upsert(
          {
            pricing_id: doc.id,
    ...
    console.log(
      `✅ Processed ${processed}/${pricingDocs.length} (${failed} failed, ${skipped} skipped, ${qualityPassed} quality passed)`
    );
...
function validatePricingQuality(enriched: any): boolean {
  return (
    enriched.product_category?.length > 0 &&
    [
      'cables',
      'consumer_units',
      'lighting',
      ...
      'safety_equipment',
      'testing_equipment',
    ].includes(enriched.product_category) &&
