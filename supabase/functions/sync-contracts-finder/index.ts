...
async function getAccessToken(): Promise<string> {
  ...
  const response = await fetch(
    'https://www.contractsfinder.service.gov.uk/Published/Notices/OCDS/token',
    {
      method: 'POST',
      headers: {
...
function extractPostcode(
  parties: OCDSNotice['parties'],
  items: OCDSNotice['tender']['items']
): string | null {
...
function determineCategories(cpvCodes: string[], description: string): string[] {
  ...
  if (
    cpvCodes.some((c) => c.startsWith('45312')) ||
    descLower.includes('fire alarm') ||
    descLower.includes('fire detection')
  ) {
    categories.push('fire_alarm');
  }
  if (
    cpvCodes.some((c) => c.startsWith('45316')) ||
    descLower.includes('emergency light') ||
    descLower.includes('exit sign')
  ) {
    categories.push('emergency_lighting');
  }
  if (
    descLower.includes('eicr') ||
    descLower.includes('electrical inspection') ||
    descLower.includes('periodic test')
  ) {
...
function determineSector(parties: OCDSNotice['parties'], description: string): string {
  ...
  if (
    ...
    descLower.includes('social housing')
  ) {
    return 'housing';
  }
  if (
...
function determineComplexity(value: number | null, description: string): string {
  ...
  if (
    descLower.includes('3 phase') ||
    descLower.includes('three phase') ||
    descLower.includes('high voltage') ||
    descLower.includes('hv ')
  ) {
    return 'complex';
  }
 ...
 */
async function scrapeContractsFinderPublic(
  supabase: any
  ...
  for (const searchTerm of searchTerms) {
    try {
      ...
      const noticeMatches = html.matchAll(
        /<a[^>]*href="\/Notice\/([a-f0-9-]+)"[^>]*>([^<]+)<\/a>/gi
      );
...
function parseContractsFinderNotice(noticeId: string, title: string, html: string): any | null {
  try {
    ...
    const valueMatch = html.match(
      /(?:Value|Budget|Contract value)[:\s]*£?([\d,]+)(?:\s*-\s*£?([\d,]+))?/i
    );
    if (valueMatch) {
    ...
    const deadlineMatch = html.match(
      /(?:Closing|Deadline|Submission)[^:]*[:\s]*(\d{1,2}[\s\/\-]\w+[\s\/\-]\d{4}|\d{4}-\d{2}-\d{2})/i
    );
    if (deadlineMatch) {
      try {
    ...
    for (const release of releases) {
      try {
        ...
        const opportunity = {
          ...
          location_text: [buyer.address?.locality, buyer.address?.region]
            .filter(Boolean)
            .join(', '),
          postcode: postcode,
          lat: coords?.lat || null,
...
function generateMockOpportunities() {
  ...
  const clientTypes = [
    ...
    },
    {
      ...
      names: [
        '{city} NHS Trust',
        '{city} University Hospitals NHS',
        '{city} Community Health NHS',
    ...
    },
    {
      type: 'housing',
      names: [
        '{city} Housing Association',
        'Metropolitan Housing {area}',
        'Sanctuary Housing {region}',
        'Places for People {city}',
      ],
      sector: 'housing',
    },
    {
  ...
  for (const location of locations) {
    ...
    for (let i = 0; i < numOpps; i++) {
      ...
      const clientName = clientType.names[Math.floor(Math.random() * clientType.names.length)]
        ...
        .replace(
          '{region}',
          location.region.replace('_', ' ').replace(/\b\w/g, (l) => l.toUpperCase())
        );
      ...
      opportunities.push({
        ...
        contact_email: `procurement@${clientName
          .toLowerCase()
          .replace(/\s+/g, '')
          .replace(/[^a-z]/g, '')}.gov.uk`,
        estimated_complexity:
          template.complexity[Math.floor(Math.random() * template.complexity.length)],
        status: 'live',
      });
  ...
  console.log(
    `[SYNC] Generated ${opportunities.length} mock opportunities across ${locations.length} UK locations`
  );
