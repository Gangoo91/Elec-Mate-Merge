...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i += chunkSize) {
      ...
      if (
        content.toLowerCase().includes('escape') ||
        content.toLowerCase().includes('evacuation')
      ) {
    ...
    await captureException(error, { functionName: 'parse-emergency-lighting', requestUrl: req.url, requestMethod: req.method });
