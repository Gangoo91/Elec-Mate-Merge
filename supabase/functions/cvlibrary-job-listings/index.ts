...
serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`CV Library API error: ${response.status}`);
    }

    const html = await response.text();
    const jobs = parseCVLibraryJobs(html);
    
    console.log(`Retrieved ${jobs.length} jobs from CV Library`);
...
function parseCVLibraryJobs(html: string) {
  const jobs = [];
  
  try {
    // Extract job listings using regex patterns for CV Library structure
    const jobPattern = /<div[^>]*class="[^"]*job[^"]*"[^>]*>[\s\S]*?<h3[^>]*>[\s\S]*?<a[^>]*href="([^"]*)"[^>]*>([^<]+)<\/a>[\s\S]*?<div[^>]*class="[^"]*company[^"]*"[^>]*>([^<]+)<\/div>[\s\S]*?<div[^>]*class="[^"]*location[^"]*"[^>]*>([^<]+)<\/div>/g;
    ...
    while ((match = jobPattern.exec(html)) !== null && count < 20) {
      ...
      // Generate unique ID
      const jobId = `cvlibrary-${count}-${Date.now()}`;
      ...
      jobs.push({
        ...
        external_url: url.startsWith('/') ? `https://www.cv-library.co.uk${url}` : url,
    ...
    console.error('Error parsing CV Library jobs:', error);
  }
