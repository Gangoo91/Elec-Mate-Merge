    ...
    const insights: AIInsights = {
      ...
      summary: {
        ...
        recommendations:
          ...
          valueAnalysis.recommendations.length +
          purchaseRecommendations.length,
      },
    };

    console.log(
      `✅ Generated AI insights with ${insights.summary.recommendations} recommendations`
    );
...
function generateSmartRecommendations(products: Product[], searchTerm: string): AIRecommendation[] {
  ...
  if (searchTerm.toLowerCase().includes('cable') && products.length > 2) {
    recommendations.push({
      ...
      description:
        'Consider bundling with cable clips, conduit, and accessories for a complete installation.',
      confidence: 0.7,
    });
  }
...
function performValueAnalysis(products: Product[]): {
  recommendations: AIRecommendation[];
  insights: string[];
} {
  const recommendations: AIRecommendation[] = [];
...
function generatePurchaseRecommendations(
  ...
  if (userLocation === 'UK') {
    const tradeFocused = products.filter(
      (p) => p.supplier === 'CEF' || p.supplier.toLowerCase().includes('trade')
    );
