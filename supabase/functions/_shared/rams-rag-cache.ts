/**
 * PHASE 2 - LAYER 2: RAG Result Cache
 * Cache expensive RAG searches (45s → <1s on cache hit)
 *
 * Cache Structure:
 * - Key: Hash of (query + knowledge_base_type)
 * - TTL: 7 days (knowledge bases rarely change)
 * - Storage: Supabase table `rams_rag_cache`
 */
...
function generateCacheKey(query: string, kbType: string): string {
  // Simple hash: first 100 chars + kb type
...
export async function checkRAGCache(params: {
  ...
  query: string;
  knowledgeBaseType: 'health_safety' | 'practical_work' | 'regulations' | 'bs7671';
...
export async function storeRAGCache(params: {
  ...
  try {
    ...
    const { error } = await params.supabase.from('rams_rag_cache').insert({
      cache_key: cacheKey,
