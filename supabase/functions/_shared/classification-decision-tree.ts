...
const C1_ABSOLUTE_TRIGGERS = [
  ...
  {
    keywords: ['incorrect polarity', 'reversed polarity', 'polarity reversed'],
    regulation: '612.6',
  },
  {
    keywords: ['conductive parts live', 'metal case live', 'exposed conductive parts live'],
    regulation: '411.3.1',
  },
  { keywords: ['arcing', 'sparking'], regulation: '526.1' },
  { keywords: ['burning smell', 'charred', 'charring', 'melted'], regulation: '526.1' },
  {
    keywords: ['missing knockout', 'broken enclosure', 'damaged enclosure'],
    regulation: '416.2',
    condition: 'and live parts visible',
  },
];
...
const NEVER_C1_DEFECTS = [
  {
    keywords: ['old wiring colours', 'old cable colours', 'red black cables'],
    correctClassification: 'C3',
    reason: 'Old colours are C3 improvement, not immediate danger',
  },
  {
    keywords: ['no SPD', 'absence of SPD', 'surge protection'],
    correctClassification: 'C3',
    reason: 'SPD absence is C3 per BPG4 page 16',
  },
  {
    keywords: ['poor labelling', 'no circuit chart', 'inadequate labelling'],
    correctClassification: 'C3',
    reason: 'Labelling issues are C3, not dangerous',
  },
  {
    keywords: ['no AFDD', 'absence of AFDD'],
    correctClassification: 'C3',
    reason: 'AFDD is recommended improvement, not required',
  },
  {
    keywords: ['plastic consumer unit'],
    correctClassification: 'C3',
    reason: 'Plastic CU in non-escape route is C3',
  },
];
...
export function validateClassification(
  ...
  if (
    (descLower.includes('zs exceed') || descLower.includes('earth fault loop')) &&
    descLower.includes('exceed')
  ) {
  ...
  if (
    descLower.includes('insulation') &&
    (descLower.includes('less than') || descLower.includes('<'))
  ) {
    if (aiClassification === 'C1' || aiClassification === 'C3') {
      return {
        valid: false,
  ...
  if (
    descLower.includes('cannot verify') ||
    descLower.includes('cannot access') ||
    descLower.includes('concealed')
  ) {
...
export function getClassificationGuidance(defectDescription: string): {
  ...
  if (
    descLower.includes('no earth') ||
    descLower.includes('no RCD') ||
    descLower.includes('exceed')
  ) {
    return {
