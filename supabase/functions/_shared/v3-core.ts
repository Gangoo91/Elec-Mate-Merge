...
export function parseJsonWithRepair(raw: string, logger: Logger, context: string): any {
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON from AI: ${parseError.message}. Repair failed: ${cleaned.substring(0, 100)}...`
    );
  }
}
