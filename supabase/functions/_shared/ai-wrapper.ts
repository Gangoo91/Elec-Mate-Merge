        ...
        async () => {
          ...
          try {
            ...
            if (fetchError.name === 'AbortError') {
              throw new AIError(
                `AI API call timed out after ${timeoutMs}ms`,
                providerName,
                undefined,
                true
              );
          ...
          if (!response.ok) {
            ...
            if (errorText.includes('invalid model') && provider === 'anthropic') {
              console.warn(
                `⚠️ Gateway rejected ${model}, suggest using Anthropic direct API or Gemini`
              );
            }
            ...
            throw new AIError(`AI call failed: ${errorText}`, providerName, response.status, false);
          }
