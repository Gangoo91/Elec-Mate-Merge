...
export async function checkInstallationMethodCache(params: {
  ...
  try {
    ...
    console.log(
      `✅ Installation method CACHE HIT! (${(match.similarity * 100).toFixed(1)}% similar, hits: ${match.hit_count + 1})`
    );
...
export async function storeInstallationMethodCache(params: {
  ...
  try {
    ...
    const { error } = await params.supabase.from('installation_method_cache').insert({
