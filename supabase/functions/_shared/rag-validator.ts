...
function checkCoverage(
  ...
  const regText = regulations
    .map((r) => (r.content + ' ' + r.regulation_number + ' ' + (r.section || '')).toLowerCase())
    .join(' ');

  return {
    hasCableSizing:
      /433\.1|Ib.*In.*Iz|overload protection|current.?carrying capacity|table 4d/i.test(regText),
    hasVoltageDrop: /525|voltage drop|mV\/A\/m|appendix 4 section 6/i.test(regText),
    hasProtection: /411\.|earth fault|automatic disconnection|rcd|protective device|531\./i.test(
      regText
    ),
...
export function generateCoverageReport(validation: ValidationResult): string {
  ...
  lines.push(
    `✓ Installation Method (Reg 521/522): ${coverage.hasInstallationMethod ? '✅' : '❌'}`
  );
