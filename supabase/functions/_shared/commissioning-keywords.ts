...
export const COMMISSIONING_FAULT_KEYWORDS = {
  ...
  causes: [
    ...
    'cross-connection',
    'parallel paths',
    'unintended earth path',
  ],

  diagnostic_tests: [
    'half-split method',
    'split load',
    'IR test each circuit',
    'IR test phase-to-phase',
    'IR test phase-to-earth',
    'disconnect loads',
    'isolate circuits',
    'test at distribution board',
    'test at socket',
    'test at light fitting',
    'dead test',
    'live test',
    'continuity check',
    'lead placement',
  ...
  common_faults: [
    'RCD Type AC on EV charger',
    'missing RCD',
    '30mA missing',
    'socket without RCD',
    'outdoor socket no RCD',
    'borrowed neutral',
    'parallel earth paths',
    'TNS to TNCS upgrade',
    'PME on outbuilding',
    'supplementary bonding missing',
    'plastic pipework',
    'no main bonding',
    'undersized bonding',
    '10mm² required',
...
export function extractCommissioningKeywords(query: string, circuitType?: string): Set<string> {
  ...
  Object.values(COMMISSIONING_TEST_KEYWORDS)
    .flat()
    .forEach((kw) => {
  ...
  Object.values(COMMISSIONING_INSTRUMENT_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_FAULT_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_SAFETY_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_CERTIFICATION_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_LOCATION_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_EARTHING_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_RCD_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_CALCULATION_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
  ...
  Object.values(COMMISSIONING_TROUBLESHOOTING_KEYWORDS)
    .flat()
    .forEach((kw) => {
      if (queryLower.includes(kw.toLowerCase())) {
        keywords.add(kw);
