...
export async function checkPartialCache(params: {
  ...
  try {
    const cacheKey = await generateCacheKey(params.jobInputs, params.agentType, params.openAiKey);
...
export async function storePartialCache(params: {
  ...
  try {
    ...
    const { error } = await params.supabase.from('circuit_partial_cache').insert({
      cache_key: cacheKey,
