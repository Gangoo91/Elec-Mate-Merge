...
export const CABLE_ENCLOSURE_RULES: Record<string, CableEnclosureMapping> = {
  'twin and earth': {
    ...
    notes:
      'Should not be exposed to mechanical damage. Not recommended outdoors - use SWA instead.',
    additionalRequirements: [
      'Domestic installations only',
      'Not suitable for commercial/industrial',
    ],
  },
  SWA: {
    indoorOptions: [
      'clipped direct',
      'on heavy-duty perforated tray',
      'on cable ladder',
      'on cable basket',
    ],
    outdoorOptions: [
      'clipped direct with SWA glands',
      'buried direct with warning tape',
      'on cable tray',
    ],
    enclosureRequired: false,
    ...
    additionalRequirements: [
      'Use cable ladder for grouped large cables (50mm²+)',
      'Heavy-duty tray for industrial runs',
    ],
  },
  'Flexible SWA': {
    ...
    additionalRequirements: [
      'Use flexible glands rated for vibration',
      'Regular inspection for flex fatigue',
    ],
  },
  'LSZH single': {
    indoorOptions: [
      'in steel conduit',
      'in metal trunking (steel/aluminum)',
      'on cable basket',
    ...
    notes:
      'BS 7671 Reg 521.5.1 - singles must be grouped together. Commercial/industrial standard.',
    additionalRequirements: [
      'All phases and neutral must be enclosed together',
      'Steel trunking for fire-rated routes',
      'PVC trunking acceptable for office areas',
    ],
  },
  'PVC single': {
  ...
  'Cat6 data cable': {
    indoorOptions: [
      'on cable basket',
      'in plastic trunking (segregated)',
      'on cable tray (separate from power)',
      'in dado trunking',
    ],
    ...
    notes:
      'Commercial data installations. Must maintain 300mm separation from power or use screened cable.',
    additionalRequirements: [
      'Segregate 300mm from power cables',
      'Use screened (FTP/STP) if close to power',
      'Maintain bend radius (4× cable diameter)',
    ],
  },
  'Cat6a data cable': {
    indoorOptions: [
    ...
    additionalRequirements: [
      'Segregate 300mm from power cables',
      'Use screened (FTP/STP) mandatory for 10Gb',
      'Maintain bend radius (4× cable diameter)',
...
export const ENVIRONMENT_ENCLOSURE_RULES: Record<string, EnvironmentEnclosureRule> = {
  ...
  commercial: {
    ...
    reason:
      'Fire-rated containment systems for commercial buildings with flexibility for modifications',
  },
  industrial: {
    preferredMethods: [
      'cable ladder',
      'heavy-duty perforated tray',
    ...
    reason:
      'Robust heavy-duty containment for harsh industrial environments with large cable loads',
  },
  'data-centre': {
...
export function detectFireEmergencyCircuit(loadType: string, description?: string): string | null {
  ...
  if (combined.includes('smoke') && (combined.includes('detect') || combined.includes('alarm')))
...
export function detectSpecialLocation(description: string, tags?: string[]): string | null {
  ...
  if (combined.includes('outdoor') || combined.includes('external') || combined.includes('outside'))
    return 'outdoor';
...
export function getRecommendedCableTypes(
  ...
  reason: string;
  bsReference?: string;
  priority: 'circuit-type' | 'special-location' | 'environment';
} {
