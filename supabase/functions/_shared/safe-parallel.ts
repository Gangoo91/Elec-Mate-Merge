 ...
 */
export async function safeAll<T>(tasks: ParallelTask<T>[]): Promise<ParallelResult<T>> {
 ...
 */
export async function safeAllStrict<T>(tasks: ParallelTask<T>[]): Promise<T[]> {
  const { successes, failures } = await safeAll(tasks);

  if (failures.length > 0) {
    throw new Error(`${failures.length} task(s) failed: ${failures.map((f) => f.name).join(', ')}`);
  }
