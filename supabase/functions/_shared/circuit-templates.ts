...
export const STANDARD_TEMPLATES: CircuitTemplate[] = [
  {
    ...
    standardDesign: {
      ...
      justifications: {
        cableSize:
          'Standard 2.5mm² + 1.5mm CPC ring final circuit per BS 7671 Appendix 15. Ring topology divides current between two parallel paths, reducing effective cable resistance.',
        protection:
          '32A Type B RCBO with 30mA RCD protection per Regulation 411.3.3. Socket outlets require additional protection by RCD to prevent electric shock.',
...

  {
    ...
    standardDesign: {
      ...
      justifications: {
        cableSize:
          '1.5mm² + 1.0mm CPC suitable for lighting loads up to 1.5kW. Provides adequate current capacity and voltage drop compliance for typical lighting circuits.',
        protection:
          '6A Type B MCB provides overcurrent protection. Rating exceeds design current with appropriate margin for inrush and lamp failures.',
        voltageDrop:
          '3% voltage drop limit for lighting circuits per BS 7671 to prevent visible flickering or dimming, stricter than 5% for power circuits.',
      },
      warnings: [],
    },
  },
...
export function applyTemplate(
  ...
  for (let i = currentIndex; i < upgradeOptions.length; i++) {
    ...
    console.log(
      `🔧 Testing ${testCable.live}mm²+${testCable.cpc}mm: Zs=${calculatedZs.toFixed(3)}Ω vs max ${maxZs}Ω`
    );
    ...
    if (calculatedZs <= maxZs) {
      ...
      if (i > currentIndex) {
        upgraded = true;
        console.log(
          `✅ Upgraded from ${originalCableSize}mm² to ${testCable.live}mm² for Ze=${ze}Ω compliance`
        );
      }
      break;
    }
...
function getR1PlusR2(cableSize: number, cpcSize: number): number {
  const r1Table: Record<number, number> = {
    1.5: 12.1,
    2.5: 7.41,
    4: 4.61,
    6: 3.08,
  ...
  const r2Table: Record<number, number> = {
    1.0: 18.1,
    1.5: 12.1,
    2.5: 7.41,
    4: 4.61,
