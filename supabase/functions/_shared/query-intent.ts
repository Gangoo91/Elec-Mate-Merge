...
export function detectIntent(query: string, conversationHistory: any[] = []): QueryIntent {
  ...
  if (
    /why (not|is|do|does|can't|couldn't|would|should)/i.test(query) ||
    /what makes|what causes|how come/i.test(query)
  ) {
  ...
  if (
    /what (cable|mcb|size|protection|breaker)/i.test(query) ||
    /design|calculate|size|select/i.test(query)
  ) {
  ...
  if (
    (lowerQuery.startsWith('what about') ||
      lowerQuery.startsWith('and ') ||
      lowerQuery.startsWith('but ')) &&
...
function extractAlternatives(query: string): [string, string] {
  const vsMatch = query.match(
    /(.+?)\s+(?:vs|versus|compared to|instead of|rather than)\s+(.+?)(\?|$)/i
  );
  if (vsMatch) {
