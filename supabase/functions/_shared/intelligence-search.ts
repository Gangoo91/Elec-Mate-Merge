...
export async function searchDesignIntelligence(
  ...
  let query = supabase.from('design_knowledge_intelligence').select('*').is('is_archived', false);
...
export async function searchRegulationsIntelligence(
  ...
  const { keywords, appliesTo = [], categories = [], regulationNumbers = [], limit = 20 } = params;
  ...
  // Build ultra-fast query using GIN indexes
  let query = supabase.from('regulations_intelligence').select('*');
...
export async function searchPracticalWorkIntelligence(
  ...
  const { keywords, appliesTo = [], cableSizes = [], activityTypes = [], limit = 25 } = params;
  ...
  // Build ultra-fast query using GIN indexes
  let query = supabase.from('practical_work_intelligence').select('*');
