  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are an expert in electrical regulations and safety. Extract structured metadata. Return valid JSON only.',
          },
          {
            role: 'user',
            content: prompt,
    ...
    try {
      ...
      if (attempt < MAX_RETRIES) {
        ...
        console.warn(
          `⚠️ Parse error for ${itemId}, retry ${attempt}/${MAX_RETRIES} after ${delay}ms`
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        return callOpenAIWithRetry(prompt, apiKey, model, itemId, maxTokens, attempt + 1);
      }
    ...
    if (
      ...
      const delay = 1000 * Math.pow(2, attempt);
      console.warn(
        `⚠️ Retry ${attempt}/${MAX_RETRIES} for ${itemId} after ${delay}ms (${error.message})`
      );
      await new Promise((resolve) => setTimeout(resolve, delay));
      return callOpenAIWithRetry(prompt, apiKey, model, itemId, maxTokens, attempt + 1);
    }
...
export async function processBatchWithErrorHandling(
  ...
  console.log(
    resumeFromId ? `▶️ Resuming from checkpoint at item ${startIndex}` : '🆕 Starting fresh batch'
  );
  ...
  for (let i = startIndex; i < items.length; i++) {
    ...
    try {
      ...
      if ((i + 1) % checkpointInterval === 0 || i === items.length - 1) {
        await saveCheckpoint(supabase, jobId, batchNumber, item.id, i + 1, {
          processed,
          failed,
          skipped,
