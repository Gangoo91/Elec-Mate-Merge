...
export function extractCircuitContext(agentContext: any): string {
  ...
  const circuitName =
    extractValue(response, /Circuit:?\s*(.+?)(?:\n|$)/i) ||
    extractValue(response, /for\s+(?:a|the)\s+(.+?)(?:\n|circuit|installation)/i) ||
    'Unknown Circuit';
  const load =
    extractValue(response, /Load:?\s*(\d+)\s*W/i) ||
    extractValue(response, /(\d+)\s*W\s+load/i) ||
    'Unknown';
  const cable =
    extractValue(response, /Cable:?\s*([\d.]+)\s*mm²/i) ||
    extractValue(response, /([\d.]+)\s*mm²\s+cable/i) ||
    'Unknown';
  const device =
    extractValue(response, /Protection:?\s*(\d+A\s*Type\s*[ABC])/i) ||
    extractValue(response, /(\d+A\s*Type\s*[ABC])\s*(?:MCB|RCBO)/i) ||
    'Unknown MCB';
  const method =
    extractValue(response, /Method:?\s*(clipped|buried|conduit|trunking)/i) ||
