  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      if (bestMatch.finalScore >= 0.8) {
        ...
        console.log(
          `✅ RAMS cache HIT! (similarity: ${(bestMatch.similarity * 100).toFixed(1)}%, combined: ${(bestMatch.finalScore * 100).toFixed(1)}%, hits: ${bestMatch.hit_count + 1})`
        );
...
export async function storeRAMSCache(params: {
  ...
  try {
    ...
    // Store in cache
    const { error } = await params.supabase.from('rams_semantic_cache').insert({
