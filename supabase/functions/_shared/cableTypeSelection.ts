...
export const selectOptimalCableType = (context: CableSelectionContext): CableSelection => {
  const { loadType, location, cableRun, mechanicalProtection, fireProtection, ambientTemp } =
    context;
  ...
  if (fireProtection === 'fire-alarm') {
    ...
    const isActualFireCircuit =
      loadType.toLowerCase().includes('fire') ||
    ...
    if (isActualFireCircuit) {
      return {
        ...
        reason:
          'BS 5839 requires fire-resistant cable for fire alarm and emergency lighting circuits',
  ...
  if (location === 'underground') {
    return {
      cableType: 'swa',
      reason:
        'SWA armoured cable required for underground burial (mechanical protection and moisture resistance)',
      alternatives: [],
    };
  }
  ...
  if (location === 'loft') {
    if (cableRun?.includes('insulation')) {
      return {
        cableType: 'xlpe-twin-earth',
        reason:
          'XLPE cable recommended when cable may contact thermal insulation (better heat dissipation)',
        alternatives: ['pvc-twin-earth'],
      };
    }
    return {
