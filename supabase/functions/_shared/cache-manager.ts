...
export async function invalidateCacheForAmendment(
  ...
  for (const regNumber of regulationNumbers) {
    await supabase.from('rag_cache').delete().ilike('query_text', `%${regNumber}%`);
  }
...
export async function getCacheStats(supabase: SupabaseClient): Promise<{
  ...
  const { data, error } = await supabase.from('rag_cache').select('created_at, cache_hits');
  ...
  const avgAge =
    ...
    (data.length * 1000 * 60 * 60); // Convert to hours
