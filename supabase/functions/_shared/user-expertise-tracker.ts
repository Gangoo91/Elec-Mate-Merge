 ...
 */
export function trackUserExpertise(messages: any[], conversationSummary?: any): UserExpertise {
  ...
  if (score >= 75) {
    level = 'expert';
    adaptationGuidance =
      'Provide concise technical specs with regulation citations. Minimal basic explanations. Focus on advanced considerations and edge cases.';
  } else if (score >= 60) {
    level = 'designer';
    adaptationGuidance =
      'Provide detailed technical specs with calculation steps. Include regulation references. Explain methodology but assume electrical fundamentals are understood.';
  } else if (score >= 40) {
    level = 'electrician';
    adaptationGuidance =
      'Provide standard technical detail with practical context. Include calculations and regulation backing. Balance technical accuracy with clarity.';
  } else {
    level = 'apprentice';
    adaptationGuidance =
      'Provide educational step-by-step explanations. Explain WHY each decision is made. Include foundational electrical principles. Use analogies where helpful.';
  }
