...
export function deduplicateRegulations(
  ...
  const { similarityThreshold = 0.9, useEmbeddings = false, keepHighestScore = true } = options;
  ...
  for (const reg of regulations) {
    ...
    for (let i = 0; i < unique.length; i++) {
      ...
      if (existing.regulation_number === reg.regulation_number) {
        ...
        const contentSim = textSimilarity(existing.content || '', reg.content || '');

        if (contentSim > similarityThreshold) {
          isDuplicate = true;
          duplicateIndex = i;
          console.log(
            `🔄 Semantic duplicate found: ${reg.regulation_number} (${Math.round(contentSim * 100)}% similar)`
          );
          break;
        }
      }
      ...
      if (useEmbeddings && existing.embedding && reg.embedding) {
        ...
        if (embeddingSim > similarityThreshold) {
          ...
          duplicateIndex = i;
          console.log(
            `🔄 Vector duplicate found: ${reg.regulation_number} vs ${existing.regulation_number} (${Math.round(embeddingSim * 100)}% similar)`
          );
          break;
        }
      }
    ...
    if (isDuplicate && duplicateIndex >= 0) {
      ...
      const existingScore =
        existing.finalScore || existing.crossEncoderScore || existing.hybrid_score || 0;

      if (keepHighestScore && newScore > existingScore) {
        console.log(
          `⬆️ Replacing with higher-scored version: ${newScore.toFixed(2)} > ${existingScore.toFixed(2)}`
        );
  ...
  console.log(
    `✅ Deduplication: ${regulations.length} → ${unique.length} regulations (${regulations.length - unique.length} duplicates removed)`
  );
 ...
 */
export function deduplicateByRegNumber(regulations: RegulationResult[]): RegulationResult[] {
