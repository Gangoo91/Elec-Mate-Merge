  ...
  try {
    ...
    for (let i = 0; i < lines.length; i += chunkSize) {
      ...
      if (
        ...
        content.toLowerCase().includes('hazard identification')
      ) {
        ...
        content.toLowerCase().includes('ppe') ||
        content.toLowerCase().includes('protective equipment')
      ) {
        topic = 'Personal Protective Equipment';
        keywords.push('PPE', 'protective equipment', 'safety gear');
      } else if (
        content.toLowerCase().includes('work at height') ||
        content.toLowerCase().includes('scaffold')
      ) {
        topic = 'Work at Height';
        keywords.push('work at height', 'scaffolding', 'fall protection');
      } else if (
        content.toLowerCase().includes('manual handling') ||
        content.toLowerCase().includes('lifting')
      ) {
