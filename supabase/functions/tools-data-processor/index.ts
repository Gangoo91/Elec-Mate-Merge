...
const generateProductId = (product: ToolProduct): string => {
  ...
  const normalized = `${product.name}-${product.supplier}`
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
...
const detectDuplicates = (products: ToolProduct[]): ToolProduct[] => {
  ...
  for (const product of products) {
    ...
    if (!seen.has(key)) {
      ...
      const hasMoreData =
        (product.description?.length || 0) > (existing.description?.length || 0) ||
...
const batchUpsertProducts = async (supabase: any, products: ToolProduct[]) => {
  ...
  for (let i = 0; i < products.length; i += batchSize) {
    ...
    try {
      const { data, error } = await supabase.from('tools_weekly_cache').upsert(
        batch.map((product) => ({
          ...product,
...
serve(async (req) => {
  ...
  try {
    ...
    if (operation === 'upsert') {
      ...
      const { data, error } = await supabase.from('tools_weekly_cache').insert(
    ...
    const categoryBreakdown = uniqueProducts.reduce(
      (acc, product) => {
    ...
    const supplierBreakdown = uniqueProducts.reduce(
      (acc, product) => {
    ...
    await captureException(error, { functionName: 'tools-data-processor', requestUrl: req.url, requestMethod: req.method });
