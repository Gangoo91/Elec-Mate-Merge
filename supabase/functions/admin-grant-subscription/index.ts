...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'revoke') {
      ...
      console.log(
        `Subscription revoked for ${userId} (${targetProfile?.full_name}) by admin ${user.id} (${callerProfile.full_name})`
      );
      ...
      console.log(
        `Subscription granted (${subscriptionTier}) to ${userId} (${targetProfile?.full_name}) by admin ${user.id} (${callerProfile.full_name})`
      );
    ...
    await captureException(error, { functionName: 'admin-grant-subscription', requestUrl: req.url, requestMethod: req.method });
