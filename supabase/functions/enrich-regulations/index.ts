...
async function processInBackground(
  ...
  console.log(
    `🚀 processInBackground STARTED: jobId=${jobId}, batch=${batchNumber}, startFrom=${startFrom}`
  );
  ...
  try {
    ...
    if (claimedBatch.data?.regulations && Array.isArray(claimedBatch.data.regulations)) {
      ...
      console.log(
        `🎯 COMPLETION MODE (batch list): ${batchList.length} regs → fetched ${regulations?.length || 0}`
      );
      ...
      console.log(
        `🎯 COMPLETION MODE (metadata list): ${metaList.length} regs → fetched ${regulations?.length || 0}`
      );
      ...
      console.log(
        `⚙️ GLOBAL MODE (no filter): fetched ${regulations?.length || 0} regs from range ${startFrom}-${startFrom + batchSize - 1}`
      );
    }
    ...
    console.log(
      `📦 Batch ${claimedBatch.batch_number} (${fetchMode}): ${regulations?.length || 0} regulations`
    );
    ...
    console.log(
      resumeFromId ? `▶️ Resuming from checkpoint at reg ${startIndex}` : '🆕 Starting fresh batch'
    );
    ...
    const heartbeat = setInterval(async () => {
      try {
        const currentReg =
          currentRegIndex < regulations.length
            ? regulations[currentRegIndex].regulation_number
            : 'complete';
        console.log(
          `💓 HEARTBEAT: batch ${claimedBatch.batch_number} - processing ${currentReg} (${processed}/${regulations.length})`
        );
    ...
    for (let i = startIndex; i < regulations.length; i++) {
      ...
      if (
        ...
        reg.regulation_number === 'General' ||
        reg.regulation_number.trim().length === 0
      ) {
    ...
    console.log(
      `✅ Batch ${claimedBatch.batch_number} (${fetchMode}) complete: ${processed} processed, ${failed} failed, ${skipped} skipped, ${newCount} new, ${qualityPassed} quality passed`
    );
    ...
    console.log(
      `✅ Background processing completed: batch ${claimedBatch.batch_number} (${fetchMode})`
    );
  } catch (error) {
...
async function processRegulation(
  ...
  for (const intelligence of intelligenceArray) {
    if (!validateIntelligence(intelligence)) {
      console.log(
        `⚠️ Failed quality check for ${reg.regulation_number} facet - intelligence:`,
        JSON.stringify(intelligence).substring(0, 200)
      );
      qualityFailedCount++;
...
async function extractWithRetry(regulation: any, apiKey: string, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(
        `⚠️ Attempt ${attempt}/${maxRetries} returned empty for ${regulation.regulation_number}`
      );
      ...
      console.error(
        `❌ Attempt ${attempt}/${maxRetries} failed for ${regulation.regulation_number}:`,
        error.message
      );
...
CRITICAL: Generate MULTIPLE facets per regulation to capture all distinct use cases. Return ONLY valid JSON with "records" array. Use UK English spelling throughout.`;
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a strict JSON formatter specialising in UK English. Output ONLY a valid JSON object with "records" array. No markdown, no code fences, no explanations. Use UK English spelling and terminology exclusively.',
            },
  ...
  if (!content || content.trim().length === 0) {
    ...
    console.error(
      '📋 Full API response (first 800 chars):',
    ...
    if (finishReason === 'length') {
      throw new Error(
        'Token budget exceeded - response truncated. Consider reducing max_completion_tokens further.'
      );
    }
  ...
  console.log(
    `✅ GPT-5 response received: ${content.length} chars, finish_reason: ${finishReason}`
  );
  if (data.usage) {
    console.log(
...
function validateIntelligence(intelligence: any): boolean {
  ...
  return (
    intelligence.keywords?.length >= 3 &&
