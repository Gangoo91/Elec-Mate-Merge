...
const INSPECTOR_RESPONSE_SCHEMA = {
  ...
  properties: {
    response: {
      type: 'string',
      description:
        'COMPREHENSIVE inspection guidance (300-400 words) covering: Inspection sequence per BS 7671 Chapter 64, specific test procedures, expected results, pass/fail criteria, certification requirements',
    },
    inspectionProcedure: {
      type: 'object',
      properties: {
...
serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    return new Response(
      JSON.stringify({
        ...
        features: [
          'Phase 5: Complete RAG Integration',
  ...
  try {
    ...
    const { data: testKnowledge, error: testError } = await supabase.rpc(
    ...
    const testContext =
      testKnowledge && testKnowledge.length > 0
        ? testKnowledge.map((test: any) => `[BS 7671] ${test.topic}: ${test.content}`).join('\n\n')
...
Include visual inspection, dead tests, live tests, expected results, and certification requirements.`;
    ...
    const enrichedResponse = enrichResponse(inspectorResult, testKnowledge, 'inspection', {
      circuitType,
      testType,
      voltage,
    });
    ...
    logger.error('Inspector V3 error', {
