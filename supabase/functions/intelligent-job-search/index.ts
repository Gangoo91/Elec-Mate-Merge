...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `📊 Total: ${allJobs.length} jobs, ${uniqueJobs.length} unique from sources: ${sources.join(', ')}`
    );
...
async function searchAdzunaJobs(query: string, location: string, filters: any) {
  ...
  const data = await response.json();
  console.log(
    `📊 Adzuna API returned ${data.count} total results, processing ${data.results?.length || 0} jobs`
  );
