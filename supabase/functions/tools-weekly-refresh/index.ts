...
serve(async (req) => {
  ...
  try {
    ...
    const createTimeoutPromise = (ms: number) =>
      new Promise((_, reject) => setTimeout(() => reject(new Error('Batch scrape timeout')), ms));
    ...
    console.log(
      `📊 Batch Summary: ${successfulBatches.length}/4 succeeded, ${failedBatches.length}/4 failed`
    );
    ...
    if (successfulBatches.length === 0) {
      ...
      throw new Error(
        'All scraping batches failed. Please check network connection and try again.'
      );
    }
      ...
      if (existingCache && existingCache.tools_data) {
        ...
        return new Response(
          JSON.stringify({
            ...
            cacheAge:
              Math.floor(
                (now.getTime() - new Date(existingCache.created_at).getTime()) / 1000 / 60 / 60
              ) + ' hours',
          }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    ...
    console.log(
      overallSuccess
        ? '🎉 Tools weekly refresh completed successfully'
        : '⚠️ Tools refresh completed with issues'
    );
