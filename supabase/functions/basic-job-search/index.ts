...
async function searchReedJobs(query: string, location: string) {
  ...
  try {
    ...
    console.log(
      `📊 Reed API returned ${data.totalResults} total results, processing ${data.results?.length || 0} jobs`
    );
    ...
    if (
      ...
      console.log(
        `Server-side filtering: ${filteredJobs.length}/${jobs.length} jobs match location`
      );
      ...
      if (filteredJobs.length < 5) {
        ...
        console.log(
          `Expanded filtering: ${expandedJobs.length}/${jobs.length} jobs in expanded area`
        );
...
function filterJobsByLocation(jobs: any[], searchLocation: string): any[] {
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    const directMatch =
      jobLocation.includes(normalizedSearch) || normalizedSearch.includes(jobLocation);
    ...
    const hasCommonWord = searchWords.some((searchWord) =>
      jobWords.some(
        (jobWord: string) => jobWord.includes(searchWord) || searchWord.includes(jobWord)
      )
    );
  ...
  console.log(
    `📊 Location filtering result: ${filteredJobs.length}/${jobs.length} jobs match "${searchLocation}"`
  );
  return filteredJobs;
}
...
function filterJobsByLocationExpanded(jobs: any[], searchLocation: string): any[] {
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    if (
      jobLocation.includes('uk') ||
      jobLocation.includes('united kingdom') ||
      jobLocation.includes('england') ||
      jobLocation.includes('scotland') ||
      jobLocation.includes('wales')
    ) {
  ...
  console.log(
    `📊 Expanded filtering result: ${filteredJobs.length}/${jobs.length} jobs in expanded area`
  );
  return filteredJobs;
}
