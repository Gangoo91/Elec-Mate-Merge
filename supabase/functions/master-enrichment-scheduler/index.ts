...
async function fetchAllPracticalWorkIds(
  ...
  while (true) {
    let query = supabase
      .from(table)
      .select(opts?.enriched ? 'practical_work_id' : 'id')
      .range(offset, offset + pageSize - 1);
...
async function fetchAllDesignKnowledgeIds(
  ...
  while (true) {
    let query = supabase
      .from(table)
      .select(opts?.enriched ? 'design_knowledge_id' : 'id')
      .range(offset, offset + pageSize - 1);
...
const ENRICHMENT_TASKS: EnrichmentTask[] = [
  ...
  {
    name: 'Health & Safety Knowledge',
    functionName: 'enrich-health-safety',
  ...
  },
  {
    ...
    functionName: 'enrich-installation-procedures',
    sourceTable: 'installation_knowledge',
    targetTable: 'installation_procedures',
    batchSize: 20,
    priority: 1,
    workerCount: 6,
  },
  ...
  // Phase 2: Specialized Domains (Priority 2)
  {
    ...
    functionName: 'enrich-inspection-procedures',
    sourceTable: 'inspection_testing_knowledge',
    targetTable: 'inspection_procedures',
    batchSize: 30,
    priority: 2,
    workerCount: 6,
  },
  {
    ...
    functionName: 'enrich-maintenance-schedules',
    sourceTable: 'maintenance_knowledge',
  ...
  },
  {
    ...
    functionName: 'enrich-project-templates',
    sourceTable: 'project_mgmt_knowledge',
    targetTable: 'project_templates',
    batchSize: 30,
    priority: 2,
    workerCount: 6,
  },
  ...
  // Phase 3: Pricing Intelligence (Priority 3)
  {
    ...
    functionName: 'enrich-pricing-intelligence',
  ...
  {
    name: 'Practical Work',
    functionName: 'enrich-practical-work',
  ...
  // Phase 5: Design Knowledge Enrichment (Priority 1) - 6-item batches, 200 workers (RAPID MODE), 8 facets/source
  {
    ...
    functionName: 'enrich-design-knowledge',
...
serve(withSentry('master-enrichment-scheduler', async (req) => {
  ...
  try {
    ...
    console.log(
      `🎯 Master Enrichment Scheduler: ${action}${scope === 'single' ? ` (SINGLE: ${jobType})` : ''}${missingRegulations ? ` [${missingRegulations.length} missing regs]` : ''}`
    );
      ...
      for (const job of activeJobs || []) {
        ...
        for (const [batchNum, batches] of batchesByNumber.entries()) {
          if (batches.length > 1) {
            ...
            console.log(
              `✅ Job ${job.job_type}, batch ${batchNum}: Kept ${keep.id}, removed ${duplicates.length} duplicates`
            );
          }
        }
        ...
        if (jobDuplicates > 0) {
          ...
          const completedCount =
            remainingBatches?.filter((b) => b.status === 'completed').length || 0;
          ...
          console.log(
            `📊 Job ${job.job_type}: Recalculated progress to ${progress}% (${completedCount}/${job.total_batches})`
          );
        }
      }

      console.log(
        `✅ Dedupe complete: Removed ${totalDuplicatesRemoved} duplicates across ${jobsAffected} jobs`
      );
    ...
    if (action === 'recover') {
      ...
      if (stuckBatches && stuckBatches.length > 0) {
        ...
        console.log(
          `✅ Recovered ${recovered} stuck batches across ${recoveredJobIds.length} jobs`
        );
        ...
        for (const jobId of recoveredJobIds) {
          const { data: job } = await supabase
            ...
            .single();
          if (job) {
            const task = ENRICHMENT_TASKS.find(
              (t) =>
                `enrich_${t.sourceTable}` === job.job_type ||
                t.functionName === job.job_type.replace('enrich_', 'enrich-')
            );
            if (task) {
        ...
        console.log(
          `🚀 Recover: Restarting continuous worker pool for ${recoveredJobIds.length} jobs`
        );
        ...
        console.log(
          '🔄 No stuck batches found, falling back to continue logic to refresh worker pool'
        );
        ...
        console.log(
          `🚀 Recover (fallback): Refreshing worker pool for ${jobIds.length} active jobs`
        );
    ...
    if (action === 'compute_missing') {
      ...
      try {
        ...
        const uniqueSourceRegs = await fetchAllRegNumbers(supabase, 'bs7671_embeddings', {
          neqGeneral: true,
        });
        const uniqueEnrichedRegs = await fetchAllRegNumbers(supabase, 'regulations_intelligence', {
          enrichmentV1: true,
        });
        ...
        console.log(
          `📊 SOURCE REGS: ${uniqueSourceRegs.length} unique | ENRICHED: ${uniqueEnrichedRegs.length} | MISSING: ${missingRegulations.length}`
        );
    ...
    if (action === 'compute_missing_practical_work') {
      ...
      try {
        const sourceIds = await fetchAllPracticalWorkIds(supabase, 'practical_work', {
          canonicalOnly: true,
        });
        const enrichedIds = await fetchAllPracticalWorkIds(
          supabase,
        ...
        console.log(
          `📊 SOURCE PROCEDURES: ${sourceIds.length} unique | ENRICHED: ${enrichedIds.length} | MISSING: ${missingIds.length}`
        );
    ...
    if (action === 'compute_missing_design_knowledge') {
      ...
      try {
        ...
        const enrichedIds = await fetchAllDesignKnowledgeIds(
          supabase,
        ...
        console.log(
          `📊 SOURCE ITEMS: ${sourceIds.length} unique | ENRICHED: ${enrichedIds.length} | MISSING: ${missingIds.length}`
        );
    ...
    if (action === 'start_missing') {
      ...
      try {
        ...
        for (const job of existingJobs || []) {
          ...
          await supabase.from('batch_progress').delete().eq('job_id', job.id);
        }
        ...
        const { error: batchError } = await supabase.from('batch_progress').insert(batches);
        ...
        EdgeRuntime.waitUntil(continuousProcessor(supabase, [newJob.id], taskMap));
    ...
    if (action === 'start') {
      ...
      if (forceNewJob && regulationNumbers && Array.isArray(regulationNumbers)) {
        console.log(
          `🆕 FORCE NEW JOB: Creating fresh run for ${regulationNumbers.length} regulations`
        );
      ...
      if (ghostLockedJobs && ghostLockedJobs.length > 0) {
        console.log(
          `🔧 Ghost lock detected on ${ghostLockedJobs.length} job(s), auto-resetting...`
        );

        for (const job of ghostLockedJobs) {
          await supabase.from('batch_jobs').update({ status: 'pending' }).eq('id', job.id);
      ...
      console.log(
        `🚀 Starting ${tasksToRun.length} enrichment task${tasksToRun.length !== 1 ? 's' : ''} (scope: ${scope})`
      );
      ...
      if (scope === 'single' && existingJobs && existingJobs.length > 0) {
        ...
        const tasksToCreate = tasksToRun.filter(
          (t) => !existingJobTypes.includes(`enrich_${t.sourceTable}`)
        );
        ...
        for (const task of tasksToCreate) {
          ...
          if (unenrichedCount <= 0) {
            console.log(
              `⏭️ Skipping ${task.name}: All ${totalCount || 0} source records already enriched (${distinctEnriched} distinct sources in ${task.targetTable})`
            );
            continue;
          }

          console.log(
            `📊 ${task.name}: ${unenrichedCount}/${totalCount || 0} source records need enrichment (${distinctEnriched} already enriched)`
          );
          ...
          for (let i = 0; i < totalBatches; i++) {
            const itemsInBatch = Math.min(
              task.batchSize,
              Math.max(0, (unenrichedCount || 0) - i * task.batchSize)
            );
      ...
      if (jobIds.length > 0) {
        ...
        console.log(
          `🔄 No new jobs created, but resuming ${existingJobs.length} existing jobs with worker pool`
        );
        ...
        if (typeof EdgeRuntime !== 'undefined') {
          ...
          console.log(
            '✅ Long-running worker initiated for existing jobs with EdgeRuntime.waitUntil()'
          );
        } else {
          console.warn('⚠️ EdgeRuntime not available, falling back to immediate processing');
    ...
    if (action === 'status') {
      const { data: jobs } = await supabase
        ...
        .in(
          'job_type',
          ENRICHMENT_TASKS.map((t) => `enrich_${t.sourceTable}`)
        )
        .order('created_at', { ascending: false })
        .limit(20);
    ...
    if (action === 'continue') {
      ...
      if (stuckBatchesForRecovery && stuckBatchesForRecovery.length > 0) {
        console.log(
          `🔧 Auto-recovering ${stuckBatchesForRecovery.length} stuck batches (>3 min)...`
        );
        await supabase
          .from('batch_progress')
          .update({ status: 'pending', started_at: null, error_message: null })
          .in(
            'id',
            stuckBatchesForRecovery.map((b: any) => b.id)
          );
      }
...
async function continuousProcessor(
  ...
  while (activeWorkers.has(workerId)) {
    ...
    if (!hasPendingWork) {
      console.log(
        `✅ Worker ${workerId} complete: All ${jobIds.length} jobs finished (${cycles} cycles)`
      );
    ...
    for (const jobId of jobIds) {
        ...
        while (true) {
          try {
            ...
            if (!pending) {
              if (processedCount > 0) {
                console.log(
                  `✅ Batch worker ${batchWorkerId} finished (processed ${processedCount} batches)`
                );
              }
              break;
            }
      ...
      console.log(
        `✅ All ${PARALLEL_WORKERS} workers completed for job ${jobId}, registry cleared`
      );
    }
    ...
    if (cycles % 10 === 0) {
      console.log(
        `💓 Worker ${workerId} heartbeat: Cycle ${cycles}, ${pendingBatches?.length || 0} pending batches`
      );
    }
  }
...
async function waitForBatchCompletion(
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    console.log(
      `⏳ Batch ${batchNumber} still processing... ${elapsed}s elapsed (${processed} items)`
    );
...
async function processNextBatch(supabase: any, jobId: string, task: EnrichmentTask) {
  ...
  if (!batch) {
    ...
    if (!processingBatches || processingBatches.length === 0) {
      ...
      console.log(
        `⏸️ Job ${jobId}: No pending batches, but ${processingBatches.length} still processing`
      );
    }
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      console.warn(
        `⚠️ Invoke attempt ${attempt + 1}/3 failed for ${task.functionName} (retry in ${Math.round(delay)}ms):`,
        response.error
      );
    ...
    const completionStatus = await waitForBatchCompletion(
      supabase,
      batch.id,
      batch.batch_number,
      20
