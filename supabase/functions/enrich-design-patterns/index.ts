...
[{
  ...
  "typical_applications": ["Domestic circuits", "Industrial distribution"]
        ...
        const response = await callOpenAIWithRetry(
          openAIKey,
          extractionPrompt,
          'You are an electrical design expert. Extract structured design patterns and calculations. Return valid JSON only.'
        );
        ...
        for (const pattern of patterns) {
          await supabase.from('design_patterns_structured').upsert(
            {
              source_id: doc.id,
    ...
    console.log(
      `✅ Batch ${batchNumber} complete: ${processed}/${documents.length} (${failed} failed, ${skipped} skipped)`
    );
...
function validateQuality(patterns: any[]): boolean {
  ...
  return (
    (first.calculation_formula || first.example_values) &&
    Object.keys(first.input_parameters || {}).length > 0
  );
}
