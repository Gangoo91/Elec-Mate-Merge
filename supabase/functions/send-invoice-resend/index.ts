...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const downloadPdf = async (url: string): Promise<boolean> => {
      try {
        ...
        if (pdfFileResponse.ok) {
          ...
          // payment/download links so delivery never fails outright.
    ...
    const emailOptions: {
      ...
      // ELE-1731 — this is the button Mark Glowacki presses, and the one his
      // customer says never arrives. Record it so the question is answerable.
      log: { template: 'invoice_send', entityId: invoiceId, userId: invoice?.user_id ?? null },
    };
    ...
    // send failed while carrying attachments, retry once without them — the
    // email still carries the payment/download links.
