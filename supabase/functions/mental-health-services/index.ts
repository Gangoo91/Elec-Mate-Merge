  ...
  try {
    ...
    try {
      ...
      const {
        ...
        error: searchError,
        status,
      } = await searchMentalHealthServices(latitude || 0, longitude || 0);
      if (searchError) {
        throw new Error(searchError);
      }

      return new Response(JSON.stringify({ services, source: 'Google Places API' }), {
...
async function searchMentalHealthServices(
  latitude: number,
  longitude: number
): Promise<{ services?: LocalResource[]; error?: string; status?: string }> {
  try {
    ...
    return {
      error: `Error searching for services: ${error instanceof Error ? error.message : 'Unknown error'}`,
    };
  }
}

// Transform a Google Place result into our LocalResource format
async function transformPlaceToResource(
  place: any,
  userLat: number,
  userLng: number
...
function determineServiceType(place: any): string {
  ...
  if (name.includes('counselling') || name.includes('counseling') || name.includes('therapy'))
    return 'Counselling';
