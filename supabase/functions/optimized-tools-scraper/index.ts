...
const optimizedCategories = [
  {
    ...
    suppliers: [
      {
        name: 'Screwfix',
        url: 'https://www.screwfix.com/search?search=electrical+hand+tools&page_size=50',
      },
    ],
  },
  {
    ...
    suppliers: [
      {
        name: 'Screwfix',
        url: 'https://www.screwfix.com/search?search=electrical+power+tools&page_size=50',
      },
    ],
  },
  {
    ...
    suppliers: [
      {
        name: 'Screwfix',
        url: 'https://www.screwfix.com/search?search=electrical+testers+multimeter&page_size=50',
      },
    ],
  },
  {
    ...
    suppliers: [
      {
        name: 'Screwfix',
        url: 'https://www.screwfix.com/search?search=electrical+safety+equipment+PPE&page_size=50',
      },
    ],
  },
  {
    ...
    suppliers: [
      {
        name: 'Screwfix',
        url: 'https://www.screwfix.com/search?search=electrical+cables+wiring+tools&page_size=50',
      },
    ],
  },
  {
    ...
    suppliers: [
      {
        name: 'Screwfix',
        url: 'https://www.screwfix.com/search?search=ladders+steps+access+equipment&page_size=50',
      },
    ],
  },
  {
    ...
    suppliers: [
      {
        name: 'Screwfix',
        url: 'https://www.screwfix.com/search?search=tool+storage+bags+cases&page_size=50',
      },
    ],
  },
];
...
async function scrapeToolsFromSupplier(
  supplierUrl: string,
  supplierName: string,
  category: string
): Promise<ToolProduct[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `❌ API request failed for ${supplierName}: ${response.status} ${response.statusText}`
      );
    ...
    console.log(
      `✅ Processed ${processedProducts.length} valid ${category} products from ${supplierName}`
    );
...
async function saveToolsToDatabase(tools: ToolProduct[]) {
  ...
  try {
    ...
    const { error } = await supabase.from('materials_weekly_cache').upsert({
