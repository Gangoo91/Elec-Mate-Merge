...
const detectIntent = (
  ...
  if (
    lowerQuery.match(/\b(install|installation|fitting|mount|fix|cable route|conduit|trunking)\b/)
  ) {
  ...
  if (
    lowerQuery.match(
      /\b(test|testing|measure|inspection|verify|commissioning|r1\+r2|zs|ze|insulation)\b/
    )
  ) {
  ...
  if (
    lowerQuery.match(
      /\b(design|calculate|sizing|cable size|rating|diversity|load|circuit|breaker size)\b/
    )
  ) {
...
serve(async (req) => {
  ...
  try {
    ...
    const needsEmbedding = knowledgeBases.some(
      (kb) => kb !== 'bs7671' && ['installation', 'testing', 'design', 'safety'].includes(kb)
    );
    ...
    const searchTasks = [
      {
        name: 'bs7671',
        execute: () =>
          logger.time('BS7671 intelligence search', () =>
            withTimeout(
    ...
    if (queryVector && knowledgeBases.includes('installation')) {
      searchTasks.push({
        name: 'installation',
        execute: () =>
          logger.time('Installation search', () =>
            withTimeout(
