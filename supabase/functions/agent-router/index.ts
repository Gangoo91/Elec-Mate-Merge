...
serve(async (req) => {
  ...
  // Health check
  if (req.method === 'GET') {
    return new Response(JSON.stringify({ status: 'healthy', function: 'agent-router' }), {
  ...
  try {
    ...
    if (!userMessage) {
      return new Response(JSON.stringify({ success: false, error: 'userMessage is required' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400,
      });
    }
    ...
    if (mode === 'method-statement') {
      ...
      if (designerAgent) {
        ...
        const designerResponse = await invokeAgentWithRetry(
          ...
          designerEndpoint,
          {
            query: contextFromPreviousAgent
              ? `${contextFromPreviousAgent}\n\n${userMessage}`
              : userMessage,
      ...
      if (remainingAgents.length > 0) {
        logger.info(
          `🔀 STAGE 2: Running ${remainingAgents.length} agents in parallel with designer context`
        );
        ...
        logger.info(
          `✅ STAGE 2 complete: ${completedResponses.length - (designerAgent ? 1 : 0)}/${remainingAgents.length} agents succeeded`
        );
      }

      agentResponses = completedResponses;
      logger.info(
        `✅ Staged parallel execution complete: ${agentResponses.length}/${selectedAgents.length} total agents succeeded`
      );
    } else {
      ...
      for (const agentType of selectedAgents) {
        ...
        const shouldUseSharedRegs =
          agentType !== 'designer' &&
          agentType !== 'designer-multi' &&
          sharedRegulations.length > 0;
        ...
        if (
          (agentType === 'designer' || agentType === 'designer-multi') &&
          ...
          totalRAGCalls++;
          logger.info(
            `🚀 Cached ${regulations.length} regulations from designer for conversation ${conversationId}`
          );
        } else if (shouldUseSharedRegs && sharedRegulations.length > 0) {
          logger.info(
            `♻️ ${agentType} reused ${sharedRegulations.length} shared regulations (0ms RAG)`
          );
        }
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (attempt < maxRetries) {
            ...
            logger.warn(`${agentType} attempt ${attempt + 1} failed, retrying in ${delay}ms`, {
              error,
            });
    ...
    const uniqueSuggestions = allSuggestions
      .filter(
        (s, index, self) =>
          index === self.findIndex((t) => t.agent === s.agent) && !selectedAgents.includes(s.agent)
      )
      .sort((a, b) => {
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          ragEfficiency:
            totalRAGCalls > 0
              ? `Saved ${selectedAgents.length - totalRAGCalls} RAG calls`
              : undefined,
        },
      }),
      {
