  ...
  try {
    ...
    for (let i = 0; i < lines.length; i += chunkSize) {
      ...
      if (
        ...
        content.toLowerCase().includes('portable appliance')
      ) {
        ...
        content.toLowerCase().includes('earth') ||
        content.toLowerCase().includes('continuity')
      ) {
        topic = 'Earth Continuity Testing';
        keywords.push('earth continuity', 'protective conductor', 'bonding');
      } else if (
        content.toLowerCase().includes('insulation') ||
        ...
        content.toLowerCase().includes('frequency') ||
    ...
    await captureException(error, { functionName: 'parse-inservice-testing', requestUrl: req.url, requestMethod: req.method });
