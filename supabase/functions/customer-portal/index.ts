...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      logStep('No Stripe customer found — user may be on App Store or free access');
      return new Response(
        JSON.stringify({ noStripeCustomer: true }),
    ...
    try {
      ...
      if (portalError?.message && portalError.message.includes('configuration')) {
        logStep('Portal configuration error, attempting direct subscription management', {
    ...
    await captureException(error, { functionName: 'customer-portal', requestUrl: req.url, requestMethod: req.method });
