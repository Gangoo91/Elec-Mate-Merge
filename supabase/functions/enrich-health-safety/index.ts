...
Focus on making this content findable via search. Extract 5-10 keywords and 3-5 search tags.`;
        ...
        console.log(
          `📦 Full OpenAI response structure for ${doc.id}:`,
        ...
        if (!aiData.choices[0].message) {
          console.error(
            `❌ No message in OpenAI choice for ${doc.id}:`,
        ...
        console.log(
          `📋 Raw OpenAI content for ${doc.id} (${content.length} chars):`,
        ...
        try {
          ...
          console.log(
            `✅ Extracted intelligence for ${doc.id}: ${intelligence.document_type} - ${intelligence.primary_topic}`
          );
        } catch (parseError) {
          console.error(
            `❌ JSON parse error for ${doc.id}:`,
            parseError.message,
            content.substring(0, 200)
          );
          failed++;
        ...
        await supabase.from('health_safety_intelligence').upsert(
          {
            source_id: doc.id,
    ...
    console.log(
      `✅ Batch ${batchNumber} complete: ${processed}/${documents.length} (${failed} failed, ${skipped} skipped, ${qualityPassed} quality passed)`
    );
