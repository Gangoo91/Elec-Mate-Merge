...
const updateMarketInsights = async (keywords: string, location: string) => {
  ...
  try {
    ...
    console.log(`Successfully updated market insights for ${keywords} in ${location}`);
    return true;
  } catch (error) {
...
serve(async (req) => {
  ...
  try {
    const { keywords = 'electrician', location = 'UK', isScheduled = false } = await req.json();

    if (isScheduled) {
      // Background scheduler mode - update common search combinations
      const searchCombinations = [
        { keywords: 'electrician', location: 'UK' },
        { keywords: 'electrician', location: 'London' },
        { keywords: 'electrician', location: 'Manchester' },
        { keywords: 'electrical engineer', location: 'UK' },
        { keywords: 'maintenance electrician', location: 'UK' },
      ];
