...
-- Invoices raised from a quote: 'Invoice/NNN'. The S-prefixed standalone
-- numbers share this column, so they are excluded here and counted below.
INSERT INTO public.document_number_counters (user_id, doc_type, last_number)
SELECT
  user_id,
  'invoice',
  COALESCE(MAX(NULLIF((regexp_match(invoice_number, '^Invoice/(\d+)$'))[1], '')::int), 0)
FROM public.quotes
WHERE user_id IS NOT NULL
  AND invoice_number ~ '^Invoice/\d+$'
GROUP BY user_id
ON CONFLICT (user_id, doc_type) DO NOTHING;
...
SELECT
  user_id,
  'standalone_invoice',
  COALESCE(MAX(NULLIF((regexp_match(invoice_number, '^Invoice/S(\d+)$'))[1], '')::int), 0)
FROM public.quotes
WHERE user_id IS NOT NULL
