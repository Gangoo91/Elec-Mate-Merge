-- Drop dependent view and function, then recreate
DROP VIEW IF EXISTS facet_distribution_view CASCADE;
DROP FUNCTION IF EXISTS public.get_facet_distribution_stats() CASCADE;
...
BEGIN
  ...
  stats AS (
    SELECT 
      (SELECT COUNT(*) FROM practical_work_intelligence) as total_facets,
      COUNT(DISTINCT source_id) as total_sources,
      ROUND(AVG(facet_count), 2) as avg_facets,
      MIN(facet_count) as min_facets,
      MAX(facet_count) as max_facets,
      ROUND((COUNT(*) FILTER (WHERE facet_count >= 8)::numeric / NULLIF(COUNT(*), 0)::numeric * 100), 2) as quality_score,
