-- Add joined_worker_id to the manage-employer-seats call on the JOIN path only,
-- so it can retire that worker's own subscription ("seats replace their own
-- sub", cancel_at_period_end). Archive/downgrade calls carry no joined_worker_id
-- → a cancellation can never fire on a removal. Everything else unchanged.
...
declare
  v_service_key text;
  v_joined boolean := false;         -- a worker just JOINED → founder alert + sub replace
      ...
      if v_billing_dirty then
        begin
          perform net.http_post(
            ...
            -- On a JOIN, also hand over the worker so their own sub is retired.
            body := jsonb_build_object('employer_id', new.employer_id)
                    || case when v_joined then jsonb_build_object('joined_worker_id', new.user_id)
                            else '{}'::jsonb end
          );
