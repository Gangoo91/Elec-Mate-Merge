...
-- This will create pricing data for all existing job types with proper job_category mapping
...
job_types_with_categories AS (
  SELECT DISTINCT job_type, job_category FROM regional_job_pricing WHERE job_category IS NOT NULL
),
...
INSERT INTO regional_job_pricing (
  region, job_type, job_category, min_price, max_price, average_price,
...
SELECT 
  ...
  j.job_type,
  j.job_category,
  CAST((200 + (RANDOM() * 100)) * l.multiplier AS numeric(10,2)) as min_price,
  CAST((400 + (RANDOM() * 200)) * l.multiplier AS numeric(10,2)) as max_price,
  CAST((300 + (RANDOM() * 150)) * l.multiplier AS numeric(10,2)) as average_price,
  CASE 
    WHEN RANDOM() < 0.3 THEN 'standard'
    WHEN RANDOM() < 0.7 THEN 'intermediate' 
    ELSE 'complex'
  END as complexity_level,
  CAST(70 + (RANDOM() * 25) AS numeric(4,1)) as confidence_score,
  'market_analysis' as data_source,
  now() as last_updated
FROM location_multipliers l
CROSS JOIN job_types_with_categories j
WHERE NOT EXISTS (
