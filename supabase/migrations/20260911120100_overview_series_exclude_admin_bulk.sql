-- Accounts created by an admin (college tutors, cohorts, demo, beta) are not
-- signups. They were counting as such on the overview: 13 tutor accounts made
-- on 11 Sep 2026 showed up as a spike. Exclude created_via = 'admin_bulk'.
...
begin
  ...
  signups as (
    ...
    from auth.users u
    where coalesce(u.raw_user_meta_data->>'created_via','') <> 'admin_bulk'
      and u.created_at >= (v_from_signups::timestamp at time zone 'Europe/London') - interval '1 day'
    group by 1
  ),
  dau as (
  ...
  yesterday as (
    select
      ...
      (select count(*) from auth.users u, yesterday_bounds b where coalesce(u.raw_user_meta_data->>'created_via','') <> 'admin_bulk' and u.created_at >= b.t0 and u.created_at < b.t1) as signups
  ),
  today as (
    select
      ...
      (select count(*) from auth.users u, today_bounds b where coalesce(u.raw_user_meta_data->>'created_via','') <> 'admin_bulk' and u.created_at >= b.t0 and u.created_at < b.t1) as signups
  )
