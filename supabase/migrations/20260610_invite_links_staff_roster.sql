-- ============================================================================
-- College Hub — staff invites link the roster (mirror of the student spine).
--
-- The staff branch of accept_college_invite only set profiles.college_id/role,
-- leaving the invited staff member with no college_staff roster row — so they
-- didn't appear in the roster and couldn't be assigned. This makes the staff
-- branch link an existing roster row by email (e.g. one a college pre-added with
-- no login), or create one. The trg_sync_staff_profile trigger then mirrors
-- college_id/role onto their profile, so the roster is the single source of truth.
--
-- v_email is now fetched once for both branches.
--
-- Rollback: restore the prior accept_college_invite body from migration history.
-- ============================================================================
...
begin
  ...
  if v_invite.invite_type = 'staff' then
    -- Link an existing roster row by email, else create one. The
    -- trg_sync_staff_profile trigger mirrors college_id/role onto profiles.
    ...
    -- Course (college-native) drives the qualification; fallbacks below.
