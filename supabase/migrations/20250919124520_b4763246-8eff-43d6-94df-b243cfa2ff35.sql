...
INSERT INTO regional_job_pricing (
  region, job_type, min_daily_price, max_daily_price, avg_daily_price,
  complexity_level, confidence_score, data_source, last_updated
)
SELECT 
  l.location_name,
  j.job_type,
  ROUND((200 + (RANDOM() * 100)) * l.multiplier, 2) as min_daily_price,
  ROUND((400 + (RANDOM() * 200)) * l.multiplier, 2) as max_daily_price,
  ROUND((300 + (RANDOM() * 150)) * l.multiplier, 2) as avg_daily_price,
  CASE 
    WHEN RANDOM() < 0.3 THEN 'standard'
    WHEN RANDOM() < 0.7 THEN 'intermediate' 
    ELSE 'complex'
  END as complexity_level,
  ROUND(70 + (RANDOM() * 25), 1) as confidence_score,
