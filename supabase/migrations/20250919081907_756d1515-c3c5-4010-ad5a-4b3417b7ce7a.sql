...
-- Add postcode_district column to existing market_insights_cache if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                  WHERE table_name = 'market_insights_cache' 
                  AND column_name = 'postcode_district') THEN
        ALTER TABLE public.market_insights_cache 
        ADD COLUMN postcode_district TEXT;
    END IF;
END $$;

-- Create indexes for performance
CREATE INDEX IF NOT EXISTS idx_uk_postcode_districts_district_code ON public.uk_postcode_districts(district_code);
CREATE INDEX IF NOT EXISTS idx_uk_postcode_districts_region ON public.uk_postcode_districts(region);
CREATE INDEX IF NOT EXISTS idx_enhanced_regional_pricing_postcode_job ON public.enhanced_regional_pricing(postcode_district, job_type);
CREATE INDEX IF NOT EXISTS idx_enhanced_regional_pricing_expires ON public.enhanced_regional_pricing(expires_at);
CREATE INDEX IF NOT EXISTS idx_community_pricing_postcode ON public.community_pricing_submissions(postcode_district);
CREATE INDEX IF NOT EXISTS idx_market_insights_postcode ON public.market_insights_cache(postcode_district);
