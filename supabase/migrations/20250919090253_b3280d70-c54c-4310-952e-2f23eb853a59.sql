...
WITH job_pricing AS (
  SELECT * FROM (VALUES
    ('domestic_rewire', 'Domestic Rewire', 'Installations', 450, 850, 650),
    ('commercial_installation', 'Commercial Installation', 'Installations', 320, 650, 485),
    ('fault_finding', 'Fault Finding & Repair', 'Maintenance', 280, 420, 350),
    ('consumer_unit_upgrade', 'Consumer Unit Upgrade', 'Installations', 380, 650, 515),
    ('outdoor_lighting', 'Outdoor Lighting Installation', 'Installations', 250, 450, 350),
    ('ev_charger', 'EV Charger Installation', 'Installations', 350, 650, 500),
    ('security_systems', 'Security System Installation', 'Installations', 280, 500, 390),
    ('data_cabling', 'Data Cabling & Networking', 'Installations', 220, 400, 310),
    ('emergency_callout', 'Emergency Call-out', 'Maintenance', 380, 650, 515),
    ('inspection_testing', 'Inspection & Testing', 'Testing', 180, 320, 250),
    ('solar_installation', 'Solar Panel Installation', 'Installations', 400, 750, 575),
    ('heat_pump_electrical', 'Heat Pump Electrical Work', 'Installations', 380, 700, 540),
    ('smart_home', 'Smart Home Systems', 'Installations', 300, 550, 425),
    ('industrial_maintenance', 'Industrial Maintenance', 'Maintenance', 350, 600, 475),
    ('fire_alarm_systems', 'Fire Alarm Systems', 'Installations', 320, 580, 450)
  ) AS t(job_code, job_title, job_category, min_price, max_price, avg_price)
)
...
SELECT 
  ...
  CASE 
    WHEN d.contractor_density > 70 THEN FLOOR(random() * 15) + 80
    WHEN d.contractor_density > 50 THEN FLOOR(random() * 20) + 70
  ...
  -- Complexity level
  CASE 
    WHEN jp.job_code IN ('solar_installation', 'heat_pump_electrical', 'smart_home') THEN 'complex'
    WHEN jp.job_code IN ('domestic_rewire', 'commercial_installation', 'fire_alarm_systems') THEN 'standard'
    ELSE 'basic'
  END,
  ...
  -- Last verified within the past 30 days
  ...
  jsonb_build_object(
    ...
    'competition_level',
    CASE 
      WHEN d.contractor_density > 70 THEN 'high'
      WHEN d.contractor_density > 50 THEN 'medium'
      ELSE 'low'
    END,
    'regional_modifier', (0.75 + (d.avg_household_income / 100000.0) * 0.5 + (d.contractor_density / 100.0) * 0.25)
  ),
  
  -- Raw data JSON
  jsonb_build_object(
    'data_source', 'baseline_2025',
    'location_data', jsonb_build_object(
      'region', d.region,
      'local_authority', d.local_authority,
      'avg_household_income', d.avg_household_income,
      'contractor_density', d.contractor_density,
      'urban_rural', d.urban_rural_classification
    )
  )
...
('EH1 1YZ', 'Scotland', 'City of Edinburgh', 'solar_installation', 650, 'GBP', 'per day', 'complex', 
