...
const JobInsights: React.FC<JobInsightsProps> = ({ jobs, location }) => {
  ...
  const [freshness, setFreshness] = useState<{
    ...
    recent7dPct: number;
    medianDays: number;
  }>({ last48hPct: 0, recent7dPct: 0, medianDays: 0 });
  ...
  const generateInsightsWithLiveData = (marketData: any) => {
    console.log('🎯 Generating insights with live data:', {
      salaryCount: marketData.salaryStats.count,
    });
  ...
  const generateInsights = () => {
    ...
    if (jobs.length > 0) {
      ...
      if (baseSalaries.length > 0) {
        const avgSalary = Math.round(
          baseSalaries.reduce((sum: number, s: number) => sum + s, 0) / baseSalaries.length
        );
        newInsights.push({
          id: 'avg-salary',
          type: 'salary',
    ...
    if (!liveData) {
      ...
      const skillCounts = skillKeywords.reduce(
      ...
      const certCounts = Object.keys(certKeywordsMap).reduce(
      ...
      if (jobs.length > 0) {
        ...
        setJobTypeMix(
          Object.entries(typeCounts).map(([label, count]) => ({ label, count: count as number }))
        );
        ...
        const skillsArr = Object.entries(skillCounts)
          .sort(([, a], [, b]) => (b as number) - (a as number))
          .slice(0, 6)
          .map(([name, count]) => ({
        ...
        const certsArr = Object.entries(certCounts)
          .sort(([, a], [, b]) => (b as number) - (a as number))
          .slice(0, 6)
  ...
  const getCardBg = (type: string) => {
    switch (type) {
      ...
      case 'trend':
        return 'border-elec-yellow/30 bg-elec-yellow/5';
      case 'hotspot':
  ...
  return (
    <div className="space-y-4">
          ...
          >
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                {insight.change && (
                  <div
                    className={`flex items-center text-xs ${insight.change > 0 ? 'text-green-400' : 'text-red-400'}`}
                  >
                    {insight.change > 0 ? (
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div>
                ...
                <p className="font-semibold text-elec-light">
              ...
              <div>
                ...
                <p className="font-semibold text-elec-light">
                  £{salaryStats.q1.toLocaleString()} – £{salaryStats.q3.toLocaleString()}
                </p>
              </div>
              <div>
            ...
            {salaryBuckets.length > 0 && (
              <div className="h-48 text-elec-yellow pb-2">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={salaryBuckets}
                    margin={{ top: 8, right: 8, left: 0, bottom: isMobile ? 24 : 8 }}
                  >
            ...
            <div className="space-y-2">
              {jobTypeMix.map((item) => {
                const total = Math.max(
                  1,
                  jobTypeMix.reduce((s, i) => s + (i.count as number), 0)
                );
          ...
          <div>
            ...
            <div className="space-y-2">
              {experienceMix.map((item) => {
                const total = Math.max(
                  1,
                  experienceMix.reduce((s, i) => s + (i.count as number), 0)
                );
                const pct = Math.round((item.count / total) * 100);
                return (
          ...
          <div>
            ...
            <div className="space-y-2">
              {workingPattern.map((item) => {
                const total = Math.max(
                  1,
                  workingPattern.reduce((s, i) => s + (i.count as number), 0)
                );
                const pct = Math.round((item.count / total) * 100);
                return (
      ...
      <Card className="border-elec-yellow/20 bg-elec-card">
        <CardHeader className="pb-2">
          <CardTitle className="text-base text-elec-light">
            Skills & Certifications Demand
          </CardTitle>
        </CardHeader>
