...
const DetailedBreakdown = ({
  ...
  const getLocationMultiplier = () => {
    ...
    const foundKey = Object.keys(locationPremiums).find((key) => locationKey.includes(key));
      ...
      <CardContent className="space-y-6">
        ...
        <div className="p-4 bg-indigo-500/10 rounded-lg border border-indigo-500/20">
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            <p>
              <strong className="text-foreground">Base Salary</strong> (experience-based)
            </p>
            <p>
              <strong className="text-foreground">×</strong> Location multiplier
            </p>
            <p>
              <strong className="text-foreground">×</strong> Qualification bonus
            </p>
            <p>
              <strong className="text-foreground">×</strong> Company size factor
            </p>
            <p>
              <strong className="text-foreground">×</strong> Contract type adjustment
            </p>
            <p>
              <strong className="text-foreground">+</strong> Specialist skill premiums
            </p>
          </div>
        </div>
        ...
        <div className="p-4 bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-lg border border-green-500/20">
          ...
          <div className="text-xs text-muted-foreground">
            <p>
              ...
              {salaryCalculation.min > 50000
                ? 'Senior/Specialist level'
                : salaryCalculation.min > 35000
                  ? 'Experienced professional'
                  : salaryCalculation.min > 25000
                    ? 'Qualified electrician'
                    : 'Entry/apprentice level'}
            </p>
            <p>
              <strong className="text-foreground">Recommendation:</strong>{' '}
              {salaryCalculation.confidence > 80
                ? 'High confidence - use for negotiations'
                : salaryCalculation.confidence > 60
                  ? 'Good estimate - add more details for accuracy'
