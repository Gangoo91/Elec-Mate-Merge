...
const InspectionOutcomeSelect = ({
  itemId,
  currentOutcome,
  onOutcomeChange,
}: InspectionOutcomeSelectProps) => {
  ...
  const getOutcomeLabel = (outcome: string) => {
    switch (outcome) {
      ...
      case 'FI':
        return 'FI - Further Investigation';
      case 'not-applicable':
        return 'N/A';
      case 'not-verified':
        return 'N/V - Not Verified';
      case 'limitation':
        return 'LIM - Limitation';
      default:
  ...
  const handleValueChange = (value: InspectionItem['outcome']) => {
    console.log(`[InspectionOutcomeSelect][${itemId}] Value change triggered:`, {
    ...
    console.log(`[InspectionOutcomeSelect][${itemId}] Outcome change callback triggered`);
  };

  console.log(`[InspectionOutcomeSelect][${itemId}] Rendering with outcome:`, {
    currentOutcome,
    timestamp: new Date().toISOString(),
  });
  ...
  return (
    <Select value={selectValue} onValueChange={handleValueChange}>
      ...
      >
        <SelectItem
          value="satisfactory"
          className="min-h-[48px] px-4 py-3 cursor-pointer focus:bg-muted"
        >
          <div className="flex items-center gap-3">
