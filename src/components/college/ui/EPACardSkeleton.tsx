/**
 * Skeleton loading card that matches the EPA tracking section card layout exactly.
      ...
      <div className="p-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 space-y-3">
            ...
            {/* Progress bar with 5 dot placeholders */}
            ...
            {/* Footer: cohort, gateway date, EPA date */}
...
export function EPACardSkeletonList({ count = 4 }: { count?: number }) {
  return (
    <div className="grid gap-3">
      {Array.from({ length: count }).map((_, i) => (
        <EPACardSkeleton key={i} />
      ))}
    </div>
  );
}
