...
export const ProjectAnalytics = () => {
  ...
  const analytics = useMemo(() => {
    ...
    const totalInvoiced = projects.reduce(
      (sum, p) => sum + (p.invoiceIssued && p.invoiceAmount ? p.invoiceAmount : 0),
      0
    );
    const totalPaid = projects.reduce(
      (sum, p) => sum + (p.invoicePaid && p.invoiceAmount ? p.invoiceAmount : 0),
      0
    );

    // Calculate total hours worked
    const totalHours = projects.reduce(
      (sum, p) => sum + p.timeEntries.reduce((h, entry) => h + entry.hours, 0),
      0
    );
    ...
    const projectsByMonth = Object.keys(monthData)
      .map((month) => ({
        month,
            ...
            <div className="h-[260px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis dataKey="month" tick={{ fill: '#999' }} axisLine={{ stroke: '#444' }} />
                  <YAxis tick={{ fill: '#999' }} axisLine={{ stroke: '#444' }} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="count" name="Projects" fill="#FFC900" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
