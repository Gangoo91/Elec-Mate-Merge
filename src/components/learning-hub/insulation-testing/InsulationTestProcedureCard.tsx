...
const InsulationTestProcedureCard = () => {
  const steps = [
    {
      number: 1,
      icon: Power,
      title: 'Safe Isolation & Prove Dead',
      description: 'Ensure circuit is safely isolated and use voltage indicator to prove dead',
      color: 'text-purple-400',
    },
    {
      number: 2,
      icon: Unplug,
      title: 'Disconnect Loads & SPDs',
      description: 'Remove all loads and isolate surge protection devices to prevent damage',
      color: 'text-violet-400',
    },
    {
      number: 3,
      icon: Zap,
      title: 'Select Test Voltage',
      description:
        'Choose appropriate DC test voltage: 250V, 500V, or 1000V based on circuit rating',
    ...
    },
    {
      ...
      description: 'Test Live-Neutral, Live-Earth, and Neutral-Earth insulation resistance',
      color: 'text-blue-400',
    },
    {
      number: 5,
      icon: ClipboardList,
      title: 'Record & Correct',
      description: 'Record readings and apply temperature correction if necessary',
      color: 'text-cyan-400',
    },
    {
      number: 6,
      icon: CheckCircle,
      title: 'Verify Minimum 1.0MΩ',
      description: 'Confirm all readings meet or exceed the minimum 1.0MΩ requirement',
  ...
  const voltageOptions = [
    ...
    {
      voltage: '500V DC',
      circuits: '50V to 500V',
      examples: 'Typical LV installations, most common',
    },
    ...
    <Card className="bg-card/50 border-border border-l-4 border-l-purple-500 hover:bg-card transition-all">
      ...
      <CardContent className="p-4 sm:p-5 md:p-6 pt-0 space-y-4 sm:space-y-6">
        ...
        <div className="bg-indigo-500/10 border border-indigo-500/20 border-l-4 border-l-indigo-500 rounded-lg p-4 sm:p-5">
          <h4 className="font-semibold text-indigo-400 mb-3 text-sm sm:text-base">
            ...
            {voltageOptions.map((option, index) => (
              <div key={index} className="bg-background/50 rounded-lg p-3 sm:p-4">
                <div className="flex items-start gap-3 flex-wrap">
                  <Badge className="bg-indigo-500/20 text-indigo-400 border-indigo-400/20 text-xs shrink-0">
                    {option.voltage}
                  </Badge>
          ...
          <p className="text-xs text-yellow-400 mt-3 flex items-start gap-2">
            ...
            <span>
              Apply test voltage for minimum 1 minute. Ensure all electronic equipment is
              disconnected.
            </span>
          </p>
        </div>
