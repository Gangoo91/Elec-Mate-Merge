    ...
    <div className="space-y-4">
      ...
      <Card className={`${colors.border} border-l-4 ${colors.bg}`}>
        <CardHeader className="pb-3">
          <div className="flex items-center gap-3">
            <div className={colors.text}>{getCategoryIcon(topic.category, 'h-8 w-8')}</div>
            <div className="flex-1">
              <CardTitle className={`text-lg sm:text-xl ${colors.text}`}>{topic.title}</CardTitle>
      ...
      <Card className="border-blue-500/20 bg-blue-500/5">
        <CardContent className="p-4">
          ...
          <p className="text-sm text-blue-300">{topic.regulation}</p>
        </CardContent>
      </Card>
    </div>
  );
