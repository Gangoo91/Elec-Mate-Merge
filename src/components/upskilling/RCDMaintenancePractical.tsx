...
const RCDMaintenancePractical = () => {
  ...
  const replacementCriteria = [
    ...
    {
      condition: 'Trip times consistently exceeding limits',
      action: 'Replace within 30 days',
      risk: 'High',
    },
    {
      condition: 'Nuisance tripping at ½×IΔn',
      action: 'Replace during next maintenance',
      risk: 'Medium',
    },
    {
      condition: 'Physical damage or deterioration',
      action: 'Assess and replace if necessary',
      risk: 'Variable',
    },
          ...
          <div className="bg-[#323232] rounded-lg p-4">
            ...
            <div className="mt-4 bg-green-600/10 border border-green-600/20 rounded p-3">
              <p className="text-foreground text-sm sm:text-base">
                <strong>Important:</strong> Users should be trained on proper test button operation
                and what to do if the RCD fails to trip or reset.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#323232] rounded-lg p-4 space-y-4">
            {annualInspection.map((section, index) => (
              <div
                key={index}
                className={`bg-${section.priority === 'critical' ? 'red' : section.priority === 'high' ? 'orange' : 'blue'}-600/10 border border-${section.priority === 'critical' ? 'red' : section.priority === 'high' ? 'orange' : 'blue'}-600/20 rounded p-4`}
              >
                <h4
                  className={`text-${section.priority === 'critical' ? 'red' : section.priority === 'high' ? 'orange' : 'blue'}-200 font-medium mb-3 flex items-center gap-2`}
                >
                  <span
                    className={`w-6 h-6 bg-${section.priority === 'critical' ? 'red' : section.priority === 'high' ? 'orange' : 'blue'}-500 text-foreground rounded-full flex items-center justify-center text-xs font-bold`}
                  >
                    {index + 1}
                  </span>
                  {section.category}
