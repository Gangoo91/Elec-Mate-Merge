    ...
    <Card className="bg-elec-gray border-transparent">
      ...
      <CardContent className="space-y-6">
        <div className="flex justify-between items-center text-sm text-foreground">
          <span>
            Question {currentQuestion + 1} of {testProceduresQuizQuestions.length}
          </span>
          <span>
            {Math.round(((currentQuestion + 1) / testProceduresQuizQuestions.length) * 100)}%
            Complete
          </span>
        </div>
        ...
        <div className="w-full bg-gray-700 rounded-full h-2">
          <div
            ...
            style={{
              width: `${((currentQuestion + 1) / testProceduresQuizQuestions.length) * 100}%`,
            }}
          />
        </div>
