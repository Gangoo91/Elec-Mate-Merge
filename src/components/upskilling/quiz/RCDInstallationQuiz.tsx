...
import { rcdInstallationQuizData } from '@/data/upskilling/rcdInstallationQuizData';

export const RCDInstallationQuiz = () => {
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < rcdInstallationQuizData.length - 1) {
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === rcdInstallationQuizData[index].correctAnswer ? 1 : 0);
    }, 0);
  };

  const getScorePercentage = () => {
    return Math.round((calculateScore() / rcdInstallationQuizData.length) * 100);
  };
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-foreground">
            ...
            Quiz Complete - RCD Installation Requirements
          </CardTitle>
        </CardHeader>
        <CardContent className="text-center space-y-6">
          <div className="space-y-2">
            <div className={`text-4xl font-bold ${getScoreColor(percentage)}`}>
              {score}/{rcdInstallationQuizData.length}
            </div>
  ...
  const question = rcdInstallationQuizData[currentQuestion];
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-foreground">
          <span className="flex items-center gap-2">
            <Award className="h-5 w-5 text-elec-yellow" />
            RCD Installation Requirements Quiz
          </span>
          <span className="text-sm text-foreground">
            {currentQuestion + 1} of {rcdInstallationQuizData.length}
          </span>
        </CardTitle>
      </CardHeader>
