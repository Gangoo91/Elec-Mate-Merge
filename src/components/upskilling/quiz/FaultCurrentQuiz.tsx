...
import { faultCurrentQuizData } from '@/data/upskilling/faultCurrentQuizData';
...
export const FaultCurrentQuiz = () => {
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < faultCurrentQuizData.length - 1) {
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === faultCurrentQuizData[index].correctAnswer ? 1 : 0);
    }, 0);
  };

  const getScorePercentage = () => {
    return Math.round((calculateScore() / faultCurrentQuizData.length) * 100);
  };
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-foreground">
            ...
            Quiz Complete - Prospective Fault Current Testing
          </CardTitle>
        </CardHeader>
        <CardContent className="text-center space-y-6">
          <div className="space-y-2">
            <div className={`text-4xl font-bold ${getScoreColor(percentage)}`}>
              {score}/{faultCurrentQuizData.length}
            </div>
          ...
          <div className="space-y-4">
            {percentage >= 80 && (
              <div className="bg-green-600/20 border border-green-600/30 rounded-lg p-4">
                ...
                <p className="text-foreground text-sm">
                  You have a strong understanding of prospective fault current testing principles and safety requirements.
                </p>
              </div>
            )}
            ...
            {percentage >= 60 && percentage < 80 && (
              <div className="bg-yellow-600/20 border border-yellow-600/30 rounded-lg p-4">
                ...
                <p className="text-foreground text-sm">
                  You understand the key concepts. Review the areas you missed to strengthen your knowledge.
                </p>
              </div>
            )}
            ...
            {percentage < 60 && (
              <div className="bg-red-600/20 border border-red-600/30 rounded-lg p-4">
                ...
                <p className="text-foreground text-sm">
                  Review the section content and safety requirements. Fault current testing is critical for electrical safety.
                </p>
              </div>
            )}
          </div>
  ...
  const question = faultCurrentQuizData[currentQuestion];
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-foreground">
          <span className="flex items-center gap-2">
            <Award className="h-5 w-5 text-elec-yellow" />
            Prospective Fault Current Testing Quiz
          </span>
          <span className="text-sm text-foreground">
            {currentQuestion + 1} of {faultCurrentQuizData.length}
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="w-full bg-[#323232] rounded-full h-2">
          <div 
            ...
            style={{ width: `${((currentQuestion + 1) / faultCurrentQuizData.length) * 100}%` }}
          />
        </div>
        ...
        <div className="flex justify-between">
          <div className="text-sm text-foreground">
            Question {currentQuestion + 1} of {faultCurrentQuizData.length}
          </div>
          ...
          <div className="space-x-2">
            ...
            {showResult && (
              <Button 
                ...
                className="bg-elec-yellow text-elec-dark hover:bg-yellow-500"
              >
                {currentQuestion < faultCurrentQuizData.length - 1 ? 'Next Question' : 'Complete Quiz'}
              </Button>
            )}
          </div>
        </div>
