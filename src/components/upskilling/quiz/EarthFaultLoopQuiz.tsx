...
import { earthFaultLoopQuizQuestions } from '@/data/upskilling/earthFaultLoopQuizData';
...
export function EarthFaultLoopQuiz() {
  ...
  const handleNext = () => {
    if (currentQuestion < earthFaultLoopQuizQuestions.length - 1) {
  ...
  const handleRestart = () => {
    ...
    setSelectedAnswers(new Array(earthFaultLoopQuizQuestions.length).fill(-1));
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === earthFaultLoopQuizQuestions[index].correctAnswer ? 1 : 0);
    }, 0);
  };

  const getScoreColor = (score: number) => {
    const percentage = (score / earthFaultLoopQuizQuestions.length) * 100;
  ...
  const question = earthFaultLoopQuizQuestions[currentQuestion];
  const progress = ((currentQuestion + 1) / earthFaultLoopQuizQuestions.length) * 100;
    ...
    <Card className="bg-elec-gray border-transparent">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          ...
          >
            {currentQuestion === earthFaultLoopQuizQuestions.length - 1 ? 'Finish Quiz' : 'Next'}
