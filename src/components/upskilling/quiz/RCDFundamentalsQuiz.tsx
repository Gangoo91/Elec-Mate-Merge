...
import { rcdFundamentalsQuizData } from '@/data/upskilling/rcdFundamentalsQuizData';

export const RCDFundamentalsQuiz = () => {
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < rcdFundamentalsQuizData.length - 1) {
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === rcdFundamentalsQuizData[index].correctAnswer ? 1 : 0);
    }, 0);
  };

  const getScorePercentage = () => {
    return Math.round((calculateScore() / rcdFundamentalsQuizData.length) * 100);
  };
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-foreground">
            ...
            Quiz Complete - RCD Fundamentals
          </CardTitle>
        </CardHeader>
        <CardContent className="text-center space-y-6">
          <div className="space-y-2">
            <div className={`text-4xl font-bold ${getScoreColor(percentage)}`}>
              {score}/{rcdFundamentalsQuizData.length}
            </div>
          ...
          <div className="space-y-4">
            {percentage >= 80 && (
              <div className="bg-green-600/20 border border-green-600/30 rounded-lg p-4">
                ...
                <p className="text-sm sm:text-base text-foreground">
                  You have a strong understanding of RCD fundamentals and safety principles.
                </p>
              </div>
            )}
            ...
            {percentage >= 60 && percentage < 80 && (
              <div className="bg-yellow-600/20 border border-yellow-600/30 rounded-lg p-4">
                ...
                <p className="text-sm sm:text-base text-foreground">
                  You understand the key concepts. Review the BS 7671 requirements and RCD operating principles.
                </p>
              </div>
            )}
            ...
            {percentage < 60 && (
              <div className="bg-red-600/20 border border-red-600/30 rounded-lg p-4">
                ...
                <p className="text-sm sm:text-base text-foreground">
                  Review the section content focusing on RCD operation, sensitivity ratings, and safety requirements.
                </p>
              </div>
            )}
          </div>
  ...
  const question = rcdFundamentalsQuizData[currentQuestion];
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-foreground">
          <span className="flex items-center gap-2">
            <Award className="h-5 w-5 text-elec-yellow" />
            RCD Fundamentals Quiz
          </span>
          <span className="text-sm text-foreground">
            {currentQuestion + 1} of {rcdFundamentalsQuizData.length}
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="w-full bg-[#323232] rounded-full h-2">
          <div 
            ...
            style={{ width: `${((currentQuestion + 1) / rcdFundamentalsQuizData.length) * 100}%` }}
          />
        </div>
        ...
        <div className="flex justify-between">
          <div className="text-sm text-foreground">
            Question {currentQuestion + 1} of {rcdFundamentalsQuizData.length}
          </div>
          ...
          <div className="space-x-2">
            ...
            {showResult && (
              <Button 
                ...
                className="bg-elec-yellow text-elec-dark hover:bg-yellow-500"
              >
                {currentQuestion < rcdFundamentalsQuizData.length - 1 ? 'Next Question' : 'Complete Quiz'}
              </Button>
            )}
          </div>
        </div>
