...
const EventTriggersQuiz = () => {
  ...
  const handleNext = () => {
    if (currentQuestion < eventTriggersQuizData.length - 1) {
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === eventTriggersQuizData[index].correctAnswer ? 1 : 0);
    }, 0);
  };

  const getScoreColor = (score: number) => {
    const percentage = (score / eventTriggersQuizData.length) * 100;
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardHeader>
          <CardTitle className="text-foreground flex items-center gap-2">
            ...
            Event Triggers & Auto-Reporting Quiz
          </CardTitle>
        </CardHeader>
  ...
  if (showResults) {
    ...
    const percentage = Math.round((score / eventTriggersQuizData.length) * 100);
  ...
  const question = eventTriggersQuizData[currentQuestion];
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle className="text-foreground">
            Question {currentQuestion + 1} of {eventTriggersQuizData.length}
          </CardTitle>
          <Badge variant="outline" className="border-gray-600 text-gray-300">
            {Math.round(((currentQuestion + 1) / eventTriggersQuizData.length) * 100)}%
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="text-gray-300 space-y-6">
...
export default EventTriggersQuiz;