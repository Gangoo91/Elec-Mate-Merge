...
import { resultsDocumentationQuizData } from '@/data/upskilling/resultsDocumentationQuizData';
...
export const ResultsDocumentationQuiz = () => {
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < resultsDocumentationQuizData.length - 1) {
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === resultsDocumentationQuizData[index].correctAnswer ? 1 : 0);
    }, 0);
  };

  const getScorePercentage = () => {
    return Math.round((calculateScore() / resultsDocumentationQuizData.length) * 100);
  };
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-foreground">
            ...
            Quiz Complete - Documenting & Verifying Results
          </CardTitle>
        </CardHeader>
        <CardContent className="text-center space-y-6">
          <div className="space-y-2">
            <div className={`text-4xl font-bold ${getScoreColor(percentage)}`}>
              {score}/{resultsDocumentationQuizData.length}
            </div>
  ...
  const question = resultsDocumentationQuizData[currentQuestion];
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-foreground">
          <span className="flex items-center gap-2">
            <Award className="h-5 w-5 text-elec-yellow" />
            Documenting & Verifying Results Quiz
          </span>
          <span className="text-sm text-gray-400">
            {currentQuestion + 1} of {resultsDocumentationQuizData.length}
          </span>
        </CardTitle>
      </CardHeader>
