      ...
      <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {regulations.map((reg, index) => (
              <Badge
                key={index}
                variant="outline"
                className="border-elec-yellow/50 text-elec-yellow justify-start p-2"
              >
                {reg}
              </Badge>
            ))}
        ...
        <div className="bg-elec-yellow/10 border border-elec-yellow/30 rounded-lg p-4">
          ...
          <p className="text-gray-300 text-sm">
            Continue to Section 2 to learn about Open Area (Anti-Panic) Lighting requirements and
            how they complement escape route lighting systems.
          </p>
        </div>
      </CardContent>
    </Card>
