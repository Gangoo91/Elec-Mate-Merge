  ...
  const takeaways = [
    {
      ...
      details:
        'BS 5266 and BS 7671 require comprehensive initial inspection before commissioning. This includes visual checks, physical verification, and electrical testing to ensure the installation matches approved drawings and complies with all safety requirements.',
    },
    {
      icon: Shield,
      ...
      details:
        'Fire-resistant cables must use non-combustible supports. Emergency circuits must be segregated from normal lighting. All systems must be clearly labelled. These checks prevent dangerous faults and ensure regulatory compliance.',
    },
    {
      icon: CheckCircle,
      ...
      details:
        'Use calibrated test equipment to verify all electrical parameters. Document all readings clearly. Any failures must be rectified and re-tested before proceeding. Never energise a circuit that has failed electrical testing.',
    },
    {
      icon: FileCheck,
      ...
      details:
        'Each luminaire must be inspected individually. Verify maintained/non-maintained classification, check battery charging status, confirm exit signs use ISO 7010 pictograms and face correct direction. Central battery connections must be verified for polarity.',
    },
  ];
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          {takeaways.map((takeaway, index) => (
            <div
              key={index}
              className="bg-[#323232] rounded-lg p-4 sm:p-6 border-l-4 border-elec-yellow/50"
            >
              <div className="flex items-start gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-foreground text-xs sm:text-sm">{takeaway.description}</p>
                </div>
              </div>
        ...
        <div className="bg-[#323232] rounded-lg p-4 sm:p-6">
          ...
          <ol className="space-y-3 sm:space-y-4">
            {criticalPoints.map((point, index) => (
              <li
                key={index}
                className="flex gap-3 text-foreground text-xs sm:text-sm leading-relaxed"
              >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-foreground text-sm sm:text-base leading-relaxed">
                Emergency lighting is a life safety critical system. Initial inspection and
                verification ensures that when an emergency occurs, the system will function
                correctly to enable safe evacuation. Thorough inspection at this stage prevents
                system failures that could endanger building occupants during the most critical
                moments. Never compromise on inspection quality or completeness.
              </p>
            </div>
          </div>
        </div>
