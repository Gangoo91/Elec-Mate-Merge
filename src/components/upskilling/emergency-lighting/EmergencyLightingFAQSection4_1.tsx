  ...
  const faqs = [
    {
      ...
      answer:
        'No — only fire-resistant cable types are permitted for emergency lighting circuits. Standard PVC twin & earth cables (6242Y, 6243Y) do not meet BS 5266-1 or BS 7671 fire-resistance requirements. These cables melt and fail within minutes at fire temperatures (160-200°C), far below the required survival duration. Always use cables complying with BS 7629-1 with appropriate fire rating classification (minimum Category F2, preferably F1 for critical installations).',
    },
    {
      ...
      <CardContent className="space-y-6">
        {faqs.map((faq, index) => (
          <div key={index} className="space-y-2">
            ...
            <p className="text-foreground text-sm leading-relaxed ml-4">{faq.answer}</p>
          </div>
        ))}
      </CardContent>
    </Card>
