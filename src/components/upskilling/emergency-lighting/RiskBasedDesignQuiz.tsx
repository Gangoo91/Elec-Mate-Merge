...
import { riskBasedDesignQuizQuestions } from '@/data/upskilling/riskBasedDesignQuizData';

export const RiskBasedDesignQuiz = () => {
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [selectedAnswers, setSelectedAnswers] = useState<number[]>(new Array(riskBasedDesignQuizQuestions.length).fill(-1));
  ...
  const handleNext = () => {
    if (currentQuestion < riskBasedDesignQuizQuestions.length - 1) {
  ...
  const handleRestart = () => {
    ...
    setSelectedAnswers(new Array(riskBasedDesignQuizQuestions.length).fill(-1));
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === riskBasedDesignQuizQuestions[index].correct ? 1 : 0);
    }, 0);
  };

  const getScoreColor = (score: number) => {
    const percentage = (score / riskBasedDesignQuizQuestions.length) * 100;
      ...
      <Card className="bg-gradient-to-br from-elec-gray to-[#1a1a1a] border border-gray-600 shadow-lg">
        <CardHeader>
          <CardTitle className="text-foreground flex items-center gap-2">
            ...
            Quiz Results: Risk-Based Design Adjustments
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-center mb-6">
            <div className={`text-4xl font-bold ${getScoreColor(score)} mb-2`}>
              {score}/{riskBasedDesignQuizQuestions.length}
            </div>
            <div className={`text-2xl ${getScoreColor(score)} mb-4`}>
              {percentage.toFixed(0)}%
            </div>
            <p className="text-gray-300">
              {percentage >= 80 ? 'Excellent work! You understand how to adapt emergency lighting for specific risks and occupants.' :
          ...
          <div className="space-y-4 mb-6">
            {riskBasedDesignQuizQuestions.map((question, index) => {
  ...
  const question = riskBasedDesignQuizQuestions[currentQuestion];
  const progress = ((currentQuestion + 1) / riskBasedDesignQuizQuestions.length) * 100;
  ...
  return (
    <Card className="bg-gradient-to-br from-elec-gray to-[#1a1a1a] border border-gray-600 shadow-lg">
      <CardHeader>
        <CardTitle className="text-foreground flex items-center gap-2">
          <HelpCircle className="h-5 w-5 text-elec-yellow drop-shadow-md" />
          Quiz: Risk-Based Design Adjustments
        </CardTitle>
        ...
        <p className="text-gray-300 text-sm">
          Question {currentQuestion + 1} of {riskBasedDesignQuizQuestions.length}
        </p>
      </CardHeader>
      <CardContent>
        <div className="mb-6">
        ...
        <div className="flex justify-between">
          ...
          >
            {currentQuestion === riskBasedDesignQuizQuestions.length - 1 ? 'Finish Quiz' : 'Next Question'}
          </Button>
        </div>
      </CardContent>
    </Card>
