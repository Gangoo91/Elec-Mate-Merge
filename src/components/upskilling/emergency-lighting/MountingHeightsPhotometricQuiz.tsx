...
import { mountingHeightsPhotometricQuizQuestions } from '@/data/upskilling/mountingHeightsPhotometricQuizData';
...
export const MountingHeightsPhotometricQuiz = () => {
  ...
  const handleNext = () => {
    if (currentQuestion < mountingHeightsPhotometricQuizQuestions.length - 1) {
  ...
  const handleRestart = () => {
    ...
    setSelectedAnswers(new Array(mountingHeightsPhotometricQuizQuestions.length).fill(-1));
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === mountingHeightsPhotometricQuizQuestions[index].correct ? 1 : 0);
    }, 0);
  };

  const getScoreColor = (score: number) => {
    const percentage = (score / mountingHeightsPhotometricQuizQuestions.length) * 100;
      ...
      <Card className="bg-gradient-to-br from-elec-gray to-[#1a1a1a] border border-gray-600 shadow-lg">
        <CardHeader>
          <CardTitle className="text-foreground flex items-center gap-2">
            ...
            Quiz Results: Mounting Heights and Photometric Considerations
          </CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const question = mountingHeightsPhotometricQuizQuestions[currentQuestion];
  const progress = ((currentQuestion + 1) / mountingHeightsPhotometricQuizQuestions.length) * 100;
    ...
    <Card className="bg-gradient-to-br from-elec-gray to-[#1a1a1a] border border-gray-600 shadow-lg">
      <CardHeader>
        ...
        <p className="text-gray-300 text-sm">
          Question {currentQuestion + 1} of {mountingHeightsPhotometricQuizQuestions.length}
        </p>
      </CardHeader>
      <CardContent>
        <div className="mb-6">
