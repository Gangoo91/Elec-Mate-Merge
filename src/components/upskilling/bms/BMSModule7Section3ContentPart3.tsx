      ...
      <CardContent className="text-foreground space-y-6">
        <div className="grid gap-4">
          {workflowSteps.map((item, index) => (
            <div
              key={index}
              className="flex items-start gap-4 bg-gradient-to-r from-elec-yellow/5 to-transparent border border-elec-yellow/20 rounded-lg p-4"
            >
        ...
        <div className="bg-gradient-to-r from-green-500/10 to-blue-500/10 border border-green-500/20 rounded-lg p-4">
          ...
          <p className="text-foreground">
            An AHU fan status wired into Digital Input 5 must show up on the BMS dashboard as "AHU-1
            Fan Running" — not as a random spare input.
          </p>
        </div>
