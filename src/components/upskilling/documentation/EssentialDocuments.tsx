  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-foreground mb-4 text-base sm:text-lg leading-relaxed">
        Before conducting visual inspection, ensure you have access to these critical document
        categories:
      </p>
          ...
          <div key={index} className="bg-[#323232] rounded-lg p-5">
            ...
            <ul className="space-y-3">
              {category.items.map((item, itemIndex) => (
                <li
                  key={itemIndex}
                  className="text-foreground flex items-start gap-2 text-sm sm:text-base leading-relaxed"
                >
      ...
      <div className="bg-red-600/10 border border-red-600/20 rounded-lg p-4 mt-6">
        <p className="text-red-200 font-medium">
          <strong>Critical Requirement:</strong> BS 7671 Regulation 514.9.1 requires that diagrams,
          charts, or tables be provided to show the type and composition of circuits, including
          points of utilisation, protective devices, and their characteristics.
        </p>
      </div>
    </div>
  );
