...
export const LayeredSecurityBenefitQuickCheck = () => {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [showResult, setShowResult] = useState(false);

  const question = "What's the benefit of combining contact sensors with PIRs?";
  const options = [
    "Layered security: perimeter + interior detection",
    "Reduces GDPR compliance requirements", 
    "Eliminates the need for CCTV systems"
  ];
  const correctAnswer = 0;
  const explanation = "Combining contact sensors (perimeter) with PIRs (interior) creates layered security. Contact sensors detect entry attempts while PIRs monitor movement inside, reducing blind spots and response gaps.";
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardContent className="p-6">
          <h3 className="text-foreground font-semibold text-base sm:text-lg mb-4 flex items-start sm:items-center gap-2 flex-wrap">
            ...
            <span className="break-words">Quick Check: Layered Security Benefits</span>
          </h3>
