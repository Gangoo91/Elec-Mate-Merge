...
const EcosystemCausesQuickCheck = () => {
  const [selectedAnswer, setSelectedAnswer] = useState<number | null>(null);
  const [showResult, setShowResult] = useState(false);

  const question = "Give one common cause of smart home ecosystem conflicts.";
  const options = [
    "Too many devices in one room",
    "Devices using different communication protocols (Zigbee vs Z-Wave vs Wi-Fi)",
    "Using wireless instead of wired connections",
    "Having devices from premium brands"
  ];
  const correctAnswer = 1;
  const explanation = "Different communication protocols (Zigbee, Z-Wave, Wi-Fi, etc.) often cannot communicate directly with each other, requiring bridges or hubs for integration.";
...
export default EcosystemCausesQuickCheck;