...
export const HeatingCoolingConflictQuickCheck = () => {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [showResult, setShowResult] = useState(false);

  const question = "Why is it important to prevent heating and cooling from running together?";
  const options = [
    "It makes the systems run louder",
    "It causes massive energy waste and system conflicts",
    "It reduces the number of available thermostats",
    "It makes programming more complicated"
  ];
  const correctAnswer = 1;
  const explanation = "When heating and cooling run simultaneously, they work against each other, wasting enormous amounts of energy and creating inefficient system conflicts that increase costs and reduce comfort.";
