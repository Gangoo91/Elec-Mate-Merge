...
export const SmartTRVCostQuickCheck = () => {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [showResult, setShowResult] = useState(false);

  const question = "Why might smart TRVs be expensive in large properties?";
  const options = [
    "They use too much electricity",
    "Each radiator requires its own smart TRV",
    "They need professional calibration",
        ...
        {showResult && (
          <div className="p-4 bg-[#1a1a1a] border border-gray-600 rounded-lg">
            <p className="text-gray-300 text-sm">
              <strong className="text-foreground">Explanation:</strong> In large properties with many radiators, the cost of smart TRVs can add up quickly since each radiator requires its own smart valve for individual zone control, potentially costing £50-100+ per radiator.
            </p>
            <Button
