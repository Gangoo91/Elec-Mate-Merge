...
export const SmartHomeModule2Section5RealWorld = () => {
  ...
  const causes = [
    {
      id: 'wifi-overload',
      text: 'Too many Wi-Fi devices causing network congestion',
      correct: true,
    },
    {
      id: 'router-limits',
      text: 'Router struggling with concurrent device connections',
      correct: true,
    },
    {
      id: 'channel-interference',
      text: 'Wi-Fi channel contention and interference',
      correct: true,
    },
    {
      id: 'cloud-dependency',
      text: 'Reliance on cloud services for device coordination',
      correct: true,
    },
  ...
  const calculateScore = () => {
    ...
    const causesScore = selectedCauses.filter(
      (id) => causes.find((c) => c.id === id)?.correct
    ).length;
    const solutionsScore = selectedSolutions.filter(
      (id) => solutions.find((s) => s.id === id)?.correct
    ).length;
            ...
            <div className="p-4 bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-600/30 rounded-lg">
              ...
              <div className="mt-4 p-3 bg-green-900/10 border border-green-600/20 rounded">
                ...
                <p className="text-green-100 text-sm">
                  Client B later added a Zigbee hub and migrated most devices. Performance
                  stabilised immediately with instant response times and no more dropouts.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {causes.map((cause) => (
                ...
                >
                  <span className="mr-3">{selectedCauses.includes(cause.id) ? '☑' : '☐'}</span>
                  {cause.text}
                </Button>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-gradient-to-r from-purple-900/20 to-blue-900/20 border border-purple-600/30 rounded-lg">
                ...
                <div>
                  ...
                  {causes.map((cause) => (
                    <div key={cause.id} className="flex items-center gap-2 text-sm mb-1">
                      ...
                      <span
                        className={
                          selectedCauses.includes(cause.id) && cause.correct
                            ? 'text-green-100'
                            : selectedCauses.includes(cause.id) && !cause.correct
                              ? 'text-red-100'
                              : !selectedCauses.includes(cause.id) && cause.correct
                ...
                <div>
                  ...
                  {solutions.map((solution) => (
                    <div key={solution.id} className="flex items-center gap-2 text-sm mb-1">
                      ...
                      <span
                        className={
                          selectedSolutions.includes(solution.id) && solution.correct
                            ? 'text-green-100'
                            : selectedSolutions.includes(solution.id) && !solution.correct
                              ? 'text-red-100'
                              : !selectedSolutions.includes(solution.id) && solution.correct
              ...
              <div className="p-3 bg-elec-dark border border-gray-600 rounded">
                ...
                <p className="text-gray-300 text-sm">
                  The hub made such a difference because it moved devices off the overloaded Wi-Fi
                  network onto a dedicated Zigbee mesh. This eliminated network congestion, reduced
                  cloud dependency for local automation, and provided more reliable device-to-device
                  communication. The small apartment worked fine because 6 devices don't saturate a
                  Wi-Fi network, but 60 devices definitely do.
                </p>
              </div>
            </div>
