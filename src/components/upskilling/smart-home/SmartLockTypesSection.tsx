  ...
  return (
    <Card className="bg-elec-gray border-gray-700">
      ...
      <CardContent className="text-gray-300 space-y-6">
        <p>
          Smart locks come in various forms, each designed for different installation requirements,
          security levels, and user preferences.
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {lockTypes.map((lockType, index) => (
            <div
              key={index}
              className={`p-4 bg-[#1a1a1a] border rounded-lg ${getColorClasses(lockType.color)}`}
            >
              <div className="flex items-start gap-3">
                <lockType.icon
                  className={`h-6 w-6 mt-1 flex-shrink-0 ${getIconColor(lockType.color)}`}
                />
                  ...
                  <div className="space-y-1">
                    {lockType.features.map((feature, featureIndex) => (
                      <div key={featureIndex} className="flex items-center gap-2">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${getIconColor(lockType.color).replace('text', 'bg')}`}
                        />
