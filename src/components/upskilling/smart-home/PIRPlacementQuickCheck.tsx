...
export const PIRPlacementQuickCheck = () => {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [showResult, setShowResult] = useState(false);

  const question = "Where are PIR sensors best placed?";
  const options = [
    "Hallways and main traffic areas, mounted 2–2.5m high",
    "Directly facing windows for maximum light detection", 
    "Behind curtains near radiators for concealment"
  ];
  const correctAnswer = 0;
  const explanation = "PIR sensors work best in hallways and main traffic areas at 2-2.5m height, where they can detect cross-movement. Avoid windows, radiators, and direct sunlight.";
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardContent className="p-6">
          <h3 className="text-foreground font-semibold text-base sm:text-lg mb-4 flex items-start sm:items-center gap-2 flex-wrap">
            ...
            <span className="break-words">Quick Check: PIR Placement Height</span>
          </h3>
