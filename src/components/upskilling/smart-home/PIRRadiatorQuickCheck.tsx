...
export const PIRRadiatorQuickCheck = () => {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [showResult, setShowResult] = useState(false);

  const question = "Why should PIRs not face radiators?";
  const options = [
    "Heat changes from radiators cause false triggers",
    "They block Wi-Fi signals from the sensor", 
    "They drain sensor batteries faster"
  ];
  const correctAnswer = 0;
  const explanation = "PIR sensors detect infrared radiation changes. Radiators and heating vents create temperature fluctuations that can trigger false alarms by mimicking heat signatures.";
      ...
      <Card className="bg-elec-gray border-transparent">
        <CardContent className="p-6">
          <h3 className="text-foreground font-semibold text-base sm:text-lg mb-4 flex items-start sm:items-center gap-2 flex-wrap">
            ...
            <span className="break-words">Quick Check: PIR and Heat Sources</span>
          </h3>
