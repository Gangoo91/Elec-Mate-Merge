...
export const ResistiveLoadQuickCheck = () => {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [showResult, setShowResult] = useState(false);

  const question = "What type of load is an incandescent lamp?";
  const options = [
    { id: 'a', text: 'Capacitive', correct: false },
    { id: 'b', text: 'Resistive', correct: true },
    { id: 'c', text: 'Inductive', correct: false },
    { id: 'd', text: 'Mixed', correct: false }
  ];
              ...
              {selectedAnswer === 'b' ? (
                <div className="bg-green-900/20 border border-green-600/50 p-3 rounded-lg">
                  <p className="text-green-200 text-sm">
                    ✓ Correct! Incandescent lamps are resistive loads - they behave like pure resistors 
                    and are easy to control with most dimming methods.
                  </p>
                </div>
              ) : (
                <div className="bg-red-900/20 border border-red-600/50 p-3 rounded-lg">
                  <p className="text-red-200 text-sm">
                    ✗ Incorrect. Incandescent lamps are resistive loads because they use a tungsten 
                    filament that behaves like a resistor when heated.
                  </p>
                </div>
              )}
              <Button 
