  ...
  const interpretationGuide = [
    {
      range: '≥ 100 MΩ',
      status: 'Excellent',
      description: 'New installation quality—no concerns',
      color: 'green',
    },
    {
      range: '10-99 MΩ',
      status: 'Good',
      description: 'Healthy insulation—acceptable for most installations',
      color: 'green',
    },
    {
      range: '2-9 MΩ',
      status: 'Acceptable',
      description: 'Meets minimum but monitor for deterioration',
      color: 'yellow',
    },
    {
      range: '1-1.9 MΩ',
      status: 'Marginal',
      description: 'Just meets minimum—investigate further',
      color: 'orange',
    },
    {
      range: '< 1 MΩ',
      status: 'Fail',
      description: 'Below minimum—fault must be found and corrected',
      color: 'red',
    },
  ];
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {interpretationGuide.map((item, index) => (
              <div
                key={index}
                className={`bg-[#323232] rounded-lg p-4 border-l-4 ${
                  item.color === 'green'
                    ? 'border-green-500/50'
                    : item.color === 'yellow'
        ...
        <div className="bg-yellow-600/10 border border-yellow-600/20 rounded-lg p-4">
          ...
          <p className="text-foreground text-sm leading-relaxed mb-3">
            Always record the actual resistance value in MΩ, not just "pass", "satisfactory", or
            tick marks. This allows for:
          </p>
