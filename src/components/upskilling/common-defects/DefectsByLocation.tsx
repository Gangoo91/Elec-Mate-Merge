...
export const DefectsByLocation = () => {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      default:
        return 'bg-gray-600/20 text-gray-200';
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-gray-300 mb-4">
        Different areas of installations have characteristic defect patterns. Focus your inspection
        on these location-specific issues:
      </p>
      <div className="space-y-4">
        {locationDefects.map((location, index) => (
          <div key={index} className="bg-[#323232] rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`px-3 py-1 rounded text-xs font-medium ${getPriorityColor(location.priority)}`}
              >
      ...
      <div className="bg-blue-600/10 border border-blue-600/20 rounded-lg p-4 mt-4">
        <p className="text-blue-200 font-medium">
          <strong>Inspection Strategy:</strong> Start with critical priority locations first, then
          work through high and medium priority areas. This approach ensures you catch the most
          dangerous defects early in your inspection process.
        </p>
      </div>
    </div>
  );
