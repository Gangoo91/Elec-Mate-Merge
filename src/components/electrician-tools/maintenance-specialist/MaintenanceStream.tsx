/**
 * Maintenance Method Stream — editorial streaming surface.
 *
 * Mirrors `InstallationStream`. Subscribes to `maintenance_method_partials`
...
interface MaintenanceStreamProps {
...
const STAGES: Stage[] = [
  {
    ...
    description: 'Parsing your description and any attached asset sheets, photos or PDFs.',
    threshold: 5,
  },
  {
    id: 'rag',
    label: 'Grounding against BS 7671',
    description: 'Pulling regulations and practical-work facets relevant to this maintenance task.',
    threshold: 32,
  },
  {
    id: 'method',
    label: 'Drafting the outline',
    description: 'Sequencing the steps, isolation points and verification gates.',
    threshold: 50,
  },
  {
    id: 'enrichment',
  ...
  },
  {
    id: 'finalise',
    label: 'Finalising',
    description: 'Assembling the printable maintenance method statement.',
    threshold: 92,
  },
];

export const MaintenanceStream = ({
  inputs,
  jobId,
  jobProgress = 0,
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        ...
        .from('maintenance_method_partials' as any)
        .select('*')
        .eq('job_id', jobId);
    ...
    const channel = supabase
      .channel(realtimeChannelName(`mmp-${jobId}`))
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
          schema: 'public',
          table: 'maintenance_method_partials',
  ...
  const typeLabel = MAINTENANCE_INSTALLATION_OPTIONS.find(
          ...
          <div className="flex items-center h-12 gap-4">
            ...
            <div className="flex-1 min-w-0 flex items-baseline gap-2.5">
              ...
              <h1 className="text-[13px] sm:text-sm font-semibold text-white truncate tracking-tight">
                {inputs?.projectName || 'Maintenance Method'}
              </h1>
            </div>
            {onCancel && !complete && (
              <button
            ...
            {complete
              ...
              : failed
                ...
                : cancelled
                  ...
                  : currentStep ||
                    'BS 7671 grounding, step sequencing and per-step depth all running in parallel.'}
          </p>
