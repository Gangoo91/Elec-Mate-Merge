      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <Badge
                variant="outline"
                className="text-blue-300 border-blue-500/50 text-sm px-3 py-1"
              >
                Fix #{index + 1}
              </Badge>
            ...
            <h4 className="text-base font-bold text-foreground mb-1.5">For: {action.forSymptom}</h4>
            <p className="text-sm text-foreground/80 leading-relaxed">{action.action}</p>
          </div>
        </div>
        ...
        {action.detailedProcedure && action.detailedProcedure.length > 0 && (
          <Accordion
            type="single"
            collapsible
            value={isExpanded ? 'procedure' : ''}
            onValueChange={(val) => setIsExpanded(val === 'procedure')}
          >
          ...
          <div className="space-y-3 mt-4">
            ...
            <div className="space-y-2">
              {action.stepByStepFix.map((step, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-3 bg-elec-dark/50 border border-blue-500/20 rounded-lg p-3 hover:border-blue-500/40 transition-colors"
                >
                ...
                <div className="flex flex-wrap gap-1.5">
                  {action.commonBrands.map((brand, idx) => (
                    <Badge
                      key={idx}
                      variant="outline"
                      className="text-xs text-blue-300 border-blue-500/50"
                    >
                      {brand}
                    </Badge>
                  ))}
