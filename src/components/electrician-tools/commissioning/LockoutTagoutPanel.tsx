      ...
      <Card className="bg-green-500/10 border-green-500/30 p-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-foreground/70">Not required for this procedure</p>
          </div>
        </div>
      </Card>
    );
      ...
      <div className="space-y-5 sm:space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-3">
          ...
          <div className="flex-1 w-full text-center sm:text-left">
            <div className="flex flex-col sm:flex-row items-center sm:items-center gap-2 mb-3 sm:mb-2">
              <Badge className="bg-red-500 text-foreground text-base sm:text-sm px-4 py-1.5 sm:px-3 sm:py-1">
                Required
              </Badge>
              <h3 className="text-xl sm:text-lg font-bold text-red-300">
                Lockout/Tagout Procedure
              </h3>
            </div>
            <p className="text-base sm:text-sm text-red-200 leading-relaxed">
              This procedure requires complete electrical isolation. Follow all steps before
              beginning work.
            </p>
          </div>
        </div>
        ...
        {lockoutTagout.procedure && lockoutTagout.procedure.length > 0 && (
          <div className="space-y-3 sm:space-y-2">
            <h4 className="text-lg sm:text-sm font-bold sm:font-semibold text-foreground flex items-center justify-center sm:justify-start gap-2">
            ...
            <ol className="space-y-3 sm:space-y-2">
              {lockoutTagout.procedure.map((step, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-4 sm:gap-3 text-base sm:text-sm text-foreground leading-relaxed text-left"
                >
        ...
        {lockoutTagout.isolationPoints && lockoutTagout.isolationPoints.length > 0 && (
          <div className="space-y-3 sm:space-y-2 bg-red-500/10 rounded-lg p-5 sm:p-4 border-2 sm:border border-red-500/30">
            <h4 className="text-lg sm:text-sm font-bold sm:font-semibold text-red-300 text-center sm:text-left">
              Isolation Points to Lock
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-2">
              {lockoutTagout.isolationPoints.map((point, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-center sm:justify-start gap-2 text-base sm:text-sm text-foreground"
                >
        ...
        <div className="bg-red-500/20 border-2 sm:border border-red-500/50 rounded-lg p-4 sm:p-3">
          <p className="text-sm sm:text-xs text-red-200 text-center sm:text-left leading-relaxed">
            <strong>Warning:</strong> Only authorized personnel should perform lockout/tagout.
            Verify all energy sources are isolated and locked before beginning work.
          </p>
        </div>
      </div>
    </Card>
