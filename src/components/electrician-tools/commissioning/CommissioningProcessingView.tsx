...
const ESTIMATED_TIME = 210; // 3:30

const CommissioningProcessingView = ({
  progress: agentProgress,
  startTime,
  backendProgress = 0,
  onCancel,
  isCancelling = false
}: CommissioningProcessingViewProps) => {
  ...
  const calculateProgress = () => {
    ...
    const normalised = Math.min(elapsedTime / ESTIMATED_TIME, 1);
    const eased = 1 - Math.pow(1 - normalised, 2);
  ...
  const getActivityMessage = () => {
    ...
    if (elapsedTime < 30) return 'Understanding testing requirements...';
    if (elapsedTime < 120) return 'Generating test procedures...';
    if (elapsedTime < 180) return 'Calculating expected Zs values...';
    return 'Verifying compliance...';
  };
            ...
            <div className="relative">
              ...
              >
                <motion.div
                  ...
                  transition={{ duration: 2, repeat: Infinity }}
                >
                  <CheckCircle2 className="h-8 w-8 text-elec-yellow drop-shadow-[0_0_10px_rgba(247,208,44,0.4)]" />
                </motion.div>
              </motion.div>
              <motion.div
        ...
        {elapsedTime > ESTIMATED_TIME && (
          ...
          >
            <p className="text-[10px] text-amber-200 text-center">
              Taking longer than usual. Complex testing may take up to 5 minutes.
            </p>
          </motion.div>
        )}
