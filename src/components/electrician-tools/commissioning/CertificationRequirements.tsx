    ...
    <div className="space-y-5 sm:space-y-4">
        ...
        <div className="space-y-3 sm:space-y-2">
          ...
          <ul className="space-y-3 sm:space-y-2">
            {certification.requiredSchedules.map((schedule, index) => (
              <li
                key={index}
                className="flex items-center justify-center sm:justify-start gap-3 sm:gap-2 p-4 sm:p-2 rounded-lg bg-elec-gray border-2 sm:border border-elec-yellow/20 text-base sm:text-sm text-foreground text-center sm:text-left"
              >
            ...
            {certification.requiredData.map((data, index) => (
              ...
              >
                ...
                <AccordionContent className="px-4 pb-4 sm:pb-3 text-center sm:text-left">
                  <p className="text-sm sm:text-xs text-foreground">{data.description}</p>
                </AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
      ...
      {certification.nextInspection && (
        <div className="bg-blue-500/10 border-2 sm:border border-blue-500/20 rounded-lg p-4 sm:p-3 text-center sm:text-left">
          ...
          <p className="text-sm sm:text-xs text-foreground">{certification.nextInspection}</p>
        </div>
      )}
      ...
      {certification.additionalNotes && (
        <div className="bg-amber-500/10 border-2 sm:border border-amber-500/20 rounded-lg p-4 sm:p-3 text-center sm:text-left">
          ...
          {Array.isArray(certification.additionalNotes) ? (
            ...
            <p className="text-sm sm:text-xs text-foreground">{certification.additionalNotes}</p>
          )}
        </div>
      )}
    </div>
