...
const InlineAIInsightsSection = ({
  ...
  const insights = useMemo(() => {
    ...
    const onSaleTools = tools.filter((t) => t.isOnSale && t.salePrice);
    ...
    return { bestValue, topRated, bestDeal, recommendations, tips };
  }, [tools, categoryName]);
    ...
    <section className="space-y-4">
      ...
      {isExpanded && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              {insights.recommendations.length === 0 && (
                <p className="text-sm text-muted-foreground italic">
                  Browse more tools to get personalized recommendations
                </p>
              )}
            </div>
          </div>
          ...
          {/* Best Deal Highlight */}
          {insights.bestDeal && (
            <div className="md:col-span-2 p-4 rounded-xl bg-gradient-to-r from-success/10 to-success/5 border border-success/20">
              <div className="flex items-center gap-3">
