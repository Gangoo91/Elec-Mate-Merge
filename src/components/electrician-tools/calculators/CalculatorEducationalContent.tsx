...
const CalculatorEducationalContent: React.FC<CalculatorEducationalContentProps> = ({
    ...
    const baseContent: Record<string, EducationalResource[]> = {
      'ohms-law': [
        ...
        },
        {
          ...
          type: 'example',
          content:
            "In electrical installations, Ohm's Law helps calculate appropriate cable sizes, protective device ratings, and voltage drops. For example, if you know the load current and cable resistance, you can calculate voltage drop to ensure compliance with BS 7671 regulations.",
        ...
        },
        {
          ...
          type: 'warning',
          content:
            'Always ensure measurements are taken with the circuit de-energised when possible. Remember that resistance values can change with temperature, and always account for this in your calculations. Never exceed the current-carrying capacity of conductors.',
          difficulty: 'beginner',
          estimatedTime: '3 min',
        },
      ],
      'voltage-drop': [
        {
          ...
          type: 'regulation',
          content:
            'BS 7671 requires that voltage drop should not exceed 3% for lighting circuits and 5% for other circuits under normal operating conditions. This ensures proper operation of electrical equipment and compliance with UK electrical regulations.',
          difficulty: 'intermediate',
          estimatedTime: '8 min',
        ...
        },
        {
          ...
          type: 'example',
          content:
            "To calculate voltage drop: VD = (mV/A/m × Ib × L) / 1000, where mV/A/m is the cable's voltage drop per ampere per metre, Ib is the design current, and L is the cable length. Always use the appropriate values from cable manufacturers' data.",
          difficulty: 'intermediate',
      ...
      'cable-sizing': [
        {
          ...
          type: 'theory',
          content:
            'Cable sizing involves considering current-carrying capacity, voltage drop, short-circuit protection, and environmental factors. The cable must carry the design current while meeting voltage drop limits and withstanding fault conditions.',
          difficulty: 'advanced',
        ...
        },
        {
          ...
          type: 'tip',
          content:
            'Apply appropriate derating factors for ambient temperature, grouping, and thermal insulation. Common factors: ambient temperature >30°C reduces capacity, grouping multiple cables reduces capacity, thermal insulation requires special consideration.',
          difficulty: 'intermediate',
          estimatedTime: '10 min',
        },
      ],
  ...
  const getContextualTips = () => {
    ...
    if (calculatorType === 'cable-sizing' && currentInputs.ambientTemp > 35) {
      tips.push(
        'High ambient temperatures require derating - consider ventilation or larger cables.'
      );
    }
        ...
        <CardContent>
          <Tabs defaultValue="all" className="w-full">
            ...
            <TabsContent value="theory" className="mt-6">
              <div className="space-y-4">
                {resources
                  .filter((r) => r.type === 'theory' || r.type === 'regulation')
                  .map((resource) => (
            ...
            <TabsContent value="practical" className="mt-6">
              <div className="space-y-4">
                {resources
                  .filter((r) => r.type === 'example' || r.type === 'tip')
                  .map((resource) => (
            ...
            <TabsContent value="regulations" className="mt-6">
              <div className="space-y-4">
                {resources
                  .filter((r) => r.type === 'regulation' || r.regulation)
                  .map((resource) => (
