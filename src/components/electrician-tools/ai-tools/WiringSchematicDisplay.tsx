              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-xs font-medium text-blue-700 dark:text-blue-400">
                    {ragSourcesCount.installation_docs_count +
                      ragSourcesCount.regulations_count +
                      ragSourcesCount.safety_docs_count}{' '}
                    Knowledge Sources
                  </p>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div>
                      ...
                      <Badge variant="secondary" className="text-xs sm:text-sm">
                        {circuitSpec.rcdRating}mA RCD Required
                      </Badge>
                    </div>
                  </TooltipTrigger>
                  <TooltipContent className="max-w-xs">
                    <p className="text-xs">
                      Residual Current Device protects against earth faults. Required for additional
          ...
          <CardContent className="px-4 pb-4 sm:px-5 sm:pb-5">
            <div className="space-y-2.5">
              {terminalConnections.map((conn, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 sm:p-3.5 bg-muted/50 rounded-lg border border-border/30 hover:border-elec-yellow/30 transition-colors"
                >
                  <div className="flex items-center gap-2.5 sm:gap-3 flex-1 min-w-0">
                    <Badge className="font-mono text-xs sm:text-sm flex-shrink-0">
                      {conn.terminal}
                    </Badge>
                    <div
                    ...
                    {conn.torque_setting && (
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p className="text-xs">
                              Recommended torque setting for secure connection
                            </p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
              ...
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start gap-2 mb-2">
                    <h4 className="text-sm sm:text-base font-semibold text-foreground leading-tight flex-1">
                      {step.title}
                    </h4>
                    {step.safety_critical && (
                  ...
                  <p className="text-xs sm:text-sm text-foreground/90 mb-2.5 leading-relaxed">
                    {step.instruction}
                  </p>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {step.ppe_required && step.ppe_required.length > 0 && (
                      <Badge
                        variant="outline"
                        className="text-xs bg-orange-500/10 border-orange-500/30 text-orange-700 dark:text-orange-400"
                      >
          ...
          <CardContent className="px-4 pb-4 sm:px-5 sm:pb-5">
            <ul className="space-y-2.5">
              {testingRequirements.map((test, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-2.5 text-xs sm:text-sm p-2 sm:p-2.5 bg-muted/30 rounded-lg"
                >
          ...
          <CardContent className="px-4 pb-4 sm:px-5 sm:pb-5">
            <ul className="space-y-2.5">
              {safetyWarnings.map((warning, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-2.5 text-xs sm:text-sm p-3 sm:p-3.5 bg-red-500/10 border border-red-500/20 rounded-lg"
                >
                  <AlertTriangle className="h-4 w-4 sm:h-5 sm:w-5 text-red-600 dark:text-red-500 flex-shrink-0 mt-0.5" />
                  <span className="text-red-700 dark:text-red-400 font-medium leading-relaxed">
                    {warning}
                  </span>
                </li>
              ))}
