  ...
  const handleRegulationSearch = async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description:
          error instanceof Error ? error.message : 'Failed to get regulations information',
        variant: 'destructive',
      });
    } finally {
              ...
              {regResult
                ...
                .map((line, index) => {
                  ...
                  const technicalTerms =
                    /\b(RCD|RCBO|MCB|MCCB|RCM|AFDD|SPD|CU|DB|EICR|PIR|EIC|PAT|Zs|Ze|Zdb|PFC|PSCC|TN-S|TN-C-S|TT|IT|IP\d{2}|CSA|CPC|PME|SWA|MICC|FP200|XLPE|PVC|LSZH)\b/gi;
                  const measurements =
                    /\b(\d+(?:\.\d+)?)\s*(A|mA|V|kV|W|kW|VA|kVA|Ω|mΩ|mm²?|m|cm|Hz|°C|lx|lm|cd)\b/g;

                  let processedText = trimmed
                    .replace(
                      regulationNumbers,
                      '<span class="px-2 py-1 bg-purple-500/25 text-purple-200 rounded font-bold">$&</span>'
                    )
                    .replace(
                      partNumbers,
                      '<span class="px-1.5 py-0.5 bg-indigo-500/20 text-indigo-200 rounded font-semibold">$&</span>'
                    )
                    .replace(
                      bsNumbers,
                      '<span class="px-2 py-1 bg-blue-500/25 text-blue-200 rounded font-bold">$&</span>'
                    )
                    .replace(
                      technicalTerms,
                      '<span class="px-1.5 py-0.5 bg-elec-yellow/20 text-elec-yellow rounded font-medium">$&</span>'
                    )
                    .replace(
                      measurements,
                      '<span class="px-1 py-0.5 bg-green-500/20 text-green-200 rounded font-mono text-xs">$&</span>'
                    );

                  // Main regulation headers
                  if (
                    trimmed.startsWith('BS 7671:') ||
                    trimmed.startsWith('Regulation ') ||
                    trimmed.match(/^(REGULATIONS?|BS\s*7671):?$/i)
                  ) {
                    return (
                  ...
                  if (trimmed.match(/^(Clause|Section|Chapter|Part|Appendix)\s*\d+/i)) {
                    return (
                      <div key={index} className="mt-3 mb-2">
                        <h5
                          className="font-semibold text-blue-300 p-2 bg-blue-500/10 rounded border border-blue-500/20"
                  ...
                  if (trimmed.match(/\b(must|shall|required|mandatory|compliance)\b/gi)) {
                    return (
                      <div
                        key={index}
                        className="my-3 p-3 bg-amber-500/15 border border-amber-500/25 rounded-lg"
                      >
                  ...
                  if (
                    trimmed.includes('⚠️') ||
                    trimmed.match(/\b(danger|warning|caution|safety|hazard)\b/gi)
                  ) {
                    return (
                      <div
                        key={index}
                        className="my-3 p-3 bg-orange-500/15 border border-orange-500/25 rounded-lg"
                      >
                        <div className="flex items-start gap-2">
