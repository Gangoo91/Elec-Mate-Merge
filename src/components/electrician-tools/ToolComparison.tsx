      ...
      <CardContent className="space-y-6">
        ...
        {selectedTools.length > 0 && (
          <div className="space-y-4">
            <h4 className="text-foreground font-medium">Comparison Results</h4>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left text-foreground/80 p-2">Feature</th>
                    {selectedTools.map((tool) => (
                      <th key={tool.id} className="text-left text-foreground p-2 min-w-48">
                        <div className="space-y-1">
                          <div className="font-medium truncate">{tool.name}</div>
                          <Badge className="bg-blue-600/90 text-foreground border-blue-400">
                            {tool.category}
                          </Badge>
                        </div>
                      </th>
                ...
                <tbody className="space-y-2">
                  <tr className="border-b border-white/5">
                    <td className="text-foreground/80 p-2 font-medium">Price</td>
                    {selectedTools.map((tool) => (
                      <td key={tool.id} className="p-2">
                        <div className="space-y-1">
                          ...
                          {tool.salePrice && (
                            <div className="text-foreground/60 line-through text-sm">
                              {tool.price}
                            </div>
                          )}
                        </div>
                  ...
                  <tr className="border-b border-white/5">
                    <td className="text-foreground/80 p-2 font-medium">Key Features</td>
                    {selectedTools.map((tool) => (
                      <td key={tool.id} className="p-2">
                        <div className="space-y-1">
                          ...
                          )) || <span className="text-foreground/60">No features listed</span>}
                        </div>
                      </td>
                    ))}
                  </tr>
                  <tr>
                    <td className="text-foreground/80 p-2 font-medium">Value Score</td>
                    {selectedTools.map((tool) => {
