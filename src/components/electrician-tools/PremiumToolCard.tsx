...
const PremiumToolCard = ({
  ...
  return (
    ...
    >
      {/* Image Section */}
      <div className={isCompact ? 'product-image-container-compact' : 'product-image-container'}>
        {/* Supplier Badge */}
        {item.supplier && <div className="supplier-badge">{item.supplier}</div>}

        {/* Sale Ribbon */}
        {discount && discount > 0 && <div className="sale-ribbon">SAVE {discount}%</div>}
      ...
      <div className="flex flex-col flex-1 p-4 sm:p-5 gap-3">
        ...
        {reviewData && (
          <div className="star-rating">
            ...
            {reviewData.count && (
              <span className="text-xs text-muted-foreground ml-1">({reviewData.count})</span>
            )}
          </div>
        )}
        ...
        <div className="price-block border-t border-border/30 pt-3">
          {item.isOnSale && item.salePrice && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {savingsAmount && <span className="price-savings">Save {savingsAmount}</span>}
            </div>
          )}
          <div className="flex items-baseline gap-1.5">
            <span className="price-primary">{item.salePrice || item.price}</span>
