...
import {
  calculateExpectedR1R2,
  getMaxZsForDevice,
  mapLoadTypeToCircuitDescription,
} from '@/utils/eic-transformer';
...
export const DesignReviewEditor = ({ design, onReset }: DesignReviewEditorProps) => {
  ...
  const sendToAgent = (agentType: AgentType) => {
    try {
      ...
      const circuitIndices =
        selectedCircuitsForExport.length > 0
  ...
  useEffect(() => {
    if (design.circuits) {
      ...
      if (needsPatch) {
        console.log(
          '🔧 Patched missing justifications for',
          patchedCircuits.filter(
            (c, i) =>
              !design.circuits![i].justifications ||
              Object.keys(design.circuits![i].justifications).length === 0
          ).length,
          'circuits'
        );
  ...
  const transformToPDFMonkeyPayload = (design: InstallationDesign) => {
    ...
    const totalConnectedLoad =
      design.circuits?.reduce((sum, c) => sum + (c.loadPower || 0), 0) || 0;
    const totalDesignCurrent = Math.round(
      design.circuits?.reduce((sum, c) => sum + (c.designCurrent || c.calculations?.Ib || 0), 0) ||
        0
    );
    const compliantCircuits =
      design.circuits?.filter((c) => {
        const zsCompliant = c.calculations?.zs
          ? c.calculations.zs <= (c.calculations?.maxZs || 999)
          : true;
        const vdCompliant = c.calculations?.voltageDrop?.compliant ?? true;
        return zsCompliant && vdCompliant;
      }).length || 0;
    const warningCount =
      design.circuits?.reduce((sum, c) => sum + (c.warnings?.length || 0), 0) || 0;
    ...
    return {
      ...
      ze: design.consumerUnit?.incomingSupply?.Ze
        ? design.consumerUnit.incomingSupply.Ze.toFixed(2)
        : 'Not specified',
      ...
      totalConnectedLoad: totalConnectedLoad
        ? totalConnectedLoad.toLocaleString()
        : 'Not specified',
      diversityFactor:
        design.diversityBreakdown?.overallDiversityFactor || design.diversityFactor
          ? `${((design.diversityBreakdown?.overallDiversityFactor || design.diversityFactor) * 100).toFixed(1)}%`
          : 'Not specified',
      diversifiedLoad:
        design.diversityBreakdown?.diversifiedLoad || totalConnectedLoad
      ...
      circuits: (design.circuits || []).map((circuit, idx) => {
        ...
        const zsCompliant = circuit.calculations?.zs
          ? circuit.calculations.zs <= (circuit.calculations?.maxZs || 999)
          : true;
        ...
        return {
          ...
          phases:
            (typeof circuit.phases === 'number' && circuit.phases === 3) ||
          ...
          deratedCapacity: (
            circuit.calculations?.deratedCapacity ||
            circuit.calculations?.Iz ||
            0
          ).toFixed(1),
          ...
          protectionSummary:
            circuit.protectionDevice?.type === 'BS88'
          ...
          installationNotes:
            circuit.installationNotes || (circuit.installationGuidance as any)?.cableRouting || '',
          ...
          justificationCable:
            ...
            `${circuit.cableSize}mm² cable selected for adequate protection.`,
          justificationProtection:
            circuit.justifications?.protection ||
            `${circuit.protectionDevice?.rating}A protection device provides adequate protection.`,
          justificationRcd:
            circuit.justifications?.rcd ||
            (circuit.rcdProtected
              ? 'RCD protection provides additional safety.'
              : 'RCD not required.'),
      ...
      // Compliance
      allCircuitsCompliant:
        compliantCircuits === (design.circuits?.length || 0) && (design.circuits?.length || 0) > 0,
  ...
  const transformToEnhancedPdfJson = (design: InstallationDesign) => {
    ...
    return {
      ...
      consumerUnit: {
        ...
        typeDescription:
          design.consumerUnit?.type === 'split-load'
            ? 'Split Load Consumer Unit'
            : design.consumerUnit?.type === 'high-integrity'
      ...
      loadAssessment: {
        ...
        diversityFactor:
          design.diversityBreakdown?.overallDiversityFactor || design.diversityFactor || 0.65,
      ...
      circuits: design.circuits.map((circuit) => ({
            ...
            circuit.loadType === 'socket'
              ...
              : circuit.loadType === 'lighting'
                ...
                : circuit.loadType === 'cooker'
                  ? 'Cooker Circuit'
                  : circuit.loadType === 'shower'
                    ? 'Electric Shower Circuit'
                    : circuit.loadType === 'immersion'
                      ? 'Immersion Heater'
                      : circuit.loadType === 'heating'
                        ? 'Heating Circuit'
                        : 'Fixed Appliance',
        ...
        cableSpecification: {
          ...
          cableType:
            circuit.cableType || `${circuit.cableSize ?? 2.5}/${circuit.cpcSize ?? 1.5}mm² T&E`,
          cableLength: circuit.cableLength ?? 0,
          cableLengthString: `${circuit.cableLength ?? 0}m`,
          installationMethod: circuit.installationMethod,
          installationMethodDescription:
            circuit.installationMethod === 'clipped-direct'
              ? 'Method C - Clipped Direct (Reference Method 100)'
              : circuit.installationMethod === 'enclosed-conduit'
                ? 'Method B - Enclosed in Conduit (Reference Method 3)'
                : circuit.installationMethod === 'insulated-wall'
                  ? 'Method A - Enclosed in Insulated Wall (Reference Method 101)'
                  : circuit.installationMethod === 'trunking'
                    ? 'Method E - In Trunking'
                    : circuit.installationMethod === 'underground'
        ...
        designCalculations: {
          ...
          cableSizing: {
            regulation: '433.1.1',
            tabulatedCapacity_It:
              circuit.deratingFactors?.Ca && circuit.calculations?.Iz
                ? circuit.calculations.Iz / circuit.deratingFactors.overall
                : 0,
            ...
            complianceText: circuit.calculations?.voltageDrop?.compliant
              ? '✓ COMPLIANT'
              : '✗ NON-COMPLIANT',
          },
        ...
        diversity: {
          ...
          justification:
            circuit.diversityJustification || 'No diversity applied - full load considered',
        },
        ...
        faultCurrentAnalysis: circuit.faultCurrentAnalysis
          ? {
              ...
              complianceText: circuit.faultCurrentAnalysis.compliant
                ? '✓ COMPLIANT'
                : '✗ NON-COMPLIANT',
        ...
        earthingRequirements: circuit.earthingRequirements
          ? {
              ...
              supplementaryBondingString: circuit.earthingRequirements.supplementaryBonding
                ? 'Yes'
                : 'No',
        ...
        expectedTestResults: circuit.expectedTestResults
          ? {
              ...
              zs: {
                ...
                complianceText:
                  circuit.expectedTestResults.zs?.compliant != null
              ...
              insulationResistance: {
                testVoltage:
                  circuit.expectedTestResults.insulationResistance?.testVoltage || '500V DC',
                minResistance:
                  circuit.expectedTestResults.insulationResistance?.minResistance || '≥1.0MΩ',
              },
              ...
              rcdTest: circuit.expectedTestResults.rcdTest
                ? {
                    ...
                    regulation:
                      circuit.expectedTestResults.rcdTest.regulation ||
                      'BS 7671 Regulation 643.2.2',
                  }
                : null,
            }
          : null,
      ...
      diversityBreakdown: design.diversityBreakdown || {
        ...
        reasoning:
          'Diversity applied in accordance with BS 7671 Appendix A. Assessment considers coincident maximum demand unlikely to occur simultaneously across all circuits.',
        bs7671Reference:
          'Appendix A - Current-carrying capacity and voltage drop for cables and flexible cords',
      ...
      designNotes: {
        general:
          'This design complies with BS 7671:2018+A3:2024 (18th Edition IET Wiring Regulations). All circuits designed for safe operation with adequate protection against overload, short circuit, and earth faults.',
        ...
        testing:
          'Installation must be inspected and tested per BS 7671 Part 6 before being put into service. Electrical Installation Certificate required.',
        specialLocations: design.circuits.some(
          (c) => c.specialLocationCompliance?.isSpecialLocation
        )
      ...
      installationRequirements: {
        minimumCableSizes: {
          mainEarthing:
            design.consumerUnit?.incomingSupply?.earthingSystem === 'TN-S' ? '16mm²' : '10mm²',
          mainBonding: '10mm²',
      ...
      certificationRequirements: {
        initialVerification:
          'Electrical Installation Certificate (BS 7671 Appendix 6) required upon completion',
        schedules:
          'Schedule of Inspections and Schedule of Test Results must accompany certificate',
        ...
        inspectionDeclaration:
          'Inspector declaration required if different from designer/installer',
      },
      ...
      maintenanceRecommendations: {
        ...
        visualInspection:
          'Annual visual inspection of installation for damage, wear, or deterioration',
        periodicInspection:
        ...
        consumerUnitCheck:
          'Check for signs of overheating, damage, loose connections, or deterioration annually',
      },

      safetyInformation: {
        isolationNotice:
          'Durable warning notice required at consumer unit: "PERIODIC INSPECTION & TESTING REQUIRED"',
        emergencyContact:
          'Emergency isolation instructions and electrical contractor contact details should be provided to client',
        restrictedAccess:
          'Consumer unit location must be readily accessible but secure from unauthorized access',
        labellingRequired:
          'All circuits must be clearly and permanently labeled at consumer unit using durable labels',
      },
      ...
      projectSpecificNotes: [
        ...
        design.circuits.some((c) => c.afddRequired)
          ? 'AFDD protection specified where required by Amendment 2'
          : 'AFDDs assessed - not required for this installation',
        design.circuits.some((c) => c.rcdProtected)
          ? 'All socket outlets protected by 30mA RCD as per Regulation 411.3.3'
          : '',
        design.circuits.some((c) => c.specialLocationCompliance?.isSpecialLocation)
          ? 'Special location requirements assessed and protection measures specified'
          : '',
      ...
      calculationMethods: {
        designCurrent:
          'Ib = Power(W) / Voltage(V) for single-phase circuits. For three-phase: Ib = Power(W) / (√3 × Voltage(V))',
        voltageDrop:
          'Vd = (mV/A/m × Ib × L) / 1000, where L is circuit length in meters. Tabulated values from BS 7671 Appendix 4.',
        earthFaultLoop:
          'Zs = Ze + R1 + R2, where Ze is external impedance, R1 is live conductor resistance, R2 is CPC resistance',
        cableSelection:
          'Selection criteria: In ≥ Ib (nominal rating ≥ design current) and Iz ≥ In (cable capacity ≥ protection rating) with correction factors Ca, Cg, Ci applied',
        maxZsCalculation:
          'Maximum Zs values from BS 7671 Appendix 3 based on protective device type, curve, and disconnection time requirement (0.4s for socket circuits, 5s for distribution)',
      },

      standardsReferences: {
        mainStandard:
          'BS 7671:2018+A3:2024 Requirements for Electrical Installations (IET Wiring Regulations 18th Edition)',
        supportingDocuments: [
      ...
      qualityAssurance: {
        designReview:
          'Design reviewed for compliance with BS 7671:2018+A3:2024 and all applicable amendments',
        calculations:
          'All electrical calculations verified using industry-standard methods and BS 7671 tabulated data',
        standards:
          'Design meets requirements of current edition of BS 7671 and all relevant British/European Standards',
        competence:
          'Design completed by qualified and competent electrical designer with up-to-date BS 7671 certification',
      },
  ...
  const handleProceedToInstaller = () => {
    try {
      ...
      const context = {
        design: design,
        selectedCircuits: selectedCircuitsForInstall
          .map((idx) => {
      ...
      toast.success(
        `Opening Installation Specialist with ${selectedCircuitsForInstall.length} circuit(s)`
      );
    } catch (error) {
  ...
  const handleSendToEIC = async () => {
    ...
    try {
      ...
      const scheduleCircuits = design.circuits.map((circuit, idx) => ({
        ...
        // Zs values - FULL
        zs: includeExpectedResults ? circuit.calculations?.zs?.toFixed(2) || '' : '',
        maxZs:
          circuit.calculations?.maxZs?.toFixed(2) ||
        ...
        insulationTestVoltage:
          circuit.expectedTests?.insulationResistance?.testVoltage || '500V DC',
        insulationResistance:
        ...
        circuitTopology:
          circuit.circuitTopology || (circuit.loadType === 'ring' ? 'ring' : 'radial'),
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const transformedDesign = {
        ...
        complianceChecks: {
          allCircuitsCompliant: design.circuits?.every(
            (c) =>
          ...
          criticalIssues: design.circuits?.filter(
            (c) =>
              !c.calculations?.voltageDrop?.compliant || c.calculations?.zs >= c.calculations?.maxZs
          ).length,
        },
  ...
  if (selectedCircuit >= design.circuits.length && selectedCircuit !== 0) {
    console.warn(
      `Selected circuit index ${selectedCircuit} is out of bounds (total: ${design.circuits.length}), resetting to 0`
    );
    setSelectedCircuit(0);
  }
          ...
          <DialogContent className="max-w-[calc(100vw-2rem)] rounded-xl">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="text-sm text-muted-foreground">
                <p>
                  <strong>{design.circuits?.length || 0} circuit(s)</strong> will be added
                  including:
                </p>
              ...
              <div className="flex items-start gap-3 p-3 bg-orange-500/10 border border-orange-500/30 rounded-lg">
                ...
                <div className="flex-1">
                  <label
                    htmlFor="includeExpectedResultsMobile"
                  ...
                  <p className="text-xs text-orange-400 mt-1 flex items-start gap-1">
                    ...
                    <span>
                      All calculated values (R1+R2, Zs) must be verified on-site during testing
                    </span>
                  </p>
                </div>
              </div>
            ...
            <DialogFooter className="flex-col gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => setShowSendToEICDialog(false)}
              ...
              <AccordionItem value="diversity" className="border-none">
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="text-left">
                      <h3 className="text-sm sm:text-base font-semibold text-foreground">
                        Load Diversity Breakdown
                      </h3>
                      <p className="text-xs sm:text-sm text-foreground/70 mt-0.5">
                        {fmt(design.diversityBreakdown.totalConnectedLoad / 1000, 1)}kW →{' '}
                        {fmt(design.diversityBreakdown.diversifiedLoad / 1000, 1)}kW
                ...
                <AccordionContent className="pt-4">
                  <div className="space-y-4">
                    ...
                    {design.diversityBreakdown.circuitDiversity &&
                      design.diversityBreakdown.circuitDiversity.length > 0 && (
                              ...
                              <div key={idx} className="py-2 px-3 bg-background/30 rounded-lg">
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  <Badge variant="outline" className="text-xs">
                                    {fmt(cd.diversityFactorApplied * 100, 0)}%
                                  </Badge>
                                </div>
                                <div className="text-xs text-foreground/60">
                                  {fmt(cd.connectedLoad, 1)}kW × {cd.diversityFactorApplied} ={' '}
                                  {fmt(cd.diversifiedLoad, 1)}kW
                                </div>
                                <div className="text-xs text-foreground/50 italic mt-1">
                                  {cd.justification}
                                </div>
                              </div>
                            ))}
                    ...
                    <div className="py-3 px-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                      ...
                      <p className="text-sm text-foreground/80 leading-relaxed">
                        {design.diversityBreakdown.reasoning}
                      </p>
                      <Badge
                        variant="outline"
                        className="mt-2 text-xs text-blue-400 border-blue-400/30"
                      >
              ...
              <table className="w-full text-xs sm:text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left py-2 px-1.5 sm:px-2 font-semibold text-xs sm:text-sm">
                      Circuit
                    </th>
                    <th className="text-left py-2 px-1.5 sm:px-2 font-semibold text-xs sm:text-sm">
                      Name
                        ...
                        <td className="py-3 px-2 text-left">
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {circuit.calculations?.Ib?.toFixed(1)}A
                            </div>
                          </div>
                        </td>
                        <td className="py-3 px-2 text-left">
                          ...
                          {hasCompliance ? (
                            ...
                            <Badge variant={statusVariant} className="text-xs gap-1">
                              ...
                              {(circuit as any).validationIssues?.length > 0 && (
                                <span className="ml-1">
                                  ({(circuit as any).validationIssues.length} issue
                                  {(circuit as any).validationIssues.length > 1 ? 's' : ''})
                                </span>
                              )}
                            </Badge>
                          )}
          ...
          <Card className="p-6">
            <div className="space-y-6">
                  ...
                  <AlertDescription>
                    <div className="space-y-3 mt-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge
                          variant="outline"
                          className="text-orange-400 border-orange-400/30 font-mono text-xs"
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                      <div className="flex items-center gap-3 py-2 px-3 bg-background/30 rounded-lg">
                        ...
                        <span className="font-medium text-foreground">
                          {currentCircuit.loadPower}W ({fmt(currentCircuit.loadPower / 1000, 1)}kW)
                        </span>
                      </div>
                      {currentCircuit.socketCount && (
                        <div className="flex items-center gap-3 py-2 px-3 bg-background/30 rounded-lg">
                          ...
                          <span className="font-medium text-foreground">
                            {currentCircuit.socketCount} outlets
                          </span>
                        </div>
                      )}
                      <div className="flex items-center gap-3 py-2 px-3 bg-background/30 rounded-lg">
                        <span className="text-sm text-foreground/80 min-w-[120px]">
                          Design Current (Ib):
                        </span>
                        <span className="font-medium text-foreground">
                          {fmt(currentCircuit.calculations?.Ib, 1)}A
                        </span>
                      </div>
                      <div className="flex items-center gap-3 py-2 px-3 bg-background/30 rounded-lg">
                        <span className="text-sm text-foreground/80 min-w-[120px]">Phases:</span>
                        <span className="font-medium text-foreground capitalize">
                          {currentCircuit.phases}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-3 sm:p-4 space-y-2.5 sm:space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="flex items-center justify-between sm:justify-start sm:gap-4 py-2 px-3 bg-background/30 rounded-lg">
                        ...
                        <span className="font-medium text-foreground">
                          {currentCircuit.protectionDevice?.type ?? 'MCB'}
                        </span>
                      </div>
                      <div className="flex items-center justify-between sm:justify-start sm:gap-4 py-2 px-3 bg-background/30 rounded-lg">
                        <span className="text-sm text-foreground/80">Rating:</span>
                        <span className="font-medium text-foreground">
                          {currentCircuit.protectionDevice?.rating ?? 'N/A'}A Type{' '}
                          {currentCircuit.protectionDevice?.curve ?? 'B'}
                        </span>
                      </div>
                      <div className="flex items-center justify-between sm:justify-start sm:gap-4 py-2 px-3 bg-background/30 rounded-lg">
                        <span className="text-sm text-foreground/80">Breaking Capacity:</span>
                        <Badge variant="secondary" className="font-medium">
                          {currentCircuit.protectionDevice?.kaRating ?? 6}kA
                        </Badge>
                      </div>
                    </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                  <div
                    className={`p-3 rounded-lg ${currentCircuit.calculations?.voltageDrop?.compliant ? 'bg-green-500/10' : 'bg-red-500/10'}`}
                  >
                    <div className="flex items-center justify-between mb-1">
                    ...
                    <p className="text-xs text-foreground/60">
                      {fmt(currentCircuit.calculations?.voltageDrop?.volts, 2)}V (Max:{' '}
                      {currentCircuit.calculations?.voltageDrop?.limit || 3}%)
                    </p>
                  </div>
                  ...
                  {(() => {
                    const zsValue =
                      currentCircuit.expectedTests?.zs?.expected ??
                      currentCircuit.calculations?.zs ??
                      0;
                    const maxZs =
                      currentCircuit.expectedTests?.zs?.maxPermitted ??
                      currentCircuit.calculations?.maxZs ??
                      999;
                    const zsCompliant =
                      currentCircuit.expectedTests?.zs?.compliant ?? zsValue <= maxZs;

                    return (
                      <div
                        className={`p-3 rounded-lg ${zsCompliant ? 'bg-green-500/10' : 'bg-red-500/10'}`}
                      >
                        <div className="flex items-center justify-between mb-1">
                        ...
                        <p className="text-xs text-foreground/60">Max: {fmt(maxZs, 3)}Ω</p>
                      </div>
                    );
                  })()}
                </div>
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    ...
                    <p className="text-foreground/70">
                      {currentCircuit.justifications?.cableSize ?? 'No justification provided'}
                    </p>
                  </div>
                  <div>
                    <p className="font-medium text-foreground mb-1">Protection:</p>
                    <p className="text-foreground/70">
                      {currentCircuit.justifications?.protection ?? 'No justification provided'}
                    </p>
                  </div>
                ...
                <div className="space-y-3 bg-card/50 p-4 rounded-lg border border-primary/10">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary">
                      {fmt(currentCircuit.diversityFactor * 100, 0)}%
                    </Badge>
                  </div>
                  <p className="text-sm text-foreground/70">
                    {currentCircuit.diversityJustification}
                  </p>
                </div>
              )}
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="text-lg font-bold text-foreground">
                        {fmt(currentCircuit.faultCurrentAnalysis?.psccAtCircuit, 2)}kA
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-foreground/60 mb-1">Device Breaking Capacity</p>
                      <p className="text-lg font-bold text-foreground">
                        {currentCircuit.faultCurrentAnalysis.deviceBreakingCapacity}kA
                      </p>
                    </div>
                  </div>
                  <div
                    className={`flex items-center gap-2 p-2 rounded ${currentCircuit.faultCurrentAnalysis.compliant ? 'bg-green-500/10 text-green-600' : 'bg-red-500/10 text-red-600'}`}
                  >
                    {currentCircuit.faultCurrentAnalysis.compliant ? (
                      <CheckCircle2 className="h-4 w-4" />
                    ) : (
                      <AlertTriangle className="h-4 w-4" />
                    )}
                    <span className="text-sm font-medium">
                      {currentCircuit.faultCurrentAnalysis.marginOfSafety}
                    </span>
                  </div>
                  <p className="text-xs text-foreground/60">
                    {currentCircuit.faultCurrentAnalysis.regulation}
                  </p>
                </div>
              )}
              ...
              {currentCircuit.earthingRequirements && (
                <div className="space-y-3 bg-card/50 p-4 rounded-lg border border-primary/10">
                  ...
                  <div className="grid md:grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-foreground">
                        {currentCircuit.earthingRequirements.cpcSize}
                      </p>
                    </div>
                    <div>
                      <p className="text-foreground/60">Supplementary Bonding</p>
                      <p className="font-medium text-foreground">
                        {currentCircuit.earthingRequirements.supplementaryBonding
                          ? 'Required'
                          : 'Not Required'}
                      </p>
                    </div>
                  ...
                  <p className="text-sm text-foreground/70">
                    {currentCircuit.earthingRequirements.justification}
                  </p>
                  <Badge variant="outline" className="text-xs">
                    {currentCircuit.earthingRequirements.regulation}
                  </Badge>
                </div>
              )}
                ...
                <div className="space-y-3 bg-card/50 p-4 rounded-lg border border-primary/10">
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center">
                    <div className="bg-primary/10 p-2 rounded">
                      ...
                      <p className="text-lg font-bold text-foreground">
                        {fmt(currentCircuit.deratingFactors?.Ca, 2)}
                      </p>
                    </div>
                    <div className="bg-primary/10 p-2 rounded">
                      <p className="text-xs text-foreground/60">Cg</p>
                      <p className="text-lg font-bold text-foreground">
                        {fmt(currentCircuit.deratingFactors?.Cg, 2)}
                      </p>
                    </div>
                    <div className="bg-primary/10 p-2 rounded">
                      <p className="text-xs text-foreground/60">Ci</p>
                      <p className="text-lg font-bold text-foreground">
                        {fmt(currentCircuit.deratingFactors?.Ci, 2)}
                      </p>
                    </div>
                    <div className="bg-primary/5 p-2 rounded border border-primary/30">
                      <p className="text-xs text-foreground/60">Overall</p>
                      <p className="text-lg font-bold text-primary">
                        {fmt(currentCircuit.deratingFactors?.overall, 2)}
                      </p>
                    </div>
                  </div>
                  <p className="text-sm text-foreground/70">
                    {currentCircuit.deratingFactors.explanation}
                  </p>
                  <p className="text-xs text-foreground/60">
                    {currentCircuit.deratingFactors.tableReferences}
                  </p>
                </div>
              )}
              ...
              {currentCircuit.specialLocationCompliance?.isSpecialLocation && (
                <div className="space-y-3 bg-orange-500/10 border border-orange-500/30 p-4 rounded-lg">
                  ...
                  {currentCircuit.specialLocationCompliance.zonesApplicable && (
                    <p className="text-sm text-foreground text-left pl-6">
                      {currentCircuit.specialLocationCompliance.zonesApplicable}
                    </p>
                  )}
              ...
              {currentCircuit.expectedTests && (
                <div className="space-y-4 bg-card/50 p-4 rounded-lg border border-primary/10">
                  ...
                  <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-foreground/50">
                              BS 7671:2018+A3:2024 Reg 643.2.2
                            </p>
                          </div>
                        </div>
                    ...
                    <div
                      className={`bg-gradient-to-br p-4 rounded-lg border hover:border-opacity-60 transition-colors ${
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-foreground/50">
                              {currentCircuit.expectedTests.zs?.regulation}
                            </p>
                          </div>
                          {currentCircuit.expectedTests.zs?.compliant ? (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-foreground/50">
                              BS 7671:2018+A3:2024 Reg 643.3.1
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-foreground/50 mb-2">
                            BS 7671:2018+A3:2024 Reg 643.4
                          </p>
                          <p className="text-sm text-foreground/90 leading-relaxed">
                            Verify correct polarity - all single-pole switching and protective
                            devices in phase conductor only
                          </p>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-foreground/60">
                                {currentCircuit.expectedTests.rcd?.regulation}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            <Badge className="bg-cyan-500/20 text-foreground border-cyan-500/30 hover:bg-cyan-500/30">
                              Max Trip: &lt;{currentCircuit.expectedTests.rcd?.maxTripTimeMs}ms at
                              1×IΔn
                            </Badge>
                          </div>
                        </div>
                ...
                <div className="space-y-2 bg-orange-500/10 border border-orange-500/20 p-4 rounded-lg">
                  ...
                  <ul className="space-y-1">
                    {currentCircuit.warnings.map((warning, idx) => (
                      <li
                        key={idx}
                        className="text-sm text-foreground text-left flex items-start gap-2"
                      >
                        <span className="flex-shrink-0">•</span>
                        <span>{warning}</span>
                      </li>
            ...
            <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
              ...
              <div className="space-y-2 max-h-[50vh] overflow-y-auto">
                {design.circuits?.map((circuit, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-2.5 sm:gap-3 p-2.5 sm:p-3 rounded-lg border border-border/50 hover:bg-accent/5 touch-manipulation min-h-[44px]"
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        if (checked) {
                          ...
                          setSelectedCircuitsForInstall(
                            selectedCircuitsForInstall.filter((i) => i !== idx)
                          );
                        }
                      }}
                    />
                    <div className="flex-1">
                      ...
                      <div className="text-[10px] sm:text-xs text-muted-foreground mt-1">
                        ...
                        {circuit.protectionDevice.rating}A {circuit.protectionDevice.type} •{' '}
              ...
              <DialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0">
                ...
                <Button
                  ...
                  className="w-full sm:w-auto min-h-[44px] touch-manipulation"
                >
                  Create Method ({selectedCircuitsForInstall.length} circuit
                  {selectedCircuitsForInstall.length > 1 ? 's' : ''})
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4 py-2">
                ...
                <div className="flex items-start gap-3 p-3 bg-orange-500/10 border border-orange-500/30 rounded-lg">
                  ...
                  <div className="flex-1">
                    <label
                      htmlFor="includeExpectedResults"
                      className="text-sm font-medium cursor-pointer"
                    >
                      Include expected test results
                    </label>
