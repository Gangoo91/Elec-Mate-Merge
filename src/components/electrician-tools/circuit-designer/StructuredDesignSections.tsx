...
export const StructuredDesignSections = ({ sections }: StructuredDesignSectionsProps) => {
  const sectionOrder: SectionConfig[] = [
    ...
    {
      key: 'cableSelectionBreakdown',
      title: '3. Cable Selection & Calculation Breakdown',
      icon: Cable,
    },
    ...
    {
      key: 'testingCommissioningGuidance',
      title: '7. Testing & Commissioning',
      icon: ClipboardCheck,
    },
    ...
    <div className="space-y-4">
      {sectionOrder.map(({ key, title, icon: Icon }) => {
        ...
        return (
          <Card
            key={key}
            className="bg-card border-elec-yellow/20 hover:border-elec-yellow/30 transition-all"
          >
