    ...
    <section className={`space-y-4 sm:space-y-5 ${className}`}>
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-white">Featured {categoryName}</h2>
            <p className="text-xs text-muted-foreground">Top picks from trusted suppliers</p>
          </div>
        </div>
      ...
      <Carousel className="w-full">
        <CarouselContent className="-ml-2 sm:-ml-4">
          {top6Tools.map((tool, index) => (
            ...
            >
              <PremiumToolCard item={tool} variant="compact" />
            </CarouselItem>
          ))}
        </CarouselContent>
