...
const BatchToolsRefreshButton: React.FC<BatchToolsRefreshButtonProps> = ({
  ...
  const pollForCacheUpdate = async (expectedCount: number, maxAttempts = 5): Promise<boolean> => {
    console.log(
      `🔍 [UPDATE-FLOW] Polling for cache update (expecting ${expectedCount} products)...`
    );
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (!error && data) {
        ...
        console.log(
          `📊 [UPDATE-FLOW] Poll ${attempt}/${maxAttempts}: ${totalInCache} products in cache`
        );
    ...
    try {
      ...
      if (toolsCount === 0) {
        ...
        toast({
          title: 'Warning',
          description:
            'No products found. The data is being collected. Please try again in a few minutes.',
          variant: 'destructive',
          duration: 5000,
        });
      } else {
