...
import { Calendar, Clock, Star, ExternalLink, ChevronLeft, ChevronRight, Truck, ShoppingCart } from "lucide-react";
...
const ToolsFeaturedCarousel = ({ tools, className }: ToolsFeaturedCarouselProps) => {
  // Show up to 6 featured tool deals
  const featuredTools = tools.filter(tool => tool.isOnSale || tool.salePrice).slice(0, 6);
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-white/80">
          Best offers from top electrical suppliers
        </p>
      </div>
              ...
              <div className="bg-gradient-to-br from-white/10 via-white/5 to-transparent rounded-xl border border-white/10 overflow-hidden group hover:border-elec-yellow/30 transition-all duration-300 hover:shadow-xl hover:shadow-elec-yellow/10 hover:scale-[1.02] h-full">
                ...
                <div className="relative h-40 sm:h-48 overflow-hidden">
                  ...
                  <div className="absolute bottom-3 left-3">
                    <div className="flex items-center gap-1 text-xs">
                      <Truck className={cn("h-3 w-3", getStockStatusColor(tool.stockStatus))} />
                ...
                <div className="p-4 sm:p-5 space-y-3 flex flex-col h-[calc(100%-10rem)] sm:h-[calc(100%-12rem)]">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/10">
                    ...
                    <Button
                      size="sm"
                      className="h-8 px-3 bg-elec-yellow text-black hover:bg-elec-yellow/90 group/btn"
                      onClick={() => tool.productUrl && window.open(tool.productUrl, '_blank')}
                    >
                      <span className="text-xs font-medium">Buy Now</span>
                      <ShoppingCart className="h-3 w-3 ml-1 transition-transform group-hover/btn:scale-110" />
                    </Button>
                  </div>
                </div>
              </div>
      ...
      <div className="md:hidden flex items-center justify-center gap-2 text-xs text-white/60">
        ...
        <span>Swipe to browse more deals</span>
