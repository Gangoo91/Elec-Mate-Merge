...
const TopDiscountsStrip = ({ deals, maxDeals = 5 }: TopDiscountsStripProps) => {
  const topDeals = deals.filter((tool) => tool.isOnSale && tool.originalPrice).slice(0, maxDeals);
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scroll-smooth scrollbar-hide">
        {topDeals.map((tool, index) => {
          ...
          return (
            <Card
              key={tool.id || index}
              className="flex-shrink-0 w-64 bg-elec-card/40 border-elec-yellow/20 hover:border-elec-yellow/40 transition-all duration-200"
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <Badge
                        variant="secondary"
                        className="text-xs font-bold bg-elec-yellow/10 text-elec-yellow border-elec-yellow/30"
                      >
                        BEST PRICE
                      </Badge>
                      {savings && (
                    ...
                    <div className="text-xs text-text-muted mb-3">
                      ...
                      {tool.competitorCount && tool.competitorCount > 0 && (
                        <span className="block text-green-400">
                          Beats {tool.competitorCount} competitor
                          {tool.competitorCount > 1 ? 's' : ''}
                        </span>
                      )}
                    </div>
                    ...
                    >
                      <Button
                        size="sm"
                        variant="outline"
                        className="w-full bg-elec-yellow/10 border-elec-yellow/30 text-elec-yellow hover:bg-elec-yellow/20 text-xs"
                      >
                        View Deal
