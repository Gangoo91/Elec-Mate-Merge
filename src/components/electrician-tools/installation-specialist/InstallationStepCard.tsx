...
export const InstallationStepCard = ({
  ...
  const parseRegulationFromSafety = (
    safetyItem: string | SafetyNote
  ): { text: string; regulation?: string } => {
  ...
  return (
    ...
    >
      <div className={cn('p-6', isMobile && 'p-4')}>
                            ...
                            {isEditing ? (
                                ...
                                {editedHazards.map((item, idx) => (
                                  <div key={idx} className="flex gap-2">
                                    <Input
                                      value={item}
                                      onChange={(e) =>
                                        updateItem(setEditedHazards, idx, e.target.value)
                                      }
                              ...
                              <div className="flex flex-wrap gap-2">
                                {linkedHazards.map((hazard: string, idx: number) => (
                                  <Badge
                                    key={idx}
                                    className="bg-amber-500/20 text-amber-200 border-amber-500/40 font-medium"
                                  >
                                ...
                                {editedReferences.map((item, idx) => (
                                  <div key={idx} className="flex gap-2">
                                    <Input
                                      value={item}
                                      onChange={(e) =>
                                        updateItem(setEditedReferences, idx, e.target.value)
                                      }
                                ...
                                {editedSafety.map((item, idx) => (
                                  <div key={idx} className="flex gap-2">
                                    <Input
                                      value={item}
                                      onChange={(e) =>
                                        updateItem(setEditedSafety, idx, e.target.value)
                                      }
                              ...
                              <ul className="space-y-0 divide-y divide-border/20">
                                {(step.safety || []).map((note, i) => {
                                  ...
                                  return (
                                    <li
                                      key={i}
                                      className="flex items-start gap-3 text-sm py-3 first:pt-0 last:pb-0"
                                    >
                                      <ShieldAlert className="h-4 w-4 text-destructive mt-0.5 flex-shrink-0" />
                                      <div className="flex-1">
                                        <span className="text-foreground/90 leading-relaxed text-left">
                                          {parsed.text}
                                        </span>
                                        {parsed.regulation && (
                                ...
                                {editedTools.map((item, idx) => (
                                  <div key={idx} className="flex gap-2">
                                    <Input
                                      value={item}
                                      onChange={(e) =>
                                        updateItem(setEditedTools, idx, e.target.value)
                                      }
                                ...
                                {editedMaterials.map((item, idx) => (
                                  <div key={idx} className="flex gap-2">
                                    <Input
                                      value={item}
                                      onChange={(e) =>
                                        updateItem(setEditedMaterials, idx, e.target.value)
                                      }
                                ...
                                {editedCheckpoints.map((item, idx) => (
                                  <div key={idx} className="flex gap-2">
                                    <Input
                                      value={item}
                                      onChange={(e) =>
                                        updateItem(setEditedCheckpoints, idx, e.target.value)
                                      }
                                ...
                                {editedQualifications.map((item, idx) => (
                                  <div key={idx} className="flex gap-2">
                                    <Input
                                      value={item}
                                      onChange={(e) =>
                                        updateItem(setEditedQualifications, idx, e.target.value)
                                      }
        ...
        <div className={cn('flex justify-end gap-2 mt-6 pt-6 border-t', isMobile && 'flex-col')}>
          {!isEditing && (
            <>
              <Button
                variant="ghost"
