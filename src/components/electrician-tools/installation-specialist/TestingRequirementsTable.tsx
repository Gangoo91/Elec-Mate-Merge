...
export const TestingRequirementsTable = ({
  testingRequirements,
}: TestingRequirementsTableProps) => {
      ...
      <CardContent className="p-5 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              {testingRequirements.length} test{testingRequirements.length !== 1 ? 's' : ''}{' '}
              ...
              {testingRequirements.map((test, index) => (
                ...
                >
                  ...
                  <td className="py-3 px-2">
                    <Badge
                      variant="outline"
                      className="bg-green-500/10 border-green-500/30 text-foreground text-xs font-mono"
                    >
                      {test.regulation}
                    </Badge>
                  </td>
          ...
          {testingRequirements.map((test, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <Badge
                  variant="outline"
                  className="bg-green-500/10 border-green-500/30 text-green-400 text-xs shrink-0"
                >
                  Test {index + 1}
                </Badge>
              </div>
        ...
        <div className="mt-5 pt-4 border-t border-border/50">
          <p className="text-xs text-muted-foreground">
            <strong>Note:</strong> All testing must be conducted in accordance with BS
            7671:2018+A3:2024 Part 6 (Inspection & Testing). Record all test results on the
