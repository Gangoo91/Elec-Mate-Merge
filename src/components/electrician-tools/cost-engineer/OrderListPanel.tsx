        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {orderList.estimatedDelivery && (
            <div className="p-4 rounded-xl bg-elec-dark/40 border-2 border-elec-yellow/10">
              ...
              <p className="mobile-small-text font-bold text-foreground leading-relaxed">
                {orderList.estimatedDelivery}
              </p>
            </div>
          )}
            ...
            >
              ...
              <div className="flex items-center justify-between pb-4 border-b border-elec-yellow/10">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="border-elec-yellow/30 text-elec-yellow mobile-small-text"
                  >
                    {data.items?.length || 0} items
                  </Badge>
                </div>
                <p className="mobile-text font-bold text-elec-yellow tabular-nums">
                ...
                {(data.items || []).map((item, idx) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="mobile-text font-semibold text-foreground leading-snug">
                        {item.description}
                      </p>
          ...
          <div className="p-4 rounded-xl bg-orange-500/10 border-2 border-orange-500/30">
            ...
            <ul className="space-y-2">
              {orderList.notes.map((note, idx) => (
                <li
                  key={idx}
                  className="mobile-small-text text-elec-light flex items-start gap-2 leading-relaxed font-medium"
                >
