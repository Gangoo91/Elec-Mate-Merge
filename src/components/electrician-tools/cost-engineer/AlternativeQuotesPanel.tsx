...
const AlternativeQuotesPanel = ({ alternatives }: AlternativeQuotesPanelProps) => {
  const [selectedTier, setSelectedTier] = useState<'budget' | 'standard' | 'premium'>(
    ...
    <Card className="border-elec-yellow/20">
      ...
      <CardContent className="space-y-4">
            ...
            return (
              ...
              >
                ...
                <div className="text-center">
                  <p
                    className={`mobile-small-text font-bold ${isSelected ? 'text-foreground' : 'text-elec-light'}`}
                  >
                    {label}
                  </p>
                  <p
                    className={`mobile-text font-bold mt-1 tabular-nums ${isSelected ? color : 'text-foreground'}`}
                  >
        ...
        <div className="space-y-3">
          {tiers
            .filter((t) => t.key === selectedTier)
            .map(({ key, color, bgColor, borderColor }) => {
              const tierData = alternatives[key];
              const isRecommended = alternatives.recommended === key;
              ...
              return (
                <div
                  key={key}
                  className={`p-5 sm:p-5 rounded-xl ${bgColor} border-2 ${borderColor}`}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="mobile-text font-bold text-foreground capitalize">
                          {key} Tier
                        </h3>
                        {isRecommended && (
                      ...
                      <p className="mobile-small-text text-elec-light font-medium leading-relaxed">
                      ...
                      {tierData.materialsTotal !== undefined && (
                        <div className="flex items-center justify-between">
                          ...
                          <span className="mobile-text font-bold text-foreground tabular-nums">
                            {formatCurrency(tierData.materialsTotal)}
                          </span>
                        </div>
                      )}
                      {tierData.labourTotal !== undefined && (
                        <div className="flex items-center justify-between">
                          ...
                          <span className="mobile-text font-bold text-foreground tabular-nums">
                            {formatCurrency(tierData.labourTotal)}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  {/* Tradeoffs */}
                  <div>
                    <p className="mobile-small-text font-bold text-elec-light mb-3">
                      What's included:
                    </p>
                    <ul className="space-y-2">
                      {tierData.tradeoffs.map((tradeoff, idx) => (
                        <li
                          key={idx}
                          className="mobile-small-text text-elec-light flex items-start gap-2 leading-relaxed"
                        >
        ...
        <div className="p-4 rounded-xl bg-elec-dark/40 border-2 border-elec-yellow/10">
          <p className="mobile-small-text text-elec-light leading-relaxed font-medium">
            💡 All tiers meet BS 7671:2018+A3:2024 compliance. The difference is in component
