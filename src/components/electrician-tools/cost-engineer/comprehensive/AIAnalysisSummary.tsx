          ...
          {sections.map((section, idx) => {
            if (section.type === 'header') {
              return (
                <h4
                  key={idx}
                  className="text-sm sm:text-base font-bold text-elec-yellow mt-4 mb-2 first:mt-0"
                >
                  {section.content}
                </h4>
              );
            ...
            if (section.type === 'calculation') {
              return (
                <div
                  key={idx}
                  className="bg-elec-yellow/10 border border-elec-yellow/20 rounded-xl p-4 font-mono text-xs sm:text-sm text-white"
                >
                  {section.content}
                </div>
              );
            ...
            if (section.type === 'citation') {
              return (
                <div
                  key={idx}
                  className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 text-xs sm:text-sm text-blue-300"
                >
                  📖 {section.content}
                </div>
              );
