        ...
        <div className="space-y-3 sm:hidden">
          {tasks.map((task, idx) => (
            <div key={idx} className="p-4 rounded-lg bg-background/50 border border-border/30">
              <div className="font-medium text-white text-base mb-3 leading-snug">{task.description}</div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="p-2.5 rounded bg-background/50">
                  <div className="text-xs text-white/70 mb-1">Hours</div>
                  <div className="font-mono text-white font-medium">{task.hours.toFixed(1)}h</div>
                </div>
                <div className="p-2.5 rounded bg-background/50">
                  <div className="text-xs text-white/70 mb-1">Rate</div>
                  <div className="font-mono text-white font-medium">£{task.rate?.toFixed(0)}/hr</div>
                </div>
                <div className="p-2.5 rounded bg-background/50">
              ...
              {(task.electricianHours || task.apprenticeHours) && (
                <div className="text-sm text-white/70 mt-3 pt-3 border-t border-border/30">
          ...
          <div className="p-4 rounded-lg bg-elec-yellow/20 border border-elec-yellow/30">
            <div className="flex justify-between items-center">
              <span className="font-bold text-white">Total Labour</span>
              <div className="text-right">
                <div className="text-sm text-white/70">{totalHours.toFixed(1)} hours</div>
