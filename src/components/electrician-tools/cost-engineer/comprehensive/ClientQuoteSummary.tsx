...
const ClientQuoteSummary = ({
  ...
  const getTierBadge = (tier: string) => {
    switch (tier) {
      case 'sparse':
        return (
          <Badge className="bg-blue-500/20 text-blue-500 border-blue-500/30">Work Sparse</Badge>
        );
      case 'busy':
        return (
          <Badge className="bg-green-500/20 text-green-500 border-green-500/30">Busy Period</Badge>
        );
      default:
        return (
          <Badge className="bg-elec-yellow/30 text-elec-yellow border-elec-yellow/50">Normal</Badge>
        );
    }
  };
    ...
    <Card className="border-0 sm:border border-elec-yellow/30 rounded-none sm:rounded-xl bg-gradient-to-br from-elec-yellow/5 to-background">
      <CardHeader className="px-4 py-4 sm:px-6 sm:py-5">
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl sm:text-xl font-bold text-foreground">
      ...
      <CardContent className="px-4 pb-5 sm:px-6 sm:pb-6">
        <div className="space-y-6">
          ...
          <div className="text-center p-6 rounded-lg bg-elec-yellow/10 border border-elec-yellow/30">
            <div className="text-base sm:text-sm text-foreground font-medium mb-1">
              Recommended Quote
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-background/20">
              ...
              <div className="text-2xl sm:text-xl font-bold text-foreground">
                £{breakEven.toFixed(2)}
              </div>
            </div>
            ...
            <div className="p-4 rounded-lg bg-background/20">
              ...
              <div className="text-2xl sm:text-xl font-bold text-green-500">
                {margin.toFixed(1)}%
              </div>
            </div>
