...
const QuickMetricsCard = ({ complexity, confidence, riskAssessment }: QuickMetricsCardProps) => {
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      {complexity && (
        ...
        >
          <div
            className={`absolute top-0 right-0 w-16 h-16 ${complexityColors?.bg} rounded-full blur-2xl -mr-4 -mt-4 opacity-50`}
          />
          <div className="relative">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span
                className={`text-xs font-semibold uppercase tracking-wide ${complexityColors?.text} opacity-80`}
              >
                Complexity
              </span>
            </div>
      ...
      {riskAssessment && (
        ...
        >
          <div
            className={`absolute top-0 right-0 w-16 h-16 ${highRisks > 0 ? 'bg-red-500/10' : 'bg-emerald-500/10'} rounded-full blur-2xl -mr-4 -mt-4 opacity-50`}
          />
          <div className="relative">
            <div className="flex items-center gap-2 mb-2">
              <div
                className={`p-1.5 rounded-lg ${highRisks > 0 ? 'bg-red-500/20' : 'bg-emerald-500/20'}`}
              >
                <AlertTriangle
                  className={`h-4 w-4 ${highRisks > 0 ? 'text-red-400' : 'text-emerald-400'}`}
                />
              </div>
              <span
                className={`text-xs font-semibold uppercase tracking-wide ${highRisks > 0 ? 'text-red-300/80' : 'text-emerald-300/80'}`}
              >
                Risk Level
              </span>
            </div>
            <div
              className={`text-2xl font-black tabular-nums ${highRisks > 0 ? 'text-red-400' : 'text-emerald-400'}`}
            >
