...
const CostStatCard = ({ icon: Icon, label, value, subtitle, iconColor = 'text-elec-yellow', trend }: CostStatCardProps) => {
  return (
    <Card className="border-elec-yellow/20 bg-elec-card/50 hover:border-elec-yellow/40 transition-all hover:scale-[1.02]">
      <CardContent className="p-4 sm:p-6">
        <div className="flex items-start justify-between mb-3">
          ...
          {trend && (
            <div className={`text-xs font-semibold ${trend === 'up' ? 'text-green-500' : trend === 'down' ? 'text-red-500' : 'text-muted-foreground'}`}>
              {trend === 'up' ? '↑' : trend === 'down' ? '↓' : '—'}
            </div>
          )}
        </div>
