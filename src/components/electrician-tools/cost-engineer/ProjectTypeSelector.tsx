...
const ProjectTypeSelector = ({
  ...
  const [expandedType, setExpandedType] = useState<'domestic' | 'commercial' | 'industrial' | null>(
    null
  );
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 w-full">
                <Icon
                  className={`h-5 w-5 shrink-0 ${isSelected ? 'text-elec-dark' : 'text-elec-yellow'}`}
                />
              ...
              <span
                className={`text-xs leading-relaxed line-clamp-2 whitespace-normal ${isSelected ? 'text-elec-dark/70' : 'text-foreground'}`}
              >
                {type.description}
              </span>
              ...
              {/* Show/Hide indicator */}
              <span
                className={`text-xs font-medium whitespace-nowrap ${isSelected ? 'text-elec-dark/60' : 'text-elec-yellow/70'}`}
              >
              ...
              {PROJECT_TYPES.find((t) => t.id === expandedType)?.examples.map((example, idx) => (
                ...
                >
                  ...
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <p className="text-sm leading-relaxed line-clamp-4 flex-1">{example}</p>
                  </div>
