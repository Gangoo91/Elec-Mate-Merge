      ...
      {((status === 'complete' && !showCelebration && outputData) || savedOutputData) && (
        <HealthSafetyResults
          data={{
            ...
            projectName:
              job?.project_info?.projectName || (outputData || savedOutputData)?.projectName,
            location: job?.project_info?.location || (outputData || savedOutputData)?.location,
            clientName:
              job?.project_info?.clientName || (outputData || savedOutputData)?.clientName,
            workType: job?.work_type || (outputData || savedOutputData)?.workType,
            assessmentDate: new Date().toISOString().split('T')[0],
            reviewDate: new Date(Date.now() + 6 * 30 * 24 * 60 * 60 * 1000)
              .toISOString()
              .split('T')[0],
          }}
