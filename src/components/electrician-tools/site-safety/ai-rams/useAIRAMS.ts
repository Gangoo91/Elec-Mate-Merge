...
export function useAIRAMS(): UseAIRAMSReturn {
  ...
  const animateProgress = useCallback(
    (targetProgress: number, duration: number = 800) => {
  ...
  const saveToDatabase = useCallback(
    async (isAutosave = false) => {
      ...
      const extractedHazardCount =
        (ramsData as any)._extractedHazardCount || ramsData.risks?.length || 0;
      const riskCount = ramsData.risks?.length || 0;
      const hazardLoss =
        extractedHazardCount > 0 ? (extractedHazardCount - riskCount) / extractedHazardCount : 0;
      ...
      try {
        ...
        if (savedDocId && !isAutosave) {
          ...
          if (!verifyError && savedDoc) {
            ...
            const postSaveLoss =
              expectedRiskCount > 0 ? (expectedRiskCount - savedRiskCount) / expectedRiskCount : 0;
  ...
  const simulateSubStepProgress = useCallback(
    (agent: 'health-safety' | 'installer', subSteps: SubStep[]) => {
      ...
      const interval = setInterval(() => {
        ...
        const subStep: SubStep = {
          ...
          progress: Math.round(
            Math.min(
              ((currentProgress % (100 / subSteps.length)) / (100 / subSteps.length)) * 100,
              95
            )
          ),
        };
        ...
        setReasoningSteps((prev) =>
          prev.map((step) =>
            step.agent === agent && step.status === 'processing' ? { ...step, subStep } : step
          )
        );
      }, updateInterval);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error(`Timeout after ${AGENT_TIMEOUT_MS}ms`)),
            AGENT_TIMEOUT_MS
          )
        );
        ...
        if (!error && data?.success) {
          ...
          if (itemCount > 0) {
            console.log(
              `✅ ${functionName} succeeded: ${itemCount} items in ${attempt} attempt(s)`
            );
    ...
    try {
      ...
      setReasoningSteps((prev) =>
        prev.map((step) =>
          step.agent === 'health-safety'
            ? { ...step, status: 'processing', subStep: HEALTH_SAFETY_SUBSTEPS[0] }
            : step
        )
      );
      ...
      try {
        ...
        toast({
          ...
          description:
            'The AI risk assessment system is currently offline. Using basic hazard template instead.',
          variant: 'destructive',
          duration: 10000,
        });
        ...
        prev.map((step) =>
          step.agent === 'health-safety'
            ? {
                ...step,
                status: 'complete',
                reasoning: `Generated ${hsExtracted.hazards.length} hazards`,
                subStep: null,
              }
            : step
        )
      ...
      if (mutableHsError) {
        console.error('❌ Health & Safety network error:', {
          ...
          isTimeout:
            hsError.message?.toLowerCase().includes('timeout') ||
            hsError.message?.toLowerCase().includes('fetch'),
          // Full error details
        ...
        const is500Error =
          mutableHsError.message?.includes('500') ||
        ...
        if (is500Error) {
          ...
          toast({
            ...
            description:
              'AI generated risk assessment successfully but server crashed. This is a known bug being fixed.',
            variant: 'destructive',
            duration: 10000,
          });
        }
      ...
      console.log('🔍 RAW EDGE FUNCTION RESPONSE - DEEP INSPECTION:', {
        ...
        nestedPaths: {
          ...
          'hsData.structuredData.riskAssessment.hazards':
            hsData?.structuredData?.riskAssessment?.hazards?.length,
          'hsData.response.structuredData.riskAssessment.hazards':
            hsData?.response?.structuredData?.riskAssessment?.hazards?.length,
          'hsData.data.structuredData.riskAssessment.hazards':
            hsData?.data?.structuredData?.riskAssessment?.hazards?.length,
        },
      ...
      const extractHazards = (responseData: any): any[] => {
        ...
        if (responseData?.data?.hazards && Array.isArray(responseData.data.hazards)) {
          ...
          console.log(
            `✅ PHASE 1: Found ${hazards.length} hazards in data.hazards (standardized schema)`
          );
        }
        // Fallback to old structure for backward compatibility
        else if (
          responseData?.structuredData?.riskAssessment?.hazards &&
          Array.isArray(responseData.structuredData.riskAssessment.hazards)
        ) {
          hazards = responseData.structuredData.riskAssessment.hazards;
          console.log(
            `✅ PHASE 1: Found ${hazards.length} hazards in structuredData (backward compat)`
          );
        } else {
        ...
        const validatedHazards = hazards
          .map((h, idx) => {
        ...
        const retentionRate =
          hazards.length > 0 ? ((validatedHazards.length / hazards.length) * 100).toFixed(1) : '0';

        console.log(
          `✅ PHASE 1: Validated ${validatedHazards.length}/${hazards.length} hazards (${retentionRate}% retention)`,
          {
        ...
        if (hazards.length > 0 && validatedHazards.length / hazards.length < 0.9) {
          console.error(
            `🚨 PHASE 1: HIGH DATA LOSS - ${100 - parseFloat(retentionRate)}% hazards lost during validation`
          );
        }
      ...
      console.log('🔍 EXTRACTION DIAGNOSTICS:', {
        ...
        retentionRate:
          unwrappedData?.structuredData?.riskAssessment?.hazards?.length > 0
        ...
        samplesFiltered:
          unwrappedData?.structuredData?.riskAssessment?.hazards
      ...
      if (hsData?.metadata?.timingBreakdown) {
        ...
        console.log(
          `✅ Health & Safety complete: ${extractedHazards.length} hazards, ${hsTimeElapsed}s`
        );
        console.log(
          `📊 H&S Time Breakdown: RAG ${(ragMs / 1000).toFixed(1)}s (${ragPercent}%), AI ${(aiMs / 1000).toFixed(1)}s (${aiPercent}%), Total ${(totalMs / 1000).toFixed(1)}s`
        );
      ...
      if (extractedHazards.length >= 1) {
        ...
        if (mutableHsError) {
          ...
          if (isTimeout) {
            ...
            if (hasValidHazards) {
              console.log(
                `✅ Valid data received despite timeout - using ALL ${extractedHazards.length} AI hazards`
              );
              hsDataToUse = hsData;
              shouldUseFallback = false;
              mutableHsError = null;
            ...
            if (hasValidHazards) {
              console.warn(
                `⚠️ Network error but valid data received - using ALL ${extractedHazards.length} AI hazards, ignoring error`
              );
              hsDataToUse = hsData;
              shouldUseFallback = false;
              mutableHsError = null;
      ...
      if (shouldUseFallback) {
        console.error('🚨 CRITICAL: AI GENERATION FAILED - NO FALLBACK', {
          ...
          paths: {
            'structuredData.riskAssessment.hazards':
              hsData?.structuredData?.riskAssessment?.hazards?.length || 0,
            'response.structuredData.riskAssessment.hazards':
              hsData?.response?.structuredData?.riskAssessment?.hazards?.length || 0,
        ...
        toast({
          ...
          description:
            'Unable to generate risk assessment. Please try again or check your connection.',
          variant: 'destructive',
          duration: 10000,
        });
      ...
      const structuredData =
        hsDataToUse?.structuredData || hsDataToUse?.response?.structuredData || {};
          ...
          prev.map((step) =>
            step.agent === 'health-safety'
              ? {
                  ...step,
                  status: 'complete',
                  reasoning: `Generated ${hazardCount || 'multiple'} hazards and control measures`,
                  subStep: null,
                  timeElapsed: hsTimeElapsed,
                }
              : step
          )
        );
      ...
      setReasoningSteps((prev) =>
        prev.map((step) =>
          step.agent === 'installer'
            ? { ...step, status: 'processing', subStep: INSTALLER_SUBSTEPS[0] }
            : step
        )
      );
      ...
      if (installerData?.metadata?.timingBreakdown) {
        ...
        const ragPercent =
          timing.totalTime > 0 ? Math.round((timing.ragRetrieval / timing.totalTime) * 100) : 0;
        const aiPercent =
          timing.totalTime > 0 ? Math.round((timing.aiGeneration / timing.totalTime) * 100) : 0;
        const cachePercent =
          timing.totalTime > 0 ? Math.round((timing.cacheCheck / timing.totalTime) * 100) : 0;
        ...
        prev.map((step) =>
          step.agent === 'installer'
            ? {
                ...step,
                status: 'complete',
                reasoning: `Generated ${installerExtracted.steps.length} steps`,
                subStep: null,
              }
            : step
        )
      ...
      const installerStructuredData =
        installerData?.structuredData || installerData?.response?.structuredData || {};
      const methodSteps =
      ...
      if (installerData?.metadata?.timingBreakdown) {
        ...
        console.log(
          `📊 Installer Time Breakdown: RAG ${(ragMs / 1000).toFixed(1)}s (${ragPercent}%), AI ${(aiMs / 1000).toFixed(1)}s (${aiPercent}%), Total ${(totalMs / 1000).toFixed(1)}s`
        );
        ...
        prev.map((step) =>
          step.agent === 'installer'
            ? {
                ...step,
                status: 'complete',
                reasoning: `Generated ${stepsCount || 'multiple'} installation steps`,
                subStep: null,
                timeElapsed: installerTimeElapsed,
              }
            : step
        )
      );
      ...
      const hazardLossRate =
        preTransformCounts.hazards > 0
          ? ((preTransformCounts.hazards - postTransformCounts.risks) /
              preTransformCounts.hazards) *
            100
          : 0;
      const ppeLossRate =
        preTransformCounts.ppe > 0
          ? ((preTransformCounts.ppe - postTransformCounts.ppeDetails) / preTransformCounts.ppe) *
            100
          : 0;
      const emergencyLossRate =
        preTransformCounts.emergencyProcs > 0
          ? ((preTransformCounts.emergencyProcs - postTransformCounts.emergencyProcs) /
              preTransformCounts.emergencyProcs) *
            100
          : 0;
      const stepLossRate =
        preTransformCounts.steps > 0
          ? ((preTransformCounts.steps - postTransformCounts.steps) / preTransformCounts.steps) *
            100
          : 0;
