...
export const FormattedTextDisplay = ({
  ...
  const processInlineFormatting = (text: string): string => {
    ...
    text = text.replace(
      /\*\*(.+?)\*\*/g,
      '<strong class="text-elec-light font-semibold">$1</strong>'
    );
  ...
  if (isEditing) {
    return (
      <div className="space-y-2">
        ...
        />
        <Button onClick={() => setIsEditing(false)} variant="outline" size="sm" className="w-full">
          Done Editing
        </Button>
      </div>
    );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-card border border-primary/20 rounded-lg p-4 pr-16 sm:pr-4 min-h-[100px] relative group">
        {value ? (
          <div className="space-y-2">{renderFormattedContent(value)}</div>
        ) : (
