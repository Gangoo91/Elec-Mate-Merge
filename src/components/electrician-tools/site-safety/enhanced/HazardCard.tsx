...
export const HazardCard = memo(
  ({
    ...
    const [collapsedSections, setCollapsedSections] = useState<Set<string>>(
      new Set(['elimination', 'substitution', 'engineering', 'administrative', 'ppe'])
    );
    ...
    const getControlHierarchyColor = (type: string) => {
      switch (type) {
        ...
        default:
          return 'bg-elec-gray/10 border-border/30';
      }
    };
    ...
    const getControlHierarchyLabel = (type: string) => {
      switch (type) {
        case 'elimination':
          return 'ELIMINATION (Priority 1)';
        case 'substitution':
          return 'SUBSTITUTION (Priority 2)';
        case 'engineering':
          return 'ENGINEERING CONTROLS (Priority 3)';
        case 'administrative':
          return 'ADMINISTRATIVE CONTROLS (Priority 4)';
        case 'ppe':
          return 'PPE (Priority 5 - Last Resort)';
        default:
      ...
      return (
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <Badge className={`text-xs ${getRiskScoreColor(riskScore)}`}>{riskScore}</Badge>
      ...
      return (
        ...
        >
          ...
          >
              ...
              <div className="space-y-3 mt-4 pt-4 border-t border-border/30 animate-accordion-down">
                ...
                {hazard.controlMeasures && (
                  <div className="space-y-2">
                    {['elimination', 'substitution', 'engineering', 'administrative', 'ppe'].map(
                      (type) => {
                        ...
                        return (
                          <div
                            key={type}
                            className={cn('rounded border p-3', getControlHierarchyColor(type))}
                          >
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {hazard.emergencyProcedures.map((proc: string, idx: number) => (
                        <div
                          key={idx}
                          className="text-xs bg-red-500/5 p-2 rounded border border-red-500/20 flex items-start gap-2 text-left"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {hazard.guidanceNotes.map((note: string, idx: number) => (
                        <div
                          key={idx}
                          className="text-xs bg-indigo-500/5 p-2 rounded border border-indigo-500/20 flex items-start gap-2 text-left"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {hazard.inspectionChecks.map((check: string, idx: number) => (
                        <div
                          key={idx}
                          className="text-xs bg-cyan-500/5 p-2 rounded border border-cyan-500/20 flex items-start gap-2 text-left"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {hazard.trainingRequired.map((training: string, idx: number) => (
                        <div
                          key={idx}
                          className="text-xs bg-purple-500/5 p-2 rounded border border-purple-500/20 flex items-start gap-2 text-left"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {hazard.realWorldScenarios.map((scenario: string, idx: number) => (
                        <div
                          key={idx}
                          className="text-xs bg-amber-500/5 p-3 rounded border border-amber-500/20 text-left"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {hazard.hseGuidanceRef.map((ref: string, idx: number) => (
                        <div
                          key={idx}
                          className="text-xs bg-blue-500/5 p-2 rounded border border-blue-500/20 font-mono text-left"
                        >
                          {ref}
                        </div>
                      ))}
            ...
            {/* Tap to expand hint */}
            {!isExpanded && (
              <div className="text-center text-xs text-white mt-2">Tap to expand</div>
            )}
          </div>
        </MobileGestureHandler>
      );
          ...
          <div className="p-4 md:p-6">
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {isBookmarked && (
                    <Bookmark className="w-5 h-5 text-elec-yellow fill-elec-yellow" />
                  )}
                </div>
            ...
            {isExpanded && (
              <div className="space-y-4 pt-4 border-t border-border/30 animate-accordion-down">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {hazard.guidanceNotes.map((note: string, idx: number) => (
                        <div
                          key={idx}
                          className="flex items-start gap-2 text-sm bg-indigo-500/5 p-2 rounded border border-indigo-500/20"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {hazard.emergencyProcedures.map((proc: string, idx: number) => (
                        <div
                          key={idx}
                          className="flex items-start gap-3 bg-red-500/5 p-3 rounded border border-red-500/20"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {hazard.inspectionChecks.map((check: string, idx: number) => (
                        <div
                          key={idx}
                          className="flex items-start gap-2 text-sm bg-cyan-500/5 p-2 rounded border border-cyan-500/20"
                        >
                          <Check className="w-3 h-3 text-cyan-400 flex-shrink-0 mt-0.5" />
                    ...
                    <div className="flex flex-wrap gap-2">
                      {hazard.trainingRequired.map((training: string, idx: number) => (
                        <Badge
                          key={idx}
                          variant="outline"
                          className="text-xs bg-amber-500/5 border-amber-500/30"
                        >
                          {training}
                        </Badge>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {hazard.realWorldScenarios.map((scenario: string, idx: number) => (
                        <div
                          key={idx}
                          className="p-3 bg-pink-500/5 rounded border border-pink-500/20"
                        >
                ...
                {/* Related Hazards */}
                {(hazard as any).relatedHazards &&
                  Array.isArray((hazard as any).relatedHazards) &&
                  (hazard as any).relatedHazards.length > 0 && (
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-wide mb-2">
                        Related Hazards
                      </div>
            ...
            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border/30">
              ...
              >
                <Bookmark
                  className={cn(
                    'w-3 h-3 mr-1',
                    isBookmarked && 'fill-elec-yellow text-elec-yellow'
                  )}
                />
