...
export const AIMethodStatementGenerator: React.FC = () => {
  ...
  const { job, startPolling, stopPolling, progress, status, currentStep, methodData, error } =
  ...
  useEffect(() => {
    const checkForInProgressJobs = async () => {
      const hasActiveSession =
        sessionStorage.getItem('method-statement-generation-active') === 'true';
  ...
  useEffect(() => {
    const hasRealData = methodData && status === 'complete' && methodData.steps?.length > 0;
      ...
      {showCelebration && methodData && (
        <MethodStatementSuccess
          ...
          onViewResults={() => {
            setShowCelebration(false);
            document
              .getElementById('method-statement-results')
