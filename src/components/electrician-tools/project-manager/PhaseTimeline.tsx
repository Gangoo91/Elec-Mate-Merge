  ...
  const timelineData = useMemo(() => {
    ...
    return phases.map((phase, idx) => {
      ...
      return {
        ...
        isCritical:
          phase.criticalPath ||
          criticalPath.some((cp) => (phase.phaseName || phase.phase || '').toString().includes(cp)),
      };
    });
            ...
            <div key={idx} className="space-y-3">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                <div className="flex items-center gap-3">
                  {/* Phase number badge */}
                  <div
                    className={`flex items-center justify-center w-10 h-10 sm:w-8 sm:h-8 rounded-full text-sm sm:text-xs font-bold shadow-lg ${
                      phase.isCritical
                  ...
                  {phase.isCritical && (
                    <Badge
                      variant="outline"
                      className="px-2 py-1 bg-pink-400/20 text-pink-400 border-pink-400/40 text-xs sm:text-[10px] font-medium"
                    >
                      Critical
                    </Badge>
                  )}
                  ...
                  <CollapsibleContent className="mt-2 space-y-1">
                    {phase.tasks.slice(0, 5).map((task, taskIdx) => {
                      ...
                      return (
                        <div
                          key={taskIdx}
                          className="text-xs sm:text-sm text-foreground/80 pl-4 border-l-2 border-pink-400/30 py-1 leading-relaxed"
                        >
                          • {taskText}
                        </div>
                      );
