...
export const EditableMaterialRow = ({ material, onUpdate, onDelete }: EditableMaterialRowProps) => {
    ...
    <div className="flex items-start gap-3 p-2 hover:bg-muted/10 rounded-md group">
      ...
      <div className="flex-1 space-y-1">
        <div className="flex items-center justify-between">
          <span
            className={`text-sm font-medium ${material.ordered ? 'line-through text-muted-foreground' : ''}`}
          >
            {material.name}
          </span>
        ...
        {material.orderBy && (
          <p className="text-xs text-orange-500">Order by: {material.orderBy}</p>
        )}
