...
export default function EnrichmentConsole() {
  ...
  const loadStatus = async () => {
    try {
      ...
      const isPracticalWork =
        selectedTask === 'practical_work' || selectedTask.startsWith('practical_work_');
      ...
      if (isPracticalWork) {
        ...
        const facetType =
          selectedTask === 'practical_work'
            ? 'primary'
            : selectedTask.replace('practical_work_', '');
        ...
        const progress =
          targetFacets > 0 ? Math.round(((facetsCreated || 0) / targetFacets) * 100) : 0;
        ...
        const { data: sourceEnriched, error: rpcError } = await supabase.rpc(
        ...
        const progress =
          sourceTotal > 0 ? Math.round(((facetsCreated || 0) / sourceTotal) * 100) : 0;
      ...
      .on(
        ...
        (payload) => {
          ...
          setStats((prev) => {
            ...
            const newProgress =
              prev.targetFacets > 0 ? Math.round((newFacetsCreated / prev.targetFacets) * 100) : 0;
  ...
  const callScheduler = async (action: string, extraParams = {}) => {
    ...
    try {
      ...
      if (
        action === 'continue' &&
        data?.message === 'No pending jobs to continue' &&
        stats.remaining > 0
      ) {
  ...
  const handleRecoverStuck = async () => {
    ...
    try {
      ...
      const { data: recoverData, error: recoverError } = await supabase.functions.invoke(
  ...
  const handleVerifyIntegrity = async () => {
    ...
    try {
      ...
      let sourceQuery = supabase.from(config.sourceTable).select('*').limit(5000);
      ...
      // Query target table
      const { data: targetData } = await supabase.from(config.targetTable).select('*').limit(5000);
      ...
      if (selectedTask === 'bs7671') {
        uniqueSource = new Set(
          (sourceData || []).map((r: any) => r.regulation_number?.trim()).filter(Boolean)
        ).size;
        uniqueEnriched = new Set(
          (targetData || []).map((r: any) => r.regulation_number?.trim()).filter(Boolean)
        ).size;
      } else if (selectedTask === 'practical_work') {
        uniqueSource = new Set((sourceData || []).map((r: any) => r.id).filter(Boolean)).size;
        uniqueEnriched = new Set(
          (targetData || []).map((r: any) => r.practical_work_id).filter(Boolean)
        ).size;
      } else {
      ...
      const computeAction =
        selectedTask === 'practical_work'
          ...
          : selectedTask === 'design_knowledge'
            ...
            : 'compute_missing';
      const startAction =
        selectedTask === 'practical_work' ? 'start_missing_practical_work' : 'start_missing';
  ...
  const handleReconcile = async () => {
    ...
    try {
      ...
      let sourceQuery = supabase.from(config.sourceTable).select('id');
      ...
      const { data: enriched } = await supabase.from(config.targetTable).select('*');

      // Extract source IDs from enriched records (handle different FK column names)
      const enrichedIds = new Set(
        (enriched || [])
          .map((e: any) => e.regulation_id || e.source_id || e.knowledge_id || e.pricing_id)
          .filter(Boolean)
      );
      ...
      if (missing.length === 0) {
        ...
        toast.warning(`Found ${missing.length} missing items`, {
          description: `First 5 IDs: ${missing
            .slice(0, 5)
          ...
          {integrityCheck && (
            <div className="mb-3 p-3 bg-muted rounded-md border border-success/30">
              ...
              <div className="text-xs text-muted-foreground font-mono space-y-1">
                ...
                <div>
                  Timestamp: {new Date(integrityCheck.beforeTimestamp).toLocaleTimeString()}
                </div>
              </div>
            </div>
          )}
            ...
            {selectedTask === 'practical_work' && (
              <MobileButton
                onClick={async () => {
                  if (
                    !confirm(
                      'Archive excess facets (rank 9+) to archive table? This is reversible and preserves all data.'
                    )
                  )
                    return;
                  setIsLoading(true);
