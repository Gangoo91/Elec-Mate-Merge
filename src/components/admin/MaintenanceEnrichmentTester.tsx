...
`;
      ...
      addResult(
        'Phase 1',
        'success',
        `Parsed ${parseResponse.data?.chunks || 0} chunks`,
        parseResponse.data
      );
      ...
      addResult(
        'Phase 2',
        'success',
        `Enriched ${enrichResponse.data?.processed || 0} records`,
        enrichResponse.data
      );
      ...
      if (categoryError) {
        ...
        const hasGarbage = categories.some(
          (cat) => cat?.includes('ombined') || cat?.includes('Mobile') || cat?.includes('Degrees')
        );

        addResult(
          'Phase 3',
          hasGarbage ? 'error' : 'success',
      ...
      const { data: facetData, error: facetError } = await supabase
        .from('practical_work_intelligence')
        .select(
          'equipment_category, tools_required, materials_needed, test_procedures, safety_requirements'
        )
        .order('created_at', { ascending: false })
        .limit(10);
      ...
      if (facetError) {
        ...
        const avgTools =
          facetData?.reduce((sum, r) => sum + (r.tools_required?.length || 0), 0) /
          (facetData?.length || 1);
        const avgMaterials =
          facetData?.reduce((sum, r) => sum + (r.materials_needed?.length || 0), 0) /
          (facetData?.length || 1);
        const avgTests =
          facetData?.reduce((sum, r) => sum + (r.test_procedures?.length || 0), 0) /
          (facetData?.length || 1);
        const hasSafety = facetData?.every(
          (r) => r.safety_requirements && Object.keys(r.safety_requirements).length > 0
        );

        addResult(
          'Phase 3',
          avgTools > 3 && avgMaterials > 3 && avgTests > 2 ? 'success' : 'warning',
