...
const CommonFaultsList = () => {
  ...
  const filteredFaults = commonFaults.filter(
    (fault) =>
      fault.symptom.toLowerCase().includes(searchTerm.toLowerCase()) ||
      fault.category.toLowerCase().includes(searchTerm.toLowerCase()) ||
      fault.possibleCauses.some((cause) => cause.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <>
      ...
      <div className="max-w-md mx-auto mb-6 sm:mb-8">
        <div className="relative">
          ...
          <Input
            ...
            className={cn(
              'bg-muted border-border text-foreground placeholder-gray-400 min-h-[44px]',
              !searchTerm && 'pl-10'
            )}
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
        {filteredFaults.map((fault, index) => (
          <Card
            key={index}
            className={`border-l-4 ${getUrgencyColor(fault.urgency)} hover:bg-card hover:border-border transition-all touch-manipulation`}
          >
            <CardHeader className="p-4 sm:p-5 md:p-6">
              <div className="flex items-start justify-between gap-3 mb-2">
                <CardTitle className="text-foreground text-base sm:text-lg md:text-xl flex-1 min-w-0">
                  {fault.symptom}
                </CardTitle>
                <Badge
                  className={`${getUrgencyBadge(fault.urgency)} text-foreground font-bold capitalize text-xs shrink-0`}
                >
                  {fault.urgency}
                </Badge>
              </div>
                  ...
                  <ol className="space-y-2">
                    {fault.diagnosticSteps.map((step, idx) => (
                      <li
                        ...
                        className="text-xs sm:text-sm text-gray-300 flex items-start gap-2 leading-relaxed"
                      >
                        <span className="text-blue-400 font-semibold min-w-[20px] shrink-0">
                          {idx + 1}.
                        </span>
