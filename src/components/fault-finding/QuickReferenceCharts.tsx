  ...
  const testLimits = [
    {
      ...
      limit: '≥1.0 MΩ',
      notes: '0.5 MΩ acceptable with investigation',
    },
    {
      test: 'Insulation Resistance',
      circuit: 'Distribution Circuits',
      limit: '≥1.0 MΩ',
      notes: 'Higher values expected',
    },
    {
      test: 'Continuity R1+R2',
      circuit: '2.5mm² Ring Final',
      limit: '≤1.67 Ω',
      notes: 'At 20°C ambient temperature',
    },
    {
      test: 'Continuity R1+R2',
      circuit: '4.0mm² Ring Final',
      limit: '≤1.04 Ω',
      notes: 'At 20°C ambient temperature',
    },
    ...
    { test: 'RCD Operation', circuit: '100mA RCD', limit: '≤300ms @ 1×In', notes: '≤200ms @ 2×In' },
    {
      ...
      circuit: 'Type B MCB',
      limit: 'See Table 41.3',
      notes: 'Depends on rating and type',
    },
    {
      test: 'Earth Fault Loop',
      circuit: 'Type C MCB',
      limit: 'See Table 41.4',
      notes: 'Lower limits than Type B',
    },
  ];

  const faultSymptoms = [
    {
      symptom: 'MCB trips immediately',
      priority: 'HIGH',
      likely: 'Short circuit, overload',
      action: 'Isolate loads, test continuity',
    },
    {
      symptom: 'RCD trips randomly',
      priority: 'HIGH',
      likely: 'Earth leakage',
      action: 'Test individual circuits',
    },
    {
      symptom: 'Electric shock',
      priority: 'CRITICAL',
      likely: 'Earth fault, insulation failure',
      action: 'Immediate isolation required',
    },
    {
      symptom: 'Burning smell',
      priority: 'CRITICAL',
      likely: 'Overheating, arcing',
      action: 'Immediate power isolation',
    },
    {
      symptom: 'Lights flickering',
      priority: 'MEDIUM',
      likely: 'Loose connections, voltage issues',
      action: 'Check connections, monitor voltage',
    },
    {
      symptom: 'No power to circuit',
      priority: 'LOW',
      likely: 'Tripped MCB/RCD, loose connection',
      action: 'Check consumer unit first',
    },
  ];

  const disconnectionTimes = [
    {
      voltage: '230V Final Circuits',
      maxTime: '0.4s',
      application: 'Socket outlets, fixed equipment',
    },
  ...
  const testEquipment = [
    {
      ...
      voltage: '500V DC',
      use: 'Circuits up to 500V',
      standard: 'BS EN 61557-2',
    },
    {
      equipment: 'Insulation Tester',
      voltage: '1000V DC',
      use: 'Circuits above 500V',
      standard: 'BS EN 61557-2',
    },
    {
      equipment: 'Continuity Tester',
      voltage: '4-24V DC',
      use: 'Low resistance measurements',
      standard: 'BS EN 61557-4',
    },
    {
      equipment: 'RCD Tester',
      type: 'AC/A Types',
      use: 'Standard RCD testing',
      standard: 'BS EN 61557-6',
    },
    {
      equipment: 'Loop Tester',
      type: 'Auto/Manual',
      use: 'Earth fault loop impedance',
      standard: 'BS EN 61557-3',
    },
    {
      equipment: 'Multimeter',
      type: 'CAT III/IV',
      use: 'General measurements',
      standard: 'BS EN 61010',
    },
  ];
                ...
                <Table className="border border-border">
                  <TableHeader>
                    <TableRow className="border-elec-yellow/30 bg-muted/50">
                      <TableHead className="text-elec-yellow font-bold border-r border-border">
                        Test Type
                      </TableHead>
                      <TableHead className="text-elec-yellow font-bold border-r border-border">
                        Circuit/Application
                  ...
                  <TableBody>
                    {testLimits.map((item, idx) => (
                      <TableRow key={idx} className="border-border hover:bg-muted/30">
                        <TableCell className="text-foreground font-semibold border-r border-border">
                          {item.test}
                        </TableCell>
                        <TableCell className="text-gray-300 border-r border-border">
                          {item.circuit}
                        </TableCell>
                ...
                <Table className="border border-border">
                  <TableHeader>
                    <TableRow className="border-elec-yellow/30 bg-muted/50">
                      ...
                      <TableHead className="text-elec-yellow font-bold text-center border-r border-border">
                        Priority
                      </TableHead>
                      <TableHead className="text-elec-yellow font-bold border-r border-border">
                        Likely Cause
                  ...
                  <TableBody>
                    {faultSymptoms.map((item, idx) => (
                      <TableRow key={idx} className="border-border hover:bg-muted/30">
                        ...
                        <TableCell className="text-center border-r border-border">
                          <Badge className={`${getPriorityColor(item.priority)} text-foreground`}>
                            {item.priority}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-gray-300 border-r border-border">
