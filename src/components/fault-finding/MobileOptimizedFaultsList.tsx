...
const MobileOptimizedFaultsList = () => {
  ...
  const commonFaults: CommonFault[] = [
    ...
    },
    {
      ...
      safetyNotes:
        'DANGER - Risk of fire. Isolate immediately and do not re-energise until fault found',
      urgency: 'critical',
  ...
  const getUrgencyIcon = (urgency: string) => {
    switch (urgency) {
      ...
      case 'high':
        return <Zap className="h-4 w-4 text-[hsl(var(--elec-yellow))]" />;
      case 'medium':
        return <Shield className="h-4 w-4 text-[hsl(var(--elec-blue))]" />;
      default:
        return <Clock className="h-4 w-4 text-[hsl(var(--elec-gray))]" />;
    }
  };
  ...
  const getUrgencyColor = (urgency: string) => {
    switch (urgency) {
      case 'critical':
        return 'bg-[hsl(var(--elec-gray))] border-destructive/30 text-destructive';
      case 'high':
        return 'bg-[hsl(var(--elec-gray))] border-[hsl(var(--elec-yellow))]/30 text-[hsl(var(--elec-yellow))]';
      case 'medium':
        return 'bg-[hsl(var(--elec-gray))] border-[hsl(var(--elec-blue))]/30 text-[hsl(var(--elec-blue))]';
      default:
        return 'bg-[hsl(var(--elec-gray))] border-[hsl(var(--elec-gray))]/50 text-foreground';
    }
  };
        ...
        {filteredFaults.map((fault) => (
          <Collapsible key={fault.id}>
            <Card className="transition-all duration-200 bg-[hsl(var(--elec-gray))] border-[hsl(var(--elec-gray))]/30">
              <CollapsibleTrigger className="w-full" onClick={() => toggleExpanded(fault.id)}>
