    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {Object.keys(supplierData).map((supplierKey) => {
          ...
          return (
            <Link key={supplierKey} to={`/electrician/suppliers/${supplierKey}`}>
              <Card className="relative border-elec-yellow/20 bg-gradient-to-br from-elec-gray to-elec-card backdrop-blur cursor-pointer hover:border-elec-yellow/60 transition-all duration-300 hover:shadow-xl hover:shadow-elec-yellow/10 hover:scale-[1.02] group overflow-hidden h-full">
                {/* Background Pattern */}
