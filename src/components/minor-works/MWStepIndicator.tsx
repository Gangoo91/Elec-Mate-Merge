...
import { CheckCircle, Users, CircuitBoard, TestTube, PenTool } from 'lucide-react';
...
const steps: StepConfig[] = [
  { id: 'details', label: 'Client & Details', shortLabel: 'Details', icon: <Users className="h-4 w-4" /> },
  { id: 'circuit', label: 'Circuit Details', shortLabel: 'Circuit', icon: <CircuitBoard className="h-4 w-4" /> },
  { id: 'testing', label: 'Test Results', shortLabel: 'Testing', icon: <TestTube className="h-4 w-4" /> },
  { id: 'declaration', label: 'Declaration', shortLabel: 'Declare', icon: <PenTool className="h-4 w-4" /> },
];

interface MWStepIndicatorProps {
  currentTab: MWTabValue;
  onTabChange: (tab: MWTabValue) => void;
  isTabComplete: (tab: MWTabValue) => boolean;
  className?: string;
}

const MWStepIndicator: React.FC<MWStepIndicatorProps> = ({
  currentTab,
  onTabChange,
  isTabComplete,
  className,
...
export default MWStepIndicator;
