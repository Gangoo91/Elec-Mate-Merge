...
import {
  useMinorWorksValidation,
  type MWValidationRule,
  MW_TAB_LABEL,
  type MWTabId,
...
const groupByTab = (rules: MWValidationRule[]): Map<MWTabId, MWValidationRule[]> => {
  const groups = new Map<MWTabId, MWValidationRule[]>();
...
const MinorWorksValidationPanel: React.FC<MinorWorksValidationPanelProps> = ({
  formData,
  className = '',
  onJumpToTab,
}) => {
            ...
            <div key={tab} className="px-3 py-2">
              <div className="flex items-baseline justify-between mb-1.5">
                <p className="text-[10px] uppercase tracking-wider text-white/50">
                  {MW_TAB_LABEL[tab]} — {rules.length} missing
                </p>
                {onJumpToTab && (
                  <button
                    type="button"
...
export default MinorWorksValidationPanel;
