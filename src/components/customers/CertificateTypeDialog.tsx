    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>Select the type of certificate you want to create</DialogDescription>
        </DialogHeader>
          ...
          {certificateTypes.map(({ type, title, description, icon: Icon, color }) => (
            ...
            >
              <div className="flex flex-col items-center text-center space-y-3">
                <div
                  className={`w-16 h-16 rounded-full bg-gradient-to-br ${color} flex items-center justify-center transition-transform group-hover:scale-110`}
                >
