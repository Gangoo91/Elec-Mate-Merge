...
const QuoteSettingsSheet = ({ open, onOpenChange, profile, onSave }: QuoteSettingsSheetProps) => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const quoteTermsJson = JSON.stringify({ selected: selectedTerms, custom: customTerms });
      ...
      <SheetContent side="bottom" className="h-[85vh] p-0 rounded-t-2xl overflow-hidden">
        <div className="flex flex-col h-full bg-background">
          ...
          <div className="px-5 pb-4 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Quote Settings</h2>
              <p className="text-xs text-white">Validity, deposit and terms</p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-6 space-y-5">
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-2">
                <Label className="text-xs font-medium text-white uppercase tracking-wider">Validity (days)</Label>
                <Input
                  type="number"
              ...
              <div className="space-y-2">
                <Label className="text-xs font-medium text-white uppercase tracking-wider">Deposit %</Label>
                <Select value={String(depositPercentage)} onValueChange={(v) => setDepositPercentage(parseInt(v, 10))}>
              ...
              <div className="space-y-2">
                <Label className="text-xs font-medium text-white uppercase tracking-wider">Warranty</Label>
                <Input
            ...
            <div className="space-y-3">
              <Label className="text-xs font-medium text-white uppercase tracking-wider">Terms & Conditions</Label>
              {Object.entries(DEFAULT_TERMS_GROUPED).map(([groupKey, group]) => {
                ...
                return (
                  <Collapsible
                    key={groupKey}
                    open={isExpanded}
                    onOpenChange={(o) => setExpandedGroups((prev) => o ? [...prev, groupKey] : prev.filter((g) => g !== groupKey))}
                  >
                        ...
                        {group.terms.map((term) => {
                          ...
                          return (
                            <label key={term.id} className="flex items-start gap-3 p-2 rounded-lg hover:bg-white/[0.02] cursor-pointer touch-manipulation">
                              <Checkbox
                                checked={isSelected}
                                onCheckedChange={(checked) => setSelectedTerms((prev) => checked ? [...prev, term.id] : prev.filter((id) => id !== term.id))}
                ...
                <div className="space-y-2 pt-2">
                  {customTerms.map((term) => (
                    <div key={term.id} className="flex items-start gap-3 p-2 rounded-lg bg-white/[0.02]">
                      ...
                      <button
                        type="button"
                        onClick={() => { setCustomTerms((prev) => prev.filter((t) => t.id !== term.id)); setSelectedTerms((prev) => prev.filter((id) => id !== term.id)); }}
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  className="h-11 w-11 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400"
                >
