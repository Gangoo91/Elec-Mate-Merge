  ...
  useEffect(() => {
    const unlockedMilestone = MILESTONES.find(
      (m) =>
        ...
        earnedPoints - items.find((i) => i.completed)?.points! < m.points
    );
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-foreground/70">Complete actions to boost visibility</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-foreground">{earnedPoints}</span>
              <span className="text-sm text-foreground/70">/ {totalPoints} points</span>
            </div>
          ...
          <div className="flex justify-between mt-1">
            {MILESTONES.map((milestone, index) => (
              ...
              >
                ...
                />
                <span className="text-[10px] text-foreground/70 mt-1">{milestone.points}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={cn('font-medium text-sm', item.completed && 'text-green-500')}>
                  {item.label}
                </p>
              ...
              <div className="flex items-center gap-2">
                ...
                {!item.completed && item.action && (
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={item.action}>
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-elec-yellow text-elec-dark rounded-xl shadow-lg">
              ...
              <div>
                <p className="font-bold">
                  {MILESTONES.find((m) => m.points === showMilestoneUnlock)?.label} Unlocked!
                </p>
