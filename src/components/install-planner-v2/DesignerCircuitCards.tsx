      ...
      {circuits.map((circuit) => (
        <Card key={circuit.id} className="border-blue-500/20 bg-card/40">
          <CardHeader className="pb-3 px-4 sm:px-6">
            <div className="space-y-3">
              <h4 className="text-xl sm:text-2xl font-bold text-foreground text-center">
                {circuit.name}
              </h4>
              ...
              <div className="flex items-center justify-center gap-4 sm:gap-6 flex-wrap">
                <div className="text-center">
                  ...
                  <p className="text-3xl sm:text-4xl font-bold text-elec-yellow">
                    {circuit.protection}
                  </p>
                </div>
                <div className="text-center">
          ...
          <Collapsible open={expandedCircuits.has(circuit.id)}>
            <div className="px-4 sm:px-6 pb-3">
              <CollapsibleTrigger onClick={() => toggleCircuit(circuit.id)} asChild>
                <button className="w-full px-4 py-2 flex items-center justify-center gap-2 border border-elec-yellow/30 rounded-lg hover:bg-elec-yellow/10 transition-colors min-h-[44px]">
                  <span className="text-sm font-medium">View Working Out</span>
                  <ChevronDown
                    className={`h-4 w-4 transition-transform ${expandedCircuits.has(circuit.id) ? 'rotate-180' : ''}`}
                  />
                </button>
              </CollapsibleTrigger>
            </div>
                ...
                <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-muted/30">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {circuit.calculations.voltageDrop.volts}V (
