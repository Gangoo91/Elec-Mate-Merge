...
export const RiskMatrixCard = ({ data }: RiskMatrixCardProps) => {
  ...
  const highestRisk =
    data.riskAssessment?.hazards?.reduce((max, h) => Math.max(max, h.riskRating), 0) || 0;
      ...
      <CardContent className="p-3 sm:p-4 space-y-3 sm:space-y-4 max-w-full overflow-hidden break-words">
        ...
        <div className="space-y-3">
          <div className={`${riskColor.bg} border ${riskColor.border} rounded p-3`}>
            ...
            <p className={`text-3xl font-bold ${riskColor.text}`}>{highestRisk}/25</p>
          ...
          {data.requiredPPE && data.requiredPPE.length > 0 && (
            <div className="bg-muted/30 rounded p-2 sm:p-3">
              <p className="text-[10px] sm:text-xs text-foreground/70 mb-1.5 sm:mb-2">
                Required PPE
              </p>
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {data.requiredPPE.map((ppe, idx) => (
                  <Badge
                    key={idx}
                    variant="outline"
                    className="text-[10px] sm:text-[10px] bg-background/50 break-words"
                  >
                    {ppe}
                  </Badge>
                ))}
                    ...
                    <div key={idx} className="border border-border/50 rounded p-2 sm:p-3 space-y-2">
                      <div className="flex flex-col sm:flex-row items-start sm:justify-between gap-2">
                        ...
                        <div className="flex gap-1 sm:gap-2 flex-wrap">
                          <Badge
                            className={`${hazardColor.bg} ${hazardColor.text} border-none text-[10px] sm:text-[10px]`}
                          >
                            Initial: {hazard.riskRating}
                          </Badge>
                          <Badge
                            className={`${residualColor.bg} ${residualColor.text} border-none text-[10px] sm:text-[10px]`}
                          >
                      ...
                      <div className="text-[10px] sm:text-xs space-y-1">
                        <div className="flex gap-2 sm:gap-4 flex-wrap">
                          <span className="text-foreground/70">
                            Likelihood: L{hazard.likelihood}
                          </span>
