  ...
  const handleExportStandalonePDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < diagrams.length; i++) {
        ...
        pdf.setFontSize(14);
        pdf.setFont('helvetica', 'bold');
        pdf.text(diagram.title, 15, yPos);
        yPos += 10;
        ...
        try {
          ...
          console.warn(`Failed to add diagram ${i}:`, err);
        }
      }
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text(
          `Generated by ElecMate AI - ${new Date().toLocaleDateString('en-GB')}`,
          15,
          pageHeight - 10
        );
        pdf.text(
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            {diagrams.length > 0 && (
              <div className="flex gap-2 w-full md:w-auto">
                <Button
                  onClick={handleExportStandalonePDF}
                  variant="default"
                  size="sm"
