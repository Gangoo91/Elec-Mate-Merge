...
export const MultiCircuitResultsDisplay = ({
  ...
  const getLoadTypeAccessories = (loadType: string): string[] => {
    const accessoriesMap: Record<string, string[]> = {
      ...
      lighting: [
        '4-6x Ceiling roses',
        '1-2x Light switches',
        '1-2x 25mm back boxes',
        'Connector blocks',
      ],
      shower: [
        '1x 45A shower pull cord isolator',
        '1x 35mm deep back box',
      ...
      ev_charger: [
        '1x EV charger unit (as specified)',
        'Outdoor mounting kit',
        'Earth rod kit (if TT system)',
      ],
      'ev-charger': [
        '1x EV charger unit (as specified)',
        'Outdoor mounting kit',
      ...
      outdoor_socket: [
        '1x IP66 weatherproof socket',
        '1x RCD socket/spur',
        'Conduit/trunking for external run',
      ],
      outdoor: [
        '1x IP66 weatherproof socket',
        '1x RCD socket/spur',
    ...
    return (
      accessoriesMap[loadTypeKey] || [
        'Accessories as per circuit specification',
        'Conduit/trunking as required',
      ]
    );
  };
  ...
  const hasSpecialLocations = (circuits: Circuit[]): boolean => {
    ...
    return circuits.some((c) =>
      specialLocationKeywords.some(
        (keyword) =>
          c.name.toLowerCase().includes(keyword) || c.loadType.toLowerCase().includes(keyword)
      )
    );
  };
      ...
      <Card className="p-5 bg-green-500/5 border-green-500/20">
        ...
        <div className="space-y-3">
          {circuits.map((circuit, index) => {
            ...
            return (
              <div
                key={`materials-${index}`}
                className="border-b border-gray-700/30 last:border-0 pb-3 last:pb-0"
              >
                ...
                <ul className="text-xs text-gray-200 space-y-1 ml-6 list-disc">
                  {circuit.cableLength && (
                    <li>
                      <span className="font-medium">{circuit.cableLength}m</span> of{' '}
                      {circuit.cableSize} {circuit.cableSpec}
                    </li>
                  )}
                  <li>1x {circuit.protection}</li>
                  {circuit.rcdRequirements && (
                    <li>
                      1x {circuit.rcdRequirements.rating} ({circuit.rcdRequirements.reason})
                    </li>
                  )}
        ...
        <div className="mt-4 pt-3 border-t border-gray-700/30">
          <p className="text-xs text-gray-400 italic">
            Note: Quantities are estimates based on typical installations. Always verify
            requirements on-site before purchasing.
          </p>
        </div>
      </Card>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between p-3 bg-muted/30 rounded-md">
                ...
                <span className="font-bold text-primary text-base">
                  ...
                  <span className="text-xs text-muted-foreground ml-1">({circuit.cableSpec})</span>
                </span>
              </div>
              {circuit.cableLength && (
                <div className="flex justify-between p-3 bg-muted/30 rounded-md">
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center p-3 bg-muted/30 rounded-md">
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`font-bold text-lg ${circuit.calculations.voltageDrop.compliant ? 'text-green-500' : 'text-red-500'}`}
                    >
                ...
                <div className="border-t pt-3 mt-3 space-y-3">
                  <div>
                    ...
                    <div className="font-mono text-sm text-muted-foreground space-y-1">
                      <div>
                        Ib: {circuit.calculations.Ib}A | In: {circuit.calculations.In}A | Iz:{' '}
                        {circuit.calculations.Iz}A
                      </div>
                    ...
                    <div className="mt-2 text-xs text-muted-foreground">
                      ...
                      {circuit.calculations.voltageDrop.max &&
                        ` - Max: ${circuit.calculations.voltageDrop.max}%`}
                    </div>

                    {/* Zs Detail */}
                    <div className="mt-1 text-xs text-muted-foreground">
                      Earth Fault Loop (Zs): {circuit.calculations.zs.calculated.toFixed(2)}Ω (Max:{' '}
                      {circuit.calculations.zs.max.toFixed(2)}Ω)
                      {circuit.calculations.zs.regulation &&
                        ` - ${circuit.calculations.zs.regulation}`}
                      {circuit.calculations.zs.compliant ? ' ✓' : ' ✗'}
                    </div>
        ...
        <div className="space-y-4 text-sm">
          ...
          <div>
            ...
            <ul className="text-xs text-gray-200 space-y-1 ml-6 list-disc">
              ...
              <li>
                Use RCD protection or mechanical protection in zones where cables could be
                penetrated
              </li>
              <li>
                Keep cables vertical or horizontal in safe zones to reduce risk of accidental damage
              </li>
              <li>
                Don't exceed 40% fill ratio in conduit/trunking - allows easy cable pulling and heat
                dissipation
              </li>
          ...
          <div>
            ...
            <ul className="text-xs text-gray-200 space-y-1 ml-6 list-disc">
              <li>
                <strong>Continuity of protective conductors:</strong> R1 + R2 test on all circuits
              </li>
              <li>
                <strong>Insulation resistance:</strong> Minimum 1MΩ at 500V DC between conductors
              </li>
              <li>
                <strong>Polarity:</strong> Verify at all socket outlets, switches, and accessories
              </li>
              <li>
                <strong>Earth fault loop impedance (Zs):</strong> Must be lower than calculated
                maximum values above
              </li>
              <li>
                <strong>RCD testing:</strong> Must trip within 300ms at 1x IΔn, and 40ms at 5x IΔn
              </li>
              <li>
                <strong>Functional testing:</strong> Verify all circuits operate correctly under
                load
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="text-xs text-gray-200 space-y-1 ml-6 list-disc">
              <li>
                Complete Minor Works Certificate for additions/alterations to existing installation
              </li>
              ...
              <li>
                Notify Building Control if work falls under Part P Building Regulations
                (England/Wales)
              </li>
          ...
          {hasSpecialLocations(circuits) && (
            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-200">
                This installation includes circuits in special locations (bathrooms, outdoor areas,
                swimming pools, etc.). These require additional requirements per{' '}
                <strong>BS 7671 Part 7</strong>:
              </p>
