...
export const MultiCircuitMode = ({ planData, updatePlanData, onReset }: MultiCircuitModeProps) => {
  ...
  const [selectedCircuitForSchematic, setSelectedCircuitForSchematic] = useState<number | null>(
    null
  );
  ...
  const handleSaveCircuit = (circuitData: Partial<FullCircuitDesign>) => {
    if (editingCircuit) {
      ...
      const newCircuit: FullCircuitDesign = {
        ...
        protectionDevice: circuitData.protectionDevice || {
          type: 'MCB',
          curve: 'B',
          rating: 0,
          kaRating: 6,
  ...
  if (compliantCircuits < circuits.length)
    warnings.push(`${circuits.length - compliantCircuits} circuit(s) non-compliant`);

  // Three-phase balancing (if applicable)
  const hasThreePhase = circuits.some((c) => c.phases === 'three');
  const loadBalancing = hasThreePhase
    ? balanceThreePhaseLoad(
        circuits.map((c) => ({
  ...
  return (
    <div className="space-y-6">
              ...
              {circuits.length >= 3 && (
                <Button
                  ...
                  onClick={() => {
                    ...
                    const diagramLayout = generateConsumerUnitDiagram(
                      circuitData,
                      mainSwitchRating
                    );
      ...
      {showTesting && eicSchedule && <TestingDocumentation eicSchedule={eicSchedule} />}
        ...
        {circuits.map((circuit) => {
          ...
          return (
            <CircuitCard
              ...
              loadingSchematic={
                loadingSchematic && selectedCircuitForSchematic === circuit.circuitNumber
              }
      ...
      {loadBalancing && <LoadBalancingView balancing={loadBalancing} />}
