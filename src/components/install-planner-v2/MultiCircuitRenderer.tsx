...
export const MultiCircuitRenderer = ({
  data,
  foundRegulations,
  ragMetadata,
  agentChain,
}: MultiCircuitRendererProps) => {
  ...
  const transformedCircuits = deduplicatedCircuits.map((circuit, index) => ({
    ...
    cableSpec:
      circuit.cableSpec || (circuit.cableSize ? `${circuit.cableSize}mm² cable` : 'Cable TBD'),
    protection:
      circuit.protection || circuit.mcbRating || circuit.protectionDevice?.rating || 'TBD',
    complianceStatus:
      circuit.complianceStatus === 'review' ? 'warning' : circuit.complianceStatus || 'pass',
  }));
