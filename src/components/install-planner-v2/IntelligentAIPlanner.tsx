...
export const IntelligentAIPlanner = ({
  planData,
  updatePlanData,
  onReset,
}: IntelligentAIPlannerProps) => {
    ...
    resumeState?.resumeMessages || [
      {
        role: 'assistant',
        content:
          "Alright mate, what are we designing today? Shower? EV charger? Just tell me what you're installing and I'll help you spec it properly - all BS 7671 compliant obviously 👍",
      },
    ]
  );
  ...
  const [agentOutputHistory, setAgentOutputHistory] = useState<
    Array<{
      agent: string;
      output: any;
      timestamp: string;
    ...
    const checkAgentHealth = async () => {
      try {
        const response = await fetch(
          'https://jtwygbeceundfgnkirof.supabase.co/functions/v1/system-health',
          {
            method: 'GET',
            headers: {
  ...
  const hasMeaningfulContent =
    messages.length >= 2 && messages.some((m) => m.role === 'assistant' && m.agentName);
  ...
  const { streamMessage, isStreaming } = useStreamingChat({
    ...
    onAgentResponse: async (agent, response, structuredData, fullAgentResponse) => {
      ...
      // Pass response into structuredData for fallback rendering - NEVER pass null
      ...
      setMessages((prev) => {
        const filtered = prev.filter(
          (m) => !(m.agentName === agent && (m.isTyping || m.isThinking))
        );
    ...
    onToolCall: (toolCall) => {
      toast.success(
        `✨ ${toolCall.toolName === 'add_circuit_to_design' ? 'Circuit added' : 'Action performed'}`
      );
    },
    onCitation: (citation) => {
  ...
  const isInputMeaningful = (text: string): boolean => {
    ...
    if (
      /\s/.test(t) ||
      /(install|cable|kw|amp|circuit|socket|shower|ev|board|meter|length|run)/i.test(t) ||
      /\d/.test(t)
    )
      return true;
    return true;
    ...
    try {
      ...
      setMessages((prev) => {
        const filtered = prev.filter((msg) => {
          ...
          if (msg.role === 'assistant' && (msg.isTyping || msg.isThinking || msg.hasError))
            return false;
          return true;
        });
        return [
  ...
  const handleExportPackage = async () => {
    const hasCompleteDesign =
      messages.length >= 5 && planData.circuits && planData.circuits.length > 0;

    if (!hasCompleteDesign) {
      toast.error('Need a complete design first', {
        description:
          'Have a conversation with the AI to design at least one circuit before exporting.',
      });
      return;
    }
  ...
  const handleAgentSelection = (agentId: string) => {
    ...
    const welcomeMessage =
      AGENT_WELCOME_MESSAGES[agentId] || `👋 I'm your ${getAgentName(agentId)}. How can I help?`;
  ...
  const handleViewResults = () => {
    ...
    const consultedAgents = [
      ...new Set(
        messages.filter((m) => m.role === 'assistant' && m.agentName).map((m) => m.agentName!)
      ),
    ];
  ...
  const parseAgentResponse = (content: string) => {
    ...
    for (const line of lines) {
      ...
      if (
        ...
        trimmed.match(
          /^(CIRCUIT SPECIFICATION|CALCULATIONS|COMPLIANCE|MATERIALS BREAKDOWN|LABOUR ESTIMATE|PROJECT TOTAL|PRICING NOTES|INSTALLATION METHOD|STEP-BY-STEP PROCEDURE|SAFETY REQUIREMENTS|MATERIAL LIST|TIME ESTIMATE|TEST SEQUENCE|DEAD TESTS|LIVE TESTS|BREAKDOWN)/i
        )
      ) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-elec-dark">
      ...
      <div ref={scrollRef} className="flex-1 bg-elec-dark overflow-y-auto">
        <div className="px-3 md:px-4 py-3 md:py-4 pb-[calc(env(safe-area-inset-bottom)+132px)] sm:pb-24 space-y-3 md:space-y-4">
              ...
              >
                ...
                {message.role === 'assistant' && message.agentName && (
                  <div className="mb-3 pb-2 border-b border-white/20">
                    <Badge
                      variant="outline"
                      className="text-xs font-semibold bg-elec-yellow/10 text-elec-yellow border-elec-yellow/30"
                    >
                      ...
                      <Alert className="mt-3 border-blue-500/30 bg-blue-500/10">
                        ...
                        <AlertDescription className="text-xs space-y-1 text-blue-200">
                          <div>
                            ...
                            {message.metadata.receivedFrom || 'none'}
                          </div>
                          {message.metadata.contextSources && (
                            <>
                              <div>
                                <strong>Shared regulations:</strong>{' '}
                                {message.metadata.contextSources.sharedRegulations
                                  ? '✅ Yes'
                                  : '❌ No'}
                              </div>
                              <div>
                                <strong>Previous outputs:</strong>{' '}
                                {message.metadata.contextSources.previousAgentOutputs?.length > 0
                                  ? `✅ ${message.metadata.contextSources.previousAgentOutputs.join(', ')}`
                                  : '❌ None'}
                              </div>
                              <div>
                                <strong>Project details:</strong>{' '}
                                {message.metadata.contextSources.projectDetails
                                  ? '✅ Yes'
                                  : '❌ No'}
                              </div>
                              <div>
                                <strong>Circuit design:</strong>{' '}
                                {message.metadata.contextSources.circuitDesign ? '✅ Yes' : '❌ No'}
                              </div>
                              {message.metadata.contextSources.coordinating !== undefined && (
                                <div>
                                  <strong>Coordinating:</strong>{' '}
                                  {message.metadata.contextSources.coordinating} specialists
                                </div>
                              )}
                            </>
                          )}
                ...
                {message.role === 'assistant' &&
                  ...
                  (() => {
                    ...
                    const messageWithSuggestions = [...messages]
                      .reverse()
                      .find(
                        (m) =>
                          m.role === 'assistant' &&
          ...
          {messages.some(
            (m) =>
              m.agentName === 'designer' &&
              m.structuredData &&
              (m.structuredData.cableSize || m.structuredData.protectionDevice)
          ) &&
            !isLoading &&
            !isStreaming && (
        ...
        <div className="max-w-5xl mx-auto px-3 md:px-5 space-y-2">
          ...
          {messages.length === 1 && (
            <div className="flex gap-1.5">
              <button
                onClick={() => setInput('9.5kW shower, 15m from board')}
              ...
              <button
                onClick={() => setInput('Full rewire 3-bed house')}
                className="h-7 px-3 text-xs rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-colors flex items-center gap-1.5 flex-1"
              >
                <Sparkles className="h-3 w-3 text-elec-yellow" />
          ...
          <div className="flex items-center gap-2">
            ...
            {completedAgentsCount > 0 && (
              <Button
                ...
                variant="outline"
                onClick={() =>
                  navigate(`/electrician/install-planner/results/${currentConversationId}`)
                }
