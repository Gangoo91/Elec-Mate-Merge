...
const ambientTemperatureData = [
  ...
  { temp: 40, factor: 1.0, status: 'standard' },
...
const groupingData = [
  { circuits: 1, factor: 1.0, description: 'Single circuit' },
  { circuits: 2, factor: 0.8, description: 'Two circuits' },
  { circuits: 3, factor: 0.7, description: 'Three circuits' },
  { circuits: 4, factor: 0.65, description: 'Four circuits' },
  { circuits: 5, factor: 0.6, description: 'Five circuits' },
  { circuits: 6, factor: 0.57, description: 'Six circuits' },
  { circuits: 9, factor: 0.5, description: 'Nine or more circuits' },
];
...
const thermalInsulationData = [
  {
    type: 'None',
    factor: 1.0,
    description: 'No thermal insulation touching cable',
    status: 'good',
  },
  {
    type: 'Partial Contact',
    factor: 0.87,
    description: 'Cable touching insulation on one side',
    status: 'caution',
  },
  {
    type: 'Fully Surrounded',
    factor: 0.78,
    description: 'Cable completely surrounded by insulation',
    status: 'critical',
  },
];
...
export const EnvironmentalFactorsSection = () => {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'caution':
        return 'bg-orange-500/10 text-orange-400 border-orange-400';
      case 'critical':
        ...
        <CardContent>
          <div className="mb-4 p-3 bg-orange-500/10 rounded-lg border border-orange-500/30">
            <p className="text-sm text-gray-300">
              <strong className="text-orange-400">BS 7671 Table 4B1:</strong> Reference temperature
              is 30°C for cables in air, 20°C for cables in ground. These factors apply to the
      ...
      <Card className="bg-muted border-border">
        ...
        <CardContent>
          <div className="mb-4 p-3 bg-purple-500/10 rounded-lg border border-purple-500/30">
            <p className="text-sm text-gray-300">
              <strong className="text-purple-400">BS 7671 Table 4C1:</strong> Applies when cables
              are grouped together and likely to be loaded simultaneously, causing mutual heating
              effects.
            </p>
          </div>
          <Table>
      ...
      <Card className="bg-muted border-border">
        ...
        <CardContent>
          <div className="mb-4 p-3 bg-green-500/10 rounded-lg border border-green-500/30">
            <p className="text-sm text-gray-300">
              <strong className="text-green-400">BS 7671 Section 523:</strong> Cables in contact
              with or surrounded by thermal insulation require derating due to reduced heat
              dissipation.
            </p>
          </div>
          <Table>
                ...
                <TableRow key={item.type}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={getStatusColor(item.status)}>
                      {item.factor === 1.0
                        ? 'None'
                        : `${Math.round((1 - item.factor) * 100)}% reduction`}
                    </Badge>
                  </TableCell>
                </TableRow>
              ))}
