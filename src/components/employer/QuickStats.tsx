          ...
          >
            ...
            <div className="text-left">
              {stat.isLoading ? (
                ...
                <p className={cn('text-lg font-bold leading-none tabular-nums', colors.value)}>
                  {stat.value}
                  {stat.suffix}
                </p>
              )}
      ...
      {stats.map((stat, index) => {
        ...
        return (
          ...
          >
            ...
            <div className={cn('p-2 sm:p-2.5 rounded-xl mb-2', colors.iconBg)}>
              <Icon className={cn('h-5 w-5', colors.icon)} />
            </div>
            {stat.isLoading ? (
              <Skeleton className="h-7 w-10 mb-1" />
            ) : (
              <p className={cn('text-xl sm:text-2xl font-bold tabular-nums', colors.value)}>
                {stat.value}
                {stat.suffix}
              </p>
            )}
        ...
        return (
          <div
            ...
            className={cn(
              ...
              colors.border,
              stat.onClick &&
                cn('cursor-pointer', colors.borderHover, 'hover:scale-[1.02] active:scale-[0.98]')
            )}
          >
            {stat.pulse && !stat.isLoading && (
