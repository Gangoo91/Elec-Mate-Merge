...
export const ElecIDCard = ({ profile, onShare, compact = false }: ElecIDCardProps) => {
  const status =
    statusConfig[profile.ecsStatus as keyof typeof statusConfig] || statusConfig.Active;
  ...
  if (compact) {
    return (
      <Card className="group relative overflow-hidden bg-gradient-to-br from-card via-card to-elec-yellow/5 border-elec-yellow/20 hover:border-elec-yellow/40 transition-all duration-300 hover:shadow-lg hover:shadow-elec-yellow/10">
        ...
        {/* Status accent bar */}
        <div
          className={`absolute left-0 top-0 bottom-0 w-1 ${status.bg} ${status.text.replace('text-', 'bg-')}`}
        />
        ...
        <CardContent className="p-4 pl-5">
          <div className="flex items-center gap-4">
            ...
            <div className={`relative flex-shrink-0`}>
              <div
                className={`w-14 h-14 rounded-xl bg-gradient-to-br from-elec-yellow/30 to-elec-yellow/10 flex items-center justify-center ring-2 ${status.border.replace('border-', 'ring-')} ring-offset-2 ring-offset-card`}
              >
                <User className="h-6 w-6 text-elec-yellow/70" />
              </div>
              <div
                className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full ${status.bg} ${status.border} border flex items-center justify-center`}
              >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                <Badge
                  variant="outline"
                  className={`${status.bg} ${status.text} ${status.border} text-xs py-0.5`}
                >
                  {profile.ecsCardType}
                </Badge>
      ...
      <CardContent className="relative p-5 md:p-6">
        ...
        <div className="flex flex-col lg:flex-row gap-5 mb-6">
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="relative flex-shrink-0">
              <div
                className={`w-24 h-24 md:w-28 md:h-28 rounded-2xl bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center border-2 ${status.border} shadow-lg ${status.glow}`}
              >
                <User className="h-12 w-12 text-foreground/50" />
              </div>
              {/* Status indicator */}
              <div
                className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full ${status.bg} ${status.border} border-2 flex items-center justify-center shadow-lg`}
              >
            ...
            <div className="flex-1">
              ...
              {profile.bio && (
                <p className="text-sm text-foreground/70 mt-2 line-clamp-2 max-w-md">
                  {profile.bio}
                </p>
              )}
              ...
              <div className="flex items-center gap-3 mt-4">
                <Badge
                  className={`${status.bg} ${status.text} ${status.border} gap-1.5 py-1.5 px-3 text-sm font-medium`}
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            ...
            <div
              key={idx}
              className="group/stat relative overflow-hidden bg-gradient-to-br from-background to-muted/30 p-4 rounded-xl border border-border/50 hover:border-elec-yellow/30 transition-colors"
            >
            ...
            <div className="flex flex-wrap gap-2">
              {profile.skills.slice(0, 8).map((skill, idx) => (
                ...
                >
                  {skill.name}
                  {skill.level && (
                    <span className="ml-1.5 text-elec-yellow/60 text-[10px] uppercase">
                      • {skill.level}
                    </span>
                  )}
                </Badge>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-foreground/70 mt-0.5">
                This Elec-ID belongs to {profile.name.split(' ')[0]} and follows them throughout
                their career
              </p>
            </div>
          </div>
