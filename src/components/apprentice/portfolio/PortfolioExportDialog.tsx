    ...
    try {
      ...
      if (exportOptions.format === 'pdf') {
        ...
        toast({
          ...
          description:
            'Word document export is currently being developed. Please use PDF or HTML export for now.',
          variant: 'default',
        });
        return;
      }
          ...
          <ResponsiveDialogBody className="space-y-6">
                  ...
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setExportOptions((prev) => ({
                        ...prev,
                        includeReflections: checked as boolean,
                      }))
                    }
                  />
                  ...
                  {categoryOptions.map((category) => (
                    <div key={category.id} className="flex items-center space-x-2">
                      ...
                      <Label htmlFor={`category-${category.id}`} className="text-sm">
                        {category.name} (
                        {entries.filter((e) => e.category.id === category.id).length} entries)
                      </Label>
                    </div>
                  ))}
                </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                <div>
                  <Label htmlFor="date-from" className="text-xs text-white">
                    From
                  </Label>
                  <Input
                    id="date-from"
                ...
                <div>
                  <Label htmlFor="date-to" className="text-xs text-white">
                    To
                  </Label>
                  <Input
                    id="date-to"
