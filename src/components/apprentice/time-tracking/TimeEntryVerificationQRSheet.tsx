 ...
 *
 * 85vh bottom sheet showing QR code and share channels for OJT time entry verification.
...
export function TimeEntryVerificationQRSheet({
  open,
  onOpenChange,
  verification,
  verificationUrl,
  activityTitle,
}: TimeEntryVerificationQRSheetProps) {
  ...
  const handleDownloadQR = () => {
    const svg = document.getElementById('te-verification-qr-code');
    if (!svg) return;
    ...
    img.onload = () => {
      ...
      link.download = `training-verification-${verification?.verification_token || 'code'}.png`;
  ...
  const handleEmail = () => {
    const subject = encodeURIComponent(
      `Please verify my training hours — ${activityTitle || 'OJT Session'}`
    );
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
            ...
            {snapshot && (
              <div className="rounded-xl bg-white/[0.04] border border-white/10 p-4">
                <p className="text-xs text-white mb-1">Training Session</p>
                <p className="text-sm font-medium text-foreground">
                  {snapshot.activity || activityTitle}
                </p>
                ...
                {snapshot.notes && (
                  <p className="text-xs text-white mt-2 line-clamp-2">{snapshot.notes}</p>
                )}
                {verification && (
                  <div className="flex items-center gap-3 mt-2 text-xs text-white">
                    <div className="flex items-center gap-1">
              ...
              <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-5 text-center space-y-3">
                ...
                {verification?.feedback_text && (
                  <div className="p-3 rounded-lg bg-white/5 text-sm text-white text-left">
                    &ldquo;{verification.feedback_text}&rdquo;
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      'They review your training details — no account needed',
...
export default TimeEntryVerificationQRSheet;
