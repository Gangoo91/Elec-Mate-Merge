...
const EvidenceItem = ({ item, onDelete }: EvidenceItemProps) => {
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            ...
            <span className="text-[11px] text-white flex items-center font-mono">
              <Calendar className="h-3 w-3 mr-1" />
              {item.date}
            </span>
            {isConverted && (
              <span className="text-[12px] text-white px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] flex items-center gap-1">
      ...
      <p className="mt-3 text-[13px] text-white leading-relaxed line-clamp-2">
        {item.description}
      </p>
        ...
        <div className="mt-3 space-y-1">
          ...
          <div className="flex flex-wrap gap-1.5 mt-1">
            {item.files.map((file: string) => (
              <div
                key={file}
                className="text-[12px] text-white px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] flex items-center"
              >
