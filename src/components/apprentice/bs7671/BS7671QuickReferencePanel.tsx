        ...
        <div className="px-4 pb-4 pt-3 mt-1 space-y-2 text-sm rounded-b-xl bg-white/[0.02]">
          {content.steps.map((step, idx) => (
            <div key={idx} className="flex items-start gap-2">
              <span
                className={`${colors.bg} ${colors.text} px-2 py-0.5 rounded text-xs font-bold min-w-[24px] text-center flex-shrink-0`}
              >
                {idx + 1}
              </span>
              ...
              <div className="space-y-1.5">
                {section.codes.map((code, cIdx) => (
                  <div key={cIdx} className="flex items-center gap-3 p-2 rounded bg-white/5">
                    <div
                      className={`w-6 h-6 rounded ${code.tailwindBg} flex-shrink-0 border border-white/20`}
                    />
