...
export const useBS7671Progress = () => {
  ...
  const isStepComplete = useCallback(
    (testId: string, stepIndex: number) => {
  ...
  const getTestProgress = useCallback(
    (testId: string): BS7671TestProgress | undefined => {
  ...
  const exportAsText = useCallback(
    (tests: { id: string; title: string; stepCount: number }[]) => {
      const lines: string[] = [
      ...
      for (const test of tests) {
        ...
        if (progress?.completedAt) {
          lines.push(
            `       Completed: ${new Date(progress.completedAt).toLocaleDateString('en-GB')}`
          );
        }
      }
