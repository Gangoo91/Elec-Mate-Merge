...
const EnhancedCourseCard = ({ course, onViewDetails }: EnhancedCourseCardProps) => {
  ...
  return (
    <Card
      className={`bg-gradient-to-br from-elec-gray to-elec-card ${categoryStyles.border} h-full flex flex-col overflow-hidden relative transition-all duration-300`}
    >
      <div
        className={`absolute top-0 right-0 w-48 h-48 ${categoryStyles.glow} rounded-full blur-3xl -translate-y-1/2 translate-x-1/2`}
      />

      <CardHeader className="pb-3 relative">
        <div className="flex justify-between items-start gap-2 mb-2">
          <Badge className={`${categoryStyles.badge} text-xs border`}>{course.category}</Badge>
