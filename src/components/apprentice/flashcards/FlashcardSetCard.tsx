...
import { Play, Clock, Target, CheckCircle, Trophy, Zap, Star } from "lucide-react";
...
const FlashcardSetCard = ({ set, onStart }: FlashcardSetCardProps) => {
  ...
  return (
    <Card className="border-elec-yellow/20 bg-elec-gray hover:bg-elec-gray/80 transition-all duration-300 hover:scale-105 h-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          {set.lastStudied && (
            <div className="text-xs text-elec-yellow/60">
              Last: {set.lastStudied}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          <Badge className={`${getDifficultyColor(set.difficulty)} flex items-center gap-1`} variant="outline">
          ...
          <Button 
            ...
            className="bg-elec-yellow/10 hover:bg-elec-yellow hover:text-black transition-all duration-200 font-medium"
          >
