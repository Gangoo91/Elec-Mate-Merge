    ...
    <Collapsible>
      <CollapsibleTrigger className="w-full flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 hover:border-white/20 transition-colors touch-manipulation h-auto min-h-[44px]">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Badge
            className={`${allDone ? 'bg-green-500/20 text-green-400 border-green-500/30' : `${config.bg} ${config.text} ${config.border}`}`}
          >
              ...
              <div key={item.id} className="space-y-0">
                <div className="flex items-start gap-3">
                  ...
                  >
                    ...
                    <span
                      className={`text-sm text-left ${checked ? 'text-green-400' : 'text-white'}`}
                    >
                      {item.text}
                    </span>
                  </button>
