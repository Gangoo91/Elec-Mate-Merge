...
export function ActivityGrid({ activityMap, variant }: ActivityGridProps) {
  ...
  return (
    <div>
      ...
      >
        <span className="text-[12px] text-white">4 weeks ago</span>
        <div className="flex items-center gap-1.5">
          <span className="text-[12px] text-white mr-1">Less</span>
          {colours.map((c, i) => (
            <div key={i} className={cn('w-3 h-3 rounded-[3px]', c)} />
          ))}
          <span className="text-[12px] text-white ml-1">More</span>
        </div>
        <span className="text-[12px] text-white">Today</span>
      </motion.div>
    </div>
  );
}
