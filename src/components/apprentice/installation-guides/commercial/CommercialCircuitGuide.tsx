...
const CommercialCircuitGuide = () => {
  const threePhaseDistribution = {
    ...
    description:
      'Commercial premises typically utilise three-phase supplies for higher power capacity and balanced loading',
    systems: [
      {
        type: 'TN-C-S (PME)',
        voltage: '400V/230V',
  ...
  const loadBalancing = {
    ...
    description:
      'Three-phase systems require careful load distribution to minimise neutral currents and maximise efficiency',
    principles: [
      {
      ...
      },
      {
        ...
        formula:
          'In = square root of (IL1 squared + IL2 squared + IL3 squared - IL1.IL2 - IL2.IL3 - IL3.IL1)',
  ...
  const subDistributionDesign = {
    ...
    description:
      'Commercial installations typically require hierarchical distribution from main switchboard to final circuits',
    hierarchy: [
      {
  ...
  const busbarTrunking = {
    ...
    description:
      'Busbar trunking provides flexible, high-current distribution in commercial and industrial applications',
    types: [
      {
      ...
      },
      {
        ...
        advantages:
          'Compact installation, proven technology, easier installation than large cables',
      },
    ],
  ...
  const cableSizing = {
    ...
    diversityFactors: {
      ...
      factors: [
        {
          load: 'General Lighting',
          diversity: '90%',
          notes: 'Offices typically fully lit during working hours',
        },
        {
          load: 'Socket Outlets (general)',
          diversity: '100% first 10A + 50% remainder',
          notes: 'Per BS 7671 for offices',
        },
        {
          load: 'Socket Outlets (process)',
          diversity: '100%',
          notes: 'Where known continuous loads',
        },
        {
          load: 'Air Conditioning',
          diversity: '100% largest + 80% others',
          notes: 'Cooling typically simultaneous',
        },
        {
          load: 'Lifts',
          diversity: '75% for 2-3 lifts, 60% for 4+ lifts',
          notes: 'Unlikely all in use simultaneously',
        },
        {
          load: 'Cooking Appliances',
          diversity: '80%',
          notes: 'Commercial kitchens near simultaneous use',
        },
        {
          load: 'Water Heating (storage)',
          diversity: '100%',
          notes: 'Off-peak charging considered separately',
        },
    ...
    commercialCableSizes: {
      ...
      cables: [
        {
          size: '1.5mm SWA',
          current: '21A (3-core, clipped)',
          use: 'Fire alarm, emergency lighting, small power',
        },
        {
          size: '2.5mm SWA',
          current: '28A (3-core, clipped)',
          use: 'General socket circuits, small equipment',
        },
        {
          size: '4mm SWA',
          current: '37A (3-core, clipped)',
          use: 'Air conditioning, dedicated equipment',
        },
        {
          size: '6mm SWA',
          current: '47A (3-core, clipped)',
          use: 'Larger A/C, cooking, small motors',
        },
        {
          size: '10mm SWA',
          current: '65A (3-core, clipped)',
          use: 'Sub-main distribution, large loads',
        },
        {
          size: '16mm SWA',
          current: '87A (3-core, clipped)',
          use: 'Floor distribution, rising mains',
        },
        {
          size: '25mm SWA',
          current: '110A (3-core, clipped)',
          use: 'Sub-main distribution boards',
        },
  ...
  const maximumDemand = {
    ...
    description:
      'Determining maximum demand is essential for specifying supply capacity and main equipment ratings',
    method: {
      steps: [
        {
          step: 1,
    ...
    exampleCalculation: {
      ...
      loads: [
        ...
        {
          item: 'General Sockets (100 doubles)',
          connected: '75kW',
          diversity: '40%',
          diversified: '30kW',
        },
  ...
  const powerFactorCorrection = {
    ...
    description:
      'Commercial installations with motor and inductive loads often require power factor correction to reduce kVA demand and avoid DNO penalties',
    basics: {
      explanation: [
  ...
  const socketCircuits = {
    ...
    description:
      'The debate between ring final circuits and radial circuits continues in commercial installations',
    comparison: {
      ring: {
    ...
    recommendation:
      'Neither is universally better - select based on specific installation requirements, maintenance capability, and client preferences',
  };

  const serverRoomPower = {
    title: 'Server Room & IT Power Requirements',
    description:
      'Critical IT infrastructure requires careful electrical design for reliability and power quality',
    supplyRequirements: {
  ...
  const circuitDesign = [
    {
      ...
      features: [
        ...
        'Daylight dimming',
        'Emergency maintained/non-maintained',
    ...
    },
    {
      ...
      features: [
        'Floor boxes for flexibility',
        'USB charging integration',
        'Clean power where needed',
    ...
    },
    {
      ...
      features: [
        'Fire rated cable throughout',
        'Separate from other circuits',
        'No RCD protection',
    ...
    },
    {
      ...
      features: [
        'Isolator at machine room',
        'Emergency recall power',
        'Intercom power',
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4 p-0">
          ...
          {loadBalancing.principles.map((principle, index) => (
            <div key={index} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <p className="text-sm text-white mb-2">{principle.explanation}</p>
              {'target' in principle && (
                <p className="text-xs text-white">
                  <strong>Target:</strong> {principle.target}
                </p>
              )}
              {'formula' in principle && (
                <p className="text-xs text-white font-mono">
                  <strong>Formula:</strong> {principle.formula}
                </p>
              )}
              {'testing' in principle && (
                <p className="text-xs text-white">
                  <strong>Testing:</strong> {principle.testing}
                </p>
              )}
              {'consequences' in principle && (
                <p className="text-xs text-white">
                  <strong>Consequence:</strong> {principle.consequences}
                </p>
              )}
              {'notes' in principle && (
                <p className="text-xs text-white">
                  <strong>Note:</strong> {principle.notes}
                </p>
              )}
              {'standard' in principle && (
                <p className="text-xs text-white">
                  <strong>Standard:</strong> {principle.standard}
                </p>
              )}
            </div>
          ))}
            ...
            <div className="space-y-3">
              {loadBalancing.practicalGuidance.map((guide, idx) => (
                <div key={idx} className="bg-white/[0.06] p-3 rounded border border-white/[0.10]">
                  <h5 className="font-medium text-white text-sm mb-1">{guide.scenario}</h5>
                  <p className="text-xs text-white mb-1">
                    <strong>Approach:</strong> {guide.approach}
                  </p>
                  <p className="text-xs text-white">
                    <strong>Benefit:</strong> {guide.benefit}
                  </p>
                </div>
              ))}
            </div>
          ...
          {subDistributionDesign.hierarchy.map((level, index) => (
            <div key={index} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <p className="text-sm text-white mb-2">
                <strong>Location:</strong> {level.location}
              </p>
              <p className="text-xs text-white mb-3">
                <strong>Construction:</strong> {level.construction}
              </p>

              <div>
                <h5 className="font-medium text-white mb-2 text-sm">Key Features</h5>
      ...
      <Card variant="plain">
        ...
        <CardContent className="space-y-4 p-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {busbarTrunking.types.map((type, index) => (
              ...
              >
                ...
                <p className="text-xs text-white mb-2">{type.features}</p>
                <div className="mb-2">
                  <span className="text-xs text-white font-medium">Applications:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              <h4 className="font-medium text-white mb-3">Installation Requirements</h4>
              <ul className="space-y-1">
            ...
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              <h4 className="font-medium text-white mb-3">Testing Requirements</h4>
              <ul className="space-y-1">
            ...
            <div className="grid grid-cols-4 md:grid-cols-6 gap-2 mb-3">
              {cableSizing.groupingFactors.table.map((row, idx) => (
                <div key={idx} className="bg-white/[0.06] p-2 rounded text-center">
                  <div className="text-white text-xs font-medium">{row.circuits} circuits</div>
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.10]">
                    <th className="text-left py-2 text-white">Length in Insulation</th>
                    <th className="text-left py-2 text-white">Derating Factor</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.10]">
                    ...
                    <th className="text-left py-2 text-white">Current Rating</th>
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.10]">
                    ...
                    <th className="text-left py-2 text-white">Diversity</th>
                    <th className="text-left py-2 text-white">Diversified</th>
                  </tr>
                </thead>
                <tbody>
                  {maximumDemand.exampleCalculation.loads.map((load, idx) => (
                    <tr key={idx} className="border-b border-white/[0.10]">
                      ...
                      <td className="py-2 text-white text-xs font-medium">{load.diversified}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t-2 border-white/[0.10]">
                    ...
                    <td className="py-2 text-white font-bold">
                      {maximumDemand.exampleCalculation.total}
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div className="mt-3 p-3 bg-white/[0.06] rounded border border-white/[0.10]">
              ...
              <p className="text-xs text-white mt-1">
                <strong>Conclusion:</strong> {maximumDemand.exampleCalculation.conclusion}
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-4 p-0">
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            ...
            <p className="mt-3 text-white font-mono text-sm">
              <strong>Formula:</strong> {powerFactorCorrection.basics.formula}
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            {powerFactorCorrection.correctionMethods.map((method, index) => (
              <div
                ...
                className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]"
              >
                <h5 className="font-medium text-white mb-2">{method.method}</h5>
                <p className="text-xs text-white mb-2">{method.description}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-white">Advantages: </span>
                    <span className="text-white">{method.advantages}</span>
                  </div>
                  <div>
                    <span className="text-white">Disadvantages: </span>
                    <span className="text-white">{method.disadvantages}</span>
                  </div>
                </div>
                <p className="text-xs text-white mt-2">
                  <strong>Suitable for:</strong> {method.suitableFor}
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              <h4 className="font-medium text-white mb-3">Typical Power Factors</h4>
              <div className="space-y-1">
                {powerFactorCorrection.typicalValues.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-xs">
                    <span className="text-white">{item.load}</span>
                    <span className="text-white">{item.pf}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <div className="space-y-2">
                <div>
                  <span className="text-xs text-white font-medium">Advantages:</span>
                  <ul className="mt-1 space-y-0.5">
                ...
                <div>
                  <span className="text-xs text-white font-medium">Disadvantages:</span>
                  <ul className="mt-1 space-y-0.5">
              ...
              <p className="text-xs text-white mt-2">
                <strong>Best for:</strong> {socketCircuits.comparison.ring.bestFor}
              </p>
            </div>
            ...
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <div className="text-xs text-white mb-3">
                ...
                <ul className="mt-1">
                  {socketCircuits.comparison.radial.options.map((opt, idx) => (
                    <li key={idx}>
                      {opt.protection} - Max {opt.maxArea}
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <p className="text-xs text-white mt-2">
                <strong>Best for:</strong> {socketCircuits.comparison.radial.bestFor}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {serverRoomPower.supplyRequirements.configurations.map((config, idx) => (
                <div key={idx} className="bg-white/[0.06] p-3 rounded border border-white/[0.10]">
                  <h5 className="font-medium text-white text-sm mb-1">{config.type}</h5>
                  ...
                  <p className="text-xs text-white">
                    <strong>Suitable for:</strong> {config.suitableFor}
                  </p>
                </div>
              ))}
            </div>
              ...
              {serverRoomPower.upsRequirements.types.map((type, idx) => (
                <div key={idx} className="bg-white/[0.06] p-3 rounded border border-white/[0.10]">
                  <div className="flex items-center gap-2 mb-1">
                    <h5 className="font-medium text-white text-sm">{type.type}</h5>
                    <Badge variant="outline" className="border-white/10 text-white text-xs">
                  ...
                  <div className="flex gap-4 text-xs">
                    <span className="text-white">
                      <strong>Efficiency:</strong> {type.efficiency}
                    </span>
                    <span className="text-white">
                      <strong>For:</strong> {type.suitableFor}
                    </span>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            ...
            <div className="space-y-3 mb-4">
              {serverRoomPower.cleanEarth.methods.map((method, idx) => (
                <div key={idx} className="bg-white/[0.06] p-3 rounded border border-white/[0.10]">
                  <h5 className="font-medium text-white text-sm mb-1">{method.method}</h5>
                  <p className="text-xs text-white">{method.description}</p>
                  <p className="text-xs text-white">
                    <strong>Installation:</strong> {method.installation}
                  </p>
                  <p className="text-xs text-white">
              ...
              <div className="space-y-2">
                {serverRoomPower.powerDistribution.components.map((comp, idx) => (
                  <div key={idx} className="bg-white/[0.06] p-2 rounded border border-white/[0.10]">
                    <h5 className="font-medium text-white text-xs">{comp.item}</h5>
        ...
        <CardContent className="space-y-4 p-0">
          {circuitDesign.map((circuit, index) => (
            <div key={index} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {circuit.features.map((feature, idx) => (
                    <div key={idx} className="text-xs text-white flex items-center gap-1">
                      <span className="w-1 h-1 bg-white/55 rounded-full"></span>
                      {feature}
                    </div>
                  ))}
                </div>
              ...
              <p className="text-xs text-white mt-2 italic">Note: {circuit.notes}</p>
            </div>
          ))}
        </CardContent>
      </Card>
