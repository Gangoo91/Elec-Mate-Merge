...
const CommercialTestingGuide = () => {
  const testSequenceOrder = {
    ...
    description:
      'Commercial installations require the same test sequence as domestic, but with additional considerations for three-phase systems and larger scale',
    note: 'Tests must be performed in order - earlier tests verify conditions required for later tests to give valid results',
    sequence: [
      {
        order: 1,
        test: 'Continuity of protective conductors (including main and supplementary bonding)',
        commercial: 'More extensive due to larger installation size',
      },
      {
        order: 2,
        test: 'Continuity of ring final circuit conductors',
        commercial: 'May have multiple rings - test each separately',
      },
      {
        order: 3,
        test: 'Insulation resistance',
        commercial: 'Three-phase testing required - more isolation needed',
      },
      {
        order: 4,
        test: 'Polarity',
        commercial: 'Phase sequence critical for three-phase equipment',
      },
      {
        order: 5,
        test: 'Earth electrode resistance (TT systems)',
        commercial: 'May have multiple electrodes - test each',
      },
      {
        order: 6,
        test: 'Earth fault loop impedance (Zs)',
        commercial: 'Test at end of longest circuits on each phase',
      },
      {
        order: 7,
        test: 'Prospective fault current (Ipf)',
        commercial: 'Higher values typical - verify equipment ratings',
      },
      {
        order: 8,
        test: 'Functional testing (including RCDs)',
        commercial: 'Type A, B, and F RCDs may all be present',
      },
    ],
  };
  ...
  const continuityTesting = {
    ...
    description:
      'Commercial installations present unique challenges for continuity testing due to scale and complexity',
    challenges: [
      {
  ...
  const insulationResistance = {
    ...
    description:
      'Three-phase testing requires systematic approach to ensure all phase-to-phase and phase-to-earth combinations are tested',
    testVoltages: [
      ...
      steps: [
        {
          ...
          details:
            'Turn off main isolator, disconnect sensitive equipment (VFDs, PLCs, UPS), remove lamps',
        },
        {
          step: 2,
        ...
        },
        {
          ...
          details:
            'Connect IR tester between linked phases and earth bar - this tests all conductors to earth simultaneously',
        },
        {
          step: 4,
    ...
    troubleshooting: [
      {
        ...
        actions: [
          'Disconnect circuit by circuit to locate',
      ...
      },
      {
        ...
        causes: [
          ...
          'Aged insulation',
          'Poor connections',
          'Electronic equipment connected',
        ],
        actions: [
          'Allow to dry if moisture suspected',
          'Test individual circuits',
          'Disconnect all equipment and retest',
        ],
      },
      {
        ...
        actions: [
          'Allow reading to stabilise (60 seconds)',
          'Test in sections',
          'Repeat test on different day',
        ],
      },
    ],
  };
  ...
  const loopImpedance = {
    ...
    description:
      'Commercial installations often have long cable runs requiring careful consideration of maximum Zs values',
    considerations: [
      {
        factor: 'Temperature Correction',
        explanation: 'Measured Zs at 20C will increase at conductor operating temperature',
        method:
          'Use 0.8 multiplier: If measured Zs exceeds 0.8 x max Zs from tables, circuit fails',
        example:
          '32A Type B max Zs = 1.37 ohm (Table 41.3), so a cold measurement must be under 1.09 ohm',
      },
      {
  ...
  const rcdTesting = {
    ...
    description:
      'Commercial installations may include various RCD types requiring different test procedures',
    rcdTypes: [
      {
        type: 'Type AC',
        symbol: '~',
    ...
    testResults: {
      ...
      tests: [
        {
          test: 'At rated current (In)',
          requirement: 'Must trip within 300ms',
          notes: 'Both polarities',
        },
        ...
        },
        {
          ...
          requirement: '130–500ms',
          notes: 'Time-delayed type — the window comes from BS EN 61008/61009',
        },
  ...
  const phaseSequenceTesting = {
    ...
    description:
      'Correct phase sequence is essential for three-phase rotating machinery and some electronic equipment',
    importance: [
  ...
  const emergencyLightingTesting = {
    ...
    description:
      'Commercial premises require regular emergency lighting testing to ensure life safety compliance',
    testTypes: [
      {
        type: 'Daily Check',
    ...
    centralBatterySystem: {
      ...
      tests: [
        {
          test: 'Battery voltage (float)',
          expected: 'Typically 110V or 220V DC nominal',
          tolerance: '+/- 5%',
        },
        {
          test: 'Battery voltage (on load)',
          expected: 'Should not drop below 85% nominal',
          tolerance: 'Check manufacturer data',
        },
        {
          test: 'Charger operation',
          expected: 'Correct charging current and voltage',
          tolerance: 'Per manufacturer spec',
        },
        {
          test: 'Changeover operation',
          expected: 'Seamless transfer to battery',
          tolerance: 'Within 0.5 seconds',
        },
        {
          test: 'Earth fault monitoring',
          expected: 'Alarm on earth fault',
          tolerance: 'Must indicate fault',
        },
        {
          test: 'Individual circuit monitoring',
          expected: 'Lamp failure indication',
          tolerance: 'Per circuit/zone',
        },
      ],
      additionalChecks: [
  ...
  const fireAlarmTesting = {
    ...
    description:
      'Electricians installing fire alarm supplies must coordinate testing with fire alarm specialists',
    ...
    certificates: [
      {
        ...
        signatures:
          'Designer, Constructor, Inspector/Tester (may be same person for smaller works)',
      },
      {
      ...
      },
      {
        ...
        classifications: [
          {
            code: 'C1',
            meaning: 'Danger present - Risk of injury. Immediate remedial action required.',
            colour: 'red',
          },
          {
            code: 'C2',
            meaning: 'Potentially dangerous - Urgent remedial action required.',
            colour: 'orange',
          },
          {
            code: 'C3',
            meaning: 'Improvement recommended - Does not meet current standard.',
            colour: 'yellow',
          },
          {
            code: 'FI',
            meaning: 'Further Investigation required without delay.',
            colour: 'purple',
          },
        ],
        frequencyGuide: [
  ...
  const eicrRequirements = {
    ...
    description:
      'Commercial installations require periodic inspection at intervals determined by premises type and use',
    inspectionScope: {
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-3">
              {testSequenceOrder.sequence.map((item) => (
                <div key={item.order} className="flex items-start gap-3">
                  <Badge
                    variant="outline"
                    className="border-white/10 text-white w-8 h-8 rounded-full flex items-center justify-center p-0 flex-shrink-0"
                  >
                    {item.order}
                  </Badge>
                  <div className="flex-1">
                    <span className="text-white text-sm">{item.test}</span>
                    <p className="text-xs text-white mt-1">{item.commercial}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {insulationResistance.threePhaseMethod.steps.map((step, idx) => (
                <div key={idx} className="bg-white/[0.06] p-3 rounded border border-white/[0.10]">
                  ...
                  <p className="text-xs text-white">{step.details}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {insulationResistance.threePhaseMethod.expectedValues.map((val, idx) => (
                <div key={idx} className="flex justify-between text-sm">
                  ...
                  <span className="text-white">{val.typical}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            {insulationResistance.troubleshooting.map((issue, index) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <span className="text-xs text-white font-medium">Possible Causes:</span>
                  ...
                  <div>
                    <span className="text-xs text-white font-medium">Actions:</span>
                    <ul className="mt-1 space-y-0.5">
        ...
        <CardContent className="space-y-4 p-0">
          ...
          {loopImpedance.considerations.map((item, index) => (
            <div key={index} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <p className="text-sm text-white mb-2">{item.explanation}</p>
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.10]">
                    <th className="text-left py-2 text-white">Device</th>
                    <th className="text-left py-2 text-white">Max Zs (20C)</th>
                    <th className="text-left py-2 text-white">Corrected (0.8)</th>
            ...
            {rcdTesting.rcdTypes.map((rcd, index) => (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <Badge variant="outline" className="border-white/10 text-white text-xs font-mono">
                    {rcd.symbol}
                  </Badge>
                </div>
                <p className="text-xs text-white mb-2">{rcd.description}</p>
                <p className="text-xs text-white mb-1">
                  <strong>Application:</strong> {rcd.application}
                </p>
                <p className="text-xs text-white mb-1">
                  <strong>Test Method:</strong> {rcd.testMethod}
                </p>
                <p className="text-xs text-white">
                  <strong>Limitations:</strong> {rcd.limitations}
                </p>
              </div>
            ))}
          </div>
              ...
              <div className="space-y-2">
                {phaseSequenceTesting.phaseColours.current.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-sm">
                    ...
                    <span className="text-white">{item.colour}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">
              {emergencyLightingTesting.title}
            </CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
            ...
            {emergencyLightingTesting.testTypes.map((test, index) => (
              ...
              >
                ...
                <p className="text-sm text-white">
                  <strong>Record:</strong> {test.record}
                </p>
              </div>
            ))}
          </div>
          ...
          {documentationRequirements.certificates.map((cert, index) => (
            <div key={index} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              {'attachments' in cert && cert.attachments && (
                <div className="mb-3">
                  <h5 className="font-medium text-white mb-2 text-sm">Required Attachments</h5>
                  <ul className="space-y-1">
              ...
              {'requirements' in cert && cert.requirements && (
                <div className="mb-3">
                  <h5 className="font-medium text-white mb-2 text-sm">Requirements</h5>
                  <ul className="space-y-1">
              ...
              {'classifications' in cert && cert.classifications && (
                <div className="mb-3">
                  <h5 className="font-medium text-white mb-2 text-sm">Classification Codes</h5>
                  <div className="space-y-1">
              ...
              {'frequencyGuide' in cert && cert.frequencyGuide && (
                <div>
                  <h5 className="font-medium text-white mb-2 text-sm">EICR Frequency Guide</h5>
      ...
      <Card variant="plain">
        ...
        <CardContent className="space-y-4 p-0">
            ...
            <div className="space-y-3">
              {eicrRequirements.inspectionScope.areas.map((area, index) => (
                <div key={index} className="bg-white/[0.06] p-3 rounded border border-white/[0.10]">
                  <h5 className="font-medium text-white text-sm mb-2">{area.area}</h5>
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h5 className="text-sm font-medium text-white mb-2">Sampling Guidance:</h5>
                <ul className="space-y-1">
              ...
              <div>
                <h5 className="text-sm font-medium text-white mb-2">100% Testing Required:</h5>
                <ul className="space-y-1">
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.10]">
                    <th className="text-left py-2 text-white">Defect</th>
                    <th className="text-left py-2 text-white">Typical Code</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {testEquipment.equipment.map((equip, index) => (
              ...
              >
                ...
                <p className="text-xs text-white mb-1">
                  <strong>Requirements:</strong> {equip.requirements}
                </p>
                <p className="text-xs text-white mb-1">
                  <strong>Calibration:</strong> {equip.calibration}
                </p>
                <p className="text-xs text-white">
                  <strong>Note:</strong> {equip.notes}
                </p>
              </div>
            ))}
          </div>
