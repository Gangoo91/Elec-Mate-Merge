...
const IndustrialRiskManagement = () => {
  ...
  const exMarkings = {
    ...
    gasGroups: [
      {
        group: 'IIA',
        substances: 'Propane, butane, methane, diesel vapours',
        ignitionEnergy: 'Lowest',
      },
      { group: 'IIB', substances: 'Ethylene, town gas, some alcohols', ignitionEnergy: 'Medium' },
      {
        group: 'IIC',
        substances: 'Hydrogen, acetylene, carbon disulphide',
        ignitionEnergy: 'Highest',
      },
    ],
    temperatureClasses: [
  ...
  const arcFlash = {
    basics: {
      definition:
        'An electrical explosion resulting from a fault that causes air to become conductive',
      hazards: [
  ...
  const workingAtHeight = {
    ...
    hierarchy: [
      {
        level: 1,
        method: 'Avoid work at height where possible',
        description: 'Ground-level alternatives',
      },
      {
        level: 2,
        method: 'Use existing safe place of work',
        description: 'Fixed platforms, walkways',
      },
      {
        level: 3,
        method: 'Provide work equipment to prevent falls',
        description: 'Scaffolds, MEWP, podiums',
      },
      {
        level: 4,
        method: 'Provide work equipment to minimise distance/consequences',
        description: 'Safety nets, air bags',
      },
      {
        level: 5,
        method: 'Provide work equipment to minimise consequences',
        description: 'Fall arrest harnesses',
      },
    ],
  };
  ...
  const confinedSpaces = {
    definition:
      'A space that is substantially enclosed, has limited access/egress, and where there is risk of serious injury from hazardous substances or conditions',
    examples: [
    ...
    atmosphereLimits: [
      ...
      { parameter: 'Carbon Monoxide', min: '-', max: '30 ppm', notes: 'Short term exposure limit' },
      {
        ...
        min: '-',
        max: '10 ppm',
        notes: 'Short term exposure limit',
      },
      {
        parameter: 'LEL (flammable gas)',
        min: '-',
        max: '10% of LEL',
        notes: 'Should be zero for hot work',
      },
    ],
  };
    ...
    <div className="space-y-6">
              ...
              <div className="bg-purple-500/10 p-4 rounded-lg border border-purple-500/20">
                ...
                <div className="space-y-2">
                  {exMarkings.gasGroups.map((group, i) => (
                    <div
                      key={i}
                      className="bg-purple-500/5 p-3 rounded border border-purple-500/10 flex items-center justify-between"
                    >
                      <div>
                    ...
                    <div key={i} className="bg-red-500/5 p-3 rounded border border-red-500/20">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-1">
                        {cat.equipment.map((equip, j) => (
                          <div key={j} className="text-xs text-white">
                            - {equip}
                          </div>
                        ))}
                      </div>
          ...
          <Card className="border-red-500/50 bg-red-600/20">
            ...
            <CardContent className="space-y-3 text-sm text-white">
              <p>
                ...
                energy released can exceed 200kJ in a fraction of a second.
              </p>
              <p>
                <strong className="text-red-300">Always de-energise if possible:</strong> The safest
                approach is to work on de-energised equipment. Live work should only be performed
                when absolutely necessary and with full risk assessment.
              </p>
              <p>
                <strong className="text-red-300">PPE is the last line of defence:</strong> Arc-rated
                PPE reduces injury severity but does not eliminate risk. Engineering controls and
                work practices are the primary protection.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
                    ...
                    <tbody className="text-white">
                      {workingAtHeight.minimumClearances.map((item, i) => (
                        <tr key={i} className="border-b border-orange-500/20">
                          ...
                          <td className="py-2 px-3 font-medium text-orange-200">
                            {item.clearance}
                          </td>
                        </tr>
                      ))}
          ...
          <Card className="border-red-500/50 bg-red-500/20">
            ...
            <CardContent className="space-y-3 text-sm text-white">
              <p>
                ...
                Many confined space fatalities involve would-be rescuers. Always have rescue
                arrangements in place BEFORE entry.
              </p>
              <p>
                <strong className="text-red-300">Rescue equipment must be available:</strong> This
                may include retrieval systems, breathing apparatus, communication equipment, and
                first aid provision.
              </p>
              <p>
                <strong className="text-red-300">Raise the alarm immediately:</strong> If an
                incident occurs, call emergency services and trained rescue personnel. Do not delay
                alerting others.
              </p>
            </CardContent>
          </Card>
      ...
      <Card className="border-red-500/50 bg-red-500/10">
        ...
        <CardContent className="space-y-3 text-sm text-white">
          ...
          <p>
            <strong className="text-red-300">Competency is Essential:</strong> Work in hazardous
            areas, confined spaces, and on high-voltage equipment requires specific training and
            certification (CompEx, HV AP, etc.).
          </p>
          <p>
            <strong className="text-red-300">Never Work Alone:</strong> High-risk activities require
            supervision, assistance, and emergency backup. Ensure someone knows where you are and
            what you are doing.
          </p>
          <p>
            <strong className="text-red-300">If in Doubt - STOP:</strong> If conditions change or
            something doesn't seem right, stop work and reassess. No job is worth risking your life.
          </p>
        </CardContent>
      </Card>
    </div>
