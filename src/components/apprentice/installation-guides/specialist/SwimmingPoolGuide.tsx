...
const SwimmingPoolGuide = () => {
  ...
  const selvRequirements = {
    ...
    description:
      'SELV provides protection by limitation of voltage to values that are not harmful under all conditions',
    voltageLimits: {
  ...
  const supplementaryBonding = {
    ...
    description:
      'Supplementary equipotential bonding connects together extraneous and exposed conductive parts within the pool area',
    requirement:
      'All extraneous-conductive-parts and exposed-conductive-parts in Zones 0, 1, and 2 must be connected by supplementary bonding conductors',
    partsToBeconnected: [
      {
    ...
    conductorSizing: [
      {
        connection: 'Between extraneous-conductive-parts',
        size: 'Minimum 2.5mm² copper (4mm² if not mechanically protected)',
      },
      {
        connection: 'To exposed-conductive-parts',
        size: 'Not less than half the CPC of connected equipment',
      },
  ...
  const poolPumpCircuits = {
    ...
    components: [
      ...
      },
      {
        ...
        protection: ['As per pump circuit', 'Timer protection where fitted'],
      ...
      },
      {
        ...
        protection: ['Low current circuit protection', 'SPD protection recommended', '30mA RCD'],
  ...
  const hotTubsSpas = {
    ...
    description:
      'Hot tubs and spa pools have similar requirements to swimming pools but with some specific considerations',
    zones: [
    ...
    <div className="space-y-6">
      ...
      <Card className="border-white/[0.06] bg-white/[0.02]">
        <CardHeader>
          <div className="flex items-center gap-3">
            <Waves className="h-8 w-8 text-white/55" />
            <div>
      ...
      <Alert className="border-red-500/50 bg-red-500/10">
        ...
        <AlertDescription className="text-red-200">
          <strong className="text-red-300">CRITICAL SAFETY WARNING:</strong> Swimming pools present
          an extremely high risk of electric shock due to water immersion reducing body resistance
          and providing excellent electrical contact. Strict adherence to zone requirements, IP
          ratings, SELV systems, and supplementary bonding is essential. All circuits must have 30mA
          RCD protection. Any deviation from BS 7671 Section 702 requirements can be fatal.
        </AlertDescription>
      </Alert>
      ...
      <Card className="border-white/[0.06] bg-white/5">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-white">Zone Definitions (BS 7671 Section 702)</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <Alert className="border-white/[0.06] bg-white/[0.02] mb-4">
            ...
            {zoneDefinitions.map((zone, idx) => (
              ...
              >
                ...
                <p className="text-xs text-white mb-3">
                  <strong>Extent:</strong> {zone.extends}
                </p>
                <p className="text-xs text-white mb-3">
                  <strong>Voltage:</strong> {zone.voltage}
                </p>

                <div className="space-y-2">
                  <div>
                    <span className="text-xs text-white/55">Permitted Equipment:</span>
      ...
      <Card className="border-white/[0.06] bg-white/5">
        <CardHeader>
          <div className="flex items-center gap-2">
            <Droplets className="h-6 w-6 text-white/55" />
            <CardTitle className="text-white">{ipRatingRequirements.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {ipRatingRequirements.ratings.map((rating, idx) => (
                  ...
                  >
                    <td className="p-3 text-white font-mono font-bold">{rating.rating}</td>
                    <td className="p-3 text-white">{rating.meaning}</td>
                    <td className="p-3 text-white">{rating.zones.join(', ')}</td>
          ...
          <div className="bg-white/[0.02] p-4 rounded-lg border border-white/[0.06]">
            ...
            <p className="text-sm text-white">
              <strong>Second digit:</strong> {ipRatingRequirements.secondDigit}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-yellow-500/30 bg-white/5">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-yellow-500/10 p-4 rounded-lg border border-yellow-500/30">
            ...
            <p className="text-sm text-white">
              <strong>DC:</strong> {selvRequirements.voltageLimits.dc}
            </p>
          </div>
      ...
      <Card className="border-white/[0.06] bg-white/5">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-white">{supplementaryBonding.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              {supplementaryBonding.conductorSizing.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-sm">
                  <span className="text-white">{item.connection}</span>
      ...
      <Card className="border-white/[0.06] bg-white/5">
        <CardHeader>
          <div className="flex items-center gap-2">
            <Fan className="h-6 w-6 text-white/55" />
            <CardTitle className="text-white">{poolPumpCircuits.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <Tabs defaultValue="pump" className="w-full">
            ...
            {poolPumpCircuits.components.map((component, idx) => (
              <TabsContent
                key={idx}
                value={component.component.toLowerCase().split(' ')[0]}
                className="space-y-4"
              >
                <div className="bg-white/[0.02] p-4 rounded-lg border border-white/[0.06]">
                      ...
                      <ul className="space-y-1">
                        {component.circuitDesign.map((item, itemIdx) => (
                          <li key={itemIdx} className="text-xs text-white flex items-start gap-2">
                            <Cable className="h-3 w-3 text-white/55 mt-0.5 flex-shrink-0" />
                            {item}
                          </li>
                        ))}
                      </ul>
      ...
      <Card className="border-white/[0.06] bg-white/5">
        <CardHeader>
          <div className="flex items-center gap-2">
            <ThermometerSun className="h-6 w-6 text-white/55" />
            <CardTitle className="text-white">{heatPumpConnections.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <ul className="space-y-1">
                      {type.electricalRequirements.map((req, reqIdx) => (
                        <li key={reqIdx} className="text-xs text-white flex items-start gap-2">
                          <Zap className="h-3 w-3 text-white/55 mt-0.5 flex-shrink-0" />
                          {req}
                        </li>
                      ))}
                    </ul>
      ...
      <Card className="border-white/[0.06] bg-white/5">
        <CardHeader>
          <div className="flex items-center gap-2">
            <Waves className="h-6 w-6 text-white/55" />
            <CardTitle className="text-white">{hotTubsSpas.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="bg-white/[0.02] p-4 rounded-lg border border-white/[0.06]">
            ...
            <ul className="space-y-1">
              {hotTubsSpas.zones.map((zone, idx) => (
                <li key={idx} className="text-sm text-white flex items-start gap-2">
                  <Building2 className="h-4 w-4 text-white/55 mt-0.5 flex-shrink-0" />
                  {zone}
                </li>
              ))}
            </ul>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {hotTubsSpas.types.map((type, idx) => (
              <div key={idx} className="bg-white/[0.02] p-4 rounded-lg border border-white/[0.06]">
                ...
                <p className="text-xs text-white mb-1">
                  <strong>Circuit:</strong> {type.circuit}
                </p>
            ...
            <div className="bg-white/[0.02] p-4 rounded-lg border border-white/[0.06]">
              ...
              <ul className="space-y-1">
                {hotTubsSpas.outdoorConsiderations.map((item, idx) => (
                  <li key={idx} className="text-xs text-white flex items-start gap-2">
                    <Sun className="h-3 w-3 text-white/55 mt-0.5 flex-shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <Card className="border-white/[0.06] bg-white/5">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-white">{testingRequirements.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          ...
          {commonIssues.map((issue, idx) => (
            <div key={idx} className="bg-red-500/10 p-4 rounded-lg border border-red-500/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <ul className="space-y-0.5 mt-1">
                    {issue.causes.map((cause, causeIdx) => (
                      <li key={causeIdx} className="text-xs text-white">
                        - {cause}
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  <span className="text-xs text-white/55">Solution:</span>
      ...
      <Alert className="border-white/[0.06] bg-white/[0.02]">
        ...
        <AlertDescription className="text-white/85">
          ...
          is critical for safety. Regular inspection and testing is vital for ongoing safety. When
          in doubt, seek specialist advice. Lives depend on getting pool installations right.
        </AlertDescription>
      </Alert>
    </div>
  );
