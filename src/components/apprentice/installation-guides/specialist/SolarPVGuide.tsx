...
const SolarPVGuide = () => {
    ...
    stages: [
      {
        ...
        color: 'yellow',
        description:
          'Solar panels convert sunlight into DC (Direct Current) electricity through the photovoltaic effect',
        details: [
      ...
      },
      {
        ...
        color: 'orange',
        description:
          'Inverters convert DC electricity from the panels to AC electricity for use in the property',
        details: [
      ...
      },
      {
        ...
        color: 'blue',
        description:
          'Connection to the grid allows export of excess electricity and import when needed',
        details: [
  ...
  const dcSafetyConsiderations = [
    {
      ...
      severity: 'Critical',
      description:
        "DC arcs are more dangerous than AC as they don't self-extinguish at zero crossing",
      risks: [
    ...
    },
    {
      ...
      severity: 'Critical',
      description:
        'Solar panels generate electricity whenever exposed to light - cannot be switched off',
      risks: [
    ...
    configurations: [
      {
        ...
        advantages: [
          ...
          'Fewer MPPT inputs needed',
        ],
        disadvantages: [
          'Shading affects entire string',
          'All panels must match',
          'Limited flexibility',
        ],
      },
      {
  ...
  const mcsCertification = {
    ...
    overview:
      'MCS (Microgeneration Certification Scheme) is required for customers to access the Smart Export Guarantee (SEG) and other incentives',
  ...
  const gridConnection = {
    ...
    g98: {
      name: 'G98',
      applies:
        'Single phase up to 16A per phase (3.68kW), Three phase up to 16A per phase (11.04kW)',
      process: [
    ...
    dcSide: [
      {
        ...
        description: 'Measure voltage of each string with no load',
        acceptable: 'Within 5% of expected value',
      },
      {
        test: 'Short Circuit Current (Isc)',
        description: 'Measure current capacity of each string',
        acceptable: 'Within 10% of expected value',
      },
      {
        test: 'Polarity Check',
        description: 'Confirm correct polarity of all connections',
        acceptable: 'Correct polarity confirmed',
      },
      {
        test: 'Insulation Resistance (DC)',
        description: 'IR between conductors and earth',
        acceptable: 'Minimum 1M ohm per kV of system voltage',
      },
      {
        test: 'Continuity of Earthing',
        description: 'Verify protective earth connections',
        acceptable: 'Low resistance, continuous path',
      },
      {
        test: 'Visual Inspection',
        description: 'Check for damage, correct installation',
        acceptable: 'No defects, compliant installation',
      },
    ],
    acSide: [
      {
        test: 'Continuity of Protective Conductors',
        description: 'R1+R2 for AC circuit',
        acceptable: 'Less than measured values',
      },
      {
        test: 'Insulation Resistance (AC)',
        description: 'Between live conductors and earth',
        acceptable: 'Minimum 1M ohm',
      },
      {
        test: 'Polarity',
        description: 'Verify L, N, E connections',
        acceptable: 'Correct polarity confirmed',
      },
      {
        test: 'Earth Fault Loop Impedance (Zs)',
        description: 'Verify protective device operation',
        acceptable: 'Less than maximum for protective device',
      },
      {
        test: 'RCD Operation',
        description: 'Test RCD on AC circuit',
        acceptable: 'Trip time within limits',
      },
      {
        test: 'Functional Testing',
        description: 'System operation and monitoring',
        acceptable: 'System generates and exports correctly',
      },
    ],
    documentation: [
  ...
  const smartExportGuarantee = {
    ...
    description:
      'Government scheme requiring licensed electricity suppliers to offer tariffs for small-scale low-carbon generation export',
    eligibility: [
    ...
    <div className="space-y-6">
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-3">
            <Sun className="h-8 w-8 text-elec-yellow" />
            <div>
              ...
              <p className="text-elec-yellow">
                BS 7671 Section 712 - Comprehensive guide for UK electricians
              </p>
            </div>
          </div>
        </CardHeader>
      ...
      <Alert className="border-white/[0.12] border-l-[3px] border-l-red-500 bg-white/[0.06]">
        ...
        <AlertDescription className="text-red-200">
          <strong className="text-red-300">CRITICAL DC SAFETY WARNING:</strong> Solar panels
          generate DC electricity whenever exposed to light and cannot be switched off. Combined
          string voltages can exceed 600V DC. Always treat all DC conductors as LIVE. DC shocks are
          more difficult to release from than AC. Use appropriate PPE and follow safe working
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">{howSolarWorks.title}</CardTitle>
          </div>
        </CardHeader>
            ...
            <div key={idx} className="relative">
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-white/85 mb-3">{stage.description}</p>
              ...
              {idx < howSolarWorks.stages.length - 1 && (
                <div className="flex justify-center my-2">
                  <ArrowRight className="h-5 w-5 text-elec-yellow rotate-90" />
                </div>
              )}
            </div>
          ))}
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">System Components</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
            ...
            {systemComponents.map((component, idx) => (
              ...
              >
                <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
                  ...
                  <p className="text-sm text-white/85 mb-4">{component.description}</p>
                      ...
                      <ul className="space-y-1">
                        {component.specifications.map((spec, specIdx) => (
                          <li key={specIdx} className="text-xs text-white flex items-start gap-2">
                            <Gauge className="h-3 w-3 text-white/70 mt-0.5 flex-shrink-0" />
                            {spec}
                          </li>
                        ))}
                      </ul>
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">DC Safety Considerations</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
          {dcSafetyConsiderations.map((item, idx) => (
            <div
              key={idx}
              className={`p-4 rounded-lg border ${item.severity === 'Critical' ? 'bg-white/[0.06] border-red-500/30' : 'bg-white/[0.06] border-white/[0.10]'}`}
            >
              ...
              <p
                className={`text-sm ${item.severity === 'Critical' ? 'text-red-200' : 'text-white/85'} mb-3`}
              >
                {item.description}
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <h5 className="font-medium text-white/85 text-sm mb-2">Mitigations</h5>
                  <ul className="space-y-1">
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">{stringSizing.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
          <div className="space-y-4">
            {stringSizing.principles.map((principle, idx) => (
              <div key={idx} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
                <h4 className="font-medium text-white/85 mb-2">{principle.principle}</h4>
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {stringSizing.configurations.map((config, idx) => (
                ...
                >
                  <h5 className="font-medium text-white/85 mb-2">{config.type}</h5>
                  <p className="text-xs text-white mb-3">{config.description}</p>
                  <div className="space-y-2">
                    <div>
                      <span className="text-xs text-white/70">Advantages:</span>
                      <ul className="space-y-0.5 mt-1">
                        {config.advantages.map((adv, advIdx) => (
                          <li key={advIdx} className="text-xs text-white">
                            + {adv}
                          </li>
                        ))}
                      </ul>
                    ...
                    <div>
                      ...
                      <ul className="space-y-0.5 mt-1">
                        {config.disadvantages.map((dis, disIdx) => (
                          <li key={disIdx} className="text-xs text-white">
                            - {dis}
                          </li>
                        ))}
                      </ul>
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">Inverter Selection Guide</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {inverterTypes.map((inverter, idx) => (
              <div key={idx} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
                <h4 className="font-medium text-white/85 mb-1">{inverter.type}</h4>
                ...
                <p className="text-xs text-white/85 mb-2">
                  <strong>Best for:</strong> {inverter.bestFor}
                </p>

                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <span className="text-xs text-white/70">Pros:</span>
                    <ul className="space-y-0.5 mt-1">
                      {inverter.pros.map((pro, proIdx) => (
                        <li key={proIdx} className="text-xs text-white">
                          + {pro}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div>
                    ...
                    <ul className="space-y-0.5 mt-1">
                      {inverter.cons.map((con, conIdx) => (
                        <li key={conIdx} className="text-xs text-white">
                          - {con}
                        </li>
                      ))}
                    </ul>
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">Installation Best Practices</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {installationBestPractices.map((category, idx) => (
              <div key={idx} className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-medium text-white/85">{category.category}</h4>
                </div>
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">{roofAccessFireSafety.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            <h4 className="font-medium text-white/85 mb-2">Roof Access Requirements</h4>
            <ul className="space-y-1">
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">{mcsCertification.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              <h4 className="font-medium text-white/85 mb-3">Installer Requirements</h4>
              <ul className="space-y-2">
                {mcsCertification.installerRequirements.map((req, idx) => (
                  <li key={idx} className="text-sm text-white">
                    <strong className="text-white">{req.requirement}:</strong>
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">{gridConnection.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <p className="text-sm text-white/85 mb-3">
                <strong>Applies to:</strong> {gridConnection.g98.applies}
              </p>
              ...
              <ul className="space-y-1">
                {gridConnection.g98.requirements.map((req, idx) => (
                  <li key={idx} className="text-xs text-white flex items-start gap-2">
                    <CheckCircle className="h-3 w-3 text-white/70 mt-0.5" />
                    {req}
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              ...
              <p className="text-sm text-white/85 mb-3">
                <strong>Applies to:</strong> {gridConnection.g99.applies}
              </p>
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">Testing Requirements</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">{smartExportGuarantee.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              <h4 className="font-medium text-white/85 mb-3">Eligibility</h4>
              <ul className="space-y-1">
            ...
            <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
              <h4 className="font-medium text-white/85 mb-3">Application Process</h4>
              <ul className="space-y-1">
          ...
          <Alert className="border-white/[0.10] bg-white/[0.06]">
            <BadgePoundSterling className="h-4 w-4 text-white/70" />
            <AlertDescription className="text-white/85 text-sm">
      ...
      <Card variant="plain">
        <CardHeader className="p-0 pb-3">
          <div className="flex items-center gap-2">
            <CardTitle className="text-[15px] font-semibold tracking-tight text-white">{batteryIntegration.title}</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0">
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            <h4 className="font-medium text-white/85 mb-3">Benefits of Battery Storage</h4>
          ...
          <div className="bg-white/[0.06] p-4 rounded-lg border border-white/[0.10]">
            <h4 className="font-medium text-white/85 mb-3">Key Considerations</h4>
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {batteryIntegration.considerations.map((item, idx) => (
                <li key={idx} className="text-sm text-white flex items-start gap-2">
                  <Lightbulb className="h-4 w-4 text-white/70 mt-0.5" />
                  {item}
                </li>
              ))}
            </ul>
      ...
      <Alert className="border-white/[0.10] bg-white/[0.06]">
        ...
        <AlertDescription className="text-white/85">
          <strong className="text-white">Remember:</strong> Solar PV installation requires MCS
          certification for customers to access the Smart Export Guarantee. Always follow
          manufacturer guidelines, MCS standards (MIS 3002), and BS 7671 Section 712 requirements.
          Ensure proper training in DC safety before working on solar PV systems.
        </AlertDescription>
      </Alert>
    </div>
  );
