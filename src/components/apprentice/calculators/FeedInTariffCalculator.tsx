...
export function FeedInTariffCalculator() {
  ...
  const calculateFeedInTariff = () => {
    ...
    if (technologyType === 'solar_pv') {
      ...
      yearlyGeneration = size * 24 * 365 * 0.4;
    }
  ...
  const getPaybackColor = (years: number) =>
    years <= 8 ? 'text-green-400' : years <= 12 ? 'text-amber-400' : 'text-red-400';
  const getROIColor = (roi: number) =>
    roi >= 10 ? 'text-green-400' : roi >= 5 ? 'text-amber-400' : 'text-red-400';
        ...
        <div className="space-y-4 animate-fade-in">
          <CalculatorResult category="renewable">
            ...
            <ResultsGrid columns={2}>
              ...
              <ResultValue
                label="FIT Rate"
                value={(result.fitRate * 100).toFixed(1)}
                unit="p/kWh"
                category="renewable"
                size="sm"
              />
              ...
              <ResultValue
                label="20-Year Total"
                value={`£${(result.totalReturn20Years / 1000).toFixed(0)}k`}
                category="renewable"
                size="sm"
              />
            </ResultsGrid>
            ...
            <div className="p-3 rounded-xl bg-orange-500/10 border border-orange-500/30">
              <div className="flex items-start gap-2">
                ...
                <p className="text-sm text-orange-200">
                  <strong>New Installations:</strong> FIT closed March 2019. SEG rates (3-7p/kWh)
                  apply instead.
                </p>
              </div>
            </div>
            ...
            <div className="calculator-card overflow-hidden" style={{ borderColor: '#60a5fa15' }}>
              ...
              <CollapsibleContent className="p-4 pt-0 space-y-2">
                <p className="text-sm text-blue-200/80">
                  <strong className="text-blue-300">Three income streams:</strong> Generation
                  payments (all electricity), export payments (surplus), and bill savings
                  (self-consumed).
                </p>
                <p className="text-sm text-blue-200/80">
                  <strong className="text-blue-300">Higher self-consumption</strong> improves
                  returns, especially for post-2016 installations with lower FIT rates.
                </p>
              </CollapsibleContent>
            </div>
          </Collapsible>
            ...
            <div className="calculator-card overflow-hidden" style={{ borderColor: '#fbbf2415' }}>
              ...
              <CollapsibleContent className="p-4 pt-0">
                <div className="space-y-2 text-sm text-amber-200/80">
                  <p>
                    <strong className="text-amber-300">FIT Duration:</strong> 20-year guaranteed
                    payments from installation date
                  </p>
                  <p>
                    <strong className="text-amber-300">RPI Linked:</strong> Generation rates
                    increase with inflation
                  </p>
                  <p>
                    <strong className="text-amber-300">Verification:</strong> Check rates with
                    energy supplier or Ofgem tables
                  </p>
                </div>
              </CollapsibleContent>
            </div>
      ...
      <div className="p-3 rounded-xl bg-green-500/10 border border-green-500/20">
        <div className="flex items-start gap-2">
          ...
          <p className="text-sm text-green-200">
            <strong>FIT closed March 2019.</strong> Existing installations receive payments for full
            20-year term.
          </p>
        </div>
      </div>
