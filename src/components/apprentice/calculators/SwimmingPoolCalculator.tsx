...
const SwimmingPoolCalculator = () => {
  ...
  const { inputs, result, errors, handleInputChange, calculateValues, resetCalculator, isValid } =
      ...
      content: (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
          <div className="space-y-4">
            <PoolInputs inputs={inputs} errors={errors} onInputChange={handleInputChange} />
    ...
    <Card className="border-elec-yellow/20 bg-white/5">
      <CardHeader className="pb-4 px-4 lg:px-6">
        ...
        <CardDescription className="text-sm lg:text-base">
          Calculate electrical requirements for swimming pools according to BS 7671 Section 702 and
          safety regulations.
        </CardDescription>
      </CardHeader>
      <CardContent className="px-4 lg:px-6">
        {isMobile ? (
          <DropdownTabs tabs={tabs} placeholder="Select Calculator Section" />
        ) : (
