...
const TimeMaterialsCalculator = () => {
  ...
  // Named `handleCopy`, not `copyToClipboard`: the local handler used to
  // shadow the imported helper of that name and then call itself, so the
  // Copy button recursed until the stack blew rather than copying anything.
  const handleCopy = () => {
    if (!result) return;
    const lines = [
  ...
  return (
    ...
    >
      <CalculatorPanes
                  ...
                  <CollapsibleContent className="pt-2">
                    ...
                    >
                      <p className="text-sm text-white">
                        Always include travel time and expenses in your hourly rate or as a separate
                        line item.
                      </p>
                      <p className="text-sm text-white">
                        Account for waste — add 10–15% to material quantities for cables, trunking
                        and containment.
                      </p>
                      <p className="text-sm text-white">
        ...
        footer={<CalculatorEditorial content={timeMaterialsContent} category={CAT} />}
      />
    </CalculatorCard>
  );
};
