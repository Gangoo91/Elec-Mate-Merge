  ...
  return (
    ...
    >
      <CalculatorPanes
            ...
            {result && (
              <>
                ...
                */}
                  ...
                  <div className="mt-3 p-2 bg-white/[0.04] border border-white/[0.12] rounded-lg">
                    <p className="text-xs text-white">
                      <strong>Inrush current:</strong> Capacitors draw high inrush on energisation.
                      For staged banks, use contactors with pre-insertion resistors to limit inrush.
                    </p>
                  </div>
                  {/*
              ...
              */}
                  <div className="mt-2 p-2 bg-red-500/10 border border-red-500/30 rounded-lg">
                    <p className="text-xs text-white">
                      ...
                      — where a capacitor that may retain a dangerous charge after switch-off is
                      installed behind a barrier or in an enclosure, a warning label shall be
                      provided (a charge is not treated as dangerous if the static-charge voltage
                      falls below 120 V DC in under 5 s). Reg 559.7 additionally requires
                      compensation capacitors of total capacitance exceeding 0.5 µF to be used only
                      with discharge resistors, though that clause sits in the luminaires section.
                    </p>
                  </div>
                </div>
                    ...
                    <div className="text-sm font-mono text-white space-y-3 p-3 rounded-xl bg-white/[0.04] border border-white/5">
                      ...
                      <div>
                        ...
                        <div className="pl-3 border-l-2 border-purple-500/30 space-y-0.5">
                          <p>
                            kVA = kW / PF = {result.currentKW.toFixed(1)} /{' '}
                            {result.currentPF.toFixed(2)} = {result.currentKVA.toFixed(1)} kVA
                          </p>
                          <p>
                      ...
                      <div className="pt-2 border-t border-purple-500/20">
                        ...
                        <div className="pl-3 border-l-2 border-purple-500/30 space-y-0.5">
                          <p>
                            Target kVA = {result.currentKW.toFixed(1)} /{' '}
                            {result.targetPF.toFixed(2)} = {result.targetKVA.toFixed(1)} kVA
                          </p>
                          <p>
                      ...
                      <div className="pt-2 border-t border-purple-500/20">
                        <p className="text-xs text-purple-400 mb-1">
                          Step 3: Required correction kVAR
                        </p>
                      ...
                      <div className="pt-2 border-t border-purple-500/20">
                        ...
                        <div className="pl-3 border-l-2 border-purple-500/30 space-y-0.5">
                          {phases === '3' ? (
                            ...
                            <>
                              <p>
                                I (before) = (kVA × 1000) / V = ({result.currentKVA.toFixed(1)} ×
                                1000) / {supplyVoltage} = {result.currentBefore.toFixed(1)}A
                              </p>
                              <p>
                      ...
                      {(result.annualReactiveSavings > 0 || result.annualMDSavings > 0) && (
                        <div className="pt-2 border-t border-purple-500/20">
                          ...
                          <div className="pl-3 border-l-2 border-purple-500/30 space-y-0.5">
                            {result.annualReactiveSavings > 0 && (
                              <p>
                                Reactive savings = {result.requiredKVAR.toFixed(1)} ×{' '}
                                {operatingHours}hrs × £{reactiveCharge}/kVARh = £
                                {result.annualReactiveSavings.toFixed(0)}/yr
                              </p>
                            )}
                            {result.annualMDSavings > 0 && (
                              <p>
                                MD savings = {(result.currentKVA - result.targetKVA).toFixed(1)} kVA
                                × £{kvaChargeRate}/kVA/month × 12 = £
                                {result.annualMDSavings.toFixed(0)}/yr
                              </p>
                            )}
                            <p>
                              Total ={' '}
            ...
            <Collapsible open={showReference} onOpenChange={setShowReference}>
              ...
              <CollapsibleContent className="p-4 pt-2">
                <div className="space-y-3 text-sm text-white">
                  <p>
                    <strong style={{ color: config.gradientFrom }}>
              ...
              */}
                  <ul className="space-y-1 ml-4 text-xs">
                    <li>
                      • <strong>Reg 331.1(l):</strong> Power factor shall be assessed as a
                      characteristic of equipment likely to have harmful effects on other equipment
                      or services, or to impair the supply
                    </li>
                    <li>
                      • <strong>Reg 331.1(f):</strong> Harmonic currents shall be assessed —
                      relevant wherever capacitors are added to a distorted supply
                    </li>
                    <li>
                      • <strong>Reg 443.4.2:</strong> Protection against switching overvoltages
                      shall be considered where capacitive equipment such as capacitor banks is
                      installed
                    </li>
                    <li>
                    ...
                    <li>
                      • <strong>Regs 462.4 / 416.2.5:</strong> Means of discharge for residual
                      energy, plus a warning label on enclosures holding a capacitor that may retain
                      a dangerous charge
                    </li>
                  ...
                  <div className="mt-2 p-2 bg-red-500/10 border border-red-500/20 rounded-lg">
                    <p className="text-xs text-white">
                      <strong>Warning:</strong> Where the supply is distorted by VFDs, LED drivers
                      or switch-mode power supplies, plain capacitors can resonate with the supply
                      ...
                      selection, not by BS 7671, which requires only that harmonic currents be
                      assessed (Reg 331.1(f)).
                    </p>
                  </div>
                </div>
              </CollapsibleContent>
        ...
        footer={<CalculatorEditorial content={powerFactorCorrectionContent} category="power" />}
      />
    </CalculatorCard>
  );
};
