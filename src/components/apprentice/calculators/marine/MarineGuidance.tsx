...
const MarineGuidance = ({ results, inputs }: MarineGuidanceProps) => {
  const getCriticalityLevel = () => {
    const criticalIssues = results.complianceChecks.filter(
      (check) => check.status === 'non-compliant'
    ).length;
    ...
    if (criticalIssues > 0)
      return {
        level: 'critical',
        color: 'error',
        message: 'Critical compliance issues require immediate attention',
      };
    if (warnings > 2)
      return {
        level: 'caution',
        color: 'warning',
        message: 'Multiple warnings - system review recommended',
      };
    if (warnings > 0)
      return {
        level: 'advisory',
        color: 'warning',
        message: 'Minor issues identified - consider improvements',
      };
    return {
      level: 'compliant',
      color: 'success',
      message: 'System meets marine electrical standards',
    };
  };
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-400 mb-2">
                Professional Marine Electrical Installation Required
              </h3>
              <p className="text-sm text-white mb-3">
                Marine electrical systems must be installed and certified by qualified marine
                electricians. This calculator provides design guidance only and does not replace
                professional installation, inspection, and certification requirements.
              </p>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <ResultCard
              ...
              value={
                Math.round(
                  (results.recommendedBatteryCapacity / results.dailyEnergyConsumption) * 10
                ) / 10
              }
              unit="days"
            ...
            <ResultCard
              ...
              value={Math.round(
                (results.dailyEnergyConsumption / (results.totalChargingCapacity * 0.8 * 8)) * 100
              )}
              unit="%"
              ...
              <div className="space-y-3 text-sm bg-elec-card p-4 rounded-lg border border-elec-yellow/10">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-elec-yellow font-medium">
                    {results.totalContinuousLoad}W (
                    {Math.round((results.totalContinuousLoad / results.peakLoad) * 100)}%)
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-white">Intermittent Loads:</span>
                  <span className="text-elec-yellow font-medium">
                    {results.totalIntermittentLoad}W (
                    {Math.round((results.totalIntermittentLoad / results.peakLoad) * 100)}%)
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-elec-yellow font-medium">
                    {(results.peakLoad / inputs.systemVoltage).toFixed(1)}A
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="bg-elec-card p-4 rounded-lg border border-elec-yellow/10">
                <ul className="space-y-3 text-sm text-white">
                  <li className="flex items-start gap-2">
                    ...
                    <span>
                      Monitor battery voltage regularly - never discharge below 50% for lead-acid
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-elec-yellow mt-1">•</span>
                    <span>
                      Use load prioritisation: Navigation equipment first, comfort loads second
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-elec-yellow mt-1">•</span>
                    <span>
                      Install battery monitor with Ah counting for accurate state of charge
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-elec-yellow mt-1">•</span>
                ...
                <div className="space-y-4 text-sm">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-elec-yellow">
                      {results.recommendedBatteryCapacity}Ah @ {results.batteryBankVoltage}V
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-white">Battery Configuration:</span>
                    <span className="text-white font-medium">
                      {results.numberOfBatteries} × 100Ah batteries
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-white">Usable Capacity:</span>
                    <span className="text-white font-medium">
                      {Math.round(results.recommendedBatteryCapacity * (inputs.maxDischarge / 100))}
                      Ah
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-white font-medium">
                      {Math.round(
                        results.recommendedBatteryCapacity * (1 - inputs.maxDischarge / 100)
                      )}
                      Ah
                    </span>
                  </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="p-4 bg-elec-card rounded-lg border border-elec-yellow/10">
                  ...
                  <div className="text-white text-xs leading-relaxed">
                    • Lowest cost • Requires ventilation • 50% DoD • 3-5 year life • Regular
                    maintenance
                  </div>
                </div>
                <div className="p-4 bg-elec-card rounded-lg border border-elec-yellow/10">
                  ...
                  <div className="text-white text-xs leading-relaxed">
                    • Sealed design • 80% DoD • 5-7 year life • Minimal maintenance • Good vibration
                    resistance
                  </div>
                </div>
                <div className="p-4 bg-elec-card rounded-lg border border-green-500/30 bg-green-500/10">
                  <div className="font-semibold mb-2 text-green-400">
                    LiFePO4 (Recommended for performance)
                  </div>
                  <div className="text-white text-xs leading-relaxed">
                    • 90% DoD • 10-15 year life • Lightweight • Fast charging • Built-in BMS
                    protection
                  </div>
                </div>
              </div>
              ...
              <div className="bg-elec-card p-6 rounded-lg border border-elec-yellow/20">
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-elec-yellow font-medium">
                      {inputs.alternatorRating}W (
                      {Math.round((inputs.alternatorRating / results.totalChargingCapacity) * 100)}
                      %)
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-elec-yellow font-medium">
                      {inputs.solarPanels}W (
                      {Math.round((inputs.solarPanels / results.totalChargingCapacity) * 100)}%)
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-white">Wind Generator:</span>
                    <span className="text-elec-yellow font-medium">
                      {inputs.windGenerator}W (
                      {Math.round((inputs.windGenerator / results.totalChargingCapacity) * 100)}%)
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-white">Shore Charger:</span>
                    <span className="text-elec-yellow font-medium">
                      {inputs.shoreCharger}W (
                      {Math.round((inputs.shoreCharger / results.totalChargingCapacity) * 100)}%)
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm font-bold border-t border-elec-yellow/30 pt-3">
                    ...
                    <span className="text-elec-yellow text-lg">
                      {results.totalChargingCapacity}W
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-elec-card rounded-lg border border-elec-yellow/10">
                  ...
                  <div className="text-white text-sm">
                    Minimum 20% of battery bank capacity (Ah) for adequate charging rate
                  </div>
                </div>
                <div className="p-4 bg-elec-card rounded-lg border border-elec-yellow/10">
                  <div className="font-semibold text-elec-yellow mb-2">Solar Array</div>
                  <div className="text-white text-sm">
                    Size for 20-30% of daily consumption to extend battery life
                  </div>
                </div>
                <div className="p-4 bg-elec-card rounded-lg border border-elec-yellow/10">
                  <div className="font-semibold text-elec-yellow mb-2">Charge Controllers</div>
                  <div className="text-white text-sm">
                    Use MPPT controllers for solar, 3-stage charging for alternators
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-white text-sm">
                  <span className="text-elec-yellow">•</span> Install galvanic isolator on shore
                  power connection
                </div>
              </div>
              <div className="text-center">
                <div className="text-white text-sm">
                  <span className="text-elec-yellow">•</span> Use sacrificial anodes on metal
                  components
                </div>
              </div>
              <div className="text-center">
                <div className="text-white text-sm">
                  <span className="text-elec-yellow">•</span> Apply anti-corrosion compounds to all
                  connections
                </div>
              </div>
              <div className="text-center">
                <div className="text-white text-sm">
                  <span className="text-elec-yellow">•</span> Regular inspection and maintenance
                  schedule essential
                </div>
              </div>
            </div>
