...
export const deviceTypeOptions: DeviceTypeOption[] = [
  ...
  {
    value: 'fuse-rewireable',
    label: 'BS 3036 Rewireable',
    standard: 'BS 3036',
    i2Multiplier: 2.0,
    maxRating: 100,
  },
...
export const validateProtectiveDevice = (
  ...
  if (!check1) {
    ...
    if (nextRating) {
      suggestions.push(
        `Increase device rating to at least ${nextRating}A (Ib = ${designCurrent}A)`
      );
    } else {
      suggestions.push(
        `Design current ${designCurrent}A exceeds maximum rating for this device type`
      );
    }
  }
  ...
  if (!check2) {
    ...
    if (validRatings.length > 0) {
      ...
      suggestions.push(
        `Reduce device rating to ${maxValidRating}A or less (Iz = ${effectiveCapacity}A)`
      );
    } else if (nextCableSizeUp) {
      suggestions.push(
        `Upgrade cable to ${nextCableSizeUp.size}mm² (Iz = ${nextCableSizeUp.capacity}A) to use ${rating}A device`
      );
    } else {
      suggestions.push(
        `Cable capacity ${effectiveCapacity}A is too low for ${rating}A device — upgrade cable size`
      );
    }
  }
  ...
  if (!check3 && check2) {
    ...
    if (deviceType === 'fuse-rewireable') {
      ...
      const validRating = ratingsByDevice['fuse-rewireable']
        .filter((r) => r <= maxRating && r >= designCurrent)
        .pop();
      if (validRating) {
      ...
      suggestions.push(
        `I₂ (${i2.toFixed(1)}A) exceeds 1.45 × Iz (${limit145Iz.toFixed(1)}A) — reduce device rating or upgrade cable`
      );
    }
  }
  ...
  return {
    ...
    checks: {
      ...
      i2LessEqual145Iz: {
        ...
        limit: Math.round(limit145Iz * 10) / 10,
      },
    },
    allPassed,
    suggestions,
