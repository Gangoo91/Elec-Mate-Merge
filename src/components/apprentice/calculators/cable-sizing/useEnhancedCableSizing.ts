...
export const useEnhancedCableSizing = () => {
  ...
  const validateInputs = useCallback(
    (inputs: EnhancedCableSizingInputs): Record<string, string> => {
  ...
  const selectSuitableCables = useCallback(
    (inputs: EnhancedCableSizingInputs, designCurrent: number, environmentalFactors: any) => {
      ...
      const suitableCables = availableCables.filter((cable) => {
        const currentRating =
          cable.currentRating.swa || cable.currentRating.pvc || cable.currentRating.xlpe;
      ...
      const cablesWithVoltageDrops = suitableCables.map((cable) => {
        const voltageDropPerMeter =
          (cable.voltageDropPerAmpereMeter * designCurrent * inputs.length) / 1000; // Convert mV to V
  ...
  const calculateCableSuitability = useCallback(
    (cable: EnhancedCableSizeOption, inputs: EnhancedCableSizingInputs, designCurrent: number) => {
      const currentRating =
        cable.currentRating.swa || cable.currentRating.pvc || cable.currentRating.xlpe;
      ...
      if (currentMargin < 0) {
        ...
        warningNotes.push(
          `Insufficient current rating: ${currentRating}A < ${designCurrent.toFixed(1)}A required`
        );
      ...
      if (cable.calculatedVoltageDrop) {
        ...
        if (voltageDropPercentage > inputs.voltageDropLimit) {
          suitabilityScore -= 40;
          warningNotes.push(
            `Voltage drop ${voltageDropPercentage.toFixed(2)}% exceeds limit of ${inputs.voltageDropLimit}%`
          );
        }
      }
  ...
  const calculateEnhancedCableSize = useCallback(() => {
    ...
    const complianceChecks = [
      ...
      },
      {
        ...
        status:
          ((recommendedCable.cable.calculatedVoltageDrop || 0) / inputs.voltage) * 100 <=
          inputs.voltageDropLimit
            ? ('pass' as const)
            : ('fail' as const),
    ...
    if (inputs.length > 100) {
      recommendations.push(
        'Long cable runs may benefit from voltage drop calculations using actual cable impedance'
      );
    }
    ...
    selectSuitableCables,
    calculateCableSuitability,
  ]);
  ...
  const saveToHistory = useCallback(
    (projectName: string, notes: string = '') => {
