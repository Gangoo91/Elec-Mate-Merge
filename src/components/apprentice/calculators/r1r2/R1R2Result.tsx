          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-mono">
                {result.lineConductorCSA}mm² {result.conductorMaterial}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-elec-light">CPC conductor:</span>
              <span className="font-mono">
                {result.cpcConductorCSA}mm² {result.conductorMaterial}
              </span>
            </div>
          </div>
          <div className="space-y-2">
      ...
      <Card className="bg-elec-card border-elec-yellow/20">
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 bg-green-500/10 border border-green-500/30 rounded">
                ...
                <span className="font-mono text-green-300 font-bold text-lg">
              ...
              <div className="flex justify-between items-center p-3 bg-yellow-500/10 border border-yellow-500/30 rounded">
                ...
                <span className="font-mono text-yellow-300 font-semibold">
                  {result.continuityLimit.toFixed(4)} Ω
                </span>
              </div>
            </div>
          </div>
          ...
          {measuredValue && testStatus && (
            <div
              className={`p-4 border rounded ${testStatus.color.includes('red') ? 'border-red-500/30' : testStatus.color.includes('yellow') ? 'border-yellow-500/30' : 'border-green-500/30'}`}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="font-medium">Test Result:</span>
                <Badge className={`${testStatus.color}`}>{testStatus.status}</Badge>
              </div>
              <div className="grid gap-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span>Margin:</span>
                  <span className="font-mono">
                    {testStatus.margin} Ω ({testStatus.marginPercent}%)
                  </span>
                </div>
        ...
        <CardContent className="space-y-4 text-sm">
          {(() => {
            const resistanceData =
              result.conductorMaterial === 'copper' ? copperResistance20C : aluminiumResistance20C;
              ...
              <>
                ...
                <div className="space-y-2">
                  <p className="text-purple-200 font-medium">
                    Step 1: Look up base resistance values from BS EN 60228
                  </p>
                  <div className="font-mono text-xs bg-purple-500/10 rounded-lg p-3 border border-purple-500/20 space-y-1">
                    <p className="text-purple-200">
                      <span className="text-purple-400">
                        Line conductor ({result.lineConductorCSA}mm² {result.conductorMaterial}):
                      </span>{' '}
                      r₁₂₀ = {r20Line} mΩ/m
                    </p>
                    <p className="text-purple-200">
                      <span className="text-purple-400">
                        CPC ({result.cpcConductorCSA}mm² {result.conductorMaterial}):
                      </span>{' '}
                      r₂₂₀ = {r20CPC} mΩ/m
                    </p>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="font-mono text-xs bg-purple-500/10 rounded-lg p-3 border border-purple-500/20 space-y-1">
                    <p className="text-purple-200">
                      <span className="text-purple-400">Formula ({result.conductorMaterial}):</span>{' '}
                      Ct = ({tempConstant} + T) ÷ ({tempConstant} + 20)
                    </p>
                    <p className="text-purple-200">
                      <span className="text-purple-400">Calculation:</span> Ct = ({tempConstant} +{' '}
                      {result.temperature}) ÷ ({tempConstant} + 20)
                    </p>
                    <p className="text-purple-200">
                      <span className="text-purple-400">Result:</span> Ct ={' '}
                      {(tempConstant + result.temperature).toFixed(1)} ÷{' '}
                      {(tempConstant + 20).toFixed(1)} ={' '}
                      <span className="text-green-300">{tempCorrection.toFixed(4)}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <p className="text-purple-200 font-medium">
                    Step 3: Calculate R1 (line conductor resistance)
                  </p>
                  <div className="font-mono text-xs bg-purple-500/10 rounded-lg p-3 border border-purple-500/20 space-y-1">
                    <p className="text-purple-200">
                    ...
                    <p className="text-purple-200">
                      <span className="text-purple-400">Calculation:</span> R1 = ({r20Line} ×{' '}
                      {result.cableLength} × {tempCorrection.toFixed(4)}) ÷ 1000
                    </p>
                    <p className="text-purple-200">
                      <span className="text-purple-400">Result:</span> R1 ={' '}
                      {(r20Line * result.cableLength * tempCorrection).toFixed(4)} ÷ 1000 ={' '}
                      <span className="text-blue-300">{result.r1.toFixed(4)} Ω</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <p className="text-purple-200 font-medium">
                    Step 4: Calculate R2 (CPC resistance)
                  </p>
                  <div className="font-mono text-xs bg-purple-500/10 rounded-lg p-3 border border-purple-500/20 space-y-1">
                    <p className="text-purple-200">
                    ...
                    <p className="text-purple-200">
                      <span className="text-purple-400">Calculation:</span> R2 = ({r20CPC} ×{' '}
                      {result.cableLength} × {tempCorrection.toFixed(4)}) ÷ 1000
                    </p>
                    <p className="text-purple-200">
                      <span className="text-purple-400">Result:</span> R2 ={' '}
                      {(r20CPC * result.cableLength * tempCorrection).toFixed(4)} ÷ 1000 ={' '}
                      <span className="text-purple-300">{result.r2.toFixed(4)} Ω</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="font-mono text-xs bg-green-500/10 rounded-lg p-3 border border-green-500/20 space-y-1">
                    ...
                    <p className="text-green-200">
                      <span className="text-green-400">Calculation:</span> R1+R2 ={' '}
                      {result.r1.toFixed(4)} + {result.r2.toFixed(4)}
                    </p>
                    <p className="text-green-200">
                      <span className="text-green-400">Result:</span> R1+R2 ={' '}
                      <span className="text-green-300 font-bold">{result.r1r2.toFixed(4)} Ω</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <p className="text-purple-200 font-medium">
                    Step 6: Calculate test limit (for ambient temperature testing)
                  </p>
                  <div className="font-mono text-xs bg-yellow-500/10 rounded-lg p-3 border border-yellow-500/20 space-y-1">
                    <p className="text-yellow-200">
                      <span className="text-yellow-400">Why 1.67?</span> When testing at ~20°C
                      ambient, multiply by 1.67 to get the maximum acceptable value at operating
                      temperature
                    </p>
                    <p className="text-yellow-200">
                    ...
                    <p className="text-yellow-200">
                      <span className="text-yellow-400">Calculation:</span> Test limit ={' '}
                      {result.r1r2.toFixed(4)} × 1.67
                    </p>
                    <p className="text-yellow-200">
                      <span className="text-yellow-400">Result:</span> Test limit ={' '}
                ...
                {measuredValue && testStatus && (
                  <div className="space-y-2">
                    ...
                    <div
                      className={`font-mono text-xs rounded-lg p-3 border space-y-1 ${testStatus.status.includes('Fail') ? 'bg-red-500/10 border-red-500/20' : 'bg-green-500/10 border-green-500/20'}`}
                    >
                      <p
                        className={
                          testStatus.status.includes('Fail') ? 'text-red-200' : 'text-green-200'
                        }
                      >
                        <span
                          className={
                            testStatus.status.includes('Fail') ? 'text-red-400' : 'text-green-400'
                          }
                        >
                          Measured:
                        </span>{' '}
                        {parseFloat(measuredValue).toFixed(4)} Ω
                      </p>
                      <p
                        className={
                          testStatus.status.includes('Fail') ? 'text-red-200' : 'text-green-200'
                      ...
                      >
                        ...
                        {parseFloat(measuredValue).toFixed(4)}{' '}
                        {parseFloat(measuredValue) <= result.continuityLimit ? '≤' : '>'}{' '}
                        {result.continuityLimit.toFixed(4)}
                      </p>
                      <p
                        className={`font-bold ${testStatus.status.includes('Fail') ? 'text-red-300' : 'text-green-300'}`}
                      >
          ...
          <div className="border-t border-purple-500/20 pt-3 mt-3">
            <p className="text-xs text-purple-300/70">
              <strong>Key:</strong> r₁₂₀ = Line conductor resistance at 20°C (mΩ/m) | r₂₂₀ = CPC
              resistance at 20°C (mΩ/m) | L = Cable length (m) | Ct = Temperature correction factor
          ...
          <div className="space-y-2 text-elec-light text-sm">
            ...
            <div className="bg-blue-500/10 border border-blue-500/30 rounded p-2">
              <p className="text-xs text-blue-200">
                <strong>Note:</strong> Test is performed at ambient temperature (~20°C). The ×1.67
                factor accounts for resistance increase at operating temperature.
              </p>
            </div>
          </div>
        </InfoBox>
            ...
            <div className="space-y-2 text-elec-light text-sm">
              ...
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded p-2">
                <p className="text-xs text-yellow-200">
                  <strong>BS7671 Requirement:</strong> CPC minimum size per Table 54.7. Line/CPC
                  ratio should not exceed 16:1 for compliance.
                </p>
              </div>
            </div>
          </InfoBox>
