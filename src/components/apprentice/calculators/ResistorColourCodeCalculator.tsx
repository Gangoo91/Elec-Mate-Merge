          ...
          <CalculatorResult category="power">
            ...
            <ResultsGrid columns={2}>
              <ResultValue
                ...
                size="sm"
              />
              <ResultValue label="Tolerance" value={result.tolerance} category="power" size="sm" />
            </ResultsGrid>
                ...
                <div className="space-y-3 text-sm">
                  <div className="p-3 rounded-lg bg-white/5">
                    ...
                    <p className="text-white font-mono">
                      Band 1 ({band1}) ={' '}
                      {hasValue(colorValues[band1]) ? colorValues[band1].value : '?'}, Band 2 (
                      {band2}) = {hasValue(colorValues[band2]) ? colorValues[band2].value : '?'}
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-white/5">
                    <p className="text-white/60 mb-1">Step 2: Form base number</p>
                    <p className="text-white font-mono">
                      {hasValue(colorValues[band1]) ? colorValues[band1].value : '?'}
                      {hasValue(colorValues[band2]) ? colorValues[band2].value : '?'} ={' '}
                      {hasValue(colorValues[band1]) && hasValue(colorValues[band2])
                        ? colorValues[band1].value * 10 + colorValues[band2].value
                        : '?'}
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-white/5">
                    ...
                    <p className="text-white font-mono">
                      {hasValue(colorValues[band1]) && hasValue(colorValues[band2])
                        ? colorValues[band1].value * 10 + colorValues[band2].value
                        : '?'}{' '}
                      × {colorValues[band3]?.multiplier?.toLocaleString() || '?'} ={' '}
                      {result.formattedValue}
                    </p>
                  </div>
                </div>
