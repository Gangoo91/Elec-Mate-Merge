  ...
  <div className="space-y-2">
    ...
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-1.5">
      {colors.map(([name, colorData]) => {
        const bgColor = typeof colorData === 'string' ? colorValues[name]?.color : colorData.color;
  ...
  return (
    ...
    >
      <CalculatorPanes
                ...
                <div className="relative flex items-center px-2 py-3 bg-amber-900/40 rounded-lg border border-amber-200/30">
                  <div className="flex gap-1.5">
                    ...
                    <div
                      className="w-3 h-8 rounded-sm border border-black/20 shadow-inner"
                      style={{
                        backgroundColor: band3
                          ? colorValues[band3]?.color
                          : 'rgba(255,255,255,0.2)',
                      }}
                    />
                    <div className="w-3" />
                    <div
                      className="w-3 h-8 rounded-sm border border-black/20 shadow-inner"
                      style={{
                        backgroundColor: band4
                          ? colorValues[band4]?.color
                          : 'rgba(255,255,255,0.2)',
                      }}
                    />
                  </div>
                  ...
                  <CollapsibleContent className="pt-2">
                    ...
                    >
                      <p className="text-sm text-white">
                        ...
                        {result.tolerance} tolerance. In practice, the actual resistance could be
                        anywhere between{' '}
                        {formatResistance(
                          calculateToleranceRange(result.resistance, result.tolerance).min
                        )}{' '}
                        and{' '}
                        {formatResistance(
                          calculateToleranceRange(result.resistance, result.tolerance).max
                        )}
                        .
                      </p>
                      <p className="text-sm text-white">
                        When measuring with a multimeter, your reading should fall within this
                        range. If it does not, the resistor may be damaged or out of specification.
                      </p>
                    </div>
                  </CollapsibleContent>
                </Collapsible>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="p-2 rounded bg-white/5 col-span-2">
                          ...
                          <p className="text-xs text-white mt-1">
                            Tolerance: Gold (±5%), Silver (±10%), Brown (±1%)
                          </p>
                        </div>
                      </div>
                      <p className="text-xs text-white pt-2 border-t border-white/10">
                        Tip: The tolerance band usually has a gap from the others. Read resistors
                        left to right.
                      </p>
                    </div>
                  </CollapsibleContent>
        ...
        footer={<CalculatorEditorial content={resistorColourCodeContent} category={CAT} />}
      />
    </CalculatorCard>
  );
};
