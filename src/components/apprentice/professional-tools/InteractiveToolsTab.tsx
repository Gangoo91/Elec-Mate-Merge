      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {toolSummary.map((tool, index) => {
          ...
          return (
            <Card
              key={index}
              className={`bg-gradient-to-br ${classes.bg} ${classes.border} transition-all overflow-hidden relative group`}
            >
              <div
                className={`absolute top-0 right-0 w-32 h-32 bg-${tool.color}/5 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity`}
              />
