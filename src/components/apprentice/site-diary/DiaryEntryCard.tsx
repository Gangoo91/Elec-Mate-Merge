              ...
              {entry.tasks_completed.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {entry.tasks_completed.slice(0, 4).map((task) => (
                    <span key={task} className="px-2 py-0.5 rounded-md bg-white/[0.08] text-[11px] text-white">{task}</span>
                  ))}
                  {entry.tasks_completed.length > 4 && (
                    <span className="px-2 py-0.5 rounded-md bg-white/[0.04] text-[11px] text-white">+{entry.tasks_completed.length - 4} more</span>
                  )}
                </div>
              )}
              ...
              {!compact && entry.skills_practised.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-2">
                  {entry.skills_practised.slice(0, 3).map((skill) => (
                    <span key={skill} className={`px-1.5 py-0.5 rounded text-[10px] font-medium border ${skillColours[skill] || 'bg-white/[0.06] text-white border-white/10'}`}>{skill}</span>
                  ))}
                  {entry.skills_practised.length > 3 && (
                    <span className="px-1.5 py-0.5 rounded text-[10px] bg-white/[0.04] text-white border border-white/[0.06]">+{entry.skills_practised.length - 3}</span>
                  )}
                </div>
              )}

              {!compact && portfolioNudge && !entry.linked_portfolio_id && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-elec-yellow/10 border border-elec-yellow/20 text-[11px] font-medium text-elec-yellow mb-2">
                  <Briefcase className="h-3 w-3 flex-shrink-0" /><span className="truncate">{portfolioNudge.nudge}</span>
                </span>
              )}
              {!compact && entry.linked_portfolio_id && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-green-500/10 border border-green-500/20 text-[11px] font-medium text-green-400 mb-2">
