...
export function DiaryWeeklySummary({ entries }: DiaryWeeklySummaryProps) {
        ...
        <div className="flex items-center gap-3">
          ...
          {collapsed && (
            <span className="text-xs text-white">
              {thisWeek.daysLogged} day{thisWeek.daysLogged !== 1 ? 's' : ''} · {thisWeek.totalTasks} tasks
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          {entryDiff !== 0 && (
            <div className={`flex items-center gap-0.5 text-[11px] font-medium ${
              ...
              {entryDiff > 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
              {entryDiff > 0 ? '+' : ''}{entryDiff}
            </div>
          )}
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            <div className="text-center py-2 rounded-lg bg-white/[0.03]">
              <div className="text-lg font-bold text-elec-yellow">{thisWeek.daysLogged}</div>
              <div className="text-[10px] text-white">Days</div>
            </div>
            <div className="text-center py-2 rounded-lg bg-white/[0.03]">
              <div className="text-lg font-bold text-green-400">{thisWeek.totalTasks}</div>
              <div className="text-[10px] text-white">Tasks</div>
            </div>
            <div className="text-center py-2 rounded-lg bg-white/[0.03]">
              <div className="text-lg font-bold text-purple-400">{thisWeek.skills.length}</div>
              <div className="text-[10px] text-white">Skills</div>
            </div>
            <div className="text-center py-2 rounded-lg bg-white/[0.03]">
              <div className="text-lg font-bold text-cyan-400">{thisWeek.sites.length}</div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[10px] text-white uppercase tracking-wider font-medium">Mood this week</span>
            </div>
            <div className="flex items-center gap-2">
