    ...
    return lines
      .map((line, index) => {
        ...
        const technicalTerms =
          /\b(RCD|RCBO|MCB|MCCB|RCM|AFDD|SPD|CU|DB|EICR|PIR|EIC|PAT|Zs|Ze|Zdb|PFC|PSCC|TN-S|TN-C-S|TT|IT|IP\d{2}|CSA|CPC|PME|SWA|MICC|FP200|XLPE|PVC|LSZH|BS\s*7671|IET|Part\s*P|SELV|PELV|FELV)\b/gi;
        const measurements =
        ...
        const processBold = (text: string) => {
          return text.replace(
            /\*\*([^*]+)\*\*/g,
            '<strong class="text-elec-yellow font-semibold">$1</strong>'
          );
        };
        ...
        if (trimmed.startsWith('### ')) {
          ...
          return (
            <h3
              key={index}
              className="text-lg font-bold text-elec-yellow mt-4 mb-2 flex items-center gap-2"
            >
              {headerText}
            </h3>
          );
        ...
        if (trimmed.match(/^\*\*[^*]+\*\*\s*[-–]?\s*$/)) {
          ...
          return (
            <h4
              key={index}
              className="text-base font-semibold text-white mt-4 mb-2 border-l-2 border-elec-yellow pl-3"
            >
              {headerText}
            </h4>
          );
        ...
        if (trimmed.match(/^\*\*Step\s+\d+:/i)) {
          ...
          return (
            <div key={index} className="mt-3 mb-1">
              <p
                className="font-semibold text-elec-yellow"
                dangerouslySetInnerHTML={{ __html: processed }}
              />
            </div>
          );
        ...
        if (
          ...
          (trimmed.includes('⚠️') && trimmed.includes('Important'))
        ) {
        ...
        if (trimmed.match(/^\*\*Key requirements:\*\*$/i)) {
          return (
            <p
              key={index}
              className="text-sm font-semibold text-white mt-3 mb-2 uppercase tracking-wide"
            >
              Key Requirements
            </p>
          );
        ...
        if (
          (trimmed.match(/^(Does|Do|Have|Has|Is|Are|What|Would|Could|Can|Should|Want|Did)\s/i) ||
            trimmed.endsWith('?')) &&
