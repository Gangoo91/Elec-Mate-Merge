...
export const QueueOverview = ({ operations, className }: QueueOverviewProps) => {
  ...
  const oldestOp =
    operations.length > 0 ? Math.max(...operations.map((op) => Date.now() - op.timestamp)) : 0;
  ...
  const oldestAge =
    oldestOp > 0
      ? oldestOp < 60000
        ? 'Just now'
        : oldestOp < 3600000
          ? `${Math.floor(oldestOp / 60000)}m`
          : oldestOp < 86400000
            ? `${Math.floor(oldestOp / 3600000)}h`
    ...
    <Card className={cn('border-border/50', className)}>
      ...
      <CardContent className="space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          <StatsCard icon={Clock} label="Pending" value={pendingOps} color="text-blue-500" />
          <StatsCard icon={RefreshCw} label="Retrying" value={retryingOps} color="text-amber-500" />
          <StatsCard icon={AlertCircle} label="Failed" value={failedOps} color="text-red-500" />
          <StatsCard
            icon={Clock}
            label="Oldest"
            value={oldestAge}
...
const StatsCard = ({ icon: Icon, label, value, color, isTime }: StatsCardProps) => {
  return (
    <div className="flex flex-col items-center p-3 sm:p-4 rounded-lg bg-muted/50 border border-border/30">
      ...
      <div className={cn(responsiveHeading.h3, 'font-bold')}>{value}</div>
      <div className={cn(responsiveBody.small, 'text-muted-foreground text-center')}>{label}</div>
    </div>
  );
};
