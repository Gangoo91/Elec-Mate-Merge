...
export const ProtectiveDeviceSelector: React.FC<ProtectiveDeviceSelectorProps> = ({
  ...
  const handleDeviceChange = (value: string) => {
    const selectedDevice = DEVICE_OPTIONS.find(
      (d) => `${d.type}-${d.curve || 'none'}-${d.rating}` === value
    );
    if (selectedDevice) {
    ...
    <Card className="bg-card border-border">
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <Select value={device ? getDeviceValue(device) : ''} onValueChange={handleDeviceChange}>
            ...
            <SelectContent className="bg-elec-gray border-elec-gray text-foreground">
              ...
              {DEVICE_OPTIONS.filter((d) => d.type === 'mcb' && d.curve === 'typeB').map(
                (device) => (
                  <SelectItem
              ...
              <div className="text-xs text-gray-400 px-2 py-1 font-semibold border-t border-neutral-600 mt-1 pt-2">
                MCB Type C
              </div>
              {DEVICE_OPTIONS.filter((d) => d.type === 'mcb' && d.curve === 'typeC').map(
                (device) => (
                  <SelectItem
                    key={getDeviceValue(device)}
                    value={getDeviceValue(device)}
