...
import { CheckCircle, FileText, Search, TestTube, User, FileCheck } from 'lucide-react';
...
const steps: StepConfig[] = [
  ...
  { id: 'inspector', label: 'Inspector Details', shortLabel: 'Inspector', icon: <User className="h-4 w-4" /> },
  { id: 'certificate', label: 'Certificate', shortLabel: 'Cert', icon: <FileCheck className="h-4 w-4" /> },
];

interface EICRStepIndicatorProps {
  currentTab: EICRTabValue;
  onTabChange: (tab: EICRTabValue) => void;
  isTabComplete: (tab: EICRTabValue) => boolean;
  className?: string;
}

const EICRStepIndicator: React.FC<EICRStepIndicatorProps> = ({
  currentTab,
  onTabChange,
  isTabComplete,
  className,
...
export default EICRStepIndicator;
