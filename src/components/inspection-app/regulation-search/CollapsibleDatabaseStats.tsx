      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-white/70">({databaseStats.total} total regulations)</span>
          </div>
          <Button
            variant="ghost"
            size="sm"
        ...
        {isExpanded && (
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm animate-fade-in">
            ...
            <div>
              <h4 className="text-white/70 mb-2">By Category:</h4>
              <div className="space-y-1">
                {Object.entries(databaseStats.byCategory).slice(0, 4).map(([cat, count]) => (
                  <div key={cat} className="flex justify-between">
                    <span className="text-white/80 capitalize">{cat.replace('-', ' ')}:</span>
                    <span className="text-elec-yellow font-semibold">{count}</span>
                  </div>
                ))}
              </div>
            ...
            <div>
              <h4 className="text-white/70 mb-2">By Frequency:</h4>
              <div className="space-y-1">
