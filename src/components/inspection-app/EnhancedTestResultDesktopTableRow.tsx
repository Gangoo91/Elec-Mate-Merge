...
const EnhancedTestResultDesktopTableRow: React.FC<EnhancedTestResultDesktopTableRowProps> = ({ 
  ...
  // Determine row background color based on validation status, zebra striping, and auto-fill status
  const getRowBgColor = () => {
    const overallCompliance = getOverallCompliance(validation);
    const isEvenRow = rowNumber % 2 === 0;

    // Validation errors/warnings take priority
    ...
    // Auto-filled circuits get a subtle blue tint
    if (result.autoFilled) {
      return isEvenRow
        ? 'bg-blue-500/5 hover:bg-blue-500/10'
        : 'bg-blue-500/10 hover:bg-blue-500/15';
    }
    // Zebra striping for regular rows
    return isEvenRow
  ...
  return (
    <>
      <TableRow
        ...
        className={`${getRowBgColor()} border-b border-border/30 transition-colors duration-150`}
      >
        ...
        {showRegulationStatus && (
          <TableCell className="text-center h-9 p-1">
