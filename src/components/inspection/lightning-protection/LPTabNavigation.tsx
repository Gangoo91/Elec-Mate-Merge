...
import CertShellFooter, { certFooterNeutralButton } from '@/components/inspection/shared/CertShellFooter';
...
export default function LPTabNavigation({
  ...
  const { loadCompanyBranding } = useLightningProtectionSmartForm();
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      // PDF (with branding) even when the user emails before tapping Generate.
      let formattedData: Record<string, unknown> | undefined;
      try {
        formattedData = formData
          ? formatLightningProtectionJson(
              {
                ...formData,
                certificateNumber:
      ...
      <Dialog open={showEmailDialog} onOpenChange={setShowEmailDialog}>
        <DialogContent className="max-w-[90vw] sm:max-w-md bg-[#111114] border border-white/[0.1] rounded-2xl">
          ...
          <div className="space-y-3 py-3">
            <div>
              <label htmlFor="lp-email" className="mb-1 block text-[12px] font-medium text-white">
                Recipient email
              </label>
              <Input
                id="lp-email"
