...
export default function FAG3Handover({ formData, onUpdate }: Props) {
  ...
  const addDefect = () => { haptic.light(); onUpdate('defectsFound', [...defects, { id: crypto.randomUUID(), description: '', severity: 'non-critical', rectified: false, rectificationDate: '' }]); };
  const removeDefect = (id: string) => { haptic.medium(); onUpdate('defectsFound', defects.filter((d: any) => d.id !== id)); };
  const updateDefect = (id: string, field: string, value: any) => onUpdate('defectsFound', defects.map((d: any) => d.id === id ? { ...d, [field]: value } : d));

  return (
    <div className="space-y-5">
      {/* Handover Date */}
      <Section title="Handover" accentColor="from-green-500/40 to-emerald-400/20">
        <Field label="Handover Date"><Input type="date" value={formData.handoverDate || ''} onChange={(e) => onUpdate('handoverDate', e.target.value)} className={inputCn} /></Field>
      </Section>
      ...
      {/* Defects */}
      <Section title="Defects & Observations" accentColor="from-red-500/40 to-rose-400/20" count={defects.length}>
        {defects.length === 0 && (
        ...
        {defects.map((d: any, idx: number) => (
          <div key={d.id} className="rounded-xl border border-white/[0.06] overflow-hidden">
            <div className="flex items-center justify-between px-3.5 py-2 bg-white/[0.04] border-b border-white/[0.06]">
              <span className="text-xs font-bold text-red-400">Defect {idx + 1} of {defects.length}</span>
              <button onClick={() => removeDefect(d.id)} className="w-9 h-9 rounded-xl flex items-center justify-center border border-red-500/20 bg-red-500/10 text-red-400 touch-manipulation active:scale-90"><Trash2 className="h-4 w-4" /></button>
            </div>
            <div className="p-3.5 space-y-3 bg-white/[0.02]">
              <Field label="Description"><Textarea value={d.description || ''} onChange={(e) => updateDefect(d.id, 'description', e.target.value)} className="touch-manipulation text-base min-h-[60px] bg-white/[0.06] border-white/[0.08] text-white focus:border-yellow-500 focus:ring-yellow-500" /></Field>
              <Field label="Severity"><ComboboxCell value={d.severity || 'non-critical'} onChange={(v) => updateDefect(d.id, 'severity', v)} options={[
                { value: 'critical', label: 'Critical' },
                { value: 'non-critical', label: 'Non-critical' },
                { value: 'recommendation', label: 'Recommendation' },
              ]} placeholder="Select..." className="h-12 text-base" allowCustom={false} /></Field>
              <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                <button type="button" onClick={() => updateDefect(d.id, 'rectified', !d.rectified)}
                  ...
                  {d.rectified && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                </button>
                <span className={cn('text-sm', d.rectified ? 'text-green-400 font-medium' : 'text-white')}>Rectified</span>
              </div>
              {d.rectified && (
                <Field label="Rectification Date"><Input type="date" value={d.rectificationDate || ''} onChange={(e) => updateDefect(d.id, 'rectificationDate', e.target.value)} className={cn(inputSmCn, '[color-scheme:dark]')} /></Field>
              )}
            </div>
          </div>
        ))}
        <button onClick={addDefect} className="w-full h-12 rounded-xl border-2 border-dashed border-red-500/20 flex items-center justify-center gap-2 text-sm font-medium text-red-400 touch-manipulation active:scale-[0.98]">
      ...
      <Section title="Commissioning Photos" accentColor="from-cyan-500/40 to-blue-400/20">
        ...
        <label onClick={(e) => { e.preventDefault(); photoInputRef.current?.click(); }}
