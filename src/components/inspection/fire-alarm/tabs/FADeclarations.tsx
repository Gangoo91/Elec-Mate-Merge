...
export default function FADeclarations({ formData, onUpdate }: Props) {
  ...
  const handlePreFillOpen = useCallback(async () => {
    ...
    try {
      ...
      const match = reports.find((r: any) => {
        const d = r.data || {};
        const matchAddr = address && (d.premisesAddress || d.installationAddress || '').toLowerCase().includes(address);
  ...
  const handlePreFillConfirm = useCallback((selectedSections: string[]) => {
    ...
    if (selectedSections.includes('thirdParty')) {
      ...
      if (cert.bafeRegistration || cert.bafeRegistered) onUpdate('bafeRegistered', cert.bafeRegistration || cert.bafeRegistered || true);
      if (cert.fiaMembership || cert.fiaRegistered) onUpdate('fiaRegistered', cert.fiaMembership || cert.fiaRegistered || true);
      if (cert.nsiSsaibCertification || cert.nsiRegistered) onUpdate('nsiRegistered', cert.nsiSsaibCertification || cert.nsiRegistered || true);
    ...
    if (selectedSections.includes('monitoring')) {
      ...
      if (mon.isMonitored || mon.systemMonitored) onUpdate('systemMonitored', mon.isMonitored || mon.systemMonitored);
      if (mon.arcName) onUpdate('arcName', mon.arcName);
      if (mon.arcAccountNumber) onUpdate('arcAccountNumber', mon.arcAccountNumber);
      if (mon.arcTelephone || mon.arcPhone) onUpdate('arcPhone', mon.arcTelephone || mon.arcPhone);
      if (mon.signallingRoute) onUpdate('signallingRoute', mon.signallingRoute);
    }

    toast.success(`Pre-filled ${selectedSections.length} section${selectedSections.length > 1 ? 's' : ''}`);
    ...
    <div className="space-y-5">
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={field} className="flex items-center gap-3">
              <Checkbox checked={formData[field] || false} onCheckedChange={(v) => onUpdate(field, v)} className={checkboxCn} />
              <Label className="text-sm text-white">{label}</Label>
            </div>
          ))}
          <Field label="Registration Number"><Input value={formData.thirdPartyCertNumber || ''} onChange={(e) => onUpdate('thirdPartyCertNumber', e.target.value)} className={inputCn} /></Field>
        </div>
      </Section>

      {/* Pre-fill from Previous */}
      <Section title="Pre-fill" accentColor="from-elec-yellow/40 to-amber-400/20">
        <Button variant="outline" onClick={handlePreFillOpen} disabled={loadingPrevious} className="w-full h-12 text-sm border-elec-yellow/20 text-elec-yellow hover:bg-elec-yellow/10 touch-manipulation active:scale-[0.98] rounded-xl">
          <FileSearch className="h-4 w-4 mr-2" />
          {loadingPrevious ? 'Searching...' : 'Pre-fill from Previous Certificate'}
        </Button>
        <PreviousCertPreFillSheet open={preFillOpen} onOpenChange={setPreFillOpen} previousData={previousCertData} onConfirm={handlePreFillConfirm} />
      </Section>
      ...
      <Section title="Installer Declaration" accentColor="from-red-500/40 to-rose-400/20">
        <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-3.5 mb-3">
          <p className="text-xs text-white leading-relaxed">I hereby certify that the fire detection and fire alarm system installation described in this certificate has been carried out in accordance with BS 5839-1:2025 and the design specification referenced above, except for any variations stated. The installation is complete and ready for commissioning.</p>
        </div>
        {(formData.installerName || formData.installerCompany) && (
          <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-3">
            <p className="text-sm font-semibold text-white">{formData.installerName || 'Unnamed'}</p>
            {formData.installerCompany && <p className="text-xs text-white mt-0.5">{formData.installerCompany}</p>}
            <p className="text-[10px] text-white mt-1 uppercase tracking-wider">From Tab 1 — Installer Details</p>
          </div>
        )}
        <Field label="Qualifications"><Input value={formData.installerQualifications || ''} onChange={(e) => onUpdate('installerQualifications', e.target.value)} className={inputCn} placeholder="e.g. FIA certified, ECS card holder" /></Field>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Signature Date"><Input type="date" value={formData.installerSignatureDate || ''} onChange={(e) => onUpdate('installerSignatureDate', e.target.value)} className={inputCn} /></Field>
          <Field label="Installation Completion Date"><Input type="date" value={formData.commissioningDate || ''} onChange={(e) => onUpdate('commissioningDate', e.target.value)} className={inputCn} /></Field>
        </div>
      </Section>

      {/* Responsible Person */}
      <Section title="Responsible Person Acknowledgement" accentColor="from-blue-500/40 to-cyan-400/20">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Name"><Input value={formData.responsiblePersonName || ''} onChange={(e) => onUpdate('responsiblePersonName', e.target.value)} className={inputCn} /></Field>
          <Field label="Position"><Input value={formData.responsiblePersonPosition || ''} onChange={(e) => onUpdate('responsiblePersonPosition', e.target.value)} className={inputCn} placeholder="e.g. Building Manager" /></Field>
        </div>
        <SignatureInput label="Responsible Person Signature" value={formData.responsiblePersonSignature || ''} onChange={(sig) => onUpdate('responsiblePersonSignature', sig || '')} />
        <Field label="Date"><Input type="date" value={formData.responsiblePersonDate || ''} onChange={(e) => onUpdate('responsiblePersonDate', e.target.value)} className={inputCn} /></Field>
      </Section>

      {/* Photos */}
      <Section title="Photos" accentColor="from-cyan-500/40 to-blue-400/20">
        <input ref={photoInputRef} type="file" accept="image/*" multiple className="sr-only" onChange={handlePhotoCapture} />
        <label
          onClick={(e) => { e.preventDefault(); photoInputRef.current?.click(); }}
        ...
        ><Camera className="h-4 w-4" /> Add Photos</label>
      ...
      <Section title="Overall Result" accentColor="from-green-500/40 to-emerald-400/20">
        <div className="space-y-2">
          <button type="button" onClick={() => onUpdate('overallResult', 'satisfactory')}
            className={cn('w-full text-left p-4 rounded-xl border touch-manipulation active:scale-[0.98] transition-all',
              formData.overallResult === 'satisfactory' ? 'bg-green-500/10 border-green-500/30' : 'bg-white/[0.03] border-white/[0.06]')}>
            <div className="flex items-center gap-3">
              <div className={cn('w-5 h-5 rounded border-2 flex items-center justify-center transition-all flex-shrink-0',
                formData.overallResult === 'satisfactory' ? 'bg-green-500 border-green-500' : 'border-white/30')}>
                {formData.overallResult === 'satisfactory' && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
              </div>
              <p className={cn('text-sm font-semibold', formData.overallResult === 'satisfactory' ? 'text-green-400' : 'text-white')}>Satisfactory</p>
            </div>
          </button>
          <button type="button" onClick={() => onUpdate('overallResult', 'unsatisfactory')}
            className={cn('w-full text-left p-4 rounded-xl border touch-manipulation active:scale-[0.98] transition-all',
              formData.overallResult === 'unsatisfactory' ? 'bg-red-500/10 border-red-500/30' : 'bg-white/[0.03] border-white/[0.06]')}>
            <div className="flex items-center gap-3">
              <div className={cn('w-5 h-5 rounded border-2 flex items-center justify-center transition-all flex-shrink-0',
                formData.overallResult === 'unsatisfactory' ? 'bg-red-500 border-red-500' : 'border-white/30')}>
                {formData.overallResult === 'unsatisfactory' && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
              </div>
              <p className={cn('text-sm font-semibold', formData.overallResult === 'unsatisfactory' ? 'text-red-400' : 'text-white')}>Unsatisfactory</p>
            </div>
          </button>
        </div>
      </Section>
      ...
      <Section title="Service Schedule" accentColor="from-amber-500/40 to-yellow-400/20">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Next Service Due"><Input type="date" value={formData.nextServiceDue || ''} onChange={(e) => onUpdate('nextServiceDue', e.target.value)} className={inputCn} /></Field>
          <Field label="Next Inspection Due"><Input type="date" value={formData.nextInspectionDue || ''} onChange={(e) => onUpdate('nextInspectionDue', e.target.value)} className={inputCn} /></Field>
        </div>
      </Section>
