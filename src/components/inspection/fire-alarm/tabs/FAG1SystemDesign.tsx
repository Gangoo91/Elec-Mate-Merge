...
import { suggestGrade, getDesignWarnings, getEstimatedPowerSpecs, findPanelById } from '@/data/fireAlarmEquipmentDatabase';
  ...
  const handlePanelSelect = (panel: any, defaults: any) => {
    if (panel) {
      ...
      if (fullPanel) {
        ...
        if (!formData.quiescentCurrent) onUpdate('quiescentCurrent', specs.quiescentCurrent.toString());
  ...
  return (
    <div className="space-y-5">
      ...
      <Section title="System Category" accentColor="from-red-500/40 to-orange-400/20">
        ...
        <Field label="Category Justification" required><Textarea value={formData.categoryJustification || ''} onChange={(e) => onUpdate('categoryJustification', e.target.value)} className={textareaCn} placeholder="Why this category was selected — must reference the FRA findings..." /></Field>
      </Section>
      ...
      <Section title="System Grade" accentColor="from-red-500/40 to-orange-400/20">
        <Field label="Grade" required>
          <div className="space-y-2">
            {[
              { value: 'A', label: 'Grade A', description: 'Wired to BS EN 54-2 panel with BS EN 54 devices' },
              { value: 'B', label: 'Grade B', description: 'Wired to BS EN 54-2 panel with proprietary devices' },
              { value: 'C', label: 'Grade C', description: 'Conventional system using BS EN 54 devices' },
              { value: 'D', label: 'Grade D', description: 'Domestic — mains-powered with battery backup' },
              { value: 'E', label: 'Grade E', description: 'Domestic — battery-only' },
              { value: 'F', label: 'Grade F', description: 'Non-BS EN 54 devices' },
            ].map((grade) => (
              <button key={grade.value} type="button" onClick={() => onUpdate('systemGrade', grade.value)}
                className={cn('w-full text-left p-4 rounded-xl border touch-manipulation active:scale-[0.98] transition-all',
                  formData.systemGrade === grade.value ? 'bg-red-500/10 border-red-500/30' : 'bg-white/[0.03] border-white/[0.06]')}>
                <div className="flex items-center gap-3">
                  <div className={cn('w-5 h-5 rounded border-2 flex items-center justify-center transition-all flex-shrink-0',
                    formData.systemGrade === grade.value ? 'bg-red-500 border-red-500' : 'border-white/30')}>
                    {formData.systemGrade === grade.value && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                  </div>
                  <div>
                    <p className={cn('text-sm font-semibold', formData.systemGrade === grade.value ? 'text-red-400' : 'text-white')}>{grade.label}</p>
        ...
        {gradeSuggestion && formData.systemGrade !== gradeSuggestion.grade && (
          <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-3">
            <div className="flex items-start gap-2">
              <Sparkles className="h-4 w-4 text-amber-400 mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-xs font-semibold text-amber-400">Suggested Grade: {gradeSuggestion.grade}</p>
      ...
      <Section title="Design Basis" accentColor="from-red-500/40 to-rose-400/20">
        <Field label="Design Basis" required><Textarea value={formData.designBasis || ''} onChange={(e) => onUpdate('designBasis', e.target.value)} className={textareaCn} placeholder="Explain the rationale for this design — what fire risks were identified and how the design addresses them..." /></Field>
        <Field label="Coverage Rationale"><Textarea value={formData.coverageRationale || ''} onChange={(e) => onUpdate('coverageRationale', e.target.value)} className={textareaCn} placeholder="How the design provides adequate coverage for the identified fire hazards..." /></Field>
      </Section>
      ...
      <Section title="Cable Specification" accentColor="from-amber-500/40 to-yellow-400/20">
        ...
        <div className="flex items-center gap-3">
          ...
          <Label className="text-sm text-white">Red cable specified for mains power circuits (BS 5839-1:2025)</Label>
        </div>
      </Section>
      ...
      <Section title="Battery Calculation" accentColor="from-green-500/40 to-emerald-400/20">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Quiescent Current (mA)"><Input type="number" inputMode="decimal" value={formData.quiescentCurrent || ''} onChange={(e) => onUpdate('quiescentCurrent', e.target.value)} className={inputCn} placeholder="e.g. 120" /></Field>
          <Field label="Alarm Current (mA)"><Input type="number" inputMode="decimal" value={formData.alarmCurrent || ''} onChange={(e) => onUpdate('alarmCurrent', e.target.value)} className={inputCn} placeholder="e.g. 850" /></Field>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Required Standby (hrs)">
            <ComboboxCell value={formData.requiredStandby || ''} onChange={(v) => onUpdate('requiredStandby', v)} options={[
      ...
      <Section title="Sound Level Design Targets" accentColor="from-blue-500/40 to-cyan-400/20">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="General Areas Min (dB)"><Input type="number" inputMode="numeric" value={formData.soundTargetGeneral || '65'} onChange={(e) => onUpdate('soundTargetGeneral', e.target.value)} className={inputCn} /></Field>
          <Field label="Sleeping Areas Min (dB)"><Input type="number" inputMode="numeric" value={formData.soundTargetSleeping || '75'} onChange={(e) => onUpdate('soundTargetSleeping', e.target.value)} className={inputCn} /></Field>
        </div>
        <Field label="Sound Design Notes"><Textarea value={formData.soundDesignNotes || ''} onChange={(e) => onUpdate('soundDesignNotes', e.target.value)} className="touch-manipulation text-base min-h-[80px] bg-white/[0.06] border-white/[0.08] text-white focus:border-yellow-500 focus:ring-yellow-500" placeholder="How the design achieves required sound levels in all areas..." /></Field>
      </Section>
      ...
      <Section title="Cause & Effect" accentColor="from-blue-500/40 to-cyan-400/20">
        ...
        <Field label="Cause & Effect Reference"><Input value={formData.causeEffectReference || ''} onChange={(e) => onUpdate('causeEffectReference', e.target.value)} className={inputCn} placeholder="e.g. CE-001 Rev A" /></Field>
        <Field label="False Alarm Management Strategy"><Textarea value={formData.falseAlarmStrategy || ''} onChange={(e) => onUpdate('falseAlarmStrategy', e.target.value)} className="touch-manipulation text-base min-h-[80px] bg-white/[0.06] border-white/[0.08] text-white focus:border-yellow-500 focus:ring-yellow-500" placeholder="e.g. Coincidence detection, investigation delay..." /></Field>
      </Section>
      ...
      <Section title="Monitoring / ARC" accentColor="from-green-500/40 to-emerald-400/20">
        <div className="flex items-center gap-3">
          ...
          <Label className="text-sm text-white">System to be monitored by Alarm Receiving Centre</Label>
        </div>
