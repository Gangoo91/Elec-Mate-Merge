...
const commonObservations = {
  ...
  'N/A': ['Not applicable to this installation', 'No observations for this category'],
};
...
const getDefectDescription = (code: string) => {
  switch (code) {
    case 'C1':
      return 'Danger Present - Immediate action required';
    case 'C2':
      return 'Potentially Dangerous - Urgent action required';
    case 'C3':
  ...
  const handleQuickSelect = (selectedText: string) => {
    ...
    if (selectedText.includes('No defects') || selectedText.includes('Not applicable')) {
      ...
      onUpdate(
        observation.id,
        'recommendation',
        'Immediate rectification required before energising'
      );
      ...
      onUpdate(
        observation.id,
        'recommendation',
        'Improvement recommended at next convenient opportunity'
      );
    ...
    <Card className={`p-5 sm:p-6 border-l-4 ${borderColor}`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <Button
          ...
          className="text-red-500 hover:text-red-700 h-11 w-11 p-0 self-end sm:self-auto"
      ...
      <div className="space-y-4">
        ...
        <div>
          <Label className="text-sm font-semibold mb-1.5 block">
            Quick Select Common Observations
          </Label>
        ...
        <div>
          ...
          <Textarea
            ...
            rows={5}
            className="resize-none text-base touch-manipulation min-h-[120px]"
          />
        </div>
