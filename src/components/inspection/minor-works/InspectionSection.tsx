...
const InspectionSection = ({ formData, onUpdate }: InspectionSectionProps) => {
  ...
  const getInspectionStats = () => {
    ...
    const total = Object.keys(inspectionCategories).reduce(
      (acc, cat) =>
        acc + inspectionCategories[cat as keyof typeof inspectionCategories].items.length,
      0
    );
    const completed = Object.keys(results).filter((key) => results[key]?.outcome).length;
    const satisfactory = Object.keys(results).filter(
      (key) => results[key]?.outcome === 'satisfactory'
    ).length;
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              Visual inspection is mandatory for all electrical work. Each item must be assessed in
              accordance with BS 7671:2018 requirements.
            </AlertDescription>
          </Alert>
                ...
                <div key={item.id} className="border border-border/30 rounded-lg p-4 space-y-3">
                  <div className="flex items-start gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <Label className="text-sm font-medium leading-relaxed">{item.text}</Label>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleInspectionUpdate(item.id, value, result?.notes)
                            }
                          >
                        ...
                        <div>
                          ...
                          <Textarea
                            ...
                            value={result?.notes || ''}
                            onChange={(e) =>
                              handleInspectionUpdate(item.id, result?.outcome || '', e.target.value)
                            }
                  ...
                  {result?.outcome && (
                    <div className="flex justify-end">
                      ...
                      {result.outcome === 'na' && <Badge variant="secondary">N/A</Badge>}
                      {result.outcome === 'limitation' && <Badge variant="outline">Limited</Badge>}
                    </div>
                  )}
                </div>
              );
