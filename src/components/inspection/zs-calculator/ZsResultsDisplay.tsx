  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="bg-gradient-to-br from-card to-background border-elec-yellow/30">
        ...
        <CardContent className="space-y-4">
          <div className="text-center">
            <div className="text-4xl font-bold text-foreground mb-2">Zs = {zs.toFixed(3)} Ω</div>
          ...
          {showBreakdown && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {breakdown.additionalResistances && breakdown.additionalResistances > 0 && (
                <div className="text-center p-3 bg-muted rounded-lg">
                  <div className="text-lg font-semibold text-foreground">
                    {breakdown.additionalResistances.toFixed(3)} Ω
                  </div>
      ...
      {compliance.maxAllowed && (
        <Card
          className={`border ${compliance.isCompliant ? 'border-green-500/30 bg-green-500/5' : 'border-red-500/30 bg-red-500/5'}`}
        >
          <CardHeader>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`font-semibold ${getComplianceColor()}`}>{zs.toFixed(3)} Ω</span>
            </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className={`font-semibold ${getComplianceColor()}`}>
                    {compliance.margin > 0 ? '+' : ''}
                    {compliance.margin.toFixed(3)} Ω
                  </span>
                  {getMarginPercentage() && (
                    <div className="text-xs text-gray-400">({getMarginPercentage()}% margin)</div>
                  )}
                </div>
              </div>
            )}
