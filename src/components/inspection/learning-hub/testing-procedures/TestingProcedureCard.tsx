...
const TestingProcedureCard = ({ procedure, onStart, onPreview }: TestingProcedureCardProps) => {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      default:
        return <Play className="h-4 w-4 sm:h-5 sm:w-5" />;
    }
  };
  ...
  return (
    <Card
      className={`bg-card/50 border-border border-l-4 ${getBorderAccent(procedure.difficulty)} hover:bg-card hover:border-border transition-all touch-manipulation active:scale-[0.98]`}
    >
      ...
      <CardContent className="space-y-3 sm:space-y-4 p-4 sm:p-5 md:p-6 pt-0">
        {/* Key Equipment */}
        <div>
          <p className="text-xs text-muted-foreground mb-2 font-medium">Key Equipment:</p>
        ...
        {procedure.realWorldExamples.length > 0 && (
          <div className="p-3 bg-muted/50 rounded-lg text-xs sm:text-sm">
            <p className="text-muted-foreground mb-1.5 font-medium">Example scenario:</p>
