...
import ZsRegulationsCard from './ZsRegulationsCard';
import ZsTemperatureSection from './ZsTemperatureSection';
import ZsPracticeSection from './ZsPracticeSection';

type View = 'hub' | 'why' | 'how' | 'tables' | 'practical' | 'regulations' | 'temperature' | 'practice';
...
const ZsTestingProcedure = ({ onBack }: Props) => {
  const [view, setView] = useState<View>('hub');

  if (view === 'why') return <WhyTestSection onBack={() => setView('hub')} />;
  if (view === 'how') return <HowToTestSection onBack={() => setView('hub')} />;
  if (view === 'tables') return <ZsTablesSection onBack={() => setView('hub')} />;
  if (view === 'practical') return <PracticalGuidanceSection onBack={() => setView('hub')} />;
  if (view === 'regulations') return <ZsRegulationsCard onBack={() => setView('hub')} />;
  if (view === 'temperature') return <ZsTemperatureSection onBack={() => setView('hub')} />;
  if (view === 'practice') return <ZsPracticeSection onBack={() => setView('hub')} />;
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-5">
        ...
        <motion.section variants={itemVariants}>
          <div className="grid grid-cols-2 gap-3">
            <BusinessCard
              title="Why Test?"
              description="ADS & disconnection times"
              icon={Shield}
              onClick={() => setView('why')}
              variant="hero"
            />
            ...
            <BusinessCard
              title="Zs Tables"
              description="Max values & 80% rule"
              icon={Calculator}
            ...
            <BusinessCard
              ...
              description="Equipment & troubleshooting"
              icon={BookOpen}
        ...
        <motion.div variants={itemVariants}>
          <BusinessCard
            title="Practice"
            description="Record Zs test results with temperature correction"
            icon={TestTube}
...
export default ZsTestingProcedure;
