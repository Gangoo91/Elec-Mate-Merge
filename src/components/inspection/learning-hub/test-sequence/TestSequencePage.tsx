...
const deadTests = [
  { number: 1, test: 'Continuity of Protective Conductors', ref: 'Reg 643.2', what: 'Verify CPC provides continuous low-resistance path for fault current. Test R1+R2 from board to furthest point of each circuit.', acceptance: 'Continuity confirmed, ≤0.5Ω typical', why: 'Must be done first — proves the earth path exists before any other test relies on it.' },
  { number: 2, test: 'Continuity of Ring Final Circuits', ref: 'Reg 643.2', what: 'End-to-end continuity of L, N and CPC. Cross-connection tests at every socket. Confirms ring is continuous and no breaks exist.', acceptance: 'Consistent readings around ring, no open circuits', why: 'Ring integrity must be verified before insulation testing — a broken ring changes the circuit behaviour.' },
  { number: 3, test: 'Insulation Resistance', ref: 'Reg 643.3', what: 'Apply 500V DC between L-E, N-E, L-N. Measures insulation quality between conductors. Circuit must be isolated, all equipment disconnected.', acceptance: '≥1.0MΩ per circuit (500V test)', why: 'Done after continuity so you know the CPC is connected. Done dead because 500V DC would destroy connected equipment.' },
  { number: 4, test: 'Polarity', ref: 'Reg 643.6', what: 'Verify L, N, E connected to correct terminals at every accessory. Single-pole devices must be in the line conductor only.', acceptance: 'Correct at all terminations — no reversals', why: 'Last dead test. Confirms wiring is correct before the circuit is energised for the first time.' },
];
...
const liveTests = [
  ...
  { number: 6, test: 'Prospective Fault Current (PFC)', ref: 'Reg 643.7.3.201', what: 'Measure maximum fault current at the origin. Record the higher of L-E and L-N PFC. Compare against device breaking capacity.', acceptance: 'PFC ≥ device minimum for magnetic trip AND ≤ breaking capacity', why: 'Verifies protective devices can safely handle the available fault current without failing.' },
  { number: 7, test: 'RCD Operation', ref: 'Reg 643.10', what: 'Test at 0.5×, 1× and 5× IΔn. Test button check. Both 0° and 180° phase angles for 1× test.', acceptance: 'No trip at 0.5×, ≤300ms at 1× (general), ≤40ms at 5×', why: 'RCDs are the last line of defence against electric shock. Must be verified working after all other tests pass.' },
  { number: 8, test: 'Functional Testing', ref: 'Reg 643.10', what: 'Operate every switch, isolator, control device, interlock, and emergency stop. Verify the installation works as designed.', acceptance: 'All assemblies function correctly as intended', why: 'Final test. Confirms everything actually works — not just that it is wired correctly.' },
];
