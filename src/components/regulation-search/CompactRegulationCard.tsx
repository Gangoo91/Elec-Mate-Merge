...
const CompactRegulationCard: React.FC<CompactRegulationCardProps> = ({
  regulation,
  onRelatedClick,
}) => {
  const partColors = {
  ...
  const FrequencyIcon =
    regulation.frequency === 'common'
      ? Star
      : regulation.frequency === 'frequent'
        ? BookOpen
        : Info;
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <Badge
                className={`border text-xs ${partColors[regulation.part as keyof typeof partColors]}`}
              >
              ...
              <Badge
                className={`border text-xs ${frequencyColors[regulation.frequency]} flex items-center gap-1`}
              >
            ...
            <h3 className="text-foreground font-semibold text-sm mb-1 leading-tight">
              {regulation.title}
            </h3>
            <p className="text-gray-400 text-xs mb-2">{regulation.section}</p>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
              ...
              {regulation.keywords.length > 3 && (
                <span className="text-gray-400">+{regulation.keywords.length - 3}</span>
              )}
              {regulation.relatedRegs.length > 0 && (
                <span className="text-gray-400 ml-2">
                  • {regulation.relatedRegs.length} related
                </span>
              )}
            </div>
          </div>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-card border-border">
              <DialogHeader>
                <DialogTitle className="text-foreground flex items-center gap-2 flex-wrap">
                  <Badge className="bg-elec-yellow text-black font-bold">{regulation.number}</Badge>
              ...
              <div className="space-y-6 text-foreground">
                ...
                <div className="flex flex-wrap gap-2">
                  <Badge
                    className={`border ${partColors[regulation.part as keyof typeof partColors]}`}
                  >
                    Part {regulation.part}
                  </Badge>
                  ...
                  <Badge
                    className={`border ${frequencyColors[regulation.frequency]} flex items-center gap-1`}
                  >
                    <FrequencyIcon className="h-3 w-3" />
                    {regulation.frequency}
                  </Badge>
                ...
                {/* Keywords */}
                <div>
                  <h4 className="font-semibold mb-2 text-gray-400">Keywords:</h4>
                  <div className="flex flex-wrap gap-2">
                ...
                {regulation.relatedRegs.length > 0 && (
                  <div>
                    <h4 className="font-semibold mb-2 text-gray-400">Related Regulations:</h4>
                    <div className="flex flex-wrap gap-2">
