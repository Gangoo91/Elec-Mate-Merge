...
> {
  ...
  render(): ReactNode {
    if (this.state.hasError) {
      ...
      return (
        <div
          className={`whitespace-pre-wrap text-[hsl(var(--text-muted))] leading-relaxed ${this.props.className || ''}`}
        >
          ...
          components={{
            ...
            blockquote: ({ children }) => (
              <Card className="bg-muted/20 border-l-4 border-[hsl(var(--primary-muted))] p-4 my-4">
                <div className="text-[hsl(var(--text-muted))] italic">{children}</div>
              </Card>
            ),
            ...
            em: ({ children }) => (
              <em className="italic text-[hsl(var(--text-muted))]">{children}</em>
            ),
          }}
        >
          {content}
...
const processElectricalContent = (children: React.ReactNode): React.ReactNode => {
  ...
  if (Array.isArray(children)) {
    return children.map((child, index) =>
      typeof child === 'string' ? processTextForElectricalElements(child) : child
    );
  }
  ...
  remainingText = remainingText.replace(/(\[BS 7671[^\]]*\])/g, (match) => {
    ...
    elements.push(
      <Badge
        key={placeholder}
        variant="outline"
        className="bg-[hsl(var(--primary-bg-muted))] text-[hsl(var(--primary-muted))] border-[hsl(var(--primary-muted))]/30 mx-1"
      >
        {match}
      </Badge>
    );
  ...
  remainingText = remainingText.replace(
    ...
    (match, value, unit) => {
      ...
      elements.push(
        <span
          key={placeholder}
          className="font-mono font-semibold text-[hsl(var(--primary-muted))]"
        >
          {value}
          {unit}
        </span>
