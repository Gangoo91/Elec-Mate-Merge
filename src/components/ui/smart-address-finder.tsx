    ...
    const searchUKPostcodes = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .or(
            `district_code.ilike.%${debouncedPostcode.toUpperCase()}%,local_authority.ilike.%${debouncedPostcode}%,region.ilike.%${debouncedPostcode}%`
          )
    ...
    <div className={cn('space-y-4', className)} ref={containerRef}>
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-elec-light/70 flex items-center gap-1">
          ...
          {searchMode === 'uk'
            ? 'Searching UK postcode database...'
            : searchMode === 'google'
              ? 'Searching international addresses...'
              : 'Type a UK postcode or international address'}
        </p>
      </div>
