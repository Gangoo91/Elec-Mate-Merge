...
const ToolCategoryDisplay = ({ categoryName }: ToolCategoryDisplayProps) => {
  ...
  const categoryTools =
    allTools?.filter((tool) => {
      ...
      const matchesCategory = possibleMatches.some(
        (match) =>
          toolCategory === match ||
  ...
  if (categoryTools.length > 0) {
    console.log(
      'Sample tool images:',
      categoryTools.slice(0, 3).map((t) => ({ name: t.name, image: t.image }))
    );
  }
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {categoryTools.map((tool, index) => (
            <Card
              key={tool.id || index}
              className="border-elec-yellow/20 bg-elec-gray hover:border-elec-yellow/50 transition-colors"
            >
