...
const MajorProjectsCard = () => {
  ...
  const staticProjects: MajorProject[] = [
    {
      ...
      description:
        'Complete electrical system upgrade for 15 underground stations including LED lighting, power distribution, and emergency systems.',
      ...
      awarded: true,
      project_link:
        'https://tfl.gov.uk/corporate/procurement-and-commercial/procurement-opportunities',
      details_link:
        'https://tfl.gov.uk/corporate/procurement-and-commercial/procurement-opportunities',
    },
    {
      ...
      description:
        'New electrical installation for major hospital expansion including critical care units, operating theatres, and backup power systems.',
      ...
      awarded: true,
      project_link:
        'https://www.contractsfinder.service.gov.uk/Search/Results?SearchType=1&Keywords=nhs+electrical',
    ...
    },
    {
      ...
      description:
        'High voltage transmission infrastructure to connect 800MW offshore wind farm to the national grid.',
  ...
  const determineSectorFromContent = (content: string): string => {
    ...
    if (
      ...
      contentLower.includes('railway') ||
      contentLower.includes('underground')
    )
      return 'Transport';
    if (contentLower.includes('school') || contentLower.includes('university')) return 'Education';
    if (
      contentLower.includes('wind') ||
      contentLower.includes('renewable') ||
      contentLower.includes('solar')
    )
    ...
    <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {project.category && (
            <Badge className={getSectorColor(project.category)}>{project.category}</Badge>
          )}
        </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-foreground font-medium">
              {(() => {
                if (
                  project.duration &&
                  project.duration.includes('T') &&
                  project.duration.includes('to')
                ) {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (statusLower.includes('tender') || statusLower === 'active')
      return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
    if (statusLower.includes('award') || statusLower === 'awarded')
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        {projects.map((project) => (
          <Card
            key={project.id}
            className="group hover:shadow-lg transition-all duration-300 border border-elec-yellow/10 bg-elec-card hover:border-elec-yellow/20 w-full min-w-0"
          >
            ...
            <CardContent className="pt-0">
              ...
              <div className="flex flex-col gap-2 mt-4 w-full min-w-0">
                {getStatusText(project.status) === 'Open for Tender' && (
                  <Button
                    size="sm"
                    className="bg-elec-yellow text-black hover:bg-elec-yellow/90 font-medium w-full sm:w-auto text-xs sm:text-sm"
                  >
