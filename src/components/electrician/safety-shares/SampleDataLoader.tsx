  ...
  const insertSampleData = async () => {
    ...
    try {
        ...
        const alertsData = [
          {
            ...
            content:
              'Following recent incidents, new safety protocols have been introduced for all electrical installations in commercial buildings. All electricians must now follow enhanced isolation procedures and use upgraded PPE when working on systems above 400V.',
            severity: 'critical',
          ...
          },
          {
            ...
            content:
              'The HSE has issued new guidance on PPE requirements for high voltage electrical work. All electricians working on systems above 1kV must now use arc-rated clothing and enhanced eye protection.',
            severity: 'high',
          ...
          },
          {
            ...
            content:
              'Recent incidents highlight the importance of proper cable detection before excavation. Always use approved cable detection equipment and follow the HSG47 guidance.',
            severity: 'medium',
            category: 'Cable Work',
          },
        ];

        const { error: alertsError } = await supabase.from('safety_alerts').insert(alertsData);
        ...
        const newsData = [
          {
            ...
            content:
              "The Institution of Engineering and Technology has published the 19th edition of BS 7671, the UK's electrical wiring regulations. Key updates include new requirements for electric vehicle charging installations, enhanced RCD protection, and updated guidance on surge protective devices.",
            regulatory_body: 'IET',
          ...
          },
          {
            ...
            content:
              'The Health and Safety Executive has released an updated version of HSG85, providing comprehensive guidance on electrical safety in the workplace. The document includes new sections on energy storage systems and smart grid technologies.',
            regulatory_body: 'HSE',
            category: 'Safety',
          },
          {
            ...
            content:
              'Ofgem has announced updated requirements for smart meter installations in commercial properties. All installations must now include enhanced cybersecurity measures and backup communication systems.',
            regulatory_body: 'Ofgem',
            category: 'Smart Technology',
          },
        ];

        const { error: newsError } = await supabase.from('industry_news').insert(newsData);
      ...
      if (dataStatus.projects === 0) {
        const projectsData = [
          {
            ...
            content:
              'Transport for London has awarded a £250 million contract for upgrading the electrical systems across the London Underground network. The project will involve replacing aging switchgear, installing new power distribution systems, and upgrading signalling infrastructure.',
          ...
          },
          {
            ...
            content:
              'National Grid has been awarded the contract to provide electrical connection infrastructure for the new Hornsea Three offshore wind farm. The project includes submarine cables, onshore transmission lines, and a new electrical substation.',
          ...
          },
          {
            ...
            content:
              'A major project to install and upgrade emergency power systems across 15 NHS hospitals in the South East. The work includes new generators, UPS systems, and automatic transfer switches to ensure continuous power during emergencies.',
        ...
        const { error: projectsError } = await supabase.from('major_projects').insert(projectsData);
      ...
      if (dataStatus.lfe === 0) {
        const lfeData = [
          ...
          },
          {
            ...
            content:
              'A 33kV underground cable joint failed catastrophically during normal operation, resulting in a significant explosion and power outage affecting 2,000 customers. Investigation revealed that the joint had been installed with incorrect torque settings, leading to gradual deterioration over 5 years.',
          ...
          },
          {
            ...
            content:
              "A fire broke out in a commercial premises due to an overloaded distribution board. The building's electrical load had been gradually increased over time without proper assessment of the existing infrastructure. The fire was contained but caused significant damage to the building.",
            incident_type: 'fire',
            key_takeaways: [
        ...
        const { error: lfeError } = await supabase.from('lfe_reports').insert(lfeData);
