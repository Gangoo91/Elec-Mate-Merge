...
interface CourseCompareModeProps {
  ...
  selectedCourseData: EnhancedCareerCourse[];
  onAddToComparison: (course: EnhancedCareerCourse) => void;
  onRemoveFromComparison: (courseId: string | number) => void;
  onClearComparison: () => void;
}

const CourseCompareMode = ({
  courses,
  onViewDetails,
  selectedCourseData,
  onAddToComparison,
  ...
  const isSelected = (courseId: string | number) => {
    return selectedCourseData.some((c) => c.id === courseId);
  };
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-elec-yellow/20">
                ...
                {selectedCourseData.map((course) => (
                  <th
                    key={course.id}
                    className="text-left py-3 px-2 font-medium text-elec-yellow min-w-48"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => onRemoveFromComparison(course.id)}
            ...
            <tbody className="divide-y divide-elec-yellow/10">
              <tr>
                ...
                {selectedCourseData.map((course) => (
                  <td key={course.id} className="py-3 px-2 text-elec-yellow">
                    {course.provider}
                  </td>
                ))}
              </tr>
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              >
                {selectedCourseData.length}/3 selected
              </Badge>
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={selectedCourseData.length < 2}
                  >
      ...
      {/* Comparison Table */}
      {showComparison && selectedCourseData.length >= 2 && <ComparisonTable />}
    </div>
  );
};
