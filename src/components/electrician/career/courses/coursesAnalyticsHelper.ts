...
export const generateCoursesAnalytics = (courses: EnhancedCareerCourse[]): CourseAnalytics => {
  ...
  const emergingTechCourses = courses.filter(
    (course) => course.category === 'Emerging Technologies'
  ).length;
  ...
  const averageSalaryImpact =
    salaryImpacts.length > 0
      ? `£${Math.round(salaryImpacts.reduce((a, b) => a + b, 0) / salaryImpacts.length).toLocaleString()}`
      : '£3,000';

  // Calculate top categories
  const categoryCount = courses.reduce(
    (acc, course) => {
