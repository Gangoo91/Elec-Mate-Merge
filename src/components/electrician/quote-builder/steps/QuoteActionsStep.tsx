...
const QuoteActionsStep = ({ quote, onQuoteUpdate }: QuoteActionsStepProps) => {
  ...
  const getAcceptanceStatusBadge = () => {
    switch (quote.acceptance_status || 'pending') {
      case 'accepted':
        return (
          <Badge className="bg-elec-green/20 text-elec-green border-elec-green/30">
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Share your quote with clients and collect signatures</CardDescription>
        </CardHeader>
          ...
          {quote.accepted_at && (
            <div className="mt-6 p-4 border border-elec-green/30 bg-elec-green/10 rounded-lg">
              ...
              <div className="text-sm text-muted-foreground space-y-1">
                ...
                <p>
                  <strong>Email:</strong> {quote.accepted_by_email}
                </p>
                <p>
                  <strong>Date:</strong> {new Date(quote.accepted_at).toLocaleDateString()}
                </p>
                <p>
                  <strong>Method:</strong> {quote.acceptance_method}
                </p>
              </div>
            </div>
          )}
