  ...
  const recoverySteps = [
    ...
    },
    {
      ...
      methods: [
        ...
        'Invoice re-submission with terms',
      ],
    ...
    },
    {
      ...
      methods: [
        ...
        'Recorded delivery formal notice',
        'Final demand with legal warning',
      ],
    ...
    },
    {
      ...
      methods: [
        ...
        'Legal representative engagement',
      ],
  ...
  const negotiationTips = [
    {
      ...
      description:
        'Thoroughly understand their financial situation, constraints, and genuine ability to pay',
      benefit: 'Helps identify realistic payment solutions that work for both parties',
    },
    {
      tip: 'Flexible Payment Plan Options',
      description:
        'Offer structured installment plans that break large amounts into manageable payments',
      benefit: 'Significantly increases likelihood of full recovery over time',
    },
    {
      tip: 'Comprehensive Documentation',
      description:
        'Maintain detailed records of all conversations, agreements, and payment commitments',
      benefit: 'Provides legal protection and evidence for future enforcement if needed',
    },
    {
      tip: 'Clear Deadlines & Accountability',
      description:
        'Agree specific payment dates and follow-up procedures with consequences for missed payments',
    ...
    },
    {
      ...
      description:
        'Keep all discussions solution-focused, respectful, and professionally conducted',
    ...
    <div className="space-y-4">
      <Alert className="border-blue-500/50 bg-blue-500/10">
        ...
        <AlertDescription className="text-blue-200">
          A structured professional recovery process increases collection rates by 300% while
                ...
                <div key={index} className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div
                        className={`${isMobile ? 'text-xs' : 'text-sm'} text-blue-200 flex items-start gap-1`}
                      >
                      ...
                      >
                        <CheckCircle className="h-3 w-3 text-green-400 flex-shrink-0 mt-1" />
                        <div>
                          <strong>Communication Tone:</strong> {step.tone}
                        </div>
                      </div>
                      <div
                        className={`${isMobile ? 'text-xs' : 'text-sm'} text-blue-200 flex items-start gap-1`}
                      >
                        <CheckCircle className="h-3 w-3 text-green-400 flex-shrink-0 mt-1" />
                        <div>
                          <strong>Primary Objective:</strong> {step.objective}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {index < recoverySteps.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
            </div>
              ...
              {communicationTemplates.map((template, index) => (
                <div key={index} className="space-y-3">
                  ...
                  <div className="bg-green-500/10 border border-green-500/30 rounded p-3">
                    ...
                    <p
                      className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-200 italic leading-relaxed`}
                    >
                      "{template.template}"
                    </p>
                  </div>

                  {index < communicationTemplates.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {paymentPlanStrategies.map((strategy, index) => (
                <div key={index} className="space-y-3">
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {strategy.structure.map((item, itemIndex) => (
                        <li
                          key={itemIndex}
                          className={`${isMobile ? 'text-xs' : 'text-sm'} text-purple-200 flex items-start gap-1`}
                        >
                          <CheckCircle className="h-3 w-3 text-green-400 flex-shrink-0 mt-1" />
                          {item}
                        </li>
                  ...
                  {index < paymentPlanStrategies.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {negotiationTips.map((tip, index) => (
                <div key={index} className="space-y-3">
                  ...
                  {index < negotiationTips.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
          ...
          <MobileAccordionContent>
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {documentationRequirements.map((category, index) => (
                <div key={index} className="space-y-3">
                  <h4
                    className={`font-medium text-foreground ${isMobile ? 'text-sm' : 'text-base'} border-b border-amber-500/20 pb-1`}
                  >
                    {category.category}
                  </h4>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {category.requirements.map((requirement, reqIndex) => (
                        <li
                          key={reqIndex}
                          className={`${isMobile ? 'text-xs' : 'text-sm'} text-amber-200 p-2 bg-amber-500/5 border border-amber-500/20 rounded flex items-start gap-2`}
                        >
                          <CheckCircle className="h-3 w-3 text-green-400 flex-shrink-0 mt-1" />
                          {requirement}
                        </li>
                  ...
                  {index < documentationRequirements.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
