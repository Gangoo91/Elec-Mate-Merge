...
const PreventionTab = () => {
  ...
  const earlyWarningSystem = [
    {
      ...
      response:
        'Initiate immediate face-to-face meeting to discuss concerns and clarify project status',
    },
    {
      ...
      response:
        'Consider requesting additional security deposits or advance payments before continuing work',
    },
    {
      ...
      response:
        'Document all interactions meticulously and prepare comprehensive evidence for potential payment disputes',
    },
  ];
  ...
  const redFlags = [
    ...
    },
    {
      ...
      action:
        'Clarify all expectations in writing and obtain signed acknowledgment before proceeding',
    },
    {
    ...
    },
    {
      ...
      warning:
        'Recently incorporated company with no established trading history or business relationships',
      action:
        'Request personal guarantees from all company directors and higher deposit requirements',
    },
    {
      category: 'Behavioural Risk',
      warning:
        'Excessive pressure to commence work immediately without proper contracts or documentation',
      action:
        'Insist on comprehensive signed contracts and full payment terms agreement before any work begins',
    },
  ];
    ...
    <div className="space-y-4">
      <Alert className="border-green-500/50 bg-green-500/10">
        ...
        <AlertDescription className="text-green-200">
          Comprehensive prevention strategies reduce bad debt risk by 95% and save thousands in
          recovery costs. Implement these measures before problems arise.
        </AlertDescription>
      </Alert>
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {riskAssessmentTools.map((tool, index) => (
                <div key={index} className="space-y-3">
                  ...
                  {index < riskAssessmentTools.length - 1 && (
          ...
          <MobileAccordionContent>
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {contractBestPractices.map((section, index) => (
                <div key={index} className="space-y-3">
                  <h4
                    className={`font-medium text-foreground ${isMobile ? 'text-sm' : 'text-base'} border-b border-purple-500/20 pb-1`}
                  >
                    {section.section}
                  </h4>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {section.requirements.map((requirement, reqIndex) => (
                        <li
                          key={reqIndex}
                          className={`${isMobile ? 'text-xs' : 'text-sm'} text-purple-200 p-2 bg-purple-500/5 border border-purple-500/20 rounded flex items-start gap-2`}
                        >
                  ...
                  {index < contractBestPractices.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
              ...
              {clientScreeningProcedures.map((category, index) => (
                <div key={index} className="space-y-3">
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    <div>
                      <h5
                        className={`font-medium text-red-300 mb-2 ${isMobile ? 'text-xs' : 'text-sm'}`}
                      >
                        Red Flags to Watch
                      </h5>
                      <ul className="space-y-1">
                        {category.redFlags.map((flag, flagIndex) => (
                          <li
                            key={flagIndex}
                            className={`${isMobile ? 'text-xs' : 'text-sm'} text-red-200 flex items-start gap-1`}
                          >
                  ...
                  {index < clientScreeningProcedures.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {paymentStructures.map((structure, index) => (
                <div key={index} className="space-y-3">
                  ...
                  {index < paymentStructures.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {earlyWarningSystem.map((system, index) => (
                <div key={index} className="space-y-3">
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {system.warnings.map((warning, warningIndex) => (
                        <li
                          key={warningIndex}
                          className={`${isMobile ? 'text-xs' : 'text-sm'} text-amber-200 flex items-start gap-1`}
                        >
                  ...
                  {index < earlyWarningSystem.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {preventionStrategies.map((strategy, index) => (
                <div key={index} className="space-y-3">
                  ...
                  {index < preventionStrategies.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
                ...
                <div key={index} className="space-y-3">
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        variant="outline"
                        className={`text-red-300 border-red-400/30 ${isMobile ? 'text-xs' : 'text-sm'}`}
                      >
                        {flag.category}
                      </Badge>
                    </div>
                  ...
                  <div className="bg-red-500/10 border border-red-500/30 rounded p-2">
                    <h5
                      className={`font-medium text-red-300 mb-1 ${isMobile ? 'text-xs' : 'text-sm'}`}
                    >
                      Recommended Action
                    </h5>
                    <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-red-200`}>
                      {flag.action}
                    </p>
                  </div>

                  {index < redFlags.length - 1 && (
                    <div className="border-t border-elec-yellow/10 pt-4"></div>
                  )}
                </div>
              ))}
