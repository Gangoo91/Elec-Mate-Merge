      ...
      <div className="space-y-4">
        ...
        <MobileAccordion type="single" collapsible className="space-y-2">
              ...
              <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
                {skillsProgressionFramework.map((competency, index) => (
                  <div key={index} className="border border-purple-500/20 rounded-lg p-3 space-y-3">
                    <h4 className="font-medium text-white text-sm">{competency.competency}</h4>
                    
                    <div className="space-y-3">
                      {competency.progression.map((level, levelIndex) => (
                        <div key={levelIndex} className="border border-purple-500/10 rounded p-2 space-y-2">
                          <h5 className="font-medium text-purple-300 text-xs">{level.level}</h5>
                          
                          <div>
                            <h6 className="font-medium text-purple-200 mb-1 text-xs">Requirements</h6>
                            <ul className="space-y-1">
                          ...
                          <div className="bg-amber-500/10 border border-amber-500/30 rounded p-2">
                            <h6 className="font-medium text-amber-300 mb-1 text-xs">Assessment</h6>
                            <p className="text-xs text-amber-200">{level.assessment}</p>
                          </div>
              ...
              <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
                {modernMentoringApproach.map((approach, index) => (
                  <div key={index} className="border border-orange-500/20 rounded-lg p-3 space-y-3">
                    ...
                    <div>
                      <h5 className="font-medium text-orange-300 mb-2 text-xs">Key Features</h5>
                      <ul className="space-y-1">
                        {approach.features.map((feature, featureIndex) => (
                          <li key={featureIndex} className="text-xs text-orange-200 flex items-center gap-1">
          ...
          <MobileAccordionItem value="costs">
            <MobileAccordionTrigger icon={<PoundSterling className="h-5 w-5 text-amber-400" />}>
            ...
            <MobileAccordionContent>
              <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
                  ...
                  <div key={index} className="border border-amber-500/20 rounded-lg p-3 space-y-3">
                    ...
                    <div className="space-y-2">
                      {category.costs.map((cost, costIndex) => (
                        <div key={costIndex} className="flex justify-between items-center text-xs p-2 bg-amber-500/5 rounded">
                          <div>
                            <div className="text-amber-200 font-medium">{cost.item}</div>
                            <div className="text-amber-300/60">{cost.period}</div>
                          </div>
                          <div className="text-right">
                            <div className="text-amber-300 font-medium">{cost.amount}</div>
                            <div className="text-amber-400/60 text-xs">{cost.funding}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="bg-elec-yellow/10 border border-elec-yellow/30 rounded-lg p-3">
                  <h4 className="font-medium text-elec-yellow mb-2 text-sm">Investment Summary</h4>
                  <p className="text-xs text-elec-yellow/80">
                    Total 3-year investment: £15,000-22,000 per apprentice. With government funding, 
                    net employer cost typically £8,000-12,000 with ROI within 18 months.
                  </p>
                </div>
              </div>
            </MobileAccordionContent>
        ...
        <Alert className="border-green-500/50 bg-green-500/10">
          ...
          <AlertDescription className="text-green-200">
            <strong>Success Tip:</strong> Structured training with digital tools and proper mentoring improves completion rates and reduces time to competency.
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <Alert className="border-blue-500/50 bg-blue-500/10">
        <Brain className="h-4 w-4 text-blue-400" />
        <AlertDescription className="text-blue-200">
          <strong>2025 Update:</strong> Modern training approaches with digital integration improve outcomes and reduce costs.
        </AlertDescription>
      </Alert>
      ...
      <div className="text-center text-muted-foreground">
        <p>Switch to mobile view for the comprehensive training development framework.</p>
      </div>
    </div>
  );
};
