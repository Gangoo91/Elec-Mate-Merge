...
const TrainingDevelopmentTab = () => {
  ...
  const modernTrainingFramework = [
    ...
    },
    {
      ...
      employerBenefit:
        'Qualified electrician ready for senior responsibilities and business growth',
      ...
      costs: [
        {
          ...
          amount: '£4,500-7,000',
          period: 'Annual',
          funding: '95-100% government funded',
        },
        {
          item: 'Assessment and EPA costs',
          amount: '£1,500-2,500',
          period: 'Total programme',
          funding: 'Employer responsibility',
        },
        {
          item: 'Learning materials and resources',
          amount: '£800-1,200',
          period: 'Annual',
          funding: 'Employer/apprentice shared',
        },
        {
          item: 'Digital platform subscriptions',
          amount: '£300-600',
          period: 'Annual',
          funding: 'Employer investment',
        },
      ],
    },
    {
      ...
      costs: [
        {
          item: 'Mentor time allocation',
          amount: '£6,000-9,000',
          period: 'Annual',
          funding: 'Internal resource cost',
        },
        {
          item: 'Reduced productivity (Year 1)',
          amount: '£3,000-5,000',
          period: 'First year',
          funding: 'Business investment',
        },
        {
          item: 'Equipment and tool allowance',
          amount: '£1,500-2,500',
          period: 'Total programme',
          funding: 'Employer provision',
        },
        {
    ...
    <div className="space-y-4">
      <Alert className="border-blue-500/50 bg-blue-500/10">
        ...
        <AlertDescription className="text-blue-200">
          Modern training approaches improve apprentice completion rates by 25% and reduce time to
          competency.
        </AlertDescription>
      </Alert>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {phase.kpis.map((kpi, kpiIndex) => (
                        <Badge
                          key={kpiIndex}
                          variant="outline"
                          className="text-purple-300 border-purple-400/30 text-xs"
                        >
                          {kpi}
                        </Badge>
                      ))}
                  ...
                  {category.providers.map((provider, providerIndex) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Badge
                          variant="outline"
                          className="text-green-300 border-green-400/30 text-xs"
                        >
                          {provider.cost}
                        </Badge>
                      </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {provider.features.map((feature, featureIndex) => (
                            <Badge
                              key={featureIndex}
                              variant="outline"
                              className="text-green-200 border-green-400/20 text-xs"
                            >
                              {feature}
                            </Badge>
                          ))}
              ...
              {skillsProgressionFramework.map((competency, index) => (
                <div key={index} className="border border-purple-500/20 rounded-lg p-3 space-y-3">
                  ...
                  {competency.progression.map((level, levelIndex) => (
                    <div
                      key={levelIndex}
                      className="border border-purple-500/20 rounded p-3 space-y-2"
                    >
                      ...
                      <div className="bg-blue-500/10 border border-blue-500/30 rounded p-2">
                        <h6 className="font-medium text-blue-300 mb-1 text-xs">
                          Assessment Method
                        </h6>
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {approach.features.map((feature, featureIndex) => (
                        <li
                          key={featureIndex}
                          className="text-xs text-amber-200 flex items-center gap-1"
                        >
                          <CheckCircle className="h-3 w-3 text-green-400 flex-shrink-0" />
                          {feature}
                        </li>
              ...
              {trainingCostAnalysis.map((category, index) => (
                <div key={index} className="space-y-3">
                  ...
                  {category.costs.map((cost, costIndex) => (
                    <div
                      key={costIndex}
                      className="border border-yellow-500/20 rounded-lg p-3 space-y-2"
                    >
                      <div className="flex items-center justify-between">
                        <h5 className="font-medium text-foreground text-sm">{cost.item}</h5>
                        <Badge
                          variant="outline"
                          className="text-yellow-300 border-yellow-400/30 text-xs"
                        >
                          {cost.amount}
                        </Badge>
                      </div>
