      ...
      <Card className="border-green-500/50 bg-green-500/10">
        ...
        <CardContent className="space-y-4">
          {marketTrends.map((trend, index) => (
            <div
              key={index}
              className="flex items-center justify-between p-4 bg-green-500/5 rounded-lg border border-green-500/20"
            >
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {region.specialties.map((specialty, specialtyIndex) => (
                    <Badge
                      key={specialtyIndex}
                      variant="outline"
                      className="text-xs text-blue-200 border-blue-400/30"
                    >
                      {specialty}
                    </Badge>
                  ))}
