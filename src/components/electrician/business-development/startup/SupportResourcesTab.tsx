    ...
    <div className="space-y-4">
      <Alert className="border-green-500/50 bg-green-500/10">
        ...
        <AlertDescription className="text-green-200">
          Support networks increase business success rates by 300% - you don't have to build your
          electrical business alone.
        </AlertDescription>
      </Alert>
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {emergencySupport.map((category, index) => (
                <div key={index} className="space-y-3">
                  <h4
                    className={`font-medium text-red-300 ${isMobile ? 'text-sm' : 'text-base'} border-b border-red-500/20 pb-1`}
                  >
                    {category.category}
                  </h4>
                  {category.services.map((service, serviceIndex) => (
                    <div
                      key={serviceIndex}
                      className="border border-red-500/20 rounded-lg p-3 space-y-2"
                    >
          ...
          <MobileAccordionContent>
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {businessHelplines.map((category, index) => (
                <div key={index} className="space-y-3">
                  <h4
                    className={`font-medium text-blue-300 ${isMobile ? 'text-sm' : 'text-base'} border-b border-blue-500/20 pb-1`}
                  >
                    {category.category}
                  </h4>
                  {category.services.map((service, serviceIndex) => (
                    ...
                    >
                      <div
                        className={`${isMobile ? 'text-center space-y-2' : 'flex items-center justify-between'}`}
                      >
                        <h5
                          className={`font-medium text-foreground ${isMobile ? 'text-sm' : 'text-base'}`}
                        >
          ...
          <MobileAccordionContent>
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {onlineResources.map((category, index) => (
                <div key={index} className="space-y-3">
                  <h4
                    className={`font-medium text-green-300 ${isMobile ? 'text-sm' : 'text-base'} border-b border-green-500/20 pb-1`}
                  >
                    {category.category}
                  </h4>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {platform.features.map((feature, featureIndex) => (
                            <Badge
                              key={featureIndex}
                              variant="outline"
                              className={`text-green-200 border-green-400/20 ${isMobile ? 'text-xs' : 'text-sm'}`}
                            >
                              {feature}
                            </Badge>
                          ))}
              ...
              {mentorshipPrograms.map((category, index) => (
                <div key={index} className="space-y-3">
                  ...
                  {category.programs.map((program, programIndex) => (
                    <div
                      key={programIndex}
                      className="border border-purple-500/20 rounded-lg p-3 space-y-3"
                    >
                      <div className="flex flex-col gap-2">
                        <div
                          className={`${isMobile ? 'text-center space-y-1' : 'flex items-center justify-between'}`}
                        >
                          <h5
                            className={`font-medium text-foreground ${isMobile ? 'text-sm' : 'text-base'}`}
                          >
          ...
          <MobileAccordionContent>
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {trainingProviders.map((category, index) => (
                <div key={index} className="space-y-3">
                  <h4
                    className={`font-medium text-orange-300 ${isMobile ? 'text-sm' : 'text-base'} border-b border-orange-500/20 pb-1`}
                  >
                    {category.category}
                  </h4>
                  {category.providers.map((provider, providerIndex) => (
                    <div
                      key={providerIndex}
                      className="border border-orange-500/20 rounded-lg p-3 space-y-2"
                    >
                      <div
                        className={`${isMobile ? 'text-center space-y-1' : 'flex items-center justify-between'}`}
                      >
                        ...
                        <div className="flex flex-wrap gap-1">
                          {provider.courses.map((course, courseIndex) => (
                            <Badge
                              key={courseIndex}
                              variant="outline"
                              className={`text-orange-200 border-orange-400/20 ${isMobile ? 'text-xs' : 'text-sm'}`}
                            >
                              {course}
                            </Badge>
                          ))}
