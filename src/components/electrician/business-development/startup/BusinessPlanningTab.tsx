...
const BusinessPlanningTab = () => {
  ...
  const marketResearchStrategy = [
    {
      ...
      businessImpact:
        'Identify pricing opportunities and service gaps worth £2,000-5,000 monthly revenue',
  ...
  const financialPlanning = [
    {
      ...
      investments: [
        ...
        {
          item: 'Test equipment (multimeters, testers)',
          amount: '£1,500-3,000',
          necessity: 'Essential',
        },
  ...
  const businessStructure = [
    ...
    },
    {
      ...
      businessImpact:
        'Limited liability protection and potential tax advantages for higher earners',
      considerations: [
        'Limited liability',
        'Corporation tax',
    ...
    <div className="space-y-4">
      <Alert className="border-blue-500/50 bg-blue-500/10">
        ...
        <AlertDescription className="text-blue-200">
          Comprehensive business planning increases success rates by 300% and reduces
          time-to-profitability by 40%.
        </AlertDescription>
      </Alert>
                  ...
                  <div className="border border-green-500/20 rounded-lg p-3 space-y-2">
                    ...
                    <div className="space-y-3">
                      {category.investments.map((investment, invIndex) => (
                        <div
                          key={invIndex}
                          className={`${isMobile ? 'text-center space-y-2' : 'flex items-center justify-between'} p-3 bg-green-500/5 border border-green-500/20 rounded`}
                        >
                          {isMobile ? (
                            <>
                              ...
                              <Badge
                                variant="outline"
                                className="text-green-300 border-green-400/30 text-sm"
                              >
        ...
        <MobileAccordionItem value="planning-checklist">
          ...
          <MobileAccordionContent>
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              ...
              <div className="space-y-4">
                <h4
                  className={`font-medium text-orange-300 ${isMobile ? 'text-sm' : 'text-base'} mb-3`}
                >
                  Success Milestones
                </h4>
                <div className="space-y-3">
                  {[
                    ...
                    <div
                      key={index}
                      className={`${isMobile ? 'text-center py-3' : 'flex items-center justify-between py-2'}`}
                    >
                      <div className={`${isMobile ? 'space-y-2' : ''}`}>
                        <div
                          className={`${isMobile ? 'text-sm font-medium' : 'text-sm'} text-orange-200 ${isMobile ? 'leading-relaxed' : ''}`}
                        >
                          {item.milestone}
                        </div>
                        <Badge
                          variant="outline"
                          className={`text-orange-300 border-orange-400/30 ${isMobile ? 'text-sm' : 'text-sm'} font-medium ${isMobile ? 'inline-block' : ''}`}
                        >
                          {item.target}
                        </Badge>
                      </div>
