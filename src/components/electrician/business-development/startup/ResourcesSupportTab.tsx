...
const ResourcesSupportTab = () => {
  ...
  const renderMobileView = () => (
    <MobileAccordion type="multiple" defaultValue={defaultOpenValues} className="space-y-3">
      {supportCategories.map((category) => {
        const IconComponent = category.icon;
        return (
          <MobileAccordionItem
            ...
            <MobileAccordionContent className="px-4 pb-4">
              <div className="space-y-4">
                {category.resources.map((resource, index) => (
                  <div
                    key={index}
                    className="space-y-3 pb-4 border-b border-border/50 last:border-b-0 last:pb-0"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
