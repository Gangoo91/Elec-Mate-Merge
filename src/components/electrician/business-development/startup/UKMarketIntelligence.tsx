        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-blue-500/20 rounded-lg border border-blue-400/30">
            ...
            <p className="text-blue-300 text-sm italic">
              "Your price is what you believe your time is worth, regardless of what others are
              charging. Factor in your skills, experience, overhead costs, and desired profit margin
              to set rates that reflect your true value."
            </p>
          </div>
            ...
            {marketStats.map((stat, index) => (
              <Card key={index} className="bg-blue-500/20 border-blue-400/30">
                <CardContent className="p-4">
                  ...
                  <Badge
                    variant="outline"
                    className="mt-2 text-xs border-blue-400/40 text-blue-300"
                  >
                    {stat.trend}
                  </Badge>
                </CardContent>
                ...
                <CardContent className="pt-0">
                  <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {opportunity.requirements.map((req, idx) => (
                          <Badge
                            key={idx}
                            variant="outline"
                            className="text-xs border-green-400/30 text-green-300"
                          >
                            {req}
                          </Badge>
                        ))}
                    ...
                    <div className="border-t border-green-500/20 pt-3">
                      <p className="text-xs font-semibold text-green-200 mb-2">
                        Job Value Breakdown:
                      </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {regionalInsights.map((region, index) => (
              <Card key={index} className="bg-orange-500/20 border-orange-400/30">
                <CardHeader className="pb-3">
                  <CardTitle className="text-orange-200">{region.region}</CardTitle>
                </CardHeader>
                <CardContent className="pt-0 space-y-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-orange-200 mb-1">
                      Popular Specialisms:
                    </p>
                    <div className="flex flex-wrap gap-1">
                      {region.specialisms.map((spec, idx) => (
                        <Badge
                          key={idx}
                          variant="outline"
                          className="text-xs border-orange-400/30 text-orange-300"
                        >
                          {spec}
                        </Badge>
                      ))}
