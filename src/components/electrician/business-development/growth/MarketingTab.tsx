  ...
  const marketingStrategies = [
    {
      ...
      description:
        'Build a comprehensive digital marketing strategy to dominate local search results and attract customers online',
      components: [
    ...
    },
    {
      ...
      description:
        'Build strong local relationships and establish your business as the go-to electrical contractor in your area',
      components: [
    ...
    },
    {
      ...
      description:
        'Develop systematic approaches to turn satisfied customers into brand ambassadors and repeat clients',
      components: [
    ...
    },
    {
      ...
      description:
        'Position yourself as the electrical expert in your area through valuable content and educational marketing',
      components: [
  ...
  const marketingChannelsBenchmarks = [
    {
      ...
      benchmarks: [
        {
          metric: 'Google Ads Cost Per Click',
          target: '£2-8 CPC',
          current: 'Electrical services average £4.50',
        },
        {
          metric: 'Facebook Ads Cost Per Lead',
          target: '£15-40 CPL',
          current: 'Local services average £25',
        },
        {
          metric: 'SEO Lead Generation',
          target: '20-50 leads/month',
          current: 'Well-optimised sites achieve 35+',
        },
        {
          metric: 'Email Marketing ROI',
          target: '£25-42 per £1 spent',
          current: 'Industry average £38 return',
        },
      ],
    },
    {
      ...
      benchmarks: [
        {
          metric: 'Local Print Advertising',
          target: '2-5% response rate',
          current: 'Community papers average 3.2%',
        },
        {
          metric: 'Direct Mail Response',
          target: '1-3% response rate',
          current: 'Targeted areas achieve 2.1%',
        },
        {
          metric: 'Radio Advertising ROI',
          target: '£3-7 per £1 spent',
          current: 'Local stations average £4.80',
        },
        {
          metric: 'Referral Programme Success',
          target: '15-30% customer participation',
          current: 'Well-designed programmes 22%',
        },
      ],
    },
  ];
    ...
    <div className="space-y-4">
      <Alert className="border-elec-yellow/50 bg-elec-yellow/10">
        ...
        <AlertDescription className="text-elec-yellow">
          Effective marketing can increase lead generation by 300-500% and reduce customer
      ...
      <MobileAccordion type="single" collapsible className="space-y-2">
        {marketingStrategies.map((strategy) => (
          <MobileAccordionItem key={strategy.id} value={strategy.id}>
            <MobileAccordionTrigger
              icon={
                strategy.id === 'digital-marketing' ? (
                  <Globe className="h-5 w-5 text-blue-400" />
                ) : strategy.id === 'local-community' ? (
                  <Building className="h-5 w-5 text-green-400" />
                ) : strategy.id === 'referral-systems' ? (
            ...
            <MobileAccordionContent>
              <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
                <div className="border border-blue-500/20 rounded-lg p-3 space-y-3">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {strategy.implementation.map((phase, phaseIndex) => (
                        ...
                        >
                          <h6
                            className={`font-medium text-elec-yellow ${isMobile ? 'text-xs' : 'text-sm'}`}
                          >
                            {phase.phase}
                          </h6>
                  ...
                  <div>
                    <h5
                      className={`font-medium text-yellow-300 mb-2 ${isMobile ? 'text-xs' : 'text-sm'}`}
                    >
                    ...
                    <ul className="space-y-1">
                      {strategy.ukSpecific2025.map((opportunity, oppIndex) => (
                        <li
                          key={oppIndex}
                          className={`${isMobile ? 'text-xs' : 'text-sm'} text-yellow-200 flex items-center gap-1`}
                        >
                ...
                >
                  {marketingChannelsBenchmarks.map((category, categoryIndex) => (
                    <div key={categoryIndex} className="space-y-4">
                      <h4
                        className={`font-semibold text-elec-yellow ${isMobile ? 'text-sm' : 'text-base'}`}
                      >
                        {category.category}
                      </h4>
                      <div className="space-y-3">
                        {category.benchmarks.map((benchmark, benchmarkIndex) => (
                          <div
                            key={benchmarkIndex}
                            className="bg-elec-gray/30 rounded-lg p-4 space-y-2"
                          >
              ...
              <div className="border border-blue-500/20 rounded-lg p-3 space-y-3">
                ...
                <div className="space-y-4">
                  {salesConversionStrategies.map((strategy, strategyIndex) => (
                    <div
                      key={strategyIndex}
                      className="border border-purple-500/20 rounded-lg p-4 space-y-3"
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <h4
                            className={`font-semibold text-purple-300 ${isMobile ? 'text-sm' : 'text-base'}`}
                          >
                            {strategy.title}
                          </h4>
                          <p
                        ...
                        <Badge
                          variant="outline"
                          className={`text-purple-300 border-purple-400/30 ml-2 ${isMobile ? 'text-xs' : 'text-sm'}`}
                        >
              ...
              <div className="border border-blue-500/20 rounded-lg p-3 space-y-3">
                ...
                >
                  {performanceMetrics.map((metricCategory, categoryIndex) => (
                    <div key={categoryIndex} className="space-y-4">
                      <h4
                        className={`font-semibold text-elec-yellow flex items-center gap-2 ${isMobile ? 'text-sm' : 'text-base'}`}
                      >
                        {metricCategory.category === 'Lead Generation' && (
                          <Megaphone className="h-4 w-4" />
                        )}
                        {metricCategory.category === 'Sales Conversion' && (
                          <Target className="h-4 w-4" />
                        )}
                        {metricCategory.category === 'Brand & Reputation' && (
                      ...
                      <div className="space-y-2">
                        {metricCategory.kpis.map((kpi, kpiIndex) => (
                          <div
                            key={kpiIndex}
                            className="flex items-start gap-2 p-2 bg-elec-gray/30 rounded-lg"
                          >
              ...
              <div className="border border-blue-500/20 rounded-lg p-3 space-y-3">
                ...
                <div className="space-y-4">
                  <div className="bg-elec-gray/30 rounded-lg p-4 space-y-3">
                    <h4
                      className={`font-semibold text-blue-300 ${isMobile ? 'text-sm' : 'text-base'}`}
                    >
                    ...
                    <ul className="space-y-2">
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          Effective marketing can increase your customer base by 200-400% within the
                          first year
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                      ...
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          Strong brand reputation leads to premium pricing opportunities (15-25%
                          higher rates)
                        </span>
                      </li>
                    </ul>
                  ...
                  <div className="bg-elec-gray/30 rounded-lg p-4 space-y-3">
                    ...
                    <ul className="space-y-2">
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          Most electrical contractors still rely heavily on word-of-mouth, creating
                          opportunities for digital-savvy businesses
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <Globe className="h-4 w-4 text-yellow-400 shrink-0 mt-0.5" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          78% of customers research electricians online before making contact,
                          making digital presence crucial
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <Users className="h-4 w-4 text-yellow-400 shrink-0 mt-0.5" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          Referral systems can generate 40-60% of new business for established
                  ...
                  <div className="bg-elec-gray/30 rounded-lg p-4 space-y-3">
                    ...
                    <ul className="space-y-2">
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          Consistent marketing creates business stability and reduces dependence on
                      ...
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          Strong brand presence enables business scaling and potential expansion
                      ...
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-foreground`}>
                          Established marketing systems increase business value for potential sale
              ...
              <div className="border border-blue-500/20 rounded-lg p-3 space-y-3">
                ...
                <div className="space-y-4">
                    ...
                    <ul className="space-y-2">
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-200`}>
                          Can start with digital marketing basics (Google My Business, social media
                          presence)
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <div className="w-1 h-1 bg-green-400 rounded-full mt-2 shrink-0" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-200`}>
                          Focus on local networking and referral systems as primary growth
                          strategies
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <div className="w-1 h-1 bg-green-400 rounded-full mt-2 shrink-0" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-200`}>
                          Can implement customer follow-up systems and review management
                          independently
                        </span>
                      </li>
                    </ul>
                    ...
                    <ul className="space-y-2">
                      <li className="flex items-start gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-blue-200`}>
                          Can invest in comprehensive digital marketing campaigns and professional
                          websites
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <div className="w-1 h-1 bg-blue-400 rounded-full mt-2 shrink-0" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-blue-200`}>
                          Have resources to hire marketing specialists or agencies for advanced
                          strategies
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <div className="w-1 h-1 bg-blue-400 rounded-full mt-2 shrink-0" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-blue-200`}>
                          Can implement multi-channel marketing approaches and track detailed
                          performance metrics
                        </span>
                      </li>
                    </ul>
                  ...
                  <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-4 space-y-3">
                    ...
                    <div
                      className={`grid gap-3 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}
                    >
                      <div className="space-y-2">
                        <h5
                          className={`font-medium text-orange-200 ${isMobile ? 'text-xs' : 'text-sm'}`}
                        >
                          Essential Skills:
                        </h5>
                        <ul className="space-y-1">
                          <li
                            className={`${isMobile ? 'text-xs' : 'text-sm'} text-orange-100 flex items-center gap-1`}
                          >
                  ...
                  <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-lg p-4 space-y-3">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <h5
                          className={`font-medium text-cyan-200 ${isMobile ? 'text-xs' : 'text-sm'}`}
                        >
                          Freelance Social Media Managers
                        </h5>
                        <div
                          className={`grid gap-2 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}
                        >
                          <div>
                            <p
                              className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100 font-medium`}
                            >
                              Where to find:
                            </p>
                            <ul className="space-y-1 mt-1">
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                          ...
                          <div>
                            ...
                            <ul className="space-y-1 mt-1">
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £300-800/month basic management
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £15-40/hour project rates
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £50-150 per content package
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £200-500 website setup
                              </li>
                            </ul>
                          </div>
                        </div>
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          <div>
                            ...
                            <ul className="space-y-1 mt-1">
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • Google "trade marketing agency UK"
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                          ...
                          <div>
                            ...
                            <ul className="space-y-1 mt-1">
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £800-2,500/month retainer
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £2,000-8,000 website build
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • 15-20% of ad spend management
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £500-1,500 strategy consultation
                              </li>
                            </ul>
                          </div>
                        </div>
                        ...
                        >
                          <div>
                            ...
                            <ul className="space-y-1 mt-1">
                              ...
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • Local college marketing courses
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                          ...
                          <div>
                            ...
                            <ul className="space-y-1 mt-1">
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £10-18/hour depending on experience
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • 8-16 hours/week commitment
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • £400-1,200/month salary cost
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                      ...
                      <div className="space-y-2">
                        ...
                        <div
                          className={`grid gap-3 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-3'}`}
                        >
                          <div>
                            <p
                              className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100 font-medium mb-1`}
                            >
                              Essential Skills:
                            </p>
                            <ul className="space-y-1">
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • Trade industry knowledge
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • Local market understanding
                              </li>
                              <li className={`${isMobile ? 'text-xs' : 'text-sm'} text-cyan-100`}>
                                • Portfolio of trade clients
                              </li>
                            </ul>
                          </div>
                          <div>
