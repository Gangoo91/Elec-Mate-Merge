    ...
    <div className="space-y-4">
      <Alert className="border-elec-yellow/50 bg-elec-yellow/10">
        ...
        <AlertDescription className="text-elec-yellow">
          Strategic pricing optimisation can increase profit margins by 15-35% while maintaining
      ...
      <MobileAccordion type="single" collapsible className="space-y-2">
        {pricingStrategies.map((strategy) => (
          <MobileAccordionItem key={strategy.id} value={strategy.id}>
            <MobileAccordionTrigger
              icon={
                strategy.id === 'cost-plus-strategy' ? (
                  <Calculator className="h-5 w-5 text-blue-400" />
                ) : strategy.id === 'value-based-pricing' ? (
                  <Target className="h-5 w-5 text-green-400" />
                ) : strategy.id === 'dynamic-pricing' ? (
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {pricingBenchmarks.map((category, categoryIndex) => (
                    <div key={categoryIndex} className="space-y-3">
                      <h4
                        className={`font-medium text-elec-yellow ${isMobile ? 'text-sm' : 'text-base'}`}
                      >
                        {category.category}
                      </h4>
                      <div className="space-y-2">
                        {category.benchmarks.map((benchmark, benchmarkIndex) => (
                          <div
                            key={benchmarkIndex}
                            className="bg-elec-gray/50 border border-blue-500/20 rounded-lg p-3"
                          >
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <Badge
                                variant="outline"
                                className="text-green-300 border-green-400/30"
                              >
                                {benchmark.target}
                              </Badge>
                            </div>
              ...
              <div className="border border-purple-500/20 rounded-lg p-3 space-y-3">
                ...
                <div className="space-y-4">
                  {competitiveAdvantages.map((advantage, advantageIndex) => (
                    <div
                      key={advantageIndex}
                      className="bg-elec-gray/50 border border-purple-500/20 rounded-lg p-4 space-y-3"
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <Badge
                          variant="outline"
                          className="text-purple-300 border-purple-400/30 ml-2"
                        >
              ...
              <div className="border border-green-500/20 rounded-lg p-3 space-y-3">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  {performanceMetrics.map((metricCategory, categoryIndex) => (
                    <div key={categoryIndex} className="space-y-3">
                      <h4
                        className={`font-medium text-elec-yellow flex items-center gap-2 ${isMobile ? 'text-sm' : 'text-base'}`}
                      >
                        {metricCategory.category === 'Pricing Performance' && (
                          <PoundSterling className="h-4 w-4" />
                        )}
                        {metricCategory.category === 'Market Position' && (
                          <Eye className="h-4 w-4" />
                        )}
                        {metricCategory.category === 'Financial Health' && (
                      ...
                      <div className="space-y-2">
                        {metricCategory.kpis.map((kpi, kpiIndex) => (
                          <div
                            key={kpiIndex}
                            className="flex items-start gap-2 p-2 bg-elec-gray/50 rounded-lg"
                          >
