...
export const ServiceDiversificationTab = () => {
  ...
  const diversificationStrategies = [
    ...
    },
    {
      ...
      description:
        'Installing electric vehicle charging points for residential and commercial customers',
      components: [
    ...
    <div className="space-y-4">
      <Alert className="border-elec-yellow/50 bg-elec-yellow/10">
        ...
        <AlertDescription className="text-elec-yellow">
          Strategic service diversification can increase annual revenue by £50-200k while reducing
          dependency on single service areas.
        </AlertDescription>
      </Alert>
      ...
      <MobileAccordion type="single" collapsible className="space-y-2">
        {diversificationStrategies.map((strategy) => (
          <MobileAccordionItem key={strategy.id} value={strategy.id}>
            <MobileAccordionTrigger
              icon={
                strategy.id === 'smart-home' ? (
                  <Home className="h-5 w-5 text-blue-400" />
                ) : strategy.id === 'ev-charging' ? (
                  <Car className="h-5 w-5 text-green-400" />
                ) : strategy.id === 'solar-renewable' ? (
                  <Sun className="h-5 w-5 text-yellow-400" />
                ) : strategy.id === 'commercial-services' ? (
                  <Building className="h-5 w-5 text-purple-400" />
                ) : strategy.id === 'testing-inspection' ? (
