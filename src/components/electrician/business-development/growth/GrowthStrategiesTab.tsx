...
export const GrowthStrategiesTab = () => {
  ...
  const growthStrategies = [
    ...
    },
    {
      ...
      timeline: '1-6 months',
      description:
        'Comprehensive risk assessment and mitigation framework to protect and strengthen your business',
      components: [
    ...
    <div className="space-y-4">
      <Alert className="border-elec-yellow/50 bg-elec-yellow/10">
        ...
        <AlertDescription className="text-elec-yellow">
          Strategic growth planning can increase business value by 200-400% over 2-3 years with
          proper execution.
        </AlertDescription>
      </Alert>
      ...
      <MobileAccordion type="single" collapsible className="space-y-2">
        {growthStrategies.map((strategy) => (
          <MobileAccordionItem key={strategy.id} value={strategy.id}>
            <MobileAccordionTrigger
              icon={
                strategy.id === 'market-expansion' ? (
                  <MapPin className="h-5 w-5 text-blue-400" />
                ) : strategy.id === 'team-scaling' ? (
                  <Users className="h-5 w-5 text-green-400" />
                ) : strategy.id === 'digital-transformation' ? (
                  <Smartphone className="h-5 w-5 text-purple-400" />
                ) : strategy.id === 'risk-management' ? (
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              <div className="border border-blue-500/20 rounded-lg p-3 space-y-3">
                ...
                >
                  <div className="space-y-3">
                    <h4
                      className={`font-semibold text-green-300 ${isMobile ? 'text-sm' : 'text-base'}`}
                    >
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-200`}>
                          3-6 months operating capital reserve
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <FileText className="h-4 w-4 text-green-400" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-200`}>
                          Updated insurance and compliance documentation
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <UserCheck className="h-4 w-4 text-green-400" />
                        <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-200`}>
                          Stable team performance and capacity
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div key={categoryIndex} className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {category.benchmarks.map((benchmark, benchmarkIndex) => (
                          <div
                            key={benchmarkIndex}
                            className="border border-blue-500/20 rounded-lg p-3"
                          >
                ...
                >
                  {performanceMetrics.map((metricCategory, categoryIndex) => (
                    <div key={categoryIndex} className="space-y-4">
                      ...
                      >
                        {metricCategory.category === 'Revenue Metrics' && (
                          <PoundSterling className="h-4 w-4" />
                        )}
                        {metricCategory.category === 'Operational Metrics' && (
                          <Settings className="h-4 w-4" />
                        )}
                        {metricCategory.category === 'Customer Metrics' && (
