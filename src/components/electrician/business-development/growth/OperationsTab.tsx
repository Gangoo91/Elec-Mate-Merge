  ...
  const operationalSystems = [
    {
      ...
      description:
        'Comprehensive digital transformation of core business processes for maximum efficiency',
      components: [
    ...
    },
    {
      ...
      description:
        'Equip field teams with mobile technology for real-time job management and customer service',
      components: [
    ...
    },
    {
      ...
      description:
        'Optimise stock management and supplier relationships for reduced costs and improved service',
      components: [
    ...
    },
    {
      ...
      description:
        'Systematic approach to quality control, safety compliance, and regulatory adherence',
      components: [
      ...
      benchmarks: [
        {
          ...
          current: 'Through optimised processes',
        },
        {
          metric: 'First-Time Fix Rate',
          target: '85-95%',
          current: 'With proper van stock management',
        },
        {
          metric: 'Customer Response Time',
          target: 'Within 2 hours',
          current: 'For non-emergency enquiries',
        },
        {
          metric: 'Quote Turnaround',
          target: 'Same day delivery',
          current: 'For standard electrical work',
        },
      ],
    },
    {
      ...
      benchmarks: [
        {
          metric: 'Travel Time Optimisation',
          target: '20-30% reduction',
          current: 'Through route planning',
        },
        {
          metric: 'Administrative Time',
          target: '50% reduction',
          current: 'Via digital automation',
        },
        {
          metric: 'Stock Accuracy',
          target: '95%+ accuracy',
          current: 'With digital tracking systems',
        },
        {
          metric: 'Documentation Speed',
          target: '70% faster',
          current: 'Using digital certificates',
        },
      ],
    },
  ];
  ...
  const scalingAdvantages = [
    {
      ...
      description:
        'Leverage digital systems to scale operations without proportional cost increases',
      advantages: [
    ...
    <div className="space-y-4">
      <Alert className="border-elec-yellow/50 bg-elec-yellow/10">
        ...
        <AlertDescription className="text-elec-yellow">
          Systematic operational improvements can increase efficiency by 25-40% while reducing costs
      ...
      <MobileAccordion type="single" collapsible className="space-y-2">
        {operationalSystems.map((system) => (
          <MobileAccordionItem key={system.id} value={system.id}>
            <MobileAccordionTrigger
              icon={
                system.id === 'digital-workflow-system' ? (
                  <Cog className="h-5 w-5 text-blue-400" />
                ) : system.id === 'mobile-field-operations' ? (
                  <Smartphone className="h-5 w-5 text-green-400" />
                ) : system.id === 'inventory-supply-chain' ? (
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              ...
              <div className="space-y-4">
                {operationalBenchmarks.map((category, index) => (
                  <div key={index}>
                    <h5
                      className={`font-medium text-orange-300 mb-3 ${isMobile ? 'text-xs' : 'text-sm'}`}
                    >
                      {category.category}
                    </h5>
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              ...
              <div className="space-y-4">
                {performanceMetrics.map((category, index) => (
                  <div key={index}>
                    <h5
                      className={`font-medium text-purple-300 mb-3 ${isMobile ? 'text-xs' : 'text-sm'}`}
                    >
                      {category.category}
                    </h5>
