...
const ManagementTab = () => {
  ...
  const managementFramework = [
    ...
    },
    {
      ...
      description:
        'Comprehensive safety leadership ensuring regulatory compliance and team welfare',
      components: [
    ...
    <div className="space-y-4">
      <Alert className="border-blue-500/50 bg-blue-500/10">
        ...
        <AlertDescription className="text-blue-200">
          Effective management increases team productivity by 25% and reduces safety incidents by
          60%.
        </AlertDescription>
      </Alert>
            ...
            <div className="bg-elec-gray border border-elec-yellow/20 rounded-b-lg p-4 space-y-4">
              {disciplinaryProcess.map((stage, index) => (
                <div key={index} className="border border-red-500/20 rounded-lg p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <h5
                      className={`font-medium text-red-200 ${isMobile ? 'text-sm' : 'text-base'}`}
                    >
                      {stage.stage}
                    </h5>
                    <Badge
                  ...
                  <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-red-100`}>
                    {stage.description}
                  </p>
                  <div className="flex flex-wrap gap-1">
                    {stage.actions.map((action, actionIndex) => (
                      <Badge
                        key={actionIndex}
                        variant="outline"
                        className={`text-red-200 border-red-400/20 ${isMobile ? 'text-xs' : 'text-sm'}`}
                      >
                        {action}
                      </Badge>
                    ))}
                  ...
                  <div key={index} className="border border-green-500/20 rounded-lg p-3 space-y-2">
                    ...
                    <ul className="space-y-1">
                      {category.metrics.map((metric, metricIndex) => (
                        <li
                          key={metricIndex}
                          className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-100 flex items-center gap-1`}
                        >
