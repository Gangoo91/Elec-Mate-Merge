  ...
  const retentionFramework = [
    {
      ...
      description:
        'Implement systematic approach to identify and address retention risks before they become problems',
      components: [
    ...
    },
    {
      ...
      description:
        'Create clear pathways for professional and financial advancement within the company',
      components: [
    ...
    },
    {
      ...
      description:
        'Position company as employer of choice through superior compensation and benefits',
      components: [
      ...
      kpis: [
        ...
        'Recruitment application quality',
    ...
    <div className="space-y-4">
      <Alert className="border-blue-500/50 bg-blue-500/10">
        ...
        <AlertDescription className="text-blue-200">
          Retaining skilled electricians is 5x more cost-effective than recruiting and drives 40%
          higher productivity.
        </AlertDescription>
      </Alert>
                  ...
                  <div className="text-left">
                    ...
                    <ul className="space-y-1">
                      {framework.components.map((component, compIndex) => (
                        <li
                          key={compIndex}
                          className={`${isMobile ? 'text-xs' : 'text-sm'} text-blue-200 flex items-start gap-2 text-left`}
                        >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-4">
                  {warningSignsData.map((category, index) => (
                    ...
                    >
                      <h5
                        className={`font-medium text-orange-200 ${isMobile ? 'text-sm' : 'text-base'}`}
                      >
                        {category.category}
                      </h5>
                      <ul className="space-y-1">
                ...
                <div className="border border-purple-500/20 rounded-lg p-3 space-y-2">
                  <h5
                    className={`font-medium text-purple-200 ${isMobile ? 'text-sm' : 'text-base'}`}
                  >
                  ...
                  <ul className="space-y-1">
                    {exitInterviewTopics.map((topic, index) => (
                      <li
                        key={index}
                        className={`${isMobile ? 'text-xs' : 'text-sm'} text-purple-100 flex items-center gap-1`}
                      >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {retentionMetrics.map((metric, index) => (
                  <div key={index} className="border border-green-500/20 rounded-lg p-3 space-y-2">
                    <h5
                      className={`font-medium text-green-200 ${isMobile ? 'text-sm' : 'text-base'}`}
                    >
                      {metric.metric}
                    </h5>
                    <div className="flex justify-between items-center">
                      <div>
                        <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-100`}>
                          Target: <span className="font-medium">{metric.target}</span>
                        </div>
                        <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-100`}>
                          Industry: <span className="font-medium">{metric.industry}</span>
                        </div>
                      </div>
                    </div>
                    <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-green-100`}>
                      {metric.description}
                    </p>
                  </div>
                ))}
