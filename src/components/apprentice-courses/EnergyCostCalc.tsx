...
export const EnergyCostCalc: React.FC = () => {
  ...
  const dailyKWh = useMemo(
    () => powerKW * hoursPerDay * duty * quantity,
    [powerKW, hoursPerDay, duty, quantity]
  );
  ...
  return (
    <section
      aria-labelledby="energy-calc-heading"
      className="rounded-xl bg-gradient-to-br from-card/80 to-card shadow-lg border border-border/40 p-6 space-y-6"
    >
      <div className="flex items-center gap-3">
        ...
        <h4 id="energy-calc-heading" className="text-lg font-semibold text-foreground">
      ...
      <div className="rounded-lg bg-card border border-border p-4 space-y-4" aria-live="polite">
          ...
          <div className="rounded-md bg-secondary/10 border border-secondary/20 p-3 shadow-sm">
            ...
            <div className="grid gap-2 text-sm text-foreground">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono text-primary font-semibold">
                  £{(dailyCost * 30).toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>Annual estimate:</span>
                <span className="font-mono text-primary font-semibold">
                  £{(dailyCost * 365).toFixed(2)}
                </span>
              </div>
            </div>
          </div>
        ...
        <p className="text-xs text-muted-foreground bg-muted/50 rounded-md p-2 border">
          <strong>Disclaimer:</strong> These are estimates only. Actual bills depend on supplier
          rates, standing charges, time-of-use tariffs, and seasonal variations.
        </p>
      </div>
    </section>
  );
