  ...
  return (
    <Card className="mb-8 p-6 bg-card border-border/20">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        Choose a material, enter length and cable size. Optional: add operating temperature and
        current to see voltage drop and heat in the conductor. Use for learning support only –
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3 items-end">
            ...
            <div className="space-y-2">
              ...
              <Button
                type="button"
                variant={useTemp ? 'default' : 'outline'}
          ...
          <div className="rounded-lg p-4 bg-sky-500/10 border border-sky-400/30">
            ...
            {results ? (
              <div className="text-sm text-foreground space-y-1">
                <p>
                  <strong>R at 20°C:</strong> {round(results.R20, dp)} Ω
                </p>
                <p>
                  <strong>R at operating T:</strong> {round(results.RT, dp)} Ω
                </p>
                {results.Vdrop !== undefined && (
                  <p>
                    <strong>Voltage drop (I × R):</strong> {round(results.Vdrop, dp)} V
                  </p>
                )}
                {results.Ploss !== undefined && (
                  <p>
                    <strong>Conductor heating (I²R):</strong> {round(results.Ploss, dp)} W
                  </p>
                )}
                <p className="text-xs text-foreground/80 mt-2">
                  Uses standard constants for copper/aluminium (20°C) with optional temperature
                  correction. Always check Appendix 4 and manufacturer data.
                </p>
              </div>
            ) : (
