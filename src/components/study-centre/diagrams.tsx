 ...
 *
 * Conventions:
 *   - Container: bg-[hsl(0_0%_16%)] border border-white/[0.06] rounded-2xl
...
interface DiagramFrameProps {
  ...
  maxWidth?: string;
  /** Readable floor before the frame scrolls. Set to `min-w-0` for a
   *  diagram already drawn narrow enough to fit a phone (see moet.tsx). */
  minWidth?: string;
}

/**
 * Exported so sibling diagram families (MOET industrial maintenance, and
 * whatever follows) reuse one frame rather than each redrawing the container.
 */
export function DiagramFrame({
  ...
  // Read the child SVG's viewBox once it is in the DOM and use its width as the
  // minimum, so a drawing never renders below 1:1 on a narrow screen.
  useLayoutEffect(() => {
    const svg = inner.current?.querySelector('svg');
    const vb = svg?.getAttribute('viewBox');
    if (!vb) return;
    const w = parseFloat(vb.trim().split(/[\s,]+/)[2]);
    if (Number.isFinite(w) && w > 0) setNatural(Math.ceil(w));
  }, [children]);
  ...
  return (
    <div
      className={cn(
        ...
        // `min-width: auto`, so without it the scroll container's min-width
        // child pushes the whole frame — and the page — wider than the phone.
        'relative w-full min-w-0 rounded-2xl bg-[hsl(0_0%_16%)] border border-white/[0.06] p-4 sm:p-5',
        className
      )}
    >
      ...
      {/* Scroll, don't shrink.
          An SVG scales uniformly to its container, so on a 390px phone a
          440-unit viewBox rendered its 11px labels at 8.2px — measured, and
          unreadable. 18 of the diagrams in this file were below 9.5px. Rather
          than relayout 48 diagrams, the frame now keeps them at a legible
          minimum and scrolls horizontally on narrow screens, which is the
          normal treatment for a schematic. `minWidth` is the readable floor;
          above it the diagram still scales up to `maxWidth` as before. */}
      <div className="-mx-1 min-w-0 overflow-x-auto px-1">
        <div
          ref={inner}
          className={cn('mx-auto w-full', minWidth, maxWidth)}
          // The readable floor is the diagram's OWN viewBox width, measured here,
          // not a fixed 380. A 460-unit viewBox squeezed into 380px renders at
          // 0.83 scale, which turned 10-unit labels into 8.3px — measured on 17
          // of 38 diagrams before this. Holding each drawing at 1:1 and scrolling
          // is what "scroll, don't shrink" has to mean. Tailwind cannot emit a
          // class built at runtime, so this floor is applied inline.
          style={natural ? { minWidth: `${natural}px` } : undefined}
        >
          {children}
        </div>
      </div>
      {caption && <div className="mt-3 text-[12px] leading-relaxed text-white">{caption}</div>}
    </div>
  );
}
...
function ResistorRect({
  ...
  return (
    <g>
      ...
      {value && (
        <text x={cx} y={y + height + 14} textAnchor="middle" fill={COLOUR.textDim} fontSize={11}>
          {value}
        </text>
      )}
    </g>
  ...
  if (vertical) {
    return (
      <g>
        ...
        />
        {label && (
          <text x={cx + 22} y={cy + 4} fill={COLOUR.text} fontSize={12} fontWeight={600}>
            {label}
          </text>
        )}
      </g>
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        {resistors.map((r, i) => {
          ...
          return <ResistorRect key={i} cx={cx} cy={topY} label={r.label} value={r.value} />;
        })}
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        {resistors.map((r, i) => {
          ...
          return (
            <g key={i}>
              <line x1={railX1} y1={y} x2={cx - 28} y2={y} stroke={COLOUR.wire} strokeWidth={1.5} />
              <line x1={cx + 28} y1={y} x2={railX2} y2={y} stroke={COLOUR.wire} strokeWidth={1.5} />
...
export function OhmsLawTriangle({
  ...
  const layout: Record<
    typeof variant,
    { top: string; left: string; right: string; formula: string }
  > = {
  ...
  return (
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        <line x1={50} y1={120} x2={W - 50} y2={120} stroke={COLOUR.wire} strokeWidth={1.5} />
        {/* Divider between bottom two letters */}
        <line x1={W / 2} y1={120} x2={W / 2} y2={H - 35} stroke={COLOUR.wire} strokeWidth={1.5} />
        ...
        <rect x={W / 2 - 9} y={H - 70} width={18} height={20} fill="hsl(0 0% 12%)" />
        <text
          x={W / 2}
          y={H - 55}
          textAnchor="middle"
...
export function PowerTriangle({ variant = 'dc', eyebrow, caption, className }: PowerTriangleProps) {
  ...
  return (
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        <text x={padL + 6} y={padT + 12} fill={COLOUR.textDim} fontSize={11}>
          {label}
        </text>
      </svg>
    </DiagramFrame>
...
export function ThreePhaseWave({
  ...
  // NEGATIVE offsets: each phase LAGS the one before it, which is the standard
  // positive (L1-L2-L3) sequence. Positive offsets draw L2 leading L1, i.e. a
  // reverse sequence — visually still 120° apart, but it implies the opposite
  // direction of motor rotation to the one the labels claim.
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        (direction === 'up'
          ? 'Current flowing out of the page (towards you).'
          : 'Current flowing into the page (away from you).')
      }
      className={className}
      maxWidth="max-w-[340px]"
    >
  ...
  return (
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        <line x1={40} y1={50} x2={coreX1 - 10} y2={50} stroke={COLOUR.wire} strokeWidth={1.5} />
        <line
          x1={coreX1 - 10}
          y1={50}
          x2={coreX1 - 10}
          ...
          strokeWidth={1.5}
        />
        <line x1={40} y1={170} x2={coreX1 - 10} y2={170} stroke={COLOUR.wire} strokeWidth={1.5} />
        <line
          x1={coreX1 - 10}
          y1={170}
          x2={coreX1 - 10}
        ...
        <line
          ...
          strokeWidth={1.5}
        />
        <line x1={coreX2 + 10} y1={50} x2={W - 40} y2={50} stroke={COLOUR.wire} strokeWidth={1.5} />
        <line
          x1={coreX2 + 10}
          y1={coreY2 - 14}
          x2={coreX2 + 10}
...
function SymbolFrame({
  ...
  children,
  className,
  // Some symbols are drawn on a wider grid than the default 80×60. They used to
  // nest their own <svg> inside this one, which scaled the drawing a second time
  // by 80/120 and rendered their labels at 4.7px on a phone — measured.
  viewBox = '0 0 80 60',
}: SymbolProps & { children: ReactNode; viewBox?: string }) {
  return (
    <div
      className={cn(
        'inline-flex flex-col items-center justify-center rounded-xl bg-[hsl(0_0%_16%)] border border-white/[0.06] p-3',
        className
      )}
      style={{ width: size + 24 }}
    >
      ...
      {label && (
        <div className="mt-2 text-[11px] text-white/85 font-medium tracking-tight">{label}</div>
      )}
    </div>
  );
}
...
export function LeverDiagram({ leverClass = 1, eyebrow, caption, className }: LeverDiagramProps) {
  const W = 440;
  const H = 220;
  const beamY = 130;
  const beamLeft = 40;
...
export function PulleySystem({ pulleys = 1, eyebrow, caption, className }: PulleySystemProps) {
  const W = 360;
  const H = 280;
  ...
  const Wheel = ({ cx, cy, r = 18 }: { cx: number; cy: number; r?: number }) => (
    <g>
      <circle cx={cx} cy={cy} r={r} fill="hsl(0 0% 14%)" stroke={COLOUR.yellow} strokeWidth={2} />
        ...
        {pulleys === 1 && (
          <g>
            ...
            <ArrowMarker
              ...
              width={2.5}
            />
            <text x={W / 2 - 60} y={H - 50} fill={COLOUR.yellow} fontSize={12} fontWeight={700}>
              Effort
            </text>
            ...
            <text x={W / 2 + 70} y={H - 50} fill={COLOUR.orange} fontSize={12} fontWeight={700}>
              Load
            </text>
          </g>
        )}
        ...
        {pulleys === 2 && (
          <g>
            ...
            <ArrowMarker
              ...
              width={2.5}
            />
            <text x={W / 2 + 70} y={H - 50} fill={COLOUR.yellow} fontSize={12} fontWeight={700}>
              Effort
            </text>
        ...
        {pulleys === 3 && (
          <g>
            ...
            <ArrowMarker
              ...
              width={2.5}
            />
            <text x={W / 2 + 78} y={H - 50} fill={COLOUR.yellow} fontSize={12} fontWeight={700}>
              Effort
            </text>
...
export function ForceVectorDiagram({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Resultant force = vector sum of all forces. If forces balance → ΣF = 0 (equilibrium).'
      }
      className={className}
      maxWidth="max-w-[400px]"
    >
...
export function EnergyTransfer({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'At the top: maximum PE, zero KE. At the bottom: zero PE, maximum KE. Total energy stays the same (ignoring friction).'
      }
      className={className}
      maxWidth="max-w-[460px]"
    >
...
export function BarMagnet({
  ...
  return (
    <DiagramFrame
      eyebrow={eyebrow}
      caption={
        caption ?? 'Field lines run N → S externally and S → N inside the magnet. They never cross.'
      }
      className={className}
      maxWidth="max-w-[460px]"
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        {withCompass && (
          <g>
            ...
            />
            <polygon
              points={`${cx + 70},${cy + 100} ${cx + 64},${cy + 96} ${cx + 64},${cy + 104}`}
              fill={COLOUR.red}
            />
...
export function SolenoidField({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Current through a coil produces a field like a bar magnet — N at one end, S at the other. Use the right-hand grip rule: fingers follow current, thumb points to N.'
      }
      className={className}
      maxWidth="max-w-[480px]"
    >
...
export function RightHandGripRule({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Thumb in the direction of current → curled fingers show the direction of the magnetic field around the conductor.'
      }
      className={className}
      maxWidth="max-w-[380px]"
    >
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
      ...
      <text x={W / 2} y={H - 12} textAnchor="middle" fill={COLOUR.textDim} fontSize={11}>
        {hand === 'left'
          ? 'LEFT hand — motor (force from current in field)'
          : 'RIGHT hand — generator (induced current from motion in field)'}
      </text>
    </svg>
  );
}
...
export function FlemingsLeftHandRule({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Use the LEFT hand for motors. Hold thumb, first and second fingers at right angles. First → field, seCond → current, thuMb → motion.'
      }
      className={className}
      maxWidth="max-w-[380px]"
    >
...
export function FlemingsRightHandRule({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Use the RIGHT hand for generators. Thumb → motion of conductor, first finger → field, second finger → induced current.'
      }
      className={className}
      maxWidth="max-w-[380px]"
    >
...
export function MotorEffect({ eyebrow = 'Motor effect', caption, className }: MotorEffectProps) {
  const W = 420;
  const H = 240;
  const cx = W / 2;
  const cy = H / 2;
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Current-carrying conductor in a magnetic field experiences a force F = BIL (perpendicular to both field and current).'
      }
      className={className}
      maxWidth="max-w-[440px]"
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        {[-30, -10, 10, 30].map((dy) => (
          <g key={dy}>
            ...
            />
            <polygon
              points={`${W - 100},${cy + dy - 3} ${W - 92},${cy + dy} ${W - 100},${cy + dy + 3}`}
              fill={COLOUR.blue}
              opacity={0.45}
            />
          </g>
...
export function DCMotorSchematic({
  ...
  const W = 420;
  const H = 298; // room for the "DC supply" label under the battery
  const cx = W / 2;
  const cy = 130;
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Commutator reverses current in the rotor coil every half-turn so the force always pushes the same way around → continuous rotation.'
      }
      className={className}
      maxWidth="max-w-[440px]"
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        />
        <polygon
          points={`${cx + 30},${cy - 34} ${cx + 38},${cy - 28} ${cx + 26},${cy - 22}`}
          fill={COLOUR.emerald}
        />
        ...
        <path
          d={`M ${cx - 20} ${cy + 80} A 20 20 0 0 1 ${cx + 20} ${cy + 80}`}
          fill="none"
          stroke={COLOUR.yellow}
          strokeWidth={2.5}
        />
        <line x1={cx} y1={cy + 70} x2={cx} y2={cy + 80} stroke={COLOUR.wire} strokeWidth={1.5} />
        {/* Clear of the commutator arc (which ends at cx+20) and above the
              brush label rather than across it. */}
        <text x={cx + 28} y={cy + 70} fill={COLOUR.textDim} fontSize={10.5}>
          commutator
        </text>
...
export function ACGenerator({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Slip rings keep the rotating coil connected to a fixed circuit. Output reverses every half-turn → AC sine wave.'
      }
      className={className}
      maxWidth="max-w-[480px]"
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        />
        <polygon
          points={`${motorCx + 30},${motorCy - 34} ${motorCx + 38},${motorCy - 28} ${motorCx + 26},${motorCy - 22}`}
          fill={COLOUR.emerald}
        />
...
export function LenzLaw({ eyebrow = 'Lenz’s law', caption, className }: LenzLawProps) {
  const W = 460;
  const H = 240;
  const cy = H / 2;
  const coilLeft = 220;
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'A moving magnet induces a current in a coil. The induced current creates a magnetic field that OPPOSES the change — pushes back against the magnet.'
      }
      className={className}
      maxWidth="max-w-[480px]"
    >
...
export function MagneticHysteresis({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Flux density (B) lags the magnetising force (H). The area inside the loop = energy lost to heat each cycle (iron losses).'
      }
      className={className}
      maxWidth="max-w-[380px]"
    >
...
export function ZenerDiodeSymbol({ label = 'Zener diode', size, className }: SymbolProps) {
  return (
    <SymbolFrame label={label} size={size} className={className}>
      ...
      <polyline
        points="46,16 52,18 52,42 58,44"
        fill="none"
        stroke={COLOUR.yellow}
        strokeWidth={2.5}
      />
...
export function BridgeRectifier({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Four-diode bridge converts AC to pulsating DC. Both halves of the AC cycle drive current through the load in the same direction.'
      }
      className={className}
      maxWidth="max-w-[380px]"
    >
...
export function RelaySymbol({ label = 'Relay (SPDT)', size, className }: SymbolProps) {
  return (
    <SymbolFrame label={label} size={(size ?? 80) * 1.5} viewBox="0 0 120 60" className={className}>
      {/* Coil */}
      <rect
        x={6}
        y={20}
      ...
      {/* Labels */}
      <text x={114} y={16} fill={COLOUR.textDim} fontSize={10}>
        NC
      </text>
      <text x={114} y={52} fill={COLOUR.textDim} fontSize={10}>
        NO
      </text>
    </SymbolFrame>
  );
  ...
  return (
    <SymbolFrame label={label} size={(size ?? 80) * 1.5} viewBox="0 0 120 60" className={className}>
      ...
      <rect
        ...
        rx={2}
      />
      <text x={111} y={14} textAnchor="middle" fill={COLOUR.textDim} fontSize={10}>
        A1
      </text>
      <text x={111} y={50} textAnchor="middle" fill={COLOUR.textDim} fontSize={10}>
        A2
      </text>
    </SymbolFrame>
  );
      ...
      >
        ...
        {type === '2-way' && (
          <g>
            ...
            <line x1={72} y1={18} x2={116} y2={18} stroke={COLOUR.wire} strokeWidth={1.5} />
            <line x1={72} y1={42} x2={116} y2={42} stroke={COLOUR.wire} strokeWidth={1.5} />
            <text x={108} y={14} fill={COLOUR.textDim} fontSize={9}>
              L1
            </text>
            <text x={108} y={56} fill={COLOUR.textDim} fontSize={9}>
              L2
            </text>
          </g>
        )}
  ...
  return (
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        <text
          x={padL + tauPx + 7}
          y={padT + plotH * (mode === 'charge' ? 0.37 - 0.11 : 0.47)}
          fill={COLOUR.emerald}
          fontSize={11}
        >
          {mode === 'charge' ? '63%' : '37%'}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        <g transform={`translate(${padL + 8}, ${H - 16})`}>
          <line
            ...
            strokeDasharray="3 3"
          />
          <text x={26} y={4} fill={COLOUR.text} fontSize={11}>
            AC input
          </text>
          <line x1={110} y1={0} x2={130} y2={0} stroke={COLOUR.yellow} strokeWidth={2.5} />
          <text x={136} y={4} fill={COLOUR.text} fontSize={11}>
            {mode === 'half' ? 'Half-wave DC' : 'Full-wave DC'}
          </text>
        </g>
      </svg>
    </DiagramFrame>
...
export function EarthingSystemDiagram({
  ...
  const blurbs = {
    'TN-S':
      'Separate neutral and protective earth conductor all the way from the source. Earth provided as a metallic sheath of the supply cable.',
    'TN-C-S':
      'Combined N + PE in the supply (PEN), separated at the service position into N and PE. Most common UK domestic system (PME).',
...
export function ConsumerUnit({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Main switch isolates everything. RCDs detect earth-fault current. MCBs trip on overload / short circuit. RCBOs combine both in a single module.'
      }
      className={className}
      maxWidth="max-w-[500px]"
    >
...
export function CableCrossSection({
  ...
  const blurb = {
    'twin-and-earth':
      'Flat profile. Two insulated cores (L + N) plus a bare CPC, all inside a grey PVC sheath. Sleeve the CPC green/yellow at terminations.',
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        {type === 'twin-and-earth' && (
          <g>
            ...
            <text x={cx} y={cy + 38} textAnchor="middle" fill={COLOUR.textDim} fontSize={10}>
              CPC (bare)
            </text>
            <text x={cx - 90} y={cy - 36} fill={COLOUR.textDim} fontSize={10}>
              grey PVC sheath
            </text>
          </g>
        )}
        ...
        {type === 'SWA' && (
          <g>
            ...
            <circle
              ...
              strokeWidth={2}
            />
            <text x={cx + 84} y={cy - 60} fill={COLOUR.textDim} fontSize={10}>
              black PVC outer
            </text>
            ...
            <text x={cx + 80} y={cy - 30} fill={COLOUR.textDim} fontSize={10}>
              SWA armour (CPC)
            </text>
            {/* Inner sheath */}
            <circle
        ...
        {type === 'flex' && (
          <g>
              ...
              <g key={i}>
                ...
                {Array.from({ length: 7 }, (_, j) => {
                  ...
                  return (
                    <circle
                      key={j}
                      cx={cx + core.dx + Math.cos(angle) * 8}
                      cy={cy + core.dy + Math.sin(angle) * 8}
                      r={2.5}
                      fill={core.c}
                      opacity={0.55}
                    />
            ...
            <text x={cx} y={cy + 90} textAnchor="middle" fill={COLOUR.textDim} fontSize={10}>
        ...
        {type === 'singles' && (
          <g>
            ...
            <text x={cx + 70} y={cy - 30} fill={COLOUR.textDim} fontSize={10}>
              PVC insulation
            </text>
            <text x={cx + 70} y={cy + 10} fill={COLOUR.textDim} fontSize={10}>
              copper conductor
            </text>
            <text x={cx} y={cy + 80} textAnchor="middle" fill={COLOUR.textDim} fontSize={10}>
...
export function EquipotentialBonding({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Main protective bonding connects extraneous-conductive-parts (gas, water, structural metalwork) to the Main Earthing Terminal (MET) — keeps them at the same potential during a fault.'
      }
      className={className}
      maxWidth="max-w-[500px]"
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        <rect
          ...
          strokeWidth={1.5}
        />
        <text x={80} y={80} textAnchor="middle" fill={COLOUR.text} fontSize={11} fontWeight={600}>
          Gas
        </text>
        {/* Water */}
        <rect
          ...
          strokeWidth={1.5}
        />
        <text x={240} y={80} textAnchor="middle" fill={COLOUR.text} fontSize={11} fontWeight={600}>
          Water
        </text>
        {/* Structural steel */}
        <rect
          ...
          strokeWidth={1.5}
        />
        <text x={400} y={78} textAnchor="middle" fill={COLOUR.text} fontSize={10} fontWeight={600}>
          Steel
        </text>
        <text x={400} y={92} textAnchor="middle" fill={COLOUR.textDim} fontSize={10.5}>
          (structure)
        </text>
        ...
        <text x={113} y={158} fill={COLOUR.emerald} fontSize={10}>
          10 mm² bond
        </text>
        <rect x={246} y={148} width={48} height={14} fill="hsl(0 0% 10%)" />
        <text x={250} y={158} fill={COLOUR.emerald} fontSize={10}>
          10 mm²
        </text>
        <rect x={300} y={148} width={70} height={14} fill="hsl(0 0% 10%)" />
        <text x={305} y={158} fill={COLOUR.emerald} fontSize={10}>
          10 mm² bond
        </text>
      </svg>
    </DiagramFrame>
...
export function InsulationResistanceTest({
  ...
  return (
    <DiagramFrame
      ...
      caption={
        caption ??
        'Apply 500 V DC between live conductors (L+N linked) and the protective conductor. Acceptable result ≥ 1 MΩ (BS 7671 Table 64). Initial verification: ≥ 100 MΩ is healthy. Always isolate, lock off and prove dead before testing, and disconnect electronics that will not survive 500 V.'
      }
      className={className}
      maxWidth="max-w-[480px]"
    >
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        ...
        <text x={meterCx} y={meterCy - 6} textAnchor="middle" fill={COLOUR.textDim} fontSize={10}>
          500 V DC
        </text>
        {/* Test button */}
        <circle
        ...
        <path
          d={`M ${meterCx - 20} ${meterCy + 60} Q ${meterCx + 30} ${H - 30} ${cuX - 30} ${meterCy - 10}`}
          fill="none"
          stroke={COLOUR.red}
          strokeWidth={2}
        />
        {/* Green lead → CPC bar */}
        <path
          d={`M ${meterCx + 20} ${meterCy + 60} Q ${meterCx + 80} ${H - 20} ${cuX - 30} ${meterCy + 30}`}
          fill="none"
          stroke={COLOUR.emerald}
          strokeWidth={2}
        />
        ...
        <line
          ...
          strokeWidth={2}
        />
        <text x={cuX + 96} y={meterCy - 36} fill={COLOUR.yellow} fontSize={11} fontWeight={700}>
          L
        </text>
        {/* N bar */}
        <line
          ...
          strokeWidth={2}
        />
        <text x={cuX + 96} y={meterCy - 12} fill={COLOUR.blue} fontSize={11} fontWeight={700}>
          N
        </text>
        {/* L+N link wire */}
        <line
        ...
        <line
          ...
          strokeWidth={2}
        />
        <text x={cuX + 96} y={meterCy + 28} fill={COLOUR.emerald} fontSize={11} fontWeight={700}>
          CPC
        </text>
      </svg>
    </DiagramFrame>
