...
export function checkFireCompliance(
  ...
  if (cableSpec.firePerformance.rating === 'Standard' && requirements.minRating !== 'Standard') {
    ...
    recommendations.push(
      `Upgrade to ${requirements.minRating} fire performance cable for ${application}`
    );
  }

  if (
    cableSpec.firePerformance.smokeEmission === 'Standard' &&
    requirements.smokeEmission !== 'Standard'
  ) {
    compliant = false;
  ...
  if (cableSpec.firePerformance.circuitIntegrity < requirements.circuitIntegrity) {
    compliant = false;
    recommendations.push(
      `Use FP cable with ${requirements.circuitIntegrity}min circuit integrity for ${application}`
    );
  }
