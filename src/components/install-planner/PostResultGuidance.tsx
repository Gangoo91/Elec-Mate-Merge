  ...
  const getProcurementSummary = () => {
    ...
    return {
      primaryCable: {
        ...
        totalEstimate:
          cable.cost === 'low'
            ? `£${(length * 3).toFixed(0)}-${(length * 4).toFixed(0)}`
            : cable.cost === 'medium'
              ? `£${(length * 6).toFixed(0)}-${(length * 8).toFixed(0)}`
      ...
      totalProjectEstimate: {
        materials:
          cable.cost === 'low' ? '£150-300' : cable.cost === 'medium' ? '£300-600' : '£600-1200',
  ...
  const getTestingChecklist = () => {
    ...
    const liveTests = [
      ...
      },
      {
        ...
        status:
          planData.protectiveDevice?.includes('rcd') || planData.protectiveDevice?.includes('rcbo')
            ? 'required'
            : 'recommended',
        category: 'live',
      },
  ...
  const getRegulatoryReferences = () => [
    {
      ...
      description:
        'The fundamental standard for electrical installation design, selection and verification in the UK',
      keyClauses: [
    ...
    },
    {
      ...
      description:
        'Legal requirements for electrical work in domestic properties in England and Wales',
      keyClauses: [
      ...
      relevantSections:
        planData.installationType === 'domestic'
          ? [
              'Self-certification via NICEIC/NAPIT',
    ...
    },
    {
      ...
      description:
        'Practical guidance on implementing BS 7671 requirements for equipment selection',
      keyClauses: [
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
            ...
            <div className="p-4 bg-elec-dark/30 rounded-lg border border-elec-blue/20">
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-medium text-foreground">
                    {procurement.protectiveDevice.type}
                  </span>
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-medium text-foreground text-sm leading-relaxed">
                    {procurement.protectiveDevice.characteristics}
                  </span>
                </div>
                <div className="flex flex-col gap-2">
                  <span className="text-foreground/80 text-sm">Quantity:</span>
                  <span className="font-medium text-foreground">
                    {procurement.protectiveDevice.quantity}
                  </span>
                </div>
                <Separator className="my-3 bg-elec-blue/20" />
                <div className="flex flex-col gap-2">
                  <span className="text-foreground/80 text-sm">Unit Cost:</span>
                  <span className="font-bold text-elec-blue text-lg">
                    {procurement.protectiveDevice.estimate}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-elec-dark/30 rounded-lg border border-elec-yellow/10">
              ...
              <div className="space-y-3">
                {procurement.accessories.map((item: any, index: number) => (
                  <div
                    key={index}
                    className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3 bg-elec-dark/20 rounded-lg border border-elec-yellow/10"
                  >
                    ...
                    <div className="flex flex-col gap-1 sm:items-center">
                      <span className="text-foreground/60 text-xs uppercase tracking-wide">
                        Quantity
                      </span>
            ...
            <div className="p-4 bg-elec-yellow/10 rounded-lg border border-elec-yellow/30">
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-medium text-foreground">
                    {procurement.totalProjectEstimate.materials}
                  </span>
                </div>
                <div className="flex flex-col gap-2">
                  <span className="text-foreground/80 text-sm">Labour:</span>
                  <span className="font-medium text-foreground">
                    {procurement.totalProjectEstimate.labour}
                  </span>
                </div>
                <div className="flex flex-col gap-2">
                  <span className="text-foreground/80 text-sm">Testing/Cert:</span>
                  <span className="font-medium text-foreground">
                    {procurement.totalProjectEstimate.testing}
                  </span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                  ...
                  <span className="font-bold text-elec-yellow text-right text-lg">
                    ...
                    {(
                      parseInt(
                        procurement.totalProjectEstimate.materials.split('-')[0].replace('£', '')
                      ) +
                      parseInt(
                        procurement.totalProjectEstimate.labour.split('-')[0].replace('£', '')
                      ) +
                      parseInt(
                        procurement.totalProjectEstimate.testing.split('-')[0].replace('£', '')
                      )
                    ).toFixed(0)}{' '}
                    - £
                    {(
          ...
          <Alert className="bg-blue-500/5 border-blue-500/20">
            ...
            <AlertDescription className="text-blue-200">
              <strong>Cost Estimates:</strong> Prices are indicative and based on typical UK market
              rates. Actual costs may vary based on supplier, location, quantity, and specific
              product selection. Always obtain formal quotations before procurement.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="grid gap-3">
              {tests.preEnergisationTests
                .filter((test) => test.status !== 'n/a')
                .map((test, index) => (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <Badge
                          variant={test.status === 'required' ? 'default' : 'secondary'}
                          className="text-xs"
                        >
          ...
          <Alert className="bg-red-500/5 border-red-500/20">
            ...
            <AlertDescription className="text-red-200">
              <strong>Safety Warning:</strong> Live testing must only be performed by qualified
              electricians with appropriate test equipment. Ensure safe isolation procedures and use
              of appropriate PPE.
            </AlertDescription>
          </Alert>
        </CardContent>
        ...
        <CardContent className="space-y-4">
          {regulations.map((reg, index) => (
            <div key={index} className="space-y-4">
              ...
              <div className="flex items-start gap-3 pb-3">
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    ...
                    <Badge
                      variant={
                        reg.compliance === 'Mandatory for all electrical installations'
                          ? 'default'
                          : reg.compliance === 'Notifiable work'
                            ? 'destructive'
                            : 'secondary'
                      }
              ...
              <div className="grid grid-cols-1 gap-4 pl-8">
                ...
                {/* Relevant Sections */}
                <div>
                  <h5 className="font-medium text-sm mb-2 text-elec-blue">
              ...
              {index < regulations.length - 1 && <Separator className="mt-6 bg-elec-yellow/20" />}
            </div>
          ))}
          ...
          <Alert className="bg-elec-primary/5 border-elec-primary/20">
            ...
            <AlertDescription>
              ...
              <a
                href="https://electrical.theiet.org/"
                target="_blank"
              ...
              , or consult your local building control authority for Part P requirements. Consider
              membership of professional bodies (NICEIC, NAPIT, ECA) for ongoing support.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
      ...
      <Alert className="border-elec-yellow bg-elec-yellow/5">
        ...
        <AlertDescription>
          <strong>Professional Verification Required:</strong> This calculation tool provides
          guidance based on BS 7671 but does not replace professional electrical design and
          verification. All installations must be designed, installed, and certified by qualified
          electrical personnel in accordance with current regulations.
        </AlertDescription>
      </Alert>

      {/* Enhanced Installation Guidance */}
      <EnhancedInstallationGuidance planData={planData} recommendedCable={recommendedCable} />
    </div>
  );
};
