...
const LoadDetailsStep = ({ planData, updatePlanData }: LoadDetailsStepProps) => {
  ...
  const getStepValue = (loadType: string) => {
    ...
    if (
      ...
      loadType.includes('heating') ||
      loadType.includes('cooker') ||
      loadType.includes('motor-small')
    ) {
      return '100';
    }
    ...
    if (
      ...
      loadType.includes('welding') ||
      loadType.includes('furnace') ||
      loadType.includes('crane') ||
      loadType.includes('motor-large') ||
      loadType.includes('ups') ||
      loadType.includes('generator')
    ) {
      return '500';
    }
    // Default: 50W steps
  ...
  const getMinValue = (loadType: string) => {
    if (
      ['lighting', 'emergency', 'commercial-lighting', 'smart-lighting'].includes(loadType) ||
      loadType.includes('lighting')
    ) {
      return '10';
    }
    if (
      ['power', 'commercial-power', 'classroom-power'].includes(loadType) ||
      loadType.includes('power')
    ) {
      return '100';
    }
    if (
      ['cooker', 'heating', 'motor', 'kitchen-equipment', 'motor-small', 'heat-pump'].includes(
        loadType
      ) ||
      loadType.includes('heating') ||
      loadType.includes('cooker') ||
    ...
    if (
      ['hvac', 'welding', 'furnace', 'crane', 'motor-large', 'ups-system'].includes(loadType) ||
      loadType.includes('hvac') ||
      loadType.includes('welding') ||
      loadType.includes('furnace') ||
      loadType.includes('crane') ||
      loadType.includes('motor-large') ||
  ...
  const getLoadGuidance = () => {
    ...
    if (template) {
      return {
        ...
        voltageOptions:
          template.voltage === 230 ? ['230'] : template.voltage === 400 ? ['400'] : ['230', '400'],
        phaseOptions:
          template.phases === 'single'
            ? ['single']
            : template.phases === 'three'
              ? ['three']
              : ['single', 'three'],
      };
    }
    ...
    return (
      guidanceMap[planData.loadType as keyof typeof guidanceMap] || {
        typical: 'Varies',
  ...
  const getSpecialConsiderations = () => {
    const specialConsiderations = {
      motor: [
        'Starting current typically 5-8x full load current',
        'DOL, Star-Delta, or VFD starting methods may be required',
      ],
      hvac: ['Seasonal diversity factors apply', 'Consider heat pump COP variations'],
      welding: [
        'High inrush currents and voltage fluctuations',
        'Dedicated supply may be required',
      ],
      'ev-charging': [
        'Smart charging and load balancing considerations',
        'DNO notification may be required for >7kW',
      ],
      medical: [
        'Isolated supply systems (IT systems) may be required',
        'Backup power supply essential',
      ],
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-8">
        ...
        <MobileSelectWrapper
          ...
          value={
            planData.voltage && planData.phases ? `${planData.voltage}-${planData.phases}` : ''
          }
          onValueChange={(value) => {
        ...
        <MobileInputWrapper
          ...
          onChange={(value) =>
            updatePlanData({ powerFactor: parseFloat(value) || guidance.defaultPF })
          }
          type="number"
          step="0.01"
          min="0.1"
        ...
        <CardHeader className="pb-4">
          <CardTitle className="text-blue-300 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm font-normal text-blue-200/80 capitalize">
                {planData.loadType?.replace('-', ' ')}
              </div>
            </div>
          </CardTitle>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="p-4 bg-blue-500/10 rounded-lg">
              <p className="text-sm text-blue-200/90">
                <span className="font-semibold">Typical Load:</span> {guidance.typical}
              </p>
            </div>
            <div className="p-4 bg-blue-500/10 rounded-lg">
              <p className="text-sm text-blue-200/90">
            ...
            <div className="p-4 bg-blue-500/10 rounded-lg">
              <p className="text-sm text-blue-200/90">
                <span className="font-semibold">Power Factor:</span> {guidance.defaultPF}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {planData.totalLoad > 0 && planData.voltage && (
        <Card className="bg-gradient-to-r from-elec-yellow/10 to-amber-500/10 border border-elec-yellow/30 rounded-xl">
          <CardHeader className="pb-4">
            <CardTitle className="text-elec-yellow flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm font-normal text-elec-yellow/80">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-elec-yellow mb-2">{calculateCurrent()}A</div>
            <p className="text-sm text-elec-light/70">
              This is the design current that will be used for cable sizing and protective device
              selection.
            </p>
          </CardContent>
        </Card>
