...
const InstallationTypeStep = ({ planData, updatePlanData }: InstallationTypeStepProps) => {
  ...
  const installationTypes = [
    {
      ...
      label: 'Domestic',
      icon: Home,
      description: 'Houses, flats, domestic properties',
    },
    {
      value: 'commercial',
      label: 'Commercial',
      icon: Building,
      description: 'Offices, shops, public buildings',
    },
    {
      value: 'industrial',
      label: 'Industrial',
      icon: Factory,
      description: 'Factories, warehouses, heavy industry',
    },
  ];
  ...
  const getLoadTypeFromTemplate = (templateKey: string) => {
    ...
    const getIconAndCategory = (key: string) => {
      ...
      if (key.includes('heating') || key.includes('heat-pump'))
        return { icon: Fan, category: 'Heating' };
      if (key.includes('cooker') || key.includes('kitchen'))
        return { icon: Microwave, category: 'Kitchen' };
      if (key.includes('motor') || key.includes('pump') || key.includes('fan'))
        return { icon: Fan, category: 'Motors' };
      if (key.includes('hvac') || key.includes('cooling'))
        return { icon: Snowflake, category: 'HVAC' };
      if (key.includes('welding') || key.includes('furnace') || key.includes('arc'))
        return { icon: Flame, category: 'Industrial' };
      if (key.includes('crane') || key.includes('hoist'))
        return { icon: Warehouse, category: 'Material Handling' };
      if (key.includes('ev') || key.includes('charging'))
        return { icon: Car, category: 'Transport' };
      if (key.includes('emergency') || key.includes('fire'))
        return { icon: Zap, category: 'Safety' };
      if (
        key.includes('it') ||
        key.includes('server') ||
        key.includes('ups') ||
        ...
        return { icon: Cpu, category: 'IT & Data' };
      if (key.includes('medical') || key.includes('hospital'))
        return { icon: Hospital, category: 'Healthcare' };
      if (key.includes('solar') || key.includes('renewable') || key.includes('battery'))
        return { icon: Sun, category: 'Renewable' };
      if (key.includes('grain') || key.includes('irrigation') || key.includes('livestock'))
        return { icon: Wind, category: 'Agricultural' };
      if (key.includes('education') || key.includes('classroom') || key.includes('lab'))
        return { icon: School, category: 'Educational' };
      if (key.includes('retail') || key.includes('pos') || key.includes('hospitality'))
        return { icon: ShoppingCart, category: 'Commercial' };
  ...
  const getAvailableLoadTypes = () => {
    ...
    const loadTypes = availableTemplates.map(getLoadTypeFromTemplate).filter(Boolean); // Remove any null entries
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 gap-8">
        ...
        <MobileSelectWrapper
          ...
          placeholder={
            planData.installationType ? 'Select load type' : 'Select installation environment first'
          }
        ...
        <Card className="bg-amber-500/10 border-amber-500/30">
          ...
          <CardContent>
            <p className="text-amber-200">
              No specific load types are defined for <strong>{planData.installationType}</strong>{' '}
              installations. You may need to select a different installation environment or contact
              support.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-green-500/10 border-green-500/30">
          ...
          <CardContent>
            <p className="text-green-200">
              ...
              <strong>{planData.installationType}</strong> installation. Click Next to continue with
              load details.
            </p>
          </CardContent>
        </Card>
