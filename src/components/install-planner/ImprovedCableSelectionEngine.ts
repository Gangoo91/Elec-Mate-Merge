...
import {
  getCableSpecification,
  checkFireCompliance,
  COMPREHENSIVE_CABLE_DATABASE,
} from './ComprehensiveCableDatabase';
import {
...
import {
  legacyStandardDeviceRatings,
  getNextStandardRating,
} from '@/lib/calculators/utils/calculatorUtils';
...
export class ImprovedCableSelectionEngine {
  static calculateCableOptions(
    ...
    for (const size of availableSizes) {
      ...
      if (currentOK && voltageDropOK && coordinationOK && zsOK && lengthOK) {
        ...
        currentOK &&
        zsOK &&
        voltageDropResult.voltageDropPercent <=
          this.getMaxVoltageDropPercentage(planData.loadType, planData.specialRequirements) * 1.2
      ) {
      ...
      if (cableSpec) {
        ...
        notes.push(
          `🔥 ${tempRating}°C rated ${cableSpec.insulationType}/${sheathType} - ${fireRating} fire performance`
        );
        ...
        if (cableSpec.firePerformance.circuitIntegrity > 0) {
          notes.push(
            `⏱️ ${cableSpec.firePerformance.circuitIntegrity}min circuit integrity at 842°C`
          );
        }
      }
      ...
      if (!currentOK)
        notes.push(
          `❌ ${capacityResult.compliance.InLeIz ? 'Current capacity adequate' : 'Current capacity insufficient'}`
        );
      if (!voltageDropOK)
        notes.push(
          `❌ Voltage drop: ${voltageDropResult.voltageDropPercent.toFixed(2)}% (max: ${this.getMaxVoltageDropPercentage(planData.loadType, planData.specialRequirements)}%)`
        );
      if (!coordinationOK)
        notes.push(
          `❌ Device coordination: Ib (${capacityResult.Ib}A) > In (${capacityResult.In}A)`
        );
      ...
      if (suitability === 'suitable') {
        notes.push(
          `✅ Complies with BS7671 - Safety margin: ${capacityResult.compliance.safetyMargin.toFixed(1)}%`
        );
      ...
      options.push({
        ...
        specialConsiderations: this.getSpecialConsiderations(
          planData,
          size,
          isRingCircuit,
          capacityResult,
  ...
  static generateEnhancedSuggestions(
    ...
    if (
      ...
      suggestions.push({
        ...
        description:
          'A 32A ring final circuit using 2.5mm² T&E may be more economical and provide better load distribution.',
        impact: 'medium',
        cost: 'low',
  ...
  static performEnhancedComplianceChecks(
    ...
    const maxZs = this.getMaxZs(
      planData.protectiveDevice,
      planData.voltage,
      recommendedCable.ratedCurrent
    );
    checks.push({
      regulation: 'BS 7671',
    ...
    const maxVoltageDropPercentage = this.getMaxVoltageDropPercentage(
      planData.loadType,
      planData.specialRequirements
    );
  ...
  private static getGroupingCircuits(planData: InstallPlanData): number {
    ...
    const groupingFactor =
      planData.groupingFactor || planData.environmentalSettings?.globalGroupingFactor || 1;
  ...
  private static parseProtectiveDevice(device: string): {
    type: string;
    curve: string;
    rating?: number;
  } {
  ...
  private static getSpecialConsiderations(
    ...
    if (capacityResult.factors.overall < 0.8) {
      considerations.push(
        `Heavy derating applied (${(capacityResult.factors.overall * 100).toFixed(0)}%) - review installation conditions`
      );
    }
    ...
    if (cableSpec && cableSpec.insulationType === 'XLPE') {
      considerations.push(
        'XLPE insulation provides enhanced thermal performance and overload capacity'
      );
      ...
      if (cableSpec.maxOperatingTemp === 90) {
        considerations.push(
          '90°C continuous rating allows higher current capacity than PVC equivalent'
        );
      }
    }
    ...
    if (cableSpec && cableSpec.firePerformance.rating === 'Superior') {
      considerations.push(
        'Enhanced fire performance cable - maintains circuit integrity during fire'
      );
    }
