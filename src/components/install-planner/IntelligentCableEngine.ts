...
import {
  getSimpleCableSpec,
  getAvailableSizes,
  SIMPLIFIED_CABLE_DATABASE,
} from './SimplifiedCableDatabase';
...
export class IntelligentCableEngine {
  ...
  static selectOptimalInstallationMethod(planData: InstallPlanData, cableType: string): string {
    ...
    return Object.entries(methodScores).reduce(
      (best, [method, score]) => (score > methodScores[best] ? method : best),
  ...
  static calculateIntelligentRecommendations(planData: InstallPlanData): CableRecommendation[] {
    ...
    const optimalInstallationMethod = this.selectOptimalInstallationMethod(
      planData,
      optimalCableType
    );
    ...
    availableSizes.forEach((size) => {
      ...
      const baseCapacity =
        cableSpec.capacity[optimalInstallationMethod] || cableSpec.capacity['clipped-direct'];
      ...
      if (
        deratedCapacity >= designCurrent * 1.2 &&
        voltageDropPercentage <= maxVoltageDropPercentage
      ) {
        suitability = 'suitable';
      } else if (
        deratedCapacity >= designCurrent &&
        voltageDropPercentage <= maxVoltageDropPercentage + 1
      ) {
  ...
  static selectOptimalProtectionDevice(
    designCurrent: number,
    cableCapacity: number
  ): { type: string; rating: number; cost: number } {
    ...
    if (requiredRating <= 63 && designCurrent < 32) {
      ...
      return {
        type: 'mcb',
        rating: this.getNextStandardRating(
          requiredRating,
  ...
  static generateCostEstimate(
    planData: InstallPlanData,
    recommendedCable: CableRecommendation,
    protectionDevice: { type: string; rating: number; cost: number }
  ): {
    materials: number;
    labour: number;
    total: number;
    ...
    breakdown.push({
      item: `${planData.cableLength}m of ${recommendedCable.size} ${recommendedCable.type}`,
      cost: cableCost,
    });

    // Protection device cost
    const deviceBaseCost =
      protectionDevice.type === 'rcbo' ? 45 : protectionDevice.type === 'mcb' ? 15 : 120;
    const deviceCost = deviceBaseCost * protectionDevice.cost;
    breakdown.push({
      item: `${protectionDevice.rating}A ${protectionDevice.type.toUpperCase()}`,
      cost: deviceCost,
    });
  ...
  // Helper methods
  private static calculateVoltageDropPercentage(
    voltage: number,
    length: number,
    current: number,
    resistanceMvPerAmpPerM: number,
    phases: string
  ): number {
  ...
  private static getBaseCablePrice(size: number): number {
    ...
    if (size <= 2.5) return 1.2;
    if (size <= 6.0) return 2.4;
    if (size <= 16.0) return 4.8;
    if (size <= 35.0) return 8.5;
    if (size <= 70.0) return 15.2;
    return 25.0;
  }

  private static getRecommendedProtectionRating(
    cableCapacity: number,
    designCurrent: number
  ): number {
    const standardRatings = [
      6, 10, 16, 20, 25, 32, 40, 50, 63, 80, 100, 125, 160, 200, 250, 315, 400, 500, 630, 800, 1000,
      1250, 1600,
    ];
    const minRating = Math.ceil(designCurrent);
    return (
      standardRatings.find((rating) => rating >= minRating && rating <= cableCapacity) ||
      standardRatings[0]
    );
  }
  ...
  private static getAvailabilityFromSpec(
    cableSpec: any,
    size: string
  ): 'common' | 'limited' | 'special-order' {
  ...
  private static generateIntelligentNotes(
    planData: InstallPlanData,
    size: string,
    suitability: string,
    cableSpec: any
