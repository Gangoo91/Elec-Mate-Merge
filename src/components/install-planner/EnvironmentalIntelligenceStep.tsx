...
const EnvironmentalIntelligenceStep = ({
  planData,
  updatePlanData,
}: EnvironmentalIntelligenceStepProps) => {
  ...
  const getEnvironmentalProfile = () => {
    ...
    return (
      profiles[planData.environmentalConditions as keyof typeof profiles] ||
      profiles['indoor-domestic']
    );
  };

  const environmentalOptions = [
    {
      value: 'indoor-domestic',
      label: 'Indoor Domestic',
      description: 'Standard indoor home/office environment',
    },
    {
      value: 'outdoor-exposed',
    ...
    {
      value: 'fire-critical',
      label: 'Fire Critical Area',
      description: 'Escape routes, public areas',
    },
    {
      value: 'high-temperature',
      label: 'High Temperature',
      description: 'Plant rooms, near heat sources',
    },
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Tell us about your installation environment and we'll recommend the optimal cable and
          installation method.
        </p>
      </div>
      ...
      <MobileSelectWrapper
        ...
        hint={
          planData.environmentalConditions
            ? environmentalOptions.find((o) => o.value === planData.environmentalConditions)
                ?.description || 'Smart recommendations will be applied'
      ...
      {planData.environmentalConditions && (
        <Card
          className={`bg-gradient-to-r from-${currentProfile.color}-500/10 to-${currentProfile.color}-600/10 border border-${currentProfile.color}-500/30 rounded-xl`}
        >
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className={`p-4 bg-${currentProfile.color}-500/10 rounded-lg`}>
                ...
                <p className={`text-lg font-bold text-${currentProfile.color}-300`}>
                  {currentProfile.recommendedCables.includes('micc')
                    ? '250°C'
                    : currentProfile.recommendedCables.includes('xlpe-lsoh')
                      ? '90°C'
                      : currentProfile.recommendedCables.includes('swa-xlpe')
                        ? '90°C'
                        : '70°C'}
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {currentProfile.recommendedCables.map((cable, index) => (
                    <Badge
                      key={index}
                      variant="outline"
                      className={`text-${currentProfile.color}-300 border-${currentProfile.color}-500/30`}
                    >
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-2">
                ...
                <span className="text-sm text-red-200">
                  LSOH (Low Smoke Zero Halogen) cables required for public escape routes
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 bg-red-400 rounded-full mt-2 flex-shrink-0"></span>
                <span className="text-sm text-red-200">
                  Fire-resistant cables may be required for emergency circuits
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 bg-red-400 rounded-full mt-2 flex-shrink-0"></span>
                <span className="text-sm text-red-200">
                  Additional fire stopping and compartmentation required
                </span>
              </div>
            </div>
          </CardContent>
          ...
          <CardHeader className="pb-4">
            <CardTitle className="text-orange-300 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm font-normal text-orange-200/80">
                  {planData.ambientTemperature}°C Ambient
                </div>
              </div>
            </CardTitle>
          </CardHeader>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-2">
                ...
                <span className="text-sm text-orange-200">
                  Temperature derating factors will significantly reduce cable capacity
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 bg-orange-400 rounded-full mt-2 flex-shrink-0"></span>
                <span className="text-sm text-orange-200">
                  Consider XLPE (90°C) or MICC (250°C) cables for high temperature areas
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 bg-orange-400 rounded-full mt-2 flex-shrink-0"></span>
                <span className="text-sm text-orange-200">
                  Ensure adequate ventilation and spacing from heat sources
                </span>
              </div>
            </div>
          </CardContent>
