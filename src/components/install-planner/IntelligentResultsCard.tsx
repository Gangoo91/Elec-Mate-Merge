      ...
      <Card className="border-red-500/30 bg-red-500/10">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-red-200/80">
            The current load requirements exceed available cable capacities. Consider reducing load
            or using multiple circuits.
          </p>
        </CardContent>
      </Card>
    );
          ...
          <CardTitle className="text-green-300 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm font-normal text-green-200/80">
                Intelligent analysis for {planData.environmentalConditions?.replace('-', ' ')}{' '}
                environment
              </div>
            </div>
          </CardTitle>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 bg-green-500/10 rounded-lg border border-green-500/30">
              ...
              <p className="text-lg font-bold text-green-200">
                {primaryRecommendation.size}{' '}
                {primaryRecommendation.type.toUpperCase().replace('-', ' ')}
              </p>
            ...
            {protectionDevice && (
              <div className="p-4 bg-green-500/10 rounded-lg border border-green-500/30">
                ...
                <p className="text-xs text-green-200/70">Auto-selected for load protection</p>
              </div>
            )}
            ...
            {costEstimate && (
              <div className="p-4 bg-green-500/10 rounded-lg border border-green-500/30">
                ...
                <p className="text-lg font-bold text-green-200">£{costEstimate.total.toFixed(0)}</p>
                <p className="text-xs text-green-200/70">Materials + Labour</p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="text-center">
              ...
              <p className="text-sm font-bold text-green-300">
                {primaryRecommendation.voltageDropPercentage.toFixed(1)}%
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm font-bold text-green-300">
                {(
                  (primaryRecommendation.currentCarryingCapacity / designCurrent - 1) *
                  100
                ).toFixed(0)}
                %
              </p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {primaryRecommendation.specialConsiderations?.map((consideration, index) => (
                <Badge
                  key={index}
                  variant="outline"
                  className="text-green-300 border-green-500/30 text-xs"
                >
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              {costEstimate.breakdown.map((item, index) => (
                <div
                  key={index}
                  className="flex justify-between items-center p-2 bg-blue-500/10 rounded"
                >
          ...
          <CardHeader className="pb-4">
            <CardTitle className="text-amber-300 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm font-normal text-amber-200/80">
                ...
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <p className="text-xs text-amber-200/70">
                      Capacity: {alt.currentCarryingCapacity.toFixed(0)}A | VD:{' '}
                      {alt.voltageDropPercentage.toFixed(1)}%
                    </p>
                  </div>
