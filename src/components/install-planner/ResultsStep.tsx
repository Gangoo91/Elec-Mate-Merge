...
const ResultsStep = ({ planData }: ResultsStepProps) => {
  ...
  const getEnhancedDeviceRecommendations = (
    ...
    return suitableDevices
      .map((deviceOption) => {
        ...
        const isRecommended =
          deviceOption.deviceType === recommendedType ||
          (currentDevice && deviceOption.deviceType === currentDevice);

        return {
          name:
            deviceInfo.type.toUpperCase() + (deviceInfo.curve ? ` Type ${deviceInfo.curve}` : ''),
          description:
            deviceInfo.characteristics.applications.join(', ') || 'General purpose protection',
  ...
  const r1r2 =
    (planData.cableLength *
      (baseCable?.size === '1.5mm²'
        ? 24.2
        : baseCable?.size === '2.5mm²'
          ? 14.8
          : baseCable?.size === '4.0mm²'
            ? 9.2
            : baseCable?.size === '6.0mm²'
              ? 6.2
              : baseCable?.size === '10.0mm²'
                ? 3.66
                : baseCable?.size === '16.0mm²'
                  ? 2.3
                  : baseCable?.size === '25.0mm²'
                    ? 1.454
                    : baseCable?.size === '35.0mm²'
                      ? 1.04
                      : baseCable?.size === '50.0mm²'
                        ? 0.727
                        : baseCable?.size === '70.0mm²'
                          ? 0.524
                          : baseCable?.size === '95.0mm²'
                            ? 0.387
                            : 1.0)) /
    1000;
  ...
  const protectionDevice = recommendedCable
    ? IntelligentCableEngine.selectOptimalProtectionDevice(
        designCurrent,
        recommendedCable.currentCarryingCapacity
      )
    : null;
  const costEstimate =
    recommendedCable && protectionDevice
      ? IntelligentCableEngine.generateCostEstimate(planData, recommendedCable, protectionDevice)
      : null;
  ...
  const getDiversityFactor = () => {
    if (planData.installationType === 'domestic') {
      switch (planData.loadType) {
        case 'power':
          return 0.4; // 40% diversity for socket outlets
        case 'lighting':
          return 0.66; // 66% diversity for lighting
        case 'cooker':
          return 0.1; // 10% diversity for cookers
        default:
          return 0.8;
      }
      ...
      <div className="space-y-6">
        <Alert className="bg-red-500/10 border-red-500/30">
          ...
          <AlertDescription className="text-red-200">
            <strong>Unable to generate recommendations.</strong> Please check your input parameters
            and try again. The design current may exceed available cable capacities for the
    ...
    <div className="space-y-6">
      <div className="text-center md:text-left">
        ...
        <p className="text-sm sm:text-base text-muted-foreground">
          Comprehensive analysis with visual circuit design, multiple recommendations, and BS 7671
          compliance verification.
        </p>
      </div>

      {/* Visual Circuit Designer - Full Width */}
      <VisualCircuitDesigner planData={planData} recommendedCable={recommendedCable} />
      ...
      <div className="space-y-4 sm:space-y-6">
          ...
          <CardContent className="space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div className="p-4 bg-elec-dark/50 rounded border border-elec-yellow/20 text-center">
                ...
                <p
                  className={`text-lg sm:text-xl font-bold ${recommendedCable ? 'text-green-400' : 'text-red-400'}`}
                >
                  {recommendedCable
                    ? recommendedCable.size
                    : closestNonCompliant
                      ? closestNonCompliant.size
                      : '--'}
                </p>
              ...
              <div className="p-4 bg-elec-dark/50 rounded border border-elec-yellow/20 text-center">
                ...
                <p
                  className={`text-lg font-bold ${recommendedCable ? 'text-blue-400' : 'text-red-400'}`}
                >
                  {recommendedCable
                    ? recommendedCable.voltageDropPercentage.toFixed(1)
                    : closestNonCompliant
                      ? closestNonCompliant.voltageDropPercentage.toFixed(1)
                      : '--'}
                  %
                </p>
                {!recommendedCable &&
                  closestNonCompliant &&
                  closestNonCompliant.voltageDropPercentage >
            ...
            <div className="p-4 bg-elec-dark/50 rounded border border-elec-yellow/20">
              ...
              <div className="space-y-3">
                {getEnhancedDeviceRecommendations(
                  designCurrent,
                  baseCable?.currentCarryingCapacity || 0,
                ...
                {designCurrent > 32 && baseCable && baseCable.ratedCurrent === 32 && (
                  <div className="p-3 bg-red-500/10 rounded border border-red-500/30">
                    ...
                    <p className="text-xs text-red-200/80">
                      <strong>
                        32A protection for {designCurrent.toFixed(1)}A load is UNSAFE!
                      </strong>
                      <br />
                      Breaker will trip immediately. Use {baseCable.ratedCurrent}A+ protection
                      device.
                    </p>
                  </div>
                )}
                ...
                {designCurrent > 125 && (
                  <div className="p-3 bg-amber-500/10 rounded border border-amber-500/30">
                    ...
                    <p className="text-xs text-amber-200/80">
                      Design current {designCurrent.toFixed(1)}A requires BS88 HRC fuses or MCCB
                      protection. Standard MCBs only available up to 125A.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="p-3 bg-elec-dark/50 rounded border border-elec-yellow/20">
                ...
                <p className="text-xs text-muted-foreground">
                  ({(diversityFactor * 100).toFixed(0)}% applied)
                </p>
              </div>
              ...
              {baseCable && (
                <div className="p-3 bg-elec-dark/50 rounded border border-elec-yellow/20">
                  ...
                  <p className="font-medium text-green-400 text-sm sm:text-base">
                    £{baseCable.size === '1.5mm²' ? 45 : baseCable.size === '2.5mm²' ? 65 : 85}-
                    {baseCable.size === '1.5mm²' ? 85 : baseCable.size === '2.5mm²' ? 125 : 165}
                  </p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span
                        className={`text-xs sm:text-sm ${zsCompliance ? 'text-green-400' : 'text-red-400'}`}
                      >
        ...
        {!recommendedCable && closestNonCompliant && (
          <Alert className="bg-red-500/10 border-red-500/30">
            ...
            <AlertDescription className="text-red-200">
              <strong>No BS 7671 Compliant Cable Found:</strong> No single cable meets all
              requirements for this design. The closest option ({closestNonCompliant.size}) has{' '}
              {closestNonCompliant.voltageDropPercentage.toFixed(2)}% voltage drop, exceeding the{' '}
              {planData.loadType === 'lighting' ? '3%' : '5%'} limit. Consider design modifications.
            </AlertDescription>
          </Alert>
        )}
        ...
        <CableRecommendationsCard
          recommendations={
            recommendedCable
              ? [recommendedCable, ...alternativeCables]
              : closestNonCompliant
                ? [closestNonCompliant]
                : []
          }
      ...
      <Alert className="bg-amber-500/10 border-amber-500/30">
        ...
        <AlertDescription className="text-amber-200">
          <strong>Professional Verification Required:</strong> This tool provides guidance based on
          BS 7671:2018+A3:2024 requirements and simplified calculations. Always verify results with
          current regulations, manufacturer specifications, and site-specific conditions.
          Professional design verification and testing is recommended for all electrical
          installations.
        </AlertDescription>
      </Alert>
    </div>
