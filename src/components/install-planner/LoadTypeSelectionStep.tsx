...
import {
  getSimplifiedLoadTypeOptions,
  isLoadTypeSupported,
  SIMPLIFIED_CIRCUIT_TEMPLATES,
} from './SimplifiedCircuitDefaults';
...
const LoadTypeSelectionStep = ({ planData, updatePlanData }: LoadTypeSelectionStepProps) => {
  ...
  const selectedTemplate = planData.loadType
    ? SIMPLIFIED_CIRCUIT_TEMPLATES[planData.loadType]
    : null;
    ...
    <div className="space-y-8">
            ...
            <CardTitle className="text-blue-300 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm font-normal text-blue-200/80">
                  Template defaults will be applied
                </div>
              </div>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-blue-500/10 rounded-lg">
                <div className="text-xs text-blue-200/70 uppercase tracking-wide mb-1">
                  Typical Load
                </div>
                <div className="text-lg font-semibold text-blue-100">
                  {selectedTemplate.totalLoad}W
                </div>
              </div>
              <div className="p-3 bg-blue-500/10 rounded-lg">
                <div className="text-xs text-blue-200/70 uppercase tracking-wide mb-1">Supply</div>
                <div className="text-lg font-semibold text-blue-100">
                  {selectedTemplate.voltage}V{' '}
                  {selectedTemplate.phases === 'three' ? '3-Phase' : '1-Phase'}
                </div>
              </div>
              <div className="p-3 bg-blue-500/10 rounded-lg">
                <div className="text-xs text-blue-200/70 uppercase tracking-wide mb-1">
      ...
      {planData.installationType && (
        <div className="flex justify-center">
          <Badge
            className={`${getCategoryColor(planData.installationType)} border-0 px-4 py-2 text-sm`}
          >
            {getCategoryIcon(planData.installationType)}
