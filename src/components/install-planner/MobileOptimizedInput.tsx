...
const MobileOptimizedInput: React.FC<MobileOptimizedInputProps> = ({
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor={id} className={cn('text-base font-medium block', isMobile && 'text-base')}>
        {label}
      ...
      {description && (
        <p
          className={cn('text-muted-foreground leading-relaxed', isMobile ? 'text-sm' : 'text-xs')}
        >
          {description}
        </p>
      )}
