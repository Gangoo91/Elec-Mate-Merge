...
export const CircuitAnalysisCard: React.FC<CircuitAnalysisCardProps> = ({
  ...
  const suitableCables = analysis.recommendations.filter(
    (rec: any) => rec.suitability === 'suitable'
  );
        ...
        {!isCompliant && closestNonCompliant && (
          <div className="p-3 bg-red-500/10 border border-red-500/30 rounded">
            <p className="text-sm text-red-300">
              <strong>No compliant cable found.</strong> Closest option ({closestNonCompliant.size})
              has {closestNonCompliant.voltageDropPercentage.toFixed(2)}% voltage drop, exceeding BS
              7671 limits. Consider design modifications.
            </p>
          </div>
        )}
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center p-3 bg-muted/30 rounded">
                ...
                <span className="font-bold text-primary">
                  {finalRecommendation.size} {finalRecommendation.type.toUpperCase()}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 bg-muted/30 rounded">
              ...
              <div className="flex justify-between items-center p-3 bg-muted/30 rounded">
                ...
                <span className={`font-bold ${isCompliant ? 'text-primary' : 'text-red-400'}`}>
                  ...
                  {!isCompliant && <span className="text-xs text-red-400 ml-1">EXCEEDS LIMIT</span>}
                </span>
              </div>
            </div>
          </div>
