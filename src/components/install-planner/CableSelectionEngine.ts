...
export class CableSelectionEngine {
  ...
  static generateSuggestions(
    ...
    if (suitableOptions.length === 0) {
      suggestions.push({
        ...
        description:
          'No cable sizes meet BS 7671 requirements. Consider increasing cable size or reducing load.',
        impact: 'high',
      ...
      const currentCapacityMargin =
        ((bestOption.currentCarryingCapacity - planData.totalLoad / planData.voltage) /
  ...
  static performComplianceChecks(
    planData: InstallPlanData,
    zsValue: number,
