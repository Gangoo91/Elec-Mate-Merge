...
export class EnhancedCableSelectionEngine {
  static calculateCableOptions(
    ...
    for (const [size, cableData] of Object.entries(cableDatabase)) {
      ...
      const baseCapacity =
        cableData.currentCarryingCapacity[installMethod] ||
        cableData.currentCarryingCapacity['clipped-direct'];
      ...
      if (isRingCircuit) {
        ...
        totalVoltageDrop =
          (voltageDropPerMetre * planData.cableLength * designCurrent) / (4 * 1000);
      } else {
      ...
      const voltageDropOK =
        voltageDropPercentage <= this.getMaxVoltageDropPercentage(planData.loadType);
      const lengthOK = planData.cableLength <= cableData.maxLength;
      const breakerCompatible = this.isBreakingDeviceCompatible(
        designCurrent,
        cableData,
        planData.protectiveDevice
      );

      // Calculate Zs to check compliance
      const r1r2 =
        (planData.cableLength * (cableData.resistance.r1 + cableData.resistance.r2)) / 1000;
      ...
      if (currentOK && voltageDropOK && lengthOK && breakerCompatible && zsOK) {
        ...
        currentOK &&
        voltageDropPercentage <= this.getMaxVoltageDropPercentage(planData.loadType) * 1.2 &&
        zsOK
      ) {
      ...
      if (!currentOK)
        notes.push(
          `❌ Current capacity insufficient (${deratedCapacity.toFixed(1)}A vs ${(designCurrent * 1.1).toFixed(1)}A required)`
        );
      if (!voltageDropOK) {
        if (vdSeverity === 'critical') {
          notes.push(
            `❌ CRITICAL: Voltage drop severely exceeds limits (${voltageDropPercentage.toFixed(2)}% vs ${this.getMaxVoltageDropPercentage(planData.loadType)}% max)`
          );
        } else {
          notes.push(
            `❌ Voltage drop exceeds limits (${voltageDropPercentage.toFixed(2)}% vs ${this.getMaxVoltageDropPercentage(planData.loadType)}% max)`
          );
        }
      }
      if (!lengthOK)
        notes.push(
          `❌ Cable length exceeds recommendations (${planData.cableLength}m vs ${cableData.maxLength}m max)`
        );
      if (!breakerCompatible)
        notes.push(`❌ Protective device rating incompatible with cable capacity`);
      if (!zsOK)
        notes.push(
          `❌ Earth fault loop impedance too high (${zsValue.toFixed(3)}Ω vs ${maxZs.toFixed(3)}Ω max)`
        );
      ...
      const ratedCurrent = this.calculateProtectiveDeviceRating(
        designCurrent,
        deratedCapacity,
        cableData
      );
  ...
  static calculateMultiCircuitOptions(
    circuits: Circuit[],
    environmentalSettings: any
  ): Array<{
    circuit: Circuit;
    ...
    return circuits
      ...
      .map((circuit) => {
        ...
        const isRingCircuit =
          circuit.loadType === 'power' && circuit.cableLength <= 106 && circuit.totalLoad <= 7200;
  ...
  static generateEnhancedSuggestions(
    planData: InstallPlanData,
    cableOptions: CableRecommendation[],
    isRingCircuit: boolean = false
  ): InstallationSuggestion[] {
    ...
    if (!isRingCircuit && planData.loadType === 'power' && planData.cableLength <= 106) {
      suggestions.push({
        ...
        description:
          'For power circuits, a ring configuration may provide better load distribution and reduced voltage drop.',
        impact: 'medium',
        cost: 'low',
    ...
    if (bestOption && bestOption.suitability !== 'suitable') {
      ...
      if (suitableOptions.length > 0) {
        const improvement =
          bestOption.voltageDropPercentage - suitableOptions[0].voltageDropPercentage;
        suggestions.push({
          type: 'cable-upgrade',
        ...
        suggestions.push({
          ...
          description:
            'Consider: 1) Reduce load, 2) Shorten cable run, 3) Use 3-phase supply, 4) Install intermediate distribution board, or 5) Use parallel cable configuration.',
          impact: 'high',
    ...
    if (
      bestOption &&
      bestOption.voltageDropPercentage > this.getMaxVoltageDropPercentage(planData.loadType)
    ) {
      if (bestOption.voltageDropPercentage > 10) {
        suggestions.push({
          ...
          description:
            'Voltage drop is excessive and may cause equipment malfunction. Consider three-phase supply or intermediate distribution board.',
          impact: 'high',
    ...
    if (planData.cableLength > 100 && planData.totalLoad > 5000) {
      suggestions.push({
        ...
        description:
          'For long runs with high loads, consider using 5-core cable with paralleled L&N conductors to reduce voltage drop. Requires careful calculation and may not be best practice for all installations.',
        impact: 'medium',
        cost: 'medium',
    ...
    if (planData.ambientTemperature > 30) {
      suggestions.push({
        ...
        description:
          'High ambient temperature requires cable derating. Consider improved ventilation or larger cable size.',
        impact: 'medium',
        regulation: 'BS 7671 Appendix 4',
      });
    }
    ...
    if (
      this.mapInstallationMethod(planData.installationMethod) === 'conduit' &&
      ...
      suggestions.push({
        ...
        description:
          'For longer runs, cable tray or trunking may provide better heat dissipation and easier maintenance.',
        impact: 'medium',
        cost: 'medium',
      });
    }
  ...
  static performEnhancedComplianceChecks(
    planData: InstallPlanData,
    zsValue: number,
    ...
    const voltageDropStatus =
      recommendedCable.voltageDropPercentage <= maxVoltageDropPercentage ? 'pass' : 'fail';
    checks.push({
      regulation: 'BS 7671',
    ...
    const protectionAdequate =
      recommendedCable.ratedCurrent >= designCurrent &&
  ...
  private static calculateDeratingFactor(planData: InstallPlanData): number {
    ...
    if (ambientTemp > 30) {
      ...
      factor *= tempDeratingFactors[nearestTemp] || 0.5;
    }
  ...
  private static isBreakingDeviceCompatible(
    designCurrent: number,
    cableData: any,
  ...
  private static calculateProtectiveDeviceRating(
    designCurrent: number,
    ...
    return (
      standardRatings.find((rating) => rating >= minRating && rating <= maxRating) || minRating
    );
  }
  ...
  private static getMaxZs(protectiveDevice: string, voltage: number): number {
    ...
    const deviceType =
      protectiveDevice.toLowerCase().includes('mcb') ||
      protectiveDevice.toLowerCase().includes('rcbo')
        ? 'mcb'
        : 'rcbo';
  ...
  private static getSpecialConsiderations(
    ...
    if (planData.loadType === 'motor') {
      considerations.push(
        'Motor starting current considerations apply (typically 5-7x full load current)'
      );
