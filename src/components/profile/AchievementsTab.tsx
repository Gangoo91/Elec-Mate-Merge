  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className={`font-medium ${isLocked ? 'text-gray-400' : ''}`}>{title}</h3>
          <p className="text-xs text-muted-foreground">{description}</p>

          {!isLocked && (
            <div className="mt-2 h-1.5 w-full bg-gray-700 rounded-full overflow-hidden">
              <div className="h-full bg-elec-yellow" style={{ width: `${progress}%` }}></div>
            </div>
          )}

          {isCompleted && <span className="text-xs text-elec-yellow mt-2 block">Completed</span>}
        </div>
      </div>
    </div>
  );
