  ...
  return (
    <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        >
          <Link to="/auth/signup" className="w-full sm:w-auto">
            <Button
              size="lg"
              className="w-full sm:w-auto h-14 px-8 text-lg font-semibold bg-yellow-500 hover:bg-yellow-400 text-black touch-manipulation transition-colors duration-200"
            >
              Start Free Trial
            </Button>
          </Link>
          <Link to="/auth/signin" className="w-full sm:w-auto">
            <Button
              size="lg"
              variant="outline"
              className="w-full sm:w-auto h-14 px-8 text-lg font-semibold border-white/20 text-white hover:bg-white/10 touch-manipulation transition-colors duration-200"
            >
              Sign In
            </Button>
          </Link>
      ...
      <motion.button
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2 text-white/40 hover:text-white/70 transition-colors touch-manipulation"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1, y: prefersReducedMotion ? 0 : [0, 6, 0] }}
        transition={{ opacity: { delay: 0.5 }, y: { duration: 2.5, repeat: Infinity, ease: 'easeInOut' } }}
