...
export const RegionalRateSelector: React.FC<RegionalRateSelectorProps> = ({
  ...
  const regionOptions = Object.entries(MARKET_RATES_2025.regionalMultipliers).map(
    ([key, multiplier]) => ({
      value: key,
  ...
  const handleRegionChange = (region: string) => {
    ...
    const adjustedRate = calculateRegionalRate(
      baseRate,
  ...
  const currentMultiplier =
    MARKET_RATES_2025.regionalMultipliers[
      selectedRegion as keyof typeof MARKET_RATES_2025.regionalMultipliers
    ] || 1;
  const adjustedRate = calculateRegionalRate(
    baseRate,
    selectedRegion as keyof typeof MARKET_RATES_2025.regionalMultipliers
    ...
    <Card className="border-elec-yellow/20 bg-elec-card">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          >
            {currentMultiplier > 1 ? '+' : ''}
            {((currentMultiplier - 1) * 100).toFixed(0)}% regional adjustment
          </Badge>
        </div>

        <div className="text-xs text-muted-foreground text-center">
          Regional rates updated for 2025 market conditions including inflation, living costs, and
          demand patterns.
        </div>
      </CardContent>
    </Card>
