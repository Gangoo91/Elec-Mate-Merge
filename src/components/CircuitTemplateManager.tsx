...
export const CircuitTemplateManager = () => {
  ...
  const seedTemplates = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('circuit_templates').insert(templateInserts);
  ...
  const deleteAllTemplates = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('circuit_templates').delete().eq('user_id', user.id);
    ...
    <div className="container mx-auto p-4 space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Seed the database with {CIRCUIT_TEMPLATES.length} pre-configured circuit templates for
            faster form completion
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Button onClick={seedTemplates} disabled={loading || seeded}>
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {seeded && <CheckCircle className="mr-2 h-4 w-4" />}
              {seeded ? 'Templates Seeded' : 'Seed Templates'}
            </Button>

            <Button onClick={deleteAllTemplates} disabled={loading} variant="destructive">
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Clear All Templates
            </Button>
          </div>
                    ...
                    <CardContent className="py-2 text-xs space-y-1">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          {template.templateData.protectiveDeviceRating}
                        </div>
                        <div>
                          <span className="font-medium">Cable:</span>{' '}
                          {template.templateData.cableCsa}
                        </div>
                        <div>
                          <span className="font-medium">Type:</span>{' '}
                          {template.templateData.protectiveDeviceType}
                        </div>
                        <div>
                          <span className="font-medium">Max Demand:</span>{' '}
                          {template.templateData.maxDemand}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
