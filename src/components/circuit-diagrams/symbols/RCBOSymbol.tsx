...
export const RCBOSymbol = ({
  ...
  return (
    <g transform={`translate(${x}, ${y})`} className="rcbo-symbol">
      ...
      <rect x="0" y="0" width="50" height="80" fill="white" stroke="black" strokeWidth="2" rx="2" />
      ...
      {/* RCD type indicator */}
      <text x="25" y="70" textAnchor="middle" fontSize="10" fill="black">
        Type {rcdType}
      </text>

      {/* Test button indicator (small circle) */}
      <circle cx="43" cy="10" r="3" fill="blue" stroke="black" strokeWidth="1" />
      <text x="43" y="12" textAnchor="middle" fontSize="8" fill="white">
        T
      </text>

      {/* Label below */}
      {label && (
        <text x="25" y="105" textAnchor="middle" fontSize="12" fill="black">
          {label}
        </text>
      )}
    </g>
