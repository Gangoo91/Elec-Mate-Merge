...
export const MCBSymbol = ({ rating, curve, x, y, label, kaRating = 6 }: MCBSymbolProps) => {
  return (
    <g transform={`translate(${x}, ${y})`} className="mcb-symbol">
      {/* MCB body */}
      <rect x="0" y="0" width="40" height="60" fill="white" stroke="black" strokeWidth="2" rx="2" />
      ...
      {/* kA rating */}
      <text x="20" y="50" textAnchor="middle" fontSize="10" fill="black">
        {kaRating}kA
      </text>

      {/* Label below */}
      {label && (
        <text x="20" y="85" textAnchor="middle" fontSize="12" fill="black">
          {label}
        </text>
      )}
    </g>
