  ...
  const calculateEnhancedFunding = () => {
    ...
    if (['level3', 'level4', 'level5', 'level6'].includes(inputs.courseLevel) && age >= 19) {
      const maxLoan = Math.min(
        cost - totalFunding,
        FUNDING_RATES_2025.advancedLearnerLoan.maxLoanPerCourse
      );
      if (maxLoan > 0) {
        fundingSources.push({
    ...
    if (
      inputs.region &&
      REGIONAL_FUNDING_2025.england[inputs.region as keyof typeof REGIONAL_FUNDING_2025.england]
    ) {
      const multiplier =
        REGIONAL_FUNDING_2025.england[inputs.region as keyof typeof REGIONAL_FUNDING_2025.england]
          .multiplier;
    ...
    if (fundingGap > 0) {
      recommendations.push(
        `Consider employer funding - speak to your manager about the £${fundingGap.toLocaleString()} shortfall`
      );
    }
    ...
    if (inputs.employmentStatus === 'employed' && !inputs.hasEmployerSupport) {
      recommendations.push(
        'Discuss with your employer - they may benefit from apprenticeship levy credits'
      );
    }
      ...
      <Card className="border-elec-yellow/20 bg-elec-card">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Comprehensive funding analysis with updated 2025 rates and professional qualification
            support
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="courseCost" className="text-foreground">
                Course Cost (£) *
              </Label>
              <Input
                id="courseCost"
            ...
            <div className="space-y-2">
              <Label htmlFor="currentSalary" className="text-foreground">
                Annual Salary (£) *
              </Label>
              <Input
                id="currentSalary"
            ...
            <div className="space-y-2">
              <Label htmlFor="employerSize" className="text-foreground">
                Employer Size
              </Label>
              <Select
                value={inputs.employerSize}
                onValueChange={(value) => handleInputChange('employerSize', value)}
              >
                <SelectTrigger className="border-elec-yellow/20 focus:border-elec-yellow">
            ...
            <div className="space-y-2">
              <Label htmlFor="employerContribution" className="text-foreground">
                Employer Contribution (£)
              </Label>
              <Input
          ...
          <div className="flex gap-4">
            <Button
              onClick={calculateEnhancedFunding}
              size="lg"
      ...
      {result && (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 gap-1 md:gap-0 bg-elec-card/50 p-1">
            <TabsTrigger
              value="overview"
              className="text-xs md:text-sm px-2 md:px-3 py-2 data-[state=active]:bg-elec-yellow data-[state=active]:text-background"
            >
              Overview
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Card
                className={`border-elec-yellow/20 bg-elec-card ${result.fundingGap > 0 ? 'border-orange-500/30' : 'border-green-500/30'}`}
              >
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p
                        className={`text-2xl font-bold ${result.fundingGap > 0 ? 'text-orange-500' : 'text-green-500'}`}
                      >
            ...
            <Card className="border-elec-yellow/20 bg-elec-card">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Comprehensive breakdown of your funding situation
                </p>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-foreground font-medium">
                      Coverage: {result.fundingCoverage.toFixed(1)}%
                    </span>
                    <span className="text-foreground">
                      £{result.totalFunding.toLocaleString()} of £
                      {result.totalCost.toLocaleString()}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {result.fundingSources.length} sources identified
                    </p>
                  ...
                  <div className="p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-foreground font-medium">
                          £
                          {FUNDING_RATES_2025.advancedLearnerLoan.repaymentThreshold.toLocaleString()}
                          + salary
                        </span>
                      </div>
                    </div>
                ...
                {result.monthlyRepayment > 0 && (
                  <div className="p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                    ...
                    <p className="text-sm text-foreground mb-2">
                      Monthly loan repayments of{' '}
                      <strong>£{result.monthlyRepayment.toFixed(0)}</strong> will start
                      automatically when your salary exceeds £
                      {FUNDING_RATES_2025.advancedLearnerLoan.repaymentThreshold.toLocaleString()}{' '}
                      per year.
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Repayments are calculated at 9% of income above the threshold and collected
                      through PAYE.
                    </p>
                  </div>
                )}
                    ...
                    <div className="space-y-2">
                      ...
                      {source.applicationUrl && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="w-full mt-3 border-elec-yellow/50 hover:bg-elec-yellow hover:text-background"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 border border-elec-yellow/20 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground mb-2">
                      Up to £2,000 for electrical qualifications
                    </p>
                    <p className="text-xs text-muted-foreground">
                  ...
                  <div className="p-4 border border-elec-yellow/20 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground mb-2">
                      Industry-specific funding available
                    </p>
                  ...
                  <div className="p-4 border border-elec-yellow/20 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground mb-2">
                      Location-based funding support
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Varies by region and priority skills
                    </p>
                  </div>
                  <div className="p-4 border border-elec-yellow/20 rounded-lg">
          ...
          <TabsContent value="timeline" className="space-y-4">
            <Card className="border-elec-yellow/20 bg-elec-card">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Your step-by-step guide to securing education funding
                </p>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-4 p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-foreground mb-2">
                        Research and prepare your applications
                      </p>
                      <p className="text-xs text-foreground">
                        Timeline: 4-6 weeks before course start
                      </p>
                  ...
                  <div className="flex items-start gap-4 p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-foreground mb-2">
                        Receive funding decisions and enroll
                      </p>
                  ...
                  <div className="flex items-start gap-4 p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-foreground mb-2">
                        Maintain funding and track progress
                      </p>
                  ...
                  <div className="flex items-start gap-4 p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-foreground mb-2">
                        Transition to repayment and career advancement
                      </p>
          ...
          <TabsContent value="recommendations" className="space-y-4">
            <Card className="border-elec-yellow/20 bg-elec-card">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Tailored next steps based on your funding analysis
                </p>
              </CardHeader>
              <CardContent className="space-y-6">
                {/* Immediate Actions */}
                <div className="space-y-4">
                  ...
                  {result.recommendations.map((rec, index) => (
                    <div
                      key={index}
                      className="flex items-start gap-3 p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                      ...
                      <p className="text-sm text-foreground mb-1">
                        Email: skillsbank@education.gov.uk
                      </p>
                      <p className="text-xs text-muted-foreground">
                        For professional qualification funding
                      </p>
                    </div>
                    <div className="p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                      ...
                      <p className="text-xs text-muted-foreground">
                        For electrical engineering funding
                      </p>
                    </div>
                    <div className="p-4 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                      <h5 className="font-semibold text-foreground mb-2">Local Skills Hub</h5>
                      <p className="text-sm text-foreground mb-1">
                        Search: gov.uk/guidance/local-skills-improvement-plans
                      </p>
                      <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Button
                      variant="outline"
                      className="h-auto p-4 flex flex-col items-start border-elec-yellow/20 hover:bg-elec-yellow hover:text-background"
                    >
                ...
                <div className="p-6 border border-elec-yellow/20 rounded-lg bg-elec-card/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-elec-yellow">
                        £{((parseFloat(inputs.currentSalary) || 0) * 0.15).toLocaleString()}
                      </p>
                      <p className="text-sm text-foreground">Potential salary increase</p>
                      <p className="text-xs text-muted-foreground">
                        15% average for qualified electricians
                      </p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-elec-yellow">
                        {Math.round(
                          (parseFloat(inputs.courseCost) || 0) /
                            ((parseFloat(inputs.currentSalary) || 0) * 0.15 || 1)
                        )}{' '}
                        months
                      </p>
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold text-elec-yellow">
                        £
                        {(
                          (parseFloat(inputs.currentSalary) || 0) * 0.15 * 10 -
                          (parseFloat(inputs.courseCost) || 0)
                        ).toLocaleString()}
                      </p>
                ...
                {result.taxBenefits > 0 && (
                  <div className="p-4 bg-elec-yellow/10 border border-elec-yellow/20 rounded-lg">
                    ...
                    <p className="text-sm text-foreground">
                      As a VAT-registered business, you can reclaim £
                      {result.taxBenefits.toLocaleString()} in VAT on course fees.
                    </p>
                  </div>
                )}
              </CardContent>
