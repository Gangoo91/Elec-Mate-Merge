...
const MyReports: React.FC<MyReportsProps> = ({ onBack, onNavigate, onEditReport }) => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      console.log('[MyReports] Initial session:', session ? 'Authenticated' : 'Not authenticated');
      setUser(session?.user ?? null);
    });

    // Listen for auth changes
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      console.log('[MyReports] Auth state changed:', session ? 'Authenticated' : 'Not authenticated');
      setUser(session?.user ?? null);
    });
  ...
  const { data: reportsData, isLoading: isLoadingReports, refetch: refetchReports } = useQuery<ReportsResponse>({
    ...
    queryFn: async () => {
      if (!user) {
        console.log('[MyReports] No user, skipping fetch');
        return { reports: [], totalCount: 0, hasMore: false };
      }
      console.log('[MyReports] Fetching reports for user:', user.id, 'page:', currentPage);
      const result = await reportCloud.getUserReports(user.id, { page: currentPage, pageSize: 20 });
      console.log('[MyReports] Fetched reports:', result.reports.length, 'of', result.totalCount);
      return result;
    },
    enabled: !!user,
  });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        console.error('[MyReports] Delete failed:', error);
      ...
      console.log('[MyReports] Report soft-deleted successfully:', reportIdToDelete);
      ...
      console.error('[MyReports] Delete exception:', error);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        console.log('[MyReports] Bulk delete successful:', successCount, 'deleted');
      ...
      if (failCount > 0) {
        ...
        console.error('[MyReports] Bulk delete failures:', failures);
      ...
      console.error('[MyReports] Bulk delete exception:', error);
  ...
  const handleBulkExport = async () => {
    ...
    try {
      ...
      const onProgress = (current: number, total: number, reportId: string) => {
        ...
        console.log(`[MyReports] Exporting ${current}/${total}: ${reportId}`);
      ...
      console.log('[MyReports] Export complete:', result);
      
      // Show final result
      if (result.successful > 0) {
        console.log('[MyReports] Showing success toast');
        toast({
      ...
      if (result.failed > 0) {
        ...
        console.log('[MyReports] Showing error toast, errors:', result.errors);
        console.error('[MyReports] Export errors:', result.errors);
      ...
      console.error('[MyReports] Bulk export error:', error);
      toast({
        title: 'Export Failed',
      ...
      console.log('[MyReports] Export finished, cleaning up...');
