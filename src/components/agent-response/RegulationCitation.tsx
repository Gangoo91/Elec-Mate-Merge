        ...
        <TooltipTrigger asChild>
          <div className="flex items-start justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/50 hover:border-elec-yellow/30 transition-colors cursor-pointer">
            <div className="flex-1 pr-4">
              <h4 className="font-semibold text-elec-yellow mb-1">{citation.regulation_number}</h4>
              <p className="text-sm text-gray-400">{citation.section}</p>
              {citation.relevanceToQuery && (
                <p className="text-xs text-gray-500 mt-1 italic">{citation.relevanceToQuery}</p>
              )}
            </div>
            {citation.confidence && <ConfidenceBadge confidence={citation.confidence.overall} />}
          </div>
        </TooltipTrigger>
