...
const RealMarketInsights = () => {
  ...
  const handleRefreshAll = async () => {
    ...
    try {
      await Promise.all([refreshPrices(true), refreshLive()]);
      toast({
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Real-time insights powered by community data and market trends
          </p>
        </div>
        <Button
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                Last updated:{' '}
                {metalPrices?.lastUpdated ? getTimeAgo(metalPrices.lastUpdated) : 'Never'}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {metalPrices?.metalPrices &&
          metalPrices.metalPrices.map((metalData) => {
                ...
                <CardContent className="p-4">
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <div className="text-2xl font-bold">£{priceValue.toFixed(2)}</div>
        ...
        <Card className="border-primary/20">
          ...
          <CardContent className="space-y-4">
            {communityStats?.popularJobTypes.length ? (
              <>
                <div className="text-2xl font-bold text-primary">
                  {communityStats.popularJobTypes.reduce((sum, item) => sum + item.count, 0)} Total
                  Jobs
                </div>
                <div className="space-y-2">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center space-y-2">
              ...
              <Badge variant="secondary" className="text-xs">
                {communityStats?.lastSubmissionTime
                  ? getTimeAgo(communityStats.lastSubmissionTime)
                  : 'No data'}
              </Badge>
            </div>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground mt-1">
                    We're collecting data to provide more accurate insights. Submit your pricing
                    data to help improve market intelligence for everyone.
                  </div>
                </div>
              </div>
            </div>
