      ...
      return (
        <Alert className="mt-4 bg-amber-500/10 border-amber-500/30 text-amber-500">
          ...
          <AlertDescription className="text-sm">
            <p className="mb-2">
              API authorization issue detected. This is likely due to one of the following:
            </p>
            ...
            {apiErrorMessage && <p className="mt-2 italic text-xs">{apiErrorMessage}</p>}
          </AlertDescription>
        </Alert>
      );
    case 'OVER_QUERY_LIMIT':
      return (
        <Alert className="mt-4 bg-amber-500/10 border-amber-500/30 text-amber-500">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription className="text-sm">
            The Google Maps API usage quota has been exceeded. Please try again later or check your
            billing settings in the Google Cloud Console.
          </AlertDescription>
        </Alert>
      );
    case 'INVALID_REQUEST':
      return (
        <Alert className="mt-4 bg-amber-500/10 border-amber-500/30 text-amber-500">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription className="text-sm">
            The request sent to the Google Maps API was invalid. This could be due to a problem with
            the postcode format.
          </AlertDescription>
        </Alert>
      );
      ...
      return (
        <Alert className="mt-4 bg-amber-500/10 border-amber-500/30 text-amber-500">
          ...
          <AlertDescription className="text-sm">
            ...
            {apiErrorMessage && <p className="mt-1 italic text-xs">{apiErrorMessage}</p>}
          </AlertDescription>
        </Alert>
      );
  }
