    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
            ...
            <div className="space-y-2">
              {circuits.map((circuit) => (
                <div
                  ...
                  className="flex items-center space-x-3 rounded-md border border-border p-3 hover:bg-accent/50 transition-colors"
                >
                  <RadioGroupItem
                    value={circuit.position.toString()}
                    id={`pos-${circuit.position}`}
                  />
        ...
        <DialogFooter>
          ...
          <Button onClick={handleInsert}>Insert Circuit</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
