...
const HERO_BUTTON =
  'inline-flex items-center gap-1.5 h-11 sm:h-10 px-4 rounded-full text-[12.5px] sm:text-sm font-medium touch-manipulation active:scale-[0.97] transition-colors';
...
const FeaturedPodcastHero = ({ podcast }: FeaturedPodcastHeroProps) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-[hsl(0_0%_12%)]">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-orange-500/70 via-amber-400/70 to-orange-500/70 opacity-70" />
      <div className="p-5 sm:p-6 md:p-8">
        <div className="mb-4 text-[10.5px] font-medium uppercase tracking-[0.18em] text-orange-300">
          Featured podcast
        </div>

        <div className="flex flex-row items-start gap-4 sm:gap-6">
          {/* Podcast mark */}
          <div className="w-14 h-14 sm:w-24 sm:h-24 rounded-2xl bg-white/[0.05] border border-white/[0.08] flex items-center justify-center shrink-0">
            <Mic className="w-6 h-6 sm:w-10 sm:h-10 text-elec-yellow" />
          </div>

          {/* Text content */}
          <div className="flex-1 min-w-0 space-y-2 sm:space-y-4">
            <div>
              <h2 className="text-lg sm:text-2xl font-semibold text-white tracking-tight mb-1">
                {podcast.name}
              </h2>
              <p className="text-[13px] sm:text-base text-white/70 font-medium">{podcast.host}</p>
            </div>

            <p className="hidden sm:block text-white/85 text-sm sm:text-base leading-relaxed max-w-2xl">
              {podcast.description}
            </p>

            {/* Topics — hidden on mobile for a cleaner hero */}
            <div className="hidden sm:flex flex-wrap gap-2">
              {podcast.topics.map((topic, idx) => (
                <span
                  key={idx}
                  className="text-xs px-3 py-1 rounded-full border border-white/[0.08] bg-white/[0.04] text-white/80"
                >
                  {topic}
                </span>
              ))}
            ...
            {/* Platform buttons — labels always visible, 44px targets on mobile */}
            <div className="flex flex-wrap gap-2 pt-1">
              {platforms.map((p) => (
                <button
                  key={p.key}
                  onClick={() => openExternalUrl(p.url)}
                  className={`${HERO_BUTTON} ${p.className}`}
