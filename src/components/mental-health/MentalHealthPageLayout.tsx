  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-4 sm:px-6 lg:px-8 pb-20">
      ...
      <div className="flex flex-col items-center justify-center mb-6 text-center">
        <div
          className={`p-3 rounded-2xl mb-4 ${color === 'red' ? 'bg-red-500/20' : color === 'orange' ? 'bg-orange-500/20' : color === 'blue' ? 'bg-blue-500/20' : color === 'green' ? 'bg-green-500/20' : 'bg-purple-500/20'}`}
        >
          {icon}
        </div>
        ...
        <p className="text-white max-w-2xl mb-4 text-sm sm:text-base">{description}</p>
        <SmartBackButton />
      </div>

      {/* Main Content Card */}
      <Card
        className={`${colorClasses[color as keyof typeof colorClasses]} bg-gradient-to-br border`}
      >
