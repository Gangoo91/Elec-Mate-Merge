...
export class CalculatorValidator {
  ...
  static validatePowerFactor(
    ...
    if (calculatedPF < 0.85) {
      warnings.push(
        `Low power factor (${calculatedPF.toFixed(3)}) - consider power factor correction`
      );
    }

    if (calculatedPF < 0.7) {
      errors.push(
        `Power factor too low (${calculatedPF.toFixed(3)}) - power factor correction required`
      );
    }
...
Timestamp: ${timestamp}
Standards: BS 7671:2018+A3:2024, IET Guidance
