  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        calibrationDue: data.filter(
          (t) => t.calibration_due && t.calibration_due <= thirtyDaysFromNow
        ).length,
        patDue: data.filter((t) => t.pat_test_due && t.pat_test_due <= thirtyDaysFromNow).length,
        needsRepair: data.filter(
          (t) => t.condition === 'needs_repair' || t.condition === 'out_of_service'
        ).length,
      };
    },
    enabled: !!vehicleId,
...
export function useUpdateTool() {
  ...
  return useMutation({
    mutationFn: async ({
      id,
      ...input
    }: UpdateToolInput & { id: string }): Promise<VehicleTool> => {
      const { data, error } = await supabase
        .from('vehicle_tools')
...
export function useDeleteTool() {
  ...
  return useMutation({
    mutationFn: async ({ id, vehicleId }: { id: string; vehicleId: string }): Promise<void> => {
      const { error } = await supabase.from('vehicle_tools').delete().eq('id', id);
