  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        nextServiceMileage:
          upcomingServices.length > 0 ? upcomingServices[0].next_service_mileage : null,
      };
    },
    enabled: !!vehicleId,
  });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const allMileages = [
        ...services
          .filter((s) => s.mileage)
          .map((s) => ({ mileage: s.mileage!, date: s.service_date })),
...
export function useCreateService() {
  ...
  return useMutation({
    mutationFn: async (input: CreateServiceInput): Promise<VehicleService> => {
      ...
      await supabase.from('vehicles').update(updates).eq('id', input.vehicle_id);
