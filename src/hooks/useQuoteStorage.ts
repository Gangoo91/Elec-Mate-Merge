  ...
  const saveQuote = useCallback(async (quote: Quote) => {
    try {
      ...
      const quoteData = {
        ...
        status: quote.status,
        tags: quote.tags || [],
        last_reminder_sent_at: quote.lastReminderSentAt ? (quote.lastReminderSentAt instanceof Date ? quote.lastReminderSentAt.toISOString() : new Date(quote.lastReminderSentAt).toISOString()) : null,
        notes: quote.notes,
        expiry_date: quote.expiryDate instanceof Date ? quote.expiryDate.toISOString() : new Date(quote.expiryDate || Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        acceptance_status: quote.acceptance_status || 'pending',
        accepted_at: quote.accepted_at ? (quote.accepted_at instanceof Date ? quote.accepted_at.toISOString() : new Date(quote.accepted_at).toISOString()) : null,
