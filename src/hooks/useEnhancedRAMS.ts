...
export const useEnhancedRAMS = () => {
  ...
  const createCustomHazard = useCallback(
    async (hazardData: Omit<CustomHazard, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => {
      try {
        const {
          data: { user },
        } = await supabase.auth.getUser();
  ...
  const linkHazardToTask = useCallback(
    async (taskId: string, hazardId: string) => {
      try {
  ...
  const createRamsMethodLink = useCallback(
    async (linkData: Omit<RAMSMethodLink, 'id' | 'user_id' | 'created_at'>) => {
      try {
        const {
          data: { user },
        } = await supabase.auth.getUser();
  ...
  const getHazardSuggestions = useCallback(
    (task: Task): LinkingSuggestion[] => {
      const suggestions: LinkingSuggestion[] = [];

      // Get hazards from the same category
      const categoryHazards = hazards.filter(
        (h) =>
      ...
      const frequentHazards = hazards.filter((h) => h.frequency > 5).slice(0, 3);
