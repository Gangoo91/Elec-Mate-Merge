...
const tabConfigs: TabConfig[] = [
  { id: 'client', label: 'Client & Premises', shortLabel: 'Client', requiredFields: ['clientName', 'premisesAddress'] },
  { id: 'system', label: 'System & Panel', shortLabel: 'System', requiredFields: ['systemCategory'] },
  { id: 'zones', label: 'Zones & Devices', shortLabel: 'Zones', requiredFields: [] },
  { id: 'equipment', label: 'Equipment', shortLabel: 'Equip', requiredFields: [] },
  { id: 'declarations', label: 'Declarations', shortLabel: 'Sign', requiredFields: ['installerName'] },
];
...
export const useFireAlarmTabs = (formData: any) => {
  ...
  const isTabComplete = (tabId: FireAlarmTabValue): boolean => {
    ...
    switch (tabId) {
      case 'client': return cs[tabId] || (formData.clientName && formData.premisesAddress);
      case 'system': return cs[tabId] || !!formData.systemCategory;
      case 'zones': return cs[tabId] || (formData.zones?.length > 0 && formData.zones[0]?.zoneName);
      case 'equipment': return cs[tabId] || true;
      case 'declarations': return cs[tabId] || (formData.installerName && formData.installerSignature);
