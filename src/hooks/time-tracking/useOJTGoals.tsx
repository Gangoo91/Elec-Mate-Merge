...
export const useOJTGoals = () => {
  ...
  const addGoal = useCallback(
    async (input: CreateOJTGoalInput): Promise<OJTGoal | null> => {
      if (!user?.id) {
        toast({
          title: 'Error',
  ...
  const updateGoal = useCallback(
    async (id: string, input: UpdateOJTGoalInput): Promise<boolean> => {
  ...
  const updateProgress = useCallback(
    async (id: string, current_value: number): Promise<boolean> => {
      if (!user?.id) return false;
      ...
      try {
        ...
        setGoals((prev) =>
          prev.map((g) =>
            g.id === id
              ? { ...g, current_value, status: newStatus, updated_at: new Date().toISOString() }
              : g
          )
        );
