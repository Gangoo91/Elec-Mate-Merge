...
export const useTrainingEvidenceDB = () => {
  ...
  const categorizeEvidence = (
    ...
    if (
      evidenceType.includes('workshop') ||
    ...
    if (
      evidenceType.includes('assessment') ||
  ...
  const generateSmartTags = (
    ...
    // Technical skills
    if (content.includes('wiring') || content.includes('electrical'))
      tags.push('Electrical Installation');
    ...
    if (content.includes('test') || content.includes('inspection'))
      tags.push('Testing & Inspection');
    if (content.includes('distribution') || content.includes('board'))
      tags.push('Distribution Systems');
  ...
  const addEvidence = async (evidenceData: Omit<TrainingEvidenceData, 'id'>, file?: File) => {
    ...
    try {
      ...
      const category = categorizeEvidence(
      ...
      await supabase.from('training_evidence').update({ portfolio_linked: true }).eq('id', data.id);
