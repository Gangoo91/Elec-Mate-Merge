  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        expiringIn7Days: data.filter(
          (d) => d.expiry_date && d.expiry_date >= today && d.expiry_date <= sevenDaysFromNow
        ).length,
        expiringIn30Days: data.filter(
          (d) =>
            d.expiry_date && d.expiry_date > sevenDaysFromNow && d.expiry_date <= thirtyDaysFromNow
        ).length,
      };
    },
    enabled: !!vehicleId,
