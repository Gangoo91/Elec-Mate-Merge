...
export function useLocationCache() {
  ...
  const geocodeWithCache = useCallback(
    async (location: string, options: LocationCacheOptions = {}): Promise<CachedLocation> => {
      setIsLoading(true);
  ...
  const reverseGeocodeWithCache = useCallback(
    async (lat: number, lng: number, options: LocationCacheOptions = {}): Promise<string> => {
