...
export const useQuizSession = () => {
  ...
  const startQuiz = useCallback(
    (assessmentId: string, questions: QuizQuestion[], mode: QuizMode = 'practice') => {
  ...
  const submitAnswer = useCallback(
    (questionId: string, selectedAnswerIndex: number, timeSpent: number) => {
  ...
  const toggleBookmark = useCallback(
    (questionId: string) => {
      ...
      const updatedAnswers = currentSession.answers.map((answer) =>
        answer.questionId === questionId ? { ...answer, bookmarked: !answer.bookmarked } : answer
      );
