...
const tabConfigs = [
  {
    id: 'project' as FAG3TabValue,
    label: 'Project',
    shortLabel: 'Project',
    requiredFields: ['premisesAddress'],
  },
  ...
  {
    id: 'declaration' as FAG3TabValue,
    label: 'Declaration',
    shortLabel: 'Sign',
    requiredFields: ['commissionerSignature'],
  },
];
...
export const useFireAlarmG3Tabs = (formData: any) => {
  ...
  const isTabComplete = (tabId: FAG3TabValue): boolean => {
    ...
    switch (tabId) {
      ...
      case 'sound':
        return !!((formData.soundLevelReadings || []).length > 0);
      case 'handover':
        return !!(formData.handoverAsBuiltDrawings && formData.handoverLogBook);
      case 'declaration':
        return !!(formData.commissionerSignature && formData.overallResult);
      default:
        return false;
    }
  };
