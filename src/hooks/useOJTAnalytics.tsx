...
const categorizeActivity = (activity: string): string => {
  ...
  if (
    lowerActivity.includes('install') ||
    lowerActivity.includes('wire') ||
    lowerActivity.includes('cable')
  ) {
  ...
  if (
    lowerActivity.includes('reg') ||
    lowerActivity.includes('7671') ||
    lowerActivity.includes('compliance')
  ) {
    return 'Regulations';
  }
  return 'General';
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const categoryMap = new Map<
        string,
        { totalMinutes: number; thisWeek: number; lastWeek: number; count: number }
      >();
