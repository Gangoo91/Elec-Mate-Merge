...
const UK_POSTCODE_AREAS: Record<string, { lat: number; lng: number }> = {
  ...
  PO: { lat: 50.8198, lng: -1.088 }, // Portsmouth
  ...
  BB: { lat: 53.75, lng: -2.4833 }, // Blackburn
  ...
  BD: { lat: 53.796, lng: -1.7594 }, // Bradford
  WF: { lat: 53.6833, lng: -1.5 }, // Wakefield
  ...
  HA: { lat: 51.5802, lng: -0.334 }, // Harrow
  UB: { lat: 51.5349, lng: -0.4756 }, // Uxbridge
  TW: { lat: 51.449, lng: -0.3252 }, // Twickenham
  SL: { lat: 51.5105, lng: -0.595 }, // Slough
  HP: { lat: 51.7518, lng: -0.5646 }, // Hemel Hempstead
  LU: { lat: 51.8787, lng: -0.42 }, // Luton
  AL: { lat: 51.752, lng: -0.3364 }, // St Albans
...
export const useTrainingCourses = (filters: TrainingCoursesFilters = {}) => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from('training_courses').select('*', { count: 'exact' });
        ...
        switch (filters.sortBy) {
          case 'price':
            processedCourses.sort(
              (a, b) => (a.price_numeric || 999999) - (b.price_numeric || 999999)
            );
            break;
          case 'title':
