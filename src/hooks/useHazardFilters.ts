...
export const useHazardFilters = () => {
  ...
  const stats = useMemo(() => {
    ...
    const riskCounts = allHazards.reduce(
      (acc, h) => {
  ...
  const getFilteredHazards = useCallback(
        ...
        return category.hazards
          .filter((hazard) => {
            ...
            const matchesSearch =
              ...
              hazard.bs7671References?.some((ref: string) =>
            ...
            const matchesWorkType =
              selectedWorkType === 'all' || hazard.workType?.includes(selectedWorkType);

            // Regulation filter
            const matchesRegulation =
              selectedRegulation === 'all' || hazard.bs7671References?.includes(selectedRegulation);
