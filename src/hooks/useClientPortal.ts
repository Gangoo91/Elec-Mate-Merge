  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const stats = {
        ...
        recentlyViewed: data.filter((l) => l.last_accessed_at && l.last_accessed_at > sevenDaysAgo)
          .length,
      };
...
export function useUpdatePortalLink() {
  ...
  return useMutation({
    mutationFn: async ({
      id,
      ...input
    }: UpdatePortalLinkInput & { id: string }): Promise<ClientPortalLink> => {
...
export function useRegenerateToken() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<ClientPortalLink> => {
      // Atomic rotation: swaps the token AND re-keys the job's message thread
      // in one transaction (owner-gated SECURITY DEFINER RPC).
      const { error: rpcError } = await supabase.rpc('regenerate_portal_token', {
        p_link_id: id,
      });
...
export function useDeletePortalLink() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      const { error } = await supabase.from('client_portal_links').delete().eq('id', id);
