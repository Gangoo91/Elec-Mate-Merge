...
export const useFormValidation = () => {
    ...
    earthElectrodeResistance: (value: any): ValidationResult => {
      ...
      if (num < 0.1 || num > 999) {
        return {
          isValid: false,
          level: 'error',
          message: 'Resistance must be between 0.1 and 999 Ω',
        };
      }
      ...
      if (num > 200) {
        return {
          isValid: true,
          level: 'warning',
          message: 'High resistance - verify earthing system',
        };
      }
    ...
    mainBondingSize: (value: any): ValidationResult => {
      ...
      if (!validSizes.some((size) => normalized.includes(size.replace('mm', '')))) {
        return {
          isValid: false,
          level: 'error',
          message: 'Use standard CSA: 6mm, 10mm, 16mm, 25mm, 35mm, or 50mm',
        };
      }
  ...
  const validateField = useCallback(
    (fieldName: string, value: any, rules: ValidationRule[]): ValidationResult => {
