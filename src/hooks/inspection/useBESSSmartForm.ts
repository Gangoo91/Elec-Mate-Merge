...
export function useBESSSmartForm() {
  ...
  const loadCompanyBranding = useCallback((): CompanyBranding | null => {
    ...
    return {
      ...
      ...brandingFromCompanyProfile(companyProfile, '#059669'),
      ...coverPayloadKeys(brandingFromCompanyProfile(companyProfile, '#059669')),
  ...
  const getGridConnectionRequirement = useCallback(
    (
      totalSiteCapacityKW: number,
      phases: 'single' | 'three' = 'single'
    ): GridConnectionRequirement => {
      const g98Limit = phases === 'three' ? 11.04 : 3.68;
      if (totalSiteCapacityKW <= 0) {
        return {
          type: 'none',
          message: 'Enter total site generation capacity',
          details: '',
          warning: false,
        };
      }
  ...
  const getChemistryGuidance = useCallback(
    (chemistry: BatteryChemistry | ''): ChemistryGuidance => {
      const { voltage, minResistance } = getDCTestVoltage(chemistry);
      switch (chemistry) {
        case 'LFP':
          return {
            dcTestVoltage: voltage,
            minResistance,
            ventilationAdvice:
              'Natural ventilation typically sufficient. LFP has low thermal runaway risk.',
            thermalRunawayRisk: 'low',
            safetyNotes:
              'Lithium Iron Phosphate — inherently stable chemistry. No toxic gas release under normal failure modes.',
          };
        case 'NMC':
          return {
            dcTestVoltage: voltage,
            minResistance,
            ventilationAdvice:
              'Consider mechanical ventilation. NMC has higher thermal runaway risk — ensure adequate clearance from combustibles.',
            thermalRunawayRisk: 'high',
            safetyNotes:
              'Nickel Manganese Cobalt — higher energy density but elevated thermal runaway risk. Fire detection and ventilation strongly recommended.',
          };
        case 'NCA':
          return {
            dcTestVoltage: voltage,
            minResistance,
            ventilationAdvice:
              'Mechanical ventilation recommended. NCA has elevated thermal runaway risk.',
            thermalRunawayRisk: 'high',
            safetyNotes:
              'Nickel Cobalt Aluminium — similar risk profile to NMC. Enhanced safety measures recommended.',
          };
        case 'lead-acid':
          return {
            dcTestVoltage: voltage,
            minResistance,
            ventilationAdvice:
              'Ventilation essential — hydrogen gas produced during charging. Do not install in sealed spaces.',
            thermalRunawayRisk: 'low',
            safetyNotes:
              'Lead-acid batteries produce hydrogen gas. Adequate ventilation per BS EN 50272-2 required.',
          };
        default:
          return {
            dcTestVoltage: voltage,
  ...
  const getPMEGuidance = useCallback(
    (earthingArrangement: string, hasGalvanicIsolation: boolean): PMEGuidance => {
      ...
      if (hasGalvanicIsolation) {
        return {
          requiresAction: false,
          recommendation:
            'Inverter provides galvanic isolation — standard PME earthing acceptable for DC side.',
      ...
      return {
        requiresAction: true,
        recommendation:
          'PME earthing with non-isolated inverter — DC earth fault could inject DC into PEN conductor. Action required.',
  ...
  const calculateZs = useCallback(
    (ze: number, r1r2: number, applyTempCorrection: boolean = true) => {
      const tempFactor = applyTempCorrection ? 1.2 : 1.0;
      return {
        calculatedZs: Math.round((ze + r1r2 * tempFactor) * 100) / 100,
        ze,
        r1r2,
  ...
  const validateTestResults = useCallback((formData: any): TestResultValidation[] => {
    ...
    if (formData.acInsulationResistance) {
      ...
      if (!isNaN(ir)) {
        validations.push({
          field: 'acInsulationResistance',
          value: ir,
          isValid: ir >= 1,
          status: ir >= 1 ? 'pass' : 'fail',
          message: ir >= 1 ? `PASS: ${ir}MΩ ≥ 1MΩ` : `FAIL: ${ir}MΩ < 1MΩ`,
          limit: 1,
        });
      }
    }
    ...
    if (formData.dcInsulationResistance && formData.batteryChemistry) {
      ...
      if (!isNaN(dcIr)) {
        validations.push({
          field: 'dcInsulationResistance',
          value: dcIr,
          isValid: dcIr >= min,
          status: dcIr >= min ? 'pass' : 'fail',
          message: dcIr >= min ? `PASS: ${dcIr}MΩ ≥ ${min}MΩ` : `FAIL: ${dcIr}MΩ < ${min}MΩ`,
          limit: min,
        });
      }
    }
    ...
    if (formData.rcdTripTimeIdn) {
      ...
      if (!isNaN(t))
        validations.push({
          field: 'rcdTripTimeIdn',
          value: t,
          isValid: t <= 300,
          status: t <= 300 ? 'pass' : 'fail',
          message: t <= 300 ? `PASS: ${t}ms ≤ 300ms` : `FAIL: ${t}ms > 300ms`,
          limit: 300,
        });
    }
    if (formData.rcdTripTime5xIdn) {
      ...
      if (!isNaN(t5))
        validations.push({
          field: 'rcdTripTime5xIdn',
          value: t5,
          isValid: t5 <= 40,
          status: t5 <= 40 ? 'pass' : 'fail',
          message: t5 <= 40 ? `PASS: ${t5}ms ≤ 40ms` : `FAIL: ${t5}ms > 40ms`,
          limit: 40,
        });
    }
    ...
    if (formData.acPolarity) {
      ...
      validations.push({
        field: 'acPolarity',
        value: formData.acPolarity,
        isValid: pass,
        status: pass ? 'pass' : 'fail',
        message: pass ? 'PASS: Polarity correct' : 'FAIL: Polarity incorrect',
      });
    }
    ...
    if (formData.dcPolarityVerified !== undefined) {
      validations.push({
        field: 'dcPolarityVerified',
        value: formData.dcPolarityVerified,
        isValid: formData.dcPolarityVerified,
        status: formData.dcPolarityVerified ? 'pass' : 'fail',
        message: formData.dcPolarityVerified
          ? 'PASS: DC polarity verified'
          : 'DC polarity not verified',
      });
    }
  ...
  const getPAS63100Compliance = useCallback(
    (formData: any): { errors: string[]; warnings: string[] } => {
      const errors: string[] = [];
      ...
      if (!formData.notInBasementNoAccess)
        errors.push('Confirm battery NOT in basement without external access');

      // Energy limits
      const energyPerEnc = parseFloat(formData.energyPerEnclosure) || 0;
      if (energyPerEnc > 20)
        errors.push(`Max 20 kWh per enclosure — currently ${energyPerEnc} kWh`);
      ...
      if (loc === 'garage' || loc === 'dedicated-enclosure') {
        if (totalEnergy > 80)
          errors.push(`Max 80 kWh in outbuilding/garage — currently ${totalEnergy} kWh`);
      ...
      if (loc === 'outdoor' && distOpenings > 0 && distOpenings < 1)
        warnings.push('Min 1m from windows/doors/vents (outdoor)');
      const distFlam = parseFloat(formData.distanceFromFlammables) || 0;
      if (distFlam > 0 && distFlam < 2)
        warnings.push('Min 2m from flammable materials/fuel storage');

      // Enclosure
      if (!formData.enclosureNonCombustible)
        warnings.push('Enclosure must be non-combustible (PAS 63100)');
      if (!formData.enclosureToolAccessOnly)
        warnings.push('Enclosure access must require tool (PAS 63100)');
      if (!formData.dcFusesToolAccessOnly)
        warnings.push('DC fuses must only be accessible via tool');

      // IK10 for garage/vehicle areas
      if (loc === 'garage' && !formData.ik10Protection)
        warnings.push('IK10 mechanical protection required in garage/vehicle area');

      // Fire detection
      if (!formData.fireDetectionGrade) warnings.push('Grade D2 fire detection required');
      if (!formData.audibleBatteryWarning)
        warnings.push('Audible warning required for dangerous battery condition');

      // Ventilation
      if (loc !== 'outdoor' && !formData.ventilationToOutdoors)
        warnings.push('Interior installation requires ventilation to outdoors');
  ...
  const getEESSClassSuggestion = useCallback(
    (formData: any): { suggested: string; description: string } => {
      const battMfr = (formData.batteryManufacturer || '').toLowerCase();
      const invMfr = (formData.inverterManufacturer || '').toLowerCase();
      if (!battMfr || !invMfr)
        return { suggested: '', description: 'Select battery and inverter manufacturers' };
      if (battMfr === invMfr) {
        const sameEnclosure = formData.installationLocation === 'dedicated-enclosure';
        if (sameEnclosure)
          return {
            suggested: '1',
            description: 'Class 1 — Same manufacturer, single enclosure, no visible DC cable',
          };
        return {
          suggested: '2',
          description: 'Class 2 — Same manufacturer, separate enclosures with DC cable link',
        };
      }
      return {
        suggested: '3',
        description: 'Class 3 — Different manufacturers (verify compatibility)',
      };
    },
    []
  );
