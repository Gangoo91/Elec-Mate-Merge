...
export const useExpensesStorage = () => {
  ...
  const deleteExpense = useCallback(async (expenseId: string): Promise<boolean> => {
    try {
      const { error } = await supabase.from('sole_trader_expenses').delete().eq('id', expenseId);
    ...
    if (filters.period !== 'all') {
      ...
      if (dateRange) {
        filtered = filtered.filter(
          (exp) => exp.date >= dateRange.startDate && exp.date <= dateRange.endDate
        );
      }
    }
  ...
  const dateGroupedExpenses = useMemo((): DateGroup[] => {
    ...
    sorted.forEach((exp) => {
      ...
      if (dateStr === todayStr) {
        ...
        label = dayNames[new Date(dateStr + 'T00:00:00').getDay()];
      } else {
        label = new Date(dateStr + 'T00:00:00').toLocaleDateString('en-GB', {
          month: 'long',
          year: 'numeric',
        });
      }
  ...
  const exportExpenses = useCallback(
    (format: 'xero' | 'sage' | 'quickbooks' | 'csv', expensesToExport?: Expense[]): string => {
  ...
  const downloadExport = useCallback(
    (format: 'xero' | 'sage' | 'quickbooks' | 'csv', expensesToExport?: Expense[]) => {
      ...
      link.setAttribute(
        'download',
        `expenses-${format}-${new Date().toISOString().split('T')[0]}.csv`
      );
