...
interface EmployerContextType {
  ...
  calculateJobLabourCost: (jobId: string) => {
...
const enhanceEmployees = (emps: typeof initialEmployees): Employee[] => {
  ...
  const payRates: Record<string, { day: number; hourly: number }> = {
    '1': { day: 280, hourly: 35 },
    '2': { day: 220, hourly: 27.5 },
    '3': { day: 120, hourly: 15 },
    '4': { day: 300, hourly: 37.5 },
    '5': { day: 240, hourly: 30 },
  ...
  return emps.map((emp) => ({
    ...
    availability: (emp.status === 'On Leave'
      ? 'On Leave'
      : emp.activeJobs > 0
        ? 'On Job'
...
export function EmployerProvider({ children }: { children: ReactNode }) {
  ...
  const [certifications, setCertifications] = useState<Certification[]>(
    initialCertifications as Certification[]
  );
  const [timesheets, setTimesheets] = useState<TimesheetEntry[]>(
    initialTimesheets as TimesheetEntry[]
  );
  ...
  const [applications, setApplications] = useState<VacancyApplication[]>(
    initialApplications as VacancyApplication[]
  );
  ...
  // Holiday/Leave State
  const [holidayAllowances, setHolidayAllowances] =
    useState<HolidayAllowance[]>(initialHolidayAllowances);
  ...
  const deleteEmployee = useCallback((id: string) => {
    setEmployees((prev) =>
      prev.map((emp) => (emp.id === id ? { ...emp, status: 'Archived' } : emp))
    );
  }, []);

  const setEmployeeAvailability = useCallback((id: string, availability: AvailabilityStatus) => {
    setEmployees((prev) => prev.map((emp) => (emp.id === id ? { ...emp, availability } : emp)));
  }, []);

  const addWorkerNote = useCallback(
    (employeeId: string, content: string, type: WorkerNote['type'], authorName: string) => {
  ...
  const setEmployeeRating = useCallback((id: string, rating: number) => {
    setEmployees((prev) => prev.map((emp) => (emp.id === id ? { ...emp, rating } : emp)));
  }, []);

  const updateEmergencyContact = useCallback((id: string, contact: EmergencyContact) => {
    setEmployees((prev) =>
      prev.map((emp) => (emp.id === id ? { ...emp, emergencyContact: contact } : emp))
    );
  }, []);

  const updatePayRates = useCallback((id: string, dayRate: number, hourlyRate: number) => {
    setEmployees((prev) =>
      prev.map((emp) => (emp.id === id ? { ...emp, dayRate, hourlyRate } : emp))
    );
  }, []);
  ...
  const bulkAssignToJob = useCallback(
    (jobId: string, jobTitle: string, jobLocation: string, startDate: string) => {
      selectedEmployeeIds.forEach((employeeId) => {
        ...
        setEmployees((prev) =>
          prev.map((emp) =>
            emp.id === employeeId
              ? { ...emp, activeJobs: emp.activeJobs + 1, availability: 'On Job' }
              : emp
          )
        );
      });
  ...
  const addCertification = useCallback(
    (certification: Omit<Certification, 'id' | 'daysRemaining'>) => {
      ...
      setEmployees((prev) =>
        prev.map((emp) =>
          emp.id === certification.employeeId
            ? { ...emp, certifications: emp.certifications + 1 }
            : emp
        )
      );
    },
      ...
      prev.map((cert) => {
        if (cert.id === id) {
          ...
          if (updates.expiryDate) {
            ...
            const today = new Date();
            updated.daysRemaining = Math.ceil(
              (expiryDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
            );
            updated.status =
              updated.daysRemaining < 0
                ? 'Expired'
                : updated.daysRemaining < 60
                  ? 'Warning'
                  : 'Active';
          }
          return updated;
  ...
  const approveTimesheet = useCallback((id: string) => {
    setTimesheets((prev) => prev.map((ts) => (ts.id === id ? { ...ts, status: 'Approved' } : ts)));
  }, []);
    ...
    setLeaveRequests((prev) =>
      prev.map((lr) => {
        if (lr.id === id && lr.status === 'pending') {
          ...
          setHolidayAllowances((ha) =>
            ha.map((a) =>
              a.employeeId === lr.employeeId
                ? {
                    ...a,
                    usedDays: a.usedDays + lr.totalDays,
                    pendingDays: Math.max(0, a.pendingDays - lr.totalDays),
                  }
                : a
            )
          );
  ...
  const getEmployeeAllowance = useCallback(
    (employeeId: string) => {
  ...
  const calculateJobLabourCost = useCallback(
    (jobId: string) => {
      const jobTimesheets = timesheets.filter((ts) => ts.jobId === jobId);
      const getHourlyRate = (empId: string) =>
        employees.find((e) => e.id === empId)?.hourlyRate || 25;
      ...
      return {
        ...
        approvedCost: approved.reduce(
          (sum, ts) => sum + ts.totalHours * getHourlyRate(ts.employeeId),
          0
        ),
        pendingHours: pending.reduce((sum, ts) => sum + ts.totalHours, 0),
        pendingCost: pending.reduce(
          (sum, ts) => sum + ts.totalHours * getHourlyRate(ts.employeeId),
          0
        ),
      };
  ...
  const updateVacancy = useCallback((id: string, updates: Partial<JobVacancy>) => {
    setVacancies((prev) => prev.map((vac) => (vac.id === id ? { ...vac, ...updates } : vac)));
  }, []);

  const closeVacancy = useCallback((id: string) => {
    setVacancies((prev) => prev.map((vac) => (vac.id === id ? { ...vac, status: 'Closed' } : vac)));
  }, []);
  ...
  const addToLabourBank = useCallback(
    (electricianId: string, electricianName: string, dayRate: number, hourlyRate: number) => {
  ...
  const removeEmployeeFromJob = useCallback((assignmentId: string) => {
    setJobAssignments((prev) => {
      ...
      return prev.map((a) => (a.id === assignmentId ? { ...a, status: 'Removed' as const } : a));
    });
  }, []);
  ...
  const assignDocument = useCallback(
    (employeeId: string, documentType: string, documentName: string, dueDate: string) => {
