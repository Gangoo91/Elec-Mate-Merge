  ...
  return (
    <div className="bg-gradient-to-br from-background via-background/98 to-background/95">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-3 gap-3">
          {toolCards.map((tool) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-2 rounded-lg ${tool.bgColor} inline-block mb-2`}
                >
                  <Icon className={`h-5 w-5 ${tool.color}`} />
                </div>
                <div
                  className={`text-sm font-semibold ${isActive ? tool.color : "text-white"}`}
                >
                  {tool.label}
                </div>
        ...
        {/* Active Tool Content */}
        {activeTool === "testing" && (
          <TestingProceduresPanel progress={progress} />
        )}
