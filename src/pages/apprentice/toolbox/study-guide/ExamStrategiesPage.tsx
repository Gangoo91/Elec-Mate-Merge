import { SmartBackButton } from '@/components/ui/smart-back-button';
import { Card, CardContent } from '@/components/ui/card';
import { CheckCircle } from 'lucide-react';
import ExamStrategiesTab from '@/components/apprentice/study-tips/ExamStrategiesTab';

const ExamStrategiesPage = () => {
  return (
    <div className="animate-fade-in max-w-2xl mx-auto px-4 pb-20 space-y-6 text-left">
      <div className="flex items-center gap-3">
        <SmartBackButton />
        <h1 className="text-2xl font-bold tracking-tight text-white">
          Exam Strategies
        </h1>
      </div>

      <Card className="border-green-500/20 bg-white/5">
        <CardContent className="p-4 space-y-4">
          <h2 className="text-lg font-semibold text-white">
            Exam Day Success
          </h2>
          <p className="text-white text-sm leading-relaxed">
            Passing your electrical exams requires more than just knowledge —
            you need a strategy. Learn how to manage exam time, tackle
            different question types, and stay calm under pressure. These
            techniques work for the 18th Edition, City & Guilds, EAL, and
            AM2 assessments.
          </p>

          <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
            <h3 className="text-green-400 font-semibold text-sm mb-3">
              What You Will Learn
            </h3>
            <ul className="space-y-2">
              {[
                'Time management strategies for timed exams',
                'How to approach multiple-choice questions',
                'Dealing with questions you cannot answer',
                'Managing exam anxiety and staying focused',
                'The night-before and morning-of routines that work',
              ].map((item) => (
                <li
                  key={item}
                  className="flex items-center gap-2 text-sm text-white"
                >
                  <CheckCircle className="h-4 w-4 text-green-400 flex-shrink-0" />
                  {item}
                </li>
              ))}
            </ul>
          </div>
        </CardContent>
      </Card>

      <ExamStrategiesTab />
    </div>
  );
};

export default ExamStrategiesPage;
