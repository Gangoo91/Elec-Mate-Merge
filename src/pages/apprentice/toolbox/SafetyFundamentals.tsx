...
const SafetyFundamentals = () => {
  ...
  const ppeItems = [
    ...
    },
    {
      ...
      requirement: 'Class 0 or higher for voltage',
      standard: 'BS EN 60903',
    },
    {
      item: 'High-vis vest',
      requirement: 'Class 2 or 3 depending on location',
    ...
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 animate-fade-in px-4 sm:px-6 lg:px-8 pb-20">
      ...
      <Card className="border-elec-yellow/30 bg-gradient-to-br from-elec-yellow/10 to-elec-yellow/5">
        <CardHeader>
          ...
          <p className="text-white text-sm">
            Fundamental safety rules that must never be compromised
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {safetyPrinciples.map((principle, index) => (
              <div
                key={index}
                className="bg-white/5 border border-white/10 p-4 rounded-lg hover:border-elec-yellow/30 transition-all"
              >
        ...
        <CardContent>
          <div className="space-y-3">
            {isolationSteps.map((step, index) => (
              <div
                key={index}
                className="bg-white/5 border border-red-500/20 p-4 rounded-lg hover:border-red-500/40 transition-all"
              >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {emergencyProcedures.map((procedure, index) => (
              <div
                key={index}
                className="bg-white/5 border border-orange-500/20 p-4 rounded-lg hover:border-orange-500/40 transition-all"
              >
                <div className="flex items-start gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="border-elec-yellow/20 bg-white/5">
          ...
          <CardContent>
            <p className="text-white text-sm">
              High-risk electrical work requires permits. Understand when permits are needed, how to
              obtain them, and never begin work without proper authorisation.
            </p>
          </CardContent>
        </Card>
      ...
      <Card className="border-red-500/40 bg-gradient-to-br from-red-500/15 to-orange-500/10">
        ...
        <CardContent className="space-y-4">
          <div className="text-center p-4 bg-red-500/20 rounded-lg border border-red-500/40">
            ...
            <p className="text-white text-sm">
              Even low voltages can be fatal in the right circumstances. Never take shortcuts with
              safety procedures.
            </p>
          </div>
      ...
      <Card className="border-green-500/30 bg-gradient-to-br from-green-500/10 to-emerald-500/5">
        ...
        <CardContent className="space-y-4">
          <p className="text-white leading-relaxed">
            Safety isn't just about following rules - it's about going home to your family every
            day. The habits you form as an apprentice will stay with you throughout your career.
            Make safety your first instinct.
          </p>
