        ...
        {studyResources.map((resource, index) => (
          <div key={index} className="bg-card/50 rounded-lg p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <div className="p-1.5 sm:p-2 rounded-lg bg-primary/10">{resource.icon}</div>
              <h3 className="text-xs sm:text-sm font-semibold text-foreground">{resource.title}</h3>
            </div>
            <p className="text-[10px] sm:text-xs text-white mb-2 line-clamp-2">
        ...
        {examTypes.map((exam, index) => (
          <div key={index} className="bg-card/50 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className={`text-[10px] sm:text-xs font-medium ${getDifficultyColor(exam.difficulty)}`}
              >
                {exam.difficulty}
              </span>
            </div>
            <p className="text-white text-[10px] sm:text-xs mb-2">{exam.description}</p>
            <div className="flex items-center gap-1 mb-3">
              <Clock className="h-3 w-3 text-white" />
              <span className="text-[10px] sm:text-xs text-white">{exam.duration}</span>
            </div>

            <div className="mb-3">
              <p className="font-medium text-foreground mb-1.5 text-[10px] sm:text-xs">
                Key Topics:
              </p>
              <div className="flex flex-wrap gap-1">
                {exam.topics.map((topic, topicIndex) => (
                  <span
                    key={topicIndex}
                    className="text-[10px] sm:text-xs px-1.5 py-0.5 rounded bg-primary/10 text-primary"
                  >
                    {topic}
                  </span>
                ))}
            ...
            <div className="mb-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <span
                  className={`text-[10px] sm:text-xs font-medium ${getProgressColor(exam.progress)}`}
                >
                  {exam.progress}%
                </span>
              </div>
            ...
            <Link to={exam.link}>
              <Button className="w-full text-sm h-11 touch-manipulation">Start Preparation</Button>
            </Link>
          </div>
        ))}
      </div>
      ...
      <div className="bg-card/50 rounded-lg p-3 sm:p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <ul className="space-y-1.5 text-[10px] sm:text-xs text-white">
