...
const CraftSkillsContent = () => {
  ...
  const handleSubsectionClick = (subsection: Subsection) => {
    if (courseSlug && unitSlug && sectionId) {
      navigate(
        `/apprentice/study/eal/${courseSlug}/unit/${unitSlug}/craft-skills/${sectionId}/subsection/${subsection.id}`
      );
    }
  };
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <div className="text-xl font-bold text-elec-yellow mr-4">{subsection.id}</div>
                  <div className="text-xl font-medium">{subsection.title}</div>
                </div>
