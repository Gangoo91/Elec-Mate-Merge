...
const DOCUMENT_COLORS: Record<string, string> = {
  ...
  driving_licence: "text-purple-400 bg-purple-500/20",
...
export default function AdminDocumentReview() {
  ...
  const { data: documents, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (statusFilter === "needs_attention") {
        ...
        query = query.or("verification_status.eq.pending,verification_status.eq.needs_review,verification_status.eq.appealed,flagged_for_review.eq.true");
      ...
      if (statusFilter === "needs_attention") {
        filtered.sort((a, b) => {
          ...
          if (!a.flagged_for_review && b.flagged_for_review) return 1;
          // Then appealed
          if (a.verification_status === "appealed" && b.verification_status !== "appealed") return -1;
          if (a.verification_status !== "appealed" && b.verification_status === "appealed") return 1;
          // Then oldest first
  ...
  const { data: stats, refetch: refetchStats } = useQuery({
    ...
    staleTime: 0,
    queryFn: async () => {
      const [needsAttentionRes, pendingRes, flaggedRes, appealedRes, verifiedRes, rejectedRes, totalRes] = await Promise.all([
        supabase.from("elec_id_documents").select("*", { count: "exact", head: true })
          .or("verification_status.eq.pending,verification_status.eq.needs_review,verification_status.eq.appealed,flagged_for_review.eq.true"),
        ...
        supabase.from("elec_id_documents").select("*", { count: "exact", head: true }).eq("verification_status", "appealed"),
  ...
  // Selection helpers
  const needsAttentionDocs = documents?.filter(
  ...
  const selectAll = () => {
    setSelectedIds(new Set(needsAttentionDocs.map((d) => d.id)));
  };
      ...
      <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-sm border-b border-white/10">
        <div className="p-4 sm:p-6">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:grid sm:grid-cols-4 lg:grid-cols-7">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-foreground/60 uppercase tracking-wide">Attention</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-foreground/60 uppercase tracking-wide">Pending</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-foreground/60 uppercase tracking-wide">Flagged</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-2">
                <Flag className={cn(
                  "h-5 w-5",
                  statusFilter === "appealed" ? "text-purple-400" : "text-purple-400/70"
                )} />
                <div className="text-left">
                  <p className={cn(
                    "text-xl font-bold",
                    statusFilter === "appealed" ? "text-purple-400" : "text-foreground"
                  )}>
                    {stats?.appealed || 0}
                  </p>
                  <p className="text-[10px] text-foreground/60 uppercase tracking-wide">Appeals</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-foreground/60 uppercase tracking-wide">Verified</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-foreground/60 uppercase tracking-wide">Rejected</p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-foreground/60 uppercase tracking-wide">Total</p>
                </div>
              </div>
            </button>
          </div>
          ...
          <div className="mt-4 flex flex-col sm:flex-row gap-3">
            ...
            {statusFilter === "needs_attention" && needsAttentionDocs.length > 0 && (
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  className="h-10 touch-manipulation gap-2"
                  onClick={selectedIds.size === needsAttentionDocs.length ? clearSelection : selectAll}
                >
                  {selectedIds.size === needsAttentionDocs.length ? (
                    <>
        ...
        {isLoading ? (
          ...
          <>
            {documents?.map((doc) => {
              ...
              const iconColors = DOCUMENT_COLORS[doc.document_type] || "text-gray-400 bg-gray-500/20";
              const statusConfig = STATUS_CONFIG[doc.verification_status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG.pending;
              const StatusIcon = statusConfig.icon;
              const isSelectable = statusFilter === "needs_attention" && doc.verification_status !== "verified" && doc.verification_status !== "rejected";
              ...
              return (
                <Card
                  ...
                  className={cn(
                    ...
                    doc.flagged_for_review && !selectedIds.has(doc.id) && "border-amber-500/30 bg-amber-500/5",
                    doc.verification_status === "appealed" && !selectedIds.has(doc.id) && "border-purple-500/30 bg-purple-500/5",
                    !doc.flagged_for_review && doc.verification_status !== "appealed" && !selectedIds.has(doc.id) && "bg-white/[0.02] border-white/10 hover:border-white/20"
                  )}
                >
                  <CardContent className="p-0">
                    <div className="flex items-stretch">
                      ...
                      >
                        {/* Document type icon */}
                        <div className={cn("p-2.5 rounded-xl flex-shrink-0", iconColors.split(" ")[1])}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge className={cn("text-[10px] border", statusConfig.color)}>
                              <StatusIcon className={cn("h-3 w-3 mr-1", doc.verification_status === "processing" && "animate-spin")} />
                              {statusConfig.label}
                            </Badge>
                          ...
                          {(doc.flag_reason || doc.appeal_notes) && (
                            <p className={cn(
                              ...
                              doc.appeal_notes ? "text-purple-400" : "text-amber-400"
                            )}>
              ...
              <SheetTitle className="flex items-center gap-3">
                {selectedDocument && (
                  <>
                    <div className={cn(
                      "p-2.5 rounded-xl",
                      DOCUMENT_COLORS[selectedDocument.document_type]?.split(" ")[1] || "bg-gray-500/20"
                    )}>
                      {(() => {
                        const Icon = DOCUMENT_ICONS[selectedDocument.document_type] || FileCheck;
                        return <Icon className={cn(
                          "h-5 w-5",
                          DOCUMENT_COLORS[selectedDocument.document_type]?.split(" ")[0] || "text-gray-400"
                        )} />;
                      })()}
                    </div>
                    <div className="text-left">
            ...
            <div className="flex-1 overflow-y-auto">
              {selectedDocument && (
                <div className="p-4 space-y-4">
                  ...
                  {(selectedDocument.flagged_for_review || selectedDocument.verification_status === "appealed") && (
                    <div className={cn(
                      ...
                      <div className="flex items-start gap-2">
                        <AlertTriangle className={cn(
                          "h-4 w-4 mt-0.5",
                          selectedDocument.verification_status === "appealed" ? "text-purple-400" : "text-amber-400"
                        )} />
                        <div>
                          <p className={cn(
                            "text-sm font-medium",
                            ...
                            {selectedDocument.verification_status === "appealed" ? "User Appeal" : "Flagged for Review"}
                          </p>
                  ...
                  {selectedDocument.rejection_reason && selectedDocument.verification_status !== "rejected" && (
                    <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/30">
                      <h4 className="text-xs font-medium text-red-400 uppercase tracking-wide mb-1">
                        Previous Rejection
                      </h4>
                      <p className="text-sm text-foreground/80">{selectedDocument.rejection_reason}</p>
                    </div>
                  )}
                      ...
                      {reviewAction && (
                        <div className="space-y-2">
                          <Label className="text-sm text-foreground/70">
                            {reviewAction === "approved" ? "Notes (optional)" : "Reason (required for rejection)"}
                          </Label>
                          <Textarea
                            value={reviewNotes}
                            ...
                            className="bg-white/5 border-white/20 min-h-[80px] text-base touch-manipulation"
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-green-500/10 border border-green-500/30 text-center">
                      ...
                      {selectedDocument.reviewed_at && (
                        <p className="text-xs text-foreground/60 mt-1">
                          Reviewed {formatDistanceToNow(new Date(selectedDocument.reviewed_at))} ago
                        </p>
                      )}
                    </div>
                  )}
              ...
              <div className="flex-shrink-0 p-4 border-t border-white/10 bg-background">
                <div className="flex gap-3">
                  ...
                  <Button
                    ...
                    disabled={!reviewAction || reviewMutation.isPending || (reviewAction !== "approved" && !reviewNotes.trim())}
                    className={cn(
                  ...
                  >
                    {reviewMutation.isPending ? (
                      ...
                      <>
                        ...
                        {reviewAction === "request_reupload" && <RefreshCw className="h-4 w-4 mr-2" />}
                        {reviewAction ? `Confirm ${reviewAction === "request_reupload" ? "Request" : reviewAction.charAt(0).toUpperCase() + reviewAction.slice(1)}` : "Select Action"}
                      </>
                    )}
                  </Button>
                </div>
      ...
      <AlertDialog open={showBulkApproveDialog} onOpenChange={setShowBulkApproveDialog}>
        <AlertDialogContent className="bg-background border-white/20">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will mark {selectedIds.size} documents as verified. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
