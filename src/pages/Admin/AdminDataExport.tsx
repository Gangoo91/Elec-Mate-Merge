...
const EXPORT_CONFIGS: ExportConfig[] = [
  {
    ...
    fields: ["id", "email", "full_name", "username", "role", "subscribed", "subscription_tier", "created_at"],
  },
  {
    table: "vacancies",
    name: "Job Vacancies",
    ...
    fields: ["id", "employer_id", "title", "description", "location", "salary_min", "salary_max", "is_active", "created_at"],
  },
  {
    table: "conversations",
    name: "Conversations",
...
export default function AdminDataExport() {
  ...
  const exportMutation = useMutation({
    mutationFn: async () => {
      ...
      let query = supabase
        .from(selectedConfig.table as any)
      ...
      await supabase.from("admin_audit_logs").insert({
        ...
        new_values: { fields: selectedFields, format: exportFormat, dateRange, rowCount: data?.length },
      });
          ...
          return (
            <Card
              ...
              className={`touch-manipulation transition-all ${
                isSuperAdmin ? "active:scale-[0.99] cursor-pointer hover:border-primary/50" : "opacity-60"
              }`}
              ...
              <CardContent className="pt-4 pb-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">{tableCounts?.[config.table]?.toLocaleString() || 0} rows</Badge>
                ...
                <CardContent className="pt-4 pb-4">
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>{tableCounts?.[selectedConfig?.table || ""]?.toLocaleString() || 0}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
