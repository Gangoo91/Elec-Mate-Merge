...
const BS7671Module5Section5 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 5 · Section 5 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            "How embedded sources — PV, battery storage and V2G — must connect, protect and disconnect from the LV network. Reg 551.7.1's new indents (c) and (d), Reg 530.3.201 bidirectional consideration, Section 826 prosumer requirements and how BS 7671 dovetails with ENA G98 / G99."
          }
        </div>
        ...
        >
          <p>
            ...
            reference but has not yet detected that and stopped exporting. Anti-islanding protection
            is the inverter feature that detects LoM within a defined time window (per BS EN 50549)
            and disconnects the inverter from the grid before any of the hazards below have time to
            develop.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Of the three, repair-crew safety is the headline driver. The other two are equipment and
            continuity-of-supply concerns; they motivate the DNO and the inverter manufacturer, but
            it is the prospect of a fatal shock to a network engineer working on what he believed to
            be a dead feeder that gets the regulators moving. Every BS EN 50549 and G98 / G99
            protection setting traces back to that primary risk.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            the trip times and the reconnection logic. The DNO does not re-verify each installation;
            they trust the type-test certificate that ships with each compliant inverter. That is
            why bypassing or overriding the inverter&rsquo;s default protection settings during
            commissioning is a hard prohibition: it invalidates the type-test and removes the basis
            on which the DNO accepted the connection.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The G99 acceptance letter is binding: any extra protection it requires must be in place
            before energising. Common requirements for medium-sized G99 installs include an external
            G59-style relay (in addition to the inverter&rsquo;s own protection), an external
            isolator accessible to the DNO, and explicit voltage-rise calculations showing the
            export will not push terminal voltage above the BS EN 50160 +10% statutory limit. The
            cert-form and the customer-handover pack must include a copy of the G99 acceptance.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Passive techniques are reliable when the island&rsquo;s load is significantly imbalanced
            from the inverter&rsquo;s output: voltage and frequency drift away quickly and the
            stages catch them. Their weakness is the &ldquo;balanced island&rdquo; case — where the
            local load happens to match the inverter&rsquo;s output exactly at the moment of
            disconnection. Voltage and frequency might stay briefly close to nominal, defeating the
            passive detector. That is why every BS EN 50549 inverter combines passive techniques
            with at least one active technique.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            but can trip during ordinary network events (a large generator trips elsewhere; the bulk
            system briefly drops in frequency before recovering — a ROCOF-sensitive inverter sees
            that as an island). Cascading inverter trips during single-event disturbances were a
            real problem with G59 settings and motivated the G98 / G99 update to less aggressive
            ROCOF combined with vector shift. The inverter&rsquo;s type-test fixes the algorithm
            mix; field changes are forbidden.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The two sets of rules are independent in the sense that you can comply with one and fail
            the other. A perfectly G98-compliant inverter, wired with the wrong RCD type and
            ...
            overridden in software fails G98 even though the wiring is perfect. The cert and the G98
            / G99 form together evidence both halves.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The practical consequence: when adding any source to an existing installation, walk the
            affected circuits and re-evaluate every device on them. Often the cleanest solution is
            to fit a dedicated &ldquo;PEI distribution board&rdquo; — a separate CU in which all the
            source(s), the bidirectional protection and the prosumer metering live — fed from a
            dedicated way upstream of the existing main CU. That keeps the existing load-only CU
            unchanged and isolates the bidirectional zone to a single, clearly identified piece of
            equipment with the right protection.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            management (PV strings, AC export cabling, battery DC), to labelling (every source must
            be identified on its own warning notice), and to the EICR observation codes when an
            existing PEI does not meet the current edition.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            its own protection regime (no traditional ADS, since the source is the PV string itself;
            instead surge protection, double / reinforced insulation and physical isolation).
            Confusion between the two regimes is a common source of EICR observations on older PV
            installs.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The hard requirement: the changeover must be physical (a contactor or motorised switch),
            not software. A software-only attempt to &ldquo;not export during a grid-outage&rdquo;
            fails BS 7671 Reg 551.7 and the G98 / G99 type-test, because there is no fail-closed
            guarantee that the inverter will not back-feed the dead grid. Every UK-compliant battery
            system with EPS capability ships with a built-in ATS or requires an external one to be
            wired during installation. The single-line diagram in the cert must show that ATS and
            the EPS sub-board it feeds.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The conceptual leap that catches installers out: the EV is mobile equipment, but the V2G
            charge point is fixed wiring, and the moment the EV plugs in and exports the FIXED
            installation behaves as a PEI. The cert must declare PEI; Section 826 applies; the G98 /
            G99 registration must be filed for the V2G charge point&rsquo;s export rating. Treating
            it &ldquo;like a normal EV charger&rdquo; misses every one of those obligations and
            produces an installation that may be perfectly safe in charging mode but non-compliant
            the moment it is asked to export.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            applications mandate an explicit voltage-rise calculation as part of the technical data
            submitted to the DNO. G98 fit-and-inform leaves the calculation to the installer&rsquo;s
            good practice — but the consequence (repeated nuisance trip-and-reconnect cycles,
            customer complaints, the DNO eventually flagging the site for excessive harmonic /
            voltage-quality contribution) is the installer&rsquo;s to deal with after the event.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            RCD type, missing PEI declaration on the cert — each is an observation in its own right.
            Most existing PEIs designed pre-A4 will have at least one of these flagged on the next
            periodic inspection; the inspector&rsquo;s code (C2 vs C3) depends on whether the issue
            presents an actual hazard or just a departure from the current edition.
          </p>
        </ConceptBlock>
