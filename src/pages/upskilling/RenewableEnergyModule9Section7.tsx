...
const inlineChecks = [
  {
    ...
    options: [
      'Same turbine',
      'Pelton: high head (50m+), low flow — water jet hits buckets on a wheel. Francis: medium head (10-100m), medium flow — reaction turbine; most common at small commercial scale. Crossflow: low head (5-50m), variable flow — water passes through twice; rugged + simple. Archimedes screw: very low head (1-10m), high flow — fish-friendly + sediment-tolerant; rivers + weirs',
      'Random',
      'Only Pelton',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'No licence needed',
      'Required for any water abstraction from a watercourse — including hydropower (water diverted through turbine and returned). Environment Agency (England) / Natural Resources Wales / SEPA (Scotland) administer. Application: typically 3-12 months; cost variable; assesses flow impact, ecology, downstream users. Eel Regulations 2009: screening at intake to prevent eel kill. Without licence: illegal abstraction; substantial fines + remediation order',
      'Customer choice',
      'Random',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'All hydro',
      'MCS Installer Standard 3008 — micro-hydro ≤50 kW. Sizing methodology (hydrological assessment, head + flow measurement, capacity factor estimation), product approval (MCS-approved turbines + inverters), installer competence, customer handover documentation. Required for grant eligibility (where applicable). Larger hydro (>50 kW) falls outside MIS 3008 into commercial-scale frameworks',
      'Random',
      'No standard',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Mainstream',
      'Hydrogen for heating: predominantly POLICY DIRECTION + LIMITED TRIALS, not widespread. HyDeploy 20% blend trials at Keele + Winlaton; hydrogen heating village trials (Whitby, Redcar) cancelled / scaled back; UK strategic decision deferred. Hydrogen-ready boilers + heat pumps marketed; actual hydrogen supply unavailable at scale. Fuel cells: niche premium (PEM + SOFC variants) — Viessmann Vitovalor, BlueGen. Section 551 applies if generating electricity. Track regulatory direction',
      'Random',
      'Banned',
    ],
    correctIndex: 1,
...
const quizQuestions = [
  {
    ...
    options: [
      'Just install',
      'Multi-trade major project. MCS company (MIS 3008) + civils contractor (intake structure + pipework / penstock + tailrace return + powerhouse) + turbine specialist + electrical installer + Environment Agency consultant (abstraction licence) + ecologist (Habitats / Eel Regs assessments) + DNO (EREC G99). 12-24 month timeline; £80-150k project. Electrical scope: dedicated three-phase circuit + Section 551 + Reg 551.7.5 + dump load + long cable from powerhouse to building',
      'Random',
      'Customer DIY',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'No impact',
      'Eel Regs 2009: intake screen mandatory at any water abstraction — typical 12mm bar spacing maximum (prevents eel entrainment). Salmon + Freshwater Fisheries Act + Habitats Regs: fish passage requirements for salmonid + protected species watercourses. Practical: intake screen + fish ladder / bypass + Hands-Off Flow (HOF) ensuring minimum flow remains in main channel. Substantial additional install scope; ecological assessment + monitoring',
      'Random',
      'No species concerns',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Same as wind',
      'Same Section 551.7.5 principle: when grid lost (anti-islanding triggers) BUT water still flowing through turbine, generator continues producing power → without load → over-speed → safety risk. Dump load dissipates the power. Hydro vs wind: water flow is much more constant than wind, so dump load load is more sustained continuous when grid lost. Some hydro installs include mechanical penstock valve closure as additional protection',
      'No dump load',
      'Random',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Same as house wiring',
      'Substantial run typical (50-500m powerhouse to grid-tie point + AC supply panel). Reg 525.202 voltage drop ≤5% is the limiting factor; Reg 411.4 ADS + Zs verified at outdoor unit. UK 2025-26 typical 15-30 kW three-phase hydro: 35-70 mm² SWA at 200-500m run. Substantial cable + civils for trenching. Often the largest single material cost in the install',
      'Bare wire',
      'Random',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Mainstream',
      'Manufacturer-tested for 20% hydrogen blend in natural gas (per HyDeploy spec); some tested for 100% pure hydrogen with firmware / hardware update. Future-proofing value if UK hydrogen rollout proceeds. Customer\'s actual gas supply is natural gas; hydrogen-ready capability is dormant until / unless supply changes. Honest installer position: future-proofing investment, not current operational difference. Track regulatory direction',
      'Random',
      'Imaginary',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Random',
      'Some emerging deployment beyond micro-CHP residential (§9.5): larger commercial fuel cells 5-200 kWe (Bloom Energy, FuelCell Energy SOFCs), backup power UPS-replacement fuel cells, hydrogen-fed fuel cells for specialist applications (data centres, hospitals, electric vehicle fast-charging support). Section 551 applies (Reg 551.1.1(f) other suitable sources). EREC G99 typical. Substantially higher upfront cost vs IC engine CHP',
      'No commercial fuel cells',
      'Mainstream',
    ],
    correctAnswer: 1,
    explanation:
