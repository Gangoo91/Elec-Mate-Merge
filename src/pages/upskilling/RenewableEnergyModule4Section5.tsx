...
const inlineChecks = [
  ...
  },
  {
    ...
    question: "Reg 826.1.1.2 — earthing arrangement for PEI island mode. What's required?",
    options: [
  ...
  },
  {
    ...
    question: 'Reg 826.1.3 — what does the PEI do when the public network goes off?',
    options: [
...
const quizQuestions = [
  ...
  {
    id: 7,
    question: 'Cert evidence bundle for a PEI hybrid PV+BESS install — what documents?',
    options: [
  ...
  {
    id: 8,
    question: 'When did Chapter 82 (PEIs) become mandatory for new installs?',
    options: [
...
export default function RenewableEnergyModule4Section5() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 5 · BS 7671:2018+A4:2026 · Chapter 82 NEW"
        title="Chapter 82 — Prosumer's Electrical Installations (PEIs)"
        backTo="../renewable-energy-module-4"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The NEW A4:2026 chapter for installs that BOTH consume AND generate / store. Reg 824.2
          operating modes (direct feeding / island); Reg 826.1.1.1-826.1.4 PEI design framework;
          multi-source isolation, bidirectional OCPDs, transient protection; the integration layer
          above Section 712 (PV) and Chapter 57 (BESS).
        </p>
        ...
        <Pullquote>
          PEI = consumes + generates / stores. Chapter 82 ties PV (Section 712) + BESS (Chapter 57)
          + multi-source coordination into one prosumer design.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Hybrid PV+BESS (any topology)</strong> — clearest
              example. Consumes (loads) + generates (PV) + stores (BESS)
            </li>
            <li>
              <strong className="text-white">PV + integrated EV charging</strong> — where the EV
              charger is PV-controlled (Zappi, Ohme, integrated PV-aware charger). The install
              consumes (EV + other loads) and generates (PV) with coordination
            </li>
            <li>
              <strong className="text-white">PV + heat pump with smart-grid coordination</strong> —
              heat pump with SG-Ready input controlled by PV surplus diverter is a PEI
            </li>
            <li>
              <strong className="text-white">Off-grid PV + BESS + generator</strong> — multi-source,
              all generation / storage, no grid. PEI by definition
            </li>
            <li>
              <strong className="text-white">Commercial multi-source installs</strong> — PV + wind +
              BESS + generator. Classic PEI scenario
            </li>
            <li>
              <strong className="text-white">PV ONLY (no storage, no controlled load)</strong> —
              boundary case. Section 712 applies; Chapter 82 arguably doesn\'t apply in the
              strictest sense (pure PV → grid + load without coordination logic). In practice, most
              modern UK PV installs include some coordination — treat as PEI in doubt
            </li>
            <li>
              <strong className="text-white">Pure consumer (no generation, no storage)</strong> —
              most existing UK homes. NOT a PEI
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          THREE PEI types: individual, collective, shared. The type sets the protection scope.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">(a) Individual PEI</strong> — single end-user
              installation. One household / business with its own PV / BESS / EV. Single connection
              to the public network. Vast majority of UK domestic and SME hybrid installs. Section
              712, Chapter 57, Chapter 82 all apply at the single-user scope
            </li>
            <li>
              <strong className="text-white">(b) Collective PEI</strong> — multiple end-users share
              a single connection point and shared resources. Typical: a multi-dwelling block with
              shared rooftop PV + landlord-owned communal BESS. Tariffs / metering allocate energy
              to occupants. Chapter 82 applies at the collective scope; tenant sub-installs are
              separate consumer units
            </li>
            <li>
              <strong className="text-white">(c) Shared PEI</strong> — multiple end-users each with
              their OWN connection points share local generation / storage. Energy is routed between
              them via an internal network and the public network. Community energy / microgrid
              pattern. Chapter 82 + DNO collaboration required; emerging in the UK
            </li>
            <li>
              <strong className="text-white">Practical implication</strong> — the design pack
              identifies which PEI type, then applies Reg 826 design content at the correct scope.
              Confusion between types = design pack faults at audit
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Direct feeding + reverse feeding + island. PEI must support all intended modes.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">(1) DIRECT FEEDING MODE</strong> — PEI receives energy
              from the public network. Net energy flow: DNO → PEI. Loads are partly or fully
              supplied by import. Anti-islanding per EREC G98 / G99 (Module 3 Section 6) on the
              inverter side ensures PEI does not energise a fault-free DNO that has gone open
            </li>
            <li>
              <strong className="text-white">(2) REVERSE FEEDING MODE</strong> — PEI exports energy
              to the public network. Net energy flow: PEI → DNO. PV / BESS surplus exported.
              Anti-islanding still required (the PEI must not continue energising a de-energised
              DNO). EREC G98 / G99 / G100 export limits apply
            </li>
            <li>
              <strong className="text-white">(3) ISLAND MODE</strong> — PEI disconnected from public
              network. Inverter generates V and freq for the protected loads from local sources (PV
              during day, BESS continuously). EPS (Emergency Power Supply) is the modern
              implementation. Used for grid outages
            </li>
            <li>
              <strong className="text-white">TRANSITION MODES</strong> — PEI moving between modes.
              Direct ↔ reverse: transparent (no contactor action — just flow direction reverses).
              Direct/reverse ↔ island: requires EPS contactor + inverter mode change + neutral
              switching (Reg 826.1.1.2.2). May be brief (50-100 ms for modern fast EPS) or scheduled
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.1 requirement</strong> — PEI shall operate
              in any intended mode; can change modes at any time; protection of persons + property
              in each mode. The design pack specifies the supported modes
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — records the supported
              operating modes and the protective measures for each, plus the EEMS configuration (Reg
              825) that orchestrates the transitions
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          EEMS is the brain of the PEI — supervising load, generation, storage, grid interface, EV.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Self-consumption optimisation</strong> — preferentially
              route PV surplus to loads + BESS rather than export, where economically advantageous
            </li>
            <li>
              <strong className="text-white">Peak shaving</strong> — discharge BESS during peak grid
              prices; charge during off-peak
            </li>
            <li>
              <strong className="text-white">Time-of-use scheduling</strong> — load shifting
              (heating / EV / appliances) to align with cheap tariff or surplus generation
            </li>
            <li>
              <strong className="text-white">Demand response</strong> — respond to grid signals (DSR
              / flex markets) — exposed via Reg 826.4
            </li>
            <li>
              <strong className="text-white">Export limitation</strong> — enforce EREC G100 export
              caps where applicable
            </li>
            <li>
              <strong className="text-white">Mode transitions</strong> — automatic switch to island
              mode on DNO loss; resync on restoration
            </li>
            <li>
              <strong className="text-white">Multi-source coordination</strong> — PV + BESS +
              generator + EV — set priorities, set thresholds
            </li>
            <li>
              <strong className="text-white">Safety supervision</strong> — monitor for faults;
              alert; initiate shutdown if necessary
            </li>
            <li>
              <strong className="text-white">Reporting + visibility</strong> — customer-facing app,
              EICR-ready event logs
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Local earth electrode for island mode. Neutral switch prevents RCD misoperation during
          transitions.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Reg 826.1.1.2 (earthing arrangement)</strong> — local
              earth electrode for island mode per Reg 542.2.2; DNO earth means need not be
              disconnected; any change of system earthing for island mode shall be reversible
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.2.2 (neutral conductor)</strong> — when in
              island mode, ALL live conductors (including neutral) disconnected from DNO. Neutral
              switch device connects local N-E without overlapping with switching of DNO neutral —
              prevents RCD misoperation during transition
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.2.3 (transfer switching device)</strong> —
              for connection to local earthing arrangement (local star point/midpoint or local
              exposed-conductive-parts), use transfer switching device per Reg 537.1.5
            </li>
          </ul>
        </ConceptBlock>
        ...
        <ContentEyebrow>
          Reg 826.1.1.3 — Selection of protective device (per source, per operating mode)
        </ContentEyebrow>

        <Pullquote>
          OCPD selection must consider the MINIMUM earth fault current per source — in island mode
          the PV inverter is a low-fault-current source.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">TN system challenge</strong> — TN ADS relies on OCPD
              opening within the required disconnection time (Chapter 41). MCB magnetic trip needs
              typically 5-10&times; I_n. PV / BESS inverter in island mode delivers ~1.1-1.5&times;
              I_n. MCBs do NOT trip — they take seconds (thermal region). Touch voltage may exceed
              safe limits
            </li>
            <li>
              <strong className="text-white">IT system challenge</strong> — IT ADS uses an IMD +
              manual response on first fault. Island PEI is rarely IT — most are derived from TN-C-S
              DNO supply
            </li>
            <li>
              <strong className="text-white">TT system in island mode</strong> — RCDs detect
              earth-fault current via L-N imbalance, not by relying on high fault current. Inverter
              only needs to deliver enough imbalance to operate the RCD (~30 mA for a domestic RCD).
              The 4-pole EPS contactor that disconnects DNO N + L conductors lets the PEI operate
              effectively as TT in island mode
            </li>
            <li>
              <strong className="text-white">Inverter-side electronic protection</strong> — many
              modern hybrid inverters (GivEnergy Gen3, Tesla Powerwall 3, SolarEdge Energy Hub) have
              built-in fault-detection that opens the inverter output within milliseconds of
              detecting a downstream fault — independent of the OCPD strategy. The PEI design pack
              records this
            </li>
            <li>
              <strong className="text-white">PEI design pack content</strong> — system earthing in
              island mode + fault-current analysis (inverter&rsquo;s island-mode short-circuit
              contribution) + verification that the protective measures operate within the required
              times
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Main switch PER SOURCE. Warning notice in the vicinity. Reg 512.1.2 island-mode switches.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">DNO main isolator</strong> — existing on every install.
              Typically the main switch at the consumer unit or origin
            </li>
            <li>
              <strong className="text-white">PV inverter AC isolator</strong> — per Reg 712.514.102
              (Module 3); BS EN 60947-3 switch-disconnector; accessible without tools
            </li>
            <li>
              <strong className="text-white">BESS AC isolator</strong> — per Reg 570.6 (Chapter 57);
              equivalent to PV inverter isolator
            </li>
            <li>
              <strong className="text-white">Generator AC isolator</strong> — per Reg 551.2.4
              (Section 551); typically near the generator
            </li>
            <li>
              <strong className="text-white">Per Reg 826.1.1.5</strong> — island-mode switching
              devices comply with Reg 512.1.2; suitable for isolation
            </li>
            <li>
              <strong className="text-white">Durable warning notice</strong> — fixed in the vicinity
              of the isolators; alerts personnel that ALL must be isolated for safe work. Typically
              positioned at: (1) origin of installation; (2) consumer unit / metering point. Notice
              content lists each source and its isolator location
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Account for ALL directions of current flow. Bidirectional OCPDs throughout the PEI.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Reg 826.1.2.1 (overcurrent magnitude)</strong> —
              overload + short-circuit currents determined at every point of the PEI where a
              protective device is installed; for all possible configurations and min/max current
            </li>
            <li>
              <strong className="text-white">Reg 826.1.2.2 (location + direction)</strong> —
              selection and erection of OCPDs shall take account of ALL POSSIBLE DIRECTIONS of
              current flow + polarity. Source connection via switchgear: Reg 551.7.2 applies
              (parallel-operation)
            </li>
            <li>
              <strong className="text-white">
                Reg 826.1.2.3 (combined short-circuit protection)
              </strong>{' '}
              — where combined short-circuit protection used per Reg 434.5.1, coordination shall
              consider all possible configurations of power supplies
            </li>
          </ul>
          <p>Practical bidirectional OCPD examples:</p>
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">PV DC OCPDs</strong> — gPV fuses per BS EN 60269-6 OR
              DC-rated MCBs per BS EN 60898-2/IEC 60898-3, bidirectional per Reg 712.533.101 (Module
              3 Section 4)
            </li>
            <li>
              <strong className="text-white">BESS DC OCPDs</strong> — bidirectional per Reg
              570.6.1.1.1 (Module 5)
            </li>
            <li>
              <strong className="text-white">Source-AC OCPDs at the consumer unit</strong> —
              bidirectional MCBs for PV / BESS / generator AC connections per Reg 551.7.1(c) NEW
              A4:2026
            </li>
            <li>
              <strong className="text-white">Combined OCPD coordination</strong> — design pack
              analysis of fault current contributions from each source under various operating modes
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Grid off = island mode OR shutdown. Transients more frequent in PEI — more SPDs.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Option (a) ISLAND MODE</strong> — PEI continues
              operating with local sources only. Requires: EPS-capable hybrid inverter; island-mode
              neutral / earth arrangement (Reg 826.1.1.2.2); protected-load partition (Section 4.6
              covers in depth)
            </li>
            <li>
              <strong className="text-white">Option (b) AUTOMATIC SHUTDOWN</strong> — PEI
              disconnects all local sources on grid loss. PV inverter anti-islanding per EREC G98 /
              G99 ensures it stops. BESS PCE disconnects. Loads have no supply until grid restored
            </li>
            <li>
              <strong className="text-white">Control + protective device duty</strong> — Reg 826.1.3
              notes that PEI control + protective devices may operate more frequently than non-PEI
              installs. Selection should account for increased duty cycle (e.g. contactors rated for
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">More frequent operation</strong> — PEI switching
              events: PV anti-islanding tests; BESS state transitions; EV plug-in / unplug; source
              switching; load shedding
            </li>
            <li>
              <strong className="text-white">SPD spec considerations</strong> — higher I_n / Imax
              ratings; possibly Type-1 + Type-2 multi-zone where direct-strike LPS is not separated
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — records SPD locations +
              specs + the 5-yearly inspection schedule
            </li>
            <li>
              <strong className="text-white">EICR-style periodic inspection</strong> — verifies SPD
              status; replaces operated SPDs; records the maintenance
            </li>
          </ul>
        </ConceptBlock>
        ...
        <ContentEyebrow>
          Regs 826.2 - 826.7 — interaction, storage, demand-response, EV, selectivity, verification
        </ContentEyebrow>

        <Pullquote>
          Chapter 82 doesn&rsquo;t stop at 826.1. Regs 826.2-826.7 cover everything the PEI touches.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">EREC G98 (UK)</strong> — generation up to 16 A per
              phase, fit-and-notify. Module 3 Section 6 covers in depth
            </li>
            <li>
              <strong className="text-white">EREC G99 (UK)</strong> — generation above 16 A per
              phase, apply-and-wait. Module 3 Section 6 covers in depth
            </li>
            <li>
              <strong className="text-white">EREC G100 (UK)</strong> — export limitation device for
              installs that would otherwise exceed the agreed export capacity. EEMS-implemented
            </li>
            <li>
              <strong className="text-white">BS EN 50549-1/-2</strong> — generator-to-DNO interface
              standard. Inverter type-test against this
            </li>
            <li>
              <strong className="text-white">Power quality</strong> — harmonics, flicker, voltage
              rise. Inverter compliance + design pack analysis
            </li>
            <li>
              <strong className="text-white">Fault-ride-through</strong> — required behaviour during
              DNO voltage dips. Modern inverters handle per BS EN 50549
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Inrush during mode switch</strong> — switching from
              direct/reverse feeding into island (or vice versa) means a closing contactor energises
              load + capacitors + transformer magnetisation. The instantaneous current can be
              several times the steady-state value
            </li>
            <li>
              <strong className="text-white">PCE inrush spec</strong> — manufacturer datasheets
              typically state the inrush curve; the design pack records it
            </li>
            <li>
              <strong className="text-white">Protective device coordination</strong> — OCPDs at the
              BESS connection must ride through the inrush without nuisance-tripping; selectivity
              per Reg 826.6 considers inrush events
            </li>
            <li>
              <strong className="text-white">Full BESS install detail</strong> — Chapter 57 (Reg
              570.x) covers battery selection, PCE, DC earthing, ventilation, fault current. Module
              5 of this course covers in depth
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">BS HD 60364-8-1</strong> — referenced standard for the
              operability framework
            </li>
            <li>
              <strong className="text-white">Sheddable vs protected loads</strong> — design-stage
              classification recorded in the PEI design pack
            </li>
            <li>
              <strong className="text-white">EEMS executes (Reg 825)</strong> — the EEMS is the
              orchestrator; Reg 826.4 sets the design requirement, Reg 825 sets the supervisory
              layer
            </li>
            <li>
              <strong className="text-white">Customer agency</strong> — design includes manual
              override; customer retains control
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Section 722</strong> — EV charging install regs (Module
              6 / 7 of this course will cover in depth)
            </li>
            <li>
              <strong className="text-white">V1G unidirectional</strong> — EV is a controllable
              load. EEMS schedules charging
            </li>
            <li>
              <strong className="text-white">V2H / V2G bidirectional</strong> — EV is a source.
              Multi-source isolation + bidirectional OCPD + Reg 551.7 + Chapter 82 PEI design pack
              updates
            </li>
            <li>
              <strong className="text-white">Diverter integration</strong> — Section 4.3 covered
              PV-surplus diverters (Eddi, Zappi). When the diverter is the EEMS the design pack
              records it as such
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Discrimination / selectivity</strong> — upstream OCPDs
              ride through downstream faults so only the nearest device opens
            </li>
            <li>
              <strong className="text-white">
                Reg 434.5.1 (combined short-circuit protection)
              </strong>{' '}
              — referenced by Reg 826.1.2.3; coordination applies across all possible configurations
            </li>
            <li>
              <strong className="text-white">Cascading + back-up protection</strong> — where
              downstream device cannot interrupt the full prospective fault current, upstream device
              backs it up
            </li>
            <li>
              <strong className="text-white">Multi-source fault contributions</strong> — DNO + PV
              inverter + BESS PCE each contribute to a fault. Inverters typically contribute 1.1-1.5
              &times; I_n (low); DNO contributes kA (high). Total = DNO + small inverter
              contributions
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">The caveat</strong> — handheld ZS testers may give
              invalid readings on PEI installs with active inverters
            </li>
            <li>
              <strong className="text-white">Safe method 1 — inverter OFF</strong> — disable
              inverter + open AC isolator + verify zero voltage + test + record
            </li>
            <li>
              <strong className="text-white">
                Safe method 2 — manufacturer&rsquo;s test method
              </strong>{' '}
              — some inverter manufacturers publish a commissioning-mode procedure
            </li>
            <li>
              <strong className="text-white">Safe method 3 — high-current method</strong> — fully
              de-energised circuit + specialist test equipment
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — records the ZS reading
              + the method + the inverter state during the test. Full commissioning workflow lives
              in Section 4.8
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          PEI design pack is the integration document. Above Section 712 + Chapter 57 + Section 722
          packs.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">1. PEI scope statement</strong> — what defines this
              install as a PEI (consumes + generates + stores; specific components)
            </li>
            <li>
              <strong className="text-white">2. Operating modes table</strong> — supported modes per
              Reg 824.2 (direct feeding / island / etc.); transition sequences; mode-specific
              behaviour
            </li>
            <li>
              <strong className="text-white">3. Single-line schematic</strong> — all sources, all
              loads, all isolators, the coordination logic between sources. The PEI integration
              diagram
            </li>
            <li>
              <strong className="text-white">4. Earthing arrangement</strong> — direct feeding mode
              (DNO earth); island mode (local electrode per Reg 542.2.2); transition behaviour per
              Reg 826.1.1.2 + .1.2.2
            </li>
            <li>
              <strong className="text-white">5. Isolation summary</strong> — every isolator listed
              (DNO / PV / BESS / generator / EV / etc.) per Reg 826.1.1.4; warning notice location +
              content
            </li>
            <li>
              <strong className="text-white">6. Bidirectional OCPD analysis</strong> — every OCPD
              listed with manufacturer / model + bidirectional capability statement per Reg
              826.1.2.2; coordination per Reg 826.1.2.3
            </li>
            <li>
              <strong className="text-white">7. Transient overvoltage protection design</strong> —
              SPDs per Reg 826.1.4; locations + specs + zones
            </li>
            <li>
              <strong className="text-white">8. Grid outage response</strong> — Reg 826.1.3 option
              (a) island OR option (b) shutdown; the specific implementation
            </li>
            <li>
              <strong className="text-white">9. Cross-references to per-domain packs</strong> —
              Section 712 PV (Module 3); Chapter 57 BESS (Module 5); Section 722 EV (Module 6 / 7);
              EREC G98 / G99 / G100; PAS 63100 for BESS
            </li>
            <li>
              <strong className="text-white">10. Emergency response procedure</strong> — what
              happens if any source fails; customer/contractor contact info; safety actions
            </li>
          </ul>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-4-section-4')}
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-4-section-6')}
