...
const inlineChecks = [
  {
    ...
    options: [
      ...
      'AT LEAST TWO separate emergency lighting circuits in high-risk areas, AND no single fault shall disable more than 20 luminaires. The two-part rule provides architectural redundancy AND consequence containment. Two circuits means a fault on one (cable damage, MCB operation, control fault) leaves the second operational — the area is not fully dark. Twenty-luminaire cap means even within one circuit, a localised fault cannot black-out a large zone. A 60-luminaire high-risk area must therefore be split into at least three sub-zones with no more than 20 luminaires each, on at least two circuits — typically three circuits of 20, two circuits of 30 with sub-zone protection, or similar architectures depending on layout.',
      'Five circuits, max 5 luminaires per fault.',
    ],
    correctIndex: 2,
    explanation:
      'The 2025 rule has TWO parts: ≥ 2 circuits AND ≤ 20 luminaires per fault. Both must be satisfied simultaneously. It is an architectural requirement reflecting lessons from major incidents where single-circuit failures left areas dark during evacuation.',
  },
  {
    ...
    options: [
      'Run them anywhere.',
      'Emergency lighting cables must be physically separated from cables of other circuits — separate cable trays, separate compartments in segregated trunking, separate conduits, or maintained physical clearance — so that a fault on a non-safety circuit (overcurrent, short-circuit, fire damage) cannot propagate to the safety circuit. The separation also addresses electrical interference, mechanical damage during work on one circuit affecting the other, and isolation requirements (you can isolate non-safety circuits without disturbing the safety service). Mixed cables in a single tray defeat the segregation.',
      'Use the same cable tray.',
      'No requirement.',
    ],
    correctIndex: 1,
    explanation:
      'BS 7671 §528.1 segregation is about preventing a fault on one circuit from affecting another. Emergency lighting circuits get their own routes, separate from non-safety circuits. The segregation is physical, electrical, and operational.',
  },
  {
    id: 'elm4-s4-isolator',
    question:
    ...
    options: [
      ...
      'Label text: "EMERGENCY LIGHTING — DO NOT SWITCH OFF" or equivalent unambiguous wording, on a permanent engraved Traffolyte (or equivalent) label fixed by screws/mechanical means. Location: at the supply origin (main isolator), at any sub-isolators, at the distribution board way, and at the central battery cabinet supply terminal — anywhere a person could reasonably switch off the supply without realising what it does. Visible from the operator\'s normal position. The label is permanent — sticky labels are not acceptable; they fall off and the warning is lost. The cost is a few pounds; the consequence of skipping it is that some maintenance electrician switches off the emergency lighting supply, and the system runs out the autonomy and goes dark.',
      'A sticky label is fine.',
      'On the floor.',
    ],
    correctIndex: 1,
    explanation:
      'Permanent engraved label, screw-fixed, "EMERGENCY LIGHTING — DO NOT SWITCH OFF", at every isolation point, visible from operator position. The cheapest detail; the most commonly missed; the most important for preventing accidental isolation.',
  },
  {
    id: 'elm4-s4-firestop',
    question:
    ...
    options: [
      'Nothing — the cable and tray are fire-rated.',
      'Some packing.',
      'CERTIFIED FIRE-STOPPING SYSTEM matching the wall rating (60 minutes), the cable type and quantity, and the containment (cable tray). Typical methods: intumescent compound, fire pillows for cable bundles, fire-rated mortar, or proprietary cable-transit systems. The system must be tested and certified for the specific configuration. Installation per the manufacturer\'s certified instructions. Documentation of the fire-stop system, certificate reference, and date on the as-built drawings. The cables and tray being fire-rated is necessary but NOT sufficient — the wall rating must be restored at every penetration. A 60-minute wall with an open hole around a cable becomes a 0-minute wall at that point.',
      'Plain mortar.',
    ],
    correctIndex: 2,
    explanation:
      'Fire-rated cable + fire-rated tray + UNSTOPPED penetration = breached compartment. The wall rating must be restored at the penetration with certified fire-stopping. This is a Building Regulations + BS 8519 + BS 7671 requirement.',
  },
];
...
const quizQuestions = [
  {
    ...
    options: [
      'One circuit, max 100 luminaires.',
      'AT LEAST TWO separate emergency lighting circuits AND no single fault shall disable more than 20 luminaires. Architectural redundancy (≥ 2 circuits) plus consequence containment (≤ 20 per fault). Both must be met simultaneously. A 60-luminaire high-risk area must therefore be split into at least three sub-zones on at least two circuits.',
      'Three circuits, no luminaire limit.',
      'No specific requirement.',
    ],
    correctAnswer: 1,
    explanation:
      'The 2025 rule has both halves: ≥ 2 circuits AND ≤ 20 per fault. Both apply together. It is the standard\'s response to major-incident lessons where single-circuit failures darkened areas during evacuation.',
  },
  {
    id: 2,
    ...
    options: [
      'One.',
      'At least THREE sub-zones with no more than 20 luminaires each, on at least two circuits. Common implementations: three circuits of 20 luminaires each (simplest); two circuits with each split into sub-zones via fused taps or distribution; combination of self-contained backup with central. The architectural design is per-area; the implementation is at the sparkie\'s discretion within the constraint.',
      'Two of 30 each (no sub-zone protection).',
      'No splitting needed.',
    ],
    correctAnswer: 1,
    explanation:
      '60 luminaires / 20 per fault = ≥ 3 sub-zones. Plus ≥ 2 circuits. Three circuits of 20 is the cleanest implementation; alternatives exist but must meet both halves of the rule.',
  },
  {
    id: 3,
    ...
    options: [
      'No specific requirement.',
      'Physical segregation — separate trays, separate compartments in segregated trunking, separate conduits, or maintained physical clearance. The segregation prevents a fault on a non-safety circuit (overcurrent, short-circuit, fire damage during work) from propagating to the safety circuit. Mixed cables in a single tray defeat the segregation.',
      'Use the same containment.',
      'Bond the circuits together.',
    ],
    correctAnswer: 1,
    explanation:
      'BS 7671 §528.1 segregation: emergency lighting cables physically separate from non-safety circuits. Separate trays/compartments/conduits. Prevents fault propagation, allows independent isolation, simplifies work on either circuit.',
  },
  {
    id: 4,
    ...
    options: [
      'No requirement.',
      'A sticky note.',
      'Permanent engraved label (Traffolyte or equivalent), screw-fixed, with text "EMERGENCY LIGHTING — DO NOT SWITCH OFF" or equivalent unambiguous wording. At every isolation point — main isolator, sub-isolators, distribution board, cabinet supply terminal. Visible from the operator\'s normal position. Lockable in OFF position.',
      'A handwritten label.',
    ],
    correctAnswer: 2,
    explanation:
      'Permanent engraved label, screw-fixed, unambiguous text, every isolation point, visible. Sticky labels fall off and are not compliant. The most commonly missed detail; the most important for preventing accidental isolation.',
  },
  {
    id: 5,
    question: 'BS 7671 §560.10 covers sources of supply for safety services. List three acceptable sources.',
    options: [
      'Only the public mains.',
      'Acceptable sources: (1) central battery (BS EN 50171:2021); (2) generating set (engine-driven, automatically started on mains failure); (3) primary cells (non-rechargeable batteries, used for very specific limited applications). Combinations are also acceptable — e.g. central battery PLUS generating set with the battery covering changeover until the generator stabilises. The supply must be INDEPENDENT of the normal supply, or arranged so failure of one does not cause failure of the other.',
      'Only candle.',
      'Only solar.',
    ],
    correctAnswer: 1,
    explanation:
      'BS 7671 §560.10 acceptable sources: central battery, generating set, primary cells (limited use), combinations. The key requirement is INDEPENDENCE — the safety supply must not fail just because the normal supply fails.',
  },
  {
    id: 6,
    ...
    options: [
      'Nothing — fire-rated cable is enough.',
      'A 90-minute certified fire-stopping system matching the floor rating, the cable type and quantity, and the containment. Typically intumescent compound, fire pillows, fire-rated mortar, or proprietary cable-transit systems tested and certified for the configuration. Installation per the manufacturer\'s certified instructions; documentation on as-built drawings with certificate reference.',
      'Just paint over.',
      'No requirement.',
    ],
    correctAnswer: 1,
    explanation:
      'Every fire-rated wall/floor penetration needs certified fire-stopping matching the rating, the cable, and the containment. Fire-rated cable does not substitute — the wall/floor rating must be restored at every penetration. Building Regs + BS 8519 + BS 7671.',
  },
  {
    id: 7,
    ...
    options: [
      'The colours clash.',
      'Single shared containment defeats segregation. A fault on the general lighting circuit (overcurrent, short-circuit, mechanical damage during work) can damage the emergency lighting cables in the same tray. Fire damage to non-safety cables in the tray can damage safety cables alongside. Independent isolation of one circuit affects access to the other. The §528.1 requirement is for physical separation — different tray, different compartment in segregated trunking, or different conduit — to prevent cross-contamination of faults.',
      'It is too tidy.',
      'The cables get hot.',
    ],
    correctAnswer: 1,
    explanation:
      'Shared containment = no segregation = fault on one can affect the other. BS 7671 §528.1 demands physical separation. The fix is dedicated trays/compartments for safety services or maintained physical clearance.',
  },
  {
    id: 8,
    question: 'What is a Class A circuit topology in BS EN 50172 fault-tolerance terms, compared with Class B?',
    options: [
      'Class A means smaller cables.',
      'Class A topology provides higher fault tolerance — typically two independent paths from the supply source to each luminaire, so any single cable / connection / device fault does not extinguish the luminaire. Class B has lower fault tolerance — a single fault may extinguish a number of luminaires depending on where it occurs. The 2025 high-risk requirement (≥ 2 circuits, ≤ 20 luminaires per fault) effectively pushes high-risk areas toward Class A topologies. The classification language varies between standards but the underlying idea — fault tolerance through redundancy — is universal.',
      'Class B is faster.',
      'They are identical.',
    ],
    correctAnswer: 1,
    explanation:
      'Class A vs Class B in BS EN 50172 reflects fault-tolerance topology. Class A = redundant paths, single fault tolerance. Class B = simpler, less tolerance. High-risk areas trend toward Class A patterns under the 2025 requirements.',
  },
  {
    id: 9,
    ...
    options: [
      'Just the original drawings.',
      'Zone plans showing the high-risk areas and their luminaire allocation; riser diagrams showing the circuit segregation and the supply path to each zone; cause-and-effect documentation describing how the system behaves during the test sequences (functional, duration, photometric); cable schedules indicating circuit identity, route, fire-resistance category, and segregation arrangement; and the as-built drawings of the installation. Together these form the audit trail showing that the architectural requirements (≥ 2 circuits, ≤ 20 luminaires per fault, fire-rated cables, fire-stopping, labelled isolators) are realised in practice.',
      'A photo of the cabinet.',
      'No documentation.',
    ],
    correctAnswer: 1,
    explanation:
      '2025 documentation: zone plans, riser diagrams, cause-and-effect, cable schedules, as-built drawings. The pack evidences that the 2025 architectural requirements are implemented and auditable.',
  },
  {
    id: 10,
    ...
    options: [
      'No — the supply must be from a totally separate supply.',
      'Yes — but the implementation must satisfy BS 7671 §560.10 INDEPENDENCE: the cabinet supply must not be lost when ordinary circuits trip or are isolated. Common implementations: a dedicated way on the main DB protected by its own MCB, with no upstream RCD that could trip on a fault elsewhere; OR a totally separate sub-DB fed from the supply origin; OR a designated supply source per §560.10. The cabinet itself has a labelled lockable isolator at its supply terminal. The mains supply to the cabinet is not "ordinary" — it is the input to a safety service and must be designed to remain available except in the event the safety service is itself energised from its battery.',
      'It does not matter.',
      'Only with a generator.',
    ],
    correctAnswer: 1,
    explanation:
      'BS 7671 §560.10 independence: the safety supply must not fail when ordinary circuits fail. Practical implementations include dedicated DB way without upstream RCD, separate sub-DB, or designated supply source. The cabinet has its own labelled lockable isolator at its supply terminal.',
  },
];
