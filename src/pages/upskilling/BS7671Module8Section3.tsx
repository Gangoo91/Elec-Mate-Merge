...
const BS7671Module8Section3 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 8 · Section 3 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            "The consolidated A4:2026 reference. What is new, what is deleted, what is amended, the cert-form changes, and how to handle work spanning the 15 April 2026 publication / 15 October 2026 A3 withdrawal dates. Companion to Module 2 §4 — same A4 topic viewed through the inspector's reference lens rather than the designer's definitions lens."
          }
        </div>
        ...
        >
          <p>
            BSI publishes each BS 7671 amendment with a defined transition window during which both
            the outgoing and incoming amendments may be cited on certificates. A4:2026 was published
            15 April 2026; the six-month transition closes 15 October 2026. From 16 October 2026,
            every newly designed installation must be certified against A4 only. Existing compliant
            installations remain compliant against the edition they were originally certified to —
            A4 is not retrospective. On an EICR of an older install, departures from A4 are reported
            as deviations from the current edition and coded per GN3 (typically C3 for historic, C2
            where the absence creates real-world risk).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            worksheets in their templates need to update them. The consequence test in Reg 443.4.1
            is more open-ended but also more honest — protection shall be provided where the
            consequence could result in (a) serious injury to, or loss of, human life, or (c)
            ...
            the Type 1 / 2 / 3 cascade and Reg 443.6 for selection at origin, sub-distribution and
            end-of-circuit.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            document with the cert-issuance target date. If that date is on or after 15 April 2026,
            design to A4 from day one regardless of whether the cert can be issued under A3. If the
            cert is issued before 15 October 2026 you can choose either citation; if after, A4 only.
            Over-engineering early to A4 standards costs little and removes transition risk
            completely.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            GN3 (Section K) is unambiguous: every observation gets a single code (C1, C2, C3 or FI).
            &quot;Satisfactory&quot; overall is incompatible with any C1 or C2 outstanding. The
            inspector's judgement on C2-vs-C3 sits on whether the deviation creates real-world risk
            in the specific premises (occupancy, condition of accessories, presence of children /
            vulnerable adults, environmental factors). Document the reasoning on the EICR — &quot;C2
            because [evidence]&quot; or &quot;C3 because [evidence]&quot; — not just the code.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            <strong>(ii) EV / V2G</strong> — as V2G volumes grow, the regs will need to address the
            prosumer EV scenario where the vehicle exports power, not just imports it; expect
            Section 722 to gain V2G- specific clauses.{' '}
            <strong>(iii) Heat pumps and DC-bus distribution</strong> — fully electrified dwellings
            (heat pump + EV + induction hob + battery + PV) push domestic boards toward 100 A+
            ratings and DC-bus distribution variants familiar from larger commercial; the regs will
            likely follow. <strong>(iv) Cyber-security and functional-safety language</strong> for
            grid-interactive prosumer systems — currently outside BS 7671 scope but on the JPEL/64
            horizon. <strong>(v) Further SPD tightening</strong> following the CRL deletion: expect
            Reg 443.4 to gain more prescriptive triggers as data accumulates.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The answer in the exam is almost always &quot;read the in-force text&quot; — but knowing
            which Section to reach for first saves time. The four NEW headline regulations (411.3.4,
            419 group, 530.3.201, 722.312.2.1) and the one major DELETION (443.5 / Annex A443 / App
            17 / Annex B443 — the CRL method) are the items most likely to feature on a written
            paper or an oral / interview-style assessment. Beyond those, expect questions on the
            cert-form changes and the transition-period mechanics.
          </p>
        </ConceptBlock>
