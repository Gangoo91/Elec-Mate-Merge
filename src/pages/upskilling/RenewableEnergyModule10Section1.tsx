...
export default function RenewableEnergyModule10Section1() {
  ...
  useSEO({
    title:
      'Hybrid systems landscape + Chapter 82 PEI framework | Renewable Energy 10.1 | Elec-Mate',
    description:
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 10 · Section 1 · BS 7671:2018+A4:2026 · Chapter 82 PEI"
        title="Hybrid systems landscape + Chapter 82 PEI framework"
        backTo="../renewable-energy-module-10"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The integration module opener. Where PV + BESS + EV + heat pump + (sometimes) wind / CHP
          coexist on one site — and BS 7671 Chapter 82 is the framework that integrates the
          per-technology rules from M2-M9. Reg 826.1.1.1 protection in all operating modes. Reg
          826.1.1.4 multi-source isolation. Reg 826.1.2.1 overcurrent across configurations. The
          architectural shift from single-source to multi-source design.
        </p>
        ...
        <Pullquote>
          Single-source design is a fixed-topology problem. Multi-source PEI design is a
          state-machine problem — every combination of sources is a different topology with
          different protective behaviour. Chapter 82 is the framework for that shift.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">One or more local sources</strong> — PV (Section 712),
              BESS (Chapter 57), wind / CHP / micro-hydro (Section 551), V2G EV (Section 722). Two
              or more = the multi-source PEI scenario this module covers
            </li>
            <li>
              <strong className="text-white">DNO connection</strong> — public-network interface for
              import + export. EREC G98 / G99 / G100 govern this interface. Off-grid PEI has no DNO
              connection (a separate rural / island scenario)
            </li>
            <li>
              <strong className="text-white">Operating modes per Reg 824.2</strong> — direct-feeding
              mode (default, in parallel with DNO), island mode (disconnected, operating on local
              sources), and the transitions between them
            </li>
            <li>
              <strong className="text-white">Producer + consumer duality</strong> — the same
              installation imports during low-generation / high-load periods + exports during
              high-generation / low-load periods. Energy flow is bidirectional through the DNO meter
            </li>
            <li>
              <strong className="text-white">EEMS — Electrical Energy Management System</strong> —
              Reg 826 recognises the EEMS as the coordination layer across sources + loads + tariffs
              + export limits. §10.2 covers EMS in detail
            </li>
            <li>
              <strong className="text-white">Interaction with the public network</strong> — Reg
              826.2 + EREC G98 / G99 / G100 govern the DNO interface. §10.3-10.4 cover SEG + G100
              export limit
            </li>
            <li>
              <strong className="text-white">Surge protection</strong>— Reg 826.1.4: PEI has more
              switching events (between sources, load shifting, mode transitions) than a non-PEI —
              SPD specification may need uplift
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — per-technology DoC +
              per-technology EIC sections + integrating PEI EIC + Chapter 82 compliance summary +
              EREC correspondence + EMS commissioning record
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Operating-mode recognition (Reg 824.2)</strong> —
              direct feeding, island, transitions. No per-technology section frames the install as a
              state-machine; Chapter 82 does
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.1 protection in all modes</strong> —
              single-source design verifies one topology; multi-source PEI must verify every
              combination
            </li>
            <li>
              <strong className="text-white">
                Reg 826.1.1.2.2 N-E bond switching for island mode
              </strong>{' '}
              — doesn’t arise in any single-source section; categorically a Chapter 82 problem
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.4 multi-source isolation</strong> —
              per-source switches + a warning notice or a suitable interlock system. Single-source
              sections require one main switch
            </li>
            <li>
              <strong className="text-white">
                Reg 826.1.2.1 overcurrent across configurations
              </strong>{' '}
              — island mode short-circuit current dramatically lower than direct-feeding mode;
              protective device design must accommodate both
            </li>
            <li>
              <strong className="text-white">Reg 826.1.4 SPD against switching transients</strong> —
              PEI has more transient events from mode + source switching
            </li>
            <li>
              <strong className="text-white">EEMS recognition</strong>— Reg 826 explicitly includes
              Electrical Energy Management System scope. Per-technology sections don’t cover this
              coordination layer
            </li>
            <li>
              <strong className="text-white">Collective + shared PEI (Reg 826.7 contents)</strong> —
              apartment blocks, community schemes, shared assets. A category that doesn’t exist in
        ...
        <Pullquote>
          Island mode isn’t ‘keep running when the DNO trips’. It’s a deliberately-designed,
          electrically-isolated operating state with its own earthing, its own neutral bond, its own
          protective behaviour — and Chapter 82 is the framework that codifies it.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Direct-feeding mode</strong>— PEI in parallel with DNO.
              DNO sets voltage + frequency. Local inverters grid-following (sync to DNO). Local
              sources export when surplus + import from DNO when deficient. UK 2025-26 default
            </li>
            <li>
              <strong className="text-white">Island mode</strong> — PEI disconnected from DNO.
              Grid-forming inverter (typically BESS) sets voltage + frequency. Local sources
              synchronise to the grid-forming inverter. Loads served from local generation + storage
              only
            </li>
            <li>
              <strong className="text-white">Transition direct → island</strong> — triggered by DNO
              loss or manual switch. Requires: detection (loss-of-mains), N-E bond change (Reg
              826.1.1.2.2 non-overlap), inverter mode change (grid-following → grid-forming),
              load-shed decisions (if local generation insufficient)
            </li>
            <li>
              <strong className="text-white">Transition island → direct</strong> — DNO restored,
              synchronisation check, N-E bond reverts to DNO, inverters resume grid-following. Reg
              551.7.5 anti-islanding still applies as final defence
            </li>
            <li>
              <strong className="text-white">Short-circuit current difference</strong> —
              direct-feeding: DNO 6-25 kA prospective + inverter contribution ~1.1× rated. Island:
              only inverter contribution (a few hundred amps at most). Reg 826.1.2.1 requires
              protective device design for both extremes
            </li>
            <li>
              <strong className="text-white">Earthing</strong> — Reg 826.1.1.2 + Reg 542.2.2: island
              mode needs its own earth electrode arrangement; DNO earth connection may be retained
              but the N-E bond changes per Reg 826.1.1.2.2
            </li>
            <li>
              <strong className="text-white">UK 2025-26 hardware</strong>— island-capable inverter /
              backup gateway: Tesla Powerwall Gateway, SolarEdge Backup Interface, GivEnergy Whole
              Home Backup, Sonnen, Enphase IQ8. Cost uplift £2-5k hardware + commissioning
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — declared operating
              modes + commissioning test of each transition + N-E bond verification per Reg
              826.1.1.2.2 + island-mode switching device per Reg 826.1.1.5
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Trigger</strong> — loss of DNO supply detected (voltage
              drop, frequency excursion, RoCoF, loss-of-mains relay). Backup gateway initiates
              transition sequence. Note: EREC G99 disallows Vector Shift as a loss-of-mains method
              for type-tested generation — RoCoF (with voltage / frequency monitoring) is the
              required method for the type-tested inverters used in virtually all LCT installs;
              Vector Shift survives only on legacy / non-type-tested kit
            </li>
            <li>
              <strong className="text-white">Step 1</strong> — DNO line conductors disconnect
              (contactor / switch-disconnector opens)
            </li>
            <li>
              <strong className="text-white">Step 2</strong> — DNO neutral disconnects. CRITICAL: at
              this instant, the PEI has no N-E reference — RCDs would not operate correctly +
              protective measures inoperative. Must be brief
            </li>
            <li>
              <strong className="text-white">Step 3</strong> — Local N-E bond establishes (local
              neutral switch closes between PEI neutral bar + PE bar / local earth electrode per Reg
              826.1.1.2)
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.2.2 non-overlap</strong> — step 2 + step 3
              must NOT overlap with a state where BOTH DNO N-E + local N-E are in circuit (would
              create circulating current through earthing system)
            </li>
            <li>
              <strong className="text-white">Result</strong> — PEI operating as a local TN-S island
              with local N-E bond + local earth electrode + grid-forming inverter setting V + f
            </li>
            <li>
              <strong className="text-white">Reverse transition (DNO restored)</strong> — step 1:
              synchronisation check (PEI inverter must match DNO V + f before reconnect); step 2:
              local N-E bond opens; step 3: DNO neutral connects; step 4: DNO line connects
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — backup gateway
              manufacturer DoC declaring Reg 826.1.1.2.2 compliance + commissioning test of
              grid-loss transition + N-E bond measurement (continuity + isolation) in each state +
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">One switch per source</strong>— DNO main switch + PV AC
              + PV DC + BESS AC + BESS DC + V2G EV + any other source. Each switch must be suitable
              for isolation per Reg 537.2 + accessible without dismantling enclosures
            </li>
            <li>
              <strong className="text-white">Warning notice</strong>— durable, permanently fixed in
              vicinity of the isolation points. Must communicate: multiple sources exist + ALL
              switches must be operated to isolate + electric shock risk if not all operated
            </li>
            <li>
              <strong className="text-white">Notice location</strong>— typically at the main
              consumer unit / PEI distribution board. Consolidated notice listing all switches +
              their locations preferred over per-switch notices (clearer to read in emergency)
            </li>
            <li>
              <strong className="text-white">DC isolation</strong>— PV DC + BESS DC isolators are
              separate from AC isolators (different switching characteristics + voltage levels).
              Both AC + DC must be operated for full isolation
            </li>
            <li>
              <strong className="text-white">Customer-accessible vs trained-person</strong> —
              customer should be able to operate customer-facing isolators in emergency;
              trained-person isolation procedure (full multi-source isolation) documented for
              engineer attendance
            </li>
            <li>
              <strong className="text-white">Reg 514 family interaction</strong> — the multi-source
              warning notice sits alongside the PME warning notice + the BESS warning notice + the
              PV array warning notice. Cluttered — consolidated notice design helps
            </li>
            <li>
              <strong className="text-white">Future-proofing</strong>— if customer plans to add EV /
              V2G / wind later, warning notice + isolation procedure must be updated; not a one-time
              install task
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — isolation schematic
              showing every source + switch + warning notice text + photograph of installed notice +
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Every protective device point</strong> — main switch,
              sub-DBs, RCBO at each circuit, generator-side breakers. Each location needs
              overcurrent calculation
            </li>
            <li>
              <strong className="text-white">Every PEI configuration</strong> — DNO+PV+BESS+EV,
              DNO+PV+BESS, DNO+PV, BESS+PV island, BESS-only island, EV V2G + PV island, etc. The
              permutations matter
            </li>
            <li>
              <strong className="text-white">Minimum AND maximum</strong>— maximum = direct-feeding
              with all sources contributing (highest PSCC for breaker breaking capacity); minimum =
              island mode with only smallest inverter contributing (lowest fault current for
              disconnection time check)
            </li>
            <li>
              <strong className="text-white">Direct-feeding maximum PSCC</strong> — DNO Ipf (∼6-25
              kA UK 2025-26 typical) + inverter contribution. Breaker breaking capacity must exceed
            </li>
            <li>
              <strong className="text-white">Island mode minimum fault current</strong> — only
              inverter contribution (~1.1× rated). E.g. 5 kW single-phase → ~25 A max fault. Compare
              against Reg 411 ADS table 41.2/41.4 disconnection requirements
            </li>
            <li>
              <strong className="text-white">RCD effectiveness in island</strong> — Reg 826.1.1.2.2
              N-E bond ensures RCD has a fault path. Reg 415.1 30 mA still applies. RCD typically
              the primary ADS path in island mode (because overcurrent disconnection may not achieve
              required time)
            </li>
            <li>
              <strong className="text-white">Reg 826.1.2.2 direction + polarity</strong> —
              protective devices selected considering all possible directions of current flow +
              polarity. Bidirectional flow = directional discrimination considerations
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — PEI overcurrent study
              covering each protective-device location × each configuration × min + max current
              magnitudes; commissioning test results per configuration
            </li>
          </ul>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-10-section-2')}
