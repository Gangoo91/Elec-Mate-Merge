...
const inlineChecks = [
  {
    ...
    options: [
      'It is identical to a single-source install; the same circuit tests cover everything with no additions',
      'Testing is reduced because the additional sources back each other up and need fewer per-circuit checks',
      'The per-circuit tests extend to each practical source configuration, each source-side isolator, and RCD operation per source combination',
      'Only the default configuration needs testing, since that source state represents all the others',
    ],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      'A single whole-site test covering all sources together is sufficient for the commissioning record',
      'Only the largest source is tested, with the smaller ones assumed compliant by extension',
    ],
    correctIndex: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Just the EIC, since that single certificate proves the whole install is compliant',
      'Only the manufacturer manuals for each component, which the customer can refer to as needed',
      'No formal pack; the customer is shown the monitoring app at handover and that is sufficient',
      'Integrated EIC and per-technology DoCs, isolation procedure, operating modes, EREC and MCS packs, manuals, SEG steps and a signed acknowledgement',
    ],
    correctIndex: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      'Only a DNO copy is required, with the customer and installer relying on it as the single source',
      'A customer copy only, with the DNO and installer holding nothing once the install is signed off',
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    ...
    options: [
      'Tests in any order will do, provided all of them are eventually completed and recorded',
      'Only the BS 7671 Part 6 circuit tests, since the grid-connection and source-combination checks are the DNO’s job',
      'No additional V2G testing is required, because the existing PV+BESS commissioning already covers the new charger',
      'Part 6 and RCD tests on the V2G circuit, anti-islanding, RCD checks per new source combination, isolation update, overcurrent spot-check, OCPP/EMS and the G99 amendment',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Identify the RCD and circuit, correlate source behaviour with trip times, check for smooth-DC leakage, verify RCD effectiveness per source combination, review SPD/EMS timing',
      'Simply replace the RCD, since repeated tripping always indicates a faulty device that needs swapping',
      'No troubleshooting is warranted, because intermittent trips on a multi-source site are normal and expected',
      'Disable the RCD on that circuit, so the customer is no longer inconvenienced by the periodic trips',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'No maintenance is needed once the system is commissioned and signed off by the installer',
      'A daily customer check of every isolator and RCD, to catch any developing fault as early as possible',
      'An annual visual and functional inspection, a periodic inspection and test at ~5 years, an annual tariff/EMS review, and evidence updates on any change',
      'Maintenance is left entirely to the customer’s discretion, with no recommended service intervals at all',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Nothing; a renewable installation transfers with the property without any documentation at all',
      'The integrated EIC and DoCs, isolation procedure, operating modes, EREC and MCS paperwork, SEG steps, maintenance log and emergency contacts',
      'Only the original purchase receipts, which prove the equipment was paid for by the first owner',
      'Just the EIC, with the EREC, MCS and SEG paperwork staying with the original owner who installed it',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'The evidence is permanently lost, and the installation can never be documented again at all',
      'It makes no difference, because all of the records are held centrally by the DNO regardless',
      'The records pass automatically to whichever installer next takes over the customer’s area',
      'The customer’s own copy is the operative document, so resilient digital and printed copies should be kept',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A ~60–90 min walk-through: source/isolator tour, EMS app demo, isolation procedure, operating modes, SEG steps, contacts, pack and signed acknowledgement',
      'Handing over the paperwork pack alone, with no face-to-face walk-through of the installation',
      'No meeting is required, since the customer can work the whole system out from the manuals',
      'A brief doorstep handover of the documents, with any questions deferred to a later phone call',
    ],
    correctAnswer: 0,
    explanation:
