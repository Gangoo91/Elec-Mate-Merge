...
const BS7671Module4Section4 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 4 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'RCD types and what they detect, where 30 mA additional protection is mandatory (Reg 411.3.3, 411.3.4), how RCDs are tested at IΔn and 5×IΔn, and how to verify selectivity between upstream Type S and downstream final-circuit RCDs (Reg 536.4.1.4).'
          }
        </div>
        ...
        >
          <p>
            ...
            handles the pulsating DC residual produced by switching power supplies, LED drivers, and
            rectifier-fed loads. Type F (BS EN 62423) extends this to high-frequency residual
        ...
        >
          <p>
            Categories: (a) sockets ≤ 32 A in locations where they are liable to be used by ordinary
            persons (BA1) or children (BA2) — RCD mandatory, no exception; (b) sockets ≤ 32 A in
            other locations (e.g. controlled-access plant rooms, dedicated industrial workstations)
            — RCD mandatory by default but exceptable with documented risk assessment; (c) mobile
            equipment ≤ 32 A used outdoors — RCD mandatory, no exception. Reg 411.3.3 expressly does
            not apply to FELV (Reg 411.7) or RLV (Reg 411.8) systems — they have their own
            protective regimes.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            typically 30 mA for additional protection, 100 mA / 300 mA / 500 mA for fault protection
            on TT systems. The actual operating current is somewhere between 50% and 100% of IΔn — a
            30 mA Type AC may trip anywhere from 15-30 mA in practice. This tolerance band is what
            the 3:1 selectivity ratio (Reg 536.4.1.4) exists to manage. The trip mechanism itself is
            electromechanical — current in the sensing winding energises a solenoid that releases a
            latched contact carrier, opening the line and neutral contacts within ~10 ms typically.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Type F RCDs sit between Type A and Type B — they extend Type A's pulsating-DC detection
            to include composite high-frequency residuals (the kind produced by single-phase
            variable-speed drives, where the inverter chops the AC at high frequency). Type F can
            typically detect smooth DC up to about 10 mA before the toroid saturates — enough for
            some applications but not for three-phase VSDs or transformerless EV chargers. The
            waveform-sensitivity hierarchy is cumulative: every type detects everything the lower
            types detect, plus its own additional waveform sensitivity. Spec the lowest type that
            handles the load.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 531.3.4.1 mandates compliance with BS EN 61008 series for Type AC and Type A RCCBs,
            and BS EN 61009 series for RCBOs intended to be operated by ordinary persons. Reg
            531.3.4.201 adds that RCDs operable by ordinary persons must be designed or installed so
            that the residual operating current and time delay cannot be modified by the user —
            sealed against tampering. Type F and Type B RCBOs / RCCBs comply with BS EN 62423 (which
            extends 61008 / 61009).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 531.3.1.201: the RCD shall disconnect all live conductors of the circuit protected.
            The protective conductor shall not pass through the sensor of the RCD except where this
            is unavoidable (very rare — usually only specific bonded-CPC architectures). Reg
            531.3.1.202: it is not permissible to introduce an external connection for the purpose
            of intentionally creating a residual current to trip an RCD. The fix for nuisance
            tripping caused by genuine leakage is to address the leakage, not to defeat the device.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            a person making contact across line and earth can produce currents of 100-200 mA — and
            the device must clear quickly enough to limit physiological effects. The 40 ms ceiling
            at 5×IΔn aligns with the IEC 60479 body-current / shock-energy curves that BS 7671
            implicitly relies on. Type S RCDs are explicitly slower at IΔn (up to 500 ms) for
        ...
        >
          <p>
            ...
            60479's body-current curves). 30 mA was set high enough to allow reasonable load mixing
            without nuisance-tripping the RCDs available in the 1980s, but tight enough to provide
            meaningful additional protection against shock. With modern electronics raising standing
            leakage, the design budget is increasingly constrained — which is why RCBOs (one device
            per circuit) outperform shared RCDs (multiple circuits behind one 30 mA device): each
            RCBO sees only its own circuit's leakage, not the cumulative leakage from the whole
            busbar.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            available from all major manufacturers; the four-pole footprint is wider on the DIN-rail
            than single-phase but the wiring effort is similar.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The 'no-trip' mode some testers offer uses very short (sub-cycle) test pulses below the
            trip threshold to estimate operating time without actually tripping the RCD. Useful for
            periodic inspections where uninterrupted supply is critical (data centres, hospitals);
            slightly less precise than the trip-the-device method but acceptable for verification
            purposes per Reg 643.8 / GN3. For Type B RCDs, the tester must support DC-residual
            injection — older non-Type-B testers may give erroneously fast readings on Type B
            devices because they inject only AC.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The general principle is the same as the Reg 710.421.1.7 medical-AFDD prohibition: loss
            of supply on a safety-critical circuit could be more dangerous than the shock-protection
            benefit the RCD provides. Designers must understand the hierarchy — the general
            regulations (Reg 411.3.3 / 411.3.4) provide a default; specialist standards (Part 7, BS
            5839, BS 5266) override the default for specific circuits where supply continuity is
            paramount. Document the design choice on the EIC under Reg 120.3 (departures) so the
        ...
        >
          <p>
            ...
            demands it (per the equipment manual). For TT systems: 100 mA Type S as main switch + 30
            mA RCBOs on final circuits with the 3:1 ratio maintaining selectivity. For mixed-load
            three-phase commercial / industrial: detailed RCD-type plan per circuit, documented on
            the design sheet.
          </p>
        </ConceptBlock>
