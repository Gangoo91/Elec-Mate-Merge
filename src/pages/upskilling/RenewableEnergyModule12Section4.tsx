...
const inlineChecks = [
  {
    ...
    question: 'What is a "PEN fault" + why is it especially dangerous for outdoor LCT?',
    options: [
  ...
  {
    id: 'm12s4-722-411-4',
    question: 'What does Reg 722.411.4 say about PME on EV charging points + outdoor use?',
    options: [
  ...
  },
  {
    ...
    question: 'What is an RDC-DD + how does it differ from a standard RCD?',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'Standard Type A 30 mA RCD on an EV circuit — what fault scenario does it MISS?',
    options: [
  ...
  {
    question: 'OPDD functional test at IV — how is it verified?',
    options: [
...
export default function RenewableEnergyModule12Section4() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 12 · Section 4 · BS 7671:2018+A4:2026 · Reg 722.411.4 + Section 712 + outdoor LCT"
        title="PEN faults + open-PEN protection for outdoor LCT"
        backTo="../renewable-energy-module-12"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The PEN-fault risk vector for outdoor LCT — EV charging point, outdoor PV inverter, ASHP
          outdoor unit, outdoor BESS. Reg 722.411.4 EV outdoor PME restriction, with A4:2026 changes
          (former indent (a) deleted; methods now (b)–(e)). OPDD (Open-PEN Detection Device) +
          RDC-DD (Residual DC Detection) + TT alternative architectures. Shared protective logic
          across outdoor LCT.
        </p>
        ...
        <Pullquote>
          Open-PEN on outdoor EV is a real risk — that\'s why Reg 722.411.4 bars relying on PME for
          the outdoor charging-point earth unless one of methods (b) to (e) is used. The same logic
          applies to outdoor PV + ASHP + outdoor BESS even where the reg doesn\'t explicitly say so.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Normal PME operation</strong> — PEN combines neutral +
              protective conductor; customer-side bonding to MET + extraneous-conductive-parts; load
              current returns via PEN to transformer; earth reference stable
            </li>
            <li>
              <strong className="text-white">PEN open scenario</strong> — broken connection between
              transformer + customer MET (transformer end, street joint, service head);
              customer-side earth reference lost
            </li>
            <li>
              <strong className="text-white">Current redirection</strong> — load current that should
              return via PEN finds alternative path: customer earthing electrode (if
              TT-supplementary) + bonded metalwork (gas + water + structural) + adjacent property\'s
              earthing
            </li>
            <li>
              <strong className="text-white">Voltage rise</strong>— exposed-conductive-parts bonded
              to the MET sit at the voltage of the load returns; can rise to almost line voltage
              (~230 V) relative to remote true earth
            </li>
            <li>
              <strong className="text-white">Shock scenario</strong>— customer touches an
              exposed-conductive-part (EV charging point, outdoor PV inverter, ASHP outdoor unit)
              while standing on wet ground or touching another earthed item (parked vehicle, fence,
              drain) = current path through them
            </li>
            <li>
              <strong className="text-white">Indoor vs outdoor</strong> — indoor: bonded metalwork +
              flooring + footwear = reduced risk vector; outdoor: wet ground, vehicle contact,
              pavement, bare feet potentially = wider risk vector
            </li>
            <li>
              <strong className="text-white">EV-specific amplification</strong> — vehicle metalwork
              is large + customer + cable + ground simultaneously contacted during plug-in
            </li>
            <li>
              <strong className="text-white">Detection + mitigation</strong> — OPDD detects the
              open-PEN voltage anomaly + disconnects; TT alternative removes PME from the outdoor
              circuit entirely
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">(b) Separate earthing (TT-style)</strong> — local earth
              electrode dedicated to the charging point, meeting the earth-electrode +
              protective-conductor resistance condition; PME not relied on for the protective
              contact; RCD architecture per Section 722.531
            </li>
            <li>
              <strong className="text-white">(c) OPDD method</strong> — a device that measures the
              voltage between the CPC of the EV charging equipment and Earth and disconnects all
              live conductors of the affected circuit on detection of an open-PEN condition. Modern
              integrated EV charger feature
            </li>
            <li>
              <strong className="text-white">(d) / (e) Other specified methods</strong> — per the
              section; protective device per Reg 543.3.3.101(b), selected per Table 537.4;
              manufacturer + DoC + BS EN compliance
            </li>
            <li>
              <strong className="text-white">A4:2026 change</strong> — the former indent (a) was
              deleted + the indent (c) voltage-monitoring device redrafted + an indent added; valid
              methods are now lettered (b) to (e)
            </li>
            <li>
              <strong className="text-white">Indoor EV exception</strong> — Reg 722.411.4 targets
              outdoor charging point; an indoor EV charger on PME is permitted without explicit OPDD
              (subject to standard Section 722 RCD architecture)
            </li>
            <li>
              <strong className="text-white">"Reasonably expected to be used outdoors"</strong> — a
              charging point physically located inside (e.g. garage) but with a cable that reaches a
              vehicle parked outside requires consideration; verify manufacturer DoC + DNO position
            </li>
            <li>
              <strong className="text-white">DNO position</strong> — some DNOs strongly recommend TT
              alternative for outdoor EV; others accept OPDD-integrated equipment; cert evidence
              bundle records the chosen architecture + DNO position where relevant
            </li>
            <li>
              <strong className="text-white">Verification</strong>— at IV: OPDD presence +
              manufacturer DoC + BS EN compliance + functional test; OR TT earth electrode
              resistance + RCD architecture verified
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          OPDD catches the open-PEN. RDC-DD catches the smooth-DC fault. Type B RCD does both. Pick
          the right architecture per source + per location.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Operating principle</strong> — monitors voltage
              relationships in the circuit; identifies anomalous shift when PEN opens; triggers
              disconnect
            </li>
            <li>
              <strong className="text-white">Disconnect time</strong> — typically within 100-500 ms
              per manufacturer; well within Reg 411.3.2 maximum disconnection times
            </li>
            <li>
              <strong className="text-white">Integration in modern EV chargers</strong> — Zappi
              V2.1+, Ohme ePod, Hypervolt 3, Easee Home, Pod Point Solo 3 all include integrated
              OPDD as standard
            </li>
            <li>
              <strong className="text-white">Separate OPDD devices</strong> — for non-integrated
              equipment + retrofit applications; installed in the upstream supply circuit
            </li>
            <li>
              <strong className="text-white">Self-test features</strong> — modern OPDDs include
              self-test triggered via commissioning interface; some run scheduled background
              self-tests + log results
            </li>
            <li>
              <strong className="text-white">Verification at IV</strong> — inspect presence;
              manufacturer DoC reference; BS EN compliance (varies per product); functional test
              method per DoC; record in cert evidence bundle
            </li>
            <li>
              <strong className="text-white">Periodic verification at EICR-equivalent</strong> —
              re-trigger self-test where available; visual inspection of status indicators; check
              event log for any prior trips
            </li>
            <li>
              <strong className="text-white">Failure mode</strong> — OPDD failure = the disconnect
              path may not operate; modern designs default to safe state (charger disabled if OPDD
              reports fault) + alarm raised
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Type AC</strong>— AC residual only. NOT permitted for
              EV circuits per Reg 722.531 + manufacturer DoC + Reg 531.3.3 update (Type AC for fixed
              equipment with no DC components only)
            </li>
            <li>
              <strong className="text-white">Type A</strong>— AC + pulsating DC. Permitted for EV
              only with RDC-DD upstream or integrated. Modern domestic EV chargers typically include
              integrated RDC-DD; the upstream is then standard Type A 30 mA
            </li>
            <li>
              <strong className="text-white">Type F</strong>— AC + pulsating DC + composite
              waveforms; rarely used for EV
            </li>
            <li>
              <strong className="text-white">Type B</strong>— AC + pulsating DC + smooth DC across
              operating range. Built-in RDC-DD capability + smooth-DC detection. More expensive
              (~£100-200) + larger physical footprint. Used where RDC-DD not integrated in
              downstream equipment
            </li>
            <li>
              <strong className="text-white">RDC-DD 6 mA threshold</strong> — typical smooth-DC
              detection threshold; lower than Type B\'s 30 mA AC threshold; designed for the
              smooth-DC fault scenario
            </li>
            <li>
              <strong className="text-white">Reg 415.1 30 mA</strong> — additional protection 30 mA
              RCD remains the umbrella requirement
            </li>
            <li>
              <strong className="text-white">Manufacturer DoC governs</strong> — each EV charger\'s
              manufacturer DoC declares the upstream RCD type required + integrated RDC-DD presence;
              verify per product
            </li>
            <li>
              <strong className="text-white">Verification</strong>— RCD type per BS EN 61557-6 test
              (trip current + time); RDC-DD presence + manufacturer DoC + functional test where
              supported; record in cert evidence bundle
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Outdoor PV inverter</strong> — wall-mounted on garage
              exterior; metal casing; exposed to extraneous-conductive-parts. Best practice: OPDD on
              AC supply circuit OR TT alternative; Reg 712.421.101 IMD for DC side
            </li>
            <li>
              <strong className="text-white">ASHP outdoor unit</strong> — ground or wall-mounted;
              metal casing; condensate drainage to ground; customer + pet contact during
              maintenance. Protective conductor architecture per manufacturer + outdoor LCT good
              practice
            </li>
            <li>
              <strong className="text-white">Outdoor BESS</strong>— Tesla Powerwall outdoor variant
              + GivEnergy outdoor-rated + others; metal casing + grounded; outdoor exposure. Apply
              Reg 722.411.4 logic
            </li>
            <li>
              <strong className="text-white">Outdoor wind turbine</strong> — small wind /
              vertical-axis on customer property; Section 551 + outdoor exposure; OPDD or TT
              alternative for the AC supply circuit
            </li>
            <li>
              <strong className="text-white">Solar thermal outdoor controls</strong> — circulation
              pumps + sensors typically indoor (boiler room) but external pump on some pressurised
              systems; consider outdoor exposure
            </li>
            <li>
              <strong className="text-white">Micro-CHP outdoor unit</strong> — typically indoor
              (utility room or external building with conditioned space); occasionally outdoor on
              some commercial installs
            </li>
            <li>
              <strong className="text-white">Micro-hydro turbine + controls</strong> — outdoor by
              definition (water source); Section 551 + outdoor LCT good practice; TT alternative
              common
            </li>
            <li>
              <strong className="text-white">DNO + verifier alignment</strong> — verify the chosen
              architecture is acceptable to the DNO for the supply + records the position
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Dedicated local earth electrode</strong> — rod
              (typically 1-2 m driven) or plate (buried horizontally); cross-section per Reg 542;
              resistance verified per Reg 643.7.3 + Reg 644
            </li>
            <li>
              <strong className="text-white">Resistance threshold</strong> — TT system: total Ra ×
              IΔn ≤ 50 V (Reg 411.5.3); for 30 mA RCD: Ra ≤ 1666 Ω; in practice aim ≤ 200 Ω for
              headroom
            </li>
            <li>
              <strong className="text-white">Protective conductor routing</strong> — outdoor LCT
              circuits CPC routed to the local earth electrode; physical separation from the PME
              main earthing terminal
            </li>
            <li>
              <strong className="text-white">Main equipotential bonding</strong> — still serves the
              rest of the install per BS 7671 standard; gas + water + structural metalwork bonded to
              MET via PME as normal
            </li>
            <li>
              <strong className="text-white">RCD architecture</strong> — Type B 30 mA OR Type A +
              RDC-DD per outdoor LCT circuit; Reg 415.1 effectiveness verified
            </li>
            <li>
              <strong className="text-white">Multi-source coordination</strong> — Reg 551.4.2 RCD
              effectiveness across source combinations verified at IV
            </li>
            <li>
              <strong className="text-white">Periodic re-test</strong> — earth electrode resistance
              trend at EICR-equivalent; growing resistance indicates electrode corrosion + may need
              replacement / addition
            </li>
            <li>
              <strong className="text-white">Documentation</strong>— cert evidence bundle: earth
              electrode resistance + RCD architecture + verification matrix + DNO acceptance +
              manufacturer DoC
            </li>
          </ul>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-12-section-5')}
