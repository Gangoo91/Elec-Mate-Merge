...
const inlineChecks = [
  {
    ...
    question: 'What did A4:2026 change in Reg 643.3 (insulation resistance test)?',
    options: [
  ...
  },
  {
    ...
    question: 'What did A4:2026 change about RCD testing and Table 3A?',
    options: [
  ...
  },
  {
    ...
    question: 'What goes into the M11 commissioning cert evidence bundle for an LCT install?',
    options: [
  ...
  {
    id: 'm11s8-eicr-scope',
    question: 'What does the EICR scope for an LCT install verify in years 5-10 post-install?',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'What is the relationship between commissioning + the Year-1 verification visit?',
    options: [
  ...
  {
    question: 'How does the EICR report handle a post-A4:2026 install vs a pre-A4:2026 install?',
    options: [
...
export default function RenewableEnergyModule11Section8() {
  ...
  useSEO({
    title:
      'Commissioning verification — Chapter 81 + lightning + fault + anti-islanding | Renewable Energy 11.8 | Elec-Mate',
    description:
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 11 · Section 8 · BS 7671 Reg 643 Part 6 + Chapter 81 + BS EN 62305 + cert evidence bundle"
        title="Commissioning verification — Chapter 81, lightning, fault, anti-islanding"
        backTo="../renewable-energy-module-11"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Reg 643 Part 6 initial verification + Reg 643.3 redrafted IR sequence + RCD verification
          by a single AC test at IΔn (Table 3A deleted in A4:2026) + Chapter 81 efficiency
          verification + BS EN 62305 LPS inspection cycle + SPD records + anti-islanding
        ...
        <Pullquote>
          Eight sections, one chain. Chapter 81 efficiency. Lightning protection. SPDs. Multi-source
          fault. Anti-islanding. Reg 643 verification. The cert evidence bundle is where it all
          lands.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              ...
              — verify low-resistance path of protective conductors (CPC, bonding) using continuity
              tester; R1+R2 measurement for fault loop impedance pre-calculation
            </li>
            <li>
              <strong className="text-white">
                Insulation resistance (Reg 643.3 — A4:2026 redraft)
              </strong>{' '}
              — 500 V DC IR test pre-equipment connection (verify cable + accessory insulation);
              connect equipment; 250 V DC IR test post-equipment to confirm IR remains acceptable
              without damaging electronics
            </li>
            <li>
              <strong className="text-white">Protection by SELV / PELV (Reg 643.4)</strong> —
              verification of separation per Chapter 41
            </li>
            <li>
              <strong className="text-white">Polarity (Reg 643.6)</strong>— verify line + neutral
              correctly connected at all relevant points; particularly important for LCT installs
              with bidirectional sources
            </li>
            <li>
              <strong className="text-white">
                RCD operation (single AC test at IΔn — Table 3A deleted in A4:2026)
              </strong>{' '}
              — functional + an AC test at IΔn verifies the RCD regardless of Type (AC / A / F / B
              per Reg 531.3.3); Type B still selected where smooth DC residual current paths exist
              (BESS / EV / heat pump VSD)
            </li>
            <li>
              <strong className="text-white">Earth fault loop impedance (Reg 643.7)</strong> —
              measure Zs at each circuit + verify against design (Z_s_max for OCPD operating time
              per Reg 411.3.2); multi-source PEI may need per-source-configuration measurement
            </li>
            <li>
              <strong className="text-white">Functional tests (Reg 643.10)</strong> — switchgear,
              RCDs, interlocks, control circuits, EEMS, anti-islanding operate per design
            </li>
            <li>
              <strong className="text-white">
                Schedule of inspection + schedule of test results
              </strong>{' '}
              — per Reg 643.1 + 644 (deleted in A4:2026 renumbering); cert evidence record
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Stage 1 — pre-equipment IR</strong> — 500 V DC IR test
              on installation with all electronics + loads disconnected (PV inverter, BESS, EV
              charger, heat pump VSD, sensitive electronics isolated). Verify cable + accessory
              insulation integrity. Typical acceptable IR &gt; 1 MΩ
            </li>
            <li>
              <strong className="text-white">Stage 2 — connect equipment</strong> — connect all
              loads + electronics in normal operating configuration
            </li>
            <li>
              <strong className="text-white">Stage 3 — post-equipment IR</strong> — 250 V DC IR test
              on installation in normal operating configuration. Verifies overall IR with
              electronics in circuit without applying 500 V DC across them (which could damage)
            </li>
            <li>
              <strong className="text-white">Threshold</strong>— acceptable IR per Reg 643.3 + Table
              64 (e.g. &gt;1 MΩ at 500 V DC for SELV / PELV / FELV; &gt;1 MΩ at 230/400 V LV;
              &gt;0.5 MΩ at 250 V DC post-equipment per equipment standard)
            </li>
            <li>
              <strong className="text-white">Equipment-specific concerns</strong> — PV inverters +
              BESS + EV chargers + heat pump VSDs have surge protection internally that can shunt
              500 V DC test voltage; manufacturer DoC declares maximum applied test voltage +
              recommended sequence
            </li>
            <li>
              <strong className="text-white">Modern tester support</strong> — Fluke 1664/1674,
              Megger MFT1741, Metrel MI3155, Kewtech KT64DL etc. — most modern multifunction testers
              support 500 V + 250 V + multiple IR ranges. Tester manufacturer DoC + last calibration
              on schedule of test
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— schedule of test records BOTH
              500 V (pre-equipment) + 250 V (post-equipment) IR results + Reg 643.3 A4:2026
              compliance
            </li>
            <li>
              <strong className="text-white">DC side PV / BESS</strong>— DC strings + BESS DC have
              manufacturer-specific test methods (often lower DC test voltage 250-500 V vs the
              inverter\'s open-circuit voltage); follow PV + BESS manufacturer instructions
            </li>
          </ul>
        </ConceptBlock>
        ...
        <ContentEyebrow>
          Integrating Chapter 81 + lightning + SPD + fault + anti-islanding
        </ContentEyebrow>

        <Pullquote>
          One install, one commissioning, one cert evidence bundle. Reg 643 safety + Chapter 81
          efficiency + BS EN 62305 lightning + Section 443/534 SPDs + Reg 826 PEI + Reg 551.7.5
          anti-islanding — all integrated.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Pre-commissioning design records</strong> — Chapter 81
              per-circuit design + Reg 826.1.2.1 multi-source fault + BS EN 62305-2 risk + SPD
              selection rationale + G99 connection offer (where applicable) + EEMS configuration
            </li>
            <li>
              <strong className="text-white">Install completion</strong>— all LCT components
              installed per design; manufacturer DoCs collected; system configured + tested
              individually
            </li>
            <li>
              <strong className="text-white">Reg 643 safety verification</strong> — schedule of
              inspection + IR (500 V DC pre + 250 V DC post per A4:2026 redraft) + continuity +
              polarity + RCD verification by a single AC test at IΔn (Table 3A deleted) + EFLI +
              functional tests; schedule of test results
            </li>
            <li>
              <strong className="text-white">Chapter 81 efficiency verification</strong> — cable CSA
              matches design + voltage drop matches install + EEMS configured per design +
              monitoring points operational + annual energy estimate documented
            </li>
            <li>
              <strong className="text-white">BS EN 62305 records</strong>— risk assessment summary +
              LPS records + separation distance calculation (where applicable) + LPS specialist
              sign-off (where applicable) + earth termination test
            </li>
            <li>
              <strong className="text-white">SPD records</strong>— per location + Type + Uc + Up +
              In/Iimp + Iscc + manufacturer DoCs + Reg 443.4.1 + Reg 534.4 family + Reg
              712.534.102.1 compliance + visual status indicators initial state
            </li>
            <li>
              <strong className="text-white">Anti-islanding test</strong>— G98 simulated by
              installer + manufacturer DoC OR G99 DNO-witnessed (or accepted equivalent);
              disconnection time measured + recorded + DNO sign-off where applicable
            </li>
            <li>
              <strong className="text-white">PEI integration</strong>— Chapter 82 documentation:
              single-line diagram, changeover (Reg 551.6), island mode (Reg 826.1.1.5) test record,
              source placement per Reg 551.7.2.1
            </li>
            <li>
              <strong className="text-white">EEMS commissioning</strong>— EEMS commissioning sheet
              (Reg 825.1): coordinated devices, tariff signal, load priority, schedule, monitoring,
              customer training
            </li>
            <li>
              <strong className="text-white">
                Final cert evidence bundle + EIC + customer handover
              </strong>{' '}
              — assembled document set delivered to customer; installer keeps copy; future EICR
              scope
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">BS 7671 EIC</strong>— Electrical Installation
              Certificate per Appendix 6 BS 7671 + schedule of inspection + schedule of test results
            </li>
            <li>
              <strong className="text-white">Chapter 81 records</strong>— per-circuit efficiency
              design table; payback model; EEMS configuration; monitoring plan; annual energy
              estimate per LCT component; year-1 verification trigger
            </li>
            <li>
              <strong className="text-white">Chapter 82 PEI documentation</strong> — single-line
              diagram; operating modes (Reg 824.2); changeover (Reg 551.6); island mode test record
              (Reg 826.1.1.5); source placement per Reg 551.7.2.1; per- protective-device fault
              assessment per Reg 826.1.2.1
            </li>
            <li>
              <strong className="text-white">EEMS records (Reg 825.1)</strong>— EEMS commissioning
              sheet; coordinated devices; tariff signal; load priority; schedule; customer training
              acknowledgement
            </li>
            <li>
              <strong className="text-white">BS EN 62305 records</strong>— risk assessment summary
              (residential) or full report (commercial / heritage); LPS records + earth termination
              test (where applicable); separation distance calculation (where PV inside LPS); LPS
              specialist sign-off (where applicable)
            </li>
            <li>
              <strong className="text-white">SPD records</strong>— per location + Type + Uc + Up +
              In/Iimp + Iscc + manufacturer DoCs + Reg 443.4.1 + Reg 534.4 family + Reg
              712.534.102.1 compliance + visual status indicators
            </li>
            <li>
              <strong className="text-white">Anti-islanding records</strong>— Reg 551.7.4 + Reg
              551.7.5 compliance via G98 / G99 reference + manufacturer DoC + protection settings
              (G99) + commissioning test record + DNO sign-off (where applicable)
            </li>
            <li>
              <strong className="text-white">LCT manufacturer DoCs</strong> — PV inverter; BESS;
              heat pump; EV charger; other LCT per install; each with relevant standards (BS EN
              50549, IEC standards, MCS approvals)
            </li>
            <li>
              <strong className="text-white">MCS handover pack (where applicable)</strong> — MIS
              3002 PV; MIS 3005 heat pump; MIS 3003 wind; MIS 3008 hydro; etc. Integrates with BS
              7671 cert evidence
            </li>
            <li>
              <strong className="text-white">EREC reference</strong>— G98 post-installation
              notification or G99 connection offer + acceptance + DNO commissioning certificate
            </li>
            <li>
              <strong className="text-white">Customer handover pack</strong> — operations guide;
              energy app overview; SPD visual check instructions; emergency contacts; LPS + EICR
              cycle dates; year-1 verification trigger
            </li>
            <li>
              <strong className="text-white">Installer record</strong>— installer keeps full copy +
              serves as basis for future EICR, customer enquiries, insurance claims
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Trigger</strong>— typically 12 months post-install;
              Chapter 81 design records set the trigger date + criteria
            </li>
            <li>
              <strong className="text-white">Data review</strong>— actual annual kWh per LCT
              component (PV; BESS in/out; heat pump; EV; immersion diverter) vs Chapter 81 design
              estimate
            </li>
            <li>
              <strong className="text-white">Deviation threshold</strong>— typical &gt;20% deviation
              triggers investigation. Smaller deviations noted + tracked; large deviations actioned
            </li>
            <li>
              <strong className="text-white">Investigation causes</strong>— customer load profile
              differs from assumption; equipment performance differs from manufacturer claim; tariff
              change; install fault; weather variance (PV)
            </li>
            <li>
              <strong className="text-white">Customer engagement</strong>— installer / designer
              visits or remote review; updated load profile; possible install adjustment (e.g. add
              immersion diverter; reschedule EEMS); updated design records
            </li>
            <li>
              <strong className="text-white">Year-1 report</strong>— attached to original cert
              evidence bundle; updated payback model where applicable; customer-facing update +
              reinforcement of relationship
            </li>
            <li>
              <strong className="text-white">Future follow-ups</strong>— year-3 / year-5 reviews
              where customer / installer agree; supports EICR + LPS inspection coordination
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — year-1 verification
              report + ongoing monitoring data summary + customer feedback
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Install-date amendment</strong>— EICR codes against the
              amendment in force at install (e.g. A2:2022 for 2020 install). New regs added
              post-install are NOT retroactively imposed
            </li>
            <li>
              <strong className="text-white">Visual inspection</strong>— accessible components;
              signs of damage / overheating / corrosion; cable + accessory condition; CU + sub-board
              condition; SPD visual status indicators
            </li>
            <li>
              <strong className="text-white">Continuity + IR</strong>— per Reg 643.2 + Reg 643.3
              (A4:2026 redraft applies to test method regardless of install date — 250 V DC
              post-equipment is good practice for electronics)
            </li>
            <li>
              <strong className="text-white">RCD operation</strong>— single AC test at IΔn (current
              method; Table 3A deleted in A4:2026); Type identification per Reg 531.3.3 + functional
              test
            </li>
            <li>
              <strong className="text-white">EFLI</strong> — Zs measurement; verify against design +
              Reg 411.3.2 disconnection time
            </li>
            <li>
              <strong className="text-white">Functional tests</strong>— switchgear, RCDs,
              anti-islanding (inverter self-test), EEMS operation, monitoring active
            </li>
            <li>
              <strong className="text-white">LPS visual</strong>— air termination integrity + down
              conductor condition + earth electrode visible; LPS specialist + measurement at LPS
              inspection cycle dates
            </li>
            <li>
              <strong className="text-white">Codes</strong> — C1 immediate danger; C2 potentially
              dangerous; C3 improvement recommended; FI further investigation. Per Appendix 6
            </li>
            <li>
              <strong className="text-white">Report references</strong>— install-date amendment +
              EICR-date amendment + any divergence + recommended actions
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle update</strong> — EICR report
              added to bundle; customer evidence chain continues
            </li>
          </ul>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-11-section-7')}
