  ...
  return (
    <HubPage>
      <HubMasthead
        ...
        backTo="../bs7671-module-8"
      />
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'The appendices are where BS 7671 keeps its working data — cable tables, device curves, model forms, statutory cross-references and the wording library that makes a cert defensible. Learn the layout once and the Brown Book stops being a haystack.'
          }
        </div>
        ...
        >
          <p>
            ...
            (Assessment), 4 (Protection), 5 (Selection &amp; Erection), 6 (Inspection &amp; Testing)
            and 7 (Special Locations) carry the &quot;shall&quot; statements. The appendices carry
            the supporting data and model forms. Some appendices are
        ...
        >
          <p>
            App 1 is informative. It does not impose product requirements directly — those come from
            the relevant regulation in the body (e.g. Reg 411.4.4 references the device standard,
            App 1 looks up the standard reference). The list is essential when specifying equipment
            from a manufacturer&apos;s datasheet: the BS or BS EN number on the datasheet should
            appear in App 1 with the same meaning. Where a standard has been withdrawn or replaced,
            App 1 records the latest version. Common references: BS EN 60898-1 (MCBs), BS EN 61009-1
            (RCBOs), BS EN 62423 (Type A / B RCDs), BS EN 50550 (PEN-fault detectors), BS EN 61851
            (EV charging equipment), BS 7671 itself.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            self-employed persons to ensure electrical systems are constructed and maintained so as
            to prevent danger. Regs 4(1)-(4), 5, 8, 12, 13, 14 and 16 are the most frequently cited
            in HSE prosecutions. HSWA 1974 (Health and Safety at Work etc. Act) is the over-arching
            statute. Building Regulations Part P (England &amp; Wales) imposes specific notification
            and self-certification regimes for domestic electrical work. The Building (Scotland)
            Regulations and the Building Regulations Northern Ireland set the equivalents in those
            jurisdictions. Reading App 2 once is the fastest way to understand why a designer who
            wires to BS 7671 is also discharging a legal duty — and why a departure under Reg 120.3
            needs documenting.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The time-current curve shows tripping/operating time on the y-axis against fault current
            (as a multiple of In) on the x-axis. Type B MCBs trip in the magnetic- instantaneous
            range at 3-5×In; Type C at 5-10×In; Type D at 10-20×In. BS 88 fuses have steeper, more
            energy-let-through-aware curves; BS 3036 rewireable fuses (still found in older domestic
            installations) have considerably worse let-through and require tighter Zs values, which
            is why GN3 typically codes them C2 / C3 on EICR for that reason alone. Tabulated Zs
            maxima in App 3 already include the temperature-correction factor (Cmin) — the designer
            reads Zs(max) directly without extra calculation. App 3 also includes data for
            low-voltage HRC fuses to BS 88-2 (general purpose) and BS 88-3 (motor-circuit
            applications), and the energy let-through (I²t) values needed for Reg 434.5 cable
            thermal-withstand calculation.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            Zs(max) calculation. Cable selection in App 4 is a five-step exercise: (1) calculate Ib,
            (2) pick a cable construction, (3) pick the Reference Method that matches your install,
            (4) apply correction factors to It to derive Iz, (5) verify voltage drop per Reg 525.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 132.5.1 makes external-influence assessment mandatory for the designer. App 5 gives
            the controlled vocabulary: rather than write &quot;wet area with some impact risk&quot;,
            the designer writes &quot;AD3 / AG3&quot;. Equipment manufacturers specify their
            products against these same codes (e.g. an outdoor weatherproof socket rated IP66 covers
            AD4 — splashes from any direction, plus dust ingress). Special Locations in Part 7 use
            App 5 codes extensively: Section 701 bathrooms specify zone codes that map to AD water
            classifications; Section 705 agricultural premises map to BA2 (children), BD2 (escape
            difficult), BE2 (fire risk).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            present, immediate remedial action required), C2 (potentially dangerous, urgent remedial
            action required), C3 (improvement recommended) or FI (further investigation required).
            Reg 644.1 and Reg 644.2 require the relevant certificate to be issued; equivalent forms
            may be used provided they include all the same information. In practice the Brown Book
            App 6 forms are the standard implemented by every certification scheme and software
            vendor. Issuing on a pre-A4 form set after 15 April 2026 (or 15 October 2026 for those
            still on A3) puts the cert on the wrong version — see Module 2 (legal framework) for the
        ...
        >
          <p>
            ...
            gap. App 7 also covers identification of conductors used as switched lines (e.g. a brown
            sleeve on what would otherwise read as a blue neutral when used as a switched line in a
            two-way switching arrangement) — Reg 514 ties this back to the regulation text.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            App 8 references Reg 543.7 (high earth-leakage equipment) and Reg 543.7.1.103 (specific
            CPC requirements where the protective-conductor current exceeds 10 mA). The design
            controls: (a) CPC of high integrity — either two CPCs each capable of carrying the
            protective-conductor current, OR a single CPC of cross-section ≥ 10 mm² copper, (b)
            dedicated final circuit with no other equipment, (c) warning label identifying high
            leakage. The label wording is itself prescribed in App 8. Where high-leakage equipment
            is on a TN-S supply, the design risk is that an open CPC would otherwise allow the
            steady leakage to drive the local exposed-conductive-parts to a hazardous voltage —
            duplicated CPC removes the single-point failure.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            automatically), (2) record the highest of the L-N (PSCC) and L-PE (PEFC) values as Ipf,
            (3) for downstream points, calculate using the impedance-based method (Ipf at any point
            = U₀ / Zs at that point, plus appropriate corrections). The breaking-capacity check is
            then per-device: every OPD must have an Icn ≥ Ipf at its location, including the
            consumer-unit main switch and any DBs. App 14 also covers the case where PSCC at origin
            exceeds the breaking capacity of any planned protective device — typically resolved by
            adding a suitably-rated upstream HRC fuse or by switchgear with higher breaking
            capacity. The cert entry is unambiguous: Ipf is the design-input number, not a test
            result that can be conveniently &quot;rounded down&quot;.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            GN3 references App 16 throughout the EICR sections. The model items cover every part of
            a typical periodic inspection: consumer unit / distribution board, accessible wiring,
            accessories, equipotential bonding, special locations, isolators and switchgear,
            lighting, supplies. Each item carries a recommended classification guidance (C1 / C2 /
            C3 / FI) tied to the typical risk profile, but the engineer in charge classifies the
            actual observation against the actual installation — the App 16 wording is the language,
            not the verdict. GN3 also makes clear that every observation must be classified (C1, C2,
            C3 or FI) — there is no &quot;satisfactory with notes&quot; option, and an EICR with any
            C1 or C2 cannot be marked &quot;satisfactory&quot;.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            the design considerations for energy efficiency are now better placed in the regulation
            body and in the broader Energy Performance of Buildings legislation, neither of which is
            duplicated by an appendix. Annex B443 (overvoltage risk assessment for SPD selection)
            was a methodology annex; A4 builds the risk-assessment route into Section 443 itself,
            which means a designer reading 443.4 (Risk Assessment Method) gets the entire procedure
            inline rather than flicking to an annex. Neither deletion changes designer obligations
            or the safety outcome — both tighten the structure of the standard.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            into a regulation number; (3) the regulation text cross-references the appendix; (4) the
            appendix gives the data. With practice the path is automatic. For everyday site-work,
            four bookmarks make life much easier: App 3 (device curves), App 4 (cable tables), App 6
            (model forms), App 16 (EICR wording). Carry sticky tabs on the spine of the Brown Book
            and you will find what you need in seconds.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The Brown Book is a tool. Four sticky tabs is the difference between &quot;flick through
            600 pages&quot; and &quot;open at App 4 in two seconds&quot;. For office / design-bench
            work, a digital copy with bookmarks (PDF or the IET app) does the same job. For site
            work, a tabbed paper copy survives spilled tea and dropped tools better than a phone.
            Choose your tool and tab it once.
          </p>
        </ConceptBlock>
