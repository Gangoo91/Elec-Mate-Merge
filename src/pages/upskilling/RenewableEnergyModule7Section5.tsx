...
const inlineChecks = [
  {
    ...
    options: [
      'Static gives each charger a fixed pre-set share; dynamic adapts in real time via a CT clamp',
      'They are the same approach with only a different marketing name',
      'Static DLM is not permitted on any commercial EV installation',
      'Dynamic DLM is not permitted because it cannot guarantee a fixed allocation',
    ],
    correctIndex: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A BESS cannot support peak-shaving on a commercial EV site',
      'Peak-shaving with a BESS only applies to residential single-charger installs',
      'It charges off-peak and discharges at peak, capping grid import at supply capacity',
      'It works exactly like static DLM, simply throttling chargers to a fixed share',
    ],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A flat single layer where all chargers share one fixed limit with no hierarchy',
      'A daisy-chain where each charger passes its spare current to the next one downstream',
      'A grouping fixed by physical phase rotation, so allocation cannot adapt to demand',
      'Hierarchical — site-level coordinates clusters, cluster-level allocates, charger applies via CP PWM',
    ],
    correctIndex: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'No future-proofing is possible once the site has been energised',
      'Spare cable, ways, transformer kVA, switchgear rating, containment and DNO headroom',
      'It is left for the customer to decide and arrange at a later date',
      'Future-proofing is unnecessary because EV demand never grows after install',
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    question:
      'Workplace site with 6 × 22 kW chargers + three-phase 100 A supply. DLM is mandatory because:',
    options: [
      'DLM is not actually mandatory and the chargers can all run at full rating',
      'DLM is fitted purely for aesthetic reasons to tidy up the installation',
      'Six chargers draw far more than the 100 A supply, so DLM throttles them to fit',
      'It was only added because the customer specifically requested the feature',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'An EMS prioritises PV to EV, then BESS, then grid within the DNO limit, via OCPP profiles',
      'Each source and charger operates independently, the BESS always charging from grid first',
      'The PV inverter alone allocates current, throttling chargers only when it is exporting',
      "Coordination is handled by the DNO smart meter, setting each charger's limit remotely",
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Dynamic DLM must always be used; static is never acceptable on any site',
      'Static DLM is obsolete and no longer permitted on any commercial site',
      'Dynamic DLM is only ever used on residential single-charger installs',
      'Static suits stable, predictable loads with comfortable headroom; dynamic where supply is tight',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Exactly one charger per cluster in all cases, whatever the site size',
      'As many chargers as the sub-DB has ways, to minimise the number of clusters',
      'Typically 4-8 chargers per cluster, balancing DLM efficiency against complexity',
      'One cluster per electrical phase, so three clusters on any three-phase site',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Around 1 kWh regardless of the site demand or charger count',
      'Multiple GWh for any commercial workplace site, however small',
      'The customer guesses the capacity with no supporting calculation',
      'The supply shortfall over the peak duration — e.g. 32 kW for 3 hours ≈ 96 kWh',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Nothing beyond exactly matching today’s charger count and demand',
      'Spare CU ways, oversized cable, transformer headroom, switchgear and DNO headroom',
      'Only the current need should be met, ignoring any possible future growth',
      'Maximum-rated everything, sizing the supply to every charger at full power at once',
    ],
    correctAnswer: 1,
    explanation:
...
export default function RenewableEnergyModule7Section5() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 7 · Section 5 · BS 7671:2018+A4:2026 · Reg 311.1 + 722.311.201 + Chapter 82"
        title="DLM at scale & charging clusters"
        backTo="../renewable-energy-module-7"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Dynamic Load Management at commercial scale — coordinating multi-charger sites within
          constrained supply. Static vs dynamic, peak-shaving via BESS, hierarchical cluster
          topology, future-proofing, Chapter 82 PEI integration.
        </p>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Fixed allocation</strong> — e.g. 4 chargers on a 100 A
              supply = 25 A each fixed, regardless of other site load
            </li>
            <li>
              <strong className="text-white">No CT clamp</strong> — no real-time monitoring of site
              current
            </li>
            <li>
            ...
            <li>
              <strong className="text-white">Inefficient</strong> — EV chargers throttled even when
              other loads are low
            </li>
            <li>
              <strong className="text-white">Predictable</strong> — operator knows exactly what each
              charger will deliver under any condition
            </li>
            <li>
              <strong className="text-white">Use cases</strong> — dedicated EV depot, stable site,
              comfortable supply headroom, simplicity prized
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">CT clamp on main tails</strong> — reads actual site
              current per phase
            </li>
            <li>
              <strong className="text-white">DLM controller</strong> — built into a master wallbox
              (MyEnergi Zappi + libbi extension; Wallbox Pulsar Plus + Power Boost; Easee +
              Equalizer), standalone hardware (EO Hub), or CPMS-managed (Driivz, ChargePoint dynamic
              profiles)
            </li>
            <li>
              <strong className="text-white">Real-time allocation</strong> — calculates available
              headroom (supply limit - non-EV current) every few seconds; allocates across chargers;
              sends via OCPP / proprietary protocol
            </li>
            <li>
              <strong className="text-white">CP PWM application</strong> — chargers apply allocated
              current via CP PWM duty cycle to vehicles; vehicles respond within sub-second
            </li>
            <li>
              <strong className="text-white">Reg 722.311.201 basis</strong> — the EV-specific load
              curtailment carve-out in BS 7671; makes the dynamic allocation legal in the max demand
              calc
            </li>
            <li>
              <strong className="text-white">Cost</strong> — ~£500-1,500 per site additional
              hardware + commissioning. Pays back via avoided supply upgrade cost
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — CT clamp position +
              supply limit + DLM allocation logic + commissioning test (verified throttle response
        ...
        <Pullquote>
          BESS turns a constrained DNO supply into an apparent infinite supply. Peak EV demand draws
          from stored energy; grid sees only the average.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Sizing</strong> — kWh = (peak EV demand minus supply) ×
              peak duration + cycle-life headroom. Example workplace: 132 kW peak EV on 100 kW
              supply = 32 kW shortfall × 3 hour peak = 96 kWh; with 80% DoD + headroom = ~150 kWh
              BESS
            </li>
            <li>
              <strong className="text-white">Charging</strong> — BESS charges during off-peak
              (overnight; tariff cheap window; PV surplus during day if PV present)
            </li>
            <li>
              <strong className="text-white">Discharging</strong> — BESS discharges during peak EV
              demand window (typically morning + evening); supplies shortfall between supply
              capacity and EV demand
            </li>
            <li>
              <strong className="text-white">Grid view</strong> — DNO sees grid import capped at
              supply capacity, never peaking. Smoother profile + lower connection cost
            </li>
            <li>
              <strong className="text-white">Tariff arbitrage</strong> — charge BESS at off-peak
              (Octopus Flux 12p; Cosy 13p; Intelligent Go 7p); discharge during peak EV (avoid
              Octopus Flux 33p+ peak). Annual savings ~£10-50k typical at commercial scale
            </li>
            <li>
              <strong className="text-white">Capacity market + frequency response</strong> — BESS
              can participate in National Grid services (capacity market auction; firm frequency
              response; dynamic containment). Additional revenue stream ~£10-30/kW/year
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — Chapter 57 BESS install
              + Section 722 EV install + Chapter 82 PEI integration. EMS configuration + OCPP
              smart-charging + BESS dispatch logic
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Priority 1: PV → EV</strong> — free energy; zero-export
              benefit; self-consumed PV is most valuable. EV allocation = PV instantaneous output
            </li>
            <li>
              <strong className="text-white">Priority 2: BESS → EV (peak)</strong> — during peak
              grid tariff hours, BESS supplies EV; grid import minimised. Tariff arbitrage benefit
            </li>
            <li>
              <strong className="text-white">Priority 3: Grid → EV</strong>— when PV insufficient +
              BESS exhausted, grid supplies; within DNO supply limit
            </li>
            <li>
              <strong className="text-white">BESS charging source</strong> — PV surplus
              (zero-export) during day; off-peak grid (cheap tariff) overnight
            </li>
            <li>
              <strong className="text-white">EMS hardware</strong> — typically the inverter’s
              manufacturer EMS (MyEnergi Zappi+Libbi, SolarEdge EMS, Tesla Powerwall+Wall Connector,
              Fronius Symo Hybrid + Wallbox) or a third-party EMS (Loxone, Modbus-based custom)
            </li>
            <li>
              <strong className="text-white">OCPP integration</strong> — CPMS receives EV demand +
              EMS dispatches via OCPP SetChargingProfile
            </li>
            <li>
              <strong className="text-white">Chapter 82 (PEI)</strong> — regulatory framework. All
              sources + sinks form a Prosumer’s Electrical Installation. Reg 826.x covers
              integration; anti-islanding (Reg 551.7.5) + neutral handling in island mode (Reg
              826.1.1.2.2)
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — integrated section 712
              + chapter 57 + section 722 + chapter 82 records
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Site level</strong> — master DLM controller; CT clamp
              on main incoming tails; reads total site current per phase; coordinates across
              clusters
            </li>
            <li>
              <strong className="text-white">Cluster level</strong> — 4-8 chargers on shared sub-DB;
              cluster DLM allocates within the cluster’s share of site supply
            </li>
            <li>
              <strong className="text-white">Charger level</strong> — each charger applies its
              allocated current to the connected vehicle via CP PWM
            </li>
            <li>
              <strong className="text-white">Priority + fairness logic</strong> — CPMS can override
              defaults: VIP / fleet priority drivers get full allocation; lower-priority sessions
              throttle first; minimum 6 A floor for any active session
            </li>
            <li>
              <strong className="text-white">OCPP 2.0.1 hierarchical profiles</strong> — site /
              cluster / charger profile hierarchy; native protocol support; CPMS issues hierarchical
              SetChargingProfile messages
            </li>
            <li>
              <strong className="text-white">Cluster sizing</strong> — 4-8 chargers typical sweet
              spot. Smaller: reduced DLM efficiency. Larger: increased complexity + reduced fairness
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — site map + cluster
              boundaries + sub-DB locations + CT clamp positions + DLM allocation logic +
              commissioning verification per cluster
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Cable cross-section</strong> — oversize by 30-50% for
              future load growth (e.g. 16 mm² where 10 mm² would suffice today)
            </li>
            <li>
              <strong className="text-white">CU / sub-DB ways</strong>— leave 50-100% spare ways for
              future chargers
            </li>
            <li>
              <strong className="text-white">Transformer kVA</strong> — size 1.5-2× current peak
              demand. UK 2025-26 typical workplace install 400-630 A LV; future-proof to 800-1,000 A
              is common
            </li>
            <li>
              <strong className="text-white">LV switchgear</strong> — spare rating + spare outgoing
              ways
            </li>
            <li>
              <strong className="text-white">Conduit + containment</strong>— pull spare conduit at
              first install — much cheaper than re-pulling later (avoid trenching + civils for
              future cables)
            </li>
            <li>
              <strong className="text-white">DNO supply agreement</strong> — negotiate
              future-expansion headroom in the original connection agreement. Pre-paid grid capacity
              is much cheaper than new application later
            </li>
            <li>
              <strong className="text-white">BESS expansion path</strong>— modular BESS that can add
              capacity in chunks (multi-module designs)
            </li>
            <li>
              <strong className="text-white">Cost uplift at install</strong> — ~15-25% typical. Pays
              back if expansion happens within ~5-7 years
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong>— design headroom +
              expansion capacity + Y5-Y10 scenarios documented
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Future-proofing costs 20% at install and saves 200% at year 5. The maths is obvious once
          the customer has been through one expansion cycle.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Per-site DLM</strong> — local CT clamp + cluster
              topology. Operates autonomously when CPMS comm lost
            </li>
            <li>
              <strong className="text-white">CPMS aggregation</strong> — corporate CPMS reads each
              site’s state via OCPP; aggregates into portfolio dashboards + reports
            </li>
            <li>
              <strong className="text-white">Corporate-wide policies</strong> — applied across all
              sites via OCPP SetChargingProfile. Examples: throttle to 50% during Triad peaks;
              preferentially PV self-consumption; demand-response participation
            </li>
            <li>
              <strong className="text-white">ESG reporting</strong> — portfolio-level CO₂ saved, kWh
              delivered, charge sessions, % PV self-consumed. Increasingly required by corporate
              sustainability reporting (CSRD, TCFD)
            </li>
            <li>
              <strong className="text-white">Demand-response revenue</strong> — large portfolios
              participate in National Grid demand response programmes (Dynamic Containment, Demand
              Flexibility Service) via CPMS-coordinated throttling. £-thousands/site/year revenue
            </li>
            <li>
              <strong className="text-white">Tariff arbitrage at scale</strong> — portfolio-level
              coordination of off-peak charging across multiple supply contracts; smarter than
              site-level alone
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — per site: local DLM +
              CPMS integration. Corporate: multi-site coordination policy + portfolio architecture
            </li>
          </ul>
        </ConceptBlock>
