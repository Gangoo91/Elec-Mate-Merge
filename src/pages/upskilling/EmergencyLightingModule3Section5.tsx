...
const inlineChecks = [
  {
    ...
    options: [
      'Free designer choice.',
      'BS 5266-1:2025 Annex referencing BS EN 60617 graphic symbols — a luminaire body with a small letter / marking distinguishing maintained (M), non-maintained (NM), high-risk (HR), self-test (ST), self-contained (SC) and central-battery-fed (CB) variants. The symbol carries the luminaire type, mounting orientation, and reference to the schedule.',
      'Architect personal preference.',
      'No standard exists.',
    ],
    correctIndex: 1,
    explanation:
      'BS 5266-1:2025 references BS EN 60617 for graphic symbols and specifies emergency-lighting-specific markings. Each symbol shows luminaire type (downlight, bulkhead, sign), emergency function (M/NM/HR), supply type (SC/CB), and links to the schedule for fitting reference, photometric distribution and circuit number.',
  },
  {
    id: 'elm3-s5-grid',
    ...
    options: [
      '5 m × 5 m.',
      'Typically 0.5 m × 0.5 m or 1 m × 1 m on the target plane (escape route at floor; vertical face for safety equipment) — fine enough that the minimum value is captured at every relevant point. Coarse grids (e.g. 5 m) miss localised dim spots; very fine grids (e.g. 0.1 m) increase computation without practical benefit.',
      '10 m × 10 m.',
      'No grid is needed.',
    ],
    correctIndex: 1,
    explanation:
      'BS EN 1838:2024 §5.3 effectively requires a calculation grid fine enough to capture the minimum at every relevant point. 0.5 m or 1 m grids are typical industry practice; the design software documents the grid and the minimum found. The grid output is part of the commissioning evidence pack.',
  },
  {
    id: 'elm3-s5-scale',
    ...
    options: [
      '1:5000.',
      '1:50 for small detail / specific areas (refuge points, high-risk task plans, sign details), 1:100 for typical floor layouts. The scale must permit clear identification of luminaire positions, route highlighting and the photometric grid annotations. Smaller scales (1:200 and below) often lose detail required for verification.',
      '1:1.',
      'Always 1:1000.',
    ],
    correctIndex: 1,
    explanation:
      'Emergency lighting drawings sit between 1:50 (detail) and 1:100 (general floor plan). The scale chosen must let the inspector read luminaire references and grid values without zoom. Drawings produced at 1:200 or smaller require larger paper or digital zoom to be useful.',
  },
  {
    id: 'elm3-s5-docs',
    question: 'BS 5266-1:2025 documentation requirements for the emergency lighting design include...?',
    options: [
      'Just a sales receipt.',
      'A design pack containing: layout drawing(s) with luminaire positions and references, schedule of luminaires (type, photometric, mounting, circuit), photometric calculation output (grid lux at every grid point), risk-assessment-referenced uplift justification, schedule of test points for commissioning, schedule of safety-equipment vertical-lux points. The pack is auditable evidence of the design.',
      'A photo of the building.',
      'Nothing — verbal handover is sufficient.',
    ],
    correctIndex: 1,
    explanation:
      'BS 5266-1:2025 §7 specifies the design documentation pack. Layout + schedule + photometric output + risk-assessment-link + commissioning schedule + safety-equipment schedule. Each component supports a different stakeholder — installer (layout + schedule), commissioning engineer (photometric + commissioning schedule), dutyholder (risk-link + schedule), inspector (everything).',
  },
];
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: [
      'Just a circle.',
      'Luminaire type (per the schedule), photometric distribution code, mounting orientation arrow (for asymmetric distributions), circuit number, and reference label linking to the schedule for full specification.',
      'Just a number.',
      'Decorative artwork.',
    ],
    correctAnswer: 1,
    explanation:
      'A luminaire symbol on a compliant drawing communicates type, photometric, orientation, circuit and reference. Each parameter feeds a different consumer — installer needs orientation; commissioning engineer needs circuit; inspector needs reference to the schedule.',
  },
  {
    id: 5,
    ...
    options: [
      ...
      'Point-by-point grid lux values (or contour map) on the target plane with minimum, maximum and uniformity ratio identified, calculated at 0% surface reflectance per BS EN 1838:2024, with the calculation grid spacing and luminaire positions clearly shown.',
      'Just maintenance factor.',
      'Marketing claims.',
    ],
    correctAnswer: 1,
    explanation:
      'The photometric report is the primary evidence of design lux compliance. Point-by-point grid + minimum + maximum + uniformity at 0% reflectance. Graphical contour maps support visual interpretation; numerical grids support precise audit.',
  },
  {
    id: 6,
    ...
    options: [
      'Every drawing.',
      'Height-critical areas: stairs (showing tread illumination), high-bay industrial (showing luminaire-to-target throw), atria with mezzanines (showing how each level receives illumination), and any refuge area where door swings or wall projections affect the lighting envelope.',
      'Cosmetic purposes.',
      'Never.',
    ],
    correctAnswer: 1,
    explanation:
      'Plan views show horizontal coverage; cross-sections show vertical relationships. Stairs, high-bay, atria, refuges all benefit from cross-section illustration of how the luminaire reaches the target. Standard floor plans do not need cross-sections; height-critical areas do.',
  },
  {
    id: 7,
    ...
    options: [
      'Just the count.',
      'Per luminaire: reference (e.g. EM1), manufacturer + model, photometric file (LDT/IES), maintained / non-maintained / high-risk function, supply type (self-contained / central battery), mounting type and height, photometric distribution, beam orientation if asymmetric, circuit number, and any specific commissioning notes.',
      'Manufacturer marketing copy.',
      'A QR code only.',
    ],
    correctAnswer: 1,
    explanation:
      'The schedule is the per-luminaire specification. Each row links a drawing reference to a fully specified product and circuit. The installer orders to the schedule; the commissioning engineer tests against the schedule; the inspector audits against the schedule.',
  },
  {
    id: 8,
    question: 'Sign details on the emergency lighting drawings should show...?',
    options: [
      'Generic sign symbols only.',
      'Each emergency exit sign with: location, sign type (illuminated / non-illuminated), pictogram size (which determines viewing distance: 100 × pictogram height), mounting height, orientation (which way the running-man points), and the luminaire (for externally illuminated signs) or self-contained classification (for internally illuminated).',
      'No sign details required.',
      'Manufacturer logo only.',
    ],
    correctAnswer: 1,
    explanation:
      'Sign details on drawings are part of the design evidence. Pictogram size determines viewing distance (100 × the pictogram height per BS EN ISO 7010). Direction (which way the running-man points) determines which exit the sign indicates. Lit / unlit / self-contained classification matters for sign duty under BS 5266.',
  },
  {
    id: 9,
    question: 'A safety-equipment schedule on the layout drawings is required to...?',
    options: [
      'List the luminaires only.',
      'List every fire alarm call point, firefighting equipment item, first-aid post and refuge call point with the dedicated luminaire that provides the 5 lx vertical illuminance on its face — so the inspector can verify each device has its required lighting.',
      'Show the cleaning schedule.',
      'Show the building manager.',
    ],
    correctAnswer: 1,
    explanation:
      'The safety-equipment schedule maps every device requiring 5 lx vertical to a specific luminaire that delivers it. Without the schedule the inspector cannot verify the vertical-lux duty per device; with it, every device is auditable.',
  },
  {
    id: 10,
    ...
    options: [
      'Only the drawings.',
      'A design pack: drawings, schedule, photometric output, risk-assessment-referenced uplift justification, schedule of commissioning test points, schedule of safety-equipment vertical-lux points, manufacturer datasheets, photometric data files (LDT/IES), maintenance factor evidence. The pack is the auditable record of the design and supports commissioning, periodic test and future modifications.',
      'A bill only.',
      'Nothing else.',
    ],
    correctAnswer: 1,
    explanation:
      'BS 5266-1:2025 §7 prescribes the documentation pack. Drawings + schedule + photometric output + risk-link + test-point schedule + safety-equipment schedule + datasheets + photometric data + MF evidence. Each item has its consumer; the pack as a whole evidences the design through its life cycle.',
  },
];
