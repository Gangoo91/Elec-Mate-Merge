import { ArrowLeft, Car, CheckCircle, Clock, BookOpen, Zap, Calculator, Shield, BarChart, Wrench, Award, FileCheck } from 'lucide-react';
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 auto-rows-fr">
            {modules.map((module) => (
              <Link
                key={module.id}
                to={module.id === "mock-exam" ? "ev-charging-mock-exam" : `../ev-charging-module-${module.id}`}
                className="block h-full"
              >
                <Card className={`bg-card border-border/30 hover:border-primary/40 transition-all duration-300 hover:bg-card/80 cursor-pointer group h-full flex flex-col active:scale-[0.98] ${module.id === "mock-exam" ? 'ring-2 ring-primary/30' : ''}`}>
                  <CardContent className="text-center space-y-2 sm:space-y-3 p-3 sm:p-4 flex-grow flex flex-col justify-center">
                    ...
                    <div className="flex justify-center">
                      <Badge
                        variant="secondary"
                        className={module.id === "mock-exam"
                          ? "bg-primary/20 text-primary hover:bg-primary/30 font-semibold text-[10px] sm:text-xs px-2 sm:px-3 py-0.5 sm:py-1 border-0"
                          : "bg-primary/10 text-primary hover:bg-primary/20 font-semibold text-[10px] sm:text-xs px-2 sm:px-3 py-0.5 sm:py-1 border-0"
                        }
                      >
                        {module.id === "mock-exam" ? "Mock Exam" : `Module ${module.id}`}
                      </Badge>
                    </div>
