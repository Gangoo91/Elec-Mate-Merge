...
const BS7671Module8Section2 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 8 · Section 2 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'Every cert is only as good as the schedules attached to it. The Schedule of Inspection, Schedule of Test Results and Schedule of Circuit Details — what each row records, how A4:2026 reshapes the columns, and how the Reg 643 test sequence drives the order in which they get filled.'
          }
        </div>
        ...
        >
          <p>
            Item 1.0 covers methods of protection against electric shock — basic protection (Section
            416), fault protection (Section 411), additional protection (Section 415). Item 2.0
            covers prevention of mutual detrimental influence. Item 3.0 covers identification
            (cabling, distribution boards, conductors). Items 4.0 to 6.0 cover cabling, conductor
            connections and protection against overcurrent. Items 7.0 onwards cover specific
            protective measures, special locations (Part 7), and energy efficiency (added in A4 as
            item 17.0). The detailed item-by-item walk-through is in Module 6 §2.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            required at IΔn × 0.5 to verify non-trip), and phase rotation. The Reg 643 sequence and
            acceptance criteria are covered in detail in Module 6 §3.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            presence per Reg 421.1.7, and a supplied-from cross-reference so distribution boards can
            be traced through the schedule. A4 schedule changes are in §3 of this module.
          </p>
        </ConceptBlock>
        ...
        <ContentEyebrow>The Reg 643 test sequence — and how it drives the schedule</ContentEyebrow>
        ...
        >
          <p>
            The headline order is dead tests first, live tests second. Reg 643.2.1 covers continuity
            of protective conductors and main equipotential bonding. Reg 643.2.2 covers continuity
            of ring final-circuit conductors (the three-test ring sequence). Reg 643.3 covers
            insulation resistance — typically 500 V DC test voltage on LV circuits, 250 V on
            SELV/PELV. Reg 643.4 covers protection by SELV / PELV / electrical separation. Reg 643.5
            covers basic protection by barriers / enclosures applied during erection. Reg 643.6
            covers polarity by dead test. Reg 643.7.2 covers earth-electrode resistance (TT or
            TN-installations with their own electrode).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            (PFC) is measured at the origin and recorded as the higher of prospective short- circuit
            current and prospective earth-fault current. Zs at the furthest point of each circuit is
            then either measured directly or calculated as Ze + (R1+R2) — both methods are accepted
            by GN3 provided the calculation is clearly noted. RCD operating time is measured at IΔn
            (must operate within 300 ms for Type AC/A general, 40 ms for Type S up to 5 IΔn — see BS
            EN 61008/61009) and at 5 IΔn (must operate within 40 ms).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 421.1.7 (introduced by Amendment 2:2022) recommends — and in some occupancies
            mandates — Arc Fault Detection Devices on AC final circuits up to 32 A. The
            recommendation hardens to a "shall" in higher-risk sleeping-accommodation occupancies.
            The cert column, the design declaration and the operational test together form the AFDD
            compliance trail.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The reference methods are listed in Appendix 4 Table 4A2 of BS 7671. The OSG Appendix F
            provides the field-friendly cross-reference plus correction factors for grouping (Cg),
            ambient temperature (Ca), and thermal insulation (Ci). Ca × Cg × Ci applied to Iz
            tabulated gives the design Iz; Iz must equal or exceed In, and In must equal or exceed
            Ib, per Reg 433.1.1.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The OSG is updated to reflect each amendment of BS 7671. The current edition (OSG to BS
            7671:2018+A4:2026) reproduces the amended Appendix 3 tables with the A4 changes built
            in, plus a new appendix on AFDDs and updated guidance on EV charging in TN-C-S. Carrying
            the wrong-edition OSG on site is a small but real risk — verify the amendment level on
            the cover before quoting Zs maxima from it.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For RCD type selection on EV charging circuits (Reg 722.531.3.101), manufacturer data is
            mandatory: the spec sheet states whether the charger has built-in 6 mA DC fault
            detection, which determines whether Type A or Type B RCD is required upstream. Choosing
            the wrong type without consulting manufacturer literature is one of the most common
            A4-era cert defects.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>1. Setup.</strong> MFT calibrated within 12 months, leads under test (lead null
            on continuity, IR self-test). Photograph the calibration certificate or instrument
            display before starting. <strong>2. Continuity (Reg 643.2.1).</strong> R1 + R2 at the
            consumer unit by short-circuit-and-test (line and CPC linked at the consumer unit,
            low-resistance ohmmeter at each accessory) or by wandering lead. R2 alone for protective
            conductors / supplementary bonding.{' '}
            <strong>3. Ring final- circuit continuity (Reg 643.2.2).</strong> The three-test ring
            sequence — end-to-end continuity of L, N, CPC; cross-link L to N and verify uniform low
            resistance at every accessory; cross-link L to CPC and verify the same.{' '}
            ...
            actual reading, not just "PASS". <strong>5. Polarity by dead test (Reg 643.6).</strong>{' '}
            At every accessory. <strong>6. Earth-electrode resistance (Reg 643.7.2).</strong> Where
            the installation has its own electrode (TT, or TN-installations with a private electrode
            for telecoms / lightning protection). Three-pole or stake-and-loop method.
            <strong> 7. Energise. Live tests.</strong> Ze at origin, PFC, Zs at every circuit's
            furthest point, RCD operating time at IΔn and 5 IΔn (and at IΔn × 0.5 to verify non-trip
            where required), phase rotation on three-phase. <strong>8. Sign and schedule.</strong>{' '}
            Cross-check measured Zs against max permitted Zs (A4 column), cross-check measured RCD
            time against device standard, code any deviation as an observation, sign the cert.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            An EICR inspector reads both schedules together. Schedule of Inspection item 1.0 ticked
            but Schedule of Test Results Zs over the limit? That's a contradiction the inspector has
            to resolve before signing — typically by re-inspecting item 1.0 (visual condition of CPC
            terminations, presence of bonding, integrity of the means of earthing) and re-measuring
            the test result. The cert is the integrated view; the schedules are the working papers
            behind it.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The discipline is the same on every job, scaled up or down by complexity. A simple MEIWC
            has a built-in single-page schedule. An EIC has three separate schedules. A large
            multi-DB commercial cert has three schedules per board with a master cross- reference.
            The form scales; the content discipline does not. Every required cell filled, every
            observation cross-referenced, every assumption (reference method, SPD type, AFDD
            presence, manufacturer data) declared. That is the cert pack the insurer, the next
            inspector, the building owner and (in the worst case) the court read.
          </p>
        </ConceptBlock>
