...
const inlineChecks = [
  {
    ...
    options: [
      'Annual only.',
      'Daily visual check on the central battery panel (or self-test summary indicator) where one is provided; monthly functional test on every luminaire; annual full duration test; and (NEW in BS 5266-1:2025) five-yearly photometric verification (point-by-point lux survey on the defined escape route). Each cadence catches different failure modes — daily catches whole-system fault flags, monthly catches step-change luminaire failures, annual catches battery capacity decay, five-yearly catches gradual lux output decay across the installation.',
      'Only at install.',
      'Weekly only.',
    ],
    correctIndex: 1,
    explanation:
      'Four overlapping cadences: daily indicator check, monthly functional, annual duration, five-yearly photometric. The five-yearly is new in 2025. Each cadence has a distinct purpose and missing any one leaves a category of failure undetected.',
  },
  {
    id: 'elm5-s3-fiveyear',
    ...
    options: [
      'Lamp colour.',
      'That the installation is still delivering the design lux on the defined escape route — point-by-point lux meter readings against the design and against the BS EN 1838 minima. The five-yearly photometric catches gradual luminaire output decay (LED depreciation, optic fogging, lamp ageing), battery capacity loss reducing emergency-mode output, and any environmental changes (new partitions, new obstructions, new finishes) that affect light delivery. It is the only test that exposes long-term performance drift.',
      'Battery weight.',
      'Switch operation.',
    ],
    correctIndex: 1,
    explanation:
      'The five-yearly photometric is a measurement, not an inspection. The lux meter on the floor of the escape route is the diagnostic instrument. It catches what no other test can catch — the slow decay of delivered lux that is invisible to the eye and to functional tests.',
  },
  {
    ...
    options: [
      ...
      'Within the calendar month for which the test is due. The standard practice is to schedule each monthly test on a fixed week of each month (e.g. first Monday of the month) for predictability. The 2025 standard expects the schedule to be kept; a slip of a few days within the same month is normal, but slipping into the following month is a non-conformance against the consistency expectation. Self-test systems automatically maintain the schedule; manual testing requires calendar discipline.',
      'Within 6 months.',
      'No tolerance.',
    ],
    correctIndex: 1,
    explanation:
      'Within the calendar month is the working tolerance. The schedule consistency is one of the 2025 emphasis areas. A test that habitually slips by several weeks per cycle eventually misses entire months — the schedule has been lost.',
  },
  {
    id: 'elm5-s3-daily',
    ...
    options: [
      'Full discharge.',
      'A visual inspection of the central battery panel indicators — typically a green "system healthy" LED. If any fault LED is illuminated (charge fault, battery fault, supply fault, communications fault, individual circuit fault), the responsible person investigates and arranges remediation. The daily check is brief (seconds per panel) but catches step-change faults early — without it, a panel fault might go unnoticed for weeks until the next monthly test exposes it. The daily check is typically delegated to the responsible person on site, recorded in the daily check book.',
      'Battery replacement.',
      'Lux survey.',
    ],
    correctIndex: 1,
    explanation:
      'The daily check is a brief indicator scan — green = healthy, anything else = investigate. It is a duty of the on-site responsible person rather than the maintenance contractor. BS EN 50172 / BS 5266-1 expects the daily check on central battery installations; self-contained installations do not have a single panel and rely on the monthly walk-through instead.',
  },
];
...
const quizQuestions = [
  {
    id: 1,
    question: 'List the four periodic test cadences specified by BS EN 50172:2024 / BS 5266-1:2025 for emergency lighting.',
    options: [
      'Just annual.',
      'Daily visual indicator check (on central battery panel where provided); monthly functional test on every luminaire; annual full duration test (typically 3 hours); five-yearly photometric verification (point-by-point lux survey on defined escape route — NEW emphasis in BS 5266-1:2025). Plus visual inspection at the same intervals as the testing. Plus BS 7671 periodic inspection of the underlying electrical installation per the relevant section of BS 7671 (typically 5-yearly for fixed wiring in commercial premises, or per the previous EICR recommended interval).',
      'Daily and weekly.',
      'Monthly only.',
    ],
    correctAnswer: 1,
    explanation:
      'Four primary cadences specific to emergency lighting (daily indicator, monthly functional, annual duration, five-yearly photometric) plus the underlying BS 7671 periodic inspection on its own cadence. Each catches different failure modes; the four-cadence regime is the BS 5266-1:2025 model.',
  },
  {
    id: 2,
    question: 'Why was the five-yearly photometric verification made explicit in BS 5266-1:2025?',
    options: [
      'No reason.',
      'Because lux output decays over time and previous practice often relied on the design calculation alone after commissioning. LED luminaires lose output (typical L70 at 50,000 hours = 70% of original at 50,000 h); optics fog with dust and grease; lamps age; batteries lose capacity reducing emergency-mode output; environmental changes (partitions, finishes, obstructions) affect light delivery. The design calculation was a year-zero snapshot that did not capture the trajectory. The five-yearly survey is the verification that the installation is still delivering what it was designed to deliver, years later.',
      'For sales.',
      'Random change.',
    ],
    correctAnswer: 1,
    explanation:
      'Lux output is a degradable parameter and the standard now treats it as one. The design calculation alone is no longer enough; the five-yearly survey is the verification. The 2025 update closes a gap that was sometimes exploited by skipping post-commissioning lux measurement.',
  },
  {
    id: 3,
    question: 'A monthly functional test is due in March. The contractor is unable to attend until 5 April. Is this a non-conformance?',
    options: [
      'No.',
      'Yes — the test should have been carried out within the calendar month of March. Slipping into April is a non-conformance against the BS 5266-1:2025 schedule consistency expectation. The contractor must document the reason for the slip (e.g. site access denied, contractor staffing) and the corrective action to prevent recurrence. The test in April covers April; the missed March test is recorded as a missed cadence in the logbook. Repeated slippage is a contract issue.',
      'Always fine.',
      'Slip a year.',
    ],
    correctAnswer: 1,
    explanation:
      'Schedule consistency is the 2025 emphasis. Monthly within the calendar month, annual within the 12-month anniversary. A drift of a few weeks per cycle compounds into months over a contract life and eventually loses the cadence entirely. The contractor and the responsible person must keep the schedule.',
  },
  {
    id: 4,
    ...
    options: [
      ...
      'The responsible person on site (typically the building manager, facility manager, or a delegate trained for the duty). The check is brief — a glance at the central battery panel to confirm "system healthy" green. Any fault indication triggers the responsible person to call the maintenance contractor for investigation. The duty is recorded in a daily check book or electronic log. The maintenance contractor visits monthly for the functional test, annually for the duration test, five-yearly for the photometric — they do not visit daily for this check.',
      'A salesperson.',
      'Anyone.',
    ],
    correctAnswer: 1,
    explanation:
      'The daily check is a duty of the on-site responsible person. It is built into the daily building rounds. Brief, simple, but high-leverage — catches step-change panel faults early. Without it, a fault might persist for weeks until the next monthly maintenance visit.',
  },
  {
    id: 5,
    question: 'For a self-contained luminaire installation (no central battery panel), how is the daily check duty satisfied?',
    options: [
      'Skip it.',
      'Self-contained installations do not have a single central panel, so the daily check duty does not apply in the same form. Instead, the monthly functional test is the working cadence, augmented by any user observations (occupants seeing a fault flag on a luminaire, reporting through the responsible person). Where a central addressable controller is provided for self-test luminaires, the controller display becomes the daily-check focal point — the responsible person checks the controller for system-level fault flags, much as on a central battery panel.',
      'Replace batteries.',
      'Run a duration test.',
    ],
    correctAnswer: 1,
    explanation:
      'Self-contained does not have a single panel to check. The monthly cadence is the routine; user reporting is the gap-filler; addressable controllers add a daily-check capability. The daily check duty as defined in BS EN 50172 is specifically for central battery installations.',
  },
  {
    id: 6,
    question: 'What is the BS 5266-1:2025 working tolerance on the annual duration test cadence?',
    options: [
      'Anytime.',
      'Within the 12-month anniversary of the previous duration test, with a small documented tolerance for scheduling. Typical practice is to schedule the duration test for the same calendar month each year (e.g. always October) to make the cadence predictable. A slip of a few weeks within the anniversary month is normal; slipping by months without documented justification is a non-conformance. Self-test systems schedule automatically; manual systems rely on contract calendar discipline.',
      '6 months.',
      'No tolerance.',
    ],
    correctAnswer: 1,
    explanation:
      '12-month anniversary with small tolerance is the working interpretation. The 2025 standard tightens the previous looser practice. Predictable scheduling (same month each year) is the easiest way to maintain consistency.',
  },
  {
    id: 7,
    ...
    options: [
      'A photo.',
      'A full point-by-point lux meter survey on the defined escape route, in emergency mode (mains off, system on battery), at the design measurement plane (typically 0.2 m above floor), at the design grid (typically 1-2 m intervals along the length plus across the non-excluded width per BS EN 1838:2024, with key points at door thresholds, junctions, stair heads/feet). The lux meter is calibrated within the past 12 months. Each reading is recorded against the design value, against the commissioning baseline (the year-zero readings), and against the BS EN 1838 minima. Comparisons identify drift, degradation, and any non-compliance.',
      'A quick walk.',
      'Battery test.',
    ],
    correctAnswer: 1,
    explanation:
      'The five-yearly survey is a real measurement exercise. Calibrated meter, design grid, point-by-point, against three references. The output is a tabulated report comparable with the commissioning baseline. The comparison is the diagnostic.',
  },
  {
    id: 8,
    question: 'A five-yearly photometric survey reveals that two specific points on the escape route now read 0.6 lx and 0.7 lx, where the design specified 1.5 lx and the BS EN 1838 minimum is 1 lx. What is the correct response?',
    options: [
      'Ignore.',
      'Both points are below the BS EN 1838 minimum and therefore non-compliant. Investigate: are the luminaires illuminating these points still functional? Are they delivering the original lux output (LED depreciation)? Are the optics dirty? Have any obstructions appeared? Has the room layout or finish changed? Identify root cause and remediate (lamp/luminaire replacement, optic clean, additional luminaire, design revision). Re-survey after remediation. Update the documentation pack with the remediation. The system is non-compliant until remediated; the responsible person should be informed in writing of the position and the remediation plan.',
      'Wait 5 years.',
      'Just clean.',
    ],
    correctAnswer: 1,
    explanation:
      'Below the BS EN 1838 minimum is non-compliance. The investigation, remediation, and re-survey is the disciplined response. Documenting the position to the responsible person is part of the discharge of the contractor\'s duty under BS 5266-1:2025.',
  },
  {
    id: 9,
    question: 'How are self-test electronic logs integrated with the BS 5266-1 logbook?',
    options: [
      'They cannot be.',
      'Self-test electronic logs from the central addressable controller can be downloaded and attached as appendices to the BS 5266-1 logbook. The competent person\'s monthly review of the controller log, the periodic visual inspection records, the photometric survey results, and the BS 7671 verification records together form the complete logbook. The electronic log alone is not the logbook — it is one component, ratified by the competent person\'s review and supplemented by the human-led elements (visual, photometric, electrical).',
      'Replace the logbook.',
      'Only paper.',
    ],
    correctAnswer: 1,
    explanation:
      'Electronic logs are accepted as a logbook component when supported by the human-led elements and ratified by competent oversight. The 2025 standard accepts this hybrid model; insurers and fire authorities increasingly prefer electronic logs because they cannot be backdated.',
  },
  {
    id: 10,
    question: 'What is the relationship between the four-cadence emergency lighting test regime and the BS 7671 EICR cycle?',
    options: [
      'No relationship.',
      'They are parallel cycles — the emergency lighting periodic regime (daily / monthly / annual / five-yearly) covers the emergency lighting performance, while the BS 7671 Electrical Installation Condition Report (EICR) covers the underlying electrical wiring on its own cycle (typically 5-yearly for commercial fixed wiring, or per previous EICR recommendation). They cross-reference: the EICR includes the emergency lighting circuits in the inspection scope, and the BS 5266-1 logbook references the most recent EICR. Both must be current for the installation to be compliant. Many maintenance contracts run the two on the same nominal calendar (e.g. a five-yearly EICR scheduled with the five-yearly photometric).',
      'Replace each other.',
      'Different countries.',
    ],
    correctAnswer: 1,
    explanation:
      'Parallel cycles, cross-referenced. The emergency lighting test regime focuses on the lighting system performance; the EICR focuses on the electrical wiring condition. Both apply, both have schedules, both feed into the building responsible person\'s compliance evidence pack.',
  },
];
