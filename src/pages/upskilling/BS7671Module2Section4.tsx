...
const BS7671Module2Section4 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 2 · Section 4 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'What A4:2026 actually changes — publication 15 April 2026, A3 withdrawn 15 October 2026, the new Reg 411.3.4 luminaire RCD, Reg 421.1.7 AFDD mandates, Section 419 alternatives, Section 443 consequence route, parallel-source rules, EV PEN ban, and the cert-form fields you must now complete.'
          }
        </div>
        ...
        >
          <p>
            ...
            inspection items, schedule of test results columns, and several free-text fields changed
            — pre-A4 templates do not capture A4 information correctly, so even if you are entitled
            to certify under A3 during the window, using the right template is non-negotiable.
          </p>
          <p>
        ...
        >
          <p>
            ...
            exception for category (b) — non-domestic socket-outlets in other locations. Reg 411.3.4
            is a separate, parallel rule for luminaire circuits in dwellings — and offers no
            exception. It applies whether the lighting points are inside a special location
        ...
        >
          <p>
            ...
            Conventional MCBs only see overcurrent, and conventional RCDs only see residual current
            — neither detects an arc fault that hasn&apos;t yet developed into an overcurrent or
            earth fault. UK fire-statistics analysis attributes roughly 25% of domestic fires of
            electrical origin to arc-fault events. The economic case for AFDDs in sleeping
            accommodation is now strong even where they remain a recommendation rather than mandate.
          </p>
          <p>
            Definitions to know: HMO — House in Multiple Occupation, three or more unrelated tenants
            sharing facilities. PBSA — Purpose-built student accommodation. HRRB — High-rise
            residential building, typically ≥ 18 m or ≥ 7 storeys per Building Regulations
            (post-Grenfell definition; check the local building-control AHJ). Care/nursing homes —
            Care Quality Commission-registered or equivalent sleeping-accommodation premises with
            vulnerable occupants. The cert form schedule of inspection item 4.23 records AFDD
            presence/absence per circuit; column 30 of the schedule of test results captures the
            AFDD test result.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            insulation, recognised by the double-square symbol. Caveat under Reg 412.1.2: cannot be
            the sole protective measure on circuits with sockets, LSCs, DCLs or cable couplers —
          ...
          <p>
            <strong>Non-conducting location.</strong> A room with insulating walls and floor and no
            extraneous-conductive-parts. Used in specialist test/research environments. Reg 418.1
            sets the requirements: floor and walls must have insulation resistance ≥ 50 kΩ at 500 V
            or 100 kΩ above 500 V; no protective conductor in the location; no simultaneous-touch
            fault path.
          </p>
          <p>
        ...
        >
          ...
          <p>
            ...
            was being widely abused — values were being computed conservatively to justify omitting
            SPDs even where the risk profile clearly warranted them. A4 removes that escape hatch
            entirely. The route is now: read Reg 443.4.1 consequences, decide if any apply, fit SPDs
            if they do, document the decision either way.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The hazard is straightforward. A 6 kA breaking-capacity rating in the forward direction
            does not guarantee a 6 kA rating in the reverse direction — the contact geometry, the
            magnetic blow-out arrangement, and the arc-extinction mechanism may all behave
            differently. Where current can flow either way (PV array exporting onto the busbar,
            battery discharging back through the consumer unit, EV in V2H mode), a fault on the
            export side draws current through the device in reverse — and a device not rated for it
            can fail to interrupt cleanly.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            current can saturate the RCD&apos;s differential core, causing nuisance trips (annoying)
            or, worse, blinding the device to genuine faults thereafter (dangerous). Connecting the
            source on the SUPPLY side of any upstream RCD avoids both modes — each downstream final
            circuit then has its own RCD/RCBO providing additional protection in the conventional
            manner.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            property cut-out. Inside the installation it is split at the MET into separate N and PE.
            If the PEN breaks above the cut-out — typically a corroded joint in a service head or a
            damaged service cable — the local &quot;earth&quot; reference floats with load on the
            system. Every Class I exposed metal part on the property rises in potential. A user
            touching the EV chassis while standing on the driveway, or holding the charging cable
            while standing on damp ground, becomes the fault path. The fault is invisible to a basic
            insulation-resistance test and only manifests under load with the PEN broken.
          </p>
          <p>
            The Reg 722.312.2.1 fix is to ensure no PEN can reach the EV circuit. Option (a) is the
            cleanest and is the dominant UK approach for new domestic EV installs — the CU is wired
            with the PEN split before the EV way, only PE reaches the charger, and even if the PEN
            fails upstream the EV PE is still tied to the rest of the installation MET (which itself
            rises with the PEN failure, but the ELV-circuit alternative measure or an open-PEN
            protection device in the charger then disconnects). Option (b) is mainly relevant where
            the consumer unit cannot easily be modified — typically older boards or shared-supply
            situations. Where the charger&apos;s own integrated open-PEN detection is relied upon,
            the manufacturer&apos;s installation manual is binding — fit a separate device only if
            the charger does not have one.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            722.312.2.1, the notice identifies the alternative employed (open-PEN protection device,
            separate electrode, etc.).
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <p>
            <strong>Schedule of test results.</strong> Column 30 records the AFDD trip test result.
            Modern AFDDs include a test button and many provide a self-test indicator; the inspector
            confirms the test has been carried out and records the result.
          </p>
          <p>
            <strong>Bidirectional / parallel-source fields.</strong> Where Reg 530.3.201 applies, a
            field captures whether the protective device is rated for bidirectional flow (typically
            referencing manufacturer documentation). Where Reg 551.7.1 applies, a field captures
            whether the parallel source is connected on the supply side of any upstream RCD.
          </p>
          <p>
            ...
            non-conducting location, earth-free local equipotential bonding) and the reason ADS was
            not feasible.
          </p>
          <p>
        ...
        >
          <p>
            ...
            defaulting to the safer choice (RCD on lighting, AFDD in sleeping accommodation, SPDs at
            origin), with the documented Reg 120.3 departure available where the choice is genuinely
            justified. The deletion of the old Reg 443.5 / Annex A443 quantitative calculation is a
            particular signal — the BSI committee is moving away from calculation-based escapes and
            towards consequence-based defaults. Designers who embrace that pattern will find A4
            straightforward; those who try to compute their way out of compliance will find A4
            unforgiving.
          </p>
        </ConceptBlock>
