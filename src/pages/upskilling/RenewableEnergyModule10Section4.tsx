...
const inlineChecks = [
  {
    ...
    question: 'What is EREC G100 and why does it exist?',
    options: [
  ...
  },
  {
    ...
    question: 'What’s the difference between G100 soft limiting and hard limiting?',
    options: [
  ...
  },
  {
    ...
    question: 'When does G100 apply vs G99?',
    options: [
  ...
  },
  {
    ...
    question: 'Who can commission a G100 export limitation scheme + what is the cert evidence?',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'G100 zero-export sites: when is zero-export the configured limit?',
    options: [
  ...
  {
    question: 'Does BS 7671 cover G100 directly, or is it external?',
    options: [
...
export default function RenewableEnergyModule10Section4() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 10 · Section 4 · ENA EREC G100 Export Limitation Schemes"
        title="EREC G100 export limit"
        backTo="../renewable-energy-module-10"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The UK Engineering Recommendation that lets customers install more generation than the
          DNO’s network can accept as gross export — by limiting actual export to an agreed lower
          value via an ENA-approved Export Limitation Scheme (ELS). Soft limiting (curtailment via
          inverter Modbus) vs hard limiting (physical contactor disconnect). UK 2025-26 reality:
          G100 is the workaround that prevents £5-100k DNO reinforcement costs blocking PV / BESS
          installs on constrained networks.
        </p>
        ...
        <Pullquote>
          G100 is the UK’s pragmatic answer to a constrained LV network: install more generation
          than the network will take, but limit what leaves the gate. Customer keeps full
          self-consumption; DNO keeps the network safe; reinforcement queue stays manageable.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Origin</strong> — ENA Engineering Recommendation G100 +
              amendments. Adopted across all UK DNOs as a network compliance instrument. Not a BS
              7671 reg
            </li>
            <li>
              <strong className="text-white">Use case</strong> — DNO LV network constrained: voltage
              rise at peak generation, transformer capacity limited, cable capacity limited.
              Customer wants to install more generation than network can take as gross export
            </li>
            <li>
              <strong className="text-white">G100 ELS</strong> — Export Limitation Scheme = the
              hardware + software that measures export at the grid connection point + curtails (or
              disconnects) the generator to keep net export ≤ the configured limit
            </li>
            <li>
              <strong className="text-white">Limit values</strong>— customer-DNO agreed. Common: 0
              kW (zero-export), 3.68 kW (G98 single-phase Type A cap), 11.04 kW (G98 three-phase
              Type A cap), or arbitrary value the network can accept
            </li>
            <li>
              <strong className="text-white">Connection flow</strong>— customer / installer submits
              G99 application → DNO evaluates + offers connection with G100 limit → customer accepts
              → install ELS + complete G99 install → DNO-witnessed or self-certified G100
              commissioning → connection live
            </li>
            <li>
              <strong className="text-white">Economic case</strong>— G100 ELS £100-1,000 + clipped
              export 200-500 kWh/year (£30-75/year lost SEG) vs DNO reinforcement £5-100k + 6-24
              month delay. Customer no-brainer in constrained network areas
            </li>
            <li>
              <strong className="text-white">UK 2025-26 prevalence</strong>— increasingly common.
              Some DNOs (UKPN, SSEN, NGED) report G100 connections growing as default-acceptance
              offer for constrained LV networks. SP Energy Networks similar trajectory
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — G100 commissioning
              certificate signed by competent person + ELS device specification + configuration
              record + commissioning test result + DNO acceptance + integrated PEI EIC
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">EREC G98</strong> — small-scale Type A (≤16 A per
              phase: ≈3.68 kW single-phase, ≈11.04 kW three-phase). Fast-track post-installation
              notification (notify DNO within 28 days of commissioning). Type-tested inverter from
              the ENA-approved list — deemed to meet Reg 551.7 requirements. UK 2025-26: most small
              residential PV (up to 4 kWp) fits G98 if no BESS
            </li>
            <li>
              <strong className="text-white">EREC G99</strong> — anything larger or multi-source.
              Formal pre-installation application. DNO design + connection offer (may include
              reinforcement requirement or G100 limit condition) + customer acceptance + install +
              DNO-witnessed commissioning + G99 completion. Lead time 6-18 weeks typical
            </li>
            <li>
              <strong className="text-white">G99 Issue 2 (2025) + storage</strong> — the current
              version is EREC G99 Issue 2 (10 March 2025). It adds mandatory requirements for Power
              Generating Modules incorporating electricity storage, in force 1 March 2026 — so new
              BESS / hybrid connections must be applied for and commissioned against these
              requirements
            </li>
            <li>
              <strong className="text-white">EREC G100</strong> — NOT a connection class; an EXPORT
              LIMITATION layer. Sits as a constraint on a G99 (or larger G98) connection. ELS device
              + commissioning evidence + ENA approval list. Customer installs more generation than
              network accepts as gross export; ELS keeps actual export ≤ the limit
            </li>
            <li>
              <strong className="text-white">G99 + G100 combination</strong> — the typical UK
              2025-26 path for PV + BESS installs on constrained networks. G99 application + DNO
              connection offer specifies the G100 limit + ELS requirement + commissioning evidence
            </li>
            <li>
              <strong className="text-white">G98 + G100 combination</strong> — less common but
              exists. Small (≤16 A) generation in a network so constrained that even 16 A per phase
              is too much. Customer chooses to limit further (e.g. 0 kW)
            </li>
            <li>
              <strong className="text-white">When G98 is enough</strong>— single-phase PV ≤3.68 kW +
              no BESS + no other source + unconstrained network. Customer-installer files G98 within
              28 days. Simplest path
            </li>
            <li>
              <strong className="text-white">When G99 alone is enough</strong> — larger generation
              (&gt;16 A) but network not constrained. G99 formal application + DNO approval without
              G100 limit. Full installed export accepted
            </li>
            <li>
              <strong className="text-white">When G99 + G100 needed</strong>— larger generation +
              constrained network. G99 + G100 limit. Customer accepts export cap; install proceeds
        ...
        <Pullquote>
          Soft limiting is a control loop that curtails generation; hard limiting is a contactor
          that disconnects it. The choice has economic + customer-experience implications — the
          DNO’s tolerance determines which.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">CT clamp</strong> — bidirectional current sensor on the
              main supply tail at MET / consumer unit incomer. Measures import + export. Connected
              to ELS controller via twisted-pair signal cable (Reg 528 segregation from LV power)
            </li>
            <li>
              <strong className="text-white">ELS controller</strong>— reads CT clamp + calculates
              net export per measurement cycle (typically 100ms-1s). Compares to configured limit.
              Computes curtailment command if export approaching limit
            </li>
            <li>
              <strong className="text-white">Inverter curtailment</strong> — ELS sends power
              setpoint command via Modbus RTU / TCP / proprietary protocol. Inverter reduces output
              to stay below the limit. Most modern hybrid inverters support this natively
            </li>
            <li>
              <strong className="text-white">Response time</strong>— typical &lt;5 s end-to-end. ENA
              G100 specifies maximum response time per device class. Faster on modern integrated
              solutions (SolarEdge native, Solis S6); slower on external Carlo Gavazzi EM340-based
              retrofits
            </li>
            <li>
              <strong className="text-white">Curtailment hierarchy</strong> — multi-source: ELS
              curtails in priority order. Typical: V2G discharge first → BESS discharge second → PV
              inverter third. EMS coordinates above this layer
            </li>
            <li>
              <strong className="text-white">Loss accounting</strong>— clipped kWh = generation that
              would have exported but didn’t. Typically 200-500 kWh/year for 10 kWp PV at 3.68 kW
              limit. Logged by inverter for monitoring + commissioning evidence
            </li>
            <li>
              <strong className="text-white">UK 2025-26 native vendor support</strong> — SolarEdge +
              Energy Meter, Solis + S6 metering, Huawei FusionSolar + Smart Power Sensor, GivEnergy
              AIO + EM340 / native, Enphase IQ + IQ Combiner + Envoy. ENA-listed
            </li>
            <li>
              <strong className="text-white">Commissioning test</strong> — deliberate
              over-generation simulation (lift BESS charge limit, defer load) → verify ELS curtails
              inverter within ENA-specified response time → measure clipped power + duration →
              record on G100 certificate
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">CT clamp + controller</strong>— same measurement as
              soft limiting. Bidirectional CT clamp at MET + ELS controller reading
            </li>
            <li>
              <strong className="text-white">Contactor / switch</strong>— DIN-rail contactor or
              switch-disconnector on the AC supply from the inverter to the LV switchgear. Coil
              controlled by ELS digital output. Reg 537.2 isolation rated
            </li>
            <li>
              <strong className="text-white">Trigger</strong> — ELS detects export &gt; limit →
              contactor opens → generator disconnected. Inverter detects loss of grid → Reg 551.7.5
              anti-islanding shuts down. Generator stays disconnected until reset
            </li>
            <li>
              <strong className="text-white">Reset</strong> — manual or automatic, configurable.
              Automatic reset after a cool-down period if conditions normalised (sun behind cloud,
              load rises); manual if DNO requires positive engineer intervention
            </li>
            <li>
              <strong className="text-white">Zero-export use cases</strong> — DNO-constrained urban
              site (no export allowed); landlord-owned roof (tenant cannot export); apartment-block
              community generation behind master meter; customer choice to avoid SEG admin
            </li>
            <li>
              <strong className="text-white">Zero-export operational impact</strong> — all surplus
              generation goes into BESS or gets clipped; with BESS, 80-95% self-consumption;
              clipping 5-20% annual. Lose all SEG income; rely entirely on avoided import
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.4 + hard limit</strong> — hard-limit
              contactor counts as one of the multi-source isolation switches if it isolates the
              generator source. Document on multi-source isolation diagram
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— G100 certificate with
              hard-limit configuration + contactor type + commissioning test (deliberate
              over-generation → verify contactor opens within response time + isolates generator) +
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-10-section-5')}
