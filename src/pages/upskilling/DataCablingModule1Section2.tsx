    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Star, bus, ring and mesh — and the more important distinction between physical topology
          (the shape of the cabling) and logical topology (the shape of the traffic). How structured
          cabling enforces a hierarchical physical star while leaving the active layer free to
          implement any logical topology over the cabling's 15-20 year life.
        </p>
        ...
        <ContentEyebrow>Physical vs logical topology — the foundational distinction</ContentEyebrow>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            <li>
              ...
              topology this is a home-run cabling pattern from each device to a central distributor.
              As a LOGICAL topology it is the forwarding pattern of a single Ethernet switch — every
              port talks to the switch and only via the switch.
            </li>
            <li>
              <strong>Bus.</strong> All nodes share a single linear medium with terminators at each
              end. Physically: 10BASE2 thin-coax, 10BASE5 thick-coax — gone. Logically: an old hub
              (multi-port repeater) imposed a logical bus on a physical star, because every frame
              was repeated to every port — also effectively gone.
            </li>
            <li>
              ...
              Logically: Token Ring (IEEE 802.5) ran a logical ring on a physical star via the MAU.
              Modern industrial protocols — PROFINET MRP, EtherCAT, ITU-T G.8032 ERPS — run logical
              rings on physical rings of fibre or copper for deterministic latency.
            </li>
            <li>
          ...
          <p>
            Structured cabling resolves the question for the physical layer once and for all: BS EN
            50173-1 fixes the horizontal subsystem to a physical star, the building backbone to a
            star anchored at the BD, and the campus backbone to a star anchored at the CD. The
            active layer is then free to run any logical topology that the equipment supports. That
            separation is the whole architectural mechanism behind service-independence.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              ...
              building distributor and the floor distributor respectively). Each telecommunications
              outlet shall be connected to a single floor distributor by a single permanent link.
              Daisy-chained or T-tapped outlets are not part of the generic cabling.
            </>
          }
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-2 text-[14px]">
            <li>
              ...
              top-of-hierarchy distributor on a multi-building site. Lives in the computer-services
              / main equipment building. Single-mode and multi-mode fibre home runs from every BD on
              campus terminate here. Houses the core routers and the campus carrier demarcation.
            </li>
            <li>
              <strong>BD — Building Distributor (TIA: IC, Intermediate Cross-Connect).</strong> The
              top-of-hierarchy distributor inside a single building. Lives in the building's
            ...
            <li>
              ...
              horizontal Cat6A (or fibre-to-the-floor) permanent link from every work-area outlet on
              the floor terminates here. Houses the floor's edge / access switches. Sized so every
              horizontal cable on the floor reaches its outlet within 90 m of cable run.
            </li>
          </ul>
          <p>
            ...
            every FD is a satellite of the BD; viewed from the floor, every outlet is a satellite of
            the FD.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-2 text-[14px]">
            <li>
              ...
              redundant power). Cost scales as n (one cable per user). The generic-cabling default.
            </li>
            <li>
              <strong>Bus.</strong> All nodes share a single linear medium with terminators. Failure
              mode: any break, loose tap or unterminated end takes down the entire segment; CSMA/CD
              performance collapses as devices and traffic are added. Cost scales as n (one tap per
              user) — historically attractive — but the failure mode is fatal. Effectively obsolete
              in commercial cabling.
            </li>
            <li>
              ...
              ring-protection protocol (G.8032 ERPS, REP, MRP), traffic on one side cannot reach the
              other. Industrial / process control still uses physical rings for deterministic
            ...
            <li>
              ...
              mode: highly resilient — multiple link failures can be tolerated — but cost scales as
              n(n−1)/2 for full mesh. Used in data-centre spine-leaf, critical industrial control,
              and high-availability backbone. Generic-cabling standards permit "additional" cables
              on top of the basic hierarchical star to build a partial mesh at backbone where
              required, but the basic shape remains the star.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-2 text-[14px]">
            <li>
              ...
              link from device to switch); the backbone — if redundant uplinks exist — is a logical
              mesh that spanning-tree (RSTP / MSTP) prunes into a loop-free tree, or that Layer-3
              routing (OSPF) treats as a routed mesh. The cabling delivers both at once.
            </li>
            <li>
              ...
              the deterministic-latency requirement (motion control, process control) is intolerant
              of spanning-tree convergence times. This is the one mainstream case where generic
              cabling's hierarchical-star default is deliberately overridden by an
            ...
            <li>
              ...
              distributes traffic across all leaf-spine paths; failure of any one path is tolerated
              by the protocol. BS EN 50173-5 (data centres) recognises this as the standard
              data-centre cabling pattern.
            </li>
          </ul>
          <p>
            In every case, the discipline is to draw the physical topology and the logical topology
            separately, label each one, and check that the active-layer protocols can actually run
            on the cabling shape provided. Most "this network has gone wrong" calls turn out to be a
            logical-topology question (spanning tree, VLAN trunking, a misconfigured aggregate) on a
            physical layer that is actually fine.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              The particular requirements of this section apply to the distribution of ELV DC power
              using balanced, information technology cables and accessories primarily designed for
              data transmission, as specified in BS EN 50173-1 using power feeding sourcing
              equipment in accordance with BS EN IEC 62368-3. Requirements are included for the
              design, erection, and verification of telecommunications infrastructure for the
              purpose of both telecommunications and distribution of ELV DC power feeding. In
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-2 text-[14px]">
            <li>
              <strong>Collapsed star.</strong> On a single-floor single-tenancy small site, the CD,
              BD and FD collapse into one cabinet. The hierarchy is not absent — it is degenerate.
              Every outlet still home-runs to that one distributor. Fully compliant; the 90 m
              permanent-link rule still applies.
            </li>
            <li>
              ...
              fit-outs where furniture-mounted outlets move around. The total channel still must not
              exceed 90 m permanent link, and the rules on length between FD-CP and CP-TO are tight
              (see BS EN 50173-1 §6.4.4 / TIA-568.0-E). The CP is tested as part of the link.
            </li>
            <li>
          ...
          <p>
            What is NOT permitted: T-piece taps, daisy chains, spliced "extension" cables to add an
            extra outlet, or splitting one permanent link into two outlets at the wall. None of
            these are recognised by BS EN 50173-1, ANSI/TIA-568.0-E or ISO/IEC 11801-1. Doing any of
            them voids the warranty and the certification, and breaks the service-independence pitch
            — the next service that needs a clean star simply will not work over a daisy-chained or
            T-tapped link.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-2 text-[14px]">
            <li>
              ...
              Protocol elects a root bridge and computes a loop-free tree from every other bridge to
              the root, BLOCKING redundant ports until needed. RSTP converges in tens of
              milliseconds; MSTP supports per-VLAN-group instances. Logical topology: a pruned tree
              on top of a physical mesh.
            </li>
            <li>
            ...
            <li>
              ...
              distributes traffic across all equal-cost paths simultaneously. The physical layer can
              be a star, a partial mesh or a full mesh — routing copes with all of them.
            </li>
          </ul>
          <p>
            From a cabling-design perspective, the practical implication is: do not deliver a
            physically-star cabling and expect the active layer to invent redundancy that the cables
            do not support. If the building needs FD-to-BD redundancy, install two fibres per FD-BD
            relationship (and route them diversely if the building's resilience case demands it).
            The active-layer protocol then has the physical paths it needs.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Contractor delivers an "as-built" that mixes cable runs (physical) with spanning-tree
              state (logical) on a single drawing. Switches are shown as rectangles connected by
              lines that are sometimes cables and sometimes logical-link-aggregation bundles. A
              future engineer reading the drawing cannot tell which lines are physical fibres and
              which are logical bonds; if a fibre fails, the failure cannot be matched to the
              diagram. The "documentation" is effectively useless.
            </>
          }
          doInstead={
            <>
              ...
              every routing peer. Both diagrams reference the same distributor / device IDs but show
              different relationships. The physical diagram is the cabling contractor's deliverable;
              the logical diagram is the network operator's deliverable; together they answer every
              question a future engineer can ask about the site.
            </>
          }
        />
        ...
        <Scenario
          ...
          whatToDo={
            <>
              ...
              (typically 12-core OM4 each, future-proofed for 40G/100G), one routed via riser shaft
              A and one via riser shaft B. The horizontal subsystem on each floor remains a strict
              physical star — Cat6A home runs to the FD. Then specify the active layer: each FD
              switch stack has two uplinks bonded as a single LACP aggregate back to the BD core; if
              either riser fails, the aggregate stays up at half bandwidth and the network keeps
              forwarding. Document both — cabling drawings show the two fibres per FD-BD pair;
              logical drawings show the LACP groups.
            </>
          }
          whyItMatters={
            <>
              Resilience is bought at the cabling stage, not the switch stage. If the building only
              has one fibre between each FD and the BD, no amount of clever switch configuration can
              route around a fibre cut. The physical topology constrains the logical topology — and
              the cabling specification is where the resilience case is actually delivered. Get this
              wrong on the spec and the only remedy is to re-pull fibre, which on a six-storey
              occupied office is brutal.
            </>
          }
        />
