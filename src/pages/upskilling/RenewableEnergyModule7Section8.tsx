...
const inlineChecks = [
  {
    ...
    options: [
      'Same',
      'Commercial layers add: per-charger sequence × N chargers; BS EN IEC 61439-7 multi-charger assembly verification; DNO connection commissioning (HV transformer + LV switchgear); CPMS OCPP integration test per charger; PCAR 2023 pillar configuration verification (if public); PAS 1899 accessibility audit (if public); multi-charger DLM commissioning + verification; fleet CPMS + telematics integration test; payment terminal commissioning + PCI-DSS compliance',
      'Less testing',
  ...
  },
  {
    ...
    options: [
      'Same as domestic',
      'Per-charger EICR (Section 722 + Part 6) extends across all N chargers; assembly-level EICR for the BS EN IEC 61439-7 LV switchgear; transformer + HV switchgear inspection on a separate cycle (HV by qualified HV engineer); CPMS-side checks (OCPP integration still working, OCPI roaming functional, open-data publication current); per-PCAR-pillar verification at audit (payment terminal operational, pricing display accurate, helpline answered, etc.). 5-year interval typical; some operators on 3-year for high-utilisation sites',
      'Skip EICR',
      'Customer’s problem',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'No tests',
      'CT clamp installation + orientation + calibration (verify CT reads matched site current); DLM controller commissioning (supply limit configured, per-charger allocation logic active); simulated load test (start a high non-EV site load + verify chargers throttle); per-charger CP PWM verification (chargers receiving + applying allocated current); CPMS integration test (DLM allocations reflected in CPMS dashboard); fail-safe test (DLM controller offline → chargers fall back to safe minimum current)',
      'Customer tests it',
      'Manufacturer self-test only',
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Just install date',
      'Comprehensive structured digital deliverable: project summary; site classification (workplace / public / fleet / hub); regulatory framework applied (8-layer stack from M7.1); cert evidence bundle table of contents with hyperlinks to each section; ongoing operator obligations clearly delineated; named contacts (installer support, CPO operator, manufacturer support, CPMS support); first-year warranty terms; EICR schedule + intervals; emergency procedures; cert evidence bundle access transfer (read-write)',
      'Photographs only',
...
const quizQuestions = [
  {
    ...
    options: [
      'Skip and energise',
      'Sequence: (1) per-charger Section 722 commissioning (M6.8 sequence × 4); (2) BS EN IEC 61439-7 multi-charger assembly verification; (3) DLM commissioning (CT clamp + supply limit + simulated load test); (4) OCPP integration per charger (boot + heartbeat + auth + smart-charging test); (5) OZEV WCS grant evidence collection + submission; (6) Final Schedule of Inspections + Schedule of Test Results + EIC per charger; (7) Customer handover with cert evidence bundle access',
      'No commissioning',
      'Customer tests',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Same as workplace',
      'Substantial scope. Per-charger Section 722 + BS EN 61851-23 manufacturer DC commissioning × 8. BS EN IEC 61439-7 LV multi-charger assembly verification. Transformer + HV switchgear commissioning (HV-qualified engineer + DNO sign-off). PCMS / OCPP integration per charger + OCPI roaming integration. PCAR 2023 pillar verification per charger (payment terminal + pricing display + helpline + open-data publication). PAS 1899 accessibility audit per bay. CMA / consumer protection compliance documentation. Cert evidence bundle: structured digital folder + handover to CPO with full ongoing operator obligations',
      'No documentation',
      'Customer guesses',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Same as 1970s install',
      'Per-charger OPDD self-test + RDC-PD self-test verification (M6.8 EV-specific tests); BS EN IEC 61439-7 multi-charger assembly inspection; DLM CT clamp + supply limit verification (still operating + reading correctly); OCPP integration verified working (CPMS dashboard shows chargers + heartbeat); SCP Regulations 2021 default off-peak schedule still active; OZEV WCS records still on file; per-PCAR-pillar check if site has become public-access',
      'No EV items',
      'Customer decides',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Cannot integrate',
      'OCPP is open-standard; chargers from any OEM (ABB, Wallbox, Easee, EO, etc.) talk OCPP to any compatible CPMS. Per-charger commissioning: enter CPMS endpoint + credentials + identifier + verify boot + heartbeat. Each manufacturer’s commissioning app may differ; the OCPP protocol layer is consistent. Test smart-charging profile reception works across vendors. Cert evidence bundle records per-charger OCPP version + OEM + integration test result',
      'Single vendor only',
      'No CPMS needed',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Nothing',
      'Formal handover document: installer scope completed (Section 722 + 61439-7 + DNO + commissioning + PCAR pillar configuration + PAS 1899 evidence); CPO ongoing scope accepted (PCAR 2023 ongoing compliance + accessibility maintenance + customer service + billing + open-data publication + EICR ownership); named contacts; warranty terms; cert evidence bundle access transfer. Both installer + CPO sign. UK 2025-26 standard practice; structured digital document via SharePoint / Confluence',
      'Verbal agreement',
      'Customer email',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Nothing changes',
      'Wallbox / charger end-of-life consideration (UK 2025-26 typical 7-15 year service life on Mode 4 ultra-rapid); transformer mid-life inspection per HV schedule; manufacturer support continuity check (some brands consolidate / change over 10 years); regulatory evolution (PCAR amendments + new accessibility standards likely); roaming hub evolution (OCPI version migration); CPMS migration consideration; total site refresh assessment. Cert evidence bundle compares year-0 baseline to current; planning for site refresh begins',
      'No EICR',
      'Customer decides',
    ],
    correctAnswer: 1,
