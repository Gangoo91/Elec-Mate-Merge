...
const BS7671Module4Section1 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 1 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'Two layers of defence — basic and fault protection — and the four protective measures the regulations actually recognise. Includes the headline A4 change (Reg 411.3.4 luminaire RCD) and how the inspector evidences ADS on site.'
          }
        </div>
        ...
        >
          <p>
            ...
            <strong> (c)</strong> automatic disconnection by an overcurrent protective device or RCD
            (Reg 411.3.2), and <strong>(d)</strong> coordination of these with the system earthing
            arrangement (Reg 411.4.4 TN, 411.5.3 TT or 411.6.3 IT).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            assessment must accompany the EIC. Reg 411.3.4 — new in A4 — has no equivalent exception
            within domestic (household) premises.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            installation's exposed-conductive-parts are connected back to that earth via a dedicated
            CPC. Reg 411.4.4 sets the Zs ≤ U₀ × Cmin / Ia condition — the fault loop impedance must
            be low enough that the OPD or RCD interrupts within Table 41.1's time. TN-S keeps
            protective and neutral conductors fully separate from the source; TN-C combines them as
            a PEN throughout (now rare in UK installations beyond the cut-out); TN-C-S (PME) is
            combined upstream of the cut-out and split into separate N and PE inside the consumer's
            installation.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            isolated or switched. Putting a switch or isolator in the PEN line creates the open-PEN
            failure that is the single most dangerous mode of TN-C-S. An open PEN with load on the
            system drives the local earth potential up — every CPC-connected exposed metal part
            rises with it. Reg 722.312.2.1 (new in A4) extends this to EV charging: the EV-charging
            circuit in a TN system shall not include a PEN conductor at all, because of the
            heightened touch-current risk in vehicles.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 411.5.3 (TT with RCD): the disconnection time shall meet Reg 411.3.2.2 or 411.3.2.4
            (1 s for distribution, 0.4 s for final), and Ra × IΔn ≤ 50 V — Ra being the sum of the
            earth-electrode and protective-conductor resistance back to exposed metal. Reg 411.5.4
            (TT with OPD only) requires Zs × Ia ≤ U₀, the same form as TN but with the higher Ra of
            a separate electrode. Reg 411.6.3 (IT) requires either continuous insulation monitoring
            or, where the second fault occurs, automatic disconnection meeting the same Table 41.1
            times.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            RMS or 120 V DC ripple-free. Reg 414.4.4 still requires basic protection (by insulation
            per Section 416, or barriers/enclosures per Reg 416.2.1) where the nominal voltage
            exceeds 25 V AC / 60 V DC ripple-free, or where equipment is immersed.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            is ≤ 50 V AC / 120 V DC ripple-free, but not all of Section 414's requirements for SELV
            or PELV are met, and SELV / PELV is not strictly required. Reg 411.7.4 forces the source
            to be either a transformer with at least simple winding separation OR a source complying
            with Section 414.3. If neither, the circuit isn't FELV — it's just a low-voltage circuit
            operating at extra-low voltage. Reg 411.7.3 then requires basic AND fault protection AT
            THE PRIMARY VOLTAGE — no shortcuts based on the secondary being 24 V.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            arrangements use the dedicated 110 V coloured connector system (BS EN 60309-2) so 110 V
            kit can never be inadvertently plugged into 230 V.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            to an extraneous-conductive-part — completing the equipotential zone within the
            location.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            correction). <strong>3. MCB rating.</strong> Type B 32 A on the ring standard, but check
            Reg 433.1.1: In ≥ Ib (32 ≥ 25 ✓), In ≤ Iz only if you accept the ring topology (BS 7671
            ring rules). <strong>4. Disconnection time target.</strong>
            0.4 s for a sub-32 A ring final on TN. <strong>5. Zs target.</strong> From the Type B 32
            A row of Appendix 3 / OSG: Zs(max) = 1.37 Ω at 230 V (corrected). Plan to measure ≤ 1.10
            Ω at the furthest point to leave a safety margin.{' '}
            ...
            <strong>8. Cert entries.</strong> EIC schedule of test results: measured Zs, Ze, R1+R2,
            RCD operating time at IΔn and 5 IΔn, MCB type and rating, RCD type and rating. The EIC
            schedule of inspection ticks Sections K (RCD additional protection) and L (luminaire RCD
            per A4) explicitly.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            verify it' in seconds. For typical UK domestic on TN-C-S: ADS via 30 mA RCBO everywhere,
            with Reg 411.3.4's domestic luminaire rule (A4) ensuring lighting circuits get the same
            protection as sockets. For commercial: same logic, more rigorous documentation, more
            attention to special locations and load types.
          </p>
        </ConceptBlock>
