...
const BS7671Module7Section2 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 7 · Section 2 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'The dedicated BS 7671 section for EV charging — Mode 2/3/4 equipment, the new A4 PEN prohibition (Reg 722.312.2.1), the RCD-type rule (Reg 722.531.3.101), open-PEN detection devices, and how to specify a compliant domestic or commercial install on TN-C-S, TN-S or TT.'
          }
        </div>
        ...
        >
          <p>
            ...
            411 (ADS), Reg 415 (additional protection), Reg 461 (PEN switching) — and TIGHTENS them
            in EV-specific ways. Part 722 does not override the general parts; it adds to them. So
            the EV circuit gets ALL the general protective measures PLUS the Section 722 overlays.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Mode 1:</strong> Direct connection to a standard socket-outlet. No communication
            with the vehicle. BS EN 61851-1 permits this only at low currents and with full circuit
            protection — most UK manufacturers do not recommend it. <strong>Mode 2:</strong>{' '}
            Standard socket with an in-cable control box (ICCB) that provides control pilot
            signalling and may include protective functions. The familiar "granny cable" supplied
            with a new EV. <strong>Mode 3:</strong> Dedicated EVSE (Electric Vehicle Supply
            Equipment) — a fixed wall box or post with a Type 2 socket or tethered cable, full
            control pilot signalling, and integrated protection. The UK domestic standard.{' '}
            <strong>Mode 4:</strong> DC off-board charger — the AC-to-DC conversion happens in the
            charger, not in the vehicle. CCS and CHAdeMO connectors. 50 kW to 350 kW+. Public
        ...
        >
          <p>
            The reason this regulation is EV-specific (rather than a blanket extension of Reg 461.2)
            is risk magnification. On a normal kettle, an open-PEN puts ~230 V on the kettle case
            but the user contacts a small handle area. On an EV, the conductive body of the vehicle
            is the touch surface — large area, frequent contact, often outdoors in wet conditions,
            often standing on a driveway in direct earth contact. The same fault that gives a
            survivable shock on a kettle gives a fatal shock on a car. A4 addresses this by removing
            the PEN from the EV circuit entirely.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            detection device), the EIC schedule should record the device manufacturer, model and the
            BS EN reference (typically BS EN 61851-1 Annex covering open-PEN detection, plus the
            device's own product standard such as BS 7671 referenced or a UK manufacturer
            declaration). For Route 3 (TT island), record the earth-electrode resistance Ra and
            confirm Ra × IΔn ≤ 50 V (Reg 411.5.3). For Route 1 (TN-S configuration), confirm that N
            and PE are split upstream of the EV circuit and that no PEN exists anywhere in that
            final circuit.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            protection against DC fault current of 6 mA or greater, the upstream device must handle
            it. Manufacturer's spec is the binding evidence.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            from Section 522 (the AD water codes, AG mechanical impact codes, AE foreign body codes)
            drive the IP/IK rating numbers. Don't skip the cable too: the charging cable connector
            itself must meet IP44 minimum when engaged and IP24 when stowed (BS EN 62196-2 for Type
            2 connectors). For tethered chargers, the cable holster integrated into the unit usually
            handles this — but check it on commissioning.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The two regimes don't conflict but they do overlap. BS 7671 Reg 311 / 433 sets the cable
            and protective device based on Ib — if load management caps Ib at 20 A peak, that's what
            you design to. The Smart Charge Point Regulations require the load-management capability
            be present and functional. Document the load-management envelope on the EIC so any
            future inspector can see why a 6 mm² cable was specified against a 32 A protective
            device.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            An EICR is assessed against current-edition risk per GN3 Section K, not just against the
            edition in force at install. An older EV install that predates A4 is not automatically
            C3 just because A4 is new — the C2 call hinges on the real-world shock-risk severity.
            Bare TN-C-S into an EV is a documented, well-understood severe hazard regardless of when
            it was installed; that warrants C2 in nearly every case with a clear remediation path
            (retrofit a charger with integrated open-PEN detection, or add an in-line open-PEN
            device, or split N/PE upstream).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            This template handles 80%+ of UK domestic EV retrofits in the A4 era. The two variations
            are: (a) a charger that does NOT have integrated 6 mA DC detection — fit Type B 30 mA
            RCBO instead (cost increase, otherwise identical); (b) a charger that does NOT have
            integrated open-PEN detection — install a separate in-line open-PEN device, or split
            N/PE upstream of the EV circuit, or drive a separate electrode for a TT island. Both
            variations are uncommon with current-generation domestic chargers but still appear on
            commercial three-phase units and older retrofit orders.
          </p>
        </ConceptBlock>
