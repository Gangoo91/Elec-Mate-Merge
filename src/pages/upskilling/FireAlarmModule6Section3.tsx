...
const inlineChecks = [
  {
    ...
    options: [
      'Faults can be silenced.',
      "Where the user has reported a fault on the system to the service organisation and the service organisation cannot complete an effective repair, fault indications should not be concealed (e.g. by suppressing the fault indication). The practical implication: do not modify the panel's output to hide the buzzer; do not put tape over the LCD; do not patch the firmware to ignore the input. The fault must remain visible until repaired. The fault is also logged, premises management is informed, and an interim measure is put in place if appropriate.",
      'Disable the panel.',
      'Cover the fault LED with tape.',
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Replace the panel.',
      "Read the panel's event log / LCD message. The CIE is the most reliable source of information about what it has detected. Modern addressable systems will name the device, the loop, the address, and the type of fault. Conventional systems will indicate the affected zone. The event log records when the fault first appeared, whether it has happened before, and what other events accompanied it. Diagnosis without reading the panel is guesswork; reading the panel first narrows the search by orders of magnitude.",
      'Open every detector head.',
  ...
  },
  {
    ...
    options: [
      'Eight failed devices simultaneously.',
      'A single point fault on the loop between devices 16 and 17 — typically a corroded terminal, a damaged conductor, or a poor termination at one device that causes the loop to fail forward of that point. Addressable loops are wired ring (out and back); a break in the out-leg loses everything beyond the break, and the panel reports those devices as "missing". Repair: visit device 16, check the outgoing terminals and the cable; if no fault there, visit device 17, check the incoming terminals.',
      'Random EMI.',
      'Detector age.',
    ],
    correctIndex: 1,
    explanation:
      'Contiguous device losses on an addressable loop almost always indicate a single break in the loop wiring. Repair at the panel-side end of the missing range. Loops should be wired as a ring with isolators so a single break loses no devices on a healthy loop — the fact that eight are lost suggests either no isolators, isolators in wrong locations, or both legs of the ring broken at different points.',
  },
  {
    id: 'fam6-s3-tools',
    question:
    ...
    options: [
      'Just a multimeter.',
      "Multifunction tester (continuity, IR, voltage), addressable loop tester (programs and queries the loop independently of the CIE), CIE-specific service software (where applicable, with appropriate authentication credentials), test smoke / heat for detector functional testing, sounder / VAD test ammeter, and the system's as-installed documentation and cause-and-effect matrix to verify expected behaviour. IR testing on fire-alarm circuits must be applied with care — some devices and electronics are damaged by 500 V test voltage and the manufacturer's instructions must be followed (often a low-voltage continuity test only is acceptable on installed loops).",
      'A hammer.',
...
const quizQuestions = [
  {
    ...
    options: [
      'Servicing intervals.',
      'Concealment or suppression of fault indications. Where the service organisation cannot complete an effective repair, fault indications must not be concealed (e.g. by suppressing the fault indication on the CIE, taping over fault LEDs, modifying firmware to ignore the input). The fault must remain visible until repaired. Interim measures and proportionate logging are how the risk is managed — not by hiding the warning.',
      'Detector cleaning.',
      'Battery dating.',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Turn off the panel.',
      "Read the CIE's LCD message and event log. The panel has the most information about what it has detected — device address, loop, fault type, time of first onset, repeat history. The event log frames the rest of the diagnostic process. Going to the device before reading the panel is the most common cause of wasted time in fault finding.",
      'Replace the most recent device.',
      'Disconnect a random conductor.',
    ],
    correctAnswer: 1,
    explanation:
      'Diagnostic discipline: panel first, device second. The CIE knows. Read the LCD, read the event log, then act.',
  },
  {
    id: 3,
    question: 'Which of the following is an OPEN-CIRCUIT fault on a conventional zone?',
    options: [
      'A short between L and earth.',
      'A break in the loop conductor between the panel and the next device, leaving the end-of-line resistor disconnected from the panel. The panel measures circuit resistance higher than the EOL value (often "infinite" or above a fault threshold) and reports open-circuit fault for the affected zone. Causes: physical break in the cable, loose terminal, failed device internal connection, removed device without re-fitting the EOL.',
      'A device drawing too much current.',
      'A panel software glitch.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A device address conflict.',
      "A reduction of circuit resistance below the panel's threshold — caused by a conductor-to-conductor contact (cable damaged and the two conductors touching), water ingress shorting the conductors at a back-box, a faulty MCP whose contacts are stuck in the alarm position, or a faulty detector whose terminal block has shorted internally. The panel reports a short fault and may also enter alarm depending on the resistance value.",
      'A loop comms error.',
      'An empty battery.',
    ],
    correctAnswer: 1,
    explanation:
      'Short-circuit = resistance lower than expected. Common causes: physical conductor short, water ingress, stuck MCP, faulty device. Some short-circuit values trigger an alarm (because they look like an MCP being operated); others trigger only a fault.',
  },
  {
    id: 5,
    question:
    ...
    options: [
      'All eight failed simultaneously.',
      'A single break in the loop wiring between device 16 and device 17. First action: read the panel\'s loop diagnostics (some CIEs report "loop is open at device 16 outgoing"). Visit device 16: check the outgoing terminals and the cable for damage. If 16 looks healthy, visit 17: check incoming terminals. Repair the connection or the cable, restore loop, panel will re-discover devices 17-24 within a discovery cycle. Note: well-designed loops have isolators so a single break loses NO devices — if eight are lost, the isolator placement / count is also a design issue to flag.',
      'Random EMI.',
      'CIE failure.',
    ],
    correctAnswer: 1,
    explanation:
      'Contiguous loss on an addressable loop = loop break, located at the panel-side end of the missing range. Diagnostic discipline: panel diagnostics first, then walk the loop. The fact that eight devices are lost is also a design red flag — isolators should limit the loss to between two adjacent devices.',
  },
  {
    id: 6,
    ...
    options: [
      'A loud bell.',
      'Two devices reporting the same address — the panel sees inconsistent responses to address polling and typically reports an address-conflict fault, naming the address and (depending on the CIE) the affected loop. Cause: an installer programmed two devices to the same address, or replaced a device without re-programming. Repair: identify the duplicate via service software, re-address one device. The fault is benign in terms of fire safety (no immediate impact on detection) but indicates a configuration error that should be resolved promptly.',
      'A fire alarm.',
      'Total loop failure.',
    ],
    correctAnswer: 1,
    explanation:
      'Address conflict = two devices think they are the same address. Service software identifies the duplicate; re-address one device. Configuration error, not hardware fault.',
  },
  {
    id: 7,
    question:
    ...
    options: [
      ...
      'Investigate the pattern before replacing the detector. Read the contamination level (high contamination is a common cause of false alarms — plan clean / replace). Examine environmental factors (steam from a kitchen, dust from refurbishment, vibration from equipment). Examine cause-and-effect (was the detector triggered by a real low-level smoke event from another zone?). Consider the detector technology — if the corridor is regularly exposed to cooking smoke or shower steam, the optical smoke detector may need to be replaced with a multi-sensor heat / smoke detector that is more tolerant of nuisance aerosols. Replacement of the detector without investigation often just moves the problem to the new device.',
      'Disable detector 14 permanently.',
      'Increase the alarm threshold for the whole panel.',
    ],
    correctAnswer: 1,
    explanation:
      'False alarm investigation: read the contamination, walk the environment, review cause-and-effect, consider technology change. Replacement-without-investigation is unprofessional and often ineffective.',
  },
  {
    id: 8,
    question:
    ...
    options: [
      'Apply 500 V dc anywhere.',
      "Follow the manufacturer's instructions. Many addressable detectors and electronics are damaged by 500 V test voltage and require either a low-voltage continuity test on the installed loop, or detector-disconnection-and-IR-on-cabling-only. Some systems explicitly forbid IR testing with detectors connected. Read the manufacturer manual; if in doubt, apply continuity-only tests on the installed system, and apply IR tests only on dead cabling before devices are connected.",
      'Apply 1000 V dc.',
      'No testing required.',
    ],
    correctAnswer: 1,
  ...
  },
  {
    ...
    options: [
      'Trivial.',
      'A loop conductor is leaking to earth somewhere — typically a damaged cable in conduit / containment, a back-box that has filled with water and is grounding through the box, or a device whose internal isolation has failed. The system continues to operate (loops are usually floating with respect to earth) but the leak is the first stage of a developing fault. With a second leak elsewhere, the loop will short to itself via earth and a much more serious fault is likely. Repair urgently — locate the leak with insulation testing on the dead loop or with a clamp-meter approach on the live loop, then trace and rectify.',
      'Always a panel fault.',
      'No action needed.',
    ],
    correctAnswer: 1,
    explanation:
      'Earth fault is a leak between a loop conductor and earth. Single leak = warning; second leak compounds to short-circuit. Investigate and repair before the second leak appears.',
  },
  {
    id: 10,
    question:
    ...
    options: [
      'Conceal the fault to keep the user happy.',
      'NOT conceal the fault indication. Continue investigating: review the panel event log for time-of-day patterns; consider environmental factors (HVAC cycle, sunlight on the detector, occupant activity); install monitoring instrumentation if available; consult the manufacturer. Brief the responsible person on the open status. If the fault remains unresolved, it stays visible on the panel until rectified — the service organisation does not modify firmware or output behaviour to suppress the indication.',
      'Disable the detector permanently.',
      'Replace the panel.',
    ],
    correctAnswer: 1,
    explanation:
      'Clause 23 — no concealment. A fault that cannot be reproduced is still a fault; the indication remains visible, the investigation continues, and interim measures may be put in place. Hiding the indication would be a clause 23 breach and likely a regulatory breach under RRO 2005.',
  },
];
