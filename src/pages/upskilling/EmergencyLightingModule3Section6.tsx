...
const inlineChecks = [
  {
    ...
    options: [
      'Word and Excel.',
      'Dialux (DIAL GmbH, Germany — free), Relux (Relux Informatik, Switzerland — free), AGi32 (Lighting Analysts, US — paid). Manufacturer-specific tools (Helvar Designer, Tridonic LumKit, Designplan emergency-lighting-design) sit alongside for product-specific calculation and design output. All accept LDT/IES photometric data and produce point-by-point grid calculations.',
  ...
  },
  {
    ...
    options: [
      'They are identical.',
      'LDT (Eulumdat) is the European format, IES (LM-63) is the North American format. Both describe luminaire intensity distribution but with different file structures, header conventions and angular reference systems. Most lighting design software reads both transparently. Manufacturers typically publish BOTH for the same luminaire so designers in any market can use the data.',
      'LDT is for indoor only.',
      'IES is obsolete.',
    ],
    correctIndex: 1,
    explanation:
      'LDT (Eulumdat) and IES (LM-63) describe the same physical thing — luminaire candela distribution — through different formal structures. European designers default to LDT, North American to IES. Modern lighting design software reads both. Manufacturers serving both markets publish both formats.',
  },
  {
    id: 'elm3-s6-pitfall',
    ...
    options: [
      ...
      'Leaving surface reflectance at the software default (typically 70/50/20 for ceiling/wall/floor) instead of forcing 0% per BS EN 1838:2024. The default produces lux values inflated by reflected-light contribution; the certified emergency lux is direct-only. Designs passing on default reflectance can fail on commissioning when measured against the standard.',
      'The software is in English.',
      'The cursor is too fast.',
    ],
    correctIndex: 1,
    explanation:
      'The reflectance default trap is the dominant software pitfall. Most lighting design software uses normal-lighting reflectance defaults; the emergency calculation requires 0%. Designers must explicitly set 0% on every surface or use the emergency-lighting calculation mode (where available) that forces it. Reports that do not state 0% are non-evidenced for emergency duty.',
  },
  {
    id: 'elm3-s6-fiveyear',
    ...
    options: [
      'Optional.',
      'Mandatory periodic re-verification of the photometric design at 5-year intervals (or after major change). The original design calculation must be re-run with current luminaire condition (light output reduced by lumen depreciation) and current building decoration changes. Compares calculated current lux with the standard minima; identifies maintenance / replacement / redesign needs before the system fails compliance at periodic test.',
      'Daily.',
      'Once at install only.',
    ],
    correctIndex: 1,
    explanation:
      'BS 5266-1:2025 §8.2 introduces 5-year photometric verification. The reasoning: lumen depreciation, dirt, decoration changes accumulate over years; the original design lux is no longer guaranteed. The 5-year re-calculation confirms current compliance and triggers preventive action. Software must support the re-verification workflow.',
  },
];
...
const quizQuestions = [
  {
    ...
    options: [
      'Word and Excel.',
      'Dialux (free, German-origin), Relux (free, Swiss-origin), AGi32 (paid, US-origin), plus manufacturer-specific tools (Helvar Designer, Tridonic LumKit, Designplan tools).',
      'Photoshop only.',
      'No software is needed.',
    ],
    correctAnswer: 1,
    explanation:
      'Dialux and Relux are the dominant free tools; AGi32 is the leading paid tool. Manufacturer-specific tools sit alongside for product workflows. All read LDT/IES.',
  },
  {
    id: 2,
    ...
    options: [
      'Identical.',
      'Different file structures expressing the same physical data — luminaire intensity distribution. LDT is European (Eulumdat), IES is North American (LM-63). Most software reads both.',
      'For different lamp types.',
      'Compressed images.',
    ],
    correctAnswer: 1,
    explanation:
      'Different file formats, same physical content. LDT for European, IES for North American. Both express candela distribution.',
  },
  {
    id: 3,
    ...
    options: [
      'Slow rendering.',
      'Leaving surface reflectance at the software default (typically 70%/50%/20%) instead of forcing 0% per BS EN 1838:2024 §5.2. The default inflates the calculated lux with reflected-light contribution that the standard does not count.',
      'Wrong colour palette.',
      'Outdated icons.',
    ],
    correctAnswer: 1,
    explanation:
      'Forgetting to set 0% reflectance is the dominant emergency-lighting design error in software. The standard requires 0%; the software defaults to typical reflectances. Designers must explicitly override.',
  },
  {
    id: 4,
    ...
    options: [
      ...
      'Re-run the original photometric calculation at approximately 5-year intervals (or after major changes), with luminaires at their current depreciated output, accounting for any decoration changes, to verify continued compliance with the standard minima.',
      'Replace all luminaires every 5 years.',
      'Switch to a different standard.',
    ],
    correctAnswer: 1,
    explanation:
      'BS 5266-1:2025 §8.2 introduces 5-year photometric verification. The original design assumed a maintenance factor; the 5-year check confirms the actual condition still meets the duty. Software supports the re-verification workflow.',
  },
  {
    id: 5,
    ...
    options: [
      'Marketing claims.',
      'Software-calculated point-by-point lux against physically measured lux at the same points using a calibrated lux meter (also at 0% reflectance assumption). Significant discrepancies trigger investigation — installation error, photometric data error, calibration error, or environmental factor.',
      'Color temperature.',
      'Battery weight.',
    ],
    correctAnswer: 1,
    explanation:
      'Commissioning verification = software prediction vs measured reality at the same points. Both must agree within tolerance. Discrepancies are diagnosed before the design is signed off.',
  },
  {
    id: 6,
    ...
    options: [
      'They are free.',
      'They integrate the manufacturer\'s photometric data directly, often include manufacturer-validated typical layouts, output design documents in a manufacturer-recognised format, and can directly feed manufacturer commissioning / DALI gateway tools. Limited to the manufacturer\'s products; not suitable for multi-manufacturer designs.',
      'They are bigger.',
      'They are decorative.',
    ],
    correctAnswer: 1,
    explanation:
      'Manufacturer tools tie design to product, simplify the workflow when committing to a single manufacturer ecosystem (DALI control, monitoring), but limit flexibility. General-purpose tools (Dialux, Relux) suit multi-manufacturer designs; manufacturer tools suit single-manufacturer.',
  },
  {
    id: 7,
    question: 'A photometric calculation report should include which evidence components?',
    options: [
      'Only the average lux.',
      'Point-by-point grid lux on the target plane, minimum / maximum / uniformity ratio, calculation grid spacing, surface reflectance values used (must show 0% for emergency), maintenance factor, luminaire schedule with photometric file references, calculation date, software version.',
      'A photo only.',
      'Sales pitch.',
    ],
    correctAnswer: 1,
    explanation:
      'A complete report evidences every parameter that affects the result. Grid + min/max/uniformity + reflectance + MF + schedule + date + software version. Each lets a future reviewer reproduce or audit the calculation.',
  },
  {
    id: 8,
    ...
    options: [
      ...
      'A third party (commissioning engineer, periodic test inspector, future modifier) can re-run the photometric calculation with the same inputs and arrive at the same outputs. Requires the software files (Dialux / Relux project), the photometric data files (LDT / IES), the schedule of luminaires, and the photometric output report to be retained and accessible.',
      'The customer signed off.',
      'The drawing is colourful.',
    ],
    correctAnswer: 1,
    explanation:
      'Verifiable = reproducible. Anyone with the same inputs can re-run and get the same outputs. The documentation pack must include the software project files, LDT/IES files, schedules and reports — all of which combine to make the design re-runnable.',
  },
  {
    id: 9,
    question: '5-year photometric verification per BS 5266-1:2025 typically identifies which issues?',
    options: [
      'No issues are ever found.',
      'Lumen depreciation reducing lux below minima at end of luminaire life; decoration changes (darker walls, soft furnishings) reducing useful light; luminaires removed / relocated during refurbishment without redesign; safety equipment relocated without re-checking vertical lux. Each is a maintenance / redesign trigger before the system fails periodic test.',
      'Only luminaire colour faults.',
      'Only software bugs.',
    ],
    correctAnswer: 1,
    explanation:
      'The 5-year check catches the slow-burn issues that monthly / annual functional tests do not — lumen depreciation, decoration drift, undocumented changes, geometry errors. Each is fixed before it becomes a safety failure.',
  },
  {
    id: 10,
    ...
    options: [
      'Verbal only.',
      'Photometric calculation in software (Dialux/Relux/AGi32 or manufacturer tool) → photometric output report → installation per layout drawing → commissioning measurement of lux at scheduled test points → comparison with calculation → certificate of compliance → 5-year periodic photometric re-verification → updated documentation pack.',
      'A sales receipt.',
      'A dance routine.',
    ],
    correctAnswer: 1,
    explanation:
      'The chain is: software design → output report → installation → measured commissioning → comparison → certificate → 5-year re-verification. Each link is documented; each link must agree. A break anywhere in the chain is non-evidenced.',
  },
];
