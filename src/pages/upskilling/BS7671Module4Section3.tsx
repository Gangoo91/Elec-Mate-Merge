    ...
    <HubPage>
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'Table 41.1 disconnection times per system / circuit type, the Reg 411.4.4 TN formula (Zs × Ia ≤ U₀ × Cmin) and the Reg 411.5.3 TT formula (Ra × IΔn ≤ 50 V), how Zs is measured and corrected for operating temperature, and why the test sequence matters.'
          }
        </div>
        ...
        >
          <p>
            Reg 411.3.2 / Table 41.1 is the source of truth. It distinguishes between final circuits
            (which can be touched directly via socket-outlets or accessible equipment) and
            distribution circuits (which usually feed downstream protective devices). Final-circuit
            times are tighter because the consequence of a failure to disconnect is direct touch
            contact with energised metalwork. Distribution times are looser because there's another
            protective device downstream that's expected to clear the fault first via selectivity.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            required time. For a Type B 32 A MCB at 0.4 s, Ia is read from the time-current curve at
            the 0.4 s line — typically 5×In = 160 A for the magnetic element. So Zs ≤ 218.5 / 160 =
            1.37 Ω. The OSG and Appendix 3 tabulate this for every common In/type combination so the
            field calculation is a table look-up.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Appendix 3 / OSG App I tabulate Zs(max) directly so you don't have to walk the curve in
            the field — the calculation has been done. But understanding what's underneath helps
            with non-standard cases: a Type C 32 A used as a fault-current device only (overload by
            separate means) might be assessed against its 5 s curve rather than 0.4 s, giving a much
            higher Zs(max). Knowing where the number comes from lets you reach for the right curve
            in the right column rather than picking a default that doesn't apply.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Ra is the sum of the earth-electrode resistance and the protective-conductor resistance
            back to the exposed-conductive-part being protected. The product Ra × IΔn is the touch
            voltage that appears on that exposed-conductive-part during a residual-current fault,
            before the RCD operates. Capping it at 50 V keeps the fault-condition touch voltage
            within band I — the threshold below which BS 7671 treats sustained contact as low-risk.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            between line and earth at the point of interest. Modern testers use a high-current pulse
            (typically 25 A for a few cycles) to give a reading that is closer to the actual
        ...
        >
          <p>
            ...
            With α = 0.004 for copper, T_op = 70 °C and T_test = 20 °C, the multiplier is 1 + 0.004
            × 50 = 1.20 — i.e. 20% higher under load. Apply the inverse (0.80) to the Appendix 3
            maximum to get the cold-measured ceiling: a tabulated 1.37 Ω becomes a measured ceiling
            of 1.10 Ω. Tighter cables (running at higher operating temperatures) need a smaller
            factor; the 0.80 figure covers typical UK 70 °C PVC installations.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            unit reference point trailed to the far point of the circuit, measuring R1+R2 directly.
            Less common because it's slower and prone to lead-resistance errors over long runs. Most
            modern multifunction testers can do both two-wire (dead Ze) and three-wire (live Zs)
            measurements via the same socket connection on a final circuit. The schedule of test
            results captures both Ze (origin) and Zs (each circuit far point) — the inspector
            compares to corroborate.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            0.45 Ω. The 0.4 s value would be approximately 0.20 Ω — over twice as demanding. The 5 s
            relaxation is what makes longer feeders practical without upsizing every conductor. But
            the second-line consequence is real: under fault on the sub-board side, the upstream OPD
            takes up to 5 s to clear, leaving exposed metalwork on the sub-board side at near-line
            voltage for that time. Selectivity with the downstream device is therefore essential —
            the downstream MUST clear first.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For typical UK domestic TN-C-S (most common): sockets and fixed equipment ≤ 32 A are 0.4
            s; distribution / sub-mains are 5 s; TT distribution would be 1 s but TT is rare in UK
            domestic. Use OSG App I as the primary look-up — it's faster than Appendix 3 / Reg
            411.4.201-204 and gives the same answers. Appendix 3 is the full reference if you need
            the underlying curve data for non-standard devices.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            the install paints a picture: if every circuit's measured Zs is significantly below the
            ceiling, the install was well-designed and is ageing gracefully. If circuits are
            creeping toward their ceilings, the install is maturing — schedule the next EICR sooner
            than the maximum interval. If circuits are above the ceilings, the install needs work —
            code C2 and recommend the specific actions (typically circuit redesign, supplementary
            RCD, or upgrade of the protective device).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The values are tabulated in Reg 411.4.204 / OSG App I — these are the ones that come up
            most often. Memorising them isn't required (the table is always available) but most
            ...
            tighter. Larger In = lower Zs(max) at the same type because the same magnetic threshold
            applies as a multiple of In, so the required current to operate is higher.
          </p>
        </ConceptBlock>
