...
const inlineChecks = [
  {
    ...
    options: [
      '32 A per phase (√3 × 400 V × 32 A ≈ 22.2 kW)',
      '16 A per phase',
      '63 A per phase',
      '100 A per phase',
    ],
  ...
  },
  {
    ...
    question:
      'What RCD architecture is correct for a three-phase 22 kW Mode 3 wallbox without integrated RDC-PD?',
    options: [
      'A Type AC single-pole RCD on the incoming line feeding all three phases together',
      'A single-pole Type B RCD switching L1 only, leaving L2 and L3 without residual protection',
      'A 4-pole Type B RCD per BS EN 62423, or a 4-pole Type A where the wallbox has integrated RDC-PD',
      'No RCD at all, relying entirely on the wallbox internal electronics for fault disconnection',
    ],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    question:
      'A single 22 kW three-phase Mode 3 wallbox = 32 A per phase. What is the DNO notification position?',
    options: [
      'No DNO notification is ever needed for a single wallbox, whatever the per-phase current',
      'A full G99 application is always mandatory for any 22 kW wallbox without exception or consultation',
      'Notification is needed only if the site exports power to the grid, never for pure import load',
      'At 32 A per phase it exceeds the G98 ≤16 A threshold; generation forces G99 — consult the DNO',
    ],
    correctIndex: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Per-phase: measure L1-PE, L2-PE and L3-PE independently at the wallbox, each ≤ the Table 41.3 limit',
      'A single combined measurement at the wallbox terminals is sufficient for the whole circuit',
      'No Zs measurement is needed on a three-phase EV circuit once the RCD test has passed',
      'The customer measures Zs from the vehicle display and reports the figure to the installer',
    ],
    correctIndex: 0,
    explanation:
...
const quizQuestions = [
  {
    question:
      'A workplace wants three 22 kW three-phase chargers on a three-phase 100 A supply. What is the design constraint?',
    options: [
      'There is no design constraint; three 22 kW chargers fit comfortably under the 100 A supply',
      '3 × 32 A = 96 A per phase plus base load over-runs the 100 A supply, so DLM is mandatory',
      'A 200 A supply must be installed before three 22 kW chargers can be run from the premises',
      'The chargers must be downrated to 7 kW each to fit within the three-phase supply capacity',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'It does not matter; the wallbox accepts any phase rotation and self-corrects internally',
      'The vehicle decides the rotation at plug-in, so the installer need not check it at commissioning',
      'Correct L1→L2→L3 rotation is needed; wrong rotation faults the unit, so verify before energising',
      'Only the L1 connection matters on a three-phase wallbox; L2 and L3 order is irrelevant',
    ],
    correctAnswer: 2,
    explanation:
  ...
  {
    question:
      'BS EN IEC 62196-2 Type 2 connector — which pins are active on a three-phase Mode 3 install?',
    options: [
      'Only the L1 power pin carries current, with L2 and L3 left as spare mechanical pins',
      'Only the PE and N pins are active, with the line pins disabled in three-phase mode',
      'All seven pins: L1, L2, L3, N, PE, CP and PP — three-phase uses every power pin',
      'Only the CP signalling pin is used, with power delivered separately through the body',
    ],
    correctAnswer: 2,
    explanation:
  ...
  {
    question:
      'For a three-phase 32 A Mode 3 install, what is the typical cable size on a ~10 m run?',
    options: [
      '6 mm² 5-core (3L + N + PE) or 6 mm² 4-core SWA with armour CPC, rated 32 A continuous',
      '1.5 mm² twin-and-earth, the same cable size as a typical domestic lighting final circuit',
      '2.5 mm² 3-core flex, run direct from the consumer unit to the three-phase wallbox',
      '25 mm² SWA in every case regardless of run length, for maximum future headroom',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'It does not matter on any EV site, since the DNO transformer absorbs any imbalance',
      'The vehicle on-board charger handles phase balancing automatically during the session',
      'Single-phase wallboxes load one phase, so they must be spread across L1/L2/L3 to stay balanced',
      'It is only relevant on 22 kW three-phase chargers, never on single-phase wallboxes',
    ],
    correctAnswer: 2,
    explanation:
  ...
  {
    question:
      'A three-phase wallbox develops a fault on one phase. What’s the protective behaviour?',
    options: [
      'It simply continues charging on the other two healthy phases with no disconnection',
      'The customer keeps charging with no protective action taken by the wallbox or the RCBO',
      'The unit overheats from the unbalanced load until the cable insulation eventually fails',
      'The 4-pole RCBO disconnects all three phases and N together and the wallbox reports the fault',
    ],
    correctAnswer: 3,
    explanation:
...
export default function RenewableEnergyModule7Section2() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 7 · Section 2 · BS 7671:2018+A4:2026 · Section 722 + Reg 722.531.3.101"
        title="Three-phase 22 kW Mode 3 install"
        backTo="../renewable-energy-module-7"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Three-phase Mode 3 EV charging — 32 A per phase, 4-pole Type B RCD architecture, phase
          balancing across the site, DNO notification under EREC G98 / G99, three-phase OPDD,
          per-phase Zs verification, three-phase Type 2 connector + cable sizing.
        </p>
        ...
        <Pullquote>
          Three-phase Mode 3 is single-phase Mode 3 with three of everything. The Section 722 logic
          doesn’t change; the count of conductors does.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Per-phase current</strong> — 32 A on each of L1, L2,
              L3. Identical to single-phase Mode 3 32 A
            </li>
            <li>
              <strong className="text-white">Three-phase voltage</strong> — 400 V line-to-line (UK
              standard); 230 V phase-to-neutral
            </li>
            <li>
              <strong className="text-white">Power formula</strong> — P = √3 × V_LL × I × cos φ. For
              balanced 32 A across all three phases at unity power factor: √3 × 400 × 32 ≈ 22,168 W
              ≈ 22 kW
            </li>
            <li>
              <strong className="text-white">Neutral current</strong> — under balanced load: ~0 A in
              N. Imbalance produces small N current; designer’s cable sizing assumes balanced
              operation but verifies N capacity for fault scenarios
            </li>
            <li>
              <strong className="text-white">Single-phase fallback</strong> — wallbox + vehicle
              negotiate at plug-in. If the vehicle’s on-board charger is single-phase, the wallbox
              falls back to L1 only (7.4 kW maximum). Some wallboxes also support 16 A single-phase
              fallback (3.7 kW)
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">L1 (Line 1)</strong> — single-phase active; three-phase
              active. Carries up to 32 A in Mode 3
            </li>
            <li>
              <strong className="text-white">L2 (Line 2)</strong> — single-phase NOT connected at
              wallbox; three-phase active. 32 A
            </li>
            <li>
              <strong className="text-white">L3 (Line 3)</strong> — single-phase NOT connected at
              wallbox; three-phase active. 32 A
            </li>
            <li>
              <strong className="text-white">N (Neutral)</strong> — single-phase active (carries
              return current); three-phase active (carries small imbalance current under balanced
              3-ph operation)
            </li>
            <li>
              <strong className="text-white">PE (Protective Earth)</strong> — always active. Wallbox
              PE per Reg 722.411.4 alternatives (OPDD / TT / TN-S)
            </li>
            <li>
              <strong className="text-white">CP (Control Pilot)</strong> — always active. ±12 V PWM
              signalling per BS EN 61851-1
            </li>
            <li>
              <strong className="text-white">PP (Proximity Pilot)</strong> — always active.
              Resistance-coded cable rating per IEC 61851-1 Annex B
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Three-phase EV install has three of every conductor — but only one phase rotation. Get the
          rotation wrong and nothing charges.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              ...
              phase-rotation function. Connect to L1, L2, L3 + N at the wallbox terminals (before
              energising the wallbox internal electronics)
            </li>
            <li>
              <strong className="text-white">Display</strong> — "L1 L2 L3" = correct clockwise; "L1
              L3 L2" = incorrect (swapped). Colour-coded LED or arrows
            </li>
            <li>
              <strong className="text-white">Correction</strong> — swap any two phase cables at the
              wallbox terminals (e.g. swap L2 and L3 at the wallbox). Never swap at the supply side
              — Henley block / CU phasing must remain consistent
            </li>
            <li>
              <strong className="text-white">Re-verify</strong> — after swap, re-test + confirm L1
              L2 L3 displayed. Record both the initial result + the post-swap result if applicable
            </li>
            <li>
              <strong className="text-white">Wallbox self-test</strong> — modern wallboxes detect
              wrong rotation via internal electronics + display error code on first energisation.
              Belt-and-braces with the manual tester check
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — phase rotation check
              result + tester model + date
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Single-phase wallbox assignment</strong> — distribute
              evenly across L1 / L2 / L3. Document each assignment on the install schematic
            </li>
            <li>
              <strong className="text-white">Three-phase wallbox</strong> — inherently balanced (32
              A per phase)
            </li>
            <li>
              <strong className="text-white">DLM coordination</strong> — DLM hardware reads
              per-phase current; if the system detects sustained imbalance, it can trigger an alarm
              + suggest re-balancing
            </li>
            <li>
              <strong className="text-white">Supply integrity</strong> — DNO supplies are typically
              balanced; large imbalance can cause voltage skew, transformer stress, neutral current
              excess. Imbalance limit varies by DNO (typically &lt;20%)
            </li>
            <li>
              <strong className="text-white">Phase rotation consistency</strong> — every
              single-phase wallbox + three-phase wallbox must use the SAME L1 reference. Phase
              labelling on the site schematic + at each CU way + at each wallbox terminal
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — phase assignment table
              per charger + supply balance calculation
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">G98 Type A scope</strong> — single-phase ≤16 A per
              phase, OR three-phase ≤16 A per phase. Microgeneration / small generation focus
            </li>
            <li>
              <strong className="text-white">G99 scope</strong> — anything above G98 Type A.
              Includes single-phase &gt;16 A, three-phase &gt;16 A per phase, any V2G, any export
            </li>
            <li>
              <strong className="text-white">EV pure-load case</strong> — a 22 kW wallbox at 32 A
              per phase is above G98 per-phase threshold. Different DNO areas interpret differently
              for pure-load chargers. Some allow G98 fast-track; others require G99
            </li>
            <li>
              <strong className="text-white">EV + BESS / V2G / PV case</strong> — always G99.
              Generation present triggers G99 regardless of load size
            </li>
            <li>
              <strong className="text-white">Multi-charger commercial</strong> — even pure-load
              multi-charger commercial sites typically G99 due to aggregate capacity. DNO often
              requires formal application for cluster sites
            </li>
            <li>
              <strong className="text-white">Lead time</strong> — G98 notification: weeks; G99
              formal: 6-18 weeks depending on DNO + connection complexity. Plan accordingly
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — DNO reference number,
              application date, approval date (if G99), conditions of approval, post-commissioning
              confirmation to DNO
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Reg 411 limit</strong> — Zs ≤ Table 41.3 value for
              protective device type + rating. 32 A Type B B-curve: Zs ≤ 1.37 Ω @ 230 V phase-to-N
            </li>
            <li>
              <strong className="text-white">L1-PE</strong> — measured with phase-PE loop test at
              wallbox terminals
            </li>
            <li>
              <strong className="text-white">L2-PE</strong> — measured separately; expect within
              ~10% of L1
            </li>
            <li>
              <strong className="text-white">L3-PE</strong> — measured separately; expect within
              ~10% of L1
            </li>
            <li>
              <strong className="text-white">High-current vs no-trip mode</strong> — high-current
              method gives true Zs but trips RCD if RCD in circuit; no-trip mode preserves RCD but
              lower accuracy. Most modern three-phase testers offer both
            </li>
            <li>
              <strong className="text-white">Phase imbalance effects</strong> — DNO transformer +
              supply cable asymmetry produces small but measurable Zs differences between phases.
              Symptom of larger imbalance: differences &gt;20% between phases — investigate supply
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — all three values
              recorded individually, not averaged. Margin to Table 41.3 documented per phase
            </li>
          </ul>
        </ConceptBlock>

        <Pullquote>
          Phase rotation, phase balance, per-phase Zs — three checks no domestic install has, every
          commercial three-phase install needs.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Verification step</strong> — phase-sequence tester at
              wallbox terminals BEFORE energising wallbox electronics
            </li>
            <li>
              <strong className="text-white">Correct result</strong> — L1 L2 L3 clockwise. Proceed
              to wallbox energisation
            </li>
            <li>
              <strong className="text-white">Wrong result</strong> — L1 L3 L2 (or similar
              non-clockwise). Swap any TWO phase conductors at the wallbox terminals
            </li>
            <li>
              <strong className="text-white">Never swap at the supply</strong> — supply-side phase
              reference is the SITE reference. Swapping at Henley block / meter tails / CU input
              cascades incorrect phasing across every downstream circuit
            </li>
            <li>
              <strong className="text-white">Re-verify post-swap</strong> — confirm clockwise L1 L2
              L3. Proceed
            </li>
            <li>
              <strong className="text-white">Update labelling</strong> — if a swap happened, update
              wallbox internal phase labelling to match (L1 = L1 of supply, not L1 of cable colour).
              Avoids future misdiagnosis at EICR
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — phase rotation check
              result + correction (if any) + post-swap re-verification
            </li>
          </ul>
        </ConceptBlock>
