    ...
    <HubPage>
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'The four functions BS 7671 actually distinguishes — isolation, switching for mechanical maintenance, emergency switching and functional switching — with the device requirements of Section 537, the PEN prohibition of Reg 461.2 and the safe-isolation procedure that ties it all to EAWR 1989 / PUWER 1998.'
          }
        </div>
        ...
        >
          <p>
            ...
            operational on/off control. Each function has its own device class, its own positioning
            rules and its own labelling. Choosing the wrong function is the most common Section 537
            design mistake on inspection.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Section 462 sets the design intent: every installation shall provide means whereby every
            circuit can be isolated from each of the live supply conductors, except where
            ...
            installation, every circuit and every item of equipment as may be required to prevent or
            remove a hazard. Reg 537.2 then translates that intent into device-level requirements —
            the physical attributes the isolating device must possess.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The reasoning is fault-current physics. In normal operation the PEN carries the neutral
            return current of the loads, so it sits at approximately 0 V relative to true earth. A
            correctly bonded MET ties every CPC-connected exposed metal part to that 0 V reference.
            Open the PEN with load on the system and the neutral current has nowhere to go — it
            diverts through whatever earth paths are available (extraneous-conductive-parts,
            plumbing, structural steel, soil moisture). The voltage of the local earth reference
            RISES toward line voltage, and every Class I exposed metal part bonded to the MET rises
            with it. The fault is INVISIBLE to a basic insulation-resistance test — it only
            manifests under load with the PEN open upstream.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            physical injury, suitable means shall be provided to switch off the supply. Reg 537.3.1
            requires the device to be inserted in the main supply circuit (so the maintenance worker
            controls the SOURCE, not just a downstream sub-circuit), to be manually operated (not
            auto-resetting), to have an OFF position that is reliably indicated, and to break all
            line conductors of the supply simultaneously. Reg 537.3.2 requires precautions against
            inadvertent or unintentional restoration of the supply during the maintenance work —
            typically by lock-off, position-within-sight, or a key-operated-handle.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            may give rise to an electric shock or other risk of injury, by removing or switching off
            the supply quickly. Reg 537.4.2 sets accessibility: the means of operating shall be
            readily accessible at every place where danger may occur. Reg 537.4.3 sets
            ...
            cannot auto-restore), reset only by deliberate manual action, and able to break the full
            load current of the controlled circuit (directly or via a contactor / shunt-trip).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 537.5.1 requires the device be suitable for the most demanding operating conditions
            of the controlled circuit (rated current, voltage, frequency of operation, breaking
            capacity at the prospective short-circuit current at that point). Reg 537.5.2 permits
            semiconductor switching devices (solid-state relays, triacs) for functional control
            where they are suitable for the load — but Reg 537.2.3 explicitly excludes such devices
            from being used as a means of isolation because they may not provide a verified contact
            gap. The mental model: a functional switch controls the load while live conductors
            remain present in the device; an isolator separates the conductors so the device can be
            safely worked on.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            lockable isolator behind a piled-up storage cupboard is inaccessible. All three must be
            present for the device to satisfy Section 537. On EICR, missing any one is typically a
            C3 (improvement) where the risk is mitigated by other circuit identification, or an FI
            (further investigation) where the failure mode prevents the isolation function being
            verified at all.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The procedure is a chain — every step protects against a different failure mode. Step 5
            / step 7 (prove the indicator works before AND after testing at the work location)
            protect against an indicator that has gone open-circuit or had its battery die mid-task
            — without the post-test on a known live source, an open-circuit indicator reads
            &quot;dead&quot; everywhere including a live conductor. Step 3 (personal padlock with
            key in your possession) protects against a co-worker, customer or cleaner switching the
            supply back on. Step 4 (tag with your name) makes the isolation visible to anyone who
            arrives later. EAWR 1989 Reg 13 (Dead Working) is the statutory hook: the procedure is
            the evidence the duty was discharged.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The reason for breaking the neutral on a double-pole or TPN isolator is fault isolation
            — if the line is broken but the neutral remains connected, a fault to earth on the
            now-isolated neutral can drive current through unexpected paths (especially in TT where
            Ra is high). Breaking both removes that risk. The reason for never breaking the CPC is
            fault protection on adjacent equipment — if the CPC is interrupted at the isolator,
            equipment fed from the same DB but on a different circuit relies on a CPC that may have
            been compromised by the act of opening the isolator. The CPC is the spine of the
            installation; isolators interrupt only the live conductors of the specific circuit being
            isolated.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The failure mode positive contact indication protects against is contact welding — high
            fault current through a closing or opening contact creates an arc that can fuse the
            contact metals. A welded contact stays closed regardless of handle position. A
        ...
        >
          <p>
            The four-function decision tree maps cleanly onto the EIC schedule of inspection: every
            isolator is recorded as Section 462 / 537.2 with its identification and lockability
            ticked, every switch for mechanical maintenance under Section 463 / 537.3, every
            emergency switching device under Reg 537.4 with its red-on-yellow labelling and latching
            action confirmed, and every functional switch as part of the normal circuit description.
            Inspectors and designers using the same vocabulary eliminates the most common Section
            537 disagreement on cert — is this device an isolator, or just a switch?
          </p>
        </ConceptBlock>
