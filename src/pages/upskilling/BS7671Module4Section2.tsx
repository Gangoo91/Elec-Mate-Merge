    ...
    <HubPage>
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'Overload (Section 433) versus fault current (Section 434), the three Reg 433.1.1 coordination conditions, MCB Type B/C/D characteristics from Appendix 3, breaking capacity vs prospective fault current, and how selectivity is actually verified per Reg 536.4.1.'
          }
        </div>
        ...
        >
          <p>
            ...
            rated). Section 434 governs fault current protection (short-circuit and earth fault — up
            to 1000×+ rated). Section 435 covers a single device handling both; Reg 435.2 covers the
            case where two separate devices split the duties (e.g. an instantaneous circuit-breaker
            plus a thermal overload relay on a motor starter).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            In, so condition (c) is automatic when (b) is met. For BS 3036 semi-enclosed fuses, I₂ ≈
            2 × In, so In must be reduced to keep I₂ within 1.45 × Iz.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For RCDs, Reg 536.4.1.3.1 sets a 3:1 ratio of IΔn between upstream and downstream, with
            the upstream being a type S (selective / time-delayed) RCD. Energy-let-through
        ...
        >
          <p>
            The mistake to avoid: looking up cable CCC in App 4, ticking In ≤ Iz against the nominal
            value, and signing off without applying rating factors. The pattern shows up most often
            in commercial work where multiple circuits are routed together — grouping factors of
            0.5-0.7 are common in 3-4-circuit cable trays. A 32 A circuit on 4 mm² T&E with nominal
            Iz of 36 A passes In ≤ Iz comfortably; in a 4-circuit grouping with Cg = 0.6 the
            corrected Iz drops to 21.6 A and the circuit fails. Always apply the factors.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            installation' decision aid. App 4 of BS 7671 is the full reference for everything else.
            The reference-method choice cascades into the CCC value; the CCC value cascades into the
            rating-factor calculation; the corrected Iz determines the maximum permitted In. Get the
            method wrong and the whole chain is wrong, often in a way that doesn't show up until a
            fault stresses the under-rated cable.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Appendix 3 of BS 7671 carries the figures: 3A4 (Type B), 3A5 (Type C), 3A6 (Type D) for
            MCBs to BS EN 60898, plus equivalent figures for RCBOs to BS EN 61009-1, and the BS 88 /
            BS 1361 / BS 3036 fuse families. Each plots time on the vertical (logarithmic, from 0.01
            s to multiple hours) against current as a multiple of In on the horizontal (also log).
            The thermal section is the inverse-square portion in the bottom-left; the magnetic
            section is the near-vertical drop where I/In hits 3 (Type B), 5 (Type C) or 10 (Type D);
            the conventional time is the horizontal band at 1 hour (≤ 63 A) or 2 hours (&gt; 63 A).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For a UK domestic install, the typical answer is: 6 kA Icn on standard CUs is acceptable
            IF measured PFC at the cut-out is comfortably below 6 kA AND the DNO quote agrees. For
            properties near a substation (less than ~100 m of LV main) you are likely above 6 kA and
            need either a 10 kA-Icn product range or back-up coordination behind a 100 A BS 88
            cut-out fuse (which the DNO usually provides) — see Reg 536.5 / 536.6 for the
            coordination route. Three-phase commercial supplies routinely run 16 kA+ — industrial
            MCCBs to BS EN 60947-2 with Icu of 36 / 50 / 65 kA are the standard answer.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            <strong>2. Cable selection.</strong> 4 mm² T&E clipped direct, Reference Method C — App
            4 gives Iz nominal 36 A; with grouping Cg = 0.8 the corrected Iz = 28.8 A.{' '}
            ...
            <strong>6. Breaking capacity.</strong> Measured PSCC at cut-out = 4.2 kA; standard 6 kA
            Icn MCB clears with margin. <strong>7. Cert entries.</strong> EIC schedule of test
            results: measured Zs ≤ 0.55, Ze, R1+R2, MCB type C 25 A 6 kA Icn. Schedule of inspection
            ticks the protective device coordination boxes.
          </p>
        </ConceptBlock>
        ...
        <ContentEyebrow>Voltage drop — the design constraint that catches long runs</ContentEyebrow>
        ...
        >
          <p>
            ...
            and the OSG provides simplified tables for typical UK installations. For most working
            engineers the rough mV/A·m estimate is enough to spot circuits that need attention; the
            precise calculation is reserved for design verification before sign-off. Note that
            voltage drop is checked at NORMAL LOAD (Ib), not fault current — different test,
            different number, different design implication.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The formula: Zs(operating) ≈ Zs(measured) × [1 + α (T_op − T_test)] where α ≈ 0.004 for
            copper, T_op is the operating temperature of the conductor (read from the cable spec —
            typically 70 °C or 90 °C), and T_test is the measurement temperature (typically 20 °C).
            For a 70 °C PVC cable measured at 20 °C, the multiplier is 1 + 0.004 × 50 = 1.2 — i.e.
            the operating Zs is 20% higher than measured. That's the source of the '80% rule' you'll
            see in the OSG: measured Zs ≤ 0.80 × Zs(max from Appendix 3) gives a 20% safety margin
            and accommodates the temperature rise.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            of about 0.37 Ω at 230 V, versus Type B 32 A at 1.37 Ω — a 4× tighter circuit-impedance
            budget. Choosing Type D when Type C would do means harder Zs targets and shorter circuit
            lengths for no safety benefit.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            more variables, more rating factors, more time spent on App 4 — the design effort scales
            with the complexity of the installation. Senior designers walk every step explicitly on
            every circuit; the discipline catches errors that intuition misses.
          </p>
        </ConceptBlock>
