...
const inlineChecks = [
  ...
  },
  {
    ...
    question: 'What is "Loss of Mains" (LoM) protection, and what methods detect it?',
    options: [
  ...
  },
  {
    ...
    question: 'What is the EREC G99 anti-islanding test process at commissioning?',
    options: [
  ...
  },
  {
    ...
    question: 'Vector shift vs ROCOF — what is the difference in detection mechanism?',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'What is "active" vs "passive" anti-islanding?',
    options: [
...
const faqs = [
  {
    question:
      'Why is anti-islanding the categorical safety anchor for any grid-paralleled generator?',
    answer:
...
export default function RenewableEnergyModule11Section7() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 11 · Section 7 · BS 7671 Reg 551.7.4 + 551.7.5 · ENA EREC G99 + G98 · Anti-islanding"
        title="Anti-islanding deep — DNO-witnessed test + Loss of Mains protection"
        backTo="../renewable-energy-module-11"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Reg 551.7.4 + 551.7.5 mechanics. ENA EREC G99 test methodology. ROCOF Loss of Mains
          detection (required for type-tested generation; G99 disallows Vector Shift on type-tested
          inverters — VS legacy only). Active vs passive anti-islanding. DNO-witnessed commissioning
          test + G98 self-declared route. Reg 551.6 changeover + Reg 826.1.1.5 island mode
          interaction.
        </p>
        ...
        <Pullquote>
          A generator that won\'t let go of a dead grid kills the engineer fixing the line. Reg
          551.7.5 isn\'t optional. Anti-islanding is the most important safety feature in any
          prosumer install.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Why it matters</strong>— without anti-islanding, a
              customer\'s generator keeps feeding the local network after DNO disconnect. DNO
              operator believes the line is dead (their disconnect operated) + can be killed
              investigating
            </li>
            <li>
              <strong className="text-white">Reg 551.7.4 + 551.7.5</strong> — combined: 551.7.4 =
              automatic switching on voltage / frequency deviation; 551.7.5 = prevent reconnection
              in event of loss of supply or deviation
            </li>
            <li>
              <strong className="text-white">Scope</strong> — any generating set that can operate in
              parallel with public supply: PV inverter, BESS inverter, wind turbine, CHP (any type),
              micro-hydro, fuel cell. Categorical
            </li>
            <li>
              <strong className="text-white">NOTE — G98 deemed compliant</strong> — "For a
              generating set with an output not exceeding 16 A intended to operate in parallel with
              a system for distribution of electricity to the public, the requirements of Regulation
              551.7 are deemed to be met by compliance with the relevant requirements of EREC G98"
            </li>
            <li>
              <strong className="text-white">G99 path</strong>— for generators above 16 A per phase,
              formal G99 application + DNO connection offer + commissioning test. Reg 551.7.5
              compliance via G99 test record
            </li>
            <li>
              <strong className="text-white">Mechanical implementation</strong> — typically built
              into inverter electronics (PV, BESS, modern wind, micro-CHP) — automatic via control
              loop. For sync generators: dedicated G99 protection relay
            </li>
            <li>
              <strong className="text-white">Test at commissioning</strong>— simulated or physical
              grid-loss test verifies anti-islanding operates within required time. DNO-witnessed
              (G99) or manufacturer-verified (G98)
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— Reg 551.7.5 compliance via G98
              or G99 reference + manufacturer DoC + commissioning test record + DNO sign-off where
              applicable. Categorical part of every PEI cert bundle
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Anti-islanding (Reg 551.7.5)</strong> — automatic trip
              on undetected grid loss; generator disconnects to prevent dangerous islanding of DNO
              network; categorical safety
            </li>
            <li>
              <strong className="text-white">Island mode (Reg 826.1.1.1)</strong> — deliberate
              operation of the installation as an island after grid is detected lost. The
              customer\'s installation is supplied by local source (typically BESS); DNO is
              disconnected; local loads served
            </li>
            <li>
              <strong className="text-white">Reg 826.1.3</strong>— when public network is not
              energised, prosumer operates PEI in island mode OR automatically disconnects all local
              power supplies
            </li>
            <li>
              <strong className="text-white">Sequence in PEI</strong>— (1) Grid loss detected by
              anti-islanding. (2) Generator disconnects from grid. (3) Changeover (Reg 551.6)
              operates to switch installation supply from grid to local source. (4) Local source
              (BESS) supplies installation in island mode
            </li>
            <li>
              <strong className="text-white">Modern integrated PEI</strong> — most modern hybrid
              inverters integrate anti-islanding + transfer switch + island-mode supply takeover in
              a single device. Manufacturer DoC declares the integrated function
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.5</strong>— switching devices for island
              mode shall comply with Reg 512.1.2 + relevant product standard + be suitable for
              isolation
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.2.2</strong>— when in island mode, all live
              conductors shall be disconnected from the DNO supply
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— manufacturer DoC for integrated
              PEI; commissioning test in both modes; Reg 551.7.5 + Reg 551.6 + Reg 826.1.1.5
              compliance + DNO sign-off
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          RoCoF catches the frequency drift, voltage / frequency protection backs it up, and active
          probing catches the matched-load case. G99 disallows Vector Shift for the type-tested
          inverters used in virtually all LCT installs, so RoCoF is the standard required method —
          VS is legacy, on older or non-type-tested sites only.
        </Pullquote>
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">
                Vector shift (legacy — disallowed by G99 for type-tested generation)
              </strong>
              — measures phase angle of voltage waveform; sudden jump (typical threshold 6-12°)
              indicates grid disconnect. Fast (~50-100 ms); can nuisance-trip on large motor starts
              / nearby disturbances. G99 removed VS for type-tested generation in the 2018 amendment
              (Authority DC0079 decision); it is now only found on older or non-type-tested sites —
              use RoCoF for the type-tested inverters in virtually all LCT installs
            </li>
            <li>
              <strong className="text-white">
                ROCOF (Rate of Change of Frequency) — the standard required LoM method for
                type-tested generation under G99
              </strong>{' '}
              — measures df/dt; threshold typically 0.5-1.0 Hz/s. Frequency drifts post-disconnect
              because generation no longer matches load. Slightly slower (~100-200 ms); can
              nuisance-trip on grid frequency events. With voltage / frequency protection, this is
              what the type-tested inverters in virtually all LCT installs use in place of Vector
              Shift
            </li>
            <li>
              <strong className="text-white">Voltage / frequency deviation</strong> — measured
              against Reg 551.7.4 declared values. UK grid 230 V ±10%, 50 Hz ±0.5 Hz typical.
              Inverter trips on sustained deviation beyond limits. Slower (1-5 s typical) but
              categorical
            </li>
            <li>
              <strong className="text-white">Active anti-islanding</strong>— inverter injects small
              disturbances (Sandia Frequency Shift, Slip-Mode Frequency Shift, reactive power
              perturbation) + watches for grid response. Grid present absorbs; grid absent allows
              disturbance to amplify → inverter detects + trips. Reliable in matched-load edge cases
            </li>
            <li>
              <strong className="text-white">Combined approach</strong>— modern inverters use
              multiple methods in parallel; trip on whichever indicates loss first
            </li>
            <li>
              <strong className="text-white">UK 2025-26 DNO settings</strong> — post-2019 GB Power
              System Event, ENA + DNOs tightened settings to reduce false trips (especially ROCOF)
              while maintaining safety. Manufacturer + DNO settings documented per G99 connection
              offer
            </li>
            <li>
              <strong className="text-white">Three-phase generators</strong> — same principle per
              phase or aggregated; three-phase imbalance during grid loss is additional signal
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— LoM methods used + DNO-mandated
              settings + manufacturer DoC + commissioning test result
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">G98 threshold</strong>— generation ≤16 A per phase. At
              230 V single-phase ≈ 3.7 kW continuous; at 400 V three-phase ≈ 11 kW continuous
            </li>
            <li>
              <strong className="text-white">G98 process</strong>— manufacturer ships G98-certified
              inverter with factory test record; installer installs + tests; installer notifies DNO
              within 14 days; no DNO pre-approval needed
            </li>
            <li>
              <strong className="text-white">G99 threshold</strong>— generation &gt;16 A per phase
              OR synchronous generator OR inverter not on G98 type-test list
            </li>
            <li>
              <strong className="text-white">G99 process</strong>— installer / customer submits
              pre-installation application to DNO; DNO assesses network capability + issues
              connection offer (typical 6-18 weeks); installer accepts + installs per offer;
              commissioning test (DNO-witnessed or accepted equivalent); DNO issues commissioning
              certificate
            </li>
            <li>
              <strong className="text-white">G99 connection offer contents</strong> — DNO-mandated
              anti-islanding settings; generator parameters; protection relay requirements (for sync
              generators); export limit conditions; commissioning test requirements; cost recovery
            </li>
            <li>
              <strong className="text-white">G99 sub-categories</strong>— A1 (≤50 kW per phase), A2
              (50-1 MW), B (&gt;1 MW) — different requirements + processes per category
            </li>
            <li>
              <strong className="text-white">Mixed sites</strong>— multi-source PEI typically all
              under one G99 application covering combined export. Each source\'s anti-islanding
              still individually compliant
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — G98: installer
              notification + manufacturer DoC + factory test + install test. G99: connection offer +
              DNO commissioning certificate + manufacturer DoC + protection relay settings +
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Pre-test agreement</strong>— installer + DNO confirm
              test date, method, witness scope, expected disconnection time per G99 settings,
              multiple-condition scope if applicable
            </li>
            <li>
              <strong className="text-white">Generator operating at design</strong> — generator
              connected + exporting at design current; PEI configuration matches commissioning
              state; loads operating normally
            </li>
            <li>
              <strong className="text-white">Test method 1 — physical disconnection</strong> — DNO
              operator opens supply switch (cut-out / fuse / supply switch). Generator output
              monitored; expected to disconnect within G99-specified time. For sync generators +
              commercial typical
            </li>
            <li>
              <strong className="text-white">Test method 2 — simulated disconnection</strong> — main
              switch in customer\'s supply panel opened; generator monitored; same expected
              behaviour. Often used for inverter-coupled sources where physical DNO disconnection is
              logistically harder
            </li>
            <li>
              <strong className="text-white">Test method 3 — manufacturer self-test</strong> —
              modern inverters have built-in anti-islanding self-test mode that injects a brief
              simulated grid loss + verifies the response internally. Result logged. DNO may accept
              as part of commissioning evidence
            </li>
            <li>
              <strong className="text-white">Verify disconnection time</strong> — measure with
              monitoring equipment or via inverter / relay logging. Typical G99 specifies &lt;1 s;
              modern inverters typically achieve 50-200 ms
            </li>
            <li>
              <strong className="text-white">Multiple conditions</strong>— DNO may require tests at
              different load levels, time of day, source configurations. Larger / commercial more
              common
            </li>
            <li>
              <strong className="text-white">DNO commissioning certificate</strong> — DNO issues
              certificate confirming test witness + sign-off; reference to G99 application +
              connection offer; final document for cert evidence bundle
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">G98 or G99 reference</strong>— the engineering
              recommendation under which the generator was installed; G98 notification or G99
              connection offer + acceptance
            </li>
            <li>
              <strong className="text-white">Manufacturer DoC</strong>— inverter / generator
              controller / protection relay declarations of conformity to relevant standards (BS EN
              50549 family for inverters; specific G99 product approval for protection relays)
            </li>
            <li>
              <strong className="text-white">Protection settings record</strong> — DNO-mandated G99
              settings (where applicable): undervoltage, overvoltage, underfrequency, overfrequency,
              vector shift, ROCOF + thresholds + operating times. Documented per G99 connection
              offer + verified at commissioning
            </li>
            <li>
              <strong className="text-white">Commissioning test record</strong> — date, method
              (physical / simulated / manufacturer self-test), result, disconnection time measured,
              witness signatory (DNO operator name + signature where applicable), reference to G99
              application
            </li>
            <li>
              <strong className="text-white">DNO commissioning certificate</strong> — DNO\'s formal
              commissioning sign-off for G99; G98 has post-installation notification record
            </li>
            <li>
              <strong className="text-white">PEI documentation</strong>— Chapter 82 PEI architecture
              diagram, changeover (Reg 551.6), island mode test record (Reg 826.1.1.5), single-line
              diagram showing source + DNO interface
            </li>
            <li>
              <strong className="text-white">Customer handover pack</strong> — anti-islanding
              overview for customer reference; what to do in grid loss event; reconnection sequence;
              emergency contacts
            </li>
            <li>
              <strong className="text-white">EICR + future inspection</strong> — verifier reviews
              cert evidence bundle; checks settings + functional test of LoM (often via inverter
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-11-section-6')}
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-11-section-8')}
