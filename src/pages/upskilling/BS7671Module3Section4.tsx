    ...
    <HubPage>
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'Reg 525.1 sets the duty: the voltage at fixed current-using equipment must stay above the lower limit demanded by its product standard. Appendix 4 Section 6.4 is the deemed-to-satisfy route — 3 % lighting, 5 % other uses, origin to terminals — and the mV/A/m method is the working tool.'
          }
        </div>
        ...
        >
          <p>
            ...
            installer-controlled component of that tolerance. Get it wrong and the symptoms show up
            not as a failed insulation test but as a mystified customer complaining that the kit at
            the end of the run will not start, runs warm, or trips erratically.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 525.201 covers the case where there is no relevant product standard: the voltage at
            the equipment shall not impair its safe functioning. Reg 525.202 then cross-references
            Appendix 4 Section 6.4 as a deemed-to-satisfy route: 3 % for lighting and 5 % for other
            uses on a public LV supply (typically 230 V single-phase or 400 V three-phase), measured
            from the origin of the installation to the terminals of the fixed current-using
            equipment. On installations supplied from a private LV source (e.g. a generator, a
            private transformer feeding the site), the figures relax to 6 % / 8 % — but Reg 525.1
            still rules and may demand tighter design where equipment product standards require it.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            In practice, a sensible designer-allocation on a typical sub-main feeder is no more than
            ~1-1.5 % on the sub-main, leaving 3.5-4 % headroom for the final circuit and its
        ...
        >
          <p>
            ...
            and the installation Reference Method (A through F). The result is in volts when mV/A/m
            is in millivolts per amp per metre, current is in amps and length in metres. Compare
            against the Reg 525.202 limit as a percentage of the nominal voltage (230 V
        ...
        >
          <p>
            ...
            <strong>Vd = (mVr × cosφ + mVx × sinφ) × Ib × L / 1000</strong> for single-phase, or the
            equivalent three-phase form. cosφ is the load power factor (named on the equipment data
            plate), and sinφ is √(1 − cos²φ). On a 5.5 kW motor at 0.85 lagging, cosφ = 0.85 and
            sinφ ≈ 0.527 — neither term dominates, both matter. The practical rule of thumb: for
            resistive heating loads, ignore reactance; for inductive motor and VSD loads on cables
            &gt;25 mm², do the proper power-factor correction or you risk over-sizing the conductor
            on the resistive estimate alone.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>1. Inputs.</strong> 4 mm² T&E (twin&earth, two cores plus CPC), single-phase 230
            V, 25 A design current Ib, 30 m run, installation Reference Method C (clipped direct to
            a non-conductive surface), copper conductor. <strong>2. Appendix 4 mV/A/m.</strong>{' '}
            Single-phase column, 4 mm² copper, Reference Method C: ≈ 11 mV/A/m.{' '}
            <strong>3. Voltage drop.</strong> Vd = (11 × 25 × 30) / 1000 = 8.25 V.{' '}
            <strong>4. Percentage.</strong> 8.25 / 230 × 100 = 3.59 %.{' '}
            ...
            limit), the design fails — 4 mm² is too small or the run is too long. The standard fix
            is to upsize to 6 mm² (mV/A/m ≈ 7.3) → Vd = (7.3 × 25 × 30) / 1000 = 5.475 V (2.38 %),
            which now passes the 3 % lighting limit. <strong>6. Sanity check.</strong> Confirm Iz of
            4 mm² T&E in Reference Method C after correction factors still &gt; In, and that Zs at
            the far point still meets Reg 411.4.4. Voltage drop is one of three coordinated checks,
        ...
        >
          <p>
            <strong>Worked example.</strong> 80 m sub-main, 25 mm² SWA copper, three-phase balanced
            80 A design current, Reference Method E (free air clipped). Appendix 4 three-phase
            column for 25 mm² copper SWA: mVz ≈ 1.55 mV/A/m at full load. Vd ≈ (1.55 × 80 × 80) /
            1000 = 9.92 V line-to-line. As a percentage of 400 V: 9.92 / 400 × 100 = 2.48 %. This is
            well within the 5 % limit for a sub-main feeding power loads, and leaves over 2.5 %
            budget for the downstream final circuits before they run out of headroom.
          </p>
          <p>
            ...
            shortcut is to design each affected single-phase final circuit separately against the 3
            % / 5 % limits and let the sub-main carry the largest of the three line currents — the
            Appendix 4 single-phase column then applies on a phase-by-phase basis. For sustained
            heavy unbalance, BS 7671 Appendix 4 directs the designer to recognise the neutral as a
            current-carrying conductor for sizing.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Design discipline for EV charging final circuits: use Ib = 32 A (or 40 A for an uprated
            charger) — full-rated, sustained — never derate the EV-charger current for voltage drop.
            Treat the run length as the as-built worst case (measured, not estimated from a plan).
            Apply the 5 % limit from origin to charger terminals, including any sub-main. Where the
            run exceeds ~25-30 m at 32 A, default to 10 mm² T&E or 6/10 mm² SWA depending on
            installation method; compute, do not assume. Special-location and Section 722 / Reg
            722.312.2.1 PEN-prohibition rules also apply but are out-of-scope for the voltage-drop
            sizing — see Module 4.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Copper resistance rises with temperature at roughly 0.4 % per °C. A 4 mm² T&E conductor
            sized for Iz = 32 A and operating at 25 A (78 % loaded) will sit well below the 70 °C
            reference — closer to 50-55 °C in typical Reference Method C — so the actual voltage
            drop is around 6-8 % less than the Appendix 4 figure suggests. Appendix 4 includes
            guidance on temperature correction: for a more accurate result on a circuit known to
            operate well below full load, the mV/A/m value can be adjusted downward by the ratio
            (230 + tp) / (230 + 70), where tp is the actual operating temperature. Most designers
            ignore the correction and accept the conservative result; it is mainly used when the
            design is otherwise unworkable and shaving margin is the only fix.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            treat the long leg as a feeder; a 16 mm² SWA at 60 m is often more cost-effective than a
            6 mm² T&E at the same length and gives far more headroom for future loads.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            the install, not after. On EV chargers, long sub-mains and any circuit where the run is
            over ~25-30 m at sustained high load, expect the check to bite — size the conductor for
            the voltage-drop check and the current-carrying check independently, then take the
            larger of the two. The best designs leave headroom on both — a circuit at 95 % of its Iz
            limit and 95 % of its Vd budget will survive one heat wave, one buried-cable derate or
            one minor route change; one at 60 % of both will survive most realistic surprises.
          </p>
        </ConceptBlock>
