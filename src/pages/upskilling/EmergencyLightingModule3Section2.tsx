...
const inlineChecks = [
  {
    ...
    options: [
      'Anywhere convenient to an existing wiring run.',
      'Each exit door intended for emergency use, near each escape stair so the steps receive direct light, near each change of direction, near each intersection of corridors, near each emergency exit sign, near each first-aid post, near each piece of firefighting equipment, and near each fire alarm manual call point. The list is non-discretionary; missing any one position is a design defect.',
      'Only at the start and end of the route.',
      'At every 3 m along the route.',
    ],
    correctIndex: 1,
    explanation:
      'BS 5266-1:2025 §5.5 enumerates the mandatory luminaire positions. The reasoning is direction-of-escape — at every decision point on the route (exit, stair, turn, intersection) the user must be able to see where to go next. Safety equipment positions (first-aid, firefighting, MCP) repeat the 5 lx vertical duty from BS EN 1838.',
  },
  {
    id: 'elm3-s2-toilet',
    ...
    options: [
      'Always.',
      'Toilets / WC compartments require emergency lighting where the floor area is greater than 8 m² OR the route from any point inside back to a defined escape route involves more than one turn / change of direction. Cubicles smaller than 8 m² with a single-turn straight path out are exempt.',
      'Never.',
      'Only in commercial premises.',
    ],
    correctIndex: 1,
    explanation:
      'BS 5266-1:2025 §5.6 — the 8 m² and multi-turn criteria. A single small cubicle with a straight path out can rely on borrowed light from the corridor when the door opens; a larger or multi-room facility cannot, because an occupant disoriented in darkness may not find the door. Apply the rule per cubicle / per WC, not per building.',
  },
  {
    id: 'elm3-s2-borrowed',
    question: 'What is "borrowed light" and what is its status under BS 5266-1:2025?',
    options: [
      'Daylight through windows — always permitted.',
      'Light from luminaires in an adjacent space, illuminating the space under design through a glazed wall or open doorway. Permissible under BS 5266-1:2016 in some cases. EXCLUDED in BS 5266-1:2025 — every space requiring emergency lighting must have its own dedicated luminaires; light from elsewhere does not count toward the duty.',
      'Light from emergency vehicles outside.',
      'Light from torches.',
    ],
    correctIndex: 1,
    explanation:
      'BS 5266-1:2025 removes the borrowed-light allowance. The reasoning: the donor space may have a different category, different switching, different integrity, or a closed door at the moment of incident. Each space requiring cover must have its own emergency luminaires verified independently. Existing installations using borrowed light are not retrospectively non-compliant but must be reviewed at the next renewal.',
  },
  {
    id: 'elm3-s2-lift',
    question: 'A passenger lift in an office building — what emergency lighting applies?',
    options: [
      'None — lifts are not part of the escape route.',
      'BS EN 81-20 emergency lighting INSIDE the car (illuminance and duration per the lift standard, typically 5 lx for at least 1 h with self-contained battery), AND BS 5266-1 escape lighting in the lift LOBBY on each floor and in the machine room. The lift car interior is governed by lifts standards; the surrounding spaces are governed by BS 5266.',
      '1 lx escape route inside the car.',
      '50 lx everywhere.',
    ],
    correctIndex: 1,
    explanation:
      'Lifts are dual-jurisdiction. Inside the car: BS EN 81-20 (and BS EN 81-72 for firefighters lifts). Outside the car (lobby, motor room): BS 5266-1. Designs frequently miss one or the other; the lobby on each floor is part of the escape route and gets full BS 5266 treatment.',
  },
];
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: [
      'To meet a luminaire-density target.',
      'Because direction-of-escape signage is meaningful only when the user can see the next leg of the route. At every change of direction, the user must be able to look toward the new direction and see both the path and the next directional sign — without a local luminaire, the next leg is dim and the user hesitates.',
      'For aesthetic balance.',
      'For wiring convenience.',
    ],
    correctAnswer: 1,
    explanation:
      'Decision-point lighting. The user makes a navigational choice at every turn; the choice requires visibility of the new direction. Missing the luminaire at a turn produces a "dark corner" that confuses occupants in smoke or stress.',
  },
  {
    id: 4,
    question: 'A toilet compartment is 6 m² with a single straight exit path. What does BS 5266-1:2025 require?',
    options: [
      'Full escape route lighting inside the compartment.',
      'No emergency lighting inside the compartment is required by BS 5266-1:2025 (below 8 m² and single-turn). Verify the corridor outside is lit to the escape-route minimum so an occupant exiting the compartment immediately enters lit space.',
      '5 lx vertical at the toilet.',
      '15 lx high-risk.',
    ],
    correctAnswer: 1,
    explanation:
      'BS 5266-1:2025 §5.6 — the 8 m² / single-turn exemption. The compartment is below the threshold AND the egress is a single straight line, so no internal emergency luminaire is mandated. The corridor outside still has the route duty.',
  },
  {
    id: 5,
    question: 'Borrowed light (illumination from an adjacent space through glazing or open doorway) under BS 5266-1:2025 is...?',
    options: [
      'Permitted to count toward the escape lighting duty in the receiving space.',
      'Excluded — every space requiring emergency lighting must have its own dedicated luminaires; light from an adjacent space does NOT count toward the duty. The 2025 revision removes the borrowed-light allowance that existed in the 2016 standard.',
      'Permitted only at night.',
      'Permitted only in retail premises.',
    ],
    correctAnswer: 1,
    explanation:
      'BS 5266-1:2025 closes the borrowed-light loophole. The donor space may have different switching, different integrity, or a closed door at the moment of incident. Each space stands on its own.',
  },
  {
    id: 6,
  ...
  },
  {
    ...
    options: [
      'At the top of the stair only.',
      'So that each individual tread receives direct light — typically a luminaire at the top and at the bottom of the flight, or one wall-mounted luminaire mid-flight whose throw covers all treads. The aim is to eliminate shadows on the tread surfaces, where shadow + smoke is the main fall risk.',
      'At the bottom of the stair only.',
      'Inside the handrail.',
    ],
    correctAnswer: 1,
    explanation:
      'Stairs are over-represented in fire-evacuation injuries because shadow from poor luminaire placement obscures the tread edge. BS 5266-1:2025 specifies luminaire placement to give direct light on every tread — multiple luminaires or carefully chosen single-luminaire optics achieve this.',
  },
  {
    id: 8,
    ...
    options: [
      'One in the middle.',
      'Near each exit door (×2) AND near the change of direction (×1) — minimum three positions, and additional luminaires as needed to meet 1 lx across the full width of the route at floor level (with permitted edge exclusions per BS EN 1838:2024). Spacing is set by the photometric calculation, not by a fixed metre interval.',
      'Three at 10 m intervals.',
      'One in each corner.',
    ],
    correctAnswer: 1,
    explanation:
      'The minimum positions are dictated by §5.5 (each exit + each change of direction); the minimum spacing between those positions is set by the photometric requirement (1 lx, full-width per 2024). Both rules apply — meet whichever is more onerous.',
  },
  {
    id: 9,
    question: 'A defined escape route passes through a 200 m² atrium 8 m wide. How does BS EN 1838:2024 apply?',
    options: [
      '1 lx on the centre line of a notional 1 m path through the atrium (the superseded 2013 wording).',
      '1 lx across the FULL 8 m WIDTH of the escape-route path through the atrium (because it is an open-area path), AND 0.5 lx anti-panic across the open atrium floor (because 200 m² ≥ 60 m²), AND 5 lx vertical at any safety equipment in the atrium. Three duties simultaneously.',
      '0.5 lx anti-panic only.',
      '15 lx high-risk only.',
    ],
    correctAnswer: 1,
    explanation:
      'Atria, lobbies and large reception spaces routinely combine all three duties. The 2024 full-width rule on the route plus the anti-panic on the open floor plus the vertical at safety devices.',
  },
  {
    id: 10,
    question: 'Direction-of-escape — what does it mean for luminaire placement?',
    options: [
      'Luminaires alternate sides of the corridor.',
      'At every point on the route the user must have an unobstructed line of sight to the next directional sign and to the next leg of the route, with the route surface lit. Luminaires are placed so the field of view in the direction of travel is illuminated; back-of-route only is non-compliant.',
      'Luminaires face the entrance.',
      'Luminaires are wall-mounted only.',
    ],
    correctAnswer: 1,
    explanation:
      'Direction-of-escape is the principle that the user must see WHERE TO GO NEXT, not just see the floor. Luminaire placement supports forward visibility — the surface ahead, the next sign, the next change of direction. A luminaire that lights only the segment behind the user is insufficient.',
  },
];
