...
const BS7671Module4Section5 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 5 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'When BS 7671 requires SPDs (Reg 443.4 — A4 deleted the old CRL risk-assessment method), how Types 1, 2 and 3 differ and where each goes (Reg 534.4.1.1), the Uc / Up / Uw coordination chain, and why connecting-conductor length is the difference between a working SPD and a defeated one.'
          }
        </div>
        ...
        >
          <p>
            ...
            human life; or (c) significant financial or data loss. Limb (b) was deleted by the BS
            7671:2018+A2:2022 Corrigendum (May 2023). For all other cases, protection shall still be
            ...
            consequential loss. Reg 443.4.2 separately requires consideration of overvoltages from
            equipment likely to produce switching surges (e.g. large inductive loads, capacitor
            banks). The previous Reg 443.5 risk-assessment route — and its supporting Annex A443 —
            were both deleted in A4:2026.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            an 8/20 of the same peak amplitude, because of the much longer tail — which is why Type
            1 SPDs (rated for 10/350) are physically larger and use different internal technology
            (typically spark gaps) than Type 2 (rated for 8/20, typically metal-oxide varistors /
            MOVs). The selection between Types is a question of the expected waveform, which depends
            on whether the structure has direct-strike risk.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            insulation coordination model. Equipment manufacturers declare the category on the data
            sheet. For an installation feeding a mix of equipment categories, the SPD design targets
            the LOWEST Uw on the supply — typically Category II for ordinary fixed equipment. Where
            Category I sensitive electronics are present (audio, telecoms, IT), additional Type 3
            close-protection at the equipment terminals brings the local protection level below 1.5
            kV.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            fundamental — the 10/350 µs (Type 1) carries roughly 25× the energy of an 8/20 µs (Type
            2) surge of the same peak current, because of the much longer tail. A Type 2 device
            exposed to a Type 1 event will fail; a Type 1 device handles both. Cost is roughly 3-5×
            for Type 1 vs Type 2, which is why Type 2 is the economic choice everywhere LPS /
            direct-strike risk doesn't apply.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            253 V. For TT and IT systems: higher (typically 1.55 × U₀ ≈ 358 V) because line-to-PE
            voltage during faults can be elevated. Pick Uc too low and the SPD clamps continuously
            on normal supply variations, ageing rapidly. Pick Uc too high and Up rises with it,
            reducing effective protection. Table 534.2 of BS 7671 tabulates the requirements.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The mathematics: V_lead = L × dI/dt. For a 1 µH lead and a typical lightning di/dt of 1
            kA/µs, V_lead = 1 µH × 1 kA/µs = 1 kV. On a 0.5 m lead that's already 500 V added to the
            SPD's nameplate Up. On a 1.5 m lead it's 1.5 kV — pushing the effective protection level
            above the BS 7671 ceiling of 2.5 kV for 230/400 V systems and defeating the device. Reg
            534 / GN3 are explicit: keep leads short and straight, no loops. Some manufacturers
            provide pre-formed busbar adapters that guarantee 0 mm of lead length — these are the
            gold standard for retrofits.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Lifespan depends on the surge environment. A rural site with frequent thunderstorms may
            see SPD end-of-life in 3-5 years; an urban site in a low-storm area may continue for
            15+. Manufacturers publish energy-handling specifications (kJ for Type 2 8/20 µs) that
            allow rough-life estimation if the local storm-day count is known. In practice the
            status indicator is the easier route — inspect and replace when red. Type 1 SPDs
        ...
        >
          <p>
            ...
            status panel. For commercial / industrial sites where SPD failure could go unnoticed for
            years, the remote indicator pays for itself the first time a degraded SPD is caught
            before the next major surge takes out the equipment it was supposed to protect.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            feeding downstream. A Type 2 SPD at the sub-board sees that 1.5 kV transient and clamps
            it to a lower Up (say 1.2 kV). A Type 3 close-protection SPD at the equipment sees the
            1.2 kV and clamps to 0.9 kV — well below the Category I Uw of 1.5 kV. Each layer reduces
            the residual; cumulative protection beats single-layer for sensitive loads.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            (typically a thermal fuse plus an indicator) — others rely on external upstream fuses or
            MCBs sized per the data sheet. The OCPD rating is bounded above by what the SPD can
            ...
            Type 2 SPDs are sized against a 32 A or 40 A MCB upstream; Type 1 origin units typically
            need 100 A gG fuses.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For every standard install (consumer unit at origin, no LPS), the answers typically line
            up: (1) Yes — Reg 443.4.1 consequence test usually triggered. (2) Type 2. (3) Uc ≥ 275 V
            for TN-S / TN-C-S; ≥ 358 V for TT. (4) In ≥ 20 kA (typical commercial product). (5) Up ≤
            1.5 kV with 0.5 m leads keeps effective well under 2.5 kV ceiling. (6) Mount adjacent to
            main switch, leads under 0.5 m. Most domestic installs reduce to picking a Type 2 SPD
            from the manufacturer's catalogue with In ≥ 20 kA / Up ≤ 1.5 kV / appropriate Uc, and
            following the wiring rules.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For commercial installs, the conversation is often about business continuity: what's the
            cost of a server / EPOS / refrigeration system being out of action for two days while
            replacement parts are sourced? £200 of SPD against a £20,000 business-interruption claim
            is an easy economic argument. Insurers increasingly recognise SPDs in their underwriting
            — some policies discount the premium for installations with documented SPD coverage,
            others exclude surge claims unless SPDs are present.
          </p>
        </ConceptBlock>
