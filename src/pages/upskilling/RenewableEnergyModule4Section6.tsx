...
const inlineChecks = [
  {
    id: 'm4s6-what-is-eps',
    question: 'Emergency Power Supply (EPS) — what does it do in a hybrid PV+BESS install?',
    options: [
  ...
  },
  {
    ...
    question: 'Hybrid PV+BESS with EPS — protected vs non-protected loads. Why partition?',
    options: [
  ...
  },
  {
    ...
    question: 'EPS implementation — how does Reg 826.1.1.2.2 neutral switching work in practice?',
    options: [
  ...
  {
    id: 'm4s6-rcd-island',
    question: 'RCD operation in EPS island mode — does it work the same as direct feeding mode?',
    options: [
  ...
  },
  {
    ...
    question: 'EPS commissioning — what tests does the installer run before customer handover?',
    options: [
  ...
  {
    id: 'm4s6-firmware',
    question: "Hybrid inverter firmware updates — what's the installer / customer discipline?",
    options: [
...
const quizQuestions = [
  ...
  {
    id: 6,
    question: 'How does the EPS protected-load partition affect the EREC G98 / G99 application?',
    options: [
  ...
  {
    id: 8,
    question: 'How does EPS interact with EV charging in a PEI?',
    options: [
...
export default function RenewableEnergyModule4Section6() {
  ...
  useSEO({
    title: 'Hybrid inverter & EPS (Emergency Power Supply) | Renewable Energy 4.6 | Elec-Mate',
    description:
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 6 · BS 7671:2018+A4:2026 · Chapter 82"
        title="Hybrid inverter & EPS (Emergency Power Supply)"
        backTo="../renewable-energy-module-4"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          EPS is the functional implementation of Chapter 82 island mode — hybrid inverter switches
          from grid-following to grid-forming on DNO loss; protected-load partition; Reg 826.1.1.2.2
          neutral switching; commissioning + firmware discipline.
        </p>
        ...
        <Pullquote>
          EPS = Chapter 82 island mode in hardware. Grid-following ↔ grid-forming via Reg
          826.1.1.2.2 neutral switching.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">EPS contactor (or Backup Gateway)</strong> — typically
              4-pole (or 2-pole single-phase) switch that disconnects ALL conductors from DNO during
              EPS mode. Modern inverter brands integrate this; some products (Tesla Backup Gateway)
              are separate units
            </li>
            <li>
              <strong className="text-white">Hybrid inverter EPS-output stage</strong> — separate AC
              output from the grid-tied output; supplies the protected-load partition; can switch
              from grid-following to grid-forming mode
            </li>
            <li>
              <strong className="text-white">Protected-load partition</strong> — subset of property
              circuits wired through the EPS output. Selected at design stage based on outage
              priorities + EPS capacity
            </li>
            <li>
              <strong className="text-white">Detection logic</strong> — inverter monitors DNO
              V/freq; trips on excursion per BS EN 50549-1; transitions to grid-forming
            </li>
            <li>
              <strong className="text-white">Sync-back logic</strong> — when DNO returns, inverter
              monitors restored V/freq; syncs in software; EPS contactor recloses; returns to direct
              feeding
            </li>
            <li>
              <strong className="text-white">Local N-E bonding</strong> — per Reg 826.1.1.2.2;
              provides RCD reference in island mode
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Grid-following = anti-islanding. Grid-forming = island. EPS switches between them.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">GRID-FOLLOWING (anti-islanding)</strong> — inverter
              MEASURES DNO V and freq; tracks and synchronises; outputs power to grid + load
              matching the DNO reference. Trips off-grid if DNO V/freq exceeds the BS EN 50549-1
              anti-islanding window. EREC G98 / G99 compliance. The default mode for direct feeding
            </li>
            <li>
              <strong className="text-white">GRID-FORMING (island)</strong> — inverter GENERATES V
              and freq itself; sets the local reference; supplies protected loads. No external grid
              reference. Local N-E bonding provides RCD reference. The EPS mode for island operation
            </li>
            <li>
              <strong className="text-white">Mode transition: GRID-LOSS</strong> — inverter detects
              DNO excursion (V/freq outside BS EN 50549-1 window); EPS contactor opens; inverter
              switches to grid-forming mode; protected loads now on EPS. Switchover: 20-200 ms
              typical (modern fast EPS &lt; 100 ms)
            </li>
            <li>
              <strong className="text-white">Mode transition: GRID-RESTORATION</strong> — inverter
              detects DNO V/freq returned to valid range; waits for stability (typical 60-300 s
              grid-stability delay per EREC G99 / BS EN 50549-1); syncs grid-forming output to DNO
              V/freq via internal phase-locked loop; EPS contactor recloses; inverter switches back
              to grid-following mode
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          EPS capacity limits the partition. Choose essential loads. Lighting + fridge + sockets +
          comms = ~1 kW typical UK.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">PROTECTED (typical UK 5 kW EPS)</strong> — lighting
              circuit (~0.2 kW typical use); fridge + freezer (~0.3-0.5 kW); essential socket
              circuit per main room (~0.3-0.5 kW intermittent); comms (router / modem / IT,
              ~0.05-0.1 kW); heating controls + boiler (~0.1-0.3 kW). Total typical 0.8-1.5 kW
              continuous use; peak ~2-3 kW during fridge compressor cycle
            </li>
            <li>
              <strong className="text-white">NOT PROTECTED</strong> — electric shower (8-10 kW);
              oven (3-4 kW); cooker hob (5-7 kW); kettle (3 kW); EV charger (3.6-22 kW); heat pump
              compressor (3-5 kW); tumble dryer (3 kW); washing machine (2 kW). Customer accepts
              these off during outage
            </li>
            <li>
              <strong className="text-white">CONDITIONAL</strong> — heat pump may be in EPS
              partition for its CONTROLS but not COMPRESSOR (heating off but boiler controls stay
              live for safe transition); EV charger may be on EPS with smart-charging logic that
              limits to EPS capacity
            </li>
            <li>
              <strong className="text-white">Customer input</strong> — partition reflects customer
              priorities. Some want maximum backup duration (minimal partition); some want maximum
              functionality (larger partition, shorter duration). Honest survey-stage decision
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Split-load CU</strong> — modify existing CU; split the
              bus bar into two sections; EPS-side and DNO-side. Each section has its own RCBOs.
              Common for new-build with fresh CU
            </li>
            <li>
              <strong className="text-white">Dedicated EPS sub-board</strong> — install a separate
              small distribution board near the hybrid inverter; protected circuits feed from this;
              main CU stays for everything else. Cleaner for retrofit (don\'t modify the working CU)
            </li>
            <li>
              <strong className="text-white">Reg 551.7.1(d) compliance</strong> — both architectures
              must respect the A4:2026 rule that source connection isn\'t on the load side of an RCD
              protecting other parts of the installation
            </li>
            <li>
              <strong className="text-white">RCBOs throughout</strong> — protected circuits have
              their own RCBOs (sized per the circuit). Operate in both direct feeding mode and EPS
              island mode
            </li>
            <li>
              <strong className="text-white">Labelling per Reg 712.514.x</strong> — additional
              labels on the EPS-side circuits indicating that they\'re supplied from EPS in island
              mode + remain energised after DNO isolation
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          4-pole EPS contactor. Local N-E bonding before DNO N disconnected. RCDs work in island
          mode.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Direct feeding mode (normal)</strong> — DNO neutral is
              the reference (TN-C-S PME typical UK); local install bonded to DNO N via main earthing
              terminal
            </li>
            <li>
              <strong className="text-white">DNO loss detected</strong> — inverter detects V/freq
              excursion per BS EN 50549-1 anti-islanding window
            </li>
            <li>
              <strong className="text-white">EPS contactor opens (typically 20-100 ms)</strong> —
              all 4 conductors (L1+L2+L3+N for 3-phase, L+N for single-phase) disconnected from DNO
              simultaneously
            </li>
            <li>
              <strong className="text-white">Inverter EPS-output stage activates</strong> —
              generates local V and freq; creates local neutral via inverter output stage
            </li>
            <li>
              <strong className="text-white">Local N-E bonding via neutral switch device</strong> —
              per Reg 826.1.1.2.2; inverter\'s internal N-E bonding closes before DNO N disconnected
              (avoiding overlap that could cause RCD misoperation)
            </li>
            <li>
              <strong className="text-white">Protected loads supplied</strong> — from the inverter
              EPS output, with local N-E reference for RCD operation
            </li>
            <li>
              <strong className="text-white">DNO restored</strong> — inverter detects valid DNO
              V/freq; waits stability period (60-300 s per BS EN 50549-1); syncs grid-forming output
              to DNO; EPS contactor recloses; transitions back to direct feeding mode. Reverse
              sequence on neutral switching
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          DNO-loss test. DNO-restoration test. RCD under island mode. Record firmware version.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Test 1 — DNO-loss simulation</strong> — open the DNO
              main isolator while the inverter is active and producing. Verify: (a) EPS contactor
              opens within manufacturer spec time (20-200 ms typical); (b) inverter transitions to
              grid-forming mode; (c) protected loads continue operating; (d) non-protected loads
              lose supply. Measure actual switchover time
            </li>
            <li>
              <strong className="text-white">Test 2 — DNO-restoration simulation</strong> — reclose
              the DNO main isolator while EPS active. Verify: (a) inverter detects DNO V/freq
              return; (b) sync stability delay (60-300 s typical per BS EN 50549-1); (c) EPS
              contactor recloses; (d) install transitions back to direct feeding; (e) all loads now
              supplied
            </li>
            <li>
              <strong className="text-white">Test 3 — Protected-load verification</strong> — with
              EPS active, verify ONLY the partitioned circuits receive power. Walk through each
              room; check the protected vs non-protected mapping matches the design pack
            </li>
            <li>
              <strong className="text-white">Test 4 — RCD operation under island mode</strong> —
              with EPS active, simulate L-E fault on a protected circuit via RCD tester. Verify RCBO
              trips within Reg 411 disconnection-time limits (typically ≤0.4 s for final circuits
              ≤32 A). Record the trip time
            </li>
            <li>
              <strong className="text-white">Test 5 — Firmware version recording</strong> — read
              current firmware version from the inverter app; record in the cert evidence bundle
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Manufacturer cadence</strong> — typical update
              frequency: GivEnergy monthly; Tesla quarterly; Sigenergy quarterly; SolarEdge
              bi-annual; varies by brand
            </li>
            <li>
              <strong className="text-white">Update mechanism</strong> — OTA via manufacturer cloud;
              OR via installer / customer app push; OR via USB/SD card for specific firmware levels
            </li>
            <li>
              <strong className="text-white">Customer auto-update policy</strong> — most
              manufacturers default to auto-update; customer can opt out via app
            </li>
            <li>
              <strong className="text-white">Update categories</strong> — security fixes (typically
              silent); bug fixes (minor behaviour changes); feature additions (new modes, new BMS
              support); regulatory compliance refinements (BS EN 50549-1 anti-islanding tightening)
            </li>
            <li>
              <strong className="text-white">Major-update verification</strong> — significant
              updates affecting EPS or anti-islanding may warrant re-running commissioning Test 1 +
              Test 2 + Test 4 to verify behaviour still meets design
            </li>
            <li>
              <strong className="text-white">EICR-style periodic inspection</strong> — verifies
              current firmware version + behaviour; updates the cert evidence bundle
