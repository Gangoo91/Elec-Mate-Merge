...
const inlineChecks = [
  {
    ...
    question: 'What does PSCC at the customer side now include in a multi-source PEI?',
    options: [
  ...
  },
  {
    ...
    question: 'How do multiple sources affect disconnection time per Reg 411.3.2?',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'How does fault contribution affect the DNO PSCC declaration at the supply position?',
    options: [
...
export default function RenewableEnergyModule11Section6() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 11 · Section 6 · BS 7671 Reg 826.1.2.1 + Chapter 41 · Multi-source fault contribution"
        title="Fault contribution from multi-source sites"
        backTo="../renewable-energy-module-11"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Short-circuit + earth-fault contribution from PV / BESS / wind / CHP. Inverter-limited vs
          synchronous generator. Reg 826.1.2.1 PEI overcurrent assessment for all configurations.
          Disconnection-time per Reg 411.3.2 + Reg 826.1.1.3 minimum fault current. DNO PSCC +
          customer-side contributions.
        </p>
        ...
        <Pullquote>
          DNO PSCC sets the breaking capacity floor. Inverter + BESS + generator add to it.
          Synchronous machines change the picture. Per Reg 826.1.2.1, every device, every
          configuration.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Electronics limit</strong>— IGBT switching + current
              sense + control loop. Inverter detects overcurrent within microseconds; trips internal
              protection within 50-100 ms (varies by manufacturer)
            </li>
            <li>
              <strong className="text-white">Typical Isc</strong>— 1.0-1.2× rated AC current. For 5
              kW inverter ~25 A nominal AC, Isc ~25-30 A brief
            </li>
            <li>
              <strong className="text-white">Duration</strong>— brief; inverter drops off within
              ~50-100 ms. Not sustained fault contribution
            </li>
            <li>
              <strong className="text-white">DC side</strong>— PV DC string Isc limited by PV cell
              characteristics (~1.1-1.2× STC short-circuit current); BESS DC Isc limited by BESS BMS
              + internal protection. DC SPDs + DC fuses + DC isolators per Section 712 / Chapter 57
            </li>
            <li>
              <strong className="text-white">Grid-forming inverters</strong> — some BESS + advanced
              PV inverters in grid-forming mode deliberately provide higher fault current to support
              grid stability. Manufacturer DoC declares per product
            </li>
            <li>
              <strong className="text-white">Anti-islanding interaction</strong> — inverter
              anti-islanding (Reg 551.7.5) detects grid loss + disconnects. During a fault, the
              inverter may interpret the disturbance as anti-islanding trigger + disconnect, further
              limiting its fault contribution duration
            </li>
            <li>
              <strong className="text-white">Practical impact</strong>— for typical UK residential
              LCT, inverter contributions sum to &lt; 100 A vs DNO 16 kA; OCPD selection drives off
              DNO PSCC; inverter contribution adds to disconnection-time calculation but rarely
              changes OCPD sizing
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— per-inverter Isc from
              manufacturer DoC + Reg 826.1.2.1 assessment per protective device
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Sub-transient (first ~3-5 cycles)</strong> — ~6-8×
              rated current; very high; driven by armature flux + damper winding response. Critical
              for OCPD breaking capacity sizing
            </li>
            <li>
              <strong className="text-white">Transient (~10-20 cycles)</strong> — ~3-4× rated; field
              winding response; field excitation begins to react
            </li>
            <li>
              <strong className="text-white">Steady-state</strong>— depends on excitation control.
              Voltage regulator + automatic voltage regulator (AVR) response. Typically ~1-3× rated
              until protection clears
            </li>
            <li>
              <strong className="text-white">Asynchronous (induction) generators</strong> — limited
              sub-transient (~3-5× rated for first cycle); field collapses quickly; not significant
              for sustained fault current
            </li>
            <li>
              <strong className="text-white">UK 2025-26 typical scope</strong> — commercial CHP
              (10-500 kW) with synchronous generator; some larger commercial wind; older /
              traditional designs
            </li>
            <li>
              <strong className="text-white">OCPD impact</strong>— Icn (rated short-circuit breaking
              capacity) must include sync generator sub-transient contribution + DNO contribution.
              Can push requirement from typical 16 kA to 25-50 kA or higher
            </li>
            <li>
              <strong className="text-white">Disconnection time</strong>— high sustained fault
              current actually helps Reg 411.3.2 (faster trip on time-current curve). Difference
              from inverter where fast drop-off limits OCPD trip-on-source-contribution
            </li>
            <li>
              <strong className="text-white">Manufacturer DoC</strong>— declares Xd" (sub-transient
              reactance), Xd\' (transient), Xd (steady-state) — designer uses to calculate per-phase
              fault current + ΔIpscc contribution
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— per-generator fault
              contribution characteristic + Reg 826.1.2.1 + Reg 434 breaking capacity +
              commissioning
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          In a PEI, current doesn\'t know which way it should flow. The OCPD must trip either way.
          Reg 826.1.2.2 says: design for both directions.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Bidirectional capability</strong>— most MCBs / RCBOs /
              MCCBs designed for current flow in either direction; manufacturer DoC declares Icn
              (rated short-circuit breaking capacity) symmetric in both directions
            </li>
            <li>
              <strong className="text-white">Unidirectional products</strong> — some older /
              specific OCPDs have "line / load" preferred direction with reduced Icn in reverse.
              Check manufacturer DoC before specifying in PEI
            </li>
            <li>
              <strong className="text-white">Reg 551.7.2.1 source placement</strong> — generating
              set on supply side of ALL protective devices (covered in M9 §1, M5 BESS). Ensures
              OCPDs see source\'s fault contribution + downstream fault current path correctly
            </li>
            <li>
              <strong className="text-white">Reg 551.7.1 prohibition</strong> — source must NOT be
              connected on load side of certain RCDs (the RCD would not see the source\'s fault
              contribution correctly)
            </li>
            <li>
              <strong className="text-white">DC OCPDs</strong>— PV DC isolators, BESS DC contactors,
              etc. DC OCPDs differ from AC (no natural zero crossing); manufacturer DoC declares DC
              waveform handling + DC switching capability + DC current rating
            </li>
            <li>
              <strong className="text-white">Time-current curve selection</strong> — Type B (3-5× In
              trip), Type C (5-10× In), Type D (10-20× In) — choice depends on load characteristic
              (e.g. LCT inverters tolerate Type B; motors / transformers may need C/D for inrush).
              Devices for protection against overcurrent are selected per Section 533; time-current
              curves per Appendix 3
            </li>
            <li>
              <strong className="text-white">RCBO vs separate MCB+RCD</strong> — RCBO combines
              overcurrent + RCD in one device; independent operation per pole. PEI installs often
              use RCBOs for per-circuit Type A or Type B (Reg 531.3.3) RCD coverage
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— OCPD manufacturer DoC + Reg
              826.1.2.2 bidirectional capability + Reg 551.7.2.1 source placement + single-line
              diagram
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Discrimination principle</strong> — upstream OCPD
              time-current curve clears later than downstream OCPD for any fault current the
              downstream might see. Reg 531.3.6 + Appendix 3 time-current curves
            </li>
            <li>
              <strong className="text-white">Source combination effect</strong> — each combination
              changes fault current at each point. PV adds to fault during day; BESS adds during day
              + night; synchronous CHP adds substantially when running
            </li>
            <li>
              <strong className="text-white">Per-combination verification</strong> — designer
              models: DNO only; DNO + PV; DNO + PV + BESS; BESS only (island mode); DNO + PV + BESS
              + CHP (where applicable). Verify discrimination time / current at each
            </li>
            <li>
              <strong className="text-white">Cascade vs full selectivity</strong> — full selectivity
              (discrimination at all fault levels) vs cascade (acceptable upstream trip at high
              fault levels). Designer choice + cert evidence
            </li>
            <li>
              <strong className="text-white">Network analysis tools</strong> — software supports
              per-source-configuration fault analysis + discrimination verification (e.g. AmTech,
              Trimble, ETAP)
            </li>
            <li>
              <strong className="text-white">Manual approach for simple PEI</strong> — for typical
              UK residential 3-source PEI, manual per-combination check is practical
            </li>
            <li>
              <strong className="text-white">Commissioning verification</strong> — fault simulation
              at commissioning where practical; OCPD operation verified per source combination + as
              close to live conditions as safety allows
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — per-configuration
              coordination report + OCPD time-current curves + manufacturer DoCs + commissioning
              sign-off + Reg 826.1.2.3 compliance
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Reg 411.3.2 times</strong>— 0.4 s for final circuits
              ≤32 A in TN (5 s in TT); 5 s for distribution circuits + final &gt;32 A. Fault current
              must ≥ I_a for OCPD to clear within time
            </li>
            <li>
              <strong className="text-white">Multi-source min scenario</strong> — fewest sources
              operating + longest impedance path back to active source = lowest fault current. Worst
              case for disconnection time
            </li>
            <li>
              <strong className="text-white">Island mode</strong>— BESS-only supply per Reg
              826.1.1.1; DNO disconnected; only the BESS inverter is the source. Inverter-limited
              Isc ~30-50 A. Fault current at a distant circuit may be &lt; I_a for an MCB → slow
              trip → Reg 411.3.2 breach
            </li>
            <li>
              <strong className="text-white">Daytime night comparison</strong> — day: DNO + PV +
              BESS all contributing, fault current high. Night: DNO only (PV off, BESS may be
              discharging or off depending on state), fault current lower. Both scenarios must meet
              Reg 411.3.2
            </li>
            <li>
              <strong className="text-white">Supplementary RCD (Reg 411.4 + 415.1)</strong> — 30 mA
              Type A or Type B RCD provides additional earth-fault protection independent of fault
              current magnitude. RCD trips on ΔI = 30 mA regardless of source. Mitigates
              low-fault-current disconnection-time risk
            </li>
            <li>
              <strong className="text-white">Faster OCPD curve</strong>— Type B MCBs (3-5× In trip)
              require lower fault current to trip than Type C (5-10× In). Where Reg 411.3.2
              marginal, Type B may achieve required time
            </li>
            <li>
              <strong className="text-white">Cable size intervention</strong> — larger cable CSA
              reduces impedance, raising fault current at the load end. Coordinates with Chapter 81
              cable upsize design
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — per-configuration min
              fault current calculation + Reg 411.3.2 + Reg 826.1.1.3 compliance + design
              intervention if required + commissioning verification
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Island mode trigger</strong>— Reg 826.1.3: when public
              network is not energised, prosumer operates PEI in island mode OR automatically
              disconnects all local power supplies. Switching device per Reg 826.1.1.5 + Reg 551.6
              changeover
            </li>
            <li>
              <strong className="text-white">Source in island</strong>— BESS inverter (typically);
              some grid-forming BESS designs; manufacturer DoC declares island-mode Isc + duration
            </li>
            <li>
              <strong className="text-white">Fault current challenge</strong> — Isc ~30-50 A. For a
              32 A Type B MCB (I_a ~3-5× In = 96-160 A for 0.4 s trip), this is below threshold. MCB
              may eventually trip on thermal magnetic but not in 0.4 s
            </li>
            <li>
              <strong className="text-white">Earth-fault path in island</strong> — Reg 826.1.1.2
              system earthing for island mode shall be connected to a suitable earth electrode per
              Reg 542.2.2. Reg 826.1.1.2.2 neutral conductor — all live conductors disconnected from
              DNO in island; neutral switch device for connection
            </li>
            <li>
              <strong className="text-white">Supplementary RCD solution</strong> — 30 mA Type A or
              Type B RCD trips on ΔI = 30 mA regardless of fault current magnitude. Independent of
              source Isc. Most reliable Reg 411.3.2 + Reg 415.1 mitigation in island mode
            </li>
            <li>
              <strong className="text-white">Earth electrode in island</strong> — Reg 826.1.1.2 +
              Reg 826.1.1.2.3 transfer switching device for connection to local earthing
              arrangement. Coordinated with BS 7671 Chapter 54
            </li>
            <li>
              <strong className="text-white">RCD type for bidirectional inverter sources</strong> —
              Reg 531.3.3: Type B where smooth DC fault current possible (BESS inverter electronics
              may produce smooth DC residual current under certain fault conditions). Manufacturer
              DoC declares
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — island mode design
              records + fault current under island scenario + RCD coverage per Reg 411.4 + 415.1 +
              531.3.3 + commissioning test in island mode
            </li>
          </ul>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-11-section-5')}
