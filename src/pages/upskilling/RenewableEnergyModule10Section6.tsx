...
const inlineChecks = [
  {
    ...
    question: 'What does it mean for an inverter to be "grid-following"?',
    options: [
  ...
  },
  {
    ...
    question: 'What does it mean for an inverter to be "grid-forming"?',
    options: [
  ...
  },
  {
    ...
    question: 'What does a UK 2025-26 PEI need to be island-capable?',
    options: [
  ...
  },
  {
    ...
    question: 'What is "virtual inertia" + why does it matter for grid-forming inverters?',
    options: [
...
export default function RenewableEnergyModule10Section6() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 10 · Section 6 · BS 7671:2018+A4:2026 · Chapter 82 island-mode"
        title="Grid-forming vs grid-following inverters"
        backTo="../renewable-energy-module-10"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The architectural choice that determines whether a PEI can island. Grid-following =
          synchronises to external reference (UK 2025-26 default for almost all residential PV /
          BESS / V2G). Grid-forming = sets voltage + frequency itself (required for island mode +
          microgrids). The Chapter 82 framework for both modes: Reg 826.1.1.2.2 N-E switching, Reg
          826.1.1.5 island-mode switching device. Virtual inertia. UK 2025-26 vendor backup
          gateways.
        </p>
        ...
        <Pullquote>
          Grid-following synchronises to a reference. Grid-forming IS the reference. The difference
          is the difference between an inverter that quits when the grid goes dark and one that
          keeps the lights on.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Control principle</strong>— phase-locked loop (PLL)
              tracks grid voltage + frequency. Inverter injects current at same phase + frequency.
              Current-source behaviour
            </li>
            <li>
              <strong className="text-white">Reference dependency</strong> — must have an external
              voltage reference to operate. Without it (DNO loss), inverter cannot synchronise +
              must disconnect per Reg 551.7.5
            </li>
            <li>
              <strong className="text-white">Reactive power</strong>— typically grid-following
              inverters operate at near-unity power factor by default; reactive power control by EMS
              / ENA G99 directive
            </li>
            <li>
              <strong className="text-white">Grid services</strong>— limited capability: voltage /
              frequency support via reactive injection + power curtailment; cannot provide
              black-start or synchronous-inertia services
            </li>
            <li>
              <strong className="text-white">UK 2025-26 default</strong>— 95%+ residential PV / BESS
              / V2G inverters grid-following. SolarEdge, Solis, Huawei, Growatt, Fronius, Enphase IQ
              (microinverters), Wallbox Quasar
            </li>
            <li>
              <strong className="text-white">ENA approval</strong>— type-tested per ENA G98 (≤16 A
              per phase) or G99 (larger); BS EN 50549 grid-following inverter standard; IEC 62116
              anti-islanding test
            </li>
            <li>
              <strong className="text-white">Cost</strong> — £500-1,500 for typical 3-5 kW
              residential PV inverter; £2,000-5,000 for hybrid PV + BESS (still grid-following in
              direct-feeding mode)
            </li>
            <li>
              <strong className="text-white">Reg 551.7.5 anti-islanding</strong> — verified by
              built-in loss-of-mains detection (RoCoF + voltage / frequency window — G99 disallows
              Vector Shift for type-tested generation, so RoCoF is the required method on the
              type-tested inverters used in virtually all LCT installs; Vector Shift is legacy /
              non-type-tested only). Disconnects inverter from grid within typical 200-500ms on DNO
              loss
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— product DoC declares
              grid-following + ENA type approval + anti-islanding method + commissioning record
              (simulated grid-loss test verifying disconnect within ENA specified time)
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Control principle</strong>— voltage-source behaviour.
              Inverter HOLDS V + f at setpoints (230 V ±10%, 50 Hz ±1%). Droop control + virtual
              inertia + reactive power management
            </li>
            <li>
              <strong className="text-white">No reference dependency</strong> — doesn’t need
              external reference; IS the reference. Can operate in standalone island or as master in
              microgrid
            </li>
            <li>
              <strong className="text-white">Droop control</strong>— V droops with reactive current
              load (Q–V droop); f droops with real power load (P–f droop). Allows multiple
              grid-forming inverters to share load proportionally
            </li>
            <li>
              <strong className="text-white">Virtual inertia</strong>— algorithm emulates mechanical
              kinetic energy of synchronous generator. Provides frequency-response during
              disturbances. Important for grid-scale stability + microgrid stability
            </li>
            <li>
              <strong className="text-white">Reactive power</strong>— broader range than
              grid-following; provides reactive support + voltage regulation in island
            </li>
            <li>
              <strong className="text-white">UK 2025-26 residential hardware</strong> — Tesla
              Powerwall + Backup Gateway 2; SolarEdge StorEdge + Backup Interface; GivEnergy AIO +
              Whole Home Backup; Enphase IQ8 + IQ System Controller; Sonnen
            </li>
            <li>
              <strong className="text-white">Cost uplift</strong>— £2,000-5,000 over equivalent
              grid-following-only BESS. Backup gateway hardware + commissioning of island operation
            </li>
            <li>
              <strong className="text-white">UK 2025-26 commercial / utility</strong> — grid-forming
              BESS at 1 MW+ scale increasingly mandated by National Grid ESO + DNOs for frequency
              response + virtual inertia services. Pillswood 100 MW BESS (Yorkshire, Habitat) is an
              example
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— product DoC declares
              grid-forming + virtual inertia capability + droop characteristics + commissioning of
              island-mode operation + Reg 826.1.1.2.2 N-E switching verification
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          The transition from grid-connected to island is the most safety-critical sequence in any
          PEI. Three contactors, one bond change, non-overlapping timing — the backup gateway
          hardware enforces this; the commissioning test verifies it.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">T=0: DNO loss detected</strong> — backup gateway
              detects voltage drop / frequency excursion / vector shift / ROCOF outside threshold.
              Detection within 50-100ms
            </li>
            <li>
              <strong className="text-white">T+0-50ms: DNO line disconnects</strong> — backup
              gateway opens DNO line contactor(s). Single-phase or 3-phase as applicable. PEI now
              electrically detached from DNO line(s)
            </li>
            <li>
              <strong className="text-white">T+50-100ms: DNO neutral disconnects</strong> — backup
              gateway opens DNO neutral contactor. PEI now has NO N-E bond. RCDs cannot operate in
              this state — must be brief. Critical safety window
            </li>
            <li>
              <strong className="text-white">T+100-150ms: Local N-E bond establishes</strong> —
              backup gateway closes local neutral switch device that bonds PEI neutral bar to PEI PE
              bar / local earth electrode. PEI now has its own N-E reference. RCDs operational again
            </li>
            <li>
              <strong className="text-white">Reg 826.1.1.2.2 non-overlap</strong> — the gap between
              steps (DNO N off, local N-E on) prevents BOTH bonds being simultaneously in circuit.
              If overlap occurred, circulating earth current + RCD false-trip + risk of unequal
              earth potential
            </li>
            <li>
              <strong className="text-white">T+200-500ms: Grid-forming activates</strong> — BESS
              inverter receives command from backup gateway, transitions from grid-following to
              grid-forming mode. Sets V + f for the island. PV + V2G inverters resynchronise to BESS
              within 1-2 seconds
            </li>
            <li>
              <strong className="text-white">Reverse transition (DNO restored)</strong> — backup
              gateway detects stable DNO supply for 30-60s minimum; signals BESS to match V + f
              phase to DNO; opens local N-E bond; closes DNO neutral; closes DNO line; BESS resumes
              grid-following. Synchronisation check at each step
            </li>
            <li>
              <strong className="text-white">Commissioning test</strong> — simulated grid-loss (open
              DNO main switch); measure transition time end-to-end; measure N-E continuity in
              direct-feeding state (DNO bond), brief no-bond window, island state (local bond);
              measure island-mode voltage / frequency stability under typical load; reverse
              transition + DNO resynchronisation
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Direct-feeding PSCC</strong>— UK 2025-26 typical 6-25
              kA at LV consumer position. DNO contribution dominates. Breaking capacity check + Reg
              434.5.1 conditions verified
            </li>
            <li>
              <strong className="text-white">Island PSCC</strong>— only inverter contribution. 5 kW
              BESS → ≈25 A peak; 7 kW BESS → ≈35 A peak. Dramatically lower than direct-feeding
            </li>
            <li>
              <strong className="text-white">MCB trip behaviour in island</strong> — a 10 A Type B
              MCB needs 30-50 A to trip in 0.1s; available 25-35 A marginal. A 32 A circuit
              downstream may not trip in time. Reg 411 ADS may not be achievable via overcurrent
              alone
            </li>
            <li>
              <strong className="text-white">RCD as primary ADS</strong> — Reg 415.1 30 mA
              additional protection becomes the primary ADS path in island. RCDs operate via the
              local N-E bond per Reg 826.1.1.2.2; trip time 40ms at 5× In on residual current
            </li>
            <li>
              <strong className="text-white">Inverter self- protection</strong> — grid-forming BESS
              inverter detects persistent fault (overcurrent, undervoltage) + self-disconnects.
              Backup ADS layer
            </li>
            <li>
              <strong className="text-white">Reg 551.4.2 effectiveness</strong> — RCD effectiveness
              across all source combinations including island. Commissioning verifies RCD operation
              in island via controlled test
            </li>
            <li>
              <strong className="text-white">Cert evidence</strong>— PEI overcurrent study +
              per-circuit island-mode minimum-fault analysis + RCD effectiveness in island
              commissioning + customer handover noting island operating-mode protective
              characteristics
            </li>
          </ul>
        </ConceptBlock>
