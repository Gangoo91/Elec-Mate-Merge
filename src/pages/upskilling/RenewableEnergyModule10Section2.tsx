...
const inlineChecks = [
  ...
  },
  {
    ...
    question: 'Default priority logic in a UK 2025-26 domestic PV + BESS + EV PEI is typically:',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'Is the EMS itself in scope of BS 7671 verification?',
    options: [
  ...
  {
    question: 'What is "load shedding" in an EMS context and when is it needed?',
    options: [
  ...
  {
    question: 'How does Reg 311.1 (max demand) interact with EMS-managed PEI?',
    options: [
...
export default function RenewableEnergyModule10Section2() {
  ...
  useSEO({
    title:
      'Energy Management Systems (EMS) for multi-source PEI | Renewable Energy 10.2 | Elec-Mate',
    description:
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 10 · Section 2 · BS 7671:2018+A4:2026 · Chapter 82 EEMS"
        title="Energy Management Systems (EMS) — multi-source coordination"
        backTo="../renewable-energy-module-10"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The coordination layer of a multi-source PEI. BS 7671 Chapter 82 recognises the EEMS
          (Electrical Energy Management System) as part of PEI scope. Priority logic across sources
          + loads. Modbus, OCPP, MQTT protocols. Vendor EMS (SolarEdge / Tesla / GivEnergy / Enphase
          / Sonnen) vs third-party (Home Assistant / openHAB / Node-RED). Tariff arbitrage, load
          shedding, DNO ANM.
        </p>
        ...
        <Pullquote>
          The EMS isn’t a magic optimisation black box. It’s a deterministic priority engine plus a
          tariff schedule plus a comms stack. Understand the priority logic + the protocols + the
          degraded-mode behaviour, and the EMS becomes designable + commissionable + handover-ready.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Source priority</strong>— which source supplies the
              local load right now. Default: PV → BESS → grid (self-consumption optimised)
            </li>
            <li>
              <strong className="text-white">Load priority</strong> — if supply limited (island,
              capped connection), which loads run. Default: critical (fridge, lighting, comms)
              first; non-critical (EV, heat pump, immersion) deferred
            </li>
            <li>
              <strong className="text-white">Surplus management</strong>— PV surplus options: charge
              BESS first; export via SEG; divert to immersion heater (Eddi / iBoost — M4); charge
              EV; run heat pump
            </li>
            <li>
              <strong className="text-white">Tariff arbitrage</strong>— ToU schedule (charge BESS
              off-peak / discharge peak); dynamic tariff (follow Agile half-hourly prices)
            </li>
            <li>
              <strong className="text-white">Export limiting</strong>— G100-compliant soft limiting
              (curtail inverter / divert to BESS / divert to immersion before hitting hard limit).
              Covered in §10.4
            </li>
            <li>
              <strong className="text-white">Island-mode coordination</strong>— EMS triggers
              grid-forming inverter mode + load shedding when DNO supply lost. Covered in §10.6
            </li>
            <li>
              <strong className="text-white">Demand response</strong>— emerging UK 2025-26 markets
              (National Grid ESO Demand Flexibility Service via Octopus Saving Sessions). EMS
              pre-warms / pre-cools or releases BESS during demand events
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — EMS architecture
              diagram + vendor / protocol list + commissioning configuration + degraded-mode test +
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">SolarEdge Smart Energy Management</strong> — covers PV
              + Energy Bank BESS + StorEdge + EV charger (SolarEdge V3). Modbus exposed for
              third-party. App: SolarEdge monitoring portal
            </li>
            <li>
              <strong className="text-white">Tesla Powerwall + Powerflow</strong> — Powerwall 2 / 3
              + Backup Gateway 2. Time-Based Control for ToU. Self-Powered / Time-Based /
              Backup-Only modes. App: Tesla mobile
            </li>
            <li>
              <strong className="text-white">GivEnergy</strong> — hybrid inverters + AC-coupled
              BESS. Eco / Timed Charge / Timed Discharge modes. GivTCP open-source library exposes
              Modbus for Home Assistant integration. App: GivEnergy mobile
            </li>
            <li>
              <strong className="text-white">Enphase Enlighten</strong>— IQ8 microinverters + IQ
              Battery 5P. Self-Consumption / Time of Use / Full Backup modes. IQ Gateway for
              control. App: Enphase mobile
            </li>
            <li>
              <strong className="text-white">Sonnen</strong> — Sonnen Eco / sonnenBatterie. Sonnen
              Manager EMS. Strong on community- grid services (Sonnen Community DE; limited UK
              presence)
            </li>
            <li>
              <strong className="text-white">Solis</strong> — S6 hybrid inverters. Time of Use Mode.
              Open Modbus exposure. SolisCloud monitoring + app
            </li>
            <li>
              <strong className="text-white">Huawei FusionSolar</strong>— SUN2000 + LUNA2000 BESS.
              FusionSolar app + cloud. UK reseller channels. Modbus exposed
            </li>
            <li>
              <strong className="text-white">myenergi</strong> — Zappi EV charger + Eddi immersion
              diverter + Libbi BESS. Designed for integrated UK 2025-26 PEI ecosystem. App: myenergi
              mobile. Open API
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          Multi-vendor PEI is the UK 2025-26 default. Customers buy PV from one company, BESS from
          another, EV charger from a third. The integration challenge is real — and the EMS protocol
          stack (Modbus + OCPP + MQTT) is how it gets solved.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Modbus RTU</strong> — RS-485 serial cabling (CAT5e or
              shielded twisted pair). Multi-drop bus, up to 32 devices, distance up to 1.2 km. Baud
              rate typically 9600-19200. Used for direct EMS ↔ inverter local
            </li>
            <li>
              <strong className="text-white">Modbus TCP</strong> — Modbus framing over Ethernet
              TCP/IP. LAN-based, no distance limit within network. Used for network-based EMS ↔
              inverter via switch / router
            </li>
            <li>
              <strong className="text-white">Register map</strong>— each vendor publishes its
              register addresses. E.g. SolarEdge Modbus map: register 40083 = PV instantaneous
              power, register 40258 = battery instantaneous power, register 40294 = StorEdge control
              mode
            </li>
            <li>
              <strong className="text-white">SunSpec</strong> — standardised Modbus register layout
              for solar industry. Subset covering common data points (PV power, voltage, current,
              energy). Cross-vendor portability for basic monitoring
            </li>
            <li>
              <strong className="text-white">Wiring</strong> — Reg 528 segregation: Modbus RTU is
              signal cabling, segregate from LV power per Reg 528.1. Modbus TCP runs on Ethernet —
              standard structured cabling rules
            </li>
            <li>
              <strong className="text-white">Security</strong> — Modbus has NO native authentication
              / encryption. UK 2025-26 best practice: keep Modbus TCP on segmented VLAN, not exposed
              to internet
            </li>
            <li>
              <strong className="text-white">Home Assistant integration</strong> — popular UK
              2025-26 third-party EMS approach: install Home Assistant + vendor-specific Modbus
              integration (HACS), read inverter + BESS state via Modbus, write control setpoints,
              build automations
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — Modbus wiring schematic
              + protocol + register map reference + commissioning test (verify EMS reads + writes
              correct registers)
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">OCPP 1.6J</strong> — JSON over WebSocket. Most UK
              2025-26 commercial + many residential chargers. Smart Charging Profile available
              (TxProfile, TxDefaultProfile)
            </li>
            <li>
              <strong className="text-white">OCPP 2.0.1</strong> — expanded smart charging, ISO
              15118 Plug-and-Charge integration, better security, device management. Emerging UK
              2025-26
            </li>
            <li>
              <strong className="text-white">Charger vendor examples</strong> — Easee, Wallbox, Pod
              Point, Zappi (myenergi), EO, Andersen, Ohme, ABB Terra, Schneider EVlink, Project EV
            </li>
            <li>
              <strong className="text-white">Backend platforms</strong>— Octopus Electroverse,
              Monta, AMPECO, Driivz, Has-to-be, ChargePoint, custom EMS (Home Assistant + OCPP
              integration)
            </li>
            <li>
              <strong className="text-white">Smart charging</strong>— OCPP commands the charger to
              follow a charging profile (current limit varying over time). EMS computes profile
              based on tariff + PV + BESS state, pushes to charger via OCPP
            </li>
            <li>
              <strong className="text-white">Authentication</strong>— OCPP 1.6 supports HTTP Basic
              Auth + TLS; OCPP 2.0.1 adds certificate-based mutual TLS. Cybersecurity considerations
              apply
            </li>
            <li>
              <strong className="text-white">V2G via OCPP 2.0.1 + ISO 15118-20</strong> —
              bidirectional charging session negotiation. UK 2025-26: limited V2G hardware
              deployment but protocol stack ready. Covered in §10.5
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — charger DoC noting OCPP
              version + commissioning record of backend connection + EMS integration verification
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Static ToU — Octopus Go</strong> — 00:30-04:30 at
              7-9p/kWh, peak 25-30p/kWh. Vendor EMS configured: charge BESS 00:30-04:30 to 100% →
              self-consume during day → discharge during 16:00-19:00 peak
            </li>
            <li>
              <strong className="text-white">Static ToU — Intelligent Octopus Go</strong> —
              cheap-rate window dynamically allocated by Octopus based on grid conditions; EV + BESS
              opt in. Vendor / Octopus app coordination
            </li>
            <li>
              <strong className="text-white">Dynamic — Octopus Agile</strong> — 48 half-hourly
              prices per day, published next day at ≈16:00. EMS pulls API + schedules BESS charge
              during cheapest half-hours + discharge during expensive. Third-party EMS typically
              (Home Assistant + ha-agile)
            </li>
            <li>
              <strong className="text-white">PredBat</strong> — open-source UK BESS arbitrage tool.
              Forecast solar + load + Agile prices → optimal BESS schedule. Home Assistant add-on
            </li>
            <li>
              <strong className="text-white">Vendor support</strong>— Tesla Time-Based Control /
              SolarEdge Time of Use / GivEnergy Eco + Timed Charge / Solis ToU Mode: static
              schedules. Few vendors support Agile natively; GivEnergy via GivTCP / OpenAPI +
              third-party tooling
            </li>
            <li>
              <strong className="text-white">Octopus Saving Sessions</strong> — demand response
              payments per kWh reduced during specified evening events (typical Nov-Mar). EMS
              pre-charges BESS + reduces import during event window
            </li>
            <li>
              <strong className="text-white">Economics</strong> — 10 kWh BESS × £0.20-0.23 arbitrage
              spread × 300 cycles = £600-690/year base; +£100-200 from Saving Sessions; +PV
              self-consumption synergy. Payback typically 7-10 years for BESS alone in UK 2025-26
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — ToU schedule
              configuration record + customer handover acknowledging operating mode + degraded-mode
              (no EMS) test result
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Island mode load shedding</strong> — PEI in island,
              BESS SoC dropping faster than expected. EMS sheds EV charge → heat pump →
              non-essential lighting in priority order. Critical loads protected
            </li>
            <li>
              <strong className="text-white">ANM-capped connections</strong> — DNO offers cheaper /
              faster connection at lower capacity (e.g. 80 A instead of 100 A) with EMS as
              enforcement constraint. Customer signs ANM contract. EMS prevents total import + load
              exceeding cap
            </li>
            <li>
              <strong className="text-white">Peak shaving (light commercial)</strong> — capacity
              charges + DUoS Red Band avoidance. EMS uses BESS + load shedding to stay below
              specified threshold during peak window
            </li>
            <li>
              <strong className="text-white">Priority sequence</strong>— documented per install.
              Typical: shed EV → heat pump → immersion heater → non-critical lighting →
              non-essential socket circuits → (preserve) critical sockets + fridge + freezer + comms
            </li>
            <li>
              <strong className="text-white">Hardware</strong> — EMS digital output → smart relay
              (Shelly, Sonoff) or DIN-rail contactor on circuit being shed. Reg 511 + Reg 530 + Reg
              132 + Reg 311.1 max demand reconciliation
            </li>
            <li>
              <strong className="text-white">Reg 311.1 reconciliation</strong> — cable + service
              sized for UNMANAGED worst-case max demand (what happens if EMS fails). EMS load
              shedding optimises normal operation but does NOT change Reg 311.1 basis. ANM
              connections are a documented exception with DNO contract
            </li>
            <li>
              <strong className="text-white">Degraded mode</strong>— if EMS fails, default
              behaviour: smart relays open (fail-safe load disconnected) OR closed (fail-safe load
              connected). Design choice; documented + tested at commissioning
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — load-shedding logic
              diagram + priority sequence + hardware list + Reg 311.1 max demand calculation (both
              managed + unmanaged) + commissioning test + ANM agreement if applicable
            </li>
          </ul>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-10-section-1')}
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-10-section-3')}
