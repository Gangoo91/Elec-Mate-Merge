...
const inlineChecks = [
  {
    ...
    question: 'Why is DC insulation resistance testing different from standard AC IR?',
    options: [
  ...
  },
  {
    ...
    question: 'Per Reg 643.3.2 + Table 64, what test voltage applies to PV string IR testing?',
    options: [
  ...
  {
    id: 'm12s2-712-imd',
    question: 'How does Reg 712.421.101 IMD verification fit into PV DC initial verification?',
    options: [
  ...
  {
    id: 'm12s2-bess-dc-iv',
    question: 'What is unique about BESS DC bus IR + IV at initial verification?',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'Why is irradiance / weather state important when recording PV string IR results?',
    options: [
  ...
  {
    question: 'Reg 643.3.3 specifies a 250 V DC re-test after equipment is connected. Why?',
    options: [
...
export default function RenewableEnergyModule12Section2() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 12 · Section 2 · BS 7671:2018+A4:2026 · Reg 643.3 + Table 64 + Reg 712.421.101"
        title="Insulation resistance + Initial Verification on DC circuits"
        backTo="../renewable-energy-module-12"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          DC IR is different from AC IR. PV strings per Section 712 with Reg 712.421.101 IMD + DC
          test voltage per Voc + Table 64. BESS DC bus per Chapter 57 + manufacturer commissioning.
          EV DC fast per Section 722 manufacturer-led commissioning. Micro-hydro DC interim bus per
          Section 551 + manufacturer DoC.
        </p>
        ...
        <Pullquote>
          DC isn\'t AC. The test voltage, the procedure, the sensitivity of the equipment —
          different at every stage. Read the manufacturer DoC + Table 64 before applying a tester to
          DC LCT equipment.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">SELV / PELV ≤50 V</strong> — test voltage 250 V DC,
              minimum IR 0.5 MΩ. LCT: rare — some low-voltage micro-installs (24 V battery for
              off-grid cabin)
            </li>
            <li>
              <strong className="text-white">Up to 500 V (LV AC + LV DC)</strong> — test voltage 500
              V DC, minimum IR 1 MΩ. LCT: BESS residential 48 V bus; PV strings with Voc ≤ 500 V;
              standard AC IV
            </li>
            <li>
              <strong className="text-white">Above 500 V</strong>— test voltage 1000 V DC, minimum
              IR 1 MΩ. LCT: PV strings with Voc 600-1000 V; high-voltage BESS commercial 400-800 V;
              high-voltage micro-hydro DC bus
            </li>
            <li>
              <strong className="text-white">Reg 643.3.3 re-test</strong> — 250 V DC between live +
              CPC after equipment connected. Verifies as-installed IR without risking equipment
              damage
            </li>
            <li>
              <strong className="text-white">Procedure</strong>— disconnect equipment likely to
              influence / be damaged → apply pre-connection IR at appropriate voltage → record per
              circuit → connect equipment → apply 250 V DC re-test → record
            </li>
            <li>
              <strong className="text-white">Polarity</strong>— IR tester typically auto-polarity;
              for DC circuits verify the test voltage is applied with the correct polarity per
              manufacturer DoC (some IMD-equipped inverters may interpret reverse-polarity test as
              fault)
            </li>
            <li>
              <strong className="text-white">Documentation</strong>— cert evidence bundle:
              per-circuit IR result + test voltage applied + connection state (equipment connected /
              disconnected) + tester serial + calibration date
            </li>
            <li>
              <strong className="text-white">Reg 643.3.3 nuance</strong>— "likely to influence"
              applies broadly to LCT: PV inverters have surge protection + capacitors + IMD; BESS
              BMS has electronics; EV charger has rectifier + protection. Default position:
              disconnect for pre-test
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Moisture</strong> — humid morning + damp panels show
              lower IR; dry midday higher. PV module surface, junction-box ingress, connector seals
              all moisture paths
            </li>
            <li>
              <strong className="text-white">Module temperature</strong> — cold modules show higher
              IR; hot lower. Polarisation effect in PV cell + cable insulation temperature-dependent
            </li>
            <li>
              <strong className="text-white">Irradiance</strong>— at low irradiance the string
              voltage is low + IR test in absence of operating voltage; at higher irradiance the
              array still produces voltage (which is why we cover or test at low light)
            </li>
            <li>
              <strong className="text-white">SoC (BESS)</strong>— BESS DC bus IR varies slightly
              with cell SoC + electrolyte condition. Manufacturer DoC specifies the SoC range for
              accurate IR test
            </li>
            <li>
              <strong className="text-white">Cable age + condition</strong> — over 10-20 years,
              cable insulation degrades; outdoor / buried cables faster than indoor. PEC
              (Potential-Induced Degradation) in PV cells lowers module IR over time
            </li>
            <li>
              <strong className="text-white">Documentation matters more than absolute value</strong>{' '}
              — record conditions every time + trend over EICR cycles. A reading of 5 MΩ on a new
              install in dry conditions is acceptable; the same reading in 5 years in similar
              conditions indicates degradation worth investigating
            </li>
            <li>
              <strong className="text-white">Threshold</strong>— Table 64 minimum 1 MΩ at the test
              voltage. Manufacturer DoCs often specify higher thresholds for healthy operation
              (often 10s of MΩ). Approaching 1 MΩ on new install = investigate
            </li>
            <li>
              <strong className="text-white">BMS-equivalent monitoring</strong> — modern PV
              inverters + BESS BMS continuously monitor insulation-equivalent metrics; the dedicated
              IR test at commissioning + EICR confirms BMS reporting
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          For PV the Voc rules; for BESS the BMS rules; for EV the manufacturer rules; for hydro the
          bespoke design rules. Read first, test second.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Pre-test preparation</strong> — cover array OR open
              string isolators OR test at low irradiance. Open inverter DC input. Wait for capacitor
              discharge per manufacturer
            </li>
            <li>
              <strong className="text-white">Test voltage selection</strong> — per string Voc at
              coldest expected ambient temperature. Voc 600 V at STC may rise to 660 V at -10°C —
              conservative use 1000 V test per Table 64 over-500 V band
            </li>
            <li>
              <strong className="text-white">IR test points</strong>— PV positive to CPC; PV
              negative to CPC; PV positive to PV negative (with PV connected through the IR tester).
              Record three readings per string
            </li>
            <li>
              <strong className="text-white">Reg 712.421.101 IMD</strong> — inspect IMD presence
              (typically integrated in inverter); trigger IMD self-test; record IMD make + model +
              BS EN 61557-8 DoC + trip threshold + self-test result
            </li>
            <li>
              <strong className="text-white">Reg 643.3.3 re-test</strong> — connect inverter, apply
              250 V DC between live + CPC, record. Confirms as-installed IR with inverter
              electronics in circuit
            </li>
            <li>
              <strong className="text-white">Functional verification</strong> — uncover /
              re-energise PV, start inverter, verify grid synchronisation, anti-islanding self-test,
              yield matches modelled output for irradiance
            </li>
            <li>
              <strong className="text-white">Documentation</strong>— Schedule of Test Results per
              string + IMD verification + weather + module temperature + irradiance + inverter
              commissioning report from manufacturer engineer
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — EIC + Schedule + MIS
              3002 PV handover + manufacturer inverter commissioning + EREC G98 / G99 reference +
              DNO confirmation
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">BMS commissioning (manufacturer)</strong> — cell
              balance, communications, fault tolerance, SoC + SoH baseline, alarm calibration,
              firmware up-to-date. Manufacturer engineer signs off
            </li>
            <li>
              <strong className="text-white">Reg 642 inspection (electrician)</strong> —
              disconnected from supply; DC + AC terminations torque-checked; polarity; isolators +
              warning notices; cable routing; cable selection per Section 522
            </li>
            <li>
              <strong className="text-white">Reg 643 AC IV</strong>— continuity (R1+R2, R2), IR at
              500 V per Table 64 ≤500 V band, polarity, loop impedance at the BESS isolator, RCD
              operation — Type B 30 mA verified per BS EN 61557-6 (BESS inverter typically produces
              DC fault leakage), PFC
            </li>
            <li>
              <strong className="text-white">DC bus IR (per manufacturer)</strong> — typically 500 V
              DC for 48 V residential bus per Table 64 ≤500 V band; commercial 400-800 V bus 1000 V
              test; manufacturer DoC mandates the procedure
            </li>
            <li>
              <strong className="text-white">Capacitor discharge wait</strong> — observe
              manufacturer-specified discharge time (seconds to minutes) before working DC side. Reg
              641.4 safety
            </li>
            <li>
              <strong className="text-white">Reg 551.7.5 anti-islanding</strong> — simulate grid
              loss; verify BESS inverter disconnects within specified time; DNO-witnessed where
              applicable
            </li>
            <li>
              <strong className="text-white">Functional verification</strong> — BESS charges from
              grid + discharges to load + PV-paired self-consumption; BMS communicates SoC + SoH;
              alarm paths verified
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — BMS commissioning
              record + EIC + Schedule of Test Results + Chapter 57 compliance + EREC notification +
              cert assembled
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Manufacturer engineer scope</strong> — DC side
              commissioning: handshake verification with vehicle test load, calibration, vehicle
              simulation tests, firmware updates, CMS / OCPP communication, manufacturer software
              access
            </li>
            <li>
              <strong className="text-white">Electrician AC supply IV</strong> — Reg 643 continuity
              + IR at 500 V + polarity + loop impedance + RCD operation (Type B + RDC-DD per Section
              722 manufacturer DoC) + PFC
            </li>
            <li>
              <strong className="text-white">Reg 722.411.4 PME architecture</strong> — outdoor EV:
              OPDD or alternative method per the manufacturer + Section 722. §12.4 covers PEN faults
            </li>
            <li>
              <strong className="text-white">DC connector physical install</strong> — DC cable
              routing, ingress protection, gland integrity, mechanical protection per Section 522
            </li>
            <li>
              <strong className="text-white">Earthing + protective conductor</strong> — TT or TN-C-S
              PME with appropriate architecture; protective conductor cross-section per Section 543;
              main equipotential bonding
            </li>
            <li>
              <strong className="text-white">Joint sign-off</strong>— manufacturer commissioning
              record + electrician EIC + Schedule of Test Results + Section 722 compliance + cert
              evidence bundle integrates
            </li>
            <li>
              <strong className="text-white">UK 2025-26 EV DC market</strong> — commercial /
              forecourt DC fast 50-350 kW (Tritium, Alpitronic, ABB, BTC Power); domestic AC
              charging 7-22 kW (Zappi, Ohme, Hypervolt, Easee, Pod Point) — covered M6-M7
            </li>
            <li>
              <strong className="text-white">Functional verification</strong> — vehicle test charge
              cycle + communication + stop-on-fault; DNO export check if V2G capability
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Manufacturer DoC governs</strong> — read first. Test
              procedure + test voltage + procedure + safety wait times all per manufacturer
            </li>
            <li>
              <strong className="text-white">Table 64 + Reg 643.3 framework</strong> — applies —
              test voltage per nominal DC voltage band; pre-connection IR + 250 V re-test per Reg
              643.3.3
            </li>
            <li>
              <strong className="text-white">Micro-hydro specifics</strong> — turbine + rectifier
              produces DC; DC bus to grid-tie inverter; voltage typically 24-300 V depending on
              system; outdoor + wet environment + buried cables — weather + ingress affect IR
              readings
            </li>
            <li>
              <strong className="text-white">Wind systems</strong>— many modern small wind systems
              are AC at turbine (induction or PMG) + inverter does AC-DC-AC conversion; some have DC
              bus between rectifier + inverter; verify per system architecture
            </li>
            <li>
              <strong className="text-white">Fuel cells</strong>— Section 551 emerging LCT; UK
              2025-26 mostly trials. DC bus from fuel cell stack + grid-tie inverter. Per
              manufacturer + Table 64
            </li>
            <li>
              <strong className="text-white">Hydrogen-blend CHP</strong> — electrical generation
              side similar to natural-gas CHP (engine + generator); DC bus rarely present in this
              generation type
            </li>
            <li>
              <strong className="text-white">Safety wait times</strong> — DC link capacitors retain
              charge after disconnect; manufacturer-specified discharge wait times (seconds to
              minutes) observed before working
            </li>
            <li>
              <strong className="text-white">Documentation</strong>— cert evidence bundle:
              manufacturer commissioning record + electrician EIC + Schedule of Test Results per
              bespoke DC bus procedure + Section 551 + Section 712 cross-references where applicable
            </li>
          </ul>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-12-section-1')}
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/renewable-energy-module-12-section-3')}
