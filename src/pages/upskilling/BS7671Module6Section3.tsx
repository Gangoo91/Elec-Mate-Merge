...
const BS7671Module6Section3 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 3 · Updated for A4:2026"
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'The Reg 643 dead and live test sequence — why the order is fixed, what each test proves, the instrument standard (BS EN 61557), and the new A4 entry for AFDD self-test verification on the schedule of test results.'
          }
        </div>
        ...
        >
          <p>
            ...
            missing CPC, undetected because IR was run first, leaves an exposed metal part floating
            during a Zs measurement. A reversed polarity, undetected, drives line voltage onto the
            wrong terminal of a downstream device. The sequence is the safety mechanism — running it
            out of order is procedurally non-compliant.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            conductors, main earthing conductor, main bonding, supplementary bonding, equipotential
            bonding to extraneous-conductive-parts. Reg 643.2.3 requires verification of ring
            final-circuit conductor continuity using the three-step end-to-end method: (a) measure
            r1, rn, r2 end-to-end; (b) cross-connect line and CPC at the origin; (c) measure (R1+R2)
            at each socket — the values should be approximately equal at every point on a healthy
            ring, with the highest reading being the mid-point.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 643.3.2 / Table 64 specifies test voltages and minimum values: SELV/PELV (≤ 50 V) →
            250 V DC, 0.5 MΩ. LV up to 500 V (covers all standard 230 V / 400 V installations) → 500
            V DC, 1 MΩ. Above 500 V → 1000 V DC, 1 MΩ. The IR test is run between (a) all live
            conductors connected together vs earth, and where practicable (b) between line and
            neutral. Where electronic equipment cannot tolerate the test voltage, Reg 643.3.3
            permits disconnection or testing live-to-live joined to earth so the equipment sees no
            voltage between its terminals during test.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            conductor) and the conductor identification rules of Section 514. The dead polarity test
            is usually performed by continuity from the origin: with line and CPC linked at the
            consumer unit, a continuity reading at each switch confirms it is in the line path; a
            continuity reading at each socket&apos;s line terminal vs origin confirms the line is on
            the correct pin. The live polarity test uses an approved voltage indicator at
            energisation to verify line is line and neutral is neutral on the actual supply.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 643.7.3 requires verification of Zs at the design stage and at the furthest point of
            every final circuit. The measured value must be compared to the maximum value at the
            conductor operating temperature — published Zs(max) tables (Appendix 3 of BS 7671 / OSG)
            are at 70 °C for thermoplastic. A cold measurement reads lower and must be
            temperature-corrected (multiply by ~1.20 for typical PVC/PVC) before comparing, or use a
            meter with temperature correction enabled. Reg 643.7.4 covers prospective fault current
            — Ipf measured at the origin, used to confirm the breaking capacity of every protective
            device downstream is adequate.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            sensitive). The values are recorded in the RCD column of the schedule of test results. A
            consistent approach: record the worse of the two readings under the "x1" and "x5"
        ...
        >
          <p>
            Reg 643.10 applies only to three-phase circuits. The verification is required at initial
            verification and recorded on the schedule of test results. Rotation assumptions matter
            most at three-phase motors, three-phase variable-frequency drives and three-phase
            distribution boards feeding rotation-sensitive loads (pumps, fans, compressors). A
            reversed-rotation pump can mechanically damage seals on first start; a reversed
            three-phase saw is a safety incident waiting to happen.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            BS EN 61557 has multiple parts: Part 1 general; Part 2 IR; Part 3 loop impedance; Part 4
            continuity of protective conductors; Part 6 RCDs; Part 7 phase sequence; Part 10
            multifunction. A multifunction tester (BS EN 61557-10 compliant) integrates the lot. The
            calibration certificate is your evidence in any cert challenge — a defective tester is
            no defence. GN3 is explicit: an out-of-calibration tester cannot be relied on as
            evidence of compliance; a certificate produced with one is open to challenge.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            accommodation) and certain higher-risk industrial / commercial circuits where series arc
            faults are a credible ignition source. Reg 643 brings the verification into the standard
            test sequence — manufacturer pre-installation testing alone is not enough; the device
            has to be verified IN THE INSTALLATION. The cert form changes mean the inspector now has
            a dedicated tick column rather than a free-text note.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            voltage indicator, and follow your isolation procedure if any retest needs dead access.
            Document any test that was not performed (e.g. PFC at every DB rather than the origin
            alone) and the reason — a complete cert is one where every box is ticked OR has a
            documented justification for being blank.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            line is on the right pin of every BS 1363 socket. <strong>5. Live polarity.</strong> AVI
            confirms supply rotation correct at the consumer unit.{' '}
            <strong>6. Zs (Reg 643.7.3).</strong> Direct loop measurement at the furthest socket
            reads 0.62 Ω (cold); temperature-corrected at ~1.20 = 0.74 Ω; published Zs(max) for B32
            at 70 °C is 1.37 Ω — comfortably within. <strong>7. RCD operation (Reg 643.8).</strong>{' '}
            30 mA RCBO at IΔn trips in 18 ms; at 5×IΔn (150 mA) trips in 11 ms. Both well within the
            ≤ 40 ms additional-protection limit. <strong>8. Functional checks (Reg 643.9).</strong>{' '}
            Switch the RCBO off and on; confirm indicator and downstream sockets de-energised under
            test. Sign the cert.
          </p>
        </ConceptBlock>
