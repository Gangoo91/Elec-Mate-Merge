    ...
    <HubPage>
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'BS 7671 mandates maximum-demand calculation as the basis for cable, OPD, supply and voltage-drop sizing. Diversity is the statistical tool for converting connected load to realistic peak demand. Get it right and the installation is appropriately sized; get it wrong and the supply or cables are inadequate (under-sized) or wasteful (over-sized).'
          }
        </div>
        ...
        >
          <p>
            ...
            cable sizing, OPD selection, voltage drop) all flows from the maximum-demand figure. Get
            it wrong — too high, you over-spec and waste money; too low, the installation can't cope
            and either trips repeatedly or overheats.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            year — same connected load, very different realistic peak demand. Document the diversity
            assumptions on the design sheet so the calculation is traceable. The OSG factors are
            reasonable for the AVERAGE UK domestic; departures are permitted with documented
            rationale.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            added. The design choices: (1) DNO supply upgrade to 100 A with smart-meter time-of-use
            management, (2) three-phase supply upgrade, (3) dynamic load-managing EV charger that
            curtails to keep within supply rating. All three are valid; the customer's preferences
            and the DNO's quotes determine the practical choice.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            not. Reg 722.311.201 (and its equivalent in other Section 7 sub-sections) lets you take
            automatic disconnection of loads into account in the maximum-demand calculation — but
            only where the disconnection is automatic and verifiable.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Reg 311.1 references the RMS values of current in lines L1, L2 and L3 (the input to the
            maximum-demand determination) plus the maximum neutral current Im (which becomes
            important on heavily-imbalanced installations or those with significant triplen-harmonic
            content from VSDs / LED drivers). Three-phase design discipline: tabulate every
            single-phase load, decide which phase each goes on, calculate the per-phase total,
            ensure no single phase exceeds the supply per-phase rating. Three-phase loads contribute
            equally to all three phases.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The design implication: three-phase domestic isn't just a power capacity upgrade, it's a
            different design template. Balance-of-loads becomes a primary design discipline.
            Three-phase RCBOs are needed where Reg 411.3.3 / 411.3.4 trigger. Three-phase cabling,
            busbars, isolators throughout. The design effort is greater; the resulting installation
            has significantly more headroom and future-proofing.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The discipline that the calculation forces: tabulate every load by phase, classify each
            by duty cycle (continuous, cycling, intermittent), apply realistic factors per category,
            identify the worst-case phase. The 11 kW lathe&apos;s 60 A per-phase inrush isn&apos;t
            in the maximum-demand calculation — that&apos;s a starting characteristic addressed by
            Type C / D MCB selection on the lathe&apos;s circuit, not by sizing the supply for
            inrush. Reg 311.1 is about steady-state demand. The EV charger pushing L3 to the binding
            phase is typical of modern light-industrial installs — if the boss adds a second EV next
            year, the supply needs upgrading or the EV chargers need positive load-balancing across
            phases (some commercial EV chargers do this automatically; documenting the feature on
            the design sheet is essential).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The cert entry for inrush-driven design is the protective-device characteristic (Type C
            / D rather than Type B) on the relevant final circuits, plus a design-sheet note about
            the starting current and the rationale for the chosen device curve. Voltage-drop
            verification at steady-state running current is the Reg 525.1 compliance figure; voltage
            drop at inrush is checked once and noted as being within limits for the start duration.
            Don&apos;t conflate the two — sizing cabling for inrush wastes copper; sizing devices
            for steady-state alone causes nuisance trips every time the lathe starts.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            factor: measured data where available, manufacturer figures where not, OSG / CIBSE / IET
            commercial guidance as the fallback. Note the source on the design sheet alongside the
            figure. Five years later, when the workshop has expanded and someone asks why the supply
            is now inadequate, the design record explains exactly what the original assumptions were
            and where the load growth has exceeded them.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            margin — it&apos;s an engineering judgement informed by the customer&apos;s likely load
            growth. Document the choice; revisit at the periodic inspection. Future expansion that
            would push demand past supply rating is the inspector&apos;s flag for a supply-upgrade
            conversation, not a non-compliance code in itself.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            rationale. Five years later when the inspector returns for the EICR, the recorded figure
            is the benchmark against which observed demand is measured — and the design sheet
            explains why the original assumptions were what they were.
          </p>
        </ConceptBlock>
