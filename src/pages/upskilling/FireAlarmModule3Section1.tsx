...
const inlineChecks = [
  {
    ...
    options: [
      'Up to 2,000 m², subject also to the 60 m search-distance limit (non-addressable).',
      'A maximum of 1,000 m² of floor area per zone, irrespective of the search distance involved.',
      'A maximum of 500 m² per zone, the smaller cap reflecting the time taken to locate a device.',
      'No floor-area limit at all, provided the building forms a single fire compartment.',
    ],
    correctIndex: 0,
    explanation:
      'BS 5839-1:2025 clause 13 retains the 2,000 m² zone size cap and the 60 m search distance rule, and designers must satisfy both. The search distance is the walk a person tracing the alarm makes to find the operated device; the smaller of the two limits governs. The 60 m rule exists to bound the time a fire warden / responder spends locating the device on a non-addressable system; addressable systems report exact device location and so the 60 m rule is relaxed.',
  },
  {
    id: 'fam3-s1-storey',
    question:
    ...
    options: [
      'It is always treated as a separate zone in its own right, whatever its area.',
      'At 60% (more than half), it counts as part of the storey it projects into.',
      'It is always part of the storey beneath, regardless of how large the gallery is.',
      'It is a separate zone only where the gallery is used for sleeping accommodation.',
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'To improve the audibility of the sounders fed from the loop.',
      'To save cable by allowing a single loop to serve more devices.',
      'To make the system detect a fire faster than a radial circuit would.',
      'To contain a short-circuit fault so it cannot remove protection from more than one zone.',
    ],
    correctIndex: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'An unacceptable variation — a non-conformity that cannot be agreed as a variation.',
      'An acceptable variation, provided the departure is recorded in the logbook.',
      'A minor variation that has little bearing on the life-safety function of the system.',
      'A matter for the responsible person alone to decide on a case-by-case basis.',
    ],
    correctIndex: 0,
    explanation:
...
const quizQuestions = [
  {
    ...
    options: [
      'Floor area only — a single 2,000 m² cap, with no search-distance constraint.',
      'Device count only — the number of detectors permitted on the zone circuit.',
      'Cable distance only — the run from the CIE to the furthest device in the zone.',
      'Floor area ≤ 2,000 m² AND search distance ≤ 60 m (non-addressable); smaller governs.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Each stairway is a separate zone in its own right, end to end.',
      'Each stairway is folded into the adjacent storey zone for design simplicity.',
      'Stairways need no zoning, since they contain no occupied floor area.',
      'All stairways in a building are grouped together as a single combined zone.',
    ],
    correctAnswer: 0,
    explanation:
      'Fire and smoke in a stairway must be identified independently of the storeys, because the stairway is the protected escape route. Mixing it with a storey zone obscures the location and undermines the evacuation strategy. Stairways are zoned separately so any indication at the CIE points specifically to the stairway — the principle "one zone per fire compartment, separate zone per escape route".',
  },
  {
    id: 3,
    question: 'Why must one zone correspond to one fire compartment, where practicable?',
    options: [
      'It is simply long-standing convention, with no particular engineering reason behind it.',
      'To keep the cable length within each zone as short as economically practicable.',
      'A zone straddling two compartments cannot tell which side of the barrier is on fire.',
      'To reduce the installed cost and device count of the detection system overall.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A single zone may cover the whole multi-storey building if under 2,000 m².',
      'Storeys may be grouped two per zone to reduce installed device count.',
      'There is no specific multi-storey zoning requirement set out in the standard.',
      'Each storey is a separate zone, plus a separate zone for each stairway.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A subdivision of the premises indicated separately from any other subdivision at the CIE.',
      'A defined floor area, measured in square metres, served by a group of detectors.',
      'A bay or group of detectors wired together onto a common detection circuit.',
      'A section of an addressable loop bounded at each end by short-circuit isolators.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Accept it as presented, since a single open-plan space is logically one zone.',
      'Reject it on the grounds that the manual call points are wrongly located.',
      'Reject it — the 2,000 m² limit is exceeded, so split it into at least two zones.',
      'Accept it as an agreed variation against the standard, recorded in the logbook.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Yes — the zone is comfortably under the 2,000 m² floor-area limit.',
      'Yes — addressable systems are exempt from the search-distance rule.',
      'It cannot be determined from the information given in the question.',
      'No — the 90 m extent means it fails the 60 m search-distance limit.',
    ],
    correctAnswer: 3,
    explanation:
      'The 60 m rule catches the long-narrow case the 2,000 m² rule alone misses. The 1,530 m² area is under 2,000 m², but the longest internal walking distance to the operated device exceeds 60 m on this non-addressable system. The fix is to split the zone (two halves, each ≤ 60 m) or upgrade to addressable so the device address resolves the location and the 60 m rule relaxes.',
  },
  {
    id: 8,
    question:
    ...
    options: [
      'Absence of a zone plan in multi-zone premises, AND absence of ARC transmission where needed.',
      'Use of a smaller standby battery, AND a non-standard colour for the system cabling.',
      'A different colour of detector head, AND a different colour of manual call point.',
      'A different type of cable, AND a different type of containment to run it in.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'In the building manager or facilities office, alongside the system records.',
      'At each manual call point on every floor throughout the building.',
      'At the Control and Indicating Equipment (CIE), per clause 22.2.5.',
      'At the main building entrance only, beside the principal access door.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'It eliminates the need for zones altogether on an addressable system.',
      'It removes the 2,000 m² maximum zone-area limit for addressable loops.',
      'It removes the rule that each storey must be its own separate zone.',
      'Zones remain, but the 60 m search-distance rule relaxes as each device is addressed.',
    ],
    correctAnswer: 3,
    explanation:
...
const FireAlarmModule3Section1 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead section="Module 3 · Section 1" title="Zone design principles" backTo=".." />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          BS 5839-1:2025 clause 13: the 2,000 m² area limit, the 60 m search-distance limit, one
          zone per fire compartment, separate zone per storey and per stairway, short-circuit
          isolation at zone boundaries, the new unacceptable-variation list, and the zone plan at
          the CIE.
        </p>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            <li>
              <strong>Subdivision of the protected premises</strong> — the zone is a portion of the
              building, identified physically.
            </li>
            <li>
          ...
          <p>
            Three things flow from that definition. First, zone size is bounded so the indication is
            precise enough to direct response (the 2,000 m² and 60 m rules). Second, zone boundaries
            align with the building&apos;s fire-engineering architecture (compartments, storeys,
            stairways). Third, the zone plan at the CIE is part of the zoning installation — the CIE
            without the plan delivers half the function.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              ...
              system separately from an indication of fire in any other subdivision. The total floor
              area of any one zone should not exceed 2,000 m², and the distance any person may have
              to travel within a zone to determine the location of a fire (i.e. to identify a fire
              detector or manual call point operated as a result of a fire) should not exceed 60 m,
              unless the system provides an indication at the CIE of the particular detector or
              manual call point operated.
            </>
          }
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            ...
            <li>
              ...
              brisk walk is roughly 45 seconds; that is the upper bound of acceptable delay between
              an indication at the CIE and a person reaching the operated device.
            </li>
            <li>
              <strong>Addressable relaxation</strong> — when the CIE shows &quot;Smoke detector 3.07
              in office 2.14, kitchen&quot;, there is no walk-to-find. The 60 m rule does not bite.
              The 2,000 m² area rule still applies because indication clarity at the
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-6 my-6">
          ...
          >
            <text x="440" y="24" textAnchor="middle" fill="#FBBF24" fontSize="14" fontWeight="bold">
            ...
            <text x="170" y="138" textAnchor="middle" fill="rgba(255,255,255,0.75)" fontSize="9.5">
              Office area
            </text>
            ...
            <text x="410" y="138" textAnchor="middle" fill="rgba(255,255,255,0.75)" fontSize="9.5">
              Workshop / store
            </text>
            ...
            <line
              ...
              strokeDasharray="4,3"
            />
            <text x="290" y="234" textAnchor="middle" fill="#EF4444" fontSize="9" fontWeight="bold">
              compartment line
            </text>
            ...
            <text x="290" y="338" textAnchor="middle" fill="rgba(255,255,255,0.75)" fontSize="9.5">
            ...
            <text x="610" y="142" textAnchor="middle" fill="rgba(255,255,255,0.75)" fontSize="9.5">
              Stairway
            </text>
            ...
            <text x="780" y="350" textAnchor="middle" fill="#EF4444" fontSize="9" fontWeight="bold">
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            ...
            <li>
              ...
              which side. They evacuate the affected compartment first; they fight the fire from the
              unaffected side.
            </li>
            <li>
              <strong>Zone-compartment alignment</strong> means the indication is meaningful in the
              building&apos;s own fire-engineering language. &quot;Zone 3 fire&quot; maps to
          ...
          <p>
            ...
            area / distance / storey / stairway rules. Where the building has many compartments, the
            zoning follows them.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            ...
            <li>
              <strong>Roof</strong> — counts as a storey for zoning purposes UNLESS accessible only
              for maintenance or repair (then excluded by the 2025 definition).
            </li>
          </ul>
          <p>
            The 50% gallery rule is a 2025 alignment and is now precise — earlier editions left the
            threshold to engineering judgement.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            ...
            <li>
              <strong>Fire-fighting</strong> uses the stairway as the bridgehead. They need to know
              if it is compromised.
            </li>
          </ul>
          <p>
            Multi-stair buildings have one zone per stair. A protected lobby on each landing may be
            in the storey zone or a separate sub-zone depending on the design and the evacuation
            strategy.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            <li>
              <strong>Loop architecture</strong> — devices in a ring, fed from the CIE both ends so
              a single break does not lose them. Each device has an address; each can also be an SCI
              in its own right (built-in isolation).
            </li>
            <li>
          ...
          <p>
            On a conventional radial system, the zone is the circuit; one circuit, one zone, no SCI
            needed (the radial topology already isolates faults to a single zone). On an addressable
            loop, the SCIs do the same job in software / hardware.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            <li>
              ...
              at-a-glance reference. Without it, a fire-fighter arriving at 02:00 reads &quot;Zone 3
              fire&quot; and cannot translate the indication into a physical location. Sleeping
          ...
          <p>
            Both entries fail life safety in a specific, foreseeable way. Both apply to populations
            that cannot self-evacuate to safety without external support. Both close loopholes the
            2017 standard left open.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              ...
              safety of life that they should not be regarded as acceptable variations: the absence
              of a zone plan (or other suitable diagrammatic representation as recommended in
              22.2.5) in premises in which there is more than one zone on any storey, particularly
              premises in which people sleep; and the absence of a facility for transmission of fire
              alarm signals to an ARC in either supported housing in which the facility is
              considered necessary to meet the recommendations of BS 5839-1 (where a Grade A system
              according to BS 5839-6:2019 is necessary), or a residential care home.
            </>
          }
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            ...
            <li>
              The fire risk assessor needs to see the full picture when reviewing the system against
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[14px]">
            ...
            <li>
              <strong>Stairways shown as separate zones</strong>, vertically connected on the plan.
            </li>
            <li>
            ...
            <li>
              <strong>Date and revision</strong> of the plan; dated to match the system as installed
              (updated on extensions and modifications).
            </li>
          </ul>
          <p>
            The plan is durable, legible, and protected against fire damage. It is mounted at or
            immediately adjacent to the CIE. It is not in a binder in a different room; it is not on
            a flip-chart that can be moved; it is not on a screen that can fail.
          </p>
        </ConceptBlock>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          <button
            type="button"
            onClick={() => navigate('/electrician/upskilling/fire-alarm-course/module-3/section-2')}
