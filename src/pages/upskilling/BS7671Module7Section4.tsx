...
const quizQuestions = [
  ...
  },
  {
    id: 2,
    question:
      'Reg 710.421.1.7 (introduced by Amendment 2:2022) addresses AFDDs in medical locations. What does it say?',
    options: [
  ...
  },
  {
    id: 5,
    question:
      'In Group 1 and Group 2 medical locations, Reg 710.419.3 modifies the safe voltage limit to:',
    options: [
    ...
    <HubPage>
      ...
      <HubBody>
        <div className="max-w-3xl text-[13px] leading-relaxed text-white">
          {
            'Section 710 medical locations (Group 0, 1, 2 classifications, IT systems, IMDs, supplementary bonding, AFDD prohibition), Section 729 operating gangways, and Section 740 temporary installations — the parts of BS 7671 where ordinary ADS architecture is replaced or supplemented to match the clinical, mechanical and access risks.'
          }
        </div>
        ...
        >
          <p>
            ...
            rather than optional. The classification is a joint engineering / clinical decision and
            must be documented per location on the medical-electrical risk register BEFORE the
            design proceeds.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            the medical staff position; (d) clear fault-management procedures so that a first-fault
            alarm triggers planned isolation, not panic disconnection. Disconnect on the first IMD
            alarm and you have defeated the entire architecture — it is the SECOND fault,
            simultaneous with the first, that drives a phase-to-phase fault and a real
            disconnection.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            the staff position; (c) be itself supplied so that loss of supply to the IMD also alarms
            (a failed IMD on a healthy IT bus is functionally identical to a healthy IMD on a
            failing IT bus — both must alarm); (d) include a test function for routine verification.
            The IMD is not an RCD — it does not disconnect, it does not measure residual current,
            and it does not provide fault protection by itself. Fault protection in the IT system
            comes from the combination of IMD (signalling first fault), supplementary equipotential
            bonding (limiting touch voltage), SELV at applied parts (limiting voltage at the patient
            interface) and OPD or RCD operation on the second fault.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            "low impedance"). The 0.2 Ω limit reflects the clinical reality of a cannulated patient:
            skin resistance is bypassed, and the body forms a direct conductive path from the saline
            catheter through internal organs including the heart. Cardiac fibrillation threshold for
            direct cardiac current is around 50 µA — orders of magnitude below the macro-shock
            threshold for skin-contact pathways. Limiting touch voltage between any two
            simultaneously-touchable items to a few millivolts requires the bonding-conductor
        ...
        >
          <p>
            ...
            trade-off inverts: an arc-fault on a non-life-critical socket can be managed; a nuisance
            trip on a life-critical socket during a procedure cannot. In a Group 0 location, the
            trade-off is also inverted but for a different reason: there are no applied parts at
            risk, but corridor and life-safety lighting circuits MUST stay energised — a nuisance
            trip in a hospital corridor at 3 am during a code blue is a patient-safety event
            regardless of how the corridor itself is classified. Section 422 (escape routes) and the
            Building Regulations Part B (fire safety) reinforce this from the fire-engineering side.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            must be able to step back from a closing breaker without colliding with a wall), manual
            handling (MCCBs and ACBs are heavy and require room to position withdrawable elements),
            and emergency egress (an injured operator must be able to exit the room without
            obstruction). The dimensions interact with CDM 2015 statutory duties on designers,
            principal designers and clients — a switchroom that fails Section 729 is not just a BS
            7671 deviation, it is a CDM design risk that the principal designer must resolve before
            sign-off. The 2.0 m headroom is also relevant for ladder-based cable-termination work at
            the top of motor control centres.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            person are defined in Part 2 (Definitions) and used throughout Part 4. A skilled person
            has the technical knowledge or experience to enable them to avoid dangers electricity
            may create. An instructed person has been adequately advised or supervised by a skilled
            person. An ordinary person is neither. Section 729 dimensional relaxations (0.7 m
            restricted gangway) only apply to areas accessed exclusively by skilled persons — and
            the access control must be appropriate to enforce that exclusivity.
            Ordinary-person-accessible spaces revert to the wider 1.0 m general operating gangway
            minimum.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Section 740 covers temporary installations for amusement, fair, circus, show, kiosk and
            similar uses. The fundamental design assumption is that the installation will be
            erected, used by ordinary persons including children for a short period, then dismantled
            and re-erected at the next venue — and the wear, mechanical damage and moisture ingress
            accumulated across multiple cycles dominate the risk profile. The 30 mA RCD is
            non-negotiable; the IP rating is dictated by the local external influences not the
            lowest the budget will allow; the TT verification (Ra × IΔn ≤ 50 V) must be done at
            every re-erection because the local soil moisture changes the earth-electrode
            resistance. The competent-person daily inspection is a Section 740 specific requirement
            and is documented on a daily inspection log alongside the ride-engineering safety log.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            available, and frequent inspection. The cost of this protection is materially higher per
            kVA than a domestic install — but the population at risk is also materially less
        ...
        >
          <p>
            Section 710 projects fail when the clinical, design and verification phases drift out of
            sync. The classification decided in design must hold throughout construction; the
            ...
            verification with LPS resistance at 0.18 Ω, then drifts to 0.45 Ω at year three because
            of corrosion at a connector, is a defect that only periodic re-verification with a
            calibrated micro-ohmmeter will catch — and only if the periodic-inspection schedule
            explicitly requires that test. Build the periodic-inspection schedule into the handover
            documentation at initial verification.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The classification is determined at the design stage by the medical / clinical authority
            responsible for the location, not by the electrical designer alone. Mis-classifying a
            Group 2 location as Group 1 (or vice versa) cascades into significant compliance
            failures — IT-system isolation transformer needed for Group 2, AFDD prohibition needed
            for Group 0 / 2, supplementary bonding bus needed for Group 2. Get the classification
            confirmed in writing and lock the design template to it.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            isolator at panel terminals; equipment beyond is covered by BS EN 60204-1 documentation
            supplied by the manufacturer.&quot; Document the boundary clearly; future inspectors
            thank you.
          </p>
        </ConceptBlock>
