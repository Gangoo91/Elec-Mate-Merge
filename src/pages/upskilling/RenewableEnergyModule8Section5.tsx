...
const inlineChecks = [
  {
    id: 'm8s5-rcd-types',
    question: 'What RCD types are recognised by BS 7671 + applicable to heat pump installs?',
    options: [
  ...
  },
  {
    ...
    question: 'What is "smooth DC fault current" in the context of VSD inverter compressors?',
    options: [
  ...
  {
    id: 'm8s5-type-b-when',
    question: 'When does a heat pump install need Type B RCD protection?',
    options: [
  ...
  {
    id: 'm8s5-type-f',
    question: 'What is Type F RCD + when does it suit a heat pump install?',
    options: [
...
const quizQuestions = [
  ...
  {
    question: 'Three-phase 16 kW ASHP, manufacturer Type A acceptable per DoC. RCD architecture?',
    options: [
  ...
  {
    question: 'Type AC RCD on a heat pump install — why never appropriate?',
    options: [
...
export default function RenewableEnergyModule8Section5() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 8 · Section 5 · BS 7671:2018+A4:2026 · Reg 415.1.1 + Reg 531.3.3 + BS EN 62423"
        title="RCD architecture for VSD inverter compressors"
        backTo="../renewable-energy-module-8"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The smooth-DC question reappears. Type AC / A / F / B per BS EN 62423; VSD inverter
          compressor smooth-DC fault current; manufacturer DoC declaration as the authoritative
          source; UK 2025-26 typical install patterns; RCD discrimination + three-phase 4-pole
          architecture.
        </p>
        ...
        <Pullquote>
          VSD inverter compressors brought heat pump efficiency to where it is today. They also
          brought the smooth-DC question back into the install conversation. The RCD type is not a
          default — it is a per-model design decision.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Rectifier stage</strong> — converts incoming 230 V AC
              (or 400 V three-phase AC) to DC bus voltage (~325 V single-phase peak or ~565 V
              three-phase peak). Uses diode bridge or PFC (Power Factor Correction) front-end
            </li>
            <li>
              <strong className="text-white">DC bus capacitors</strong> — smooth the rectified
              voltage. Electrolytic capacitors typically ~2,000 μF in domestic heat pumps. Smooth-DC
              characteristic in steady-state
            </li>
            <li>
              <strong className="text-white">Inverter stage</strong> — IGBT or MOSFET power
              electronics switch the DC bus at high frequency (typically 4-16 kHz) to synthesise
              variable-frequency AC at the compressor motor. PWM (Pulse Width Modulation) waveform
              creates the variable-frequency output
            </li>
            <li>
              <strong className="text-white">Compressor motor</strong> — permanent magnet or
              induction motor; runs at the synthesised variable frequency. Variable frequency =
              variable speed = variable heat output
            </li>
            <li>
              <strong className="text-white">Insulation fault to earth</strong> — can occur in any
              electrical part: rectifier bridge fault → pulsating DC leakage; DC bus capacitor fault
              → smooth DC leakage; inverter IGBT fault → high-frequency leakage; motor winding fault
              → AC leakage at variable frequency. Different fault locations produce different
              waveforms
            </li>
            <li>
              <strong className="text-white">RCD detection requirement</strong> — protective RCD
              must detect whichever waveform the manufacturer’s VSD design can produce under
              credible fault conditions
            </li>
            <li>
              <strong className="text-white">Modern integrated protection</strong> — UK 2025-26
              typical small-medium domestic ASHP includes integrated internal protection that limits
              the smooth-DC fault current to within Type A capability — manufacturer DoC declares
              Type A acceptable
            </li>
            <li>
              <strong className="text-white">Larger units + older models</strong> — may produce
              smooth-DC fault current exceeding Type A capability → Type B required per manufacturer
              DoC
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Type AC (legacy)</strong> — BS EN 61008 / 61009.
              Detects AC fault current only. Reg 531.3.3: "Only to be used for fixed equipment,
              where it is known that the load current contains no DC components." NEVER appropriate
              for VSD heat pump
            </li>
            <li>
              <strong className="text-white">Type A</strong> — BS EN 62423. Detects AC + pulsating
              DC (half-wave rectified). Tolerates smooth-DC leakage up to ~6 mA before saturation.
              UK 2025-26 default for heat pumps where manufacturer declares smooth-DC ≤6 mA
            </li>
            <li>
              <strong className="text-white">Type F</strong> — BS EN 62423. Detects AC + pulsating
              DC + composite waveforms + high-frequency (up to ~1 kHz). Develops for VSD inverter
              loads where high-frequency switching creates fault current harmonics. Less common in
              UK product range
            </li>
            <li>
              <strong className="text-white">Type B</strong> — BS EN 62423. Detects AC + pulsating
              DC + smooth DC + composite + high-frequency. Complete fault-current waveform coverage.
              Required where manufacturer DoC declares smooth-DC leakage &gt; Type A capability
            </li>
            <li>
              <strong className="text-white">UK 2025-26 product market</strong> — Type A widely
              available from all leading manufacturers (Hager, Schneider, Wylex, MK / Honeywell).
              Type B available but specialist sourcing. Type F limited product range
            </li>
            <li>
              <strong className="text-white">Cost differential</strong>— single-pole 32 A: Type A
              ~£40-80; Type F ~£70-130; Type B ~£150-300. 4-pole 32 A: Type A ~£200-400; Type B
              ~£600-1,200
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — RCD product + Type +
              curve + rating + manufacturer DoC + verification trip-time at 1× and 5× IΔn
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Pullquote>
          The manufacturer DoC is the authoritative answer to "which RCD type?" — not the
          installer’s default, not the wholesaler’s stock, not what was fitted on the last job.
        </Pullquote>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Install manual</strong> — electrical specification
              section (typically "Electrical Connection Requirements", "RCD Compatibility", "Earth
              Leakage", or similar heading)
            </li>
            <li>
              <strong className="text-white">Declaration of Conformity</strong> — issued under UKCA
              / CE marking; references applicable standards (BS EN 61851, BS EN 62423, etc.) +
              product test results
            </li>
            <li>
              <strong className="text-white">Technical bulletin</strong>— some manufacturers issue
              model-specific bulletins clarifying RCD compatibility. Check the manufacturer website
              / installer portal
            </li>
            <li>
              <strong className="text-white">Terminology to find</strong>— "RCD type required",
              "earth leakage current", "DC component of leakage current", "smooth DC fault current",
              "RCD compatibility", "leakage to earth"
            </li>
            <li>
              <strong className="text-white">Type A acceptable</strong>— most UK 2025-26
              small-medium domestic ASHP. The manufacturer has designed internal protection that
              keeps smooth-DC leakage below the Type A threshold (~6 mA)
            </li>
            <li>
              <strong className="text-white">Type B required</strong>— manufacturer declares
              smooth-DC leakage &gt; Type A threshold. Typically larger commercial units, older
              models, or specialist applications
            </li>
            <li>
              <strong className="text-white">Type F suitable</strong>— manufacturer specifically
              declares Type F or "Type A with high-frequency capability". Less common
            </li>
            <li>
              <strong className="text-white">If unclear or absent</strong> — contact manufacturer
              technical support; document the response in the cert evidence bundle
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — manufacturer + model +
              install manual section + DoC reference + RCD type declared + chosen product
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-[13.5px] text-white/85 leading-relaxed">
            <li>
              <strong className="text-white">Upstream main RCD</strong> — Type S (time-delayed,
              40-150 ms intentional delay) at 100 mA or 300 mA. Provides bulk protection +
              discriminates from downstream instantaneous
            </li>
            <li>
              <strong className="text-white">Downstream RCBO</strong> — heat pump dedicated circuit
              RCBO at 30 mA. Instantaneous trip on downstream fault. Clears before upstream Type S
            </li>
            <li>
              <strong className="text-white">Discrimination verification</strong> — at
              commissioning, induce a downstream fault (test button at the RCBO); verify only the
              RCBO trips, main RCD stays in. Sequential testing
            </li>
            <li>
              <strong className="text-white">Two-tier vs single-tier CU</strong> — some older CUs
              have only single-tier 30 mA RCDs covering whole circuits group. Inferior
              discrimination; upgrade to two-tier architecture during heat pump install is good
              practice
            </li>
            <li>
              <strong className="text-white">CU change typical scope</strong> — UK 2025-26 retrofit
              heat pump install often triggers CU change to two-tier architecture; cost £400-800
              typical (excluding Type B if required)
            </li>
            <li>
              <strong className="text-white">Cert evidence bundle</strong> — CU architecture + RCD
              types + discrimination test results + sequence verified
            </li>
          </ul>
        </ConceptBlock>
