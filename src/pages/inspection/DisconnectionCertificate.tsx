...
const inputCn = 'h-12 text-base touch-manipulation bg-white/[0.04] border-white/[0.08] text-white rounded-lg focus:border-elec-yellow/60 focus:ring-elec-yellow/20 [color-scheme:dark]';
const textareaCn = 'touch-manipulation text-base min-h-[80px] bg-white/[0.04] border-white/[0.08] text-white rounded-lg focus:border-elec-yellow/60 focus:ring-elec-yellow/20';
...
const defaultData = (): DisconnectionData => ({
  ...
  selectedCustomerId: '', clientName: '', clientPhone: '', clientEmail: '', installationAddress: '',
  disconnectionType: 'appliance',
  circuitDisconnected: '', applianceDisconnected: '', reasonForDisconnection: '', isolationMethod: DEFAULT_METHOD,
  madeDead: false, terminationsSafe: false, labelled: false, noReconnectionRisk: false,
  remainderSafe: false, reconnectionAdvised: false,
...
const Field = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <div><Label className="text-white/55 text-[11px] font-medium mb-1.5 block tracking-wide">{label}</Label>{children}</div>
);
...
const Segment = ({ value, options, onChange }: { value: string; options: { value: string; label: string }[]; onChange: (v: string) => void }) => (
    ...
    {options.map((o) => (
      <button
        ...
        className={cn(
          ...
          value === o.value ? 'bg-elec-yellow/15 border border-elec-yellow/45 text-elec-yellow' : 'text-white/60'
        )}
      >
        {o.label}
      </button>
...
const ConfirmRow = ({ checked, label, onToggle }: { checked: boolean; label: string; onToggle: () => void }) => (
  <button
    type="button"
    onClick={onToggle}
    className={cn(
      ...
      checked ? 'bg-elec-yellow/[0.08] border-elec-yellow/35' : 'bg-white/[0.03] border-white/[0.08]'
    )}
  >
    <span className={cn('text-[13px] leading-tight', checked ? 'text-white' : 'text-white/70')}>{label}</span>
    <span className={cn('text-[10px] font-semibold uppercase tracking-[0.12em] shrink-0', checked ? 'text-elec-yellow' : 'text-white/30')}>
...
export default function DisconnectionCertificate() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (result) { setData((prev) => ({ ...prev, ...(result as Partial<DisconnectionData>) })); setExistingReportId(editId); }
    });
  }, [editId]);
  ...
  const update = useCallback(<K extends keyof DisconnectionData>(field: K, value: DisconnectionData[K]) => {
    setData((prev) => ({ ...prev, [field]: value }));
  }, []);

  const handleSelectCustomer = (customer: Customer | null) => {
    if (!customer) { update('selectedCustomerId', ''); return; }
    setData((prev) => ({
      ...prev,
  ...
  const applyAppliancePreset = (preset: { appliance: string; circuit: string }) => {
    setData((prev) => ({
      ...
      circuitDisconnected: prev.circuitDisconnected.trim() ? prev.circuitDisconnected : preset.circuit,
    }));
  };

  const confirmAll = () => setData((prev) => ({ ...prev, madeDead: true, terminationsSafe: true, labelled: true, noReconnectionRisk: true, remainderSafe: true, reconnectionAdvised: true }));
  ...
  const handleSave = async () => {
    ...
    if (!data.inspectorSignature) { toast.error('Please add your signature'); return; }
    setIsSaving(true);
    try {
      ...
      if (existingReportId) { await reportCloud.updateReport(existingReportId, user.id, data as unknown as Record<string, unknown>, customerId); }
      else {
        const result = await reportCloud.createReport(user.id, 'disconnection' as never, data as unknown as Record<string, unknown>, customerId);
      ...
      try {
        ...
        if (pdfError) {
          ...
          try {
            ...
            console.warn('[DisconnectionCertificate] Permanent PDF storage failed, using temp URL:', storageErr);
          ...
          await openOrDownloadPdf(permanentPdfUrl, `Disconnection-Certificate-${data.referenceNumber}.pdf`);
  ...
  const allConfirmed = data.madeDead && data.terminationsSafe && data.labelled && data.noReconnectionRisk && data.remainderSafe && data.reconnectionAdvised;
        ...
        <div className="px-4 sm:px-6 py-2 mx-auto w-full max-w-5xl">
          <div className="flex items-center justify-between gap-3 h-11">
            <div className="flex items-center gap-2.5 min-w-0">
              <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="text-white hover:text-white hover:bg-white/10 rounded-xl h-10 w-10 shrink-0 touch-manipulation active:scale-[0.98]"><ArrowLeft className="h-5 w-5" /></Button>
              <h1 className="text-[15px] font-semibold text-white truncate">Disconnection Certificate</h1>
            </div>
            <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-white/40 border border-white/[0.12] rounded px-1.5 py-0.5 shrink-0">BS 7671</span>
          </div>
        </div>
      </div>

      <motion.main variants={containerVariants} initial="hidden" animate="visible" className="px-4 sm:px-6 pt-5 pb-28 mx-auto w-full max-w-5xl space-y-7">
        {/* Intro */}
        <motion.div variants={itemVariants}>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-elec-yellow/90">Electrical Disconnection</p>
          <p className="text-[12.5px] text-white/55 mt-1.5 leading-relaxed">
            A record confirming a circuit or appliance has been safely disconnected and made dead. For disconnections only — no live testing is carried out, so this is not a Minor Works certificate.
          </p>
        </motion.div>
        ...
        <motion.section variants={itemVariants}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Record No."><Input value={data.referenceNumber} onChange={(e) => update('referenceNumber', e.target.value)} className={cn(inputCn, 'font-mono')} /></Field>
            <Field label="Date of Work"><Input type="date" value={data.workDate} onChange={(e) => update('workDate', e.target.value)} className={inputCn} /></Field>
          </div>
        </motion.section>
        ...
        <motion.section variants={itemVariants}>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Phone"><Input type="tel" inputMode="tel" value={data.contractorPhone} onChange={(e) => update('contractorPhone', e.target.value)} className={inputCn} /></Field>
              <Field label="Email"><Input type="email" inputMode="email" autoCapitalize="none" value={data.contractorEmail} onChange={(e) => update('contractorEmail', e.target.value)} className={inputCn} /></Field>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <motion.section variants={itemVariants}>
          ...
          <div className="mb-3">
            <ClientSelector onSelectCustomer={handleSelectCustomer} selectedCustomerId={data.selectedCustomerId || undefined} />
          </div>
          <div className="space-y-3">
            <Field label="Name"><Input value={data.clientName} onChange={(e) => update('clientName', e.target.value)} className={inputCn} placeholder="Client name" /></Field>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Phone"><Input type="tel" inputMode="tel" value={data.clientPhone} onChange={(e) => update('clientPhone', e.target.value)} className={inputCn} /></Field>
              <Field label="Email"><Input type="email" inputMode="email" autoCapitalize="none" value={data.clientEmail} onChange={(e) => update('clientEmail', e.target.value)} className={inputCn} /></Field>
            </div>
            <Field label="Installation Address"><Input value={data.installationAddress} onChange={(e) => update('installationAddress', e.target.value)} className={inputCn} placeholder="Where the work was carried out" /></Field>
          </div>
        </motion.section>
        ...
        <motion.section variants={itemVariants}>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {APPLIANCE_PRESETS.map((p) => (
              <Chip key={p.label} label={p.label} active={data.applianceDisconnected === p.appliance} onClick={() => applyAppliancePreset(p)} />
            ))}
          </div>
          ...
          <div className="space-y-3.5">
            <Field label="Type">
              <Segment
                ...
                options={[{ value: 'appliance', label: 'Appliance' }, { value: 'circuit', label: 'Circuit' }, { value: 'both', label: 'Both' }]}
              />
            </Field>
            <Field label="Appliance Disconnected"><Input value={data.applianceDisconnected} onChange={(e) => update('applianceDisconnected', e.target.value)} className={inputCn} placeholder="e.g. Electric cooker / 9.5kW shower" /></Field>
            <Field label="Circuit Disconnected"><Input value={data.circuitDisconnected} onChange={(e) => update('circuitDisconnected', e.target.value)} className={inputCn} placeholder="e.g. Cooker circuit, 32A from way 6" /></Field>

            <Field label="Reason for Disconnection">
              <div className="flex flex-wrap gap-2 mb-2">
                {REASON_PRESETS.map((r) => (
                  <Chip key={r} label={r} active={data.reasonForDisconnection === r} onClick={() => update('reasonForDisconnection', r)} />
                ))}
              </div>
              <Textarea value={data.reasonForDisconnection} onChange={(e) => update('reasonForDisconnection', e.target.value)} className={textareaCn} placeholder="Tap a reason above or type your own" />
            </Field>

            <Field label="Method of Isolation / Making Dead">
              <Textarea value={data.isolationMethod} onChange={(e) => update('isolationMethod', e.target.value)} className={textareaCn} placeholder="How the circuit was isolated and proved dead" />
              {data.isolationMethod !== DEFAULT_METHOD && (
                <button type="button" onClick={() => update('isolationMethod', DEFAULT_METHOD)} className="mt-2 text-[11.5px] font-medium text-elec-yellow/90 touch-manipulation">Use standard GS38 wording</button>
              )}
            </Field>
          </div>
        </motion.section>
        ...
        <motion.section variants={itemVariants}>
          ...
          <button
            ...
            className={cn(
              ...
              allConfirmed ? 'bg-white/[0.03] border border-white/[0.06] text-white/30' : 'bg-elec-yellow/15 border border-elec-yellow/45 text-elec-yellow'
            )}
          >
            {allConfirmed ? 'All checks confirmed' : 'Confirm all'}
          </button>
          <div className="space-y-2">
            <ConfirmRow checked={data.madeDead} label="Circuit / appliance safely isolated and proved dead" onToggle={() => update('madeDead', !data.madeDead)} />
            <ConfirmRow checked={data.terminationsSafe} label="Exposed conductors safely terminated / made safe" onToggle={() => update('terminationsSafe', !data.terminationsSafe)} />
            <ConfirmRow checked={data.labelled} label="Disconnected point labelled where required" onToggle={() => update('labelled', !data.labelled)} />
            <ConfirmRow checked={data.noReconnectionRisk} label="No risk of inadvertent reconnection / left safe" onToggle={() => update('noReconnectionRisk', !data.noReconnectionRisk)} />
            <ConfirmRow checked={data.remainderSafe} label="Remainder of installation left in safe working order" onToggle={() => update('remainderSafe', !data.remainderSafe)} />
            <ConfirmRow checked={data.reconnectionAdvised} label="Client advised: reconnection by a competent person only" onToggle={() => update('reconnectionAdvised', !data.reconnectionAdvised)} />
          </div>
        </motion.section>

        {/* Photos */}
        <motion.section variants={itemVariants}>
          <SectionHeader title="Photos" hint={data.photos.length > 0 ? `${data.photos.length} added` : 'optional'} />
          ...
          {/* Gallery upload */}
          <input ref={photoInputRef} type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" multiple className="hidden" onChange={handlePhotoCapture} />
          <div className="grid grid-cols-2 gap-2.5">
            <button type="button" disabled={isProcessingPhoto} onClick={() => cameraInputRef.current?.click()} className="h-12 rounded-lg border border-white/[0.12] bg-white/[0.04] flex items-center justify-center gap-2 text-[13px] font-medium text-white touch-manipulation active:scale-[0.98] disabled:opacity-50">
              {isProcessingPhoto ? <Loader2 className="h-4 w-4 animate-spin" /> : <Camera className="h-4 w-4" />} Take Photo
            </button>
            <button type="button" disabled={isProcessingPhoto} onClick={() => photoInputRef.current?.click()} className="h-12 rounded-lg border border-dashed border-white/[0.15] flex items-center justify-center gap-2 text-[13px] font-medium text-white/70 touch-manipulation active:scale-[0.98] disabled:opacity-50">
              Upload
            </button>
          </div>
        ...
        <motion.section variants={itemVariants}>
          ...
          <div className="rounded-lg bg-white/[0.03] border border-white/[0.06] p-3.5 mb-3.5">
            <p className="text-[12px] text-white/65 leading-relaxed">I confirm that the circuit / appliance described above has been safely disconnected and made dead in accordance with safe isolation practice (GS38). No live testing has been carried out as part of this disconnection.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
            ...
            <div className="space-y-3">
              <ConfirmRow checked={data.clientRefusedToSign} label="Client declined to sign" onToggle={() => update('clientRefusedToSign', !data.clientRefusedToSign)} />
      ...
      <div className="sticky bottom-0 left-0 right-0 z-40 bg-background/90 backdrop-blur-md border-t border-white/[0.08] px-4 sm:px-6 pt-2.5 pb-[calc(0.625rem+env(safe-area-inset-bottom))]">
        <div className="mx-auto w-full max-w-5xl">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[10px] uppercase tracking-[0.14em] text-white/45">{progress === 100 ? 'Ready to issue' : `${progress}% complete`}</span>
          ...
          <div className="flex gap-2.5">
            <Button variant="outline" className="h-12 px-4 text-[13px] font-medium touch-manipulation active:scale-[0.98] border-white/[0.12] text-white hover:bg-white/[0.06]" onClick={() => { storageSetJSONSync(DRAFT_KEY, data); toast.success('Draft saved'); }}>Draft</Button>
            <Button className="flex-1 h-12 text-[13.5px] font-semibold touch-manipulation active:scale-[0.98] bg-elec-yellow text-black hover:bg-elec-yellow/90" onClick={handleSave} disabled={isSaving}>
              {isSaving ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Saving...</> : existingReportId ? 'Update Certificate' : 'Generate PDF'}
            </Button>
          </div>
        </div>
      </div>
