...
export default function G98CommissioningCertificate() {
  ...
  const { currentTab, setCurrentTab, currentTabIndex, totalTabs, canNavigateNext, canNavigatePrevious, navigateNext, navigatePrevious, isCurrentTabComplete, getProgressPercentage } = useG98CommissioningTabs(data);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <Field label="Equipment Type" required>
          <MobileSelectPicker value={data.equipmentType} onValueChange={(v) => update('equipmentType', v)} options={[{ value: 'Solar PV', label: 'Solar PV' }, { value: 'Battery Storage', label: 'Battery Storage' }, { value: 'Combined PV+Battery', label: 'Combined PV + Battery' }, { value: 'Wind', label: 'Wind' }, { value: 'Micro CHP', label: 'Micro CHP' }]} placeholder="Select..." triggerClassName={pickerTrigger} />
        </Field>
        <div className="grid grid-cols-2 gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <Field label="Phases">
            <MobileSelectPicker value={data.numberOfPhases} onValueChange={(v) => update('numberOfPhases', v)} options={[{ value: '1', label: 'Single-phase' }, { value: '3', label: 'Three-phase' }]} triggerClassName={pickerTrigger} />
          </Field>
        </div>
  ...
  return (
    <div className="bg-background min-h-screen">
      ...
      <main className="py-4 pb-48 sm:px-4 sm:pb-8">
        <SmartTabs tabs={smartTabs} value={currentTab} onValueChange={(v) => setCurrentTab(v as G98TabValue)} />
      </main>
      ...
      <AlertDialog open={showRecoveryDialog} onOpenChange={setShowRecoveryDialog}>
        <AlertDialogContent className="bg-background border-white/[0.08]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white">A previous unsaved G98 form was found. Would you like to recover it?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
