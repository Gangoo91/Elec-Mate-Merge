...
const examFeatures = [
  ...
  { icon: ShieldCheck, label: '80% pass mark (16/20)', description: 'Matches the standard expected for ILM coaching and mentoring assessments.' },
...
const preparationTips = [
  { title: 'Review all five modules', description: "Questions are drawn from How People Learn, The Mentor's Toolkit, Supporting Apprentices, Assessment & Evaluation, and Challenging Situations. Cover every module before attempting the exam." },
  { title: 'Know your learning theories', description: "Knowles' andragogy, Kolb's cycle, Bloom's Taxonomy, Vygotsky's ZPD, and Honey & Mumford learning styles are heavily tested. Understand the key principles and how they apply to mentoring on site." },
  { title: 'Master the mentoring models', description: "The GROW model, Pendleton's Rules, SBI feedback model, Johari Window, and Kirkpatrick's four levels appear frequently. Know the steps in each model and when to use them." },
  { title: 'Understand the JIB framework', description: 'JIB 4-stage apprenticeship structure, ECS grade progression, AM2 assessment, NVQ evidence types and VACSR principles. These practical topics form a significant portion of the question bank.' },
  { title: 'Remember key legislation', description: 'HSWA 1974 Section 2(2)(c), CDM 2015 Regulation 13, Electricity at Work Regulations 1989 Regulation 16, MHSWR 1999 Regulation 13, and Equality Act 2010 protected characteristics all feature in questions.' },
  ...
  return (
    <div className="min-h-screen bg-[hsl(0_0%_8%)] text-white">
      <div className="px-4 sm:px-6 lg:px-8 pt-2 pb-24">
        <PageFrame>
          <button
            onClick={() => navigate('../mentoring-developing-others')}
            className="inline-flex items-center gap-2 h-10 px-3 rounded-full bg-white/[0.06] border border-white/[0.1] text-white text-[13px] font-medium touch-manipulation hover:bg-white/[0.1] mb-1 self-start"
          >
            <ArrowLeft className="h-4 w-4" /> Mentoring & developing others
          </button>

          <PageHero
            eyebrow="Module 6 · Final assessment"
            title="Mock exam"
            description="Put your mentoring knowledge to the test under timed exam conditions. Questions are drawn from a 200-question bank covering all five content modules."
            tone="yellow"
          />
          ...
          <Link to="../md-mock-exam" className="block touch-manipulation focus:outline-none">
            <div className="rounded-2xl bg-gradient-to-br from-elec-yellow/15 via-amber-500/10 to-transparent border border-elec-yellow/30 p-5 sm:p-6 active:scale-[0.99] transition-transform">
              <div className="flex items-center justify-between gap-3">
                ...
                <PrimaryButton size="lg" onClick={() => navigate('../md-mock-exam')}>
          ...
          <ListCard>
            <div className="relative px-5 sm:px-6 py-3.5 sm:py-4 border-b border-white/[0.06]">
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-indigo-500/70 via-violet-400/70 to-purple-400/70 opacity-70" />
            ...
            <div>
              {categories.map((c) => (
                <ListRow
                  key={c.name}
                  lead={<span className="h-2 w-2 rounded-full bg-indigo-400 block" />}
                  title={c.name}
                  subtitle={c.module}
                  trailing={
