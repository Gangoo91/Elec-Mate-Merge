...
const examFeatures = [
  ...
  { icon: ShieldCheck, label: '80% pass mark (16/20)', description: 'Matches the standard required by industry assessments.' },
...
const preparationTips = [
  { title: 'Review all five modules', description: 'Questions are drawn equally from Understanding Conflict, Communication, Client Disputes, Site Conflicts and Prevention. Cover every module before attempting the exam.' },
  { title: 'Know your conflict styles', description: 'The Thomas-Kilmann model — competing, collaborating, compromising, avoiding, accommodating — appears throughout. Be able to identify each style from a scenario.' },
  { title: 'Master the frameworks', description: 'Crucial Conversations (STATE, mutual purpose), Nonviolent Communication (observation, feelings, needs, requests) and the SBI feedback model are heavily tested.' },
  { title: 'Understand the legal rights', description: 'The Late Payment Act 1998, Consumer Rights Act 2015 and Construction Act 1996 each give you specific rights. Know the key provisions and time limits.' },
  { title: 'Know de-escalation techniques', description: 'Verbal Judo principles, the amygdala hijack, the 24-hour rule and the Positive No are practical tools you should be able to apply to scenarios.' },
    ...
    <div className="min-h-screen bg-[hsl(0_0%_8%)] text-white">
      <div className="px-4 sm:px-6 lg:px-8 pt-2 pb-24">
        <PageFrame>
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Conflict resolution & difficult conversations
          </button>

          <PageHero
            eyebrow="Module 6 · Final assessment"
            title="Mock exam"
            description="Put your conflict resolution knowledge to the test under timed exam conditions. Questions are drawn from a 200-question bank covering all five content modules."
            tone="red"
          />
          ...
          >
            <div className="rounded-2xl bg-gradient-to-br from-red-500/15 via-rose-500/10 to-transparent border border-red-500/30 p-5 sm:p-6 active:scale-[0.99] transition-transform">
              <div className="flex items-center justify-between gap-3">
                ...
                <PrimaryButton size="lg" onClick={() => navigate('../cr-mock-exam')}>
          ...
          <ListCard>
            <div className="relative px-5 sm:px-6 py-3.5 sm:py-4 border-b border-white/[0.06]">
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-red-500/70 via-rose-400/70 to-orange-400/70 opacity-70" />
          ...
          <ListCard>
            ...
            <div>
              {categories.map((c) => (
                <ListRow
                  key={c.name}
                  lead={<span className="h-2 w-2 rounded-full bg-red-400 block" />}
                  title={c.name}
                  subtitle={c.module}
                  trailing={
            ...
            <div>
              {preparationTips.map((tip, i) => (
                <ListRow
                  key={tip.title}
                  lead={
                    <div className="h-7 w-7 rounded-full bg-red-500/10 border border-red-500/30 flex items-center justify-center shrink-0">
