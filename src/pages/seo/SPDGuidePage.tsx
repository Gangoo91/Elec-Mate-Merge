...
const faqs = [
  ...
  },
  {
    ...
    answer:
      'It depends on the consumer unit. If the existing consumer unit has a spare way, you can fit an SPD module into it (subject to manufacturer compatibility and space). Many SPD manufacturers produce DIN-rail-mounted units that fit standard consumer unit enclosures. However, if the consumer unit is old, full, or does not have a suitable mounting arrangement, a separate enclosure may be needed. The SPD must still be installed as close to the origin as possible, with short protective conductor runs. If the consumer unit is being replaced (for example, as part of a consumer unit upgrade to meet Amendment 3 requirements for AFDDs), fitting an SPD at the same time is the most cost-effective approach. When retrofitting, ensure the SPD is compatible with the supply system (TN-S, TN-C-S, or TT) and that the earth fault loop impedance is within the SPD manufacturer specified limits.',
  },
];
