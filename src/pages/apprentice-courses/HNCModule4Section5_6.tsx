...
const quickCheckQuestions = [
  ...
  },
  {
    ...
    options: ['Modbus or BACnet', 'HTTP only', 'USB', 'Bluetooth'],
    correctIndex: 0,
    explanation:
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: ['WiFi only', 'Modbus RTU/TCP', 'No communication available', 'Analogue signals only'],
    correctAnswer: 1,
    explanation:
...
const HNCModule4Section5_6 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 5 · Subsection 6"
      ...
      <HubBody>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              written for. Cutting CTs into live busbars or adding meter PSU loads to a fully-loaded
              sub-main without re-checking the assembly’s rating, fault level and earthing
              arrangement is precisely what 132.16 forbids. The HNC designer’s sign-off is the
              gatekeeper — verify before installation, document the check on the Variation/Minor
              Works certificate.
            </>
          }
        ...
        <ConceptBlock title="Sub-Metering for Part L">
          ...
          <p>
            <strong>
              Part L end-use categories (category / typical loads / metering approach):
            </strong>
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>Heating — boilers, heat pumps, electric heating — kWh electricity, heat meters</li>
        ...
        <ConceptBlock title="Energy Management Systems">
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              Load profiling — time-based consumption patterns — identify efficiency opportunities
            </li>
        ...
        <ConceptBlock title="Worked Examples">
          ...
          <p>
            <strong>Example 2 — CT selection:</strong> Specify CTs for 800A main switchboard meter.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
        ...
        <Scenario
          ...
          situation={
            <>
              Existing 1990s 4-storey office, fiscal meter at landlord cut-out, tenants separately
              billed by service charge. Client wants Part L 2021 / BREEAM In-Use sub-metering
              retrofitted: per-floor, per-end-use (lighting, small power, HVAC). You’ve been asked
              to scope and design.
            </>
          }
          whatToDo={
            <>
              Survey first. Read the existing sub-main loadings and confirm the riser conductors and
              the main switchboard busbar rating are not at their limit. Apply Reg 132.16 — adding
              30 split-core CTs and meter PSUs is a non-trivial alteration: each meter PSU draws ~15
              W but the CTs require live access for fitting. Strategy: shutdown windows per floor,
              use split-core CTs (no busbar drop-out needed), x/5 A secondaries to MID Class 1
              meters, Modbus RTU on a daisy-chained RS-485 segment up to a floor gateway, then
              Modbus TCP/IP back to the BMS head-end. 16 mm² spare way for meter PSU per board.
              Document the safe-isolation method statement (EAWR Reg 4 territory — see SOUL/EAWR —
              “prove dead” is mandatory). Issue a Minor Works Certificate per board with the Reg
              132.16 “addition adequate” verification completed and recorded.
            </>
          }
          whyItMatters={
            <>
              Sub-metering looks like “bolt-on tech”. It isn’t. Live busbar work, CT polarity
              errors, undersized PSU circuits and missing 132.16 verification are how this turns
              into a fire or a failed Part L sign-off. The design is half BS 7671, half BMS comms.
            </>
          }
        />
