...
const TITLE = 'Inductance, capacitance and reactance | Level 3 Module 3.2.5 | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  {
    id: 'l3-m3-2-5-xl',
    question: 'Inductive reactance of a 0.05 H inductor at 50 Hz:',
  ...
  {
    id: 'l3-m3-2-5-xc',
    question: 'Capacitive reactance of a 100 μF capacitor at 50 Hz:',
    options: ['31.8 Ω', '3.18 Ω', '318 Ω', '0.318 Ω'],
    correctIndex: 0,
    explanation:
  ...
  {
    id: 'l3-m3-2-5-z',
    question: 'A series RL circuit: R = 6 Ω, X_L = 8 Ω. Impedance Z =',
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: ['Tesla', 'Henry', 'Farad', 'Joule'],
    correctAnswer: 2,
  ...
  {
    id: 7,
    question: 'Series R-L-C: R = 8, X_L = 6, X_C = 0. Impedance:',
    options: ['8 Ω', '6 Ω', '14 Ω', '10 Ω'],
    correctAnswer: 3,
    explanation:
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          X_L = 2πfL. X_C = 1/(2πfC). Pythagoras gives impedance Z. The maths behind every motor
          winding and every PFC capacitor.
        </p>
        ...
        >
          <p>
            <strong>v(t) = L × di/dt</strong> — voltage across an inductor depends on the rate of
            change of current.
          </p>
          <p>
            Inductance is measured in henries (H). Practical inductors at 50 Hz are usually tens to
            hundreds of mH. A motor winding might be 50-200 mH. A fluorescent choke ballast was
            typically 1-3 H. EMC filters use μH.
          </p>
          <p>
            Energy stored: <strong>W = ½ × L × I²</strong>. A 0.5 H inductor carrying 10 A stores 25
            J — released as a spark if you suddenly break the circuit (which is why you fit a
            freewheel diode across DC inductive loads).
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <p>
            X_L = 2π × 50 × 0.5 = 157 Ω. So with 230 V across it, current = 230 / 157 = 1.46 A — and
            it lags the voltage by 90°.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>i(t) = C × dv/dt</strong> — current depends on the rate of change of voltage.
          </p>
          <p>
            Capacitance is measured in farads (F). Practical capacitors are μF, nF, pF. PFC
            capacitors are tens of μF. EMC filter caps are nF. Smoothing caps in PSUs are hundreds
            of μF up to thousands.
          </p>
          <p>
            Energy stored: <strong>W = ½ × C × V²</strong>. A 100 μF cap charged to 325 V (peak of
            UK mains) stores 5.3 J — enough to give a memorable shock if discharged through you.
            Always test capacitors discharged before working on them.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <p>
            Note the difference from X_L: as frequency rises, X_L grows but X_C shrinks. They are
        ...
        >
          ...
          <p>
            Worked example: two 100 μF capacitors. In parallel: C_T = 200 μF. In series: C_T = (100
            × 100)/(100 + 100) = 10 000/200 = 50 μF. Voltage rating doubles in series (handy when
            you need a higher-voltage cap from two lower-rated ones — common in high-voltage
            strings, but you need balancing resistors).
          </p>
          <p>
            Power-factor correction banks parallel multiple capacitors for higher capacitance at the
            line voltage. Smoothing in PSUs uses parallel caps to lower equivalent series resistance
            and share ripple current.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A naive PFC capacitor bank can resonate with the inductance of the supply transformer
              at a harmonic frequency (typically 5th or 7th — 250 Hz / 350 Hz), causing capacitor
              failure and voltage distortion. Detuning reactors lower the resonant frequency well
              below the dominant harmonic so the cap stops the lagging-PF current without amplifying
              anything.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Poor power factor inflates line current for the same real power, so cable voltage drop
              rises and the terminal voltage at the equipment falls. Sizing for cos φ = 0.7 lagging
              without correction often pushes V<sub>terminal</sub> below the appliance's
              product-standard floor — Reg 525.1 then drives you to either upsize cable or fit PFC
              at the load.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A detuned PFC bank is one half of harmonic management; the other half is cable sizing.
              Where the load mix is rich in non-linear devices (LED drivers, VFDs, rectifiers),
              Appendix 4 §C harmonic factors apply on top of grouping and ambient de-rating. Treat
              the harmonic content of the load as a separate de-rating axis — power factor
              correction does not on its own clean the harmonics.
            </>
          }
        ...
        >
          <p>
            <strong>Capacitor size: C = Q_c / (2πf × V²)</strong> (single-phase) or per phase for
            3-phase delta-connected banks.
          </p>
          <p>
            Worked example continued: for the 34.6 kvar required at 400 V 3-phase, per phase (delta)
            Q = 34.6 / 3 = 11.5 kvar at 400 V (line-line). C per phase = 11 500 / (2π × 50 × 400²) =
            11 500 / (314 × 160 000) = 11 500 / 50 240 000 = 229 × 10⁻⁶ F = 229 μF. Sourced as a
            standard 250 μF / 440 V cap (next size up).
          </p>
          <p>
            After correction: total current = P / (√3 × V × pf) = 50 000 / (1.732 × 400 × 0.95) = 76
            A — down from 50 000 / (1.732 × 400 × 0.7) = 103 A before. Same useful work, 26 % less
            line current → smaller cable, smaller switchgear, smaller DNO bill.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Sum says: R = 6 Ω, X_L = 8 Ω. Apprentice writes Z = 6 + 8 = 14 Ω. Calculates current =
              230 / 14 = 16.4 A. Real Z is only 10 Ω, so current is actually 23 A — cable now
              under-sized.
            </>
          }
          doInstead={
            <>
              R and X are 90° apart. Use Pythagoras: Z = √(R² + X²) = √(36 + 64) = 10 Ω, not 14 Ω.
              This is exactly the same mistake as adding the legs of a right-angled triangle as if
        ...
        <Scenario
          ...
          situation={
            <>
              A control circuit drives a 24 V AC contactor coil with inductance 0.6 H and resistance
              8 Ω. What current does it draw on a 24 V 50 Hz supply, and what power factor does the
              coil present?
            </>
          }
          whatToDo={
          ...
          whyItMatters={
            <>
              Most of the apparent VA is reactive — the coil is just sloshing energy back and forth
              in its magnetic field. The current the coil draws is set by Z, not R alone. Sizing the
              supply transformer or PSU to R alone (24 / 8 = 3 A) would massively over-spec it. The
              Z calculation tells you the real story.
            </>
          }
        />
