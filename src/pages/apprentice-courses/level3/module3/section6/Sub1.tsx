    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Electromagnetic switching for power circuits. Utilisation categories. Coil ratings.
          Snubbing inductive loads.
        </p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Contactor</strong> — power switch for motors, heaters, lighting circuits ≥16
              A.
            </li>
            <li>
        ...
        >
          <p>
            <strong>
              The arc chute is the part that defines whether a contactor can do its job safely.
            </strong>{' '}
            When the moving contact opens under load, the inductive load tries to maintain current —
            the air ionises and a small electric arc forms. Without help, that arc could persist for
            tens of milliseconds and weld the contacts together. The chute uses two effects:
            blow-out from the magnetic field of the load current itself drives the arc upward into
            the chute, and the chute's metal plates split the single long arc into many small ones,
            each of which extinguishes at the next AC zero crossing.
          </p>
          <p>
            ...
            contactors need much more aggressive blow-out arrangements (permanent magnets, larger
            chutes) and have far lower DC ratings than their AC equivalents. Never use an AC
            contactor on a DC circuit beyond a tiny fraction of its AC rating.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The shading ring is a single-turn copper or brass loop set into a slot in the pole face.
            The main flux passing through it induces a small EMF in the loop, which drives a current
            that creates a SECOND flux component, phase-shifted from the main flux by about 60–90°.
            The two fluxes (main and shaded) overlap such that when one is at zero, the other is
            near peak. Net flux through the armature never reaches zero — armature stays clamped.
          </p>
          <p>
            This is the same trick as a shaded-pole motor (Sub 5.2), used here for clamping rather
            than rotation. The moment AC was invented, this problem appeared and this fix was
            developed. Universal across every AC-coil contactor since the 1920s.
          </p>
        </ConceptBlock>
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>AC-3</strong> — squirrel-cage motor normal start. Make at 6×I_e, break at I_e.
              Standard for motor circuits.
            </li>
            <li>
              <strong>AC-4</strong> — squirrel-cage motor reversing/jogging. Make at 6×I_e, break at
              6×I_e. Tougher duty — derate or oversize.
            </li>
            <li>
        ...
        >
          <p>
            <strong>For DC coils, fit a freewheel (flyback) diode</strong> across the coil, anode to
            negative side, cathode to positive side. In normal operation the diode is reverse-biased
            and does nothing. When the coil de-energises, the collapsing field reverses the coil's
            voltage; the diode now sees forward bias and provides a path for the inductive current
            to circulate harmlessly until the energy dissipates as heat in the coil's own
            resistance. Coil voltage is clamped at one diode drop (~0.7 V) above supply. Cheap,
            foolproof, universal.
          </p>
          <p>
            <strong>For AC coils, fit a parallel RC snubber</strong> (typically 100 Ω in series with
            0.1 µF metallised polypropylene) across the coil. The capacitor absorbs the
            high-frequency switching transient; the resistor dissipates the energy and prevents the
            cap-coil pair becoming a high-Q resonant circuit. AC coils can't use a simple diode
            (would short-circuit the AC every alternate half-cycle).
          </p>
          <p>
            <strong>Symptoms of an unsnubbed circuit:</strong> nuisance trips on neighbouring
            electronics, premature relay contact wear, EMC test failure, occasional contactor coil
            burnouts when an unrelated downstream contactor opens. Snubbing is a £1 fix that
        ...
        >
          <p>
            Two construction types matter: <strong>direct-acting</strong> (the plunger IS the valve
            — small bore, low pressure, fast response, used for low-flow water/gas/air
            applications); <strong>pilot-operated or 'servo'</strong> (the plunger opens a tiny
            pilot port, and supply pressure does the work of moving a much bigger main diaphragm —
            used for high-flow water valves and large gas valves where coil power alone could never
            lift the pressure-sealed disc directly).
          </p>
          <p>
            <strong>Inrush vs holding current.</strong> A solenoid coil at the moment of switch-on
            draws several times its steady current — same physics as a contactor coil, plus the air
            gap (plunger fully out) makes the magnetic circuit weak so more current is needed to
            develop the pulling force. Once the plunger is seated, the air gap closes and the coil's
            effective inductance rises sharply, dropping current to its steady value. Always size
            cabling and protective devices for inrush, not just steady current.
          </p>
          <p>
            Most modern solenoid valves carry both AC and DC ratings on the body — the same coil can
            often be wound for either, but the rated voltage MUST match. A 24 V DC valve on 24 V AC
            will run hot and may chatter; a 230 V AC valve on 230 V DC will burn the coil in seconds
        ...
        >
          <p>
            <strong>Latching (impulse) relays</strong> use a mechanical latch (sometimes magnetic)
            to hold the contact in its last commanded state. A short pulse to one coil flips them
            ON; a pulse to the other (or sometimes the same) flips them OFF. Used for whole-room
            corridor lighting where multiple wall switches each toggle the same lamp circuit. Saves
            the cost of two-way and intermediate switching for runs over three switch points.
          </p>
          <p>
            <strong>Time-delay relays</strong> come in four flavours: ON-delay (close N seconds
            after energise), OFF-delay (stay closed N seconds after de-energise), interval (close on
            energise, open after N seconds, ignore further input until reset), and pulse (output a
            fixed-width pulse on each input). Found in motor sequencers, fan run-on for bathroom
            extracts, and emergency lighting test cycles.
          </p>
          <p>
            <strong>Thermal overload relays</strong> sit downstream of the contactor and contain
            three small bimetallic strips (one per phase) heated by the load current via a small
            replica winding. Sustained over-current bends the strips, releasing a trip mechanism
            that opens an N/C contact wired in series with the contactor coil — contactor drops out,
            motor stops. Set the dial to between 1.0 and 1.15 × FLA. Modern electronic overloads do
            the same job with a thermistor or current transformer plus a microcontroller, with much
            wider adjustment range and remote diagnostics.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The plate on every contactor shows I_e for AC-1, AC-3, AC-4 (often three different
              numbers — same contactor). Pick by the right column. AC-3 23 A means 23 A in normal
              motor duty; the same contactor might only be 8 A in AC-4 reversing duty.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The standard contactor circuit — start button momentarily energises the coil, an NO
              auxiliary contact "seals in" the coil voltage so the contactor stays in once the start
              button is released, and a stop button breaks the seal-in path — does exactly this. A
              supply dip drops the coil out, the seal-in opens, and a fresh start button press is
              needed to restart. Hard-wired contactor logic is safer than wiring start direct from a
              switch.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              machine. Reg 132.15.202 is the rule that distinguishes the contactor (a control switch
              you operate routinely) from the isolator (a safety device you lock-off for
              maintenance). Contactor coils can fail closed; only a physical isolator guarantees the
              motor cannot restart while a worker is on the load.
            </>
          }
        ...
        >
          <p>
            <strong>Polarity matters.</strong> A DC contactor is often POLARISED — the + terminal
            must be connected to one specific side of each pole, marked on the body. This is so the
            magnetic blow-out (a permanent magnet near each contact) drives the arc upward into the
            chute in the correct direction. Reverse polarity and the blow-out drives the arc the
            WRONG way — into the contactor body — and the contactor self-destructs on the first
            switching event under load.
          </p>
          <p>
            <strong>Series-poling for higher voltage.</strong> A 250 V DC contactor cannot safely
            switch 500 V DC even on a half-rated current. The fix used in EV chargers, traction
            control and battery storage: wire several poles in series so each pole sees a fraction
            of the voltage. The arc breaks across each pole's gap simultaneously and is much easier
            to extinguish per-pole. Modern DC contactors for EV charging rate themselves at 1000+ V
            using this approach with 2–4 poles in series.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Apprentice picks a 25 A "AC-1" rated contactor for a 22 A motor. Sounds fine — until
              the motor draws 6 × 22 = 132 A inrush every start. Contacts pit, then weld closed.
              Motor runs continuously regardless of stop button.
            </>
          }
          doInstead={
            <>
              Pick by AC-3 (or AC-4 for reversing) for motors. A contactor showing 25 A AC-1 might
              only be 11 A AC-3. Always read the right utilisation column for your application.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              spike (200+ V) arcs across the relay contacts. Within months the relay contacts are
              pitted and welded. Plus a nearby PLC analogue input glitches randomly with spurious
              readings.
            </>
          }
          doInstead={
            <>
              Fit a flyback diode (e.g. 1N4007 for small coils) directly across the coil terminals.
              Cathode to positive supply, anode to negative. The diode clamps the spike to ~0.7 V on
              collapse and protects the rest of the circuit. £0.10 part, saves callouts.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              FLA ≈ 14 A. DOL start, normal duty (AC-3). Allow for occasional reverse running (AC-4
              considered). Coil voltage to suit panel control 24 V DC.
            </>
          }
          whatToDo={
            <>
              ...
              <br />
              Aux contacts: 1 N/O for "running" lamp + 1 N/C for interlock with the standby pump
              contactor. Seal-in via N/O aux for restart prevention per §552.1.3.
            </>
          }
          whyItMatters={
            <>
              The right utilisation category, properly snubbed, plus appropriate aux contacts gives
              a panel that runs reliably for years. Wrong choice = field call within months.
            </>
          }
        />
