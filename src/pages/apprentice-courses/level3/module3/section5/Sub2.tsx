...
const faqs = [
  ...
  {
    question:
      'What is the difference between a "capacitor-start" and "permanent split capacitor" (PSC) motor?',
    answer:
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Capacitor-start, induction-run, PSC, shaded-pole and universal motors. The five families
          that cover almost every domestic and small commercial motor.
        </p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>Inductance</strong> — phase-shift via differing inductance between start and
              run windings (induction-start, less common today).
            </li>
            <li>
              <strong>Shading ring</strong> — magnetic shading on one pole corner produces a weakly
              rotating field (shaded-pole).
            </li>
            <li>
        ...
        >
          <p>
            ...
            starting trick but doesn't need it once running. At zero speed, slip with respect to the
            forward field equals slip with respect to the backward field (both equal 1) — equal and
            opposite torques. Once a rotor is rotating at, say, 4 % slip on the forward field, it
            sees 196 % slip on the backward field, where the backward torque has fallen off the side
            of the torque-slip curve.
          </p>
          <p>
            Practical consequence — the backward field never goes away. It remains as a small
            braking torque and as a 100 Hz (twice supply frequency) torque pulsation. That pulsation
            is what makes single-phase motors noisier than three-phase, and why they're rarely used
            above ~7.5 kW. Every full cycle of mains, the rotor receives one helpful pulse and one
            mild brake. You can hear it on a quiet washing machine pump.
          </p>
        </ConceptBlock>
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              centrifugal switch. Cap drops out at ~75 % speed. Strong start, run on main winding
              only. Compressors, pumps, conveyors. ~70 % efficiency.
            </li>
            <li>
              <strong>Permanent Split Capacitor (PSC)</strong> — smaller motor-run cap, always in
              circuit. Lower start torque, no switch, quieter. Fans, AC blowers. ~75 % efficiency.
            </li>
            <li>
              <strong>Capacitor-start, capacitor-run</strong> — best of both. Large electrolytic for
              start + small motor-run for steady. Higher cost. Premium fan and HVAC drives.
            </li>
            <li>
              <strong>Shaded-pole</strong> — copper ring on a corner of each pole. Tiny phase shift
              starts the rotor weakly. Cheap, low efficiency (~30 %). Cooling fans, microwaves,
              small clocks.
            </li>
            <li>
              <strong>Universal (series brushed)</strong> — runs AC or DC. High speed (10–25 krpm).
              Vacuum cleaners, angle grinders, blenders, drills. Brush wear, RFI emission.
            </li>
            <li>
        ...
        >
          <p>
            ...
            degrees away from the run winding around the stator. The capacitor in series with the
            start winding shifts its current 90° in time. The combination of spatial-90° plus
            time-90° produces a true (if temporary) rotating field. That's the whole purpose of the
            capacitor — without it, both windings would carry currents in phase, and the result
            would still be a pulsating field.
          </p>
          <p>
            <strong>Why the centrifugal switch ages.</strong> Spring-loaded weights pivot outward as
            the rotor speeds up. After 50 000+ start cycles the spring weakens, the contacts pit, or
            grease in the pivots gums up. Symptoms: motor hums and won't start (switch failed open),
            or motor runs hot and start cap cooks itself (switch failed closed).
          </p>
          <p>
            <strong>Cap selection.</strong> The motor-START cap is electrolytic, 70–400 µF typical,
            AC-rated to ~250 V, designed for ≤3 % duty cycle. The motor-RUN cap is metallised
            polypropylene, 2–60 µF typical, AC-rated 370 or 450 V, designed for 100 % continuous
            duty. Swap them at your peril — a start cap left permanently in circuit boils itself dry
            within hours.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            eddy losses on AC). Speed is set by the load — light load → very high speed (10 000–30
            000 rpm typical), heavy load → speed plummets. There's no synchronous-speed limit
            because the rotating field concept doesn't apply.
          </p>
          <p>
            <strong>Strengths:</strong> 200–400 W from a motor that fits in your hand. High starting
            torque. Speed easily controlled by triac phase-control (the £2 dimmer hidden inside
            every variable-speed drill).
          </p>
          <p>
            <strong>Weaknesses:</strong> brush wear (typically 200–1000 hours of duty), commutator
            arcing creates radio-frequency interference (every universal motor needs an RFI
            suppression capacitor and often a small choke for CE/UKCA compliance), audible noise,
            and short life vs an induction motor. Not used for continuous-duty industrial machines —
            but unbeatable for portable hand tools.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Performance is modest at best:</strong> starting torque ~50 % of rated running
            torque, full-load efficiency 15–35 %, power factor 0.4–0.6, slip 8–15 % (much higher
            than a properly-engineered single-phase motor). But the cost is unbeatable — no
            capacitor, no centrifugal switch, no auxiliary winding, no electrolytic. Just a single
            main winding and a few grams of copper ring.
          </p>
          <p>
            <strong>Where you'll meet it:</strong> cooker hood extract fans, bathroom extract fans,
            microwave oven turntables, fridge condenser fans, small mains-powered clocks, low-cost
            desktop fans. Anywhere the application can tolerate low torque, low efficiency and no
            speed control beyond a series resistor or a 2-tap winding.
          </p>
          <p>
            <strong>Diagnostic tip:</strong> shaded-pole motors are non-reversible by terminal
            swapping. The shading rings are physically fixed in one corner of each pole, defining
            one rotation direction. If a customer wants to reverse a shaded-pole fan, you replace it
            — you can't rewire it.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Inside an ECM:</strong> the input mains is rectified to a DC bus; an electronic
            3-phase inverter (built into the motor body) drives the permanent-magnet stator winding;
            rotor position is sensed by Hall-effect ICs or sensorless back-EMF detection; speed is
            set by a 0–10 V signal, PWM input or built-in pressure transducer. From outside it looks
            like an old-school PSC motor. Inside it's a miniature VFD-fed PMSM.
          </p>
          <p>
            <strong>Why the change?</strong> ErP/Ecodesign Lot 11 (circulators) and Lot 32 (fans)
            made the inefficient PSC motor effectively illegal for new product placement. A 100 W
            traditional pump runs at ~30 W as an ECM equivalent. A wet-rotor circulator that ran
            24/7 saves around £30 per year per unit. Multiply across millions of UK homes and the
            regulation pays back nationally.
          </p>
          <p>
            <strong>Implications for the electrician:</strong> ECM and BLDC motors leak DC and HF
            current to earth via internal EMC capacitors — they need Type A or Type B RCD protection
            (depending on the drive design), not Type AC. Always read the unit data sheet before
            specifying the protective device.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              UK Ecodesign requirements for single-phase motors below 0.75 kW are less strict than
              3-phase, but pf and η are still part of CE/UKCA marking. For larger single-phase
              units, switching to 3-phase + VFD often gives better efficiency at lower cost over
              life.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              contactor or VFD. The exception covers small appliance motors that already comply with
              their own product standard (e.g. a washing machine drive). For installed single-phase
              motors on site, always specify the overload device on the motor control schematic.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Single-phase pumps, fans and compressors all need a local isolator within sight of the
              motor — a lockable rotary or pull-cord switch is the standard. Reg 132.15.202 is what
              enforces "isolator at the machine" on every fixed installation regardless of phase. On
              a capacitor-start motor the isolator must break the line conductor cleanly so the
              start capacitor can discharge through its bleed resistor before any maintenance work
              begins.
            </>
          }
        ...
        >
          <p>
            Inside a capacitor-start motor are four winding terminals — two for the run winding, two
            for the start winding. The capacitor and centrifugal switch are in series with the start
            winding. Swapping the two start-winding terminals (or the two run-winding terminals —
            but never both at once) flips the spatial sense of one winding's field, reverses the
            direction of the rotating field, and the motor spins the other way.
          </p>
          <p>
            ...
            (run). Swap Z1 and Z2 leads to reverse rotation. Some motors have a built-in reversing
            switch that does this internally; many don't, and the customer thinks the motor is
            non-reversible until you open the box.
          </p>
          <p>
            <strong>Shaded-pole motors are the exception</strong> — they cannot be electrically
            reversed, full stop. The shading rings are physically fixed in one corner of each pole,
            defining the rotation direction at manufacture. A 'reversible' shaded-pole product is
            actually two motors back-to-back sharing a shaft, with one or the other energised. If a
            customer wants reverse rotation on a shaded-pole fan, replace the unit with a different
            model.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Old motor-run cap was 50 µF 450 V AC. Apprentice fits a 47 µF 400 V cap from the van.
              Motor runs but vibrates badly; cap swells and fails within weeks because 400 V AC
              rating is below the 230 V × √2 = 325 V peak supply.
            </>
          }
          doInstead={
            <>
              Match exact µF (or within 5 %); use a cap rated ≥ 450 V AC for 230 V mains operation;
              specify motor-run (continuous duty) not motor-start (intermittent). Wrong cap = poor
              starting + early failure.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          doInstead={
            <>
              ...
              capacitance range, or substitute a known-good cap. ~80 % of "won't start" calls on
              single-phase motors are failed run capacitors. £2 part, 10-minute job. Saves replacing
        ...
        <Scenario
          ...
          situation={
            <>
              Old central-heating pump (single-phase, ~100 W). PSC motor (small cap visible). Pump
              intermittent — sometimes won't start. Cap reads 6 µF on the meter, the plate says 10
              µF.
            </>
          }
          whatToDo={
            <>
              Cap has lost capacitance. Replace with same µF (10 µF) and same V rating (typically
              450 V AC) motor-run cap.
              <br />
              Test: with new cap, pump starts immediately. If still intermittent, motor windings are
              at fault (open or shorted run winding) — replace pump unit.
              <br />
          ...
          whyItMatters={
            <>
              ~80 % of single-phase motor "won't start" calls are failed run capacitors. £2 part,
              10-minute job. Saves replacing a perfectly good motor and identifies the modern
              ECM/RCD-type compatibility issue early.
            </>
          }
        />
