...
const TITLE =
  'Synchronous motors, VFDs, and motor control | Level 3 Module 3.5.4 (AC 3.4) | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  ...
  },
  {
    ...
    options: ['Half', 'No reduction', 'One-tenth', 'One-third (~3×)'],
    correctIndex: 3,
    explanation:
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: ['Same as DOL', 'About 1/3 of DOL', 'Zero', 'About 1/10 of DOL'],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: ['Same speed as 50 Hz', 'Twice speed', 'Half speed', 'Stopped'],
    correctAnswer: 2,
    explanation:
  ...
  return (
    <HubPage>
      <HubMasthead
        ...
        backTo="/study-centre/apprentice/level3-module3-section5"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Synchronous machines locked to grid frequency. VFDs synthesising variable f. DOL,
          star-delta, soft-starter, VFD compared.
        </p>
        ...
        >
          ...
          <p>
            <strong>Speed:</strong> exactly N_s = 120f/P. With grid f locked to 50 Hz, the only way
            to vary speed is to vary f — i.e. via a VFD/inverter.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Wound-rotor synchronous</strong> uses a separate DC excitation supply through
            slip rings to set the rotor field strength. By over-exciting the rotor, the motor draws
            leading reactive current — it acts as a synchronous condenser and corrects power factor
            for the rest of the site. Big factories with 1+ MW synchronous compressors get the PFC
            for free.
          </p>
          <p>
            <strong>PMSM</strong> is hands-down the most efficient electric motor topology in mass
            production. Tesla, BMW, Toyota all use them; high-end industrial servos use them; the
            latest IE5 ultra-premium efficiency band (BS EN 60034-30-2) is essentially defined by
            what PMSMs can achieve. The price is rare-earth magnet cost and the need for an inverter
            — they cannot start direct on a 3-phase mains supply.
          </p>
          <p>
            <strong>Synchronous reluctance</strong> has a rotor that's just shaped iron — no copper,
            no magnets, no slip rings. Cheap to build, robust, no rare earths. Locks to the rotating
            stator field by magnetic preference (the rotor wants to align its high-permeability axis
            with the field). Performance approaching PMSM at ferrite-magnet cost. ABB SynRM range is
            the best-known commercial example. Becoming more popular as rare-earth supply concerns
            grow.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Speed control:</strong> lower f → lower N_s → lower motor speed. V proportional
            to f keeps flux roughly constant. Below ~10 Hz, voltage boost compensates for stator
            resistance.
          </p>
          <p>
            <strong>Affinity laws on a centrifugal pump or fan:</strong> Q ∝ N, P ∝ N³. Halving
            speed → 1/8 power. Massive energy savings in variable-flow applications.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>PWM and motor inductance:</strong> the IGBTs switch the motor between full +V
            and full −V at high frequency, varying the duty cycle to approximate a sine wave. The
            motor inductance acts as a low-pass filter — it sees the rectangular voltage pulses but
            forces a roughly sinusoidal current through itself. The result is acceptable torque
            ripple but harsh dV/dt at the motor terminals.
          </p>
          <p>
            <strong>The dead-time problem.</strong> The two IGBTs in each leg of the H-bridge must
            NEVER conduct at the same instant — that would short-circuit the DC bus. The control
            inserts a 1–5 µs "dead time" between switching one off and the other on. During dead
            time, the motor inductance forces current through whichever freewheel diode happens to
            be available. This causes small voltage distortions, output harmonic content, and a bit
            of audible noise.
          </p>
          <p>
            <strong>Cable-length problems.</strong> Long motor cables (&gt;30 m) act as transmission
            lines. The fast IGBT edges (rise time ~50 ns) reflect off the motor's high impedance and
            double the voltage at the motor terminals — peak voltage can hit 1200 V on a 400 V
            drive. Insulation breaks down. Solution: dV/dt filter at the drive output, or sine
            filter for very long runs, or use motor cable rated for inverter duty (BS EN 50525 'INV'
            rating).
          </p>
        </ConceptBlock>
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>Star-delta</strong> — switches windings from star (start, 1/3 V per winding)
              to delta (run, full V) when motor near full speed. Cuts inrush ~3×. Used for medium
              motors 7.5-100 kW.
            </li>
            <li>
              <strong>Soft-starter</strong> — thyristor controller phases voltage on smoothly over
              5-30 seconds. Smooth start, low inrush. Bypassed when at speed. Common 5-200 kW.
            </li>
            <li>
              <strong>VFD</strong> — full electronic control of frequency and voltage. Smooth start
              with no inrush. Variable-speed in service. Best efficiency on variable-flow pumps and
              fans. Cost premium 2-4× DOL.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>The dangerous moment is the changeover.</strong> Open the star contactor, wait
            ~50 ms (the changeover dead time, set in the timer relay), then close the delta
            contactor. If the gap is too short, the star and delta contactors might briefly both be
            closed — short-circuit between phases through the windings, big arc, blown fuses. If the
            gap is too long, the motor decelerates and the inrush when delta finally closes is
            nearly as big as a fresh DOL start.
          </p>
          <p>
            <strong>Mechanical interlocks</strong> between star and delta contactors prevent both
            being closed simultaneously even if the timer relay fails — a steel mechanical link
            physically blocks one when the other is closed. Standard supply on quality starters from
            Schneider, ABB, Siemens; verify it's there before commissioning.
          </p>
          <p>
            Star-delta only works on motors built for delta running on the supply voltage. A 400 V
            delta / 690 V star motor on a 400 V supply must be wired delta for normal running —
            star-delta starting is then valid. The same motor on a 690 V supply must be wired star —
            and star-delta starting cannot be used. On a 230 V Δ / 400 V Y motor on 400 V, the motor
            is permanently star-connected — no star-delta possible.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            That last point is a real limit. Soft-starters cannot deliver more starting torque than
            DOL — they can only reduce it (and the inrush). For a high-inertia load like a packed
            conveyor or a centrifugal compressor, the soft-starter has to ramp slowly enough not to
            stall, and the cumulative I²t through the motor windings during the long ramp can exceed
            a DOL inrush. Always verify by motor manufacturer's allowed start times.
          </p>
          <p>
            Once the motor reaches full speed and full voltage, the thyristors are bypassed by an
            internal contactor. Otherwise their continuous voltage drop (~1.5 V per leg, so 4.5 V ×
            √3 = 8 V line-to-line) wastes power and produces continuous harmonic current. The bypass
            contactor saves both energy and harmonic distortion in steady state.
          </p>
          <p>
            Soft-starters also offer "soft stop" — ramping voltage down on shutdown to gently slow
            the motor and prevent water-hammer in pumps or shock loads in conveyors. Better than DOL
            stop, especially for lifts and large pumps. But a true four-quadrant ride (acceleration
            AND regenerative braking) is only possible with a VFD.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Type AC RCDs are now restricted to a narrow set of pure-sinusoidal AC loads. Anything
              with internal rectification — VFDs, EV chargers, modern PV inverters, ECM pumps, LED
              drivers — produces DC components and CANNOT be protected by Type AC. Use Type A (AC +
              pulsed DC) for most modern installations, and Type B (full DC + AC + HF) for VFD-fed
              motor circuits and EV charging points.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The contactor / starter must include thermal overload protection set close to FLA —
              usually 1.05–1.15× FLA on a thermal overload relay, or set in the VFD parameters as
              motor protection. Soft-starters and VFDs typically include electronic overload as
              standard; with a plain DOL contactor you need a separate thermal overload relay
              downstream.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A VFD's switching IGBTs put kilohertz-rate dV/dt onto the motor cable — spectacular HF
              emission unless contained. Reg 444.4.2.1 mandates the EMI measures that are routine in
              VFD installs: screened motor cable with 360° screen termination at both ends, separate
              motor and signal cable trays, ferrite cores on the supply side. These are the
              regulation answers to the EMC Regulations 2016 referenced from Reg 523.1.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              VFD-fed motor circuit. Type AC 30 mA RCD installed. The VFD's DC leakage component
              "blinds" the RCD core — it stops detecting AC leakage. A real fault no longer trips.
              Could leave a dangerous condition undetected.
            </>
          }
          doInstead={
            <>
              VFD circuits ALWAYS use Type B RCDs per §531.3.3. Type B detects pure DC, AC, pulsed
              DC and high-frequency leakage. More expensive (~3× Type A) but the only safe choice.
              Read the VFD data sheet for the manufacturer's recommended RCD type.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              15 kW VFD, 80 m motor cable. Built-in EMC filter leaks ~25 mA to earth at HF. 30 mA
              Type B RCD trips intermittently — sometimes immediately, sometimes after hours.
              Customer loses production every time.
            </>
          }
          doInstead={
            <>
              For VFD circuits with significant cable length or built-in EMC filter, size the Type B
              RCD at 100 mA or 300 mA where the application allows (not for life protection of
              socket-outlets, but for fault protection upstream). Or use a 30 mA Type B RCD with the
              EMC filter capacitor downstream of it where geometry permits.
            </>
          }
        />
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Shaft grounding ring</strong> — a brush of conductive carbon fibres riding on
              the shaft, gives the high-frequency current a direct path to earth bypassing the
              bearings (Aegis SGR is the dominant brand).
            </li>
            <li>
            ...
            <li>
              <strong>dV/dt or sine filter</strong> at the drive output to slow the edge rates and
              reduce the common-mode capacitive current in the first place.
            </li>
            <li>
              <strong>Properly bonded shielded motor cable</strong> with both ends terminated 360°
              to the metalwork.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Scenario
          ...
          situation={
            <>
              Customer wants a 22 kW supply and exhaust fan in a kitchen. Variable speed required (4
              settings). Existing supply 100 A 3-phase. Considering DOL+contactor for cost vs VFD
              for variable speed.
            </>
          }
          whatToDo={
            <>
              Steady-state FLA: 22 / 0.92 / (1.732 × 0.4 × 0.86) = 40 A. DOL inrush: 6 × 40 = 240 A
              briefly. Customer's 100 A supply can handle this.
              <br />
              BUT: customer wants 4 speeds. DOL = on/off only. Star-delta only gives 2 speeds (start
              + full). VFD gives infinite speed control plus 25-40 % energy saving on average duty
              cycle.
              <br />
              ...
              <br />
              Add motor overload via VFD parameters per §552.1.2. Restart prevention via VFD "no
              auto-restart" parameter per §552.1.3.
            </>
          }
          whyItMatters={
            <>
              Variable-flow loads (fans, pumps) save more energy with VFD than the VFD costs to buy.
              Plus the customer gets the speed control they actually wanted, and the regulatory
              requirements (RCD type, overload, restart) all flow through the same drive parameters.
            </>
          }
        />
