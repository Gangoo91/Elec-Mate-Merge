...
const TITLE =
  'Three-phase induction motors, slip and torque | Level 3 Module 3.5.3 (AC 3.2, 3.3) | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  {
    ...
    options: ['750 rpm', '6000 rpm', '3000 rpm', '1500 rpm'],
    correctIndex: 3,
...
const quizQuestions = [
  {
    ...
    options: ['120/P × f', '120 × f / P', 'P × f', 'f / P'],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: ['1500 rpm', '1425 rpm', '1500 × 1.05 rpm', '1575 rpm'],
    correctAnswer: 1,
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          N_s = 120f/P. Slip = (N_s − N) / N_s. Squirrel-cage and wound-rotor types. Torque-speed
          curves and starting current.
        </p>
        ...
        >
          <p>
            <strong>N_s = 120f / P</strong> (rpm), where P = number of poles (always even). For 50
            Hz UK supply:
          </p>
        ...
        <VideoCard
          ...
          caption={
            <>
              The Engineering Mindset cuts through a squirrel-cage induction motor and animates the
              rotating stator field, the induced rotor currents and the slip that produces torque.
              Pairs directly with the N_s and slip equations above.
            </>
          }
        />
        ...
        >
          <p>
            ...
            offset spatially); B_c(t) = B_max cos(ωt − 240°) along axis C. Sum these three vectors
            at every instant and the resultant is a vector of magnitude (3/2)B_max rotating at
            angular frequency ω. The field is genuinely rotating — not pulsating.
          </p>
          <p>
            <strong>Reverse the rotation:</strong> swap any two of the three line connections (e.g.
            L1 ↔ L2). The phase sequence becomes A-C-B instead of A-B-C, and the resultant rotates
            the other way. That's why every three-phase motor reverses by swapping any two phases at
            the terminal box.
          </p>
        </ConceptBlock>
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Squirrel-cage</strong> — aluminium or copper bars cast into rotor laminations,
              joined by end rings. Robust, almost no maintenance. Cheap. Performance fixed by design
              — can't adjust externally.
            </li>
            <li>
              <strong>Wound-rotor (slip-ring)</strong> — 3-phase rotor winding terminated at slip
              rings; external resistance can be inserted at start (high R = high starting torque,
              low inrush) and progressively shorted as motor accelerates. More expensive, brushes
              need maintenance.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>
            The voltage across each phase winding differs by √3 between the two configurations. In{' '}
            <strong>star</strong>, line voltage 400 V appears across two windings in series via the
            neutral point — each winding sees 400/√3 = 230 V. In <strong>delta</strong>, line
            voltage 400 V appears directly across each single winding. So a winding designed for 230
            V can be wired star on a 400 V supply; the same winding can be wired delta on a 230 V
            three-phase supply.
          </p>
          <p>
            ...
            <strong>star</strong> sees only 230 V per winding, develops only 1/3 the torque and
            draws only 1/3 the current — which is the core idea behind star-delta starting (Sub
            5.4): start in star to limit inrush, then switch to delta for full operation.
          </p>
          <p>
            <strong>Read the rating plate carefully.</strong> A plate marked "Δ 400 V / Y 690 V"
            means delta-connect on a 400 V supply; do NOT star-connect on 400 V (each winding then
            sees only 230 V — motor will run slow and underpowered). Plates marked "Y 400 V / Δ 230
            V" go the other way. Wrong jumper position = wrong torque, wrong current, possibly burnt
            windings.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The fix on a per-motor basis is correctly sizing the motor to the load. A 22 kW motor
            driving a 5 kW load is a permanent power factor disaster; a 7.5 kW motor doing the same
            job at ~70 % load runs at near-nameplate efficiency and pf. Where the motor must be
            oversized for occasional surge loads, fit centrally-controlled power factor correction
            capacitors on the main switchboard — typical kVAr rating ≈ 1/3 of the connected motor
            kVA.
          </p>
          <p>
            <strong>VFDs change the picture.</strong> A VFD-fed motor presents close to unity power
            factor to the supply at the input rectifier (because the DC bus capacitor naturally
            fills in the gaps), but draws non-sinusoidal current with significant harmonic content.
            Different problem, different fix (line reactor, harmonic filter, active front end).
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>V (Δ / Y)</strong> — voltages for each connection. Pick the one matching your
              supply.
            </li>
            <li>
              <strong>A (FLA, full-load amps)</strong> — corresponding line current at rated kW.
              Sets cable rating and overload setting.
            </li>
            <li>
            ...
            <li>
              <strong>rpm</strong> — full-load speed (NOT synchronous). Tells you pole count: rpm
              2880 ≈ 2-pole, 1440 ≈ 4-pole, 960 ≈ 6-pole.
            </li>
            <li>
              <strong>cos φ</strong> — full-load power factor. Used in I = P / (√3 × V × cos φ × η).
            </li>
            <li>
              <strong>η or IE class</strong> — full-load efficiency or its IEC class (IE2, IE3, IE4
              — higher is better, IE3 is UK minimum since 2017 for most ratings).
            </li>
            <li>
            ...
            <li>
              <strong>IP rating</strong> — IP55 = dust protected + low-pressure jets, common for
              industrial sites. IP56 for washdown.
            </li>
            <li>
              <strong>Insulation class</strong> — F (155 °C max winding temp) or H (180 °C). Sets
              the temperature derating and overload margin.
            </li>
          </ul>
          <p>
            Every figure on the plate connects to a calculation in BS 7671 design — cable selection,
            MCB rating, contactor utilisation category, RCD type, overload relay setting. Get the
            plate wrong and the rest cascades into either a too-small or too-big install, both
            expensive.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Cable, contactor, MCB and any switching device on the motor circuit must carry the FLA
              continuously, AND must withstand the cumulative thermal effect of repeated starts on
              intermittent-duty motors. Type B MCBs trip too fast on motor inrush; Type C (5-10×
              rated) and Type D (10-20× rated) ride through inrush. Soft-starters and VFDs let you
              use Type B safely.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Motors must not auto-restart after a supply dip — the operator could be working on the
              load, having assumed the motor stopped for safety. Implement with a latching-type
              contactor circuit (start button latches via NO auxiliary, stop button breaks the
              latch) rather than direct switching. Where auto-restart is deliberately required (some
              pumps, fans), the risk assessment must justify it and the design must provide other
              safeguards.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Three-phase induction motors driving conveyors, pumps and fans all need a lockable
              local isolator at the machine. Reg 132.15.202 is the regulatory anchor for the
              "isolator within sight" rule that lets a maintenance operative physically lock-off,
              padlock and tag the supply. Distance switching from a control room never substitutes
              for the local isolator — both are needed.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              11 kW motor, FLA 21 A. Apprentice fits a 25 A Type B MCB. Motor inrush 6 × 21 = 126 A
              → above Type B 3-5× threshold (75-125 A) → MCB trips on every start.
            </>
          }
          doInstead={
            <>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Motor plate says 7.5 kW S3 25 % — meaning intermittent duty, 25 % of cycle running.
              Apprentice sizes the cable for steady FLA only. After a few weeks of heavy
              intermittent operation, the cable insulation has overheated and the contactor coil is
              browning.
            </>
          }
          doInstead={
            <>
              ...
              cumulative thermal effect of frequent starts can exceed steady-state FLA. Apply the
              correct rating factor per BS 7671 §552.1.1 and the manufacturer's data sheet.
            </>
          }
        />
        ...
        >
          <p>
            <strong>Why this matters:</strong> rotor reactance X_r = 2π f_r L_r. At standstill, X_r
            is huge — rotor current is limited by the reactance, lags voltage badly, and the
            resulting torque per amp is poor. As the rotor speeds up, f_r drops, X_r drops, the
            rotor circuit becomes more resistive than reactive, current comes into phase with rotor
            EMF, and torque per amp rises sharply.
          </p>
          <p>
            This is why the torque-speed curve has the shape it does. From standstill, torque
            actually rises slightly as slip falls from 1 toward the breakdown slip (~20 %) — peak
            torque — then falls quickly as slip falls further toward operating slip. Beyond
            breakdown, more load means more slip means LESS torque, and the motor stalls back to
            zero speed in fractions of a second.
          </p>
          <p>
            <strong>Diagnostic use:</strong> a stroboscope flashing at exactly 50 Hz freezes a
            stationary mark on a 2-pole rotor running at exactly 3000 rpm (impossible — needs slip).
            Setting it to 49 Hz freezes the mark on a rotor running at ~2940 rpm (slip 2 %).
            Engineers use this to measure slip without intruding on the running machine.
          </p>
        </ConceptBlock>
        ...
        <Scenario
          ...
          situation={
            <>
              Motor: 18.5 kW (output), 4-pole, 400 V 3-phase, η 92 %, pf 0.86. DOL start. What FLA,
              supply cable, and MCB size?
            </>
          }
          whatToDo={
            <>
              ...
              <br />
              Add overload relay set to 1.05 × FLA for thermal protection per §552.1.2. Contactor
              rated AC-3 ≥ 40 A.
              <br />
              Restart prevention per §552.1.3 — latching contactor circuit with stop button breaking
              the seal-in path.
            </>
          }
          whyItMatters={
            <>
              ...
              selection — is the daily 3-phase motor design routine. Each step uses standards from
              this section and §3.
            </>
          }
        />
