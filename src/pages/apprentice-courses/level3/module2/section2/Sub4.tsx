...
const TITLE = 'ENA G98 / G99 grid notification deeper (2.4) | Level 3 Module 2.2.4 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  {
    id: 1,
    question: 'Who notifies the DNO under G98, and when?',
    options: [
  ...
  {
    id: 2,
    question: 'What is the practical timeline for a G99 connection compared with a G98?',
    options: [
  ...
  {
    id: 3,
    question: 'Why does the inverter have a type-test certificate and what is it for?',
    options: [
  ...
  {
    id: 5,
    question: 'Who is the DNO and how does the L3 apprentice know which DNO covers the install?',
    options: [
  ...
  {
    id: 6,
    question: 'When does an installer need to apply for G100 instead of G98 / G99?',
    options: [
  ...
  {
    id: 7,
    question: 'What is the practical commissioning sequence for a G98 PV install?',
    options: [
  ...
  {
    id: 8,
    question: 'What goes in the G98 install pack the customer should keep for life?',
    options: [
...
const faqs = [
  ...
  {
    question:
      'What if the customer adds a generator without telling anyone and the EICR picks it up?',
    answer:
...
export default function Sub4() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 2 · Section 2 · Subsection 4"
        title="ENA G98 / G99 grid notification deeper"
        backTo="/study-centre/apprentice/level3-module2-section2"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Every grid-connected generator in the UK passes through the ENA framework. G98 fast-track
          up to 16 A per phase, G99 pre-application above. The L3 apprentice should recognise the
          documentation chain, the timeline implications and where the inverter type-test
          certificate fits in.
        </p>
        ...
        >
          <p>The two documents in plain terms:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>G98 — fast-track</strong>. Single inverter up to 16 A per phase (3.68 kW
              single-phase, 11 kW three-phase). Install first, notify the DNO within 28 days.
              Allowed because the inverter holds a type-test certificate proving its protection
              settings meet G98.
            </li>
            <li>
              <strong>G99 — pre-application</strong>. Above the G98 cap, multi-inverter systems, all
              three-phase commercial, every second generator at a site that already has G98
              generation, and any system the DNO flags as needing network modelling. Submit
              application, receive Connection Offer, accept Offer, then commission. Timeline 4-12
              weeks domestic, longer commercial.
            </li>
            <li>
              <strong>G100 — active export limitation</strong>. A technical specification inside G99
              that allows oversized inverters to connect on networks that cannot accept their full
              export, by capping export with a CT clamp and limiter. Lets a 12 kWp array with
              battery sit on a network that would otherwise refuse 12 kW export.
            </li>
          </ul>
          <p>
            All three documents are downloadable from the ENA website. The MCS-certified designer
            reads the live editions; the L3 apprentice should recognise the names and the role each
            plays in the install map.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              G98 sets out the fit-and-notify route for micro-generators. The installer notifies the
              DNO within 28 days of commissioning. The inverter must hold a current type-test
              certificate evidencing compliance with the G98 protection settings (over-voltage,
              under-voltage, over-frequency, under-frequency, loss-of-mains, trip times, recovery
              delay).
            </>
          }
          meaning={
            <>
              G98 is the document behind almost every domestic single-phase PV install in the UK.
              The L3 apprentice meets it indirectly — the MCS-certified installer files the form,
              but the type-test certificate sits in the install pack and the apprentice should
              recognise it as the technical foundation of the connection. ENA G98 has been updated
              through Issue 6 to align with EU EN 50549; check the live version on the ENA website.
            </>
          }
        ...
        >
          <p>Worked examples — which document applies?</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>3.68 kW single-phase PV inverter on a new install</strong> — G98. Fast-track.
              Notify the DNO within 28 days of commissioning.
            </li>
            <li>
              <strong>5 kW single-phase PV inverter (single)</strong> — G99. Above the 16 A per
              phase ceiling. Pre-application required. Wait for Connection Offer before
              commissioning.
            </li>
            <li>
              <strong>
                3.68 kW PV inverter PLUS a 5 kW battery inverter capable of grid export, same site
              </strong>{' '}
              — G99. The site now has more than one generator. G98 fast-track only allows one.
              Pre-application required.
            </li>
            <li>
              <strong>11 kW three-phase PV inverter (16 A on each of three phases)</strong> — at or
              just under G98 ceiling, depending on inverter rated current. Read the data sheet
              carefully; many DNOs treat any three-phase generator as G99 by policy regardless of
              capacity.
            </li>
            <li>
              <strong>
                Customer adds a 3.68 kW PV system at a property that already has a G98-registered
                3.68 kW PV system
              </strong>{' '}
              — G99. Second generator triggers G99 even if both are individually under the cap.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Why the DNO insists on this:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              A planned DNO outage to repair a substation should leave the local feeders dead. If a
              customer-owned inverter is still energising the local LV cable, the line workers can
              be killed.
            </li>
            <li>
              A fault on a feeder should self-clear under DNO protection. An islanded customer
              inverter prolongs the fault, damages equipment, and can re-ignite arcs that the
              upstream protection thought it had cleared.
            </li>
            <li>
              Reconnection of a trip needs the local network to be fully de-energised before
              re-closing. Islanded inverters block this and can damage equipment during
              reconnection.
            </li>
          </ul>
          <p>
            The trip thresholds vary slightly between G98 / G99 / G100 and across jurisdictions but
            typically include over-voltage above ~1.10 p.u., under-voltage below ~0.85 p.u.,
            over-frequency above 51-52 Hz, under-frequency below 47-48 Hz, with disconnection times
            in the order of 0.2 to 2 seconds depending on the trip category. ROCOF detection
        ...
        <RegsCallout
          ...
          clause={
            <>
              G99 sets out the application-and-offer process. The Connection Offer issued by the DNO
              contains the agreed export capacity, any export limitation requirement (under G100),
              the protection settings (loss-of-mains method and thresholds), reactive power
              capability requirements, and the network charging implications. Generation equipment
              must hold a current type-test certificate aligned to the EU EN 50549 / EREC G99
              protection schedule.
            </>
          }
          meaning={
            <>
              G99 is the document the customer project sits inside whenever the system exceeds G98
              fast-track or whenever the DNO requires pre-application. The L3 apprentice interface
              is recognising that the Connection Offer governs commissioning — you cannot energise
              the inverter on the AC side before the Offer is in place. On a fast-moving install
              programme, the G99 timeline is often the gating factor and the MCS designer should
              warn the customer up front.
            </>
          }
        ...
        >
          <p>Common reasons G99 takes longer than expected:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Network constraint area</strong> — the local LV network is at or near voltage
              / thermal capacity. The DNO needs to model whether your install can be accommodated
              and whether export limitation under G100 is needed.
            </li>
            <li>
              <strong>Other applications in queue</strong> — the DNO processes applications in
              order. Busy areas (parts of the South West with high PV density, parts of Scotland
              with rural single-phase networks) have multi-month queues.
            </li>
            <li>
              <strong>Three-phase upgrades</strong> — if the application requires the customer
              supply to be upgraded from single-phase to three-phase, the DNO quotes the upgrade
              work separately and the timeline includes scheduling the physical upgrade.
            </li>
            <li>
              <strong>Application errors</strong> — incomplete site address, missing inverter
              type-test certificate reference, incorrect MPAN. The DNO returns the application for
              correction and the clock restarts. Submit a complete application first time.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>What the certificate records — at apprentice level:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Inverter model and firmware version</strong>. Different firmware can change
              the protection behaviour — the certificate matches a specific version. Field-flashing
              firmware on a commissioned inverter without checking certificate validity is a
              regulatory pitfall.
            </li>
            <li>
              <strong>Protection thresholds</strong>. Over-voltage, under-voltage, over-frequency,
              under-frequency, ROCOF or vector shift settings, plus trip times.
            </li>
            <li>
              <strong>Connection categories</strong>. The certificate states the EREC document(s)
              the inverter is approved against — G98 only, G99 only, both, EN 50549. Match the
              certificate to the application route.
            </li>
            <li>
              <strong>Country of test</strong>. EU EN 50549 type-tests are accepted in the UK under
              post-Brexit equivalence, but the DNO can ask for a UK-specific G98 / G99 certificate.
              The MCS designer confirms acceptance.
            </li>
            <li>
              <strong>Validity period</strong>. Certificates have a validity. Check the expiry date
              before each install if you handle older stock.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>What you can expect to see on a 2026-aligned install pack:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Generator notice at the consumer unit</strong> — durable label identifying
              that the property has a parallel generator, signposting to isolation points. Reg
              514.10 (warning notice — alternative supply) drives this.
            </li>
            <li>
              <strong>DC isolator labelling</strong> — clearly marked DC isolators with voltage
              rating, polarity warning and instruction not to operate under load. Section 712
              redraft reinforces.
            </li>
            <li>
              <strong>Reg 551.7 parallel-operation references</strong> — the install pack cites Reg
              551.7 alongside G98 / G99 because Reg 551.7 added requirements for generating sets
              operating in parallel with the public network.
            </li>
            <li>
              <strong>Battery storage as energy source</strong> — battery installs now treated as
              parallel generation under both BS 7671 and ENA G98 / G99, not as load-side kit. Pack
              should reflect that with explicit isolation and labelling for the battery DC and AC
              sides.
            </li>
            <li>
              <strong>EV bi-directional charging readiness</strong> — Section 722 in A4:2026
              acknowledges V2G / V2H bi-directional units; pack should flag whether the EV charger
              is bi-directional and, if so, treat as ENA-notifiable generation.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The six GB DNO regions, plus distribution operators in NI:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>UK Power Networks (UKPN)</strong> — South East England, East of England,
              London.
            </li>
            <li>
            ...
            <li>
              <strong>SP Energy Networks</strong> — Central and Southern Scotland, Merseyside,
            ...
            <li>
              <strong>National Grid Electricity Distribution</strong> — Midlands, South Wales, South
              West England (formerly Western Power Distribution).
            </li>
            <li>
              <strong>SSEN (Scottish and Southern Electricity Networks)</strong> — Northern Scotland
              and Central Southern England.
            </li>
            <li>
              <strong>Northern Ireland Electricity Networks (NIE)</strong> — Northern Ireland
              (separate framework, not GB G98 / G99).
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              ESQCR 2002 places statutory obligations on distributors and on anyone connecting
              generation equipment in parallel with the distributor network. Connection without
              proper notification and without protection arrangements compliant with the distributor
              published technical requirements is a breach of the regulations.
            </>
          }
          meaning={
            <>
              ESQCR 2002 is the statutory backbone behind G98 / G99. The Engineering Recommendations
              are the technical implementation; ESQCR is what makes connection without notification
              an offence rather than just a paperwork nicety. The L3 apprentice does not need to
              cite ESQCR clause numbers, but should recognise that ignoring G98 / G99 is not just
              bad practice — it is statutory non-compliance.
            </>
          }
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Customer payback is unchanged for energy used on site — the export limit only caps
              what goes to grid. Self-consumption is unaffected.
            </li>
            <li>
              Smart Export Guarantee tariff payments are limited by the export cap. A customer with
              a 12 kWp array on a 3.68 kW G100 export limit will see much of their summer surplus
              self-consumed (battery, EV, hot water diverter) rather than exported.
            </li>
            <li>
              The CT clamp must be correctly sized and oriented. Reverse-fitted CTs (the arrow
              pointing the wrong way) cause the inverter to read import as export and clip output
              unnecessarily. Common commissioning error.
            </li>
            <li>
              The Connection Offer states the CT type and any specific data-cable requirements.
              Wireless CT-to-inverter links exist but are less reliable than a hardwired pair on a
              long property.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Customer wants a 5 kW PV inverter (above the G98 cap). Sales rep quotes a one-week
              install timeline. Survey happens, design happens, install kit lands on site — and the
              G99 application has not been filed. The customer is now waiting 4-12 weeks for the DNO
              to issue a Connection Offer. The kit is in their garage, the firm has been paid a
              deposit, and the customer is angry because they were never told. Worst case the firm
              tries to commission anyway without an Offer in place — that is a regulatory breach and
              an ESQCR offence.
            </>
          }
          doInstead={
            <>
              File the G99 application as the first step after the customer signs the design
              proposal — before kit is ordered. Tell the customer that the G99 timeline (typically
              4-12 weeks domestic) is outside the firm control. Schedule install only after the
              Connection Offer has been issued and accepted. As the L3 apprentice on site, do not
              commit to commissioning dates until you have seen the accepted Offer.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Customer rings up: I want to add a battery to my existing solar. Installer turns up,
              fits an AC-coupled battery inverter, commissions the system, hands over. No new DNO
              notification. The site now has two generators on a G98 framework that only allows one.
              The customer home insurer may decline cover; the next EICR will pick up the unnotified
              second generator; the DNO can require remedial G99 paperwork retrospectively (and may
              fine the customer for the non-compliant period).
            </>
          }
          doInstead={
            <>
              Treat any addition of a second generator at an existing G98 site as a G99 trigger.
              File the G99 application before commissioning the battery. The MCS-certified designer
              reads the battery inverter data sheet to confirm whether it is grid-export-capable; if
              it is, G99 applies. As the L3 apprentice the test is simple — does the battery
              inverter have an AC export rating in its specs? If yes, it is a generator, and the
              site is now a multi-generator site under ENA framework.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You are the apprentice on a service van. The firm office sends you to a domestic
              property — the customer has an existing 3.68 kW PV system installed five years ago
              under G98 fast-track. They have bought a 5 kWh battery from a high-street retailer and
              want it fitted alongside the PV. They expect a one-day visit. The PV inverter is
              grid-tied only (no battery support); the battery is an AC-coupled hybrid inverter with
              full grid export capability.
            </>
          }
          whatToDo={
            <>
              Stop. Ring the office and explain. The site has G98 generation and this addition is a
              second grid-export-capable inverter, which is outside G98 scope. The MCS-certified
              designer needs to file a G99 application with the DNO before the battery can be
              commissioned to grid-export. The kit can be fitted physically — cabling, isolators,
              bonding, controls — but the AC export side stays disabled until the G99 Connection
              Offer is issued. Tell the customer politely: the kit will be installed; the export
              will be enabled the day the DNO Offer comes through; the timeline is typically 4-12
              weeks; the firm is doing this because the alternative is an unsafe, unregistered
              second generator that the customer home insurer would not cover.
            </>
          }
          whyItMatters={
            <>
              A lot of homeowners assume add a battery is a quick upgrade. The ENA framework treats
              it as a new generator addition and the paperwork follows accordingly. As the L3
              apprentice you do not run the G99 process, but you should recognise the trigger and
              not commit the firm to commissioning the export side before the Offer is in place.
              Your professional credibility hangs on getting that conversation right.
            </>
          }
        />
