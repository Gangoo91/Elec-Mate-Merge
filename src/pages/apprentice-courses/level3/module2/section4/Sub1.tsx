...
const checks = [
  {
    ...
    correctIndex: 3,
    explanation:
      "The MCS-vs-legal-requirement distinction is one of the most commonly muddled points on environmental tech. MCS is a competence and quality scheme, not a regulator — Building Regulations Part L is the legal framework that applies regardless of MCS, and Part L can be demonstrated by various pathways. MCS enforcement comes through funding gateways (BUS grant currently £7,500 toward an ASHP, SEG export tariff) and through manufacturer warranty conditions. So in practice almost every install is MCS even though it isn't a legal install requirement: without MCS the customer can have a working heat pump, they just don't get the grant or the warranty. The pragmatic answer is usually 'go MCS' but the legal answer is 'Part L is the legal floor; MCS is the funding gateway'.",
  },
  {
    ...
    correctIndex: 3,
    explanation:
      "Future Homes Standard is the regulatory mechanism that will normalise heat pumps + PV + MVHR as the default new-build kit. In practice it effectively rules out fossil-fuel boilers in new-build (heat pump or hydrogen-ready boiler instead), pushes higher fabric standards, and credits PV / MVHR / smart controls in the SAP calculation. It sits downstream of the Climate Change Act 2008 (as amended for the 2050 net-zero duty). The phased timeline reflects implementation lead-times for the supply chain. As the L3 electrician you'll see Future Homes Standard new-build sites where the integration of heat pump + PV + MVHR + smart controls is the routine kit, not the exotic add-on.",
  },
  {
    ...
    correctIndex: 0,
    explanation:
      "The BUS grant is the current main UK government incentive for low-carbon heating retrofits. It replaced the Renewable Heat Incentive (RHI) which closed in 2022. The grant is paid to the installer, who passes it through to the customer as a price reduction; without MCS sign-off the customer can't claim it. As the L3 electrician on the install you carry out the electrical work to the certified installer's design, complete the BS 7671 inspection-and-testing, and contribute to the customer handover pack — you support the MCS process but don't substitute for it unless you are personally MCS-certified for heat pumps.",
  },
];
...
const quizQuestions = [
  {
    ...
    correctAnswer: 0,
    explanation:
      "Part L is the energy-efficiency regulator for buildings. It applies to new dwellings, extensions, replacement boilers, replacement windows, controlled fittings and major refurbishments, regardless of whether the customer 'wants' to install environmental tech — the SAP / SBEM target rate has to be met to get Building Regs sign-off. PV adds generation credits to the SAP score, heat pumps reduce the regulated carbon emissions, MVHR reduces ventilation losses; successive Part L revisions have tightened the target rate, so environmental tech is increasingly the routine route to compliance rather than an optional add-on.",
  },
  {
    id: 2,
    question:
    ...
    correctAnswer: 2,
    explanation:
      "SAP is the calculation engine behind Part L compliance for dwellings (SBEM is the equivalent for non-domestic). It produces a SAP rating (1-100+) and a regulated CO₂ emissions figure that must beat the Target Emission Rate (TER); PV, heat pumps, MVHR, smart controls and fabric measures all feed in. As the L3 electrician you don't run the SAP yourself, but the MCS-certified installer's design pack includes the system's contribution to the SAP score, and that contribution is what gets the building Building Regs sign-off.",
  },
  {
    id: 3,
    question:
    ...
    correctAnswer: 3,
    explanation:
      "The MCS Code is the consumer-protection framework for the certified-installer industry — sales practices, contracts, performance estimates honestly disclosed. It sits alongside the technology-specific MCS Installation Standards (MIS 3001 solar thermal, 3002 PV, 3003 wind, 3004 biomass boiler, 3005 heat pump, 3006 biomass stove, 3007 EV, 3008 hydro, 3012 battery storage) which carry the technical detail. As an apprentice on an MCS-certified install you don't sign off the Code yourself, but customer-facing conversations, performance estimates and handover documentation all sit within its framework.",
  },
  {
    id: 4,
    question:
  ...
  },
  {
    ...
    correctAnswer: 1,
    explanation:
      "BUS is the de facto subsidy that makes heat-pump retrofit financially competitive with fossil-fuel replacement for many UK households. Eligible properties are existing dwellings (not new-build) with a valid EPC and no outstanding insulation recommendations; the grant has been extended several times and is currently confirmed through the late 2020s. The MCS sign-off is the gateway. As the L3 electrician on a BUS-funded install you don't claim the grant yourself but your work has to support the MCS install pack that triggers it.",
  },
  {
    id: 6,
    question:
    ...
    correctAnswer: 2,
    explanation:
      "SEG is the current export-payment mechanism for domestic PV. It replaced the Feed-in Tariff (FiT) which closed to new entrants in 2019; tariffs vary by supplier (typically 5-15p/kWh in 2026), so customers shop around, signing up with their chosen supplier (it isn't automatic). The MCS sign-off is the gateway, the smart meter is the measurement, the supplier is the payer. Customer-facing conversations should mention SEG eligibility — without it the customer's payback calculation is wrong by the export value.",
  },
  {
    id: 7,
    question:
    ...
    correctAnswer: 3,
    explanation:
      "The Code is largely historical now in England — withdrawn for new applications in March 2015 and replaced by enhanced Part L (and, for higher-rated developments, local-authority-specific sustainability requirements). You may still meet the Code referenced on older properties (a Code Level 4 or Level 5 home from 2010-2014 will have been built to Code spec). The 2357 Unit 602 syllabus references it by name (AC 1.4) because the qualification text pre-dates the withdrawal. As an apprentice in 2026 you should recognise the Code by name and explain its historical role, but you'll meet Part L (and Future Homes Standard) as the live regulators on current work.",
  },
  {
    id: 8,
    question:
