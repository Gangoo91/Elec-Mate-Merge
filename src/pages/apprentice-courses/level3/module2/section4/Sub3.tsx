...
const checks = [
  {
    ...
    options: [
      'Accept it. With twelve strings the array is large enough that the combiner busbar shares the reverse-fault current evenly, so individual string protection is never needed above two strings. The 16 mm² cable is more than adequate for an 11 A string, so the install is compliant as it stands.',
      'Reject it, but only because the cable is undersized for the running current. An 11 A string needs at least 25 mm² cable to carry its own output, so the fix is to upsize the conductor; per-string overcurrent devices are not a Section 712 requirement at any array size.',
      'Accept it, provided a single combiner-level fuse rated at 1.1 × total Isc (about 145 A) is fitted on the array output. Section 712 only requires one overcurrent device for the whole array, not per string, so a single combiner fuse satisfies the reverse-current requirement.',
  ...
  },
  {
    ...
    options: [
      'G99 — any PV array of 5 kWp or more must be treated as a large generator and requires prior DNO approval before energising. The 5 kWp figure, not the inverter current, is the deciding threshold, so this install cannot be connected until the DNO signs off the G99 application.',
      'Neither G98 nor G99 — domestic single-phase PV is exempt from DNO notification altogether. The installer simply commissions the array and registers it with the MCS database; the DNO finds out automatically through the smart meter, so no separate form is submitted.',
      'G83 — this is the current connect-and-notify recommendation for domestic micro-generation up to 16 A per phase. The installer notifies the DNO within 28 days using the G83/2 form; G98 and G99 only apply to commercial three-phase installations.',
  ...
  },
  {
    ...
    options: [
      "To protect the inverter's own power electronics. When the mains fails the grid voltage can swing wildly, and the inverter shuts down to avoid damaging its internal switching transistors. It is a self-preservation feature for the equipment rather than a network safety measure, and the array could safely keep exporting if the inverter were more robust.",
      ...
      "To stop the battery from over-discharging. When the grid drops, the inverter shuts down so it does not drain the storage battery into the property's loads. The 0.5 second figure is the battery management system's reaction time, and a system without a battery would simply continue to export through the power cut.",
      "To prevent the array from feeding the property's own circuits while they are being worked on. The inverter shuts down so that a domestic electrician isolating the consumer unit is not back-fed by the panels. It is a personal-protection feature for in-house work, not a network-side requirement, and is set by Part P rather than by G98/G99.",
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    ...
    options: [
      'Section 712 covers only stand-alone (off-grid) PV systems with no connection to the public network. Grid-tied PV is excluded and dealt with under the general requirements of Parts 1 to 6. A4:2026 narrowed the scope to remove grid-connected systems, which are now covered solely by the IET Code of Practice rather than by BS 7671.',
      ...
      "Section 712 covers all forms of renewable generation — PV, wind, micro-hydro and micro-CHP — under a single set of requirements. A4:2026 broadened it from PV-only to a general 'low-carbon generation' section, so a wind or hydro install is now certified to Section 712 in the same way as a PV array.",
      'Section 712 was withdrawn in A4:2026 and its content merged into Section 826 on energy storage systems. PV is now treated as a sub-set of storage, and there is no longer a free-standing PV section; installers cite Section 826 for both the array and any battery.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Outdoor PV electrical equipment shall have a degree of protection not less than IP65 to BS EN 60529 and need not carry any impact rating. Full weatherproof sealing to IP65 is the single outdoor requirement; mechanical impact protection is only specified for equipment installed in agricultural or industrial locations.',
      'Outdoor PV electrical equipment shall have a degree of protection not less than IP2X to BS EN 60529, the same as any indoor consumer unit. Section 712 sets no higher ingress requirement for outdoor equipment because the modules themselves shield the enclosures from direct weather.',
      ...
      'Outdoor PV electrical equipment shall have an impact rating not less than IK10 to BS EN 62262, with no ingress-protection requirement specified. Section 712 prioritises mechanical robustness against hail and debris over water ingress, so only the IK rating is mandated for outdoor enclosures.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Where an IMD is provided, it shall be selected in accordance with BS 7671 Table 41.3 and proven by a loop-impedance test at commissioning. Compliance is evidenced by the Zs reading recorded on the schedule of test results; no separate product-standard conformity is required for the device itself.',
      'Where an IMD is provided, it shall be a Type B RCD rated at 30 mA, the same device used on EV charging circuits. Compliance is evidenced by an RCD trip-time test at commissioning, recording an operating time within the BS EN 61008 limits.',
      'Where an IMD is provided, it shall be selected to BS EN 60898 as for any miniature circuit-breaker, and compliance is shown by the CE mark on the device. The IMD performs the same overcurrent role as an MCB, so the same product standard and evidence apply.',
  ...
  },
  {
    ...
    options: [
      ...
      'The maximum rated current of the automatic disconnecting device shall be 30 mA for all array sizes, the same as the additional-protection RCD on the AC side. Section 712 applies a single fixed value rather than a stepped scale, because the functional bonding device is performing residual-current protection.',
      "The maximum rated current shall be 100 A for arrays in the 50 to 100 kW band, matching the array's full short-circuit current so the device only operates under a genuine fault. The rating scales directly with the array's Isc rather than following a fixed table.",
      'There is no rated-current requirement — the functional bonding conductor simply has to be at least 16 mm² copper for any array above 50 kW. Section 712 specifies a minimum conductor size rather than a disconnecting-device rating for functional bonding.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'G99 prior-approval applies because the array is grid-connected and any export to the public network triggers the larger-system regime. The installer must submit a G99 application and wait for DNO approval before energising; the 16 A inverter rating does not bring it within the connect-and-notify route.',
      ...
      'No DNO notification is required at all for a 4 kWp domestic array. The installer simply lodges the MCS certificate, and because the system is below 16 A per phase it is exempt from both G98 and G99; the supplier picks up the export arrangement separately.',
      "G98 applies, but the homeowner must notify the DNO in advance and obtain written approval before the installer can commission. The 28-day window is for the DNO to respond to the homeowner's pre-application, not for post-commissioning notification by the installer.",
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Reverse-current protection. The function detects current trying to flow backwards from the grid into the inverter when the mains fails, and opens to protect the inverter's output stage. It is the same mechanism as a string OCPD and is set by the cable rating rather than by any grid-code window.",
      'Over-voltage protection, known as the surge function. When the mains fails, the grid voltage can spike, and the inverter disconnects to ride through the transient. The engineering term is surge-protective shutdown, governed by the let-through voltage limits in BS EN 61643.',
      ...
      'Phase-failure protection. On a three-phase supply the inverter monitors for the loss of any one phase and shuts down to avoid unbalanced export. The engineering term is phase-loss detection, and it only applies to three-phase systems; single-phase domestic inverters have no equivalent shutdown.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A standard grid-tied inverter shuts down because the panels stop producing as soon as the mains drops. The grid supply is what energises the array, so without it the modules generate nothing; the alternative is simply to add a battery, which keeps the panels powered through the cut with no other changes.',
      "A standard grid-tied inverter keeps running but trips the property's main switch to stay safe, cutting the whole house off. The fix is a larger consumer unit with a separate generation way, allowing the array to keep feeding the home through a power cut without a hybrid inverter.",
      'A standard grid-tied inverter cannot run in a power cut only because its export meter stops logging, which the supplier prohibits. The alternative is to install an islanding meter that keeps recording during the outage, after which the existing inverter can legally continue to power the home.',
  ...
  },
  {
    ...
    options: [
      ...
      "The PV metal structures shall be connected directly to a dedicated earth electrode driven beneath the array, forming a separate TT earthing arrangement isolated from the building's main earthing terminal. Bonding back to the property earth is specifically prohibited to prevent fault current entering the array.",
      "The PV metal structures shall be left unbonded and electrically isolated, relying on the modules' Class II double insulation. Bonding the frames is discouraged because it would create a lightning-attracting path, so Section 712 requires the array metalwork to float.",
      'The PV metal structures shall be bonded with a minimum 25 mm² conductor sized as a main protective bonding conductor, the same as for incoming gas and water services. The acceptance criterion is conductor cross-section alone; no continuity test is specified for the array frames.',
    ],
    correctAnswer: 0,
    explanation:
...
const faqs = [
  {
    question:
      "Section 712 keeps citing 'Uoc max' and 'Upc max' — what do these mean and why do they matter for equipment selection?",
    answer:
  ...
  {
    question:
      "How does the A4:2026 revision of Section 712 affect a PV install I'm working on now?",
    answer:
  ...
  {
    question:
      "Anti-islanding sometimes trips for no obvious reason and the customer loses generation. What's going on?",
    answer:
...
export default function Sub3() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 2 · Section 4 · Subsection 3"
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Section 712 of BS 7671:2018+A4:2026 was extensively revised — string protection routes,
          equipment voltage selection, enclosure ratings, IMDs, equipotential bonding and functional
          bonding conductor device ratings. ENA G98 and G99 set the grid-connection regime, and
          anti-islanding loss-of-mains protection keeps DNO engineers safe.
        </p>
        ...
        >
          <p>
            The A4:2026 revision touches almost every part of Section 712. The clauses you are most
            likely to meet on site:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>712.2</strong> — table of functional bonding conductor automatic disconnecting
              device ratings (1 A under 25 kW, ascending to 5 A above 250 kW).
            </li>
            <li>
              <strong>712.412</strong> — protective measure of double or reinforced insulation on
              the DC side; acceptance criterion requires intact insulation verified before handover.
            </li>
            <li>
              <strong>712.414</strong> — SELV / PELV on the DC side, with measured Ugc limited to
              120 V DC for SELV / PELV compliance.
            </li>
            <li>
              <strong>712.421</strong> — fire protection, with the acceptance criterion that no
              obvious ignition sources remain after installation.
            </li>
            <li>
              <strong>712.431 / 712.433</strong> — string protection rules and the two compliance
              routes for arrays of more than two parallel sub-arrays.
            </li>
            <li>
              <strong>712.433.103</strong> — PV array cable Iz (after correction) shall be at least
              the documented array maximum DC current.
            </li>
            <li>
              <strong>712.511</strong> series — module conformity (BS EN 61215 series), residential
              enclosures (BS EN 60670-24).
            </li>
            <li>
              <strong>712.512.102</strong> — outdoor enclosures shall meet IP44 (BS EN 60529) and
              IK07 (BS EN 62262) as a minimum.
            </li>
            <li>
              <strong>712.534.102.1</strong> — SPD selection; Type 2 SPDs acceptable where no direct
              lightning protection or where separation distance s in BS EN 62305-3 is observed.
            </li>
            <li>
              <strong>712.537.2.2.104 / .105</strong> — measures to prevent DC on-load interruption,
              and remote operation of devices for opening operation in combiner boxes.
            </li>
            <li>
              <strong>712.538.101</strong> — IMD selection per BS EN 61557-8.
            </li>
            <li>
              <strong>712.542.101</strong> — equipotential bonding of PV metal structures with
              continuity to earthing terminal demonstrable by test.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              <p className="mb-2">Acceptance criterion:</p>
              <p>
                &quot;For acceptance on site, the enclosure shall be labelled or accompanied by
                manufacturer documentation showing a degree of protection not less than IP44 (BS EN
                60529) and impact rating not less than 1K07 (BS EN 62262). Absence of such evidence
                constitutes non-compliance with Regulation 712.512.102.&quot;
              </p>
            </>
          }
          meaning={
            <>
              IP44 (splash protection plus protection against tools and small objects 1 mm and over)
              and IK07 (a 2 J impact rating) is the floor for outdoor PV equipment. The acceptance
              criterion is documentary — labelling on the enclosure or manufacturer paperwork must
              evidence both ratings. Verbal assurance is not acceptance. A verifier confronted with
              neither shall record non-compliance under Section 712.
            </>
          }
        ...
        >
          <p>
            The numbers, made concrete with a worked example. Suppose N = 6 parallel strings each
            with Isc = 11 A, common DC bus, no string OCPDs:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Route (a) requirement</strong> — each string cable Iz at least (N − 1) times
              Isc max = 5 × 11 = 55 A continuous. That is well above the normal 11 A operating
              current of one string and forces a much larger cable than the operating current alone
              would suggest.
            </li>
            <li>
              <strong>Route (b) requirement</strong> — fit per-string OCPDs between 1.1 × Isc max =
              12.1 A and the cable Iz. A 15 A or 16 A PV string fuse (gPV) sits comfortably in that
              band and lets you size the cable to its normal operating current plus margin, not to
              the back-fed worst case.
            </li>
            <li>
              <strong>The 1.1 coefficient is not fixed</strong> — Reg 712.433.102(b)(ii) notes it
              shall be adapted upward for special module technologies or reflective conditions
              (snow, water, rooftop reflective surfaces). The designer documents the chosen
              coefficient and the rationale.
            </li>
            <li>
              <strong>Cable Iz includes all derate factors</strong> — temperature, grouping,
              installation method. Reg 712.433.103 acceptance criterion is that recorded Iz
              (corrected) is at least the recorded PV array maximum DC current.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              &quot;For PV arrays with N greater than 2 parallel sub-arrays, compliance is achieved
              by meeting one of two acceptance criteria from Reg. 712.433.102(b): (a) no OCPD and
              continuous cable current-carrying capacity Iz is at least (N − 1) times Isc max, or
              (b) fit OCPDs with rated current I satisfying 1.1 times Isc max less than I and I at
              most Iz. Either criterion shall be used to demonstrate compliance.&quot;
            </>
          }
          meaning={
            <>
              The two-route framing is deliberate — designers have a genuine choice between heavier
              cable with no devices, or lighter cable with per-string OCPDs. Acceptance on site
              requires evidence of which route was chosen and that the relevant numerical condition
              is met. A verifier should reject any array of N greater than 2 that meets neither
              route — that is the most common Section 712 non-compliance you will encounter on
              inspection.
            </>
          }
        ...
        >
          <p>Three other equipment-selection rules from Section 712 worth committing to memory:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>PV module conformity</strong> — Reg 712.511.101 requires modules to comply
              with the relevant electrical equipment standard. Acceptance is by documentary evidence
              of compliance with BS EN 61215 series (or equivalent). No certificate, no acceptance.
            </li>
            <li>
              <strong>Residential enclosures</strong> — Reg 712.511.103 acceptance route allows
              compliance with BS EN 60670-24 for residential premises enclosures with documented
              evidence.
            </li>
            <li>
              <strong>IMD selection</strong> — Reg 712.538.101 requires any IMD provided to be
              selected per BS EN 61557-8. Documentary evidence required for acceptance — no
              evidence, non-compliant.
            </li>
            <li>
              <strong>SPD selection</strong> — Reg 712.534.102.1 acceptance route allows Type 2 SPDs
              where direct lightning protection is not specified, or where separation distance s per
              BS EN 62305-3 is observed. Otherwise Type 1 SPDs are required.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Things that go wrong with PV array bonding and how to spot them:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Aluminium-on-stainless interface corrosion</strong> — bonding lugs bolted
              directly to aluminium rails without an anti-corrosion paste fail continuity over a few
              winters as oxide builds up. Reseal or replace.
            </li>
            <li>
              <strong>Single-point bonding on long rail runs</strong> — long aluminium rails
              sometimes rely on one bonding lug at one end, with the rail joints providing the rest
              of the path. Joints can corrode. Multiple bonding points are safer.
            </li>
            <li>
              <strong>Roof-tile clip continuity</strong> — some installation methods use tile-hook
              clips that are themselves part of the bonding path. Verify the continuity test covers
              the full route.
            </li>
            <li>
              <strong>Lightning protection coordination</strong> — if the building has a BS EN 62305
              lightning protection system, the PV bonding must coordinate with it (separation
              distance s, or bonding to the LPS earth where separation cannot be achieved).
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              Acceptance criterion: &quot;Where an IMD is provided but cannot be shown to be
              selected in accordance with BS EN 61557-8 (no documentation, marking or declaration of
              conformity), the installation does not meet the requirement of Regulation 712.538.101
              and should be treated as non-compliant until evidence of conformity is produced.&quot;
            </>
          }
          meaning={
            <>
              Insulation Monitoring Devices detect insulation degradation on the DC side before it
              becomes a hazard. Section 712 requires their use in defined configurations (the
              section itself sets when an IMD is required). Where fitted, the device shall be
              selected to BS EN 61557-8 — the product standard for IMDs. Documentary evidence of
              conformity is the acceptance test on site. Verbal assurance is not enough.
            </>
          }
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Aggregation matters</strong> — multiple G98 inverters on one site can
              aggregate above the threshold and trip the install into G99 territory. The DNO will
              tell you on application; do not assume.
            </li>
            <li>
              <strong>Type-test certificates are inverter-specific</strong> — every inverter sold
              for UK grid connection should hold a current G98 or G99 type-test certificate. The MCS
              installer references the certificate on the notification form. No certificate, no
              compliant connection.
            </li>
            <li>
              <strong>The DNO can require disconnection</strong> if a system is connected without
              notification or against an unapproved design. That disconnection right is in the
              connection conditions; it is not a soft request.
            </li>
            <li>
              <strong>Smart Export Guarantee depends on it</strong> — the customer cannot receive
              SEG payments without the G98 / G99 paperwork on file with the DNO and the supplier.
            </li>
            <li>
              <strong>Three-phase domestic is increasingly common</strong> — high-power EV chargers
              and heat pumps are pushing more domestic supplies onto three-phase, and three-phase PV
              inverters of any capacity must be checked against the G98 or G99 threshold per phase.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Documentation that should be in the customer handover pack on every PV install:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Electrical Installation Certificate (EIC)</strong> — covers the electrical
              install per BS 7671 including the Section 712 verification.
            </li>
            <li>
              <strong>MCS certificate</strong> — issued by the MCS installer to the customer,
              evidence of MCS-certified installation.
            </li>
            <li>
              <strong>G98 or G99 notification</strong> — copy of the form sent to the DNO with
              confirmation of receipt.
            </li>
            <li>
              <strong>SAP / yield estimate</strong> — the MCS-calculated annual energy yield (kWh
              per year) that informed the customer's purchasing decision.
            </li>
            <li>
              <strong>Manufacturer documentation</strong> — module datasheets, inverter manual,
              battery manual where present, all warranties.
            </li>
            <li>
              <strong>Maintenance schedule</strong> — recommended periodic inspection and cleaning
              intervals.
            </li>
            <li>
              <strong>Building Regs / Building Control evidence</strong> — Part P notification
              (England and Wales) or relevant local authority confirmation.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              G98 applies to embedded generators connected in parallel with the low-voltage public
              distribution network where the total installed capacity is up to and including 16 A
              per phase per inverter. The installer connects the system on commissioning and
              notifies the DNO within 28 days using the standard G98 Installation Document. No prior
              DNO approval is required for G98 connections.
            </>
          }
          meaning={
            <>
              G98 is a streamlined regime that recognises the engineering case — small inverters
              with type-test certificates have a known impact on the network and do not need bespoke
              approval. The 28-day notification keeps the DNO's records current. Crossing the 16 A
              per phase threshold (or aggregating multiple inverters above it) escalates the project
              to G99, which requires prior approval and a connection agreement before any work
              begins on site.
            </>
          }
        ...
        >
          <p>Why anti-islanding cannot be turned off, even when the customer asks:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>DNO engineer safety</strong> — the primary case. The Electricity Safety,
              Quality and Continuity Regulations 2002 place safety duties on the DNO and on anyone
              connecting generation to the network.
            </li>
            <li>
              <strong>Asynchronous reclosure damage</strong> — when the DNO restores the network, an
              island that has drifted out of phase will smash equipment when it reconnects.
              Anti-islanding stops the island forming in the first place.
            </li>
            <li>
              <strong>Quality of supply for other customers</strong> — a small inverter trying to
              support an islanded network cannot maintain proper voltage and frequency for other
              customers' equipment. Damage to neighbours' kit follows.
            </li>
            <li>
              <strong>Connection conditions</strong> — every G98 or G99 connection agreement
              requires the inverter to remain compliant with the type-test settings. Tampering
              breaches the agreement and the DNO can require disconnection.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Three things to verify on a hybrid install during commissioning:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Transfer time and behaviour</strong> — simulate a grid loss (typically by
              opening the supply isolator) and confirm the inverter transitions to island mode
              within the design specification. Confirm the circuits intended to stay alive do, and
              that circuits intended to drop actually drop.
            </li>
            <li>
              <strong>Resync and reclose</strong> — restore the grid and confirm the inverter
              resyncs cleanly, the transfer recloses without nuisance, and grid-tied export resumes
              within the expected time.
            </li>
            <li>
              <strong>Battery state-of-charge management</strong> — the inverter should not deplete
              the battery to zero in island mode if the customer needs the battery for grid-tied
              time-shifting once the supply returns. Verify the MCS-designed reserve.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              G99 sets out the loss-of-mains protection requirements for embedded generators
              connected to the public distribution network. Generators shall cease to export within
              the timing windows defined by the type-test regime when grid reference is lost.
              Methods include voltage and frequency window detection, vector shift detection, and
              rate-of-change-of-frequency detection. Settings shall remain at type-tested values;
              alteration without DNO agreement is a breach of the connection conditions.
            </>
          }
          meaning={
            <>
              The combined effect of G98 and G99 LoM requirements is that no grid-connected inverter
              may continue to export onto a section of network that has lost its supply reference.
              The engineering reasons are operational safety (DNO engineers), equipment safety
              (asynchronous reclosure), and quality of supply (other customers). The regulations
              make LoM a non-optional feature of every grid-tied inverter sold for the UK market.
            </>
          }
        ...
        >
          <p>The documentation chain on a typical MCS PV install:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Site survey</strong> — pitch, azimuth, shading, roof structural assessment,
              existing electrical infrastructure.
            </li>
            <li>
              <strong>Design pack</strong> — module layout, string design, inverter selection, cable
              schedule, Uoc max calculation, OCPD selection, bonding arrangement, single-line
              diagram.
            </li>
            <li>
              <strong>SAP yield estimate</strong> — the kWh per year prediction the customer is
              buying.
            </li>
            <li>
            ...
            <li>
              <strong>Install records</strong> — install photos, label photos, test results, EIC.
            </li>
            <li>
              <strong>Commissioning records</strong> — inverter settings, grid-protection settings,
              hand-over checklist.
            </li>
            <li>
              <strong>Notification</strong> — G98 or G99 to the DNO, Part P (England and Wales) to
              Building Control, MCS certificate to customer.
            </li>
            <li>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              A 600 V DC isolator is fitted on a string with a calculated Uoc max of 720 V at the
              lowest expected ambient. The installer claims it has been fine for months. On the
              first frosty morning the array open-circuit voltage exceeds the isolator rating, the
              dielectric strength of the isolator is overstressed, and an arc develops. The result
              is a fire risk, an inverter trip, and a non-compliance under Reg 712.512.1.1 that
              should have been caught at design stage.
            </>
          }
          doInstead={
            <>
              Always check DC equipment voltage labels against the design pack Uoc max calculation,
              not against the array nameplate STC voltage. STC is at 25 degrees Celsius cell
              temperature; UK winter ambients drop well below that and Uoc rises proportionally. The
              MCS designer calculates Uoc max with the cold coefficient. The installer and verifier
              check the labels. A 1000 V DC rated chain is standard on most modern domestic
              installs.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              A six-string array is installed without per-string OCPDs and with cables sized to the
              operating current of one string (around 11 A). Section 712 offers two compliance
              routes — neither is met. Route (a) demands cable Iz of at least (N − 1) times Isc max,
              which here would be 55 A. Route (b) demands per-string OCPDs sized between 1.1 times
              Isc max and Iz, which were not fitted. The install is non-compliant and a
              single-string fault can back-feed from the other strings into the cable in a way the
              cable cannot safely carry.
            </>
          }
          doInstead={
            <>
              For any array of more than two parallel sub-arrays, work the two routes at design
              stage and pick one explicitly. Route (b) (per-string OCPDs in a combiner box) is the
              normal commercial answer on larger arrays — lighter cable, modest device cost, clean
              compliance. Route (a) is occasionally economic on small arrays with short cable runs.
              Document the choice on the certification.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You are commissioning a domestic 5 kWp PV array on a single-phase 16 A inverter, two
              strings of 8 modules each (N at most 2 — no string OCPDs needed under Reg 712.431.101
              acceptance). The MCS designer's Uoc max calculation says 690 V at the design lowest
              ambient. The fitted DC isolator is rated 1000 V DC. The array is on a south-facing
              pitched tile roof with no shading. Inverter is G98 type-test certified and labelled
              accordingly. You arrive at 7am, frost on the panels, and the inverter is showing 685 V
              DC open-circuit on the string display.
            </>
          }
          whatToDo={
            <>
              Start with the document check — design pack present, MCS certificate ready to issue,
              EIC template open. Work the Section 712 acceptance checklist — module conformity
              certificates (Reg 712.511.101), enclosure ratings IP44 plus IK07 (Reg 712.512.102), DC
              isolator voltage rating against design Uoc max (1000 V at most 690 V — pass), bonding
              continuity test on the array metalwork (Reg 712.542.101 — measure, record), insulation
              resistance of the DC side per Reg 712.412 acceptance, fire-protection visual (Reg
              712.421 — no obvious ignition sources). Read the inverter grid-protection settings and
              confirm they match the G98 type-test values. Commission the inverter, confirm export
              onto the grid, watch the LoM operation by simulating a grid trip via the AC isolator
              and confirming the inverter ceases export within the design timing. Sign the EIC,
              issue the MCS certificate, prepare the G98 notification form, photograph every label
              and every test reading, hand the customer the handover pack.
            </>
          }
          whyItMatters={
            <>
              The frosty 685 V open-circuit reading is exactly the scenario Section 712 is designed
              to catch. A 600 V DC isolator on this string would have failed the test on day one of
              operation. The 1000 V DC isolator passes by a wide margin. The two-string
              configuration is the easy compliance route — N at most 2 needs no string OCPDs by Reg
              712.431.101 acceptance. The G98 notification keeps the customer's SEG entitlement
              alive. Section 712 plus MCS plus G98 work together as a single coherent quality
              regime, and the customer gets a paper trail that survives any future audit.
            </>
          }
        />
        ...
        <Quiz
          title="Section 712 PV deep, G98/G99 and anti-islanding — knowledge check"
