...
const TITLE = 'EV charging deep dive (1.4) | Level 3 Module 2.1.4 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  ...
  {
    id: 2,
    question: 'Where does the BS 7671 framework for electric vehicle charging installations sit?',
    options: [
  ...
  {
    id: 6,
    question: 'What does anti-tamper signage on a charge point installation mean in practice?',
    options: [
...
export default function Sub4() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 2 · Section 1 · Subsection 4"
        title="EV charging deep dive"
        backTo="/study-centre/apprentice/level3-module2-section1"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          EV charging at recognition level for the L3 electrician — Modes 1-4, the BS 7671 Section
          722 framework, the OZEV Smart Charge Points Regulations 2021, dynamic load management for
          constrained supplies, PEN-fault protection on TN-C-S, and the apprentice contribution on a
          Mode 3 install.
        </p>
        ...
        >
          <p>The four modes summarised:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Mode 1</strong> — direct AC connection through a standard socket with no
              in-cable control or protection. Effectively banned for EVs in the UK; appears in some
              industrial battery applications.
            </li>
            <li>
              <strong>Mode 2</strong> — AC through a standard socket with in-cable control box
              (ICCB) providing pilot signal and protection. The 'granny cable' that ships with most
              EVs. 13 A maximum on a UK domestic socket — typically 2.3 kW. Emergency / occasional
              use only; sustained use risks burning out the socket.
            </li>
            <li>
              <strong>Mode 3</strong> — AC charging through a dedicated charge point with the
              control pilot and protection built into the fixed installation. The standard UK home
              install. Typically 7.4 kW single-phase (32 A at 230 V) or 22 kW three-phase. BS 7671
              Section 722 governs.
            </li>
            <li>
              <strong>Mode 4</strong> — DC charging where the rectifier is in the charger, not the
              car. Bypasses the on-board AC charger. 50 kW upwards; motorway rapid (50-150 kW),
              ultra-rapid (150-350 kW). Different install specialism.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The five Section 722 details an apprentice should recognise:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>PEN-fault protection</strong> — typically built into the charge point (modern
              units include automatic disconnection on PEN fault) or via a separate TT electrode for
              the EV.
            </li>
            <li>
              <strong>DC fault current detection</strong> — Type A RCD plus 6 mA DC detection
              (RDC-DD) built into the unit, or upstream Type B RCD.
            </li>
            <li>
              <strong>Cable rating</strong> — typically 6 mm or 10 mm singles or T+E for a 7.4 kW
              (32 A) circuit, depending on length and installation method.
            </li>
            <li>
              <strong>Isolation</strong> — local isolator close to the unit, plus the upstream RCBO
              / AFDD in the consumer unit.
            </li>
            <li>
              <strong>Labelling</strong> — supply origin, earthing arrangement, isolation point,
        ...
        <RegsCallout
          ...
          clause={
            <>
              "The requirements of this section apply to circuits intended to supply electric
              vehicles for charging purposes. Additional requirements apply for protection against
              electric shock — in particular Regulation 722.411.4.1 (PEN-fault detection on TN-C-S
              supplies) and Regulation 722.531.3.101 (RCD requirements including the detection of
              smooth DC fault current)."
            </>
          }
          meaning={
            <>
              Section 722 is the EV regulation anchor. It is not a stand-alone document — it works
              alongside Section 411 (general protective measures), Section 532 (RCDs), Section 514
              (identification and labelling), Appendix 4 (cable rating) and the rest of the BS 7671
              framework. A4:2026 has refined the technical detail of acceptable PEN-fault detection
              methods and clarified the interaction between the unit's built-in protection and the
              upstream protective device. The IET Code of Practice for Electric Vehicle Charging
              Equipment Installation is the practical implementation companion.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A4:2026 is the current edition for EV charging. If your reference book or training
              notes pre-date A4:2026 you must check Section 722 again — the changes touch PME
              handling, RCD selection and connector requirements. The IET Code of Practice for EV
              Charging Equipment Installation is the practical companion document.
            </>
          }
        ...
        <RegsCallout
          ...
          clause={
            <>
              The requirements of Section 722 do not apply to wireless charging, such as inductive
              charging. Installations employing wireless/inductive EV charging are outside the scope
              of Section 722 and require separate consideration.
            </>
          }
          meaning={
            <>
              Inductive (no-cable) charging is not covered by Section 722. If you encounter an
              inductive pad — currently rare on UK domestic — you fall back on the
        ...
        >
          <p>Five things the Regulations require:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              <strong>Default off-peak charging</strong> — pre-configured charging windows that
              avoid peak demand.
            </li>
            <li>
              <strong>Randomised delay</strong> — up to 600 seconds at the start of a charge session
              to spread the demand.
            </li>
            <li>
              <strong>Data privacy and cybersecurity</strong> — baseline standards for protecting
              customer data and preventing remote attack.
            </li>
            <li>
              <strong>Safety provision</strong> — disconnects on certain fault conditions detected
              by the unit.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Load-management decision factors:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Main fuse rating</strong> — most UK domestic supplies are 60, 80 or 100 A.
              Anything below 100 A is increasingly constrained as the load mix grows.
            </li>
            <li>
              <strong>Diversity calculation</strong> — the MCS-certified designer runs the
              calculation per the IET On-Site Guide. If the headline maximum demand exceeds the
              cut-out, load management or a fuse upgrade is required.
            </li>
            <li>
              <strong>CT clamp orientation</strong> — manufacturer-specific. Get it wrong and the
              load management does the opposite of what was intended.
            </li>
            <li>
              <strong>Customer expectation management</strong> — the charger may pause or slow
              during heavy concurrent loads. This is not a fault; it is the design intent.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The connector landscape in 2026:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Type 2 (Mennekes)</strong> — UK / EU AC standard, single or three phase, up to
              22 kW (32 A per phase). Used on 99% of UK AC home chargers and public AC posts.
            </li>
            <li>
              <strong>Type 1 (J1772)</strong> — older single-phase-only AC connector, US / Asia
              origin. Some older Mitsubishi Outlander PHEVs and pre-2018 Nissan Leafs. Adapters
              available.
            </li>
            <li>
              <strong>CCS Combo 2</strong> — UK / EU DC rapid standard. Type 2 plus two DC pins
              below. 50-350 kW typical at public rapid hubs.
            </li>
            <li>
              <strong>CHAdeMO</strong> — older Japanese DC standard. Phased out; survives at some
              legacy rapid sites for older Nissan Leafs.
            </li>
            <li>
              <strong>Tesla NACS</strong> — North American Tesla connector; Tesla UK Superchargers
              now offer CCS Combo 2 as well, and the NACS standard is being adopted by some North
              American manufacturers but is not the UK public-charging standard.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The two flavours and their differences:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>V2H (Vehicle-to-Home)</strong> — car discharges into the property during peak
              hours or grid outages. Acts as a giant backup battery. No grid-export contract needed;
              the energy is consumed on-site.
            </li>
            <li>
              <strong>V2G (Vehicle-to-Grid)</strong> — car discharges into the grid in response to
              grid-services signals (frequency response, peak shaving). The customer earns revenue
              per kWh exported. Requires a grid-services contract with an aggregator (OVO, Octopus,
              etc.) and ENA G99 connection.
            </li>
            <li>
              <strong>Battery degradation question</strong> — bidirectional cycling adds wear to the
              EV battery. Manufacturers are starting to warrant V2G use; some still void the
              warranty. Customer needs to check.
            </li>
            <li>
              <strong>Regulation overlap</strong> — Section 722 (EV) plus Section 826 (EESS) plus
              G99 (export). MCS qualifications 2921 (EV) and 3012 (EESS) both apply. The trade
              specialism is real.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Three-phase install considerations:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Vehicle on-board charger limit</strong> — most domestic EVs cap at 7.4 kW or
              11 kW AC. A 22 kW post on a 7.4 kW car gives 7.4 kW. Match the unit rating to the
              customer&apos;s actual fleet.
            </li>
            <li>
              <strong>Cable size</strong> — 32 A per phase typically needs 4 mm or 6 mm 4-core SWA
              depending on length, install method and ambient. Check Appendix 4 of BS 7671 for the
              actual rating.
            </li>
            <li>
              <strong>RCD selection</strong> — Type B RCD is the typical specifier on three-phase EV
              circuits because three-phase units may produce smooth DC fault current that defeats
              Type A. Some units have integrated DC fault detection per phase that allows Type A
              upstream.
            </li>
            <li>
              <strong>Workplace context</strong> — workplace charging often uses load-balancing
              across multiple posts (back-office controller distributes a fixed total supply across
              the connected vehicles). Smarter than per-post load management for fleet sites.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The supply-upgrade decision tree:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>60 A or 80 A single-phase</strong> — load management lets a 7.4 kW charger
              coexist with most domestic loads. Upgrade to 100 A may be worth discussing if the
              customer is also adding a heat pump.
            </li>
            <li>
              <strong>100 A single-phase</strong> — typically fine for a 7.4 kW charger plus heat
              pump plus other loads with sensible diversity.
            </li>
            <li>
              <strong>Single to three-phase</strong> — required for 22 kW charging and for some
              larger commercial installs. Significant DNO works (typically 4-12 weeks); cost varies
              by area and existing infrastructure.
            </li>
            <li>
              <strong>G98 vs G99</strong> — G98 covers connect-and-notify within 16 A per phase.
              Anything above (typical for V2G or larger commercial) needs G99 pre-application.
            </li>
            <li>
              <strong>Network reinforcement programmes</strong> — Some DNOs offer free or subsidised
              supply upgrades as part of low-carbon transition initiatives. Check before quoting the
              customer.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Apprentice runs 6 mm cable, fits a Type A RCBO in the consumer unit, terminates in the
              charge point, jobs done. Six months later a PEN fault occurs on the local network. The
              property's earthing system rises to dangerous voltage; the customer's wet drive plus
              the metal car bodywork become a shock hazard; the unit does not disconnect because
              PEN-fault detection was not enabled. Outcome ranges from burnt cables to serious
              injury. The MCS-certified designer caught it on the design but the apprentice did not
              know to verify it on commissioning.
            </>
          }
          doInstead={
            <>
              Confirm during commissioning that PEN-fault protection is operational. Either the
              unit's built-in detection is enabled and tested per the manufacturer's instructions,
              or a TT electrode is installed and the EV side is on a dedicated TT earth. Section 722
              is non-negotiable on this. If the design specifies one method and the install delivers
              the other (or neither), the install is non-compliant.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              7.4 kW charger on a 60 A main fuse with a 9.5 kW shower and a 4 kW oven. Customer
              cooks dinner, partner showers, EV starts charging on schedule — main fuse trips. DNO
              callout to replace the cut-out fuse. Customer angry. Apprentice and contractor both
              look bad. Repeats every couple of weeks until somebody enables the CT clamp and the
              load-management logic.
            </>
          }
          doInstead={
            <>
              Check the main fuse rating before fitting any 7.4 kW charger. Run the diversity
              calculation with the designer. If the property is constrained, fit the CT clamp around
              the supply tail, configure the unit's load-management settings to the main-fuse
              rating, and test by deliberately switching on a known load to verify the charger
              throttles. Brief the customer that the charger may pause or slow during heavy
              concurrent demand — this is the design intent.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You are the apprentice on a Mode 3 Zappi install at a 1990s semi. Single-phase 60 A
              main fuse. Existing loads: 9.5 kW electric shower, 3.5 kW oven, 3 kW immersion heater
              (rarely used now the boiler is condensing). The MCS-certified electrician has
              specified a 7.4 kW Zappi v2.1 fed from a new 32 A Type A RCBO in the consumer unit,
              with the supplied CT clamp on the supply tail to enable dynamic load management. The
              unit has built-in PEN-fault detection. Cable run is 8 m of 6 mm T+E through a wall to
              the garage where the unit will mount.
            </>
          }
          whatToDo={
            <>
              Pull the cable, terminate at the consumer unit on the new RCBO, terminate at the
              charge point per the manufacturer's wiring diagram. Fit the CT clamp around the live
              supply tail in the orientation the manual specifies (arrow toward the meter, away from
              the property side — confirm against the install manual for this specific model). Test
              PEN-fault detection per the unit's commissioning sequence. Verify load management:
              with the EV charging at 7.4 kW, switch on the shower; the Zappi should reduce its draw
              within seconds to keep total property current below the configured limit. Brief the
              customer: the charger may pause or slow when other heavy loads are running, this is
              the load-management feature working as intended. Hand over the labelling and the
              MCS-certified electrician's commissioning paperwork.
            </>
          }
          whyItMatters={
            <>
              This is the typical L3 apprentice job on EV chargers. The cable pull, the
              consumer-unit termination, the CT clamp installation, the labelling — all standard
              scope. Recognising that the PEN-fault protection and the load-management commissioning
              are non-optional (not 'nice to have') is the difference between a compliant install
              and a callback. The MCS-certified person signs off the design and commissioning; the
              apprentice executes the wiring and contributes to the commissioning checks.
            </>
          }
        />
