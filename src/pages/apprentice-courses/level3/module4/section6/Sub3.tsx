...
const checks = [
  {
    ...
    options: [
      'Type B RCDs (designed for DC residual currents from power-electronics loads like EV chargers, PV inverters, VSDs) have wider trip-time tolerances than Type AC and Type A — at IΔn the BS 7671 limit is 300 ms maximum (Tables 41.1A / 41.1B), so 280 ms is within spec but at the high end. CORRECTION: confirm the test was done correctly (instrument calibrated, 0° and 180° phase angles tested for AC components, smooth DC residual current tested for the Type B characteristic), and CHECK 5×IΔn trip-time which should be under 40 ms — if 5× is also at the upper end, the RCD may be approaching end of life and replacement is prudent. Type B RCDs (Doepke DFS4, ABB F204, Hager CDA470F) are expensive (£200–400) and have longer lead-time than standard Type AC; plan replacement.',
      "Significant phase imbalance. The DNO supply at the cut-out should give all three phases within a few volts of each other, all around 230 V (the UK statutory tolerance is +10% / -6%, so 216–253 V acceptable). Your readings — L2 218 V (low end of tolerance), L3 245 V (high end) — span 27 V across phases. Probable cause: severe load imbalance somewhere on the supply transformer feeder, OR a partial open-PEN at the DNO side. The flicker on the unit's lights and the contactor drop-out are classic symptoms — the contactor coil voltage drops below holding threshold when L2 sags under additional load. L3 RESPONSE: this is a DNO-side investigation, not an apprentice fix. Document the readings, log the time, escalate to supervisor, supervisor calls the DNO with the readings. NEVER try to 'fix' supply-side imbalance on the customer's side — it's not within scope and it's not the cause.",
  ...
  },
  {
    ...
    options: [
      ...
      'PV systems have TWO supplies — the AC side from the consumer unit (which you isolate at the PV breaker / RCBO), AND the DC side from the panels themselves. The DC side is LIVE WHENEVER LIGHT FALLS ON THE PANELS — at midday on a sunny day each panel produces 30–40 V DC, and a string of 12 panels delivers 350–480 V DC. Isolating the AC alone leaves the DC fully live up to the inverter input terminals. L3 ROUTINE: (1) Isolate the AC side at the dedicated PV breaker; lock-off; prove dead at the inverter AC terminals. (2) Identify the DC isolator (BS EN 60947-3 DC-PV2 rated, typically a Santon or similar mounted next to the inverter) and switch it to OFF. (3) Prove dead at the inverter DC terminals with a DC-rated meter (Fluke 117 on V DC; Megger MFT). (4) NEVER attempt to disconnect MC4 connectors under load — they are NOT designed for breaking DC current and will arc-flash. (5) If access to the panels themselves is needed, COVER them with an opaque tarp before any work on the array side.',
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: [
      ...
      "Three-step diagnostic. (1) MEASURE the motor's actual running current with a clamp meter under normal load — should be at or below the motor's nameplate Full Load Amps (FLA). If the motor is running over-current, the load is excessive (tight bearing, jammed driven equipment, undersized motor for the application) — investigate the mechanical load. (2) CHECK the overload setting on the starter — should be set to the motor's FLA from the nameplate (typically 1.0× FLA, sometimes 1.05× for ambient compensation). If the overload is set too low (e.g. 0.7× FLA), it will trip on normal running. (3) CHECK the overload itself — old bi-metal overloads degrade and trip at lower currents than calibrated; replacement is straightforward and inexpensive. NEVER disable the overload to 'stop the trips' — the overload exists to protect the motor windings from burnout; without it the motor will fail catastrophically. The CORRECTION is to identify the cause and fix it (mechanical load, wrong setting, degraded overload).",
  ...
  },
  {
    id: 3,
    question:
      "A 22 kW (32 A per phase) commercial EV charger on a TT-supplied site is failing periodic RCD trip-time tests. The Type B 30 mA RCD is reading trip-times of 280 ms at IΔn. What's the analysis and correction?",
    options: [
    ...
    correctAnswer: 3,
    explanation:
      'Type B RCDs are the protection device of choice for power-electronic loads. The L3 apprentice should know the trip-time limits (300 ms at IΔn, 40 ms at 5×IΔn) and the practical interpretation — readings at the upper end suggest the device is ageing and replacement is sensible even if technically still within spec.',
  },
  {
    id: 4,
    question:
  ...
  },
  {
    ...
    options: [
      ...
      "Classic capacitor failure. The run capacitor on a single-phase induction motor provides the phase shift between the run winding and the start winding that creates the rotating field needed to start the motor. If the capacitor degrades (electrolytic dries out, paper-and-foil constructions go open) the motor energises but can't start — it draws stalled-rotor current, hums, and trips on thermal overload after 10–30 seconds. Diagnostic: ISOLATE; PROVE DEAD; DISCHARGE the cap through a 5–10 kΩ resistor (live-storage hazard); MEASURE the cap with the MFT capacitance function (should read within ±10% of marked value, e.g. 16 µF reads 14.4–17.6 µF; failed caps read near zero or open). CORRECTION: replace the cap with same μF value, same voltage rating (typically 250 V AC), same physical size to fit the bracket. Verify the motor starts immediately on energise.",
...
export default function Sub3() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 6 · Subsection 3"
        title="Apply techniques to specialised systems"
        backTo="/study-centre/apprentice/level3-module4-section6"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Three-phase distribution, single-phase motor controls (DOL, contactors, overloads), EV
          charging under BS 7671 722 (A4:2026 PNB / Open PEN), domestic solar PV under BS 7671 712.
          What an L3 apprentice CAN diagnose and rectify under supervision, and the EAWR Reg 16
          competence boundary that says when to escalate.
        </p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Phase imbalance</strong> at the cut-out — DNO-side issue; document and
              escalate.
            </li>
            <li>
              <strong>Single-phasing</strong> of three-phase motors — one phase open; motor draws
              excess current on remaining phases.
            </li>
            <li>
              <strong>DOL starter overload trip</strong> — investigate load, setting, overload
              condition before touching the protection.
            </li>
            <li>
              <strong>Contactor coil drop-out</strong> — check coil voltage stability under load;
              phase imbalance can cause this on the affected phase.
            </li>
            <li>
              <strong>VSD / soft-start fault codes</strong> — read the fault code from the drive
              display; consult the manufacturer's fault-code list; supervisor-led for anything
              beyond reset.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              "The effectiveness of the measures for fault protection by automatic disconnection of
              supply shall be verified: (a) for TN systems by measurement of the earth fault loop
              impedance, and verification of the characteristics and/or the effectiveness of the
              associated protective device (visual inspection in the case of overcurrent protective
              devices and testing in the case of RCDs); (b) for TT systems by measurement of the
              resistance of the earth electrode for exposed-conductive-parts of the installation,
              and verification of the characteristics and/or the effectiveness of the associated
              protective device."
            </>
          }
          meaning={
            <>
              On three-phase work the L3 verification still hangs off Reg 643.7.1 &mdash; measure
              earth fault loop impedance, verify the protective device, confirm disconnect time.
              Three-phase prospective fault current (Ipf) is typically much higher than single-phase
              &mdash; 6&ndash;25 kA at the cut-out is normal for small-commercial &mdash; so the
              protective device&apos;s breaking capacity must be at least equal to the Ipf at its
              location (Reg 434.5.1). Standard 6 kA MCBs may NOT be suitable on the supply side of a
              small-commercial installation; 10 kA or higher devices (Hager NDN, Schneider C60H, ABB
              S203M) are normal. The L3 apprentice&apos;s verification work measures Ipf at the
              cut-out and at downstream sub-boards and confirms each device&apos;s rating is
              adequate.
            </>
          }
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>PEN fault detection device</strong> — built into the charger; monitors L-N and
              L-PE; disconnects on PEN compromise.
            </li>
            <li>
              <strong>Earth electrode (TT conversion of the EV circuit)</strong> — dedicated
              electrode local to the charger; EV circuit operates as TT, isolated from the main
              installation's earth.
            </li>
            <li>
              <strong>Dual earthing arrangement</strong> — main installation TN-C-S / PNB; EV
              circuit has additional dedicated electrode in parallel.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <ConceptBlock
          ...
          onSite="A typical 4 kWp domestic PV system has 12–14 panels in a string, producing 350–500 V DC at the inverter's DC input under full sun. The DC side is LIVE WHENEVER LIGHT FALLS ON THE PANELS — early morning, late evening, even moonlight gives a few volts, full midday gives the full string voltage. Isolating the AC side at the CU does NOT make the DC side safe; the DC isolator (located adjacent to the inverter) is the dedicated means."
        >
          <p>The L3 PV isolation routine:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>(1) Isolate AC at the dedicated PV breaker</strong> in the CU; lock-off; prove
              dead at the inverter AC terminals.
            </li>
            <li>
              <strong>(2) Switch the DC isolator OFF</strong> (Santon, ABB OTDC, Hager SBN — DC-PV2
              rated); prove dead at the inverter DC terminals with a DC-rated meter.
            </li>
            <li>
              <strong>(3) NEVER open MC4 connectors under load</strong> — they are not arc-rated for
              DC current breaking; would arc-flash and weld.
            </li>
            <li>
              <strong>(4) If working at the array</strong>, cover the panels with an opaque tarp
              before any work that involves disconnection.
            </li>
            <li>
              <strong>(5) When restoring</strong>, DC isolator ON first (allows the inverter to
              start its sync routine), then AC breaker ON; verify export to grid via the inverter
              display.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              Section 712 sets the requirements for the DC side of solar PV installations, including
              isolation, identification and the requirement that DC-side equipment (combiner boxes,
              inverter DC isolators) carry a permanent warning that PV arrays and battery storage
              can continue to feed live parts after local isolation. Routine maintenance must assume
              DC-side parts are energised even if the AC supply or inverter is off.
            </>
          }
          meaning={
            <>
              BS 7671 Section 712 (A4:2026) sets the DC-side requirements for PV installations. A
              dedicated DC isolator is needed so the inverter can be safely worked on without
              exposure to the DC supply; the isolator must be DC-rated (BS EN 60947-3 DC-PV1 or
              DC-PV2) because standard AC isolators will not safely break DC current. The L3
              apprentice&apos;s first move on any PV fault is to identify the DC isolator and
              operate it before any work near the inverter or DC cabling, and to treat DC-side parts
              as energised even after local isolation where battery storage or array feed-back is
              possible.
            </>
          }
        ...
        <ConceptBlock
          title="Why battery storage isn’t just ‘another DC system’"
          plainEnglish="Domestic battery storage units (Tesla Powerwall, GivEnergy, Sonnen, Pylontech) sit on the DC side of the inverter and hold lethal stored energy after the AC is isolated. Fault correction needs the manufacturer’s isolation procedure followed in order — AC down, then battery breaker, then internal isolator."
          onSite="Battery storage faults frequently come back to the BMS rather than the cells — communications loss, firmware mismatch, temperature sensor failure. The L3 apprentice supports a competent battery-trained engineer; solo unsupervised work on storage is not L3 apprentice scope."
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Sequenced isolation</strong> &mdash; manufacturer&apos;s order, every time.
              Out-of-order isolation can damage the BMS or trip the cells into a fault state.
            </li>
            <li>
              <strong>Stored DC energy</strong> &mdash; cells stay live until physically
              disconnected; isolating the AC supply doesn&apos;t make the battery safe to touch.
            </li>
            <li>
              <strong>Thermal runaway</strong> &mdash; rare but catastrophic. A swollen cell, a heat
              spot or a smell of electrolyte is a STOP and evacuate situation.
            </li>
            <li>
              <strong>BMS communications</strong> &mdash; loss of comms can mimic a fault. Check
              inverter / BMS log before assuming a hardware failure.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <ConceptBlock
          ...
          plainEnglish="Air-source heat pumps (Mitsubishi Ecodan, Daikin Altherma, Vaillant aroTHERM) draw smooth DC residual currents, run continuous mid-load profiles for hours, and are protected differently from a conventional immersion. A fault on the heat-pump circuit is rarely the heat pump itself — usually it’s the protection coordination."
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>RCD type</strong> &mdash; Type B is required where the heat pump can produce
              smooth DC residual; manufacturer data sheet is the source of truth.
            </li>
            <li>
              <strong>Cable sizing</strong> &mdash; sustained mid-load operation differs from
              intermittent peak. Verify against the design ambient and grouping factors.
            </li>
            <li>
              <strong>Soft-start inrush</strong> &mdash; modern inverter-driven heat pumps have low
              inrush, but legacy single-speed compressors do not. A nuisance trip on first
              compressor cycle isn&apos;t a fault, it&apos;s a curve mismatch.
            </li>
            <li>
              <strong>External unit isolator</strong> &mdash; mandatory; inspect first on any fault
              visit (corrosion, water ingress, terminal condition).
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Three-phase motor IR test thresholds:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Above 100 megohms</strong> — healthy windings; safe to re-energise after a
              service.
            </li>
            <li>
              <strong>10-100 megohms</strong> — ageing insulation, monitor trend; not a failure but
              worth flagging on the customer record.
            </li>
            <li>
              <strong>1-10 megohms</strong> — degraded insulation; investigate cause (water,
              contamination, thermal cycling); consider rewind or replacement.
            </li>
            <li>
              <strong>Below 1 megohm</strong> — failure imminent; do not re-energise; refer to motor
              specialist for rewind or replacement decision.
            </li>
            <li>
              <strong>Test voltage</strong> — 500 V DC for LV motors up to 500 V working; 1000 V DC
              for sub-mains and HV motors.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>EV charger diagnostic touchpoints:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Manufacturer app event log</strong> — pulls cloud OCPP events; first place to
              look for a recent fault history.
            </li>
            <li>
              <strong>Control pilot voltage</strong> — measured at the connector; idle 12 V DC,
              vehicle connected 9 V, charging 6 V, ventilation request 3 V. Off-pattern voltage =
              pilot fault.
            </li>
            <li>
              <strong>Proximity pilot</strong> — confirms cable rating; wrong reading triggers a
              charger refusal-to-charge.
            </li>
            <li>
              <strong>RCD type</strong> — Type B is mandatory under A4:2026 Section 722 unless the
              unit has integral DC residual current detection rated for the application.
            </li>
            <li>
              <strong>Open-PEN protection</strong> — verify whether built-in detection, separate
              device, or TT electrode arrangement is in use; do not bypass.
            </li>
            <li>
              <strong>Connector inspection</strong> — water, dirt, corrosion at pins; clean and
              re-test.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Common battery-storage fault modes:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>BMS comms loss</strong> — check the comms cable between battery and inverter;
              loose connector or rodent damage common.
            </li>
            <li>
              <strong>Firmware mismatch</strong> — inverter and battery firmware versions must be on
              the supported pair list; update via the manufacturer's installer app.
            </li>
            <li>
              <strong>High cell temperature</strong> — check ambient, ventilation, and whether the
              unit has been heavily cycled by a tariff schedule.
            </li>
            <li>
              <strong>Inverter handshake failure</strong> — battery and inverter from different
              manufacturers may need bridge hardware (CAN-Bus protocol converter); diagnose with the
              installer app.
            </li>
            <li>
              <strong>Cell-level faults</strong> — manufacturer warranty; do not open the battery
              enclosure unless authorised.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Specialised systems (EV, PV, battery, heat-pump) sit downstream of the same TN
              earthing arrangement. Their fault behaviour depends on the upstream PEN being intact
              &mdash; a compromised PEN turns an EV chassis or a PV frame into a hazard for anyone
              touching it from true earth. That&apos;s why supply-side reading at the cut-out comes
              before any specialised-system fault work.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Apprentice arrives at a PV system fault at midday on a sunny day. Switches off the AC
              breaker in the CU. Goes up to the roof and disconnects an MC4 connector to "have a
              look" at the panel side. The MC4 connector is rated for DC voltage but NOT for
              breaking DC current under load &mdash; the string is delivering 8&nbsp;A at 400&nbsp;V
              DC into the inverter. The connector arcs as it separates; the arc sustains because DC
              has no zero-crossing; the connector welds, the cable insulation ignites, the
              apprentice is blinded by the flash and is on a sloping roof. Best case &mdash; severe
              burn and a dropped tool. Worst case &mdash; fall from height plus arc-flash injury.
            </>
          }
          doInstead={
            <>
              Switch the DC isolator OFF before any work on the DC side. The DC isolator is designed
              to break DC current safely (multiple breaks in series, magnetic blow-out, long contact
              travel). Verify dead at the inverter DC terminals. THEN access MC4 connectors if
              required. NEVER use an MC4 connector to break a live DC circuit.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Apprentice or installer is called back to a Wallbox Pulsar Plus that disconnects the
              vehicle every time it sees a small N-E voltage variation. To "fix" the complaint, they
              go into the charger's installer menu and disable the PEN fault detection. The charger
              now charges happily &mdash; until the upstream PEN actually opens (a routine DNO
              operation, a fault at the substation, a damaged service cable). With detection
              disabled, the charger keeps charging; the vehicle's chassis rises to a fraction of
              phase voltage; the customer touches the car while standing on the driveway; serious
              shock or electrocution. The contractor faces criminal prosecution under EAWR +
          ...
          doInstead={
            <>
              ...
              under different load conditions; involve the DNO if N-E is consistently elevated. If
              the supply is genuinely poor, escalate &mdash; the cure may be a TT conversion of the
              EV circuit, NOT disabling the protection. PEN fault detection exists because the
              open-PEN hazard exists; disabling it does not remove the hazard, it just removes the
              protection.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              ...
              reports the RCD trips intermittently &mdash; usually mid-charge, sometimes within the
              first 10 minutes. You're sent to investigate with the supervisor on call.
            </>
          }
          whatToDo={
            <>
              (1) GREET the customer; brief the diagnostic plan; confirm the charger is physically
              isolated and locked-off before approach. (2) ISOLATE at the dedicated charger circuit
              breaker on the small-commercial DB; lock-off; prove dead at the charger's incoming
              terminals (Martindale VI-13800 across each phase to neutral and to earth). (3) Open
              the charger access panel; visually inspect the wiring &mdash; no scorching, no loose
              terminations, no water ingress at the gland. (4) Restore supply; ask the customer to
              begin a charge cycle on a vehicle; observe the charger's display and the building's
              RCD; note the trip time, the conditions (vehicle SoC, ambient temperature, AC vs DC
              charge mode). (5) Trips at 12 minutes into the cycle. ISOLATE; lock-off; prove dead.
              (6) MFT 1741 RCD trip-time test on the Type B RCD &mdash; trip-time at I&Delta;n
              285&nbsp;ms (within 300&nbsp;ms limit but high-end), at 5&times;I&Delta;n 38&nbsp;ms
              (within 40&nbsp;ms limit). The RCD is technically within spec but at the upper edge.
              (7) Phone call to the supervisor &mdash; recommendation to escalate to RCD replacement
              (ABB F204 supersedes can be sourced same-day from CEF). Supervisor agrees; orders the
              replacement; books the supervisor on-site for the replacement (three-phase live-edge
              work). (8) MAKE-SAFE the charger pending replacement; brief the customer on the
              next-day visit. (9) Update the diagnostic notes; close today's visit with a
          ...
          whyItMatters={
            <>
              ...
              trip-time test, recognition of the upper-edge reading. The RECTIFICATION (Type B RCD
              replacement on a three-phase circuit) is supervisor-led because of the three-phase
              voltage and the certification requirements. The L3 apprentice's role is to do the
              diagnostic, document it, and escalate the rectification to the right competence level.
              EAWR Reg 16 in action.
            </>
          }
        />
        ...
        <Quiz
          title="Specialised systems — three-phase, EV, PV — knowledge check"
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          >
            ...
            <div className="mt-1 text-[14px] font-semibold text-white truncate">
              6.2 Common faults — correction techniques
            </div>
          </button>
          <button
          ...
          >
            ...
            <div className="mt-1 text-[14px] font-semibold text-black truncate">
              6.4 Capstone — multi-fault case study + reflection
            </div>
          </button>
        </div>
      </HubBody>
