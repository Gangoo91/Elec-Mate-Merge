...
const checks = [
  {
    ...
    correctIndex: 3,
    explanation:
      "Open circuit is the simplest fault category — the current path is broken, the load doesn't work. R = ∞, V = 0 on the load side, EFLI = OPEN. Causes: broken conductor (cut, snapped at termination), failed terminal screw, blown fuse, failed switch or contactor. Diagnosis is usually visual or with a continuity test from one end of the circuit to the other; the break is at the point continuity fails.",
  },
  {
    ...
    correctIndex: 3,
    explanation:
      "A short circuit reads near zero ohms L–N — a few hundred milliohms of supply cable plus the bridging fault. Causes: insulation breakdown between L and N inside an appliance, a damaged cable where conductors touch, a cross-wired terminal at an accessory. It draws the highest fault current the system will see, limited only by supply impedance + cable resistance; on a 6 kA PSCC supply a phase-neutral short draws 4–5 kA briefly and the MCB's magnetic trip operates in under 10 ms. Visual inspection often shows scorching or melted insulation at the fault point.",
  },
  {
    id: 'mod4-s3-sub1-hrj',
    question:
    ...
    correctIndex: 1,
    explanation:
      "HRJ is the single most dangerous common fault because it doesn't trip a protective device — the current is below the breaker's rating, but the heat at the joint is enough to start a fire. A 0.5 Ω HRJ carrying 20 A dissipates 200 W in a few millimetres. Causes: loose terminal screw, oxidised aluminium-to-copper contact, undersized crimp, conductor strand damage, scorched contact from a previous fault. It is hard to diagnose because the joint reads OK on a multimeter — only thermal imaging or voltage drop under load reveals it. FIA fire-investigation data consistently puts HRJ in the top three causes of dwelling fires of electrical origin.",
  },
];
...
const quizQuestions = [
  ...
  },
  {
    ...
    correctAnswer: 2,
    explanation:
      "Parallel arc faults are the canonical case for AFDD. Pre-AFDD, these faults caused fires that no other protective device could prevent — the current was always within the breaker's rating. AFDD's signature analysis uses the high-frequency 'noise' on the current waveform (caused by the arc itself) to identify and trip. BS 7671 A4:2026 Reg 421.1.7 (redrafted) now requires AFDDs on single-phase socket-outlet final circuits up to 32 A in HRRBs, HMOs, purpose-built student accommodation and care homes, and recommends them for all other premises.",
  },
  {
    id: 7,
    ...
    correctAnswer: 3,
    explanation:
      "L–L faults are a three-phase-only category. Fault current is high but limited by supply + cable impedance — typically 5–10 kA on a commercial supply. Common cause: insulation breakdown in three-phase cable or motor windings, or accidental contact between phases at a terminal block. The unaffected phase remains live so loads on it keep working (which can be confusing — 'half the loads still work'). Three-phase diagnosis requires knowing which loads are line-to-neutral vs line-to-line vs three-phase — the symptoms reveal the fault location.",
  },
  {
    id: 8,
