...
const checks = [
  {
    ...
    options: [
      'Single-pole switches and protective devices are in the line conductor only, and the screw-lampholder centre contact is connected to the line.',
      'Single-pole switches and protective devices are in the neutral conductor only, so switching off isolates the return path.',
      'The line and neutral conductors are the same cross-sectional area throughout, so current shares evenly between them under load.',
      'The protective conductor is connected to the centre contact of every lampholder, so a faulty bulb trips the protective device.',
    ],
    correctIndex: 0,
    explanation:
      'Reg 132.14 (the design rule) requires single-pole devices in the line conductor; Reg 643.6 (the verification rule) requires the test that confirms this, and that the E14/E27 centre contact is on line. Reverse polarity at a switch means flipping it "off" only breaks the neutral — line voltage is still present at the switched terminal, lampholder contacts, or appliance casing. A user touching the metal lamp body or switching out a bulb expects the circuit to be dead and finds it live. Polarity verification catches this before energisation.',
  },
  {
    id: 'm5-s3-sub5-method',
    question: 'The standard method for verifying polarity during dead testing on a final circuit:',
    options: [
      'A live voltage test at every accessory — energise, then confirm the line terminal reads 230 V to earth at each point.',
      'An insulation resistance test between line and neutral — a reading above 1 MΩ confirms the conductors have not been crossed.',
      'A continuity test plus visual check — link L to CPC at the CU, then measure L-CPC at each accessory; a low reading confirms correct polarity.',
      'A measurement of Ze at each accessory — comparing the external loop impedance at every point confirms the line conductor is unbroken.',
    ],
    correctIndex: 2,
    explanation:
      'The dead-test polarity method exploits the same temporary L-CPC link used for R1+R2. With L linked to CPC at the CU and you measuring L-CPC at the accessory, a low reading confirms that the conductor you are calling "line" at the accessory really is connected to the line of the circuit at the CU. Combined with visual inspection of conductor colours at every terminal — brown for line, blue for neutral, green/yellow for CPC — the polarity of the circuit is verified at every point.',
  },
  {
    id: 'm5-s3-sub5-lampholder',
    question:
      'BS 7671 requires that the centre contact of an E14 / E27 screw-type lampholder is connected to:',
    options: [
      'The neutral conductor — the deeper, harder-to-touch contact goes to neutral, leaving the screw thread on line.',
      'The line conductor — the deeper, harder-to-touch contact goes to line, leaving the exposed screw thread on neutral.',
      'The protective conductor (CPC) — the centre contact is earthed so a failed bulb shorts to earth and trips the device.',
      'Either live conductor — screw lampholders are symmetrical, so centre-contact polarity does not matter if RCD protected.',
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    id: 1,
    question:
      'A single-pole switch on a lighting circuit has reverse polarity (the switch is in the neutral). Practical consequence:',
    options: [
      'The light will not switch on at all — interrupting the neutral breaks the return path, so no current can flow.',
      'The light still works, but the lampholder stays live with the switch "off" — changing a bulb risks a shock from the live thread.',
      'The light flickers because the switch chops the neutral, producing an unstable return current the lamp cannot smooth.',
      'The protective device trips on first switch-on, because the neutral break creates a residual imbalance the RCD detects.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'The consumer unit outgoing terminals only. Once the conductors leave the CU correctly identified, BS 7671 assumes they remain correctly polarised to every accessory.',
      'The CU and the first accessory on each circuit. A correct reading at these two points proves the conductors were not crossed anywhere along the run.',
      'Every point on every circuit — every accessory, every JB, every termination — because conductors can be crossed at intermediate points.',
      'The furthest accessory only. Testing at the electrical end of the circuit captures any swap between the CU and that point.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Energise the circuit and check each accessory with a voltage indicator — line to earth reading 230 V confirms correct polarity.',
      'Measure insulation resistance between line and neutral at each accessory — a reading above 1 MΩ confirms the conductors are not crossed.',
      'Compare R1+R2 readings at every accessory — equal values around the circuit prove the line and neutral are not swapped anywhere.',
      'Link L to CPC at the CU, then measure continuity between local "line" and local CPC at each accessory — a low reading confirms polarity.',
    ],
    correctAnswer: 3,
    explanation:
      'The dead-test polarity method using the L-CPC temporary link makes L and CPC into one continuous loop. At any accessory, measuring continuity between the conductor designated "line" and the local CPC should give a low reading because both ends of the test path are joined at the CU via the link. If the reading is OL, the local "line" is actually neutral and someone has swapped them at the back-box. The same temporary link is used for the R1+R2 measurement — efficient combined test.',
  },
  {
    id: 4,
    question: 'After dead-test polarity verification, you also need to:',
    options: [
      'Verify polarity LIVE during energised testing — confirm supply-origin polarity and that single-pole switches really interrupt the line.',
      'Skip live polarity verification — the dead test using the L-CPC link already proves the internal wiring and the incoming supply.',
      'Re-test insulation resistance at 1000 V DC to confirm the dead-test polarity findings before the circuit is energised.',
      'Repeat the dead polarity test a second time at every accessory to confirm consistency, then sign off without live verification.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Red for line (single-phase), black for neutral, green for the protective conductor — still in force for all new work.',
      'Brown for line (brown/black/grey for L1/L2/L3), blue for neutral, green-and-yellow exclusively for protective conductors.',
      'Blue for line, brown for neutral, bare copper for the protective conductor — line and neutral swapped under the harmonised code.',
      'Brown for line, blue for neutral, green-and-yellow for either a protective conductor or a switched line where clearly labelled.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    id: 6,
    question:
      'GN3 notes that polarity testing of lighting circuits with multiple lampholders requires:',
    options: [
      'Testing each lampholder with the lamps left in place, so the lamp current confirms the holder is correctly polarised.',
      'Energising the circuit and checking each lamp lights, since only a correctly polarised lampholder will illuminate its lamp.',
      'Removing all lamps first — filaments and drivers give parallel L-N paths that can mask a polarity error — then verify each holder.',
      'Linking all the lampholder centre contacts together, so a single continuity reading confirms polarity across the circuit at once.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    id: 7,
    question:
      'On a circuit feeding a bayonet (B22) lampholder, why is polarity still important even though there is no exposed centre contact?',
    options: [
      'Because the bayonet holder has two pins that must each be at the correct polarity, or the lamp will not seat correctly.',
      'Because reverse polarity at a bayonet holder raises the contact temperature, shortening lamp life even without a shock hazard.',
      'Because the protective conductor terminates at the bayonet holder body, and reversed polarity would leave that body live.',
      'Because the single-pole switch must still interrupt the LINE — reverse polarity leaves the holder live when "off", a safety issue.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    id: 8,
    question:
      'After completing dead polarity testing, before energising for live polarity verification you should:',
    options: [
      'Re-make every termination, visually confirm each conductor to its correct terminal, refit lamps, finish any remaining dead tests, then energise.',
      'Leave the L-CPC link in place at the CU so the dead-test reference is retained, then energise and take live readings against that link.',
      'Energise immediately with the conductors still lifted at the CU, so the live polarity test runs against an isolated reference point.',
      'Refit the lamps but leave the protective-device terminals open, energising through a temporary supply lead for the live test.',
    ],
    correctAnswer: 0,
    explanation:
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Polarity testing per Reg 643.6. Confirming single-pole devices interrupt the line
          conductor (Reg 132.14), screw lampholder centre contacts to line, conductor identification
          at every accessory. Method, lamp removal requirement, and what reverse polarity means for
          safety.
        </p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              control and protective devices shall be connected in the line conductor only. This is
              the design rule.
            </li>
            <li>
            ...
            <li>
              <strong>Reg 643.6 — Verification of polarity.</strong> A test for polarity shall be
              made to verify that single-pole devices are connected in the line conductor only and
              that the centre contact of E14/E27 lampholders is connected to the line conductor.
              This is the verification rule.
            </li>
          </ul>
          <p>
            Reg 132.14 and 132.13 are what the designer / installer must do. Reg 643.6 is your test
            that confirms they did it correctly. The dead-test polarity check is the principal
            verification method during initial verification.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              single-pole interruption of an earthed neutral and requires linked switches in the
              neutral to break all related line conductors. Polarity verification (Reg 643.6) is the
              dead test that confirms compliance with both — it is not optional or 'best practice',
              it is a regulatory requirement for every initial verification and every periodic
              inspection.
            </>
          }
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              At the CU, disconnect the line of the circuit under test from the protective device.
              Disconnect the CPC from the earth bar.
            </li>
            <li>
              Bridge L and CPC together at the CU end with a low-resistance jumper (the same link
              used for R1+R2 measurement).
            </li>
            <li>Null the MFT continuity leads (touch probes together, press null).</li>
            <li>
              ...
              <ul className="space-y-1 list-disc pl-5 marker:text-white/40 mt-1">
                ...
                <li>Connect MFT probes to the conductor labelled 'line' and the local CPC.</li>
                <li>
                  Press TEST. Reading should be low (typically below 1 Ω, similar to your R1+R2
                  reading) — confirms the local 'line' is the same conductor as the CU line.
                </li>
              </ul>
            </li>
            <li>
              <strong>For switches:</strong> with the switch closed, test continuity between the
              line terminal of the supply side and the switched line terminal — should be low. With
              switch open, should be OL.
            </li>
            <li>
              <strong>For screw lampholders:</strong> verify the centre contact connects to line
              (continuity to the L-CPC link) and the screw thread connects to neutral.
            </li>
            <li>
              Document polarity confirmation per circuit on the Schedule of Test Results (a tick or
              pass mark in the polarity column).
            </li>
          </ol>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>OL between local L and local CPC:</strong> the conductor you call 'line' at
              the accessory is actually neutral — they have been swapped at the back-box.
              Investigate, correct the wiring, retest.
            </li>
            <li>
              <strong>Switch reads continuity in BOTH positions:</strong> the switch is bridged,
              faulty, or wired wrong. Investigate.
            </li>
            <li>
              <strong>Lampholder centre contact reads OL to L-CPC link:</strong> centre contact
              wired to neutral instead of line. Hazardous — the user touches a live screw thread
        ...
        <RegsCallout
          ...
          meaning={
            <>
              GN3 codifies the practical method and the gotchas. Verify at every accessory (not just
              sample). Remove lamps before testing (filament / driver paths can mask a polarity
              error). Centre contact of screw lampholders to line — this is the safety-critical case
              for E14/E27 holders. Visual + instrument together.
            </>
          }
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>Green-and-yellow</strong> — protective conductor (CPC, bonding, equipotential
              bonding) — exclusive use, never anything else
            </li>
          </ul>
          <p>Pre-2004 (legacy) colours you will still see on older installations:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              <strong>Yellow / blue / red</strong> — three-phase legacy. Sleeve over with brown /
              black / grey at terminations into modern equipment.
            </li>
            <li>
              <strong>Black</strong> — was neutral pre-2004; now reused as L2 in three-phase. A
              common confusion point on mixed installations.
            </li>
          </ul>
          <p>
            <strong>Visual polarity confirmation</strong> at every termination is part of the
            polarity test. The instrument test confirms the conductor identity electrically; the
            visual confirms the labelling matches. Mismatch = label error or terminal swap;
        ...
        <ContentEyebrow>
          Lampholders and switches — the access points where polarity matters most
        </ContentEyebrow>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Centre contact = line.</strong> The deeper, harder-to-touch contact at the
              bottom of the lampholder cup. Carries line voltage to the bulb.
            </li>
            <li>
              <strong>Screw thread = neutral.</strong> The exposed thread the user touches when
              unscrewing a bulb. At ~0 V to earth under normal conditions.
            </li>
            <li>
              <strong>Why this way:</strong> the user can touch the screw thread during normal
              bulb-change operations. Neutral on the thread = safe. Line on the thread = potentially
              fatal shock.
            </li>
          </ul>
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              Identify which terminal of the switch is the supply side (incoming line) and which is
              the load side (switched line going to the lampholder / appliance).
            </li>
            <li>
              With the L-CPC link in place at the CU, test continuity from the supply side of the
              switch to the local CPC. Should be low — supply side is the live line.
            </li>
            <li>
              Close the switch. Test continuity from the load side of the switch to the local CPC.
              Should also be low — switch closed = continuity through to the load.
            </li>
            <li>
              Open the switch. Re-test load side to CPC. Should be OL — switch open = no continuity
              to the load. Confirms the switch IS in the line.
            </li>
            <li>
        ...
        >
          <p>Three-phase polarity test points and what to check:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Supply origin / DB busbars</strong> — visual brown / black / grey colour
              identification, plus rotation indicator confirming L1-L2-L3.
            </li>
            <li>
              <strong>Sub-main / sub-DB</strong> — repeat at the sub-DB. Phases should carry
              through; if the sub-main was lifted and reterminated, rotation can flip.
            </li>
            <li>
              <strong>Three-phase outlets and isolators</strong> — rotation indicator socket; any 16
              A / 32 A / 63 A three-phase commando and CEEform outlets.
            </li>
            <li>
              <strong>Motor terminal box</strong> — pre-coupling, jog the motor and verify
              direction. Many motors are reversible by swapping two phases at the local isolator;
              some (single-direction pumps, conveyors with one-way bearings) need rotation right
              first time.
            </li>
            <li>
              <strong>VSDs and inverter loads</strong> — most modern VSDs are rotation-tolerant on
              the AC input but tag the input rotation on the terminal cover so anyone re-terminating
              preserves it.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Recognising and rectifying switched-neutral findings:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Polarity test signature</strong> — load-side continuity to CPC with the switch
              open. The single-pole switch is in the wrong conductor.
            </li>
            <li>
              <strong>Visual signature</strong> — black conductor (legacy neutral) running into the
              switch and back, with the red (legacy line) running straight through to the
              lampholder.
            </li>
            <li>
              <strong>Code on the EICR</strong> — typically C2 (potentially dangerous); the lamp at
              the affected fitting is energised at switch-off.
            </li>
            <li>
              <strong>Remediation</strong> — re-wire the switch into the line, or replace the
              affected light circuit. Document on the rectification certificate (MEIWC).
            </li>
            <li>
              <strong>Customer brief</strong> — explain the historic safety risk in plain terms;
              customer often appreciates the inspection picked it up.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              polarity is consistent at every accessory using the L-CPC link method. Catches wiring
              errors within the installation.
            </li>
            <li>
              <strong>Live test (Section 4 Sub 1, Reg 643.7.1).</strong> At energisation, use a
              voltage indicator or live polarity tester at the supply origin and at representative
              outlets. Confirms incoming supply polarity matches the install, and that switches
              operate correctly under load.
            </li>
          </ol>
          <p>
            Documentation: tick or pass mark in the polarity column on the STR per circuit. For a
            periodic inspection (EICR), polarity is one of the inspection items — recorded as
            observed pass / fail with any defects noted in the schedule of items inspected.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You test polarity at the CU outgoing terminals and the first socket. Both pass. You
              sign off the circuit. Three months later the customer rings: the metal lamp in the
              back bedroom has tingled the kid changing the bulb. Investigation: at the back-box of
              the bedroom switch, L and N were swapped during the install. The CU and first socket
              polarity were correct, but the swap at the bedroom switch put line on the screw thread
              of the lampholder when the switch was 'off'. A polarity test at every accessory would
              have caught it.
            </>
          }
          doInstead={
            <>
              Test polarity at every accessory on every circuit. The L-CPC link method makes the
              test fast — typically 10-15 seconds per accessory once the link is set up at the CU.
              The cost of testing every accessory is small; the cost of missing a swap can be
              electrocution. Document on the STR — a tick per circuit if all accessories pass, or a
              note of any failures and the corrective action taken.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              Two-bedroom flat in Liverpool. Refurbishment includes new lighting circuit: living
              room (1-gang switch, 4 GU10 LED downlights), hallway (2-way switching between front
              door and bedroom doors, single E27 pendant), main bedroom (1-gang switch, 1 E27
              pendant), spare bedroom (1-gang switch, 1 BC22 pendant). Wired in 1.5/1.0 mm² flat
              T&E. The continuity and IR tests have passed; now to polarity using the L-CPC link
              method on a Megger MFT1741+.
            </>
          }
          whatToDo={
            <>
              Safe isolation, lock off the lighting MCB. Lift L and CPC of the lighting circuit at
              the CU. Bridge them with a 4 mm² ferruled jumper. Null the MFT leads. Remove all bulbs
              from the circuit (filament / LED driver paths would mask polarity errors). Walk the
              circuit. At each switch: confirm visual identification (brown supply, brown switched,
              green/yellow CPC, blue neutral routed through if loop-in). Test continuity from the
              supply terminal to local CPC — low (~0.4 Ω, consistent with R1+R2 for the cable run).
              Close the switch — test continuity from the switched terminal to local CPC, also low.
              Open the switch — switched terminal to CPC reads OL. Confirms switch in line. At each
              E27 pendant: verify the centre contact lead (brown core) connects to line — low
              continuity from centre contact to local CPC. Verify the screw thread lead (blue core)
              connects to neutral — OL from thread to local CPC (because neutral is not in the L-CPC
              link). At the BC22 pendant: similar trace, switch operation as before. At the hallway
              2-way switching: verify both ends of the strappers are isolated when appropriate, and
              that the line through to the lampholder routes correctly via whichever switch is
              operated. All accessories pass. Document polarity tick per circuit on the STR. Remove
              L-CPC jumper, re-land L into the MCB, CPC into the earth bar, refit all bulbs, prepare
              for live polarity verification during the live test sequence.
            </>
          }
          whyItMatters={
            <>
              Lighting circuits are where polarity errors most often hurt the user — bulb changes
              are routine, screw lampholders give exposed access to the centre / thread contacts.
              The dead test at every accessory catches wiring errors before the customer or another
              tradesperson finds them the hard way. Combined with visual conductor identification,
              the polarity verification provides the evidence chain that supports the EIC sign-off —
              and gives the next inspector ten years from now the confidence to test the same
              circuit and see consistent results.
            </>
          }
        />
        ...
        >
          <p>Per-system polarity verification at energisation:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>TN-S</strong> — separate neutral and earth from the DNO. Live polarity at the
              cut-out: brown line, blue neutral, green-and-yellow earth (DNO sheath). Cross-check at
              the consumer unit MET.
            </li>
            <li>
              <strong>TN-C-S (PNB)</strong> — combined PEN from the DNO, split at the cut-out into N
              and PE. Polarity check verifies the N tail and the PE are correctly bonded at the
              cut-out and not swapped at the consumer unit terminals.
            </li>
            <li>
              <strong>TT</strong> — DNO supplies L and N only; customer's own earth electrode
              supplies the PE. Polarity check confirms the earth electrode conductor is connected to
              the MET, the electrode is in good condition, and Ze (Ra in TT terms) is within design.
            </li>
            <li>
              <strong>Live polarity tester</strong> — purpose-built tools like the Martindale
              VIPDLOK and the Megger LRCD are safer than a multimeter for sustained live work at the
              cut-out. GS38-compliant test leads with fused tips throughout.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              Every exposed-conductive-part which is to be protected by a single protective device
              shall be connected, via the main earthing terminal, to a common earth electrode.
              However, if two or more protective devices are in series, the exposed-conductive-parts
              may be connected to separate earth electrodes corresponding to each protective device.
            </>
          }
          meaning={
            <>
              Earth electrode resistance is the input variable that decides whether the TT
              installation can satisfy 411.5.3 (Ra × IΔn ≤ 50 V). The electrode test measures Ra;
              the RCD rating provides IΔn. Both are needed for the fault-protection calculation. A
              high electrode resistance can be tolerated only if the RCD trip current is low enough
              to keep the product under 50 V.
            </>
          }
