...
const checks = [
  {
    ...
    options: [
      ...
      'FI — further investigation required. The exposed conductor needs to be traced back to confirm whether the strain relief failure extends along the cable before a code can be assigned.',
  ...
  },
  {
    ...
    options: [
      ...
      'C3 — improvement recommended. RCD protection was not required when the installation was built, so its absence is only an upgrade suggestion and does not affect the safety status.',
      'No code — every test result is within limits, so the installation is satisfactory as found and the absence of an RCD is not recordable on the report.',
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: [
      ...
      'No code — conductor colours are a cosmetic identification matter outside the scope of an EICR, so the finding is noted in the comments rather than coded.',
  ...
  },
  {
    ...
    options: [
      ...
      'C2 — potentially dangerous. A consumer unit under wooden stairs on an escape route is potentially dangerous regardless of enclosure material, because a fire could impede escape.',
      'C1 — danger present. Locating a consumer unit under timber stairs is an immediate fire risk to the means of escape and must be made safe before the inspector leaves.',
    ],
    correctAnswer: 0,
    explanation:
