...
const quizQuestions = [
  ...
  },
  {
    id: 5,
    question:
      'On a 30 kW resistive electric heater bank at 400 V three-phase, balanced, the design current Ib is:',
    options: [
  ...
  },
  {
    id: 7,
    question:
      'A 230 V domestic ring final circuit serving a kitchen has connected load of 30 A worth of appliances. The On-Site Guide standard circuit assumption for ring final design current is:',
    options: [
...
const faqs = [
  ...
  {
    question:
      'Where do I find power factor for a load if the manufacturer datasheet does not state it?',
    answer:
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 3 · Subsection 1"
        title="Design current Ib — the first calc on every circuit"
        backTo="/study-centre/apprentice/level3-module6-section3"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Calculating the design current Ib for any final circuit. Resistive loads, inductive loads
          with power factor, three-phase, and the diversity adjustment that turns connected load
          into design current. The first number on every cable schedule and the input to BS 7671 Reg
          433.1.1.
        </p>
        ...
        >
          <p>
            In BS 7671 terminology Ib is the design current of a circuit — the current intended to
            be carried by the circuit in normal service. It is not the fault current (Ipfc), not the
            rated current of the protective device (In), and not the maximum current-carrying
            capacity of the cable (Iz). Ib is the demand the circuit has to deliver, and it is the
            input from which the protective device rating (In) and the cable size (Iz) are then
            chosen.
          </p>
          <p>
            The chain runs: assess the load → calculate Ib → choose In greater than or equal to Ib
            (Reg 433.1.1(a)) → choose a cable with Iz greater than or equal to In (Reg 433.1.1(b)) →
            verify Iz is greater than or equal to In and that the device operating current does not
            exceed 1.45 x Iz (Reg 433.1.1(c)) → check voltage drop (Reg 525) → check Zs and
            disconnection time (Reg 411). Every step uses Ib as the starting input. An incorrect Ib
            propagates through the whole calc.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              protection: the device rating must be at least the design current (so the circuit can
              run normally without nuisance trip) and at most the cable capacity (so the cable does
              not get damaged before the device operates). The 1.45 x Iz limit covers the small
              overcurrent zone where a device has not yet operated but the cable is being stressed.
              Get Ib accurate and the whole chain falls into place; get it wrong and the chain
              breaks.
            </>
          }
        ...
        >
          <p>For a resistive load on a single-phase 230 V supply:</p>
          <p className="font-mono bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-[13px]">
            Ib = P / V
          </p>
          <p>Where P is the rated power in watts and V is the nominal supply voltage. Examples:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              3 kW immersion heater at 230 V: Ib = 3000 / 230 = 13.04 A. Round up to 14 A. Standard
              16 A protected radial.
            </li>
            <li>
              9 kW electric shower at 230 V: Ib = 9000 / 230 = 39.13 A. Round up to 40 A. Standard
              40 A protected radial, 6 mm² T&E (subject to Reference Method derate).
            </li>
            <li>
              2 kW kettle on a ring final: Ib = 2000 / 230 = 8.7 A. Adds to ring loading; ring still
              at 32 A protective device.
            </li>
            <li>
              10 kW shower at 230 V: Ib = 10000 / 230 = 43.5 A. Round up to 45 A. Use 45 A or 50 A
              protected radial, 10 mm² T&E.
            </li>
            <li>
              1.5 kW LED lighting array at 230 V (treating as resistive — see below): Ib = 1500 /
              230 = 6.5 A. Standard 6 A or 10 A lighting circuit.
            </li>
          </ul>
          <p>
            Pure resistive loads — heating elements, incandescent and halogen lamps, electric
            showers, immersions, ovens with element heating only — have power factor cos φ = 1 and
            the formula is straightforward. Watch out for LED lighting and switchmode supplies: at
            the lamp / driver level the power factor can vary with quality, but for circuit-level
            design the assumption is generally cos φ near 1 for the lighting circuit overall.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <p>
            Why the cos φ term? Inductive loads draw both real power (P, watts, doing useful work)
            and reactive power (Q, VAr, magnetising the inductance). The vector sum is apparent
            power S in volt-amps:
          </p>
          <p className="font-mono bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-[13px]">
            S = sqrt(P² + Q²) = P / cos φ
          </p>
          <p>
            The line current carries S, not P. So Ib = S / V = P / (V x cos φ). The cable sees the
            larger apparent-power current, and the design current is correspondingly larger than the
            resistive equivalent for the same useful work. Examples:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              4 kW single-phase motor at 230 V, cos φ 0.85: Ib = 4000 / (230 x 0.85) = 20.46 A.
              Round to 21 A.
            </li>
            <li>
              2.2 kW single-phase fan motor at 230 V, cos φ 0.80: Ib = 2200 / (230 x 0.80) = 11.96
              A. Round to 13 A.
            </li>
            <li>
              1 kW magnetic-ballast fluorescent fitting at 230 V, cos φ 0.90 (uncorrected): Ib =
              1000 / (230 x 0.90) = 4.83 A.
            </li>
            <li>
              0.5 kW electronic transformer for low-voltage halogen at 230 V, cos φ 0.95: Ib = 500 /
              (230 x 0.95) = 2.29 A.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            does not have to be drawn from the supply. The effect: cos φ at the supply moves from
            (say) 0.75 lag towards 0.95 lag or 1.0, and the line current Ib falls correspondingly.
          </p>
          <p>
            Example: a 22 kW three-phase motor at 0.75 cos φ uncorrected: Ib = 22000 / (1.732 x 400
            x 0.75) = 42.3 A. With PFC to 0.95 cos φ: Ib = 22000 / (1.732 x 400 x 0.95) = 33.4 A — a
            21 percent reduction. The cable size, intake fuse and supply size all benefit. PFC is
            therefore a design choice on inductive-heavy installations (workshops, commercial
            kitchens, refrigeration plant) that pays back through smaller supply, smaller cables,
            lower DNO charges and lower power-factor penalties on the electricity bill.
          </p>
          <p>
            ...
            near-unity displacement power factor at the supply but inject harmonics that have their
            own diversity / sizing implications (see industrial load assessment Sub 2.5).
          </p>
        </ConceptBlock>
        ...
        >
          <p>For a balanced three-phase load:</p>
          <p className="font-mono bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-[13px]">
            Ib = P / (sqrt(3) x VL x cos φ)
          </p>
          <p>
            Where P is the total real power (watts), VL is the line-to-line voltage (400 V on UK LV
            three-phase), cos φ is the load power factor and sqrt(3) approx 1.732. The formula gives
            the line current — the current in any one line conductor of a balanced three-phase
            system. Examples:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              30 kW resistive heater bank, 400 V three-phase, cos φ 1: Ib = 30000 / (1.732 x 400) =
              43.3 A. Round to 44 A.
            </li>
            <li>
              22 kW three-phase induction motor, 400 V, cos φ 0.86, efficiency 0.93. Input power =
              22000 / 0.93 = 23656 W. Ib = 23656 / (1.732 x 400 x 0.86) = 39.7 A.
            </li>
            <li>
              11 kW three-phase EV rapid charger, 400 V, cos φ 0.99 (modern AC charger): Ib = 11000
              / (1.732 x 400 x 0.99) = 16.05 A. Round to 16 A on dedicated circuit.
            </li>
            <li>
              50 kW three-phase commercial kitchen mixed load, balanced, cos φ 0.92: Ib = 50000 /
              (1.732 x 400 x 0.92) = 78.5 A.
            </li>
          </ul>
          <p>
            For unbalanced three-phase, calculate per phase (treat each line as single-phase with
            phase-to-neutral voltage 230 V) and report the heaviest phase. The neutral conductor
            carries the imbalance; on harmonic-rich loads (LED-heavy lighting, multiple VSDs) the
            third-harmonic neutral current can exceed the phase current and require upsized neutral
            per Appendix 4.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            In a three-phase wye (star) system, the line-to-line voltage VL and the line-to-neutral
            voltage VLN are related by:
          </p>
          <p className="font-mono bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-[13px]">
            VL = sqrt(3) x VLN
          </p>
          <p>
            ...
            ratio 400 / 230 = 1.739, very close to sqrt(3) (the small discrepancy is the BS EN 60038
            harmonised tolerance band).
          </p>
          <p>Total three-phase power for a balanced load:</p>
          <p className="font-mono bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-[13px]">
            P = sqrt(3) x VL x IL x cos φ
          </p>
          <p>Re-arranging for the line current:</p>
          <p className="font-mono bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-2 text-[13px]">
            IL = P / (sqrt(3) x VL x cos φ)
          </p>
          <p>
            That IL is what BS 7671 calls Ib for the three-phase circuit. The same conductor carries
            the same line current regardless of whether the load is connected in star or delta,
            provided the load is balanced. The cable size is then selected on the per-conductor
            basis with appropriate Reference Method and grouping factors.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Ring final circuits (multiple sockets, mixed appliance use) — built into the standard
              arrangement at 32 A.
            </li>
            <li>
              Cooker circuits (hob plus oven plus extras on a single connection unit) — On-Site
              Guide formula: first 10 A at 100 percent + 30 percent of remainder + 5 A if the
              connection unit feeds a 13 A socket.
            </li>
            <li>
              Lighting circuits (multiple lamps, simultaneity less than 100 percent on larger
              circuits) — diversity in the order of 75-90 percent depending on installation type.
            </li>
            <li>
              Multi-outlet small power on commercial floors — assess by tenant fit-out category.
            </li>
          </ul>
          <p>Diversity is NOT applied to:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Dedicated single-load circuits — shower, immersion, EV charger, single oven, single
              hob, dedicated heat-pump compressor. The load draws full rated current when on; design
              at rated current.
            </li>
            <li>
              Continuous-rated loads — anything intended to run at full load for prolonged periods
              (commercial fridge plant, server-room AC, life-safety equipment).
            </li>
            <li>
              Fixed mechanical loads where the duty is known — for example a lift designed to run at
              100 percent during peak, with starting allowance added.
            </li>
          </ul>
          <p>
            The L3 designer's discipline is to know the difference. The protective device for an EV
            charger circuit is sized at 32 A because Ib is 30.4 A (no diversity); the protective
            device for a kitchen ring final is sized at 32 A because the standard arrangement covers
            diversity statistically. Same protective device rating, two entirely different design
            routes.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              calculation. It permits — but does not require — diversity to be applied. The designer
              judges where it is engineering-justified (multi-outlet, multi-load circuits with
              statistical simultaneity) and where it is not (dedicated single-load circuits). The
              On-Site Guide Appendix A and IET Guidance Note 1 give standard diversity assumptions
              for typical installation types; the designer cites the source on the cable schedule.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              230 = 17.4 A per motor. You spec a 20 A device and 2.5 mm² cable. The motors actually
              draw 4000 / (230 x 0.85) = 20.5 A continuously, and the starting transient takes them
              to 130 A peak. The 20 A device starts nuisance-tripping; the 2.5 mm² cable runs hot
              under sustained 20-21 A and the insulation degrades over months. The engineering
              report after the failure points straight at the missing cos φ term.
            </>
          }
          doInstead={
            <>
              On any load that is not pure resistive, look up cos φ on the manufacturer datasheet or
              the motor catalogue, or use a conservative default (0.85 for general motors, 0.80 for
              older fluorescent banks, 0.95 for modern electronics). Apply the formula Ib = P / (V x
              cos φ) for single-phase or Ib = P / (sqrt(3) x VL x cos φ) for three-phase. Always
              state the cos φ assumption on the cable schedule against the circuit row so it can be
              verified.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              staff. Cumulative concurrent draw routinely 30-35 A at 9 a.m. The ring final trips,
              recovers, trips again. The protective device replacement does not fix it. The
              diversity assumption was wrong — domestic 'sockets diverse' became commercial 'kitchen
              kit on simultaneously'.
            </>
          }
          doInstead={
            <>
              On commercial small power, assess the actual load profile rather than dropping the
              domestic ring assumption in. For a busy break-room consider a 32 A ring with the heavy
              appliances on dedicated radials (3 kW kettle on a 13 A FCU off the ring is fine; two 3
              kW kettles in concurrent use need their own provision). For server rooms, A/V suites
              or any continuous-load environment, design for the assessed continuous draw with no
              diversity. The On-Site Guide ring assumptions are domestic-rated; commercial design
              needs commercial assumptions.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              ...
              (replacing a 7.5 kW unit), a 7 kW EV charger on the driveway, and an 8 kW air-source
              heat pump replacing the gas boiler. Existing: lighting, sockets ring, immersion,
              cooker — to be re-protected on the new RCBO board.
            </>
          }
          whatToDo={
            <>
              Calculate Ib for each new circuit individually. Shower 9.5 kW resistive at 230 V: Ib =
              9500 / 230 = 41.3 A; spec a 45 A or 50 A RCBO and 10 mm² T&E. EV charger 7 kW at 230 V
              (manufacturer states cos φ near unity at full charge): Ib = 7000 / 230 = 30.4 A; spec
              32 A Type B RCBO with O-PEN protection (TN-C-S supply), 6 mm² T&E or 4 mm² 6242Y SWA
              depending on route. Heat pump 8 kW at 230 V single-phase, cos φ 0.95 (manufacturer
              datasheet): Ib = 8000 / (230 x 0.95) = 36.6 A; spec 40 A Type B or C RCBO depending on
              starting transient (compressor inrush), 6 mm² T&E. Existing circuits keep their
              existing Ib (lighting, sockets) — re-derive only if loads changed. Sum all circuit Ib
              into the load schedule with diversity at consolidation, then verify the 100 A intake
              fuse is sufficient for the consolidated MD.
            </>
          }
          whyItMatters={
            <>
              Per-circuit Ib is the foundation. The shower needs a 45-50 A device because Ib is 41.3
              A; a smaller device nuisance-trips. The EV charger needs 32 A because Ib is 30.4 A;
              smaller fails to deliver full charge. The heat pump needs 40 A because Ib is 36.6 A
              and the compressor inrush needs starting headroom. Each circuit designed to its own
              Ib. The consolidated schedule then verifies the 100 A supply handles the combined
              demand under diversity. Skip Ib at the circuit level and the board ends up
              nuisance-tripping on the first cold morning when shower, EV and heat pump all run.
            </>
          }
        />
        ...
        >
          <p>
            Loads with rectifier front ends (LED drivers, switchmode power supplies, computer power
            supplies, single-phase VSDs) draw current in chopped pulses near the voltage peaks. The
            result:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              The RMS line current is higher than the average current predicted by P / V — the crest
              factor of the current is greater than the sinusoidal sqrt(2) = 1.414.
            </li>
            <li>
              Harmonic distortion appears on the supply — third (150 Hz), fifth (250 Hz), seventh
              (350 Hz) and so on.
            </li>
            <li>
              On three-phase systems, third-harmonic currents in each phase are in-phase with each
              other (triplen harmonics) and add in the neutral conductor — the neutral can carry up
              to 1.73 x the phase current on heavily harmonic loads.
            </li>
          </ul>
          <p>
            For Ib calc on harmonic-rich circuits: use the formula as normal but apply a harmonic
            loading factor (typically 1.10 to 1.25 for LED-heavy lighting, 1.30 plus for VSD-heavy
            industrial) to the cable sizing. Reg 524.1 and Appendix 4 give the rating-factor
            methodology. For residential and small commercial work the effect is small enough to
            absorb in the standard derate; for serious VSD installations it is a specific design
            step.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Protective device coordination — the disconnection time at fault depends on Zs, which
              depends on Ze (TN-S typically 0.8 ohms maximum, TN-C-S typically 0.35 ohms, TT can be
              100 ohms or more) and on the cable R1+R2.
            </li>
            <li>
              EV charger O-PEN protection — required on TN-C-S per Reg 722.411.4.1, achieved either
              by purpose-designed charger (built-in O-PEN protection) or by an earth electrode
              arrangement that effectively converts the charger to TT for fault purposes.
            </li>
            <li>
              Disconnection time — TN supplies use OPD or RCD to meet Table 41.1 (0.4 s for
              socket-outlet circuits up to 63 A on a 230 V system); TT supplies usually rely on RCD
              additional protection because Zs is too high for OPD-only disconnection.
            </li>
          </ul>
          <p>
            The L3 designer calculates Ib first, then the protection scheme based on the supply
            earthing. The cable size and protective device rating come out the same; the earthing
            and the additional-protection requirements differ.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              For TN-S, TN-C-S and TN-C circuits the earthing relies on the supply-side conductor
              remaining intact. The distributor carries the duty for the external portion; the
              designer carries the duty for the installation-side. A circuit designed for a TN
              system that is later found to be on a different earthing arrangement may not satisfy
              ADS — the earthing arrangement is a fundamental input to the design.
            </>
          }
