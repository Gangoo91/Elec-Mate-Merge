...
const TITLE = 'Reference Methods A–G and 100–103 (4.2) | Level 3 Module 6.4.2 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  ...
  {
    id: 5,
    question: 'Method 102 in BS 7671 Appendix 4 covers which install?',
    options: [
...
const faqs = [
  {
    question:
      'Why does BS 7671 not just publish one number for each cable and stop messing about with methods?',
    answer:
  ...
  {
    question:
      'What is the difference between Method 100 and Method 103 — they both involve total enclosure in insulation?',
    answer:
  ...
  {
    question:
      "Where does the 'in conduit in a thermally insulated wall' Method A actually show up on real jobs?",
    answer:
  ...
  {
    question:
      'Can I use Method E figures for a cable that I have just spaced from a wall with proprietary cleats?',
    answer:
...
export default function Sub2() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 4 · Subsection 2"
        title="Reference Methods A–G and 100–103"
        backTo="/study-centre/apprentice/level3-module6-section4"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The BS 7671 install-method catalogue. Methods A through G for cable in conduit, in
          trunking, on tray, clipped or in free air; Methods 100–103 for cable in thermal
          insulation. Practical examples for each, plus the worst-section-governs rule that decides
          the calc when a single cable crosses two or three environments.
        </p>
        ...
        >
          <p>
            When you read a column of Appendix 4 Table 4D5, the It value is the answer to a specific
            question: at the reference ambient of 30 °C, with the cable installed in this exact
            configuration, what is the maximum continuous current that brings the conductor to its
            insulation-temperature limit. Different configurations dissipate heat differently, so
            different methods give different It values for the same cable.
          </p>
          <p>
            The classification matters because heat-dissipation modelling is intricate — the IEC
            committees that produce these values run heat-balance calculations that account for
            cable geometry, surrounding-medium thermal conductivity, convective and radiative
            cooling, and any thermal coupling between adjacent cables. Each Reference Method is the
            result of one such calculation. There is no universal safe CCC; only CCC for a given
            install.
          </p>
          <p>
            The standard publishes around a dozen methods (A through G in the everyday catalogue,
            plus 100–103 for thermal insulation cases, plus a few specialised methods for buried
            cables and free air). For the L3 designer, the workload is to pick the right method for
            each section of each circuit and apply the matching column.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              determination. Equivalent safety opens the door to manufacturer data or specialist
              software for non-standard cables, but for everyday work the Appendix 4 / OSG route is
              what an inspector expects to see in the design paperwork. The Reference Method is part
              of the calc you must record.
            </>
          }
        ...
        >
          <p>
            Geometry: a single insulated conductor (or a small group of conductors) inside a conduit
            that is itself buried in or against an insulated wall. Heat dissipation is limited
            because:
          </p>
          ...
          <p>
            Result: among the lowest CCC of any method for the same cable. Used wherever conduit is
            concealed in insulated construction — typical commercial fit-out walls and modern
            domestic stud partitions.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Geometry: single insulated conductors in a conduit clipped to a wall in the open. The
            wall provides one face of heat-sinking; the conduit’s outer surface convects freely into
            the room air. CCC is higher than Method A but lower than Method C (clipped direct)
            because the conduit’s trapped air still adds a thermal layer between the cable and the
            room.
          </p>
          <p>
            Watch the conduit fill. BS 7671 Method B implicitly assumes the conduit is not packed
            full of cables — if you have stuffed eight singles into a 20 mm conduit, the spacing
            factor is exceeded and you are no longer at Method B CCC. The cables are in effect
            grouped within the conduit and you must apply Cg on top.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Geometry: a multi-core cable clipped direct to a non-metallic surface (timber joist,
            plasterboard, plaster) in still air. The surface is non-metallic so it does not function
            as a meaningful heat sink, and the cable is in open air on at least three of four faces.
          </p>
          <p>
            Method C is the dominant install method on domestic final circuits. A typical T&E radial
            in a loft is Method C for any section that is not buried in insulation; an extension to
            that radial that runs in the wall cavity (clipped to studs in still air) is also Method
            C; a sub-main from a CU to a tail-fed garage that is clipped to a brick wall outside is
            Method C as well.
          </p>
          <p>
            The cable must genuinely be in still air for Method C to apply. If the cable is sat in a
            wall cavity that is filled with insulation around it, you have moved to Method 102 or
            103. If the cable is clipped to a metallic surface (e.g. a steel beam or a metal-clad
            surface), you have moved to a different effective method that BS 7671 sometimes treats
            more generously and sometimes more conservatively depending on the metal’s thermal mass.
          </p>
        </ConceptBlock>
        ...
        >
          <p>Geometry requirements (and they are tight):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>
            Miss any one of those and you are no longer in Method E. Stack the cables — you are
            grouped, Cg applies. Run them touching — grouping again. Lay them on a solid sheet
            rather than perforated tray — cooling is reduced and the CCC drops.
          </p>
          <p>
            On commercial designs, Method E is often the cheapest way to size a sub-main — a 70 mm²
            thermosetting four-core in Method E carries appreciably more than the same cable in
            Method C, sometimes enough to drop a bracket and save real money on copper. The
            trade-off is the install: a perforated ladder with proper spacing is more labour and
            more material than a clipped run.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Geometry: cable buried at the standard reference depth (usually 0.7 m for direct burial;
            0.6 m for duct) in soil of standard thermal resistivity (2.5 K·m/W is the BS 7671
            reference). The tabulated CCC values assume those conditions; corrections are applied
            for non-standard depth, non-standard resistivity, or grouped buried cables.
          </p>
          <p>
            Damp clay has thermal resistivity around 1.0 K·m/W (carries heat better than the
            reference) so the correction is above 1.0 — you can carry more current. Dry sand can hit
            3.0 K·m/W (worse than the reference) and the correction goes below 1.0. BS 7671 Table
            4B3 gives the values, and good practice is to do a soil thermal-resistivity test before
            designing any large buried sub-main, especially on new-build sites where the topsoil has
            been disturbed.
          </p>
        </ConceptBlock>
        ...
        >
          <p>The four methods cover increasing degrees of thermal isolation:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Method 100</strong> — cable totally surrounded by thermal insulation for a
              length under 0.5 m. The brief excursion case: a cable that pops through an insulated
              wall plate, or briefly traverses an insulated zone before re-emerging into clear air.
              Derate is modest because the surrounding cable in clear air helps cool the embedded
              section by conduction along the conductor.
            </li>
            <li>
              <strong>Method 101</strong> — cable in contact with a thermally insulated wall on one
              face, the other face in clear air. Common where T&E sits against the back of a stud
              cavity insulation board with the room face of the cavity vented or boarded but not
              insulated. Derate is meaningful but moderate — the cable can still convect on three
              faces.
            </li>
            <li>
              <strong>Method 102</strong> — cable embedded in insulation on more than one face
              (typically two or three faces) but not totally enclosed. The everyday partial-fill
              case: T&E in a stud wall sat between layers of mineral wool, with air gaps top and
              bottom. Derate is significant.
            </li>
            <li>
              <strong>Method 103</strong> — cable totally enclosed in thermal insulation for a
              length of 0.5 m or longer. The worst case: full burial in loft mineral wool, full
              encapsulation in foam-filled cavities, or full surround by spray-foam insulation.
              Derate is brutal — typically 35–50 percent of the Method C value for the same cable.
            </li>
          </ul>
          <p>
            <strong>Critical:</strong> the Ci correction for thermal insulation is built into the
            tabulated It values for Methods 100–103. You do <em>not</em> apply Ci as a separate
            multiplier on top — that would double-count the derate and over-engineer the cable. Pick
            the method, read the column, apply only Ca and Cg from there.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The standard is unambiguous: if you have picked a Method 100–103 column, the
              in-insulation derate is already in the number, so do not apply Ci on top. If the
              install is unusual and not covered by the four methods (e.g. cable touching insulation
              only on one short section of a longer run), the standard lets you derive a custom Ci
              from first principles — but for the everyday cases, picking the right method is the
              entire job.
            </>
          }
        ...
        >
          <p>
            The principle is thermal: a cable’s steady-state temperature in any section is set by
            the heat-balance in <em>that section</em>, not the average across the run. If the 3 m
            loft-insulation section reaches 70 °C while the rest of the run is sat at 50 °C, the
            cable as a whole is at its operating-temperature limit — the rest of the run’s headroom
            does not help.
          </p>
          <p>
            Mechanically, the rule means you must walk the route on site (or read the survey
            carefully), identify every change of install method, and pick the one that gives the
            lowest tabulated It at your candidate CSA. That is the column you take into the calc.
          </p>
          <p>
            The reverse is also true: if you can re-engineer the worst section out of the run, the
            calc resolves at a smaller CSA. Clear a 3 m channel through the loft insulation so the
            cable runs in clear air and the entire calc moves from Method 103 to Method C, often
            saving a CSA bracket. The labour cost of clearing the channel is usually less than the
            material cost of the larger cable, plus the heavier glands and terminations it brings.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              This is the verbatim worst-section-governs rule. The standard puts it in Section 5.1
              of Appendix 4 because it applies universally — to install methods, to grouping, to
              ambient temperatures. Wherever a cable’s conditions vary along its length, the calc
              must be done for the worst conditions. Inspectors will look for evidence in the design
              paperwork that you actually walked the route and identified the worst section,
              especially on installs where loft insulation, sub-floor voids or external runs are
              involved.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              The drawing says &quot;ladder rack, single layer&quot; and the apprentice reads Method
              E off the table. On site the cables end up touching each other, single layer but no
              spacing, and on a warm summer afternoon two of the bunched cables are already running
              at the cable’s temperature limit. The CCC the calc claimed is 15–20 percent above what
              the install actually delivers — and the cable ages prematurely.
            </>
          }
          doInstead={
            <>
              Method E demands at least one cable diameter spacing between cables on the tray, a
              perforated ladder or tray (not a solid sheet), and free air all around the run. If the
              tray is solid, the cables are touching, or the run is in a tight ceiling void with no
              air movement, you are not in Method E — you are in Method C with grouping (Cg) on top.
              Always validate the geometry against the table’s geometric requirements before reading
              the column.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You are designing a 100 A sub-main from a domestic CU to a detached garage 60 m away.
              The route is mostly external SWA clipped to a fence (Method E, generous CCC) and then
              enters the garage through a wall plate that the customer has just had spray-foamed.
              The last 4 m of the run is totally enclosed in spray foam — Method 103 territory. At
              100 A the SWA you specced for Method E is borderline; the same SWA in Method 103 fails
              the calc by 12 A.
            </>
          }
          whatToDo={
            <>
              Two engineering options. (1) Re-size the entire 60 m run to satisfy Method 103 across
              that 4 m section — expensive, because the bigger cable also drives bigger glands,
              bigger terminations and a heavier pull. (2) Re-engineer the 4 m so it is no longer
              Method 103: clear a sleeve through the spray foam, run the cable in conduit with an
              air gap, or re-route the wall entry to a non-foamed area. Option 2 is almost always
              cheaper and gives a better install. Document the design choice and the install detail
              on the cable schedule.
            </>
          }
          whyItMatters={
            <>
              Modern building regulations are pushing more and more thermal insulation into the
              building envelope, and electricians are increasingly meeting Method 103 conditions on
              routes that historically would have been Method C all the way. The rule of thumb: any
              cable that crosses an insulated envelope wall will spend a few metres in Method 102 or
              103. Plan for it at design stage rather than discover it on site.
            </>
          }
        />
