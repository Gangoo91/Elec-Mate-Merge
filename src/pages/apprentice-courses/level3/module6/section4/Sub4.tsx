...
const TITLE = 'Voltage drop design — mV/A/m method (4.4) | Level 3 Module 6.4.4 | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  {
    ...
    options: ['≈7.01 V', '≈70.1 V', '7.0 V', '≈7.30 V'],
    correctIndex: 0,
    explanation:
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: ['5 percent', '1 percent', '3 percent', '10 percent'],
    correctAnswer: 2,
    explanation:
  ...
  {
    id: 3,
    question: 'The voltage drop formula Vd = (mV/A/m × Ib × L) / 1000 — why divide by 1000?',
    options: [
  ...
  {
    id: 4,
    question: 'A 25 m lighting radial at 6 A on 1.5 mm² T&E (mV/A/m ≈ 29) gives Vd of:',
    options: ['4.35 V', '1.5 V', '7.2 V', '14.5 V'],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: ['2.3 V', '23.0 V', '11.5 V', '6.9 V'],
    correctAnswer: 3,
    explanation:
...
const faqs = [
  ...
  {
    question:
      'Why is the percentage measured against 230 V (or 400 V) rather than the actual supply voltage at the property?',
    answer:
  ...
  {
    question:
      'Does the Vd limit apply to the cable in isolation, or to the cumulative drop from supply to terminals?',
    answer:
  ...
  {
    question:
      'How do I do voltage drop on a circuit with multiple branches, like a lighting radial with several luminaires?',
    answer:
...
export default function Sub4() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 4 · Subsection 4"
        title="Voltage drop design — the mV/A/m method"
        backTo="/study-centre/apprentice/level3-module6-section4"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The Vd gate end to end. mV/A/m method, the 3 percent lighting / 5 percent other-loads
          limits from BS 7671 Appendix 4 §6.4, percentage conversion against the nominal supply
          voltage, and engineering judgement on when to size up rather than scrape the limit.
        </p>
        ...
        >
          <p>
            Every cable has resistance. Push current through that resistance and you lose volts — P
            = I²R is the cable’s heat output (the CCC concern), and V = IR is the same physics
            expressed as the voltage lost across the cable’s length (the Vd concern). For a typical
            6 mm² T&E carrying 32 A over 25 m, the loop resistance is around 0.154 Ω and the lost
            voltage is 32 × 0.154 = 4.93 V. That is roughly 2.1 percent of 230 V — fine for a
            sockets circuit.
          </p>
          <p>
            The same current through a 1.5 mm² cable over the same 25 m gives a loop resistance
            closer to 0.61 Ω and a lost voltage of 32 × 0.61 = 19.5 V — 8.5 percent of 230 V,
            comfortably non-compliant. The Vd gate is what stops you putting a 32 A radial on 1.5
            mm² cable for a long run, even if the CCC happened to allow it (which it does not, but
            the example illustrates the principle).
          </p>
          <p>
            The mV/A/m figure published in BS 7671 Appendix 4 condenses all of this into a single
            number per CSA per cable family per phase configuration: the millivolt drop, per amp of
            load, per metre of route length. Multiply mV/A/m by Ib by L, divide by 1000 (to convert
            millivolts to volts), and you have the Vd in volts. Done.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              regulation tells you to read those values and not exceed them. The limits themselves
              are 3 percent for lighting circuits and 5 percent for other circuits, measured from
              the origin of the installation through to the load terminals (not just the cable in
              isolation). Sub-mains plus final circuits in series must add up to at most the limit.
            </>
          }
        ...
        >
          <p>The structure of the formula:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>mV/A/m</strong> — read from the BS 7671 Appendix 4 table matching your cable
              family (4D5 for 70 °C T&E, 4E5 for 90 °C thermosetting two-core, 4D2 / 4E2 for
              singles, etc.). The figure is in millivolts, per amp, per metre.
            </li>
            <li>
              <strong>Ib</strong> — the design current of the circuit (Sub 1). This is the
              steady-state load current you used in the CCC calc, not the device rating In.
            </li>
            <li>
              <strong>L</strong> — the cable route length in metres, one way (the loop is already in
              the mV/A/m for two-core; for three-phase the figure is line-to-line and assumes
              balanced load).
            </li>
            <li>
          ...
          <p>
            Indicative mV/A/m values from BS 7671 Appendix 4 Table 4D5 (70 °C T&E, two-core,
            single-phase):
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>
            Notice the pattern — mV/A/m drops roughly in proportion to CSA. A 6 mm² cable has
            roughly half the mV/A/m of a 4 mm²; a 10 mm² has roughly 60 percent of the 6 mm². The Vd
            gate is the most CSA-sensitive of the cable selection gates, which is why it is the most
            common driver of size-up decisions on long runs.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The Appendix 4 §6.4 limits, expressed both as percentages and as volts on a 230 V
            single-phase supply:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Lighting circuits:</strong> 3 percent of nominal voltage = 6.9 V on 230 V
              single-phase, 12 V on 400 V three-phase line-to-line.
            </li>
            <li>
              <strong>
                Other circuits (including socket-outlets, fixed appliances, motor loads):
              </strong>{' '}
              5 percent of nominal voltage = 11.5 V on 230 V single-phase, 20 V on 400 V three-phase
              line-to-line.
            </li>
          </ul>
          <p>
            The limits are cumulative from the origin of the installation (typically the
            consumer-unit terminals, sometimes the supply intake) through to the load terminals. On
            a system with a sub-main feeding a sub-board feeding a final circuit, the three Vd
            contributions in series must add up to at most the overall limit. That is why sub-mains
            are usually designed to an internal target of around 2 percent — the final circuits
            downstream still need their 3 percent of headroom for lighting compliance.
          </p>
          <p>
            The historical rationale for the 3 percent lighting limit is incandescent and discharge
            lamp behaviour. Filament lamps lose noticeably more output below 220 V at the
            lampholder; fluorescent ballasts can stop striking; older dimmers behave erratically.
            Modern LED drivers tolerate the wider range much better, but the regulation has not been
            relaxed because lighting installations still mix legacy and new gear, and decorative /
            architectural lighting often uses voltage-sensitive control gear.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Appendix 4 §6.4 publishes the actual numbers that Reg 525.202 references. The phrase
              &quot;in the absence of other considerations&quot; matters — the regulation
              acknowledges that some equipment requires tighter Vd (motors with soft-start
              sensitivity, precision instrumentation, certain medical equipment) or accepts looser
              Vd (some industrial loads, contactor coils with wide voltage tolerance). For everyday
              domestic and commercial work, the 3 percent / 5 percent figures are the design floor.
            </>
          }
        ...
        >
          <p>
            For three-phase balanced loads, the BS 7671 Appendix 4 tables publish a single mV/A/m
            figure that represents the line-to-line voltage drop. The formula is the same as for
            single-phase:
          </p>
          <p>
          ...
          <p>
            The percentage check is against the nominal three-phase line-to-line voltage (400 V in
            the UK), not the single-phase line-to-neutral voltage. So 5 percent of 400 V = 20 V is
            the non-lighting limit; 3 percent = 12 V is the lighting limit.
          </p>
          <p>Indicative mV/A/m values for 70 °C thermoplastic four-core (Table 4D2):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>
            For unbalanced three-phase loads (single-phase loads connected line-to-neutral on
            different phases), the maths gets more complex — the neutral carries return current and
            contributes its own Vd. For typical commercial work with reasonably balanced loads, the
            standard’s line-to-line figure is good enough; for large unbalanced installations,
            specialist software handles the maths properly.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For small CSAs, the cable’s impedance is dominated by its resistance and the reactance
            is negligible. The standard publishes a single mV/A/m figure that bakes the (small)
            reactance contribution in at the assumed load power factor. For larger CSAs, the
            reactance becomes a meaningful fraction of the impedance, and the actual Vd depends on
            the load’s power factor.
          </p>
          <p>
            Above 70 mm² (or so — the threshold varies slightly by cable family), Appendix 4
            publishes three columns:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>mVR</strong> — resistive component of the voltage drop, in mV/A/m. Used
              directly for unity-power-factor loads (resistive heating, incandescent lighting).
            </li>
            <li>
              <strong>mVX</strong> — reactive component, in mV/A/m. Used directly for purely
              reactive loads (rare in practice).
            </li>
            <li>
              <strong>mVZ</strong> — the magnitude of the impedance vector, in mV/A/m. Used as a
              worst-case figure assuming the load draws current at the impedance angle of the cable;
              conservatively over-estimates Vd for typical loads.
            </li>
          </ul>
          <p>For a load with power factor cos φ and angle φ:</p>
          <p>
            <strong>Vd = (mVR × cos φ + mVX × sin φ) × Ib × L / 1000</strong>
          </p>
          <p>
            The cos φ term reflects the in-phase (resistive) current; the sin φ term reflects the
            out-of-phase (reactive) current. For a typical motor load at PF = 0.85, the formula
            gives a Vd somewhere between the pure mVR and the pure mVZ figures. For everyday
            small-CSA design you do not need this level of detail; it kicks in on industrial
            sub-mains.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A4:2026 renumbered the design-documentation duty from the old 132.12 to 132.13. The
              duty bites on Vd as much as on any other gate — your mV/A/m calc, the cable family
              used, the assumed Ib and the route length all need to be on the design pack so the
              next person looking at the install can see how the cable was sized. Reg 536.5
              (selectivity) explicitly bolts coordination documentation onto the 132.13 pack at
        ...
        >
          <p>
            Vd is directly proportional to load current. A 10 percent uplift in Ib gives a 10
            percent uplift in Vd. So a circuit that clears at 4.95 percent on first install fails at
            5.45 percent after a modest load increase — perhaps the customer adds a heat pump (Ib up
            by 8–10 A on a 50 A sub-main), retrofits an EV charger (Ib up by 30 A on a household
            supply), or upgrades to a more powerful electric oven (Ib up by 5 A on a kitchen
            circuit).
          </p>
          <p>
            Designing with 1 percent of Vd headroom means the install can absorb a 20–30 percent
            load uplift before going non-compliant on Vd. On a domestic CU upgrade where the
            customer is unsure of their five-year future plans, that headroom is cheap insurance
            against a future call-back.
          </p>
          <p>Specific size-up triggers:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Vd above ~80 percent of the limit (above 2.4 percent for lighting, above 4 percent for
              other) — step up one CSA bracket.
            </li>
            <li>Long buried runs (above 25 m) where future re-cabling would be disruptive.</li>
            <li>
              Sub-mains feeding sub-boards — always design to a tighter internal target so
              downstream final circuits have margin.
            </li>
            <li>
              EV-ready or heat-pump-ready supplies in domestic refurbs — size to the future load,
              not the current load.
            </li>
            <li>
              Concealed cable runs that are mechanically expensive to upgrade later (in walls, under
              floors, in conduit chases).
            </li>
          </ul>
          <p>
            The opposite case — size <em>down</em> to bare compliance — is rarely worth the saving.
            The copper price difference between, say, 6 mm² and 10 mm² over a 30 m run is usually
            under £50; the labour saving is zero (the install effort is the same); the customer
            goodwill saving from never having a Vd complaint is meaningful.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Worked example. A semi-detached property has a main CU at the supply intake, a sub-main
            running 25 m to a garage CU, and from the garage CU a 15 m radial to a 32 A EV charger.
            All the cabling is single-phase 230 V.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Stage 1 — origin to garage CU.</strong> Sub-main 25 m, 16 mm² SWA, Ib = 32 A
              (the EV charger plus a small lighting load). mV/A/m for 16 mm² four-core SWA ≈ 2.8
              (line-to-line at 400 V) — but here single-phase from an L1 + N pair, use the
              single-phase column ≈ 2.8 mV/A/m × 2 (single-phase loop) = 5.6 mV/A/m equivalent. Vd =
              (5.6 × 32 × 25) / 1000 = 4.48 V = 1.95 % of 230 V.
            </li>
            <li>
              <strong>Stage 2 — garage CU to EV charger.</strong> Final circuit 15 m, 6 mm² T&E, Ib
              = 32 A. mV/A/m ≈ 7.3. Vd = (7.3 × 32 × 15) / 1000 = 3.50 V = 1.52 % of 230 V.
            </li>
            <li>
              <strong>Cumulative.</strong> 1.95 + 1.52 = 3.47 % at the EV charger terminals. Inside
              the 5 % non-lighting limit. Pass.
            </li>
          </ul>
          <p>
            Now consider what happens if the sub-main is undersized — say 10 mm² SWA on the same
            run. mV/A/m for 10 mm² four-core SWA ≈ 4.4 single-phase loop. Vd = (4.4 × 32 × 25) /
            1000 = 3.52 V = 1.53 %. Adding the final circuit's 1.52 % gives a cumulative 3.05 % —
            still passes 5 %, but if the EV charger ever upgrades to 7.4 kW (32 A continuous instead
            of typical 25 A average) or the sub-main also feeds garage lighting and sockets, the
            headroom evaporates fast.
          </p>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Single-tier sub-main + non-lighting final.</strong> Sub-main ≤ 2 %, final ≤ 3
              %, total ≤ 5 %.
            </li>
            <li>
              <strong>Single-tier sub-main + lighting final.</strong> Sub-main ≤ 1 %, lighting final
              ≤ 2 %, total ≤ 3 %. The lighting limit is the binding constraint.
            </li>
            <li>
              <strong>Two-tier sub-mains (main → sub-main → sub-sub-main → final).</strong> Each
              tier ≤ 1 %, final ≤ 2 % for non-lighting (5 % total) or ≤ 1 % for lighting (3 %
              total). Three-stage cumulative budgets are tight.
            </li>
          </ul>
          <p>
            Document the cumulative Vd calc on the design pack — every stage's contribution plus the
            cumulative total at the worst-case load. The next inspector / future designer can verify
            that any added load doesn't break the cumulative budget.
          </p>
        </ConceptBlock>
        ...
        >
          <p>Motor starting Vd workflow:</p>
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Get the motor's starting current</strong> from the nameplate or manufacturer
              datasheet. Typical direct-on-line start: Ist = 6-7 × IFL (full-load current).
              Star-delta start: Ist = 2-3 × IFL. Soft-start: Ist = 1.5-3 × IFL programmable. VFD:
              Ist = 1-1.5 × IFL.
            </li>
            <li>
              <strong>Calculate Vd with Ist instead of IFL.</strong> Vd_start = (mV/A/m × Ist × L) /
              1000. Use the same cable mV/A/m as for the steady-state calc; the cable doesn't
              change.
            </li>
            <li>
              <strong>Compare to Reg 552.1.1 limit.</strong> The limit is typically 4 % during
              starting (some manufacturers / installations target 3 % for sensitive control gear).
              Vd_start as a percentage of nominal voltage = Vd_start / 230 (single-phase) or / 400
              (three-phase line-to-line).
            </li>
            <li>
              <strong>If Vd_start fails:</strong> options are (a) larger cable (lower mV/A/m → lower
              Vd at same current), (b) soft-start or VFD on the motor (lower starting current), (c)
              nearer DB (shorter cable run).
            </li>
          </ol>
          <p>
            <strong>Worked example.</strong> Workshop compressor, 7.5 kW three-phase 400 V, IFL = 14
            A, DOL start with Ist = 6 × IFL = 84 A. Cable run from DB to compressor 30 m, 4 mm²
            four-core SWA (mV/A/m ≈ 11 line-to-line). Steady-state Vd = (11 × 14 × 30)/1000 = 4.62 V
            = 1.16 % — fine. Starting Vd = (11 × 84 × 30)/1000 = 27.7 V = 6.93 % of 400 V — exceeds
            4 % limit. Options:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Upsize cable to 10 mm² four-core</strong> (mV/A/m ≈ 4.4): Vd_start = (4.4 × 84
              × 30)/1000 = 11.1 V = 2.77 % — passes. Cable cost up but problem solved.
            </li>
            <li>
              <strong>Specify a soft-start</strong> (Ist limited to ~3 × IFL = 42 A): Vd_start = (11
              × 42 × 30)/1000 = 13.86 V = 3.47 % — passes on existing 4 mm² cable. Soft-start cost
              typically £200-500 for this motor size, often less than the cable upsize on long runs.
            </li>
            <li>
              <strong>Specify a VFD</strong> (Ist limited to ~1.5 × IFL): Vd_start = (11 × 21 ×
              30)/1000 = 6.93 V = 1.73 % — passes comfortably. VFD also gives soft-stop, speed
              control and energy savings — often the right answer for new industrial installs.
            </li>
          </ul>
          <p>
            Document both the steady-state Vd and the starting Vd calculations on the design pack,
            with the starting current source clearly cited. On industrial work the starting Vd is
            often the binding constraint and frequently drives a cable size up by one or two
            brackets above what steady-state alone would suggest.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              The apprentice runs the CCC calc, picks 6 mm² because that clears 32 A with derates,
              and writes the cable up. They never run the Vd gate because CCC was comfortable. The
              50 m kitchen radial in the converted barn comes in at 7.3 percent Vd at the furthest
              socket. Kettle takes 30 percent longer to boil, dishwasher struggles to start,
              customer complains, an inspector codes the install on a periodic five years later for
              non-compliance with Reg 525.202.
            </>
          }
          doInstead={
            <>
              Always run both gates. CCC is about the cable cooking; Vd is about the load seeing
              enough voltage. They are independent checks measuring different things. A short fat
              cable can comfortably pass CCC and still fail Vd if the run is long enough. Size to
              satisfy whichever gate demands the larger CSA, then verify both pass independently.
              Document both calcs on the design sheet.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You are designing a 60 A sub-main from a domestic CU to a detached studio 75 m away
              across the garden. Buried SWA in clay soil. CCC alone resolves at 16 mm² (Iz
              comfortably above In = 60 A). The Vd calc on 16 mm² four-core (mV/A/m ≈ 2.4) gives Vd
              = (2.4 × 60 × 75) / 1000 = 10.8 V — that is 10.8 / 230 = 4.7 percent on the
              single-phase supply. Just under 5 percent non-lighting limit, but the studio has its
              own sub-board with several final circuits downstream that need their own Vd headroom.
            </>
          }
          whatToDo={
            <>
              Size the sub-main on Vd, not on CCC. Step up to 25 mm² SWA (mV/A/m ≈ 1.5) and Vd drops
              to (1.5 × 60 × 75) / 1000 = 6.75 V = 2.93 percent. The downstream final circuits now
              have over 2 percent of headroom for their own Vd and the cumulative compliance is
              comfortably inside both the 3 percent lighting limit (where applicable) and the 5
              percent other-loads limit. The CSA step from 16 mm² to 25 mm² SWA is a real cost
              (perhaps £300–£500 more in copper and slightly heavier glands) but it bullet-proofs
              the studio supply against future load growth.
            </>
          }
          whyItMatters={
            <>
              Sub-mains nearly always size on Vd rather than CCC. The route lengths are long, the
              design currents are high, and the cumulative Vd budget has to be shared with
              downstream final circuits. Always treat sub-mains as a Vd-first problem and verify CCC
              second.
            </>
          }
        />
