...
const TITLE = 'Max Zs vs measured Zs — the 80% rule (5.3) | Level 3 Module 6.5.3 | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  {
    ...
    options: [
      '1.44 Ω — the pre-A4 figure, which remains valid for design work under the current edition.',
      '1.37 Ω — A4:2026 tightened the figure to reflect the formal Cmin = 0.95 voltage factor in the underlying calculation.',
      '0.68 Ω — the value for a Type C 32 A device, which shares the same Zs limit as a Type B.',
      '2.30 Ω — derived from the full 230 V nominal supply divided by the 100 A device threshold.',
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      '1.37 Ω — the full Table 41.3 figure is the verification limit, because the measurement is taken with the cable already at operating temperature.',
      '1.71 Ω — that is 1.25 × 1.37, allowing a 25 percent uplift for measurement uncertainty and instrument tolerance at verification.',
      '0.68 Ω — that is 0.5 × 1.37, because the measured value must be half the table maximum to allow for both temperature and grouping.',
      '1.10 Ω — that is 0.8 × 1.37, because the measurement is taken cold but the table value assumes the conductor is hot at ~70 °C.',
    ],
    correctIndex: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Accept — 1.30 Ω is below the 1.37 Ω table max, so ADS is satisfied and the 0.8 figure is only a design guideline, not a verification limit.',
      'Reject — measured exceeds 0.8 × table max, so the hot Zs in service will probably exceed it; investigate the device and cable and upsize or downrate.',
      'Accept — round the 1.30 Ω down to the 1.10 Ω acceptance figure, since the difference is within instrument tolerance.',
      'Reject — the measured value must be at or below the full 1.37 Ω table max, which it exceeds, so the circuit fails outright.',
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    id: 1,
    question: 'What does the maximum permitted Zs in BS 7671 Table 41.3 represent physically?',
    options: [
      'The earth fault loop impedance at which the cable just reaches its current-carrying capacity Iz under normal full load.',
      'The highest loop impedance at which the device still clears the fault inside the Table 41.1 disconnection time, with Cmin applied to U0.',
      'The minimum loop impedance below which the prospective fault current would exceed the protective device breaking capacity.',
      'The loop impedance at which the voltage drop across the circuit just reaches the 5 percent limit set by Reg 525.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  {
    id: 2,
    question: 'What is the design-stage formula for Zs?',
    options: [
      'Zs = Ze − (R1 + R2), subtracting the cable resistance from the external impedance because the conductors run back toward the source.',
      'Zs = (R1 + R2) × Ze, multiplying the cable resistance by the external impedance to give the total earth fault loop impedance.',
      'Zs = Ze + (R1 + R2 × temperature multiplier), the multiplier (1.20 for 70 °C thermoplastic) lifting cold cable resistance to operating temperature.',
      'Zs = Ze + R1 + R2 + Cmin, adding the 0.95 voltage factor to the loop impedance to allow for the worst-case supply tolerance.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  {
    id: 3,
    question: 'Why is the measured Zs at verification typically lower than the design-stage Zs?',
    options: [
      'Because the test instrument applies the Cmin 0.95 factor automatically, lowering the displayed reading below the design figure.',
      'Because the measured value includes only the external Ze, whereas the design Zs adds the full final-circuit cable resistance on top.',
      'Because the measurement is taken with the circuit energised under full load, warming the cable and lowering its resistance below design.',
      'Because the measurement is taken cold at ~20 °C, but the design Zs assumes the conductor is hot at ~70 °C — cold is roughly 0.83 of hot.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'It formally applied the Cmin = 0.95 voltage factor, tightening every figure by ~5 percent, so B32 went from 1.44 Ω to 1.37 Ω.',
      'It relaxed every figure by ~5 percent by raising the assumed supply voltage, so B32 went from 1.37 Ω to 1.44 Ω.',
      'It left the figures unchanged but added a separate column for the 0.8 verification acceptance values.',
      'It doubled every figure for the higher fault currents on modern PME supplies, so B32 went from 1.37 Ω to 2.74 Ω.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'No — design Zs of 2.55 Ω exceeds the 0.8 acceptance figure of 2.18 Ω, so ADS will not clear in time and the design fails outright.',
      'Yes — design Zs ≤ Table 41.3 max, so ADS clears in time, though the design should still target under 0.8 × 2.73 = 2.18 Ω for handover headroom.',
      'No — a B16 device requires Zs below 1.37 Ω, the same limit as a B32, so 2.55 Ω is far too high.',
      'Yes — but only because a 30 mA RCD is fitted; without it the 2.55 Ω design Zs would fail the ADS requirement.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Reject — 0.95 Ω is too close to the 1.10 Ω acceptance figure, leaving insufficient margin for measurement uncertainty.',
      'Reject — the measured value must be below 0.8 Ω for a B32 device, so 0.95 Ω fails the verification.',
      'Accept only after re-measuring with the cable warmed to operating temperature, since cold readings cannot be used for sign-off.',
      'Accept — 0.95 Ω is below both the table max (1.37) and the 0.8 figure (1.10), so the ~1.14 Ω hot Zs stays clear; document and sign off.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      'C1 — danger present; an immediate hazard exists requiring the circuit to be made safe on the day.',
      'C3 — improvement recommended; the reading is non-compliant with best practice but presents no safety implication.',
      'No code — the reading is below the full table max of 1.37 Ω, so the circuit is compliant and passes without comment.',
    ],
    correctAnswer: 0,
    explanation:
      'Consistent with the 0.8 acceptance pedagogy taught in this subsection, a measured Zs above the 0.8 figure but below the table max is coded C2. The reasoning: the measurement is cold; the cable in service will be hot; the hot Zs will probably exceed the Table 41.3 max; ADS may not clear in time at the worst case, so the install could be unsafe under fault conditions even though it is currently functional. Note that EICR coding is an inspector judgement (BS 7671 itself does not assign codes — the codes come from IET Guidance Note 3 and the industry Best Practice Guides); some inspectors take the view that a measured Zs below the Table 41.3 maximum satisfies ADS and would code it lower. The taught position here is C2 because the cold-to-hot temperature rise will likely push the in-service Zs past the table max. C1 is reserved for "danger present" (immediate hazard, e.g. accessible live parts); C3 is "improvement recommended" with no immediate safety implication. The remediation is the same as on a new install — investigate, upsize cable if needed, and re-test.',
  },
];
...
const faqs = [
  ...
  {
    question:
      'What if the measured Zs comes out lower than expected — say much lower than the design predicted?',
    answer:
...
export default function Sub3() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 5 · Subsection 3"
        title="Maximum Zs vs measured Zs — the 80% acceptance rule"
        backTo="/study-centre/apprentice/level3-module6-section5"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The bridge between design-stage Zs (calculated with Cmin and temperature factors) and
          verification-stage Zs (measured at ambient with the cable cold). BS 7671 A4:2026 Table
          41.3 maxima with the new Cmin treatment, the 0.8 acceptance criterion, Reg 411.4.5, and
          how to handle borderline measurements.
        </p>
        ...
        >
          <p>At the design stage, Zs is a calculation. The formula is straightforward:</p>
          <p>
            <strong>Zs(design) = Ze + (R1 + R2) × temperature multiplier</strong>
          </p>
          <p>
            Ze is taken from the DNO declaration (or measured at the supply intake if unknown). For
            TN-C-S, declared Ze is typically 0.35 Ω; for TN-S, typically 0.8 Ω; for TT, typically
            200 Ω or higher. R1 + R2 is calculated from the chosen cable’s mΩ/m figure (cold)
            multiplied by the route length, then by a temperature multiplier (1.20 for 70 °C
            thermoplastic insulation; 1.28 for 90 °C thermosetting) to bring the cold resistance up
            to operating temperature.
          </p>
          <p>
            The result is compared directly to BS 7671 Table 41.3. If Zs(design) ≤ Table 41.3 max
            for the chosen device, ADS will clear the fault inside the maximum disconnection time
            (Table 41.1) at worst case. If above, ADS will not — the design fails.
          </p>
          <p>
            ...
            injects a small test current and reads the loop impedance. The reading is at ambient
            temperature, with the cable cold — quite different from the operating- temperature
            condition the design assumed.
          </p>
        </ConceptBlock>
        ...
        <VideoCard
          ...
          caption={
            <>
              Craig Wiltshire walks the live Ze measurement at the intake — the same Ze that seeds
              Zs(design) = Ze + (R1 + R2) covered above. Watch how the value compares to the typical
              declared figures for TN-C-S / TN-S before you reach the 0.8 acceptance rule.
            </>
          }
        />
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The Chapter 41 ADS framework is the regulatory hook for Zs design and verification in
              TN systems. Practical effect: at design, calculate Zs at conductor operating
              temperature with Cmin = 0.95 applied to U0, and verify Zs(design) ≤ Table 41.3 max. At
              verification, measure Zs at ambient temperature and apply the 0.8 acceptance criterion
              to allow for the cold-vs-hot difference. The wording of Reg 411.4.5 itself was
              redrafted in A4:2026; consult the published amendment for the exact text and any
        ...
        >
          <p>
            Indicative maximum Zs values from BS 7671 A4:2026 Table 41.3 for B-curve MCBs at U0 =
            230 V, 0.4 s disconnection time (verify against your edition before signing
            calculations):
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>B6:</strong> ≈ 7.28 Ω (was 7.66 pre-A4)
            </li>
            <li>
              <strong>B10:</strong> ≈ 4.37 Ω (was 4.60 pre-A4)
            </li>
            <li>
              <strong>B16:</strong> ≈ 2.73 Ω (was 2.87 pre-A4)
            </li>
            <li>
              <strong>B20:</strong> ≈ 2.19 Ω (was 2.30 pre-A4)
            </li>
            <li>
              <strong>B25:</strong> ≈ 1.75 Ω (was 1.84 pre-A4)
            </li>
            <li>
              <strong>B32:</strong> = 1.37 Ω (was 1.44 pre-A4) — the most-quoted figure on a typical
              domestic radial.
            </li>
            <li>
              <strong>B40:</strong> ≈ 1.09 Ω (was 1.15 pre-A4)
            </li>
            <li>
              <strong>B50:</strong> ≈ 0.87 Ω (was 0.92 pre-A4) — typical for shower circuits.
            </li>
            <li>
              <strong>B63:</strong> ≈ 0.69 Ω (was 0.73 pre-A4)
            </li>
          </ul>
          <p>
            ...
            double the fault current to operate instantaneously, hence half the maximum Zs). For
            D-curve, divide again. For HBC fuses the figures are derived from the fuse curve at the
            required disconnection time.
          </p>
          <p>
            The historical 1.44 Ω figure for B32 is what most apprentices remember from training
            material printed before A4:2026. It is wrong for current design and verification. The
            1.37 Ω A4:2026 figure is what you must use.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 411.3.1.1 is the underlying duty that a Zs verification proves at handover. Every
              exposed-conductive-part connects to a CPC; the CPC is sized under Chapter 54; the
              device on the circuit must clear within the Table 41.1 time at the actual fault
              current. A measured Zs at or below the 0.8 × Table 41.3 max value is the proof that
              the device will operate inside that time when the cable is hot in service. Without
              that proof, 411.3.1.1 is unverified.
            </>
          }
        ...
        >
          <p>
            The temperature coefficient of resistance for copper is approximately 0.004 per °C. From
            20 °C (typical ambient at measurement) to 70 °C (typical operating temperature for
            thermoplastic insulation), the resistance ratio is:
          </p>
          <p>R(70°C) / R(20°C) = 1 + 0.004 × (70 − 20) = 1 + 0.20 = 1.20</p>
          <p>
            So the hot R1 + R2 is 1.20 times the cold R1 + R2. The cold/hot ratio is 1/1.20 = 0.833.
            A measured (cold) Zs at 0.833 of the Table 41.3 max should result in a hot Zs in service
            that is right at the table max — borderline acceptable.
          </p>
          <p>The 0.8 acceptance criterion rounds 0.833 down to 0.8 and adds a small margin for:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Measurement uncertainty</strong> — typical Zs test instruments are accurate to
              ±10 percent at 95 percent confidence, sometimes worse on low impedance ranges. A
              reading of 1.10 Ω could really be 1.21 Ω.
            </li>
            <li>
            ...
            <li>
              <strong>Future degradation</strong> — connection contact resistance and cable ageing
              may slightly increase R1 + R2 over the install life.
            </li>
          </ul>
          <p>
        ...
        >
          <p>
            ...
            1.5 mm²), feeding kitchen sockets. Protective device: B32 RCBO. Supply: TN-C-S, declared
            Ze = 0.35 Ω, U0 = 230 V. Insulation: 70 °C thermoplastic (PVC).
          </p>
          <p>
            <strong>Step 1 — Design-stage R1 + R2 cold.</strong> From OSG / cable manufacturer
            tables, R1 + R2 cold for 4 mm² line / 1.5 mm² CPC ≈ 17.07 mΩ/m. Over 25 m: 0.01707 × 25
            = 0.427 Ω.
          </p>
          <p>
          ...
          <p>
            <strong>Step 4 — Compare to Table 41.3.</strong> A4:2026 max Zs for B32 at 230 V = 1.37
            Ω. 0.862 ≤ 1.37 — pass. ADS will clear the fault within 0.4 s at worst case.
          </p>
          <p>
            <strong>Step 5 — Predict measured Zs.</strong> Measured (cold) Zs ≈ Zs(design) / 1.20 =
            0.862 / 1.20 = 0.718 Ω. This is what you should expect to see on the test instrument at
            handover, give or take a few percent for instrument tolerance.
          </p>
          <p>
            <strong>Step 6 — Compare predicted measurement to 0.8 acceptance.</strong> 0.8 × 1.37 =
            1.096 Ω. Predicted measurement of 0.718 Ω is comfortably below 1.096 — the circuit will
            pass verification with comfortable margin.
          </p>
          <p>
            <strong>Step 7 — At handover, take the measurement.</strong> Suppose the actual measured
            Zs comes back as 0.74 Ω. This is close to the 0.718 prediction (small variation from
            real Ze vs declared, real cable resistance vs nominal, instrument tolerance). 0.74 ≤
            1.10 — pass. Document on the schedule of test results.
          </p>
          <p>
            The whole sequence is one continuous story: the design predicts the measurement, the
            measurement confirms the design. When the two diverge significantly, investigate.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A4:2026 renumbered the design-documentation duty from the old 132.12 to 132.13, and
              Reg 132.2(a) is one of the supply-characteristics items that has to land on that
              documentation. Both the design-stage Zs trace (Sub 5.2) and the verification
              measurement compared against the 0.8 acceptance criterion (this Sub) belong on the
              design / EIC pack. The selectivity / coordination evidence required by Reg 536.5 also
              bolts onto this pack at initial verification. A Zs reading that lacks a documented
              design comparator is an EIC trace failure, not just a missed step.
            </>
          }
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              designed? An installer-substituted C-curve in place of a designed B-curve will shift
              the table max significantly (C32 max Zs is roughly half of B32 max Zs).
            </li>
            <li>
              <strong>Confirm the cable.</strong> Is the actual installed cable the size you
              designed? A 4 mm² installed where 6 mm² was designed will give a much higher R1 + R2
              and a higher Zs.
            </li>
            <li>
              <strong>Confirm the route length.</strong> Is the cable run actually the length you
              designed? A re-routed cable can be 30 percent longer than the design path.
            </li>
            <li>
              <strong>Confirm Ze.</strong> Measure the Ze at the supply intake and compare to the
              declared figure. A Ze higher than declared (because the DNO supply has degraded) is a
              DNO problem, not a design problem.
            </li>
            <li>
              <strong>Recalculate the design.</strong> With the as-installed figures (real cable,
              real route, real device, real Ze) recompute Zs(design) and re-compare to Table 41.3.
            </li>
          </ol>
          <p>
            The remedy depends on what the investigation shows. If the install matches the design
            and the design was right, the problem is probably an unexpectedly high real Ze — engage
            the DNO. If the install deviates from the design (substituted device, wrong cable,
            longer route) the install needs to be brought back to design. If the design was
            marginal, upsize the cable to bring R1 + R2 down or downrate the device to a smaller
            rating with a larger Table 41.3 max.
          </p>
        </ConceptBlock>
        ...
        <Scenario
          ...
          whatToDo={
            <>
              Code the observation as C2 (potentially dangerous) on the EICR. The reasoning: the
              cold measured Zs of 1.20 Ω predicts a hot Zs in service of approximately 1.20 × 1.20 =
              1.37 Ω, which exceeds the 1.37 Ω table max. ADS may not clear within 0.4 s at worst
              case under fault conditions when the cable is hot. Document the observation, recommend
              remediation (investigate the cable run for unexpected length, check Ze at intake, and
              either upsize the cable or move the device down to a B25 or B20 with a larger table
              max), and flag the C2 to the customer with the requirement to remediate within an
              agreed period.
            </>
          }
          whyItMatters={
            <>
              The 0.8 acceptance criterion is not a "nice to have" — it is the safety margin that
              ensures the install remains compliant under the operating conditions, not just at the
              cold ambient when the test instrument was plugged in. Coding a borderline reading as
              C3 ("improvement recommended") instead of C2 because the table max is technically not
              exceeded is a misreading of the regulation and leaves the customer with an unsafe
              install carrying a clean certificate. The whole point of the 0.8 rule is to catch
              exactly this case.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              The apprentice memorised 1.44 Ω from older training material, OSG editions or online
              "BS 7671 cheat sheets" that have not been updated for A4:2026. They use 1.44 Ω in
              design calculations and at verification. A circuit measuring 1.40 Ω gets accepted
              because "it’s under 1.44". The hot Zs in service will be approximately 1.68 Ω,
              comfortably exceeding the actual A4:2026 max of 1.37 Ω. ADS may not clear in time
              under fault conditions.
            </>
          }
          doInstead={
            <>
              Always work from the current BS 7671 A4:2026 Table 41.3, not from memory or pre-A4
              reference material. The A4:2026 figures are tighter by ~5 percent across the board
              because the Cmin = 0.95 voltage factor is now formally applied to U0 in the underlying
              calculation. Print the current Table 41.3 and pin it to the inside cover of your van
              clipboard. If you see 1.44 Ω quoted anywhere, that source is out of date.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              The apprentice measures Zs = 1.30 Ω on a B32 circuit, sees 1.30 ≤ 1.37 (table max),
              and accepts. They sign the schedule of test results and the EIC. The hot Zs in service
              will be approximately 1.30 × 1.20 = 1.56 Ω, comfortably exceeding the table max. ADS
              may not clear within 0.4 s at worst case. The install is certified as compliant when
              it is not.
            </>
          }
          doInstead={
            <>
              Always apply the 0.8 acceptance criterion. Measured Zs ≤ 0.8 × Table 41.3 max. For B32
              that is 0.8 × 1.37 = 1.10 Ω. A reading above 1.10 Ω fails verification and must be
              investigated. The 0.8 rule is the practical implementation of Reg 411.4.5 acceptance —
              bypassing it is the same as bypassing the regulation.
            </>
          }
        />
        ...
        >
          <p>PFC reading interpretation:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Domestic typical</strong> — 1-3 kA. Standard 6 kA RCBOs and MCBs are well
              within rating.
            </li>
            <li>
              <strong>Sub-station-adjacent or large commercial</strong> — 6-10 kA or higher. Verify
              every device's Icn / Ics rating against the measured PFC.
            </li>
            <li>
              <strong>Three-phase services</strong> — read PFC line-to-earth and line-to-line; the
              line-to-line value drives the upstream device rating on a phase-to-phase fault.
            </li>
            <li>
              <strong>Margin and degradation</strong> — supplies tighten over time as the network is
              reinforced; design with margin so the install still complies if PFC rises 20-30
        ...
        >
          <p>Temperature multipliers by cable type:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Thermoplastic (PVC) — 70 degrees C operating</strong> — multiplier 1.20.
            </li>
            <li>
              <strong>Thermosetting (XLPE) — 90 degrees C operating</strong> — multiplier 1.28.
            </li>
            <li>
              <strong>MICC (mineral-insulated copper-clad) — 105 degrees C</strong>— multiplier 1.34
              (specialist application).
            </li>
            <li>
              <strong>Why the multiplier matters</strong> — a circuit measured stone-cold in
              February reads 0.8 ohms; that same circuit at design load in August reads 0.96 ohms.
              Acceptance must reference the hot-design value, not the cold-measured value.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Three-phase Zs readings — what to look for:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Three readings per test point</strong> — Zs(L1-PE), Zs(L2-PE), Zs(L3-PE).
              Per-line columns on the EIC schedule.
            </li>
            <li>
              <strong>Imbalance under 10 percent</strong> — typical balanced supply; nothing to
              investigate.
            </li>
            <li>
              <strong>Imbalance 10-20 percent</strong> — investigate terminations, transformer tap,
              single-phase loading on the offending line.
            </li>
            <li>
              <strong>Imbalance above 20 percent</strong> — significant defect; possibly a damaged
              conductor, a high-resistance neutral or a line-to-earth partial fault.
            </li>
            <li>
              <strong>Compare against the same Table 41.3 max</strong> — the highest of the three
              readings is the binding figure for the circuit's protective device.
            </li>
          </ul>
        </ConceptBlock>
