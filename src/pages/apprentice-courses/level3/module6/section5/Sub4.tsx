...
const TITLE = 'TT systems & earth electrode design (5.4) | Level 3 Module 6.5.4 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  {
    id: 1,
    question: 'When must you design a TT installation rather than TN-S or TN-C-S?',
    options: [
  ...
  {
    id: 3,
    question: 'The TT acceptance criterion in BS 7671 Reg 411.5 is:',
    options: [
  ...
  {
    id: 4,
    question: 'Why is an RCD essential on a TT system, where it might be optional on a TN system?',
    options: [
...
const faqs = [
  ...
  {
    question:
      'Do I need to bond every metal pipe and structure on a TT install the same way as TN?',
    answer:
...
export default function Sub4() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 5 · Subsection 4"
        title="TT systems and earth electrode design"
        backTo="/study-centre/apprentice/level3-module6-section5"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The TT system end to end. When the DNO supply forces TT, earth electrode types and
          installation, the Reg 411.5 acceptance criterion RA × IΔn ≤ 50 V, RCD-based ADS on TT, the
          new Reg 542.2.8 earth electrode requirements in BS 7671 A4:2026, and the verification
          routine that confirms compliance at handover.
        </p>
        ...
        >
          <p>The three earthing arrangements you encounter in UK practice:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>TN-C-S (PME / PNB)</strong> — the dominant UK domestic supply. The DNO
              supplies a combined PEN conductor at the cut-out; the installer separates PEN into PE
              and N at the consumer’s main earthing terminal (MET). Declared Ze is typically 0.35 Ω.
              Fault currents are high. MCB-based ADS is the norm; RCDs are additional protection.
            </li>
            <li>
              <strong>TN-S</strong> — older urban supplies (sometimes called "earthed concentric" or
              "earthed lead sheath"). The DNO provides a separate earth (often via the supply
              cable’s lead sheath) and a separate neutral. Declared Ze is typically 0.8 Ω. Fault
              currents lower than TN-C-S but still high enough for MCB ADS. Increasingly rare as
              DNOs replace lead-sheath cables.
            </li>
            <li>
              ...
              typically 50–500 Ω, depending on soil and electrode design. Fault currents are too
              small for MCB-based ADS; RCDs are essential.
            </li>
          </ul>
          <p>
            The arrangement is announced at the cut-out: TN-C-S or PME on the cut-out label means
            TN-C-S; the absence of an earth terminal at the cut-out means TT (no alternative).
            Always verify by reading the cut-out label and, if uncertain or the cut-out is unmarked,
            calling the DNO with the property MPAN to confirm.
          </p>
        </ConceptBlock>
...
The requirements of this regulation are met if the earth fault loop impedance of the circuit protected by the RCD meets the requirements of Table 41.5.`}
          meaning={
            <>
              Reg 411.5.3 is the TT system acceptance criterion. The 50 V is the conventional safe
              AC touch voltage (UL); higher voltages cause harmful current to flow through the body.
              The product RA × IΔn is the worst-case touch voltage that would appear on
              exposed-conductive-parts during a fault before the RCD trips. Practical effect: even a
              high earth electrode resistance can satisfy the criterion if combined with a
              sufficiently sensitive RCD — 1500 Ω × 30 mA = 45 V passes; 1000 Ω × 100 mA = 100 V
              fails. Table 41.5 publishes the equivalent maximum Zs values directly for non-delayed
              and 'S' type RCDs at U₀ = 230 V.
            </>
          }
        ...
        >
          ...
          <p>
            If = U0 / (Ze + Zsource), where Ze is the electrode resistance plus protective conductor
            resistance (RA) and Zsource is the source-side impedance.
          </p>
          <p>
          ...
          <p>If ≈ U0 / RA</p>
          <p>
            The voltage on the exposed-conductive-part during the fault is the voltage drop across
            RA (since the source-side impedance is negligible):
          </p>
          <p>Vfault = If × RA = (U0 / RA) × RA = U0</p>
          <p>
            Wait — that suggests the touch voltage is the full supply voltage. It is, in principle,
            until the RCD trips. The RCD trips when the residual current exceeds IΔn. The current
            that flows from the appliance casing to the local soil and back to the source is the
            residual current — it is the imbalance between line and neutral. So the RCD trip
            threshold sets a ceiling on how long the touch voltage persists, but the formula
            relevant to the 50 V criterion is different:
          </p>
          <p>
            The "deemed safe" arrangement in Reg 411.5 says: the touch voltage that the RCD will
            tolerate before tripping is RA × IΔn. If that product is at most 50 V, the touch voltage
            during the fault stays below the conventional safe AC limit — the person touching the
            casing is exposed to at most 50 V briefly before the RCD clears. Above 50 V the touch
            voltage becomes harmful.
          </p>
          <p>Examples:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              RA = 600 Ω, IΔn = 100 mA: 600 × 0.1 = 60 V — fail; either improve electrode or move to
              a 30 mA RCD.
            </li>
          ...
          <p>
            ...
            forgiving — even high electrode resistances pass. 100 mA and 300 mA (S-type) upstream
            devices used for general fault clearance and discrimination are tighter and the
            electrode resistance must be controlled.
          </p>
        </ConceptBlock>
        ...
        >
          <p>BS 7671 Chapter 54 recognises the following earth electrode types:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Driven rods</strong> — the most common UK choice. Standard sizes 5/8" (16 mm)
              diameter copper-bonded steel core, supplied in 1.2 m, 1.5 m or 2.4 m sections that
              screw together for deeper installs. Driven vertically with a sledge hammer or
              pneumatic driver. Connected to the MET via an earthing conductor in green/yellow PVC,
              typically 16 mm² for domestic, 25 mm² or larger for commercial.
            </li>
            <li>
              <strong>Plate electrodes</strong> — typically 600 mm × 600 mm copper or galvanised
              steel plate buried at depth (1.5 m or more). Higher contact area than a single rod,
              but expensive to install and inflexible. Used in difficult-rod soil where rocks
              prevent driving.
            </li>
            <li>
              <strong>Tape and strip electrodes</strong> — buried horizontally, typically 25 mm × 4
              mm copper tape, in a 0.5–1 m deep trench. Used as foundation earths or where
              rod-driving is impractical (e.g. rocky surface with thinner topsoil).
            </li>
            <li>
              <strong>Foundation earth electrodes</strong> — galvanised steel mesh or rebar in
              concrete foundations, deliberately bonded to the building’s MET. Common in modern
              commercial construction, increasingly in domestic new-build.
            </li>
            <li>
              <strong>Structural metalwork</strong> — large structural steel members (e.g. piling,
              columns) in deliberate earth contact. Used in industrial and infrastructure projects.
            </li>
            <li>
              <strong>Chemical electrodes (electrolytic)</strong> — copper rod in a Bentonite- or
              other-electrolyte-filled enclosure. Significantly lowers resistance in difficult
              soils. More expensive than a simple rod but transformative on dry / rocky / sandy
              sites.
            </li>
          </ul>
          <p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Peat / marsh:</strong> 5–30 Ω·m. Excellent for earthing.
            </li>
            <li>
              <strong>Damp clay loam:</strong> 30–80 Ω·m. Good.
            </li>
            <li>
              <strong>Sandy clay:</strong> 80–200 Ω·m. Moderate.
            </li>
            <li>
              <strong>Sand / gravel:</strong> 200–500 Ω·m. Poor; needs longer / multiple rods.
            </li>
            <li>
              <strong>Chalk / limestone:</strong> 200–1000+ Ω·m. Often poor; chemical electrodes
              help.
            </li>
            <li>
              <strong>Granite / hard rock:</strong> 1000–10000 Ω·m. Very poor; specialist solutions
              needed.
            </li>
          </ul>
          <p>
          ...
          <p>
            ...
            deeper soil layers are damper (below the seasonal drying zone). Adding a second parallel
            rod at 4 m spacing typically halves again. Three rods in a triangle at 4 m spacing
            reduce further. For very high resistivity sites, a chemical electrode or a deeper plate
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A4:2026 introduced Reg 542.2.8 as a new earth electrode provision. The fragment in
              publicly available extracts is index-style ("542.2.8 have been introduced concerning
              earth electrodes") and does not include the full normative wording. Designers,
              installers and inspectors working on TT installations or any installation involving
              earth electrodes must obtain and read the full A4:2026 text of Reg 542.2.8 before
              specifying, installing or certifying electrodes. Treat this regulation as a pointer to
              the published amendment for the authoritative requirements.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              soundly made, electrically and mechanically satisfactory, corrosion-protected, and
              labelled. Practical implementation: use a proprietary clamp (not a hose clip) rated
              for the conductor size; install a cover or junction box (e.g. a "carrot top") over the
              rod top to protect from mechanical damage and corrosion; add the "Safety Electrical
              Connection — Do Not Remove" label per Reg 514.13.1(a).
            </>
          }
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              installation-wide earth faults that for any reason were not cleared downstream (e.g. a
              fault in the meter tails or main board itself), and it limits the worst-case touch
              voltage during such a fault per Reg 411.5.
            </li>
            <li>
              <strong>Final-circuit RCDs</strong> — 30 mA Type A on every final circuit, normally
              supplied by RCBOs (combined RCD + MCB) so each circuit has its own residual current
              protection without nuisance-tripping other circuits. Type A handles AC and pulsed DC
              residual currents (the modern default); Type AC handles only AC and is increasingly
              inadequate for modern loads.
            </li>
            <li>
              <strong>Special-circuit RCDs</strong> — Type B RCDs are required on EV charger
              circuits per Section 722 (or Type A combined with DC-residual monitoring inside the EV
              unit). PV inverter circuits typically need Type A or B per Section 712 and the
              inverter manufacturer’s spec.
            </li>
          </ul>
          <p>
            Discrimination (selectivity) between the upstream S-type and downstream 30 mA ensures
            that local faults trip only the affected circuit, while still providing full RCD
            coverage at the installation level. This is a significant advance over older schemes
            where all circuits were on a single 30 mA RCD and any one fault dropped the whole
            install.
          </p>
        </ConceptBlock>
        ...
        >
          <p>When the supplementary electrode is most worthwhile:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>EV charger installations</strong> — Section 722 already requires open-PEN
              protection (built-in detection per IEC 61851-1, dedicated TT electrode, or external
              open-PEN device); the supplementary electrode supports the open-PEN detection scheme
              on properties where the EV chassis exposure during a fault is the lead concern.
            </li>
            <li>
              <strong>Heat pumps and outdoor metalwork</strong> — outdoor units sit at ground
              potential through their fixings; an open-PEN event lifts the chassis to phase voltage
              and creates a touch-shock hazard for anyone approaching the unit.
            </li>
            <li>
              <strong>Older overhead supplies</strong> — rural and semi-rural TN-C-S where the DNO
              PEN integrity is statistically less reliable; the local electrode is cheap insurance.
            </li>
            <li>
              <strong>Properties with extensive bonded extraneous-conductive-parts</strong>— large
              kitchens with stainless steel, swimming pools, agricultural sheds with bonded metal
              frames; the supplementary electrode reduces the touch voltage on the bonded metal
              during an open-PEN event.
            </li>
            <li>
              <strong>Documentation</strong> — the EIC notes that the installation is TN-C-S with a
              supplementary electrode at the MET per Reg 411.4.2. Record measured electrode
              resistance for the next inspector to compare against.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The TT RCD selection logic in plain L3 terms:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Supply-side device</strong> — 100 mA Type S Type A RCD typically. S-type
              delays operation by ~50 ms so downstream 30 mA devices clear local faults first.
              Handles installation-wide earth faults the downstream devices missed.
            </li>
            <li>
              <strong>Final-circuit devices</strong> — 30 mA Type A RCBO per circuit. Each circuit
              has its own residual current protection without nuisance-tripping other circuits. Type
              A handles the AC and pulsed DC residual currents that modern loads produce.
            </li>
            <li>
              <strong>EV charger circuits</strong> — Reg 722.531.3.101 requires 30 mA RCD per
              connecting point with DC fault current protection in addition. Type B RCBO, or Type A
              plus the EV's built-in 6 mA DC residual monitor. The DC fault current protection
              cannot be omitted on grounds of expense — the regulation is explicit.
            </li>
            <li>
              <strong>PV inverter circuits</strong> — Type A or Type B per Section 712 and the
              inverter manufacturer's stated requirement. Type AC is not acceptable on a modern PV
              inverter circuit.
            </li>
            <li>
              <strong>Heat pump supplies</strong> — Type A at 30 mA on the dedicated circuit.
              Variable-speed drives in heat pumps inject pulsed DC residuals that Type AC misses.
            </li>
            <li>
              <strong>Verification per A4:2026 Reg 643.7</strong> — single AC test at 1×IΔn,
              regardless of RCD type. Operating time recorded against BS EN 61008 / 61009 limits.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The TT verification sequence:</p>
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Measure earth electrode resistance.</strong> Use a "fall of potential"
              three-electrode stake-out method, or the no-trip Zs function on a multifunction tester
              (which measures Ze, equal to electrode resistance for TT). For a single rod expect
              50–500 Ω depending on soil; for a multi-rod array proportionally less. Document the
              measured value and the test method on the schedule.
            </li>
            <li>
              <strong>Test every RCD at IΔn.</strong> Operating time at the rated residual current
              must be ≤ 300 ms for general use, ≤ 40 ms at 1×IΔn for additional protection. Document
              the operating times on the schedule.
            </li>
            <li>
              <strong>Calculate RA × IΔn.</strong> For each RCD-protected circuit, multiply the
              measured RA by the RCD’s rated IΔn and verify ≤ 50 V. Document the calculation and
              pass result.
            </li>
            <li>
              <strong>Verify all main protective bonding.</strong> Continuity test from MET to each
              extraneous-conductive-part (water, gas, structural metalwork). Document the pass
              result.
            </li>
            <li>
              <strong>Verify electrode connection per Reg 542.3.2.</strong> Check that the
              connection is sound, mechanically secure, corrosion-protected, and labelled per Reg
              514.13.1(a). Document the visual inspection.
            </li>
            <li>
              <strong>Confirm A4:2026 Reg 542.2.8 requirements.</strong> Check the published
              amendment text and verify any specific obligations introduced by this new regulation.
            </li>
            <li>
              <strong>Issue EIC plus schedule of test results plus schedule of inspections.</strong>{' '}
              The TT-specific entries (Ze = electrode resistance, RA × IΔn calculation, RCD trip
              times) provide the periodic inspector with a baseline for comparison in 5 years.
            </li>
          </ol>
        </ConceptBlock>
        ...
        <Scenario
          ...
          situation={
            <>
              ...
              pole-mounted DNO transformer 200 m away. The cut-out has no earth terminal — TT is the
              only option. The site is clay loam soil; you have driven a single 2.4 m copper-bonded
              rod 5 m from the consumer unit, connected to the MET via 16 mm² green/yellow earthing
              conductor with a proprietary clamp on a "carrot top" rod-cover.
            </>
          }
          whatToDo={
            <>
              Measure the electrode resistance: 75 Ω (typical for clay loam with a 2.4 m rod).
              Specify the consumer unit as a 100 mA Type S supply-side RCD with 30 mA Type A RCBOs
              on every final circuit. Verify Reg 411.5 acceptance: for the supply RCD, RA × IΔn = 75
              × 0.1 = 7.5 V (passes); for the per-circuit 30 mA RCBOs, RA × IΔn = 75 × 0.03 = 2.25 V
              (passes comfortably). Test every RCD at IΔn and 1×IΔn — operating times within 300 ms
              and 40 ms respectively. Confirm the Reg 542.3.2 connection requirements (clamp sound,
              label fitted, cover protecting from mechanical damage and corrosion). Confirm Reg
              411.3.1.2 main protective bonding to extraneous-conductive-parts (water main,
              oil-fired boiler return). Issue EIC with schedule of test results recording all
              measurements and pass criteria, and a clear note on the design pack that the install
              is TT and the electrode resistance was 75 Ω at handover.
            </>
          }
          whyItMatters={
            <>
              TT installs are normal practice in rural Britain. The Reg 411.5 acceptance criterion
              is generous if you fit 30 mA RCBOs on every final circuit, but the install must still
              be properly tested, documented and labelled. The next inspector in 5 years will
              compare their measurements to your handover figures and flag any deterioration — soil
              drying, rod corrosion, connection failure.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              The apprentice arrives at a rural site, sees a normal-looking consumer unit, fits MCBs
              on every way without RCDs, and signs the EIC. The TT system has fault currents in
              single-digit amps; the MCBs will never operate on an earth fault. A fault in any
              appliance will sit there forever, the touch voltage on the appliance casing will be at
              supply voltage, and someone will eventually be killed.
            </>
          }
          doInstead={
            <>
              Always identify the supply earthing arrangement before designing. Read the cut-out
              label or call the DNO. If the supply is TT, fit RCDs on every circuit (either as RCBOs
              on a per-circuit basis, or as RCDs grouping circuits) and verify Reg 411.5 acceptance
              with measured RA. Document the supply arrangement clearly on the EIC and on the
              consumer unit label. Never assume "it’s domestic, it’s TN-C-S" — rural domestic
              supplies are routinely TT.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              ticks "earth electrode" on the EIC. The water company replaces a section of the supply
              pipe with plastic during a routine repair. The "earth electrode" no longer has any
              earth contact. The TT install loses its earth path silently. A fault occurs months
              later and the touch voltage on appliances reaches supply voltage with no clearing.
            </>
          }
          doInstead={
            <>
              The water main is bonded as an extraneous-conductive-part for equipotential bonding
              (Reg 411.3.1.2), not used as an earth electrode. Earth electrodes must be dedicated,
              installer-controlled rods / plates / tapes installed per Reg 542 and Chapter 54. Never
              rely on a service pipe (water, gas) for the earth electrode function. Install a proper
              rod, measure its resistance, and document it as the earth electrode on the EIC.
            </>
          }
        />
        ...
        <Quiz
          title="TT systems & earth electrode design — knowledge check"
