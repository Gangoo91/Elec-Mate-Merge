...
const checks = [
  {
    ...
    options: [
      'About 74 A — retail (38 A) on the heaviest phase plus two flats (18 A each) that land on the same phase.',
      'About 110 A — add the retail load and all four flats together (38 + 72) on a single phase, since the intake feeds everything from one point.',
      'About 38 A — only the retail counts toward the heaviest phase, because the flats are diversified to zero against the commercial load.',
      'About 27.5 A — the consolidated load (110 A) divided across the four supplies equally, then averaged per phase.',
    ],
    correctIndex: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Because PME and TN-S use different meter tail colours, and the schedule records which colour code the meter installer should expect at the intake.',
      "Because a PME supply attracts a higher DNO connection charge, and the schedule is the document used to calculate the customer's connection bill.",
      'Because only TN-S supplies require an RCD, so recording PME on the schedule tells the installer the additional RCD protection can be omitted.',
      'Because the earthing arrangement governs Zs at every final circuit and sets the main earthing conductor and bonding sizes (Reg 544).',
    ],
    correctIndex: 3,
    explanation:
      'TN type is the parameter that propagates through the rest of the design, which is why it is the most consequential supply parameter on the schedule. PME (TN-C-S) requires a 16 mm² minimum main earthing conductor for a 100 A supply (Table 54.7 / 544.1.1), 10 mm² minimum main bonding to gas, water and oil (Table 54.8), specific care around metal water service entries, and an EV charger O-PEN protection or earth electrode arrangement (Reg 722.411.4.1). TN-S has different defaults, and the A4:2026 PNB / TN-C-S notes around BS 7430 conditions also turn on the type. Designing without knowing the TN type is designing without a foundation. The schedule records it on the cover page and every Zs calc downstream uses the declared Ze.',
  },
  {
    id: 'sched-future',
    question:
    ...
    options: [
      'Leave the future loads off the schedule entirely; the next contractor can redo the whole calc when the chargers are actually installed.',
      "Add a 'future load reservation' line for each foreseeable load and sum it into a 'design ultimate MD' that the supply and tails are sized for.",
      "Add the full nameplate of every possible future charger to today's MD with no diversity, so the supply is guaranteed never to be short.",
      'Record the future loads as a verbal note to the client only, keeping the design pack limited to loads connected on the day of handover.',
    ],
    correctIndex: 1,
    explanation:
      "Reg 132.13 documentation is for the whole installation life, not just today. The competent way is a 'design ultimate' column alongside 'design current', with a note explaining what is committed today vs reserved — supply, intake, main tails and CU busbars sized for the ultimate; circuit-level installs done as required. The DNO connection enquiry quotes the ultimate MD with the staging plan; the EIC notes which loads are commissioned vs reserved. This avoids the rip-out problem when the EV charger is added two years later and the tails are 16 mm where 25 mm was needed.",
  },
];
...
const quizQuestions = [
  {
    id: 1,
    question:
      'Why is consolidating diversity across mixed occupancy harder than the per-section calc?',
    options: [
      'Because a mixed-occupancy building always uses a single global diversity factor, which is harder to look up than the per-section tables.',
      "Because each block uses its own diversity assumptions and the schedule has to add them phase by phase without losing each block's logic.",
      'Because BS 7671 forbids applying diversity to any installation that mixes commercial and residential loads on one intake.',
      "Because mixed occupancy means the DNO sizes the supply, so the designer's diversity calc is purely advisory and rarely used.",
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    id: 2,
    question:
      'The single most consequential supply parameter recorded on a consolidated load schedule is:',
    options: [
      'The total connected load in kW, because it is the largest number on the schedule and therefore the most important.',
      'The colour of the meter tails, because it tells the installer which terminals to use at the intake.',
      'The TN type (TN-S, TN-C-S, TT), because earthing, conductor sizing and every Zs calc depend on it.',
      'The number of dwellings, because it sets the coincidence factor and nothing else on the schedule matters as much.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    id: 3,
    question:
      'On a mixed retail-plus-residential block sharing a single intake, the heaviest single-phase MD is found by:',
    options: [
      'Adding every load on every phase together, then declaring that total as the single-phase MD.',
      'Taking the average of the three phase loads, since the supply only needs to carry the mean demand.',
      'Dividing the total consolidated load by three, assuming the intake automatically balances itself.',
      'Allocating each block to the phase it lands on and reporting the phase with the highest sum.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    id: 4,
    question:
      'Reg 311.1 (BS 7671 A4:2026) requires the designer to assess maximum demand. The consolidated schedule satisfies this when it shows:',
    options: [
      'Connected load, per-block diversity with citations, after-diversity demand, per-phase split and a declared MD.',
      'A single headline MD figure only, since showing the working would let competitors copy the design.',
      'The connected load at full nameplate, leaving the diversity to be applied by the DNO at connection.',
      'The protective device ratings alone, because Reg 311.1 is concerned with discrimination rather than demand.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'The total connected load at full nameplate, so the DNO always sizes the supply generously.',
      "The consolidated after-diversity per-phase MD plus a labelled 'design ultimate' with the staging plan.",
      'The rating of the intake fuse offered, since that is the figure the DNO is actually metering against.',
      'Only the largest single load on the schedule, because the supply must at minimum cover the biggest appliance.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    id: 6,
    question:
      'On the consolidated schedule, the line that triggers the most conversation with Building Control is usually:',
    options: [
      'The chosen brand of consumer unit, because Building Control must approve every manufacturer used on a project.',
      'The cable colours specified, because Building Control checks the harmonised colour code on every schedule.',
      'The total floor area and occupancy classification, because it drives the life-safety load and Part B fire compliance.',
      'The diversity factor applied to the lighting circuits, because that is the figure Building Control scrutinises most closely.',
    ],
    correctAnswer: 2,
    explanation:
      'Floor area and occupancy classification drive life-safety load — emergency lighting, fire alarm, smoke ventilation — and determine whether the installation falls under HRRB / Building Safety Act 2022 scrutiny. Emergency lighting on Cat 3 changeover, fire alarm panel and standby supply, smoke ventilation interlocks, accessibility lift if Part M applies — all of these add to the load schedule and bring Building Control questions. On a residential block above 18 m the Building Safety Regulator gateway 2 review will ask to see this line proven.',
  },
  {
    id: 7,
    question:
      "What does the consolidated schedule's 'per-phase balance' figure tell the design team?",
    options: [
      "The total energy the building will use over a year, which sets the customer's electricity tariff band.",
      'Which phase the DNO transformer is wound on, so the meter installer can match the rotation at the intake.',
      'The prospective fault current at each final circuit, which the per-phase figure is used to calculate directly.',
      'Whether the load is well balanced across the three phases or imbalanced with one phase dominant and a high neutral current.',
    ],
    correctAnswer: 3,
    explanation:
      'Per-phase balance is an indicator the L3 designer should always read. Balanced loading gives similar values on all three phases — efficient, low neutral current. Imbalance above ~15-20 percent between phases at the consolidated MD point means heavy neutral current, possible 4th-wire derate, possible voltage imbalance at the most-loaded tenant, and a derate consideration on the neutral conductor for harmonic-rich loads. Rebalance by re-assigning sub-meters to phases before going to the DNO with the enquiry.',
  },
  {
    id: 8,
    question:
      "The consolidated schedule for a four-flat-plus-retail block reports headline MD 74 A on the heaviest phase. The DNO standard is 80 A per phase domestic. The L3 designer's next step is:",
    options: [
      'Check the design ultimate column, and request a 100 A or load-management upgrade now if the future loads exceed the standard 80 A per phase.',
      'Sign off the supply at 80 A per phase immediately, since 74 A is within the standard offer and no further review is needed.',
      'Rebalance the load so each phase reads exactly 74 A, then request a 74 A per-phase supply to match the calculated demand precisely.',
      'Remove the two flats sharing the retail phase from the schedule, so the declared headline figure drops below 80 A on paper.',
    ],
    correctAnswer: 0,
    explanation:
      "Designing only to today's MD is short-termism. If the design ultimate (with EVs and heat pumps that the brief flags as foreseeable) exceeds the standard 80 A per phase, ask now for the upgrade or for a load-management connection rather than reapplying when the spare way is wired in two years. The DNO charges for an upgrade later are usually more disruptive and more expensive than asking up front while the trench is open and the meters are being wired. This is the L3 design pen at its most useful.",
  },
];
...
const faqs = [
  {
    question:
      "Do I have to balance loads across phases as a designer, or is that the DNO's problem?",
    answer:
  ...
  {
    question:
      'How do I cite diversity assumptions on a consolidated schedule when each block uses a different reference?',
    answer:
...
export default function Sub6() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 2 · Subsection 6"
        title="Load schedule consolidation — mixed-use worked example"
        backTo="/study-centre/apprentice/level3-module6-section2"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The page where every diversity rule from 2.1 to 2.5 collides on one schedule. Anchor calc:
          ground-floor retail plus four one-bed flats above, single TN-C-S intake. Walk the maths
          line by line and produce the consolidated maximum demand the L3 designer hands to the DNO.
        </p>
        ...
        >
          <p>
            ...
            block uses its own diversity assumption. A flat uses domestic On-Site Guide Appendix A /
            On-Site Guide Appendix A logic. A retail unit uses commercial after-diversity factors
            based on the tenant's likely fit-out. A lift uses the manufacturer datasheet plus a
            starting allowance. The consolidated schedule preserves each block's calc and cites it,
            then sums on a per-phase basis to give the supply-sizing figure.
          </p>
          <p>
            Compare with the wrong approach: drop every connected load onto a spreadsheet, apply one
            global diversity factor (60 percent, 75 percent — pick a number) and report the total.
            That is unverifiable and almost always wrong — sometimes high (oversized supply,
            customer pays for unused capacity), sometimes dangerously low (undersized supply, intake
            fuse blowing on a busy Friday). The consolidated approach is the professional standard.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 311.1 says two things. First, you must assess maximum demand — not guess, not copy
              from the previous job, but assess. Second, diversity may be taken into account —
              meaning the assessment is allowed to use after-diversity demand rather than raw
              connected load. The consolidated schedule is how you demonstrate compliance with 311.1
              on a multi-block installation: each block assessed, each diversity factor cited, the
              per-phase consolidation visible, the headline MD declared.
            </>
          }
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              fridge x2, microwave x2, induction hob 7 kW, oven 5 kW, hot water 9 kW, lighting 1.5
              kW, sockets ring, water boiler 3 kW, EPOS / refrigerated display.
            </li>
            <li>
              Four one-bed flats — each with electric shower 9.5 kW, oven 5 kW, hob 7 kW (two of the
              four), gas hob (other two), immersion 3 kW (two of the four), lighting 0.4 kW, two
              ring socket circuits.
            </li>
            <li>
              Common services — entrance and corridor lighting (LED, 0.5 kW), entry door release,
              CCTV (0.2 kW), small lift (5.5 kW motor, 11 A FLC three-phase), refuse-store lighting.
            </li>
            <li>
              Life safety — emergency lighting (self-contained, 0.05 kW continuous trickle), Grade A
              fire alarm panel and standby (0.1 kW continuous).
            </li>
            <li>
            ...
            <li>
              Supply offered — TN-C-S (PME), three-phase 400 V, declared Ze 0.35 ohms, PSCC 16 kA at
              the intake, intake cut-out BS 88-3 100 A per phase as standard offer.
            </li>
          </ul>
          <p>
        ...
        >
          <p>Connected load (single-phase 230 V where unspecified):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              Espresso, water boiler, microwave x2 — combined 6 kW @ 50 percent (intermittent): 3
              kW.
            </li>
            <li>
              Fridges, EPOS, display refrigeration — combined 1.8 kW @ 100 percent (continuous): 1.8
              kW.
            </li>
          ...
          <p>
            After-diversity demand: 7 + 5 + 6.75 + 3 + 1.8 + 1.35 + 2 = 26.9 kW. At single-phase 230
            V the line current would be 26900 / 230 = 117 A. That exceeds the practical single-phase
            service for a retail unit. The L3 designer specifies a three-phase TP+N intake to the
            retail meter (DNO standard for commercial), balancing across three phases.
            After-diversity per phase = 26900 / (3 x 230) = 39 A per phase, balanced.
          </p>
          <p>
            Cite: 'After-diversity assessment per IET Guidance Note 1, commercial cafe load survey
            base; manufacturer hot-water cycle assumption 75 percent steady; sockets assessed at 2
            kW based on tenant equipment list.'
          </p>
        </ConceptBlock>
        ...
        >
          <p>Per flat with electric hob and electric shower (worst case, two of the four flats):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>Shower 9.5 kW @ 100 percent (largest single load — counted full): 9.5 kW.</li>
            <li>
              Hob 7 kW: first 10 A at 100 percent + 30 percent of remainder (OSG cooking diversity
              for households other than small flats — adjusted for one-bed flat): assessed 4 kW.
            </li>
            <li>Oven 5 kW: 30 percent of remainder of cooking allowance: 1.5 kW.</li>
            <li>
              Immersion 3 kW (where present): 100 percent (water heater on its own circuit): 3 kW.
            </li>
          ...
          <p>
            ...
            sockets): 9.5 + 4 + 1.5 + 3 + 0.26 + 2 = 20.26 kW. At 230 V single-phase that is 88 A
            per flat — uncomfortably close to the 100 A intake fuse on a per-flat basis, hence the
            importance of diversity in the assessment.
          </p>
          <p>
        ...
        >
          <p>Common services and life safety (continuous, 100 percent demand):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>
            Total common services load: 0.95 kW = 4.1 A on whichever phase the common-services meter
            sits.
          </p>
          <p>
            Lift (small four-stop, 5.5 kW three-phase motor, FLC 11 A balanced across all three
            phases). Running diversity 100 percent (lift will run at peak), starting allowance 1.25
            x for sequential allocation: 11 x 1.25 = 13.75 A per phase peak. The supply must cope
            with the lift starting on top of the flats and retail running, so the consolidation row
            uses 14 A per phase for the lift block.
          </p>
          <p>
            Cite: 'Manufacturer lift datasheet 5.5 kW motor, 11 A FLC balanced; 1.25 x starting
            allowance per IET Guidance Note 1; common services at 100 percent per BS 5266 / BS 5839
            standby continuous loads.'
          </p>
        </ConceptBlock>
        ...
        >
          <p>Three-phase intake L1 / L2 / L3. Allocations:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              Flat 4 (51 A) — allocated to L1 (rebalancing, putting larger flat first then loaded
              smaller flat to same phase as larger of remaining options).
            </li>
            <li>Common services (4.1 A) — allocated to L2.</li>
            <li>Lift (14 A balanced) — adds 14 A to each of L1, L2, L3.</li>
          </ul>
          <p>Per-phase totals (rounded):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>
            ...
            one of them is dominated by it. The honest engineering answer is: the standard 100 A per
            phase supply does not fit this block once you account for two-flat-per-phase loading on
            shower-plus-hob flats. The L3 designer either lobbies the client to allow gas hobs in
            the bigger flats (drops MD substantially), or applies for a 200 A per phase supply, or
            specifies a load-management scheme on the flat shower circuits.
          </p>
          <p>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              installations into circuits and sub-mains so faults, harmonic currents and imbalance
              do not propagate widely. On a small mixed block, allocation is manageable; on a larger
              block with heavy single-phase cooking loads the imbalance directly limits the
              practical supply size and may require either a phase-balanced load-management scheme
              or moving the heaviest appliances onto a balanced TP+N circuit. The consolidated
              schedule shows the per-phase split; Reg 314.1 is the regulation framework that drives
              the response.
            </>
          }
        ...
        >
          <p>Future load reservations:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Retail staff EV charger 7 kW single-phase year 2 — committed. After-diversity at 100
              percent for the EV (slow steady charge during evening): 30 A on its assigned phase.
            </li>
            <li>
              Residents' EV chargers, four in total, foreseeable within five years. Residential EV
              diversity per OSG / IET CoP for EV charging: roughly 1.0 + 0.4 x (n - 1) where n is
              number of chargers, capped — for four chargers (4 x 7 kW = 28 kW raw) the
          ...
          <p>
            Design ultimate per phase (with current MD plus committed plus foreseeable future loads,
            balanced as well as practical): roughly 220-240 A per phase on the heaviest phase. The
            L3 designer presents this to the DNO as the design ultimate and asks whether the supply
            offer can be upgraded now to a 250 A or 300 A per-phase service, or whether a
            load-management connection (DNO-approved demand limiter) is more economic for the
            project.
          </p>
          <p>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 132.16 is why the consolidated schedule has to capture the design ultimate
              (committed plus foreseeable future loads) and not just today's load. Every future EV,
              heat pump or change-of-use is an addition that must be assessed against the rating and
              condition of the existing supply, intake, sub-mains and earthing. A consolidated
              schedule that records what supply capacity was assumed and what headroom remains is
              the document the next designer relies on to discharge their Reg 132.16 duty without
              re-doing the entire calc.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              comes out enormous (300 A per phase for our small block), the DNO offers a substation
              upgrade at five figures, the customer panics, the project loses six months while the
              supply is engineered for a load that will never actually exist.
            </>
          }
          doInstead={
            <>
              Always quote after-diversity MD with the diversity assumptions visible. Show the
              connected load for traceability, but the figure for supply sizing is the consolidated
              after-diversity per-phase MD. Add the design-ultimate column for committed and
              foreseeable future loads. The DNO sizes the supply on engineering data, not on a
              worst-case flat-add.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              The schedule shows 'consolidated MD: 350 A'. The DNO asks 'per phase?' and you have no
              answer. You guess 'about 120 A per phase' — but on a heavy single-phase load like our
              shower-plus-hob flats the actual heaviest phase is 192 A, not 120. The supply is sized
              for 120 A, the heaviest phase trips on a Saturday morning, the inspector asks why the
              schedule was not phase-balanced.
            </>
          }
          doInstead={
            <>
              Always show the per-phase split openly. Allocate single-phase loads to specific
              phases, balance where possible, accept imbalance where the loads do not balance, and
              report the heaviest phase as the supply-sizing figure. The DNO needs the per-phase
              figure; the inspector needs to see the allocation; the customer needs to see the
              supply is sized for the worst-case usage, not the average.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              Our anchor block. Brief delivered by the developer in March; design due to the DNO by
              mid-April for a June groundwork start. The developer assumed a 100 A three-phase
              service — that is what the previous block they built had. You produce the consolidated
              schedule in week one of design.
            </>
          }
          whatToDo={
            <>
              ...
              including committed retail EV plus foreseeable residents' EV plus possible heat pump
              retrofit. Lodge the DNO connection enquiry for a 250 A per-phase service with a
              load-management secondary option, and forward the schedule with the enquiry. The DNO
              offers a 250 A per-phase dedicated cable from the local substation; the installation
              cost lands in the developer's planning budget rather than as a surprise bolt-on.
            </>
          }
          whyItMatters={
            <>
              Consolidating diversity at design stage is the single highest-leverage L3 design
              activity on a mixed-use project. It surfaces the supply-sizing problem when there is
              still time to engineer it; it produces the documentation Reg 311.1 and Reg 132.13
              require; it lets the DNO engineer the connection properly; and it gives the customer a
              project budget that includes the right supply rather than a six-figure upgrade three
              years after handover. The schedule is not just a calc — it is the document that
              prevents the most common cause of post-occupation supply complaints.
            </>
          }
        />
        ...
        >
          <p>Cover sheet content:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>Project, address, designer name, date, revision letter and revision note.</li>
            <li>
              Supply parameters — TN type (TN-S / TN-C-S (PME) / TT), declared Ze (ohms), declared
              PSCC (kA), nominal voltage, phases, intake fuse rating offered.
            </li>
            ...
            <li>
              Floor area and occupancy classification (drives life-safety load and Building Control
              engagement).
            </li>
        ...
        >
          <p>
            ...
            additional protection becomes the disconnection-time mechanism rather than ADS via the
            OPD. Reg 411.5 governs.
          </p>
          <p>
            For single-phase 100 A, the consolidation simplifies — no per-phase split, just one
            after-diversity column. Heavy single-phase mixed loads (showers, hobs, EV chargers) push
            the headline MD against the 100 A intake fuse and force the L3 designer to consider
            load-management or to upgrade to three-phase before the foreseeable future loads exceed
            the supply.
          </p>
          <p>
            For rural overhead supplies, the supply impedance is often higher than declared — Ze may
            be 0.8 ohms or higher rather than the urban 0.35 ohm typical. Add a margin to the Zs
            calc for older overhead networks. For long service-line single-phase supplies the
            voltage drop on the service line itself can dominate the per-load Vd budget; raise this
            with the DNO at the connection enquiry stage.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            balanced. The consolidated schedule's per-phase row gives the imbalance directly: divide
            the highest-phase current by the lowest-phase current and read the ratio.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Imbalance ratio under 1.10 — well balanced. Neutral current low. Standard CSA on the
              neutral.
            </li>
            <li>
              Imbalance ratio 1.10 to 1.30 — moderate. Neutral current significant; check N CSA
              matches phase or is rated for the imbalance.
            </li>
            <li>
              Imbalance ratio above 1.30 — heavy. Neutral approaches phase current; consider
              rebalancing the load allocation, or a fourth-wire neutral upsize.
            </li>
            <li>
              Harmonic-rich loads (LED-heavy lighting, multiple VSDs, switchmode supplies) —
              third-harmonic returns on the neutral and can add. Reg 524.1 and Appendix 4 cover the
              upsize.
            </li>
          </ul>
          <p>
            On our worked example the imbalance ratio is 196 / 53 = 3.7 — extremely heavy. That is
            the trigger to either rebalance (move common services and a flat to L3, swap meter
            assignments) or to consider load management before the DNO connection enquiry is
            finalised. The consolidated schedule reads imbalance plainly; the L3 designer acts on
            it.
          </p>
        </ConceptBlock>
