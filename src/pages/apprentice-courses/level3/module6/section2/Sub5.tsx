...
const checks = [
  {
    ...
    options: [
      'Around 1.25 x FLC = 51 A held for several minutes, matching a typical thermal overload setting.',
      'Around 0.5 x FLC = 20 A, because a motor draws less than running current while it accelerates from rest.',
      'Equal to FLC (41 A) throughout, because direct-on-line means the motor draws rated current from the instant it is switched on.',
      'Around 6-8 x FLC = 250-330 A peak for ~1-3 seconds, decaying as the motor accelerates to speed.',
    ],
    correctIndex: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Largest motor at 100 percent + 50-75 percent of the others running, plus a check on the largest motor starting on top.',
      'All five motors at 100 percent FLC simultaneously, plus all five starting together, with no running diversity applied.',
      'Only the largest motor counted, because the others are assumed never to run at the same time as it does.',
      'A flat 40 percent of the FLC sum, the same factor used for a second domestic ring final.',
    ],
    correctIndex: 0,
    explanation:
      'Industrial motor diversity considers two things: running diversity (which motors run simultaneously?) and starting coordination (the largest starting transient on top of the running others). Typical: largest motor at 100 percent + 50-75 percent of others (running), plus check that the supply can handle the largest motor DOL transient added to the others running. For larger workshops use sequence starting (motors start one at a time with a delay) to avoid concurrent transients, or specify soft-starts / VSDs.',
  },
  {
    id: 'vsd-harmonics',
    question:
    ...
    options: [
      'Higher starting torque only — the VSD raises motor inrush above DOL, so the supply needs a larger transient allowance.',
      'Harmonic injection (5th, 7th, 11th, 13th orders) raises supply RMS current above the calc figure and needs mitigation.',
      'A leading power factor the DNO penalises, requiring capacitor banks to be removed from the supply entirely.',
      'No impact at all — a VSD draws a perfect sinusoid, so the diversity calc treats it exactly like a resistive load.',
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    ...
    options: [
      'The use of single-phase supplies only, since industrial sites rarely have a three-phase connection.',
      'Motor starting transients, process load coincidence, VSD harmonics, and cyclic welder loads.',
      'A legal ban on applying diversity, meaning every industrial circuit must be sized at full nameplate.',
      'The fact that industrial lighting is always assessed at 66 percent, unlike commercial lighting at 100 percent.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Equal to FLC, applied from the instant of switch-on',
      '1.25 x FLC for 1-5 seconds',
      '6-8 x FLC for 1-5 seconds',
      '20-30 x FLC for 1-5 seconds',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'No reduction — a soft-start draws the same 6-8 x FLC as direct-on-line.',
      'A 90 percent reduction, bringing starting current below full-load current.',
      'An increase — a soft-start raises starting current to 10-12 x FLC.',
  ...
  },
  {
    id: 4,
    question:
      'A welding circuit with 80 A peak transient lasting 0.5-2 seconds per weld, intermittent 30 percent duty cycle. Diversity for the welder on the design pack:',
    options: [
      '50-75 percent of peak nameplate, with cable and protection still coordinated to the peak transient.',
      '100 percent of peak nameplate continuously, sizing the supply as though the welder arcs without interruption.',
      '10 percent of peak nameplate, because a welder is so intermittent that its demand is effectively negligible.',
      'Full nameplate plus 50 percent, to allow for the inrush transient each time the arc is struck.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Maximum earth fault loop impedance values for industrial protective devices.',
      'Harmonic distortion limits at the point of common coupling for connections to the public network.',
      'Minimum insulation resistance values for motor windings on industrial installations.',
      'The recommended diversity factors for industrial socket-outlet circuits.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Always 100 percent of FLC continuously, because a compressor never stops once started.',
      'Always 10 percent of FLC, since the compressor spends most of its time at rest.',
      'Per duty cycle — RMS current near 0.7 x FLC at 50 percent duty, sized 70-100 percent of FLC.',
      'A flat 40 percent of FLC, the same factor used for a second domestic ring final.',
    ],
    correctAnswer: 2,
    explanation:
      'Compressor cyclic load: when the air receiver is below set pressure the compressor runs at FLC; when at pressure it stops. Duty cycle depends on demand. RMS current for cyclic load ~ I_peak × √duty. For 50 percent duty, RMS ~0.7 x FLC. Sub-main sizing at 70-100 percent of FLC is typical (depending on duty cycle and number of compressors); protection is coordinated with the starting transient if DOL.',
  },
  {
    id: 7,
    question: 'Reg 132.3 special conditions for industrial typically include:',
    options: [
      'Only the prospective fault current at the origin, which fully defines the industrial demand profile.',
      'Only the lighting load, since lighting is the dominant special condition on industrial sites.',
      'Only the ambient temperature, because it is the single correction factor that matters industrially.',
      'Harmonics from non-linear loads, motor and transformer inrush, welder flicker, and demand variation.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A multi-row diversity calc covering motor running, starting, welder duty, harmonics and PFC.',
      'A single global diversity factor of 60 percent applied to the whole site, the same as a dwelling.',
      'No diversity calc at all, since industrial supplies are always sized at full connected load.',
      'Only a motor schedule, because motors are the sole load category that affects an industrial supply.',
    ],
    correctAnswer: 0,
    explanation:
...
export default function Sub5() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 2 · Subsection 5"
        title="Industrial load assessment"
        backTo="/study-centre/apprentice/level3-module6-section2"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Small / medium industrial workshop with motor circuits, process equipment, welders and
          compressors. Motor starting currents, sequenced starts, VSD harmonic loading and the
          broader Reg 311 picture for industrial sites.
        </p>
        ...
        >
          <p>Three motor start types and their transient profile:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Direct-on-line (DOL)</strong> — full voltage applied at start. Starting
              current = 6-8 x full-load current (FLC). Duration: 1-5 seconds for typical fan / pump
              loads, 5-30+ seconds for high-inertia loads. Cheapest option, most disruptive on
              supply. Suitable for motors up to ~5-7 kW on most supplies.
            </li>
            <li>
              <strong>Soft-starter (electronic)</strong> — voltage ramped up over 5-30 seconds.
              Starting current reduced to 3-4 x FLC. Duration similar but transient lower. Mid-cost.
              Used for motors 5-50 kW and for any motor where DOL transient causes supply problems.
            </li>
            <li>
              <strong>Variable-speed drive (VSD)</strong> — full electronic control of motor speed.
              Starting current 1.5-2 x FLC. Duration controlled by ramp setting. Highest cost but
              lowest transient and additional benefit of running speed control. Used for 5-1000+ kW
              motors and for any motor where speed control adds process value (variable air volume,
              variable pump flow).
            </li>
          </ul>
          <p>Worked example: 22 kW three-phase induction motor, FLC = 41 A.</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              DOL: starting current 7 × 41 = 287 A for 2-3 seconds. Supply must absorb this
              transient on top of running other loads. Type D MCB or motor-rated fuse coordinated
              with thermal overload.
            </li>
            <li>
              Soft-starter: starting current 3.5 × 41 = 144 A over 10-20 seconds ramp. Less peak but
              longer duration; thermal overload sees more I²t. Type C MCB.
            </li>
            <li>
              VSD: starting current 1.8 × 41 = 74 A over 5-30 seconds ramp. Minimal supply
              transient; VSD has internal protection so external MCB just for upstream isolation
              (Type B or C).
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Motor starting, welder transients, capacitor switching and other industrial inrush
              events drive circuit-division decisions under Reg 314.1(e) — mitigate the effects of
              electromagnetic disturbances. The design must show the supply can absorb the
              transients without causing voltage flicker on neighbouring loads (lighting, IT,
              sensitive equipment). Sequence starting, soft-starts, VSDs and segregated sub-mains
              all sit under the Reg 314.1 framework. The L3 designer documents the inrush profile
              and the chosen mitigation for each significant transient source.
            </>
          }
        ...
        >
          <p>Industrial motor diversity has two layers:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Running diversity</strong> — across n motors that may run simultaneously, what
              percentage of the FLC sum is the realistic running peak? Typical: largest motor at 100
              percent + 50-75 percent of remaining motors. For a workshop with 5 x 11 kW motors (FLC
              21 A each, sum 105 A), running peak: 21 + 4 × 21 × 0.6 = 21 + 50.4 = 71 A.
            </li>
            <li>
              <strong>Starting coordination</strong> — when the largest motor starts on top of the
              others running, can the supply absorb the transient? For DOL of the 11 kW (transient 7
              × 21 = 147 A) on top of 4 others running (50 A): peak transient = 147 + 50 = 197 A for
              2-3 seconds. The supply rating must absorb this; the supply main and the motor circuit
              protection must coordinate.
            </li>
          </ul>
          <p>
            Sequence starting prevents concurrent starts. A typical workshop with 5 motors might
            allow only one motor to start at any given time — the second motor waits 5-10 seconds
            after the first reaches running speed. This contains the transient to a single motor at
            a time, manageable on a smaller supply. Sequence logic implemented in the motor control
            centre (MCC) or via PLC.
          </p>
          <p>
            For sites with very large motors (above 50 kW), a soft-start or VSD on the largest motor
            is usually justified — reduces the starting transient by 50-75 percent and removes the
            supply impact issue. The VSD also adds running speed control which often pays for itself
            in process savings.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Welders, compressors, presses, conveyors with stop-start and other cyclic loads have a
            fundamentally different load profile from continuous motors. The peak demand is brief;
            the RMS demand over a work cycle is much lower.
          </p>
          <p>RMS for intermittent / cyclic load:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>Two design implications:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Sub-main and supply sizing</strong> — use RMS. Welder appears on the diversity
              calc as 110 A, not 200 A. Compressor appears as 25 A. Sub-main and origin coincidence
              then applied as normal.
            </li>
            <li>
              <strong>Circuit cable and protection</strong> — must accept the peak transient. Cable
              sized for thermal capacity over the full duty cycle (RMS heating equivalent);
              protection set to allow peak through without nuisance trip (Type D MCB or motor-rated
              fuse with appropriate I²t rating).
            </li>
          </ul>
          <p>
            Both the peak and RMS values go on the diversity calc page. The design pack flags the
            two values explicitly so the future maintainer knows the cable is sized for RMS and the
            protection for peak.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Non-linear loads draw current in pulses rather than smooth sinusoids. The RMS current
            includes the fundamental + the harmonic components. For a typical six-pulse VSD without
            mitigation, the input current THD is 30-80 percent — the RMS is 1.05-1.30 times the
            active-power calc would predict.
          </p>
          <p>Three impacts on the diversity calc:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Cable and supply sizing</strong> — use RMS, not active-power. For VSD-heavy
              sites, multiply the active-power calc Ib by a harmonic factor (typically 1.10-1.25
              depending on VSD count and mitigation).
            </li>
            <li>
              <strong>Neutral conductor sizing</strong> — triplen harmonics (3rd, 9th, 15th) from
              each phase add in the neutral instead of cancelling. For heavy IT load, neutral
              current can equal or exceed line current. Reg 524 covers neutral sizing for harmonic
              loading.
            </li>
            <li>
              <strong>ENA EREC G5/5 compliance</strong> — at the point of common coupling,
              individual harmonic order and total THD limits apply. Sites that exceed must mitigate.
            </li>
          </ul>
          <p>Mitigation options (in order of cost):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Line reactor</strong> — series inductor on VSD input. 40-50 percent harmonic
              reduction. Cheap. Suitable for 1-2 small VSDs.
            </li>
            <li>
              <strong>Passive harmonic filter</strong> — LC tuned to dominant harmonic orders
              (typically 5th and 7th). 60-80 percent reduction. Mid-cost. Per VSD or per group.
            </li>
            <li>
              <strong>12-pulse VSD</strong> — phase-shifted transformer feeds two 6-pulse rectifiers
              in series; lower-order harmonics cancel. Higher cost but very low harmonics intrinsic.
            </li>
            <li>
              <strong>Active harmonic filter</strong> — power electronic device that injects
              compensating current. Broad-spectrum suppression. Highest cost but most flexible.
              Single unit can mitigate multiple VSDs.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The L3 designer's contribution to demand-side optimisation:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Half-hourly meter</strong> — every site over 100 kW peak demand is on
              half-hourly metering. The data is available from the supplier or directly from the
              meter via the meter operator.
            </li>
            <li>
              <strong>Contract demand review</strong> — typically every 3-5 years. The diversity
              calc plus historic data argues the case for an increase or decrease.
            </li>
            <li>
              <strong>Demand response contracts</strong> — for large sites, aggregators (Limejump,
              Flexitricity, Octopus Flex) pay the customer to curtail load during system stress.
              Curtailable load needs to be identified — bulk freezers, EV fleet charging,
              non-critical compressors.
            </li>
            <li>
              <strong>On-site generation</strong> — PV plus battery shifts the customer's kVA peak
              away from the grid peak. The DNO sees a smoother profile; the customer sees a lower
              bill. The L3 designer covers the load assessment; the MCS-certified team designs the
        ...
        <RegsCallout
          ...
          meaning={
            <>
              For industrial sites, daily and yearly variation can be substantial — three-shift
              operation gives a different demand profile from single-shift; seasonal product mix
              changes the motor running pattern; maintenance shutdowns change the supply demand. The
              L3 designer documents the demand profile (typical, peak, low-season, shift-change
              transient) on the diversity calc page. This affects supply sizing, contract demand
              with the DNO, time-of-use tariff selection, and any demand-response contract.
            </>
          }
        ...
        <Scenario
          ...
          situation={
            <>
              Workshop fit-out: 5 x 11 kW machine motors (DOL), 1 x 22 kW compressor (soft-start), 1
              x 30 kW MIG welder (200 A peak, 30 percent duty), shop lighting 8 kW, sockets 12 kW,
              small office 5 kW. Existing 200 A three-phase service. Single-shift 08:00-17:00
              typical; occasional weekend overtime. Customer expects production growth in year 2-3.
            </>
          }
          whatToDo={
            <>
              Diversity calc per category. Motors: largest 22 kW soft-start at FLC 41 A + 5 x 11 kW
              at running diversity 21 + 4 × 21 × 0.6 = 71 A. Total motor running ~112 A across all
              phases (37 A per phase if balanced). Compressor cyclic 50 percent duty: I_RMS = 41 ×
              √0.5 = 29 A across phases (~10 A per phase). Welder 200 A peak at 30 percent duty:
              I_RMS = 110 A on welder phase (assume single-phase 230 V; if three-phase welder,
              distribute across phases). Lighting 8 kW = ~12 A per phase balanced. Sockets 12 kW ×
              0.6 (workshop typical) = 7.2 kW = ~10 A per phase. Office 5 kW × 0.7 = ~5 A per phase.
              {'\n\n'}
              Sum per phase (assume single-phase welder on L1): L1 = 37 + 10 + 110 + 12 + 10 + 5 =
              184 A; L2 = 37 + 10 + 12 + 10 + 5 = 74 A; L3 = 37 + 10 + 12 + 10 + 5 = 74 A. Massive
              imbalance — welder dominates L1.
              {'\n\n'}
              Better: specify three-phase welder with rotating phase distribution OR move other
              loads to balance. With three-phase welder distributed: each line carries 37 A welder
              share added to motor, lighting and socket: L1 = L2 = L3 = ~110 A. Within 200 A service
              with 90 A margin — healthy.
              {'\n\n'}
              Starting coordination: largest motor (22 kW soft-start) starts. Soft-start limits to
              144 A on the soft-start phase mix — acceptable. DOL of any 11 kW motor: 147 A peak.
              With sequence starting (one motor at a time), peak transient added to running ~110 A =
              257 A for 2-3 seconds. Service rated 200 A — transient briefly exceeds. Either: (a)
              accept as transient (cut-out fuse will not blow on 30 percent over-rating for 2
              seconds, but lighting may flicker); (b) soft-start the larger machine motors too; (c)
              upgrade to 250 A service for headroom. Recommend (b) — soft-start on the two largest
              machine motors, accept DOL on smaller. Sequence-start interlocked.
              {'\n\n'}
              Harmonic loading: soft-starts contribute moderate harmonics during the start ramp
              only; no continuous harmonic load. No mitigation needed. ENA G5/5 compliance assumed
              satisfactory based on no continuous VSD load. (Future state if VSD is added on large
              motor, revisit.)
              {'\n\n'}
              Growth: customer expects 20-30 percent growth in year 2-3. Current diversified Ib at
              110 A per phase on 200 A service = 55 percent of rating. Growth to 145 A per phase
              still leaves 55 A margin. HOLD — service supports growth without upgrade. Document
              growth path.
            </>
          }
          whyItMatters={
            <>
              Industrial diversity calcs separate good designers from average ones. The above
              scenario has 4-5 different load profiles (continuous motor running, motor starting
              transient, cyclic compressor, cyclic welder, mixed lighting and socket) all
              interacting. Get the welder phase wrong and L1 trips on overload while L2 / L3 sit at
              half-load. Get the starting coordination wrong and the supply trips at every Monday
              morning warm-up. Get the growth allowance wrong and the customer pays for a rebuild in
              year 3. The L3 designer's diversity calc is what makes the difference.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              A designer calculates motor running diversity (5 motors × 0.6 coincidence × FLC), gets
              a sub-main Ib of 71 A, sizes the 80 A breaker and 25 mm² SWA. The first cold morning
              the largest motor starts DOL on top of the others running — peak transient of 200+ A
              for 3 seconds. The 80 A breaker trips, the 100 A cut-out blows, production stops for
              the morning.
            </>
          }
          doInstead={
            <>
              Always run the starting coordination check. For each significant motor (above ~5 kW),
              calculate the DOL or soft-start transient on top of the others running. The supply
              must absorb the worst-case transient briefly without tripping. Mitigations: sequence
              starting, soft-starts on the larger motors, supply upgrade. Document the chosen
              mitigation on the diversity calc page.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              A designer puts a 200 A peak welder on L1 of the workshop three-phase service, no
              balancing of other loads. RMS demand ~110 A on L1 alone; L2 and L3 carry 60-70 A.
              Heavy imbalance overloads L1 fuse during heavy welding day; voltage on L1 sags
              noticeably affecting other equipment.
            </>
          }
          doInstead={
            <>
              For heavy single-phase loads: (1) specify three-phase equivalent if available
              (three-phase welder, three-phase compressor); (2) if single-phase only, balance other
              loads by allocating heavy single-phase items to under-loaded phases; (3) for very
              large single-phase loads on a three-phase service, consider supply upgrade or
              contractual transfer of unbalanced load to the DNO. Show the per-phase balance on the
              design pack and target under-10-percent imbalance.
            </>
          }
        />
        ...
        >
          <p>Why power factor correction matters at the diversity calc:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Supply capacity</strong> — a 200 kVA service at 0.75 power factor delivers
              only 150 kW of active power. Correcting to 0.95 frees up 40 kW of useful capacity at
              no DNO cost.
            </li>
            <li>
              <strong>DNO availability charge</strong> — billed in kVA. Reducing the kVA drawn
              (without changing kW) cuts the standing charge each quarter.
            </li>
            <li>
              <strong>Cable and switchgear sizing</strong> — sized on amps which equal kVA / V.
              Reactive current heats the cable for no useful work; PFC reduces cable size on the
              next refurb.
            </li>
            <li>
              <strong>VSD interaction</strong> — VSDs draw distorted current at near unity power
              factor on the input. Mixing PFC capacitors with VSD-heavy sites without detuning
              reactors invites resonance — investigate with a PQ logger before specifying.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Common voltage-drop traps on industrial sub-mains:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Long runs to remote plant</strong> — yard pumps, gatehouse, outbuilding
              compressors. 100 m of 35 mm² SWA on a 100 A sub-main loses more volts than the on-site
              DNO transformer impedance.
            </li>
            <li>
              <strong>DOL on a borderline supply</strong> — first motor of the day sees its own
              starting drop; the third motor in sequence sees the cumulative running drop plus its
              own starting drop.
            </li>
            <li>
              <strong>Welder strikes</strong> — instantaneous step from idle to full weld current
              causes a 5-15 percent dip on the welder phase. Adjacent circuits flicker; sensitive
              electronics may reset.
            </li>
            <li>
              <strong>Transformer Zps</strong> — small transformers (200-500 kVA) have 4-6 percent
              impedance. The starting drop at the LV terminals is effectively the upstream motor
              inrush divided into the transformer impedance — bigger transformer, smaller drop.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              Allocate heavy single-phase loads (large welders, single-phase EV chargers) first to
              whichever phase is otherwise lightest.
            </li>
            <li>
              Distribute lighting and small-power circuits to fill remaining capacity per phase.
            </li>
            <li>
              Document the phase-by-phase schedule so future extensions can extend the balance
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Phase balancing aligns with Reg 314.1 — splitting and arranging circuits to contain
              fault impact and limit electromagnetic disturbance. A badly imbalanced three-phase
              install can produce neutral currents that disturb sensitive electronics and trigger
              nuisance protective-device operation. The regulation framework ties physical phase
              allocation back to the design objectives.
            </>
          }
          cite="Source: BS 7671:2018+A4:2026 Regulation 314.1 — full text from published amendment."
        />
