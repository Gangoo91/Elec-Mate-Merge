...
const TITLE = 'Single-line schematic (SLD) production (5.1) | Level 3 Module 6.6.1 | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  {
    ...
    options: [
      'The cable schedule — listing every cable run first fixes the materials, with topology drawn around the cables already chosen.',
      'The single-line diagram (SLD) — it fixes the topology that every other document in the pack then hangs off.',
      'The layout drawings — pinning physical positions of every accessory and DB first lets the topology be inferred from the floor plan.',
      'The Electrical Installation Certificate — completing the EIC up front sets out what is certified, and the pack is built to match it.',
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    question:
      'On the SLD the design Zs at the end of a Type C 32 A circuit-breaker final circuit comes out at 0.85 ohm. The tabulated maximum Zs for a Type C 32 A is 0.68 ohm (it trips magnetically at 10x In, so the same figure applies for both 0.4 s and 5 s). What do you do at SLD level?',
    options: [
      'Leave it — 0.85 ohm is close enough to 0.68 ohm that the cold cable at test absorbs the difference, so no design change is needed.',
      'Re-label the circuit as a 5 s distribution circuit, which gives the Type C 32 A a higher Zs ceiling and makes 0.85 ohm pass.',
      'Accept the 0.85 ohm and rely on the main switch to clear any fault eventually, even if the local breaker does not operate in time.',
      "Annotate the SLD with the calc'd Zs and the regs maximum, flag the fail, then resolve it (add a 30 mA RCD/RCBO, upsize the cable, or change device type).",
    ],
    correctIndex: 3,
    explanation:
      'The SLD is the design dashboard. Each protective device should be annotated with the design Zs vs the regs ceiling (Table 41.3). For a Type C 32 A the ceiling is 0.68 ohm and, because it trips on its 10x In magnetic element, the same figure applies to both the 0.4 s and 5 s criteria — so re-labelling the circuit does not help. A design Zs of 0.85 ohm fails. The fix is a design choice: add a 30 mA RCD/RCBO (which provides fault protection by detecting the residual current irrespective of Zs); upsize the cable to drop Zs below 0.68 ohm; change device characteristic; or relocate the DB closer. Re-annotate after the fix.',
  },
];
...
const quizQuestions = [
  {
    ...
    options: [
      'The exact physical positions of every socket, switch and luminaire on a scaled floor plan — the SLD is a to-scale layout drawing of where every accessory sits.',
      'The topology — supply origin, sub-mains, distribution boards, protective devices and final-circuit groups — drawn one line per cable and annotated with ratings and fault levels.',
      'Every individual conductor drawn separately — line, neutral and CPC each shown as their own line so the reader can count the cores in every cable.',
      'Only the consumer unit and its circuits — the SLD stops at the main board and does not show sub-mains, distribution boards or the supply origin.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      'Graphical symbols for diagrams — the recognised symbol set for breakers, fuses, switches, cables and the other components on UK electrical drawings.',
      'Inspection and testing procedures — the standard that defines the test sequence and acceptance criteria for initial verification.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Only the device rating in amps — the SLD shows just the breaker size and nothing else about the device on the circuit.',
      "Only the manufacturer's part number — so the installer can re-order the exact device, with no rating, curve or Zs information shown.",
      'Only the cable size feeding the device — the protective device itself is left unannotated because the cable defines the circuit.',
      'Type and rating, curve, breaking capacity, design Zs against the regs maximum, and the discrimination check with the upstream device.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Power topology, annotations, revision clouds and the title block — with optional sub-discipline layers that can be toggled on or off for clarity.',
      'Everything on a single layer — keeping the whole SLD on one layer is simplest and avoids the reader having to toggle anything between views.',
      'By colour of cable only — separating brown, blue and green-and-yellow onto their own layers so each conductor colour can be hidden independently.',
      'By the electrician who drew each part — one layer per operative so it is clear who drafted which section for accountability later.',
    ],
    correctAnswer: 0,
    explanation:
      "At minimum the layers separate power topology (cables, breakers, DBs), annotations (ratings, calc results), revision clouds and notes, and the legend and title block. Some designers add layers for fault current, voltage drop, disconnection time and sub-discipline (emergency lighting, fire alarm, IT critical) so each can be toggled for that discipline's checks. The topology layer is always on; the rest toggle per audit purpose. Without layer discipline an SLD becomes unreadable as the project grows. CAD packages enforce layers automatically; hand-drawn SLDs need explicit colour or hatching coding.",
  },
  {
    id: 5,
    question: 'Why does the SLD include Ze, PSCC and Zs values directly on the drawing?',
    options: [
      'So the customer can see how much the protection cost — the fault-level figures are a pricing reference that justifies the breaker selection to the client.',
      'So the tester can compare measured Ze and Zs against the design values at verification, with any mismatch flagging a measurement or install issue.',
      'Only as decoration — the Ze, PSCC and Zs figures are not used after design and are printed purely to make the drawing look thorough.',
      'So Building Control can set the notification fee — the fault-level figures determine the cost of the Part P submission for the job.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'By the date the file was last saved — whoever has the most recent save date holds the current version, with no revision letters needed.',
      'By the file size — the largest file is assumed to be the most complete and therefore the current revision in active use.',
      'Revision letters or numbers in the title block, with a revision history table, and superseded revisions marked VOID.',
      'By the colour of the printout — the current revision is always printed in colour and superseded ones in black and white, so site can tell at a glance.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'By drawing the two devices the same size — equal symbol sizes on the SLD indicate the devices discriminate with each other.',
      'By colouring the downstream device green and the upstream device red — the colour pair signals which device should trip first.',
      'Discrimination is not shown on the SLD at all — it is purely a verification check carried out with an MFT after the install.',
      'A discrimination annotation against each upstream/downstream device pair, citing the test method and the manufacturer coordination table referenced.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    id: 8,
    question:
      'Which BS 7671 regulation underpins the requirement to produce and retain the SLD as part of the design pack?',
    options: [
      'Regulation 132.13 — the design documentation requirement, with Reg 514.9.1 on the per-DB diagram and Reg 644.1.1 on documentation backing the EIC.',
      'Regulation 411.3.2 — the automatic disconnection of supply requirement, which is the regulation that calls for an SLD as part of the protection design.',
      'Regulation 643.7 — the RCD test requirement at verification, which mandates the SLD so the tester knows which RCDs to test on site.',
      'Regulation 522.6 — the cable installation method requirement, which is taken as the basis for producing the single-line diagram.',
    ],
    correctAnswer: 0,
    explanation:
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 6 · Section 6 · Subsection 1"
        title="Single-line schematic (SLD) production"
        backTo="/study-centre/apprentice/level3-module6-section6"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The SLD is the spine of the design pack. Every other document hangs off it. What it must
          show, IEC 60617 conventions, CAD layer discipline, fault-level and breaker annotation,
          version control, and the audit trail back to BS 7671 Reg 132.13 and 514.9.1.
        </p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Supply origin</strong> — DNO service position, service fuse rating, meter,
              main isolator, main earthing terminal (MET) and the connection to the earthing system
              (TN-S, TN-C-S PME or PNB, TT).
            </li>
            <li>
              <strong>Main switchgear and sub-mains</strong> — every sub-main breaker or fuse from
              the origin out to the distribution boards, with cable specs (type, size, length) and
              the protective device on each end.
            </li>
            <li>
              <strong>Distribution boards (DBs)</strong> — every DB drawn as a labelled rectangular
              block, with its supply breaker, busbar rating, way count and any RCD or RCBO grouping.
            </li>
            <li>
              <strong>Final-circuit protective devices</strong> — every breaker, fuse, RCBO or AFDD
              on the DB, with type, rating, curve characteristic, breaking capacity and BS
              reference.
            </li>
            <li>
              <strong>Cable annotations</strong> — type (e.g. T+E, FP200, XLPE/SWA), size (mm²
              conductor cross-section), length (metres), and Reference Method per BS 7671 Appendix 4
              if relevant.
            </li>
            <li>
              <strong>Fault-level annotations</strong> — Ze at the origin (DNO declared), PSCC at
              the origin, calculated Zs at the end of each final circuit, regs maximum Zs from
              Tables 41.3/41.4 or 41.5.
            </li>
            <li>
              <strong>Discrimination annotations</strong> — design check between each
              upstream/downstream device pair.
            </li>
          </ul>
          <p>
            Crucially, the SLD does NOT show physical positions of accessories (those go on layout
            drawings) and does NOT show every individual conductor (each cable is one line
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The Reg 132.1 framework is the design documentation hook for the whole pack. It does
              not name the SLD specifically but the SLD is the document that makes the determination
              and recording requirement achievable — without a topology diagram, you cannot operate,
              maintain, alter or extend the installation through its life because nobody can see
              what is connected to what. Cite the Reg 132.1 framework in the title block of the SLD
        ...
        >
          <p>
            IEC 60617 (Graphical Symbols for Diagrams) is the IEC standard adopted in the UK that
            defines the graphical alphabet of electrical drawings. The relevant parts for SLD work
            include:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Part 2</strong> — Symbol elements, qualifying symbols and other symbols having
              general application.
            </li>
            <li>
              <strong>Part 3</strong> — Conductors and connecting devices (cables, terminals, plugs,
              sockets, junctions).
            </li>
            <li>
              <strong>Part 4</strong> — Basic passive components (resistors, capacitors, inductors).
            </li>
            <li>
              <strong>Part 5</strong> — Semiconductors and electron tubes (diodes, transistors,
              thyristors — relevant for inverter and EV charger drawings).
            </li>
            <li>
              <strong>Part 6</strong> — Production and conversion of electrical energy (generators,
              transformers, motors).
            </li>
            <li>
              <strong>Part 7</strong> — Switchgear, controlgear and protective devices (breakers,
              fuses, isolators, RCDs, RCBOs, contactors).
            </li>
            <li>
              <strong>Part 8</strong> — Measuring instruments, lamps and signalling devices.
            </li>
            <li>
              <strong>Part 11</strong> — Architectural and topographical installation plans and
              diagrams (the symbol set for layout drawings).
            </li>
          </ul>
          <p>
            For SLDs the most-used parts are 2, 3, 6 and 7. Most professional CAD packages ship with
            a 60617-compatible symbol library; verify before relying on it. Hand-drawn SLDs should
            reference a printed IEC 60617 quick-card pinned to the desk so the symbols are drawn
            consistently. Symbol consistency is what lets a tester two years later read the drawing
            without translation.
          </p>
        </ConceptBlock>
        ...
        >
          <p>The high-frequency symbols on a UK commercial SLD:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Service fuse</strong> — rectangular outline with diagonal slash and rating
              annotation (e.g. 'BS 88-3 100 A').
            </li>
            <li>
              <strong>Main switch / isolator</strong> — circle with diagonal slash and contact lines
              (open or closed depending on default state); annotated with rating and pole count.
            </li>
            <li>
              <strong>MCB</strong> — small rectangular outline with curve indicator (B, C, D) and
              rating; annotated with breaking capacity (Icn).
            </li>
            <li>
              <strong>RCBO</strong> — MCB symbol with attached RCD (round symbol with delta/sigma
              marker for residual current sensing) and trip rating annotation (e.g. 30 mA Type A).
            </li>
            <li>
              <strong>AFDD</strong> — RCBO-style symbol with arc fault indicator (often a stylised
              lightning or 'AFDD' text label per A4:2026).
            </li>
            <li>
              <strong>MCCB</strong> — larger rectangular outline with adjustable trip indication
              (thermal-magnetic or electronic) and pole count slash; annotated with breaking
              capacity (Icu/Ics) and trip range.
            </li>
            <li>
              <strong>Distribution board</strong> — labelled rectangular block with internal busbar
              representation; annotated with way count, busbar rating and TP+N or SP+N designation.
            </li>
            <li>
              <strong>Cable</strong> — single line with annotation (e.g. 'XLPE/SWA 4c × 25 mm² +
              CPC, 65 m, Ref Meth E').
            </li>
            <li>
              <strong>Earth electrode</strong> — three-bar earth symbol with annotation (e.g. 'Earth
              rod 16 mm × 2.4 m to BS 7430').
            </li>
            <li>
              <strong>Main earthing terminal (MET)</strong> — labelled terminal block with bonding
        ...
        >
          <p>The professional drafting order:</p>
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Service position and meter</strong> — DNO service fuse, meter, isolator.
              Annotate Ze (DNO declared), PSCC, earthing arrangement.
            </li>
            <li>
              <strong>Main switchgear</strong> — main switch, MET, primary distribution. Annotate
              Icu, Ics, bonding conductor sizes per Reg 543/544.
            </li>
            <li>
              <strong>Sub-mains</strong> — sub-main breakers and cables to each downstream DB.
              Annotate device rating, cable spec, length, design Zs at sub-DB.
            </li>
            <li>
              <strong>Distribution boards</strong> — each DB block with way count, busbar rating,
              RCD or RCBO grouping.
            </li>
            <li>
              <strong>Final circuit protective devices</strong> — every breaker on every DB.
              Annotate type, rating, curve, breaking capacity.
            </li>
            <li>
              <strong>Final circuit cables</strong> — cable spec and length per circuit. Annotate
              Reference Method, derating factors if non-typical.
            </li>
            <li>
              <strong>Calc back-fill</strong> — once topology is fixed, run the calc sheet (cable
              CCC, voltage drop, fault Ip, Zs at end of circuit) and back-fill the SLD with the
              calc'd values. Highlight any failures with revision clouds.
            </li>
            <li>
              <strong>Discrimination check</strong> — pair-by-pair check of upstream/downstream
              devices using manufacturer coordination tables. Annotate pass / fail /
              cascading-allowed per pair.
            </li>
          </ol>
          <p>
            Following this order means the topology is fixed before detailed annotations, which
            prevents the most common drafting waste — drafting cable annotations against a topology
            that then changes. Get topology right first, annotate second.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The fault-level cascade is the single most-tested calculation chain in any audit. The
            SLD makes it visible:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Ze (origin)</strong> — DNO declared external earth fault loop impedance.
              Typical TN-C-S PME default 0.35 ohm; TN-S 0.8 ohm or per DNO; TT measured (cannot be
              declared, depends on electrode quality).
            </li>
            <li>
              <strong>PSCC (origin)</strong> — prospective short-circuit current at the origin per
              DNO declaration. Determines the minimum breaking capacity (Icu) of the main switchgear
              and the cascading limits downstream.
            </li>
            <li>
              <strong>Sub-main impedance</strong> — calculated from cable resistance per metre ×
              length, hot-cable correction. Adds to Ze to give Zdb (impedance at the sub-DB).
            </li>
            <li>
              <strong>Final-circuit impedance</strong> — adds to Zdb to give Zs at the end of the
              final circuit. Compare against regs ceiling per Table 41.3 (0.4 s for circuits up to
              32 A on TN systems for socket outlets) or Table 41.4 (5 s for distribution circuits
              and fixed-equipment circuits).
            </li>
            <li>
              <strong>Discrimination ratio</strong> — at the sub-main breaker, the design fault
              current must be selectively cleared by the downstream device first. Manufacturer
              coordination tables are the authority.
            </li>
          </ul>
          <p>
            Annotating the fault-level cascade on the SLD turns abstract numbers into a visual chain
            — the reader can trace the impedance growth from origin to final circuit and see
            immediately if any final circuit busts the regs ceiling. The SLD is then self-checking;
            the calc sheet provides the back-up for audit.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 514.9.1 is the per-DB diagram requirement. The SLD discharges the topology aspect
              at the design pack level; a derived per-DB schedule (a copy of the relevant slice of
              the SLD plus the schedule of circuit details) discharges the on-site requirement. The
              A4:2026 amendment introduced an exception specifically for domestic premises — read
              the amended text to confirm the precise scope. For all non-domestic work, the per-DB
              diagram requirement is full force.
            </>
          }
        ...
        >
          <p>The minimum layer set on a commercial SLD:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>00-Title-block</strong> — frame, title block, revision history table. Always
              on.
            </li>
            <li>
              <strong>10-Topology</strong> — cables, breakers, DB blocks, isolators, MET. Always on.
            </li>
            <li>
              <strong>20-Annotations-rating</strong> — device ratings, cable specs, lengths. Always
              on for design issue.
            </li>
            <li>
              <strong>30-Annotations-fault</strong> — Ze, PSCC, Zs, discrimination check results.
              Toggled on for fault-level audit.
            </li>
            <li>
              <strong>31-Annotations-vd</strong> — voltage drop per circuit. Toggled on for
              voltage-drop audit.
            </li>
            <li>
              <strong>40-Sub-emergency-lighting</strong> — emergency lighting circuits highlighted.
              Toggled on for emergency lighting compliance check.
            </li>
            <li>
              <strong>41-Sub-fire-alarm</strong> — fire alarm circuits highlighted. Toggled on for
              BS 5839 compliance check.
            </li>
            <li>
              <strong>42-Sub-IT-critical</strong> — IT and critical circuits highlighted. Toggled on
              for resilience check.
            </li>
            <li>
              <strong>90-Revision-clouds</strong> — revision clouds and change markers. Toggled on
              for the current revision review.
            </li>
            <li>
              <strong>99-Notes-general</strong> — general notes panel, legend, abbreviations. Always
              on.
            </li>
          </ul>
          <p>
            ...
            fault-level auditor turns on layers 10, 20, 30 only and sees a clean fault-level view; a
            fire-engineer auditor turns on layers 10, 20, 41 and sees the fire alarm topology only.
            Without layers, every reader sees every annotation and the drawing becomes a wall of
            text.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Revision letters or numbers</strong> — go up sequentially (Rev A first issue,
              B, C, D... or Rev 1, 2, 3, 4...). Some firms use both (Rev P1, P2, P3 for preliminary
              issues; Rev A, B, C for issued-for-construction).
            </li>
            <li>
              <strong>Revision history table</strong> — one row per revision: date, drafter
              initials, checker initials, reason for change (specific — 'added EV charger circuit at
              Customer request 2026-04-15' not 'updated'), what changed (cross-references the
              changed elements).
            </li>
            <li>
              <strong>Revision clouds</strong> — visual markers on the drawing showing what changed
              in the current revision. Cleared when the next revision is issued.
            </li>
            <li>
              <strong>Title block revision indicator</strong> — large clear revision letter in the
              title block so any reader sees instantly which revision they have.
            </li>
            <li>
              <strong>Superseded copies</strong> — physically marked VOID (large red X or VOID
              stamp); preferably destroyed; if retained for historical reference, filed in a
              separate archive folder.
            </li>
            <li>
              <strong>CDE enforcement</strong> — on Common Data Environment (CDE) projects (Procore,
              Asite, BIM 360, Autodesk Construction Cloud), the CDE enforces version control
              automatically; older revisions are archived; only the current revision is downloadable
              for active use.
            </li>
          </ul>
          <p>
            The single most common version-control failure: an installer prints Rev C, takes it to
            site, the designer issues Rev D the next week, the installer keeps working from Rev C
            because nobody told them. Three weeks later the install is built to Rev C, the designer
            comes back to a site that does not match Rev D, and the discussion about who pays for
            the rework starts. Avoid by enforcing CDE access, voiding superseded copies and
            including a 'check revision before working' note in the title block.
          </p>
        </ConceptBlock>
        ...
        >
          <p>The A4:2026-aligned SLD annotation checklist:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>AFDD-RCBO on socket-outlet final circuits ≤ 32 A</strong> — per Reg 421.1.7
              mandatory in HRRBs, HMOs, PBSA, care homes; recommended in other premises. SLD shows
              AFDD symbol with rating and BS EN reference.
            </li>
            <li>
              <strong>SPD at the origin</strong> — per Reg 443.4 mandatory where the consequences of
              transient overvoltage are serious. The risk-assessment method (Reg 443.5) has been
              deleted; designers refer to the categories in 443.4. SLD shows SPD type, location and
              connection scheme.
            </li>
            <li>
              <strong>EV charger dedicated protection</strong> — per Reg 722.531.3.101 each
              connecting point individually protected by 30 mA RCD with DC fault current protection.
              SLD shows RCD type, IΔn, DC sensing method.
            </li>
            <li>
              <strong>RCD test method</strong> — note in the title block or general notes that RCD
              verification per Reg 643.7 is by single AC test at 1×IΔn in line with A4:2026,
              regardless of RCD type.
            </li>
            <li>
              <strong>Generator and battery storage</strong> — per Reg 551.7 additional requirements
              for generating sets in parallel with the public network. SLD shows the parallel
              source, the AC and DC isolation points, the warning notice references per Reg 514.10.
            </li>
            <li>
              <strong>Earth electrode under Reg 542.2.8</strong> — A4:2026 introduced Reg 542.2.8
              with additional electrode requirements; SLD shows electrode location, type, measured
              resistance at handover, connection per Reg 542.3.2.
            </li>
            <li>
              <strong>Edition reference in title block</strong> — 'BS 7671:2018+A4:2026' cited as
              design basis. Anyone reading the pack knows which edition's limits apply.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <Scenario
          ...
          situation={
            <>
              ...
              meter position, Ze 0.35 ohm declared, PSCC 16 kA. The landlord has installed a 400 A
              main switchgear cabinet with a four-pole switch-disconnector at the head; each retail
              unit has been allocated a sub-main of up to 60 A single-phase or 32 A three-phase per
              the lease. Your unit is Unit 7, a small cafe. The cafe brief is: kitchen with
              combi-oven, induction hob, dishwasher, coffee machine, refrigeration; customer area
              with 12-seat dining and small counter; emergency lighting and fire alarm to BS 5839-1
              Cat L1; EV-ready conduit to a future kerbside charger (not installed at fit-out). The
              cafe sub-main allocation is 60 A single-phase with the landlord MCCB at the main
              switchgear set to 60 A trip.
            </>
          }
          whatToDo={
            <>
              ...
              switchgear to your unit's metering position, calculated impedance 0.05 ohm, Zsubmain
              at unit DB = 0.40 ohm. Unit DB: 12-way SP+N RCBO board, 60 A busbar, all ways RCBO
              Type A 30 mA per Reg 411.3.4 and Reg 422 fire-risk consideration for the cafe. Final
              circuits: kitchen combi-oven RCBO Type C 32 A, 6 mm² T+E 8 m, design Zs 0.45 ohm vs
              Type C 32 A 5 s ceiling 1.43 ohm — pass; induction hob RCBO Type C 32 A, 6 mm² T+E 6
              m, similar pass; dishwasher RCBO Type C 16 A, 2.5 mm² T+E 7 m; refrigeration RCBO Type
              C 16 A; ring final RCBO Type B 32 A, 2.5 mm² T+E 38 m; lighting general RCBO Type B 6
              A 1.5 mm² T+E 18 m; emergency lighting maintained circuit on dedicated RCBO Type B 6 A
              with self-contained luminaires per BS 5266; fire alarm panel dedicated RCBO Type B 6 A
              FP200 cable per BS 5839-1; EV-ready spare way reserved with conduit pre-installed.
              Annotate every device with rating, curve, breaking capacity (Icn 6 kA matches the
              cascading allowed by the landlord MCCB Icu 25 kA — manufacturer coordination table
              cited). Annotate every cable with Reference Method (Method C clipped direct typical).
              Annotate Zs at end of every circuit. Run the discrimination check with the landlord
              MCCB and confirm cascading per the manufacturer table is acceptable. Title block shows
              project, drawing number, Rev A first issue, drafter initials, BS 7671:2018+A4:2026
              cited as design basis, IEC 60617 cited as symbol standard. Issue Rev A for client
              review.
            </>
          }
          whyItMatters={
            <>
              The SLD is the spine. The cable schedule (Sub 2) reads off the SLD cable annotations.
              The lighting layout reads off the SLD lighting circuits. The schedule of circuit
              details reads off the SLD device-and-cable pairs. The calc sheet back-fills the SLD
              fault-level annotations. The EIC at handover quotes the SLD revision in the design
              basis section. Every other document is downstream of the SLD. Get the SLD right and
              the rest of the pack falls into place; get the SLD wrong and every downstream document
              carries the error. The SLD is the single document where 80 percent of the design audit
              lives.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              productive. They size cables based on an assumed topology. Two days later the design
              review changes the topology — a sub-DB moves, two final circuits get regrouped, an EV
              charger gets added. Every cable in the schedule now needs re-checking against the new
              topology. The cable schedule was wasted work because topology was not fixed first.
            </>
          }
          doInstead={
            <>
              ...
              follows directly from the SLD cable annotations. Topology changes after the schedules
              are drafted should trigger a revision of both — never let the schedule drift from the
              SLD. The SLD is the single source of truth for topology.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              icon copied from an old project. The reader has to translate between three symbol sets
              to match the same device across the pack. Errors creep in.
            </>
          }
          doInstead={
            <>
              One symbol set across the whole pack — IEC 60617 — enforced by a shared CAD symbol
              library. When working in a CAD package, lock the symbol library at project kick-off
              and require all team members to use the locked library. When working hand-drawn or in
              mixed tooling, agree the symbol set in writing and circulate a one-page symbol legend
              with every issued drawing. Symbol consistency is what makes a pack look professional
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 514.13.1 is one of the warning-notice regs that must be reflected on the SLD and
              the layout drawings. The MET symbol on the SLD should be flagged with a 'Safety
              Electrical Connection' notice indicator; the layout drawing should show the physical
              position of the notice. The 'Do Not Remove' wording is mandatory and the notice must
              be durable and visible. Reg 514.13.2 has a different mandatory wording for
              special-locations electrical separation. Section 514 series cover all the statutory
              notices the design pack must prescribe.
            </>
          }
