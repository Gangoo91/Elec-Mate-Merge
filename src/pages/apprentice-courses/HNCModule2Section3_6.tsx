...
const quizQuestions = [
  ...
  {
    id: 7,
    question: 'A dry-bulb economiser changeover can waste energy on humid days because:',
    options: [
...
const HNCModule2Section3_6 = () => {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 2 · Section 3 · Subsection 6"
        title="Applications to Air-Conditioning and HVAC Load Calcs"
        backTo="/study-centre/apprentice/h-n-c-module2-section3"
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Putting the whole psychrometric toolkit to work on a real air handling unit — mixing,
          coils, economisers and loads.
        </p>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              CIBSE publishes the design guidance the UK industry works to for AHU configuration,
              coil selection and control strategy, and it is the body of guidance a design is
              normally judged against at review. Always work from the current edition in front of
              you rather than a remembered figure, and record in the design file which edition and
              which internal design conditions your load calculation assumed — that record is what
              makes the calculation auditable when the plant is challenged in year one.
            </>
          }
        ...
        >
          <p>
            Everything in the previous five subsections was a single process in isolation. A real
            air handling unit strings those processes together in a fixed order, and the state of
            the air leaving one component is the state entering the next. Draw them end to end and
            the AHU becomes one continuous path across the chart, from the outside condition to the
            supply grille.
          </p>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Outside air intake:</strong> the starting state. It is a design assumption in
              the calculation and a moving target in operation, which is why summer and winter cases
              are calculated separately.
            </li>
            <li>
              <strong>Mixing box:</strong> outside air meets return air. The mixed state lands on
              the straight line between the two state points. This is the one place in the unit
              where two paths become one.
            </li>
            <li>
              <strong>Filter:</strong> no move on the chart. It removes particulate, not heat or
              moisture. It does add resistance, which the fan pays for and which returns downstream
              as heat.
            </li>
            <li>
              <strong>Preheat coil:</strong> a horizontal move to the right. Sensible only, no
              moisture change, relative humidity falls. Its job in winter is frost protection for
              the plant behind it as much as comfort.
            </li>
            <li>
              <strong>Cooling coil:</strong> a move down and to the left, angling toward the
              saturation curve whenever the coil surface is below the entering dew point. Both
              temperature and moisture content fall.
            </li>
            <li>
              <strong>Humidifier:</strong> a move upward. Steam injection is near vertical with a
              slight temperature rise; an evaporative or adiabatic humidifier tracks a line of
              constant wet-bulb, so the air cools as it wets.
            </li>
            <li>
              <strong>Reheat coil:</strong> a horizontal move to the right again, correcting air
              that was deliberately over-cooled to strip moisture out of it.
            </li>
            <li>
              <strong>Supply fan:</strong> a small horizontal move to the right. Fan work becomes
              heat in the airstream, typically a fraction of a degree to about a degree.
            </li>
          </ul>
          <p>
            <strong>The closing check:</strong> the supply state must be able to absorb the room
            sensible and latent gains and arrive back at the return state. If the path you have
            drawn does not close, either the supply condition or the air volume is wrong — and the
            chart will show you which.
          </p>
        </ConceptBlock>
        ...
        <ConceptBlock
          ...
          onSite={
            <>
              Mixing-box temperature is one of the few numbers you can take on site with a single
              probe and turn straight into a damper position. It is the fastest honesty check there
              is on an economiser.
            </>
          }
        >
          <p>
            ...
            conserved. Because moisture content and specific enthalpy are both defined per kilogram
            of dry air, both mix as a straight mass-weighted average:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>ṁ_mix = ṁ₁ + ṁ₂</li>
          ...
          <p>
            <strong>On the chart:</strong> join the two state points with a straight line. The mixed
            state lies on that line, dividing it so that the two segment lengths are in inverse
            proportion to the mass flows. In plain terms, the mixture sits nearest the stream that
            contributes the most air. Twenty per cent outside air on a winter morning barely drags
            the mixed point away from the return condition; a hundred per cent outside air puts the
            mixed point exactly on the intake state.
          </p>
          <p>
            <strong>Why temperature mixing is only almost linear:</strong> enthalpy and moisture
            content mix exactly linearly, and dry-bulb temperature is recovered from those two. The
            small cross-term in the enthalpy relationship means the temperature average is a very
            close approximation rather than an identity — for the ranges seen in comfort HVAC the
            difference is a few hundredths of a degree, well inside chart reading accuracy. Use the
            linear form with confidence; just do not present it as exact in an exam answer that asks
            you to justify it.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            A cooling coil does not choose to dehumidify. Whether it does or not falls out of one
            comparison: the effective surface temperature of the coil against the dew point of the
            air entering it.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Surface above the entering dew point — a dry coil.</strong> Air is cooled but
              no vapour condenses. Moisture content is unchanged, so the chart move is horizontal to
              the left and relative humidity rises. All the duty is sensible.
            </li>
            <li>
              <strong>Surface below the entering dew point — a wet coil.</strong> Air in contact
              with the fins is cooled past saturation, vapour condenses, and the condensate runs to
              the drain pan. Moisture content falls, so the move is down and to the left. Duty is
              now part sensible and part latent.
            </li>
          </ul>
          <p>
            <strong>The effective surface condition:</strong> the coil surface is not one uniform
            temperature, so we characterise it with a single equivalent point on the saturation
            curve — the apparatus dew point covered in the previous subsection. It is the state the
            air would reach if every particle of it touched the coil. Real air does not: some passes
            through the fin gaps barely altered, so the off-coil state sits on the line between
            entering air and that surface condition, short of saturation.
          </p>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Off-coil air from a wet coil typically reads high on the humidity sensor, often in the
              region of 90-95% relative humidity. That is expected, not a fault.
            </li>
            <li>
            ...
            <li>
              ...
              point. At part load, with a modulating valve, that condition can quietly stop being
              true.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>
            Two coils can carry identical total duty and behave completely differently, because duty
            divides into two parts that the chart keeps strictly separate:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Sensible load</strong> — the horizontal component. Temperature change at
              constant moisture: Q̇ₛ = ṁ × cp × Δt, with cp taken as about 1.02 kJ/kg·K for moist
              air.
            </li>
            <li>
              <strong>Latent load</strong> — the vertical component. Moisture change at constant
              temperature: Q̇ₗ = ṁ × Δg × h_fg, with the latent heat of vaporisation around 2450
              kJ/kg at comfort conditions.
            </li>
            <li>
              <strong>Total load</strong> — the whole diagonal, read straight off the chart as an
              enthalpy difference: Q̇ₜ = ṁ × Δh. This is the reliable one, because it needs no
              assumption about cp or h_fg.
            </li>
            <li>
              <strong>Sensible heat ratio</strong> — SHR = Q̇ₛ / Q̇ₜ. It is the slope of the process
              line, and it is the number that has to match between the room and the coil.
            </li>
          </ul>
          <p>
            <strong>Where the two loads come from:</strong> sensible gains are solar, fabric,
            lighting, equipment and the sensible part of occupancy. Latent gains are almost entirely
            people, infiltration of humid outside air, and any wet process — kitchens, pools,
            showers, laboratory washing. A dense meeting room is a latent problem; a south-facing
            glazed office at midday is a sensible one.
          </p>
          <p>
        ...
        >
          <p>
            An economiser is nothing more than a modulating damper set: outside air, return air and
            exhaust, driven together so that the fresh-air proportion varies between a ventilation
            minimum and a hundred per cent. When conditions are right, moving the dampers open
            substitutes free outside air for mechanical cooling, and the coil valve closes down or
            shuts.
          </p>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Dry-bulb changeover:</strong> compare outside dry-bulb with return dry-bulb,
              or with a fixed high limit. Cheap, robust, only needs temperature sensors. Its
              weakness is that it is blind to moisture.
            </li>
            <li>
              <strong>Enthalpy changeover:</strong> compare outside specific enthalpy with return
              specific enthalpy. This is the correct comparison, because enthalpy is total heat —
              sensible and latent together — and total heat is exactly what the coil has to remove.
              It needs a humidity sensor on both airstreams and those sensors must stay calibrated
              to be worth having.
            </li>
            <li>
          ...
          <p>
            <strong>Why enthalpy wins in the UK:</strong> a maritime climate produces long spells of
            cool, damp air. Outside air at 21 °C and high moisture content can be three degrees
            cooler than the return air and still carry more total heat per kilogram. Open the
            dampers on the dry-bulb reading and the coil load goes up, not down — the unit works
            harder to remove the moisture you just invited in. The enthalpy comparison sees it and
            keeps the dampers at minimum.
          </p>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Full free cooling:</strong> outside air is favourable and cold enough to hold
              the supply setpoint on its own. Dampers modulate to hold supply temperature; the coil
              is off.
            </li>
            <li>
              <strong>Integrated free cooling:</strong> outside air is favourable but not cold
              enough alone. Dampers go fully open and the coil trims the remainder. This is the mode
              most often missing from a poorly written sequence.
            </li>
            <li>
          ...
          <p>
            <strong>Never below the ventilation minimum:</strong> whatever the energy logic says,
            the fresh-air rate required for occupants is a floor, not a variable. An economiser
            sequence may open above it freely; it may never close below it while the space is
            occupied.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            capacity of the air roughly doubles for every ten degrees or so of heating, so relative
            humidity collapses.
          </p>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Lift the intake to a safe temperature before it reaches anything that can freeze — a
              wet cooling coil left full of water, a humidifier, a wet filter, a heat recovery
              device.
            </li>
            <li>
              Give the downstream humidifier warm enough air to absorb the moisture it is asked to
              add. Cold air simply cannot take it.
            </li>
            <li>
              Carry frost protection independent of the temperature control loop — a frost
              thermostat that shuts the unit down and drives the valve open regardless of what the
              sequence wants.
            </li>
          </ul>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>Evaporative or adiabatic:</strong> the air supplies the latent heat itself, so
              it cools as it wets. The move follows a line of essentially constant wet-bulb
              temperature, up and to the left. Cheap to run, needs more preheat upstream, and needs
              disciplined water hygiene.
            </li>
          </ul>
          <p>
            <strong>Sizing the moisture:</strong> the humidifier duty is a mass flow of water, not a
            temperature. ṁ_water = ṁ_air × Δg, with Δg in kilograms of moisture per kilogram of dry
            air. Multiply by 3600 to quote it in kg/h, which is how humidifiers are scheduled and
            sold.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            Reading one against the other is the core professional skill this whole section has been
            building toward, and the method does not change from job to job.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>1. Identify the fixed points.</strong> Outside design condition and room or
              return condition. Everything else is derived.
            </li>
            <li>
            ...
            <li>
              <strong>3. Walk the components left to right.</strong> Each one is one chart move of a
              known shape — horizontal, diagonal toward saturation, vertical, or along a wet-bulb
              line.
            </li>
            <li>
            ...
            <li>
              ...
              return state. If it does not close, list the possible reasons before touching anything
              — wrong air volume, wrong fresh-air proportion, a coil not achieving its schedule, or
              a sensor lying to you.
            </li>
          </ul>
          <p>
            ...
            linkage is intact, and whether the sensors are calibrated. All three are chart questions
            answered with a probe, not drawing-office questions.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Note on precision:</strong> chart values below are read to about 0.5 °C and 0.2
            g/kg, which is realistic for hand plotting. Where a figure is a chart reading it is
            quoted as approximate. Small closure errors between the total duty and the sum of
          ...
          <p>
            ...
            which 30% is outside air. Outside is 30 °C at approximately 12 g/kg (about 61 kJ/kg);
            return is 24 °C at approximately 9.3 g/kg (about 48 kJ/kg). Find the mixed state.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>Mass flows: outside 0.30 × 6 = 1.8 kg/s; return 0.70 × 6 = 4.2 kg/s</li>
            <li>
              Temperature: t = (0.30 × 30) + (0.70 × 24) = 9.0 + 16.8 = <strong>25.8 °C</strong>
            </li>
            <li>
              Moisture: g = (0.30 × 12) + (0.70 × 9.3) = 3.6 + 6.51 = <strong>10.1 g/kg</strong>
            </li>
            <li>
              Enthalpy: h = (0.30 × 61) + (0.70 × 48) = 18.3 + 33.6 = <strong>51.9 kJ/kg</strong>{' '}
              (approximately 52 kJ/kg)
            </li>
            <li>
              Chart check: the point 25.8 °C, 10.1 g/kg does read at approximately 52 kJ/kg, so the
              three answers are consistent — always worth the ten seconds it takes.
            </li>
          </ul>
          <p>
            <strong>Example 2: Cooling coil duty and the sensible/latent split.</strong> The mixed
            air from Example 1 (6 kg/s at 25.8 °C, 10.1 g/kg, about 52 kJ/kg) leaves the coil at
            approximately 12.5 °C and 8.5 g/kg, which reads at about 34 kJ/kg.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              Latent: Q̇ₗ = ṁ × Δg × h_fg = 6 × 0.0016 × 2450 = 6 × 3.92 = <strong>23.5 kW</strong>
            </li>
            <li>
              Closure check: 81.4 + 23.5 = 104.9 kW against 108 kW total — about 3% apart, which is
              chart reading tolerance on the two enthalpy values.
            </li>
            <li>
              SHR = Q̇ₛ / Q̇ₜ = 81.4 / 108 ≈ <strong>0.75</strong>, or 0.78 if taken against the
              summed total of 104.9 kW. The spread between those two answers is the chart tolerance
              made visible — roughly three-quarters of the coil duty is sensible and about one
              quarter is drying the air.
            </li>
            <li>
              Note the off-coil state: 8.5 g/kg at 12.5 °C sits very close to the saturation curve,
              around 94% relative humidity. That is a wet coil doing its job.
            </li>
          </ul>
          <p>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              ...
              already paid for. Simultaneous heating and cooling is the single most common energy
              waste in air conditioning, which is why supply air reset, dual-path units and
              desiccant options are all worth examining before accepting a reheat-heavy design.
            </li>
          </ul>
          <p>
            <strong>Example 4: Economiser changeover — the muggy-day trap.</strong> Return air is 24
            °C at 9.3 g/kg (about 48 kJ/kg). Compare two outside conditions on the same 6 kg/s unit.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              <strong>Case B — outside 14 °C at 7 g/kg, about 32 kJ/kg.</strong> Dry-bulb says open
              and enthalpy agrees: 32 &lt; 48. Both comparisons point the same way.
            </li>
            <li>
            ...
            <li>
              <strong>Conclusion:</strong> the two strategies agree in Case B and disagree in Case
              A. The disagreement costs 54 kW every hour the sequence gets it wrong, which is the
              whole business case for enthalpy changeover or a humidity lockout.
            </li>
          </ul>
          <p>
            <strong>Example 5: Winter mixed state and the humidity problem.</strong> The same unit
            runs at 15% fresh air on a winter morning. Outside is −2 °C at approximately 3.3 g/kg;
            return is 22 °C at approximately 7.5 g/kg.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Temperature: t = (0.15 × −2) + (0.85 × 22) = −0.3 + 18.7 = <strong>18.4 °C</strong>
            </li>
            <li>
              Moisture: g = (0.15 × 3.3) + (0.85 × 7.5) = 0.495 + 6.375 = <strong>6.9 g/kg</strong>
            </li>
            <li>
              Now heat that mixed air to a 30 °C supply for a heating-mode unit. The move is
              horizontal, so g stays at 6.9 g/kg while the saturation capacity at 30 °C is roughly
              27 g/kg.
            </li>
            <li>
              Resulting relative humidity ≈ 6.9 / 27 ≈ <strong>25%</strong> — dry enough to generate
              complaints about throats, static and shrinking joinery.
            </li>
            <li>
              Humidification duty to lift the supply from 6.9 g/kg to a 9.0 g/kg target: ṁ_water = 6
              × (0.0090 − 0.0069) = 6 × 0.0021 = 0.0126 kg/s
            </li>
            <li>
          ...
          <p>
            <strong>Example 6: Proving a damper position on site.</strong> A commissioning engineer
            measures 5 °C outside, 22 °C in the return and 15 °C in the mixing box, on a unit whose
            minimum fresh air is set to 20%. What is the actual outside-air fraction?
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>Verify: (0.41 × 5) + (0.59 × 22) = 2.05 + 12.98 = 15.03 °C ✓</li>
            <li>
              <strong>Interpretation:</strong> the unit is drawing roughly twice its minimum fresh
              air. Look for a stuck or disconnected damper linkage, a reversed actuator, a damper
              commanded from the wrong output, or a return damper that is not closing. The heating
              bill is paying for the difference.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>t_mix = (ṁ₁t₁ + ṁ₂t₂) / ṁ_total</strong> — mixed state, same form for g and h
            </li>
            <li>
              <strong>Q̇ₜ = ṁ × Δh</strong> — total duty, the reading to trust
            </li>
            <li>
              <strong>Q̇ₛ = ṁ × cp × Δt</strong> — sensible component, cp ≈ 1.02 kJ/kg·K moist air
            </li>
            <li>
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Does sensible plus latent add back to total within chart reading tolerance? If it is
              out by more than a few per cent, one of the state points is misread.
            </li>
            <li>
              Is the off-coil state below the saturation curve and not on top of it? A point plotted
              above saturation is physically impossible and means an arithmetic slip.
            </li>
            <li>
            ...
            <li>
              Does the supply state, plus the room gains, land back on the return state? If not, the
        ...
        <CommonMistake
          ...
          whatHappens={
            <ul className="space-y-1.5 list-disc pl-5 marker:text-orange-400/70">
              ...
              <li>
                <strong>Sizing a coil on total duty alone:</strong> ignores the SHR, and the space
                ends up warm-and-dry or cool-and-clammy
              </li>
              <li>
                <strong>Dry-bulb economiser changeover on a humid site:</strong> imports latent load
                and increases coil duty
              </li>
              <li>
              ...
              <li>
                <strong>Quoting chart readings to two decimal places:</strong> a hand-plotted point
                is not that precise, and false precision hides real error
              </li>
            </ul>
          }
        ...
        <Scenario
          ...
          situation={
            <>
              ...
              alarming. Return humidity, when you finally plot it, has drifted well above the design
              intent.
            </>
          }
          whatToDo={
            <>
              Measure and plot four states: outside, return, mixed and off-coil. Check whether the
              off-coil state is near the saturation curve. If it is well clear of it, the coil
              surface is no longer below the entering dew point and the coil has gone dry — it is
              delivering sensible cooling only. Check the chilled water flow temperature against
              design, and check whether a valve authority or part-load control issue is letting the
              surface warm up. Compare the delivered SHR with the SHR the room actually needs.
            </>
          }
          whyItMatters={
            <>
              A thermostat cannot see this fault, because temperature is being held. Only the chart
              shows it. Raising chilled water temperature is a popular energy-saving measure and it
              can silently disable humidity control, converting a compliant design into a comfort
              complaint with no alarm anywhere in the system.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              A school AHU is running heating well into mild spring weather, and the site manager is
              querying the gas bill. Outside is 12 °C, return is 21 °C, and the mixing box reads 20
              °C. The BMS graphic shows the economiser commanded to 100% outside air.
            </>
          }
          whatToDo={
            <>
              Reverse the mixing calculation. With outside at 12 °C and return at 21 °C, a true 100%
              outside-air position would put the mixing box at 12 °C, and even minimum fresh air of
              20% would give about 19.2 °C. A reading of 20 °C corresponds to roughly 11% outside
              air — the damper is barely open, whatever the graphic claims. Go and look at the
              linkage, the actuator, and whether the sensor sits in a stratified pocket rather than
              in the mixed stream.
            </>
          }
          whyItMatters={
            <>
              ...
              fails silently. Nothing alarms, nothing overheats, and the only symptom is a bill. The
              mixing calculation turns a three-probe measurement into proof, and proof is what gets
              a defective actuator replaced under warranty.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              Occupants report drips from a ceiling grille and staining on the plasterboard beneath
              a duct run crossing an unconditioned ceiling void. The AHU is a dehumidifying unit
              with off-coil air at about 12 °C, close to saturation. The void is warm and humid.
            </>
          }
          whatToDo={
            <>
              Plot the void air condition and read its dew point. Compare that dew point with the
              surface temperature of the duct, which will sit close to the air temperature inside
              it. If the duct skin is below the void dew point, moisture condenses on the outside of
              the duct — the fault is insulation and vapour control, not the AHU. Check the
              specification of the lagging, its continuity at hangers and flanges, and the integrity
              of the vapour barrier, then check whether the void itself is being ventilated with
              unconditioned humid air.
            </>
          }
          whyItMatters={
            <>
              Off-coil air near saturation is the intended output of a dehumidifying coil, so the
              AHU is behaving correctly. Chasing the plant wastes weeks. The chart identifies in
              minutes that the problem is a surface below a dew point, which reframes it as a fabric
              and insulation defect with a completely different remedy and a completely different
              party responsible.
            </>
          }
        />
