...
const quizQuestions = [
  ...
  },
  {
    id: 2,
    question:
      'A 30 mA Type A RCBO on a kitchen ring final tests at trip time 28 ms at 1 × IΔn. Pass or fail per A4:2026?',
    options: [
  ...
  },
  {
    id: 3,
    question:
      'During functional testing of switchgear interlocks on a TP+N distribution board, what would you check?',
    options: [
  ...
  },
  {
    id: 4,
    question:
      'BS 7671 Reg 415.1.1 — the use of RCDs with a rated residual operating current not exceeding 30 mA is recognised in:',
    options: [
  ...
  },
  {
    id: 6,
    question:
      'You are functional-testing the manual test button on an AFDD (arc fault detection device). The device clicks but the LED status indicator does not change. What action?',
    options: [
  ...
  },
  {
    id: 7,
    question:
      'A 100 mA delay-type S RCD at the incoming side of a sub-mains feed. You instrument-test at 1 × IΔn. What trip time range is acceptable per A4:2026?',
    options: [
  ...
  },
  {
    id: 8,
    question:
      'After functional testing, where do RCD trip times go on the schedule of test results?',
    options: [
...
export default function Sub5() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 6 · Subsection 5"
        title="Test functionality"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Functional verification of switchgear, controls, interlocks, emergency stops and
          protective devices — including the mandatory instrument-based RCD trip-time test per BS
          7671 A4:2026 (≤ 300 ms at 1 × IΔn for general non-delay).
        </p>
        ...
        >
          <p>Reg 643.10 functional testing covers (non-exhaustive list):</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Switchgear and controlgear assemblies</strong> — main switches, isolators,
              distribution boards, and the mechanical interlocks within them. Verify operation under
              load (where safe), correct latching / unlatching, and that mechanical interlocks
              prevent unsafe operations (e.g. opening an enclosure while energised).
            </li>
            <li>
            ...
            <li>
              ...
              correctly per Reg 643.10. This is in addition to the instrument-based RCD test of Reg
              643.7.3 / 643.8.
            </li>
            <li>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              must be functionally tested. The regulation also makes clear that both the test
              facility on protective devices (the user test button) and the regulation-mandated
              instrument tests (Reg 643.7.3, 643.8) must be carried out for RCDs and AFDDs.
            </>
          }
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Identify the RCD characteristics:</strong> rated residual operating current
              (IΔn) and type (general non-delay or delay-type S). For domestic 30 mA Type A or AC
              RCBOs, IΔn = 30 mA, type = general non-delay.
            </li>
            <li>
              <strong>Connect the MFT</strong> across the load side of the RCD — typically by
              plugging into a downstream socket on a circuit protected by the RCD. The instrument
              injects test current via the L and CPC of that socket.
            </li>
            <li>
              <strong>Set the test parameters:</strong> AC test current = 1 × IΔn (e.g. 30 mA for a
              30 mA RCD). Test phase = 0° and 180° (most MFTs do both automatically and report the
              worst case).
            </li>
            <li>
              <strong>Press TEST.</strong> The instrument injects the test current; the RCD trips;
              the meter records the trip time. Reset the RCD before the next test.
            </li>
            <li>
          ...
          <p>
            Most modern MFTs also automatically perform a 0.5 × IΔn pre-test (RCD must NOT trip at
            half rated current) and may report the older 5 × IΔn result for diagnostic continuity.
            These auxiliary tests are good practice but A4:2026 acceptance is based on the 1 × IΔn
            trip time alone.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            'The effectiveness of automatic disconnection of supply by RCDs shall be verified using suitable test equipment according to BS EN 61557-6 (see Regulation 643.1) to confirm that the relevant requirements of Chapter 41 are met, taking into account the operating characteristic of the device. NOTE: Regardless of RCD Type, effectiveness is deemed to have been verified where an RCD disconnects within the time stated below with an alternating current test at rated residual operating current (IΔn): (a) for general non-delay type, 300 ms maximum; (b) for delay "S" type RCD, between 130 ms minimum and 500 ms maximum.'
          }
          meaning={
            <>
              The new A4:2026 acceptance criteria. Single AC test at IΔn. Single trip-time limit per
              RCD category. Old type-specific Table 3A is deleted. Use a calibrated MFT to BS EN
              61557-6, run the test, record the trip time, compare to ≤ 300 ms (non-delay) or
              130-500 ms (delay-type S).
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 643.8 is the additional-protection variant. Same test (AC at IΔn), same acceptance
              (300 ms maximum for general non-delay). Reg 415.1.1 defines the additional-protection
              function of 30 mA RCDs in AC systems; Reg 411.3.3 mandates them on socket-outlets up
              to 32 A (A4:2026 update). The functional verification under Reg 643.8 ties them all
              together: every 30 mA RCD on a circuit must be instrument-tested.
            </>
          }
        ...
        >
          ...
          <p>
            The integral test button bypasses the CT entirely — it connects a small resistor between
            the load-side L and the supply-side N (or similar arrangement) to inject an artificial
            residual current via a path that does NOT exercise the CT. This proves the trigger
            mechanism and the spring work, but says nothing about whether the CT and sensing
            electronics are correctly detecting real residual currents.
          </p>
          <p>
            The instrument test injects a real residual current via the load-side L and CPC of a
            downstream socket — exactly the same path a real fault current would follow. The CT must
            sense the imbalance correctly, the electronics must detect it within the rated time, and
            the trip mechanism must operate. Full system test.
          </p>
          <p>
            Both tests have their place. The instrument test is mandatory at initial verification
            and at every EICR (periodic inspection). The integral test button is exercised by the
            user every six months as part of operational maintenance.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Main isolator</strong> — verify on/off operation, padlocking facility (locks
              in the off position), and any auxiliary contacts that signal the isolator state to a
              control system.
            </li>
            <li>
              <strong>Door interlocks</strong> — open the panel door with the equipment
              de-energised: should be possible. Energise: door must lock or refuse to open without
              operating the isolator first. Some assemblies have key interlocks where a key released
              by the isolator is needed to unlock the door.
            </li>
            <li>
              <strong>Racking interlocks on draw-out circuit-breakers</strong> — circuit-breaker
              must be in the off position before it can be racked from service to test or test to
              disconnected positions. Verify both directions.
            </li>
            <li>
            ...
            <li>
              ...
              E-stop, confirm the equipment de-energises immediately. Reset the E-stop (typically
              twist or pull); confirm the equipment does NOT auto-restart and that manual restart
              via the normal start sequence works correctly.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              supply-side N (or a similar arrangement) — bypassing the current transformer entirely.
            </li>
            <li>
              Pressing the button causes a small artificial current to flow through that bypass
              path, generating an imbalance the trip electronics see (if working) and triggering the
              trip mechanism.
            </li>
            <li>The trip latch releases the contacts; the device opens.</li>
          </ol>
          <p>
            What the button proves: the trip electronics + the trip mechanism work. What it does NOT
            prove:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              The device responds correctly to the full range of fault current waveforms that its
              type rating requires (Type A vs Type AC vs Type B sensing differences).
            </li>
          </ul>
          <p>
            The instrument test (with the MFT injecting a calibrated AC current at 1 × IΔn via the
            load-side L and CPC of a downstream socket) exercises the complete path from real
            residual-current sensing through to trip mechanism operation, and gives you the trip
            time figure that Reg 643.7.3 and 643.8 demand. Both tests serve different purposes; both
            are required.
          </p>
          <p>
            On periodic inspections (EICR), the user is also expected to operate the integral test
            button at least every six months — a quick functional check that the trip mechanism is
            still operational. Many manufacturers recommend monthly testing. Document this
            expectation on the customer handover pack.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>Type A.</strong> Detects sinusoidal AC plus pulsating DC residual currents up
              to 6 mA smooth DC. Suitable for most domestic and commercial circuits including those
              feeding LED drivers, dimmers, washing machines, computer power supplies and similar
              single-phase electronics. The new domestic default — most current-edition CUs are
              populated with Type A RCBOs.
            </li>
            <li>
            ...
            <li>
              <strong>Type B.</strong> Detects sinusoidal AC, pulsating DC AND smooth DC residual
              currents. Required for three-phase converters, certain EV charger installations (Mode
              3 chargers without integral DC fault detection), some PV inverters. More expensive and
              physically larger than Type A. Required by Reg 722.531.3.101 (electric vehicles) where
              the charger doesn't include its own Type A + 6 mA DC fault detection.
            </li>
          </ul>
          <p>
            <strong>Functional testing implications:</strong> instrument-based RCD test must use a
            test waveform appropriate for the RCD type. Modern MFTs auto-detect the type or let you
            select it; the trip-time test at 1 × IΔn is the same acceptance regardless of type (≤
            300 ms general non-delay). But specifying the wrong type for the load is a design defect
            that no amount of testing will fix — the device physically cannot detect the fault
            waveforms the load can produce.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              circuit. Functional testing: plug MFT into a downstream socket, run the 1 × IΔn test,
              only that one circuit trips. Reset the RCBO. Move to the next circuit. The rest of the
              installation stays live. Fast, diagnostic, customer-friendly because their freezer
              doesn't defrost during testing.
            </li>
            <li>
              <strong>Older split-load CU with group RCDs.</strong> Two main RCDs cover groups of
              circuits. Functional testing: the test trips the whole group RCD, taking down 4-6
              circuits at once. Slower, less diagnostic (you can tell the RCD works but not which
              circuit caused a real-world trip), and disruptive (large portions of the install go
              dark each time you test). Many older CUs have only one RCD covering the whole
              installation — testing it kills everything.
            </li>
            <li>
              <strong>Hybrid arrangements.</strong> Some commercial / industrial installations have
              a mix — 100 mA Type S delay RCD at the incoming side for selectivity, 30 mA Type A
              RCBOs per circuit downstream. Testing has two phases: per-circuit RCBO testing first;
              then the upstream delay RCD test (will not trip during downstream tests due to the
              time delay).
            </li>
          </ul>
          <p>
            <strong>Isolation impact during testing:</strong> on an all-RCBO board, testing is one
            circuit at a time and the rest of the board stays live. Plan accordingly — warn the
            customer if their freezer is on the test target circuit. On a group-RCD board, the whole
            group goes down — schedule testing when the customer can tolerate the disruption.
            Document the testing approach on the STR.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Main isolator.</strong> Operate on/off cycle. Verify padlock facility works in
              the OFF position. Verify any auxiliary contacts (signalling the isolator state to a
              control system) operate correctly with isolator state changes.
            </li>
            <li>
              <strong>Door interlocks.</strong> With the panel de-energised, the door should open
              freely. With the panel energised, the door should be locked or refuse to open without
              operating the isolator first. Some assemblies use key interlocks — a key released by
              the isolator is needed to unlock the door, ensuring the door cannot be opened while
              live.
            </li>
            <li>
              <strong>Racking interlocks on draw-out circuit-breakers.</strong> The breaker must be
              in the OFF position before it can be racked from service to test or test to
              disconnected positions. Verify both directions. The interlock should physically
              prevent racking with the breaker closed.
            </li>
            <li>
              <strong>Contactor and relay control circuits.</strong> Start/stop/latching
              arrangements: verify the latch engages on start, drops out on stop, that interlocks
              (forward-reverse mutual exclusion etc.) work correctly. Time delays: verify
              soft-start, dahlander, star-delta sequences run as designed.
            </li>
            <li>
              <strong>Emergency stops and emergency switching.</strong> The big red mushroom
              buttons. Test under load: energise the protected equipment, operate the E-stop,
              confirm the equipment de-energises immediately and any downstream contactors drop out.
              Reset the E-stop (twist or pull); confirm equipment does NOT auto-restart and that
              manual restart via the normal start sequence works.
            </li>
            <li>
              <strong>Lift safety circuits.</strong> Door interlocks, floor sensors, over-travel
              limits, emergency lighting on the car. Test each in turn per the lift design
              specification.
            </li>
            <li>
              <strong>Insulation monitoring devices on IT systems</strong> (operating theatres,
              certain laboratories). Simulate an earth fault by connecting a known resistor between
              live and earth on the IT system; confirm the IMD detects and signals the fault
              correctly within its rated time.
            </li>
          </ul>
          <p>
            On a domestic install the bulk of functional testing is RCD trip-time verification. On a
            commercial / industrial install you build a comprehensive functional test plan from the
            design documents — every switching function, interlock and protective device gets
            exercised and the result documented.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You energise a domestic CU, press the test buttons on each RCBO and confirm each one
              trips when the button is pressed. You sign off the EIC with "RCD test: pass". A week
              later the kitchen ring final develops a leakage fault — wet appliance, water tracking
              from L to E — and the RCBO fails to trip in time because the residual-current sensing
              electronics had a latent fault that the button test bypassed. The customer reports a
              shock and you are looking at a serious incident investigation.
            </>
          }
          doInstead={
            <>
              Always perform the instrument-based RCD test at 1 × IΔn for every RCD/RCBO at initial
              verification and at every EICR. Record the actual trip time on the STR. The test
              button is part of functional testing per Reg 643.10 (verify that test facility) but it
              is NOT the test that satisfies Reg 643.7.3 or 643.8 for RCD effectiveness. Both tests
              required, both recorded. Modern MFTs make the instrument test fast — about 30 seconds
              per RCD including the auto-recorded ramp test.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You have just commissioned a kitchen ring final on a domestic CU swap-out. The circuit
              is protected by a 32 A Type B Type A RCBO with IΔn = 30 mA. All dead tests have
              passed. The CU is energised. You need to confirm the RCD function and document for the
              EIC.
            </>
          }
          whatToDo={
            <>
              ...
              Press TEST. The RCBO trips; meter records trip time — for a healthy 30 mA RCBO, expect
              20-50 ms.
              <br />
              <br />
              Reset the RCBO. Verify the integral test button operates (functional test per Reg
              643.10). Re-test on the other test polarity (0° and 180°) — most MFTs do this
              automatically and report worst-case trip time.
              <br />
              <br />
              Document: trip time at 1 × IΔn = (e.g.) 28 ms, well below 300 ms maximum → pass. Test
              button function verified. Record on STR in the RCD trip-time column. Repeat for every
              RCD/RCBO in the CU. On a 12-circuit board with all RCBOs you will have 12 trip-time
              entries.
            </>
          }
          whyItMatters={
            <>
              The 28 ms trip time gives over an order of magnitude of headroom compared to the 300
              ms regulatory limit. If a circuit shows a trip time of, say, 250 ms — passing Reg
              643.7.3 but at the edge — that is worth investigating. Most likely a slightly degraded
              device that will either drift over time (eventually exceeding 300 ms) or that has a
              latent fault. Replace the device rather than sign off marginal performance — the cost
              of a replacement RCBO is trivial compared to the consequences of slow disconnection
              during a real fault.
            </>
          }
        />
