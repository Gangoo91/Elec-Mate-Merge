...
const quizQuestions = [
  {
    id: 1,
    question:
      'STR header section — which of these belongs in the header (top of the form, common to every per-circuit row)?',
    options: [
  ...
  },
  {
    id: 4,
    question:
      'A 30 mA Type A RCBO on a kitchen ring tested at 28 ms trip time at 1 × IΔn. What goes in the RCD trip-time column?',
    options: [
  ...
  },
  {
    id: 6,
    question:
      'Modern MFTs export test data via Bluetooth or USB. On a digital STR, the auto-imported data should:',
    options: [
...
const faqs = [
  ...
  {
    question:
      'What if a column does not apply to a particular circuit (e.g. RCD trip time on a circuit without an RCD)?',
    answer:
  ...
  {
    question:
      "I've completed the STR and noticed a single typo in a Zs reading after I've already signed. Can I fix it?",
    answer:
...
export default function Sub8() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 6 · Subsection 8"
        title="Completing the Schedule of Test Results"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Column-by-column walk through every field on the IET model STR — header, identification,
          wiring, protective device, test readings — with a fully worked example for a Type B 32 A
          RCBO ring final.
        </p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              Format varies by certification software (e.g. NICEIC uses a numeric reference; NAPIT
              uses a different format). Critical for the audit trail.
            </li>
            <li>
              <strong>Address of installation</strong> — full postal address.
            </li>
            <li>
              <strong>Date of testing</strong> — single date, or date range if testing spanned
              multiple visits (with notes per row indicating which visit).
            </li>
            <li>
              <strong>Person responsible for testing</strong> — name and qualification (e.g. "John
              Smith, 18th Edition I&T qualified, NICEIC reg #...").
            </li>
            <li>
              <strong>Instrument serial numbers</strong> — MFT model and serial, plus the
              calibration date / next-cal-due date. For traceability of any disputed reading back to
              the calibrated instrument.
            </li>
            <li>
              <strong>Notes / limitations</strong> — anything common to all circuits that the form
              should record (e.g. "Tested 2026-04-26. Limitation: Circuit C7 not fully tested at
              first visit due to access; completed 2026-04-28").
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <div className="hidden sm:block bg-[hsl(0_0%_10%)] border border-white/[0.08] rounded-xl p-4 text-[13px]">
            <div className="grid grid-cols-3 gap-3 text-white/90">
              ...
              <div>1. Circuit number</div>
              <div>Sequential identifier</div>
              <div>5</div>
              <div>2. Description</div>
              <div>Plain-English purpose</div>
              <div>Kitchen ring final</div>
              <div>3. Wiring type</div>
              <div>Cable type</div>
              <div>T&E</div>
              <div>4. Reference method</div>
              <div>BS 7671 Appendix 4 code</div>
              <div>100 (clipped direct)</div>
              <div>5. CSA L (mm²)</div>
              <div>Line conductor cross-section</div>
              <div>2.5</div>
              <div>6. CSA CPC (mm²)</div>
              <div>CPC cross-section</div>
              <div>1.5</div>
              <div>7. OCPD type / rating</div>
              <div>Protective device</div>
              <div>RCBO B32 BS EN 61009</div>
              <div>8. Max Zs (Ω)</div>
              <div>From A4:2026 Table 41.3</div>
              <div>1.37</div>
              <div>9. IΔn (mA)</div>
              <div>RCD residual current rating</div>
              <div>30</div>
              <div>10. RCD type</div>
              <div>AC, A, F, or B</div>
              <div>A</div>
              <div>11. r1 (Ω)</div>
              <div>Ring end-to-end L (rings only)</div>
              <div>0.32</div>
              <div>12. rn (Ω)</div>
              <div>Ring end-to-end N (rings only)</div>
              <div>0.34</div>
              <div>13. r2 (Ω)</div>
              <div>Ring end-to-end CPC (rings only)</div>
              <div>0.55</div>
              <div>14. R1+R2 (Ω)</div>
              <div>From radial test or ring cross-connection</div>
              <div>0.22</div>
              <div>15. IR L-N (MΩ)</div>
              <div>Live to live</div>
              <div>&gt;999</div>
              <div>16. IR L-E (MΩ)</div>
              <div>Live to earth</div>
              <div>&gt;999</div>
              <div>17. IR N-E (MΩ)</div>
              <div>Neutral to earth</div>
              <div>&gt;999</div>
              <div>18. Polarity</div>
              <div>Tick or P/F</div>
              <div>P</div>
              <div>19. Zs (calc) (Ω)</div>
              <div>Ze + R1+R2</div>
              <div>0.57</div>
              <div>20. Zs (measured) (Ω)</div>
              <div>Live test at far end</div>
              <div>0.59</div>
              <div>21. RCD trip time (ms)</div>
              <div>1 × IΔn AC test</div>
              <div>28</div>
              <div>22. AFDD test</div>
              <div>Tick (where AFDD fitted)</div>
              <div>N/A</div>
              <div>23. Notes</div>
              <div>Limitations, anomalies</div>
              <div>Spur to washing-machine outlet documented</div>
            </div>
          </div>
          ...
          <div className="sm:hidden space-y-2">
            {[
              ...
              {
                col: '23. Notes',
                what: 'Limitations, anomalies',
                ex: 'Spur to washing-machine outlet documented',
              },
            ].map((row, i) => (
              <div
                key={i}
              ...
              >
                ...
                <div className="text-white/60 text-[11px] uppercase tracking-wide mt-1.5">
                  Example
                </div>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              supplementary. Every required field must be completed. Defects found during testing
              must be corrected before the certificate is issued (so the STR cannot show a fail and
              the EIC also be issued). Cross-reference the STR to the EIC by certificate number;
        ...
        >
          <p>
            Scenario: Circuit 5, kitchen ring final, T&E 2.5/1.5 mm² clipped direct, ~28 m loop, 8
            sockets. 32 A Type B RCBO Type A 30 mA. TN-C-S supply, measured Ze = 0.32 Ω.
          </p>
          ...
          <div className="hidden sm:block bg-[hsl(0_0%_10%)] border border-white/[0.08] rounded-xl p-4 text-[13px]">
            <div className="grid grid-cols-2 gap-3 text-white/90">
              ...
              <div>Circuit number</div>
              <div>5</div>
              <div>Description</div>
              <div>Kitchen ring final</div>
              <div>Wiring type</div>
              <div>T&E (flat twin and earth)</div>
              ...
              <div>CSA CPC</div>
              <div>1.5 mm²</div>
              <div>OCPD type / rating</div>
              <div>RCBO B32 to BS EN 61009</div>
              <div>Max Zs (Ω) [A4:2026 Table 41.3]</div>
              <div>1.37 Ω</div>
              <div>IΔn</div>
              <div>30 mA</div>
              <div>RCD type</div>
              ...
              <div>r2</div>
              <div>0.55 Ω</div>
              <div>R1+R2 (highest cross-connection L-CPC)</div>
              <div>0.22 Ω</div>
              <div>IR L-N</div>
              <div>&gt;999 MΩ</div>
              <div>IR L-E</div>
              ...
              <div>Polarity</div>
              <div>P</div>
              <div>Zs (calc) [Ze + R1+R2]</div>
              <div>0.32 + 0.22 = 0.54 Ω</div>
              <div>Zs (measured) [live test at far socket]</div>
              <div>0.57 Ω</div>
              <div>RCD trip time at 1 × IΔn</div>
              <div>28 ms</div>
              <div>AFDD test</div>
              <div>N/A — no AFDD on this circuit</div>
              <div>Notes</div>
              <div>
                Spur to washing-machine outlet (3 m of 2.5 mm² T&E) added 2026-04-26; documented for
                future inspection
              </div>
            </div>
          </div>
          ...
          <div className="sm:hidden space-y-1.5">
            {[
              ...
              <div
                key={i}
                className="bg-[hsl(0_0%_10%)] border border-white/[0.08] rounded-xl p-3 text-[13px] flex justify-between gap-3"
              >
          ...
          <p>
            Pass / fail check on the row: Zs (calc and measured) both well below corrected Table
            41.3 limit of 1.10 Ω for B32. IR all above range. Polarity verified. RCD trip time 28 ms
            well below 300 ms. AFDD N/A. Spur documented. Row complete and defensible.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <div className="hidden sm:block bg-[hsl(0_0%_10%)] border border-white/[0.08] rounded-xl p-4 text-[13px]">
            <div className="grid grid-cols-2 gap-3 text-white/90">
              ...
              <div>
                Use A4:2026 Table 41.3 (B32 = 1.37); update certification software to A4:2026 form
                templates
              </div>

              <div>Mixing units (mΩ vs Ω in same column)</div>
              <div>
                Use column header units consistently; for ad-hoc rows include units explicitly
              </div>
              ...
              <div>
                Use 100 for clipped direct; 101 for above plasterboard; 102 for thermal-insulated
                void
              </div>

              <div>r2 not in expected ratio to r1 on a ring final but no investigation</div>
              <div>
                If r2 not ≈ r1 × cable size ratio, investigate before recording (mixed cable sizes?
                hidden joint?)
              </div>
            </div>
          </div>
          ...
          <div className="sm:hidden space-y-2">
            {[
              {
                e: 'Blank IΔn on an RCBO-protected circuit',
                f: 'Always fill: 30 mA (general) or 100 mA (sub-mains)',
              },
              {
                e: 'Quoting max Zs from older A2 table (B32 = 1.44)',
                f: 'Use A4:2026 Table 41.3 (B32 = 1.37)',
              },
              {
                e: 'Mixing units (mΩ vs Ω in same column)',
                f: 'Column header units consistent; include explicit units when needed',
              },
              {
                e: 'Polarity recorded as "yes" instead of P or tick',
                f: "Use the form's standard symbol (P/F or tick/cross)",
              },
              {
                e: 'RCD result "Pass" not the actual ms',
                f: 'Always record measured value (e.g. 28 ms)',
              },
              {
                e: "Notes blank when there's a spur or limitation",
                f: 'Document spurs, test limitations, equipment disconnected',
              },
              {
                e: 'Wrong reference method (e.g. "A" for clipped direct)',
                f: 'Use 100 / 101 / 102 codes correctly',
              },
              {
                e: 'r2 not in expected ratio to r1 on a ring with no investigation',
                f: 'Investigate (mixed sizes? hidden joint?)',
              },
            ].map((row, i) => (
              <div
                key={i}
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              contractor details, work description, certificate reference number. This data appears
              on every form (EIC, Schedule of Inspections, STR) automatically.
            </li>
            <li>
              <strong>Supply characteristics.</strong> Entered on the EIC (Ze, PFC, system type,
              voltage, frequency, number of phases) and cascades to the STR for use in Zs
              calculations and acceptance limits.
            </li>
            <li>
              <strong>Instrument data.</strong> Meter model, serial, calibration date entered once
              and appears on the STR header for traceability.
            </li>
            <li>
              <strong>Person responsible.</strong> Inspector name and qualification entered once and
              appears on every form's signature panel.
            </li>
            <li>
              <strong>Per-circuit rows.</strong> Added to the STR; each row pulls supply
              characteristics from the project header for Zs calc and max Zs lookup from the A4:2026
              Table 41.3 reference table.
            </li>
            <li>
              <strong>Auto-import from instrument.</strong> Test data flows from the meter into the
              STR per-circuit rows; inspector reviews and adds visual check items to the Schedule of
              Inspections.
            </li>
          </ol>
          <p>
            <strong>Manual cross-check before signing:</strong> even with full cascade, spot-check
            that the EIC supply data matches what's on the STR (Ze used in calculations should equal
            Ze documented in the EIC). Mismatches happen when the project header is updated mid-job
            after STR rows have been calculated.
          </p>
          <p>
            <strong>Avoiding duplicate data entry errors:</strong> the cascade approach eliminates
            the most common manual-entry error (typing the address slightly differently on each
            form, or transposing the certificate reference number). On a paper STR, write the
            certificate reference number once at the top and refer to it; for digital, the cascade
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Decimal-point errors.</strong> Meter shows 0.45 Ω; transcribed as 4.5 Ω. The
              10× error is invisible if you don't sanity-check against expected values. R1+R2 of 4.5
              Ω on a 25 m kitchen ring is impossible (would be ~30× the expected value); R1+R2 of
              0.45 Ω is reasonable. Always ask: does this number make sense for this circuit?
            </li>
            <li>
              <strong>Unit mix-ups (mΩ vs Ω).</strong> Meter has been switched to the mΩ range
              during continuity testing; transcribed without unit conversion. 450 mΩ gets written as
              450 Ω instead of 0.45 Ω. Same kind of order-of-magnitude error. Some meters auto-range
              and the unit displays clearly; others don't, so check the unit on every reading.
            </li>
            <li>
              <strong>Reading errors (transposed digits).</strong> Meter shows 0.32 Ω; transcribed
              as 0.23 Ω. Hard to catch unless the value falls out of expected range. Cross-check by
              asking: is r1 ≈ rn for this ring? If r1 = 0.32 Ω and rn = 0.34 Ω that's plausible; r1
              = 0.23 Ω with rn = 0.34 Ω suggests one is wrong.
            </li>
            <li>
              <strong>Wrong column / wrong circuit.</strong> Reading taken at circuit 5 gets
              transcribed onto circuit 6's row. Spot by noting the description column — if "Kitchen
              ring final" has a value that fits "Lighting downstairs" better, investigate.
              Sequential test order helps reduce this error.
            </li>
            <li>
              <strong>Stale data on auto-import.</strong> Meter wasn't cleared between jobs; an old
              reading from yesterday's site appears in today's STR. Always clear the meter (or use a
              date-stamped capture mode) at the start of each job.
            </li>
            <li>
              <strong>Missing IΔn on RCBO rows.</strong> The IΔn column gets left blank because the
              meter doesn't auto-record it (you typed it in for the test setup but the result import
              doesn't carry it through). Fill manually before signing.
            </li>
          </ul>
          <p>
            <strong>Practical sanity-check ranges to memorise:</strong> R1+R2 on 2.5/1.5 T&E ring
            final ≈ 0.18-0.30 Ω for typical domestic length; r1 ≈ rn (within 10%); r2 ≈ r1 ×
            1.6-1.7. Lighting circuit R1+R2 on 1.5/1.0 T&E ≈ 0.5-1.5 Ω. IR on healthy new wiring
            &gt; 100 MΩ (usually &gt; 999). RCD trip time on healthy 30 mA device 20-50 ms. If a
            reading falls way outside these ranges, investigate before recording.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Spurs documented.</strong> "Single unfused spur to washing machine outlet (3 m
              of 2.5 mm² T&E) tapped from socket adjacent to sink; documented per BS 7671 ring spur
              rules."
            </li>
            <li>
              <strong>Equipment disconnected for IR test.</strong> "SPD module lifted from base for
              500 V DC test; reconnected and 250 V DC follow-up per Reg 643.3.3 = 35 MΩ at incoming
              side."
            </li>
            <li>
              <strong>Test deferred or partial.</strong> "Limitation: circuit C7 IR test not
              performed at first visit; equipment could not be disconnected. Scheduled for follow-up
              visit 2026-05-10."
            </li>
            <li>
              <strong>A4:2026 specific notes.</strong> "Type B 32 A RCBO confirmed Type A 30 mA
              (rather than older Type AC); Reg 531.3.3 type selection compliance verified."
            </li>
            <li>
              <strong>Departure from BS 7671.</strong> "Socket-outlet RCD omitted on circuit C5
              (warehouse roller-door power) per documented risk assessment under Reg 411.3.3
              exception; risk assessment dated 2026-04-20 held by client."
            </li>
            <li>
              <strong>Cable run details where unusual.</strong> "Outdoor cable section (10 m)
              direct-buried at 600 mm depth per detail drawing 03; route inspected during install."
            </li>
            <li>
              <strong>Customer requests / comments.</strong> "Customer requested early periodic
              inspection at 5 years rather than 10 (planning for property sale)."
            </li>
            <li>
              <strong>Test result anomalies investigated.</strong> "C4 initial IR reading 480 MΩ at
              500 V DC; investigated and traced to pinched cable at upstairs landing back-box; cable
              re-routed and protected; retest &gt; 999 MΩ."
            </li>
          </ul>
          <p>
            The principle: anything that explains, qualifies, or contextualises the standard-column
            data goes in Notes. Future inspectors reading the STR five years from now should be able
            to understand the full picture without needing to call you for clarification.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>EIC scope: every circuit, every test.</strong> On a new installation or major
              alteration the STR captures every applicable test on every circuit. Continuity, IR,
              polarity, Zs (calc and measured), RCD trip time per circuit. Any defect must be
              corrected before the EIC is issued (Reg 644.1.1).
            </li>
            <li>
              <strong>EICR scope: sample-based, with judgement.</strong> On a periodic inspection
              the inspector samples circuits according to IET GN3 guidance and their professional
              judgement. Higher-risk installations (commercial kitchens, special locations, anywhere
              DIY may have happened) get tested more thoroughly. The STR captures what was tested;
              what wasn't tested is documented in the EICR scope statement.
            </li>
            <li>
              <strong>EIC defect handling: must be corrected before sign-off.</strong> Reg 644.1.1
              explicit. You cannot issue a clean EIC for an installation with known defects. Find,
              fix, retest, certify.
            </li>
            <li>
              <strong>
                EICR defect handling: classified, not always required to be corrected.
              </strong>{' '}
              Defects on an EICR are classified C1 (danger present — immediate action required), C2
              (potentially dangerous — action required), C3 (improvement recommended — not dangerous
              but improves safety), or FI (further investigation required). The customer decides
              whether to action the classifications, with C1 being mandatory immediate action.
            </li>
            <li>
              <strong>EICR comparison to historical data.</strong> A useful EICR practice is to
              record current readings alongside historical readings from the original EIC STR. Drift
              in IR or Zs over the periodic interval can indicate slow degradation that warrants
              attention even if the current reading still passes.
            </li>
            <li>
              <strong>EICR pass/fail vs satisfactory/unsatisfactory.</strong> EICR doesn't pass or
              fail — it's "satisfactory" (no C1 or C2 defects) or "unsatisfactory" (one or more C1
              or C2 defects requiring action). Customer must understand the difference between an
              unsatisfactory EICR (which has dangerous or potentially dangerous items) and a
              satisfactory EICR with C3 recommendations (which is safe but has improvement
              opportunities).
            </li>
          </ul>
          <p>
            <strong>Practical workflow:</strong> on a new install your STR is completed
            circuit-by-circuit during commissioning, all clean readings, EIC issued with full pack.
            On an EICR your STR is completed during the inspection visit with sample circuits
            tested, observations classified per IET GN3 categories, EICR issued with the
            satisfactory/unsatisfactory verdict and recommendations.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              obsolete standard. At a future EICR, the new inspector quotes the current figures and
              finds your circuits non-compliant retrospectively, raising a query about whether the
              original certification was correct.
            </>
          }
          doInstead={
            <>
              Update certification software to A4:2026 templates; update any printed STR templates
              to the latest IET model. Always quote the current edition Table 41.3 value, not
              whichever was the active version when the OCPD was first installed. A4:2026 changes
              from A2: B6 unchanged 7.28; B10 unchanged 4.37; B16 2.87→2.73; B20 2.30→2.19; B32
              1.44→1.37; B40 1.15→1.09. Memorise or carry a printed table until the values are
              second nature.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You are completing the STR for circuit 8 of an 8-circuit domestic CU swap-out: an EV
              charger sub-circuit. C40 RCBO Type A 30 mA, 10/4 SWA, 15 m radial, T&E Reference
              Method 102 (in cable tray) for the indoor section, direct-buried for the outdoor
              section. Charger isolated at port for IR test. Dead-test results: R1+R2 = 0.06 Ω; IR
              L-N / L-E / N-E all &gt;999 MΩ at 500 V DC; IR follow-up at 250 V DC (after charger
              reconnection) = 12 MΩ; polarity P. Live: Zs measured at charger port = 0.39 Ω; RCD
              trip time 31 ms at 1 × IΔn.
            </>
          }
          whatToDo={
            <>
              ...
              <br />
              <br />
              Circuit 8 / EV charger sub-circuit / SWA / Reference Method varies (102 indoor, direct
              buried outdoor — note in column or add limitation in notes) / CSA L = 10 mm² / CSA CPC
              = 4 mm² / OCPD = RCBO C40 Type A BS EN 61009 / Max Zs (A4:2026 Table 41.3 Type C 40 A
              at 230 V) = 0.55 Ω / IΔn = 30 mA / RCD type = A / r1, rn, r2 = N/A (radial, not ring)
              / R1+R2 = 0.06 Ω / IR L-N = &gt;999 MΩ / IR L-E = &gt;999 MΩ / IR N-E = &gt;999 MΩ /
              Polarity = P / Zs calc (Ze=0.32 + 0.06) = 0.38 Ω / Zs measured = 0.39 Ω / RCD trip 31
              ms / AFDD = N/A / Notes = "EV charger isolated at port for 500 V DC IR test per Reg
              643.3.3; reconnected and 250 V DC follow-up = 12 MΩ. Outdoor cable section
              direct-buried per detail drawing 03."
              <br />
              <br />
              Pass check: Zs (0.39) below corrected Type C 40 A limit (0.55 × 0.8 = 0.44 Ω); IR all
              above range; RCD trip 31 ms well below 300 ms; polarity verified; A4:2026 follow-up
              test recorded with reading. Row complete.
            </>
          }
          whyItMatters={
            <>
