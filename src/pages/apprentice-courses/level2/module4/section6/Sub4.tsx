...
const checks = [
  {
    ...
    options: [
      ...
      'Top centre pin / terminal as you face the socket.',
      'Either outer pin — line and neutral are interchangeable on a BS 1363 socket.',
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    question:
      'A plug-in socket tester (the three-LED kind) shows "OK" at every socket on a circuit. Polarity proven?',
    options: [
      'Yes — three green LEDs is the definitive proof of correct polarity; it directly satisfies Reg 643.6 and no further test is needed before sign-off.',
      'Yes, provided you test every socket on the circuit — a full set of "OK" results across all outlets confirms polarity to the standard required for certification.',
      'Only on a TN-S supply — socket testers prove polarity reliably on TN-S but not on TN-C-S, where a continuity test is needed instead.',
  ...
  },
  {
    ...
    options: [
      'Open (OL) in the L2 position, closed in the L1 position — test both to confirm switching is in the line.',
      'Closed (low resistance) in both toggle positions, because COM is permanently linked to both L1 and L2 inside a 2-way switch.',
      'Open (OL) in both toggle positions, because the lamp is OFF so no path exists through the switch at all.',
      'A steady reading of around 0.3 Ω regardless of toggle position, matching the strap conductor resistance between the two switches.',
    ],
    correctIndex: 0,
    explanation:
...
const quizQuestions = [
  {
    id: 1,
    question:
      'BS 7671 A4:2026 Reg 643.6 lists three things that must be verified during the polarity test. Which set?',
    options: [
      '(a) Insulation resistance is above 1 MΩ; (b) continuity of the CPC is below the table limit; (c) the RCD trips within the required time. The three checks that prove the circuit is safe.',
      ...
      '(a) Line and neutral are not swapped at the socket; (b) the earth pin is the longest of the three; (c) the supply voltage is within 230 V +10%/−6%. The three checks the polarity test confirms.',
      '(a) Every protective device is double-pole; (b) the neutral is bonded to earth at the board; (c) every accessory is fed in 2.5 mm² cable. The three things the polarity test verifies.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Perfectly acceptable — on AC the switch interrupts the circuit wherever it sits, so a neutral switch isolates the load just as effectively as a line switch.',
      'Acceptable only on a TT system, where the neutral is not at earth potential, so switching it does isolate the load safely.',
      ...
      'Preferred for two-way lighting because switching the neutral prevents the line conductor from being interrupted, which reduces arcing at the switch contacts.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    id: 3,
    question:
      'You are testing polarity at a 13 A unswitched FCU feeding an immersion heater. With the FCU isolated and the cable disconnected from the consumer unit at the line, what continuity reading would prove correct line polarity into the FCU?',
    options: [
      'Low-resistance continuity between the line of the incoming cable and the N terminal of the FCU; OL between the line of the incoming cable and the L terminal of the FCU.',
      'Low-resistance continuity between the line of the incoming cable and the earth terminal of the FCU; OL to both L and N terminals.',
      'OL between the incoming line and every FCU terminal — a correctly wired FCU isolates all terminals until switched on, so no continuity should be read.',
  ...
  },
  {
    id: 4,
    question:
      'After a successful dead polarity test, polarity must also be confirmed live as part of the live test sequence. Why both?',
    options: [
      'The dead test proves the wiring; the live test proves the assumed CU line is the actual supply line.',
      'The dead test only checks the line conductor; the live test is needed to confirm the neutral and earth are also correctly connected, which a continuity test cannot show.',
      'The dead test can be fooled by the test instrument battery polarity, so the live test repeats the same checks with mains voltage to rule out a meter error.',
      'The live test measures the actual supply voltage at each accessory, and polarity is only considered correct if every socket reads within 230 V +10%/−6%.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    id: 5,
    question:
      'A bayonet (BC) lampholder per BS 7671 Reg 643.6(b) — which contact must be connected to the neutral?',
    options: [
      'The centre contact (the spring-loaded pin the bulb base presses onto). Outer cup = switched line. This keeps the neutral on the contact the bulb touches first when fitted.',
      ...
      'Either contact — bayonet lampholders are symmetrical, so neutral can go to whichever contact is most convenient to wire.',
      'The earth contact of the lampholder — neutral and earth share the outer shell on a BC fitting, so both connect to the metal cup.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    id: 6,
    question:
      "You discover during polarity testing that a junction box in the loft has the line and neutral swapped on one outgoing tail. The CU end is correct. What's the safe approach?",
    options: [
      'Reverse the same two conductors at the consumer unit so the swap cancels out — this restores correct polarity at the accessory without disturbing the loft junction box.',
      'Leave it as found and note it on the certificate — a single swapped tail is acceptable provided the rest of the installation is correctly polarised.',
      ...
      'Swap the line and neutral at the accessory served by that tail, so the two reversals downstream of the junction box cancel and the accessory reads correct.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    id: 7,
    question:
      'Polarity testing on a circuit with both a 2-way switching arrangement and an intermediate switch in the middle. How do you test the polarity of the strap conductors?',
    options: [
      'Energise the circuit and use a voltage indicator on the straps — the straps should read 230 V when the lamp is on, which proves they are correctly polarised.',
      'Insulation-test each strap conductor to earth at 500 V DC — a reading above 1 MΩ confirms the straps are correctly wired into the line side of the switching.',
      'There is no need to test the straps — only the COM and switched-line terminals carry polarity significance, so the strap conductors are excluded from the polarity test.',
      'Continuity-test from CU line to the lamp through the switching, toggling each switch and checking the meter responds.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A dedicated polarity column on the per-circuit row of the STR, usually a tick or a Pass/Fail.',
      'In the insulation-resistance column — polarity and IR share a single combined field on the standard STR because both are dead tests done at the same stage.',
      'It is not recorded on the STR at all — polarity is a visual inspection item only, so it goes on the Schedule of Inspections and never on the test results sheet.',
      'In the comments box at the foot of the EIC as free text, because there is no fixed field for polarity on the model schedule of test results.',
    ],
    correctAnswer: 0,
    explanation:
...
const faqs = [
  {
    question:
      "Why is polarity even a thing on a 230 V single-phase circuit — both ends of the load are connected, surely it doesn't matter which way the current flows?",
    answer:
  ...
  {
    question:
      'What if the customer has fitted their own light fitting after the test and reversed the polarity?',
    answer:
  ...
  {
    question:
      'My continuity meter shows a reading on what should be an "OL" path during polarity testing — what could cause that?',
    answer:
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Verify line is on the correct terminal at every accessory, that single-pole switches and
          fuses are in the line conductor, and that lampholder centre contacts are line, outer
          contacts are neutral. Dead test by continuity from CU outwards.
        </p>
        ...
        >
          <p>
            ...
            conductor (not the neutral) so that when they open, the load is properly isolated from
            the supply.
          </p>
          <p>On a TN system at 230 V single-phase:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Line conductor</strong> — at 230 V to earth in normal operation. This is the
              dangerous one.
            </li>
            <li>
          ...
          <p>
            ...
            broken, no current can flow, the load (and any exposed metalwork on the load side) sits
            at neutral / earth potential. Reverse the polarity and that safety guarantee vanishes —
            the off switch is now in the neutral, the line is still connected, and touching what
            looks like a dead terminal completes a circuit through your body to earth.
          </p>
          <p>
            The polarity test is the dead test that proves the switching device is in the correct
            conductor. It is also the test that proves line, neutral and CPC are correctly
            identified at every accessory throughout the installation.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Three explicit verifications: (a) all single-pole devices in the line; (b) BC and ES
              lampholders correctly orientated (outer = neutral, centre = line); (c) overall
              correctness of conductor identification through the installation. The polarity test
              must address all three. The opening sentence — verify supply polarity before
              energising — is also notable: a swapped supply at the meter tails would corrupt every
              downstream test, so supply polarity comes before installation polarity.
            </>
          }
        ...
        >
          ...
          <div className="sm:hidden space-y-2">
            {[
              {
                acc: 'BS 1363 13 A socket-outlet (face on)',
                line: 'Right-hand pin / terminal (L)',
              },
              ...
              {
                acc: '1-way single-pole switch',
                line: 'L incoming → COM. Switched line → L1 / out',
              },
              { acc: '2-way switch', line: 'L incoming → COM. Strap conductors → L1 and L2' },
              {
                acc: 'Intermediate switch',
                line: 'Two pairs of strap terminals; line never appears here',
              },
              { acc: 'BC bayonet lampholder', line: 'Centre contact = switched L; outer cup = N' },
              {
                acc: 'ES Edison-screw lampholder',
                line: 'Centre contact = switched L; screw thread = N',
              },
              {
                acc: 'Ceiling rose (loop-in)',
                line: 'L loop-in block; switched L from switch via SL',
              },
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              ...
              accessory. Press TEST. Reading should be OL — confirms the line is NOT cross-wired to
              the neutral at this accessory.
            </li>
            <li>
              <strong>Repeat for every accessory on the circuit.</strong> Tick off each accessory as
              both readings (low to L, OL to N) confirm correct polarity.
            </li>
            <li>
              <strong>For switches:</strong> test with the switch in both positions. Switch open =
              OL between line and switched-line terminal; switch closed = low resistance. Confirms
              the switch is in the line, not the neutral.
            </li>
            <li>
              <strong>For lampholders (BC and ES):</strong> test continuity from line of CU to
              centre contact (low) and to outer / screw contact (OL). The outer contact must be on
              neutral.
            </li>
            <li>
              <strong>Reconnect the line at the CU.</strong> Document on the STR — single pass/fail
              entry per circuit (or tick column).
            </li>
          </ol>
        </ConceptBlock>
        ...
        >
          <p>
            The three-LED socket tester (Martindale, Megger 1100, Kewtech LoopCheck-style) indicates
            wiring conditions by which combination of LEDs lights when the tester is plugged into a
            live socket. It's a useful quick check — but the manufacturers themselves and IET GN3
            are explicit: it is not a safe-isolation proof and not a verification instrument.
          </p>
          <p>What the socket tester can miss:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Reversed polarity at the supply (meter tails swapped).</strong> The tester
              sees the L pin at 230 V to earth and shows OK — even though that 230 V is actually
              coming from the supply neutral. Live polarity at the CU is the cure.
            </li>
            <li>
              <strong>Borrowed neutral.</strong> A circuit with a borrowed neutral from another
              circuit can present the correct potentials at sockets even when its own L-N wiring is
              reversed. Tester shows OK; protective device opening would not isolate the load.
            </li>
            <li>
              <strong>Multiple compounding defects.</strong> Particular combinations can produce LED
              patterns that look like "OK" or that don't match documented fault codes. The tester
              logic can't always distinguish.
            </li>
            <li>
              <strong>It cannot prove safe isolation.</strong> Even if the tester says "no voltage"
              — it could be that the tester is faulty, the test contacts haven't made proper
              contact, or there's a momentary glitch. Always use an approved voltage indicator and a
              proving unit for safe isolation, never a socket tester.
            </li>
          </ul>
          <p>
            The dead continuity polarity test (continuity from CU line out to L of every accessory)
            exercises the actual wiring — line conductor from the CU to the L terminal of every
            accessory, OL to the N terminal — and cannot be fooled by the cross-wiring patterns that
            confuse a socket tester. It's the test that satisfies Reg 643.6 and the test that goes
            in the polarity column of the STR.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Polarity per phase.</strong> Test each line conductor (L1, L2, L3) and the
              neutral as for single-phase — continuity from CU line conductor out to the correctly
              labelled terminal at every accessory. Each line conductor gets its own dead polarity
              test pass.
            </li>
            <li>
              ...
              indicator. Connect to the three line conductors at the supply or at a three-phase
              outlet. The instrument indicates "correct" (L1-L2-L3 clockwise) or "reverse" (L1-L3-L2
              counter-clockwise). Most modern MFTs include phase rotation as a built-in function.
            </li>
            <li>
              <strong>Why rotation matters.</strong> Three-phase induction motors run in the
              direction set by the phase rotation. A reversed rotation will run the motor backwards
              — fine for a centrifugal pump if you spotted it before commissioning, catastrophic for
              a saw blade or conveyor. Always confirm rotation BEFORE connecting the load.
            </li>
            <li>
              <strong>PSC (prospective short-circuit current) indicators.</strong> On three-phase,
              the PSC measurement also confirms balanced phase impedances — large discrepancies
              between phases suggest a broken phase or a poor neutral.
            </li>
          </ul>
          <p>
            Single-phase installs don't need phase sequence — you only have one line. Move to
            three-phase commercial or industrial work and rotation testing becomes a fundamental
            part of every commissioning. Document phase sequence on the STR and EIC for any
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              conductors between them (COM line in, two straps out, COM line out). Intermediate adds
              a 4-terminal switch in the middle that crosses the straps when toggled.
            </li>
            <li>
              <strong>At the first switch (typically the one fed by the line):</strong>
              continuity from CU line to COM should read low resistance (line arrives at COM). COM
              to L1 and COM to L2 vary depending on toggle position — test in BOTH positions and
              verify the reading swaps between L1 and L2 as you toggle. This proves the switch
              mechanism is in the line side, not the neutral.
            </li>
            <li>
              <strong>At the intermediate switch (if fitted):</strong> the four terminals form two
              pairs of strap connections. Test continuity between strap pairs in both toggle
              positions of the intermediate switch — readings should swap predictably as you toggle.
              Line never appears at the COM of an intermediate switch (intermediate switches don't
              have a COM in the same way).
            </li>
            <li>
              <strong>At the last switch (the one feeding the lamp):</strong> continuity from CU
              line through the whole switching chain to COM of the last switch should respond to
              toggle action of every switch in the chain. With all switches in a position that
              closes the circuit, COM of the last switch should read low resistance from CU line.
            </li>
            <li>
              <strong>At the lamp itself:</strong> continuity from CU line through the closed
              switching chain to the lamp's switched-line terminal (centre contact for BC/ES) should
              read low. The other terminal (outer/screw for BC/ES) should be on neutral — OL from CU
              line, low from CU neutral.
            </li>
          </ol>
          <p>
            <strong>Common error:</strong> someone wires the strap conductors so the COM at one
            switch is on the L1 strap and the COM at the other is on the L2 strap of the opposite
            switch. The lights still work in 2-way mode (because the loop closes one way or the
            other) but the switching is in the neutral, not the line — a Reg 643.6 fail. Polarity
            test in every position catches it.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Class I appliances and accessories.</strong> Have an exposed-conductive metal
              enclosure connected to the CPC. Polarity matters absolutely — single-pole switching in
              the line keeps the enclosure safe when the device is off; reverse the polarity and the
              enclosure is at line potential when the switch is open.
            </li>
            <li>
              ...
              matters less for the appliance's own safety but the wiring polarity to the socket
              still matters because the appliance can be replaced with a Class I device. Test
              polarity to every socket regardless.
            </li>
            <li>
              <strong>SELV/PELV equipment (12 V LEDs etc.).</strong> The SELV side is below the
              touch-safe threshold (50 V AC); polarity on the SELV conductors doesn't affect safety.
              But the LV primary side of the transformer/driver needs correct polarity tested to
              standard.
            </li>
            <li>
              <strong>LED drivers and switched-mode supplies.</strong> Internally rectify the AC
              input, so the load doesn't care about input polarity. But the wiring to the driver
              still needs correct polarity tested — single-pole switching of the input must be in
              the line conductor.
            </li>
            <li>
              <strong>Electronic dimmers and smart switches.</strong> Often three-wire connections
              (L in, switched L out, N) with internal electronics. Polarity tests must verify L in,
              L out and N are all on the correct terminals per the manufacturer's wiring diagram.
              Manufacturers publish specific wiring diagrams because the internal electronics depend
              on a specific polarity for sensing.
            </li>
          </ul>
          <p>
            The principle: polarity test EVERY accessory's L and N terminal with a continuity test
            from CU line. Whether the current load installed cares about polarity or not is
            irrelevant — the wiring must be correct so any future device installed there is safe.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            indicates wiring conditions by which combination of LEDs lights when plugged into a live
            socket. The standard pattern: all three lit = OK; specific outage patterns indicate
            reversed polarity, missing earth, missing neutral, etc.
          </p>
          <p>Why it can be fooled:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Borrowed neutral.</strong> A circuit where the neutral is borrowed from
              another circuit can present the correct potentials at the socket terminals even when
              the wiring of THIS circuit is reversed — the indicator pattern says "OK" but the
              protective device on this circuit, when opened, will not actually isolate the load.
            </li>
            <li>
          ...
          <p>
            ...
            verification instrument for Reg 643.6 — the dead continuity polarity test is the one
            that goes on the certificate.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Cross-references the polarity discussion: a semiconductor switching device (electronic
              switch, SSR, dimmer triac) cannot be relied on as the isolation device. Even if it is
              fitted in the line conductor, it cannot guarantee complete isolation when in the off
              state because semiconductors leak. Mechanical single-pole switches in the line are
              required for isolation. The polarity test applies to those mechanical isolators, not
              to electronic switches that may also be present.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              "polarity verified by socket tester". A few weeks later the customer gets a shock from
              a switched FCU — investigation reveals the FCU has the line and neutral reversed (a
              back-box terminal swap during installation), but the socket tester showed OK at the
              downstream sockets because the cumulative wiring presented the correct potentials at
              those points. You are looking at a defect-correction visit and a worried customer.
            </>
          }
          doInstead={
            <>
              Always run the dead continuity polarity test at sign-off, on every accessory of every
              circuit. The continuity test exercises the actual wiring — line conductor from the CU
              to the L terminal of the accessory, OL to the N terminal — and cannot be fooled by the
              cross-wiring patterns that confuse a socket tester. The socket tester is a useful live
              confirmation tool but is not a substitute for the dead continuity test required by Reg
              643.6.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You have just installed a hallway light controlled by a pair of 2-way switches, one at
              the front door and one at the foot of the stairs. The lamp itself is a pendant on a BC
              lampholder. Cabling: line and neutral from the consumer unit run up to the rose;
              switched-line is dropped down to the front-door switch as a 3-core cable (line, strap,
              strap); 3-core strap conductors run from front-door switch to the stairs switch. Lamp
              is in OFF position at both switches. You need to prove polarity before sign-off.
            </>
          }
          whatToDo={
            <>
              ...
              pass). Probe 2 to the loop-in N block (OL — pass). Probe 2 to the centre contact of
              the BC lampholder lead from the rose (depends on switch positions — see below). Probe
              2 to the outer / screw contact of the BC lampholder lead (OL — pass, proving outer is
              on neutral).
              <br />
              <br />
              <strong>At front-door switch:</strong> probe 2 to COM terminal (low resistance — pass;
              line arrives at COM via the dropper from the rose). Probe 2 to L1 and L2 terminals —
              toggle the switch and verify continuity changes between L1 and L2 positions. Probe 2
              to the back-box CPC (OL — pass, line not bridging to earth).
              <br />
              <br />
              <strong>At stairs switch:</strong> probe 2 to COM (depends on front-door switch
              position — toggle front-door switch and verify the COM at the stairs switch changes
              from L1 to L2 strap connection). Probe 2 to back-box CPC (OL — pass).
              <br />
              <br />
              <strong>Lamp polarity:</strong> with both switches set so the lamp is "on" (closed),
              probe 2 to the centre contact of the BC lampholder. Reading should be low resistance
              through the switching — proves switched line lands on the centre contact. Probe 2 to
              the outer / screw contact — OL — proves the outer is on neutral.
              <br />
              <br />
          ...
          whyItMatters={
            <>
              ...
              someone wires "L1" and "L2" the wrong way round, or uses the brown sleeve as the strap
              and the blue as the line, or fits an intermediate switch the wrong way up. A live
              socket tester would not catch any of these errors because there is no socket.
              Continuity polarity testing of every COM, every strap and every lampholder terminal is
              the only way to prove the wiring is correct before energising.
            </>
          }
        />
