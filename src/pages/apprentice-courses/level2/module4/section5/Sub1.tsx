...
const quizQuestions = [
  {
    id: 1,
    question: 'BS 7671 Reg 642.1 says inspection shall normally be done:',
    options: [
  ...
  },
  {
    ...
    options: ['Section 514', 'Section 522', 'Section 526', 'Section 411'],
    correctAnswer: 2,
    explanation:
...
const faqs = [
  ...
  {
    question:
      'My supervisor told me to "tug-test" every termination during inspection. Is that a thing?',
    answer:
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 5 · Subsection 1"
        title="Verify wiring systems conform to IET standards"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The dead inspection per BS 7671 Section 642. Visual checks on connections, identification,
          routing, conductor selection, barriers, accessories and notices — before any test
          instrument touches the board.
        </p>
        ...
        >
          <p>
            <strong>Why inspect at all?</strong> Because most defects on a finished install are
            visible to the eye before any instrument is connected. A loose terminal, a missing CPC
            sleeve, an unlabelled CU way, a fuse in the neutral — none of these need a test
            instrument to spot. Catching them at inspection is cheap; catching them at testing means
            re-opening the work; catching them after energising means a fault, a callback or worse.
          </p>
          <p>
            <strong>Why dead?</strong> Because you cannot safely walk a CU with the cover off and
            your eyes on terminations if those terminations are at 230 V. Reg 642.1 is explicit —
            inspection is normally done with the part of the install under inspection disconnected
            from the supply.
          </p>
          <p>
            <strong>What does "visible and accessible" actually mean?</strong> You inspect what you
            can see and reach without dismantling the install. CU terminations with the cover off —
            yes. Accessory terminations during second-fix — yes. Cable runs in containment before
            the lid goes on — yes. The terminations on an RCBO buried under a busbar shroud on a
            finished CU — that is a Limitation (LIM), and you record it as such.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 642.1 is the regulation that fixes the order. Inspection is first, testing is
              second. Both are done dead — testing then continues with the install energised for the
              live tests (loop impedance, RCD trip times, functional checks, prospective fault
              current). The reason is safety — you cannot reasonably inspect terminations and
              accessory wiring with the circuit live, and you cannot reasonably test continuity or
              insulation resistance with it live either.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 642.2 is the three-part question you are answering at every inspection step. (a)
              Is the kit itself the right kit — BS / BS EN marked, CE / UKCA marked, fit for the
              job? (b) Has it been installed correctly per BS 7671 AND per the manufacturer&rsquo;s
              instructions — torque values, orientation, terminal grouping, ambient limits? (c) Is
              anything visibly broken, cracked, damaged or worn so as to compromise safety? Every
              Schedule of Inspections item is one slice of those three questions.
            </>
          }
              ...
              >
                <div className="flex items-center justify-between border-b border-white/[0.08] pb-2 mb-2.5">
                  <div className="flex items-baseline gap-2.5">
                    ...
                    <span className="text-white text-[14px] font-medium">{row.item}</span>
                  </div>
                </div>
                <p className="text-[13px] text-white/85 leading-relaxed">{row.visual}</p>
              </div>
            ))}
          </div>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 642.3 is the master list. At Level 2 you are not expected to memorise all twenty
              items verbatim, but you are expected to recognise each one on the Schedule of
              Inspections, know what BS 7671 chapter it maps to, and know what to look at to verify
              it. Items (a)–(g) are the workmanship items — what most apprentice inspections focus
              on. Items (h)–(t) are the design and presentation items — what the supervisor signs
              off on the EIC.
            </>
          }
        ...
        >
          <p>
            <strong>Origin (Items 1.0 – 2.0):</strong> Distributor&rsquo;s cut-out, meter tails (CSA
            + sheath + sleeving + termination at MET / switch), MET clearly identified, earthing
            conductor present and to the right CSA for the supply type. Main switch labelled. Main
            bonding clamps to gas / water / oil / structural steel within 600 mm of the meter, all
            BS 951 labels in place.
          </p>
          <p>
            <strong>Distribution (Items 3.0 – 10.0):</strong> CU way labels match the circuit
            schedule. RCBOs / MCBs / RCDs present and of the right type per the design. AFDDs where
            Reg 421.1.7 requires. SPD present where Section 443 requires. Single-pole devices in
            line conductors only. Every termination inside the CU secure, no copper showing past the
            terminal, no insulation trapped, CPCs sleeved and to the earth bar.
          </p>
          <p>
            <strong>Circuit walk (Items 11.0 – 13.0):</strong> Cable runs visible in containment —
            supports at the right intervals, no kinks, bend radii respected, no damage to sheath.
            Containment grounded where it needs to be. Accessories — every back-box grommet in, no
            copper showing, CPC sleeved at every point including the back-box flying lead on a metal
            box. Faceplate fixings tight, alignment good.
          </p>
          <p>
            <strong>Bonding and earthing (Items 11.0 – 13.0):</strong> Earthing conductor continuous
            from MET to consumer unit. Main bonding clamps clean, on bright metal, BS 951 label
            visible. Supplementary bonding present where required (older bathrooms, certain
            locations Part 7). CPC continuity at every accessory.
          </p>
          <p>
            <strong>Notices and labels (Items 9.0 + 14.0 onwards):</strong> Periodic inspection
            notice. RCD test notice (if RCDs present). Mixed-cable-colours notice if the install has
            both old and new colours. Photovoltaic notice if PV is connected. EV charging notice if
            applicable. CU way labels legible and matching the circuit schedule.
          </p>
        </ConceptBlock>
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>✓ — Compliant.</strong> You inspected the item, it meets BS 7671, you are
              happy with it. Most items on a well-built new install end up here.
            </li>
            <li>
              <strong>N/A — Not applicable.</strong> The item does not apply to this installation.
              "Earth electrode" on TN-C-S. "Main bonding to oil" on a gas house. "Supplementary
              bonding" in a modern bathroom on a 30 mA RCD with no extraneous-conductive-parts.
              Always think before marking N/A — sometimes an item that looks N/A is actually a
              missed requirement.
            </li>
            <li>
              <strong>LIM — Limitation.</strong> The item applies but you could not access it
              without dismantling the install. Sealed busbar shrouds, buried cable runs after the
              wall is plastered, terminations behind a fixed appliance. Record clearly what was
              limited and why, and flag the supervisor — they may decide to revisit it or accept the
              limitation on the EIC.
            </li>
            <li>
              <strong>✗ — Non-compliant.</strong> The item fails the inspection. Loose terminal,
              missing CPC, fuse in neutral, no main bonding clamp on the gas pipe. Must be reported,
              fixed, re-inspected. The install does not get energised until every ✗ is resolved.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Basic protection (preventing contact with normally-live parts):</strong> Live
            parts insulated. Enclosures complete — CU lid in place once inspection is done, no
            missing busbar shrouds (or recorded as LIM if you cannot inspect behind), no missing
            knockouts in metal accessory boxes, accessory faceplates not cracked or missing.
          </p>
          <p>
            <strong>Fault protection (automatic disconnection of supply):</strong> Every Class I
            exposed-conductive-part connected to the CPC system (Reg 411.3.1.1). Protective devices
            of correct type and rating per the design. RCDs / RCBOs / MCBs / AFDDs all installed
            correctly, not bypassed, not modified.
          </p>
          <p>
            <strong>Additional protection (30 mA RCD as a backstop):</strong> Reg 411.3.3 — 30 mA
            RCD on socket outlets up to 32 A in any installation. Reg 411.3.4 — 30 mA RCD on AC
            final circuits supplying luminaires in domestic premises. Reg 522.6.202 — 30 mA RCD on
            cables in walls outside prescribed zones. Reg 415.1.1 in special locations (bathrooms,
            swimming pools, certain Part 7 applications).
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>IP2X kitchen socket fitted within 600 mm of a sink. Should be IP44 minimum.</li>
            <li>
              Standard plastic faceplate fitted in a workshop where regular impact is likely. Should
              be IK-rated metal-clad.
            </li>
            <li>
              Indoor luminaire fitted in an outdoor / damp location. Should be IP-rated for the
              environment.
            </li>
            <li>
              Standard accessory in a Zone 1 bathroom location. Section 701 is specific — IP X4
              minimum, with additional restrictions on the type of accessory permitted.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Presence:</strong> SPD fitted at the origin. None removed during second-fix or
              maintenance.
            </li>
            <li>
              <strong>Type:</strong> Type 2 typical for small commercial / domestic CU. Type 1 where
              building has BS EN 62305 lightning protection or exposed overhead supply. Type 3 for
              downstream sensitive equipment (in addition to upstream Type 2).
            </li>
            <li>
              <strong>Status:</strong> Modules indicate healthy (typically a green flag or LED).
              End-of-life indicators (red flag) mean the SPD has absorbed a surge and needs
              replacement — this is a non-conformance.
            </li>
            <li>
              <strong>Connection:</strong> 6 mm² minimum connecting conductor per Reg 534.4.10, as
              short as practicable, connected to the MET and the line conductor.
            </li>
            <li>
              <strong>Notice:</strong> SPD warning notice fitted where required — informs
              maintenance staff that the SPD will isolate certain circuits if it fails closed.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Zone 0 (inside the bath) — only SELV equipment specifically designed for Zone 0. No
              mains accessories, no luminaires.
            </li>
            <li>
              Zone 1 (above the bath up to 2.25 m) — IP X4 minimum, only SELV / fixed equipment
              specifically suitable for Zone 1.
            </li>
            <li>
              Zone 2 (extending 0.6 m horizontally from Zone 1) — IP X4 minimum, fixed equipment,
              restricted accessories.
            </li>
            <li>
              Outside zones — standard accessories (with the whole bathroom on a 30 mA RCD per Reg
              415.1.1).
            </li>
            <li>
              Supplementary bonding (Reg 701.415.2) — only required if an extraneous-
              conductive-part is present that does not have main bonding to the MET, AND 30 mA RCD
              additional protection is not provided across the whole bathroom.
            </li>
          </ul>
          <p>
            <strong>EV charger inspection (Section 722) — typical items:</strong> Type B RCD or Type
            A with integrated DC RCM in the charger; cable type appropriate to route (SWA buried,
            T&E indoors); EV charger location accessible; EV warning notice present; bonding back to
            MET if required by the manufacturer&rsquo;s instructions.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Phase 1 — observe.</strong> First few inspections, you carry the clipboard, hold
            the torch, write down what the senior electrician calls out. You watch the order they go
            in. You notice what they pause at and look closely at. You ask why after, not during.
          </p>
          <p>
            <strong>Phase 2 — assist.</strong> You start running the Schedule of Inspections form
            yourself — calling out each item, the senior electrician verifies it. You are catching
            the obvious defects yourself by this point — missing CPC sleeve, unlabelled way, loose
            terminal — and flagging them to be fixed.
          </p>
          <p>
            <strong>Phase 3 — lead with sign-off.</strong> Towards the end of Level 2 / into Level
            3, you are running the inspection on a small install yourself, filling out the Schedule
            of Inspections, then handing it to the supervisor for review and counter-signature. By
            Level 3 portfolio time, leading two or three full inspections under supervision is
            exactly what the qualification wants to see.
          </p>
          <p>
            <strong>Phase 4 — sign off independently.</strong> Once qualified and on the tools, you
            can sign off your own inspections on installs you have built or supervised. EICR work
            has its own additional certification pathway.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You hear "inspection and testing" lumped together so often that you start to think
              they are the same thing. So at the inspection stage you bring out the multifunction
              tester, plug it in, and start measuring continuity. You tick the inspection items
              based on test results — "loop impedance is fine so identification of conductors must
              be fine". You skip the actual visual walk-round because the instrument tells you the
              circuit works.
            </>
          }
          doInstead={
            <>
              Keep them separate. Inspection (Section 642) is eyes, hands, torch — no instrument.
              You verify what you can see and access.{' '}
              <strong>
                Testing (Section 643) is the instrument stage — and it only happens after inspection
                is complete and signed off.
              </strong>{' '}
              Reg 642.1 is unambiguous about the order. Plenty of defects only show up at the visual
              inspection — fuse in the neutral, missing CPC sleeve, wrong RCD type, missing notice —
              and an instrument will not catch any of those. You do both because they catch
              different things.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              Friday morning. The CU has just been mounted on the wall in a new-build three-bed
              semi. The team has wired in eleven circuits — six lighting (two upstairs / two
              downstairs / hall + landing / external), three socket rings (upstairs / downstairs /
              kitchen), two dedicated radials (cooker / shower). Every cable is in, every terminal
              is landed, every way is labelled. The CU cover is off. The Schedule of Inspections is
              on the clipboard. Your supervisor says "inspect it — I’ll be back in 20 minutes". The
              installation is dead — no incomer connection yet.
            </>
          }
          whatToDo={
            <>
              Start at the origin. Tails into the main switch — CSA correct, sheath stripped to the
              right point, no copper past the terminals, sleeve at the CPC. MET — earthing conductor
              in, sized correctly. Main bonding clamps — gas pipe, water pipe, both within 600 mm of
              their meters, BS 951 labels. Then into the CU — every RCBO seated, every terminal
              screw torqued, no copper showing, no insulation trapped, CPCs to the earth bar. Then
              circuit by circuit — way 1 lighting upstairs first. Open the circuit drawing. Walk to
              each accessory in order. Faceplate loose so you can see the terminations. Brown to L,
              blue to N, green-yellow sleeved CPC to E. Switched live identified at both ends. No
              copper past the terminal. No back-box without a grommet. Repeat for every circuit.
              Notices — periodic inspection notice, RCD test notice. Done. Schedule of Inspections
              marked up — ✓ for everything that complies, LIM for anything you could not access
              (probably the cable runs buried in the plastered walls), ✗ for anything you found
              wrong. Hand it to your supervisor.
            </>
          }
          whyItMatters={
            <>
              Twenty minutes is plenty for an apprentice working a new domestic CU inspection at a
              steady rhythm. The supervisor is going to spot-check you — they will pick three or
              four items at random and verify them themselves. If your ✓ on those items holds up,
              your inspection rhythm is sound and you start getting trusted to inspect alone. This
              is the exact build-up of trust that takes you from observing to signing off over the
