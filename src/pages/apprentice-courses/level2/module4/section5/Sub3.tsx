...
const TITLE = 'Dead testing — preparation and sequence (5.3) | Level 2 Module 4.5.3 | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  ...
  },
  {
    ...
    question: 'Before any dead test, the absolute prerequisite is:',
    options: [
...
const quizQuestions = [
  ...
  {
    id: 2,
    question: 'The R1 + R2 test verifies:',
    options: [
  ...
  {
    id: 3,
    question: 'Why is continuity testing carried out before insulation resistance testing?',
    options: [
  ...
  {
    id: 6,
    question: 'The polarity test (Reg 643.6) at every accessory verifies:',
    options: [
  ...
  {
    id: 7,
    question: 'For a TT installation, an additional dead test is required:',
    options: [
...
const faqs = [
  ...
  {
    question: 'What if a test fails — for example, IR comes back at 0.5 MΩ on a 230 V circuit?',
    answer:
...
export default function Sub3() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 4 · Section 5 · Subsection 3"
        title="Dead testing — preparation and sequence"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The BS 7671 Reg 643 dead-test sequence in order — continuity, ring final, insulation
          resistance, polarity, earth electrode — with the prep that makes it work.
        </p>
        ...
        >
          <p>
            <strong>What changes between inspection and testing:</strong> The instrument comes out.
            You stop relying on eyes alone and start relying on calibrated measurements. The
            discipline shifts from &ldquo;does it look right?&rdquo; to &ldquo;does it measure
            within tolerance?&rdquo;
          </p>
          <p>
            <strong>What stays the same:</strong> The install is still dead. The JIB safe isolation
            procedure has been verified. The work is methodical, in order, recorded as you go.
          </p>
          <p>
            <strong>Where this Sub fits:</strong> Sub 5.1 and 5.2 covered the inspection. Section 6
            covers each individual test in detail (continuity in Sub 6.1, ring final in Sub 6.2, IR
            in Sub 6.3, polarity in Sub 6.4, functional testing in Sub 6.5, recording in Sub 6.6).
            This Sub is the bridge — the prep and the order, before you start the individual tests.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 643.1 is the regulation that fixes the dead-test order. Tests in Regs 643.2 to
              643.6 are done before energisation, in the order written — that is the dead-test
              sequence. Reg 643.7 (earth electrode) is added where applicable. Live tests come
              after. Instruments must be BS EN 61557 compliant — that is what your modern MFT
              carries certification for.
            </>
          }
        ...
        <ContentEyebrow>
          Pre-test preparation — the ten minutes that make testing safe
        </ContentEyebrow>
        ...
        >
          <ol className="space-y-2 list-decimal pl-5 marker:text-elec-yellow/80 text-[13.5px]">
            <li>
              <strong>Inspection complete and signed.</strong> Schedule of Inspections walked, ✓
              throughout, honest LIMs, zero ✗. If the inspection was not clean, fix the ✗ items
              first — testing a non-compliant install wastes time.
            </li>
            <li>
              <strong>Safe isolation verified — JIB procedure.</strong> Switched off and locked off
              at the upstream device. Proving unit proved alive on a known live source. Install
              proved dead at the point of work. Proving unit re-proved alive. Lock-off tag in place,
              key in your pocket. The proving unit is the only thing that can declare the install
              dead — your multifunction tester does NOT do this job.
            </li>
            <li>
              <strong>Instrument calibration in date.</strong> Check the calibration sticker on the
              MFT. Most instruments are on a 12-month cycle. Out of calibration = results not
              accepted on the EIC and a callback if the test pack is audited.
            </li>
            <li>
              <strong>Leads checked and nulled.</strong> Visually inspect leads for damage (cracked
              insulation, exposed conductor, broken probe tips). On the continuity range, short the
              leads together and null / zero the instrument so lead resistance is subtracted from
              your readings.
            </li>
            <li>
              <strong>Customer briefed.</strong> If the customer is on site, brief them — &ldquo;the
              install will be off for the next two hours while we test, no power to the kettle until
              we&rsquo;re done&rdquo;. Lifts off, fish tanks have battery backup connected if
              applicable, freezers warned about. Manage expectations.
            </li>
            <li>
              <strong>Equipment connected to circuits considered.</strong> Anything that could be
              damaged by 500 V DC IR test? Computers, modems, smart lighting controllers,
              low-voltage transformers, dimmers, appliances with electronic boards. Disconnect or
              short out per manufacturer&rsquo;s guidance. Reg 643.3 specifically allows for this —
              equipment that could be damaged is disconnected, then a 250 V DC test is repeated on
              reconnection.
            </li>
          </ol>
        </ConceptBlock>
        ...
        >
          <div className="space-y-3">
            {testSequence.map((t) => (
              <div key={t.n} className="rounded-xl border border-white/10 bg-white/[0.03] p-3.5">
                <div className="flex items-baseline gap-2.5 border-b border-white/[0.08] pb-2 mb-2.5">
                  ...
                  <span className="text-white text-[14px] font-semibold flex-1">{t.test}</span>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              400 V LV the test is at 500 V DC and the minimum IR is 1 MΩ. For SELV / PELV / safety
              circuits the test is at 250 V DC. Where equipment could be damaged by the 500 V DC
              test, the equipment is disconnected for the main test, then reconnected and a 250 V DC
              test applied per the A4:2026 clarification. Modern MFTs select the test voltage on the
              front panel.
            </>
          }
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Continuity → IR.</strong> If the CPC is broken, IR pushes 500 V DC between L
              and CPC and the test current finds an unintended path (through another circuit,
              through wet plaster, through the customer if you are very unlucky). The IR result is
              meaningless and potentially hazardous. Continuity first establishes the reference.
            </li>
            <li>
              <strong>Ring final continuity → IR / polarity.</strong> If the ring is actually wired
              as two long radials bridged at the CU, you cannot interpret R1+R2 readings or polarity
              at the &ldquo;far&rdquo; sockets correctly. Ring final continuity confirms the
              topology before the rest of the tests.
            </li>
            <li>
              <strong>IR → polarity.</strong> If insulation is poor, the test current from polarity
              testing finds unintended paths and the polarity reading may be ambiguous (continuity
              through a low-IR fault rather than through the intended L conductor).
            </li>
            <li>
              <strong>Polarity → energisation.</strong> If polarity is reversed at any accessory,
              energising the install means a switch leaves the lamp permanently live, or a socket
              has live on the wrong pin. Polarity is confirmed before energisation.
            </li>
            <li>
              <strong>Earth electrode → live tests (TT only).</strong> On TT, the earth electrode
              resistance has to be known before live loop impedance tests can be interpreted,
              because the loop runs through the electrode.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70 text-[13.5px]">
            ...
            <li>
              At the consumer unit, with the circuit isolated, measure resistance from line to CPC
              at the way / RCBO output.
            </li>
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70 text-[13.5px]">
            ...
            <li>
              Take the other lead (extension lead added if needed) to the first accessory and touch
              it to the CPC there.
            </li>
            <li>Read the resistance — that is R2 from CU to that accessory.</li>
            <li>Move to the next accessory and repeat.</li>
            <li>
              Null the leads first; account for the long lead resistance in the recorded value.
            </li>
          </ol>
          <p>
            <strong>Polarity overlap:</strong> The long-lead method is also used during polarity
            verification — the lead at the CU is on the line conductor at the CU, and you touch the
            line terminal at every accessory. Reading should be consistent (the line conductor R1
            from CU to that accessory), confirming line is in the line terminal.
          </p>
        </ConceptBlock>
        ...
        <ContentEyebrow>
          Ring final continuity — the cross-connection method in detail
        </ContentEyebrow>
        ...
        >
          ...
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70 text-[13.5px]">
            <li>
              Disconnect both ends of L, both ends of N, both ends of CPC at the CU (so you have six
              free conductor ends).
            </li>
            <li>
              Identify the &ldquo;outgoing&rdquo; and &ldquo;returning&rdquo; ends of each conductor
              — typically by measuring end-to-end loop resistance. Should give a sensible value
              matching the cable run length.
            </li>
            <li>
              <strong>End-to-end test (r1, rn, r2):</strong> Measure each conductor end-to-end. r1
              (line), rn (neutral), r2 (CPC). All three should be approximately equal (within ~5 %)
              on a homogeneous ring.
            </li>
            <li>
              <strong>Cross-connect:</strong> At the CU, link L1 to L2 (line cross-connection) and
              CPC1 to CPC2 (CPC cross-connection). Now line and CPC form a single loop through the
              ring.
            </li>
            <li>
              <strong>Walk the ring:</strong> At each socket on the ring, plug in the long lead and
              measure R1+R2 between L and CPC at that socket. Record the value at each socket. All
              readings should be approximately equal, because each socket is at the
              &ldquo;mid-loop&rdquo; point of the cross-connected ring.
            </li>
            <li>
              <strong>Spot the radial:</strong> If readings vary significantly along the ring (low
              at one end, high at the other, or one reading way off from the others), the ring is
              not a true ring. Investigate.
            </li>
            <li>
              Remove cross-connection links. Reconnect L, N, CPC to the original ways at the CU.
            </li>
          </ol>
          <p>
            <strong>Why this works:</strong> On a true ring with cross-connection, every socket sees
            the same loop resistance because the loop length from L1 → socket → CPC2 is matched by
            the loop length from L2 → socket → CPC1. Vary one and the matching changes — the
            readings give you the topology of the ring.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              Smart lighting controllers, dimmer switches, electronic transformers for low-voltage
              downlights.
            </li>
            <li>Modems, routers, smart hubs.</li>
            <li>
              Appliances with electronic boards — modern fridges, washing machines, dishwashers,
              ovens, hobs.
            </li>
            <li>
              SPD modules — disconnect SPD before 500 V DC test (the SPD will trip during the test
              and may give a false low IR reading, or be damaged).
            </li>
            <li>
              RCBO and RCD trip electronics (test on the line side of the device, or follow
              manufacturer&rsquo;s instructions for in-situ testing).
            </li>
          </ul>
          <p>
        ...
        >
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Set the circuit identifier on the MFT</strong> before each test. Most modern
              MFTs let you save the reading against a circuit number.
            </li>
            <li>
              <strong>Take the reading and save it.</strong> Confirm the value before moving on.
            </li>
            <li>
              <strong>At end of test session</strong>, download the instrument memory to PDF or to
              the Schedule of Test Results software. Most MFT manufacturers have a companion app for
              this.
            </li>
            <li>
              <strong>Review the downloaded values</strong> against the design predictions. Anything
              anomalous gets investigated and re-tested before the EIC is issued.
            </li>
            <li>
              <strong>Schedule of Test Results signed and dated</strong> by the competent person who
              carried out the testing.
            </li>
          </ul>
          <p>
            The Schedule of Test Results is the test-stage equivalent of the Schedule of Inspections
            — both attach to the EIC, both prove what was done, both are signed by the competent
            person.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You finish the inspection. The install looks great. You grab the MFT, set it to IR
              mode, push 500 V DC between L+N and CPC. You get a reading of around 12 MΩ — passes
              Table 64, you record it as a clear pass and move on. Later, when you do continuity,
              R1+R2 on circuit 4 comes back open — the CPC is broken at a junction box in the loft.
              Your IR reading of 12 MΩ was not a measurement of the insulation between live and
              earth — it was a measurement of leakage current finding an unintended path through
              another circuit&rsquo;s CPC. The actual insulation could be far worse than 12 MΩ; you
              have no way of knowing.
            </>
          }
          doInstead={
            <>
              Continuity first. Always. Reg 643.1 fixes the order for a reason — every later test
              needs a known earth reference, which only continuity establishes. Get into the habit
              on every job: low-resistance ohmmeter range, lead null, R1+R2 at every circuit,
              recorded on the Schedule of Test Results, every CPC verified continuous,{' '}
              <strong>then</strong> switch the MFT to IR and start the insulation testing. The order
              is not optional and it is not a preference — it is a regulation.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              Three-bed semi, new CU, eight circuits — three lighting (upstairs / downstairs /
              external), three socket rings (upstairs / downstairs / kitchen), one cooker radial,
              one shower radial. TN-C-S supply. Inspection complete and signed. Customer is at work,
              lock-off in place, MFT calibrated and nulled. Time to start the dead-test sequence.
            </>
          }
          whatToDo={
            <>
              <strong>Step 1 — Continuity (R1 + R2) at the CU.</strong> Disconnect the main earth
              from the MET (carefully, recording the original Ze for live test later). At the far
              end of each circuit, link L to CPC. At the CU, measure the loop with the LR ohmmeter.
              Record R1+R2 per circuit. Lighting ~0.5 Ω, sockets ~0.7 Ω, cooker ~0.4 Ω, shower ~0.3
              Ω. Reconnect at MET.
              <br />
              <br />
              <strong>Step 2 — Ring final continuity.</strong> For each ring (3 of them): disconnect
              both ends of L, N, CPC at the CU. End-to-end on each: L1 to L2, N1 to N2, CPC1 to
              CPC2. Each should give a sensible loop value. Cross-connect L1 to L2 and CPC1 to CPC2.
              Walk to every socket, plug in the long lead, read R1+R2 — should be roughly the same
              at every socket on the ring. Record values.
              <br />
              <br />
              <strong>Step 3 — Insulation resistance.</strong> Switch MFT to IR mode, 500 V DC
              selected. Disconnect anything sensitive. Test L+N to CPC on each circuit — should be
              &gt;200 MΩ on a brand new install (minimum 1 MΩ per Table 64). Then L to N on each
              circuit — same. Record values per circuit.
              <br />
              <br />
              <strong>Step 4 — Polarity.</strong> Continuity from CU line conductor to line pin /
              line terminal at every accessory. Verify L on L, N on N, CPC on E. Mark Schedule of
              Test Results &ldquo;polarity confirmed&rdquo; per circuit.
              <br />
              <br />
              <strong>Step 5 — Earth electrode.</strong> N/A — this is TN-C-S, no electrode
              required.
              <br />
              <br />
              Dead testing complete. Hand the Schedule of Test Results to the supervisor for review.
              Energise in controlled manner. Live testing follows.
            </>
          }
          whyItMatters={
            <>
              A clean dead-test sequence takes 60–90 minutes on an 8-circuit domestic install once
              you are practised. The discipline of running the same order every time, recording each
              value as you go, and never skipping a test — that is what builds you into a competent
              engineer. Apprentices who try to remember readings to write up at the end always lose
              values. Record as you go.
            </>
          }
        />
