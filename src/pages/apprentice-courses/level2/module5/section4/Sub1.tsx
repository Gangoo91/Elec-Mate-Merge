...
const TITLE = 'Purpose of customer information (2.3) | Level 2 Module 5.4.1 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  ...
  {
    id: 3,
    question: "What's the PURPOSE of a job sheet (sometimes called a worksheet or visit record)?",
    options: [
...
const faqs = [
  ...
  {
    question: 'If the customer signs the quote, does that mean we have a contract?',
    answer:
  ...
  {
    question: "Do I need to give the customer a job sheet if they didn't ask for one?",
    answer:
  ...
  {
    question: "What goes in a 'handover pack' and is it different from the cert?",
    answer:
  ...
  {
    question: 'Is a digital copy of the EIC as good as a paper one?',
    answer:
  ...
  {
    question: "What's the legal minimum I have to give a domestic customer at the end of a job?",
    answer:
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 5 · Section 4 · Subsection 1"
        title="Purpose of customer information"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Every customer-facing document on a job has a single defensible purpose. Get any one of
          them wrong and the customer (or their solicitor) has the gap they need.
        </p>
        ...
        >
          <p>The chain works in two directions:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Forwards (defence) — when the customer complains six months later, the chain is the
              evidence of what was agreed (quote/contract), what was done (job sheet, cert), and
              what was handed over (handover pack). Each document references the next.
            </li>
            <li>
              Backwards (clarity) — when you arrive on site, the chain tells you what the customer
              was promised, what other trades have already done, and what equipment is in the
              install. A clean chain on a returning visit cuts your time on site dramatically.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>A good quote contains:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>Customer details, site address, your firm's details and registration numbers.</li>
            <li>
              Scope — itemised, with quantities (number of sockets, length of trunking, model and
              rating of CU, etc.).
            </li>
            <li>Price — fixed quote or labour-and-materials estimate, clearly labelled.</li>
            <li>
              Exclusions — what you're NOT doing (decorating, plastering chases beyond first-fix,
              building work, customer-supplied equipment).
            </li>
            <li>Payment terms — deposit, milestones, final payment, methods accepted.</li>
            <li>Lead-time, expected start and finish dates, validity period of the quote.</li>
            <li>T&Cs — variations, cancellations, warranty, dispute resolution.</li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Key contract issues that catch apprentices and small firms out:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Variations</strong> — extras added on the day. The contract should require
              variations to be agreed in writing (text, email, signed VO sheet) BEFORE the work is
              done. Verbal 'while you're there' requests that get billed later are the classic
              dispute trigger.
            </li>
            <li>
              <strong>Cooling-off period</strong> — for off-premises consumer contracts (e.g. a
              quote you give in the customer's kitchen rather than from your trade counter), the
              Consumer Contracts Regulations 2013 give the consumer 14 days to cancel. You have to
              tell them this in writing and provide a cancellation form, or the period extends to 12
              months.
            </li>
            <li>
              <strong>Statutory terms</strong> — Consumer Rights Act 2015 s.49 (reasonable care and
              skill), s.50 (information binding), s.52 (performance within reasonable time). These
              apply automatically and override any conflicting T&C.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>A defensible job sheet records:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>Date, start and finish times, names of operatives on site.</li>
            <li>Reference to the quote / contract / works order it was carried out under.</li>
            <li>Tasks completed (with reference to the scope), tasks NOT completed (and why).</li>
            <li>Materials used (so the invoice can be reconciled and stock can be deducted).</li>
            <li>Test results (where applicable — these flow through to the cert too).</li>
            <li>
              Photos — before, during, after — particularly of anything customer-visible that might
              be disputed later.
            </li>
            <li>
              Customer comments and customer signature confirming acceptance of the work done on the
              visit.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>A compliant UK trade invoice contains:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Your firm's name, address, registration numbers (Companies House if Ltd, VAT number if
              VAT-registered).
            </li>
            <li>Customer name and billing address (which may differ from the site address).</li>
            <li>Unique invoice number, invoice date, due date.</li>
            <li>
              Itemised lines for labour, materials and any other costs, with quantities and unit
              prices where relevant.
            </li>
            <li>
              Subtotal, VAT (with rate stated), total. For VAT invoices, the VAT must be shown
              separately (HMRC rules).
            </li>
            <li>
              Payment terms (net 14, net 30, etc.), payment methods accepted, bank details or
              payment link.
            </li>
            <li>Reference back to the quote / works order and the job sheet.</li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>The four cert types you'll meet most often:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              <strong>MWC (Minor Electrical Installation Works Certificate)</strong> — for additions
              and alterations that don't include a new circuit or a replacement consumer unit (the
              alternative allowed by Reg 644.4.201). Common for adding a socket or replacing a
              fitting.
            </li>
            <li>
              <strong>EICR (Electrical Installation Condition Report)</strong> — for periodic
              inspection of an existing install. Lists observations with C1/C2/C3/FI codes and an
              overall satisfactory/unsatisfactory verdict.
            </li>
            <li>
              <strong>Scheme certificate</strong> — issued by your contractor scheme (NICEIC, NAPIT,
              ELECSA, STROMA) and serves as Building Regs Part P notification for notifiable work.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>What the customer needs in plain English:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              How to operate the install day to day — turning circuits on/off, using the main
              switch, resetting an RCBO after a trip.
            </li>
            <li>
              How to perform any user-level routine checks (fire alarm weekly test, RCD test button
              quarterly, heat alarm battery check).
            </li>
            <li>
              What the indicator lights mean — green is healthy, amber is fault, red is emergency.
              Apply to fire alarm panel, EV charger, smart heating.
            </li>
            <li>
              What to do in an emergency — where the cut-out is, where the main switch is, who to
              ring out of hours.
            </li>
            <li>
              When to call you back — symptoms that mean it's not safe to keep using (burning smell,
              frequent trips, flickering, hot accessories).
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>What manufacturer data needs to cover:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Make, model, serial number — so the customer (and any future trade) can identify
              exactly what's installed.
            </li>
            <li>Warranty terms — duration, what's covered, how to claim.</li>
            <li>
              Servicing requirements — what the user can do, what needs a competent person, what the
              service interval is.
            </li>
            <li>
            ...
            <li>Manufacturer's contact details for in-warranty support.</li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>An as-installed drawing should record:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Cable routes — in walls, under floors, in ceiling voids — with depth and orientation
              indicated.
            </li>
            <li>
              Accessory positions — sockets, switches, lights, smoke alarms, fire alarm devices.
            </li>
            <li>
              Distribution equipment — CU position, sub-board positions, isolators, meter positions.
            </li>
            <li>
              Junction boxes and concealed terminations — particularly anything that's been buried
              in plaster or under flooring.
            </li>
            <li>
              Cable sizes used (especially where they differ from the design) and circuit references
              that match the cert schedule.
            </li>
            <li>
              Any deviations from the design and the reason for each (e.g. 'route changed to avoid
              existing gas pipe').
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              s.49 is the headline statutory duty for every consumer service contract, including
              electrical work. &quot;Reasonable care and skill&quot; means what a competent
              electrician would do &mdash; aligned with BS 7671, the manufacturer&apos;s
              instructions and current industry guidance. It can&apos;t be excluded by your T&amp;Cs
              (s.57). The customer&apos;s remedy when the standard isn&apos;t met is repeat
              performance (s.55) or a price reduction (s.56). The cert and the schedule of test
              results are your evidence that you discharged the duty.
            </>
          }
        ...
        <RegsCallout
          ...
          clause={
            <>
              &quot;Except where Regulation 644.4.201 applies, upon completion of the verification
              of a new installation or an addition or alteration to an existing installation,
              including the replacement of a distribution board or consumer unit, an Electrical
              Installation Certificate based on the model given in Appendix 6 shall be issued to the
              person ordering the work.&quot;
            </>
          }
          meaning={
            <>
              Reg 644.1 is the legal hook for the EIC. &quot;Person ordering the work&quot; is
              usually the customer for domestic work, often the principal contractor on commercial.
              Reg 644.3 then requires the certificate to be accompanied by the Schedule(s) of
              Inspection and the Schedule(s) of Circuit Details and Test Results &mdash; an EIC on
              its own without the schedules is incomplete and won&apos;t satisfy the reg or the
              contractor scheme audit. Issuing the cert is what closes the regulatory loop &mdash;
              without it the install is technically non-compliant even if the wiring is perfect.
            </>
          }
        ...
        >
          <p>A standard domestic handover pack includes:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Front sheet</strong> — summary of the install, date, your firm, contact
              details, a one-line emergency-contact card.
            </li>
            <li>
              <strong>Certificates</strong> — EIC (or MWC) plus the schedules. For an EICR visit,
              the EICR plus its schedule.
            </li>
            <li>
              <strong>Test results</strong> — the schedule of test results showing every circuit's
              R1+R2, IR, Zs etc.
            </li>
            <li>
              <strong>System spec</strong> — equipment installed, with model and serial numbers.
            </li>
            <li>
              <strong>Zone chart / circuit list</strong> — what each circuit serves, in plain
              English.
            </li>
            <li>
              <strong>Manufacturer manuals</strong> — for every piece of new kit (paper or links).
            </li>
            <li>
              <strong>Warranty info</strong> — registration confirmations, claim contacts, expiry
              dates.
            </li>
            <li>
              <strong>Maintenance schedule</strong> — what needs doing when (annual EICR if rented,
              fire-alarm service intervals, EV charger checks).
            </li>
            <li>
              <strong>Emergency card</strong> — cut-out location, main switch location, out-of-hours
              number.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          clause={
            <>
              Paraphrased: A diagram, chart or table (or equivalent form of information) shall be
              provided indicating in particular: the type and composition of each circuit; the
              method used for protection against electric shock; the information necessary for the
              identification of devices performing protection, isolation and switching; and any
              circuit or equipment vulnerable to a typical insulation or other test. For domestic
              premises a simplified equivalent is acceptable.
            </>
          }
          meaning={
            <>
              Reg 514.9.1 makes the as-installed schematic part of the install itself, not just the
              handover paperwork. On a CU change you typically fix a copy of the schematic inside
              the CU door &mdash; small, laminated, labelled per circuit. The customer&apos;s
              handover-pack copy is the same drawing in larger format. The reg ties the
              documentation requirement back to the consumer unit, where it&apos;s most useful in an
              emergency or fault-finding visit.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Customer rings the office at 11pm in a panic &mdash; smell of burning, sparks from a
              socket, can&apos;t find the main switch. The cert front sheet has &quot;cut-out
              location&quot; left blank because the apprentice was rushing. The customer searches
              the house for ten minutes while the fault gets worse. In the worst case it leads to a
              small fire that becomes a big fire because they couldn&apos;t kill the supply quickly.
              The complaint that follows points straight at the cert &mdash; &quot;why didn&apos;t
          ...
          doInstead={
            <>
              ...
              &quot;under-stairs cupboard, behind the boxing&quot;, &quot;external meter box, left
              of front door&quot;. Add the same information to the handover pack&apos;s emergency
              card and to the as-installed drawing. Walk the customer to the cut-out during the
              handover and physically point at it. Twenty seconds of pointing on the day saves
              twenty minutes of panic at 11pm.
            </>
          }
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Apprentice scribbles &quot;Kitchen rewire &mdash; £2,800&quot; on a quote pad, hands
              it over, customer agrees verbally. Two weeks in the customer says &quot;and the
              downlights, obviously&quot;. Apprentice didn&apos;t know downlights were in scope.
              There&apos;s no detail on the quote to fall back on. The customer escalates, leaves a
              one-star review, the firm absorbs the cost of supplying and fitting downlights to keep
              the peace.
            </>
          }
          doInstead={
            <>
              ...
              included, what&apos;s explicitly EXCLUDED. The exclusions list is the most important
              paragraph because it&apos;s where misunderstandings get prevented. Use a templated
              quote builder in your firm&apos;s field-service app, get the customer to sign (or
              accept by email) before tools come out, and any variation gets its own VO sheet signed
              before the variation work starts.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              Six months ago you installed a Grade D LD2 fire alarm system in a converted HMO
              &mdash; mains-powered interlinked smokes plus a heat in the kitchen. The customer
              rings the office today asking for the user manual because their letting agent wants to
              see the system spec before renewing the gas-safe and EICR cycle. Nobody at the office
              can find a copy.
            </>
          }
          whatToDo={
            <>
              Pull up the original handover pack &mdash; if your firm uses a field-service app, the
              PDF bundle should still be on the customer&apos;s record. The pack should contain:
              front sheet (system summary), the EIC for the install, the Grade D / LD2 system
              specification (manufacturer model, sensor positions), the zone chart showing where
              each device is fitted, the maintenance schedule (BS 5839-6 user-checks plus the annual
              contractor service), the manufacturer manual for the panel and the sounders, and the
              emergency contact card. Re-issue the whole pack to the customer by email and post a
              paper copy if they want one. If the original pack ISN&apos;T on file, that&apos;s a
              process gap to escalate &mdash; raise it with the supervisor so the firm&apos;s
              handover SOP gets updated to enforce digital archiving on every job.
            </>
          }
          whyItMatters={
            <>
              Fire-alarm handover packs are not optional &mdash; BS 5839-1 (commercial) and BS
              5839-6 (domestic) both require the user to be left with a logbook, a system
              specification and instructions. For a rented property the letting agent and the
              landlord both have legal duties under the Regulatory Reform (Fire Safety) Order 2005
              (HMOs) and the Smoke and Carbon Monoxide Alarm (England) Regulations 2015 (general
              lets) that depend on having the system paperwork. Losing the pack puts the landlord in
              regulatory breach and reflects badly on the installing firm. Defensible record
              retention is what stops this being a one-customer problem and starts being an audit
              issue across every job.
            </>
          }
        />
