...
const checks = [
  ...
  },
  {
    ...
    question: 'A 4 kW domestic solar PV array generates electricity at the panel as:',
    options: [
...
const quizQuestions = [
  {
    ...
    options: ['Coal', 'Gas (CCGT)', 'Nuclear', 'Onshore wind'],
    correctAnswer: 1,
    explanation:
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 3 · Section 5 · Subsection 1"
        title="Methods of generating electricity for distribution"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Six ways the UK makes the electricity that ends up at your CU. The mix matters because it
          changes when the electrician works (peak vs off-peak), what protection devices have to
          handle (DC injection from PV inverters), and how the wider network behaves on a still,
          dark winter evening.
        </p>
        ...
        >
          <p>
            Every electrical generation method on the planet does the same trick at the business
            end: it spins a generator (the AC alternator from Module 2 Sub 5.5) and produces
            three-phase AC. The differences are upstream — what energy is used to spin the shaft.
            That’s what splits the methods into thermal, nuclear, renewable and hybrid categories.
          </p>
          <p>
            Solar PV is the one exception. It produces DC directly from sunlight and uses an
            inverter to convert to AC. We’ll cover that separately because the inverter is the bit
            that affects what protective devices the electrician fits.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            Natural gas (mostly methane) burns in an aircraft-derivative turbine that drives one
            generator. The exhaust gases — still around 600 °C — pass through a heat recovery boiler
            to raise steam, which drives a second turbine and a second generator. Two electrical
            outputs from one fuel input.
          </p>
          <p>
            Round figures: a typical UK CCGT unit is 400–800 MW, runs at 55–60% thermal efficiency,
            and can ramp from cold to full output in under an hour. CCGTs currently supply roughly a
            third to 40% of GB annual electricity — the biggest single share since coal closed in
            2024.
          </p>
          <p>
            <strong>Open-cycle gas turbines (OCGT)</strong> are the same idea without the steam
            recovery side — simpler, cheaper, dirtier, and faster to start. They sit on standby as
            peakers, fire up for the morning teatime kettle peak, and shut down again. Less common
            than CCGT but they exist (e.g. Indian Queens in Cornwall).
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Nuclear plants run as <strong>baseload</strong> — they sit at maximum continuous output
            for as long as the fuel cycle and maintenance schedule allow (often eighteen months
            between refuels). They’re mechanically and economically built to run flat-out. Cycling
            them up and down is technically possible but rarely done in the UK; gas does the swing
            job.
          </p>
          <p>
            The economics are upside-down compared with gas: very high capital cost (Hinkley Point C
            is a £35bn build), very low fuel cost per kWh, very long operating life (60+ years).
            That’s why governments rather than private investors tend to underwrite new nuclear —
            the payback runs to decades.
          </p>
          <p>
            The output at the alternator terminals is identical to a CCGT — 26 kV three-phase 50 Hz,
            stepped up to 400 kV by an on-site transformer. Once it’s on the grid the electrons
            don’t carry a label saying which fuel made them.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            A modern utility wind turbine is huge — Dogger Bank machines have 220 m blades and stand
            higher than the Shard. Each one is rated 13–15 MW. The generator inside the nacelle is
            usually a permanent-magnet synchronous machine, with power electronics (a back-to-back
            converter) on the output to decouple the rotor speed from the grid frequency.
          </p>
          <p>
            Wind is <strong>non-dispatchable</strong> — operators can’t order more wind when demand
            spikes. If the wind drops, gas and pumped-storage hydro have to fill the gap within
            seconds to keep frequency at 50 Hz. The grid operator (NESO) does this balancing minute
            by minute through the Balancing Mechanism — paying dispatchable plant to ramp up or down
            on short notice.
          </p>
          <p>
            Offshore export is at high voltage AC for shorter routes (33–220 kV through subsea cable
            to a coastal substation) and HVDC for the longest routes — Dogger Bank uses HVDC because
            the cable run is over 130 km, where AC would lose too much to capacitive charging.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Why the electrician needs to care about the inverter:</strong> a PV inverter can
            leak smooth (non-pulsating) DC residual current onto the AC side under certain fault
            conditions. A standard Type AC RCD is blind to that — DC saturates the toroid core and
            freezes the trip. BS 7671 Section 712 (PV systems) requires a{' '}
            <strong>Type B RCD</strong> where smooth DC is possible, or Type F where only pulsating
            DC (from a half-bridge inverter) is. This is one of the few places in an install where
            the choice of generation method directly drives the protective device specification.
          </p>
          <p>
            Utility solar farms can be 50–350 MW. Cleve Hill in Kent (373 MW) is the largest. They
            use central inverters in containerised cabins rather than the microinverters or string
            inverters you see on roofs. Output is typically combined at 33 kV and fed straight into
            the distribution network at a primary substation.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Conventional hydro</strong> diverts water from a river or impoundment reservoir
            through a turbine and back to the watercourse. Capacity factor is set by rainfall — high
            in winter, low in summer.
          </p>
          <p>
            <strong>Pumped storage</strong> is the more interesting trick. Two reservoirs at
            different heights. At off-peak (3am, low demand, cheap power) the turbines run backwards
            as pumps, lifting water from the bottom reservoir to the top. At peak demand (kettle-on
            after the news) gravity does its bit: water falls back through the same turbines, now
            generating, into the bottom reservoir. Round-trip efficiency is around 75% — you get
            back roughly three quarters of the electricity you put in. Pumped storage exists to
            time-shift cheap baseload into expensive peak supply, and to provide the fastest
            response on the network when wind output drops or a CCGT trips.
          </p>
          <p>
            Dinorwig is hidden inside a Welsh mountain (Elidir Fawr). Six 300 MW Francis-type
            pump-turbines in a cavern the size of a cathedral. Built in the late 1970s when the grid
            needed somewhere to dump cheap nuclear at night and recover it at peak. Forty years on,
            it’s still doing the same job — just soaking up wind output instead of nuclear.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              <strong>49.5 Hz to 50.5 Hz</strong>. NESO actually holds the grid much tighter than
              that — within ±0.2 Hz under normal conditions. When wind drops or a generator trips,
              pumped-storage hydro and gas plants ramp within seconds to keep generation matched to
              demand and frequency inside the legal window. The generation mix isn’t just an
              environmental story — it’s a statutory frequency-control requirement.
            </>
          }
        ...
        >
          <p>
            CHP makes economic sense only where there is a constant, year-round demand for the heat.
            A hospital is the classic example — hot water, sterilisation steam and space heating run
            24/7/365. A factory boiler house with continuous process demand is the same idea at
            industrial scale.
          </p>
          <p>
            Electrically, a CHP unit is just a gas turbine + alternator (or a reciprocating gas
            engine + alternator on smaller units). The thermal kit on the back end — heat
            exchangers, hot-water flow and return loop, sometimes an absorption chiller for cooling
            — is what makes it CHP rather than plain power generation.
          </p>
          <p>
            Most CHP units run grid-connected and export any surplus electricity, but they can also
            run in island mode if the grid drops — useful for hospitals and data centres where the
            heating system needs to keep running through a power cut.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>
                Imports (interconnectors to France, Norway, Belgium, Netherlands, Denmark):
              </strong>{' '}
              ~10–12%
            </li>
            <li>
          ...
          <p>
            Two trends matter for the tools. First, more <strong>inverter-based generation</strong>{' '}
            (PV, battery storage, HVDC links, modern wind) means more harmonics, more DC residual
            current potential, and less rotational inertia in the grid — which makes frequency
            stability harder to maintain. Second, more
            <strong> behind-the-meter generation</strong> (rooftop PV, home batteries) means local
            distribution feeders see two-way power flow for the first time in their lives, which
            changes how voltage drop and protective device coordination have to be designed.
          </p>
          <p>
            For the next decade you can expect: more offshore wind, the first new nuclear in a
            generation (Hinkley C, Sizewell C, possibly SMRs), big growth in grid-scale battery
            storage, continued retirement of older CCGTs, and effectively zero return of coal.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              This is the classic example of generation method driving protective device choice. A
              grid-tied PV array is the only common domestic install where you have to think about{' '}
              <strong>DC fault current on an AC circuit</strong>. Get the RCD wrong (fitting Type AC
              where the inverter can produce smooth DC) and the RCD will fail to trip on a fault,
              leaving the touch voltage live. Always check the inverter datasheet — most modern
              transformerless inverters explicitly state ‘Type B RCD required’ or ‘internal Type B
              equivalent fitted’.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              Apprentice answers an exam question about generation by saying ‘the grid produces
              electricity’ as if the grid were a fuel source. The grid is the wires — it doesn’t
              produce anything. Electricity is produced by the six methods covered in this Sub, then
              injected into the grid. Mixing up the generator (fuel + alternator) with the network
              (transmission + distribution wires) loses easy marks and shows up on site when you
              can’t tell a customer which actual plants are running on a given evening. Real
              example: an electrician quotes a customer ‘the grid is mostly gas these days’ during a
              Heat Pump survey — but it’s a windy Sunday and wind is supplying 50% of the mix at
              that moment. The customer has the National Grid Carbon Intensity dashboard open on
              their phone, sees the figures don’t match what you said, and now distrusts every other
              number you quote on the job.
            </>
          }
          doInstead={
            <>
              Use the right vocabulary. ‘Generated by [method] at [location]. Transmitted at
              [voltage] through National Grid wires. Distributed at [voltage] through [DNO] network.
              Delivered at the cut-out at 230 V single-phase.’ Each stage owned by a different
              organisation, governed by different regulations. Section 5 covers all four stages —
              keep them separate in your head. And if you’re going to quote live mix figures to a
              customer, check the Carbon Intensity API or the NESO live dashboard first — both are
              free and update every half hour.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You’re commissioning a 4 kW domestic PV install. The customer mentions that their
              neighbour’s identical system regularly drops to zero export at lunchtime on the
              sunniest days of the year, even though the panels are obviously generating. They ask
              if you’ll have the same problem and what causes it.
            </>
          }
          whatToDo={
            <>
              It’s the inverter doing exactly what BS 7671 and ENA G98/G99 told it to do. When local
              distribution voltage rises above the upper statutory limit (~253 V at the supply
              terminals), the inverter’s anti-islanding and overvoltage protection trips it offline
              to stop pushing the voltage even higher and damaging neighbours’ kit. On a sunny
              midday, with all the rooftop PV in the street exporting at once and demand low, the
              local 400 V feeder voltage rises — at some point the first inverter on the feeder hits
              its trip limit and shuts down. Standard behaviour, not a fault. Mitigations are
              limited to: clipping the export limit, installing a battery to soak up the midday
              generation locally, or asking the DNO to look at the feeder voltage profile (rare —
              they’ll just say it’s within the legal window).
            </>
          }
          whyItMatters={
            <>
              This is what ‘non-dispatchable, inverter-based, behind-the-meter generation’ means in
              practice. The customer thought they’d bought a fixed asset — 4 kW of generation. What
              they’ve actually bought is up to 4 kW of generation, subject to the local feeder
              voltage and the inverter’s protective settings. The mix and topology of generation in
              their street determines what they actually get. That’s why the electrician needs to
              understand the grid context, not just wire the panels up and walk away.
            </>
          }
        />
        ...
        <Quiz title="Generation methods — knowledge check" questions={quizQuestions} />
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          <button
            onClick={() => navigate('/study-centre/apprentice/level2/module3/section5/5-2')}
