...
const TITLE = 'Battery storage deep dive (6.1, 6.2) | Level 2 Module 3.6.3 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  ...
  {
    id: 7,
    question: 'Why is "thermal runaway" the headline safety concern with lithium batteries?',
    options: [
...
const faqs = [
  ...
  {
    question: 'What does BS EN 62619 cover and where does it apply to the install?',
    answer:
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Battery storage is the fastest-growing micro-generation install in the UK after PV.
          Lithium iron phosphate (LFP) plus AC- or DC-coupled to a PV system. The reg framework is
          still maturing — Chapter 57 (Stationary Secondary Batteries) is now the home, with PAS
          63100, BS EN 62619 and the IET Code of Practice for EESS sitting alongside.
        </p>
        ...
        >
          <p>
            ...
            written for PV, Chapter 53 covered general isolation, and the rest came from
            manufacturer instructions and the IET Code of Practice for EESS. The A4:2026 amendment
            to BS 7671 changed that with the introduction of Chapter 57 (Stationary Secondary
            Batteries) — the first dedicated reg framework for domestic and small commercial battery
            installs.
          </p>
          <p>
        ...
        >
          <p>Three chemistry families dominate the UK market:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              5,000–10,000 cycles to 80 % capacity. Thermal runaway threshold around 270 °C. Tends
              to vent rather than ignite. The default for in-home storage.
            </li>
            <li>
              <strong>Nickel manganese cobalt (NMC)</strong> — higher energy density (150–250
              Wh/kg), which is why it dominates EV traction packs. Cycle life 1,500–3,000. Thermal
              runaway threshold around 200 °C. Higher fire risk if the BMS fails or the pack is
              damaged. Used in some domestic products but losing market share to LFP for in-home
              use.
            </li>
            <li>
              <strong>Lead-acid</strong> — flooded, AGM (absorbed glass mat) or gel. Energy density
              30–40 Wh/kg. Cycle life 500–1,500 cycles, and only with shallow discharge. Vents
              hydrogen during charging — must be installed in ventilated spaces away from ignition
              sources. Survives on legacy off-grid systems and as back-up batteries on emergency
              lighting.
            </li>
          </ul>
          <p>
            For any new domestic install in 2026, the realistic chemistry choice is LFP unless the
            customer has a specific reason for an NMC product. The fire-safety story alone is enough
            to make LFP the default — it is the same chemistry the National Fire Chiefs Council and
            most domestic battery manufacturers now recommend for in-home use.
          </p>
        </ConceptBlock>
        ...
        >
          <p>Three numbers describe a battery pack:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Capacity (kWh):</strong> how much energy the pack stores. Typical domestic
              ratings sit at 5, 10 or 15 kWh of usable capacity. Note "usable" — manufacturers often
              quote nameplate capacity above the actual usable figure because the BMS keeps the pack
              between roughly 5 % and 95 % state of charge to preserve cycle life.
            </li>
            <li>
              <strong>Continuous power (kW):</strong> how fast the pack can deliver or absorb
              energy. Most domestic packs sit at 3–7 kW continuous. A bigger inverter does not help
              if the battery cannot deliver the current.
            </li>
            <li>
              <strong>C-rate:</strong> power divided by capacity. A 10 kWh pack discharging at 5 kW
              is at 0.5C — would take 2 hours to fully discharge. Higher C-rates heat the cells
              harder and shorten cycle life. Most domestic batteries are designed for continuous
              0.5C with brief peaks at 1C.
            </li>
          </ul>
          <p>
            The numbers matter because customers often ask for "a 15 kWh battery" without knowing
            whether they need the energy capacity or the power capacity. A 15 kWh pack with 5 kW
            power output runs the house overnight. A 10 kWh pack with 7 kW output runs the EV
            charger and the oven simultaneously for short periods. Understand which one the customer
            actually needs before specifying.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>Measures the current flowing in and out of the pack.</li>
            <li>Measures temperatures at multiple points across the pack.</li>
            <li>
              Balances cells during charging — gently bleeding charge from the highest-voltage cells
              so the lowest catch up.
            </li>
            <li>
              Cuts off charge or discharge if any cell goes outside the safe voltage, temperature or
              current window.
            </li>
            <li>Reports state of charge, state of health and any fault codes to the inverter.</li>
          </ul>
          <p>
            The BMS is the single most important piece of electronics on the pack. PAS 63100 and the
            IET Code of Practice for EESS both treat it as the primary safety control. On a domestic
            install you never service or modify the BMS — if it faults, the whole pack goes back to
            the manufacturer or to recycling.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>DC-coupled.</strong> The PV strings and the battery share the DC bus inside one
            hybrid inverter. Sunlight charges the battery directly at DC; the inverter only converts
            to AC when the battery discharges to the load or to export. One conversion step instead
            of three, so round-trip efficiency lands at 90–95 %. The right choice for any new
            install where PV and battery go in together.
          </p>
          <p>
            <strong>AC-coupled.</strong> The battery has its own inverter and connects to the AC
            side of the consumer unit, alongside an existing PV inverter. PV-DC is converted to AC
            by the PV inverter, then back to DC to charge the battery, then back to AC to use it —
            three conversion steps, round-trip efficiency around 85–88 %. The right choice when
            adding a battery to an existing PV install where the PV inverter is already installed
            and the customer does not want to replace it.
          </p>
          <p>
            Some installs use a third option — a stand-alone battery with its own inverter, no PV at
            all, charging from the grid on time-of-use tariffs (off-peak overnight) and discharging
            during peak hours. The economics can work if the tariff has a wide off-peak / peak gap.
            From an install point of view this is identical to AC-coupling minus the PV inverter.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              lead-acid does during charging). On any new install, the chemistry choice should be a
              deliberate decision documented in the design — not an accident of whichever pack the
              wholesaler had in stock.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Same wording, same logic as Reg 712.531.3.5.1 for PV. A transformerless battery PCE
              (or hybrid inverter) can leak smooth DC fault current onto the AC side, which blinds
              Type AC and Type A RCDs. Default is Type B unless one of the three exceptions applies.
              With nothing in the manual, fit Type B.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Two isolators on the battery DC circuit: one at the battery end and one at the PCE
              end. The "either end" rule matters because the cable between them can fault and you
              need to be able to break the circuit from either side without relying on the other.
              Many modern battery products integrate the battery-end isolator inside the pack
              enclosure; if so, that satisfies the requirement at that end. If not, an external DC
        ...
        <RegsCallout
          ...
          meaning={
            <>
              For dwellings, this reg makes PAS 63100:2024 ("Electrical installations. Protection
              against fire of battery energy storage systems intended for use in dwellings") a
              normative reference. PAS 63100 governs where the battery can go in a house — not above
              habitable rooms in a loft, not in escape routes, not in bedrooms, with a minimum
              separation from the dwelling boundary, with smoke / heat detection in the same room,
              and with a non-combustible enclosure or location. Treat PAS 63100 as a required read
              for any in-dwelling battery install.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              EESS is the practical install handbook the trade uses — covers things the regs do not
              spell out, like cable management between battery and PCE, cooling considerations,
              commissioning checks, handover documentation and ongoing maintenance. BS EN 62619 is
              the cell-level safety standard the manufacturer must conform to before the product can
              be sold for stationary use; you will not test it on site, but you should check the
              battery datasheet declares conformance.
            </>
          }
        ...
        >
          <p>The chain that triggers thermal runaway:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              A cell sees a stress event — over-charge, over-discharge, internal short from cell
              defect or external impact, or external heat from a fire elsewhere.
            </li>
            ...
            <li>
              ...
              oxygen as it decomposes — water and conventional extinguishers cool the surroundings
              but do not stop the cell-level reaction.
            </li>
          </ul>
          <p>
            The mitigation chain is the chain that BS 7671 Chapter 57, PAS 63100 and BS EN 62619
            collectively try to enforce:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Chemistry choice</strong> — LFP raises the runaway threshold and reduces the
            ...
            <li>
              <strong>Enclosure and location</strong> — non-combustible enclosure, segregated from
              sleeping areas and escape routes, with adequate ventilation. PAS 63100 sets the rules.
            </li>
            <li>
              <strong>Detection</strong> — smoke and heat detection in the room, alerting occupants
              in time to escape.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <p>
            In practice, most modern hybrid inverters on PV plus battery installs sit at 5 kW or 6
            kW (24–26 A per phase at 230 V), which falls into G99. The G99 application includes:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>
            The DNO assesses whether the local LV network can take the export, sets the protection
            settings, and issues a connection agreement. Only after that agreement is in place does
            the installer commission the system. Skipping the G99 process is not a paperwork lapse —
            it is illegal under the Electricity Safety, Quality and Continuity Regulations (ESQCR)
            and the DNO can require disconnection.
          </p>
        </ConceptBlock>
        ...
        >
          <p>A reasonable battery commissioning checklist:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              Visual inspection — pack mounted per manufacturer instructions, ventilation clearances
              correct, non-combustible substrate, separation from sleeping accommodation per PAS
              63100.
            </li>
            <li>
            ...
            <li>
              DC isolation test — both isolators (at battery and at PCE per Reg 570.6.5) operate,
              prove dead procedure can be performed at both.
            </li>
            <li>
              BMS communication — inverter sees the battery, reports correct chemistry, capacity,
              voltage, no fault codes.
            </li>
            <li>
              Anti-islanding test — disconnect the grid (open the AC isolator), confirm the inverter
              shuts down within roughly 200 ms, reconnect and confirm grid synchronisation.
            </li>
            <li>
              Charge / discharge test — battery accepts charge from PV (or grid), delivers charge to
              load, state of charge updates correctly.
            </li>
            <li>
            ...
            <li>
              Customer demonstration — how to read state of charge, how to isolate in emergency,
              fire safety briefing, what to do if a fault code appears.
            </li>
            <li>
              Handover pack — manuals, MCS certificate (if applicable), G99 connection agreement,
              EIC, commissioning records, scheduled maintenance plan.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>When a domestic battery reaches end of life, three things happen in sequence:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Decommissioning.</strong> The battery is isolated at both ends per Reg
              570.6.5, the pack is fully discharged where the manufacturer permits, and the wiring
              is removed.
            </li>
            <li>
            ...
            <li>
              <strong>Recycling.</strong> Lithium batteries fall under the UK Waste Batteries and
              Accumulators Regulations 2009 and the WEEE directive. They go to a registered battery
              recycler — never to landfill, never to general waste, never left at a kerbside
              collection. Manufacturers and producers have legal take-back obligations under these
              regulations.
            </li>
          </ul>
          <p>
            The expected service life depends on chemistry, cycle depth and temperature: 7–10 years
            for typical NMC packs, 10–15 years for LFP packs operated within their spec. The
            customer should know at quote that the battery is a replaceable consumable on a
            10-ish-year cycle, not a fit-and-forget asset.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You quote and fit a 5 kW hybrid inverter with a 10 kWh battery on a domestic PV plus
              storage install. The inverter exports up to 24 A per phase. You assume G98 covers it
              because the customer "only has a small battery". Six months later, the DNO discovers
              the install during a network audit and requires immediate disconnection plus a
              retrospective G99 application. Customer is without their system for weeks; you are
              facing a difficult conversation about who pays for the rework.
            </>
          }
          doInstead={
            <>
              Check the inverter export rating in amps per phase before quoting. Anything above 16 A
              per phase is G99 territory regardless of whether it is PV-only, battery-only or
              hybrid. Submit the G99 application before installing — the DNO sets protection
              settings, agrees the connection, and issues the connection agreement. Only then do you
              commission. The G99 process takes weeks, so it has to be in the project plan from day
              one. A 3.68 kW single-phase inverter (16 A per phase) stays in G98; anything bigger
              needs G99.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              A customer fitted a 4 kW PV system in 2018 — single string, string inverter, 16 A per
              phase export, G98 notification on file. Now wants to add a 10 kWh battery for evening
              self-consumption. The existing inverter is healthy and has years of life left.
            </>
          }
          whatToDo={
            <>
              Two options. AC-couple the battery — fit a separate battery inverter on the AC side of
              the consumer unit, alongside the existing PV inverter. Three conversion steps so
              round-trip efficiency lands at 85–88 %. Cheapest because the existing PV inverter
              stays in place. Or replace the existing PV inverter with a hybrid that handles both PV
              and battery on a shared DC bus. One conversion step so round-trip 90–95 %, but you
              have written off a working inverter that has years of useful life. For most customers
              in this scenario, AC-coupling is the right economic call — the efficiency gain from
              DC-coupling rarely justifies replacing a working PV inverter. Either way, check the
              resulting export current per phase — adding a battery inverter can push the install
              into G99 territory and trigger a fresh DNO application. Document the upgrade with an
              EIC for the new work and update the G98 notification (or submit a fresh G99 if the
