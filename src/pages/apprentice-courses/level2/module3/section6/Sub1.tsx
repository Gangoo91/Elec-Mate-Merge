...
const TITLE = 'Types of micro-renewable energies (6.1) | Level 2 Module 3.6.1 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  ...
  {
    id: 6,
    question: 'Micro-CHP (Combined Heat and Power) at domestic scale — what’s the principle?',
    options: [
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 3 · Section 6 · Subsection 1"
        title="Types of micro-renewable energies"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Six micro-generation technologies a UK electrician might wire — solar PV, wind,
          micro-hydro, micro-CHP, heat pumps and battery storage. Each one has a typical scale, a
          typical UK install context, and a BS 7671 (or ENA) document that governs how it gets
          connected.
        </p>
        ...
        >
          <p>
            In Section 5 you traced electricity from the power station down to the cut-out —
            generation, transmission, distribution, the meter. In Section 6 you stay at the cut-out
            and look the other way. When a customer fits PV, a battery, a heat pump or any other
            micro-generator, your installation stops being a one-way load. It becomes a small
            generator in its own right, exporting power back up the same cables that fed it.
          </p>
          <p>
            That’s why the rules tighten. The DNO doesn’t want surprises on its network, the
            customer doesn’t want to electrocute the linesman trying to fix a fault on the street,
            and BS 7671 doesn’t want you fitting kit that can’t be safely isolated. Sub 6.1
            introduces the kit. Sub 6.2 covers the connection paperwork. Sub 6.3 weighs up the
            trade-offs.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            A photovoltaic cell is a thin slice of doped silicon. When daylight hits it, electrons
            are knocked loose and a small DC voltage appears across the cell — typically about 0.5
            V. Wire 60–72 cells in series and you get a panel at roughly 30–50 V open-circuit. Wire
            8–12 panels in series and you get a “string” at 300–500 V DC, which is then fed into an
            inverter that converts it to grid-synchronous 230 V 50 Hz AC.
          </p>
          <p>Typical UK domestic install:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Power:</strong> 3.68 kW or 4 kW (deliberately just under the 16 A per phase
              G98 threshold).
            </li>
            <li>
              <strong>Panels:</strong> 10–12 modules at 380–450 W each, on a south or south-west
              facing pitched roof.
            </li>
            <li>
              <strong>DC voltage:</strong> One or two strings at 300–500 V DC running into the
              inverter.
            </li>
            <li>
            ...
            <li>
              <strong>Connection:</strong> Dedicated MCB/RCBO in the consumer unit, fed with 6 mm²
              T&amp;E or similar.
            </li>
          </ul>
          <p>
            That covers the “ordinary” job. Bigger installs (commercial roofs, ground-mount farms)
            are three-phase and run into G99 territory rather than G98 — covered in Sub 6.2.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Section 712 is the home regulation for every UK domestic and small-commercial PV
              install. From the panel itself, through the DC string cables, the DC isolator, the
              inverter, the AC isolator and into the consumer unit — all of it falls inside Section
              712. Sub 6.2 unpacks the specific installation rules. For now, just lock the section
              number in: PV = 712.
            </>
          }
        ...
        >
          <p>
            A small wind turbine is mechanically the same beast as the multi-megawatt machines on
            offshore farms — a rotor coupled to a generator on top of a mast — just shrunk to
            domestic or agricultural scale. The output is variable AC (frequency rises and falls
            with wind speed), which is why most modern small-wind systems rectify to DC and then
            re-invert to grid-synchronous 230 V 50 Hz, the same way a hybrid PV inverter does.
          </p>
          <p>Typical UK install scenarios:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>1–6 kW:</strong> A pole-mounted turbine on an exposed rural property —
              single-phase, G98 territory if ≤ 16 A per phase.
            </li>
            <li>
              <strong>6–50 kW:</strong> Farm-scale on a tall mast — three-phase, G99 application
              required.
            </li>
            <li>
              <strong>Over 50 kW:</strong> Approaching commercial scale — different planning regime,
              more involved DNO process.
            </li>
          </ul>
          <p>
            Wind doesn’t have a dedicated BS 7671 Part 7 section the way PV does. The connection
            rules come from Section 551 (low voltage generating sets), specifically 551.7 (parallel
            operation), plus the ENA G98 or G99 documents covered in Sub 6.2.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            A micro-hydro system uses falling water — a stream with a head of a few metres or more —
            to spin a turbine, which spins a generator. Output is then conditioned (rectifier +
            inverter, just like wind) and synchronised to the grid. Realistic domestic/private scale
            is 1–100 kW depending on the available head and flow.
          </p>
          <p>
            Practical reality: micro-hydro is rare in the UK. You need a stream with reliable
            year-round flow, a useful drop, a Water Resources Act abstraction licence (issued by the
            Environment Agency or its devolved equivalents), planning permission, and either a
            willing landowner or the rights to do work on someone else’s. Where all those line up,
            the kit looks similar to wind from the electrician’s point of view: an inverter, a DC
            isolator, a grid-tie connection, and the same G98/G99 paperwork depending on output.
          </p>
          <p>
            Apprentice expectation: know it exists, know the rough scale, know it falls under
            Section 551 and ENA G98/G99 like wind does. You won’t commission one in your first five
            years on site, but you should be able to recognise the kit if you ever see it.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            “Combined Heat and Power” means producing electricity and useful heat from the same
            fuel. At power-station scale that’s gas-turbine CHP. At domestic scale it’s a small
            Stirling engine or a fuel cell sitting where your boiler used to be. You burn natural
            gas (or LPG, or hydrogen in future), capture the heat for the central heating and DHW,
            and capture the small electrical output to run the house or export.
          </p>
          <p>Typical units:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Stirling-engine micro-CHP:</strong> ~1 kWe electrical + ~5–12 kWth thermal.
              Domestic scale.
            </li>
            <li>
              <strong>Fuel-cell micro-CHP:</strong> ~0.7–1.5 kWe + ~1–2 kWth. More
              electrically-biased.
            </li>
            <li>
              <strong>Larger commercial/care-home CHP:</strong> 5–50 kWe + matched thermal.
              Three-phase, G99.
            </li>
          </ul>
          <p>
            From the electrician’s side, the install is similar to PV at the connection point — an
            inverter, a dedicated MCB/RCBO, anti-islanding protection, the same G98 or G99
            paperwork. The difference is everything upstream of the inverter is a gas appliance,
            which means a Gas Safe engineer is involved as well. CHP is a multi-trade install.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The trick is the refrigerant cycle. A compressor drives a refrigerant around a closed
            loop: the refrigerant evaporates outside (absorbing heat from the air or ground),
            compresses, condenses inside (releasing heat to the central heating water), and expands
            back outside. The electricity drives the compressor; the heat itself is moved, not
            created.
          </p>
          <p>
            The performance metric is the <strong>Coefficient of Performance (CoP)</strong> — heat
            output ÷ electrical input. A modern ASHP averages a CoP around 3 across a UK heating
            season. A GSHP runs closer to 4 because the ground stays warmer than winter air. Either
            way, the renewable bit is the heat scavenged from the environment — typically two-thirds
            to three-quarters of the heat delivered.
          </p>
          <p>From the electrician’s point of view:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Dedicated circuit:</strong> 32 A radial in 6 mm² T&amp;E (sometimes 4 mm²
              depending on the unit’s rated current and install method) from the consumer unit.
            </li>
            <li>
            ...
            <li>
              <strong>RCD:</strong> Type A typically; check the manufacturer’s instructions because
              some inverters require Type B.
            </li>
            <li>
              <strong>Supply check:</strong> Always do a maximum-demand calc. ASHP plus shower plus
              oven plus EV charger will exceed an 80 A or even 100 A main fuse on some installs.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>A hybrid inverter</strong> handling PV plus battery in one box (the most
              common install today), or
            </li>
            <li>
              <strong>An AC-coupled battery system</strong> with its own inverter, sitting alongside
              an existing PV string inverter.
            </li>
          </ul>
          <p>
            The chemistry matters. Lead-acid batteries (open or VRLA) still appear on off-grid
            systems and older installs, but their cycle life is short and their depth-of-discharge
            is poor. Lithium-ion (specifically LiFePO₄ — lithium iron phosphate) has taken over the
            domestic market because it’s safer, lasts longer (5,000–10,000 cycles), and gives
            near-100% useful depth of discharge.
          </p>
          <p>From a regulations point of view, a battery is another DC source. That means:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              DC isolation requirements (Section 712.537 territory — same family of rules as PV).
            </li>
            <li>DC overcurrent protection on the battery cabling.</li>
            <li>Anti-islanding on the inverter (G98/G99 — same as PV).</li>
            <li>
              Fire-segregation considerations — non-combustible enclosure, location away from
              sleeping accommodation, often with a smoke/heat detector nearby.
            </li>
            <li>
              An MCS-listed installer for any install that wants to claim Smart Export Guarantee
              (SEG) payments.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The single most important thing to internalise about DC-coupled generation. Pulling
              the AC isolator at the consumer unit doesn’t kill the panels. As long as daylight hits
              a PV string or a battery has charge, the DC side is live. Any work on the DC side —
              combiner box, string cables, isolator, inverter terminals — needs the DC isolator open
              and proven dead with a proper DC voltmeter, not just the AC side off.
            </>
          }
        ...
        >
          <p>
            Solar thermal isn’t in your scope as an electrical apprentice — it’s a plumbing job —
            but it’s worth knowing what it is so you can tell a customer apart from a PV install:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Solar PV:</strong> Sealed silicon panels, electrical output, fed via an
              inverter into the consumer unit. BS 7671 Section 712. Your job.
            </li>
            <li>
              <strong>Solar thermal:</strong> Glazed flat plates or evacuated tubes carrying a
              glycol fluid loop, plumbed to a coil in the hot water cylinder. Plumbing job. The only
              electrical involvement is a small pump (often a 230 V circulating pump or a
              low-voltage DC pump on a PV-powered control board) and a controller.
            </li>
          </ul>
          <p>
            If the customer’s after solar thermal, the right move is to refer them to a heating
            engineer or MCS-registered plumber who installs that kit. You might still wire up the
            small pump-and-controller circuit, but the design and the bulk of the install is
            plumbing.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              assumption. You turn up to survey, and the customer means solar thermal — they want
              their hot water heated, not their lights powered. You’ve wasted half a day, and
              they’re now confused because your quote and their expectation don’t line up.
            </>
          }
          doInstead={
            <>
              On the first phone call, ask: “Are you after solar PV — that’s panels that generate
              electricity — or solar thermal, which heats your hot water?” Most customers haven’t
              thought about the distinction. A 30-second clarifying question saves a half-day site
              visit and keeps you out of a job that isn’t in your scope.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You’re shadowing on a survey at a rural farmhouse. The owner wants to “go renewable”
              and has a vague idea about solar panels. The site has: a south-facing slate roof, a
              small fast-flowing stream with about 8 m of head running through the property, an
              oil-fired central heating system, and an 80 A single-phase supply. He’s asking what
              the electrician recommends.
            </>
          }
          whatToDo={
            <>
              Walk through the realistic options. PV on the south roof is the obvious headline — a 4
              kW system would slot under G98. An ASHP could replace the oil boiler — but it’s a big
              call because the radiators and insulation may need upgrading, and the maximum demand
              check on the 80 A supply needs doing carefully (existing loads + ASHP could exceed
              it). Micro-hydro on the stream is technically possible but requires an Environment
              Agency abstraction licence and specialist installers — flag it as “interesting but
              expensive to investigate, get a hydro specialist in”. A battery makes sense alongside
              the PV. Don’t commit to anything without proper design — set the customer’s
              expectations, get the right specialists involved.
            </>
          }
          whyItMatters={
            <>
              Renewable surveys are where the apprentice-electrician conversation has to be honest.
              Customers hear “renewable” and assume it’s all the same job. Knowing which technology
              fits which site — and which ones aren’t worth pursuing — is what marks out a competent
              first-year electrician from someone repeating sales-brochure copy.
            </>
          }
        />
