...
const TITLE = 'Solar PV deep dive (6.1, 6.2) | Level 2 Module 3.6.2 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  ...
  {
    id: 3,
    question: 'What is the role of an MPPT (Maximum Power Point Tracker) inside a string inverter?',
    options: [
...
export default function Sub2() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 3 · Section 6 · Subsection 2"
        title="Solar PV deep dive"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Solar PV is the dominant micro-generation install in the UK. Most other micro-renewable
          kit is rare — PV is what an electrician will likely see weekly. This Sub goes deep on the
          silicon, the strings, the inverters, BS 7671 Section 712 reg by reg, and the failure modes
          you actually meet on UK roofs.
        </p>
        ...
        >
          <p>
            Sub 6.1 walked through six categories of micro-generation with a paragraph on each. PV
            got the headline because it is genuinely the dominant install — tens of thousands of UK
            roofs go up every year, against a few hundred small wind turbines and a handful of
            micro-hydro schemes. This Sub stays on PV from physics to paperwork.
          </p>
          <p>
        ...
        >
          <p>
            ...
            face to be n-type (extra free electrons) and on the other face to be p-type (extra holes
            — missing electrons). At the boundary between them sits a p-n junction with a built-in
            electric field across it.
          </p>
          <p>
            When a photon of sunlight strikes the silicon and carries enough energy to bridge the
            roughly 1.1 eV band gap, it knocks an electron out of a covalent bond, leaving behind a
            positively charged hole. The junction field sweeps the electron one way and the hole the
            other. Wire the front and back of the cell to a circuit and the freed electrons flow
            round the external loop — a small DC current at roughly 0.5–0.6 V per cell.
          </p>
          <p>
            ...
            constant across a wide range of irradiance. Second, the cell never switches off. As long
            as photons land on it, electrons flow. Cover the panels, shade them with a tarpaulin,
            work at night — those are your only real ways of de-energising a string.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            panel at roughly 30–45 V open-circuit and 8–11 A short-circuit, sealed under tempered
            glass with an EVA encapsulant and a polymer backsheet. Two cell technologies dominate:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Mono-crystalline</strong> — each cell cut from a single silicon crystal ingot.
              Higher efficiency (typically 20–22 % at the panel level), uniform black appearance,
              slightly more expensive. The default on new UK installs.
            </li>
            <li>
              <strong>Poly-crystalline</strong> — cells cast from multiple silicon crystals, visible
              blue grain pattern. Slightly cheaper, slightly lower efficiency (16–18 %). Mostly seen
              on installs from the 2010s.
            </li>
          </ul>
          <p>
            A typical residential panel today is 60-cell or 72-cell, rated at 380–450 W under
            Standard Test Conditions (1000 W/m², 25 °C cell temperature, AM1.5 spectrum). The actual
            operating point on the roof is below STC most of the time — colder weather raises the
            voltage, hotter weather drops it, partial shade collapses the output of any cell in the
            string that gets shaded.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            A string is the series chain of panels feeding one MPPT input on an inverter. Wire ten
            400 W panels in series and you get a string of roughly 450 V open-circuit and 11 A
            short-circuit. The inverter operates the string somewhere below open-circuit voltage
            (typically 350–400 V under load) at close to short-circuit current — the voltage and
          ...
          <p>
            ...
            inverter’s DC input rating. Reg 712.431.101 sets out the protection rules for parallel
            strings — for two strings or fewer, no string fuse is required; above that, each string
            needs an overcurrent protective device sized to handle reverse current from the other
            strings under fault.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            short circuit the voltage is zero, and somewhere in between sits the maximum power point
            where V × I is highest. The MPPT (Maximum Power Point Tracker) inside the inverter is
            the algorithm that finds that point and stays on it.
          </p>
          <p>
            The standard algorithm is "perturb and observe": nudge the operating voltage up a
            fraction of a volt, watch whether output power rises or falls, then nudge again in the
            direction that improved things. Repeat hundreds of times a second. As clouds roll
            across, as panel temperature changes through the day, as a chimney throws shade across
            one panel, the MPPT keeps the string at peak power.
          </p>
          <p>
        ...
        >
          <p>Three families of inverter dominate the UK market:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>String inverter</strong> — one box at the bottom of the run, taking one or two
              strings of 8–12 panels each. Cheapest, most common, but the whole string is limited by
              the worst-performing panel. Best for simple roofs with no shade.
            </li>
            <li>
              <strong>Microinverter</strong> — a small inverter under each individual panel,
              converting that one panel’s DC to grid AC at the panel itself. Each panel operates at
              its own MPP, so shade on one does not affect the others. Higher cost per kW, more
              failure points, but ideal for partially shaded or multi-aspect roofs.
            </li>
            <li>
              <strong>Hybrid inverter</strong> — a string inverter with built-in battery management
              and a DC battery port. Lets the same box handle PV input, battery charge and
              discharge, and grid export. The default choice on new installs because it
              future-proofs for storage even if the customer adds the battery years later.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Section 712 is the home regulation for every UK PV install. It covers the wiring from
              the panel itself, through the DC string cables, the DC isolator, the inverter, the AC
              isolator and into the consumer unit. Every other reg in this Sub sits inside Section
              712. Lock the section number in: PV = 712.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              consumer unit does not kill the panels. As long as daylight hits a string, the DC side
              sits at open-circuit voltage — typically 300–500 V DC. Any work on the DC side (string
              cables, DC isolator, combiner box, inverter DC terminals) requires the DC isolator
              open AND the line proven dead with a DC-rated voltmeter. AC isolation is irrelevant on
              the DC side.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              insulation resistance of the DC conductors to earth continuously and flags a drop long
              before the insulation breaks down enough to sustain a DC arc. Most modern inverters
              integrate the IMD function (Reg 712.538.101) so it is rarely a separate box — but the
              requirement for the function itself is unconditional.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Default Type B. A transformerless PV inverter can leak smooth DC fault current onto
              the AC side, and smooth DC magnetically saturates the toroidal core of a Type AC or
              Type A RCD — blinding it to genuine AC faults at the same time. The three exceptions
              are specific: simple separation in the inverter, transformer separation in the
              install, or an explicit manufacturer statement that Type B is not required. Anything
              else, the reg defaults to Type B.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              isolator (BS EN 60947-3) with proper DC breaking capacity and rated for the string Voc
              max. Fitting an AC-only rotary switch is a real fire risk.
            </>
          }
        ...
        >
          <p>
            When a section of the LV network is switched off — whether deliberately for maintenance
            or by a fault tripping a substation breaker — every load in that section should fall
            dead. If a PV inverter on the same network keeps generating, it can hold up the local
            voltage and frequency, creating an unintended "island" of live network. The linesman
            approaches expecting dead conductors and finds them live.
          </p>
          <p>
            Anti-islanding is the inverter’s answer. Inverters meeting BS EN 50549-1 (the standard
            called up by ENA G98 for ≤ 16 A per phase generators and required by Reg 551.7.4 for any
            generator running in parallel with the public supply) detect the loss of mains by
            watching for shifts in voltage, frequency and grid impedance, and disconnect within
            roughly 200 ms. The full settings live in BS EN 50549-1; on a UK install you do not
            adjust them.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The 16 A per phase threshold in this reg is the same number that splits ENA G98 from
              G99. Below 16 A per phase, settings are fixed by BS EN 50549-1 and the inverter does
              it all in software. Above 16 A per phase, the protection has to be agreed with the DNO
              as part of the G99 application — the DNO sets voltage and frequency limits and
              disconnection times to suit its network and the other generators on it.
            </>
          }
        ...
        >
          ...
          <p>
            ...
            microgenerators up to and including 16 A per phase at 230 V (3.68 kW single-phase, 11.04
            kW three-phase). It is an informal post-installation notification — the installer fits
            the system and notifies the DNO within 28 days, supplying a standard completion form.
          </p>
          <p>
            ...
            certificates, protection settings and earthing arrangement. The DNO assesses whether the
            local network can take the export, sets the protection settings, and issues a connection
            agreement before the installer commissions.
          </p>
          <p>
            The 3.68 kW size that dominates UK domestic PV is no accident. It sits exactly at 16 A
            at 230 V, the upper boundary of G98. Sizing 5 kW or 6 kW means tripping into G99 and a
            much heavier paperwork chain — which most installers and customers want to avoid.
          </p>
        </ConceptBlock>
        ...
        >
          <p>The big PV failure modes you will meet on site:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>DC connector arcing.</strong> MC4 connectors are the standardised quick-fit
              plugs on PV string cables. A loose or corroded MC4, baked by years of UV and thermal
              cycling, can develop high contact resistance and start arcing under load. DC arcs do
              not self-extinguish — they sustain and ignite the surrounding plastic. Leading cause
              of PV-related house fires.
            </li>
            <li>
              <strong>Wrong DC isolator.</strong> A startling number of older installs used AC-only
              rotary switches as the DC isolator beside the inverter. They look almost identical
              externally but have no DC breaking capacity. Open one under load on a 400 V DC string
              and the arc can sustain inside the switch. Always check the rating plate is BS EN
              60947-3 with a DC voltage rating at or above the string Voc max.
            </li>
            <li>
              <strong>Inverter end-of-life.</strong> Inverters typically last 10–15 years before the
              electrolytic capacitors dry out, the cooling fan fails or the firmware loses
              manufacturer support. The customer notices their generation has dropped or the display
              has died. Replacement is usually a like-for-like swap, but the new inverter may need a
              fresh G98 notification if it changed model.
            </li>
            <li>
              <strong>Backsheet delamination.</strong> Cheap panels from the early 2010s have a
              history of polymer backsheet delamination — the rear plastic layer peels and lets
              water onto the cells. Voltage drops, hotspots form, and insulation resistance falls.
              The IMD should catch it; an inspection picks up the visible cracking.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              consumer unit, and head up the ladder. It is a sunny afternoon. The string is still
              sitting at roughly 450 V DC. You disconnect the live MC4 under load, draw a sustained
              DC arc, and the cable insulation ignites.
            </>
          }
          doInstead={
            <>
              Reg 712.410.101 makes the rule explicit: DC equipment is energised whenever light is
              on the panels, regardless of what is happening on the AC side. Open the purpose-built
              DC isolator beside the inverter, use a DC-rated voltmeter to prove dead at the
              connector you are about to disconnect, and where possible cover the relevant section
              of the array with an opaque sheet to drop the string voltage to near zero. Treat every
              PV string as a live battery in daylight.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You are doing a periodic inspection on a four-bed semi. The customer mentions the 4 kW
              PV system was fitted in 2014 and "nobody has touched it since". The MCS certificate is
              in a folder. The system is still generating — display shows roughly 3.2 kW at midday
              in June.
            </>
          }
          whatToDo={
            <>
              Walk the install in order. At the consumer unit, check the inverter circuit RCD type
              (a 2014 install almost certainly used Type AC — Reg 712.531.3.5.1 in its current form
              would now require Type B unless the inverter has internal separation, which most early
              ones do not). Check the AC isolator labelling and operation. Trace to the DC isolator
              at the inverter — verify it is BS EN 60947-3 rated for DC at or above the string Voc
              max, not an AC-only rotary repurposed at install. If safe access is available, check
              MC4 connectors at the inverter end for discoloration, melted plastic or signs of
              arcing. Check the labels required by Reg 712.514 are still in place at the origin, the
              meter position and the consumer unit. Anything that fails on RCD type or DC isolator
              rating is at minimum a C3 (improvement recommended); active overheating or arcing is a
              C2 (potentially dangerous). Document everything in the EICR comments.
            </>
          }
          whyItMatters={
            <>
              PV installs from 2010–2015 often pre-date some of the current Section 712 rules. The
              current standard is what an EICR codes against. A 12-year-old install with a Type AC
              RCD on a transformerless inverter is a real risk that needs flagging, even if it has
              been running fine.
            </>
          }
        />
