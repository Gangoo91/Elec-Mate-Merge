...
const TITLE = 'Cable sizing worked end-to-end (3.3) | Level 2 Module 3.3.7 | Elec-Mate';
const DESCRIPTION =
...
const quizQuestions = [
  ...
  {
    id: 3,
    question: 'Why is the BS 7671 inequality Ib ≤ In ≤ Iz, and not Ib ≤ Iz ≤ In?',
    options: [
  ...
  },
  {
    ...
    options: ['~10.56 V', '1.05 V', '~33 V', '~3.5 V'],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: ['0.35 Ω', '1.72 Ω', '~1.02 Ω', '1.37 Ω'],
    correctAnswer: 2,
    explanation:
...
export default function Sub7() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 3 · Section 3 · Subsection 7"
        title="Cable sizing worked end-to-end"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          One real circuit, ten gates, top to bottom — the real-world cable sizing process. Take a
          32 A radial through every cable-sizing step end-to-end: design current, Reference Method,
          derate stack, minimum CSA, voltage drop, thermal check, Zs sanity check, mechanical
          install constraints, and the final Ib ≤ In ≤ Iz inequality.
        </p>
        ...
        >
          <p>
            Cable sizing is the calculation that decides how big a conductor you put in the wall.
            Get it wrong and the cable either nuisance-trips, overheats, drops too much voltage at
            the load, or fails to disconnect on a fault. Get it right and the installation is
            invisible — it just works, for decades.
          </p>
          <p>
            The discipline is to walk the same ten gates every time, in the same order. You design
            at gate one, you verify at gates seven, eight and nine, and you commit at gate ten. The
            maths is mostly Year 9 algebra; the judgement is everything.
          </p>
        </ConceptBlock>
        ...
        >
          <ol className="space-y-1.5 list-decimal pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Design current Ib</strong> — the full load the circuit will actually carry,
              including any diversity adjustment.
            </li>
            <li>
              <strong>Protective device In</strong> — pick a standard rating where In ≥ Ib (Reg
              433.1.1 first half).
            </li>
            <li>
              <strong>Reference Method</strong> — pick the Appendix 4 method that matches the worst
              section of the route (Method A, B, C, 100, 103 etc.).
            </li>
            <li>
              <strong>Derate factors</strong> — Ca (ambient), Cg (grouping), Ci (in insulation).
              Factors stack multiplicatively.
            </li>
            <li>
              <strong>Required tabulated It</strong> — It ≥ Ib / (Ca × Cg × Ci). This is the floor
              your cable&rsquo;s tabulated CCC must clear.
            </li>
            <li>
              <strong>Pick CSA from the table</strong> — smallest CSA in Table 4D5 (or matching
              method table) whose column shows It at or above the required figure.
            </li>
            <li>
              <strong>Voltage drop check</strong> — Vd = (mV/A/m &times; Ib &times; L) / 1000, must
              be ≤ 3 % (lighting) or ≤ 5 % (other) of nominal voltage.
            </li>
            <li>
              <strong>I&sup2;R thermal sanity check</strong> — power dissipated per metre at full
              load, sense-check that it is not cooking the cable.
            </li>
            <li>
              <strong>Zs sanity check</strong> — Ze + (R1 + R2) ≤ Table 41.3 maximum for the
              protective device fitted (light cross-ref to Section 4).
            </li>
            <li>
              <strong>Mechanical and install constraints</strong> — does it pull through the
              conduit, meet bend radius, fit the gland, satisfy the spacing factor (Sub 3.6)?
            </li>
          </ol>
          <p>
            The final compliance line you write on the design sheet is the BS 7671 inequality from
            Reg 433.1.1: <strong>Ib ≤ In ≤ Iz</strong>. The other gates back it up.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 433.1.1 is the structural line your whole sizing process is trying to satisfy. Ib
              comes from the load, In comes from the device you fit, and Iz is the derated tabulated
              CCC of the cable you have chosen. Every gate in the ten-gate process feeds one of
              those three numbers. The other gates (Vd, I²R, Zs, mechanical) are separate checks
              that have to clear independently — but Reg 433.1.1 is the headline inequality.
            </>
          }
        ...
        >
          <p>
            You have been asked to design a new 32 A radial for a first-floor extension that will
            feed four double sockets in a small studio. Supply is 230 V single-phase TN-C-S,
            declared Ze = 0.35 Ω. Protective device is a 32 A BS EN 60898 Type B MCB. Cable is 70 °C
            thermoplastic T&E (6242Y).
          </p>
          <p>The route, in order from the consumer unit out:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              0&ndash;0.5 m — out of the CU, into the loft, grouped with four other circuits at the
              entry zone.
            </li>
            <li>0.5&ndash;2.5 m — buried in 100 mm of loft insulation across the gable end.</li>
            <li>
              2.5&ndash;25 m — clipped direct to the joists, in still air, ambient up to 35 °C.
            </li>
          </ul>
          <p>We will walk the ten gates. Every number below is verified.</p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            current equals the device rating. If this were a dedicated load (an oven, a shower, an
            EV charger) you would derive Ib from the rated power instead — for example a 7 kW EV
            charger gives Ib = 7000 / 230 = 30.43 A, sized to a 32 A device, but
            <em> Ib for the cable calc is still the actual 30.43 A, not 32 A</em>. For our case, the
            design current and the device rating coincide.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            inrush (no large motors, no transformers); Type B has the tightest magnetic trip window
            and gives the easiest Zs target for ADS. This decision feeds the Zs gate later — a Type
            C or D would loosen the Zs target but raise the disconnection risk.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For the clipped-direct portion, BS 7671 Appendix 4 gives Reference Method C (Table 4D5
            column 6). For the in-insulation portion, the relevant method is
            <strong> Method 103</strong> (cable totally enclosed in thermal insulation, for cable
            lengths greater than 0.5 m). Method 103 already builds the Ci insulation derate into its
            tabulated CCC values — you do <em>not</em>
        ...
        >
          <p>
            From BS 7671 Appendix 4 Table 4B1 (correction factor for ambient air temperature, 70 °C
            cable):
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Ca = 0.94</strong> at 35 °C ambient.
            </li>
          </ul>
          <p>
            From Table 4C1 (grouping correction factor, multi-core cables clipped direct, touching,
            single layer): five circuits grouped gives:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Cg = 0.75</strong> for five grouped circuits — but this only applies to the
              brief grouped section. Strictly, you can apply Cg only where the grouping persists for
              more than ~1 m, and use the worst grouping factor that occurs for any extended length.
              For a tight loft-hatch entry zone of under a metre, BS 7671 Note 14 to Appendix 4 §5.1
              lets you ignore Cg. For this calc we will be conservative and apply{' '}
              <strong>Cg = 0.75</strong> anyway as a safety margin.
            </li>
          </ul>
          <p>
        ...
        >
          ...
          <p>That is what we now take into the BS 7671 Appendix 4 Table 4D5 Method 103 column.</p>
        </ConceptBlock>
        ...
        >
          <p>
            Indicative Method 103 figures from BS 7671 Appendix 4 Table 4D5 (70 °C T&E, two cables
            loaded in totally enclosed insulation, single phase):
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              16 mm² → ~54 A — <strong>passes</strong>.
            </li>
          </ul>
          <p>
            <strong>Chosen CSA: 16 mm² T&E.</strong> Iz = 54 &times; 0.705 = 38.07 A &ge; 32 A ✓.
            The Method 103 derate is what is forcing this huge step. If we could eliminate the
            in-insulation portion (mechanical fix — clear a channel, use a metal capping route
            through the insulation, or re-route entirely above the insulation line), the controlling
            method would revert to Method C and the calc would resolve at 6 mm² instead.{' '}
            <em>
              This is the size-up rationale in action — and the obvious place to push back on the
              design.
            </em>
          </p>
          <p>
            Most apprentices would take this finding back to the supervisor. A 2 m channel cleared
            through the insulation costs an hour; the 16 mm² versus 6 mm² cable uplift costs the
            better part of a hundred quid plus heavier gland and termination kit. Engineering
            judgement, not just sums.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            For 16 mm² T&E, BS 7671 Appendix 4 Table 4D5 gives mV/A/m ≈<strong> 2.8 mV/A/m</strong>.
          </p>
          <p>
          ...
          <p>
            The non-lighting limit (Reg 525.202 / Appendix 4 §6.4) is 5 %. We are well under. For
            comparison, if the calc had landed at 6 mm² (mV/A/m = 7.3) instead, the Vd would have
            been (7.3 &times; 32 &times; 25) / 1000 = 5.84 V = 2.54 % — also under 5 %. So the
            size-up to 16 mm² was driven by CCC alone, not by Vd.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 525.202 points you at Appendix 4 §6.4 for the actual numbers — 3 % for lighting
              and 5 % for other loads (single-phase 230 V, public supply). The regulation itself is
              short because the limits are tabulated. If you exceed those, the equipment at the end
              of the cable starts seeing voltages outside its operating range — LED drivers flicker,
              motors run hot, kettles take longer to boil. Compliance keeps the customer&rsquo;s
              equipment within its design envelope.
            </>
          }
        ...
        >
          <p>
            ...
            limit). Loop length is 2 &times; 25 = 50 m (out and back). Loop resistance ≈ 50 &times;
            0.00115 = <strong>0.0575 Ω</strong>.
          </p>
          <p>Power dissipated as heat across the loop at full design current:</p>
          <p>
            P = I² &times; R = 32² &times; 0.0575 = 1024 &times; 0.0575 = <strong>≈ 58.9 W</strong>.
          </p>
          <p>
            Spread over 50 m of conductor that is 1.18 W/m — comfortable. By contrast, had the calc
            landed on 6 mm² (Cu resistance ~3.08 mΩ/m, loop resistance 50 &times; 0.00308 = 0.154
            Ω), the dissipation would have been 32² &times; 0.154 = <strong>≈ 158 W</strong> total,
            or 3.16 W/m — exactly the kind of heat density that, sat in 100 mm of loft insulation,
            will eventually cook the cable. The thermal check confirms what Method 103 already told
            us: you cannot run a 6 mm² loaded to 32 A through that environment safely.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 423.1 sets the burn-risk limits for accessible equipment surfaces. The I²R thermal
              check on a cable is the upstream version of the same idea — keep the heat dissipation
              low enough that the cable insulation does not degrade and adjacent equipment stays
              within its rated temperature. A 6 mm² cable pushing 158 W of heat into loft insulation
              is not just a cable problem; it is a Chapter 42 thermal effects problem too.
            </>
          }
        ...
        >
          <p>
            From BS 7671 Table 41.3, max Zs for a 32 A Type B BS EN 60898 device on a 0.4 s
            disconnection time, single-phase 230 V, is approximately
          ...
          <p>
            For 16 mm² / 6 mm² T&E (the standard 16 mm² T&E carries a 6 mm² CPC), the combined R1 +
            R2 from BS 7671 Appendix I tables is ~3.36 mΩ/m at 20 °C, or ~4.36 mΩ/m at the operating
            temperature multiplier of 1.20.
          </p>
          <p>
            Over 25 m: (R1 + R2) ≈ 25 &times; 0.00436 = <strong>0.109 Ω</strong>.
          </p>
          <p>
            Comfortably inside the 1.02 Ω budget. <strong>Zs ≈ 0.35 + 0.109 = 0.46 Ω</strong>, well
            under the 1.37 Ω limit. ADS gate clears with massive margin — typical for a circuit that
            has been size-bumped to satisfy CCC.{' '}
            <em>Pre-size on CCC and you almost always pass Zs by accident.</em>
          </p>
        </ConceptBlock>
        ...
        >
          <p>Quick mechanical pass for our 16 mm² T&E:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Bend radius</strong> — 16 mm² T&E is ~12 mm overall diameter; minimum bend
              radius is 8&times;OD per IET guidance, so ~96 mm at any change of direction. Easy on a
              clipped run, tight inside a JB.
            </li>
            <li>
              <strong>Containment fill</strong> — if any of the route is in trunking or conduit,
              redo the Sub 3.6 spacing-factor calc with the new larger CSA before committing.
            </li>
            <li>
              <strong>Termination</strong> — 16 mm² T&E into a 32 A MCB needs proper boot lugs and a
              torqued terminal. Some compact RCBO cages cap at 10 mm² — check the device datasheet.
            </li>
            <li>
              <strong>Pull route</strong> — 25 m of 16 mm² T&E weighs noticeably more than 25 m of 6
              mm². Plan the install: rollers, lubrication, second person on the pulling end.
            </li>
          </ul>
          <p>
            All four pass for our route — the cable is clipped direct for the bulk of its length,
            terminations are at a standard CU and a JB at the studio, no containment fill issues.{' '}
            <strong>Ten-gate process complete.</strong>
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              The cable comfortably carries the design current — CCC says 6 mm² is fine for a 32 A
              radial. You do not bother with the Vd gate. The customer&rsquo;s 50 m radial in a
              converted barn comes in at 7.3 % drop at the furthest socket. LED drivers start
              cutting out, the kettle takes 30 % longer to boil, and an inspector writes you up at
              periodic.
            </>
          }
          doInstead={
            <>
              Always run the Vd gate, even when CCC is comfortable. The two checks measure different
              things — CCC is about the cable cooking, Vd is about the load seeing enough voltage. A
              cable that is fine on CCC can fail Vd badly on a long run, and the fix is always to
              size up. Reg 525.202 / Appendix 4 §6.4 is non-negotiable, not a recommendation.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You have just finished the ten-gate calc on a 32 A oven circuit. 6 mm² T&E clears CCC,
              Vd, I²R and Zs. The cable run goes through 8 m of existing 20 mm galvanised conduit
              between the kitchen and the consumer unit. You start the pull and the cable jams two
              metres in — there is already a redundant 1.5 mm² pair in the conduit and the spacing
              factor is now 53 %, well over the 45 % limit from Sub 3.6.
            </>
          }
          whatToDo={
            <>
              Stop the pull. Three options: (1) remove the redundant 1.5 mm² pair if it is truly
              disused — quickest fix and gets the spacing factor down to ~28 %. (2) Add a draw box
              midway and split the pull into two shorter runs — does not fix the fill issue but
              sometimes lets you get past a tight bend. (3) Replace the 20 mm conduit with 25 mm if
              the route allows — proper fix but higher cost. Whatever you choose, redo the
              spacing-factor calc before committing.
              <strong>
                {' '}
                Never reduce the conductor CSA to make a mechanical problem go away
              </strong>{' '}
          ...
          whyItMatters={
            <>
              ...
              because each one catches a class of problem the others miss. Mechanical constraints
              are gate ten for a reason — they have a habit of unwinding all the gates above them,
              and the only safe response is to redesign upstream, not compromise downstream.
            </>
          }
        />
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              tabulated CCC is given at a 30 °C ambient air reference temperature; any installation
              hotter than that needs a Ca correction below 1.0. A 35 °C loft gives Ca = 0.94. A 50
              °C plant room gives Ca = 0.71. Get the ambient wrong and the cable runs over its
              limiting temperature in normal operation, accelerating insulation ageing and
              increasing fault risk.
            </>
          }
