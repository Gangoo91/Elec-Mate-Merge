...
const checks = [
  {
    ...
    question: 'Which equation correctly defines Zs for a final circuit on a TN system?',
    options: [
      'Zs = Ze − (R1 + R2)',
      'Zs = Ze × (R1 + R2)',
      'Zs = Ze + (R1 + R2)',
  ...
  },
  {
    ...
    options: ['1.37 Ω', '4.37 Ω', '0.42 Ω', '7.28 Ω'],
    correctIndex: 0,
    explanation:
  ...
  },
  {
    ...
    options: ['0.45 Ω', '0.75 Ω', '1.37 Ω', '1.02 Ω'],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    ...
    options: [
      'The installation-side impedance — the line conductor plus CPC of the final circuit only.',
      'The earth fault loop impedance external to the installation, measured at the MET with the installation isolated.',
      'The total earth fault loop impedance, including both the supply side and the final-circuit conductors.',
      'The resistance of the main earthing conductor between the MET and the cut-out earth terminal.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  {
    id: 2,
    question: 'On a final circuit, R1+R2 represents:',
    options: [
      'The supply transformer resistance plus the DNO service cable resistance back to source.',
      'The neutral conductor resistance (Rn) plus the line conductor resistance (R1) of the circuit.',
      'The line conductor (R1) plus the CPC (R2), end to end — the installation-side part of the loop.',
      'The resistance of the earth electrode plus the protective conductor on a TT system.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Zs = 0.27 Ω. Compliant — well under the 1.37 Ω table value.',
      'Zs = 1.37 Ω. Non-compliant — exactly on the uncorrected limit.',
      'Zs = 0.97 Ω. Non-compliant — exceeds the 0.80 Ω corrected max.',
  ...
  },
  {
    ...
    options: [
      'MCBs to BS EN 60898 (Types B, C, D) and the overcurrent part of RCBOs to BS EN 61009-1.',
      'RCDs and RCBOs relying on residual-current detection rather than the overcurrent element.',
      'Surge protective devices (SPDs) installed at the origin of the installation.',
      'Earth electrodes on TT systems, whose resistance sets the maximum permitted Zs.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'The MCB on a TT system is wired into the neutral, so it cannot detect an earth fault at all.',
      'Ze on TT is typically 21 Ω or higher, exceeding the Table 41.3 max Zs for any practical MCB — so RCDs are required.',
      'TT systems use a higher nominal voltage, so the Table 41.3 values no longer apply to them.',
      'The MCB magnetic trip is disabled on TT supplies, leaving only the slow thermal element working.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A change in the supply-side Ze caused by the DNO, which has nothing to do with the circuit itself.',
      'The cable being one size too large, which raises rather than lowers the measured loop impedance.',
      'A poor termination on the circuit — a loose neutral, slack CPC or corroded joint adding resistance.',
      'A faulty MFT reading high; the actual Zs is fine and the meter simply needs recalibrating.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'The multiplier accounts for the supply voltage being 6 % below the 230 V nominal at the worst case.',
      'The multiplier adds a safety margin demanded by the DNO for prospective fault current readings.',
      'The multiplier converts a single-phase Zs value into the equivalent three-phase value.',
      'Table 41.3 assumes hot cable; tests are at ~20 °C, so the 0.8 factor corrects the table value to test temperature.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'No — measured Zs = 0.35 + 1.95 = 2.30 Ω, which exceeds the 2.18 Ω corrected max, so it fails.',
      'Yes — measured Zs = 1.95 Ω, which is under the 2.18 Ω corrected limit, so it passes comfortably.',
      'Yes — R1+R2 alone is below 2.73 Ω, so the circuit complies without needing to add in Ze.',
      'No — measured Zs = 2.30 Ω, which is under the uncorrected 2.73 Ω table limit, so it actually passes.',
    ],
    correctAnswer: 0,
    explanation:
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The path a fault current takes from a fault back to source. Every link adds resistance.
          Total too high = MCB doesn’t trip = fault stays live = somebody dies. Worked Zs maths
          verified against Table 41.3.
        </p>
        ...
        >
          <p>
            The earth fault loop is the complete circuit a fault current flows around when a line
            conductor shorts to an exposed-conductive-part. Every component in that loop has its own
            impedance, and they add in series. The total is what BS 7671 calls the earth fault loop
            impedance, Zs.
          </p>
          <p>On a TN-C-S installation, the loop is:</p>
          ...
          <p>
            For convenience BS 7671 splits the total impedance into two halves: <strong>Ze</strong>{' '}
            (everything outside your installation — items 1, 2, 8, 9, 10) and <strong>R1+R2</strong>{' '}
            (the installation-side portion — items 3 and 6). Add them together to get{' '}
            <strong>Zs</strong> — the full loop.
          </p>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The 0.4 s disconnection time on a TN final circuit at 230 V is what every Zs calc is
              racing. The protective device must clear the fault before the touch voltage on the
              exposed-conductive-part can persist long enough to be lethal. Tables 41.3 and 41.5
              then give you the max Zs values that allow each device family to achieve that
        ...
        >
          <p>
            Ze is everything outside your installation — the bit you can’t change. The DNO sets it
            through the design of their supply network. Common values you’ll see on real installs:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>TN-C-S (PME):</strong> typically 0.35 Ω. Often measured between 0.2 and 0.4 Ω
              in urban areas with short service cables. Specific DNOs publish assumed maxima (e.g.
              UK Power Networks: 0.35 Ω for LV PME).
            </li>
            <li>
              <strong>TN-S:</strong> typically 0.8 Ω. Higher than TN-C-S because the protective
              conductor is the cable sheath, which has a smaller effective cross-section than the
              combined PEN.
            </li>
            <li>
              <strong>TT:</strong> typically 21 Ω as a published worst-case maximum, but real values
              often higher — 50, 100, or more depending on soil conditions, electrode construction
              and the distance to the substation electrode. The high Ze is exactly why TT
              installations rely on RCDs rather than MCBs for fault clearance.
            </li>
          </ul>
          <p>
            Always measure Ze before signing off the installation. Don’t rely on the DNO’s published
            worst-case figure for design verification — measured Ze tells you what you actually
            have, and lets you calculate the maximum permitted R1+R2 for each circuit.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            R1+R2 is the part of the loop you can control — by cable size, route length, number of
            joints, and quality of terminations. For the standard T&E sizes used in UK domestic, the
            resistance per metre at 20 °C (from BS 7671 Appendix 4 / IET On-Site Guide):
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>1.0 mm² T&E (1.0/1.0)</strong> — R1 ≈ 18.1 mΩ/m, R2 ≈ 18.1 mΩ/m, R1+R2 ≈ 36.2
              mΩ/m.
            </li>
            <li>
              <strong>1.5 mm² T&E (1.5/1.0)</strong> — R1 ≈ 12.1 mΩ/m, R2 ≈ 18.1 mΩ/m, R1+R2 ≈ 30.2
              mΩ/m.
            </li>
            <li>
              <strong>2.5 mm² T&E (2.5/1.5)</strong> — R1 ≈ 7.41 mΩ/m, R2 ≈ 12.1 mΩ/m, R1+R2 ≈ 19.5
              mΩ/m.
            </li>
            <li>
              <strong>4 mm² T&E (4.0/1.5)</strong> — R1 ≈ 4.61 mΩ/m, R2 ≈ 12.1 mΩ/m, R1+R2 ≈ 16.71
              mΩ/m.
            </li>
            <li>
              <strong>6 mm² T&E (6.0/2.5)</strong> — R1 ≈ 3.08 mΩ/m, R2 ≈ 7.41 mΩ/m, R1+R2 ≈ 10.49
              mΩ/m.
            </li>
            <li>
        ...
        >
          <p>
            ...
            overcurrent devices to achieve the disconnection times in Table 41.1. The table values
            are calculated assuming the protective device sees enough current to operate its
            magnetic-instantaneous trip — typically 5× In for Type B, 10× In for Type C, 20× In for
            Type D — with the cable at its maximum operating temperature.
          </p>
          <p>For Type B MCBs to BS EN 60898, 0.4 s disconnection at U₀ = 230 V:</p>
          ...
          <p>
            Two things to note: (1) bigger MCB rating = lower max Zs (because more fault current is
            needed to hit the magnetic threshold); (2) Type C max Zs values are roughly half the
            Type B figures (Type C trips at 10× In rather than 5× In, so it needs roughly double the
            fault current, requiring half the loop impedance).
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Use Table 41.3 to look up the max Zs your circuit can have for its protective device
              family and rating. Cross-check against measured Zs (with the 0.8 temperature
              correction) at initial verification and at every periodic inspection. If measured Zs
              exceeds the corrected limit, the disconnection time can’t be guaranteed under
        ...
        >
          <p>
            Scenario: 32 A Type B RCBO on a kitchen ring final circuit. TN-C-S supply with measured
            Ze = 0.35 Ω. Need to verify the cable run is short enough to hit a 0.4 s disconnection.
          </p>
          <p>
          ...
          <p className="font-mono text-[14px] text-emerald-300">max R1+R2 = 1.10 − 0.35 = 0.75 Ω</p>
          <p>
            <strong>Step 4 — cable check.</strong> 2.5 mm² T&E with 1.5 mm² CPC has R1+R2 ≈ 19.5
            mΩ/m at 20 °C. Maximum total run before exceeding 0.75 Ω:
          </p>
          <p className="font-mono text-[14px] text-emerald-300">
            max length = 0.75 ÷ 0.0195 ≈ 38.5 m
          </p>
          <p>
            For a typical 30 m kitchen ring on 2.5/1.5 T&E, you’ll be well within limits with R1+R2
            ≈ 0.59 Ω, Zs ≈ 0.94 Ω. If the design called for 50 m (large ring or radial), you’d need
            to either uprate to 4.0/2.5 T&E (R1+R2 ≈ 11.7 mΩ/m → 50 m gives 0.59 Ω, still fine), or
            split the circuit, or downgrade the MCB.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Scenario: 16 A Type B RCBO on a domestic lighting circuit. TN-C-S, Ze = 0.35 Ω. Cable is
            1.5 mm² T&E with 1.0 mm² CPC. Cable run = 30 m to the furthest point.
          </p>
          <p>
          ...
          <p>
            <strong>Step 4:</strong> 1.5/1.0 T&E R1+R2 ≈ 30.2 mΩ/m. For 30 m run: R1+R2 = 30 ×
            0.0302 = 0.906 Ω. Zs = 0.35 + 0.906 = 1.26 Ω. Well within the 2.18 Ω limit.
          </p>
          <p>
            Push it further: at what length does this circuit start failing? Max length = 1.83 ÷
            0.0302 ≈ 60.6 m. So a 60 m run on 1.5/1.0 T&E with B16 protection is right at the edge —
            anything longer and you need thicker cable or a smaller MCB. This is exactly why long
            lighting circuits in big country houses sometimes get 2.5 mm² with 1.5 mm² CPC even
            though 1.5/1.0 would carry the load — they’re sized for Zs, not for ampacity.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You measure Zs = 1.30 Ω on a Type B 32 A circuit and see Table 41.3 max = 1.37 Ω. You
              sign off the circuit as compliant and move on. Six months later in summer, the cable
              runs hot at full load. R1+R2 climbs by ~20%. Real Zs goes up to maybe 1.55 Ω. A
              line-to-earth fault now produces 230 ÷ 1.55 = 148 A — below the magnetic-trip
              threshold of a Type B 32 (5× In = 160 A minimum). MCB doesn’t trip instantaneously;
              thermal trip eventually fires after several seconds. Touch voltage stays high for the
              entire delay.
            </>
          }
          doInstead={
            <>
              ...
              limit is 1.37 × 0.8 = 1.10 Ω, not 1.37 Ω. A measured 1.30 Ω fails the corrected limit
              — design out the issue with thicker cable, shorter run, or different MCB rating before
              sign-off.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              ...
              Everything tested fine — Zs measured at 1.20 Ω at the equipment, well under the Table
              41.3 max of 1.37 Ω for the Type B 32 RCBO fitted. The customer now reports
              intermittent tripping during heavy summer use, and Zs re-measured today reads 1.42 Ω
              at the same point.
            </>
          }
          whatToDo={
            <>
              Go through the loop systematically. The 0.22 Ω rise across one season suggests one of
              three culprits: (a) a marginal termination that has worked loose under thermal cycling
              — check every joint from the CU outwards, particularly the RCBO outgoing terminal and
              any junction boxes; (b) corrosion at a back-box CPC connection in a damp area — clean
              and re-make the joint; (c) the cable itself is genuinely undersized and you’re seeing
              temperature-driven resistance rise — confirm by measuring with the load running and
              again after cool-down. Also re-check Ze at the MET to rule out a DNO-side change. Once
              the root cause is identified, fix it and re-test. Original design should have applied
              the 0.8 multiplier and accounted for summer ambient — 1.20 Ω at commissioning was
              already at the 0.8 × 1.37 = 1.10 Ω limit, marginal.
            </>
          }
          whyItMatters={
            <>
              Zs isn’t a fixed number — it changes with cable temperature, joint quality and DNO
              supply conditions. Every Zs reading is a snapshot. Initial verification documents the
              cold-cable best case; the design margin (the 0.8 multiplier) absorbs operational rise.
              Skip the multiplier and you’re commissioning circuits that work in the test room and
              fail in service.
            </>
          }
        />
        ...
        >
          <p>The textbook causes of an out-of-spec Zs reading on a real installation:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Long thin cables</strong> — undersized for the run length. The R1+R2 climbs
              above what the protective device can tolerate. The fix is bigger cable. This ties back
              to Module 2 §3.4 voltage drop calculations: a circuit that fails Vd is also heading
              for a Zs problem because both are driven by R1+R2.
            </li>
            <li>
              <strong>Bad terminations</strong> — every loose, corroded or under-torqued joint adds
              extra resistance to the loop. CU outgoing terminals are the classic culprit (vibration
              loosens screws over years), followed by hidden junction boxes and back-box CPC
              connections. Trace, find, re-make.
            </li>
            <li>
              <strong>Missing parallel earth paths</strong> — if the original design assumed certain
              bonding or armour as a parallel CPC and that bond has been disconnected during refurb
              work, the entire circuit’s Zs reading climbs. Particularly common on commercial
              sub-mains where SWA armour was the original CPC and a later contractor replaced the
              gland incorrectly.
            </li>
          </ul>
        </ConceptBlock>
