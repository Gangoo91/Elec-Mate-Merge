...
const checks = [
  {
    ...
    question: 'What is the function of the Main Earthing Terminal (MET) in the ADS chain?',
    options: [
...
const quizQuestions = [
  ...
  {
    id: 7,
    question: 'On a TN-C-S domestic, the protective device’s job in the ADS chain is to:',
    options: [
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Automatic Disconnection of Supply isn’t one thing. It’s a chain. Source, earth, MET,
          earthing conductor, CPC, bonding, protective device — break any link and protection fails.
        </p>
        ...
        >
          <p>
            The principle of ADS: design the installation so that a low-impedance fault between line
            and an exposed-conductive-part causes enough fault current to flow that the protective
            device (MCB, RCBO or RCD) automatically disconnects the line within a defined time.
            Combined with equipotential bonding to limit touch voltage during that disconnection
            time, the result is that no one in contact with the metalwork during the fault is
            exposed to a lethal shock.
          </p>
          <p>
            Two parts work together. <strong>Disconnection</strong> by the protective device,
            triggered by high fault current. <strong>Bonding</strong> to keep all the metalwork at
            roughly the same potential during the few hundred milliseconds before the device opens.
            Reg 411.3.1 covers both, and they are not interchangeable — bonding without
        ...
        <RegsCallout
          ...
          meaning={
            <>
              411.3.1.1 makes the CPC mandatory at every accessory (one tiny exception for a Class
              II pendant). 411.3.1.2 makes main bonding mandatory at every extraneous-
              conductive-part liable to introduce a potential. Both regs together define the two
              halves of ADS — the earthing path that makes fault current flow, and the bonding that
              keeps touch voltages survivable while the protective device clears the fault.
            </>
          }
        ...
        >
          <p>
            Take a TN-C-S domestic with a Type B 32 A RCBO on a kitchen ring. A live phase shorts to
            the metal back-box of a socket. Walk the loop in order:
          </p>
          <ol className="space-y-2 list-decimal pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              ...
              the 2.5 mm² cable from the consumer unit to the socket. This is the "line" part of R1.
            </li>
            <li>
              <strong>The fault itself</strong> — assumed negligible impedance for design purposes
            ...
            <li>
              ...
              unit (or adjacent to it) where every CPC, the main bonding and the earthing conductor
              meet.
            </li>
            <li>
          ...
          <p>
            ...
            that’s 230 A — comfortably above the magnetic trip threshold of any Type B MCB. The MCB
            opens within milliseconds. ADS works.
          </p>
        </ConceptBlock>
        ...
        <ConceptBlock title="The source — substation transformer (or generator on private networks)">
          <p>
          ...
          <p>
            ...
            sites with on-site transformers, generator-fed installations) the source is local. On IT
            systems the source isn’t directly earthed at all — fault current limited by the source
            impedance.
          </p>
        </ConceptBlock>
        ...
        >
          <p>How the source’s earth gets into your building depends on the system:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>TN-C-S (PME or PNB):</strong> the combined PEN conductor in the supply, split
              into N and PE at the service position.
            </li>
            <li>
        ...
        >
          <p>
            ...
            Sizing is by Table 54.7 (cross-sectional area in line with the line conductors and fault
            current calc) or Table 54.8 for installations with a PEN or PME-supplied origin (sized
            against the supplier’s neutral). On a TT system buried portions must also meet Table
            54.1 minimum sizes for buried conductors with allowance for corrosion.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The MET is where the earthing conductor (back to source/electrode), the main protective
            bonding conductors (out to extraneous parts) and every CPC in the installation come
            together. Physically it’s usually a brass or copper terminal block inside or next to the
            consumer unit, often labelled "MET" or with the earth symbol.
          </p>
          <p>
            The MET’s job is twofold: provide a single low-impedance node so that every earthed and
            bonded part of the installation sits at the same potential during a fault, and provide a
            single inspection point where every connection can be tested and recorded on the EIC
            schedule.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            Sized per Reg 544.1 — typically half the cross-sectional area of the earthing conductor
            with a 6 mm² minimum (and 10 mm² minimum on PME unless protected against mechanical
            damage). Visualised in the diagram below.
          </p>
        ...
        >
          <p>
            At the end of the ADS chain sits the protective device that interrupts the supply. The
            device’s sole job in this context: trip within the time required by Reg 411.3.2.2 once
            the loop impedance and the resulting fault current allow it to.
          </p>
          <p>Three families of device do this work:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>MCB</strong> (overcurrent only) — relies on high fault current to trigger the
              magnetic trip. Works on TN systems where Ze is low enough; will not trip in time on TT
              systems.
            </li>
            <li>
        ...
        >
          ...
          <p>
            Why the times differ between TN and TT: TT relies on RCDs which trip in around 25–40 ms
            regardless of fault current — getting under 0.2 s is straightforward. TN MCBs trip on
            magnetic instantaneous if fault current is high enough (around 5× In for Type B), which
            is also fast. The longer times for distribution circuits acknowledge that touch contact
            with sub-mains is rare compared with final-circuit accessories.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              circuits. Distribution circuits and circuits outside 411.3.2.2 get up to 5 s on TN and
              1 s on TT (Regs 411.3.2.3 and 411.3.2.4). Pick a protective device whose tripping
              characteristic and the measured Zs together meet that time.
            </>
          }
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You’re replacing a metal-bodied light fitting on a stairwell. The CPC into the back of
              the rose looks corroded so you cut it back and never reconnect it, reasoning that "the
              building’s well bonded anyway — the gas and water are bonded back to the MET, so the
              fitting will get an earth through the structure". A fault months later means the metal
              fitting sits at line voltage; the next person to clean it touches it and drops dead.
            </>
          }
          doInstead={
            <>
              CPC and bonding are two different jobs. The CPC carries fault current back to the MET;
              bonding equalises potential between extraneous parts and the MET. Bonding is NOT a
              return path for fault current — its conductors aren’t routed for fault-loop use, the
              path through the structure is unpredictable, and the impedance is too high to trip the
              protective device. Every accessory needs its own continuous CPC back to the MET (Reg
              411.3.1.1). No exceptions outside the Class II pendant case.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              You’re doing an EICR on a 1980s ground-floor flat. At the MET the earthing conductor
              is loose in its terminal — a previous installer over-tightened the screw, stripped the
              thread, and the strap is now sitting in the terminal under spring tension only.
              Insulation resistance and continuity tests still pass because there’s physical
              contact, but you can lift the conductor out by hand.
            </>
          }
          whatToDo={
            <>
              ...
              connection, vibration, thermal cycling or routine work on the consumer unit could lift
              it clear at any moment, breaking the entire ADS chain. Make safe immediately — fit a
              new earth bar terminal or replace the strap, retest Ze and Zs at the MET.
            </>
          }
          whyItMatters={
            <>
              ...
              fault-current return path meets the supply earth. Treat that joint with the same care
              as a final-circuit termination, and never assume a "passes continuity test" reading
              means the connection is mechanically sound.
            </>
          }
        />
