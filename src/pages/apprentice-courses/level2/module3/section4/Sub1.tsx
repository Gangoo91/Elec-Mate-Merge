...
const checks = [
  ...
  },
  {
    ...
    options: [
      'A 32 A Type B MCB on every final circuit — the electrode resistance is low enough to trip it on a fault.',
      'No RCD at all — the local earth electrode gives a fault path good enough for overcurrent disconnection.',
      'A 100 mA time-delayed RCD (or 30 mA) at origin — TT relies on RCD protection because Ze is too high for overcurrent devices.',
      'A Type D MCB at origin — its higher magnetic trip threshold copes with the high electrode resistance.',
    ],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'In PNB the supply is delivered as fully separate neutral and earth conductors, with no combined PEN section anywhere on the network.',
      'In PNB the PEN is earthed at a single point close to the consumer (between transformer and supply terminals), not at multiple points as in PME.',
      'In PNB the consumer must always drive their own local earth electrode, exactly as they would on a TT system with no DNO earth.',
      'In PNB the PEN conductor is earthed at far more points along the DNO network than the multiple electrodes used in PME.',
    ],
    correctIndex: 1,
    explanation:
...
const quizQuestions = [
  {
    id: 1,
    question: 'Which letters in "TN-C-S" tell you what about the system?',
    options: [
      ...
      'T = three-phase supply, N = neutral conductor present, C = commercial premises only, S = single-phase derived.',
      'T = transmission voltage level, N = nominal 230 V, C = combined-core cable, S = sheathed metallic cable.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  {
    id: 4,
    question: 'An IT system is most likely to be found in which of these locations?',
    options: [
      'A hospital operating theatre or a process plant where supply continuity matters more than instant disconnection.',
      'A new-build domestic estate where the DNO has installed a PME supply to each plot.',
      'A rural farmhouse on an overhead supply with its own driven earth electrode.',
      'A high-street retail unit on a standard TN-C-S single-phase 100 A supply.',
    ],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      'It trips far too quickly on modern loads, causing constant nuisance disconnection of LED and EV charging circuits.',
      'It only detects sinusoidal AC faults — modern electronic loads (LED drivers, EV chargers, inverters) inject DC components that saturate the RCD core, blinding it.',
      'It cannot be used on TN-C-S supplies at all, only ever on TT, because of the PME diverted-neutral current risk.',
      'It has a fixed 100 mA trip threshold that is far too coarse to give additional protection on domestic socket circuits.',
    ],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      'Because PME supplies carry far more fault current, so the bonding has to be larger purely to survive the higher I²t let-through energy.',
      'Because the bonding conductor on a PME supply doubles as the circuit CPC and so must match the largest final-circuit cable.',
      'Because if the PEN goes open-circuit the earthing terminal can rise toward line voltage, and heavier bonding limits the touch voltage on extraneous parts.',
      'Because PME supplies have a much lower declared Ze, which the regs deliberately offset by demanding heavier bonding conductors.',
    ],
    correctAnswer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      'A TT earthing conductor must always be a 6 mm² Cu minimum, so a 16 mm² conductor is just wastefully oversized.',
      'TT earthing conductors must match the supply neutral exactly, so 16 mm² is far too small for any TT supply.',
      'The TN-C-S specification is perfectly fine for TT too; the only real error is that the conductor was not labelled.',
      'The conductor runs to a buried electrode and must meet Table 54.1 buried-conductor sizes — 16 mm² unprotected Cu in soil corrodes and undersizes the run.',
    ],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    ...
    options: [
      'PME is prohibited in the explosive atmosphere — a broken-PEN fault could spark at the metalwork, so the dispensers are derived as a separate TT zone.',
      'Forecourts must run on a three-phase supply only, and TT is the sole earthing system that is available for three-phase supplies.',
      'TN-C-S is not permitted to supply any outdoor equipment at all, so a forecourt must always be wired as TT regardless of the explosion risk.',
      'A TT zone gives a much lower Ze than PME, which is what is required for the fast disconnection times demanded on a forecourt.',
    ],
    correctAnswer: 0,
    explanation:
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 3 · Section 4 · Subsection 1"
        title="Earthing systems"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Five earthing arrangements you’ll meet on UK installations — TN-S, TN-C-S (PME), TN-C-S
          (PNB), TT and IT. The DNO chose for you. Your job is to recognise which one and respect
          it.
        </p>
        ...
        >
          ...
          <p>
            ...
            <strong>Third and fourth letters (TN systems only)</strong> = the arrangement of neutral
            and protective conductors (S = separated, C = combined as PEN). TN-C-S means combined in
            part of the system (the supply), separated in the rest (the installation).
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Two TN variants matter to you on UK installs: TN-S (separate N and PE all the way) and
              TN-C-S (combined as PEN in the supply, separated at the service position into N and
              PE). A4:2026 has expanded TN-C-S to include both PME and the new PNB layout — see
              Sub-clause 312.2.1.1 for the figures and requirements.
            </>
          }
        ...
        >
          <p>
            ...
            the metallic sheath of the supply cable (lead in older properties, steel-wire armour in
            some). The DNO maintains the integrity of the sheath as a continuous earth path.
          </p>
          <p>
            Identify it: at the cut-out you’ll see line, neutral and a separate earth conductor or a
            sheath-bonding clamp going to the MET. Critically, the earth strap does <em>not</em>{' '}
            ...
            earth faults reliably without needing RCD assistance, though additional protection (30
            mA RCD) is still required by BS 7671 for socket outlets, mobile equipment, and
        ...
        >
          <p>
            In TN-C-S, the supply network uses a combined Protective Earth + Neutral conductor — the
            PEN — between the source and the consumer. At the consumer’s service position the PEN is
            split: one terminal becomes the installation neutral, another (linked) becomes the
            installation PE. From that point inwards the installation is wired with separate N and
            PE conductors. The "C" refers to the combined supply portion, the "S" to the separated
            installation portion.
          </p>
          <p>
            The PME variant — the most common UK form — earths the PEN at multiple points along the
            DNO’s network (Protective Multiple Earthing). Ze is typically around{' '}
            <strong>0.35 Ω</strong> on a healthy PME supply. That low impedance is why MCB-only
            earth fault protection works so reliably on TN-C-S — fault currents are huge, MCBs trip
            in milliseconds.
          </p>
          <div className="flex justify-center pt-2">
        ...
        >
          ...
          <p>
            When PNB is installed within the consumer’s installation (not a DNO arrangement), PME
            conditions don’t apply — which has knock-on effects on bonding sizes and on which
            circuits can be supplied. A4:2026 also added new schedule columns to the EIC/MWC to
            capture PNB explicitly, so you’ll need to tick the right box on the certificate when you
            meet one.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              rules and removes the PME-prohibited-location restrictions. Identify whether you’re on
              PME or PNB before you spec the bonding.
            </>
          }
        ...
        >
          <p>
            ...
            earthed via a local electrode (T) — independent of the source earth. The fault loop goes
            line → fault → CPC → MET → earthing conductor → local rod → soil → substation electrode
            → source neutral. That soil-and-electrode return path is high impedance: Ze is typically{' '}
            <strong>20–200 Ω</strong>, sometimes much more on sandy or rocky ground.
          </p>
          <p>
            At those Ze values, fault current is too low for an MCB to trip in 0.4 s. So TT
            installations rely on <strong>RCDs</strong> for fault protection — Reg 411.5.3 requires
            that Ra × IΔn ≤ 50 V and Reg 411.3.2.4 allows up to 1 s disconnection on distribution
            circuits. The standard arrangement on a domestic TT is a 100 mA time-delayed S-type RCD
            at origin (for selectivity) with 30 mA RCD/RCBOs on final circuits.
          </p>
          <div className="flex justify-center pt-2">
        ...
        <RegsCallout
          ...
          meaning={
            <>
              On a 30 mA RCD, the maximum allowable Zs from Table 41.5 is <strong>1667 Ω</strong>.
              So even a 200 Ω electrode is comfortably inside the limit. The headline test on a TT
              install isn’t whether the MCBs trip — it’s whether Ra × IΔn stays under 50 V and
              whether the RCD operates within the disconnection time. Without an RCD, a TT
              installation can’t comply.
            </>
          }
        ...
        >
          <p>
            ...
            characteristic feature: a first earth fault generates only a small current — limited by
            the source impedance — and is not required to cause automatic disconnection. Instead an
            insulation monitoring device (IMD) raises an alarm so the fault can be found and cleared
            at a planned time.
          </p>
          <p>
            On a second fault occurring on a different conductor, full short-circuit current flows
            and the protective device must operate. That’s why IT is a specialist arrangement — the
            protective scheme has to handle two distinct fault scenarios, the IMD has to be
            maintained, and isolation procedures matter as much as the wiring. Apprentices won’t
            install or sign off IT systems at Level 2; you need to recognise one and know to
            escalate.
          </p>
          <div className="flex justify-center pt-2">
        ...
        >
          <p>A 30-second visual at any cut-out should tell you which system you’re on:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>TN-C-S (PNB):</strong> harder to spot at a single property. Usually called out
              on private distribution networks or new sub-mains where there’s a single neutral-earth
              link near the consumer.
            </li>
            <li>
              <strong>TT:</strong> only line and neutral arriving from DNO; a separate green/ yellow
              earthing conductor leaves the MET and runs to a local earth electrode (rod or buried
              plate). Often signposted by an earth-electrode test pit.
            </li>
            <li>
              <strong>IT:</strong> industrial/medical only. Identifiable by an insulation monitoring
              device on the supply and the absence of a direct source earth.
            </li>
          </ul>
          <p>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You wire a new dispenser controller on a forecourt and bond its metal enclosure to the
              customer’s PME earth like you would in a domestic. A broken-PEN fault then drags the
              whole forecourt’s exposed-conductive-parts toward line voltage; the resulting spark in
              an ATEX zone ignites vapour at the dispenser. People die. The investigation finds you
              ignored the PME-prohibited locations.
            </>
          }
          doInstead={
            <>
              ...
              protection — completely separate from the building’s PME earthing. Reference BS 7671
              Part 7 for the exact location-specific requirements before designing.
            </>
          }
        />
        ...
        <Scenario
          ...
          whatToDo={
            <>
              ...
              installation without creating a parallel earth path between two unrelated DNO earthing
              systems. Apart from the metering issue (which is a regulatory offence in its own
              right), you’d be relying on the lead sheath of an old TN-S cable to carry fault
              currents from a PME-class installation upstairs. Each supply stays on its own earthing
              arrangement, with its own MET, its own bonding, and its own DNO meter.
            </>
          }
          whyItMatters={
            <>
              Mixing earthing systems within a single building is one of the higher-risk faults
              you’ll encounter on EICRs. It usually happens during piecemeal extensions where a new
              supply is added but the wiring quietly bridges back to the old supply’s earth. Spot
              it, code it (typically C2), and recommend separation before any further work proceeds.
            </>
          }
        />
