...
const checks = [
  ...
  },
  {
    id: 'transistor-gain',
    question:
      'A small NPN BJT has a current gain (β) of 100. If you push 1 mA into the base, what’s the maximum collector current?',
    options: ['1 A', '10 mA', '100 mA', '10 A'],
    correctIndex: 2,
    explanation:
...
const quizQuestions = [
  ...
  },
  {
    id: 4,
    question:
      'A microcontroller switches a 12 V relay coil that draws 80 mA. Why does the designer almost always put a transistor between the chip pin and the coil?',
    options: [
  ...
  },
  {
    id: 7,
    question:
      'Why does a transistor used as a switch need a heatsink for higher-current applications?',
    options: [
  ...
  },
  {
    id: 8,
    question:
      'You’re troubleshooting a smart RCBO and find the trip relay is being driven by a small transistor on the PCB. The transistor reads short-circuit between two of its leads. Conclusion?',
    options: [
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          The transistor replaced every mechanical switch in modern electronics. A tiny signal in
          controls a much bigger current out. Same idea drives every dimmer, every VFD, every smart
          relay.
        </p>
        ...
        >
          <p>
            ...
            current path; the third is the control terminal. Vary the signal at the control terminal
            and you vary how much current flows through the main path — anywhere from fully off to
            fully on.
          </p>
          <p>
            That’s the entire idea. Everything else — amplifiers, switches, oscillators, logic
            chips, microcontrollers, the inverter inside an EV charger — is built from transistors
            arranged in clever combinations. Modern microchips contain billions of them, all etched
            into a piece of silicon the size of a fingernail.
          </p>
        ...
        >
          <p>Two main families dominate the kit you’ll meet on site:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>BJT — Bipolar Junction Transistor</strong>. Three terminals: base, collector,
              emitter. Controlled by a current into the base. Two flavours: NPN (current flows from
              collector to emitter when the base goes positive) and PNP (the mirror image). Common
              in older designs and small-signal jobs. Typical part numbers: 2N3904 (NPN), 2N3906
              (PNP), BC547.
            </li>
            <li>
              <strong>MOSFET — Metal-Oxide Semiconductor Field-Effect Transistor</strong>. Three
              terminals: gate, drain, source. Controlled by a voltage on the gate — the gate is
              insulated, so essentially no gate current flows. Dominant in modern power switching.
              Two flavours: N-channel (most common for switching loads) and P-channel (used for
              high-side switching). Typical part numbers: IRF540, IRFZ44, BSS138.
            </li>
            <li>
              <strong>IGBT — Insulated-Gate Bipolar Transistor</strong>. The hybrid: voltage control
              like a MOSFET, current handling like a BJT. The workhorse of every VFD, solar
              inverter, EV charger and induction welder you’ll ever fit. Hundreds of volts, hundreds
              of amps, switched thousands of times a second.
            </li>
          </ul>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              glance. Arrow pointing OUT — NPN BJT or N-channel MOSFET (most common). Arrow pointing
              IN — PNP BJT or P-channel MOSFET. Once you can read the symbol you can read the
              function in any manual you open.
            </>
          }
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Transistors (and TRIACs, IGBTs and the rest) are recognised by BS 7671 as functional
              switching devices — they can control current without physically breaking the circuit.
              That’s fine for switching loads on and off. But it’s a fundamental reason why they can
              never replace a proper isolator (next reg).
            </>
          }
        ...
        >
          ...
          <p>The current gain β (also written hFE) tells you the multiplier:</p>
          <p className="text-center text-[16px] font-semibold tracking-wide">Ic = β × Ib</p>
          <p>
            Worked example: a 2N3904 has β ≈ 100. Push 1 mA into the base → you can switch up to 100
            mA at the collector. To switch a 100 mA relay coil from a microcontroller pin (which can
            typically source 5–20 mA), this works perfectly. You drive the base with a few mA
            through a current-limiting resistor; the transistor handles the coil.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            A MOSFET is controlled by gate <em>voltage</em>, not gate current. The gate sits behind
            a thin oxide insulator — there’s essentially no DC current path into the gate. Once
            you’ve charged the gate above the threshold voltage (usually a few volts for
            "logic-level" MOSFETs), the device snaps fully on. Take the voltage away and it snaps
            fully off.
          </p>
          <p>
            The advantage: a 3.3 V or 5 V microcontroller can drive a MOSFET switching tens of amps
            directly, with nothing more than the chip pin and the FET itself. That’s why MOSFETs
            have replaced BJTs in nearly every modern switching application.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            electronics. Instead of running a transistor in its lossy linear region, you switch it
            fully on and fully off at a high frequency (typically 1–20 kHz). The fraction of time
            it’s on — the <strong>duty cycle</strong> — sets the average power delivered:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
          ...
          <p>
            Real-world examples: a VFD controlling motor speed PWM-switches the DC bus at a few kHz;
            an LED dimmer chops the mains at hundreds of Hz; an EV charger PWM-controls the charging
            current. In every case, fast on/off switching beats running a transistor at half-bias
            because the losses are tiny when the device is fully on or fully off, and non-existent
            when off.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            high frequency — typically 4–16 kHz — to synthesise a variable-frequency AC output for
            the motor. The output ISN’T a clean sine. It’s a <strong>PWM square wave</strong>
            that AVERAGES to a sine wave when you look at the current the motor actually draws.
          </p>
          <p>
            Here’s the trick. The IGBTs only ever sit fully on or fully off (low losses). The gate
            driver varies the on-time of each pulse — wider pulses near the peak of the intended
            sine, narrower pulses near the zero-crossing. The motor windings are highly inductive,
            and inductance resists rapid changes in current. So while the voltage at the motor
            terminals is a chopped square wave, the current through the windings smooths out to
            something close to sinusoidal at the fundamental frequency the drive is asking for.
          </p>
          <p>That square edge is why VFD-fed motors need extra care on site:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Shielded cable</strong> — the fast switching edges radiate EMI. A screened
              cable with the screen bonded both ends keeps the noise off neighbouring circuits.
            </li>
            <li>
              <strong>Output filters on long runs</strong> — over about 30–50 m the cable acts like
              a transmission line, the square edges reflect at the motor end and can double the peak
              voltage on the windings. Manufacturers spec a dV/dt or sine-wave filter between the
              drive and the motor.
            </li>
            <li>
              <strong>Inverter-duty motors</strong> — standard motor windings can’t take the voltage
              spikes from a chopped supply. A motor rated for inverter duty has thicker winding
              insulation and sometimes shaft-current bearings.
            </li>
            <li>
              <strong>True-RMS meter weirdness</strong> — even a True-RMS meter from Sub5.6 can
              mis-read the chopped waveform if its bandwidth doesn’t cover the switching frequency.
              Check the meter spec before trusting a VFD-output reading.
            </li>
          </ul>
          <p>
            You won’t design any of this — but you’ll meet the consequences when a VFD install keeps
            tripping a nearby RCD, or a motor wired up on standard cable runs hot. Knowing WHY
            (square edges, not a clean sine) gets you to the right answer faster than blind
            substitution.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              DC bus to make a variable-frequency, variable-voltage supply for AC induction motors.
              Every modern motor speed control depends on this.
            </li>
            <li>
              <strong>EV chargers</strong> — IGBTs handle the DC fast-charging side; MOSFETs and
              power-management chips run the AC side. The pilot signal between charger and car is
              itself a PWM signal at 1 kHz.
            </li>
            <li>
              <strong>Solar inverters</strong> — IGBTs convert the panel’s DC into grid-quality AC.
              Same chopping principle as a VFD, in reverse.
            </li>
            <li>
              <strong>LED dimmers</strong> — trailing-edge dimmers use a MOSFET; older leading-edge
              dimmers use a TRIAC (a related thyristor device). Both control the fraction of each
              mains cycle that reaches the lamp.
            </li>
            <li>
              <strong>Smart RCBOs and AFDDs</strong> — small signal transistors drive the trip coil
              based on the chip’s analysis of the current waveform.
            </li>
            <li>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              A 7.5 kW VFD throws an "IGBT fault" code. Someone with a soldering iron and too much
              confidence cracks the case open and replaces the IGBT module they think looks burnt.
              The drive now blows up the new IGBT seconds after powering on.
            </>
          }
          doInstead={
            <>
              ...
              board to swap with it, and the test sequence. We don’t component-repair power modules
              — either swap the whole drive (most common in commercial settings) or send it for
              factory refurb.
            </>
          }
        />
        ...
        <Scenario
          ...
          whatToDo={
            <>
              At low duty cycles the MOSFET is only on for a tiny fraction of each mains cycle. The
              LED driver inside each lamp needs a minimum current to start its internal oscillator —
              below that, it stutters. Either fit dimmable LEDs designed for deep dimming, fit a
              "minimum load" device across the dimmer to give it something steady to see, or accept
              that 30% is the practical bottom of that combination.
            </>
          }
          whyItMatters={
            <>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              A transistor or TRIAC turning off doesn’t physically separate the contacts — there’s
              still a tiny leakage current and, if the device fails short, full mains voltage
              downstream. That’s why every drive, every smart switch, every dimmer ALSO needs a
              mechanical isolator upstream that you can lock off. Never trust the electronics alone
              for safe isolation.
            </>
          }
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          >
            ...
            <div className="mt-1 text-[14px] font-semibold text-white truncate">6.3 Capacitors</div>
          </button>
          <button
          ...
          >
            ...
            <div className="mt-1 text-[14px] font-semibold text-black truncate">6.5 Sensors</div>
          </button>
        </div>
      </HubBody>
    </HubPage>
