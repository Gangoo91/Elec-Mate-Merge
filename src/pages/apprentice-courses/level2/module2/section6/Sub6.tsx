...
const quizQuestions = [
  ...
  },
  {
    id: 2,
    question:
      'You’re replacing a 3-phase contactor on a compressor running off a 400 V supply. The new contactor’s coil is marked 110 V. Why is that not necessarily wrong?',
    options: [
  ...
  },
  {
    ...
    options: ['100 mA', '10 mA', '300 mA', '30 mA'],
    correctAnswer: 3,
    explanation:
  ...
  },
  {
    id: 7,
    question:
      'Why do SPD installation rules say connecting conductors must be kept "as short and as straight as possible"?',
    options: [
...
export default function Sub6() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 2 · Section 6 · Subsection 6"
        title="Real on-site electronics — relays, contactors, RCD/AFDD, SPDs"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          Pull the previous five subs together. Inside every modern protective device on a UK
          installation, you’ll find resistors, diodes, capacitors, transistors and sensors — wired
          around an electromagnetic relay or contactor.
        </p>
        ...
        >
          <p>
            ...
            transistors (small signal switches a big load). Sub6.5 covered sensors (turn the world
            into voltage). This Sub puts every one of those into the consumer unit on a real
            distribution board — wired around the relays, contactors, RCDs, AFDDs and SPDs you fit
            on every job.
          </p>
          <p>
            The MFT you met in Sub1.5 is what you’ll use to verify any of the protection devices in
            this Sub — push-button trip test, IΔn measurement, ramp test, all in one instrument.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            A <strong>relay</strong> is a small electromagnetically operated switch. A coil wound
            around a soft-iron core pulls in a moving armature when energised; the armature carries
            one or more contact pairs onto fixed contacts, completing a separate circuit.
          ...
          <p>
            ...
            four power poles for switching motors and heaters, plus auxiliary contacts for control
            logic. Contactors have proper arc chutes (insulating barriers and magnetic "blowouts"
            that stretch and quench the arc as the contacts open under load).
          </p>
        ...
        >
          ...
          <p>On a typical industrial panel:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              ...
              industrial control panels. The 110 V supply is centre-tapped earth so worst-case shock
              is 55 V.
            </li>
            <li>
            ...
            <li>
              <strong>400 V AC coil</strong> — occasionally used for direct phase-to-phase control
              on three-phase systems.
            </li>
          </ul>
          <p>
            Inside the coil itself there’s a fair bit of inductance — the same physics covered way
            back in Section 5 — which is why switching the coil off can produce a back-EMF spike.
            That’s the reason you’ll see a small flyback diode (DC coils) or RC snubber (AC coils)
            wired across the coil terminals on most modern parts.
          </p>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              Chapter 53. Pick parts certified for the job, sized for the actual load (with margin
              for inrush) and rated for the duty cycle. A relay rated AC1 (resistive loads) will
              fail early on AC3 (motor) duty even if the steady-state current looks OK on paper.
            </>
          }
        ...
        >
          <p>
            An RCD’s core technology is dead simple in principle. A toroidal (ring-shaped) iron core
            surrounds both the line and neutral conductors. In a healthy circuit, the line current
            flowing one way exactly equals the neutral current flowing the other way — the magnetic
            fluxes cancel and there’s no net flux in the core.
          </p>
          <p>
            If some current leaks to earth (through a person, through a fault), then line and
            neutral no longer balance. The difference creates a net flux in the core, which induces
            a tiny current in a sense winding wound around the same core. That sense current feeds
            an electronic trip circuit — once it crosses the threshold (30 mA for a domestic RCD),
            the trip coil energises, the latch releases, and the contacts open.
          </p>
          <p>
            All of this happens in roughly 25–40 ms — fast enough that even a serious shock current
            is interrupted before it crosses the threshold for ventricular fibrillation.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              30 mA is the residential standard — below the 30–80 mA range where a healthy adult can
              no longer let go of a live conductor. BS 7671 also requires ≤30 mA RCDs on socket
              outlets up to 32 A, mobile equipment used outdoors, and (now) lighting circuits in
              dwellings. There’s effectively no domestic final circuit left where 30 mA RCD
        ...
        >
          <p>
            The "type" classification tells you which fault current waveforms the RCD can reliably
            detect:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Type AC</strong> — sinusoidal AC only. Now considered obsolete for most
              domestic applications because nearly every electronic load can produce non-sinusoidal
              fault currents.
            </li>
            <li>
              <strong>Type A</strong> — AC plus pulsating DC. The new domestic default. Handles
              fault waveforms from electronic loads (LED drivers, smart switches, washing machines).
            </li>
            <li>
              <strong>Type F</strong> — like Type A but rated for higher-frequency components, used
              with single-phase variable-speed drives.
            </li>
            <li>
              <strong>Type B</strong> — AC + pulsating DC + smooth DC. Required on EV charge points
              (Type 2 charging where the car’s onboard charger can produce DC fault current) and
              many three-phase variable-speed drives.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>
            An AFDD is built around the same chassis as a normal MCB or RCBO, but it adds a small
            electronics package: a bridge rectifier feeding a switched-mode supply, a current
            sensor, a microcontroller and signal-processing software. The microcontroller samples
            the current waveform thousands of times per second, looking for the characteristic
            high-frequency spectral signature of an arc — random, broadband "hash" on top of the
            smooth 50 Hz fundamental.
          </p>
          <p>
            The clever part is discrimination. Brush motors (vacuums, hairdryers, drills), switching
            power supplies and even ordinary light switches turning on all produce short bursts of
            arc-like noise. The AFDD has to ignore those (or it nuisance trips) while still spotting
            genuine sustained arcs from a fault. That’s where the BS EN 62606 test profile comes in
            — it’s the standard the device has to meet.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Reg 421.1.7 mandates AFDDs in specified higher-risk locations (high-rise residential,
              premises with sleeping accommodation in certain cases) and recommends them more
              widely. Whatever AFDD you fit must conform to BS EN 62606 — you can’t substitute a
              generic device. Note also Reg 710.421.1.7: AFDDs shall NOT be used in medical
              locations of groups 0 and 2.
            </>
          }
        ...
        >
          <p>
            A Surge Protection Device is built around a non-linear element — typically a metal oxide
            varistor (MOV), a gas discharge tube, or both in combination. At normal supply voltage
            it acts as a near-open circuit: tiny leakage, no effect on the installation. The moment
            the line-to-earth voltage exceeds its threshold (usually a few hundred volts above peak
            nominal), the device clamps — voltage held down, surge current diverted to earth.
          </p>
          <p>Surges come from two main sources:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Atmospheric (lightning)</strong> — a strike up to a kilometre away can induce
              thousands of volts on the supply line in microseconds.
            </li>
            <li>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              badly dressed SPD leads can mean the equipment downstream still sees most of the surge
              — making the SPD pointless. Keep the install tight: shortest practical run, straight
              in/straight out, no loops, and meet the minimum CSA in 534.4.10.
            </>
          }
        ...
        >
          <p>
            Each surge degrades the MOV slightly. Big surges degrade it a lot. Eventually the device
            reaches end of life and stops clamping properly. The indicator window is your only
            warning. Plug-in SPDs are designed to be swapped out (no need to disturb the wiring);
            fixed SPDs need to be isolated and replaced like any other consumer-unit module.
          </p>
          <div className="flex justify-center pt-2">
        ...
        >
          <p>A modern consumer unit fitted to current BS 7671 might contain:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              A row of <strong>RCBOs</strong>, each containing an RCD (current transformer + sense
              coil + electronics) and an MCB (thermal-magnetic trip). Type A or B depending on the
              load.
            </li>
            <li>
              Possibly an <strong>AFDD</strong> on circuits where Reg 421.1.7 mandates one — with
              rectifier, microcontroller and signal processing inside.
            </li>
            <li>
              Possibly a <strong>smart RCBO</strong> with energy monitoring and remote comms — add a
              radio chip and an LED driver to the list above.
            </li>
          </ul>
          <p>
            You don’t design any of this. But every time you fit, swap or fault-find one of these
            devices, you’re putting your signature next to it. Knowing what’s inside — down to the
            component families covered in this whole section — is what makes you competent rather
            than just trained.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              You’re fitting a consumer-unit SPD. The leads come long, you bunch them up neatly
              inside the enclosure to make the lid go on. A surge hits the supply, the SPD does its
              job, but the connecting-lead inductance has already let several hundred extra volts
              through to the customer’s smart RCBOs and EV charger.
            </>
          }
          doInstead={
            <>
              BS 7671 Section 534 requires the connecting-conductor length to be kept to a minimum —
              preferably under 0.5 m total, never above 1.0 m. Cut the leads to the shortest
              practical length, route them straight, and accept that the install looks a bit less
              tidy than a coiled bundle. The surge protection has to function, not just exist.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              The compressor on a small workshop has a failed 4-pole contactor. The supply is 400 V
              three-phase. The original contactor has a 110 V coil. The customer says "just put
              another one in" and you’ve got a 230 V coil version on the van.
            </>
          }
          whatToDo={
            <>
              Don’t fit it. The 110 V coil is fed from a control transformer (or 110 V CTE supply)
              inside the panel — swap to a 230 V coil and the existing control circuit will either
              burn out the new coil (if it’s only providing 110 V) or work at half pull-in force and
              chatter. Either get the matching 110 V part from the local wholesaler or, if the
              customer wants you to convert, design and install the right supply for the new coil.
              Don’t bodge.
            </>
          }
          whyItMatters={
            <>
              Coil voltage and load voltage are independent for a reason. Mixing them up is one of
              the most common contactor-related faults you’ll see, and it’s genuinely dangerous on
              industrial kit.
            </>
          }
        />
        ...
        >
          <p>
            ...
            shower (this is for the en-suite off the back of the same job), a 7 kW oven, four 8 W
            LED downlights and a couple of sockets fed from a 32 A radial. The CU is in the hall;
            the new sub-board lives in the kitchen, 22 m of SWA away on a tray run through the loft.
            Walk through every Section of Module 2 in turn — every one of them feeds into this
            single job.
          </p>
          <p>
            <strong>Section 1 — units and quantities.</strong> Convert each load to current at 230
            V. The shower draws 9500 W ÷ 230 V = <strong>41.30 A</strong>. The oven 7000 ÷ 230 ={' '}
            <strong>30.43 A</strong>. The four LED downlights at 8 W each total 32 W — with a power
            factor around 0.9 that’s about <strong>0.15 A</strong>, vanishingly small. The sockets,
            after diversity, sit around <strong>13 A</strong>. Section 1 gave you the unit
            conversions and the sense of scale to spot which circuit is actually the heavyweight
            (the shower, by miles).
          </p>
          <p>
            <strong>Section 2 — mechanics.</strong> The 22 m of SWA from the CU to the new sub-board
            has to physically get up into the loft, along the tray, and back down into the kitchen.
            The drum weighs around 25 kg — Sub2.1 set the lifting limits and Sub2.3 walked through
            the SWA-up-the-riser routine. You won’t solo-lift that drum into the loft; you’ll get
            the labourer on the other end of it. None of this is in the regulations — it’s in Module
            2 because it’s the part the regs assume you already know.
          </p>
          <p>
            ...
            <strong>4.4 mV/A/m</strong>. At full shower current on a 22 m run: Vd = (4.4 × 41 × 22)
            ÷ 1000 = <strong>3.97 V</strong>, which is <strong>3.97 ÷ 230 = 1.73%</strong> — well
            inside the 5% non-lighting limit. Then Sub3.5’s I²R sanity check: with Vd = 3.97 V
            across the whole loop at 41 A, the power dissipated as heat in the cable run is V × I =
            3.97 × 41 ≈ <strong>163 W</strong>. That’s why the SWA needs the tray and the air gap —
            163 W of low-grade heat sitting in roof insulation is exactly how cables cook.
          </p>
          <p>
            ...
            through the cable feeding them is the <em>sum</em> of each driver’s current, not divided
            between them: 4 × ~0.04 A ≈ 0.15 A total. Section 4.6’s KVL/KCL sanity check still
            applies — what goes in at the JB has to come back out, drivers and all.
          </p>
          <p>
            <strong>Section 5 — AC origin.</strong> The 230 V at the cut-out is an <em>RMS</em>{' '}
            value — Sub5.6 explained that the actual peak voltage on each cycle is{' '}
            <strong>230 × √2 = 325 V</strong>. The LED drivers’ input rectifier (Sub6.2) has to be
            rated to handle that 325 V peak — the smoothing capacitor (Sub6.3) downstream of it sits
            charged near that peak. The shower element, by contrast, just heats with the RMS value:
            P = V²/R, RMS in, real power out, no peak nonsense involved.
          </p>
          <p>
            <strong>Section 6 — electronic kit on the board.</strong> The new sub-board needs an RCD
            — Sub6.6 (this Sub) pinned that down to <strong>Type A as the minimum</strong> because
            the LED drivers and oven electronics generate pulsating DC fault currents that a Type AC
            would miss. The shower circuit gets its own RCBO; if the manufacturer specifies it,
            that’s a <strong>Type B</strong>. An <strong>SPD</strong> sits at the sub-board origin
            to clamp surges before they reach the drivers. Inside each downlight driver there’s a
            bridge rectifier (Sub6.2 — diodes) feeding a smoothing capacitor (Sub6.3). Recognise all
            of those because Section 6 has just walked you through the component families they’re
            built from.
          </p>
          <p>
            Every Sub in Module 2 just helped you spec, install and verify one extension. That’s the
            point of the module — not 35 disconnected topics, one continuous build.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            You’ve specced the kitchen extension on paper. Module 3 is where you actually put it in
            the wall.
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
            ...
            <li>
              Three-phase distribution (the proper version of Sub5.6’s preview) — how 400 V between
              phases turns into 230 V at the socket, and how a balanced three-phase load behaves on
              the supply.
            </li>
            <li>
              Consumer units and sub-boards in detail — RCBO and AFDD selection, breaking capacity,
              busbar configuration, neutral arrangement.
            </li>
            <li>
              Special locations — bathrooms, kitchens, outdoors, agricultural premises, marinas —
              where the BS 7671 rules tighten and the zoning starts to matter.
            </li>
            <li>
              The cable-selection process end-to-end — Method A and Method B from Sub3.4 applied to
              real sub-mains and final circuits, with all the correction factors BS 7671 makes you
              apply.
            </li>
          </ul>
          <p>
            Module 3 takes everything you’ve just learned and shows you how to install it properly.
          </p>
        </ConceptBlock>
