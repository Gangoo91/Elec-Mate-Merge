...
const checks = [
  ...
  },
  {
    id: 'rectifier-output',
    question:
      'A bridge rectifier feeds a load from a 230 V AC supply. What does the load see (before smoothing)?',
    options: [
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          A diode is the simplest one-way valve in electronics. Stack four of them in a bridge and
          you’ve got a rectifier — the heart of every charger, every LED driver, every AFDD.
        </p>
        ...
        >
          <p>
            ...
            direction (forward-biased, anode positive with respect to cathode) and blocks it in the
            other (reverse-biased).
          </p>
          <p>
            ...
            of every rectifier you’ll meet. Germanium diodes drop about 0.3 V but are rare on modern
            kit. Schottky diodes drop around 0.3–0.5 V and are common in switch-mode power supplies.
          </p>
        ...
        >
          <p>
            ...
            end. The end with the painted band is the <strong>cathode</strong>. Surface-mount diodes
            have the band marked on the package. The symbol on schematics works the same way — the
            bar is always the cathode.
          </p>
          <p>Common part numbers you’ll see on UK boards:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>1N400x series (1N4001 to 1N4007)</strong> — general-purpose mains rectifiers,
              rated 1 A, 50 V to 1000 V reverse.
            </li>
            <li>
              <strong>1N5400 series</strong> — 3 A versions of the same family for slightly bigger
              supplies.
            </li>
            <li>
        ...
        >
          <p>
            The simplest possible rectifier is one diode in series with the load. When the AC swings
            positive, the diode conducts and current reaches the load. When the AC swings negative,
            the diode blocks and the load sees nothing. The result is a pulsating DC that’s only
            “on” half the time.
          </p>
          <p>
            Half-wave is cheap and dirty — it wastes half the supply, gives a low average DC
            voltage, and needs a much bigger smoothing capacitor to clean up the gaps. You’ll still
            see it inside very low-cost LED candle bulbs and battery chargers where a few flickers
            don’t matter, but it’s rare on serious kit.
          </p>
          <HalfWaveRectified />
        </ConceptBlock>
        ...
        >
          <p>
            ...
            pulsating DC, but now there’s a hump for every half-cycle (so 100 humps per second from
            a 50 Hz mains) — twice as often as half-wave, with no gaps.
          </p>
          <p>That’s why full-wave rectification is the standard:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>Average DC output is roughly double what half-wave gives for the same AC input.</li>
            <li>Pulses are twice as frequent, so a much smaller smoothing capacitor will do.</li>
            <li>
              The supply transformer (if there is one) gets used for both halves of the cycle — no
              wasted capacity.
            </li>
          </ul>
          <p>
            Look at the diagram. Top half-cycle of the AC input: current flows through D1 → load →
            D3 back to source. Bottom half-cycle: D2 and D4 take over — but the LOAD still sees
            current flowing the same direction. That’s why a bridge rectifier turns AC into pulsing
            DC.
          </p>
          <BridgeRectifier />
          <FullWaveRectified />
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Every AFDD you fit must be a BS EN 62606 product. Inside one of those, a small bridge
              rectifier feeds a microcontroller which samples the current waveform thousands of
              times a second, looking for the high-frequency hash that an arc fault produces. Diodes
              are the very first stage — they make the AFDD’s electronics possible.
            </>
          }
        ...
        >
          <p>
            ...
            infrared LEDs use one mix, blue and white LEDs use another (white LEDs are usually blue
            LEDs with a yellow phosphor coating).
          </p>
          <p>
            Forward voltage drop is higher than a regular silicon diode and depends on colour: red
            ~1.8 V, green ~2.2 V, blue/white ~3.0–3.4 V. Typical forward currents are 10–30 mA for
            indicator LEDs, hundreds of mA for high-power lighting LEDs.
          </p>
          <p>
            Crucially — an LED has almost no internal resistance once it conducts. Without a series
            resistor or (better) a current-limited driver, it self-destructs the moment you energise
            it.
          </p>
          <div className="flex justify-center pt-2">
        ...
        >
          <p>
            ...
            starts conducting hard, holding its terminal voltage at the rated value while the extra
            burns off in the resistor in series.
          </p>
          <p>
        ...
        >
          <p>
            An AFDD is required by BS 7671 Reg 421.1.7 in certain locations and recommended for all
            socket-outlet final circuits in domestic premises. Inside it you’ll find:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
          ...
          <p>
            Modern AFDDs also have to discriminate between “nuisance” arcs (light switches, motor
            brushes) and dangerous ones (loose terminals, damaged cable). That signal processing is
            what costs money — the diodes themselves are pennies.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              AFDD trips on the customer’s old vacuum. You replace the AFDD with another of the same
              brand. It trips again. Customer is fuming.
            </>
          }
          doInstead={
            <>
        ...
        <Scenario
          ...
          whatToDo={
            <>
              ...
              (leading-edge) dimmer left over from filament days. Forward-phase dimmers chop the AC
              waveform on its way up — that hard cut hits the LED driver’s input diode bridge and
              produces a burst of harmonics every half-cycle. Those harmonics excite the little
              choke inside the dimmer (and sometimes the driver itself) into mechanical vibration at
              exactly the frequencies your ear can hear. Fit a proper trailing-edge (reverse-phase)
              dimmer designed for LEDs, or use lamps marketed as "dimmable" with the right driver
              topology. The hum disappears.
            </>
          }
          whyItMatters={
            <>
              This is the most common "my lights are humming" callout in domestic work. Knowing that
              the diodes inside the LED driver are what make the noise — not the lamp itself — turns
              a swap-out warranty job into a five-minute dimmer change.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              A 7 kW EV charger you fitted last week throws an error and refuses to charge any car.
              The fault code says “DC residual current detected”. The customer wants it fixed before
              the morning.
            </>
          }
          whatToDo={
            <>
              Don’t panic. The charger has a Type B RCD or built-in DC leakage detection because the
              car’s onboard charger is full of diodes that can rectify a fault current to DC, which
              an ordinary AC RCD can’t see. The detection circuit has noticed something. Check the
              car (some EVs have known DC leakage faults), check the charger’s earthing, and only
              after that consider the unit itself.
            </>
          }
          whyItMatters={
            <>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Modern EV chargers contain bridge rectifiers (lots of diodes) on the car side. A fault
              on the DC bus can produce smooth DC leakage current that an ordinary AC RCD won’t see
              — and worse, that DC can magnetically saturate the RCD’s sense core and blind it to a
              real AC fault on the same circuit. That’s why Type B (or Type A paired with a 6 mA
              RDC-DD per BS IEC 62955) is the minimum.
            </>
          }
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          >
            ...
            <div className="mt-1 text-[14px] font-semibold text-black truncate">6.3 Capacitors</div>
          </button>
        </div>
      </HubBody>
    </HubPage>
