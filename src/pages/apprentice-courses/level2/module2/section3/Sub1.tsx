...
const checks = [
  ...
  },
  {
    ...
    question: 'A free electron in a copper cable drifts along the wire at roughly what speed?',
    options: [
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: ['Negative to positive', 'Positive to negative', 'Source to switch', 'Earth to live'],
    correctAnswer: 1,
    explanation:
...
const faqs = [
  ...
  {
    question:
      "Why is conventional current the 'wrong way round' if we know electrons go the other way?",
    answer:
...
export default function Sub1() {
  ...
  return (
    <HubPage>
      <HubMasthead
        section="Module 2 · Section 3 · Subsection 1"
        title="Electron theory — atoms, charge and current flow"
        backTo=".."
      />
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          What current actually is at atomic level. Why metals conduct, why plastics don't, and why
          every diagram you'll ever read draws the arrows backwards.
        </p>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>Neutrons</strong> — no charge. Also in the nucleus. They keep the protons from
        ...
        >
          <p>
            ...
            nucleus, with the inner shells filling first. The electrons in the outermost shell are
            called <strong>valence electrons</strong>, and they decide how the atom behaves
          ...
          <p>
            ...
            they drift along the conductor. That drift is the current we measure with a clamp meter.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              One sentence, but it's the whole game. Charge moves; we measure how fast it's moving
              past a point. Faster flow → more current. <strong>One amp</strong> = one coulomb of
              charge passing a point every second.
            </>
          }
        ...
        >
          <p>
            In a copper conductor, every atom donates one or two of its outer electrons to a shared
            pool. Those electrons are no longer 'owned' by any single atom — they roam freely
            between the atoms that make up the lattice. This is the{' '}
            <strong>sea of free electrons</strong> model, and it's the cleanest way to picture
            what's going on inside a wire.
          </p>
          <p>
            When you connect a battery or close a switch, you put a{' '}
            <strong>potential difference</strong> across the cable. That's a push. Every free
            electron feels the push at the same time and starts to drift along the cable. They don't
            move fast (we come back to that) but there are unimaginable numbers of them — a 1 mm
            copper cable has around 10²² free electrons per metre.
          </p>
          <p>
            In a plastic or rubber insulator, the outer electrons are locked into bonds with their
            atoms — there's no shared pool. Push as hard as you like with a normal voltage and very
            few electrons can move. That's why the same copper wire is harmless once it's wrapped in
            PVC: the conductor carries the current, the insulator stops it leaking out.
          </p>
        </ConceptBlock>

        <ConceptBlock title="Charge — the bit being moved around">
          <p>
            Electric charge is a property carried by certain particles. Electrons carry one unit of
            negative charge each; protons carry one unit of positive. Charge itself is measured in{' '}
            <strong>coulombs (C)</strong>. One coulomb is roughly{' '}
            <strong>6.24 × 10¹⁸ electrons</strong> — six billion billion. You'll never count
            individual ones; we deal in bulk.
          </p>
          <p>Current is just how much charge is moving past a point each second:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>1 ampere = 1 coulomb per second.</strong> A 6 A lighting circuit moves 6 C of
              charge past any point every second it's switched on.
            </li>
            <li>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Just the formal version of <strong>Q = I × t</strong>. You won't write coulombs on a
              certificate, but they're the bridge between 'how many electrons' and 'how many amps' —
              useful when you read about battery capacity (Ah) later in 3.6.
            </>
          }
        ...
        >
          <p>
            Back in the 1700s, Benjamin Franklin had to guess which way charge moved before anyone
            knew about electrons. He picked positive-to-negative. By the time JJ Thomson discovered
            the electron in 1897 and proved it carried negative charge (and therefore moved the
            opposite way to Franklin's arrow), every textbook in the world was already using
            Franklin's convention. The world chose to keep the original direction rather than redraw
            a century of work.
          </p>
          ...
          <p>
            ...
            "north-bound" or "south-bound" depending on which side you're on. For everything you'll
            do as an electrician, use conventional current and forget about the contradiction.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            ...
            the lamp lights up immediately, the electrons themselves are crawling at maybe 0.1 mm
            per second through the cable. So how does the lamp get the energy that fast?
          </p>
          <p>
            The trick is that the cable is <strong>already full of electrons</strong>. When you
            close the switch, you set up an electric field along the conductor at near the speed of
            light. That field pushes the entire sea of electrons everywhere at once. The electron at
            the lamp end of the cable starts moving at the same instant as the one at the switch
            end. Each electron only has to shuffle a tiny distance to deliver the energy to the lamp
            filament.
          </p>
          <p>
            Easy mental picture: a long row of marbles already touching each other end to end. Push
            one marble at one end, and the marble at the other end pops out almost immediately — but
            no single marble has travelled the whole length of the row. The push (the signal) is
            fast; the marbles (the electrons) crawl.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              The bit doing the carrying is the <strong>conductor</strong> — copper or aluminium in
              nearly every BS 7671 install. A live part is anything energised in normal use. The
              reason a live conductor is dangerous is that touching it gives the free electrons a
              new path — through you, to earth — and you become part of the circuit.
            </>
          }
          cite="Source: BS 7671:2018+A4:2026 — Part 2."
        />
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              ...
              current". Both descriptions mash together two different things. Voltage is the push;
              current is what moves. They're not interchangeable, and writing it like that in your
              AM2 paper will lose you marks.
            </>
          }
          doInstead={
            <>
              Use the right word. <strong>Voltage (V)</strong> is the potential difference — the
              push between two points, measured in volts. <strong>Current (I)</strong> is the rate
              of charge flow — the actual movement, measured in amps. Voltage doesn't flow. Current
              does. Voltage is the reason current flows.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              Customer says the bedside lamp went dead overnight. You isolate at the consumer unit,
              plug the lamp into a known-good socket and it works. Plug a known-good lamp into the
              original socket and that's dead too.
            </>
          }
          whatToDo={
            <>
              The fault's in the supply path to that socket — break in the conductor, loose
              terminal, broken switch. Somewhere in the conductor, the sea of free electrons doesn't
              have a continuous path. No path = no drift = no current = no light. Trace back from
              the dead socket using a continuity test (after isolating, locking off and proving
              dead, obviously).
            </>
          }
          whyItMatters={
            <>
              Electron theory is what tells you why an open circuit kills the lamp. The voltage is
              still there at the consumer unit — but with no continuous conductor, the electrons
              have nowhere to drift to, so there's no current. Diagnosis works because you
              understand what current actually <em>is</em>.
            </>
          }
        />
        ...
        >
          <p>
            Under DC, electrons drift slowly from negative to positive. Under AC the polarity swaps
            50 times a second (in the UK), so the electrons reverse direction 100 times a second —
            they oscillate in place rather than drifting net anywhere. A given electron on the
            lighting circuit in your kitchen has been wiggling back and forth over a fraction of a
            millimetre for years and has essentially stayed put.
          </p>
          <p>
            So how does the load still get power? Because the{' '}
            <strong>energy doesn’t live in the electrons</strong> — it lives in the electromagnetic
            field that surrounds the conductor. That field is what propagates from source to load at
            near light speed. The electrons are just the medium the field uses to push and pull.
            Same physics as the DC case earlier in this Sub; the field still transfers the energy.
            Only the electron motion looks different.
          </p>
          <p>
            That oscillation is also why AC measurements need a different approach to averaging. You
            can’t take a simple average — over one cycle the current goes positive then negative and
            the average is zero. The trick is RMS (root mean square), which gives you the equivalent
            steady DC value that would do the same heating in a resistor.{' '}
            <strong>You’ll see exactly why this matters when we get to RMS in Sub5.6</strong> — for
            now, just know that AC current and voltage figures (the 230 V at your sockets, the 13 A
            at your plug) are RMS values, not peaks.
          </p>
        </ConceptBlock>
