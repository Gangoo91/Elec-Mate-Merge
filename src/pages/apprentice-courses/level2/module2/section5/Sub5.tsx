...
const TITLE = 'The single-loop AC generator (5.4) | Level 2 Module 2.5.5 | Elec-Mate';
const DESCRIPTION =
...
const checks = [
  {
    ...
    options: ['A sine wave', 'A sawtooth wave', 'Constant DC', 'A square wave'],
    correctIndex: 0,
    explanation:
  ...
  },
  {
    ...
    options: ['No output at all', 'Pure DC', 'Pulsating DC (always positive)', 'AC sine wave'],
    correctIndex: 3,
    explanation:
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: ['Diode bridge', 'Commutator (split ring)', 'Capacitor', 'Slip rings'],
    correctAnswer: 3,
    explanation:
  ...
  {
    id: 4,
    question: 'The peak EMF of an AC generator is given by:',
    options: ['EMF = NBAω', 'EMF = NBA/ω', 'EMF = NB/Aω', 'EMF = NA/(Bω)'],
    correctAnswer: 0,
    explanation:
  ...
  },
  {
    ...
    options: ['100 rad/s', '314 rad/s', '50 rad/s', '157 rad/s'],
    correctAnswer: 1,
    explanation:
  ...
  },
  {
    ...
    options: ['10 V peak', '100 V peak', 'No change', '1000 V peak'],
    correctAnswer: 3,
    explanation:
    ...
    <HubPage>
      ...
      <HubBody>
        <p className="max-w-3xl text-[13px] leading-relaxed text-white">
          A coil. Two magnets. A spinning shaft. That’s it. Every UK kilowatt-hour ever generated
          comes from this one trick — and the maths behind it falls out of Faraday’s law in three
          lines.
        </p>
        ...
        >
          <p>
            Section 5 has built up to this. Sub3 showed you that a current creates a magnetic field.
            Sub4 showed you that a changing magnetic flux induces an EMF. Now we put the two
            together in the most useful machine ever built: take a coil, put it in a field, and
            rotate it. As the coil turns, the flux through it changes — and an EMF appears at its
            terminals.
          </p>
          <p>
            Spin it at a constant speed and the output is a clean sine wave that alternates
            direction once per revolution. Connect a load and current flows. That’s an alternator —
            the heart of every power station and the source of every joule of mains electricity in
            the UK.
          </p>
        </ConceptBlock>
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              ...
              machines, electromagnet field windings) provides a steady field across the gap where
              the coil rotates. N pole one side, S pole the other.
            </li>
            <li>
              <strong>The coil (the rotor or armature).</strong> A rectangular loop of wire wound
              onto an iron core, free to rotate on a central shaft. Real machines have many turns to
              multiply the output.
            </li>
            <li>
              <strong>The shaft.</strong> Whatever is doing the spinning — a steam turbine, hydro
              turbine, wind turbine, diesel engine, hand crank — connects to this shaft and provides
              the mechanical input.
            </li>
            <li>
              <strong>Slip rings + brushes.</strong> Two metal rings, one connected to each end of
              the coil, rotating with the shaft. Stationary carbon brushes ride on the rings and
              carry the induced current out to the external circuit. (Use a split-ring commutator
              instead and you get DC out — same coil, different connection.)
            </li>
            <li>
              <strong>The external circuit (load).</strong> Whatever you’re powering — a lamp, a
              motor, the National Grid.
            </li>
          </ul>
        </ConceptBlock>
        ...
        >
          <p>Picture the coil at four positions through one revolution:</p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>
                Position 1 (0°) — coil flat in the field, plane parallel to the field lines.
              </strong>{' '}
              Maximum flux is threading the coil. But at this exact instant the flux is at its peak
              and not yet changing — the rate of change dΦ/dt is zero. So the EMF is zero.
              Counterintuitive but correct.
            </li>
            <li>
              <strong>
                Position 2 (90°) — coil edge-on to the field, plane perpendicular to the field
                lines.
              </strong>{' '}
              No flux through it at all (the field passes parallel to the coil plane). But at this
              moment the flux is changing FASTEST — every degree of rotation makes a big difference.
              Maximum dΦ/dt → maximum EMF. This is the peak of your sine wave.
            </li>
            <li>
              <strong>Position 3 (180°) — coil flat again, but flipped over.</strong> Maximum flux
              through it again, but in the opposite direction. dΦ/dt is back to zero, EMF is back to
              zero.
            </li>
            <li>
          ...
          <p>
            Plot the EMF against time as the coil rotates and you trace out a perfect sine wave. One
            revolution = one full cycle. Spin the coil twice per second and you’ve got a 2 Hz
            output. Spin it 50 times per second and you’ve got UK mains 50 Hz.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            The maths quietly implies it (sin(0) = 0), but it’s the kind of thing nobody states out
            loud — and it’s the one thing that catches every apprentice the first time they meet a
            generator.
          </p>
          <p>
            When the coil sides are right next to the poles — looking ‘closest’ to the magnets, with
            the maximum number of field lines threading through the loop — the EMF is at its{' '}
            <strong>MINIMUM</strong>, not its maximum. The flux through the coil is highest at that
            instant, but it’s not <em>changing</em>. And Faraday only cares about{' '}
            <strong>change</strong>, not size.
          </p>
          <p>
            EMF only happens during change, not during size. Big static field = nothing. Tiny field
            that’s changing fast = plenty of voltage. That’s why a coil sitting still in the
            strongest magnet on Earth produces zero EMF — and a coil moving through a weak field at
            speed produces real, useful voltage. The peak of the output sine wave lives at the
            position the coil ‘looks’ least productive: edge-on, with no flux through it but the
            rate of change at its absolute maximum.
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              Sub4’s Faraday’s law plus a bit of school maths gets you the equation that runs every
              power station in the UK: <strong>EMF_peak = N × B × A × ω</strong>. You don’t need to
              do the calculus on the exam — but the formula and what each letter means is fair game
        ...
        >
          ...
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            ...
            <li>
              <strong>ω</strong> = angular velocity (rad/s) — equal to 2π × f, where f is frequency
              in Hz
            </li>
          </ul>
          <p>
            Each variable scales the output linearly. Double any one and the peak voltage doubles.
            Halve any one and the voltage halves. That gives the designer four independent dials to
            tune.
          </p>
          <p>
            For UK 50 Hz generation, ω is fixed at 2π × 50 = 314 rad/s for a 2-pole machine. That’s
            why N, B and A are the variables left to play with — they decide how big and how heavy
            the alternator has to be to produce a given voltage.
          </p>
        </ConceptBlock>
        ...
        <ConceptBlock title="Worked example — peak EMF of a typical machine">
          ...
          <p>
            That’s the peak. The next subsection (Sub6) explains why we usually quote AC in RMS
            terms instead — and it works out that 157 V peak corresponds to about 111 V RMS, the
            kind of low-voltage output you’d find on a small portable genset before any step-up
            transformation.
          </p>
          <p>
            For the same machine, double the rotation speed (ω = 628 rad/s) and you get 314 V peak —
            but at 100 Hz instead of 50 Hz. That’s the trade-off: bigger voltage costs you
            frequency. To keep frequency fixed at 50 Hz while raising voltage, you’d have to add
            turns, increase B, or grow the coil area.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            <strong>Slip rings.</strong> Two continuous metal rings, one bonded to each end of the
            coil. They rotate with the shaft. Carbon brushes ride on the outside of the rings and
            connect to the load. Each brush always touches the same end of the coil — so when the
            coil reverses direction, the load sees the reversal too. Output =<strong> AC</strong>.
          </p>
          <p>
            <strong>Split-ring commutator.</strong> One ring, sawn in half. Each half is connected
            to one end of the coil — but the brushes are positioned so they swap which half they
            touch every half-revolution. The swap happens at the exact moment the coil EMF reverses,
            so the brush ends up always being connected to the "currently positive" side of the
            coil. Output = <strong>pulsating DC</strong> (always positive, but not flat). Add a
            smoothing capacitor or more coils at different angles and you get something close to
            flat DC.
          </p>
          <p>
            <strong>The same coil + same magnets + same rotation</strong> produces both AC and DC.
            The connection at the shaft is the only difference. That’s a useful thing to hold in
            your head: AC and DC machines are not separate species. They’re the same machine wired
            up two different ways.
          </p>
          <p>
            <strong>Look at the diagram below.</strong> DC version of the same machine — the
            split-ring commutator (the gapped ring at the shaft) is what physically swaps the brush
            connection every half-turn so the load only ever sees positive voltage. Wipe that ring
            off and replace it with two continuous slip rings and you’d be back to AC.
          </p>
        </ConceptBlock>
        ...
        >
          <p>
            Real alternators are scaled-up, multi-coil versions of the single loop you’ve just met.
            A few practical refinements:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>Many turns per coil.</strong> The single loop in a textbook produces tiny
              voltages. Wrap a few hundred turns around an iron former and the output multiplies.
              Hundreds or thousands of turns per coil is normal.
            </li>
            <li>
              <strong>Three coils set 120° apart.</strong> Take three identical coils, space them
              evenly round the rotor, and you generate three sine waves of the same amplitude and
              frequency, each shifted by 120°. That’s exactly what UK three-phase supply is.
            </li>
            <li>
              <strong>Multiple pole pairs.</strong> Put two pairs of N-S poles round the stator
              instead of one and the coil sees a complete N-S-N-S cycle every half-rotation — so the
              output frequency is twice the rotational frequency. That’s the relationship:{' '}
              <strong>f = (poles × rpm) / 120</strong>.
            </li>
            <li>
              <strong>Field windings instead of permanent magnets.</strong> Instead of fixed
              magnets, real alternators use electromagnets fed from a small DC supply (called the
              "excitation"). Vary the excitation current and you can vary B — which gives you
              control over the output voltage.
            </li>
          </ul>
          <p>
            UK National Grid frequency is 50 Hz. To deliver that, every alternator on the grid has
            to spin at the right rpm for its number of poles: a 2-pole machine at 3000 rpm, a 4-pole
            at 1500 rpm, a 6-pole at 1000 rpm, and so on. They’re all locked in electromagnetic step
            with each other through the grid itself — a property called "synchronisation".
          </p>
        </ConceptBlock>
        ...
        <RegsCallout
          ...
          meaning={
            <>
              ...
              Network Operator) is in breach of the law and has to disconnect customers. Underneath
              all the network engineering, this regulation is just enforcing that ω = 2π × 50 stays
              constant at every alternator across the country.
            </>
          }
        ...
        >
          <p>
            Don’t let the simplicity of the single-loop drawing fool you — it’s not a teaching toy.
            It’s the same machine you’ll meet on site, just with more turns, more poles, and a
            beefier shaft. Three places it’s already working in kit you’ve seen:
          </p>
          <ul className="space-y-1.5 list-disc pl-5 marker:text-elec-yellow/70">
            <li>
              <strong>The alternator on your van.</strong> A spinning rotor with field windings
              inside a stator with output coils, slip rings + brushes (or solid-state equivalents),
              driven by the engine via a belt. Same Sub5.4 induction, same Sub5.5
            ...
            <li>
              <strong>Power-station generators.</strong> A coal, gas, nuclear, hydro or wind turbine
              spins a shaft. The shaft turns a rotor through field windings inside a stator. EMF
              appears at the stator terminals. The size of the alternator is terrifying — 1 GW
              machines stand two storeys tall — but the physics is identical to the
              bar-magnet-and-loop drawing earlier in this Sub.
            </li>
            <li>
              <strong>The site genset on the back of a van.</strong> A small diesel engine spinning
              a 5 to 20 kVA alternator. Slip rings on the rotor, brushes on the stator side, output
              filtered and regulated to give 230 V at 50 Hz. Same five parts (field, coil, shaft,
              rings + brushes, load) listed in the "What’s in the box" block above.
            </li>
          </ul>
          <p>
            The slip-ring vs commutator choice from earlier in this Sub is exactly what divides AC
            from DC machines in the wild — alternators on the grid use slip rings (AC out),
            old-school DC dynamos and brushed DC motors use a split-ring commutator. Both are
            sitting on Sub5.4’s induction. The next Sub (5.6) takes the sine wave you’ve just seen
            come out and analyses it as RMS, peak and frequency — which is how you put numbers on
            every measurement you’ll make on UK mains.
          </p>
        </ConceptBlock>
        ...
        <CommonMistake
          ...
          whatHappens={
            <>
              It feels intuitive: when the coil is flat to the field, the maximum number of field
              lines pass through it. So that must be when the EMF is biggest, right? Wrong — that’s
              when the FLUX is biggest, but Faraday cares about the <em>rate of change</em> of flux,
              not the flux itself. At flat-on, the flux is at a momentary maximum and isn’t changing
              at all. EMF = zero.
            </>
          }
          doInstead={
            <>
              Remember: EMF is biggest when the coil is <strong>edge-on to the field</strong> (its
              plane parallel to the field). At that moment the coil sides are sweeping across the
              field lines as fast as possible — maximum dΦ/dt → maximum EMF. The coil cuts through
              field lines fastest when it’s passing through "edge-on", not "face-on". Burn that in.
            </>
          }
        />
        ...
        <Scenario
          ...
          situation={
            <>
              A new apprentice on the job asks why UK mains is exactly 50 Hz. "Did someone just pick
              that number? Why not 60 Hz like the Americans?" What do you tell her?
            </>
          }
          whatToDo={
            <>
              Two parts. The CHOICE of 50 Hz over 60 Hz was a historical/political decision made
              when the UK grid was being built in the 1920s — it’s arbitrary, the same way
              left-hand-drive vs right-hand-drive is. But the way 50 Hz is actually PRODUCED today
              is a direct consequence of EMF = NBAω. Every alternator on the grid is mechanically
              locked to the grid frequency: a 2-pole machine spinning at 3000 rpm, a 4-pole at 1500
              rpm. Spin them faster and the grid frequency rises; slow them down and it drops. The
              grid operator (NESO) maintains 50 Hz to within ±1% by constantly balancing how much
              fuel goes into the generators against how much load is being drawn at any instant.
              It’s 50 Hz because we collectively spin every generator on the island at the right
              speed to make it so.
            </>
          }
          whyItMatters={
            <>
              Knowing that frequency comes from rotational speed means you understand why a site
              genset has to be carefully synchronised before it can be paralleled with the grid
              (frequency, phase angle, voltage all matched), and why a 60 Hz American appliance
              won’t run properly off UK 50 Hz mains (motors run slower, transformers heat up). All
              of it traces back to ω = 2πf in this subsection.
            </>
          }
        />
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            onClick={() => navigate('/study-centre/apprentice/level2/module2/section5/5-4')}
