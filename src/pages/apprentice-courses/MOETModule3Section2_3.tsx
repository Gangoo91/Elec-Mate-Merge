        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          <div className="bg-white/5 border border-elec-yellow/20 rounded-lg p-4 sm:p-5">
            ...
            <ul className="text-left text-white text-sm space-y-1.5">
              <li>
                - <strong>Principle:</strong> Start in star (1/3 current), run in delta (full speed)
              </li>
              <li>
                - <strong>Transition:</strong> Open (simple) or closed (no spike)
              </li>
              <li>
                - <strong>Timer:</strong> 3-10 seconds, motor must reach 80-90% speed
              </li>
              <li>
                - <strong>Cables:</strong> Six required (all six terminals individually)
              </li>
            </ul>
          </div>
          <div className="bg-white/5 border border-elec-yellow/20 rounded-lg p-4 sm:p-5">
          ...
          <div className="text-white space-y-4 leading-relaxed">
            ...
            <div className="my-4 p-4 rounded-lg bg-white/5">
              ...
              <ul className="text-white space-y-2 text-left">
                <li>
                  <strong className="text-elec-yellow">DOL Starting Current:</strong> 6 to 8 times
                  FLC
                </li>
                <li>
                  <strong className="text-elec-yellow">Star Starting Current:</strong> Approximately
                  2 to 2.7 times FLC (one-third of DOL)
                </li>
                <li>
                  <strong className="text-elec-yellow">DOL Starting Torque:</strong> 100% of
                  locked-rotor torque
                </li>
                <li>
                  <strong className="text-elec-yellow">Star Starting Torque:</strong> Approximately
                  33% of locked-rotor torque (one-third of DOL)
                </li>
              </ul>
            </div>
            <p>
          ...
          <div className="text-white space-y-4 leading-relaxed">
            ...
            <div className="my-4 p-4 rounded-lg bg-white/5">
              <ul className="text-white space-y-3 text-left">
                <li>
                  <strong className="text-elec-yellow">Timer too short:</strong> The motor has not
                  reached sufficient speed when transition occurs. The delta contactor closes on a
                  motor still drawing high current, creating a large current spike (may approach or
                  exceed DOL levels) and a mechanical shock to the drive coupling. The overload
                  relay may trip.
                </li>
                <li>
                  <strong className="text-elec-yellow">Timer too long:</strong> The motor runs in
                  star longer than necessary. The motor may overheat in star (it is designed to run
                  continuously in delta, not star). Wasted energy and unnecessary supply disturbance
                  from the extended starting period.
                </li>
                <li>
                  <strong className="text-elec-yellow">Correct setting:</strong> The timer should
                  allow the motor to reach approximately 80-90% of full speed in star before
                  transition. This is typically 3 to 10 seconds depending on motor size, load
                  inertia and the torque required to accelerate the load.
                </li>
              </ul>
            </div>
            <p>
            ...
            <div className="my-4 p-4 rounded-lg bg-white/5">
              <ul className="text-white space-y-3 text-left">
                <li>
                  <strong className="text-elec-yellow">Main Contactor (KM1):</strong> Connects the
                  supply to the motor winding starts (U1, V1, W1). Closes first at the start of the
                  sequence and remains closed throughout operation (both star and delta phases).
                </li>
                <li>
                  <strong className="text-elec-yellow">Star Contactor (KM2):</strong> Short-circuits
                  the winding ends (U2, V2, W2) to form the star point. Closes during the star
                  starting period, then opens at transition. Sized for approximately 58% of motor
                  FLC.
                </li>
                <li>
                  <strong className="text-elec-yellow">Delta Contactor (KM3):</strong> Connects each
                  winding end to the opposite phase supply to form the delta configuration. Closes
                  after the star contactor opens and remains closed during running. Sized for
                  approximately 58% of motor FLC.
                </li>
              </ul>
            </div>
            <p className="text-sm text-elec-yellow/70">
              <strong>Critical safety requirement:</strong> The star and delta contactors (KM2 and
              KM3) must be electrically and mechanically interlocked to prevent both closing
              simultaneously. If both closed at the same time, a short circuit would occur across
              the supply, with potentially catastrophic results.
            </p>
          </div>
        </section>
        ...
        <section className="mb-10 mt-10">
          ...
          <div className="text-white space-y-4 leading-relaxed">
            ...
            <div className="my-4 p-4 rounded-lg bg-white/5">
              <ul className="text-white space-y-2 text-left">
                <li>
                  - Starting torque is only one-third of DOL -- unsuitable for high-torque loads
                </li>
                <li>
                  - The motor must have all six winding terminals accessible (400/690 V rating)
                </li>
                <li>
                  - Six cables required from starter to motor (more copper, larger containment,
                  higher cost)
                </li>
                <li>
                  - Current transient at changeover (open transition) can approach or exceed DOL
                  levels
                </li>
                <li>
                  - No smooth acceleration -- a fixed two-step voltage profile with an abrupt
                  transition
                </li>
          ...
          <div className="text-white space-y-4 leading-relaxed">
            ...
            <div className="my-4 p-4 rounded-lg bg-white/5">
              <ul className="text-white space-y-4 text-left">
                <li>
                  <strong className="text-elec-yellow">Motor trips on transition:</strong> Timer set
                  too short (motor has not reached speed). Overload relay trips on the high delta
                  transition current. Solution: increase timer duration. If the problem persists,
                  check for mechanical overload on the driven equipment preventing the motor from
                  reaching speed in star.
                </li>
                <li>
                  <strong className="text-elec-yellow">
                    Motor runs in star but will not transition:
                  </strong>{' '}
                  Timer fault (no output signal), delta contactor coil failure, or interlocking
                  issue preventing the delta contactor from closing. Check: timer output voltage,
                  delta coil continuity, and interlock contact status.
                </li>
                <li>
                  <strong className="text-elec-yellow">Motor runs in wrong direction:</strong> Two
                  phases swapped at the motor terminals or starter output. Correction: swap any two
                  of the three main supply connections at the starter input. Do NOT swap at the
                  motor terminal box, as this can disrupt the star-delta winding configuration.
                </li>
                <li>
                  <strong className="text-elec-yellow">Loud bang at transition:</strong> Open
                  transition with poor timing. The residual motor voltage is out of phase with the
                  supply when delta closes, causing a violent current transient. Solution: check and
                  adjust timer setting. If persistent, consider closed transition modification.
                </li>
                <li>
                  <strong className="text-elec-yellow">
                    High current persists after transition:
                  </strong>{' '}
                  One phase of the delta contactor not making contact (single-phasing in delta). The
                  motor draws excessive current on the remaining two phases. Check delta contactor
                  contacts and connections for damage or poor contact.
                </li>
                <li>
                  <strong className="text-elec-yellow">Motor does not start at all:</strong> Check:
                  supply voltage at the starter input; main contactor operation; star contactor
                  operation; control circuit fuses and connections; overload relay status (reset if
                  tripped).
                </li>
              </ul>
            </div>
