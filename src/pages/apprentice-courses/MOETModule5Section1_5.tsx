...
const DESCRIPTION =
  'Comprehensive guide to signal conditioning for electrical maintenance technicians: amplification, filtering, linearisation, signal conversion, 4-20 mA loops and noise reduction techniques. ST1426 aligned.';
...
const quickCheckQuestions = [
  {
    ...
    correctIndex: 1,
    explanation:
      'Signal conditioning takes the raw, often weak or noisy signal from a sensor and processes it into a standardised, clean signal that the PLC or DCS can accurately read. This may involve amplification, filtering, linearisation, isolation or conversion between signal types.',
  },
  {
    id: '4-20ma-live-zero',
    ...
    correctIndex: 2,
    explanation:
      "The 'live zero' at 4 mA means that if the loop current drops to 0 mA, the system knows there is a fault such as a broken wire or failed transmitter. A genuine zero process reading still produces 4 mA. This is a fundamental safety feature of the 4-20 mA standard.",
  },
  {
    id: 'noise-filtering',
    ...
    correctIndex: 2,
    explanation:
      'A low-pass filter attenuates frequencies above a set cut-off point. In instrumentation, the process variable changes slowly compared to electrical noise, so a low-pass filter removes the unwanted high-frequency noise while preserving the genuine measurement signal.',
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'Galvanic isolation uses transformers or opto-couplers to electrically separate the field wiring from the control system. This prevents earth loops (which cause measurement errors), protects expensive control equipment from field faults, and blocks hazardous voltages from reaching the control room.',
  },
];
...
const quizQuestions = [
  {
    ...
    correctAnswer: 1,
    explanation:
      'Signal conditioning encompasses all the processing steps between the raw sensor output and the control system input. This includes amplification, filtering, linearisation, isolation and signal conversion.',
  },
  {
    id: 2,
    ...
    correctAnswer: 2,
    explanation:
      'The 4-20 mA range spans 16 mA (from 4 to 20). 12 mA is 8 mA above the zero point (4 mA), which is 8/16 = 50% of the measured range. The formula is: percentage = (mA - 4) / 16 x 100.',
  },
  {
    id: 3,
    ...
    correctAnswer: 2,
    explanation:
      'Thermocouples produce very small DC voltages in the millivolt range, typically 0-50 mV depending on the thermocouple type and temperature range. This weak signal requires amplification and cold junction compensation as part of signal conditioning before it can be used by a control system.',
  },
  {
    id: 4,
    ...
    correctAnswer: 1,
    explanation:
      'Many sensors have a non-linear relationship between the measured variable and their output. For example, thermocouple voltage does not change linearly with temperature. Linearisation applies a mathematical correction so the conditioned output is directly proportional to the actual temperature, pressure or flow.',
  },
  {
    id: 5,
    question:
    ...
    correctAnswer: 1,
    explanation:
      'Instrumentation amplifiers are specifically designed for measurement applications. They offer very high common-mode rejection ratio (CMRR) to reject noise common to both inputs, very high input impedance so they do not load the sensor, and precise, stable gain set by a single resistor.',
  },
  {
    id: 6,
  ...
  },
  {
    ...
    correctAnswer: 1,
    explanation:
      'Proper installation of signal conditioning equipment requires: correct supply voltage, proper single-point earthing, screened cables routed away from power cables (minimum 300 mm separation), and verification of calibration after installation. All work should be documented with wiring diagrams, calibration records and equipment data sheets.',
  },
];
...
const faqs = [
  {
    ...
    answer:
      "A transducer converts one form of energy to another (e.g., a thermocouple converts heat to voltage). A transmitter is a complete signal conditioning device that takes the transducer output and produces a standardised signal (typically 4-20 mA) suitable for transmission to the control system. Many modern instruments combine both functions in a single device — a 'smart transmitter' that includes the sensor, signal conditioning, linearisation and communication in one housing.",
  },
  {
    question: 'Why is 4-20 mA preferred over 0-10 V in industrial environments?',
    answer:
      'Current signals are preferred because they are immune to voltage drops caused by cable resistance over long distances. A 4-20 mA signal reads the same regardless of cable length, whereas a 0-10 V signal would lose accuracy due to IR drop. Current loops also have better noise immunity in electrically noisy industrial environments and provide built-in fault detection through the live zero at 4 mA.',
  },
  {
    question: 'What is a HART communicator and when would I use one?',
    answer:
      'A HART communicator is a handheld device that connects to a 4-20 mA loop and communicates digitally with HART-enabled transmitters. It allows you to configure the transmitter (range, damping, units), read diagnostic information, perform calibration and access additional process variables — all without disconnecting the loop. As a maintenance technician, you would use it during commissioning, calibration and fault-finding on HART-enabled instruments.',
  },
  {
    question: 'How do I choose the correct signal conditioning module?',
    answer:
      "Select based on: (1) the sensor type and its output signal (mV, resistance, frequency, etc.), (2) the required output signal for your control system (4-20 mA, 0-10 V, digital), (3) environmental conditions (temperature, humidity, hazardous area classification), (4) isolation requirements (galvanic isolation needed if earth loops are likely), and (5) accuracy and response time requirements. Always check the manufacturer's datasheet for compatibility with your specific sensor and control system.",
  },
  {
    question: 'What is an earth loop and how does it affect measurements?',
    answer:
      'An earth loop occurs when there are multiple earth connections in a signal circuit, creating a closed loop through which unwanted currents can flow. These currents add to the measurement signal, causing errors — often seen as a fluctuating or offset reading. Prevention methods include: using galvanically isolated signal conditioners, earthing cable screens at one end only, using differential inputs on the PLC, and ensuring a single-point earthing strategy for the instrumentation system.',
  },
];
        ...
        <section className="mb-10">
          ...
          <div className="text-white space-y-4 leading-relaxed">
            <p>
              ...
              millivolts, RTDs change resistance, strain gauges produce microvolts, and capacitive
              sensors produce tiny changes in capacitance. Signal conditioning is the collective
              term for all the processing applied to convert these raw signals into standardised,
              reliable outputs suitable for measurement, control and recording.
            </p>
            <p>
              For the maintenance technician, understanding signal conditioning is essential because
              most instrument faults present as signal problems. A drifting reading, an intermittent
              signal, an offset error or a noisy display — all of these may originate in the signal
        ...
        <section className="mb-10 mt-10">
          ...
          <div className="text-white space-y-4 leading-relaxed">
            <p>
              ...
              over long distances and in electrically noisy environments. Understanding this
