...
const validateZs = (value: string, protectiveDevice: string): ValidationResult => {
  ...
  return {
    isValid: true,
    level: 'pass',
    message: 'Zs within typical range (specify protective device for accurate validation)',
  };
};
...
const validateRCDTiming = (rating: string, oneX: string): ValidationResult => {
  ...
  if (!limits) {
    return {
      isValid: false,
      level: 'warning',
      message: 'Unusual RCD rating - verify requirements',
    };
  }
...
const validateFunctionalTesting = (value: string): ValidationResult => {
  ...
  return {
    isValid: false,
    level: 'warning',
    message: 'Functional testing result unclear - use ✓ or ✗',
  };
};
...
): { status: 'pass' | 'warning' | 'fail'; criticalIssues: string[] } => {
...
export const validateNeutralCurrent = (
  ...
  const neutralCurrent = Math.sqrt(l1 * l1 + l2 * l2 + l3 * l3 - l1 * l2 - l2 * l3 - l1 * l3);
 ...
 */
export const getThreePhaseCompliance = (
  validation: ThreePhaseValidationResult
): {
