...
export const formatFireAlarmG6Json = (formData: Record<string, any>) => {
  ...
  return {
    ...
    // Previous Defects (G6 unique)
    previous_defects: (formData.previousDefects || []).map((d: any, i: number) => ({
      number: i + 1,
      description: d.description || '',
    ...
    // Extent of Inspection (G6 unique)
    extent_of_inspection: get('extentOfInspection'),
    is_full_inspection: get('extentOfInspection') === 'full-system',
    is_partial_inspection: get('extentOfInspection') === 'partial',
    agreed_scope: get('agreedScope'),
    inspection_limitations: get('inspectionLimitations'),

    // Building Changes (G6 unique)
    changes_structural: getBool('changesStructural'),
    changes_occupancy: getBool('changesOccupancy'),
    changes_escape_routes: getBool('changesEscapeRoutes'),
    changes_environmental: getBool('changesEnvironmental'),
    building_change_notes: get('buildingChangeNotes'),
    has_building_changes: !!(
      getBool('changesStructural') ||
      getBool('changesOccupancy') ||
      getBool('changesEscapeRoutes') ||
      getBool('changesEnvironmental')
    ),

    // Logbook
    logbook_reviewed: getBool('logbookReviewed'),
    logbook_notes: get('logbookNotes'),

    // Weekly Testing (G6 unique)
    weekly_testing_confirmed: getBool('weeklyTestingConfirmed'),
    weekly_records_reviewed: getBool('weeklyRecordsReviewed'),

    // False Alarms (G6 unique)
    false_alarm_count: get('falseAlarmCount'),
    false_alarm_cause: get('falseAlarmCause'),
