...
const REGULATION_EXPLANATIONS: Record<string, RegulationExplanation> = {
  'BS 7671 Regulation 411.4.5': {
    ...
    plainEnglish:
      'The earth fault loop impedance (Zs) must be low enough to ensure the protective device operates quickly in case of an earth fault.',
  ...
  'BS 7671 Regulation 411.3.3': {
    ...
    plainEnglish:
      'Socket outlets and circuits supplying portable equipment must have RCD protection (usually 30mA).',
  ...
  'BS 7671 Appendix 15': {
    ...
    plainEnglish:
      'Ring final circuits must be properly designed and tested to ensure load is shared equally between both legs.',
  ...
  'BS 7671 Regulation 526.3': {
    ...
    plainEnglish:
      'All electrical connections must be accessible for inspection, testing and maintenance.',
  ...
  'BS 7671 Regulation 314.1': {
    ...
    plainEnglish:
      'Every installation must be divided into circuits to avoid danger and minimise inconvenience in case of fault.',
...
export const getContextualSuggestions = (warning: RegulationWarning): string[] => {
  ...
  if (!explanation) {
    ...
    if (
      warning.title.toLowerCase().includes('zs') ||
      warning.title.toLowerCase().includes('impedance')
    ) {
...
export const generateRegulationReport = (
  ...
  if (criticalCount > 0) {
    ...
    overallAssessment =
      'No regulation compliance issues detected. Installation meets BS 7671 requirements.';
  }
