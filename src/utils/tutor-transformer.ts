...
export function transformTutorOutputToPDF(
  ...
  const conceptContent =
    ...
    (tutorOutput.conceptExplanation.commonMisconceptions &&
    tutorOutput.conceptExplanation.commonMisconceptions.length > 0
  ...
  if (tutorOutput.calculations) {
    ...
    calcContent += calc.variablesExplained
      .map((v) => `• **${v.symbol}** = ${v.meaning} (${v.unit})`)
      .join('\n');
  ...
  const studyContent =
    ...
    `**Recommended Study Activities:**\n${tutorOutput.studyPlan.recommendedActivities
      .map((a, i) => {
  ...
  const referencesContent = tutorOutput.bs7671References
    .map((ref) => {
      const freq = ref.examFrequency
        ? ` [Exam frequency: ${ref.examFrequency.replace('_', ' ')}]`
        : '';
