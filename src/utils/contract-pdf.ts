...
function htmlToSections(html: string): {
  title: string;
  sections: Array<{ heading: string; content: string[]; isList?: boolean }>;
} {
...
export function generateContractPDF(options: ContractPdfOptions): jsPDF {
  ...
  const addFooter = (pageNum: number, totalPages: number) => {
    ...
    // Right: Page number
    doc.setFont('helvetica', 'normal');
    doc.text(`Page ${pageNum} of ${totalPages}`, pageWidth - margin, footerY + 5, {
      align: 'right',
    });
    ...
    doc.text('Generated by Elec-Mate HR Management System', pageWidth / 2, footerY + 10, {
      align: 'center',
    });
  };
  ...
  doc.text(
    `CON-${category.substring(0, 3).toUpperCase()}-${version.replace('.', '')}`,
    col2,
    ctrlY
  );
  ...
  const statusColor =
    status === 'Active' ? COLORS.SUCCESS : status === 'Draft' ? COLORS.WARNING : COLORS.TEXT_MUTED;
  ...
  doc.setFontSize(7);
  doc.setFont('helvetica', 'bold');
  doc.text(status.toUpperCase(), col2 + statusBadgeWidth / 2 - 2, ctrlY + 26.5, {
    align: 'center',
  });
  ...
  const declaration =
    'By signing below, both parties confirm they have read, understood, and agree to be bound by the terms and conditions set out in this contract. This contract becomes legally binding upon signature by both parties.';
