...
const CATEGORY_COLORS: Record<string, [number, number, number]> = {
  ...
  "Subcontractor": COLORS.INFO,
  "HR Letters": COLORS.SUCCESS,
};

// Category labels for cover page
const CATEGORY_LABELS: Record<string, string> = {
  "Employment": "EMPLOYMENT CONTRACT",
  "Subcontractor": "SUBCONTRACTOR AGREEMENT",
  "HR Letters": "HR DOCUMENT",
};
...
export function generateContractPDF(options: ContractPdfOptions): jsPDF {
  ...
  const addFooter = (pageNum: number, totalPages: number) => {
    ...
    doc.text("CONFIDENTIAL - Legally binding document", margin, footerY + 5);
    ...
    doc.text("Generated by Elec-Mate HR Management System", pageWidth / 2, footerY + 10, { align: "center" });
  };
  ...
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text(categoryLabel, pageWidth / 2, 30, { align: "center" });
  ...
  // Large contract title
  doc.setTextColor(...COLORS.PRIMARY);
  doc.setFontSize(24);
  doc.setFont("helvetica", "bold");

  const titleLines = doc.splitTextToSize(name, contentWidth - 20);
  ...
  doc.text(`CON-${category.substring(0, 3).toUpperCase()}-${version.replace(".", "")}`, col2, ctrlY);
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...COLORS.TEXT_MUTED);
  doc.text("Start Date:", col1, ctrlY + 16);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...COLORS.TEXT_DARK);
  const startDateStr = userContract?.start_date
    ? format(new Date(userContract.start_date), "dd MMMM yyyy")
    : "[Start Date]";
  doc.text(startDateStr, col2, ctrlY + 16);

  doc.setFont("helvetica", "normal");
  doc.setTextColor(...COLORS.TEXT_MUTED);
  doc.text("Employee:", col3, ctrlY + 16);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...COLORS.TEXT_DARK);
  doc.text(userContract?.party_name || "[Employee Name]", col4, ctrlY + 16);

  // Row 4 - Status badge
  const status = userContract?.status || "Active";
  const statusColor = status === "Active" ? COLORS.SUCCESS :
                      status === "Draft" ? COLORS.WARNING : COLORS.TEXT_MUTED;
  ...
  // Legal notice at bottom of cover
  yPos = pageHeight - 45;
  ...
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
  doc.text("LEGAL NOTICE", margin + 8, yPos + 6);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6.5);
  const legalText = "This document constitutes a legally binding agreement. Both parties should read all terms carefully before signing. Seek independent legal advice if required. This contract complies with UK employment law including the Employment Rights Act 1996.";
  ...
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.text("CONTRACT EXECUTION", pageWidth / 2, yPos + 10, { align: "center" });
  ...
  const declaration = "By signing below, both parties confirm they have read, understood, and agree to be bound by the terms and conditions set out in this contract. This contract becomes legally binding upon signature by both parties.";
...
export async function downloadContractPDF(options: ContractPdfOptions): Promise<void> {
  const name = options.userContract?.title || options.template?.name;
  if (!name) {
    throw new Error("No contract data provided");
  }
