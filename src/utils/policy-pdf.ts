...
export function generatePolicyPDF(options: PolicyPdfOptions): jsPDF {
  ...
  const addFooter = (pageNum: number, totalPages: number) => {
    ...
    doc.text('Generated by Elec-Mate Policy Management System', pageWidth / 2, footerY + 10, {
      align: 'center',
    });
  };
  ...
  doc.text(
    `POL-${category.substring(0, 3).toUpperCase()}-${version.replace('.', '')}`,
    col2,
    ctrlY
  );
  ...
  const declaration =
    'I confirm that I have read, understood, and agree to comply with this policy. I understand my responsibilities as outlined and the consequences of non-compliance. I have had the opportunity to ask questions and clarify any points.';
