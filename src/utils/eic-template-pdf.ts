...
export function generateEICTemplatePDF(data: EICData): jsPDF {
  ...
  doc.setTextColor(0, 0, 0);
  doc.text(
    'Please attach the completed Schedule of Test Results to this certificate.',
    pageWidth / 2,
    yPos + 15,
    { align: 'center' }
  );
  doc.text(
    'See Test Schedule Template document for recording actual test values.',
    pageWidth / 2,
    yPos + 20,
    { align: 'center' }
  );
  ...
  doc.text('Electrical Installation Certificate - Template', pageWidth / 2, footerY, {
    align: 'center',
  });
  doc.text(
    'This is a template document. All sections must be completed by qualified personnel.',
    pageWidth / 2,
    footerY + 5,
    { align: 'center' }
  );
