...
export function generateComplianceAuditPack(options: AuditPackOptions): jsPDF {
  ...
  const addFooter = (pageNum: number, totalPages: number) => {
    ...
    doc.text(
      `Generated by Elec-Mate | ${generatedDate} ${generatedTime}`,
      pageWidth / 2,
      footerY + 8,
      { align: 'center' }
    );
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(
    `CP-${format(new Date(), 'yyyyMMdd')}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`,
    margin + 60,
    infoY
  );
  ...
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  doc.text(String(totalQualifications), statsStart + statsWidth / 2, yPos + 12, {
    align: 'center',
  });
  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(
    String(totalBusinessDocs),
    statsStart + statsWidth + statsGap + statsWidth / 2,
    yPos + 12,
    { align: 'center' }
  );
  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  doc.text('Business', statsStart + statsWidth + statsGap + statsWidth / 2, yPos + 18, {
    align: 'center',
  });
  doc.text('Documents', statsStart + statsWidth + statsGap + statsWidth / 2, yPos + 22, {
    align: 'center',
  });
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(
    String(totalComplianceDocs),
    statsStart + (statsWidth + statsGap) * 2 + statsWidth / 2,
    yPos + 12,
    { align: 'center' }
  );
  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  doc.text('Compliance', statsStart + (statsWidth + statsGap) * 2 + statsWidth / 2, yPos + 18, {
    align: 'center',
  });
  doc.text('Documents', statsStart + (statsWidth + statsGap) * 2 + statsWidth / 2, yPos + 22, {
    align: 'center',
  });
  ...
  tocItems.forEach((item) => {
    ...
    doc.line(
      margin + doc.getTextWidth(item.title) + 5,
      yPos - 1,
      pageWidth - margin - 15,
      yPos - 1
    );
  ...
  if (employeeQualifications.length > 0) {
    ...
    employeeQualifications.forEach((qual, index) => {
      ...
      doc.setTextColor(...COLORS.WHITE);
      doc.setFontSize(6);
      doc.text(statusText.substring(0, 10), qColStatus + statusWidth / 2, yPos + 5, {
        align: 'center',
      });
      doc.setFontSize(8);
  ...
  if (businessCompliance.length > 0) {
    ...
    businessCompliance.forEach((item, index) => {
      ...
      doc.setTextColor(...COLORS.WHITE);
      doc.setFontSize(6);
      doc.text(statusText.substring(0, 8), bColStatus + statusWidth / 2, yPos + 5, {
        align: 'center',
      });
      doc.setFontSize(8);
  ...
  if (complianceDocuments.length > 0) {
    ...
    complianceDocuments.forEach((doc_item, index) => {
      ...
      doc.setTextColor(...COLORS.WHITE);
      doc.setFontSize(6);
      doc.text(statusText.substring(0, 8), cColStatus + statusWidth / 2, yPos + 5, {
        align: 'center',
      });
      doc.setFontSize(8);
