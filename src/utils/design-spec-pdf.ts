...
export function generateDesignSpecificationPDF(design: InstallationDesign): jsPDF {
  ...
  const addFooter = () => {
    ...
    doc.text('Confidential - For Client Use Only', pageWidth / 2, pageHeight - 10, {
      align: 'center',
    });
  };
  ...
  const summaryData = [
    ...
    [
      'Diversified Load',
      design.diversityApplied
        ? `${(design.totalLoad * (design.diversityFactor || 0.8)).toFixed(0)}W`
        : 'N/A',
    ],
  ...
  design.circuits.forEach((circuit, idx) => {
    ...
    const circuitSpecs = [
      ...
      [
        'Voltage',
        circuit.phases === 'single' ? `${circuit.voltage}V Single Phase` : `400V 3Φ (230V P-N)`,
      ],
    ...
    const cableProtection = [
      ...
      [
        'Cable Type',
        circuit.cableType || `${circuit.cableSize ?? 'N/A'}/${circuit.cpcSize ?? 'N/A'}mm² T&E`,
      ],
      ...
      [
        'RCD Protected',
        circuit.rcdProtected ? `Yes (${circuit.protectionDevice?.rcdRating || 30}mA)` : 'No',
      ],
    ...
    const calculations = [
      ...
      [
        'Zs (Calculated)',
        `${(circuit.expectedTests?.zs?.expected ?? circuit.calculations.zs).toFixed(3)}Ω`,
      ],
      [
        'Max Zs Permitted',
        `${(circuit.expectedTests?.zs?.maxPermitted ?? circuit.calculations.maxZs).toFixed(3)}Ω`,
      ],
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    `Document Reference: DS-${design.projectName.replace(/\s+/g, '_')}-${format(new Date(), 'ddMMyyyy-HHmm')}`,
    15,
    yPos
  );
