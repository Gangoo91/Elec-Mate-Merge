...
class MethodStatementPDFGenerator {
  ...
  private addTitlePage(data: MethodStatementData, options: PDFOptions): void {
    ...
    this.doc.setFont('helvetica', 'normal');
    const purposeText =
      'This Method Statement provides detailed procedures for safe execution of work activities, ensuring compliance with Health & Safety regulations and industry best practices.';
  ...
  private addJobDescription(data: MethodStatementData): void {
    ...
    this.doc.setFont('helvetica', 'normal');
    const wrappedDesc = this.doc.splitTextToSize(
      description,
      this.pageWidth - 2 * this.MARGIN - 10
    );
    ...
    const summaryData = [
      ...
      [
        'Medium Risk Steps',
        data.steps.filter((step) => step.riskLevel === 'medium').length.toString(),
      ],
  ...
  private addMethodSteps(data: MethodStatementData): void {
    ...
    this.doc.setFont('helvetica', 'bold');
    this.doc.text(
      '4. METHOD STATEMENT - SAFE SYSTEM OF WORK',
      this.pageWidth / 2,
      this.yPosition + 8,
      { align: 'center' }
    );
  ...
  private renderStepContent(step: MethodStep): void {
    ...
    const stepBgColor: [number, number, number] =
      step.stepNumber % 2 === 0 ? [250, 250, 250] : [255, 255, 255];
    ...
    if (hasProcedureSteps) {
      ...
      lines.forEach((line) => {
        ...
        if (trimmed) {
          ...
          const wrappedLine = this.doc.splitTextToSize(
            trimmed,
            this.pageWidth - indent - this.MARGIN - 5
          );
  ...
  private renderStepDetailBoxes(step: MethodStep): void {
    ...
    if (step.safetyRequirements && step.safetyRequirements.length > 0) {
      ...
      step.safetyRequirements.forEach((req) => {
        this.checkPageBreak(6);
        const wrappedReq = this.doc.splitTextToSize(
          `• ${req}`,
          this.pageWidth - 2 * this.MARGIN - 20
        );
    ...
    if (step.equipmentNeeded && step.equipmentNeeded.length > 0) {
      ...
      step.equipmentNeeded.forEach((equip) => {
        this.checkPageBreak(6);
        const wrappedEquip = this.doc.splitTextToSize(
          `• ${equip}`,
          this.pageWidth - 2 * this.MARGIN - 20
        );
    ...
    if (step.qualifications && step.qualifications.length > 0) {
      ...
      step.qualifications.forEach((qual) => {
        this.checkPageBreak(6);
        const wrappedQual = this.doc.splitTextToSize(
          `• ${qual}`,
          this.pageWidth - 2 * this.MARGIN - 20
        );
    ...
    if (step.notes && step.notes.toLowerCase().includes('critical')) {
      ...
      this.doc.setFont('helvetica', 'normal');
      this.doc.setTextColor(0, 0, 0);
      const wrappedNotes = this.doc.splitTextToSize(
        step.notes,
        this.pageWidth - 2 * this.MARGIN - 20
      );
  ...
  private addSafetyRequirements(data: MethodStatementData): void {
    ...
    const safetyData = Array.from(allSafetyReqs).map((req, index) => [`${index + 1}`, req]);
...
export async function downloadMethodStatementPDF(
  ...
  const filename = `${safeFileName(data.jobTitle)}_Method_Statement_${formatDate(new Date(), 'ddMMyyyy')}.pdf`;
