...
export function generateRAMSPDF(data: RAMSData, options: RAMSReportOptions & { signOff?: any } = {}) {
  const doc = new jsPDF();
...
// New function for PDF preview (returns blob URL instead of downloading)
export function generateRAMSPDFPreview(data: RAMSData, options: RAMSReportOptions & { signOff?: any } = {}): Promise<string> {
  return new Promise((resolve) => {
    ...
    if (options.signOff && options.includeSignatures) {
      doc.addPage();
      yPosition = 20;
      ...
      const signOffData = [];
      if (options.signOff.preparedBy?.name) {
        signOffData.push(["Prepared by:", options.signOff.preparedBy.name, "Date:", options.signOff.preparedBy.date || ""]);
      }
      if (options.signOff.reviewedBy?.name) {
        signOffData.push(["Reviewed by:", options.signOff.reviewedBy.name, "Date:", options.signOff.reviewedBy.date || ""]);
      }
      if (options.signOff.approvedBy?.name) {
        signOffData.push(["Approved by:", options.signOff.approvedBy.name, "Date:", options.signOff.approvedBy.date || ""]);
      }
