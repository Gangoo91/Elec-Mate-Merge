...
function addQuoteContent(doc: jsPDF, data: any, startY: number): number {
  ...
  const items = data.items || [
    { description: 'Installation', quantity: 1, unitPrice: 0, total: 0 },
  ];
...
function addRAMSContent(doc: jsPDF, data: any, startY: number): number {
  ...
  if (data.hazards && data.hazards.length > 0) {
    (doc as any).autoTable({
      ...
      body: data.hazards.map((h: any) => [h.hazard, h.risk, h.likelihood, h.severity, h.controls]),
      theme: 'grid',
      headStyles: { fillColor: [30, 58, 138] },
      columnStyles: {
        4: { cellWidth: 60 },
