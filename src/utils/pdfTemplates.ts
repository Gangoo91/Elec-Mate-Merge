...
export const createEICRTemplate = (): EICRPDFTemplate => {
  const addHeader = (pdf: jsPDF, formData: any): number => {
    ...
    const certRef =
      formData.certificateReference || formData.certificateNumber || `EICR-${year}-${randomId}`;
  ...
  const addFooter = (pdf: jsPDF, pageNumber: number, totalPages: number, formData?: any): void => {
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(0, 0, 0);
    pdf.text(`Page ${pageNumber} of ${totalPages}`, pageWidth - 20, pageHeight - 8, {
      align: 'right',
    });
  ...
  const addBrandingElements = (pdf: jsPDF, formData?: any): void => {
    ...
    if (formData?.companyName || formData?.companyTagline) {
      ...
      if (formData.companyWebsite) {
        ...
        pdf.setFont('helvetica', 'normal');
        const websiteY =
          formData.companyName && formData.companyTagline
            ? startY + 10
            : formData.companyName || formData.companyTagline
              ? startY + 5
              : startY;
  ...
  const addInspectionChecklistSection = (
    pdf: jsPDF,
    inspectionItems: any[],
    yPosition: number
  ): number => {
    ...
    pdf.setTextColor(102, 102, 102);
    pdf.text(
      'Schedule of items inspected in accordance with BS 7671:2018 (IET Wiring Regulations)',
      20,
      yPosition
    );
    yPosition += 12;
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(
      completionRate >= 90 ? 0 : completionRate >= 70 ? 255 : 204,
      completionRate >= 70 ? 150 : 102,
      0
    );
    ...
    sectionMap.forEach((items, sectionId) => {
      ...
      items.forEach((item: any, index: number) => {
        ...
        const outcomeColor =
          item.outcome === 'satisfactory'
            ? [34, 139, 34]
            : ['C1', 'C2', 'not-satisfactory'].includes(item.outcome)
              ? [220, 20, 60]
              : item.outcome === 'C3'
                ? [255, 140, 0]
                : [128, 128, 128];
