...
export const generateLaTeXStylePDF = async (
  ...
  try {
    ...
    const createTableOfContents = (sections: DocumentSection[]) => {
      ...
      sections.forEach((section) => {
        if (section.level > 0 && section.level <= 3) {
          ...
          const dotsSpace =
            pageWidth - margins.right - margins.left - indent - textWidth - pageNumWidth - 5;
    ...
    const renderContent = (sections: DocumentSection[]) => {
      sections.forEach((section, index) => {
        ...
        if (section.level > 0) {
          ...
          if (section.level <= 2) {
            pdf.setDrawColor(
              section.level === 1 ? 70 : 150,
              section.level === 1 ? 130 : 150,
              section.level === 1 ? 180 : 150
            );
            pdf.setLineWidth(section.level === 1 ? 1.5 : 0.8);
            pdf.line(
              margins.left,
              yPosition + 2,
              margins.left + pdf.getTextWidth(section.title),
              yPosition + 2
            );
          }
    ...
    const addSignatureSection = () => {
      ...
      // Client signature
      pdf.rect(
        margins.left + signatureBoxWidth + 10,
        yPosition,
        signatureBoxWidth,
        signatureBoxHeight
      );
      pdf.text(
        'For the person ordering the work:',
        margins.left + signatureBoxWidth + 12,
        yPosition + 8
      );
    ...
    throw new Error(
      `LaTeX-style PDF generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
};
