...
export class FirecrawlService {
  ...
  static async fetchLiveNews(
    onProgress?: (progress: { current: number; total: number; source: string }) => void
  ): Promise<CrawlResult> {
    try {
      ...
      const formattedArticles: NewsArticle[] =
  ...
  private static parseScrapedContent(
    scrapedData: any,
    source: { name: string; regulatoryBody: string; category: string }
  ): Omit<NewsArticle, 'id'>[] {
    ...
    try {
      ...
      if (source.regulatoryBody === 'HSE') {
        ...
        contentSections = content.split(
          /(?=Press release|Safety alert|Enforcement notice|Improvement notice)/gi
        );
      ...
      contentSections.forEach((section: string, index: number) => {
        ...
        if (source.regulatoryBody === 'HSE') {
          const hseTitleMatch = lines.find(
            (line) =>
          ...
          title =
            hseTitleMatch?.replace(/^#+\s*|\*\*|\*|<[^>]*>/g, '').trim() ||
            `HSE Safety Update - ${index + 1}`;
        } else if (source.regulatoryBody === 'IET') {
          const ietTitleMatch = lines.find(
            (line) =>
          ...
          title =
            ietTitleMatch?.replace(/^#+\s*|\*\*|\*|<[^>]*>/g, '').trim() ||
            `BS7671/IET Update - ${index + 1}`;
        } else if (source.category === 'Major Projects') {
          const projectTitleMatch = lines.find(
            (line) =>
          ...
          title =
            projectTitleMatch?.replace(/^#+\s*|\*\*|\*|<[^>]*>/g, '').trim() ||
        ...
        const dateMatch = cleanSection.match(
      ...
      return articles.sort((a, b) => b.keywords.length - a.keywords.length).slice(0, 4); // Limit to 4 articles per source
    } catch (error) {
  ...
  private static generateSummary(content: string): string {
    ...
    const relevantSentences = sentences.filter((sentence) =>
      electricalKeywords.some((keyword) => sentence.toLowerCase().includes(keyword.toLowerCase()))
    );

    const sentencesToUse =
      relevantSentences.length > 0 ? relevantSentences.slice(0, 2) : sentences.slice(0, 2);
      ...
      sections.forEach((section, index) => {
        ...
        const title =
          lines
            ...
            ?.replace(/^#+\s*|\*\*|\*|<[^>]*>/g, '')
            .trim() || `Electrical Project ${index + 1}`;
        ...
        const locationMatch = cleanSection.match(
          /(London|Manchester|Birmingham|Leeds|Glasgow|Edinburgh|Cardiff|Belfast|[A-Z][a-z]+ [A-Z][a-z]+, UK)/i
        );
