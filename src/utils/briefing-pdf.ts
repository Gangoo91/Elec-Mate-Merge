...
export function generateBriefingPDF(options: BriefingPdfOptions): jsPDF {
  ...
  const addFooter = (pageNum: number, totalPages: number) => {
    ...
    doc.text(`Generated by Elec-Mate | ${generatedDate}`, pageWidth / 2, footerY + 8, {
      align: 'center',
    });
  };
  ...
  const infoItems = [
    ...
    {
      label: 'Duration',
      value: briefing.duration_minutes ? `${briefing.duration_minutes} min` : '-',
    },
  ];
  ...
  if (attendees.length > 0) {
    ...
    for (const attendee of attendees) {
      ...
      const rowBg = attendees.indexOf(attendee) % 2 === 0 ? COLORS.WHITE : COLORS.LIGHTER_BG;
