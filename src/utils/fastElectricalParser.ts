...
export function fastParseElectrical(text: string, availableFields: any[]): Record<string, any> {
  ...
  if (lowerText.includes('tn-s') || lowerText.includes('tns') || lowerText.match(/\btn\s*s\b/)) {
    if (availableFields.some((f) => f.name === 'earthingArrangement'))
      extracted.earthingArrangement = 'TN-S';
  } else if (
    lowerText.includes('tn-c-s') ||
    lowerText.includes('tncs') ||
    lowerText.includes('tn c s') ||
    lowerText.includes('pme')
  ) {
    if (availableFields.some((f) => f.name === 'earthingArrangement'))
      extracted.earthingArrangement = 'TN-C-S';
  } else if (lowerText.match(/\btt\b/) || lowerText.includes('t t')) {
    if (availableFields.some((f) => f.name === 'earthingArrangement'))
      extracted.earthingArrangement = 'TT';
  }

  // Phases
  if (
    lowerText.includes('single phase') ||
    lowerText.match(/\bsingle\b/) ||
    lowerText.includes('1 phase')
  ) {
    if (availableFields.some((f) => f.name === 'phases')) extracted.phases = '1';
  } else if (
    lowerText.includes('three phase') ||
    lowerText.match(/\bthree\b/) ||
    lowerText.includes('3 phase')
  ) {
  ...
  if (
    postcodeMatch &&
    availableFields.some((f) => f.name === 'clientAddress' || f.name === 'installationAddress')
  ) {
    const formatted = postcodeMatch[1]
      .toUpperCase()
      .replace(/\s+/g, '')
      .replace(/^(.+)(\d[A-Z]{2})$/, '$1 $2');
    if (availableFields.some((f) => f.name === 'clientAddress'))
      extracted.clientAddress = formatted;
  }
