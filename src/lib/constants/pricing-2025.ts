...
export const MARKET_RATES_2025 = {
  ...
  hourlyRates: {
    apprentice: { min: 12.21, max: 18.0, typical: 15.0 }, // Based on 2025 minimum wage
    improver: { min: 18.0, max: 25.0, typical: 22.0 },
    qualified: { min: 24.0, max: 35.0, typical: 28.0 },
    experienced: { min: 30.0, max: 45.0, typical: 38.0 },
    specialist: { min: 40.0, max: 65.0, typical: 52.0 },
  },

  // Regional multipliers for 2025
  regionalMultipliers: {
    london: 1.35, // Increased from 1.25 due to living costs
    southeast: 1.2, // Increased from 1.15
  ...
  materialInflation: {
    ...
    general: 1.1, // 10% average across all materials
  },
...
export const calculateRegionalRate = (
  baseRate: number,
...
export const calculateMaterialCost = (
  baseCost: number,
  materialType: keyof typeof MARKET_RATES_2025.materialInflation
): number => {
  return baseCost * MARKET_RATES_2025.materialInflation[materialType];
};

export const getRecommendedHourlyRate = (
