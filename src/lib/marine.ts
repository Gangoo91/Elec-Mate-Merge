...
export function calculateMarine(inputs: MarineInputs): MarineResults {
  ...
  const requiredBatteryCapacity =
    (dailyEnergyConsumption * autonomyDays * temperatureFactor) / usableDepth;
  ...
  const chargingDeficit =
    dailyEnergyConsumption - totalChargingCapacity * 0.8 * (dailyUsageHours * 0.5); // 80% efficiency, 50% of day charging
  ...
  const calculateCableSize = () => {
    ...
    const actualVoltageDrop =
      ((current * selectedCable.resistance * 2 * cableLength) / 1000 / systemVoltage) * 100;
  ...
  const complianceChecks: Array<{
  ...
  }> = [
    {
      ...
      status:
        systemVoltage <= 50 && actualVoltageDropPercentage <= voltageDropLimit
          ? 'compliant'
          : 'warning',
      message:
        systemVoltage <= 50
          ? 'Voltage within safe extra-low voltage range'
          : 'Voltage exceeds SELV limits',
    },
    {
      regulation: 'ABYC E-11 (AC & DC Electrical Systems)',
      status: actualVoltageDropPercentage <= 10 ? 'compliant' : 'non-compliant',
      message:
        actualVoltageDropPercentage <= 3
          ? 'Excellent voltage drop'
          : actualVoltageDropPercentage <= 10
            ? 'Acceptable voltage drop'
            : 'Voltage drop exceeds 10% limit',
    },
    {
      regulation: 'RCD Protection (IEC 60364-7-709)',
      status: saltwaterExposure ? 'warning' : 'compliant',
      message: saltwaterExposure
        ? 'RCD protection essential for saltwater environment'
        : 'Standard protection adequate',
    },
    {
      ...
      message:
        batteryType === 'lead-acid'
          ...
          : 'LiFePO4 batteries have minimal gas emission',
    },
  ];
