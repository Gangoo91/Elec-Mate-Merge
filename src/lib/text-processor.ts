  ...
  return (
    text
      ...
      .replace(
        ...
        '<span class="text-primary font-medium">BS 7671$1$2</span>'
      )

      // Format regulation numbers - Subtle blue with background
      .replace(
        /(\d{3}\.\d+\.\d+)/g,
        '<code class="text-blue-300 font-mono text-sm px-1 py-0.5 rounded bg-blue-500/10">$1</code>'
      )
      ...
      .replace(
        /\b(SATISFACTORY|PASS)\b/gi,
        '<strong class="text-green-400 font-semibold">$1</strong>'
      )
      .replace(
        /\b(UNSATISFACTORY|FAIL|DANGEROUS)\b/gi,
        '<strong class="text-red-400 font-semibold">$1</strong>'
      )
      .replace(
        /\b(INVESTIGATION REQUIRED|IMPROVEMENT RECOMMENDED)\b/gi,
        '<strong class="text-orange-400 font-semibold">$1</strong>'
      )
...
export const extractKeyPoints = (text: string): string[] => {
  ...
  if (regulations?.length) {
    points.push(
      `BS 7671 regulations referenced: ${regulations.slice(0, 3).join(', ')}${regulations.length > 3 ? '...' : ''}`
    );
  }
