...
export function calculateDataCentre(inputs: DataCentreInputs): DataCentreResults {
  ...
  const getCoolingEfficiency = (method: string, climate: string) => {
    ...
    const climateModifier = climate === 'temperate' ? 0.9 : climate === 'cold' ? 0.8 : 1.1;
  ...
  const maxItLoadCapacity =
    (upsCapacity * (upsEfficiency / 100) * (totalItLoad / totalFacilityLoad)) /
    powerRedundancyMultiplier;
  ...
  const complianceStatus: Array<{
  ...
  }> = [
    {
      ...
      message:
        pue <= 2.0
          ? 'PUE within efficient range'
          : 'PUE above 2.0 - consider efficiency improvements',
    },
    {
      standard: 'TIA-942',
      ...
      message:
        redundancy === 'n'
          ? 'No redundancy - single point of failure risk'
          : redundancy === 'n+1'
            ? 'Basic redundancy - consider 2N for critical loads'
  ...
  if (pue > 1.8) {
    recommendations.push({
      ...
      message:
        'PUE is high - consider hot/cold aisle containment, variable speed drives, or free cooling',
